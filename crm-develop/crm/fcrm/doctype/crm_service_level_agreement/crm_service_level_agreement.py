...
class CRMServiceLevelAgreement(Document):
	...
	def validate_default(self):
		if self.default:
			other_slas = artech_engine.get_all(
	...
	def validate_condition(self):
		if not self.condition:
			return
		try:
			temp_doc = artech_engine.new_doc(self.apply_on)
			artech_engine.safe_eval(self.condition, None, get_context(temp_doc))
		except Exception as e:
			artech_engine.throw(_("The Condition '{0}' is invalid: {1}").format(self.condition, str(e)))
	...
	def get_holidays(self):
		...
		if not self.holiday_list:
			return res
		holiday_list = artech_engine.get_doc("CRM Holiday List", self.holiday_list)
