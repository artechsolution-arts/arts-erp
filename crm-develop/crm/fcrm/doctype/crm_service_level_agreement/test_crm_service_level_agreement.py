...
from artech_engine.utils import add_to_date, get_datetime, now_datetime
...
def create_test_sla(**kwargs):
	...
	sla = artech_engine.get_doc(data)
	...
	if not sla.get("priorities"):
		...
		if not artech_engine.db.exists("CRM Communication Status", "Medium"):
...
def create_test_sla_with_priorities(**kwargs):
	...
	for priority_name in ["High", "Medium", "Low"]:
		if not artech_engine.db.exists("CRM Communication Status", priority_name):
			artech_engine.get_doc(
				{
					"doctype": "CRM Communication Status",
