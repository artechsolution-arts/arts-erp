...
class TestCRMCallLog(IntegrationTestCase):
	...
	def test_parse_call_log_incoming(self):
		...
		if not artech_engine.db.exists("User", "test@example.com"):
	...
	def test_create_lead_from_call_log_basic(self):
		...
		lead_name = create_lead_from_call_log(
			call_log=artech_engine.as_json({"name": call.name}),
			lead_details=artech_engine.as_json({"first_name": "John", "last_name": "Doe"}),
		)

		# Verify lead was created
		self.assertTrue(artech_engine.db.exists("CRM Lead", lead_name))
	...
	def test_create_lead_from_call_log_with_owner(self):
		...
		lead_name = create_lead_from_call_log(
			call_log=artech_engine.as_json({"name": call.name}),
			lead_details=artech_engine.as_json({"first_name": "Jane", "lead_owner": "Administrator"}),
		)
	...
	def test_create_lead_from_call_log_no_details(self):
		...
		lead_name = create_lead_from_call_log(call_log=artech_engine.as_json({"name": call.name}))
	...
	def test_create_lead_from_call_log_invalid_call_log(self):
		...
		with self.assertRaises(artech_engine.DoesNotExistError):
			create_lead_from_call_log(call_log=artech_engine.as_json({"name": "invalid_name"}))
	...
	def test_create_lead_from_call_log_permission_check(self):
		...
		with self.assertRaises(artech_engine.ValidationError):
			create_lead_from_call_log(call_log=artech_engine.as_json({}))
	...
	def test_call_log_status_filtering(self):
		...
		completed_logs = artech_engine.get_all("CRM Call Log", filters={"status": "Completed"})
		failed_logs = artech_engine.get_all("CRM Call Log", filters={"status": "Failed"})
...
def create_test_call_log(**kwargs):
	...
	call_log = artech_engine.get_doc(data)
	call_log.insert()
	return call_log
