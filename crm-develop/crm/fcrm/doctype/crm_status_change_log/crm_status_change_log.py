...
from artech_engine.utils import add_to_date, get_datetime
...
def add_status_change_log(doc):
	to_status_type = artech_engine.db.get_value("CRM Deal Status", doc.status, "type") if doc.status else None

	if not doc.is_new():
		previous_status = doc.get_doc_before_save().status if doc.get_doc_before_save() else None
		previous_status_type = (
			artech_engine.db.get_value("CRM Deal Status", previous_status, "type") if previous_status else None
		)
		if not doc.status_change_log and previous_status:
			...
			doc.append(
				"status_change_log",
				{
					...
					"to_date": "",
					"log_owner": artech_engine.session.user,
				},
			)
		...
		last_status_change.log_owner = artech_engine.session.user
