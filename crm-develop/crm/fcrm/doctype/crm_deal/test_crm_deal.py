...
class TestCRMDeal(IntegrationTestCase):
	...
	def test_update_deal_owner(self):
		...
		docshare = artech_engine.db.exists(
			"DocShare",
	...
	def test_create_deal_api(self):
		...
		org = artech_engine.get_doc("CRM Organization", deal.organization)
		self.assertEqual(org.organization_name, "API Deal Org")

		# Verify contact was created
		contact = artech_engine.get_doc("Contact", deal.contacts[0].contact)
	...
	def test_validate_lost_reason_required(self):
		...
		if not artech_engine.db.exists("CRM Deal Status", "Lost"):
			artech_engine.get_doc({"doctype": "CRM Deal Status", "name": "Lost", "type": "Lost"}).insert()
	...
	def test_validate_lost_reason_other(self):
		...
		if not artech_engine.db.exists("CRM Lost Reason", "Other"):
			artech_engine.get_doc({"doctype": "CRM Lost Reason", "reason": "Other"}).insert()
	...
	def test_closed_date_set_on_won(self):
		...
		if not artech_engine.db.exists("CRM Deal Status", "Won"):
			artech_engine.get_doc({"doctype": "CRM Deal Status", "name": "Won", "type": "Won"}).insert()
	...
	def test_forecasting_fields_validation(self):
		...
		# Enable forecasting
		settings = artech_engine.get_single("FCRM Settings")
		...
		try:
			...
			with self.assertRaises(artech_engine.exceptions.MandatoryError):
...
def create_test_deal(**kwargs):
	...
	if "organization" in kwargs and isinstance(kwargs["organization"], str):
		...
		if not artech_engine.db.exists("CRM Organization", {"organization_name": org_name}):
			org = artech_engine.get_doc({"doctype": "CRM Organization", "organization_name": org_name}).insert()
			kwargs["organization"] = org.name
		else:
			kwargs["organization"] = artech_engine.db.get_value(
				"CRM Organization", {"organization_name": org_name}, "name"
			)

	data = {"doctype": "CRM Deal"}
	data.update(kwargs)
	return artech_engine.get_doc(data).insert()


def create_test_contact(**kwargs):
	"""Helper function to create a Contact for testing"""
	contact = artech_engine.get_doc({"doctype": "Contact"})
