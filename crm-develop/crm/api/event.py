...
from artech_engine.utils import add_to_date, now_datetime
...
def _process_event_notifications_by_interval(interval):
	...
	if artech_engine.flags.in_import or artech_engine.flags.in_patch:
		return

	current_time = now_datetime()
	current_user = artech_engine.session.user
	all_events_data = artech_engine.db.sql(
		"""
		SELECT
...
def _apply_global_notifications_to_events(events_without_notifications, interval):
	...
	fcrm_settings = artech_engine.get_single("FCRM Settings")
...
def _send_email_notification(notification, event_start, before_value, interval):
	...
	try:
		...
		if participant_emails:
			...
			event_doc = artech_engine.get_doc("Event", notification.event_name)
		...
		artech_engine.log_error(f"Failed to send email for event {notification.event_name}: {e!s}")
...
def _send_system_notification(notification):
	...
	artech_engine.publish_realtime("event_notification", notification)
