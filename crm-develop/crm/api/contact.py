...
def update_deals_email_mobile_no(doc):
	linked_deals = artech_engine.get_all(
		"CRM Contacts",
	...
	for linked_deal in linked_deals:
		deal = artech_engine.db.get_values("CRM Deal", linked_deal.parent, ["email", "mobile_no"], as_dict=True)[0]
...
def get_linked_deals(contact: str):
	...
	if not artech_engine.has_permission("Contact", "read", contact):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	deal_names = artech_engine.get_all(
		"CRM Contacts",
	...
	deals = []
	for d in deal_names:
		deal = artech_engine.get_cached_doc(
			"CRM Deal",
			d.parent,
			fields=[
				"name",
...
def create_new(contact: str, field: str, value: str):
	...
	if not artech_engine.has_permission("Contact", "write", contact):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)
	...
	if field == "email":
		...
		artech_engine.throw(_("Invalid field"))
...
def search_emails(txt: str):
	...
	results = artech_engine.get_list(
		doctype,
		filters=filters,
