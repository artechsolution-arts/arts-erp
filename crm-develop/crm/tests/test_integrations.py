...
class TestIntegrations(IntegrationTestCase):
	...
	def test_is_call_integration_enabled_both_disabled(self):
		...
		artech_engine.db.set_single_value("CRM Twilio Settings", "enabled", 0)
		artech_engine.db.set_single_value("CRM Exotel Settings", "enabled", 0)
	...
	def test_is_call_integration_enabled_twilio_only(self):
		...
		artech_engine.db.set_single_value("CRM Twilio Settings", "enabled", 1)
		artech_engine.db.set_single_value("CRM Exotel Settings", "enabled", 0)
	...
	def test_is_call_integration_enabled_exotel_only(self):
		...
		artech_engine.db.set_single_value("CRM Exotel Settings", "enabled", 1)
		artech_engine.db.set_single_value("CRM Twilio Settings", "enabled", 0)
	...
	def test_get_user_default_calling_medium_no_agent(self):
		...
		if artech_engine.db.exists("CRM Telephony Agent", artech_engine.session.user):
			artech_engine.delete_doc("CRM Telephony Agent", artech_engine.session.user)
	...
	def test_set_default_calling_medium_creates_new_record(self):
		...
		self.assertTrue(artech_engine.db.exists("CRM Telephony Agent", artech_engine.session.user))

		# Verify the record was created correctly
		agent = artech_engine.get_doc("CRM Telephony Agent", artech_engine.session.user)
	...
	def test_add_note_to_call_log_creates_new_note(self):
		...
		self.assertTrue(artech_engine.db.exists("FCRM Note", result.name))
	...
	def test_add_task_to_call_log_creates_new_task(self):
		...
		self.assertTrue(artech_engine.db.exists("CRM Task", result.name))
