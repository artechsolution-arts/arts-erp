...
def create_demo_leads(demo_users):
	...
	for data, days in zip(leads_data, _LEAD_DAYS, strict=False):
		...
		lead = artech_engine.get_doc({"doctype": "CRM Lead", **data}).insert(ignore_permissions=True)
...
def delete_demo_leads(lead_names):
	for name in lead_names:
		if artech_engine.db.exists("CRM Lead", name):
			artech_engine.delete_doc("CRM Lead", name, ignore_permissions=True, force=True)
