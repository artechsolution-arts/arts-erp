...
from artech_engine.integrations.utils import make_get_request
...
class FacebookSyncSource:
	...
	def sync_single_lead(self, lead, raise_exception=False):
		...
		except (artech_engine.UniqueValidationError, DuplicateLeadError):
			self.create_failure_log(lead, "Duplicate")
			if raise_exception:
				raise
		except Exception:
			self.create_failure_log(lead, traceback=artech_engine.get_traceback(with_context=True))
			if raise_exception:
				raise
	...
	def fetch_leads(self):
		...
		if self.last_synced_at:
			timestamp = artech_engine.utils.data.get_timestamp(self.last_synced_at)
			filtering.append({"field": "time_created", "operator": "GREATER_THAN", "value": timestamp})
			params["filtering"] = artech_engine.as_json(filtering)
	...
	def get_form_questions_mapping(self):
		...
		form_questions = artech_engine.db.get_all(
		...
		return artech_engine.get_doc(
			{
				...
				"type": type,
				"lead_data": artech_engine.as_json(lead_data),
	...
	def get_source_name(self):
		...
		return artech_engine.db.get_value("Lead Sync Source", {"facebook_lead_form": self.form_id}, "name")
	...
	def validate_duplicate_lead(self, lead_data: dict, field_mapping: dict):
		...
		if artech_engine.db.exists("CRM Lead", validation_filters):
...
def fetch_and_store_pages_from_facebook(access_token: str) -> list[dict]:
	if not access_token:
		artech_engine.throw(artech_engine._("Access token is required"))

	account_details = get_fb_account_details(access_token)
	if not account_details.get("id"):
		artech_engine.throw(artech_engine._("Invalid access token provided for Facebook."))

	url = get_fb_graph_api_url("/me/accounts")
	pages = make_get_request(url, params={"access_token": access_token}).get("data", [])
	for page in pages:
		page_id = page["id"]
		already_synced = artech_engine.db.exists("Facebook Page", page_id)
...
def get_fb_account_details(access_token: str) -> dict:
	...
	try:
		...
		artech_engine.throw(artech_engine._("Please check your access token"))
	return response
...
def fetch_and_store_leadgen_forms_from_facebook(page_id: str, page_access_token: str) -> list[dict]:
	...
	for form in forms:
		form_id = form["id"]
		already_synced = artech_engine.db.exists("Facebook Lead Form", form_id)
		if already_synced:
			continue
...
def create_facebook_lead_form_in_db(form: dict, page_id: str) -> None:
	form_doc = artech_engine.get_doc(
		{
...
def get_pages_with_forms() -> list[dict]:
	pages = artech_engine.db.get_all("Facebook Page", fields=["id", "name"])
	for page in pages:
		forms = artech_engine.db.get_all("Facebook Lead Form", filters={"page": page["id"]}, fields=["id", "name"])
		page["forms"] = forms
	return pages
