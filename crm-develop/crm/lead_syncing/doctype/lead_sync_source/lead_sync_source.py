...
class LeadSyncSource(Document):
	...
	def validate_same_fb_form_active(self):
		...
		already_active = artech_engine.db.exists(
			"Lead Sync Source",
			{"enabled": 1, "facebook_lead_form": self.facebook_lead_form, "name": ["!=", self.name]},
		)

		if already_active:
			artech_engine.throw(artech_engine._("A lead sync source is already enabled for this Facebook Lead Form!"))
	...
	def sync_leads(self):
		...
		artech_engine.enqueue_doc(self.doctype, self.name, "_sync_leads", queue="long")

	def _sync_leads(self):
		if self.type == "Facebook" and self.access_token:
			if not self.facebook_lead_form:
				artech_engine.throw(artech_engine._("Please select a lead gen form before syncing!"))
