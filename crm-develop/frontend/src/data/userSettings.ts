...
export function useUserSettings(): UseUserSettingsReturn {
  ...
  const get = async (doctype: string): Promise<UserSettings> => {
    ...
    try {
      ...
      const response: string = await call(
        'artech_engine.model.utils.user_settings.get',
        {
          doctype,
        },
      )
  ...
  const save = async (
    ...
    if (
      (window as Window & { artech_engine?: { session?: { user?: string } } })?.artech_engine
        ?.session?.user === 'Guest'
    ) {
  ...
  const update = async (
    ...
    try {
      ...
      const response: UserSettings = await call(
        'artech_engine.model.utils.user_settings.save',
        {
          doctype,
