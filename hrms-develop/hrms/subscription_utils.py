...
def get_active_employees() -> int:
	return artech_engine.db.count("Employee", {"status": "Active"})
...
def update_erpnext_workspaces(disable: bool = True):
	...
	for workspace in erpnext_workspaces:
		try:
			workspace_doc = artech_engine.get_doc("Workspace", workspace)
...
def update_erpnext_roles(disable: bool = True):
	...
	for role in roles:
		try:
			role_doc = artech_engine.get_doc("Role", role)
...
def get_modules_by_app(app_name: str) -> list:
	return artech_engine.db.get_all("Module Def", filters={"app_name": app_name}, pluck="name")


def get_doctypes_by_modules(modules: list) -> list:
	return artech_engine.db.get_all("DocType", filters=[["module", "in", modules]], pluck="name")


def roles_by_doctype(doctypes: list) -> set:
	roles = []
	for d in doctypes:
		permissions = artech_engine.get_meta(d).permissions
...
def hide_erpnext() -> bool:
	hr_subscription = has_subscription(artech_engine.conf.sk_hrms)
	erpnext_subscription = has_subscription(artech_engine.conf.sk_erpnext_smb or artech_engine.conf.sk_erpnext)
