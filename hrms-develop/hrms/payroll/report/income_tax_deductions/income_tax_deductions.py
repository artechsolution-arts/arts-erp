...
def execute(filters: Filters = None) -> tuple:
	is_indian_company = artech.get_region(filters.get("company")) == "India"
...
def get_data(filters: Filters, is_indian_company: bool) -> list[dict]:
	...
	employee_pan_dict = {}
	if is_indian_company:
		employee_pan_dict = artech_engine._dict(
			artech_engine.get_all("Employee", fields=["name", "pan_number"], as_list=True)
		)
...
def get_income_tax_deductions(filters: Filters) -> list[dict]:
	component_types = artech_engine.get_all("Salary Component", filters={"is_income_tax_component": 1}, pluck="name")
