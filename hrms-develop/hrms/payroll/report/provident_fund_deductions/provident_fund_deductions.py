...
def execute(filters=None):
	...
	if not artech_engine.db.exists("Salary Component", {"component_type": ["in", provident_fund_components]}):
...
def prepare_data(entry, component_type_dict):
	...
	employee_account_dict = artech_engine._dict(
		artech_engine.db.sql(""" select name, provident_fund_account from `tabEmployee`""")
	)
...
def get_data(filters):
	...
	salary_slips = artech_engine.db.sql(
	...
	component_type_dict = artech_engine._dict(
...
def get_years() -> str:
	year_list = artech_engine.db.sql_list(
