...
class Arrear(Document):
	...
	def payroll_period_details(self):
		if not hasattr(self, "__payroll_period_details"):
			self.__payroll_period_details = artech_engine.get_doc("Payroll Period", self.payroll_period)
	...
	def validate_salary_structure_assignment(self):
		...
		assignment = artech_engine.db.get_value(
		...
		if not assignment:
			artech_engine.throw(
				_(
					...
					artech_engine.bold(self.salary_structure),
					artech_engine.bold(self.arrear_start_date) or "",
				)
			)
		...
		if artech_engine.db.exists(
			...
			artech_engine.throw(
				_(
					...
					artech_engine.bold(self.employee),
					artech_engine.bold(self.salary_structure),
					artech_engine.bold(self.payroll_period),
				)
			)
	...
	def fetch_existing_salary_components(self, salary_slips: list):
		...
		"""
		SalarySlipDetail = artech_engine.qb.DocType("Salary Detail")
		SalaryComponent = artech_engine.qb.DocType("Salary Component")

		slip_details = (
			artech_engine.qb.from_(SalarySlipDetail)
		...
		if not (earnings_totals or deductions_totals or accrual_totals):
			artech_engine.throw(_("No arrear components found in the existing salary slips."))
	...
	def fetch_existing_accrual_components(self, salary_slips: list):
		...
		AccruedBenefit = artech_engine.qb.DocType("Employee Benefit Detail")
		SalaryComponent = artech_engine.qb.DocType("Salary Component")

		accrual_details = (
			artech_engine.qb.from_(AccruedBenefit)
	...
	def fetch_existing_payroll_corrections(self, salary_slips: list):
		...
		PCChild = artech_engine.qb.DocType("Payroll Correction Child")
		SalaryComponent = artech_engine.qb.DocType("Salary Component")
	...
	def generate_preview_components(self, salary_slips: list):
		...
		def is_arrear_component(component):
			return artech_engine.get_cached_value("Salary Component", component, "arrear_component")

		for slip in salary_slips:
			# Build a preview salary slip doc
			salary_slip_doc = artech_engine.get_doc(
				{
					"doctype": "Salary Slip",
					"employee": self.employee,
					"salary_structure": self.salary_structure,
	...
	def validate_arrear_details(self):
		...
		if not (self.earning_arrears or self.deduction_arrears or self.accrual_arrears):
			artech_engine.throw(_("No arrear details found"))
	...
	def create_benefit_ledger_entry(self):
		for component in self.accrual_arrears or []:
			...
			is_flexible_benefit = artech_engine.db.get_value(
