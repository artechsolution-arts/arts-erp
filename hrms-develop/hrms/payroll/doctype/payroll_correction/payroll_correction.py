...
class PayrollCorrection(Document):
	...
	def validate(self):
		if self.days_to_reverse <= 0:
			artech_engine.throw(_("Days to Reverse must be greater than zero."))
		self.validate_days()
	...
	def validate_days(self):
		if self.days_to_reverse and self.salary_slip_reference:
			salary_slip = artech_engine.get_doc("Salary Slip", self.salary_slip_reference)
			...
			payroll_corrections = artech_engine.get_all(
				"Payroll Correction",
				filters={
					"docstatus": 1,
		...
		if not slips:
			artech_engine.msgprint(
				_("No Salary Slips with {0} found for employee {1} for payroll period {2}.").format(
					artech_engine.bold("Leave Without Pay"), self.employee, self.payroll_period
				)
			)
			return
	...
	def populate_breakup_table(self):
		...
		precision = (
			...
			or artech_engine.db.get_single_value("System Settings", "currency_precision")
			or 2
		)
		if not salary_slip:
			artech_engine.throw(_("Salary Slip not found."))
		...
		if salary_slip_components:
			arrear_components = artech_engine.db.get_list(
				"Salary Component",
				filters={
					"arrear_component": 1,
