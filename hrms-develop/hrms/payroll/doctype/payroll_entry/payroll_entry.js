...
artech_engine.ui.form.on("Payroll Entry", {
	onload: function (frm) {
		frm.ignore_doctypes_on_cancel_all = ["Salary Slip", "Journal Entry"];

		if (!frm.doc.posting_date) {
			frm.doc.posting_date = artech_engine.datetime.nowdate();
		}
		...
		artech_engine.realtime.off("completed_overtime_slip_creation");
		artech_engine.realtime.on("completed_overtime_slip_creation", function () {
			frm.reload_doc();
		});

		artech_engine.realtime.off("completed_overtime_slip_submission");
		artech_engine.realtime.on("completed_overtime_slip_submission", function () {
			frm.reload_doc();
		});

		artech_engine.realtime.off("completed_salary_slip_creation");
		artech_engine.realtime.on("completed_salary_slip_creation", function () {
			frm.reload_doc();
		});

		artech_engine.realtime.off("completed_salary_slip_submission");
		artech_engine.realtime.on("completed_salary_slip_submission", function () {
			frm.reload_doc();
		});
	},
	...
	refresh: (frm) => {
		...
		if (
			...
			!artech_engine.model.has_workflow(frm.doctype) &&
	...
	setup: function (frm) {
		...
		frm.set_query("employee", "employees", () => {
			...
			mandatory_fields.forEach((field) => {
				if (!frm.doc[field]) {
					error_fields.push(artech_engine.unscrub(field));
				}
			});
	...
	set_payable_account_and_currency: function (frm) {
		artech_engine.db.get_value("Company", { name: frm.doc.company }, "default_currency", (r) => {
	...
	currency: function (frm) {
		...
		if (!frm.doc.company) {
			company_currency = artech.get_currency(artech_engine.defaults.get_default("Company"));
		} else {
			company_currency = artech.get_currency(frm.doc.company);
		}
		if (frm.doc.currency) {
...
const submit_salary_slip = function (frm) {
	artech_engine.confirm(
		...
		function () {
			if (artech_engine.dom.freeze_count) {
...
let make_bank_entry = function (frm, for_withheld_salaries = 0) {
	...
	if (doc.payment_account) {
		...
		artech_engine.msgprint(__("Payment Account is mandatory"));
...
let render_employee_attendance = function (frm, data) {
	frm.fields_dict.attendance_detail_html.html(
		artech_engine.render_template("employees_with_unmarked_attendance", {
			data: data,
		}),
	);
};
