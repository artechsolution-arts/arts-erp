...
class AdditionalSalary(Document):
	...
	def validate(self):
		...
		if self.amount < 0:
			artech_engine.throw(_("Amount should not be less than zero"))

	def validate_salary_structure(self):
		salary_structure = artech_engine.db.get_value(
		...
		if self.overwrite_salary_structure_amount:
			is_structure_component = artech_engine.db.get_value(
				"Salary Detail",
				{
	...
	def validate_recurring_additional_salary_overlap(self):
		if self.is_recurring:
			AdditionalSalary = artech_engine.qb.DocType("Additional Salary")
	...
	def validate_dates(self):
		...
		if self.is_recurring and not (self.from_date and self.to_date):
			artech_engine.throw(_("From and to dates are madatory for recurring type additional salaries."))
		elif (not self.is_recurring) and (not self.payroll_date):
			artech_engine.throw(_("Payroll date is mandatory for non-recurring type additional salaries."))

		if date_of_joining:
			if self.payroll_date and getdate(self.payroll_date) < getdate(date_of_joining):
				artech_engine.throw(_("Payroll date can not be less than employee's joining date."))
		...
		if relieving_date:
			if self.to_date and getdate(self.to_date) > getdate(relieving_date):
				artech_engine.throw(_("To date can not be greater than employee's relieving date."))
			if self.payroll_date and getdate(self.payroll_date) > getdate(relieving_date):
				artech_engine.throw(_("Payroll date can not be greater than employee's relieving date."))

	def validate_employee_referral(self):
		if self.ref_doctype == "Employee Referral":
			referral_details = artech_engine.db.get_value(
				"Employee Referral",
				self.ref_docname,
			...
			if self.type == "Deduction":
				artech_engine.throw(_("Earning Salary Component is required for Employee Referral Bonus."))
			...
			if referral_details.status != "Accepted":
				artech_engine.throw(
					...
					).format(artech_engine.bold(_("Accepted")))
				)
	...
	def validate_duplicate_additional_salary(self):
		...
		if existing_additional_salary:
			...
			).format(artech_engine.bold(_("Overwrite Salary Structure Amount")))
			msg += "<br><br>"
			msg += _("Reference: {0}").format(
				get_link_to_form("Additional Salary", existing_additional_salary)
			)
			artech_engine.throw(msg, title=_("Duplicate Overwritten Salary"))
	...
	def validate_tax_component_overwrite(self):
		...
		if self.overwrite_salary_structure_amount:
			artech_engine.msgprint(
				...
				).format(artech_engine.bold(self.salary_component)),
				title=_("Warning"),
				indicator="orange",
			)
		else:
			msg = _("{0} has {1} enabled").format(
				...
				artech_engine.bold(_("Variable Based On Taxable Salary")),
			)
			msg += "<br><br>" + _(
				"To overwrite the salary component amount for a tax component, please enable {0}"
			).format(artech_engine.bold(_("Overwrite Salary Structure Amount")))
			artech_engine.throw(msg, title=_("Invalid Additional Salary"))

	def validate_accrual_component(self):
		if artech_engine.db.get_value("Salary Component", self.salary_component, "accrual_component"):
			artech_engine.msgprint(
				_(
	...
	def update_return_amount_in_employee_advance(self):
		if self.ref_doctype == "Employee Advance" and self.ref_docname:
			return_amount = artech_engine.db.get_value("Employee Advance", self.ref_docname, "return_amount")
			...
			artech_engine.db.set_value("Employee Advance", self.ref_docname, "return_amount", return_amount)
			advance = artech_engine.get_doc("Employee Advance", self.ref_docname)
			advance.set_status(update=True)

	def update_employee_referral(self, cancel=False):
		if self.ref_doctype == "Employee Referral":
			status = "Unpaid" if cancel else "Paid"
			artech_engine.db.set_value("Employee Referral", self.ref_docname, "referral_payment_status", status)
...
def get_additional_salaries(employee, start_date, end_date, component_type):
	...
	additional_sal = artech_engine.qb.DocType("Additional Salary")
	...
	additional_salary_list = (
		artech_engine.qb.from_(additional_sal)
		.select(
			additional_sal.name,
			component_field,
			additional_sal.type,
		...
		if d.overwrite:
			if d.component in components_to_overwrite:
				artech_engine.throw(
					...
					).format(artech_engine.bold(d.component), start_date, end_date),
					title=_("Error"),
				)
