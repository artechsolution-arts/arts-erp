...
def add_custom_roles_for_reports():
	for report_name in (
		...
		if not artech_engine.db.get_value("Custom Role", dict(report=report_name)):
			doc = artech_engine.new_doc("Custom Role")
			doc.update(
				dict(
					report=report_name,
...
def create_gratuity_rule_for_india():
	if not artech_engine.db.exists("DocType", "Gratuity Rule"):
		return

	if artech_engine.db.exists("Gratuity Rule", "Indian Standard Gratuity Rule"):
		return

	rule = artech_engine.new_doc("Gratuity Rule")
	rule.update(
		{
