...
from artech_engine.desk.page.setup_wizard.install_fixtures import (
...
def update_hr_defaults():
	hr_settings = artech_engine.get_doc("HR Settings")
...
def set_single_defaults():
	for dt in ("HR Settings", "Payroll Settings"):
		default_values = artech_engine.get_all(
			"DocField",
			filters={"parent": dt},
...
def get_post_install_patches():
	return (
		"artech.patches.v13_0.move_tax_slabs_from_payroll_period_to_income_tax_slab",
		"artech.patches.v13_0.move_doctype_reports_and_notification_from_hr_to_payroll",
		"artech.patches.v13_0.move_payroll_setting_separately_from_hr_settings",
		"artech.patches.v13_0.update_start_end_date_for_old_shift_assignment",
		"artech.patches.v13_0.updates_for_multi_currency_payroll",
		"artech.patches.v13_0.update_reason_for_resignation_in_employee",
		"artech.patches.v13_0.set_company_in_leave_ledger_entry",
		"artech.patches.v13_0.rename_stop_to_send_birthday_reminders",
		"artech.patches.v13_0.set_training_event_attendance",
		"artech.patches.v14_0.set_payroll_cost_centers",
		"artech.patches.v13_0.update_employee_advance_status",
		"artech.patches.v13_0.update_expense_claim_status_for_paid_advances",
		"artech.patches.v14_0.delete_employee_transfer_property_doctype",
		"artech.patches.v13_0.set_payroll_entry_status",
		# HRMS
...
def run_post_install_patches():
	...
	artech_engine.flags.in_patch = True
...
def add_lending_docperms_to_ess():
	doc = artech_engine.get_doc("User Type", "Employee Self Service")
...
def create_custom_role(data):
	if data.get("role") and not artech_engine.db.exists("Role", data.get("role")):
...
def create_user_type(user_type, data):
	if artech_engine.db.exists("User Type", user_type):
		doc = artech_engine.get_cached_doc("User Type", user_type)
		doc.user_doctypes = []
	else:
		doc = artech_engine.new_doc("User Type")
		doc.update(
			{
				"name": user_type,
				"role": data.get("role"),
	...
	if doc.role == "Employee Self Service" and "lending" in artech_engine.get_installed_apps():
...
def update_select_perm_after_install():
	if not artech_engine.flags.update_select_perm_after_migrate:
		return

	artech_engine.flags.ignore_select_perm = False
	for row in artech_engine.get_all("User Type", filters={"is_standard": 0}):
		print("Updating user type :- ", row.name)
		doc = artech_engine.get_doc("User Type", row.name)
		doc.flags.ignore_links = True
		doc.save()

	artech_engine.flags.update_select_perm_after_migrate = False
