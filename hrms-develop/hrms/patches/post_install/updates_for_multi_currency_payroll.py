...
def execute():
	artech_engine.reload_doc("payroll", "doctype", "Salary Component Account")
	if artech_engine.db.has_column("Salary Component Account", "default_account"):
	...
	for item in doctype_list:
		artech_engine.reload_doc(item["module"], "doctype", item["doctype"])
	...
	all_companies = artech_engine.db.get_all(
	...
	for d in all_companies:
		...
		if not default_payroll_payable_account:
			default_payroll_payable_account = artech_engine.db.get_value(
				"Account",
				{
