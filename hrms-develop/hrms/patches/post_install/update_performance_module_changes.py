...
def create_kras():
	...
	if not artech_engine.db.has_column("Appraisal Template Goal", "kra"):
		return

	template_goals = artech_engine.get_all(
	...
	for name, kra in template_goals:
		...
		if not artech_engine.db.exists("KRA", kra_title):
...
def update_kra_evaluation_method():
	...
	(
		artech_engine.qb.update(Appraisal)
