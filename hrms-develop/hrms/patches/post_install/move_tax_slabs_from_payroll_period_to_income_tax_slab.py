...
def execute():
	if not (artech_engine.db.table_exists("Payroll Period") and artech_engine.db.table_exists("Taxable Salary Slab")):
		return

	if artech_engine.db.a_row_exists("Income Tax Slab"):
		return
	...
	for doctype in (
		...
		artech_engine.reload_doc("Payroll", "doctype", doctype)

	standard_tax_exemption_amount_exists = artech_engine.db.has_column(
	...
	for company in artech_engine.get_all("Company"):
		payroll_periods = artech_engine.db.sql(
			f"""
			SELECT
				{select_fields}
			FROM
	...
	if not artech_engine.db.table_exists("Employee Tax Exemption Proof Submission"):
		return

	if not artech_engine.db.has_column("Employee Tax Exemption Proof Submission", "income_from_other_sources"):
		return

	migrated = []
	proofs = artech_engine.get_all(
	...
	for proof in proofs:
		if proof.income_from_other_sources:
			employee_other_income = artech_engine.new_doc("Employee Other Income")
	...
	if not artech_engine.db.table_exists("Employee Tax Exemption Declaration"):
		return

	if not artech_engine.db.has_column("Employee Tax Exemption Declaration", "income_from_other_sources"):
		return

	declerations = artech_engine.get_all(
