...
hrms.HierarchyChart = class {
	...
	*/
	...
	make_node_element(node) {
		let node_card = artech_engine.render_template("node_card", {
			id: node.id,
			name: node.name,
			title: node.title,
			image: node.image,
		...
		let company = this.page.add_field({
			...
			change: () => {
				...
				if (company.get_value()) {
					...
					artech_engine.throw(__("Please select a company first."));
				}
			},
		});
	...
	export_chart() {
		artech_engine.dom.freeze(__("Exporting..."));
	...
	setup_edit_node_action(node) {
		...
		node_element.find(".btn-edit-node").click(function () {
			artech_engine.set_route("Form", me.doctype, node.id);
		});
	}
