...
$.extend(hrms, {
	...
	get_current_employee: async (frm) => {
		const employee = (
			await artech_engine.db.get_value("Employee", { user_id: artech_engine.session.user }, "name")
		)?.message?.name;
	...
	setup_employee_filter_group: (frm) => {
		...
		artech_engine.model.with_doctype("Employee", () => {
			frm.filter_list = new artech_engine.ui.FilterGroup({
	...
	render_employees_datatable: (
		...
		frm.employees_datatable = new artech_engine.DataTable(employee_wrapper.get(0), datatable_options);
	},
	...
	notify_bulk_action_status: (doctype, failure, success, for_processing = false) => {
		...
		if (failure.length) {
			...
			message += " " + artech_engine.utils.comma_and(failure) + "<hr>";
			message += __(
	...
	fetch_geolocation: async (frm) => {
		...
		artech_engine.dom.freeze(__("Fetching your geolocation") + "...");
	...
	get_doctype_fields_for_autocompletion: (doctype) => {
		const fields = artech_engine.get_meta(doctype).fields;
		const autocompletions = [];

		fields
			.filter((df) => !artech_engine.model.no_value_type.includes(df.fieldtype))
			.map((df) => {
	...
	add_shift_tools_button_to_list: (list_view, action = "Assign Shift") => {
		list_view.page.add_inner_button(
			...
			() => {
				const doc = artech_engine.model.get_new_doc("Shift Assignment Tool");
				doc.action = action;
				doc.company = artech_engine.defaults.get_default("company");
				doc.status = "Active";
				artech_engine.set_route("Form", "Shift Assignment Tool", doc.name);
			},
			__("Shift Tools"),
		);
