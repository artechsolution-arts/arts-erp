...
class LeaveAdjustment(Document):
	...
	def before_validate(self):
		system_precision = cint(artech_engine.db.get_single_value("System Settings", "float_precision")) or 3
	...
	def validate_duplicate_leave_adjustment(self):
		duplicate_adjustment = artech_engine.db.exists(
			"Leave Adjustment",
	...
	def validate_non_zero_adjustment(self):
		if self.leaves_to_adjust == 0:
			artech_engine.throw(_("Enter a non-zero value to adjust."))
		...
		if max_leaves_allowed and (new_allocation > max_leaves_allowed):
			artech_engine.throw(
				_("Allocation is greater than the maximum allowed {0} for leave type {1}").format(
					artech_engine.bold(max_leaves_allowed), artech_engine.bold(self.leave_type)
				)
			)
		...
		if leave_balance < self.leaves_to_adjust:
			artech_engine.throw(
				_("Reduction is more than {0}'s available leave balance {1} for leave type {2}").format(
					artech_engine.bold(self.employee_name), artech_engine.bold(leave_balance), artech_engine.bold(self.leave_type)
				)
			)
	...
	def create_leave_ledger_entry(self, submit):
		is_lwp = artech_engine.db.get_value("Leave Type", self.leave_type, "is_lwp")
