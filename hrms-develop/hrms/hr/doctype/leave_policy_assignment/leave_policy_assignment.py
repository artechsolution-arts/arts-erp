...
class LeavePolicyAssignment(Document):
	...
	def set_dates(self):
		if self.assignment_based_on == "Leave Period":
			self.effective_from, self.effective_to = artech_engine.db.get_value(
				"Leave Period", self.leave_period, ["from_date", "to_date"]
			)
		elif self.assignment_based_on == "Joining Date":
			self.effective_from = artech_engine.db.get_value("Employee", self.employee, "date_of_joining")
			if not self.effective_to:
				self.effective_to = get_last_day(add_months(self.effective_from, 12))

	def validate_policy_assignment_overlap(self):
		leave_policy_assignment = artech_engine.db.get_value(
	...
	def warn_about_carry_forwarding(self):
		...
		leave_policy = artech_engine.get_doc("Leave Policy", self.leave_policy)
		...
		for policy in leave_policy.leave_policy_details:
			...
			if not leave_type.is_carry_forward:
				...
				).format(artech_engine.bold(get_link_to_form("Leave Type", leave_type.name)))
				artech_engine.msgprint(msg, indicator="orange", alert=True)

	def grant_leave_alloc_for_employee(self):
		if self.leaves_allocated:
			artech_engine.throw(_("Leave already have been assigned for this Leave Policy Assignment"))
		else:
			leave_allocations = {}
	...
	def create_leave_allocation(self, annual_allocation, leave_details, date_of_joining):
		...
		if new_leaves_allocated == 0 and not leave_details.is_earned_leave:
			...
			).format(artech_engine.bold(leave_details.name))
	...
	def get_new_leaves(self, annual_allocation, leave_details, date_of_joining):
		...
		precision = get_field_precision(artech_engine.get_meta("Leave Allocation").get_field("new_leaves_allocated"))
		current_date = getdate(artech_engine.flags.current_date) or getdate()
...
def calculate_pro_rated_leaves(
	...
	precision = cint(artech_engine.db.get_single_value("System Settings", "float_precision", cache=True))
...
def create_assignment_for_multiple_employees(employees: str | list[str], data: str | dict) -> list[str]:
	...
	for employee in employees:
		assignment = create_assignment(employee, artech_engine._dict(data))
...
@artech_engine.whitelist()
def create_assignment(employee: str, data: artech_engine._dict) -> Document:
	assignment = artech_engine.new_doc("Leave Policy Assignment")
...
def get_leave_type_details():
	leave_type_details = artech_engine._dict()
