artech_engine.ui.form.on("Employee Attendance Tool", {
	refresh(frm) {
	...
	onload(frm) {
		frm.set_value("date", artech_engine.datetime.get_today());
	},
	...
	show_employees_to_mark(frm, html_fieldname, multicheck_fieldname, employee_list) {
		...
		frm.fields_dict[multicheck_fieldname] = artech_engine.ui.form.make_control({
	...
	render_datatable(frm, data, summary_wrapper) {
		...
		if (!frm.marked_emp_datatable) {
			...
			frm.marked_emp_datatable = new artech_engine.DataTable(
