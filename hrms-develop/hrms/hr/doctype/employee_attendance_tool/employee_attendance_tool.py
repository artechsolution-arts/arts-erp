...
def get_employees(
	...
	employee_list = artech_engine.get_list(
		"Employee",
	...
	# marked attendance
	attendance_list = artech_engine.get_list(
		"Attendance",
	...
	half_day_attendance_list = artech_engine.get_list(
		"Attendance",
		fields=["employee", "employee_name"],
		filters={
			"attendance_date": date,
...
def _get_unmarked_attendance_with_shift(unmarked_attendance, shift, date):
	...
	shift_assigned_employees = artech_engine.get_list(
		"Shift Assignment",
		filters={
			"shift_type": shift,
			"start_date": ["<=", artech_engine.utils.getdate(date)],
		},
		fields=["employee"],
	)
	# Fetch employees based on default shifts
	default_shift_employees = artech_engine.get_list(
		"Employee",
		filters={
			"default_shift": shift,
		},
...
def mark_employee_attendance(
	...
	if mark_half_day:
		...
		for employee in half_day_employee_list:
			artech_engine.qb.update(Attendance).where(
