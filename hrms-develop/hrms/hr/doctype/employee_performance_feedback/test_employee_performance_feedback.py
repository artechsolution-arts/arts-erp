...
class TestEmployeePerformanceFeedback(HRMSTestSuite):
	def setUp(self):
		...
		self.appraisal = artech_engine.db.get_all("Appraisal", filters={"appraisal_cycle": cycle.name})[0].name
	...
	def test_validate_employees(self):
		...
		self.assertRaises(artech_engine.ValidationError, feedback.insert)
	...
	def test_update_avg_feedback_score_in_appraisal(self):
		...
		avg_feedback_score = artech_engine.db.get_value("Appraisal", self.appraisal, "avg_feedback_score")
