...
class EmployeePerformanceFeedback(Document, AppraisalMixin):
		...
		if self.employee == self.reviewer:
			artech_engine.throw(
				_("Employees cannot give feedback to themselves. Use {0} instead: {1}").format(
					artech_engine.bold(_("Self Appraisal")), get_link_to_form("Appraisal", self.appraisal)
				)
			)
	...
	def validate_appraisal(self):
		employee = artech_engine.db.get_value("Appraisal", self.appraisal, "employee")
	...
	def update_avg_feedback_score_in_appraisal(self):
		...
		appraisal = artech_engine.get_doc("Appraisal", self.appraisal)
	...
	def set_feedback_criteria(self):
		...
		template = artech_engine.db.get_value("Appraisal", self.appraisal, "appraisal_template")
		template = artech_engine.get_doc("Appraisal Template", template)
