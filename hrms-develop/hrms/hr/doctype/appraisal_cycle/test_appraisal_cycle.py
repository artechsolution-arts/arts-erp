...
class TestAppraisalCycle(HRMSTestSuite):
	...
	def test_create_appraisals(self):
		...
		appraisals = artech_engine.db.get_all("Appraisal", filters={"appraisal_cycle": cycle.name})
		self.assertEqual(len(appraisals), 1)

		appraisal = artech_engine.get_doc("Appraisal", appraisals[0].name)
...
def create_appraisal_cycle(**args):
	...
	if artech_engine.db.exists("Appraisal Cycle", name):
		artech_engine.delete_doc("Appraisal Cycle", name, force=True)

	appraisal_cycle = artech_engine.get_doc(
		{
