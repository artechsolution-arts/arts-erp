...
class TestLeaveAllocation(HRMSTestSuite):
	def setUp(self):
		employee = artech_engine.get_doc("Employee", {"first_name": "_Test Employee"})
		self.original_doj = employee.date_of_joining
		employee.date_of_joining = add_months(getdate(), -24)
		employee.save()

		self.employee2 = artech_engine.get_doc("Employee", {"first_name": "_Test Employee 1"})
		...
		artech_engine.db.set_value("Email Account", "_Test Email Account 1", "default_outgoing", 1)
	...
	def test_earned_leave_allocation(self):
		...
		artech_engine.flags.current_date = add_days(get_last_day(getdate()), -1)
	...
	def test_earned_leave_update_after_submission(self):
		...
		allocation = artech_engine.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
			"name",
		)
		allocation = artech_engine.get_doc("Leave Allocation", allocation)
		allocation.new_leaves_allocated = 2
		self.assertRaises(artech_engine.ValidationError, allocation.save)
	...
	def test_alloc_based_on_leave_period(self):
		...
		artech_engine.flags.current_date = get_first_day(getdate())
	...
	def test_alloc_on_month_end_based_on_leave_period(self):
		...
		artech_engine.flags.current_date = get_last_day(getdate())
		leave_policy_assignments = make_policy_assignment(self.employee, start_date=start_date)
	...
	def test_alloc_based_on_leave_period_with_cf_leaves(self):
		...
		artech_engine.flags.current_date = get_last_day(add_months(getdate(), -1))
		...
		details = artech_engine.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
	...
	def test_alloc_based_on_joining_date(self):
		...
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
		effective_from = artech_engine.db.get_value(
	...
	def test_earned_leaves_creation(self):
		artech_engine.flags.current_date = get_year_start(getdate())
		make_policy_assignment(
			self.employee,
			annual_allocation=6,
			allocate_on_day="First Day",
			start_date=artech_engine.flags.current_date,
		)

		# leaves for 6 months = 3, but max leaves restricts allocation to 2
		artech_engine.db.set_value("Leave Type", self.leave_type, "max_leaves_allowed", 2)
		allocate_earned_leaves_for_months(6)
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 2
		)

		# validate earned leaves creation without maximum leaves
		artech_engine.db.set_value("Leave Type", self.leave_type, "max_leaves_allowed", 0)
		allocate_earned_leaves_for_months(6)
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 5
		)
	...
	def test_overallocation(self):
		...
		artech_engine.db.set_value("Leave Type", self.leave_type, "rounding", 1.0)
		allocate_earned_leaves_for_months(11)
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 22
		)
	...
	def test_overallocation_with_carry_forwarding(self):
		...
		artech_engine.flags.current_date = year_start
		...
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 27
		)
	...
	def test_allocate_on_first_day(self):
		...
		artech_engine.flags.current_date = prev_month_last_day
		leave_policy_assignments = make_policy_assignment(
		...
		artech_engine.flags.current_date = first_day
	...
	def test_allocate_on_last_day(self):
		...
		artech_engine.flags.current_date = last_day
		allocate_earned_leaves()
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
		self.assertEqual(leaves_allocated, 2)

		# Case 3: Doesn't allocate before the current month's last day (via scheduler)
		artech_engine.flags.current_date = add_days(last_day, -1)
		allocate_earned_leaves()
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
		# balance is still 2
		self.assertEqual(leaves_allocated, 2)
	...
	def test_allocate_on_date_of_joining(self):
		...
		artech_engine.flags.current_date = doj
		leave_policy_assignments = make_policy_assignment(
		...
		artech_engine.flags.current_date = add_days(current_month_doj, -1)
		allocate_earned_leaves()
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
		...
		artech_engine.flags.current_date = current_month_doj
		allocate_earned_leaves()
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
	...
	def test_backdated_pro_rated_allocation(self):
		...
		artech_engine.flags.current_date = getdate("2023-05-16")
		leave_policy_assignments = make_policy_assignment(
			self.employee,
			allocate_on_day="First Day",
	...
	def test_no_pro_rated_leaves_allocated_before_effective_date(self):
		...
		artech_engine.flags.current_date = add_days(doj, -1)
		leave_policy_assignments = make_policy_assignment(
			self.employee, allocate_on_day="Date of Joining", start_date=start_date
		)
		leaves_allocated = get_allocated_leaves(leave_policy_assignments[0])
	...
	def test_get_earned_leave_details_for_dashboard(self):
		artech_engine.flags.current_date = get_year_start(getdate())
		first_sunday = get_first_sunday(self.holiday_list, for_date=artech_engine.flags.current_date)

		leave_policy_assignments = make_policy_assignment(
			self.employee,
			annual_allocation=6,
			allocate_on_day="First Day",
			start_date=add_months(artech_engine.flags.current_date, -3),
		)
		allocation = artech_engine.db.get_value(
			"Leave Allocation",
			{"leave_policy_assignment": leave_policy_assignments[0]},
		...
		details = get_leave_details(self.employee.name, artech_engine.flags.current_date)
		leave_allocation = details["leave_allocation"][self.leave_type]
		expected = {
			"total_leaves": 5.0,
			"expired_leaves": 0.0,
	...
	def test_allocate_leaves_manually(self):
		...
		leave_allocation = artech_engine.get_last_doc(
		...
		leave_allocation.allocate_leaves_manually(1)
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 6
		)

		leave_allocation.allocate_leaves_manually(5)
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date), 11
		)

		# manually set from_date - applicable from the next day
		leave_allocation.allocate_leaves_manually(1, add_days(artech_engine.flags.current_date, 1))
		...
		self.assertEqual(
			get_leave_balance_on(self.employee.name, self.leave_type, add_days(artech_engine.flags.current_date, 1)),
			12,
		)

		self.assertRaises(artech_engine.ValidationError, leave_allocation.allocate_leaves_manually, 1)

	def test_quarterly_earned_leaves_allocated_on_last_day_in_the_middle_of_leave_period(self):
		# allocated after one quarter
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 4)
		...
		total_leaves_allocated = artech_engine.get_value(
			"Leave Allocation",
	...
	def test_quarterly_earned_leaves_allocated_by_the_scheduler(self):
		...
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 3)
		...
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 9)
		allocate_earned_leaves()
	...
	def test_quarterly_leaves_allocated_pro_rated(self):
		...
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), months=1, days=10)
		assignment = make_policy_assignment(
			self.employee2,
			allocate_on_day="Last Day",
			earned_leave_frequency="Quarterly",
		...
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), months=3, days=-1)
		allocate_earned_leaves()
	...
	def test_half_yearly_earned_leaves_allocated_on_last_day_in_the_middle_of_leave_period(self):
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 7)
	...
	def test_half_yearly_earned_leaves_allocated_by_the_scheduler(self):
		...
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 6)
	...
	def test_half_yearly_leaves_allocated_pro_rated(self):
		...
		# make policy assignment on the same day
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), months=3, days=25)
		assignment = make_policy_assignment(
			self.employee2,
			allocate_on_day="Last Day",
			earned_leave_frequency="Half-Yearly",
		...
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), months=6, days=-1)
		allocate_earned_leaves()
	...
	def test_yearly_leaves_allocated_on_last_day_in_the_middle_of_the_period(self):
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), years=2)
		# 4 year leave policy
		assignment = make_policy_assignment(
			self.employee2,
			allocate_on_day="Last Day",
	...
	def test_yearly_leaves_allocated_by_scheduler(self):
		...
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 12)
	...
	def test_yearly_leaves_allocated_pro_rated(self):
		...
		# make policy assignment on the same day
		artech_engine.flags.current_date = add_to_date(get_year_start(getdate()), months=7, days=15)
		assignment = make_policy_assignment(
			self.employee2,
			allocate_on_day="Last Day",
			earned_leave_frequency="Yearly",
		...
		artech_engine.flags.current_date = get_year_ending(getdate())
		allocate_earned_leaves()
	...
	def test_error_logging_failed_allocations(self):
		...
		artech_engine.db.set_value("Leave Type", self.leave_type, "max_leaves_allowed", 2)
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 1)
		allocate_earned_leaves()
		error_log = artech_engine.db.get_value("Error Log", {"reference_doctype": "Leave Allocation"})
	...
	def test_send_email_for_failed_allocations(self):
		...
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 1)
		allocate_earned_leaves()
		email = artech_engine.db.get_values(
	...
	def test_retry_failed_allocations(self):
		...
		leave_allocation = artech_engine.get_doc(
		...
		allocate_earned_leaves()
		# third month failed
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 2)
		allocate_earned_leaves()
		# total failed should be 2
		failed_allocations = artech_engine.get_all(
	...
	def test_permission_check_for_retrying_failed_allocation(self):
		...
		artech_engine.set_user(self.employee.user_id)
		self.assertRaises(
			artech_engine.PermissionError, leave_allocation.retry_failed_allocations, failed_allocations
		)
		...
		artech_engine.get_doc("User", self.employee.user_id).remove_roles("HR Manager")

	def test_allocating_earned_leave_when_schedule_doesnt_exist(self):
		artech_engine.flags.current_date = get_year_start(getdate())
		employee1 = self.employee2
		employee2 = artech_engine.copy_doc(employee1)
		...
		leave_policy = artech_engine.get_doc(
			{
		...
		leave_policy_assignments = create_assignment_for_multiple_employees(
			[self.employee.name, employee1.name, employee2.name], artech_engine._dict(data)
		)
		leave_allocations = artech_engine.db.get_values(
			"Leave Allocation", {"employee": ("in", (employee1.name, employee2.name))}, pluck=True
		)
		artech_engine.db.delete("Earned Leave Schedule", {"parent": ("in", leave_allocations)})
		artech_engine.flags.current_date = add_months(get_year_start(getdate()), 1)
		allocate_earned_leaves()
		total_leaves_allocated_with_no_schedule = artech_engine.db.get_values(
			"Leave Allocation",
			{
		...
		total_leaves_allocated_with_schedule = artech_engine.get_value(
			"Leave Allocation",
		...
		artech_engine.delete_doc_if_exists("Employee", employee2.name, force=1)
	...
	def test_exceed_newly_allocated_leaves(self):
		...
		artech_engine.flags.current_date = this_year_start
		...
		artech_engine.db.set_value("Leave Type", "Test Earned Leave", "max_leaves_allowed", 12)
		artech_engine.flags.current_date = get_last_day(this_year_start)
		# earned leave according to policy is 2 but allocates 1 due to max leave limit
		allocate_earned_leaves()
		leave_balance = get_leave_balance_on(self.employee.name, self.leave_type, artech_engine.flags.current_date)
		leave_allocation = artech_engine.get_value(
			"Leave Allocation",
		...
		earned_leaves_allocated = artech_engine.get_value(
			"Earned Leave Schedule",
			{"parent": leave_allocation.name, "allocation_date": artech_engine.flags.current_date},
			"number_of_leaves",
		)
...
def create_earned_leave_type(
	...
	if artech_engine.db.exists("Leave Type", leave_type):
		doc = artech_engine.get_doc("Leave Type", leave_type)
		doc.update(
			{
...
def create_leave_period(name, start_date=None, end_date=None):
	artech_engine.delete_doc_if_exists("Leave Period", name, force=1)
...
def make_policy_assignment(
	...
	leave_policy_assignments = create_assignment_for_multiple_employees([employee.name], artech_engine._dict(data))
...
def allocate_earned_leaves_for_months(months):
	for _ in range(0, months):
		artech_engine.flags.current_date = add_months(artech_engine.flags.current_date, 1)
		allocate_earned_leaves()
