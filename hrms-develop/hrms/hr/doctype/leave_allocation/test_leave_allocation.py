...
from artech_engine.utils import add_days, add_months, getdate, nowdate
...
class TestLeaveAllocation(HRMSTestSuite):
	...
	def test_overlapping_allocation(self):
		...
		artech_engine.get_doc(leaves[0]).save()
		self.assertRaises(artech_engine.ValidationError, artech_engine.get_doc(leaves[1]).save)
	...
	def test_validation_for_over_allocation_based_on_leave_setup(self):
		artech_engine.delete_doc_if_exists("Leave Period", "Test Allocation Period")
	...
	def test_carry_forward_leaves_expiry_after_partially_used_leaves(self):
		...
		process_expired_allocation()
		expired_leaves = artech_engine.db.get_value(
			"Leave Ledger Entry",
			dict(
	...
	def test_carry_forward_leaves_expiry_after_completely_used_leaves(self):
		...
		process_expired_allocation()
		expired_leaves = artech_engine.db.exists(
			"Leave Ledger Entry",
			dict(
				transaction_name=leave_allocation.name,
				is_expired=1,
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		self.assertFalse(artech_engine.db.exists("Leave Ledger Entry", {"transaction_name": leave_allocation.name}))
	...
	def test_leave_addition_after_submit(self):
		...
		updated_entry = artech_engine.db.get_all(
			"Leave Ledger Entry",
			{"transaction_name": leave_allocation.name},
			pluck="leaves",
	...
	def test_validation_against_leave_application_after_submit(self):
		...
		self.assertRaises(artech_engine.ValidationError, leave_allocation.submit)
...
def create_leave_allocation(**args):
	...
	employee = artech_engine.get_doc("Employee", emp_id)
