artech_engine.ui.form.on("Appraisal", {
	refresh(frm) {
	...
	add_custom_buttons(frm) {
		frm.add_custom_button(__("View Goals"), function () {
			...
			artech_engine.set_route("Tree", "Goal");
		});
	},
...
artech_engine.ui.form.on("Appraisal Goal", {
	score(frm, cdt, cdn) {
		let d = artech_engine.get_doc(cdt, cdn);

		if (flt(d.score) > 5) {
			artech_engine.msgprint(__("Score must be less than or equal to 5"));
			d.score = 0;
	...
	set_score_earned(frm, cdt, cdn) {
		...
		artech_engine.model.set_value(cdt, cdn, "score_earned", score_earned);
