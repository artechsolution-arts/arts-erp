...
class TestEmployeeCheckin(HRMSTestSuite):
	def setUp(self):
		artech_engine.db.delete("Shift Type")
		artech_engine.db.delete("Shift Assignment")
		artech_engine.db.delete("Employee Checkin")
		artech_engine.db.set_single_value("HR Settings", "allow_geolocation_tracking", 0)
	...
	def test_geolocation_tracking(self):
		...
		artech_engine.db.set_single_value("HR Settings", "allow_geolocation_tracking", 1)

		checkin.save()
		self.assertEqual(
			checkin.geolocation,
			artech_engine.json.dumps(
				{
	...
	def test_add_log_based_on_employee_field(self):
		employee = make_employee("test_add_log_based_on_employee_field@example.com", company="_Test Company")
		employee = artech_engine.get_doc("Employee", employee)
	...
	def test_mark_attendance_and_link_log(self):
		...
		logs_count = artech_engine.db.count(
		...
		now_date = nowdate()
		artech_engine.db.delete("Attendance", {"employee": employee})
		...
		attendance_count = artech_engine.db.count(
			"Attendance",
	...
	def test_unlink_attendance_on_cancellation(self):
		...
		linked_logs = artech_engine.db.get_all("Employee Checkin", {"attendance": attendance.name})
	...
	def test_calculate_working_hours(self):
		...
		logs_type_1 = [artech_engine._dict(x) for x in logs_type_1]
		logs_type_2 = [artech_engine._dict(x) for x in logs_type_2]
	...
	def test_fetch_shift_based_on_default_shift(self):
		...
		date = getdate()
		artech_engine.db.set_value("Employee", employee, "default_shift", default_shift.name)
	...
	def test_bulk_fetch_shift(self):
		...
		artech_engine.db.set_value("Employee", emp1, "default_shift", shift1.name)
		artech_engine.db.set_value("Employee", emp2, "default_shift", shift1.name)
	...
	def test_validate_time_change(self):
		...
		log.reload()
		log.time = timestamp
		self.assertRaises(artech_engine.ValidationError, log.save)
	...
	def test_modifying_half_attendance_created_from_leave(self):
		...
		employee = artech_engine.get_doc("Employee", emp)
...
def make_shift_location(location_name, latitude, longitude, checkin_radius=500):
	shift_location = artech_engine.get_doc(
		{
