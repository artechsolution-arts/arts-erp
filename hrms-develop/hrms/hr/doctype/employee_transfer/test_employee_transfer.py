...
class TestEmployeeTransfer(HRMSTestSuite):
	...
	def test_submit_before_transfer_date(self):
		...
		transfer_obj = artech_engine.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": artech_engine.get_value("Employee", {"user_id": "employee2@transfers.com"}, "name"),
				"transfer_details": [
					{
		...
		transfer_obj.save()
		self.assertRaises(artech_engine.DocstatusTransitionError, transfer_obj.submit)
		transfer = artech_engine.get_doc("Employee Transfer", transfer_obj.name)
	...
	def test_new_employee_creation(self):
		...
		transfer = artech_engine.get_doc(
			{
				"doctype": "Employee Transfer",
				"employee": artech_engine.get_value("Employee", {"user_id": "employee3@transfers.com"}, "name"),
		...
		self.assertEqual(artech_engine.get_value("Employee", transfer.new_employee_id, "status"), "Active")
		self.assertEqual(artech_engine.get_value("Employee", transfer.employee, "status"), "Left")
...
def create_company():
	if not artech_engine.db.exists("Company", "Test Company"):
