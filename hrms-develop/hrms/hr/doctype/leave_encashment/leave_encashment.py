...
from artech_engine.utils import flt, format_date, get_link_to_form, getdate
...
class LeaveEncashment(AccountsController):
		...
		if not self._salary_structure:
			artech_engine.throw(
				_("No Salary Structure assigned to Employee {0} on the given date {1}").format(
					self.employee, artech_engine.bold(format_date(self.encashment_date))
				)
			)

	def before_submit(self):
		if not self.encashment_amount or self.encashment_amount <= 0:
			artech_engine.throw(_("You can only submit Leave Encashment for a valid encashment amount"))
	...
	def on_cancel(self):
		if self.additional_salary:
			artech_engine.get_doc("Additional Salary", self.additional_salary).cancel()
		...
		if self.leave_allocation:
			artech_engine.db.set_value(
				...
				artech_engine.db.get_value("Leave Allocation", self.leave_allocation, "total_leaves_encashed")
	...
	def set_actual_encashable_days(self):
		...
		if not encashment_settings.allow_encashment:
			artech_engine.throw(_("Leave Type {0} is not encashable").format(self.leave_type))
	...
	def create_additional_salary(self):
		...
		additional_salary.company = artech_engine.get_value("Employee", self.employee, "company")
		additional_salary.employee = self.employee
		additional_salary.currency = self.currency
		earning_component = artech_engine.get_value("Leave Type", self.leave_type, "earning_component")
		if not earning_component:
			artech_engine.throw(_("Please set Earning Component for Leave type: {0}.").format(self.leave_type))
	...
	def set_encashment_amount(self):
		...
		per_day_encashment = artech_engine.get_value(
		...
		if not per_day_encashment:
			per_day_encashment = artech_engine.db.get_value(
				"Salary Structure",
	...
	def set_total_advance_paid(self):
		...
		if flt(paid_amount) > self.encashment_amount:
			artech_engine.throw(_("Row {0}# Paid Amount cannot be greater than Encashment amount"))
...
def create_leave_encashment(leave_allocation):
	...
	for allocation in leave_allocation:
		if not get_assigned_salary_structure(allocation.employee, allocation.to_date):
			continue
		leave_encashment = artech_engine.get_doc(
			dict(
