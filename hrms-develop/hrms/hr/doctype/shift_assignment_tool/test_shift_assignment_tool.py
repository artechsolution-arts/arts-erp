...
class TestShiftAssignmentTool(HRMSTestSuite):
	...
	def test_get_employees_for_assigning_shifts(self):
		...
		artech_engine.db.set_single_value("HR Settings", "allow_multiple_shift_assignments", 1)
		employees = shift_assignment_tool.get_employees(advanced_filters)
	...
	def test_bulk_assign_shift(self):
		...
		shift_assignment_employees = artech_engine.get_list(
			"Shift Assignment",
			filters={
				"shift_type": self.shift1.name,
				"status": "Active",
	...
	def test_bulk_assign_shift_schedule(self):
		...
		assigned_employees = artech_engine.get_list(
	...
	def test_bulk_process_shift_requests(self):
		...
		processed_shift_requests = artech_engine.get_list(
			"Shift Request",
		...
		status, docstatus = artech_engine.db.get_value("Shift Request", request3.name, ["status", "docstatus"])
		self.assertEqual(status, "Approved")
		self.assertEqual(docstatus, 1)

		shift_assignment = artech_engine.db.exists("Shift Assignment", {"shift_request": request3.name})
