artech_engine.provide("artech.job_offer");

artech_engine.ui.form.on("Job Offer", {
	onload: function (frm) {
	...
	select_terms: function (frm) {
		artech.utils.get_terms(frm.doc.select_terms, frm.doc, function (r) {
			if (!r.exc) {
	...
	refresh: function (frm) {
		if (
			...
			frm.add_custom_button(__("Create Employee"), function () {
				artech.job_offer.make_employee(frm);
			});
		}

		if (frm.doc.__onload && frm.doc.__onload.employee) {
			frm.add_custom_button(__("Show Employee"), function () {
				artech_engine.set_route("Form", "Employee", frm.doc.__onload.employee);
			});
		}
	},
});
