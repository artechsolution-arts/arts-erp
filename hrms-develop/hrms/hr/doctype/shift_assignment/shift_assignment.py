...
class MultipleShiftError(artech_engine.ValidationError):
	pass
...
class ShiftAssignment(Document):
	...
	def validate_employee_checkin(self):
		checkins = artech_engine.get_all(
			"Employee Checkin",
			filters={
	...
	def validate_attendance(self):
		attendances = artech_engine.get_all(
			"Attendance",
			filters={
				"employee": self.employee,
				"shift": self.shift_type,
	...
	def validate_same_date_multiple_shifts(self, overlapping_dates):
		if cint(artech_engine.db.get_single_value("HR Settings", "allow_multiple_shift_assignments")):
			...
			msg += _("To allow this, enable {0} under {1}.").format(
				artech_engine.bold(_("Allow Multiple Shift Assignments for Same Date")),
	...
	def get_overlapping_dates(self):
		...
		shift = artech_engine.qb.DocType("Shift Assignment")
		query = (
	...
	def throw_overlap_error(self, shift_details):
		...
		if shift_details.docstatus == 1 and shift_details.status == "Active":
			...
			artech_engine.throw(msg, title=_("Overlapping Shifts"), exc=OverlappingShiftError)
...
def has_overlapping_timings(shift_1: str, shift_2: str) -> bool:
	...
	s1 = artech_engine.db.get_value("Shift Type", shift_1, ["start_time", "end_time"], as_dict=True)
	s2 = artech_engine.db.get_value("Shift Type", shift_2, ["start_time", "end_time"], as_dict=True)
...
def get_events(start: str | date, end: str | date, filters: list | None = None):
	employee = artech_engine.db.get_value(
		"Employee", {"user_id": artech_engine.session.user}, ["name", "company"], as_dict=True
	)
	if employee:
...
def mark_expired_shift_assignments_as_inactive():
	...
	for assignment in expired_assignments:
		artech_engine.db.set_value("Shift Assignment", assignment, "status", "Inactive")
...
def get_shift_events(assignments: list[dict]) -> list[dict]:
	...
	for d in assignments:
		...
		while daily_event_start <= daily_event_end:
			start_timing = artech_engine.utils.get_datetime(daily_event_start) + shift_start

			if shift_start > shift_end:
				# shift spans across 2 days
				end_timing = artech_engine.utils.get_datetime(daily_event_start) + shift_end + delta
			else:
				end_timing = artech_engine.utils.get_datetime(daily_event_start) + shift_end
...
def get_shifts_for_date(employee: str, for_timestamp: datetime) -> list[dict[str, str]]:
	...
	assignment = artech_engine.qb.DocType("Shift Assignment")
	return (
...
def get_employee_shift(
	...
	default_shift = artech_engine.db.get_value("Employee", employee, "default_shift", cache=True)
...
def get_prev_or_next_shift(
	...
	if consider_default_shift and default_shift:
		...
		shift_dates = artech_engine.get_all(
			"Shift Assignment",
			["start_date", "end_date"],
			{
				"employee": employee,
