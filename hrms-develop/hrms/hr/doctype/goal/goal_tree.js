...
artech_engine.treeview_settings["Goal"] = {
	...
	filters: [
		...
		},
		{
			...
			get_query() {
				const company =
					artech_engine.treeview_settings["Goal"].page.fields_dict.company.get_value();
	...
	fields: [
		...
		},
		{
			...
			reqd: 1,
			default() {
				const treeview = artech_engine.treeview_settings["Goal"].treeview;
				let employee =
	...
	onload(treeview) {
		artech_engine.treeview_settings["Goal"].page = {};
		$.extend(artech_engine.treeview_settings["Goal"].page, treeview.page);
		treeview.make_tree();
	...
	post_render(treeview) {
		artech_engine.treeview_settings["Goal"].treeview = {};
		$.extend(artech_engine.treeview_settings["Goal"].treeview, treeview);
	},
	get_label(node) {
	...
	toolbar: [
		...
		},
		{
			...
			click: function (node) {
				artech_engine.confirm(__("Mark {0} as Completed?", [node.label.bold()]), () =>
...
function update_progress(node, progress) {
	return artech_engine
		...
		.then((r) => {
			if (!r.exc && r.message) {
				artech_engine.treeview_settings["Goal"].treeview.tree.load_children(
					artech_engine.treeview_settings["Goal"].treeview.tree.root_node,
					true,
				);
				...
				artech_engine.msgprint(__("Could not update Goal"));
			}
		});
}
