artech_engine.listview_settings["Goal"] = {
	...
	trigger_error_dialogs: function (checked_items, status) {
		if (!checked_items.length) {
			artech_engine.throw(__("No items selected"));
			return;
		}
		...
		if (checked_items.some((item) => !applicable_statuses.includes(item.status)))
			artech_engine.msgprint({
				...
				message: __("Only {0} Goals can be {1}", [
					artech_engine.utils.comma_and(applicable_statuses),
					status,
				]),
				indicator: "orange",
			});
		...
		artech_engine
			...
			.then((r) => {
				if (!r.exc && r.message) {
					...
					artech_engine.msgprint(__("Could not update goals"));
				}
