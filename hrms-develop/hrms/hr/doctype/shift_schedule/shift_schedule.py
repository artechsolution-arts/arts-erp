...
class ShiftSchedule(Document):
	...
	if TYPE_CHECKING:
		from artech_engine.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
...
def get_or_insert_shift_schedule(shift_type: str, frequency: str, repeat_on_days: list[str]) -> str:
	shift_schedules = artech_engine.get_all(
		"Shift Schedule",
		pluck="name",
	...
	for shift_schedule in shift_schedules:
		shift_schedule = artech_engine.get_doc("Shift Schedule", shift_schedule)
