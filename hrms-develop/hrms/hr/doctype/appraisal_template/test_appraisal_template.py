...
class TestAppraisalTemplate(HRMSTestSuite):
	def test_incorrect_weightage_allocation(self):
		...
		self.assertRaises(artech_engine.ValidationError, template.save)
...
def create_kras(kras):
	for entry in kras:
		if not artech_engine.db.exists("KRA", entry):
...
def create_criteria(criteria):
	for entry in criteria:
		if not artech_engine.db.exists("Employee Feedback Criteria", entry):
			artech_engine.get_doc(
				{
...
def create_appraisal_template(title=None, kras=None, rating_criteria=None):
	...
	if artech_engine.db.exists("Appraisal Template", name):
		return artech_engine.get_doc("Appraisal Template", name)
	...
	appraisal_template = artech_engine.new_doc("Appraisal Template")
