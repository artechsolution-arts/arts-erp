artech_engine.ui.form.on("Interview", {
	...
	add_custom_buttons: async function (frm) {
		...
		const has_submitted_feedback = await artech_engine.db.get_value(
			"Interview Feedback",
			{
				interviewer: artech_engine.session.user,
		...
		const allow_feedback_submission = frm.doc.interview_details.some(
			(interviewer) => interviewer.interviewer === artech_engine.session.user,
		);
	...
	get_fields_for_feedback: async function () {
		return new Promise((resolve, reject) => {
			artech_engine.model.with_doctype("Skill Assessment", () => {
				let meta = artech_engine.get_meta("Skill Assessment");
	...
	job_applicant: function (frm) {
		if (!frm.doc.interview_type) {
			frm.set_value("job_applicant", "");
			artech_engine.throw(__("Select Interview Type First"));
		}
	...
	render_feedback(frm) {
		artech_engine.require("interview.bundle.js", () => {
			const wrapper = $(frm.fields_dict.feedback_html.wrapper);
			const feedback_html = artech_engine.render_template("interview_feedback", {
