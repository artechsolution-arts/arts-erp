...
class TestShiftType(HRMSTestSuite):
	...
	def test_auto_update_last_sync_of_checkin_for_single_day_shift(self):
		...
		artech_engine.flags.current_datetime = datetime.combine(getdate(), get_time("14:00:00"))
		...
		artech_engine.flags.current_datetime = add_days(datetime.combine(getdate(), get_time("12:00:00")), 1)
		update_last_sync_of_checkin()
		shift_type.reload()
		self.assertEqual(shift_type.last_sync_of_checkin, datetime.combine(getdate(), get_time("13:01:00")))
	...
	def test_auto_update_last_sync_of_checkin_for_shifts_spanning_two_days_due_to_buffer(self):
		...
		# case 1: last sync updates from none to shift end after the shift end time
		artech_engine.flags.current_datetime = datetime.combine(getdate(), get_time("02:00:00"))
		update_last_sync_of_checkin()
		shift_type.reload()
		...
		# case 2: last sync doesn't update in the middle of the shift
		artech_engine.flags.current_datetime = datetime.combine(getdate(), get_time("19:00:00"))
		update_last_sync_of_checkin()
		shift_type.reload()
		self.assertEqual(shift_type.last_sync_of_checkin, datetime.combine(getdate(), get_time("01:01:00")))
	...
	def test_auto_update_last_sync_of_checkin_for_two_day_shift(self):
		...
		# case 1: last sync updates from none to shift end after the shift end time
		artech_engine.flags.current_datetime = datetime.combine(getdate(), get_time("08:00:00"))
		update_last_sync_of_checkin()
		shift_type.reload()
		...
		# case 2: last sync doesn't update in the middle of the shift
		artech_engine.flags.current_datetime = add_days(datetime.combine(getdate(), get_time("01:00:00")), 1)
		update_last_sync_of_checkin()
		shift_type.reload()
		self.assertEqual(shift_type.last_sync_of_checkin, datetime.combine(getdate(), get_time("07:01:00")))
	...
	def test_auto_update_last_sync_of_checkin_when_when_job_runs_on_the_next_day(self):
		...
		artech_engine.flags.current_datetime = datetime.combine(add_days(getdate(), 1), get_time("00:01:00"))
		update_last_sync_of_checkin()
	...
	def test_working_hours_threshold_for_absent_and_half_day_2(self):
		...
		attendance = artech_engine.db.get_value("Attendance", {"shift": shift_type.name}, "status")
	...
	def test_mark_absent_for_dates_with_no_attendance(self):
		...
		absent_records = artech_engine.get_all(
			"Attendance",
			{
		...
		todays_attendance = artech_engine.db.get_value(
	...
	def test_do_not_mark_absent_before_shift_actual_end_time(self):
		...
		self.assertIsNone(artech_engine.db.get_value("Attendance", {"attendance_date": today, "employee": employee}))
	...
	def test_skip_marking_absent_on_a_holiday(self):
		...
		shift_type.process_auto_attendance()
		attendance = artech_engine.db.get_value("Attendance", {"employee": employee}, "status")
		self.assertIsNone(attendance)
	...
	def test_get_start_and_end_dates(self):
		...
		attendance = artech_engine.db.get_value("Attendance", {"attendance_date": doj, "employee": employee}, "name")
		self.assertIsNone(attendance)
	...
	def test_validation_for_unlinked_logs_before_changing_important_shift_configuration(self):
		...
		self.assertRaises(artech_engine.ValidationError, shift.save)
		...
		shift.save()
		self.assertEqual(
			artech_engine.get_value("Shift Type", shift.name, "begin_check_in_before_shift_start_time"), 120
		)
		...
		shift.save()
		self.assertEqual(
			get_time(artech_engine.get_value("Shift Type", shift.name, "start_time")), get_time("10:15:00")
		)

	def test_circular_shift_times(self):
		# single day shift
		shift_type = artech_engine.get_doc(
			{
				"doctype": "Shift Type",
		...
		self.assertRaises(artech_engine.ValidationError, shift_type.save)
	...
	def test_bg_job_creation_for_large_checkins(self):
		shift = setup_shift_type(shift_type="Test Shift", start_time="10:00:00", end_time="18:00:00")
		artech_engine.flags.test_bg_job = True
		shift.process_auto_attendance(is_manually_triggered=True)
		job = artech_engine.get_all("RQ Job", {"job_id": f"process_auto_attendance_{shift.name}"})
		self.assertTrue(job)
	...
	def test_precision_for_working_hours_threshold(self):
		...
		check_out = make_checkin(employee, out_time)
		shift.process_auto_attendance()
		attendance = artech_engine.get_doc(
	...
	def test_working_hours_threshold_for_half_day_holiday(self):
		...
		shift.process_auto_attendance()
		attendance1 = artech_engine.get_doc(
		...
		attendance2 = artech_engine.get_doc(
		...
		attendance3 = artech_engine.get_doc(
...
def setup_shift_type(**args):
	...
	date = getdate()
	if not artech_engine.db.get_value("Shift Type", args.shift_type):
		shift_type = artech_engine.get_doc(
			{
				"doctype": "Shift Type",
		...
		shift_type = artech_engine.get_doc("Shift Type", args.shift_type)

	holiday_list = "Employee Checkin Test Holiday List"
	if not artech_engine.db.exists("Holiday List", "Employee Checkin Test Holiday List"):
...
def make_shift_assignment(
	...
	shift_assignment = artech_engine.get_doc(
		{
