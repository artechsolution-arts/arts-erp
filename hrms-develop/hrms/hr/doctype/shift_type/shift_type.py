...
class ShiftType(Document):
	...
	def process_auto_attendance(self, is_manually_triggered: int | bool = False) -> None | str:
		...
		if is_manually_triggered:
			if len(logs) > 1000 or artech_engine.flags.test_bg_job:
				job_id = "process_auto_attendance_" + self.name
				job = artech_engine.enqueue(self._process, logs=logs, timeout=1200, job_id=job_id, deduplicate=True)
				...
				artech_engine.get_doc(
					{
						...
						"content": artech_engine._("Employee was marked Absent due to missing Employee Checkins."),
					}
	...
	def get_start_and_end_dates(self, employee):
		...
		"""
		date_of_joining, relieving_date, employee_creation = artech_engine.get_cached_value(
	...
	def get_assigned_employees(self, from_date: datetime.date, consider_default_shift=False) -> list[str]:
		...
		assigned_employees = artech_engine.get_all(
			"Shift Assignment", filters=filters, or_filters=or_filters, pluck="employee"
		)

		if consider_default_shift:
			default_shift_employees = artech_engine.get_all(
		...
		inactive_employees = artech_engine.db.get_all("Employee", {"status": "Inactive"}, pluck="name")
	...
	def mark_absent_for_half_day_dates(self, employee):
		half_day_attendances = artech_engine.get_all(
			"Attendance",
			filters={
				"employee": employee,
				"status": "Half Day",
...
def update_last_sync_of_checkin():
	...
	current_datetime = artech_engine.flags.current_datetime or get_datetime()
	for shift in shifts:
...
def process_auto_attendance_for_all_shifts():
	"""Called from hooks"""
	shift_list = artech_engine.get_all("Shift Type", filters={"enable_auto_attendance": "1"}, pluck="name")
	for shift in shift_list:
		doc = artech_engine.get_cached_doc("Shift Type", shift)
