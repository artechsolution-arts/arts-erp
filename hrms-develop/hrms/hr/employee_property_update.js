artech_engine.ui.form.on(cur_frm.doctype, {
	setup: function (frm) {
	...
	setup_employee_property_button: function (frm, table) {
		frm.fields_dict[table].grid.add_custom_button(__("Add Employee Property"), () => {
			...
			artech_engine.model.with_doctype("Employee", () => {
				...
				artech_engine.get_meta("Employee").fields.forEach((d) => {
...
var render_dynamic_field = function (d, fieldtype, options, fieldname) {
	d.data.new = null;
	var dynamic_field = artech_engine.ui.form.make_control({
		df: {
			fieldtype: fieldtype,
			fieldname: fieldname,
			options: options || "",
...
var add_to_details = function (frm, d, table) {
	...
	if (data.fieldname) {
		if (validate_duplicate(frm, table, data.fieldname)) {
			artech_engine.show_alert({ message: __("Property already added"), indicator: "orange" });
			return false;
		}
		if (data.current == data.new) {
			artech_engine.show_alert({ message: __("Nothing to change"), indicator: "orange" });
			d.get_primary_btn().attr("disabled", false);
			return false;
		}
		frm.add_child(table, {
		...
		d.set_value("current", "");
		artech_engine.show_alert({ message: __("Added to details"), indicator: "green" });
		d.data = {};
	} else {
		artech_engine.show_alert({ message: __("Value missing"), indicator: "red" });
	}
};
