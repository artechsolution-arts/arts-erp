artech_engine.query_reports["Leave Ledger"] = {
	filters: [
		{
			fieldname: "from_date",
			label: __("From Date"),
			fieldtype: "Date",
			reqd: 1,
			default: artech_engine.defaults.get_default("year_start_date"),
		},
		{
			fieldname: "to_date",
			label: __("To Date"),
			fieldtype: "Date",
			reqd: 1,
			default: artech_engine.defaults.get_default("year_end_date"),
		},
		{
	...
	onload: () => {
		if (
			artech_engine.query_report.get_filter_value("from_date") &&
			artech_engine.query_report.get_filter_value("to_date")
		)
			return;
