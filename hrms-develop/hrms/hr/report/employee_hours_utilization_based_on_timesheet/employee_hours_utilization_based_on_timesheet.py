...
class EmployeeHoursReport:
	...
	def validate_dates(self):
		...
		if self.day_span <= 0:
			artech_engine.throw(_("From Date must come before To Date"))

	def validate_standard_working_hours(self):
		self.standard_working_hours = artech_engine.db.get_single_value("HR Settings", "standard_working_hours")
		if not self.standard_working_hours:
			msg = _("The metrics for this report are calculated based on {0}. Please set {0} in {1}.").format(
				artech_engine.bold(_("Standard Working Hours")),
				artech_engine.utils.get_link_to_form("HR Settings", "HR Settings"),
			)
	...
	def filter_stats_by_department(self):
		filtered_data = artech_engine._dict()
		for emp, data in self.stats_by_employee.items():
	...
	def generate_filtered_time_logs(self):
		...
		self.filtered_time_logs = artech_engine.db.sql(
			f"""
	...
	def generate_stats_by_employee(self):
		self.stats_by_employee = artech_engine._dict()

		for emp, hours, is_billable, __ in self.filtered_time_logs:
			self.stats_by_employee.setdefault(emp, artech_engine._dict()).setdefault("billed_hours", 0.0)
	...
	def set_employee_department_and_name(self):
		for emp in self.stats_by_employee:
			emp_name = artech_engine.db.get_value("Employee", emp, "employee_name")
			emp_dept = artech_engine.db.get_value("Employee", emp, "department")
