...
from artech.tests.utils import ERPNextTestSuite
...
class BootStrapTestData:
	...
	def make_master_data(self):
		...
		# TODO: clean up
		if artech_engine.db.get_value("Holiday List Assignment", {"assigned_to": "_Test Company"}, "docstatus") == 0:
			artech_engine.get_doc("Holiday List Assignment", {"assigned_to": "_Test Company"}).submit()
	...
	def update_email_account_settings(self):
		email_account = artech_engine.get_doc("Email Account", "Jobs")
	...
	def make_records(self, key, records):
		...
		for x in records:
			...
			if not artech_engine.db.exists(doctype, filters):
				doc = artech_engine.get_doc(x).insert()
