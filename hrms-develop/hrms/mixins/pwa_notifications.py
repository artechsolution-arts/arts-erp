...
from artech_engine import bold
...
class PWANotificationsMixin:
	...
	def notify_approval_status(self):
		...
		if self.has_value_changed(status_field) and status in ["Approved", "Rejected"]:
			from_user = artech_engine.session.user
			...
			notification = artech_engine.new_doc("PWA Notification")
	...
	def _get_employee_user(self) -> str:
		return artech_engine.db.get_value("Employee", self.employee, "user_id", cache=True)

	def _get_user_name(self, user) -> str:
		return artech_engine.db.get_value("User", user, "full_name", cache=True)
