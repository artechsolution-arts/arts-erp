...
from artech_engine.model import get_permitted_fields
from artech_engine.model.workflow import get_workflow_name
from artech_engine.query_builder import Order
from artech_engine.utils import add_days, date_diff, getdate, strip_html
...
def get_current_employee() -> str:
	...
	if not employee:
		artech_engine.throw(_("Employee not found"), artech_engine.PermissionError)
	return employee
...
def get_hr_settings() -> dict:
	settings = artech_engine.db.get_singles_dict("HR Settings", cast=True)
...
def get_unread_notifications_count() -> int:
	return artech_engine.db.count(
		"PWA Notification",
		{"to_user": artech_engine.session.user, "read": 0},
	)
...
def are_push_notifications_enabled() -> bool:
	try:
		return artech_engine.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
...
def get_shift_requests(
	...
	shift_requests = artech_engine.get_list(
		"Shift Request",
		fields=fields,
		filters=filters,
...
def get_attendance_requests(
	...
	attendance_requests = artech_engine.get_list(
		"Attendance Request",
		fields=fields,
		filters=filters,
		order_by="creation desc",
...
def get_shift_request_approvers(employee: str) -> str | list[str]:
	shift_request_approver, department = artech_engine.get_cached_value(
		"Employee",
		employee,
	...
	if department:
		...
		if not shift_request_approver:
			shift_request_approver = artech_engine.db.get_value(
				"Department Approver",
				{"parent": department, "parentfield": "shift_request_approver", "idx": 1},
				"approver",
			)

	shift_request_approver_name = artech_engine.db.get_value("User", shift_request_approver, "full_name", cache=True)
...
def get_shifts() -> list[dict[str, str]]:
	employee = get_current_employee()
	ShiftAssignment = artech_engine.qb.DocType("Shift Assignment")
	ShiftType = artech_engine.qb.DocType("Shift Type")
	return (
		artech_engine.qb.from_(ShiftAssignment)
		.join(ShiftType)
...
def get_leave_applications(
	...
	applications = artech_engine.get_list(
		"Leave Application",
		fields=fields,
		filters=filters,
...
def get_leave_approval_details(employee: str) -> dict:
	leave_approver, department = artech_engine.get_cached_value(
		"Employee",
		employee,
	...
	leave_approver_name = artech_engine.db.get_value("User", leave_approver, "full_name", cache=True)
	...
	return dict(
		...
		is_mandatory=artech_engine.db.get_single_value(
...
def get_department_approvers(department: str, parentfield: str) -> list[str]:
	...
	department_details = artech_engine.db.get_value("Department", department, ["lft", "rgt"], as_dict=True)
	departments = artech_engine.get_all(
		"Department",
		filters={
	...
	Approver = artech_engine.qb.DocType("Department Approver")
	User = artech_engine.qb.DocType("User")
	department_approvers = (
		artech_engine.qb.from_(User)
		.join(Approver)
...
def get_expense_claim_summary() -> dict:
	...
	Claim = artech_engine.qb.DocType("Expense Claim")

	pending_claims_case = (
		artech_engine.qb.terms.Case().when(Claim.approval_status == "Draft", Claim.total_claimed_amount).else_(0)
	)
	...
	approved_total_claimed_case = (
		artech_engine.qb.terms.Case().when(Claim.approval_status == "Approved", Claim.total_claimed_amount).else_(0)
	)
	sum_approved_total_claimed = Sum(approved_total_claimed_case).as_("total_claimed_in_approved")

	rejected_claims_case = (
		artech_engine.qb.terms.Case().when(Claim.approval_status == "Rejected", Claim.total_claimed_amount).else_(0)
	)
	sum_rejected_claims = Sum(rejected_claims_case).as_("total_rejected_amount")

	summary = (
		artech_engine.qb.from_(Claim)
		.select(
			sum_pending_claims,
			sum_approved_claims,
			sum_rejected_claims,
	...
	currency = artech_engine.db.get_value("Company", summary.company, "default_currency")
...
def get_expense_type_description(expense_type: str) -> str:
	return artech_engine.db.get_value("Expense Claim Type", expense_type, "description")


@artech_engine.whitelist()
def get_expense_claim_types() -> list[dict]:
	ClaimType = artech_engine.qb.DocType("Expense Claim Type")

	return (artech_engine.qb.from_(ClaimType).select(ClaimType.name, ClaimType.description)).run(as_dict=True)


@artech_engine.whitelist()
def get_expense_approval_details(employee: str) -> dict:
	expense_approver, department = artech_engine.get_cached_value(
		"Employee",
		employee,
	...
	if not expense_approver and department:
		expense_approver = artech_engine.db.get_value(
			"Department Approver",
			{"parent": department, "parentfield": "expense_approvers", "idx": 1},
			"approver",
		)

	expense_approver_name = artech_engine.db.get_value("User", expense_approver, "full_name", cache=True)
	...
	return dict(
		...
		department_approvers=department_approvers,
		is_mandatory=artech_engine.db.get_single_value("HR Settings", "expense_approver_mandatory_in_expense_claim"),
	)
...
def get_employee_advance_balance() -> list[dict]:
	...
	advances = (
		artech_engine.qb.from_(Advance)
		.select(
			Advance.name,
			Advance.employee,
			Advance.status,
...
def get_company_currencies() -> dict:
	...
	Currency = artech_engine.qb.DocType("Currency")
...
def get_currency_symbols() -> dict:
	...
	currencies = (artech_engine.qb.from_(Currency).select(Currency.name, Currency.symbol)).run(as_dict=True)
...
def get_doctype_fields(doctype: str) -> list[dict]:
	fields = artech_engine.get_meta(doctype).fields
	return [
		field
		for field in fields
...
def get_doctype_states(doctype: str) -> dict:
	states = artech_engine.get_meta(doctype).states
...
def upload_base64_file(
	...
	from artech_engine.handler import ALLOWED_MIMETYPES

	decoded_content = base64.b64decode(content)
	content_type = guess_type(filename)[0]
	if content_type not in ALLOWED_MIMETYPES:
		artech_engine.throw(_("You can only upload JPG, PNG, PDF, TXT or Microsoft documents."))
...
def delete_attachment(filename: str):
	artech_engine.delete_doc("File", filename)
...
def _download_pdf(doctype: str, docname: str) -> str:
	...
	default_print_format = artech_engine.get_meta(doctype).default_print_format or "Standard"

	try:
		download_pdf(doctype, docname, format=default_print_format)
	except Exception as e:
		artech_engine.throw(_("Failed to download PDF: {0}").format(str(e)))

	base64content = base64.b64encode(artech_engine.local.response.filecontent)
	content_type = artech_engine.local.response.type
...
def get_workflow(doctype: str) -> dict:
	...
	if not workflow:
		return artech_engine._dict()
	return artech_engine.get_doc("Workflow", workflow)
...
def get_workflow_state_field(doctype: str) -> str | None:
	...
	override_status, workflow_state_field = artech_engine.db.get_value(
		"Workflow",
		workflow_name,
...
def get_allowed_states_for_workflow(workflow: dict, user_id: str) -> list[str]:
	user_roles = artech_engine.get_roles(user_id)
