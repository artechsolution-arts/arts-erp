import json

import artech_engine
from artech_engine import _
from werkzeug.wrappers import Response

from artech_crm.integrations.api import get_contact_by_phone_number

from .twilio_handler import IncomingCall, Twilio, TwilioCallDetails


def validate_twilio_request(args, require_application_sid: bool = False):
	twilio = Twilio.connect()
	if not twilio:
		artech_engine.throw(_("Twilio configuration is missing"), artech_engine.PermissionError)

	account_sid = artech_engine.utils.cstr(args.get("AccountSid"))
	if not account_sid or account_sid != artech_engine.utils.cstr(twilio.account_sid):
		artech_engine.throw(_("Invalid Twilio account"), artech_engine.PermissionError)

	if require_application_sid:
		application_sid = artech_engine.utils.cstr(args.get("ApplicationSid"))
		if not application_sid or application_sid != artech_engine.utils.cstr(twilio.application_sid):
			artech_engine.throw(_("Invalid Twilio application"), artech_engine.PermissionError)

	return twilio


@artech_engine.whitelist()
def is_enabled():
	return artech_engine.db.get_single_value("CRM Twilio Settings", "enabled")


@artech_engine.whitelist()
def generate_access_token():
	"""Returns access token that is required to authenticate Twilio Client SDK."""
	twilio = Twilio.connect()
	if not twilio:
		return {}

	from_number = artech_engine.db.get_value("CRM Telephony Agent", artech_engine.session.user, "twilio_number")
	if not from_number:
		return {
			"ok": False,
			"error": "caller_phone_identity_missing",
			"detail": "Phone number is not mapped to the caller",
		}

	token = twilio.generate_voice_access_token(identity=artech_engine.session.user)
	return {"token": artech_engine.safe_decode(token)}


@artech_engine.whitelist(allow_guest=True)
def voice(**kwargs):
	"""This is a webhook called by twilio to get instructions when the voice call request comes to twilio server."""

	def _get_caller_number(caller):
		identity = caller.replace("client:", "").strip()
		user = Twilio.emailid_from_identity(identity)
		return artech_engine.db.get_value("CRM Telephony Agent", user, "twilio_number")

	args = artech_engine._dict(kwargs)
	twilio = validate_twilio_request(args, require_application_sid=True)

	# Generate TwiML instructions to make a call
	from_number = _get_caller_number(args.Caller)
	resp = twilio.generate_twilio_dial_response(from_number, args.To)

	call_details = TwilioCallDetails(args, call_from=from_number)
	create_call_log(call_details)
	return Response(resp.to_xml(), mimetype="text/xml")


@artech_engine.whitelist(allow_guest=True)
def twilio_incoming_call_handler(**kwargs):
	args = artech_engine._dict(kwargs)
	validate_twilio_request(args)

	call_details = TwilioCallDetails(args)
	create_call_log(call_details)

	resp = IncomingCall(args.From, args.To).process()
	return Response(resp.to_xml(), mimetype="text/xml")


def create_call_log(call_details: TwilioCallDetails):
	details = call_details.to_dict()

	call_log = artech_engine.get_doc({**details, "doctype": "CRM Call Log", "telephony_medium": "Twilio"})

	# link call log with lead/deal
	contact_number = details.get("from") if details.get("type") == "Incoming" else details.get("to")
	link(contact_number, call_log)

	call_log.save(ignore_permissions=True)
	artech_engine.db.commit()
	return call_log


def link(contact_number, call_log):
	contact = get_contact_by_phone_number(contact_number)
	if contact.get("name"):
		doctype = "Contact"
		docname = contact.get("name")
		if contact.get("lead"):
			doctype = "CRM Lead"
			docname = contact.get("lead")
		elif contact.get("deal"):
			doctype = "CRM Deal"
			docname = contact.get("deal")
		call_log.link_with_reference_doc(doctype, docname)


def update_call_log(call_sid, status=None):
	"""Update call log status."""
	twilio = Twilio.connect()
	if not (twilio and artech_engine.db.exists("CRM Call Log", call_sid)):
		return

	try:
		call_details = twilio.get_call_info(call_sid)
		call_log = artech_engine.get_doc("CRM Call Log", call_sid)
		call_log.status = TwilioCallDetails.get_call_status(status or call_details.status)
		call_log.duration = call_details.duration
		call_log.start_time = get_datetime_from_timestamp(call_details.start_time)
		call_log.end_time = get_datetime_from_timestamp(call_details.end_time)
		call_log.save(ignore_permissions=True)
		artech_engine.db.commit()
		return call_log
	except Exception:
		artech_engine.log_error(title="Error while updating call record")
		artech_engine.db.commit()


def get_twilio_settings():
	return artech_engine.get_single("CRM Twilio Settings")


@artech_engine.whitelist(allow_guest=True)
def update_recording_info(**kwargs):
	args = artech_engine._dict(kwargs)
	validate_twilio_request(args)

	recording_url = args.RecordingUrl
	call_sid = args.CallSid
	call_log = update_call_log(call_sid)
	if not call_log:
		artech_engine.throw(_("Call log not found"), artech_engine.DoesNotExistError)

	try:
		artech_engine.db.set_value("CRM Call Log", call_sid, "recording_url", recording_url)
		artech_engine.db.commit()
	except Exception as exc:
		artech_engine.log_error(title=_("Failed to capture Twilio recording"))
		raise exc


@artech_engine.whitelist(allow_guest=True)
def update_call_status_info(**kwargs):
	args = artech_engine._dict(kwargs)
	validate_twilio_request(args)

	parent_call_sid = args.ParentCallSid
	call_log = update_call_log(parent_call_sid, status=args.CallStatus)
	if not call_log:
		artech_engine.throw(_("Call log not found"), artech_engine.DoesNotExistError)

	call_info = {
		"ParentCallSid": args.ParentCallSid,
		"CallSid": args.CallSid,
		"CallStatus": args.CallStatus,
		"CallDuration": args.CallDuration,
		"From": args.From,
		"To": args.To,
	}

	try:
		client = Twilio.get_twilio_client()
		client.calls(args.ParentCallSid).user_defined_messages.create(content=json.dumps(call_info))
	except Exception as exc:
		artech_engine.log_error(title=_("Failed to update Twilio call status"))
		raise exc


def get_datetime_from_timestamp(timestamp):
	from datetime import datetime
	from zoneinfo import ZoneInfo

	if not timestamp:
		return None

	datetime_utc_tz_str = timestamp.strftime("%Y-%m-%d %H:%M:%S%z")
	datetime_utc_tz = datetime.strptime(datetime_utc_tz_str, "%Y-%m-%d %H:%M:%S%z")
	system_timezone = artech_engine.utils.get_system_timezone()
	converted_datetime = datetime_utc_tz.astimezone(ZoneInfo(system_timezone))
	return artech_engine.utils.format_datetime(converted_datetime, "yyyy-MM-dd HH:mm:ss")
