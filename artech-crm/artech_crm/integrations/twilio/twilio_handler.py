...
from artech_engine.utils.password import get_decrypted_password
...
class Twilio:
	...
	def connect(cls):
		...
		settings = artech_engine.get_doc("CRM Twilio Settings")
	...
	def get_recording_status_callback_url(self):
		url_path = "/api/method/artech_crm.integrations.twilio.api.update_recording_info"
		return get_public_url(url_path)

	def get_update_call_status_callback_url(self):
		url_path = "/api/method/artech_crm.integrations.twilio.api.update_call_status_info"
		return get_public_url(url_path)
	...
	def get_twilio_client(self):
		twilio_settings = artech_engine.get_doc("CRM Twilio Settings")
		if not twilio_settings.enabled:
			artech_engine.throw(_("Please enable twilio settings before making a call."))
...
def get_twilio_number_owners(phone_number):
	...
	user_voice_settings = artech_engine.get_all(
		"CRM Telephony Agent",
	...
	user_general_settings = artech_engine.get_all(
...
def get_the_call_attender(owners, caller=None):
	...
	if len(current_loggedin_users) > 1 and caller:
		deal_owner = artech_engine.db.get_value("CRM Deal", {"mobile_no": caller}, "deal_owner")
		if not deal_owner:
			deal_owner = artech_engine.db.get_value(
