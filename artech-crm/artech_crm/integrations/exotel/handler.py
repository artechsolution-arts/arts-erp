...
from artech_engine.integrations.utils import create_request_log
...
# Incoming Call:
# <site>/api/method/artech_crm.integrations.exotel.handler.handle_request?key=<exotel-webhook-verify-token>
...
def handle_request(**kwargs):
	...
	request_log = create_request_log(
		...
		service_name="Exotel",
		request_headers=artech_engine.request.headers,
		is_remote_request=1,
	)
	...
	try:
		...
		artech_engine.publish_realtime("exotel_call", call_payload)
		...
		request_log.error = artech_engine.get_traceback()
		artech_engine.db.rollback()
		artech_engine.log_error(title="Error while creating/updating call record")
...
def make_a_call(to_number: str, from_number: str | None = None, caller_id: str | None = None):
	if not is_integration_enabled():
		artech_engine.throw(_("Please setup Exotel intergration"), title=_("Integration Not Enabled"))

	endpoint = get_exotel_endpoint("Calls/connect.json?details=true")

	if not from_number:
		from_number = artech_engine.get_value("CRM Telephony Agent", {"user": artech_engine.session.user}, "mobile_no")

	if not caller_id:
		caller_id = artech_engine.get_value("CRM Telephony Agent", {"user": artech_engine.session.user}, "exotel_number")
	...
	if caller_id and caller_id not in get_all_exophones():
		artech_engine.throw(_("Exotel Number {0} is not valid").format(caller_id), title=_("Invalid Exotel Number"))
	...
	record_call = artech_engine.db.get_single_value("CRM Exotel Settings", "record_call")
	...
	try:
		...
		if exc := response.json().get("RestException"):
			artech_engine.throw(exc.get("Message"), title=_("Exotel Exception"))
	else:
		res = response.json()
		...
		create_call_log(
			...
			call_type="Outgoing",
			agent=artech_engine.session.user,
		)
...
def get_status_updater_url():
	from artech_engine.utils.data import get_url

	webhook_verify_token = artech_engine.db.get_single_value("CRM Exotel Settings", "webhook_verify_token")
	return get_url(f"api/method/artech_crm.integrations.exotel.handler.handle_request?key={webhook_verify_token}")


def get_exotel_settings():
	return artech_engine.get_single("CRM Exotel Settings")
...
def validate_request():
	...
	webhook_verify_token = artech_engine.db.get_single_value("CRM Exotel Settings", "webhook_verify_token")
	key = artech_engine.request.args.get("key")
	is_valid = key and key == webhook_verify_token

	if not is_valid:
		artech_engine.throw(_("Unauthorized request"), exc=artech_engine.PermissionError)


@artech_engine.whitelist()
def is_integration_enabled():
	return artech_engine.db.get_single_value("CRM Exotel Settings", "enabled", True)
...
def create_call_log(
	...
	call_log = artech_engine.new_doc("CRM Call Log")
	call_log.id = call_id
...
def get_call_log(call_payload):
	...
	if artech_engine.db.exists("CRM Call Log", call_log_id):
		return artech_engine.get_doc("CRM Call Log", call_log_id)
