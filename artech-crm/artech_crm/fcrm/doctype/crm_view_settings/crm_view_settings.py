...
from artech_engine.model.document import Document, get_controller
from artech_engine.utils import parse_json
...
def create(view: dict):
	...
	doc = artech_engine.new_doc("CRM View Settings")
	doc.name = view.label
	doc.label = view.label
	doc.type = view.type or "list"
	doc.icon = view.icon
	doc.dt = view.doctype
	doc.user = artech_engine.session.user
...
def update(view: dict):
	...
	doc = artech_engine.get_doc("CRM View Settings", view.name)
...
def delete(name: str | int):
	if artech_engine.db.exists("CRM View Settings", name):
		doc = artech_engine.get_doc("CRM View Settings", name)
		check_permission(doc)
		artech_engine.delete_doc("CRM View Settings", name)


@artech_engine.whitelist()
def public(name: str | int, value: bool | int):
	if artech_engine.session.user != "Administrator" and "Sales Manager" not in artech_engine.get_roles():
	...
	doc.user = "" if value else artech_engine.session.user
	doc.save()
...
def check_permission(doc):
	...
	if artech_engine.session.user == "Administrator" or "System Manager" in artech_engine.get_roles():
		pass
	elif doc.public and "Sales Manager" in artech_engine.get_roles():
		pass
	elif doc.user and doc.user == artech_engine.session.user:
		pass
	else:
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)
...
def sync_default_columns(view):
	...
	if view.type == "kanban" and view.column_field:
		field_meta = artech_engine.get_meta(doctype).get_field(view.column_field)
		if field_meta.fieldtype == "Link":
			columns = artech_engine.get_all(
				field_meta.options,
				fields=["name"],
...
def set_as_default(name: str | int | None = None, type: str | None = None, doctype: str | None = None):
	if name:
		artech_engine.db.set_value("CRM View Settings", name, "is_default", 1)
	else:
	...
	artech_engine.db.set_value(
		"CRM View Settings",
		{"name": ("!=", name), "user": artech_engine.session.user, "is_default": 1},
		"is_default",
		0,
	)
...
def create_or_update_standard_view(view: dict):
	...
	doc = artech_engine.db.exists(
		"CRM View Settings",
		{"dt": view.doctype, "type": view.type or "list", "is_standard": True, "user": artech_engine.session.user},
	)
	if doc:
		doc = artech_engine.get_doc("CRM View Settings", doc)
		doc.label = view.label
		doc.type = view.type or "list"
		doc.route_name = view.route_name or get_route_name(view.doctype)
		doc.load_default_columns = view.load_default_columns or False
