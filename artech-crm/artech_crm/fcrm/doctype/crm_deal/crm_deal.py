...
class CRMDeal(Document):
	...
	def validate(self):
		...
		if self.has_value_changed("status"):
			...
			if artech_engine.db.get_value("CRM Deal Status", self.status, "type") == "Won":
				self.closed_date = artech_engine.utils.nowdate()
	...
	def after_insert(self):
		if self.deal_owner:
			if self.deal_owner != artech_engine.session.user:
				self.share_with_agent(self.deal_owner)
			self.assign_agent(self.deal_owner)
	...
	def validate_status(self):
		if self.is_new() and not self.status:
			if artech_engine.db.exists("CRM Deal Status", "Qualification"):
				self.status = "Qualification"
			else:
				self.status = artech_engine.get_all("CRM Deal Status", {"type": "Open"}, pluck="name")[0]
	...
	def set_primary_email_mobile_no(self):
		...
		if len([contact for contact in self.contacts if contact.is_primary]) > 1:
			artech_engine.throw(_("Only one {0} can be set as primary.").format(artech_engine.bold("Contact")))
	...
	def share_with_agent(self, agent):
		...
		docshares = artech_engine.get_all(
			"DocShare",
		...
		for user in shared_with:
			if user == agent and not artech_engine.db.exists(
				"DocShare",
	...
	def apply_sla(self):
		...
		if not self.sla:
			return
		sla = artech_engine.get_last_doc("CRM Service Level Agreement", {"name": self.sla})
		if sla:
			sla.apply(self)
	...
	def update_default_probability(self):
		...
		if not self.probability or self.probability == 0:
			self.probability = artech_engine.db.get_value("CRM Deal Status", self.status, "probability") or 0
	...
	def update_expected_deal_value(self):
		...
		"""
		if (
			artech_engine.db.get_single_value("FCRM Settings", "auto_update_expected_deal_value")
	...
	def validate_forecasting_fields(self):
		...
		if artech_engine.db.get_single_value("FCRM Settings", "enable_forecasting"):
			if not self.expected_deal_value or self.expected_deal_value == 0:
				artech_engine.throw(_("Expected deal value is required."), artech_engine.MandatoryError)
			if not self.expected_closure_date:
				artech_engine.throw(_("Expected closure date is required."), artech_engine.MandatoryError)

	def validate_lost_reason(self):
		"""
		Validate the lost reason if the status is set to "Lost".
		"""
		if self.status and artech_engine.get_cached_value("CRM Deal Status", self.status, "type") == "Lost":
			if not self.lost_reason:
				artech_engine.throw(_("Please specify a reason for losing the deal."), artech_engine.ValidationError)
			elif self.lost_reason == "Other" and not self.lost_notes:
				artech_engine.throw(_("Please specify the reason for losing the deal."), artech_engine.ValidationError)
		if self.has_value_changed("status"):
			add_or_remove_lost_reason_section_in_sidepanel(self)

	def update_exchange_rate(self):
		if self.has_value_changed("currency") or not self.exchange_rate:
			system_currency = artech_engine.db.get_single_value("FCRM Settings", "currency") or "USD"
			exchange_rate = 1
...
def add_contact(deal: str, contact: str):
	if not artech_engine.has_permission("CRM Deal", "write", deal):
		artech_engine.throw(_("Not allowed to add contact to Deal"), artech_engine.PermissionError)

	deal = artech_engine.get_cached_doc("CRM Deal", deal)
...
def remove_contact(deal: str, contact: str):
	if not artech_engine.has_permission("CRM Deal", "write", deal):
		artech_engine.throw(_("Not allowed to remove contact from Deal"), artech_engine.PermissionError)
...
def set_primary_contact(deal: str, contact: str):
	if not artech_engine.has_permission("CRM Deal", "write", deal):
		artech_engine.throw(_("Not allowed to set primary contact for Deal"), artech_engine.PermissionError)
...
def create_organization(doc):
	...
	existing_organization = artech_engine.db.exists(
		"CRM Organization", {"organization_name": doc.get("organization_name")}
	)
	if existing_organization:
		return existing_organization

	organization = artech_engine.new_doc("CRM Organization")
	organization.update(
		{
...
def contact_exists(doc):
	email_exist = artech_engine.db.exists("Contact Email", {"email_id": doc.get("email")})
	mobile_exist = artech_engine.db.exists("Contact Phone", {"phone": doc.get("mobile_no")})
...
def create_deal(doc: dict):
	deal = artech_engine.new_doc("CRM Deal")
