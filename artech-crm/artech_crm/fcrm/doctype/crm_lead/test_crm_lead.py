...
class TestCRMLead(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		if not artech_engine.db.exists("Salutation", "Mr"):
			artech_engine.get_doc({"doctype": "Salutation", "salutation": "Mr"}).insert(ignore_permissions=True)
	...
	def test_set_lead_name_scenarios(self):
		...
		lead1 = artech_engine.get_doc({"doctype": "CRM Lead", "organization": "Tech Corp"})
		...
		lead2 = artech_engine.get_doc({"doctype": "CRM Lead", "email": "contact@company.com"})
		...
		lead3 = artech_engine.get_doc({"doctype": "CRM Lead"})
	...
	def test_update_lead_owner(self):
		...
		initial_docshares = artech_engine.get_all(
			"DocShare",
		...
		after_docshares = artech_engine.get_all(
			"DocShare",
			filters={"share_name": lead.name, "share_doctype": "CRM Lead"},
		)
		...
		old_docshare = artech_engine.db.exists(
			"DocShare",
			{"user": "Administrator", "share_name": lead.name, "share_doctype": "CRM Lead"},
		)
	...
	def test_create_contact_from_lead(self):
		...
		contact = artech_engine.get_doc("Contact", contact_name)
	...
	def test_create_organization_from_lead(self):
		...
		org = artech_engine.get_doc("CRM Organization", org_name)
	...
	def test_convert_lead_with_existing_contact_and_org(self):
		...
		existing_contact = artech_engine.get_doc(
			{
				"doctype": "Contact",
				"first_name": "Existing",
				"last_name": "Contact",
		...
		existing_org = artech_engine.get_doc(
			{
