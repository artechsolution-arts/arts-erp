...
from artech_engine.utils import has_gravatar, validate_email_address

from artech_crm.fcrm.doctype.crm_service_level_agreement.utils import get_sla
from artech_crm.fcrm.doctype.crm_status_change_log.crm_status_change_log import (
...
class CRMLead(Document):
	...
	def after_insert(self):
		if self.lead_owner:
			if self.lead_owner != artech_engine.session.user:
				self.share_with_agent(self.lead_owner)
			self.assign_agent(self.lead_owner)
	...
	def validate_status(self):
		if self.is_new() and not self.status:
			if artech_engine.db.exists("CRM Lead Status", "New"):
				self.status = "New"
			else:
				self.status = artech_engine.get_all("CRM Lead Status", {"type": "Open"}, pluck="name")[0]
	...
	def validate_lost_reason(self):
		...
		"""
		if self.status and artech_engine.get_cached_value("CRM Lead Status", self.status, "type") == "Lost":
			if not self.lost_reason:
				artech_engine.throw(_("Please specify a reason for losing the lead."), artech_engine.ValidationError)
			elif self.lost_reason == "Other" and not self.lost_notes:
				artech_engine.throw(_("Please specify the reason for losing the lead."), artech_engine.ValidationError)
		if self.has_value_changed("status"):
	...
	def create_organization(self, existing_organization=None):
		...
		existing_organization = existing_organization or artech_engine.db.exists(
	...
	def update_lead_contact(self, contact):
		contact = artech_engine.get_cached_doc("Contact", contact)
	...
	def contact_exists(self, throw=True):
		email_exist = artech_engine.db.exists("Contact Email", {"email_id": self.email})
		phone_exist = artech_engine.db.exists("Contact Phone", {"phone": self.phone})
		mobile_exist = artech_engine.db.exists("Contact Phone", {"phone": self.mobile_no})
		...
		if name:
			...
			contact = artech_engine.db.get_value(doctype, name, "parent")
	...
	def create_deal(self, contact, organization, deal=None):
		new_deal = artech_engine.new_doc("CRM Deal")
...
def convert_to_deal(
	...
	if not (doc and doc.flags.get("ignore_permissions")) and not artech_engine.has_permission(
		"CRM Lead", "write", lead
	):
		artech_engine.throw(_("Not allowed to convert Lead to Deal"), artech_engine.PermissionError)

	lead = artech_engine.get_cached_doc("CRM Lead", lead)
	if artech_engine.db.exists("CRM Lead Status", "Qualified"):
		lead.db_set("status", "Qualified")
	lead.db_set("converted", 1)
	if lead.sla and artech_engine.db.exists("CRM Communication Status", "Replied"):
