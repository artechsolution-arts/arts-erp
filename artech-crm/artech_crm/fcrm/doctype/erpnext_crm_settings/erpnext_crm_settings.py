...
from artech_engine.custom.doctype.custom_field.custom_field import create_custom_fields as _create_custom_fields
from artech_engine.custom.doctype.property_setter.property_setter import make_property_setter
from artech_engine.frappeclient import FrappeClient
from artech_engine.model.document import Document
from artech_engine.utils import get_url_to_form, get_url_to_list


def _is_erpnext_installed():
	return "artech" in artech_engine.get_installed_apps()


def _log_and_throw(message: str, title: str | None = None):
	artech_engine.log_error(artech_engine.get_traceback(), title or message)
	artech_engine.throw(_(message))


def _get_enabled_settings():
	settings = artech_engine.get_single("ERPNext CRM Settings")
	if not settings.enabled:
		artech_engine.throw(_("ERPNext is not integrated with the CRM"))
	return settings
...
class ERPNextCRMSettings(Document):
	...
	def add_quotation_to_option(self):
		if not self.is_erpnext_in_different_site:
			if not artech_engine.db.exists("Property Setter", {"name": "Quotation-quotation_to-link_filters"}):
	...
	def create_custom_fields_in_remote_site(self):
		...
		try:
			...
			_log_and_throw(
				...
				f"Error while creating custom field in the remote artech site: {self.erpnext_site_url}",
			)

	def create_crm_form_script(self):
		if not artech_engine.db.exists("CRM Form Script", "Create Quotation from CRM Deal"):
...
def get_customer_link(crm_deal: str):
	...
	if not erpnext_crm_settings.is_erpnext_in_different_site:
		customer = artech_engine.db.exists("Customer", {"crm_deal": crm_deal})
		if not customer:
			customer = artech_engine.db.get_value("CRM Deal", crm_deal, "erpnext_customer")
...
def create_prospect_in_remote_site(crm_deal, erpnext_crm_settings):
	try:
		client = get_erpnext_site_client(erpnext_crm_settings)
		doc = artech_engine.get_cached_doc("CRM Deal", crm_deal)
...
def get_organization_address(organization: str | None = None):
	if not organization:
		return None
	address = artech_engine.db.get_value("CRM Organization", organization, "address")
	address = artech_engine.get_cached_doc("Address", address) if address else None
	if not address:
		return None
	return {
		"name": address.name,
...
def create_customer_in_erpnext(doc, method):
	erpnext_crm_settings = artech_engine.get_single("ERPNext CRM Settings")
	if (
	...
	if not doc.organization:
		artech_engine.throw(_("Organization is required to create a customer"))
	...
	if customer_name:
		artech_engine.db.set_value("CRM Deal", doc.name, "erpnext_customer", customer_name)
		artech_engine.publish_realtime("crm_customer_created")
...
def get_crm_form_script():
	...
	onLoad() {
		...
		call(
			"artech_engine.client.get_single_value",
			{
