...
def get_fields_layout(doctype: str, type: str, parent_doctype: str | None = None):
	...
	if artech_engine.db.exists("CRM Fields Layout", {"dt": doctype, "type": type}):
		layout = artech_engine.get_doc("CRM Fields Layout", {"dt": doctype, "type": type})
	...
	if type == "Required Fields":
		required_fields = [
			field for field in artech_engine.get_meta(doctype, False).fields if field.reqd and not field.default
		]
...
def get_sidepanel_sections(doctype: str):
	if not artech_engine.db.exists("CRM Fields Layout", {"dt": doctype, "type": "Side Panel"}):
		return []
	layout = artech_engine.get_doc("CRM Fields Layout", {"dt": doctype, "type": "Side Panel"}).layout
...
def get_permlevel_access(permission_type="write", doctype=None, parent_doctype=None):
	...
	if meta.istable and parent_doctype:
		meta = artech_engine.get_meta(parent_doctype)
...
def save_fields_layout(doctype: str, type: str, layout: str):
	if not artech_engine.has_permission("CRM Fields Layout", "write"):
		artech_engine.throw(_("Not permitted to modify fields layout"), artech_engine.PermissionError)

	if artech_engine.db.exists("CRM Fields Layout", {"dt": doctype, "type": type}):
		doc = artech_engine.get_doc("CRM Fields Layout", {"dt": doctype, "type": type})
	else:
