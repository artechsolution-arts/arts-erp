...
class FCRMSettings(Document):
	...
	def do_not_allow_to_delete_if_standard(self):
		...
		if deleted_standard_items:
			...
			if not deleted_standard_items.intersection(standard_dropdown_items):
				return
			artech_engine.throw(_("Cannot delete standard items {0}").format(", ".join(deleted_standard_items)))
	...
	def add_forecasting_section_in_sidepanel(self):
		doc = artech_engine.get_doc("CRM Fields Layout", "CRM Deal-Side Panel")
		layout = doc.layout
...
def get_standard_dropdown_items():
	return [item.get("name1") for item in artech_engine.get_hooks("standard_dropdown_items")]
...
def sync_table(key, hook):
	...
	for item in artech_engine.get_hooks(hook):
...
def create_forecasting_script():
	if not artech_engine.db.exists("CRM Form Script", "Forecasting Script"):
...
def get_forecasting_script():
    ...
    async updateProbability() {
        let status = await call("artech_engine.client.get_value", {
