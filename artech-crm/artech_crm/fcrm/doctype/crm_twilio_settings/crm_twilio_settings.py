...
class CRMTwilioSettings(Document):
	...
	def validate(self):
		old_account_sid = artech_engine.db.get_single_value("CRM Twilio Settings", "account_sid")
	...
	def validate_twilio_account(self):
		try:
			...
			artech_engine.throw(_("Invalid Account SID or Auth Token."))
	...
	def create_api_key(self, twilio):
		...
		try:
			...
			artech_engine.log_error(title=_("Twilio API credential creation error."))
			artech_engine.throw(_("Twilio API credential creation error."))

	def get_twilio_voice_url(self):
		url_path = "/api/method/artech_crm.integrations.twilio.api.voice"
	...
	def fetch_applications(self):
		...
		if not twilio:
			artech_engine.throw(_("Unable to connect to Twilio account to fetch applications."))
			return
