...
def parse_call_log(call):
	...
	if call.get("type") == "Incoming":
		...
		receiver = (
			artech_engine.db.get_values("User", call.get("receiver"), ["full_name", "user_image"])[0]
		...
		caller = (
			artech_engine.db.get_values("User", call.get("caller"), ["full_name", "user_image"])[0]
			if call.get("caller")
			else [None, None]
		)
		call["_caller"] = {
...
def get_call_log(name: str):
	call = artech_engine.get_cached_doc(
		"CRM Call Log",
		name,
		fields=[
			"name",
	...
	if call.get("note"):
		note = artech_engine.get_cached_doc("FCRM Note", call.get("note")).as_dict()
		notes.append(note)
	...
	if call.get("links"):
		for link in call.get("links"):
			if link.get("link_doctype") == "CRM Task":
				task = artech_engine.get_cached_doc("CRM Task", link.get("link_name")).as_dict()
				tasks.append(task)
			elif link.get("link_doctype") == "FCRM Note":
				note = artech_engine.get_cached_doc("FCRM Note", link.get("link_name")).as_dict()
				notes.append(note)
...
def create_lead_from_call_log(call_log: str | dict, lead_details: str | dict | None = None):
	call_log_data = artech_engine.parse_json(call_log or {})
	...
	if not call_log_name:
		artech_engine.throw(_("A valid call log is required."), artech_engine.ValidationError)

	call_doc = artech_engine.get_doc("CRM Call Log", call_log_name)

	if not call_doc.has_permission("write"):
		artech_engine.throw(_("You are not permitted to update this call log."), artech_engine.PermissionError)

	if not artech_engine.has_permission("CRM Lead", "create"):
		artech_engine.throw(_("You are not permitted to create leads."), artech_engine.PermissionError)

	lead_details_data = artech_engine.parse_json(lead_details or {})
	if lead_details_data and not isinstance(lead_details_data, dict):
		artech_engine.throw(_("Invalid lead details supplied."), artech_engine.ValidationError)

	lead = artech_engine.new_doc("CRM Lead")
	meta = artech_engine.get_meta("CRM Lead")
	...
	if "lead_owner" in valid_fieldnames and not sanitized_details.get("lead_owner"):
		sanitized_details["lead_owner"] = artech_engine.session.user
