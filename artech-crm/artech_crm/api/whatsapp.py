...
def validate_access(reference_doctype=None, reference_name=None, permtype="read"):
	if not any(role in ALLOWED_WHATSAPP_ROLES for role in artech_engine.get_roles()):
		artech_engine.throw(_("Only sales users can access WhatsApp features."), artech_engine.PermissionError)

	if reference_doctype and reference_name:
		if not artech_engine.db.exists(reference_doctype, reference_name):
			artech_engine.throw(
				_("Reference document {0} {1} does not exist.").format(reference_doctype, reference_name),
				artech_engine.DoesNotExistError,
			)
		reference_doc = artech_engine.get_doc(reference_doctype, reference_name)
...
def validate(doc, method):
	...
	if phone_number:
		try:
			...
			artech_engine.log_error(artech_engine.get_traceback(), "CRM WhatsApp: failed to resolve contact from number")
...
def notify_agent(doc):
	if doc.type == "Incoming":
		...
		safe_reference_name = artech_engine.utils.escape_html(doc.reference_name)
		notification_text = f"""
...
def is_whatsapp_enabled():
	if not artech_engine.db.exists("DocType", "WhatsApp Settings"):
		return False
	default_outgoing = artech_engine.get_cached_value(
		"WhatsApp Settings", "WhatsApp Settings", "default_outgoing_account"
	)
	if not default_outgoing:
		return False
	status = artech_engine.get_cached_value("WhatsApp Account", default_outgoing, "status")
...
def get_whatsapp_messages(reference_doctype: str, reference_name: str):
	...
	if reference_doctype == "CRM Deal":
		...
		if lead:
			...
			messages = artech_engine.get_all(
				"WhatsApp Message",
				filters={
	...
	for template_message in template_messages:
		...
		if not artech_engine.db.exists("WhatsApp Templates", template_message["template"]):
			continue
		template = artech_engine.get_doc("WhatsApp Templates", template_message["template"])
...
def create_whatsapp_message(
	...
	doc = artech_engine.new_doc("WhatsApp Message")

	if reply_to:
		if not artech_engine.db.exists("WhatsApp Message", reply_to):
			artech_engine.throw(_("Referenced WhatsApp message does not exist."), artech_engine.DoesNotExistError)
		reply_doc = artech_engine.get_doc("WhatsApp Message", reply_to)
		if not reply_doc.has_permission("read"):
			artech_engine.throw(
				_("Not permitted to access the referenced WhatsApp message."), artech_engine.PermissionError
			)
...
def react_on_whatsapp_message(emoji: str, reply_to_name: str):
	validate_access()
	if not artech_engine.db.exists("WhatsApp Message", reply_to_name):
		artech_engine.throw(_("Referenced WhatsApp message does not exist."), artech_engine.DoesNotExistError)
	reply_to_doc = artech_engine.get_doc("WhatsApp Message", reply_to_name)

	if not reply_to_doc.has_permission("read"):
		artech_engine.throw(_("Not permitted to access the referenced WhatsApp message."), artech_engine.PermissionError)
...
def get_from_name(message):
	doc = artech_engine.get_doc(message["reference_doctype"], message["reference_name"])
	from_name = ""
...
def add_roles():
	if "frappe_whatsapp" not in artech_engine.get_installed_apps():
		return
	...
	for doctype in doctypes:
		for role in role_list:
			if artech_engine.db.exists("Custom DocPerm", {"parent": doctype, "role": role}):
				continue
