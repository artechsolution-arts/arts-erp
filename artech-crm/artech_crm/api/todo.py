...
def after_insert(doc, method):
	if doc.reference_type in ["CRM Lead", "CRM Deal"] and doc.reference_name and doc.allocated_to:
		...
		owner = artech_engine.db.get_value(doc.reference_type, doc.reference_name, fieldname)
		if not owner:
...
def notify_assigned_user(doc, is_cancelled=False):
	_doc = artech_engine.get_doc(doc.reference_type, doc.reference_name)
	owner = artech_engine.get_cached_value("User", artech_engine.session.user, "full_name")
...
def get_redirect_to_doc(doc):
	if doc.reference_type == "CRM Task":
		reference_doc = artech_engine.get_doc(doc.reference_type, doc.reference_name)
