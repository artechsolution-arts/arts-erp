...
from artech_engine.desk.form.assign_to import set_status
from artech_engine.model import no_value_fields
from artech_engine.model.delete_doc import get_dynamic_linked_docs, get_linked_docs
from artech_engine.model.document import get_controller
...
def get_filterable_fields(doctype: str):
	...
	meta = artech_engine.get_meta(doctype).as_dict()

	# append standard fields (getting error when using artech_engine.model.std_fields)
	standard_fields = [
...
def get_quick_filters(doctype: str, cached: bool = True):
	meta = artech_engine.get_meta(doctype, cached)
	quick_filters = []

	if global_settings := artech_engine.db.exists("CRM Global Settings", {"dt": doctype, "type": "Quick Filters"}):
		_quick_filters = artech_engine.db.get_value("CRM Global Settings", global_settings, "json")
...
def create_update_global_settings(doctype, quick_filters):
	if global_settings := artech_engine.db.exists("CRM Global Settings", {"dt": doctype, "type": "Quick Filters"}):
		artech_engine.db.set_value("CRM Global Settings", global_settings, "json", json.dumps(quick_filters))
	else:
		# create CRM Global Settings doc
		doc = artech_engine.new_doc("CRM Global Settings")
		doc.dt = doctype
...
def update_in_standard_filter(fieldname, doctype, value):
	if property_name := artech_engine.db.exists(
		"Property Setter",
		{"doc_type": doctype, "field_name": fieldname, "property": "in_standard_filter"},
	):
		artech_engine.db.set_value("Property Setter", property_name, "value", value)
	else:
...
def get_data(
	...
	rows = artech_engine.parse_json(rows or "[]")
	columns = artech_engine.parse_json(columns or "[]")
	kanban_fields = artech_engine.parse_json(kanban_fields or "[]")
	kanban_columns = artech_engine.parse_json(kanban_columns or "[]")
	...
	for key in filters:
		...
		if isinstance(value, list):
			if "@me" in value:
				value[value.index("@me")] = artech_engine.session.user
			elif "%@me%" in value:
				index = [i for i, v in enumerate(value) if v == "%@me%"]
				for i in index:
					value[i] = "%" + artech_engine.session.user + "%"
		elif value == "@me":
			filters[key] = artech_engine.session.user

	if default_filters:
		default_filters = artech_engine.parse_json(default_filters)
	...
	if view_type != "kanban":
		if columns or rows:
			custom_view = True
			is_default = False
			columns = artech_engine.parse_json(columns)
			rows = artech_engine.parse_json(rows)
		...
		if not custom_view and artech_engine.db.exists("CRM View Settings", default_view_filters):
			list_view_settings = artech_engine.get_doc("CRM View Settings", default_view_filters)
			columns = artech_engine.parse_json(list_view_settings.columns)
			rows = artech_engine.parse_json(list_view_settings.rows)
			is_default = False
	...
	if view_type == "kanban":
		...
		if not kanban_columns and column_field:
			field_meta = artech_engine.get_meta(doctype).get_field(column_field)
			if field_meta.fieldtype == "Link":
				kanban_columns = artech_engine.get_all(
					field_meta.options,
					fields=["name"],
		...
		for kc in kanban_columns:
			...
			if kc.get("delete"):
				...
				if order:
					...
					column_data = artech_engine.get_list(
						doctype,
						fields=rows,
				...
				all_count = artech_engine.get_list(
					doctype,
					filters=column_filters,
					fields=[COUNT_NAME],
				)[0].total_count
	...
	if not is_default and custom_view_name:
		is_default = artech_engine.db.get_value("CRM View Settings", custom_view_name, "load_default_columns")
	...
	return {
		...
		"total_count": artech_engine.get_list(doctype, filters=filters, fields=[COUNT_NAME])[0].total_count,
		"row_count": len(data),
...
def get_records_based_on_order(doctype, rows, filters, page_length, order):
	...
	records = artech_engine.get_list(
		doctype,
		fields=rows,
		filters=in_filters,
	...
	if len(records) < page_length:
		...
		remaining_records = artech_engine.get_list(
			doctype,
			fields=rows,
...
def remove_assignments(doctype: str, name: str, assignees: str | list, ignore_permissions: bool = False):
	assignees = artech_engine.parse_json(assignees)
...
def get_assigned_users(doctype: str, name: str | int, default_assigned_to: str | None = None):
	assigned_users = artech_engine.get_all(
		"ToDo",
		fields=["allocated_to"],
		filters={
...
def get_fields(doctype: str, allow_all_fieldtypes: bool = False):
	not_allowed_fieldtypes = [*list(artech_engine.model.no_value_fields), "Read Only"]
...
def getCounts(d, doctype):
	...
	d["_email_count"] = d["_email_count"] + artech_engine.db.count(
		"Communication",
		filters={
			"reference_doctype": doctype,
			"reference_name": d.get("name"),
...
def get_linked_docs_of_document(doctype: str, docname: str):
	...
	for doc in linked_docs:
		...
		try:
			data = artech_engine.get_doc(doc["reference_doctype"], doc["reference_docname"])
		except (artech_engine.DoesNotExistError, artech_engine.ValidationError):
			continue
...
def remove_doc_link(doctype, docname):
	...
	try:
		linked_doc_data = artech_engine.get_doc(doctype, docname)
...
def remove_linked_doc_reference(items: str | list, remove_contact: bool = False, delete: bool = False):
	if isinstance(items, str):
		items = artech_engine.parse_json(items)

	for item in items:
		if not item.get("doctype") or not item.get("docname"):
			continue

		if not artech_engine.has_permission(item["doctype"], "write", item["docname"]):
			continue
		...
		try:
			...
			if delete:
				artech_engine.delete_doc(item["doctype"], item["docname"])
		except (artech_engine.DoesNotExistError, artech_engine.ValidationError):
...
def delete_bulk_docs(doctype: str, items: str | list, delete_linked: bool = False):
	from artech_engine.desk.reportview import delete_bulk

	if not doctype:
		artech_engine.throw(_("Doctype is required"))

	if not items:
		artech_engine.throw(_("Items are required"))

	items = artech_engine.parse_json(items)
	if not isinstance(items, list):
		artech_engine.throw(_("Items must be a list"))

	for doc in items:
		try:
			if not artech_engine.db.exists(doctype, doc):
				artech_engine.log_error(f"Document {doctype} {doc} does not exist", "Bulk Delete Error")
				continue
			...
			artech_engine.log_error(f"Error processing linked docs for {doctype} {doc}: {e!s}", "Bulk Delete Error")

	if len(items) > 10:
		artech_engine.enqueue("artech_engine.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
