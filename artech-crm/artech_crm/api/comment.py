...
def notify_mentions(doc):
	...
	reference_doc = artech_engine.get_doc(doc.reference_doctype, doc.reference_name)
	for mention in mentions:
		owner = artech_engine.get_cached_value("User", doc.owner, "full_name")
...
def extract_mentions(html):
	...
	for d in soup.find_all("span", attrs={"data-type": "mention"}):
		mentions.append(artech_engine._dict(full_name=d.get("data-label"), email=d.get("data-id")))
	return mentions
...
def add_comment(reference_doctype: str, reference_name: str, content: str, attachments: list | None = None):
	...
	comment = frappe_add_comment(
		...
		comment_by=get_fullname(artech_engine.session.user),
	)
...
def add_attachments(name: str, attachments: Iterable[str | dict]) -> None:
	...
	for a in attachments:
		if isinstance(a, str):
			attach = artech_engine.db.get_value("File", {"name": a}, ["file_url", "is_private"], as_dict=1)
			file_args = {
				"file_url": attach.file_url,
				"is_private": attach.is_private,
			}
		elif isinstance(a, dict) and "fcontent" in a and "fname" in a:
			# dict returned by artech_engine.attach_print()
			file_args = {
				"file_name": a["fname"],
				"content": a["fcontent"],
				"is_private": 1,
		...
		_file = artech_engine.new_doc("File")
