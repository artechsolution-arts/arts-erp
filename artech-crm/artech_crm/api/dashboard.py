...
from artech_engine.query_builder import Case, DocType
...
def reset_to_default():
	artech_engine.only_for("System Manager", True)
...
def get_dashboard(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	if not from_date or not to_date:
		from_date = artech_engine.utils.get_first_day(from_date or artech_engine.utils.nowdate())
		to_date = artech_engine.utils.get_last_day(to_date or artech_engine.utils.nowdate())

	roles = artech_engine.get_roles(artech_engine.session.user)
	...
	dashboard = artech_engine.db.exists("CRM Dashboard", "Manager Dashboard")
	...
	if not dashboard:
		...
		layout = json.loads(artech_engine.db.get_value("CRM Dashboard", "Manager Dashboard", "layout") or "[]")
...
def get_total_leads(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	"""
	diff = artech_engine.utils.date_diff(to_date, from_date)
	if diff == 0:
		diff = 1

	prev_from_date = artech_engine.utils.add_days(from_date, -diff)
	to_date_plus_one = artech_engine.utils.add_days(to_date, 1)
	...
	query = artech_engine.qb.from_(Lead).select(
...
def get_average_time_to_close_a_lead(
	...
	time_diff = TimestampDiff(
		artech_engine.qb.terms.LiteralValue("DAY"), Coalesce(Lead.creation, Deal.creation), Deal.closed_date
	)
...
def get_average_time_to_close_a_deal(
	...
	time_diff = TimestampDiff(artech_engine.qb.terms.LiteralValue("DAY"), Deal.creation, Deal.closed_date)
...
def get_sales_trend(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	# Build leads query
	leads_query = (
		artech_engine.qb.from_(Lead)
		.select(
			Date(Lead.creation).as_("date"),
			Count("*").as_("leads"),
			artech_engine.qb.terms.ValueWrapper(0).as_("deals"),
			artech_engine.qb.terms.ValueWrapper(0).as_("won_deals"),
		)
	...
	deals_query = (
		...
		.select(
			...
			artech_engine.qb.terms.ValueWrapper(0).as_("leads"),
			Count("*").as_("deals"),
	...
	daily = (
		artech_engine.qb.from_(union_query)
		.select(
	...
	sales_trend = [
		{
			"date": artech_engine.utils.get_datetime(row.date).strftime("%Y-%m-%d"),
...
def get_forecasted_revenue(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	twelve_months_ago = artech_engine.utils.add_months(artech_engine.utils.nowdate(), -12)
	...
	query = (
		artech_engine.qb.from_(CRMDeal)
		.join(CRMDealStatus)
	...
	for row in result:
		row["month"] = artech_engine.utils.get_datetime(row["month"]).strftime("%Y-%m-01")
...
def get_funnel_conversion(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	query = (
		artech_engine.qb.from_(CRMLead)
...
def get_deals_by_stage_axis(
	...
	query = (
		...
		.orderby(Count("*"), order=artech_engine.qb.desc)
	)
...
def get_deals_by_territory(from_date: str | None = None, to_date: str | None = None, user: str | None = None):
	...
	query = (
		...
		.orderby(Count("*"), order=artech_engine.qb.desc)
		.orderby(
			Sum(Coalesce(CRMDeal.deal_value, 0) * IfNull(CRMDeal.exchange_rate, 1)), order=artech_engine.qb.desc
		)
	)
...
def get_base_currency_symbol():
	...
	"""
	base_currency = artech_engine.db.get_single_value("FCRM Settings", "currency") or "USD"
	return artech_engine.db.get_value("Currency", base_currency, "symbol") or ""
...
def get_deal_status_change_counts(
	...
	query = (
		artech_engine.qb.from_(CRMStatusChangeLog)
		.join(CRMDeal)
