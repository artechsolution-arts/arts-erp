...
from artech_engine.desk.form.load import get_docinfo
...
def get_activities(name: str):
	if artech_engine.db.exists("CRM Deal", name):
		return get_deal_activities(name)
	elif artech_engine.db.exists("CRM Lead", name):
		return get_lead_activities(name)
	else:
		artech_engine.throw(_("Document not found"), artech_engine.DoesNotExistError)


def get_deal_activities(name: str):
	if not artech_engine.has_permission("CRM Deal", "read", name):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	get_docinfo("", "CRM Deal", name)
	docinfo = artech_engine.response["docinfo"]
	deal_meta = artech_engine.get_meta("CRM Deal")
	deal_fields = {
	...
	doc = artech_engine.db.get_values("CRM Deal", name, ["creation", "owner", "lead"])[0]
	lead = doc[2]
...
def get_lead_activities(name: str):
	if not artech_engine.has_permission("CRM Lead", "read", name):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	get_docinfo("", "CRM Lead", name)
	docinfo = artech_engine.response["docinfo"]
	lead_meta = artech_engine.get_meta("CRM Lead")
	lead_fields = {
	...
	doc = artech_engine.db.get_values("CRM Lead", name, ["creation", "owner"])[0]
	activities = [
		{
			"activity_type": "creation",
			"creation": doc[0],
...
def get_linked_calls(name: str):
	calls = artech_engine.db.get_all(
		"CRM Call Log",
	...
	linked_calls = artech_engine.db.get_all(
	...
	if linked_calls:
		CallLog = artech_engine.qb.DocType("CRM Call Log")
		Link = artech_engine.qb.DocType("Dynamic Link")
		query = (
			artech_engine.qb.from_(CallLog)
			.select(
				CallLog.name,
				CallLog.caller,
				CallLog.receiver,
	...
	if notes:
		notes = artech_engine.db.get_all(
			"FCRM Note",
	...
	if tasks:
		tasks = artech_engine.db.get_all(
			"CRM Task",
