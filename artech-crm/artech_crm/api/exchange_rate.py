...
def get_exchange_rate(from_currency: str, to_currency: str, date: str | None = None):
	...
	cache_date = artech_engine.utils.today() if date == "latest" else date
	cache_key = f"exchange_rate_{from_currency}_{to_currency}_{cache_date}"

	cached_rate = artech_engine.cache().get_value(cache_key)
	...
	if rate is not None:
		artech_engine.cache().set_value(cache_key, rate)
		return rate
	...
	if not settings.access_key:
		artech_engine.throw(
			_("Access Key is required for Service Provider: {0}").format(
				artech_engine.bold(settings.service_provider)
			)
		)
...
def _raise_exchange_rate_error(from_currency: str, to_currency: str, date: str, api_used: str):
	...
	if api_used == "frankfurter":
		...
		is_manager = (
			"System Manager" in artech_engine.get_roles(user)
			or "Sales Manager" in artech_engine.get_roles(user)
