...
def execute():
	deal_statuses = artech_engine.get_all("CRM Deal Status", fields=["name", "probability", "deal_status"])
	...
	for status in deal_statuses:
		if status.probability is None or status.probability == 0:
			...
			artech_engine.db.set_value("CRM Deal Status", status.name, "probability", probability)
