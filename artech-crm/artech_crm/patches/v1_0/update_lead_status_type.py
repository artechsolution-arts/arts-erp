...
def execute():
	if not artech_engine.db.exists("CRM Lead Status", "Converted"):
		status_doc = artech_engine.get_doc(
			{
	...
	lead_statuses = artech_engine.get_all("CRM Lead Status", fields=["name", "type", "lead_status"])
	...
	for status in lead_statuses:
		if not status.type or status.type is None or status.type == "Open":
			...
			artech_engine.db.set_value("CRM Lead Status", status.name, "type", type)
