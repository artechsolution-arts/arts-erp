...
def add_default_lead_statuses():
	...
	for status in statuses:
		if artech_engine.db.exists("CRM Lead Status", status):
			continue

		doc = artech_engine.new_doc("CRM Lead Status")
...
def add_default_deal_statuses():
	...
	for status in statuses:
		if artech_engine.db.exists("CRM Deal Status", status):
			continue

		doc = artech_engine.new_doc("CRM Deal Status")
...
def add_default_communication_statuses():
	...
	for status in statuses:
		if artech_engine.db.exists("CRM Communication Status", status):
			continue

		doc = artech_engine.new_doc("CRM Communication Status")
		doc.status = status
		doc.insert()
...
def add_default_fields_layout(force=False):
	...
	for layout in quick_entry_layouts:
		if artech_engine.db.exists("CRM Fields Layout", layout):
			if force:
				artech_engine.delete_doc("CRM Fields Layout", layout)
			else:
				continue

		doc = artech_engine.new_doc("CRM Fields Layout")
...
def add_property_setter():
	if not artech_engine.db.exists("Property Setter", {"name": "Contact-main-search_fields"}):
		doc = artech_engine.new_doc("Property Setter")
...
def add_email_template_custom_fields():
	if not artech_engine.get_meta("Email Template").has_field("enabled"):
...
def add_email_account_custom_field():
	if not artech_engine.get_meta("Email Account").has_field("create_lead_from_incoming_email"):
		...
		artech_engine.clear_cache(doctype="Email Account")
...
def add_default_industries():
	...
	for industry in industries:
		if artech_engine.db.exists("CRM Industry", industry):
			continue

		doc = artech_engine.new_doc("CRM Industry")
...
def add_default_lead_sources():
	...
	for source in lead_sources:
		if artech_engine.db.exists("CRM Lead Source", source):
			continue

		doc = artech_engine.new_doc("CRM Lead Source")
...
def add_default_lost_reasons():
	...
	for reason in lost_reasons:
		if artech_engine.db.exists("CRM Lost Reason", reason["reason"]):
			continue

		doc = artech_engine.new_doc("CRM Lost Reason")
...
def add_default_quick_filters():
	...
	for quick_filter in quick_filters:
		if artech_engine.db.exists("CRM Global Settings", {"dt": quick_filter}):
			continue
...
def add_standard_dropdown_items():
	crm_settings = artech_engine.get_single("FCRM Settings")
	...
	for item in artech_engine.get_hooks("standard_dropdown_items"):
...
def add_assignment_rule_property_setters():
	...
	if not artech_engine.db.exists("Property Setter", {"name": "Assignment Rule-assign_condition-depends_on"}):
	...
	if not artech_engine.db.exists("Property Setter", {"name": "Assignment Rule-unassign_condition-depends_on"}):
		artech_engine.get_doc(
			{
				**default_fields,
...
def create_assignment_rule_custom_fields():
	if not artech_engine.get_meta("Assignment Rule").has_field("assign_condition_json"):
		...
		artech_engine.clear_cache(doctype="Assignment Rule")
