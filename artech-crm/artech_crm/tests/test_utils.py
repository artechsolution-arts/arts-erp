...
class TestUpdateModifiedTimestamp(IntegrationTestCase):
	def setUp(self):
		super().setUp()
		# Patch artech_engine.enqueue to run update_modified_background synchronously in tests
		self._enqueue_patch = patch("artech_engine.enqueue", self._immediate_enqueue)
	...
	def setUpClass(cls):
		...
		artech_engine.db.set_single_value("FCRM Settings", "update_timestamp_on_new_communication", 1)
		artech_engine.db.set_single_value("FCRM Settings", "auto_reopen_on_new_communication", 0)
		artech_engine.db.set_single_value("FCRM Settings", "auto_mark_replied_on_response", 0)
	...
	def test_timestamp_updated_on_new_communication(self):
		...
		original_modified = artech_engine.db.get_value("CRM Lead", lead.name, "modified")
		...
		updated_modified = artech_engine.db.get_value("CRM Lead", lead.name, "modified")
	...
	def test_timestamp_not_updated_when_setting_disabled(self):
		...
		artech_engine.db.set_single_value("FCRM Settings", "update_timestamp_on_new_communication", 0)
	...
	def test_timestamp_not_updated_when_no_reference(self):
		...
		with patch("artech_engine.db.set_value") as mock_set_value:
	...
	def test_timestamp_direct_call_updates_lead(self):
		...
		before = artech_engine.db.get_value("CRM Lead", lead.name, "modified")
		time.sleep(0.1)
		on_communication_update(comm)
		after = artech_engine.db.get_value("CRM Lead", lead.name, "modified")
...
class TestUpdateCommunicationStatus(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		artech_engine.db.set_single_value("FCRM Settings", "auto_reopen_on_new_communication", 1)
		artech_engine.db.set_single_value("FCRM Settings", "auto_mark_replied_on_response", 1)
		artech_engine.db.set_single_value("FCRM Settings", "update_timestamp_on_new_communication", 0)
	...
	def test_status_set_to_open_on_received_communication(self):
		...
		status = artech_engine.db.get_value("CRM Lead", lead.name, "communication_status")
	...
	def test_status_not_updated_when_reopen_setting_disabled(self):
		...
		original_status = artech_engine.db.get_value("CRM Lead", lead.name, "communication_status")
		self._insert_communication(lead, "Received")
	...
	def test_only_last_communication_drives_status(self):
		...
		self._insert_communication(lead, "Received")
		self.assertEqual(artech_engine.db.get_value("CRM Lead", lead.name, "communication_status"), "Open")

		self._insert_communication(lead, "Sent")
		self.assertEqual(artech_engine.db.get_value("CRM Lead", lead.name, "communication_status"), "Replied")
...
class TestCreateLeadFromIncomingEmail(IntegrationTestCase):
	...
	def _make_email_account(self, create_lead=1):
		...
		email_account = artech_engine.get_doc(
			{
	...
	def test_lead_created_from_incoming_email(self):
		...
		self.assertTrue(artech_engine.db.exists("CRM Lead", {"email": "newlead@example.com"}))
	...
	def test_lead_not_created_when_setting_disabled(self):
		...
		self.assertFalse(artech_engine.db.exists("CRM Lead", {"email": "disabled@example.com"}))
	...
	def test_lead_not_created_when_no_email_account(self):
		...
		with patch("artech_engine.new_doc") as mock_new_doc:
	...
	def test_lead_not_created_when_communication_already_referenced(self):
		...
		email_account = self._make_email_account()
		existing = artech_engine.get_doc(
		...
		self.assertFalse(artech_engine.db.exists("CRM Lead", {"email": "referenced@example.com"}))
	...
	def test_lead_not_created_when_lead_already_exists_for_sender(self):
		...
		self.assertEqual(artech_engine.db.count("CRM Lead", {"email": "dupe@example.com"}), 1)
	...
	def test_lead_first_name_from_sender_full_name(self):
		...
		lead = artech_engine.db.get_values(
	...
	def test_lead_first_name_falls_back_to_email_prefix(self):
		...
		lead = artech_engine.db.get_value("CRM Lead", {"email": "prefix@example.com"}, "first_name")
	...
	def test_communication_linked_back_to_created_lead(self):
		...
		lead_name = artech_engine.db.get_value("CRM Lead", {"email": "linked@example.com"}, "name")
	...
	def test_lead_source_set_to_email_when_source_exists(self):
		...
		if not artech_engine.db.exists("CRM Lead Source", "Email"):
			artech_engine.get_doc({"doctype": "CRM Lead Source", "name": "Email"}).insert(ignore_permissions=True)

		email_account = self._make_email_account()
		doc = self._incoming_comm("leadsource@example.com", email_account.name)
		create_lead_from_incoming_email(doc)

		source = artech_engine.db.get_value("CRM Lead", {"email": "leadsource@example.com"}, "source")
	...
	def test_lead_created_for_sent_communication_with_communication_type(self):
		...
		self.assertTrue(artech_engine.db.exists("CRM Lead", {"email": "sentcomm@example.com"}))
