...
class TestDashboard(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		cls.test_start_time = artech_engine.utils.now()
	...
	def test_get_base_currency_symbol(self):
		...
		if not artech_engine.db.exists("FCRM Settings"):
			...
			artech_engine.db.set_single_value("FCRM Settings", "currency", "USD")
	...
	def test_deal_counts_consistency(self):
		...
		lost_deals = artech_engine.db.count(
			"CRM Deal",
			{
				"creation": ["between", [self.from_date, self.to_date]],
				"status": ["in", artech_engine.db.get_list("CRM Deal Status", {"type": "Lost"}, pluck="name")],
			},
		)

		total_deals_by_type = ongoing + won + lost_deals
		total_deals = artech_engine.db.count("CRM Deal", {"creation": ["between", [self.from_date, self.to_date]]})
