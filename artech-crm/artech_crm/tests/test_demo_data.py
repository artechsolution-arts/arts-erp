...
class TestDemoData(IntegrationTestCase):
	...
	def _check_demo_records_exist(self, doctype, record_names):
		...
		for name in record_names:
			if artech_engine.db.exists(doctype, name):
				return True
		return False
	...
	def test_demo_data_lifecycle(self):
		...
		for user in DEMO_USERS:
			self.assertFalse(artech_engine.db.exists("User", user["email"]))

		# Check that demo data defaults are not set
		self.assertIsNone(artech_engine.db.get_default(DEMO_LEADS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_NOTES_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_TASKS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_CALL_LOGS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_DEALS_KEY))
		...
		# Users
		for user in DEMO_USERS:
			doc = artech_engine.get_doc("User", user["email"])
		...
		demo_lead_names = json.loads(artech_engine.db.get_default(DEMO_LEADS_KEY) or "[]")
		self.assertEqual(len(demo_lead_names), 12)
		for lead_name in demo_lead_names:
			lead = artech_engine.get_doc("CRM Lead", lead_name)
		...
		demo_note_names = json.loads(artech_engine.db.get_default(DEMO_NOTES_KEY) or "[]")
		demo_task_names = json.loads(artech_engine.db.get_default(DEMO_TASKS_KEY) or "[]")
		demo_call_log_names = json.loads(artech_engine.db.get_default(DEMO_CALL_LOGS_KEY) or "[]")
		demo_deal_data = json.loads(artech_engine.db.get_default(DEMO_DEALS_KEY) or "{}")
		...
		# Site defaults set
		self.assertEqual(artech_engine.db.get_default(DEMO_STATE_KEY), "1")
		self.assertTrue(artech_engine.db.get_default(DEMO_LEADS_KEY))
		self.assertTrue(artech_engine.db.get_default(DEMO_NOTES_KEY))
		self.assertTrue(artech_engine.db.get_default(DEMO_TASKS_KEY))
		self.assertTrue(artech_engine.db.get_default(DEMO_CALL_LOGS_KEY))
		self.assertTrue(artech_engine.db.get_default(DEMO_ACTIVITIES_KEY))
		self.assertTrue(artech_engine.db.get_default(DEMO_DEALS_KEY))
		...
		self.assertIsNone(artech_engine.db.get_default(DEMO_STATE_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_LEADS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_NOTES_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_TASKS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_CALL_LOGS_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_ACTIVITIES_KEY))
		self.assertIsNone(artech_engine.db.get_default(DEMO_DEALS_KEY))
