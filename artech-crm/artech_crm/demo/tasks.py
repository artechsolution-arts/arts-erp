...
def create_demo_tasks(lead_names, demo_users):
	...
	for data in tasks_data:
		...
		task = artech_engine.get_doc({"doctype": "CRM Task", **data}).insert(ignore_permissions=True)
...
def delete_demo_tasks(task_names):
	for name in task_names:
		if artech_engine.db.exists("CRM Task", name):
			artech_engine.delete_doc("CRM Task", name, ignore_permissions=True, force=True)
