...
def create_demo_users():
	for user_data in DEMO_USERS:
		if not artech_engine.db.exists("User", user_data["email"]):
...
def delete_demo_users(demo_user_emails):
	...
	artech_engine.qb.from_(Notification).delete().where(
	...
	contact_names = artech_engine.get_all("Contact", filters={"user": ["in", demo_user_emails]}, pluck="name")

	for email in demo_user_emails:
		if artech_engine.db.exists("User", email):
			artech_engine.delete_doc("User", email, ignore_permissions=True, force=True)

	if contact_names:
		for child_doctype in ("Contact Email", "Contact Phone", "Dynamic Link"):
			Child = DocType(child_doctype)
			artech_engine.qb.from_(Child).delete().where(Child.parent.isin(contact_names)).run()
		Contact = DocType("Contact")
		artech_engine.qb.from_(Contact).delete().where(Contact.name.isin(contact_names)).run()
