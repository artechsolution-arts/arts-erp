...
def create_demo_deals(lead_names, demo_users):
	...
	for d_name, days, d_owner, li in zip(
		...
		org = artech_engine.db.get_value("CRM Deal", d_name, "organization")
		if org:
			logo = _org_logos.get(org)
			if logo:
				artech_engine.db.set_value("CRM Organization", org, "organization_logo", logo, update_modified=False)
...
def delete_demo_deals(deal_data, lead_names):
	...
	if lead_names:
		demo_lead_deals = artech_engine.get_all("CRM Deal", filters={"lead": ["in", lead_names]}, pluck="name")
		deal_names.update(demo_lead_deals)

	for name in communication_names:
		if artech_engine.db.exists("Communication", name):
			artech_engine.delete_doc("Communication", name, ignore_permissions=True, force=True)

	for name in deal_names:
		if not artech_engine.db.exists("CRM Deal", name):
			continue
		# Collect linked contacts and organization before deleting deal
		contact_names = artech_engine.get_all(
			"CRM Contacts", filters={"parent": name, "parenttype": "CRM Deal"}, pluck="contact"
		)
		org = artech_engine.db.get_value("CRM Deal", name, "organization")
		artech_engine.delete_doc("CRM Deal", name, ignore_permissions=True, force=True)
		for contact in contact_names:
			if contact and artech_engine.db.exists("Contact", contact):
				for child_doctype in ("Contact Email", "Contact Phone", "Dynamic Link"):
					Child = DocType(child_doctype)
					artech_engine.qb.from_(Child).delete().where(Child.parent == contact).run()
				Contact = DocType("Contact")
				artech_engine.qb.from_(Contact).delete().where(Contact.name == contact).run()
		if org and artech_engine.db.exists("CRM Organization", org):
			artech_engine.delete_doc("CRM Organization", org, ignore_permissions=True, force=True)
