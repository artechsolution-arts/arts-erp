...
def create_demo_call_logs(lead_names, demo_users):
	...
	for data in call_logs_data:
		log_owner = data.get("caller") or data.get("receiver") or artech_engine.session.user
		log_ts = data.get("start_time")
		log = artech_engine.get_doc(
			{"doctype": "CRM Call Log", "id": artech_engine.generate_hash(length=10), **data}
		).insert(ignore_permissions=True)
		patch = {"owner": log_owner, "modified_by": log_owner}
		if log_ts:
			patch["creation"] = log_ts
			patch["modified"] = log_ts
		artech_engine.db.set_value("CRM Call Log", log.name, patch, update_modified=False)
...
def delete_demo_call_logs(call_log_names):
	for name in call_log_names:
		if artech_engine.db.exists("CRM Call Log", name):
			artech_engine.delete_doc("CRM Call Log", name, ignore_permissions=True, force=True)
