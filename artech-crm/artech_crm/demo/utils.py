...
def backdate(doctype, name, owner, ts, *, set_creation=True):
	...
	if set_creation:
		fields["creation"] = ts
	artech_engine.db.set_value(doctype, name, fields, update_modified=False)
...
def fix_auto_records(doctype, docname, owner, ts):
	...
	(
		artech_engine.qb.update(Version)
	...
	(
		artech_engine.qb.update(Comment)
