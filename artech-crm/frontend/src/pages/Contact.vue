...
import { useTelemetry } from 'frappe-ui/artech_engine'
...
const deals = createResource({
  url: 'artech_crm.api.contact.get_linked_deals',
...
async function setAsPrimary(field, value) {
  let d = await call('artech_crm.api.contact.set_as_primary', {
...
async function createNew(field, value) {
  if (!value) return
  let d = await call('artech_crm.api.contact.create_new', {
    contact: contact.doc.name,
    field,
    value,
  })
...
async function editOption(doctype, name, fieldname, value) {
  let d = await call('artech_engine.client.set_value', {
    doctype,
    name,
    fieldname,
    value,
...
async function deleteOption(doctype, name) {
  await call('artech_engine.client.delete', {
    doctype,
    name,
  })
