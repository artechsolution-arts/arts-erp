...
function createUpdateNote() {
  createResource({
    url: 'artech_crm.integrations.api.add_note_to_call_log',
    params: {
...
function createUpdateTask() {
  createResource({
    url: 'artech_crm.integrations.api.add_task_to_call_log',
    params: {
      call_sid: callData.value.CallSid,
      task: task.value,
    },
...
function makeOutgoingCall(number) {
  ...
  createResource({
    url: 'artech_crm.integrations.exotel.handler.make_a_call',
