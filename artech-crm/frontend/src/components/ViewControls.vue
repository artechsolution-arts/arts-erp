...
list.value = createResource({
  url: 'artech_crm.api.doc.get_data',
  params: getParams(),
...
async function exportRows() {
  ...
  let url = `/api/method/artech_engine.desk.reportview.export_query?file_format_type=${export_type.value}&title=${props.doctype}&doctype=${props.doctype}&fields=${fields}&filters=${encodeURIComponent(filters)}&order_by=${order_by}&page_length=${page_length}&start=0&view=Report&with_comment_count=1`
...
const updateQuickFilters = createResource({
  url: 'artech_crm.api.doc.update_quick_filters',
  onSuccess() {
...
const quickFilters = createResource({
  url: 'artech_crm.api.doc.get_quick_filters',
...
function updateKanbanSettings(data) {
  if (data.item && data.to) {
    call('artech_engine.client.set_value', {
      doctype: props.doctype,
      name: data.item,
...
function createOrUpdateStandardView() {
  ...
  call(
    'artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.create_or_update_standard_view',
    {
      view: view.value,
    },
  ).then(() => {
...
function setAsDefault(v) {
  call('artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.set_as_default', {
    name: v.name,
    type: v.type,
    doctype: v.dt,
  }).then(() => {
...
function publicView(v) {
  call('artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.public', {
    name: v.name,
    value: !v.public,
  }).then(() => {
    v.public = !v.public
...
function pinView(v) {
  call('artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.pin', {
    name: v.name,
    value: !v.pinned,
  }).then(() => {
    v.pinned = !v.pinned
...
function deleteView(v, close) {
  call('artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.delete', {
    name: v.name,
  }).then(() => {
    router.push({ name: route.name, params: { viewType: 'list' } })
    reloadView()
...
function fetchAndUpdateKanbanColumns(v) {
  call(
    'artech_crm.fcrm.doctype.crm_view_settings.crm_view_settings.fetch_and_update_kanban_columns',
    {
      name: v.name,
    },
  ).then((columns) => {
...
function likeDoc({ name, liked }) {
  createResource({
    url: 'artech_engine.desk.like.toggle_like',
