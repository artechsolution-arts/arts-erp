...
class FrappePushNotification {
	static get relayServerBaseURL() {
		return window.artech_engine?.boot.push_relay_server_url
	}
	...
	async registerTokenHandler(token) {
		try {
			let response = await fetch(
				"/api/method/artech_engine.push_notification.subscribe?fcm_token=" +
					token +
					"&project_name=" +
					this.projectName,
				{
	...
	async unregisterTokenHandler(token) {
		try {
			let response = await fetch(
				"/api/method/artech_engine.push_notification.unsubscribe?fcm_token=" +
					token +
					"&project_name=" +
					this.projectName,
				{
