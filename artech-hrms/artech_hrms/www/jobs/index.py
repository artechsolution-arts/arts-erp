...
from artech_engine.utils import pretty_date
...
def get_job_openings(filters=None, txt=None, sort=None, limit=20, offset=0):
	jo = artech_engine.qb.DocType("Job Opening")
	ja = artech_engine.qb.DocType("Job Applicant")

	query = (
		artech_engine.qb.from_(jo)
		.left_join(ja)
	...
	for d in filters:
		query = query.where(artech_engine.qb.Field(d).isin(filters[d]))
...
def get_all_filters(filters=None):
	job_openings = artech_engine.get_all(
		"Job Opening",
...
def get_filters_txt_sort_offset(page_len=20):
	args = artech_engine.request.args.to_dict(flat=False)
	filters = {}
	txt = ""
	sort = None
	offset = 0
