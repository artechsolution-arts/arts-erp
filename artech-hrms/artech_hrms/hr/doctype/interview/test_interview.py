...
from artech_engine.utils import add_days, get_datetime, get_time, getdate, nowtime
...
class TestInterview(HRMSTestSuite):
	...
	def test_notification_on_rescheduling(self):
		...
		artech_engine.db.sql("DELETE FROM `tabEmail Queue`")
		...
		notification = artech_engine.get_all(
	...
	def test_notification_for_scheduling(self):
		...
		artech_engine.db.set_single_value("HR Settings", "send_interview_reminder", 0)
		send_interview_reminder()
		self.assertFalse(get_email_by_subject("Subject: Interview Reminder"))

		artech_engine.db.set_single_value("HR Settings", "send_interview_reminder", 1)
		send_interview_reminder()
		import time
	...
	def test_notification_for_feedback_submission(self):
		...
		artech_engine.db.set_single_value("HR Settings", "send_interview_feedback_reminder", 0)
		send_daily_feedback_reminder()
		self.assertFalse(get_email_by_subject("Subject: Interview Feedback Reminder"))

		artech_engine.db.set_single_value("HR Settings", "send_interview_feedback_reminder", 1)
		send_daily_feedback_reminder()
	...
	def test_get_feedback(self):
		...
		make_employee(
			...
			first_name="Test",
			date_of_joining=artech_engine.utils.add_years(getdate(), -2),
...
def create_interview_type(name, skill_set, interviewers=None, designation=None, save=True):
	...
	interview_type = artech_engine.new_doc("Interview Type")
...
def create_skill_set(skill_set):
	for skill in skill_set:
		if not artech_engine.db.exists("Skill", skill):
			doc = artech_engine.new_doc("Skill")
			doc.skill_name = skill
			doc.save()


def setup_reminder_settings():
	if not artech_engine.db.exists("Email Template", _("Interview Reminder")):
	...
	if not artech_engine.db.exists("Email Template", _("Interview Feedback Reminder")):
		base_path = artech_engine.get_app_path("artech", "hr", "doctype")
		response = artech_engine.read_file(
