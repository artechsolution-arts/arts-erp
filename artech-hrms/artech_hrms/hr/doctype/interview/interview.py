...
from artech_engine.utils import cint, cstr, get_datetime, get_link_to_form, getdate, nowtime


class DuplicateInterviewRoundError(artech_engine.ValidationError):
	pass
...
class Interview(Document):
	...
	def validate_duplicate_interview(self):
		duplicate_interview = artech_engine.db.exists(
			"Interview",
		...
		if duplicate_interview:
			artech_engine.throw(
				_(
					...
					artech_engine.bold(get_link_to_form("Interview", duplicate_interview)),
					artech_engine.bold(self.job_applicant),
				)
			)

	def validate_designation(self):
		applicant_designation = artech_engine.db.get_value("Job Applicant", self.job_applicant, "designation")
		if self.designation:
			if self.designation != applicant_designation:
				artech_engine.throw(
					...
					).format(self.interview_type, artech_engine.bold(self.designation), applicant_designation),
	...
	def show_job_applicant_update_dialog(self):
		...
		job_application_name = artech_engine.db.get_value("Job Applicant", self.job_applicant, "applicant_name")

		artech_engine.msgprint(
			_("Do you want to update the Job Applicant {0} as {1} based on this interview result?").format(
				artech_engine.bold(job_application_name), artech_engine.bold(job_applicant_status)
			),
	...
	def reschedule_interview(
		...
		artech_engine.msgprint(_("Interview Rescheduled successfully"), indicator="green")
...
def get_interviewers(interview_type: str) -> list[dict]:
	return artech_engine.get_all("Interviewer", filters={"parent": interview_type}, fields=["user as interviewer"])


def get_recipients(name, for_feedback=0):
	interview = artech_engine.get_doc("Interview", name)
	interviewers = [d.interviewer for d in interview.interview_details]

	if for_feedback:
		feedback_given_interviewers = artech_engine.get_all(
		...
		recipients.append(artech_engine.db.get_value("Job Applicant", interview.job_applicant, "email_id"))
...
def get_skill_wise_average_rating(interview: str) -> list[dict]:
	skill_assessment = artech_engine.qb.DocType("Skill Assessment")
	interview_feedback = artech_engine.qb.DocType("Interview Feedback")
	return (
		artech_engine.qb.select(
...
def update_job_applicant_status(status: str, job_applicant: str):
	try:
		if not job_applicant:
			artech_engine.throw(_("Please specify the job applicant to be updated."))

		job_applicant = artech_engine.get_doc("Job Applicant", job_applicant)
...
def send_interview_reminder():
	reminder_settings = artech_engine.db.get_value(
		"HR Settings",
		"HR Settings",
	...
	remind_before = cstr(artech_engine.db.get_single_value("HR Settings", "remind_before")) or "01:00:00"
	...
	interviews = artech_engine.get_all(
		"Interview",
		filters=[
	...
	interview_template = artech_engine.get_doc("Email Template", reminder_settings.interview_reminder_template)

	for d in interviews:
		doc = artech_engine.get_doc("Interview", d.name)
		context = doc.as_dict()
		message = artech_engine.render_template(interview_template.response, context)
...
def send_daily_feedback_reminder():
	...
	interview_feedback_template = artech_engine.get_doc(
	...
	for interview in interviews:
		...
		doc = artech_engine.get_doc("Interview", interview)
		context = doc.as_dict()

		message = artech_engine.render_template(interview_feedback_template.response, context)
...
def create_interview_feedback(data: str | dict, interview_name: str, interviewer: str, job_applicant: str):
	...
	if isinstance(data, str):
		data = artech_engine._dict(json.loads(data))

	if artech_engine.session.user != interviewer:
		artech_engine.throw(_("Only Interviewer Are allowed to submit Interview Feedback"))

	interview_feedback = artech_engine.new_doc("Interview Feedback")
...
def get_events(start: str, end: str, filters: str | None = None):
	...
	interviews = artech_engine.db.sql(
		f"""
			SELECT DISTINCT
