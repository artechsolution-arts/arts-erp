...
class TestJobOpening(HRMSTestSuite):
	...
	def test_vacancies_fulfilled(self):
		...
		self.assertRaises(artech_engine.ValidationError, opening_2.insert)
...
def get_job_opening(**args):
	...
	opening = artech_engine.db.exists("Job Opening", {"job_title": args.job_title or "Designer"})
	if opening:
		return artech_engine.get_doc("Job Opening", opening)

	opening = artech_engine.get_doc(
		{
			"doctype": "Job Opening",
			"job_title": "Designer",
