...
class JobOpening(WebsiteGenerator):
	...
	def validate(self):
		if not self.route:
			self.route = f"jobs/{artech_engine.scrub(self.company)}/{artech_engine.scrub(self.job_title).replace('_', '-')}"
	...
	def validate_current_vacancies(self):
		if not self.staffing_plan:
			...
			self.planned_vacancies = artech_engine.db.get_value(
		...
		if self.staffing_plan and self.planned_vacancies:
			...
			number_of_positions = artech_engine.db.get_value(
				"Staffing Plan Detail",
				{"parent": self.staffing_plan, "designation": self.designation},
				"number_of_positions",
			)
			...
			if number_of_positions <= current_count:
				artech_engine.throw(
					_(
						...
						artech_engine.bold(self.designation), get_link_to_form("Staffing Plan", self.staffing_plan)
					),
	...
	def update_job_requisition_status(self):
		if self.status == "Closed" and self.job_requisition:
			job_requisition = artech_engine.get_doc("Job Requisition", self.job_requisition)
	...
	def get_context(self, context):
		context.no_of_applications = artech_engine.db.count("Job Applicant", {"job_title": self.name})
...
def close_expired_job_openings():
	...
	Opening = artech_engine.qb.DocType("Job Opening")
	openings = (
		artech_engine.qb.from_(Opening)
		.select(Opening.name)
		.where((Opening.status == "Open") & (Opening.closes_on.isnotnull()) & (Opening.closes_on < today))
	).run(pluck=True)

	for d in openings:
		doc = artech_engine.get_doc("Job Opening", d)
		doc.status = "Closed"
