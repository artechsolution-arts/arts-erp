	...
	def validate(self):
		...
		for d in self.get("leave_block_list_dates"):
			...
			if d.block_date in dates:
				artech_engine.msgprint(_("Date is repeated") + ":" + d.block_date, raise_exception=1)
...
def get_applicable_block_lists(employee=None, company=None, all_lists=False, leave_type=None):
	...
	if company:
		...
		add_block_list(artech_engine.db.get_all("Leave Block List", filters=conditions, pluck="name"))

	if employee:
		# per department
		department = artech_engine.db.get_value("Employee", employee, "department")
		if department:
			block_list = artech_engine.db.get_value("Department", department, "leave_block_list")
			block_list_leave_type = artech_engine.db.get_value("Leave Block List", block_list, "leave_type")
...
def is_user_in_allow_list(block_list):
	return artech_engine.db.get_value(
		...
		{"parent": block_list, "allow_user": artech_engine.session.user},
		"allow_user",
	)
