...
from artech_engine.utils import DATE_FORMAT, flt, formatdate, get_link_to_form, getdate, today


class InvalidLeaveLedgerEntry(artech_engine.ValidationError):
	pass
...
class LeaveLedgerEntry(Document):
		...
		if getdate(self.from_date) > getdate(self.to_date):
			artech_engine.throw(
				_(
					...
					artech_engine.bold(formatdate(self.from_date)),
					artech_engine.bold(formatdate(self.to_date)),
				),
	...
	def on_cancel(self):
		...
		if self.is_expired:
			artech_engine.db.set_value("Leave Allocation", self.transaction_name, "expired", 0)
		elif self.transaction_type != "Leave Adjustment":
			artech_engine.throw(_("Only expired allocation can be cancelled"))
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	leave_application_records = artech_engine.db.sql_list(
		"""
...
def create_leave_ledger_entry(ref_doc, args, submit=True):
	...
	if submit:
		doc = artech_engine.get_doc(ledger)
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = artech_engine.db.get_value(
		"Leave Ledger Entry",
		filters={
...
def process_expired_allocation():
	...
	leave_type_records = artech_engine.db.get_values(
	...
	expire_allocation = artech_engine.db.sql(
		"""
		SELECT
...
def expire_allocation(allocation: str | Document | artech_engine._dict, expiry_date: datetime.date | None = None):
	...
	artech_engine.db.set_value("Leave Allocation", allocation.name, "expired", 1)
...
def on_doctype_update():
	artech_engine.db.add_index("Leave Ledger Entry", ["transaction_type", "transaction_name"])
