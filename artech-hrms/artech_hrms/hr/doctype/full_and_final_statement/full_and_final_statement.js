artech_engine.ui.form.on("Full and Final Statement", {
	...
	set_queries: function (frm, type) {
		...
		frm.set_query("reference_document", type, function (doc, cdt, cdn) {
			let fnf_doc = artech_engine.get_doc(cdt, cdn);

			artech_engine.model.with_doctype(fnf_doc.reference_document_type, function () {
				if (artech_engine.model.is_tree(fnf_doc.reference_document_type)) {
					filters["is_group"] = 0;
				}

				if (artech_engine.model.is_submittable(fnf_doc.reference_document_type)) {
					filters["docstatus"] = ["!=", 2];
				}

				if (artech_engine.meta.has_field(fnf_doc.reference_document_type, "company")) {
					filters["company"] = frm.doc.company;
				}

				if (artech_engine.meta.has_field(fnf_doc.reference_document_type, "employee")) {
...
artech_engine.ui.form.on("Full and Final Outstanding Statement", {
	reference_document: function (frm, cdt, cdn) {
		...
		if (child.reference_document_type && child.reference_document) {
			artech_engine.call({
				...
				callback: function (r) {
					if (r.message) {
						artech_engine.model.set_value(cdt, cdn, "account", r.message[0]);
						artech_engine.model.set_value(cdt, cdn, "amount", r.message[1]);
					}
				},
			});
		}
...
artech_engine.ui.form.on("Full and Final Asset", {
