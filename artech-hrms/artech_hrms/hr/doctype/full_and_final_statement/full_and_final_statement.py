...
class FullandFinalStatement(Document):
		...
		if not self.relieving_date:
			artech_engine.throw(
				_("Please set {0} for Employee {1}").format(
					artech_engine.bold(_("Relieving Date")),
	...
	def validate_assets(self):
		...
		for data in self.assets_allocated:
			if data.action == "Return":
				if data.status == "Owned":
					pending_returns.append(_("Row {0}: {1}").format(data.idx, artech_engine.bold(data.asset_name)))
		...
		if pending_returns:
			...
			artech_engine.throw(msg, title=_("Pending Asset Returns"))
	...
	def get_assets_movement(self):
		asset_movements = artech_engine.get_all(
			"Asset Movement Item",
			filters={"docstatus": 1},
		...
		for movement in inward_movements:
			...
			if inwards_counts > outwards_count:
				cost = artech_engine.db.get_value("Asset", movement.asset, "total_asset_cost")
				data.append(
					{
						"reference": movement.parent,
						"asset_name": movement.asset_name,
						"date": artech_engine.db.get_value("Asset Movement", movement.parent, "transaction_date"),
						"actual_cost": cost,
						"cost": cost,
						"action": "Return",
						"status": "Owned",
	...
	def update_reference_document_payment_status(self, payable):
		doc = artech_engine.get_cached_doc(payable.reference_document_type, payable.reference_document)
...
def get_account_and_amount(ref_doctype: str, ref_document: str, company: str) -> list | None:
	...
	if ref_doctype == "Salary Slip":
		salary_details = artech_engine.db.get_value(
			"Salary Slip", ref_document, ["payroll_entry", "net_pay"], as_dict=1
		)
		amount = salary_details.net_pay
		payable_account = (
			artech_engine.db.get_value("Payroll Entry", salary_details.payroll_entry, "payroll_payable_account")
	...
	if ref_doctype == "Gratuity":
		payable_account, amount = artech_engine.db.get_value("Gratuity", ref_document, ["payable_account", "amount"])
		return [payable_account, amount]
	...
	if ref_doctype == "Leave Encashment":
		amount = artech_engine.db.get_value("Leave Encashment", ref_document, "encashment_amount")
		payable_account = artech_engine.get_cached_value("Company", company, "default_payroll_payable_account")
		return [payable_account, amount]
...
def update_full_and_final_statement_status(doc, method=None):
	...
	for entry in doc.accounts:
		if entry.reference_type == "Full and Final Statement":
			fnf = artech_engine.get_doc("Full and Final Statement", entry.reference_name)
