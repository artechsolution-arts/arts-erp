...
def process_loan_accrual(doc: "FullandFinalStatement"):
	...
	for loan in loan_receivables:
		loan_doc = artech_engine.get_doc("Loan", loan)
		loan_repayment_schedule = artech_engine.get_doc("Loan Repayment Schedule", {"loan": loan, "docstatus": 1})
		if loan_repayment_schedule.repayment_schedule:
			...
			for repayment_schedule in loan_repayment_schedule.repayment_schedule:
				...
				if not repayment_schedule.is_accrued:
					...
					artech_engine.db.set_value("Repayment Schedule", repayment_schedule.name, "is_accrued", 1)
...
def cancel_loan_repayment(doc: "FullandFinalStatement"):
	...
	for loan in loan_receivables:
		posting_date = artech_engine.utils.getdate(doc.transaction_date)
		loan_repayment = artech_engine.get_doc(
