...
class TestInterviewFeedback(HRMSTestSuite):
	def test_validation_for_skill_set(self):
		...
		artech_engine.set_user(interviewer)

		self.assertRaises(artech_engine.ValidationError, interview_feedback.save)
...
def get_skills_rating(interview_type):
	...
	skills = artech_engine.get_all("Expected Skill Set", filters={"parent": interview_type}, fields=["skill"])
	for d in skills:
