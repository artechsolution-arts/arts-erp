...
class OvertimeSlip(Document):
	...
	def validate_overlap(self):
		overtime_slips = artech_engine.db.get_all(
			"Overtime Slip",
			filters={
				"docstatus": ("!=", 2),
	...
	def validate_overtime_date_and_duration(self):
		...
		for detail in self.overtime_details:
			...
			if detail.date in dates:
				artech_engine.throw(_("Date {0} is repeated in Overtime Details").format(detail.date))
			...
			if detail.overtime_type not in overtime_type_cache:
				overtime_type_cache[detail.overtime_type] = artech_engine.db.get_value(
	...
	def get_frequency_and_dates(self):
		...
		if salary_structure:
			payroll_frequency = artech_engine.db.get_value("Salary Structure", salary_structure, "payroll_frequency")
			date_details = get_start_end_dates(
				payroll_frequency, date, artech_engine.db.get_value("Employee", self.employee, "company")
			)
	...
	def create_overtime_details_row_for_attendance(self, records):
		...
		for record in records:
			if record.overtime_type not in overtime_type_cache:
				overtime_type_cache[record.overtime_type] = artech_engine.db.get_value(
	...
	def process_overtime_slip(self):
		...
		precision = artech_engine.db.get_single_value("System Settings", "currency_precision") or 2
	...
	def _bulk_load_overtime_types(self, overtime_type_names):
		...
		overtime_types_data = artech_engine.get_all(
			"Overtime Type",
		...
		if salary_component_based_types:
			salary_components_data = artech_engine.get_all(
	...
	def get_overtime_type_details(self, name):
		details = artech_engine.get_value(
			"Overtime Type",
			filters={"name": name},
			fieldname=[
				"name",
		...
		if details.overtime_calculation_method == "Salary Component Based":
			components = artech_engine.get_all(
...
def create_overtime_slips_for_employees(employees, args):
	...
	for emp in employees:
		...
		try:
			artech_engine.get_doc(args).get_emp_and_overtime_details()
			count += 1
		except Exception as e:
			artech_engine.clear_last_message()
			errors.append(_("Employee {0} : {1}").format(emp, str(e)))
			artech_engine.log_error(artech_engine.get_traceback(), _("Overtime Slip Creation Error for {0}").format(emp))
	...
	artech_engine.get_doc("Payroll Entry", args.get("payroll_entry")).db_set({"status": status})
	artech_engine.publish_realtime("completed_overtime_slip_creation", user=artech_engine.session.user)
...
def submit_overtime_slips_for_employees(overtime_slips, payroll_entry):
	...
	for overtime_slip in overtime_slips:
		try:
			doc = artech_engine.get_doc("Overtime Slip", overtime_slip)
			...
			artech_engine.log_error(
				artech_engine.get_traceback(), _("Overtime Slip Submission Error for {0}").format(overtime_slip)
			)
	if count:
		artech_engine.msgprint(
	...
	status = "Failed" if errors else "Draft"
	payroll_entry = artech_engine.get_doc("Payroll Entry", payroll_entry).db_set({"status": status})
	artech_engine.publish_realtime("completed_overtime_slip_submission", user=artech_engine.session.user)
