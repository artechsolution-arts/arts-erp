...
from artech_engine.utils import add_days, flt, get_first_day, getdate, nowdate, today
...
class TestOvertimeSlip(HRMSTestSuite):
	def test_overtime_calculation_and_additional_salary_creation(self):
		...
		actual_overtime_amount = artech_engine.db.get_value(
	...
	def test_overtime_slip_creation_via_payroll_entry(self):
		...
		artech_engine.db.set_single_value("Payroll Settings", "create_overtime_slip", 1)
		...
		overtime_slip = artech_engine.db.exists(
			"Overtime Slip",
			{
				"employee": employee,
...
def create_overtime_slip(employee):
	date = getdate()
	month_start_date = get_first_day(date)
	slip = artech_engine.new_doc("Overtime Slip")
...
def setup_overtime(employee, overtime_calculation_method="Salary Component Based"):
	...
	overtime_details = artech_engine.get_all(
		"Overtime Details",
