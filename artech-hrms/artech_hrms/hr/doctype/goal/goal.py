...
class Goal(NestedSet):
	...
	def validate_parent_fields(self):
		...
		parent_details = artech_engine.db.get_value(
	...
	def validate_progress(self):
		if flt(self.progress) > 100:
			artech_engine.throw(_("Goal progress percentage cannot be more than 100."))
	...
	def update_kra_in_child_goals(self, doc_before_save):
		...
		if doc_before_save.kra != self.kra and self.is_group:
			...
			(artech_engine.qb.update(Goal).set(Goal.kra, self.kra).where(Goal.parent_goal == self.name)).run()

			artech_engine.msgprint(_("KRA updated for all child goals."), alert=True, indicator="green")
	...
	def update_parent_progress(self, old_parent=None):
		...
		parent_goal_doc = artech_engine.get_doc("Goal", parent_goal)
...
def get_children(doctype: str, parent: str, is_root: bool = False, **filters) -> list[dict]:
	...
	if filters.get("date_range"):
		date_range = artech_engine.parse_json(filters.get("date_range"))
...
def _update_goal_completion_status(goals: list[dict]) -> list[dict]:
	for goal in goals:
		if goal.expandable:  # group node
			total_goals = artech_engine.db.count("Goal", dict(parent_goal=goal.value))

			if total_goals:
				completed = artech_engine.db.count("Goal", {"parent_goal": goal.value, "status": "Completed"}) or 0
...
def update_progress(progress: float, goal: str) -> None:
	goal = artech_engine.get_doc("Goal", goal)
...
def add_tree_node():
	...
	if args.parent_goal == "All Goals" or not artech_engine.db.exists("Goal", args.parent_goal):
