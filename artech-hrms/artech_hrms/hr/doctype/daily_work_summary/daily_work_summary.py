...
from artech_engine.utils import global_date_format
...
class DailyWorkSummary(Document):
	...
	def get_message_details(self):
		...
		dws_group = artech_engine.get_doc("Daily Work Summary Group", self.daily_work_summary_group)
		...
		for d in replies:
			user = artech_engine.db.get_values(
			...
			try:
				if original_image:
					file_name = artech_engine.get_list("File", {"file_url": original_image})
		...
		did_not_reply = [
			(artech_engine.db.get_value("User", {"email": email}, "full_name") or email) for email in did_not_reply
		]
...
def get_user_emails_from_group(group):
	...
	if isinstance(group_doc, str):
		group_doc = artech_engine.get_doc("Daily Work Summary Group", group)
