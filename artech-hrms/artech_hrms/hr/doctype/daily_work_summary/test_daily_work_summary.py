...
class TestDailyWorkSummary(HRMSTestSuite):
	...
	def test_email_trigger_failed(self):
		hour = "00:00"
		if artech_engine.utils.nowtime().split(":")[0] == "00":
			hour = "01:00"
	...
	def test_incoming(self):
		...
		# pull the mail
		email_account = artech_engine.get_doc("Email Account", "_Test Email Account 1")
		email_account.db_set("enable_incoming", 1)
		email_account.receive(test_mails=test_mails)

		daily_work_summary = artech_engine.get_doc(
			"Daily Work Summary", artech_engine.get_all("Daily Work Summary")[0].name
		)
	...
	def setup_and_prepare_test(self, hour=None):
		artech_engine.db.sql("delete from `tabDaily Work Summary`")
		artech_engine.db.sql("delete from `tabEmail Queue`")
		artech_engine.db.sql("delete from `tabEmail Queue Recipient`")
		artech_engine.db.sql("delete from `tabCommunication`")
		artech_engine.db.sql("delete from `tabDaily Work Summary Group`")
		...
		self.emails = artech_engine.db.sql(
	...
	def setup_groups(self, hour=None):
		...
		if not hour:
			hour = artech_engine.utils.nowtime().split(":")[0]
			hour = hour + ":00"

		groups = artech_engine.get_doc(
			dict(
