...
class TestAttendance(HRMSTestSuite):
	...
	def test_mark_absent(self):
		...
		fetch_attendance = artech_engine.get_value(
	...
	def test_unmarked_days(self):
		...
		artech_engine.db.set_value("Employee", employee, "holiday_list", self.holiday_list)
	...
	def test_get_events_returns_attendance(self):
		...
		attendance = artech_engine.get_value("Attendance", attendance_name, "status")
		...
		self.assertEqual(
			...
			artech_engine.db.get_value("Employee", employee.name, "employee_name"),
		)
	...
	def test_bulk_attendance_marking_through_bg(self):
		...
		artech_engine.set_user(user1)
		data1 = artech_engine._dict(unmarked_days=[getdate()], employee=employee1, status="Present", shift="")
		data2 = artech_engine._dict(unmarked_days=[getdate()], employee=employee2, status="Present", shift="")
		mark_bulk_attendance(data1)
		self.assertStartsWith(
			artech_engine.message_log[-1].message, "Bulk attendance marking is queued with a background job."
		)
		artech_engine.set_user(user2)
		mark_bulk_attendance(data1)
		self.assertStartsWith(
			artech_engine.message_log[-1].message, "Bulk attendance marking is already in progress for employee"
		)
		mark_bulk_attendance(data2)
		self.assertStartsWith(
			artech_engine.message_log[-1].message, "Bulk attendance marking is queued with a background job."
		)
		artech_engine.flags.test_bg_job = False
		mark_bulk_attendance(data2)
		artech_engine.set_user("Administrator")
		attendance_records = artech_engine.get_all("Attendance", {"employee": employee2})
