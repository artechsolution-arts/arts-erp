...
artech_engine.ui.form.on("Leave Allocation", {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		if (!frm.doc.from_date) frm.set_value("from_date", artech_engine.datetime.get_today());
		...
		if (frm.doc.docstatus === 1 && !frm.doc.expired) {
			...
			if (valid_expiry) {
				...
				frm.add_custom_button(
					...
					function () {
						artech_engine.confirm("Are you sure you want to expire this allocation?", () => {
	...
	expire_allocation: function (frm) {
		artech_engine.call({
			...
			callback: function (r) {
				if (!r.exc) {
					artech_engine.msgprint(__("Allocation Expired!"));
				}
				frm.refresh();
			},
		});
	...
	set_indicator: function (frm) {
		const df = artech_engine.meta.get_docfield(
...
artech_engine.tour["Leave Allocation"] = [
	{
		fieldname: "employee",
		title: "Employee",
