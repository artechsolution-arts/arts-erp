...
class TestCompensatoryLeaveRequest(HRMSTestSuite):
	...
	def test_allocation_update_on_submit(self):
		...
		leaves_allocated = artech_engine.db.get_value(
			"Leave Allocation",
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = artech_engine.get_all("Leave Ledger Entry", fields="*", filters=filters)
	...
	def test_half_day_compensatory_leave(self):
		...
		date = today()
		compensatory_leave_request = artech_engine.new_doc("Compensatory Leave Request")
		...
		self.assertRaises(artech_engine.ValidationError, compensatory_leave_request.submit)
	...
	def test_request_on_leave_period_boundary(self):
		artech_engine.db.delete("Leave Period")
...
def get_compensatory_leave_request(employee, leave_date=None):
	...
	prev_comp_leave_req = artech_engine.db.get_value(
	...
	if prev_comp_leave_req:
		return artech_engine.get_doc("Compensatory Leave Request", prev_comp_leave_req)
...
def create_holiday_list(from_date=None, to_date=None):
	...
	if artech_engine.db.exists("Holiday List", list_name):
		artech_engine.db.delete("Holiday List", list_name)
		artech_engine.db.delete("Holiday", {"parent": list_name})
