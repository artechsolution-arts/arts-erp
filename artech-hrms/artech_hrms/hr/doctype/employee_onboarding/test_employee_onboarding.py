...
class TestEmployeeOnboarding(HRMSTestSuite):
	def test_employee_onboarding_incomplete_task(self):
		...
		project_name = artech_engine.db.get_value("Project", onboarding.project, "project_name")
		...
		# complete the task
		project = artech_engine.get_doc("Project", onboarding.project)
		for task in artech_engine.get_all("Task", dict(project=project.name)):
			task = artech_engine.get_doc("Task", task.name)
	...
	def test_mark_onboarding_as_completed(self):
		...
		for task_status in artech_engine.get_all("Task", dict(project=project.name), pluck="status"):
...
def get_job_applicant():
	if artech_engine.db.exists("Job Applicant", "test@engineer.com"):
		return artech_engine.get_doc("Job Applicant", "test@engineer.com")
	applicant = artech_engine.new_doc("Job Applicant")
...
def get_job_offer(applicant_name):
	job_offer = artech_engine.db.exists("Job Offer", {"job_applicant": applicant_name})
	if job_offer:
		return artech_engine.get_doc("Job Offer", job_offer)
...
def create_employee_onboarding():
	...
	onboarding = artech_engine.new_doc("Employee Onboarding")
...
def get_task_dates(task: str) -> tuple[str, str]:
	start_date, end_date = artech_engine.db.get_value("Task", task, ["exp_start_date", "exp_end_date"])
