artech_engine.ui.form.on("Employee Onboarding", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		if (frm.doc.employee) {
			frm.add_custom_button(
				__("Employee"),
				function () {
					artech_engine.set_route("Form", "Employee", frm.doc.employee);
				},
				__("View"),
			);
		}
		if (frm.doc.project) {
			frm.add_custom_button(
				__("Project"),
				function () {
					artech_engine.set_route("Form", "Project", frm.doc.project);
				},
				__("View"),
			);
			frm.add_custom_button(
				__("Task"),
				function () {
					artech_engine.set_route("List", "Task", { project: frm.doc.project });
				},
				__("View"),
			);
		}
