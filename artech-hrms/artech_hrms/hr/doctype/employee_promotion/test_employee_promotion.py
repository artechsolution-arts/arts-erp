...
class TestEmployeePromotion(HRMSTestSuite):
	def test_submit_before_promotion_date(self):
		...
		promotion = artech_engine.get_doc(
			{
		...
		self.assertRaises(artech_engine.DocstatusTransitionError, promotion.submit)
	...
	def test_employee_history(self):
		for grade in ["L1", "L2"]:
			artech_engine.get_doc({"doctype": "Employee Grade", "__newname": grade}).insert()
