...
from artech_engine.utils import cstr, flt, get_link_to_form, today
...
class ExpenseApproverIdentityError(artech_engine.ValidationError):
	pass


class MismatchError(artech_engine.ValidationError):
	pass
...
class ExpenseClaim(AccountsController, PWANotificationsMixin):
	...
	def onload(self):
		self.get("__onload").make_payment_via_journal_entry = artech_engine.db.get_single_value(
			"Accounts Settings", "make_payment_via_journal_entry"
		)
		self.set_onload(
			"self_expense_approval_not_allowed",
			artech_engine.db.get_single_value("HR Settings", "prevent_self_expense_approval"),
		)
	...
	def validate(self):
		...
		if self.task and not self.project:
			self.project = artech_engine.db.get_value("Task", self.task, "project")
	...
	def validate_company_and_department(self):
		if self.department:
			company = artech_engine.db.get_value("Department", self.department, "company")
	...
	def validate_for_self_approval(self):
		self_expense_approval_not_allowed = artech_engine.db.get_single_value(
		...
		if (
			...
			artech_engine.throw(_("Self-approval for Expense Claims is not allowed"))
	...
	def before_submit(self):
		if not self.payable_account and not self.is_paid:
			artech_engine.throw(_("Payable Account is mandatory to submit an Expense Claim"))
	...
	def on_submit(self):
		if self.approval_status == "Draft":
			artech_engine.throw(_("""Approval Status must be 'Approved' or 'Rejected'"""))
		...
		if not artech_engine.db.get_single_value("Accounts Settings", "make_payment_via_journal_entry"):
	...
	def update_claimed_amount_in_employee_advance(self):
		for d in self.get("advances"):
			artech_engine.get_doc("Employee Advance", d.employee_advance).update_claimed_amount()

	def update_task_and_project(self):
		if self.task:
			task = artech_engine.get_doc("Task", self.task)

			ExpenseClaim = artech_engine.qb.DocType("Expense Claim")
			task.total_expense_claim = (
				artech_engine.qb.from_(ExpenseClaim)
			...
			artech_engine.get_doc("Project", self.project).update_project()
	...
	def get_gl_entries(self):
		...
		make_payment_via_je = artech_engine.db.get_single_value(
			"Accounts Settings", "make_payment_via_journal_entry"
		)
	...
	def set_default_accounting_dimension(self):
		...
		for dim in get_checks_for_pl_and_bs_accounts():
			...
			field = artech_engine.scrub(dim.fieldname)
	...
	def create_exchange_gain_loss_je(self):
		...
		if total_advance_exchange_gain_loss:
			gain_loss_account = artech_engine.get_cached_value("Company", self.company, "exchange_gain_loss_account")
			self.db_set(
				{
			...
			if not data.cost_center:
				artech_engine.throw(
					_("Row {0}: {1} is required in the expenses table to book an expense claim.").format(
						data.idx, artech_engine.bold(_("Cost Center"))
					)
				)

		if self.is_paid:
			if not self.mode_of_payment:
				artech_engine.throw(_("Mode of payment is required to make a payment").format(self.employee))
	...
	def validate_advances(self):
		...
		for d in self.get("advances"):
			advance_employee = artech_engine.db.get_value("Employee Advance", d.employee_advance, "employee")
			if self.employee != advance_employee:
				artech_engine.throw(_("Selected employee advance is not of employee {}").format(self.employee))
		...
		if self.total_advance_amount:
			...
			if flt(self.total_advance_amount, precision) > amount_with_taxes:
				artech_engine.throw(_("Total advance amount cannot be greater than total sanctioned amount"))
	...
	def update_against_claim_in_pe(self):
		...
		if reference_against_pe:
			for pe_ref in reference_against_pe:
				payment_entry = artech_engine.get_doc("Payment Entry", pe_ref.voucher_no)
...
def update_reimbursed_amount(doc):
	...
	artech_engine.db.set_value("Expense Claim", doc.name, "total_amount_reimbursed", total_amount_reimbursed)
...
def get_total_reimbursed_amount(doc):
	if doc.is_paid:
		...
		amount_via_jv = artech_engine.db.get_value(
		...
		amount_via_payment_entry = artech_engine.db.get_value(
...
def get_outstanding_amount_for_claim(claim):
	precision = artech_engine.get_precision("Expense Claim", "grand_total")

	if isinstance(claim, str):
		claim = artech_engine.db.get_value(
			"Expense Claim",
			claim,
			(
...
def make_bank_entry(dt: str, dn: str) -> dict:
	...
	expense_claim = artech_engine.get_doc(dt, dn)
	...
	je.append(
		"accounts",
		{
			...
			"cost_center": artech.get_default_cost_center(expense_claim.company),
...
def get_expense_claim_account_and_cost_center(expense_claim_type: str, company: str) -> dict:
	...
	cost_center = artech.get_default_cost_center(company)
	...
	if not account:
		artech_engine.throw(
			_("Set the default account for the {0} {1}").format(
				artech_engine.bold(_("Expense Claim Type")),
...
def get_advances(expense_claim: str | dict | Document, advance_id: str | None = None):
	...
	if isinstance(expense_claim, str):
		expense_claim = artech_engine._dict(json.loads(expense_claim))
	expense_claim_doc = artech_engine.get_doc(expense_claim)
	expense_claim_doc.advances = []

	advance = artech_engine.qb.DocType("Employee Advance")

	query = artech_engine.qb.from_(advance).select(
		advance.name,
		advance.purpose,
	...
	payment_via_journal_entry = artech_engine.db.get_single_value(
		"Accounts Settings", "make_payment_via_journal_entry"
	)
...
def get_expense_claim(employee_advance: str | dict, payment_via_journal_entry: str | int | bool) -> Document:
	if isinstance(employee_advance, str):
		employee_advance = artech_engine.get_doc("Employee Advance", employee_advance)

	company = employee_advance.company
	default_payable_account = artech_engine.get_cached_value(
		"Company", company, "default_expense_claim_payable_account"
	)
	default_cost_center = artech_engine.get_cached_value("Company", company, "cost_center")

	expense_claim = artech_engine.new_doc("Expense Claim")
	...
	expense_claim.payable_account = (
		...
		if employee_advance.currency == artech.get_company_currency(company)
		else None
	)
...
def get_expense_claim_advances(expense_claim, employee_advance):
	...
	if int(employee_advance.payment_via_journal_entry):
		...
		exchange_rate = artech_engine.db.get_value(
			"Advance Payment Ledger Entry",
			{
		...
		pe_ref = artech_engine.qb.DocType("Payment Entry Reference")
		payment_entries = (
...
def update_payment_for_expense_claim(doc, method=None):
	...
	for d in doc.get(payment_table):
		if d.get(doctype_field) == "Expense Claim" and d.reference_name:
			expense_claim = artech_engine.get_doc("Expense Claim", d.reference_name)
...
def update_outstanding_amount_in_payment_entry(expense_claim: dict, pe_reference: str):
	...
	# TODO: refactor convoluted code after artech payment entry becomes extensible
	outstanding_amount = get_outstanding_amount_for_claim(expense_claim)
	artech_engine.db.set_value("Payment Entry Reference", pe_reference, "outstanding_amount", outstanding_amount)
...
def get_allocation_amount(
	...
	if unclaimed_amount is not None and return_amount is not None:
		...
		artech_engine.throw(_("Invalid parameters provided. Please pass the required arguments."))
