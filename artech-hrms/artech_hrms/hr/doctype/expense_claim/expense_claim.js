...
artech_engine.ui.form.on("Expense Claim", {
	...
	refresh: function (frm) {
		...
		if (
			...
			artech_engine.model.can_create("Payment Entry")
		) {
	...
	update_fields_label: function (frm) {
		var company_currency = artech.get_currency(frm.doc.company);
	...
	add_ledger_buttons: function (frm) {
		...
		if (!frm.doc.__islocal && frm.doc.docstatus === 1) {
			...
			if (
				...
				artech_engine.model.can_read(entry_doctype)
			) {
				...
				frm.add_custom_button(
					...
					function () {
						...
						artech_engine.set_route("List", entry_doctype);
					},
					__("View"),
				);
			}
	...
	mode_of_payment: async function (frm) {
		if (frm.doc.mode_of_payment) {
			var mode_of_payment_type = (
				await artech_engine.db.get_value("Mode of Payment", frm.doc.mode_of_payment, "type")
			)?.message?.type;
		...
		if (frm.doc.employee) {
			return artech_engine.call({
				...
				callback: function (r, rt) {
					artech_engine.model.clear_table(frm.doc, "advances");
					if (r.message) {
...
artech_engine.ui.form.on("Expense Claim Detail", {
	expense_type: function (frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		if (!frm.doc.company) {
			d.expense_type = "";
			artech_engine.msgprint(__("Please set the Company"));
	...
	amount: function (frm, cdt, cdn) {
		...
		artech_engine.model.set_value(cdt, cdn, "sanctioned_amount", child.amount);
	...
	cost_center: function (frm, cdt, cdn) {
		artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "expenses", "cost_center");
	},
});

artech_engine.ui.form.on("Expense Claim Advance", {
	employee_advance: function (frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		if (!frm.doc.employee) {
			artech_engine.msgprint(__("Select an employee to get the employee advance."));
			frm.doc.advances = [];
			refresh_field("advances");
		} else {
			return artech_engine.call({
...
artech_engine.ui.form.on("Expense Taxes and Charges", {
