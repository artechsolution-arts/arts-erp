...
from artech_engine.utils import flt, nowdate, random_string, today
...
class TestExpenseClaim(HRMSTestSuite):
	def setUp(self):
		if not artech_engine.db.get_value("Cost Center", {"company": company_name}):
			cost_center = artech_engine.new_doc("Cost Center")
			cost_center.update(
				{
					"doctype": "Cost Center",
			...
			artech_engine.db.set_value("Company", company_name, "default_cost_center", cost_center)
	...
	def test_total_expense_claim_for_project(self):
		...
		task = artech_engine.new_doc("Task")
		task.update(
		...
		self.assertEqual(artech_engine.db.get_value("Task", task, "total_expense_claim"), 200)
		self.assertEqual(artech_engine.db.get_value("Project", project, "total_expense_claim"), 200)
		...
		self.assertEqual(artech_engine.db.get_value("Task", task, "total_expense_claim"), 700)
		self.assertEqual(artech_engine.db.get_value("Project", project, "total_expense_claim"), 700)
	...
	def test_expense_claim_status_as_payment_from_journal_entry(self):
		...
		gl_entry = artech_engine.get_all("GL Entry", {"voucher_type": "Expense Claim", "voucher_no": claim.name})
	...
	def test_expense_claim_against_fully_paid_advances(self):
		...
		artech_engine.db.delete("Employee Advance")
	...
	def test_expense_approver_perms(self):
		...
		expense_claim.save()
		self.assertTrue(expense_claim.name in artech_engine.share.get_shared("Expense Claim", user))

		# check shared doc revoked
		expense_claim.reload()
		expense_claim.expense_approver = "test@example.com"
		expense_claim.save()
		self.assertTrue(expense_claim.name not in artech_engine.share.get_shared("Expense Claim", user))
	...
	def test_expense_claim_against_delivery_trip(self):
		from artech.stock.doctype.delivery_trip.test_delivery_trip import (
			create_address,
	...
	def test_accounting_dimension_mapping(self):
		...
		dimensions = artech_engine.db.get_value(
			"GL Entry",
			{
	...
	def test_repost(self):
		...
		ledger_balance = artech_engine.db.get_all(
			"GL Entry",
		...
		artech_engine.db.set_value("GL Entry", gl_entries[0].name, "credit", 0)
	...
	def test_self_expense_approval(self):
		artech_engine.db.set_single_value("HR Settings", "prevent_self_expense_approval", 0)
	...
	def test_self_expense_approval_not_allowed(self):
		artech_engine.db.set_single_value("HR Settings", "prevent_self_expense_approval", 1)
		...
		self.assertRaises(artech_engine.ValidationError, expense_claim.submit)
		expense_claim.reload()

		artech_engine.set_user(expense_approver)
		expense_claim.submit()
	...
	def test_advance_claim_multicurrency_gain_loss(self):
		...
		journal = artech_engine.db.get_value(
			"Journal Entry Account",
			filters={"reference_type": "Expense Claim", "reference_name": claim.name, "docstatus": 1},
			fieldname="parent",
		)
		gain_loss_jv = artech_engine.get_doc("Journal Entry", journal)
	...
	def test_expense_claim_status_as_payment_after_unreconciliation(self):
		...
		unreconcile_doc = artech_engine.new_doc("Unreconcile Payment")
...
def get_payable_account(company):
	return artech_engine.get_cached_value("Company", company, "default_payable_account")
...
def generate_taxes(company=None, rate=None) -> dict:
	...
	cost_center = artech_engine.db.get_value("Company", company, "cost_center")
...
def make_expense_claim(
	...
	if not employee:
		employee = artech_engine.db.get_value("Employee", {"status": "Active", "company": company})
		if not employee:
			employee = make_employee("test_employee@expenseclaim.com", company=company)

	currency, cost_center = artech_engine.db.get_value("Company", company, ["default_currency", "cost_center"])
	expense_claim = {
	...
	expense_claim = artech_engine.get_doc(expense_claim)
...
def make_journal_entry(expense_claim, do_not_submit=False):
	...
	je = artech_engine.get_doc(je_dict)
...
def create_project(project_name, **args):
	project = artech_engine.db.exists("Project", {"project_name": project_name})
	if project:
		return project

	doc = artech_engine.new_doc("Project")
