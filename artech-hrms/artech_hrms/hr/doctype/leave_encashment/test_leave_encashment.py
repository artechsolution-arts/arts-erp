...
class TestLeaveEncashment(HRMSTestSuite):
	def setUp(self):
		...
		create_assignment_for_multiple_employees([self.employee], artech_engine._dict(data))
	...
	def test_leave_balance_value_and_amount(self):
		...
		additional_salary_amount = artech_engine.db.get_value(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		artech_engine.db.delete("Additional Salary", {"ref_docname": leave_encashment.name})
		leave_encashment.cancel()
		self.assertFalse(artech_engine.db.exists("Leave Ledger Entry", {"transaction_name": leave_encashment.name}))
	...
	def test_unused_leaves_after_leave_encashment_for_carry_forwarding_leave_type(self):
		...
		self.assertEqual(artech_engine.get_value("Leave Type", self.leave_type, "is_carry_forward"), 1)
		leave_ledger_entry = artech_engine.get_all(
	...
	def test_leave_expiry_after_leave_encashment_for_non_carry_forwarding_leave_type(self):
		...
		self.assertEqual(artech_engine.get_value("Leave Type", self.leave_type, "is_carry_forward"), 0)
		leave_ledger_entry = artech_engine.get_all(
			"Leave Ledger Entry",
			fields="*",
		...
		expired_leaves = artech_engine.get_value(
			"Leave Ledger Entry",
	...
	def get_encashment_created_after_leave_period(self, employee, is_carry_forward, encashment_days):
		artech_engine.db.delete("Leave Period", {"name": self.leave_period.name})
		...
		leave_policy = artech_engine.get_value("Leave Policy", {"title": "Test Leave Policy"}, "name")
		data = {
			"assignment_based_on": "Leave Period",
			"leave_policy": leave_policy,
			"leave_period": self.leave_period.name,
		}
		create_assignment_for_multiple_employees([employee], artech_engine._dict(data))
	...
	def test_status_of_leave_encashment_after_payment_via_payment_entry_and_fnf(self):
		...
		artech_engine.db.set_value("Employee", self.employee, "relieving_date", getdate())
		...
		jv.accounts[1].account = (
			artech_engine.get_cached_value("Company", "_Test Company", "default_bank_account") or "_Test Bank - _TC"
		)
		jv.cheque_no = "123456"
...
def create_leave_encashment(**args):
	...
	leave_encashment = artech_engine.new_doc("Leave Encashment")
	...
	leave_encashment.currency = args.currency or artech_engine.get_cached_value(
	...
	if leave_encashment.pay_via_payment_entry:
		leave_encashment.payable_account = args.payable_account or artech_engine.get_cached_value(
