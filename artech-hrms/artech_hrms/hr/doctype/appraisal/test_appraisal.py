...
from artech_hrms.hr.doctype.appraisal_cycle.appraisal_cycle import get_appraisal_cycle_summary
...
from artech_hrms.hr.doctype.goal.test_goal import create_goal
...
class TestAppraisal(HRMSTestSuite):
	def setUp(self):
		artech_engine.db.delete("Goal")
		artech_engine.db.delete("Appraisal")
		artech_engine.db.delete("Employee Performance Feedback")
	...
	def test_validate_duplicate(self):
		...
		self.assertRaises(artech_engine.DuplicateEntryError, appraisal.insert)

	def test_manual_kra_rating(self):
		cycle = create_appraisal_cycle(designation="Engineer", kra_evaluation_method="Manual Rating")
		cycle.create_appraisals()

		appraisal = artech_engine.db.exists("Appraisal", {"appraisal_cycle": cycle.name, "employee": self.employee1})
	...
	def test_cycle_completion(self):
		...
		self.assertRaises(artech_engine.ValidationError, cycle.complete_cycle)
		...
		self.assertRaises(artech_engine.ValidationError, appraisal.insert)
