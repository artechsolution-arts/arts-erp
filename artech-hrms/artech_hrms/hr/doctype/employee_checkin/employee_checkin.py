...
class CheckinRadiusExceededError(artech_engine.ValidationError):
	pass
...
class EmployeeCheckin(Document):
	...
	def validate_duplicate_log(self):
		...
		if doc:
			doc_link = artech_engine.get_desk_link("Employee Checkin", doc)
			artech_engine.throw(
	...
	def validate_distance_from_shift_location(self):
		...
		if not (self.latitude or self.longitude):
			artech_engine.throw(_("Latitude and longitude values are required for checking in."))

		assignment_locations = artech_engine.get_all(
			"Shift Assignment",
			filters={
				"employee": self.employee,
		...
		checkin_radius, latitude, longitude = artech_engine.db.get_value(
...
def add_log_based_on_employee_field(
	...
	if not employee_field_value or not timestamp:
		artech_engine.throw(_("'employee_field_value' and 'timestamp' are required."))

	employee = artech_engine.db.get_values(
		"Employee",
	...
	doc = artech_engine.new_doc("Employee Checkin")
...
def bulk_fetch_shift(checkins: list[str] | str) -> None:
	if isinstance(checkins, str):
		checkins = artech_engine.json.loads(checkins)
	for d in checkins:
		doc = artech_engine.get_doc("Employee Checkin", d)
		doc.fetch_shift()
...
def mark_attendance_and_link_log(
	...
	if attendance_status not in ("Present", "Absent", "Half Day"):
		artech_engine.throw(_("{0} is an invalid Attendance Status.").format(attendance_status))

	try:
		artech_engine.db.savepoint("attendance_creation")
...
def create_or_update_attendance(
	...
	if attendance := get_existing_half_day_attendance(employee, attendance_date):
		...
		return artech_engine.get_doc("Attendance", attendance.name)
	else:
...
def get_overtime_data(shift_name, working_hours):
	...
	shift_type_details = artech_engine.db.get_value(
		doctype="Shift Type",
...
def get_existing_half_day_attendance(employee, attendance_date):
	...
	if attendance_name:
		attendance_doc = artech_engine.get_doc("Attendance", attendance_name)
...
def handle_attendance_exception(log_names: list, error_message: str):
	artech_engine.db.rollback(save_point="attendance_creation")
...
def add_comment_in_checkins(log_names: list, error_message: str):
	text = "{prefix}<br>{error_message}".format(
		prefix=artech_engine.bold(_("Reason for skipping auto attendance:")), error_message=error_message
	)
