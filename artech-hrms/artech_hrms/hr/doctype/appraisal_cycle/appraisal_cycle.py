...
class AppraisalCycle(Document):
	...
	def set_employees(self):
		...
		if employees:
			...
			self.set("appraisees", [])
			artech_engine.msgprint(_("No employees found for the selected criteria"))
	...
	def get_appraisal_template_map(self):
		designations = artech_engine.get_all("Designation", fields=["name", "appraisal_template"])
		appraisal_templates = artech_engine._dict()
	...
	def show_missing_template_message(self, raise_exception=False):
		...
		).format(f"""<a href='{artech_engine.utils.get_url_to_list("Designation")}'>Designations</a>""")
	...
	def complete_cycle(self):
		...
		draft_appraisals = artech_engine.db.count("Appraisal", {"appraisal_cycle": self.name, "docstatus": 0})

		if draft_appraisals:
			link = artech_engine.utils.get_url_to_list("Appraisal") + f"?status=Draft&appraisal_cycle={self.name}"
			link = f"""<a href="{link}">documents</a>"""

			msg = _("{0} Appraisal(s) are not submitted yet").format(artech_engine.bold(draft_appraisals))
			msg += "<br><br>"
			msg += _("Please submit the {0} before marking the cycle as Completed").format(link)
			artech_engine.throw(msg, title=_("Unsubmitted Appraisals"))
...
def create_appraisals_for_cycle(appraisal_cycle: AppraisalCycle, publish_progress: bool = False):
	...
	for employee in appraisal_cycle.appraisees:
		try:
			appraisal = artech_engine.get_doc(
				{
					"doctype": "Appraisal",
...
def validate_active_appraisal_cycle(appraisal_cycle: str) -> None:
	if artech_engine.db.get_value("Appraisal Cycle", appraisal_cycle, "status") == "Completed":
		msg = _("Cannot create or change transactions against an Appraisal Cycle with status {0}.").format(
			artech_engine.bold(_("Completed"))
		)
		msg += "<br><br>"
		msg += _("Set the status to {0} if required.").format(artech_engine.bold(_("In Progress")))
...
def get_appraisal_cycle_summary(cycle_name: str) -> dict:
	summary = artech_engine._dict()

	summary["appraisees"] = artech_engine.db.count(
		"Appraisal", {"appraisal_cycle": cycle_name, "docstatus": ("!=", 2)}
	)
	summary["self_appraisal_pending"] = artech_engine.db.count(
...
def get_employees_without_feedback(cycle_name: str | None = None) -> int:
	Feedback = artech_engine.qb.DocType("Employee Performance Feedback")
	Appraisal = artech_engine.qb.DocType("Appraisal")
	count = Count("*").as_("count")
	if not cycle_name:
		cycle_name = artech_engine.get_value(
			"Appraisal Cycle", {"status": "In Progress"}, order_by="start_date desc"
		)

	filtered_records = SubQuery(
		artech_engine.qb.from_(Feedback)
