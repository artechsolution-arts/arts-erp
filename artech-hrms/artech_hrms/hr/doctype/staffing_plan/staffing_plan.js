artech_engine.ui.form.on("Staffing Plan", {
	setup: function (frm) {
	...
	get_job_requisitions: function (frm) {
		new artech_engine.ui.form.MultiSelectDialog({
			...
			action(selections) {
				...
				artech_engine
					...
					.then(() => {
						// hack to retain prompt name that gets lost on artech_engine.call
...
artech_engine.ui.form.on("Staffing Plan Detail", {
	...
	vacancies: function (frm, cdt, cdn) {
		...
		if (child.vacancies < child.current_openings) {
			artech_engine.throw(__("Vacancies cannot be lower than the current openings"));
		}
		set_number_of_positions(frm, cdt, cdn);
	},
...
var set_number_of_positions = function (frm, cdt, cdn) {
	...
	artech_engine.call({
		...
		callback: function (data) {
			if (data.message) {
				artech_engine.model.set_value(cdt, cdn, "current_count", data.message.employee_count);
				artech_engine.model.set_value(cdt, cdn, "current_openings", data.message.job_openings);
				...
				if (cint(child.number_of_positions) < total_positions) {
					artech_engine.model.set_value(cdt, cdn, "number_of_positions", total_positions);
				}
			} else {
				// No employees for this designation
				artech_engine.model.set_value(cdt, cdn, "current_count", 0);
				artech_engine.model.set_value(cdt, cdn, "current_openings", 0);
			}
		},
	});
	refresh_field("staffing_details");
...
var set_total_estimated_cost = function (frm, cdt, cdn) {
	...
	if (child.vacancies > 0 && child.estimated_cost_per_position) {
		...
		artech_engine.model.set_value(cdt, cdn, "total_estimated_cost", 0);
	}
