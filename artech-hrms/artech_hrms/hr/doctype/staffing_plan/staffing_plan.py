...
class SubsidiaryCompanyError(artech_engine.ValidationError):
	pass


class ParentCompanyError(artech_engine.ValidationError):
	pass
...
class StaffingPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from artech_hrms.hr.doctype.staffing_plan_detail.staffing_plan_detail import StaffingPlanDetail
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		if not artech_engine.get_cached_value("Company", self.company, "parent_company"):
		...
		parent_company = artech_engine.db.get_value("Staffing Plan", parent_plan_details[0].name, "company")
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or flt(
			...
			artech_engine.throw(
				_(
					...
					artech_engine.bold(staffing_plan_detail.designation),
		...
		lft, rgt = artech_engine.get_cached_value("Company", parent_company, ["lft", "rgt"])
		all_sibling_details = artech_engine.db.sql(
	...
	def validate_with_subsidiary_plans(self, staffing_plan_detail):
		...
		children_details = artech_engine.db.sql(
			"""select sum(spd.vacancies) as vacancies,
			sum(spd.total_estimated_cost) as total_estimated_cost
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
	...
	def set_job_requisitions(self, job_reqs: list[str]) -> Document:
		if job_reqs:
			requisitions = artech_engine.db.get_list(
				"Job Requisition",
...
def get_designation_counts(designation: str, company: str, job_opening: str | None = None) -> dict | bool:
	...
	employee_count = artech_engine.db.count(
	...
	job_openings = artech_engine.db.count("Job Opening", filters)
...
def get_active_staffing_plan_details(
	...
	if not company or not designation:
		artech_engine.throw(_("Please select Company and Designation"))

	staffing_plan = artech_engine.db.sql(
		"""
	...
	if not staffing_plan:
		parent_company = artech_engine.get_cached_value("Company", company, "parent_company")
		if parent_company:
