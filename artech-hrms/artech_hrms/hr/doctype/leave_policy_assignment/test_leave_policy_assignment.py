...
from artech_engine.utils import add_days, add_months, get_first_day, get_year_ending, get_year_start, getdate

from artech_hrms.hr.doctype.leave_application.test_leave_application import get_employee, get_leave_period
...
class TestLeavePolicyAssignment(HRMSTestSuite):
	...
	def test_grant_leaves(self):
		...
		self.assertEqual(
			artech_engine.db.get_value("Leave Policy Assignment", assignments[0], "leaves_allocated"),
			1,
		)
		...
		leave_alloc_doc = artech_engine.get_doc("Leave Allocation", allocation)
	...
	def test_pro_rated_leave_allocation(self):
		...
		assignments = create_assignment_for_multiple_employees([self.employee.name], artech_engine._dict(data))
	...
	def test_get_leaves_for_passed_months(self):
		...
		self.employee.save()
		assignment = create_assignment(self.employee.name, artech_engine._dict(data))
	...
	def test_pro_rated_leave_allocation_for_custom_date_range(self):
		...
		leave_policy_assignment = artech_engine.new_doc("Leave Policy Assignment")
		...
		new_leaves_allocated = artech_engine.db.get_value(
			"Leave Allocation",
			{
	...
	def test_earned_leave_allocation_if_leave_policy_assignment_submitted_after_period(self):
		...
		artech_engine.flags.current_date = add_days(year_end_date, 10)
		leave_type = create_leave_type(
		...
		earned_leave_allocation = artech_engine.get_value(
	...
	def test_earned_leave_allocation_for_leave_period_spanning_two_years(self):
		...
		artech_engine.flags.current_date = add_months(second_year_end_date, -6)
		leave_type = create_leave_type(
			leave_type_name="_Test Earned Leave", is_earned_leave=True, allocate_on_day="Last Day"
		)
	...
	def test_skip_zero_allocation_leaves(self):
		...
		comments = artech_engine.get_all(
			"Comment",
			filters={
		...
		allocations = artech_engine.get_all(
			"Leave Allocation",
