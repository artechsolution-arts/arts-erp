...
class TestAttendanceRequest(HRMSTestSuite):
	...
	def test_attendance_request_overlap(self):
		...
		attendance_request = artech_engine.get_doc(
			{
	...
	def test_overwrite_attendance(self):
		...
		prev_attendance = artech_engine.get_doc("Attendance", attendance_name)
	...
	def test_skip_attendance_on_holiday(self):
		today = getdate()
		artech_engine.db.delete("Holiday", {"parent": self.holiday_list})
	...
	def test_skip_attendance_on_leave(self):
		...
		artech_engine.delete_doc_if_exists("Leave Type", "Test Skip Attendance", force=1)
	...
	def test_validate_no_attendance_to_create(self):
		...
		self.assertRaises(artech_engine.ValidationError, attendance_request.save)
	...
	def test_half_day_status_change(self):
		...
		half_day_status = artech_engine.get_value(
	...
	def test_overlap_with_different_shifts(self):
		...
		attendances = artech_engine.db.get_all(
			"Attendance",
...
def create_shift(name, start_time, end_time):
	if artech_engine.db.exists("Shift Type", name):
		return artech_engine.get_doc("Shift Type", name)
