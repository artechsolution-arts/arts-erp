...
from artech_engine.utils import add_days, date_diff, format_date, get_link_to_form, getdate
...
class AttendanceRequest(Document):
	...
	def validate_half_day(self):
		if self.half_day:
			if not getdate(self.from_date) <= getdate(self.half_day_date) <= getdate(self.to_date):
				artech_engine.throw(_("Half day date should be in between from date and to date"))
	...
	def validate_request_overlap(self):
		...
		Request = artech_engine.qb.DocType("Attendance Request")
		query = (
			artech_engine.qb.from_(Request)
			.select(Request.name)
			.where(
	...
	def throw_overlap_error(self, overlapping_request: str):
		...
		artech_engine.throw(msg, title=_("Overlapping Attendance Request"), exc=OverlappingAttendanceRequestError)

	def on_submit(self):
		self.create_attendance_records()

	def on_cancel(self):
		attendance_list = artech_engine.get_all(
			"Attendance", {"employee": self.employee, "attendance_request": self.name, "docstatus": 1}
		)
		if attendance_list:
			for attendance in attendance_list:
				attendance_obj = artech_engine.get_doc("Attendance", attendance["name"])
		...
		if doc:
			...
			if old_status != status:
				...
				if status == "Half Day":
					...
					).format(artech_engine.bold(old_status), artech_engine.bold(status), artech_engine.bold("Absent"))
				else:
					text = _("Changed the status from {0} to {1} via Attendance Request").format(
						artech_engine.bold(old_status), artech_engine.bold(status)
					)
				doc.add_comment(comment_type="Info", text=text)

				artech_engine.msgprint(
					_("Updated status from {0} to {1} for date {2} in the attendance record {3}").format(
						artech_engine.bold(old_status),
						artech_engine.bold(status),
						artech_engine.bold(format_date(date)),
		...
		if not self.include_holidays and is_holiday(self.employee, attendance_date):
			artech_engine.msgprint(
				_("Attendance not submitted for {0} as it is a Holiday.").format(
					artech_engine.bold(format_date(attendance_date))
				)
			)
			return False
		...
		if self.has_leave_record(attendance_date):
			artech_engine.msgprint(
				_("Attendance not submitted for {0} as {1} is on leave.").format(
					artech_engine.bold(format_date(attendance_date)), artech_engine.bold(self.employee)
				)
			)
			return False
	...
	def get_attendance_doc(self, attendance_date: str) -> str | None:
		attendance = artech_engine.db.exists(
			"Attendance",
			{
				"employee": self.employee,
		...
		return artech_engine.get_doc("Attendance", attendance) if attendance else None
