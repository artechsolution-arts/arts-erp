artech_engine.ui.form.on("Employee Advance", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		if (
			...
			artech_engine.model.can_create("Expense Claim")
		) {
			frm.add_custom_button(
				__("Expense Claim"),
				function () {
		...
		if (
			...
			if (
				...
				artech_engine.model.can_create("Journal Entry")
			) {
				frm.add_custom_button(
					__("Return"),
					function () {
				...
				artech_engine.model.can_create("Additional Salary")
			) {
				frm.add_custom_button(
	...
	make_expense_claim: function (frm) {
		return artech_engine.call({
			...
			callback: function (r) {
				const doclist = artech_engine.model.sync(r.message);
				artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
		});
	},
