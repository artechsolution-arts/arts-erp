...
from artech_engine.utils import flt, get_link_to_form, nowdate
...
class EmployeeAdvance(Document):
	...
	def before_submit(self):
		if not self.get("advance_account"):
			default_advance_account = artech_engine.db.get_value(
				"Company", self.company, "default_employee_advance_account"
			)
			same_currency = self.currency == artech.get_company_currency(self.company)
	...
	def validate_advance_account_type(self):
		...
		account_type = artech_engine.db.get_value("Account", self.advance_account, "account_type")
		if not account_type or (account_type != "Receivable"):
			artech_engine.throw(
				_("Employee advance account {0} should be of type {1}.").format(
					get_link_to_form("Account", self.advance_account), artech_engine.bold(_("Receivable"))
				)
			)

	def validate_advance_account_currency(self):
		if self.currency and self.advance_account:
			account_currency = artech_engine.db.get_value("Account", self.advance_account, "account_currency")
			if self.currency != account_currency:
				artech_engine.throw(
					_(
						"Advance Account {} currency should be same as Salary Currency of Employee {}. Please select same currency Advance Account"
					).format(artech_engine.bold(self.advance_account), artech_engine.bold(self.employee))
				)
	...
	def set_total_advance_paid(self):
		aple = artech_engine.qb.DocType("Advance Payment Ledger Entry")

		account_type = artech_engine.get_value("Account", self.advance_account, "account_type")
		...
		if account_type == "Receivable":
			...
			artech_engine.throw(
				_("Employee advance account {0} should be of type {1}.").format(
					...
					artech_engine.bold(_("Receivable")),
				)
			)
		...
		if return_amount > 0 and return_amount > flt(paid_amount - self.claimed_amount, precision):
			artech_engine.throw(_("Return amount cannot be greater than unclaimed amount"))
	...
	def update_claimed_amount(self):
		...
		eca = artech_engine.qb.DocType("Expense Claim Advance")
		...
		artech_engine.db.set_value("Employee Advance", self.name, "claimed_amount", flt(claimed_amount))
		self.reload()
		self.set_status(update=True)
	...
	def check_linked_payment_entry(self):
		...
		if artech_engine.db.get_single_value("HR Settings", "unlink_payment_on_cancellation_of_employee_advance"):
...
def make_bank_entry(dt: str, dn: str) -> dict:
	...
	je.multi_currency = 1 if doc.currency != artech.get_company_currency(doc.company) else 0
	...
	je.append(
		"accounts",
		{
			...
			"party_type": "Employee",
			"cost_center": artech.get_default_cost_center(doc.company),
			"party": doc.employee,
			"is_advance": "Yes",
		},
	)
...
def create_return_through_additional_salary(doc: str | dict | Document) -> Document:
	...
	additional_salary = artech_engine.new_doc("Additional Salary")
...
def make_return_entry(
	...
	advance_account_currency = artech_engine.db.get_value("Account", advance_account, "account_currency")
	...
	je.multi_currency = 1 if advance_account_currency != artech.get_company_currency(company) else 0
...
def get_same_currency_bank_cash_account(company, currency, mode_of_payment=None):
	...
	if not account:
		accounts = artech_engine.get_all(
			"Account",
			filters={
				"company": company,
		...
		if not accounts:
			artech_engine.throw(
				_("No Bank/Cash Account found for currency {0}. Please create one under company {1}.").format(
					artech_engine.bold(currency), company
				),
...
def get_voucher_type(mode_of_payment=None):
	...
	if mode_of_payment:
		mode_of_payment_type = artech_engine.get_cached_value("Mode of Payment", mode_of_payment, "type")
