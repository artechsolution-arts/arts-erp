...
class TestEmployeeAdvance(HRMSTestSuite):
	def setUp(self):
		...
		artech_engine.db.set_value("Account", "_Test Employee Advance - _TC", "account_type", "Receivable")
	...
	def test_partly_claimed_and_returned_status(self):
		...
		entry = artech_engine.get_doc(entry)
		entry.insert()
		entry.submit()
	...
	def test_unlink_payment_entries(self):
		employee_name = make_employee("_T@employee.advance", "_Test Company")
		self.assertTrue(artech_engine.db.exists("Employee", employee_name))
	...
	def test_employee_advance_when_different_account_currency(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "default_employee_advance_account", advance_account)
	...
	def test_employee_advance_when_different_advance_currency(self):
		...
		self.assertRaises(artech_engine.ValidationError, advance.save)

	def update_company_in_fiscal_year(self):
		fy_entries = artech_engine.get_all("Fiscal Year")
		for fy_entry in fy_entries:
			fiscal_year = artech_engine.get_doc("Fiscal Year", fy_entry.name)
...
def make_journal_entry_for_advance(advance):
	artech_engine.db.set_single_value("Accounts Settings", "make_payment_via_journal_entry", True)
	journal_entry = artech_engine.get_doc(make_bank_entry("Employee Advance", advance.name))
...
def make_employee_advance(employee_name, args=None, do_not_submit=False):
	emp_details = artech_engine.db.get_value(
		"Employee", employee_name, ["salary_currency", "employee_advance_account"], as_dict=True
	)
	doc = artech_engine.new_doc("Employee Advance")
	doc.employee = employee_name
	doc.company = "_Test Company"
	doc.purpose = "For site visit"
	doc.currency = emp_details.salary_currency or artech.get_company_currency("_Test company")
	doc.advance_amount = 1000
	doc.posting_date = nowdate()
	doc.advance_account = emp_details.employee_advance_account or "_Test Employee Advance - _TC"
	account_type = artech_engine.db.get_value("Account", "_Test Employee Advance - _TC", "account_type")
	if not account_type:
		artech_engine.db.set_value("Account", "_Test Employee Advance - _TC", "account_type", "Receivable")
