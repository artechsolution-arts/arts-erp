...
def create_job_applicant(source_name: str, target_doc: str | Document | None = None) -> Document:
	emp_ref = artech_engine.get_doc("Employee Referral", source_name)
	...
	job_applicant = artech_engine.new_doc("Job Applicant")
...
def create_additional_salary(employee_referral: str) -> Document:
	doc = artech_engine.get_doc("Employee Referral", employee_referral)

	if not artech_engine.db.exists("Additional Salary", {"ref_docname": doc.name}):
		additional_salary = artech_engine.new_doc("Additional Salary")
		additional_salary.employee = doc.referrer
		additional_salary.company = artech_engine.db.get_value("Employee", doc.referrer, "company")
