...
class LeaveType(Document):
	...
	def validate_leave_types(self):
		...
		if self.is_lwp and self.is_ppl:
			artech_engine.throw(_("Leave Type can either be without pay or partial pay"), title=_("Not Allowed"))

		if self.is_ppl and (
			self.fraction_of_daily_salary_per_leave < 0 or self.fraction_of_daily_salary_per_leave > 1
		):
			artech_engine.throw(_("The fraction of Daily Salary per Leave should be between 0 and 1"))
	...
	def validate_allocated_earned_leave(self):
		...
		if (
			...
			earned_leave_allocation_exists = artech_engine.db.exists(
				"Leave Allocation",
	...
	def clear_cache(self):
		from artech_hrms.payroll.doctype.salary_slip.salary_slip import LEAVE_TYPE_MAP

		artech_engine.cache().delete_value(LEAVE_TYPE_MAP)
