...
from artech_engine.model.workflow import get_workflow_name
...
class AttendanceAlreadyMarkedError(artech_engine.ValidationError):
	pass


class NotAnOptionalHoliday(artech_engine.ValidationError):
	pass


class InsufficientLeaveBalanceError(artech_engine.ValidationError):
	pass


class LeaveAcrossAllocationsError(artech_engine.ValidationError):
	pass
...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def validate(self):
		...
		if artech_engine.db.get_value("Leave Type", self.leave_type, "is_optional_leave"):
	...
	def on_update(self):
		if self.status == "Open" and self.docstatus < 1:
			...
			if artech_engine.db.get_single_value("HR Settings", "send_leave_notification"):
	...
	def on_submit(self):
		if self.status in ["Open", "Cancelled"]:
			artech_engine.throw(_("Only Leave Applications with status 'Approved' and 'Rejected' can be submitted"))
		...
		can_expire = not artech_engine.db.get_value("Leave Type", self.leave_type, "is_carry_forward")
	...
	def validate_applicable_after(self):
		if self.leave_type:
			leave_type = artech_engine.get_doc("Leave Type", self.leave_type)
			if leave_type.applicable_after > 0:
				...
				if number_of_days >= 0:
					holidays = 0
					if not artech_engine.db.get_value("Leave Type", self.leave_type, "include_holiday"):
	...
	def validate_dates(self):
		if artech_engine.db.get_single_value("HR Settings", "restrict_backdated_leave_application"):
			if self.from_date and getdate(self.from_date) < getdate():
				allowed_role = artech_engine.db.get_single_value(
					"HR Settings", "role_allowed_to_create_backdated_leave_application"
				)
				user = artech_engine.get_doc("User", artech_engine.session.user)
				user_roles = [d.role for d in user.roles]
				if not allowed_role:
					artech_engine.throw(
						_("Backdated Leave Application is restricted. Please set the {} in {}").format(
							artech_engine.bold(_("Role Allowed to Create Backdated Leave Application")),
	...
	def validate_dates_across_allocation(self):
		...
		if not (alloc_on_from_date or alloc_on_to_date):
			artech_engine.throw(_("Application period cannot be outside leave allocation period"))
	...
	def get_allocation_based_on_application_dates(self) -> tuple[dict, dict]:
		...
		def _get_leave_allocation_record(date):
			LeaveAllocation = artech_engine.qb.DocType("Leave Allocation")
			allocation = (
				artech_engine.qb.from_(LeaveAllocation)
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance_name = artech_engine.db.exists(
				"Attendance",
				dict(
			...
			if date in holiday_dates:
				if attendance_name:
					...
					attendance = artech_engine.get_doc("Attendance", attendance_name)
					attendance.flags.ignore_permissions = True
					if attendance.docstatus == 1:
						attendance.cancel()
					artech_engine.delete_doc("Attendance", attendance_name, force=1)
				continue
	...
	def create_or_update_attendance(self, attendance_name, date):
		...
		if attendance_name:
			...
			doc = artech_engine.get_doc("Attendance", attendance_name)
			...
			doc = artech_engine.new_doc("Attendance")
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			...
			for name in attendance:
				artech_engine.db.set_value("Attendance", name, "docstatus", 2)

	def validate_salary_processed_days(self):
		if not artech_engine.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			return

		last_processed_pay_slip = artech_engine.db.sql(
			"""
	...
	def show_block_day_warning(self):
		...
		if block_dates:
			artech_engine.msgprint(_("Warning: Leave application contains following block dates") + ":")
			for d in block_dates:
				artech_engine.msgprint(formatdate(d.block_date) + ": " + d.reason)
	...
	def validate_block_days(self):
		...
		if block_dates and self.status == "Approved":
			artech_engine.throw(_("You are not authorized to approve leaves on Block Dates"), LeaveDayBlockedError)

	def validate_balance_leaves(self):
		precision = cint(artech_engine.db.get_single_value("System Settings", "float_precision")) or 2
		...
		if artech_engine.db.get_value("Leave Type", self.leave_type, "allow_negative"):
			if leave_balance_for_consumption != self.leave_balance:
				msg = _("Warning: Insufficient leave balance for Leave Type {0} in this allocation.").format(
					artech_engine.bold(self.leave_type)
				)
				msg += "<br><br>"
				msg += _(
			...
			artech_engine.msgprint(msg, title=_("Warning"), indicator="orange")
		else:
			artech_engine.throw(
				_("Insufficient leave balance for Leave Type {0}").format(artech_engine.bold(self.leave_type)),
	...
	def throw_overlap_error(self, d):
		...
		artech_engine.throw(msg, OverlapError)

	def get_total_leaves_on_half_day(self):
		leave_count_on_half_day_date = artech_engine.db.sql(
	...
	def validate_max_days(self):
		max_days = artech_engine.db.get_value("Leave Type", self.leave_type, "max_continuous_days_allowed")
		if not max_days:
			return
		...
		if details.total_consecutive_leaves > cint(max_days):
			...
			artech_engine.throw(msg, title=_("Maximum Consecutive Leaves Exceeded"))
	...
	def get_consecutive_leave_details(self) -> dict:
		...
		def _get_first_from_date(reference_date):
			...
			application = artech_engine.db.get_value(
				"Leave Application",
				{
					"employee": self.employee,
					"leave_type": self.leave_type,
	...
	def validate_attendance(self):
		attendance_dates = artech_engine.get_all(
			"Attendance",
			filters=[
				["employee", "=", self.employee],
	...
	def validate_optional_leave(self):
		...
		if not leave_period:
			artech_engine.throw(_("Cannot find active Leave Period"))
		optional_holiday_list = artech_engine.db.get_value(
	...
	def notify_employee(self):
		...
		parent_doc = artech_engine.get_doc("Leave Application", self.name)
		args = parent_doc.as_dict()

		template = artech_engine.db.get_single_value("HR Settings", "leave_status_notification_template")
		if not template:
			artech_engine.msgprint(_("Please set default template for Leave Status Notification in HR Settings."))
			return
		email_template = artech_engine.get_doc("Email Template", template)
		subject = artech_engine.render_template(email_template.subject, args)
		message = artech_engine.render_template(email_template.response_, args)
	...
	def notify_leave_approver(self):
		if self.leave_approver:
			...
			template = artech_engine.db.get_single_value("HR Settings", "leave_approval_notification_template")
			if not template:
		...
		if cint(self.follow_via_email):
			...
			if not isinstance(contact, list):
				if not args.notify == "employee":
					contact = artech_engine.get_doc("User", contact).email or contact

			sender = dict()
			sender["email"] = artech_engine.get_doc("User", artech_engine.session.user).email
			...
			try:
				...
				artech_engine.msgprint(_("Email sent to {0}").format(contact))
			except artech_engine.OutgoingEmailError:
				pass
	...
	def create_leave_ledger_entry(self, submit=True):
		...
		lwp = artech_engine.db.get_value("Leave Type", self.leave_type, "is_lwp")
		...
		if expiry_date:
			...
			if self.is_separate_ledger_entry_required(alloc_on_from_date, alloc_on_to_date):
				...
				raise_exception = False if artech_engine.flags.in_patch else True
				args = dict(
	...
	def validate_for_self_approval(self):
		self_leave_approval_not_allowed = artech_engine.db.get_single_value(
			"HR Settings", "prevent_self_leave_approval"
		)
		employee_user = artech_engine.db.get_value("Employee", self.employee, "user_id")
		if (
			self_leave_approval_not_allowed
			and employee_user == artech_engine.session.user
			and not get_workflow_name("Leave Application")
			and self.status == "Approved"
		):
			artech_engine.throw(_("Self-approval for leaves is not allowed"))

	def onload(self):
		self.set_onload(
			"self_leave_approval_not_allowed",
			artech_engine.db.get_single_value("HR Settings", "prevent_self_leave_approval"),
		)
	...
	def validate_half_day_date(self) -> bool:
		...
		if is_holiday(employee=self.employee, date=self.half_day_date):
			artech_engine.throw(_("Half Day Date cannot be a holiday"))

		if not (getdate(self.from_date) <= getdate(self.half_day_date) <= getdate(self.to_date)):
			artech_engine.throw(_("Half Day Date should be between From Date and To Date"))
...
def get_allocation_expiry_for_cf_leaves(
	...
	Ledger = artech_engine.qb.DocType("Leave Ledger Entry")
	expiry = (
		artech_engine.qb.from_(Ledger)
...
def get_leave_metrics_and_details(
	...
	artech_engine.has_permission("Employee", "read", employee, throw=True)
...
def get_number_of_leave_days(
	...
	if not artech_engine.db.get_value("Leave Type", leave_type, "include_holiday"):
...
def get_leave_details(employee: str, date: str | datetime.date, for_salary_slip: bool = False) -> dict:
	...
	for d in allocation_records:
		allocation = allocation_records.get(d, artech_engine._dict())
	...
	lwp = artech_engine.get_list("Leave Type", filters={"is_lwp": 1}, pluck="name")
...
def get_leave_balance_on(
	...
	allocation = allocation_records.get(leave_type, artech_engine._dict())
	end_date = (
...
def get_leave_allocation_records(employee, date, leave_type=None):
	...
	Ledger = artech_engine.qb.DocType("Leave Ledger Entry")
	LeaveAllocation = artech_engine.qb.DocType("Leave Allocation")
	LeaveAdjustment = artech_engine.qb.DocType("Leave Adjustment")

	cf_leave_case = artech_engine.qb.terms.Case().when(Ledger.is_carry_forward == "1", Ledger.leaves).else_(0)
	sum_cf_leaves = Sum(cf_leave_case).as_("cf_leaves")

	new_leaves_case = artech_engine.qb.terms.Case().when(Ledger.is_carry_forward == "0", Ledger.leaves).else_(0)
	...
	allocated_leaves = artech_engine._dict()
...
def get_remaining_leaves(
	...
	return artech_engine._dict(leave_balance=leave_balance, leave_balance_for_consumption=remaining_leaves)
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if leave_entry.leaves % 1:
				half_day = 1
				half_day_date = artech_engine.db.get_value(
...
def is_lwp(leave_type):
	lwp = artech_engine.db.sql("select is_lwp from `tabLeave Type` where name = %s", leave_type)
...
def get_events(start: str, end: str, filters: str | None = None) -> list[dict]:
	...
	employee = artech_engine.db.get_value(
		"Employee", filters={"user_id": artech_engine.session.user}, fieldname=["name", "company"], as_dict=True
	)
	...
	if "Employee" in artech_engine.get_roles():
...
def add_department_leaves(events, start, end, employee, company):
	if department := artech_engine.db.get_value("Employee", employee, "department"):
		department_employees = artech_engine.get_list(
...
def add_leaves(events, start, end, filters=None):
	...
	show_leaves_of_all_members = artech_engine.db.get_single_value(
		"HR Settings", "show_leaves_of_all_department_members_in_calendar"
	)
	if cint(show_leaves_of_all_members):
		leave_applications = artech_engine.get_all("Leave Application", filters=filters, fields=fields)
	else:
		leave_applications = artech_engine.get_list("Leave Application", filters=filters, fields=fields)
...
def add_holidays(events, start, end, employee, company):
	...
	for holiday in artech_engine.db.sql(
...
def get_mandatory_approval(doctype: str) -> str | int | bool:
	...
	if doctype == "Leave Application":
		mandatory = artech_engine.db.get_single_value("HR Settings", "leave_approver_mandatory_in_leave_application")
	else:
		mandatory = artech_engine.db.get_single_value("HR Settings", "expense_approver_mandatory_in_expense_claim")
...
def get_leave_approver(employee: str) -> str:
	leave_approver, department = artech_engine.db.get_value("Employee", employee, ["leave_approver", "department"])

	if not leave_approver and department:
		leave_approver = artech_engine.db.get_value(
			"Department Approver",
...
def on_doctype_update():
	artech_engine.db.add_index("Leave Application", ["employee", "from_date", "to_date"])
...
def validate_leave_access(employee):
	employee_user = artech_engine.db.get_value("Employee", employee, "user_id")
	leave_approver = get_leave_approver(employee)

	if artech_engine.session.user not in (employee_user, leave_approver) and (
		not artech_engine.has_permission("Employee", "read", employee)
	):
