artech_engine.ui.form.on("Leave Application", {
	setup: function (frm) {
	...
	onload: function (frm) {
		...
		if (!frm.doc.posting_date) {
			frm.set_value("posting_date", artech_engine.datetime.get_today());
		}
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.employee) {
			...
			frm.dashboard.add_section(
				artech_engine.render_template("leave_application_dashboard", {
					data: leave_details,
				}),
				__("Allocated Leaves"),
			);
	...
	refresh: function (frm) {
		...
		frm.set_intro("");
		if (frm.doc.__islocal && !in_list(artech_engine.user_roles, "Employee")) {
	...
	async set_employee(frm) {
		...
		const employee = await artech_hrms.get_current_employee(frm);
		if (employee) {
	...
	leave_approver: function (frm) {
		if (frm.doc.leave_approver) {
			frm.set_value("leave_approver_name", artech_engine.user.full_name(frm.doc.leave_approver));
		}
	},
		...
		if (to_date < from_date) {
			...
			artech_engine.show_alert({
				message: __("Changing '{0}' to {1}.", [
					...
					artech_engine.datetime.str_to_user(frm.doc[updated_field]),
				]),
				indicator: "blue",
			});
		}
	...
	half_day_datepicker: function (frm) {
		...
		half_day_datepicker.update({
			minDate: artech_engine.datetime.str_to_obj(frm.doc.from_date),
			maxDate: artech_engine.datetime.str_to_obj(frm.doc.to_date),
		});
	},
	...
	get_leave_balance: function (frm) {
		if (
			...
			return artech_engine.call({
				method: "artech_hrms.hr.doctype.leave_application.leave_application.get_leave_balance_on",
				args: {
					employee: frm.doc.employee,
	...
	calculate_total_days: function (frm) {
		if (frm.doc.from_date && frm.doc.to_date && frm.doc.employee && frm.doc.leave_type) {
			...
			return artech_engine.call({
				method: "artech_hrms.hr.doctype.leave_application.leave_application.get_number_of_leave_days",
				args: {
					employee: frm.doc.employee,
					leave_type: frm.doc.leave_type,
	...
	set_leave_approver: function (frm) {
		if (frm.doc.employee) {
			return artech_engine.call({
				method: "artech_hrms.hr.doctype.leave_application.leave_application.get_leave_approver",
				args: {
					employee: frm.doc.employee,
				},
				callback: function (r) {
...
artech_engine.tour["Leave Application"] = [
	{
		fieldname: "employee",
		title: "Employee",
