...
from artech_hrms.hr.doctype.shift_request.shift_request import OverlappingShiftRequestError
...
class TestShiftRequest(HRMSTestSuite):
	def setUp(self):
		for doctype in ["Shift Request", "Shift Assignment", "Shift Type"]:
			artech_engine.db.delete(doctype)

	def test_make_shift_request(self):
		"Test creation/updation of Shift Assignment from Shift Request."
		setup_shift_type(shift_type="Day Shift")
		department = artech_engine.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		approver = artech_engine.db.sql(
		...
		shift_assignment = artech_engine.db.get_value(
			"Shift Assignment",
		...
		shift_assignment_docstatus = artech_engine.db.get_value(
	...
	def test_shift_request_approver_perms(self):
		setup_shift_type(shift_type="Day Shift")
		employee = artech_engine.get_doc("Employee", "_T-Employee-00001")
		...
		self.assertTrue(shift_request.name in artech_engine.share.get_shared("Shift Request", user))

		# check shared doc revoked
		shift_request.reload()
		department = artech_engine.get_value("Employee", "_T-Employee-00001", "department")
		set_shift_approver(department)
		department_approver = artech_engine.db.sql(
			"""select approver from `tabDepartment Approver` where parent= %s and parentfield = 'shift_request_approver'""",
			(department),
		)[0][0]
		shift_request.approver = department_approver
		shift_request.save()
		self.assertTrue(shift_request.name not in artech_engine.share.get_shared("Shift Request", user))
	...
	def test_overlap_for_request_without_to_date(self):
		...
		shift_request = artech_engine.get_doc(
			{
...
def set_shift_approver(department):
	department_doc = artech_engine.get_doc("Department", department)
...
def make_shift_request(
	...
	approver = approver or artech_engine.db.get_value("Employee", employee, "shift_request_approver")
