...
from artech_engine.query_builder import Case, Criterion, Interval
...
class ShiftAssignmentTool(Document):
	...
	def get_employees_for_assigning_shift(self, filters):
		...
		query = artech_engine.qb.get_query(
			Employee,
			fields=[
				Employee.employee,
		...
		self.allow_multiple_shifts = artech_engine.db.get_single_value(
	...
	def get_shift_requests(self, filters):
		Employee = artech_engine.qb.DocType("Employee")
		ShiftRequest = artech_engine.qb.DocType("Shift Request")
		query = (
	...
	def get_query_for_employees_with_same_shift_schedule(self):
		days = artech_engine.get_all("Assignment Rule Day", {"parent": self.shift_schedule}, pluck="day")

		ShiftScheduleAssignment = artech_engine.qb.DocType("Shift Schedule Assignment")
		ShiftSchedule = artech_engine.qb.DocType("Shift Schedule")
		Day = artech_engine.qb.DocType("Assignment Rule Day")

		query = (
			artech_engine.qb.from_(ShiftScheduleAssignment)
		...
		if self.allow_multiple_shifts:
			shift_type = artech_engine.db.get_value("Shift Schedule", self.shift_schedule, "shift_type")
	...
	def get_query_checking_overlapping_shift_timings(self, query, doctype, shift_type):
		shift_start, shift_end = artech_engine.db.get_value("Shift Type", shift_type, ["start_time", "end_time"])
	...
	def bulk_assign(self, employees: list):
		if self.action == "Assign Shift":
			...
			artech_engine.throw(_("Invalid Action"))
		...
		artech_engine.enqueue(self._bulk_assign, timeout=3000, employees=employees)
	...
	def _bulk_assign(self, employees: list):
		...
		for d in employees:
			...
			count += 1
			artech_engine.publish_progress(count * 100 / len(employees), title=_("Creating {0}...").format(doctype))
	...
	def _bulk_process_shift_requests(self, shift_requests: list, status: str):
		...
		for d in shift_requests:
			try:
				shift_request = artech_engine.get_doc("Shift Request", d["shift_request"])
			...
			count += 1
			artech_engine.publish_progress(count * 100 / len(shift_requests), title=_("Processing Requests..."))
	...
	def create_shift_schedule_assignment(self, employee: str) -> str:
		assignment = artech_engine.new_doc("Shift Schedule Assignment")
...
def create_shift_assignment(
	...
	assignment = artech_engine.new_doc("Shift Assignment")
	assignment.employee = employee
