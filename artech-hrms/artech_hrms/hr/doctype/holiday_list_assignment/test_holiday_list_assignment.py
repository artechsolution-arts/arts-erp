...
from artech_engine.utils import add_months, get_year_ending, get_year_start, getdate
...
def create_holiday_list_assignment(
	...
	if not artech_engine.db.exists(
		...
		if not from_date:
			from_date = artech_engine.db.get_value("Holiday List", holiday_list, "from_date")
...
def assign_holiday_list(holiday_list, company_name):
	...
	HolidayListAssignment = artech_engine.qb.DocType("Holiday List Assignment")
	try:
		previous_assignment = (
			artech_engine.qb.from_(HolidayListAssignment)
			.join(HolidayList)
		...
		from_date = artech_engine.get_value("Holiday List", holiday_list, "from_date")
