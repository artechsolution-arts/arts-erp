artech_engine.query_reports["Monthly Attendance Sheet"] = {
	filters: [
		{
		...
		},
		{
			...
			default: artech_engine.datetime.str_to_obj(artech_engine.datetime.get_today()).getMonth() + 1,
	...
	onload: function () {
		return artech_engine.call({
			method: "artech_hrms.hr.report.monthly_attendance_sheet.monthly_attendance_sheet.get_attendance_years",
			callback: function (r) {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		const summarized_view = artech_engine.query_report.get_filter_value("summarized_view");
		const group_by = artech_engine.query_report.get_filter_value("group_by");
...
function validate_date_range(report) {
	let start_date = artech_engine.query_report.get_filter_value("start_date");
	let end_date = artech_engine.query_report.get_filter_value("end_date");
	if (!(start_date && end_date)) return;

	let start = artech_engine.datetime.str_to_obj(start_date);
	let end = artech_engine.datetime.str_to_obj(end_date);
