...
from artech_hrms.hr.doctype.vehicle_log.test_vehicle_log import get_vehicle, make_vehicle_log
from artech_hrms.hr.doctype.vehicle_log.vehicle_log import make_expense_claim
from artech_hrms.hr.report.vehicle_expenses.vehicle_expenses import execute
from artech_hrms.tests.utils import HRMSTestSuite


class TestVehicleExpenses(HRMSTestSuite):
	def setUp(self):
		employee_id = artech_engine.db.sql("""select name from `tabEmployee` where name='testdriver@example.com'""")
	...
	def test_vehicle_expenses_based_on_fiscal_year(self):
		...
		# clean up
		vehicle_log.cancel()
		artech_engine.delete_doc("Expense Claim", expense_claim.name)
		artech_engine.delete_doc("Vehicle Log", vehicle_log.name)
