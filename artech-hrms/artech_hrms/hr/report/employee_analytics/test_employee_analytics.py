...
class TestEmployeeAnalytics(HRMSTestSuite):
	...
	def test_branches(self):
		...
		filters = artech_engine._dict({"company": self.company, "parameter": "Branch"})
	...
	def test_employee_grade(self):
		...
		filters = artech_engine._dict({"company": self.company, "parameter": "Grade"})
		report = execute(filters=filters)
...
def get_employees_without_set_parameter(parameter, company):
	return artech_engine.db.count("Employee", {parameter: ("is", "not set"), "company": company, "status": "Active"})
