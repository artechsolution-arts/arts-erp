...
from artech_engine.utils import add_days, add_months, flt, get_year_ending, get_year_start, getdate
...
class TestLeaveLedger(HRMSTestSuite):
	def setUp(self):
		...
		artech_engine.db.delete("Employee", {"company": "_Test Company"})
		...
		self.employee_1 = artech_engine.get_doc(
		...
		self.employee_2 = artech_engine.get_doc(
	...
	def create_earned_leave_allocation(self):
		...
		artech_engine.flags.current_date = add_months(self.year_start, 2)
		# 3 leaves allocated
		...
		self.earned_leave_allocation = artech_engine.get_doc("Leave Allocation", allocation)
