...
from artech_engine.utils import format_datetime
...
class TestShiftAttendance(HRMSTestSuite):
	...
	def test_user_permission_on_attendance_records(self):
		manager = artech_engine.get_doc("Employee", {"user_id": "employee1@example.com"})
		assistant = artech_engine.get_doc("Employee", {"user_id": "employee2@example.com"})
		...
		artech_engine.set_user("employee1@example.com")  # only see their own records
		report = execute(filters)
		chart_data = get_chart_data(report)
		...
		artech_engine.set_user("employee2@example.com")  # only see their own records
		report = execute(filters)
		chart_data = get_chart_data(report)
		...
		artech_engine.set_user("employee1@example.com")  # see their own and their reporter's records
		report = execute(filters)
		chart_data = get_chart_data(report)
