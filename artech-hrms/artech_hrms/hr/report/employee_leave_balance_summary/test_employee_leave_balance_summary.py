...
from artech_engine.utils import add_days, flt, get_year_ending, get_year_start, getdate
...
class TestEmployeeLeaveBalance(HRMSTestSuite):
	...
	def test_employee_leave_balance_summary(self):
		artech_engine.get_doc(test_records[0]).insert()
		...
		expected_data = [
			[
				...
				artech_engine.db.get_value("Employee", self.employee_id, "department"),
				flt(
	...
	def test_employee_status_filter(self):
		...
		artech_engine.db.set_value("Employee", inactive_emp, "status", "Inactive")
