...
def execute(filters: Filters | None = None) -> tuple:
	if filters.to_date <= filters.from_date:
		artech_engine.throw(_('"From Date" can not be greater than or equal to "To Date"'))
...
def get_data(filters: Filters) -> list:
	...
	for leave_type in leave_types:
		if consolidate_leave_types:
			...
			row = artech_engine._dict({"leave_type": leave_type})
...
def get_leave_types() -> list[str]:
	...
	return (artech_engine.qb.from_(LeaveType).select(LeaveType.name).orderby(LeaveType.name)).run(pluck="name")


def get_employees(filters: Filters) -> list[dict]:
	Employee = artech_engine.qb.DocType("Employee")
	query = artech_engine.qb.from_(Employee).select(
		Employee.name,
	...
	for key, group in groupby(employee_data, lambda x: x["employee_name"]):
		for grp in group:
			if grp.closing_balance:
				leaves.append(
					artech_engine._dict({"leave_type": grp.leave_type, "closing_balance": grp.closing_balance})
				)
