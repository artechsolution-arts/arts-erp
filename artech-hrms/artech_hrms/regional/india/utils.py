...
def calculate_annual_eligible_hra_exemption(doc):
	basic_component, hra_component = artech_engine.db.get_value(
	...
	if hra_component and basic_component:
		...
		if not assignments and doc.docstatus == 1:
			artech_engine.throw(_("Salary Structure must be submitted before submission of {0}").format(doc.doctype))

		period_start_date = artech_engine.db.get_value("Payroll Period", doc.payroll_period, "start_date")
		...
		for idx, assignment in enumerate(assignments):
			if has_hra_component(assignment.salary_structure, hra_component):
				...
				frequency = artech_engine.get_value(
...
def get_end_date_for_assignment(assignment_dates, idx, payroll_period):
	...
	if not end_date:
		end_date = artech_engine.db.get_value("Payroll Period", payroll_period, "end_date")
...
def validate_house_rent_dates(doc):
	if not doc.rented_to_date or not doc.rented_from_date:
		artech_engine.throw(_("House rented dates required for exemption calculation"))

	if date_diff(doc.rented_to_date, doc.rented_from_date) < 14:
		artech_engine.throw(_("House rented dates should be atleast 15 days apart"))

	proofs = artech_engine.db.sql(
		"""
		select name
	...
	if proofs:
		artech_engine.throw(_("House rent paid days overlapping with {0}").format(proofs[0][0]))
