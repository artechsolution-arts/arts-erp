...
from artech_engine.utils import add_days, add_months, comma_sep, getdate, today
...
def send_reminders_in_advance_weekly():
	to_send_in_advance = int(artech_engine.db.get_single_value("HR Settings", "send_holiday_reminders"))
	frequency = artech_engine.db.get_single_value("HR Settings", "frequency")
...
def send_advance_holiday_reminders(frequency):
	...
	employees = artech_engine.db.get_all("Employee", filters={"status": "Active"}, pluck="name")
...
def send_birthday_reminders():
	...
	to_send = int(artech_engine.db.get_single_value("HR Settings", "send_birthday_reminders"))
	if not to_send:
		return
...
def get_birthday_reminder_text_and_message(birthday_persons):
	if len(birthday_persons) == 1:
		...
		birthday_person_text = comma_sep(person_names, artech_engine._("{0} & {1}"), False)
...
def get_employees_having_an_event_today(event_type):
	...
	employees_born_today = artech_engine.db.multisql(
		{
			"mariadb": f"""
...
def send_work_anniversary_reminders():
	...
	to_send = int(artech_engine.db.get_single_value("HR Settings", "send_work_anniversary_reminders"))
	if not to_send:
		return
...
def get_sender_email() -> str | None:
	return artech_engine.db.get_single_value("HR Settings", "sender_email")
