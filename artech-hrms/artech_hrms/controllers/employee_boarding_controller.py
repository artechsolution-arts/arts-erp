...
from artech_engine.utils import add_days, flt, unique
...
class EmployeeBoardingController(Document):
	...
	def create_task_and_notify_user(self):
		...
		for activity in self.activities:
			...
			if activity.role:
				user_list = artech_engine.db.sql_list(
					"""
					SELECT
		...
		if self.doctype == "Employee Separation":
			...
			if self.employee:
				...
				if not self.holiday_list:
					artech_engine.throw(_("Please set the Holiday List."), artech_engine.MandatoryError)
				else:
	...
	def on_cancel(self):
		# delete task project
		project = self.project
		for task in artech_engine.get_all("Task", filters={"project": project}):
			artech_engine.delete_doc("Task", task.name, force=1)
		artech_engine.delete_doc("Project", project, force=1)
		self.db_set("project", "")
		for activity in self.activities:
...
def update_employee_boarding_status(project, event=None):
	employee_onboarding = artech_engine.db.exists("Employee Onboarding", {"project": project.name})
	employee_separation = artech_engine.db.exists("Employee Separation", {"project": project.name})
	...
	if employee_onboarding:
		artech_engine.db.set_value("Employee Onboarding", employee_onboarding, "boarding_status", status)
	elif employee_separation:
		artech_engine.db.set_value("Employee Separation", employee_separation, "boarding_status", status)


def update_task(task, event=None):
	if task.project and not task.flags.from_project:
		update_employee_boarding_status(artech_engine.get_cached_doc("Project", task.project))
