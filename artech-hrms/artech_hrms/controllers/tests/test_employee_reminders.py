...
class TestEmployeeReminders(HRMSTestSuite):
	def setUp(self):
		from artech.setup.doctype.holiday_list.test_holiday_list import make_holiday_list
		...
		test_employee = artech_engine.get_doc("Employee", make_employee("test@gopher.io", company="_Test Company"))
		...
		test_employee_2 = artech_engine.get_doc("Employee", test_employee_2, company="_Test Company")
		...
		artech_engine.db.sql("delete from `tabEmail Queue Recipient`")
	...
	def test_birthday_reminders(self):
		employee = artech_engine.get_doc("Employee", artech_engine.db.sql_list("select name from tabEmployee limit 1")[0])
		employee.date_of_birth = "1992" + artech_engine.utils.nowdate()[4:]
		...
		hr_settings = artech_engine.get_doc("HR Settings", "HR Settings")
		hr_settings.send_birthday_reminders = 1
		hr_settings.save()

		send_birthday_reminders()

		email_queue = artech_engine.db.sql("""select * from `tabEmail Queue`""", as_dict=True)
	...
	def test_advance_holiday_reminders_monthly(self):
		...
		recipients = artech_engine.db.get_all("Email Queue Recipient", pluck="recipient")
