...
from artech_engine.utils import add_days, date_diff
...
def get_default_company() -> str:
	return artech_engine.defaults.get_user_default("Company")
...
def get_schedule_from_assignment(shift_schedule_assignment: str):
	shift_schedule = artech_engine.db.get_value(
		"Shift Schedule Assignment", shift_schedule_assignment, "shift_schedule"
	)
	frequency = artech_engine.db.get_value("Shift Schedule", shift_schedule, "frequency")
	repeat_on_days = artech_engine.get_all("Assignment Rule Day", filters={"parent": shift_schedule}, pluck="day")
...
def create_shift_schedule_assignment(
	...
	shift_schedule_assignment = artech_engine.get_doc(
		{
...
def delete_shift_schedule_assignment(shift_schedule_assignment: str) -> None:
	for shift_assignment in artech_engine.get_all(
		"Shift Assignment", {"shift_schedule_assignment": shift_schedule_assignment}, pluck="name"
	):
		doc = artech_engine.get_doc("Shift Assignment", shift_assignment)
		if doc.docstatus == 1:
			doc.cancel()
		artech_engine.delete_doc("Shift Assignment", shift_assignment)
	artech_engine.delete_doc("Shift Schedule Assignment", shift_schedule_assignment)
...
def swap_shift(
	...
	if src_shift == tgt_shift:
		artech_engine.throw(_("Source and target shifts cannot be the same"))

	if tgt_shift:
		tgt_shift_doc = artech_engine.get_doc("Shift Assignment", tgt_shift)
		tgt_company = tgt_shift_doc.company
		break_shift(tgt_shift_doc, tgt_date)
	else:
		tgt_company = artech_engine.db.get_value("Employee", tgt_employee, "company")

	src_shift_doc = artech_engine.get_doc("Shift Assignment", src_shift)
...
def break_shift(assignment: str | ShiftAssignment, date: str) -> None:
	if isinstance(assignment, str):
		assignment = artech_engine.get_doc("Shift Assignment", assignment)

	if assignment.end_date and date_diff(assignment.end_date, date) < 0:
		artech_engine.throw(_("Cannot break shift after end date"))
	if date_diff(assignment.start_date, date) > 0:
		artech_engine.throw(_("Cannot break shift before start date"))
...
def insert_shift(
	...
	prev_shift = artech_engine.db.exists(dict({"end_date": add_days(start_date, -1)}, **filters))
	next_shift = (
		artech_engine.db.exists(dict({"start_date": add_days(end_date, 1)}, **filters)) if end_date else None
	)

	if prev_shift:
		if next_shift:
			end_date = artech_engine.db.get_value("Shift Assignment", next_shift, "end_date")
			artech_engine.db.set_value("Shift Assignment", next_shift, "docstatus", 2)
			artech_engine.delete_doc("Shift Assignment", next_shift)
		artech_engine.db.set_value("Shift Assignment", prev_shift, "end_date", end_date or None)

	elif next_shift:
		artech_engine.db.set_value("Shift Assignment", next_shift, "start_date", start_date)
...
def get_holidays(month_start: str, month_end: str, employee_filters: dict[str, str]) -> dict[str, list[dict]]:
	...
	for employee in artech_engine.get_list("Employee", filters=employee_filters, pluck="name"):
		if not (
		...
		if holiday_list not in holiday_lists:
			holiday_lists[holiday_list] = artech_engine.get_all(
				"Holiday",
