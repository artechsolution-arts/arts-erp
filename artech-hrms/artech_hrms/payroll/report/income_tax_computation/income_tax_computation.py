...
class IncomeTaxComputationReport:
	def __init__(self, filters=None):
		...
		self.columns = []
		self.data = []
		self.employees = artech_engine._dict()
		self.payroll_period_start_date = None
		self.payroll_period_end_date = None
		if self.filters.payroll_period:
			self.payroll_period_start_date, self.payroll_period_end_date = artech_engine.db.get_value(
	...
	def get_employee_details(self):
		...
		employees = artech_engine.get_all("Employee", filters=filters, fields=fields)
		...
		if not self.employees:
			artech_engine.throw(_("No employees found with selected filters and active salary structure"))
	...
	def get_ss_assignments(self, employees):
		ss_assignments = artech_engine.get_all(
		...
		employee_ss_assignments = artech_engine._dict()
		for d in ss_assignments:
			if d.employee not in list(employee_ss_assignments.keys()):
				tax_slab = artech_engine.get_cached_value(
	...
	def get_future_salary_slips(self):
		self.future_salary_slips = artech_engine._dict()
		for employee in list(self.employees.keys()):
			...
			while getdate(ss_start_date) < getdate(self.payroll_period_end_date) and (
				...
				ss = artech_engine.new_doc("Salary Slip")
	...
	def get_last_salary_slip(self, employee):
		last_salary_slip = artech_engine.db.get_value(
			"Salary Slip",
			{
				"employee": employee,
				"docstatus": 1,
	...
	def get_gross_earnings(self):
		...
		existing_ss = artech_engine._dict(
			(
	...
	def get_tax_exempted_earnings_and_deductions(self):
		...
		ss = artech_engine.qb.DocType("Salary Slip")
		ss_comps = artech_engine.qb.DocType("Salary Detail")
		...
		existing_ss_exemptions = artech_engine._dict()
		for d in records:
	...
	def get_employee_tax_exemptions(self):
		# add columns
		exemption_categories = artech_engine.get_all("Employee Tax Exemption Category", {"is_active": 1})
	...
	def get_tax_exemptions(self, source):
		...
		par = artech_engine.qb.DocType(source)
		child = artech_engine.qb.DocType(child_doctype)
	...
	def get_hra(self):
		if not artech_engine.get_meta("Employee Tax Exemption Declaration").has_field("monthly_house_rent"):
			return
	...
	def get_applicable_tax(self):
		...
		is_tax_rounded = artech_engine.db.get_value(
			"Salary Component",
		...
		for emp, emp_details in self.employees.items():
			...
			if tax_slab:
				tax_slab = artech_engine.get_cached_doc("Income Tax Slab", tax_slab)
	...
	def get_data_for_eval(self, emp: str, emp_details: dict) -> tuple:
		...
		if last_ss:
			salary_slip = artech_engine.get_cached_doc("Salary Slip", last_ss.name)
		else:
			salary_slip = artech_engine.new_doc("Salary Slip")
			...
			salary_slip.payroll_frequency = artech_engine.db.get_value(
	...
	def get_total_deducted_tax(self):
		...
		tax_components = (
			artech_engine.qb.from_(SalaryComponent)
		...
		ss = artech_engine.qb.DocType("Salary Slip")
		ss_ded = artech_engine.qb.DocType("Salary Detail")
