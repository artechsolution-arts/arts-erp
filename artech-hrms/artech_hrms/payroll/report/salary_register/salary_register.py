...
salary_slip = artech_engine.qb.DocType("Salary Slip")
salary_detail = artech_engine.qb.DocType("Salary Detail")
salary_component = artech_engine.qb.DocType("Salary Component")
...
def execute(filters=None):
	...
	for ss in salary_slips:
		...
		for e in earning_types:
			row.update({artech_engine.scrub(e): ss_earning_map.get(ss.name, {}).get(e)})

		for d in ded_types:
			row.update({artech_engine.scrub(d): ss_ded_map.get(ss.name, {}).get(d)})
...
def get_columns(earning_types, ded_types):
	...
	for earning in earning_types:
		columns.append(
			{
				"label": earning,
				"fieldname": artech_engine.scrub(earning),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
	...
	for deduction in ded_types:
		columns.append(
			{
				"label": deduction,
				"fieldname": artech_engine.scrub(deduction),
				"fieldtype": "Currency",
				"options": "currency",
				"width": 120,
			}
...
def get_salary_components(salary_slips):
	return (
		artech_engine.qb.from_(salary_detail)
...
def get_salary_component_type(salary_component):
	return artech_engine.db.get_value("Salary Component", salary_component, "type", cache=True)


def get_salary_slips(filters, company_currency):
	doc_status = {"Draft": 0, "Submitted": 1, "Cancelled": 2}

	query = artech_engine.qb.from_(salary_slip).select(salary_slip.star)
...
def get_employee_doj_map():
	...
	result = (artech_engine.qb.from_(employee).select(employee.name, employee.date_of_joining)).run()
...
def get_salary_slip_details(salary_slips, currency, company_currency, component_type):
	...
	result = (
		artech_engine.qb.from_(salary_slip)
		.join(salary_detail)
	...
	for d in result:
		ss_map.setdefault(d.parent, artech_engine._dict()).setdefault(d.salary_component, 0.0)
		if currency == company_currency:
