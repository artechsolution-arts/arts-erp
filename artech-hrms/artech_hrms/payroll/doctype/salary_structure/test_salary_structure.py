...
from artech_engine.utils import add_years, cstr, date_diff, get_first_day, nowdate
...
class TestSalaryStructure(HRMSTestSuite):
	...
	def test_amount_totals(self):
		artech_engine.db.set_single_value("Payroll Settings", "include_holidays_in_total_working_days", 0)
		emp_id = make_employee("test_employee_2@salary.com", company="_Test Company")
		salary_slip = artech_engine.get_value("Salary Slip", {"employee": emp_id})
	...
	def test_salary_structures_assignment(self):
		...
		salary_structure_assignment = artech_engine.get_doc(
	...
	def test_employee_grade_defaults(self):
		...
		structure, base = artech_engine.db.get_value(
...
def make_salary_structure(
	...
	if (
		employee
		and not artech_engine.db.get_value("Salary Structure Assignment", filters)
...
def create_salary_structure_assignment(
	...
	if not allow_duplicate and artech_engine.db.exists("Salary Structure Assignment", {"employee": employee}):
		artech_engine.db.sql("""delete from `tabSalary Structure Assignment` where employee=%s""", (employee))

	if not payroll_period:
		payroll_period = create_payroll_period(company="_Test Company")

	income_tax_slab = artech_engine.db.get_value("Income Tax Slab", {"currency": currency, "docstatus": 1})
	...
	salary_structure_assignment = artech_engine.new_doc("Salary Structure Assignment")
	...
	if not from_date:
		...
		joining_date = artech_engine.get_cached_value("Employee", employee, "date_of_joining")
...
def get_payable_account(company=None):
	...
	return artech_engine.db.get_value("Company", company, "default_payroll_payable_account")
