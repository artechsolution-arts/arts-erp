artech_engine.ui.form.on("Salary Structure", {
	onload: function (frm) {
		frm.alerted_rows = [];
		...
		let help_button = $(`<a class = 'control-label'>
			...
			let message_html = artech_engine.render_template("condition_and_formula_help");
	...
	mode_of_payment: function (frm) {
		artech.accounts.pos.get_payment_mode_account(
			frm,
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				__("Bulk Assignments"),
				() => {
					const doc = artech_engine.model.get_new_doc("Bulk Salary Structure Assignment");
					doc.salary_structure = frm.doc.name;
					doc.company = frm.doc.company;
					artech_engine.set_route("Form", "Bulk Salary Structure Assignment", doc.name);
				},
				__("Create"),
			);
			...
			frm.add_custom_button(
				__("Income Tax Slab"),
				() => {
					artech_engine.model.with_doctype("Income Tax Slab", () => {
						const doc = artech_engine.model.get_new_doc("Income Tax Slab");
						artech_engine.set_route("Form", "Income Tax Slab", doc.name);
					});
				},
				__("Create"),
			);
...
var validate_date = function (frm, cdt, cdn) {
	...
	if (doc.to_date && doc.from_date) {
		var from_date = artech_engine.datetime.str_to_obj(doc.from_date);
		var to_date = artech_engine.datetime.str_to_obj(doc.to_date);

		if (to_date < from_date) {
			artech_engine.model.set_value(cdt, cdn, "to_date", "");
			artech_engine.throw(__("From Date cannot be greater than To Date"));
		}
	}
};
...
artech_engine.ui.form.on("Salary Detail", {
	...
	amount_based_on_formula: function (frm, cdt, cdn) {
		...
		if (child.amount_based_on_formula == 1) {
			artech_engine.model.set_value(cdt, cdn, "amount", null);
			const index = frm.alerted_rows.indexOf(cdn);
			if (index > -1) frm.alerted_rows.splice(index, 1);
		} else {
			artech_engine.model.set_value(cdt, cdn, "formula", null);
		}
	},
});
