...
from artech_engine.utils import add_days, add_months, floor, flt, get_datetime, get_first_day, getdate
...
class TestGratuity(HRMSTestSuite):
	...
	def test_gratuity_based_on_current_slab_via_additional_salary(self):
		...
		# amount calculation
		component_amount = artech_engine.get_all(
			"Salary Detail",
			filters={
				"docstatus": 1,
		...
		self.assertTrue(artech_engine.db.exists("Additional Salary", {"ref_docname": gratuity.name}))
	...
	def test_gratuity_based_on_all_previous_slabs_via_payment_entry(self):
		...
		for slab in [
			...
			new_slab = artech_engine.get_doc(
				{
	...
	def test_gratuity_amount_consistent_irrespective_of_payment_days(self):
		...
		artech_engine.db.delete("Gratuity", gratuity.name)
...
def setup_gratuity_rule(name: str) -> dict:
	...
	rule = artech_engine.get_doc("Gratuity Rule", name)
...
def create_gratuity(do_not_submit=False, **args):
	...
	gratuity = artech_engine.new_doc("Gratuity")
...
def set_mode_of_payment_account():
	if not artech_engine.db.exists("Account", "Payment Account - _TC"):
