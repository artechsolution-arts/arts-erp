...
from artech_engine.utils import cstr, flt, get_datetime, get_link_to_form
...
class Gratuity(AccountsController):
	...
	def gratuity_settings(self):
		if not hasattr(self, "_gratuity_settings"):
			self._gratuity_settings = artech_engine.db.get_value(
				"Gratuity Rule",
				self.gratuity_rule,
				[
	...
	def get_gl_entries(self):
		...
		if self.amount:
			...
			artech_engine.throw(_("Total Amount cannot be zero"))
	...
	def set_total_advance_paid(self):
		...
		if flt(paid_amount) > self.amount:
			artech_engine.throw(_("Row {0}# Paid Amount cannot be greater than Total amount"))
	...
	def get_total_working_days(self) -> float:
		...
		payroll_based_on = artech_engine.db.get_single_value("Payroll Settings", "payroll_based_on") or "Leave"
	...
	def get_non_working_days(self, relieving_date: str, status: str) -> float:
		...
		if status == "On Leave":
			lwp_leave_types = artech_engine.get_all("Leave Type", filters={"is_lwp": 1}, pluck="name")
			filters["leave_type"] = ("IN", lwp_leave_types)

		record = artech_engine.get_all("Attendance", filters=filters, fields=[{"COUNT": "*", "as": "total_lwp"}])
	...
	def get_total_component_amount(self) -> float:
		...
		if not salary_slip:
			artech_engine.throw(_("No Salary Slip found for Employee: {0}").format(bold(self.employee)))
	...
	def get_applicable_components(self) -> list[str]:
		applicable_earning_components = artech_engine.get_all(
...
def get_last_salary_slip(employee: str) -> dict | None:
	salary_slip = artech_engine.db.get_value(
		"Salary Slip", {"employee": employee, "docstatus": 1}, order_by="start_date desc"
	)
	if salary_slip:
		return artech_engine.get_doc("Salary Slip", salary_slip)
