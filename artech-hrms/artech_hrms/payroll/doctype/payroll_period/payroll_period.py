...
def get_payroll_period_days(start_date, end_date, employee, company=None):
	...
	payroll_period = artech_engine.db.sql(
		"""
	...
	if len(payroll_period) > 0:
		...
		if not cint(artech_engine.db.get_single_value("Payroll Settings", "include_holidays_in_total_working_days")):
...
def get_payroll_period(from_date, to_date, company):
	PayrollPeriod = artech_engine.qb.DocType("Payroll Period")

	payroll_period = (
		artech_engine.qb.from_(PayrollPeriod)
