...
class EmployeeBenefitLedger(Document):
	...
	def validate(self):
		type = artech_engine.get_cached_value("Salary Component", self.salary_component, "type")
		if type != "Earning":
			artech_engine.throw(
				_(
...
def create_employee_benefit_ledger_entry(ref_doc, args=None, delete=False):
	...
	for component in components:
		...
		artech_engine.get_doc(entry).insert()


def delete_employee_benefit_ledger_entry(ref_field, ref_value):
	EmployeeBenefitLedger = artech_engine.qb.DocType("Employee Benefit Ledger")
	(
		artech_engine.qb.from_(EmployeeBenefitLedger).delete().where(EmployeeBenefitLedger[ref_field] == ref_value)
	).run()
	return
...
def get_benefit_amount(employee, payroll_period, salary_component):
	...
	EmployeeBenefitLedger = artech_engine.qb.DocType("Employee Benefit Ledger")
	query = (
		artech_engine.qb.from_(EmployeeBenefitLedger)
