...
from artech_engine.utils import add_days, add_to_date, cint, get_link_to_form, getdate
		...
		if duplicate:
			artech_engine.throw(
				_("Salary Withholding {0} already exists for employee {1} for the selected period").format(
					...
					artech_engine.bold(f"{self.employee}: {self.employee_name}"),
				),
...
def get_payroll_frequency(employee: str, posting_date: str | date) -> str | None:
	...
	return artech_engine.db.get_value("Salary Structure", salary_structure, "payroll_frequency")
...
def link_bank_entry_in_salary_withholdings(salary_slips: list[dict], bank_entry: str):
	...
	(
		artech_engine.qb.update(WithholdingCycle)
...
def update_salary_withholding_payment_status(doc: "SalaryWithholding", method: str | None = None):
	...
	WithholdingCycle = artech_engine.qb.DocType("Salary Withholding Cycle")
	withholdings = (
		artech_engine.qb.from_(WithholdingCycle)
...
def _update_payment_status_in_payroll(withholdings: list[dict], cancel: bool = False) -> None:
	...
	PayrollEmployee = artech_engine.qb.DocType("Payroll Employee Detail")
	(
		artech_engine.qb.update(PayrollEmployee)
...
def _update_salary_withholdings(withholdings: list[dict], cancel: bool = False) -> None:
	...
	for withholding in withholdings:
		withholding_doc = artech_engine.get_doc("Salary Withholding", withholding.salary_withholding)
