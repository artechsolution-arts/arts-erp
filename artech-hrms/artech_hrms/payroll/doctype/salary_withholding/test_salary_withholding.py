...
class TestSalaryWithholding(HRMSTestSuite):
	def setUp(self):
		self.company = artech_engine.get_doc("Company", COMPANY_NAME)
		default_payroll_payble_account = artech_engine.get_value(
			"Company", self.company.name, "default_payroll_payable_account"
		)
		artech_engine.db.set_value("Account", default_payroll_payble_account, "account_type", "Payable")
...
def create_salary_withholding(employee: str, from_date: str, number_of_withholding_cycles: int = 0):
	doc = artech_engine.new_doc("Salary Withholding")
	doc.update(
		{
			"employee": employee,
			"from_date": from_date,
