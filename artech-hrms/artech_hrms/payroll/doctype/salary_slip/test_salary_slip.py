...
from artech_engine.utils.make_random import get_random
...
class TestSalarySlip(HRMSTestSuite):
	def setUp(self):
		...
		artech_engine.db.set_single_value("Payroll Settings", "email_salary_slip_to_employee", 0)
		artech_engine.db.set_single_value("HR Settings", "leave_status_notification_template", None)
		artech_engine.db.set_single_value("HR Settings", "leave_approval_notification_template", None)
		create_ss_email_template()
		artech_engine.flags.pop("via_payroll_entry", None)
	...
	def test_employee_status_inactive(self):
		...
		artech_engine.db.sql("""delete from `tabSalary Structure` where name='Test Inactive Employee Salary Slip'""")
	...
	def test_payment_days_based_on_attendance(self):
		...
		artech_engine.db.set_value("Employee", emp_id, {"relieving_date": None, "status": "Active"})

		artech_engine.db.set_value("Leave Type", "Leave Without Pay", "include_holiday", 0)
	...
	def test_payment_days_for_mid_joinee_including_holidays(self):
		...
		artech_engine.delete_doc("Salary Slip", new_ss.name, force=True)
		new_ss = make_employee_salary_slip(
			new_emp_id,
			"Monthly",
			"Test Payment Based On Attendence",
	...
	def test_payment_days_in_salary_slip_based_on_timesheet(self):
		from artech.projects.doctype.timesheet.test_timesheet import make_timesheet
		...
		artech_engine.db.set_value("Employee", emp, {"relieving_date": None, "status": "Active"})
	...
	def test_consider_marked_attendance_on_holidays(self):
		...
		artech_engine.db.set_single_value("Payroll Settings", "consider_marked_attendance_on_holidays", 0)
		ss.save()
		self.assertEqual(ss.total_working_days, no_of_days[0])
	...
	def test_consider_marked_attendance_on_holidays_with_unmarked_attendance(self):
		...
		artech_engine.db.delete("Attendance", {"employee": emp_id, "attendance_date": add_days(first_sunday, 1)})
	...
	def test_consider_marked_attendance_on_holidays_with_half_day_on_holiday(self):
		...
		artech_engine.db.set_single_value("Payroll Settings", "consider_marked_attendance_on_holidays", 1)
		ss.save()
		self.assertEqual(ss.total_working_days, no_of_days[0])
	...
	def test_payment_days(self):
		...
		if date_of_joining.day > 1:
			self.assertRaises(artech_engine.ValidationError, ss.save)
	...
	def test_email_salary_slip(self):
		artech_engine.db.delete("Email Queue")
	...
	def test_year_to_date_computation(self):
		...
		artech_engine.db.sql("DELETE FROM `tabSalary Slip` where employee_name = 'test_ytd@salary.com'")

		create_salary_slips_for_payroll_period(
			applicant, salary_structure.name, payroll_period, deduct_random=False, num=6
		)

		salary_slips = artech_engine.get_all(
			"Salary Slip",
	...
	def test_component_wise_year_to_date_computation(self):
		...
		# clear salary slip for this employee
		artech_engine.db.sql("DELETE FROM `tabSalary Slip` where employee_name = '%s'" % employee_name)
		...
		year_to_date = dict()
		for slip in salary_slips:
			doc = artech_engine.get_doc("Salary Slip", slip.name)
	...
	def test_tax_for_payroll_period(self):
		...
		try:
			...
			raise
		artech_engine.db.sql("""delete from `tabSalary Slip` where employee=%s""", (employee))
		...
		try:
			...
			print("\nSalary Slip - Tax calculation failed on following case\n", data, "\n")
			raise
		artech_engine.db.sql("""delete from `tabAdditional Salary` where employee=%s""", (employee))
	...
	def test_salary_slip_from_timesheet(self):
		...
		timesheet = artech_engine.get_doc("Timesheet", timesheet.name)
	...
	def make_activity_for_employee(self):
		activity_type = artech_engine.get_doc("Activity Type", "_Test Activity Type")
	...
	def test_salary_slip_generation_against_opening_entries_in_ssa(self):
		...
		employee_doc = artech_engine.get_doc("Employee", emp)

		tax_slab = create_tax_slab(
			payroll_period, effective_date="2022-04-01", allow_tax_exemption=True, currency="INR"
		)

		effective_date = artech_engine.db.get_value("Income Tax Slab", tax_slab, "effective_from")

		if effective_date != "2022-04-01":
			artech_engine.db.set_value("Income Tax Slab", tax_slab, "effective_from", "2022-04-01")

		salary_structure_name = "Test Salary Structure for Opening Balance"
		if not artech_engine.db.exists("Salary Structure", salary_structure_name):
		...
		ssa = artech_engine.get_value(
			"Salary Structure Assignment",
			{"employee": emp, "salary_structure": salary_structure_doc.name},
			"name",
		)
		ssa_doc = artech_engine.get_doc("Salary Structure Assignment", ssa)
	...
	def test_income_tax_breakup_fields(self):
		...
		payroll_period = artech_engine.get_doc("Payroll Period", "_Test Payroll Period")
	...
	def test_lwp_calculation_based_on_relieving_date(self):
		...
		artech_engine.db.set_value("Employee", emp_id, {"relieving_date": relieving_date, "status": "Left"})
	...
	def test_zero_value_component(self):
		...
		payroll_period = artech_engine.get_all("Payroll Period", filters={"company": "_Test Company"}, limit=1)
		payroll_period = artech_engine.get_cached_doc("Payroll Period", payroll_period[0].name)
	...
	def test_variable_tax_component(self):
		...
		test_tds = artech_engine.get_doc("Salary Component", "_Test TDS")
		...
		income_tax = artech_engine.get_doc("Salary Component", "_Test Income Tax")
	...
	def test_tax_payable_with_tax_relief_and_marginal_relief_limits(self):
		...
		tax_relief_limit, marginal_relief_limit = artech_engine.db.get_value(
...
class TestSalarySlipSafeEval(HRMSTestSuite):
	def test_safe_eval_for_salary_slip(self):
		...
		self.assertRaises(NameError, _safe_eval, "artech_engine.utils.os.path", {})

		# Doc/dict objects
		user = artech_engine.new_doc("User")
...
def make_employee_salary_slip(
	...
	employee = artech_engine.db.get_value("Employee", emp_id, ["name", "company", "employee_name"], as_dict=True)
	...
	salary_slip_name = artech_engine.db.get_value("Salary Slip", {"employee": emp_id})
	...
	if not salary_slip_name:
		...
		salary_slip = artech_engine.get_doc("Salary Slip", salary_slip_name)
...
def make_salary_component(salary_components, test_tax, company_list=None):
	for salary_component in salary_components:
		if artech_engine.db.exists("Salary Component", salary_component["salary_component"]):
			artech_engine.delete_doc("Salary Component", salary_component["salary_component"], force=True)
		...
		doc = artech_engine.new_doc("Salary Component")
...
def set_salary_component_account(sal_comp, company_list=None):
	...
	if not isinstance(sal_comp, Document):
		sal_comp = artech_engine.get_doc("Salary Component", sal_comp)

	if not sal_comp.get("accounts"):
		for d in company_list:
			company_abbr = artech_engine.get_cached_value("Company", d, "abbr")
...
def create_account(account_name, company, parent_account, account_type=None):
	...
	account = artech_engine.db.get_value("Account", account_name + " - " + company_abbr)
	if not account:
		artech_engine.get_doc(
			{
				"doctype": "Account",
...
def get_tax_paid_in_period(employee):
	tax_paid_amount = artech_engine.db.sql(
...
def create_exemption_declaration(employee, payroll_period):
	create_exemption_category()
	declaration = artech_engine.get_doc(
		{
...
def create_proof_submission(employee, payroll_period, amount):
	...
	proof_submission = artech_engine.get_doc(
		{
...
def create_tax_slab(
	...
	if company:
		currency = artech.get_company_currency(company)
	...
	income_tax_slab_name = artech_engine.db.get_value("Income Tax Slab", {"currency": currency, "docstatus": 1})

	if not income_tax_slab_name:
		income_tax_slab = artech_engine.new_doc("Income Tax Slab")
...
def make_payroll_period(company=None):
	...
	for company in company_based_payroll_period:
		payroll_period = artech_engine.db.get_value(
			"Payroll Period",
			{
				"company": company,
...
def make_holiday_list(
	...
	artech_engine.delete_doc_if_exists("Holiday List", name, force=True)

	holiday_list = artech_engine.get_doc(
		{
...
def make_salary_structure_for_payment_days_based_component_dependency(test_statistical_comp=False):
	...
	if artech_engine.db.exists("Salary Structure", salary_structure):
		artech_engine.db.delete("Salary Structure", salary_structure)
	...
	salary_structure_doc = artech_engine.get_doc(details)
...
def make_salary_slip_for_payment_days_dependency_test(employee, salary_structure):
	...
	salary_slip_name = artech_engine.db.get_value("Salary Slip", {"employee": employee.name})
...
def make_salary_structure_for_timesheet(employee, company=None):
	...
	if not artech_engine.db.exists("Salary Component", "Timesheet Component"):
		artech_engine.get_doc({"doctype": "Salary Component", "salary_component": "Timesheet Component"}).insert()
	...
	if not artech_engine.db.get_value("Salary Structure Assignment", {"employee": employee, "docstatus": 1}):
		artech_engine.db.set_value("Employee", employee, "date_of_joining", add_months(nowdate(), -5))
...
def create_employee_other_income(employee, payroll_period, company):
	other_income = artech_engine.db.get_value(
	...
	if not other_income:
		other_income = artech_engine.get_doc(
			{
...
def create_ss_email_template():
	if not artech_engine.db.exists("Email Template", "Salary Slip"):
		ss_template = artech_engine.get_doc(
			{
...
def clear_cache():
	for key in [
		...
		artech_engine.cache().delete_value(key)
