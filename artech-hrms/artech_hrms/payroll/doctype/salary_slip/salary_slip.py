...
from artech_engine.query_builder.functions import Count, Sum
...
class SalarySlip(TransactionBase):
	...
	def has_custom_naming_series(self):
		if not hasattr(self, "__has_custom_naming_series"):
			self.__has_custom_naming_series = artech_engine.db.exists(
				"Property Setter",
				{
	...
	def joining_date(self):
		if not hasattr(self, "__joining_date"):
			self.__joining_date = artech_engine.get_cached_value(
				"Employee",
				self.employee,
				"date_of_joining",
			)
	...
	def relieving_date(self):
		if not hasattr(self, "__relieving_date"):
			self.__relieving_date = artech_engine.get_cached_value(
				"Employee",
				self.employee,
				"relieving_date",
			)
	...
	def validate(self):
		...
		max_working_hours = artech_engine.db.get_single_value(
	...
	def on_submit(self):
		if self.net_pay < 0:
			artech_engine.throw(_("Net Pay cannot be less than 0"))
		else:
			self.set_status()
			...
			if not artech_engine.flags.via_payroll_entry and not artech_engine.flags.in_patch:
				email_salary_slip = cint(
					artech_engine.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee")
				)
	...
	def update_payment_status_for_gratuity_and_leave_encashment(self):
		additional_salary_docs = artech_engine.db.get_all(
			"Additional Salary",
			filters={
	...
	def on_trash(self):
		from artech_engine.model.naming import revert_series_if_last
	...
	def validate_dates(self):
		...
		if not self.joining_date:
			artech_engine.throw(
				_("Please set the Date Of Joining for employee {0}").format(artech_engine.bold(self.employee_name))
			)

		if date_diff(self.end_date, self.joining_date) < 0:
			artech_engine.throw(_("Cannot create Salary Slip for Employee joining after Payroll Period"))

		if self.relieving_date and date_diff(self.relieving_date, self.start_date) < 0:
			artech_engine.throw(_("Cannot create Salary Slip for Employee who has left before Payroll Period"))

	def is_rounding_total_disabled(self):
		return cint(artech_engine.db.get_single_value("Payroll Settings", "disable_rounded_total"))
	...
	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			...
			for data in self.timesheets:
				if artech_engine.db.get_value("Timesheet", data.time_sheet, "status") == "Payrolled":
					artech_engine.throw(
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			...
			Timesheet = artech_engine.qb.DocType("Timesheet")
			timesheets = (
				artech_engine.qb.from_(Timesheet)
	...
	def check_sal_struct(self):
		ss = artech_engine.qb.DocType("Salary Structure")
		ssa = artech_engine.qb.DocType("Salary Structure Assignment")

		query = (
			artech_engine.qb.from_(ssa)
			.join(ss)
	...
	def get_working_days_details(self, lwp=None, for_preview=0, lwp_days_corrected=None):
		payroll_settings = artech_engine.get_cached_value(
			"Payroll Settings",
			None,
			(
				"payroll_based_on",
		...
		if not cint(payroll_settings.include_holidays_in_total_working_days):
			...
			if working_days < 0:
				artech_engine.throw(_("There are more holidays than working days this month."))

		if not payroll_settings.payroll_based_on:
			artech_engine.throw(_("Please set Payroll based on in Payroll settings"))
	...
	def get_payment_days(self, include_holidays_in_total_working_days):
		...
		if self.relieving_date:
			employee_status = artech_engine.db.get_value("Employee", self.employee, "status")
	...
	def get_holidays_for_employee(self, start_date, end_date):
		...
		holiday_dates = artech_engine.cache().hget(HOLIDAYS_BETWEEN_DATES, key)

		if not holiday_dates:
			holiday_dates = get_holiday_dates_between(holiday_list, start_date, end_date)
			artech_engine.cache().hset(HOLIDAYS_BETWEEN_DATES, key, holiday_dates)
	...
	def get_leave_type_map(self) -> dict:
		...
		def _get_leave_type_map():
			leave_types = artech_engine.get_all(
				"Leave Type",
		...
		return artech_engine.cache().get_value(LEAVE_TYPE_MAP, _get_leave_type_map)
	...
	def set_salary_structure_assignment(self):
		self._salary_structure_assignment = artech_engine.db.get_value(
		...
		if not self._salary_structure_assignment:
			artech_engine.throw(
				_(
					...
					artech_engine.bold(formatdate(self.actual_start_date)),
				)
			)
	...
	def get_future_period_non_taxable_earnings(self):
		salary_slip = artech_engine.copy_doc(self)
	...
	def get_amount_from_formula(self, struct_row, sub_period=1):
		if self.payroll_frequency == "Monthly":
			start_date = artech_engine.utils.add_months(self.start_date, sub_period)
			end_date = artech_engine.utils.add_months(self.end_date, sub_period)
			posting_date = artech_engine.utils.add_months(self.posting_date, sub_period)
			...
			start_date = artech_engine.utils.add_days(self.start_date, days_to_add)
			end_date = artech_engine.utils.add_days(self.end_date, days_to_add)
	...
	def set_salary_structure_doc(self) -> None:
		self._salary_structure_doc = artech_engine.get_cached_doc("Salary Structure", self.salary_structure)
	...
	def add_structure_component(self, struct_row, component_type):
		...
		if struct_row.statistical_component or struct_row.accrual_component:
			...
			remove_if_zero_valued = artech_engine.get_cached_value(
	...
	def get_data_for_eval(self):
		...
		employee = artech_engine.get_cached_doc("Employee", self.employee).as_dict()
	...
	def get_component_abbr_map(self):
		def _fetch_component_values():
			return {
				component_abbr: 0
				for component_abbr in artech_engine.get_all("Salary Component", pluck="salary_component_abbr")
			}

		return artech_engine.cache().get_value(SALARY_COMPONENT_VALUES, generator=_fetch_component_values)
	...
	def add_employee_benefits(self):
		...
		SalaryComponent = artech_engine.qb.DocType("Salary Component")
		EmployeeBenefitDetail = artech_engine.qb.DocType(self.benefit_details_doctype)
		employee_benefits = (
			artech_engine.qb.from_(EmployeeBenefitDetail)
	...
	def get_current_period_employee_benefit_amounts(self, employee_benefits: dict) -> dict:
		...
		precision = artech_engine.get_precision("Employee Benefit Detail", "amount")
	...
	def get_tax_components(self) -> list:
		...
		"""
		tax_components = artech_engine.cache().get_value(
	...
	def handle_additional_salary_tax_component(self) -> bool:
		...
		additional_salary = artech_engine.db.get_value(
			"Additional Salary",
	...
	def get_income_tax_slabs(self):
		...
		income_tax_slab_doc = artech_engine.get_cached_doc("Income Tax Slab", income_tax_slab)
		if income_tax_slab_doc.disabled:
			artech_engine.throw(_("Income Tax Slab: {0} is disabled").format(income_tax_slab))
	...
	def get_salary_slip_details(
		...
		ss = artech_engine.qb.DocType("Salary Slip")
		sd = artech_engine.qb.DocType("Salary Detail")
	...
	def get_future_recurring_period(
		...
		if not to_date:
			to_date = artech_engine.db.get_value("Additional Salary", additional_salary, "to_date", cache=True)
	...
	def get_total_exemption_amount(self):
		...
		if self.tax_slab.allow_tax_exemption:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = artech_engine.db.get_value(
				...
				declaration = artech_engine.db.get_value(
	...
	def email_salary_slip(self):
		receiver = artech_engine.db.get_value("Employee", self.employee, "prefered_email", cache=True)
		payroll_settings = artech_engine.get_single("Payroll Settings")
		...
		if payroll_settings.email_template:
			email_template = artech_engine.get_doc("Email Template", payroll_settings.email_template)
			context = self.as_dict()
			subject = artech_engine.render_template(email_template.subject, context)
			message = artech_engine.render_template(email_template.response, context)
		...
		if receiver:
			...
			email_args = {
				...
				"subject": subject,
				"attachments": [
					artech_engine.attach_print(self.doctype, self.name, file_name=self.name, password=password)
				],
			...
			if not artech_engine.flags.in_test:
				enqueue(method=artech_engine.sendmail, queue="short", timeout=300, is_async=True, **email_args)
			else:
				artech_engine.sendmail(**email_args)
		else:
	...
	def update_status(self, salary_slip=None):
		for data in self.timesheets:
			if data.time_sheet:
				timesheet = artech_engine.get_doc("Timesheet", data.time_sheet)
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
		salary_component = artech_engine.db.get_value(
	...
	def compute_year_to_date(self):
		...
		salary_slip_sum = artech_engine.get_list(
			"Salary Slip",
...
def get_benefits_details_parent(employee, payroll_period, salary_structure_assignment):
	...
	mandatory_benefit_application = artech_engine.db.get_single_value(
	...
	employee_benefit_application = artech_engine.db.get_value(
...
def unlink_ref_doc_from_salary_slip(doc, method=None):
	...
	linked_ss = artech_engine.get_all(
		"Salary Slip", filters={"journal_entry": doc.name, "docstatus": ["<", 2]}, pluck="name"
	)

	if linked_ss:
		for ss in linked_ss:
			ss_doc = artech_engine.get_doc("Salary Slip", ss)
			artech_engine.db.set_value("Salary Slip", ss_doc.name, "journal_entry", "")


def generate_password_for_pdf(policy_template, employee):
	employee = artech_engine.get_cached_doc("Employee", employee)
...
def eval_tax_slab_condition(condition, eval_globals=None, eval_locals=None):
	...
	try:
		...
		if condition:
			return artech_engine.safe_eval(condition, eval_globals, eval_locals)
		...
		artech_engine.throw(_("Syntax error in condition: {0} in Income Tax Slab").format(err))
	except Exception as e:
		artech_engine.throw(_("Error in formula or condition: {0} in Income Tax Slab").format(e))
		raise
...
def get_lwp_or_ppl_for_date_range(employee, start_date, end_date):
	...
	leave_date_mapper = artech_engine._dict()
	for leave in leaves:
...
def make_salary_slip_from_timesheet(source_name: str, target_doc: str | Document | None = None) -> Document:
	target = artech_engine.new_doc("Salary Slip")
...
def set_missing_values(time_sheet, target):
	doc = artech_engine.get_doc("Timesheet", time_sheet)
...
def throw_error_message(row, error, title, description=None):
	...
	artech_engine.throw(message, title=title)
...
def verify_lwp_days_corrected(employee, start_date, end_date, lwp_days_corrected):
	...
	PayrollCorrection = artech_engine.qb.DocType("Payroll Correction")
	SalarySlip = artech_engine.qb.DocType("Salary Slip")

	actual_days_reversed = (
		artech_engine.qb.from_(PayrollCorrection)
		.join(SalarySlip)
...
def on_doctype_update():
	artech_engine.db.add_index("Salary Slip", ["employee", "start_date", "end_date"])


def _safe_eval(code: str, eval_globals: dict | None = None, eval_locals: dict | None = None):
	"""Old version of safe_eval from framework.

	Note: current artech_engine.safe_eval transforms code so if you have nested
...
def _check_attributes(code: str) -> None:
	...
	from artech_engine.utils.safe_exec import UNSAFE_ATTRIBUTES

	unsafe_attrs = set(UNSAFE_ATTRIBUTES).union(["__"]) - {"format"}

	for attribute in unsafe_attrs:
		if attribute in code:
			raise SyntaxError(f'Illegal rule {artech_engine.bold(code)}. Cannot use "{attribute}"')
	...
	for node in ast.walk(tree):
		...
		if isinstance(node, ast.Attribute) and isinstance(node.attr, str) and node.attr in UNSAFE_ATTRIBUTES:
			raise SyntaxError(f'Illegal rule {artech_engine.bold(code)}. Cannot use "{node.attr}"')
...
def email_salary_slips(names) -> None:
	for name in names:
		salary_slip = artech_engine.get_doc("Salary Slip", name)
