		...
		if payment.total_payment > total_amount:
			artech_engine.throw(
				_(
					...
					payment.idx,
					artech_engine.bold(payment.total_payment),
					artech_engine.bold(total_amount),
					artech_engine.bold(payment.loan),
				)
			)
...
def _get_loan_details(doc: "SalarySlip") -> dict[str, Any]:
	loan_details = artech_engine.get_all(
		"Loan",
...
def process_loan_interest_accrual_and_demand(doc: "SalarySlip"):
	...
	loan_demand_exists = artech_engine.db.exists("DocType", "Loan Demand")
...
def make_loan_repayment_entry(doc: "SalarySlip"):
	...
	process_payroll_accounting_entry_based_on_employee = artech_engine.db.get_single_value(
	...
	for loan in doc.get("loans", []):
		...
		artech_engine.db.set_value("Salary Slip Loan", loan.name, "loan_repayment_entry", repayment_entry.name)
...
def cancel_loan_repayment_entry(doc: "SalarySlip"):
	...
	for loan in doc.get("loans", []):
		if loan.loan_repayment_entry:
			repayment_entry = artech_engine.get_doc("Loan Repayment", loan.loan_repayment_entry)
