artech_engine.ui.form.on("Salary Component", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		artech_hrms.payroll_utils.set_autocompletions_for_condition_and_formula(frm);
	...
	update_salary_structures: function (frm, df, structures) {
		let msg = __("{0} will be updated for the following Salary Structures: {1}.", [
			__(df),
			artech_engine.utils.comma_and(
				structures.map((d) =>
					artech_engine.utils.get_form_link("Salary Structure", d, true).bold(),
				),
			),
		]);
		msg += "<br>";
		msg += __("Are you sure you want to proceed?");
		artech_engine.confirm(msg, () => {
			artech_engine
				.call({
	...
	create_salary_structure: function (frm) {
		artech_engine.model.with_doctype("Salary Structure", () => {
			const salary_structure = artech_engine.model.get_new_doc("Salary Structure");
			const salary_detail = artech_engine.model.add_child(
				salary_structure,
				frm.doc.type === "Earning" ? "earnings" : "deductions",
			);
			salary_detail.salary_component = frm.doc.name;
			artech_engine.set_route("Form", "Salary Structure", salary_structure.name);
		});
	},
});
