...
class TestEmployeeTaxExemptionDeclaration(HRMSTestSuite):
	def setUp(self):
		artech_engine.db.delete("Employee Tax Exemption Declaration")
		artech_engine.db.delete("Salary Structure Assignment")
		artech_engine.db.delete("Salary Slip")
	...
	def test_duplicate_category_in_declaration(self):
		declaration = artech_engine.get_doc(
			{
				...
				"employee": artech_engine.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
				"company": "_Test Company",
		...
		self.assertRaises(artech_engine.ValidationError, declaration.save)
	...
	def test_duplicate_entry_for_payroll_period(self):
		...
		duplicate_declaration = artech_engine.get_doc(
			{
				"doctype": "Employee Tax Exemption Declaration",
				"employee": artech_engine.get_value("Employee", {"user_id": "employee@taxexemption.com"}, "name"),
				"company": "_Test Company",
		...
		duplicate_declaration.employee = artech_engine.get_value(
	...
	def test_india_hra_exemption_with_multiple_assignments(self):
		...
		payroll_period = artech_engine.get_doc("Payroll Period", PAYROLL_PERIOD_NAME)
		create_tax_slab(
			payroll_period,
...
def create_payroll_period(**args):
	...
	if not artech_engine.db.exists("Payroll Period", name):
		from datetime import date

		payroll_period = artech_engine.get_doc(
		...
		return artech_engine.get_doc("Payroll Period", name)


def create_exemption_category():
	if not artech_engine.db.exists("Employee Tax Exemption Category", "_Test Category"):
		artech_engine.get_doc(
			{
	...
	if not artech_engine.db.exists("Employee Tax Exemption Sub Category", "_Test Sub Category"):
		artech_engine.get_doc(
			{
	...
	if not artech_engine.db.exists("Employee Tax Exemption Sub Category", "_Test1 Sub Category"):
		artech_engine.get_doc(
			{
				"doctype": "Employee Tax Exemption Sub Category",
