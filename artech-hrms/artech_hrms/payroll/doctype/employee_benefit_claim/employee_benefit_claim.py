...
class EmployeeBenefitClaim(Document):
	...
	def validate_date_and_benefit_claim_amount(self):
		...
		if self.claimed_amount <= 0:
			artech_engine.throw(_("Claimed amount of employee {0} should be greater than 0").format(self.employee))
	...
	def validate_duplicate_claim(self):
		...
		if existing_claim:
			msg = _(
				...
				artech_engine.bold(self.earning_component),
				artech_engine.bold(artech_engine.utils.formatdate(self.payroll_date, "MMMM yyyy")),
				artech_engine.bold(get_link_to_form("Employee Benefit Claim", existing_claim)),
			)
			artech_engine.throw(msg, title=_("Duplicate Claim Detected"))

	def on_submit(self):
		self.create_additional_salary()

	def get_existing_claim_for_month(self):
		month_start_date = artech_engine.utils.get_first_day(self.payroll_date)
		month_end_date = artech_engine.utils.get_last_day(self.payroll_date)
	...
	def get_component_details(self, payroll_period, salary_structure_assignment):
		...
		EmployeeBenefitDetail = artech_engine.qb.DocType(benefit_details_doctype)
...
def get_benefit_components(
	...
	try:
		...
		artech_engine.log_error("Error fetching benefit components", e)
		return []
