...
from artech_engine.utils import add_days, add_months, cstr, date_diff, flt
...
class TestPayrollEntry(HRMSTestSuite):
	def setUp(self):
		...
		artech_engine.db.set_value("Account", "Employee Advances - _TC", "account_type", "Receivable")
		# set default payable account
		default_account = artech_engine.db.get_value("Company", "_Test Company", "default_payroll_payable_account")
		...
		payroll_account = artech_engine.get_doc("Account", "_Test Payroll Payable - _TC")
		if payroll_account and payroll_account.account_type != "Payable":
			artech_engine.db.set_value("Account", "_Test Payroll Payable - _TC", "account_type", "Payable")

		if "lending" in artech_engine.get_installed_apps():
			artech_engine.db.set_value("Company", "_Test Company", "loan_accrual_frequency", "Monthly")

	def test_payroll_entry(self):
		company = artech_engine.get_doc("Company", "_Test Company")
		employee = artech_engine.db.get_value("Employee", {"company": "_Test Company"})
	...
	def test_multi_currency_payroll_entry(self):
		...
		salary_slip = artech_engine.db.get_value("Salary Slip", {"payroll_entry": payroll_entry.name}, "name")
		salary_slip = artech_engine.get_doc("Salary Slip", salary_slip)

		payroll_entry.reload()
		payroll_je = salary_slip.journal_entry
		if payroll_je:
			payroll_je_doc = artech_engine.get_doc("Journal Entry", payroll_je)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_debit)
			self.assertEqual(salary_slip.base_gross_pay, payroll_je_doc.total_credit)

		payment_entry = artech_engine.db.sql(
			"""
			select
	...
	def test_payroll_entry_with_employee_cost_center(self):
		...
		je = artech_engine.db.get_value("Salary Slip", {"payroll_entry": pe.name}, "journal_entry")
		je_entries = artech_engine.db.sql(
			"""
	...
	def test_employee_cost_center_breakup(self):
		...
		new_assignment = artech_engine.db.get_value(
			"Salary Structure Assignment",
			{"employee": employee, "salary_structure": salary_structure.name, "docstatus": 1},
			"name",
		)
		new_assignment = artech_engine.get_doc("Salary Structure Assignment", new_assignment)
		...
		old_assignment = artech_engine.copy_doc(new_assignment)
	...
	def test_loan_with_settings_enabled(self):
		...
		artech_engine.db.delete("Loan")

		[applicant, branch, currency, payroll_payable_account] = setup_lending()
		loan = create_loan_for_employee(applicant)
		dates = artech_engine._dict({"start_date": add_months(getdate(), -1), "end_date": getdate()})
	...
	def test_salary_slip_operation_queueing(self):
		...
		artech_engine.flags.enqueue_payroll_entry = True
		payroll_entry.submit()
		payroll_entry.reload()

		self.assertEqual(payroll_entry.status, "Queued")
		artech_engine.flags.enqueue_payroll_entry = False
	...
	def test_salary_slip_operation_failure(self):
		...
		component = artech_engine.get_doc("Salary Component", salary_structure.earnings[0].salary_component)
		component.accounts = []
		component.save()
		...
		artech_engine.db.set_value("Employee", employee, "status", "Inactive")
		payroll_entry.submit()
		payroll_entry.reload()
		self.assertEqual(payroll_entry.status, "Failed")
		self.assertIsNotNone(payroll_entry.error_message)

		artech_engine.db.set_value("Employee", employee, "status", "Active")
		...
		# set accounts
		for data in artech_engine.get_all("Salary Component", pluck="name"):
	...
	def test_payroll_entry_cancellation(self):
		company_doc = artech_engine.get_doc("Company", "_Test Company")
	...
	def test_payroll_entry_cancellation_against_cancelled_journal_entry(self):
		...
		for jv in jvs:
			jv_doc = artech_engine.get_doc("Journal Entry", jv.parent)
	...
	def test_advance_deduction_in_accrual_journal_entry(self):
		...
		deduction_entry = artech_engine.get_all(
	...
	def test_employee_wise_bank_entry_with_cost_centers(self):
		...
		debit_entries = artech_engine.db.get_all(
			"Journal Entry Account",
	...
	def test_validate_attendance(self):
		...
		artech_engine.db.set_value("Employee", employee, "relieving_date", relieving_date)
	...
	def run_test_for_loan_repayment_from_salary(self):
		...
		loan = create_loan_for_employee(applicant)
		loan_doc = artech_engine.get_doc("Loan", loan.name)
		...
		salary_slip_name = artech_engine.db.get_value("Salary Slip", {"payroll_entry": payroll_entry.name}, "name")
		...
		bank_entry = artech_engine.db.sql(
			"""
	...
	def test_component_exclusion_from_accounting_entries(self):
		...
		salary_slip = artech_engine.get_doc("Salary Slip", {"payroll_entry": payroll_entry.name})
		...
		# verify jv & accounts
		journal_entry = artech_engine.get_doc("Journal Entry", salary_slip.journal_entry)
	...
	def test_employee_benefits_accruals_in_salary_slip(self):
		...
		payroll_period = artech_engine.get_last_doc("Payroll Period", filters={"company": "_Test Company"})
		...
		for benefit in salary_slip.accrued_benefits:
			if benefit.salary_component != "Accrued Earnings":
				payout_method = artech_engine.db.get_value(
		...
		additional_salary = artech_engine.get_doc(
			{
		...
		next_salary_slip = artech_engine.get_doc("Salary Slip", {"payroll_entry": next_month_payroll_entry.name})
		...
		artech_engine.db.delete("Salary Slip", {"employee": emp})
		artech_engine.db.delete("Employee Benefit Ledger")
		...
		salary_slip = artech_engine.get_all(
			"Salary Slip", filters={"employee": emp}, order_by="posting_date desc", limit=1, pluck="name"
		)
		salary_slip = artech_engine.get_doc("Salary Slip", salary_slip[0])
...
def get_payroll_entry(**args):
	...
	payroll_entry: PayrollEntry = artech_engine.new_doc("Payroll Entry")
...
def get_payment_account():
	return artech_engine.get_value(
		"Account",
...
def create_assignments_with_cost_centers(employee1, employee2):
	...
	ssa = artech_engine.db.get_value(
		"Salary Structure Assignment",
...
def setup_lending():
	...
	if not artech_engine.db.exists("Branch", branch):
		artech_engine.get_doc({"doctype": "Branch", "branch": branch}).insert()
	...
	if not artech_engine.db.exists("Loan Product", "Car Loan"):
		create_loan_accounts()
		create_loan_product(
			"Car Loan",
			"Car Loan",
...
def get_repayment_party_type(loan):
	loan_repayment = artech_engine.db.get_value(
...
def submit_bank_entry(payroll_entry_id):
	...
	jv_doc = artech_engine.get_doc("Journal Entry", jv)
