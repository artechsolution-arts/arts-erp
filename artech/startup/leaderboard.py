...
def get_leaderboards():
	leaderboards = {
		"Customer": {
			...
			"method": "artech.startup.leaderboard.get_all_customers",
			"icon": "customer",
		},
		"Item": {
			"fields": [
			...
			"method": "artech.startup.leaderboard.get_all_items",
			"icon": "stock",
		},
		"Supplier": {
			"fields": [
			...
			"method": "artech.startup.leaderboard.get_all_suppliers",
			"icon": "buying",
		},
		"Sales Partner": {
			"fields": [
			...
			"method": "artech.startup.leaderboard.get_all_sales_partner",
			"icon": "hr",
		},
		"Sales Person": {
			"fields": [{"fieldname": "total_sales_amount", "fieldtype": "Currency"}],
			"method": "artech.startup.leaderboard.get_all_sales_person",
			"icon": "customer",
		},
	}
...
def get_all_items(date_range: str, company: str, field: str, limit: int | None = None):
	if field in ("available_stock_qty", "available_stock_value"):
		...
		results = artech_engine.db.get_all(
			"Bin",
		...
		readable_active_items = set(artech_engine.get_list("Item", filters={"disabled": 0}, pluck="name"))
...
def get_date_condition(date_range, field):
	date_condition = ""
	if date_range:
		date_range = artech_engine.parse_json(date_range)
		from_date, to_date = date_range
		date_condition = f"and {field} between {artech_engine.db.escape(from_date)} and {artech_engine.db.escape(to_date)}"
