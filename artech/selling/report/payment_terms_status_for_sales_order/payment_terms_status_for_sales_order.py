...
from artech_engine import _, qb, query_builder
from artech_engine.query_builder import Criterion, functions
from artech_engine.utils.dateutils import getdate
...
def get_conditions(filters):
	...
	"""
	filters = artech_engine._dict(filters) if filters else artech_engine._dict({})
	conditions = artech_engine._dict({})

	conditions.company = filters.company or artech_engine.defaults.get_user_default("company")
	conditions.end_date = filters.period_end_date or artech_engine.utils.today()
	conditions.start_date = filters.period_start_date or artech_engine.utils.add_months(conditions.end_date, -1)
...
def set_payment_terms_statuses(sales_orders, invoices, filters):
	...
	for so in sales_orders:
		so.currency = artech_engine.get_cached_value("Company", filters.get("company"), "default_currency")
		so.invoices = ""
