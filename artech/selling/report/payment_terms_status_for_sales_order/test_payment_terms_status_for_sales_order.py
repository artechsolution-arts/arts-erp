...
from artech_engine.utils import add_days, add_months, nowdate
...
from artech.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order import (
	execute,
)
...
class TestPaymentTermsStatusForSalesOrder(ArtechTestSuite):
	def create_payment_terms_template(self):
		# create template for 50-50 payments
		template = None
		if artech_engine.db.exists("Payment Terms Template", "_Test 50-50"):
			template = artech_engine.get_doc("Payment Terms Template", "_Test 50-50")
		else:
			...
			doc = artech_engine.get_doc(
				{
					...
					"date": date,
					"from_currency": "USD",
					"to_currency": artech_engine.get_cached_value("Company", "_Test Company", "default_currency"),
					"exchange_rate": 70,
					"for_buying": True,
					"for_selling": True,
				}
	...
	def test_03_group_filters(self):
		...
		for idx, g in enumerate(group_filters, 0):
			# build filter
			filters = artech_engine._dict({}).update(base_filters).update(g)
