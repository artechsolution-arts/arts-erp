...
def get_columns(filters):
	...
	columns = [
		...
		f"{artech_engine.unscrub(str(party_type_value))}::150",
		{
			"label": _("Address"),
			"fieldtype": "Link",
			"options": "Address",
...
def get_party_addresses_and_contact(party_type, party, party_group, filters):
	...
	fetch_party_list = artech_engine.get_list(
	...
	for d in party_list:
		party_details.setdefault(d, artech_engine._dict())
...
def get_party_details(party_type, party_list, doctype, party_details):
	...
	records = artech_engine.get_list(doctype, filters=filters, fields=fields, as_list=True)

	for d in records:
		details = party_details.get(d[0])
		details.setdefault(artech_engine.scrub(doctype), []).append(d[1:])
...
def should_add_party_name(party_type):
	...
	if party_type in settings_map:
		...
		return artech_engine.db.get_single_value(doctype, fieldname) in ["Naming Series", "Auto Name"]
