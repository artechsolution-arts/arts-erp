...
from artech.selling.report.sales_order_analysis.sales_order_analysis import execute
from artech.stock.doctype.item.test_item import create_item
from artech.tests.utils import ArtechTestSuite


class TestSalesOrderAnalysis(ArtechTestSuite):
	...
	def test_01_so_to_deliver_and_bill(self):
		...
		expected_value = {
			"status": "To Deliver and Bill",
			"sales_order": so.name,
			"delay_days": artech_engine.utils.date_diff(artech_engine.utils.datetime.date.today(), so.delivery_date),
			"qty": 10,
			"delivered_qty": 0,
			"pending_qty": 10,
			"qty_to_bill": 10,
