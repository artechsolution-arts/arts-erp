...
artech_engine.query_reports["Sales Analytics"] = {
	filters: [
		{
			fieldname: "tree_type",
			label: __("Tree Type"),
		...
		},
		{
			...
			fieldtype: "Date",
			default:
				artech_engine.defaults.get_user_default("sales_start_date") ||
				artech.utils.get_fiscal_year(artech_engine.datetime.get_today(), true)[1],
			reqd: 1,
		},
		{
			fieldname: "to_date",
			...
			fieldtype: "Date",
			default:
				artech_engine.defaults.get_user_default("sales_end_date") ||
				artech.utils.get_fiscal_year(artech_engine.datetime.get_today(), true)[2],
			reqd: 1,
		},
		{
			fieldname: "company",
		...
		return Object.assign(options, {
			...
			events: {
				onCheckRow: function (data) {
					...
					const tree_type = artech_engine.query_report.filters[0].value;
					if (data_doctype != tree_type) return;

					const row_name = data[2].content;
					const raw_data = artech_engine.query_report.chart.data;
					...
					const new_options = Object.assign({}, artech_engine.query_report.chart_options, {
						data: new_data,
					});
					artech_engine.query_report.render_chart(new_options);

					artech_engine.query_report.raw_chart_data = new_data;
				},
			},
		});
	},
