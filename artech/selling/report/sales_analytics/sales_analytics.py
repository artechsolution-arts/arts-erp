...
from artech_engine.utils import add_days, add_to_date, flt, getdate
...
class Analytics:
	def __init__(self, filters=None):
		...
		if self.filters.doc_type == "Payment Entry" and self.filters.value_quantity == "Quantity":
			artech_engine.throw(_("Only Value available for Payment Entry"))
		self.date_field = (
			"transaction_date"
	...
	def update_company_list_for_parent_company(self):
		...
		if (
			...
			and artech_engine.db.get_value("Company", selected_company, "is_group")
		):
			lft, rgt = artech_engine.db.get_value("Company", selected_company, ["lft", "rgt"])
			child_companies = artech_engine.db.get_list(
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		self.entries = artech_engine.get_all(
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = (
			artech_engine.qb.from_(doctype_item)
			.join(doctype)
		...
		for d in reversed(self.group_entries):
			...
			for end_date in self.periodic_daterange:
				...
				if d.parent and (self.filters.tree_type != "Order Type" or d.parent == "Order Types"):
					self.entity_periodic_data.setdefault(d.parent, artech_engine._dict()).setdefault(period, 0.0)
	...
	def get_periodic_data(self):
		self.entity_periodic_data = artech_engine._dict()

		for d in self.entries:
			if self.filters.tree_type == "Supplier Group":
				d.entity = self.parent_child_map.get(d.entity)
			period = self.get_period(d.get(self.date_field))
			self.entity_periodic_data.setdefault(d.entity, artech_engine._dict()).setdefault(period, 0.0)
	...
	def get_groups(self):
		...
		self.depth_map = artech_engine._dict()

		self.group_entries = artech_engine.db.sql(
	...
	def get_supplier_parent_child_map(self):
		self.parent_child_map = artech_engine._dict(
			artech_engine.db.sql(""" select name, supplier_group from `tabSupplier`""")
		)
