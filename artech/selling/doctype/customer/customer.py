...
from artech_engine import _, msgprint, qb
...
from artech_engine.model.naming import set_name_by_naming_series, set_name_from_naming_options
from artech_engine.model.utils.rename_doc import update_linked_doctypes
from artech_engine.query_builder import Field, functions
from artech_engine.utils import cint, cstr, flt, get_formatted_email, today
...
from artech.controllers.website_list_for_contact import add_role_for_portal_user
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from artech.accounts.doctype.party_account.party_account import PartyAccount
		from artech.selling.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from artech.selling.doctype.sales_team.sales_team import SalesTeam
		from artech.selling.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from artech.utilities.doctype.portal_user.portal_user import PortalUser
	...
	def autoname(self):
		cust_master_name = artech_engine.defaults.get_global_default("cust_master_name")
		if cust_master_name == "Customer Name":
			self.name = self.get_customer_name()
		elif cust_master_name == "Naming Series":
			set_name_by_naming_series(self)
		else:
			set_name_from_naming_options(artech_engine.get_meta(self.doctype).autoname, self)

	def get_customer_name(self):
		self.customer_name = self.customer_name.strip()
		if artech_engine.db.get_value("Customer", self.customer_name) and not artech_engine.flags.in_import:
			name_prefix = f"{self.customer_name} - %"

			if artech_engine.db.db_type == "postgres":
	...
	def validate(self):
		...
		if self.sales_team:
			if sum(member.allocated_percentage or 0 for member in self.sales_team) != 100:
				artech_engine.throw(_("Total contribution percentage should be equal to 100"))

	@artech_engine.whitelist()
	def get_customer_group_details(self):
		doc = artech_engine.get_doc("Customer Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		artech_engine.flags.customer_group_changed = False

		if not self.get("__islocal"):
			if self.customer_group != artech_engine.db.get_value("Customer", self.name, "customer_group"):
				artech_engine.flags.customer_group_changed = True

	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = artech_engine.db.get_value(
				"Bank Account", self.default_bank_account, "is_company_account"
			)
			if not is_company_account:
				artech_engine.throw(
					_("{0} is not a company bank account").format(artech_engine.bold(self.default_bank_account))
				)
	...
	def validate_internal_customer(self):
		...
		internal_customer = artech_engine.db.get_value(
			"Customer",
			{
		...
		if internal_customer:
			artech_engine.throw(
				_("Internal Customer for company {0} already exists").format(
					artech_engine.bold(self.represents_company)
				)
			)
	...
	def update_customer_groups(self):
		...
		if artech_engine.flags.customer_group_changed:
	...
	def create_primary_contact(self):
		if not self.customer_primary_contact and not self.lead_name:
			...
			artech_engine.set_value("Contact", self.customer_primary_contact, "is_primary_contact", 1)  # ensure
	...
	def create_primary_address(self):
		...
		if self.flags.is_new_doc and self.get("address_line1"):
			...
			artech_engine.set_value("Address", self.customer_primary_address, "is_primary_address", 1)  # ensure

	def update_lead_status(self):
		"""If Customer created from Lead, update lead status to "Converted"
		update Customer link in Quotation, Opportunity"""
		if self.lead_name:
			artech_engine.db.set_value("Lead", self.lead_name, "status", "Converted")
	...
	def link_address_and_contact(self):
		...
		for doctype, docname in linked_documents.items():
			...
			linked_contacts_and_addresses = artech_engine.get_all(
				"Dynamic Link",
				filters=[
			...
			for row in linked_contacts_and_addresses:
				linked_doc = artech_engine.get_doc(row.doctype, row.name)
	...
	def copy_communication(self):
		if not self.lead_name or not artech_engine.db.get_single_value(
			"CRM Settings", "carry_forward_communication_and_comments"
		):
			return

		from artech.crm.utils import copy_comments, link_communications

		copy_comments("Lead", self.lead_name, self)
		link_communications("Lead", self.lead_name, self)

	def validate_name_with_customer_group(self):
		if artech_engine.db.exists("Customer Group", self.name):
			artech_engine.throw(
				_(
	...
	def validate_customer_group(self):
		...
		is_group = artech_engine.db.get_value("Customer Group", self.customer_group, "is_group")
		if is_group:
			artech_engine.throw(
	...
	def on_trash(self):
		...
		if self.lead_name:
			artech_engine.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)
	...
	def after_rename(self, olddn, newdn, merge=False):
		if artech_engine.defaults.get_global_default("cust_master_name") == "Customer Name":
		...
		if len(loyalty_program) == 1:
			...
			artech_engine.msgprint(
				_("Multiple Loyalty Programs found for Customer {}. Please select manually.").format(
					artech_engine.bold(self.customer_name)
				)
			)
...
def make_quotation(source_name: str, target_doc: str | Document | None = None):
	...
	price_list, currency = artech_engine.db.get_value(
...
def _set_missing_values(source, target):
	...
	if contact:
		...
		target.contact_display, target.contact_email, target.contact_mobile = artech_engine.get_value(
...
def get_loyalty_programs(doc: Document):
	...
	lp_details = []
	loyalty_programs = artech_engine.get_all(
		"Loyalty Program",
...
def get_nested_links(link_doctype, link_name, ignore_permissions=False):
	from artech_engine.desk.treeview import _get_children
...
def check_credit_limit(customer, company, ignore_outstanding_sales_order=False, extra_amount=0):
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		credit_controller_role = artech_engine.get_single_value("Accounts Settings", "credit_controller")
		if not credit_controller_role or credit_controller_role not in artech_engine.get_roles():
			...
			artech_engine.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "artech.selling.doctype.customer.customer.send_emails",
...
def send_emails(
	...
	artech_engine.sendmail(recipients=credit_controller_users_list, subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	cond = ""
	if cost_center:
		lft, rgt = artech_engine.get_cached_value("Cost Center", cost_center, ["lft", "rgt"])

		cond = f""" and cost_center in (select name from `tabCost Center` where
			lft >= {lft} and rgt <= {rgt})"""

	outstanding_based_on_gle = artech_engine.db.sql(
		f"""
	...
	if not ignore_outstanding_sales_order:
		outstanding_based_on_so = artech_engine.db.sql(
			"""
	...
	unmarked_delivery_note_items = artech_engine.db.sql(
		"""select
	...
	si_amounts = artech_engine.db.sql(
		"""
		SELECT
		...
		GROUP BY dn_detail""".format(
			", ".join(artech_engine.db.escape(dn_item.name) for dn_item in unmarked_delivery_note_items)
		)
	)
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = artech_engine.db.get_value(
			"Customer Credit Limit",
		...
		if not credit_limit:
			customer_group = artech_engine.get_cached_value("Customer", customer, "customer_group")

			result = artech_engine.db.get_values(
				"Customer Credit Limit",
	...
	if not credit_limit:
		credit_limit = artech_engine.get_cached_value("Company", company, "credit_limit")
...
def make_contact(args, is_primary_contact=1):
	...
	contact = artech_engine.get_doc(values)
