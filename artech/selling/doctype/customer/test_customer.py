...
from artech.exceptions import PartyDisabled, PartyFrozen
from artech.selling.doctype.customer.customer import (
	get_credit_limit,
...
class TestCustomer(ArtechTestSuite):
	def test_get_customer_group_details(self):
		doc = artech_engine.new_doc("Customer Group")
		...
		c_doc = artech_engine.new_doc("Customer")
	...
	def test_party_details_tax_category(self):
		...
		settings = artech_engine.get_single("Accounts Settings")
	...
	def test_rename(self):
		...
		# add comments
		comment = artech_engine.get_doc("Customer", "_Test Customer 1").add_comment(
			"Comment", "Test Comment for Rename"
		)

		# rename
		artech_engine.rename_doc("Customer", "_Test Customer 1", new_name)

		# check if customer renamed
		self.assertTrue(artech_engine.db.exists("Customer", new_name))
		self.assertFalse(artech_engine.db.exists("Customer", "_Test Customer 1"))
		...
		artech_engine.rename_doc("Customer", new_name, "_Test Customer 1")

	def test_freezed_customer(self):
		artech_engine.db.set_value("Customer", "_Test Customer", "is_frozen", 1)
		...
		artech_engine.db.set_value("Customer", "_Test Customer", "is_frozen", 0)

		so.save()

	def test_delete_customer_contact(self):
		customer = artech_engine.get_doc(get_customer_dict("_Test Customer for delete")).insert(
		...
		artech_engine.delete_doc("Customer", customer.name)

	def test_disabled_customer(self):
		artech_engine.db.set_value("Customer", "_Test Customer", "disabled", 1)
		...
		artech_engine.db.set_value("Customer", "_Test Customer", "disabled", 0)

		so.save()

	def test_duplicate_customer(self):
		if not artech_engine.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = artech_engine.get_doc(get_customer_dict("_Test Customer 1")).insert(
				ignore_permissions=True
			)
		else:
			test_customer_1 = artech_engine.get_doc("Customer", "_Test Customer 1")

		duplicate_customer = artech_engine.get_doc(get_customer_dict("_Test Customer 1")).insert(
			ignore_permissions=True
		)
	...
	def test_customer_credit_limit(self):
		...
		self.assertRaises(artech_engine.ValidationError, so.submit)
	...
	def test_customer_credit_limit_after_submit(self):
		from artech.controllers.accounts_controller import update_child_qty_rate
		from artech.selling.doctype.sales_order.test_sales_order import make_sales_order
		...
		modified_item = artech_engine._dict()
		for x in fields:
	...
	def test_customer_credit_limit_on_change(self):
		...
		self.assertRaises(artech_engine.ValidationError, customer.save)
	...
	def test_customer_payment_terms(self):
		...
		artech_engine.db.set_value("Customer", "_Test Customer With Template", "payment_terms", "")
...
def create_internal_customer(customer_name=None, represents_company=None, allowed_to_interact_with=None):
	...
	existing_representative = artech_engine.db.get_value("Customer", {"represents_company": represents_company})
	...
	if not artech_engine.db.exists("Customer", customer_name):
		...
		customer_name = artech_engine.db.get_value("Customer", customer_name)
