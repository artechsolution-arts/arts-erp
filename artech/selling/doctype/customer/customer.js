...
artech_engine.ui.form.on("Customer", {
	setup: function (frm) {
		...
		frm.make_methods = {
			Quotation: () =>
				artech_engine.model.open_mapped_doc({
					method: "artech.selling.doctype.customer.customer.make_quotation",
					frm: frm,
				}),
			"Sales Order": () =>
				artech_engine.model.with_doctype("Sales Order", function () {
					var so = artech_engine.model.get_new_doc("Sales Order");
					so.customer = frm.doc.name; // Set the current customer as the SO customer
					artech_engine.set_route("Form", "Sales Order", so.name);
				}),
			Opportunity: () =>
				artech_engine.model.open_mapped_doc({
					method: "artech.selling.doctype.customer.customer.make_opportunity",
					frm: frm,
				}),
			"Payment Entry": () =>
				artech_engine.model.open_mapped_doc({
					method: "artech.selling.doctype.customer.customer.make_payment_entry",
					frm: frm,
				}),
			"Pricing Rule": () => frm.trigger("make_pricing_rule"),
			"Bank Account": () => artech.utils.make_bank_account(frm.doc.doctype, frm.doc.name),
		};
		...
		frm.set_query("customer_primary_contact", function (doc) {
			return {
				query: "artech.selling.doctype.customer.customer.get_customer_primary",
				filters: {
					customer: doc.name,
					type: "Contact",
				},
	...
	customer_primary_address: function (frm) {
		if (frm.doc.customer_primary_address) {
			artech_engine.call({
				...
				callback: function (r) {
					frm.set_value("primary_address", artech_engine.utils.html2text(r.message));
				},
			});
		}
		if (!frm.doc.customer_primary_address) {
	...
	refresh: function (frm) {
		if (artech_engine.defaults.get_default("cust_master_name") != "Naming Series") {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.set_route("query-report", "Accounts Receivable", {
						party_type: "Customer",
						party: frm.doc.name,
					});
				},
			...
			if (
				cint(artech_engine.defaults.get_default("enable_common_party_accounting")) &&
				artech_engine.model.can_create("Party Link")
			) {
				frm.add_custom_button(
			...
			// indicator
			artech.utils.set_party_dashboard_indicators(frm);
		} else {
	...
	validate: function (frm) {
		if (frm.doc.lead_name) artech_engine.model.clear_doc("Lead", frm.doc.lead_name);
	},
	...
	show_party_link_dialog: function (frm) {
		const dialog = new artech_engine.ui.Dialog({
			...
			primary_action: function ({ supplier }) {
				artech_engine.call({
					method: "artech.accounts.doctype.party_link.party_link.create_party_link",
					args: {
	...
	make_pricing_rule: function (frm) {
		artech_engine.new_doc("Pricing Rule", {
