...
from artech_engine.utils import add_days, add_months, flt, getdate, nowdate

from artech.controllers.accounts_controller import InvalidQtyError, update_child_qty_rate
from artech.selling.doctype.quotation.quotation import make_sales_order
from artech.tests.utils import ArtechTestSuite


class TestQuotation(ArtechTestSuite):
	def setUp(self):
	...
	def test_disallow_due_date_before_transaction_date(self):
		...
		self.assertRaises(artech_engine.ValidationError, qo.save)
	...
	def test_update_child_rate_change(self):
		...
		self.assertRaises(artech_engine.ValidationError, update_child_qty_rate, "Quotation", trans_item, qo.name)
		trans_item = json.dumps(
			[
				{
					"item_code": qo.items[0].item_code,
	...
	def test_make_sales_order_terms_copied(self):
		...
		quotation = artech_engine.copy_doc(self.globalTestRecords["Quotation"][0])
	...
	def test_gross_profit(self):
		...
		from artech.stock.get_item_details import ItemDetailsCtx, insert_item_price
		...
		selling_price_list = artech_engine.get_all("Price List", filters={"selling": 1}, limit=1)[0].name
			...
			ItemDetailsCtx(
				{
					...
					"conversion_factor": 1,
					"discount_amount": 0.0,
					"currency": artech_engine.db.get_value("Price List", selling_price_list, "currency"),
	...
	def test_maintain_rate_in_sales_cycle_is_enforced(self):
		...
		maintain_rate = artech_engine.db.get_single_value("Selling Settings", "maintain_same_sales_rate")
		...
		self.assertRaises(artech_engine.ValidationError, sales_order.save)

		artech_engine.db.set_single_value("Selling Settings", "maintain_same_sales_rate", maintain_rate)
	...
	def test_make_sales_order(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_sales_order, quotation.name)
		quotation.submit()
	...
	def test_valid_till_before_transaction_date(self):
		...
		self.assertRaises(artech_engine.ValidationError, quotation.validate)

	def test_so_from_expired_quotation(self):
		from artech.selling.doctype.quotation.quotation import make_sales_order

		artech_engine.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 0)
		...
		artech_engine.db.set_single_value("Selling Settings", "allow_sales_order_creation_for_expired_quotation", 1)
	...
	def test_create_quotation_with_margin(self):
		...
		quotation = artech_engine.copy_doc(test_record)
		quotation.transaction_date = nowdate()
		quotation.valid_till = add_months(quotation.transaction_date, 1)
		quotation.insert()
	...
	def test_quotation_expiry(self):
		from artech.selling.doctype.quotation.quotation import set_expired_status
	...
	def test_alternative_items_sales_order_mapping_with_stock_items(self):
		...
		artech_engine.flags.args = artech_engine._dict()
		item_list = []
		stock_items = {
			"_Test Simple Item 1": 100,
			"_Test Alt 1": 120,
		...
		artech_engine.flags.args.selected_items = [quotation.items[2]]
		sales_order = make_sales_order(quotation.name)
	...
	def test_uom_validation(self):
		...
		if not artech_engine.db.exists("UOM", "lbs"):
			artech_engine.get_doc({"doctype": "UOM", "uom_name": "lbs", "must_be_whole_number": 1}).insert()
		else:
			artech_engine.db.set_value("UOM", "lbs", "must_be_whole_number", 1)

		quotation = make_quotation(item_code=item, qty=1, rate=100, do_not_submit=1)
		quotation.items[0].uom = "lbs"
		quotation.items[0].conversion_factor = 2.23
		self.assertRaises(artech_engine.ValidationError, quotation.save)
	...
	def test_item_tax_template_for_quotation(self):
		...
		if not artech_engine.db.exists("Account", {"account_name": "_Test Vat", "company": "_Test Company"}):
		...
		if not artech_engine.db.exists("Item Tax Template", "Vat Template - _TC"):
			artech_engine.get_doc(
				{
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_zero_qty_in_quotation": 1})
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"allow_pegged_currencies_exchange_rates": True})
	...
	def test_over_order_limit(self):
		...
		so1.submit()
		self.assertRaises(artech_engine.ValidationError, so2.submit)
...
def enable_calculate_bundle_price(enable=1):
	selling_settings = artech_engine.get_doc("Selling Settings")
...
def make_quotation(**args):
	qo = artech_engine.new_doc("Quotation")
