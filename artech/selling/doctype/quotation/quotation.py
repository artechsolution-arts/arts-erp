...
from artech_engine.utils import cint, flt, getdate, nowdate
...
class Quotation(SellingController):
	...
	if TYPE_CHECKING:
		...
		from artech.crm.doctype.competitor_detail.competitor_detail import CompetitorDetail
		from artech.selling.doctype.quotation_item.quotation_item import QuotationItem
		from artech.setup.doctype.quotation_lost_reason_detail.quotation_lost_reason_detail import (
	...
	def validate_valid_till(self):
		if self.valid_till and getdate(self.valid_till) < getdate(self.transaction_date):
			artech_engine.throw(_("Valid till date cannot be before transaction date"))
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not artech_engine.get_single_value("Selling Settings", "allow_zero_qty_in_quotation"):
			return
	...
	def update_lead(self):
		if self.quotation_to == "Lead" and self.party_name:
			artech_engine.get_doc("Lead", self.party_name).set_status(update=True)

	def set_customer_name(self):
		if self.party_name and self.quotation_to == "Customer":
			self.customer_name = artech_engine.db.get_value("Customer", self.party_name, "customer_name")
		elif self.party_name and self.quotation_to == "Lead":
			lead_name, company_name = artech_engine.db.get_value(
			...
			self.customer_name = artech_engine.db.get_value("CRM Deal", self.party_name, "organization")
	...
	def update_opportunity_status(self, status, opportunity=None):
		...
		opp = artech_engine.get_doc("Opportunity", opportunity)
	...
	def declare_enquiry_lost(
		...
		if not (self.is_fully_ordered() or self.is_partially_ordered()):
			get_lost_reasons = artech_engine.get_list("Quotation Lost Reason", fields=["name"])
				...
				if reason.get("lost_reason") in lost_reasons_lst:
					...
					artech_engine.throw(
						_("Invalid lost reason {0}, please create a new lost reason").format(
							artech_engine.bold(reason.get("lost_reason"))
						)
					)
			...
			artech_engine.throw(_("Cannot set as Lost as Sales Order is made."))
...
def make_sales_order(
	...
	if not artech_engine.db.get_singles_value(
		"Selling Settings", "allow_sales_order_creation_for_expired_quotation"
	):
		quotation = artech_engine.db.get_value(
			"Quotation", source_name, ["transaction_date", "valid_till"], as_dict=1
		)
		if quotation.valid_till and (
			quotation.valid_till < quotation.transaction_date or quotation.valid_till < getdate(nowdate())
		):
			artech_engine.throw(_("Validity period of this quotation has ended."))
...
def _make_sales_order(source_name, target_doc=None, ignore_permissions=False, args=None):
	...
	selected_rows = [x.get("name") for x in artech_engine.flags.get("args", {}).get("selected_items", [])]

	# 0 qty is accepted, as the qty uncertain for some items
	has_unit_price_items = artech_engine.db.get_value("Quotation", source_name, "has_unit_price_items")
	...
	def set_missing_values(source, target):
		...
		if source.referral_sales_partner:
			...
			target.commission_rate = artech_engine.get_value(
...
def _make_customer(source_name, ignore_permissions=False):
	...
	if quotation.quotation_to == "Customer":
		return artech_engine.get_doc("Customer", quotation.party_name)
	elif quotation.quotation_to == "CRM Deal":
		return artech_engine.get_doc("Customer", {"crm_deal": quotation.party_name})

	# Check if a Customer already exists for the Lead or Prospect.
	existing_customer = None
	if quotation.quotation_to == "Lead":
		existing_customer = artech_engine.db.get_value("Customer", {"lead_name": quotation.party_name})
	elif quotation.quotation_to == "Prospect":
		existing_customer = artech_engine.db.get_value("Customer", {"prospect_name": quotation.party_name})

	if existing_customer:
		return artech_engine.get_doc("Customer", existing_customer)
...
def create_customer_from_lead(lead_name, ignore_permissions=False):
	from artech.crm.doctype.lead.lead import _make_customer
	...
	try:
		customer.insert()
		return customer
	except artech_engine.MandatoryError as e:
		handle_mandatory_error(e, customer, lead_name)


def create_customer_from_prospect(prospect_name, ignore_permissions=False):
	from artech.crm.doctype.prospect.prospect import make_customer as make_customer_from_prospect
...
def handle_mandatory_error(e, customer, lead_name):
	...
	artech_engine.throw(message, title=_("Mandatory Missing"))
