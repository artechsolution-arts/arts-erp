artech_engine.listview_settings["Sales Order"] = {
	add_fields: [
		"base_grand_total",
		"customer_name",
		"currency",
	...
	get_indicator: function (doc) {
		if (doc.status === "Closed") {
			...
			if (artech_engine.datetime.get_diff(doc.delivery_date) < 0) {
	...
	onload: function (listview) {
		var method = "artech.selling.doctype.sales_order.sales_order.close_or_unclose_sales_orders";
		...
		if (artech_engine.model.can_create("Sales Invoice")) {
			listview.page.add_action_item(__("Sales Invoice"), () => {
				artech.bulk_transaction_processing.create(listview, "Sales Order", "Sales Invoice");
			});
		}
		...
		if (artech_engine.model.can_create("Delivery Note")) {
			listview.page.add_action_item(__("Delivery Note"), () => {
				artech_engine.call({
					method: "artech.selling.doctype.sales_order.sales_order.is_enable_cutoff_date_on_bulk_delivery_note_creation",
					callback: (r) => {
						if (r.message) {
								...
								fields: [
									{
										...
										default: artech_engine.datetime.add_days(artech_engine.datetime.nowdate(), 1),
									},
								],
							});
							dialog.set_primary_action(__("Select"), function (values) {
								var until_delivery_date = values.delivery_date;
								artech.bulk_transaction_processing.create(
									listview,
									"Sales Order",
									"Delivery Note",
									{
		...
		if (artech_engine.model.can_create("Payment Entry")) {
			listview.page.add_action_item(__("Advance Payment"), () => {
				artech.bulk_transaction_processing.create(listview, "Sales Order", "Payment Entry");
			});
		}
	},
};
