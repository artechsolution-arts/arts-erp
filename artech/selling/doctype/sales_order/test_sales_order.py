...
import artech_engine.permissions
from artech_engine.core.doctype.user_permission.test_user_permission import create_user
from artech_engine.tests import change_settings
from artech_engine.utils import add_days, flt, getdate, nowdate, today

from artech.controllers.accounts_controller import InvalidQtyError, get_due_date, update_child_qty_rate
from artech.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_maintenance_schedule,
)
from artech.maintenance.doctype.maintenance_visit.test_maintenance_visit import (
	make_maintenance_visit,
)
from artech.manufacturing.doctype.blanket_order.test_blanket_order import make_blanket_order
from artech.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from artech.selling.doctype.sales_order.sales_order import (
	WarehouseRequired,
	create_pick_list,
	make_delivery_note,
...
from artech.stock.get_item_details import get_bin_details
from artech.tests.utils import ArtechTestSuite


class TestSalesOrder(ArtechTestSuite):
	...
	def test_sales_order_expired_item_price(self):
		...
		artech_engine.db.delete("Item Price", {"item_code": item_1.item_code})
		...
		self.assertEqual(artech_engine.db.get_value("Item Price", item_price.name, "price_list_rate"), 100)
		self.assertEqual(
			artech_engine.db.count("Item Price", {"item_code": item_1.item_code, "price_list": price_list}),
			2,
		)
		all_item_prices = artech_engine.get_all(
			"Item Price", filters={"item_code": item_1.item_code}, order_by="valid_from desc"
		)
		self.assertEqual(artech_engine.db.get_value("Item Price", all_item_prices[0].name, "price_list_rate"), 1000)
	...
	def test_sales_order_with_full_material_request(self):
		...
		mr.submit()
		mr.reload()
		self.assertRaises(artech_engine.ValidationError, make_material_request, so.name)
	...
	def test_sales_order_qty(self):
		...
		self.assertRaises(artech_engine.NonNegativeError, so.save)
	...
	def test_make_delivery_note(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_delivery_note, so.name)
	...
	def test_make_production_plan(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_production_plan, so.name)
	...
	def test_make_sales_invoice(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_sales_invoice, so.name)
	...
	def test_reserved_qty_for_over_delivery(self):
		...
		artech_engine.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
	...
	def test_reserved_qty_for_over_delivery_via_sales_invoice(self):
		...
		# set over-delivery allowance
		artech_engine.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 50)
		artech_engine.db.set_value("Item", "_Test Item", "over_billing_allowance", 20)
	...
	def test_sales_order_on_hold(self):
		...
		si = make_sales_invoice(so.name)
		self.assertRaises(artech_engine.ValidationError, create_dn_against_so, so.name)
	...
	def test_reserved_qty_for_over_delivery_with_packing_list(self):
		...
		# set over-delivery allowance
		artech_engine.db.set_value("Item", "_Test Product Bundle Item", "over_delivery_receipt_allowance", 50)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(artech_engine.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name)
	...
	def test_update_child_with_precision(self):
		from artech_engine.custom.doctype.property_setter.property_setter import make_property_setter
		from artech_engine.model.meta import get_field_precision

		precision = get_field_precision(artech_engine.get_meta("Sales Order Item").get_field("rate"))
	...
	def test_update_child_perm(self):
		...
		with self.set_user(test_user.name):
			...
			self.assertRaises(
				artech_engine.ValidationError, update_child_qty_rate, "Sales Order", trans_item, so.name
			)
	...
	def test_update_child_qty_rate_with_workflow(self):
		from artech_engine.model.workflow import apply_workflow
		...
		test_user = artech_engine.get_doc("User", user)
		...
		test_user2 = artech_engine.get_doc("User", user2)
	...
	def test_material_request_for_product_bundle(self):
		...
		if not artech_engine.db.exists("Item", "_Test Product Bundle Item New 1"):
	...
	def test_update_child_product_bundle(self):
		# test Update Items with product bundle
		if not artech_engine.db.exists("Item", "_Product Bundle Item"):
	...
	def test_update_child_with_tax_template(self):
		...
		"""
		if not artech_engine.db.exists("Item", "Test Item with Tax"):
			make_item(
				"Test Item with Tax",
				{
					"is_stock_item": 1,
		...
		if not artech_engine.db.exists("Item Tax Template", {"title": "Test Update Items Template"}):
		...
		new_item_with_tax = artech_engine.get_doc("Item", "Test Item with Tax")
		...
		if not artech_engine.db.exists("Item Tax", {"parent": item, "item_tax_template": tax_template}):
		...
		old_stock_settings_value = artech_engine.db.get_single_value("Stock Settings", "default_warehouse")
		artech_engine.db.set_single_value("Stock Settings", "default_warehouse", "_Test Warehouse - _TC")
		...
		artech_engine.get_doc("Item Tax Template", "Test Update Items Template - _TC").delete()
		artech_engine.db.set_single_value("Stock Settings", "default_warehouse", old_stock_settings_value)

	def test_warehouse_user(self):
		test_user = create_user("test_so_warehouse_user@example.com", "Sales User", "Stock User")

		test_user_2 = artech_engine.get_doc("User", "test2@example.com")
		test_user_2.add_roles("Sales User", "Stock User")
		test_user_2.remove_roles("Sales Manager")

		artech_engine.permissions.add_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		artech_engine.permissions.add_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		artech_engine.permissions.add_user_permission("Company", "_Test Company 1", test_user_2.name)
		...
		with self.set_user(test_user.name):
			...
			self.assertRaises(artech_engine.PermissionError, so.insert)

		with self.set_user(test_user_2.name):
			so.insert()

		artech_engine.permissions.remove_user_permission("Warehouse", "_Test Warehouse 1 - _TC", test_user.name)
		artech_engine.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
		artech_engine.permissions.remove_user_permission("Company", "_Test Company 1", test_user_2.name)
	...
	def test_block_delivery_note_against_cancelled_sales_order(self):
		...
		self.assertRaises(artech_engine.CancelledLinkError, dn.submit)
	...
	def test_auto_insert_price(self):
		...
		item_price = artech_engine.db.get_value(
			"Item Price", {"price_list": "_Test Price List", "item_code": "_Test Item for Auto Price List"}
		)
		if item_price:
			artech_engine.delete_doc("Item Price", item_price)
		...
		artech_engine.delete_doc("Item Price", item_price.name)

		artech_engine.db.set_single_value("Stock Settings", "update_price_list_based_on", "Rate")
		...
		artech_engine.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 0)
		...
		artech_engine.db.set_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing", 1)
	...
	def test_update_existing_item_price(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 0)

	def test_drop_shipping(self):
		from artech.buying.doctype.purchase_order.purchase_order import update_status
		from artech.selling.doctype.sales_order.sales_order import (
			make_purchase_order,
		)
		from artech.selling.doctype.sales_order.sales_order import update_status as so_update_status
		...
		if artech_engine.db.get_value("Item", "_Test Regular Item", "is_stock_item") == 1:
		...
		bin_po_item = artech_engine.get_all(
			"Bin",
		...
		self.assertEqual(flt(artech_engine.db.get_value("Sales Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_drop_shipping_full_for_default_suppliers(self):
		...
		if not artech_engine.db.exists("Item", "_Test Item for Drop Shipping 1"):
			make_item("_Test Item for Drop Shipping 1", {"is_stock_item": 1, "delivered_by_supplier": 1})

		if not artech_engine.db.exists("Item", "_Test Item for Drop Shipping 2"):
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		...
		"""
		from artech.selling.doctype.sales_order.sales_order import make_purchase_order
	...
	def test_create_so_with_margin(self):
		...
		new_so = artech_engine.copy_doc(so)
	...
	def test_make_work_order(self):
		from artech.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		artech_engine.db.set_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order", 0)
		...
		so_doc = artech_engine.get_doc("Sales Order", so.name)

		self.assertRaises(artech_engine.LinkExistsError, so_doc.cancel)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		po_doc = artech_engine.get_doc("Payment Entry", pe.name)
		po_doc.cancel()
	...
	def test_work_order_pop_up_from_sales_order(self):
		...
		from artech.controllers.item_variant import create_variant
		from artech.manufacturing.doctype.production_plan.test_production_plan import make_bom
		from artech.selling.doctype.sales_order.sales_order import get_work_order_items
	...
	def test_request_for_raw_materials(self):
		...
		mr = artech_engine.db.sql(
			"""select name from `tabMaterial Request` ORDER BY creation DESC LIMIT 1""", as_dict=1
		)[0]
		mr_doc = artech_engine.get_doc("Material Request", mr.get("name"))
	...
	def test_so_optional_blanket_order(self):
		...
		so_doc = artech_engine.get_doc("Sales Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		...
		self.assertRaises(artech_engine.ValidationError, so.cancel)
	...
	def test_so_cancellation_after_si_submission(self):
		...
		so.load_from_db()
		self.assertRaises(artech_engine.LinkExistsError, so.cancel)
	...
	def test_so_cancellation_after_work_order_submission(self):
		...
		Expected result: Sales Order should not get cancelled
		"""
		from artech.manufacturing.doctype.work_order.test_work_order import make_wo_order_test_record
	...
	def test_so_back_updated_from_wo_via_mr(self):
		...
		from artech.stock.doctype.material_request.material_request import raise_work_orders
		...
		wo = artech_engine.get_doc("Work Order", wo_name)
		...
		se = artech_engine.get_doc(make_se_from_wo(wo.name, "Manufacture", 2))
		se.submit()  # Finish WO
	...
	def test_sales_order_partial_advance_payment(self):
		...
		artech_engine.get_doc(customer).insert()
	...
	def test_packed_items_for_partial_sales_order(self):
		...
		for product_bundle in [
			...
			if not artech_engine.db.exists("Item", product_bundle):
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"editable_bundle_item_rates": 1})
	def test_expired_rate_for_packed_item(self):
		...
		for scenario in [
			...
			with self.subTest(scenario=scenario):
				...
				so = artech_engine.new_doc("Sales Order")
				so.transaction_date = nowdate()
	...
	@patch(
		...
		"artech.accounts.doctype.payment_request.payment_request.PaymentRequest.get_payment_url",
		return_value=None,
	)
	def test_sales_order_advance_payment_status(self, mocked_get_payment_url):
		...
		so.submit()
		self.assertEqual(artech_engine.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested")
		...
		self.assertEqual(artech_engine.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested")
		...
		self.assertEqual(artech_engine.db.get_value(so.doctype, so.name, "advance_payment_status"), "Fully Paid")
		...
		pr.cancel()
		self.assertEqual(
			artech_engine.db.get_value(so.doctype, so.name, "advance_payment_status"), "Not Requested"
		...
		pe.cancel()
		self.assertEqual(
			artech_engine.db.get_value(so.doctype, so.name, "advance_payment_status"), "Requested"
		)  # here: reset
	...
	def test_pick_list_for_batch(self):
		from artech.stock.doctype.pick_list.pick_list import create_delivery_note
		...
		pick_list.save()
		batch_no = artech_engine.get_all(
	...
	def test_auto_update_price_list(self):
		...
		item_price = artech_engine.db.get_value("Item Price", {"item_code": item.name}, "price_list_rate")
		...
		artech_engine.db.set_single_value("Stock Settings", "update_existing_price_list_rate", 1)
		so = make_sales_order(
	...
	def test_credit_limit_on_so_reopning(self):
		# set credit limit
		company = "_Test Company"
		customer = artech_engine.get_doc("Customer", "_Test Customer")
		...
		self.assertRaises(artech_engine.ValidationError, so1.update_status, "Draft")

	@ArtechTestSuite.change_settings("Stock Settings", {"enable_stock_reservation": True})
	def test_warehouse_mapping_based_on_stock_reservation(self):
		...
		sre_doc = artech_engine.get_doc("Stock Reservation Entry", sres[0].name)
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_zero_qty_in_sales_order": 1})
	...
	def test_item_tax_transfer_from_sales_to_purchase(self):
		...
		item_tax = artech_engine.new_doc("Item Tax Template")
		...
		item_group = artech_engine.get_doc("Item Group", "_Test Item Group")
	...
	def test_selling_price_validation_for_manufactured_item(self):
		...
		self.assertRaises(artech_engine.ValidationError, so.save)
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "reserved_qty"))


def make_sales_order_workflow():
	if artech_engine.db.exists("Workflow", "SO Test Workflow"):
		doc = artech_engine.get_doc("Workflow", "SO Test Workflow")
		doc.set("is_active", 1)
		doc.save()
		return doc

	artech_engine.get_doc(doctype="Role", role_name="Test Junior Approver").insert(ignore_if_duplicate=True)
	artech_engine.get_doc(doctype="Role", role_name="Test Approver").insert(ignore_if_duplicate=True)
	artech_engine.cache().hdel("roles", artech_engine.session.user)

	workflow = artech_engine.get_doc(
		{
			"doctype": "Workflow",
