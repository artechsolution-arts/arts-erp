...
artech.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
artech.accounts.taxes.setup_tax_validations("Sales Order");
artech.sales_common.setup_selling_controller();

artech_engine.ui.form.on("Sales Order", {
	setup: function (frm) {
	...
	delivery_date(frm) {
		if (frm.doc.delivery_date) {
			frm.doc.items.forEach((d) => {
				artech_engine.model.set_value(d.doctype, d.name, "delivery_date", frm.doc.delivery_date);
			});
		}
	},
	transaction_date(frm) {
		...
		frm.doc.items.forEach((d) => {
			artech_engine.model.set_value(d.doctype, d.name, "delivery_date", "");
		});
	},
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 1) {
			if (
				...
				frm.add_custom_button(__("Update Items"), () => {
					artech.utils.update_child_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				artech_engine.model.can_cancel("Stock Reservation Entry") &&
				!frm.doc.is_subcontracted
			) {
				frm.add_custom_button(
					__("Unreserve"),
			...
			if (!frm.doc.is_subcontracted) {
				frm.doc.items.forEach((item) => {
					if (
						...
						artech_engine.model.can_read("Stock Reservation Entry")
					) {
						frm.add_custom_button(
							__("Reserved Stock"),
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0 && !frm.doc.is_subcontracted) {
				artech_engine.call({
					method: "artech.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
	...
	get_items_from_internal_purchase_order(frm) {
		if (!artech_engine.model.can_read("Purchase Order")) {
			return;
		}
		...
		frm.add_custom_button(
			...
			() => {
				artech.utils.map_current_doc({
					method: "artech.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
	...
	onload: function (frm) {
		if (!frm.doc.transaction_date) {
			frm.set_value("transaction_date", artech_engine.datetime.get_today());
		}
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "artech.controllers.queries.warehouse_query";
		...
		const dialog = new artech_engine.ui.Dialog({
			...
			fields: [
				...
				{ fieldtype: "Column Break" },
				{
					...
					get_query: () => {
						return {
							query: "artech.controllers.queries.get_filtered_child_rows",
							filters: {
			...
			primary_action: () => {
				...
				if (data.items && data.items.length > 0) {
					...
					artech_engine.msgprint(__("Please select items to reserve."));
				}
			},
		});
	...
	cancel_stock_reservation_entries(frm) {
		const dialog = new artech_engine.ui.Dialog({
			...
			primary_action: () => {
				...
				if (data.sr_entries && data.sr_entries.length > 0) {
					...
					artech_engine.msgprint(__("Please select items to unreserve."));
				}
			},
		});
		...
		artech_engine
			.call({
				method: "artech.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
	...
	show_reserved_stock(frm) {
		...
		artech_engine.set_route("query-report", "Reserved Stock");
	},
	...
	prepare_delivery_schedule(frm, row, data) {
		let fields = [
			{
				...
				reqd: 1,
				default: row.delivery_date || frm.doc.delivery_date || artech_engine.datetime.get_today(),
			},
			{
				fieldtype: "Float",
				fieldname: "qty",
			...
			},
			{
				...
				click: () => {
					artech_engine.db.get_value("UOM", row.uom, "must_be_whole_number", (r) => {
		...
		frm.schedule_dialog = new artech_engine.ui.Dialog({
			...
			primary_action: (data) => {
				if (!data.delivery_schedule || !data.delivery_schedule.length) {
					artech_engine.throw(__("Please enter at least one delivery date and quantity"));
				}

				let total_qty = 0;
				data.delivery_schedule.forEach((d) => {
					if (!d.qty) {
						artech_engine.throw(__("Please enter a valid quantity"));
					}
					total_qty += flt(d.qty);
				});
	...
	add_delivery_schedule(frm, row, must_be_whole_number) {
		...
		if (!frequency) {
			artech_engine.throw(__("Please select a frequency for delivery schedule"));
		}
		if (!first_delivery_date) {
			artech_engine.throw(__("Please enter the first delivery date"));
		}
		if (no_of_deliveries <= 0) {
			artech_engine.throw(__("Please enter a valid number of deliveries"));
		}
		...
		for (let i = 0; i < no_of_deliveries; i++) {
			...
			if (!is_last) {
				if (frequency === "Weekly") {
					first_delivery_date = artech_engine.datetime.add_days(first_delivery_date, 7);
				} else {
					first_delivery_date = artech_engine.datetime.add_months(
						first_delivery_date,
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return artech_engine.call({
			method: "artech.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return artech_engine.call({
			method: "artech.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
...
artech_engine.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
	...
	add_schedule(frm, cdt, cdn) {
		...
		if (row.__islocal) {
			artech_engine.throw(__("Please save the Sales Order before adding a delivery schedule."));
		}
	...
	fg_item: async function (frm, cdt, cdn) {
		if (frm.doc.is_subcontracted) {
			...
			if (row.fg_item) {
				...
				if (result.message && Object.keys(result.message).length) {
					artech_engine.model.set_value(cdt, cdn, "item_code", result.message.service_item);
					...
					artech_engine.model.set_value(cdt, cdn, "uom", result.message.service_item_uom);
				}
			}
		}
	},
...
artech.selling.SalesOrderController = class SalesOrderController extends artech.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
			...
			if (doc.status !== "Closed") {
				if (doc.status !== "On Hold") {
					...
					if (
						...
						flt(doc.per_delivered) < 100 &&
						artech_engine.model.can_create("Pick List") &&
					...
					if (
						...
						if (artech_engine.model.can_create("Delivery Note")) {
							this.frm.add_custom_button(
								__("Delivery Note"),
						...
						if (artech_engine.model.can_create("Work Order") && !doc.is_subcontracted) {
							this.frm.add_custom_button(
								__("Work Order"),
						...
						if (artech_engine.model.can_create("Production Plan") && !doc.is_subcontracted) {
							this.frm.add_custom_button(
								__("Production Plan"),
					...
					// sales invoice
					if (
						(flt(doc.per_billed) < 100 && artech_engine.model.can_create("Sales Invoice")) ||
						doc.is_subcontracted
					) {
						this.frm.add_custom_button(
							__("Sales Invoice"),
					...
					if (
						...
						artech_engine.model.can_create("Material Request") &&
						!doc.is_subcontracted
					) {
						if (!doc.is_subcontracted) {
							this.frm.add_custom_button(
					...
					if (
						...
						artech_engine.model.can_create("Purchase Order") &&
						!doc.is_subcontracted
					) {
						this.frm.add_custom_button(
							__("Purchase Order"),
					...
					if (flt(doc.per_delivered) < 100 && (order_is_maintenance || order_is_a_custom_sale)) {
						if (artech_engine.model.can_create("Maintenance Visit")) {
							this.frm.add_custom_button(
						...
						if (artech_engine.model.can_create("Maintenance Schedule")) {
							this.frm.add_custom_button(
					...
					// project
					if (flt(doc.per_delivered) < 100 && artech_engine.model.can_create("Project")) {
				...
				// payment request
				if (flt(doc.per_billed) < 100 + artech_engine.boot.sysdefaults.over_billing_allowance) {
					this.frm.add_custom_button(
						__("Payment Request"),
					...
					if (artech_engine.model.can_create("Payment Entry")) {
						this.frm.add_custom_button(
							__("Payment"),
		...
		if (
			...
			artech_engine.model.can_read("Quotation") &&
			!this.frm.doc.is_subcontracted
		) {
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = artech.utils.map_current_doc({
						method: "artech.selling.doctype.quotation.quotation.make_sales_order",
	...
	create_pick_list() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "artech.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
			...
			callback: function (r) {
				if (r.message.length === 0) {
					...
					var d = new artech_engine.ui.Dialog({
						title: __("Select Items to Manufacture"),
						fields: fields,
						primary_action: function () {
							var data = { items: d.fields_dict.items.grid.get_selected_children() };
							if (!data.items.length) {
								artech_engine.throw(__("Please select atleast one item to continue"));
							}
							me.frm.call({
	...
	make_production_plan() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_production_plan",
			frm: this.frm,
		});
	}
	...
	make_material_request() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new artech_engine.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "artech.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note_based_on_delivery_date(for_reserved_stock = false) {
		...
		if (!item_grid.get_selected().length && delivery_dates.length > 1) {
			var dialog = new artech_engine.ui.Dialog({
						...
						<div class="list-item">
							<div class="list-item__content list-item__content--flex-2">
								...
								<input
									type="checkbox"
									data-date="${date}"
									${artech_engine.datetime.get_day_diff(new Date(date), today) > 0 ? "" : 'checked="checked"'}
								/>
								${artech_engine.datetime.str_to_user(date)}
								</label>
							</div>
						</div>
					`
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_sales_invoice() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_sales_invoice",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
	...
	make_purchase_order() {
		...
		var dialog = new artech_engine.ui.Dialog({
			...
			primary_action(args) {
				...
				dialog.hide();
				return artech_engine.call({
					method: "artech.selling.doctype.sales_order.sales_order.make_purchase_order",
					...
					callback: function (r) {
						if (!r.exc) {
							if (r.message.length == 1) {
								artech_engine.model.sync(r.message[0]);
								artech_engine.set_route("Form", r.message[0].doctype, r.message[0].name);
							} else {
								artech_engine.route_options = {
									sales_order: me.frm.doc.name,
								};
								artech_engine.set_route("List", "Purchase Order");
							}
						}
					},
				});
	...
	hold_sales_order() {
		...
		var d = new artech_engine.ui.Dialog({
			...
			primary_action: function () {
				var data = d.get_values();
				artech_engine.call({
					method: "artech_engine.desk.form.utils.add_comment",
					args: {
						reference_doctype: me.frm.doctype,
						reference_name: me.frm.docname,
						content: __("Reason for hold:") + " " + data.reason_for_hold,
						comment_email: artech_engine.session.user,
						comment_by: artech_engine.session.user_fullname,
					},
					callback: function (r) {
						if (!r.exc) {
	...
	update_status(label, status) {
		...
		artech_engine.call({
			method: "artech.selling.doctype.sales_order.sales_order.update_status",
	...
	make_subcontracting_inward_order() {
		artech_engine.model.open_mapped_doc({
			method: "artech.selling.doctype.sales_order.sales_order.make_subcontracting_inward_order",
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new artech.selling.SalesOrderController({ frm: cur_frm }));
