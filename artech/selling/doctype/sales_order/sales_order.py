...
from artech_engine.contacts.doctype.address.address import get_company_address
from artech_engine.desk.notifications import clear_doctype_notifications
...
from artech_engine.utils import add_days, cint, cstr, flt, get_link_to_form, getdate, nowdate, parse_json, strip_html
...
from artech.manufacturing.doctype.blanket_order.blanket_order import (
...
from artech.stock.stock_balance import get_reserved_qty, update_bin_qty

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class WarehouseRequired(artech_engine.ValidationError):
	pass
...
class SalesOrder(SellingController):
	...
	if TYPE_CHECKING:
		...
		from artech.selling.doctype.sales_order_item.sales_order_item import SalesOrderItem
	...
	def onload(self) -> None:
		...
		if artech_engine.get_single_value("Stock Settings", "enable_stock_reservation"):
	...
	def can_update_items(self) -> bool:
		...
		if self.is_subcontracted:
			if artech_engine.db.exists("Subcontracting Inward Order", {"sales_order": self.name, "docstatus": 1}):
				result = False
	...
	def validate(self):
		...
		if self.coupon_code:
			from artech.accounts.doctype.pricing_rule.utils import validate_coupon_code
		...
		if self.is_subcontracted:
			for item in self.items:
				if not item.fg_item:
					...
					if not artech_engine.get_value("Item", item.fg_item, "is_sub_contracted_item"):
						artech_engine.throw(
							_("Row #{0}: Finished Good Item {1} must be a sub-contracted item").format(
								item.idx, item.fg_item
							)
						)
					if not artech_engine.db.get_value(
						"Subcontracting BOM",
						{"finished_good": item.fg_item, "is_active": 1},
						"finished_good_bom",
					) and not artech_engine.get_value("Item", item.fg_item, "default_bom"):
						artech_engine.throw(
							_("Row #{0}: BOM not found for FG Item {1}").format(item.idx, item.fg_item)
						)
				if not item.fg_item_qty:
					artech_engine.throw(_("Row #{0}: Finished Good Item Qty can not be zero").format(item.idx))
		else:
			for item in self.items:
	...
	def enable_auto_reserve_stock(self):
		if self.is_new() and artech_engine.get_single_value("Stock Settings", "auto_reserve_stock"):
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not artech_engine.get_single_value("Selling Settings", "allow_zero_qty_in_sales_order"):
			return
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			...
			if so and so[0][0]:
				if cint(
					artech_engine.get_single_value("Selling Settings", "allow_against_multiple_purchase_orders")
				):
					artech_engine.msgprint(
						_(
							"Warning: Sales Order {0} already exists against Customer's Purchase Order {1}"
						).format(artech_engine.bold(so[0][0]), artech_engine.bold(self.po_no)),
						alert=True,
					)
				else:
					artech_engine.throw(
						_(
							...
							artech_engine.bold(so[0][0]),
							artech_engine.bold(self.po_no),
							artech_engine.bold(
	...
	def validate_for_items(self):
		for d in self.get("items"):
			...
			tot_avail_qty = artech_engine.db.sql(
	...
	def validate_delivery_date(self):
		if self.order_type == "Sales" and not self.skip_delivery_note:
			...
			if self.delivery_date:
				...
				artech_engine.throw(_("Please enter Delivery Date"))
	...
	def validate_warehouse(self):
		...
		for d in self.get("items"):
			if (
				(
					artech_engine.get_cached_value("Item", d.item_code, "is_stock_item") == 1
					or (
	...
	def validate_drop_ship(self):
		for d in self.get("items"):
			if d.delivered_by_supplier and not d.supplier:
				artech_engine.throw(_("Row #{0}: Set Supplier for item {1}").format(d.idx, d.item_code))
	...
	def on_submit(self):
		...
		if self.coupon_code:
			from artech.accounts.doctype.pricing_rule.utils import update_coupon_code_count
	...
	def delete_removed_delivery_schedule_items(self):
		...
		doctype = artech_engine.qb.DocType("Delivery Schedule Item")
	...
	def on_cancel(self):
		...
		if self.status == "Closed":
			artech_engine.throw(_("Closed order cannot be cancelled. Unclose to cancel."))
	...
	def update_project(self):
		...
		if self.project:
			project = artech_engine.get_lazy_doc("Project", self.project)
	...
	def check_nextdoc_docstatus(self):
		linked_invoices = artech_engine.db.sql_list(
	...
	def check_modified_date(self):
		mod_db = artech_engine.db.get_value("Sales Order", self.name, "modified")
		date_diff = artech_engine.db.sql(f"select TIMEDIFF('{mod_db}', '{cstr(self.modified)}')")
		if date_diff and date_diff[0][0]:
			artech_engine.throw(_("{0} {1} has been modified. Please refresh.").format(self.doctype, self.name))
	...
	def update_subcontracting_order_status(self):
		from artech.subcontracting.doctype.subcontracting_inward_order.subcontracting_inward_order import (
			update_subcontracting_inward_order_status as update_scio_status,
		)

		if self.is_subcontracted:
			scio = artech_engine.get_cached_value(
	...
	def update_reserved_qty(self, so_item_rows=None):
		...
		def _valid_for_reserve(item_code, warehouse):
			if (
				...
				and artech_engine.get_cached_value("Item", item_code, "is_stock_item")
			):
	...
	def validate_supplier_after_submit(self):
		...
		for item in self.items:
			if item.supplier:
				supplier = artech_engine.db.get_value("Sales Order Item", item.name, "supplier")
	...
	def update_delivery_status(self):
		...
		for item in self.items:
			if item.delivered_by_supplier:
				item_delivered_qty = artech_engine.db.sql(
					"""select sum(qty)
	...
	def update_picking_status(self):
		...
		for so_item in self.items:
			if cint(
				artech_engine.get_cached_value("Item", so_item.item_code, "is_stock_item")
		...
		if total_picked_qty and total_qty:
			...
			pick_percentage = artech_engine.get_single_value("Stock Settings", "over_picking_allowance")
			if pick_percentage:
	...
	def on_recurring(self, reference_doc, auto_repeat_doc):
		def _get_delivery_date(ref_doc_delivery_date, red_doc_transaction_date, transaction_date):
			...
			if delivery_date <= transaction_date:
				delivery_date_diff = artech_engine.utils.date_diff(ref_doc_delivery_date, red_doc_transaction_date)
				delivery_date = artech_engine.utils.add_days(transaction_date, delivery_date_diff)
		...
		for d in self.get("items"):
			reference_delivery_date = artech_engine.db.get_value(
				"Sales Order Item",
		...
		for item in self.items:
			if item.ensure_delivery_based_on_produced_serial_no:
				...
				if item.item_code not in reserved_items:
					if not artech_engine.get_cached_value("Item", item.item_code, "has_serial_no"):
						artech_engine.throw(
							_(
								"Item {0} has no Serial No. Only serialized items can have delivery based on Serial No"
							).format(item.item_code)
						)
					if not artech_engine.db.exists("BOM", {"item": item.item_code, "is_active": 1}):
						artech_engine.throw(
							_(
	...
	def validate_reserved_stock(self):
		...
		enable_stock_reservation = artech_engine.get_single_value("Stock Settings", "enable_stock_reservation")
	...
	@artech_engine.whitelist()
	def create_delivery_schedule(self, child_row: dict | artech_engine._dict, schedules: str | list[dict]):
		if isinstance(child_row, dict):
			child_row = artech_engine._dict(child_row)
		...
		for row in schedules:
			...
			if artech_engine.db.exists("Delivery Schedule Item", row.name):
				doc = artech_engine.get_doc("Delivery Schedule Item", row.name)
			else:
				doc = artech_engine.new_doc("Delivery Schedule Item")
	...
	def delete_delivery_schedule_items(self, sales_order_item=None, ignore_names=None):
		...
		query = artech_engine.qb.from_(doctype).delete().where(doctype.sales_order == self.name)
...
def is_enable_cutoff_date_on_bulk_delivery_note_creation():
	return artech_engine.get_single_value("Selling Settings", "enable_cutoff_date_on_bulk_delivery_note_creation")


@artech_engine.whitelist()
def close_or_unclose_sales_orders(names: str, status: str):
	if not artech_engine.has_permission("Sales Order", "write"):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	names = json.loads(names)
	for name in names:
		so = artech_engine.get_lazy_doc("Sales Order", name)
		if so.docstatus == 1:
			if status == "Closed":
	...
	artech_engine.local.message_log = []


def get_requested_item_qty(sales_order):
	result = {}

	so = artech_engine.get_doc("Sales Order", sales_order)
		...
		if is_product_bundle(item.item_code):
			for packed_item in so.get("packed_items"):
				if (
					...
					result[packed_item.name] = artech_engine._dict({"qty": packed_item.requested_qty})
		else:
			result[item.name] = artech_engine._dict({"qty": item.requested_qty})
...
def make_material_request(source_name: str, target_doc: str | Document | None = None):
	...
	def postprocess(source, target):
		if source.tc_name and artech_engine.db.get_value("Terms and Conditions", source.tc_name, "buying") != 1:
			target.tc_name = None
			target.terms = None
	...
	def get_remaining_packed_item_qty(so_item):
		delivered_qty = artech_engine.db.get_value(
			"Sales Order Item", {"name": so_item.parent_detail_docname}, ["delivered_qty"]
		)

		bundle_item_qty = artech_engine.db.get_value(
	...
	def update_item(source, target, source_parent):
		...
		ctx.update(
			{
				...
				"price_list": artech_engine.db.get_single_value("Buying Settings", "buying_price_list"),
		...
		target.rate = flt(
			get_price_list_rate(ctx, item_doc=artech_engine.get_cached_doc("Item", target.item_code)).get(
				"price_list_rate"
			)
		)
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Sales Order Item": {
				...
				"condition": lambda item: not artech_engine.db.exists(
	...
	if doc and doc.items:
		...
		artech_engine.throw(_("Material Request already created for the ordered quantity"))
...
def make_delivery_note(
	...
	if not kwargs:
		kwargs = {
			"for_reserved_stock": artech_engine.flags.args and artech_engine.flags.args.for_reserved_stock,
			"skip_item_mapping": artech_engine.flags.args and artech_engine.flags.args.skip_item_mapping,
		}
	...
	has_unit_price_items = artech_engine.db.get_value("Sales Order", source_name, "has_unit_price_items")
	...
	def set_missing_values(source, target):
		...
		if artech_engine.flags.bulk_transaction:
	...
	def condition(doc):
		...
		# make_mapped_doc sets js `args` into `artech_engine.flags.args`
		if artech_engine.flags.args and artech_engine.flags.args.delivery_dates:
			if cstr(doc.delivery_date) not in artech_engine.flags.args.delivery_dates:
				return False
		if artech_engine.flags.args and artech_engine.flags.args.until_delivery_date:
			if cstr(doc.delivery_date) > artech_engine.flags.args.until_delivery_date:
				return False
	...
	def update_item(source, target, source_parent):
		...
		if item:
			target.cost_center = (
				artech_engine.db.get_value("Project", source_parent.project, "cost_center")
	...
	so = artech_engine.get_doc("Sales Order", source_name)
	...
	if not kwargs.skip_item_mapping and artech_engine.flags.bulk_transaction and not target_doc.items:
...
def make_sales_invoice(
	...
	def update_item(source, target, source_parent):
		def get_billed_qty(so_item_name):
			...
			table = artech_engine.qb.DocType("Sales Invoice Item")
			query = (
		...
		if source_parent.project:
			target.cost_center = artech_engine.db.get_value("Project", source_parent.project, "cost_center")
		if target.item_code:
			item = get_item_defaults(target.item_code, source_parent.company)
			item_group = get_item_group_defaults(target.item_code, source_parent.company)
	...
	def add_self_rm(doclist):
		parent = artech_engine.qb.DocType("Subcontracting Inward Order")
		child = artech_engine.qb.DocType("Subcontracting Inward Order Received Item")
		query = (
	...
	if artech_engine.get_cached_value("Sales Order", source_name, "is_subcontracted"):
		add_self_rm(doclist)
...
def make_maintenance_schedule(source_name: str, target_doc: str | Document | None = None):
	maint_schedule = artech_engine.db.sql(
		"""select t1.name
...
def get_events(start: str, end: str, filters: str | dict | None = None):
	...
	"""
	from artech_engine.desk.calendar import get_event_conditions
...
def make_purchase_order(
	...
	def set_missing_values(source, target):
		...
		company_currency = artech_engine.db.get_value(
			"Company", filters={"name": target.company}, fieldname=["default_currency"]
		)
		supplier_currency = artech_engine.db.get_value(
		...
		default_price_list = artech_engine.get_value("Supplier", supplier, "default_price_list")
		if default_price_list:
			target.buying_price_list = default_price_list

		default_payment_terms = artech_engine.get_value("Supplier", supplier, "payment_terms")
...
def is_product_bundle(item_code):
	return artech_engine.db.exists("Product Bundle", {"name": item_code, "disabled": 0})
...
def make_work_orders(items: str, sales_order: str, company: str, project: str | None = None):
	...
	for i in items:
		if not i.get("bom"):
			artech_engine.throw(_("Please select BOM against item {0}").format(i.get("item_code")))
		if not i.get("pending_qty"):
			artech_engine.throw(_("Please select Qty against item {0}").format(i.get("item_code")))

		work_order = artech_engine.get_doc(
			doctype="Work Order",
...
def make_production_plan(source_name: str, target_doc: str | Document | None = None):
	sales_order = artech_engine.get_doc("Sales Order", source_name)

	production_plan = artech_engine.new_doc(
		"Production Plan",
	...
	if sales_order.name not in open_so:
		artech_engine.throw(_("Sales Order {0} is not available for production").format(sales_order.name))
...
def update_status(status: str, name: str):
	so = artech_engine.get_doc("Sales Order", name, check_permission="submit")
	so.update_status(status)


@artech_engine.whitelist()
def make_raw_material_request(
	items: str | artech_engine._dict, company: str, sales_order: str, project: str | None = None
):
	if not artech_engine.has_permission("Sales Order", "write"):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	if isinstance(items, str):
		items = artech_engine._dict(json.loads(items))
	...
	if not raw_materials:
		artech_engine.msgprint(_("Material Request not created, as quantity for Raw Materials already available."))
		return

	material_request = artech_engine.new_doc("Material Request")
	...
	for item in raw_materials:
		item_doc = artech_engine.get_cached_doc("Item", item.get("item_code"))
...
def create_pick_list(source_name: str, target_doc: str | Document | None = None):
	from artech.stock.doctype.packed_item.packed_item import is_product_bundle
		...
		for item in so.items:
			if item.stock_reserved_qty > 0:
				artech_engine.throw(
					...
					).format(artech_engine.bold(source_name))
				)
...
def update_produced_qty_in_so_item(sales_order, sales_order_item):
	...
	linked_wo_with_so_item = artech_engine.db.get_all(
		"Work Order",
		["produced_qty"],
	...
	if not total_produced_qty and artech_engine.flags.in_patch:
		return

	artech_engine.db.set_value("Sales Order Item", sales_order_item, "produced_qty", total_produced_qty)
...
def get_work_order_items(sales_order: str, for_raw_material_request: int = 0):
	...
	if sales_order:
		...
		product_bundle_parents = [
			pb.new_item_code
			for pb in artech_engine.get_all(
				"Product Bundle", {"new_item_code": ["in", item_codes], "disabled": 0}, ["new_item_code"]
			)
		]

		overproduction_percentage_for_sales_order = (
			artech_engine.get_single_value("Manufacturing Settings", "overproduction_percentage_for_sales_order")
			/ 100
		)
...
def get_stock_reservation_status():
	return artech_engine.get_single_value("Stock Settings", "enable_stock_reservation")
...
def make_subcontracting_inward_order(source_name: str, target_doc: str | Document | None = None):
	if not is_so_fully_subcontracted(source_name):
		...
		artech_engine.throw(_("This Sales Order has been fully subcontracted."))


def is_so_fully_subcontracted(so_name):
	table = artech_engine.qb.DocType("Sales Order Item")
	query = (
		artech_engine.qb.from_(table)
		.select(table.name)
...
def get_mapped_subcontracting_inward_order(source_name, target_doc=None):
	def post_process(source_doc, target_doc):
		if (
			artech_engine.db.count(
				"Warehouse", {"customer": source_doc.customer, "disabled": 0, "is_rejected_warehouse": 0}
			)
			== 1
		):
			target_doc.customer_warehouse = artech_engine.get_cached_value(
				"Warehouse",
