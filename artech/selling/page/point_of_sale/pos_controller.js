artech.PointOfSale.Controller = class {
	constructor(wrapper) {
	...
	fetch_opening_entry() {
		return artech_engine.call("artech.selling.page.point_of_sale.point_of_sale.check_opening_entry", {
	...
	create_opening_voucher() {
		...
		const fetch_pos_payment_methods = () => {
			...
			artech_engine.db.get_doc("POS Profile", pos_profile).then(({ payments }) => {
		...
		const dialog = new artech_engine.ui.Dialog({
			...
			primary_action: async function ({ company, pos_profile, balance_details }) {
				if (!balance_details.length) {
					...
					return artech_engine.utils.play_sound("error");
				}

				// filter balance details for empty rows
				balance_details = balance_details.filter((d) => d.mode_of_payment);

				const method = "artech.selling.page.point_of_sale.point_of_sale.create_opening_voucher";
				const res = await artech_engine.call({
					method,
	...
	async prepare_app_defaults(data) {
		...
		artech_engine.db.get_value("Stock Settings", undefined, "allow_negative_stock").then(({ message }) => {
			this.allow_negative_stock = flt(message.allow_negative_stock) || false;
		});

		const invoice_doctype = await artech_engine.db.get_single_value("POS Settings", "invoice_type");

		artech_engine.call({
			method: "artech.selling.page.point_of_sale.point_of_sale.get_pos_profile_data",
	...
	async fetch_invoice_fields() {
		...
		const pos_settings = await artech_engine.db.get_doc("POS Settings", undefined);
	...
	setup_listener_for_pos_closing() {
		artech_engine.realtime.on(`poe_${this.pos_opening}`, (data) => {
			const route = artech_engine.get_route_str();
			if (data && route == "point-of-sale") {
				artech_engine.dom.freeze();
				const title =
				...
				const msg =
					data.operation === "Closed"
						? __("POS has been closed at {0}. Please refresh the page.", [
								artech_engine.datetime.str_to_user(data.doc?.creation).bold(),
						  ])
	...
	check_outdated_pos_opening_entry() {
		if (artech_engine.datetime.get_day_diff(artech_engine.datetime.get_today(), this.pos_opening_time.slice(0, 10))) {
			artech_engine.msgprint({
		...
		this.page.set_title_sub(
			`<span class="indicator orange">
				<a class="text-muted" href="#Form/POS%20Opening%20Entry/${this.pos_opening}">
					Opened at ${artech_engine.datetime.str_to_user(this.pos_opening_time)}
				</a>
			</span>`
		);
	}
	...
	open_form_view() {
		artech_engine.model.sync(this.frm.doc);
		artech_engine.set_route("Form", this.frm.doc.doctype, this.frm.doc.name);
	}
	...
	close_pos() {
		...
		let voucher = artech_engine.model.get_new_doc("POS Closing Entry");
		voucher.pos_profile = this.frm.doc.pos_profile;
		voucher.user = artech_engine.session.user;
		voucher.company = this.frm.doc.company;
		voucher.pos_opening_entry = this.pos_opening;
		voucher.period_end_date = artech_engine.datetime.now_datetime();
		voucher.posting_date = artech_engine.datetime.now_date();
		voucher.posting_time = artech_engine.datetime.now_time();
		artech_engine.set_route("Form", "POS Closing Entry", voucher.name);
	}

	init_item_selector() {
		this.item_selector = new artech.PointOfSale.ItemSelector({
	...
	init_item_cart() {
		this.cart = new artech.PointOfSale.ItemCart({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
	...
	init_item_details() {
		this.item_details = new artech.PointOfSale.ItemDetails({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
				...
				form_updated: (item, field, value) => {
					const item_row = artech_engine.model.get_doc(item.doctype, item.name);
	...
	init_payments() {
		this.payment = new artech.PointOfSale.Payment({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm || {},
	...
	init_recent_order_list() {
		this.recent_order_list = new artech.PointOfSale.PastOrderList({
			wrapper: this.$components_wrapper,
			events: {
				open_invoice_data: (doctype, name) => {
					if (!["POS Invoice", "Sales Invoice"].includes(doctype)) return;
					artech_engine.db.get_doc(doctype, name).then((doc) => {
	...
	init_order_summary() {
		this.order_summary = new artech.PointOfSale.PastOrderSummary({
			wrapper: this.$components_wrapper,
			settings: this.settings,
			events: {
				get_frm: () => this.frm,
				...
				delete_order: (doctype, name) => {
					artech_engine.model.with_doctype(doctype, () => {
						artech_engine.model.delete_doc(doctype, name, () => {
				...
				open_in_form_view: (doctype, name) => {
					artech_engine.run_serially([
						() => artech_engine.dom.freeze(),
						() => artech_engine.set_route("Form", doctype, name),
						() => artech_engine.dom.unfreeze(),
					]);
				},
			},
	...
	get_new_frm(_frm, doctype = this.settings.frm_doctype) {
		const page = $("<div>");
		const frm = _frm || new artech_engine.ui.form.Form(doctype, page, false);
		const name = artech_engine.model.make_new_doc_and_get_name(doctype, true);
		frm.refresh(name);
	...
	sync_draft_invoice_to_frm(doctype, invoice) {
		return artech_engine.db.get_doc(doctype, invoice).then((doc) => {
			artech_engine.model.sync(doc);
		});
	}
		...
		return artech_engine.call({
			method:
				doc.doctype == "POS Invoice"
					? "artech.accounts.doctype.pos_invoice.pos_invoice.make_sales_return"
					: "artech.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			args: {
			...
			callback: (r) => {
				...
				artech_engine.get_doc(r.message.doctype, r.message.name).__run_link_triggers = false;
				this.set_pos_profile_data();
			},
		});
	}
		...
		try {
			...
			if (item_row_exists) {
				...
				if (this.is_current_item_being_edited(item_row) || from_selector) {
					await artech_engine.model.set_value(item_row.doctype, item_row.name, field, value);
					if (item.serial_no && from_selector) {
						await artech_engine.model.set_value(
							item_row.doctype,
							item_row.name,
							"serial_no",
							item_row.serial_no + `\n${item.serial_no}`
				...
				if (rate == undefined || rate == 0) {
					...
					artech_engine.utils.play_sound("error");
					return;
				}
	...
	async check_stock_availability(item_row, qty_needed, warehouse) {
		...
		if (!(available_qty > 0)) {
			if (is_stock_item) {
				artech_engine.model.clear_doc(item_row.doctype, item_row.name);
				artech_engine.throw({
	...
	async check_serial_no_availablilty(item_code, warehouse, serial_no) {
		const method = "artech.stock.doctype.serial_no.serial_no.get_pos_reserved_serial_nos";
		const args = { filters: { item_code, warehouse } };
		const res = await artech_engine.call({ method, args });
	...
	get_available_stock(item_code, warehouse) {
		const me = this;
		return artech_engine.call({
			method: "artech.accounts.doctype.pos_invoice.pos_invoice.get_stock_availability",
			args: {
				item_code: item_code,
				warehouse: warehouse,
			},
	...
	remove_item_from_cart() {
		...
		return artech_engine.model
			.set_value(doctype, name, "qty", 0)
			.then(() => {
				artech_engine.model.clear_doc(doctype, name);
