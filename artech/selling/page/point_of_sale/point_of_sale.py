...
from artech_engine.query_builder import DocType, Order
from artech_engine.utils import cint, get_datetime
from artech_engine.utils.nestedset import get_root_of

from artech.accounts.doctype.pos_invoice.pos_invoice import get_item_group, get_stock_availability
from artech.accounts.doctype.pos_profile.pos_profile import get_child_nodes, get_item_groups
from artech.stock.get_item_details import get_conversion_factor
from artech.stock.utils import scan_barcode
...
def filter_result_items(result, pos_profile):
	if result and result.get("items"):
		pos_profile_doc = artech_engine.get_cached_doc("POS Profile", pos_profile)
...
def get_parent_item_group(pos_profile: str):
	...
	if not item_groups:
		item_groups = artech_engine.get_all("Item Group", {"lft": 1, "is_group": 1}, pluck="name")
...
def get_items(
	...
	warehouse, hide_unavailable_items = artech_engine.db.get_value(
	...
	if not artech_engine.db.exists("Item Group", item_group):
		item_group = get_root_of("Item Group")

	condition = get_conditions(search_term)
	condition += get_item_group_condition(pos_profile)

	lft, rgt = artech_engine.db.get_value("Item Group", item_group, ["lft", "rgt"])
	...
	items_data = artech_engine.db.sql(
		"""
		SELECT
	...
	current_date = artech_engine.utils.today()
	...
	for item in items_data:
		...
		item_prices = (
			artech_engine.qb.from_(ItemPrice)
			.select(
...
def get_conditions(search_term):
	...
	condition += """item.name like {search_term}
		or item.item_name like {search_term}""".format(search_term=artech_engine.db.escape("%" + search_term + "%"))
...
def add_search_fields_condition(search_term):
	condition = ""
	search_fields = artech_engine.get_all("POS Search Fields", fields=["fieldname"])
	if search_fields:
		for field in search_fields:
			if not field.get("fieldname"):
				continue
			condition += " or item.`{}` like {}".format(
				field["fieldname"], artech_engine.db.escape("%" + search_term + "%")
			)
	return condition
...
def check_opening_entry(user: str):
	open_vouchers = artech_engine.db.get_all(
		"POS Opening Entry",
...
def create_opening_voucher(pos_profile: str, company: str, balance_details: str):
	...
	new_pos_opening = artech_engine.get_doc(
		{
			"doctype": "POS Opening Entry",
			"period_start_date": artech_engine.utils.get_datetime(),
			"posting_date": artech_engine.utils.getdate(),
...
def get_past_order_list(search_term: str, status: str, limit: int = 20):
	...
	if search_term and status:
		pos_invoices_by_customer = artech_engine.db.get_list(
			"POS Invoice",
		...
		pos_invoices_by_name = artech_engine.db.get_list(
			"POS Invoice",
		...
		sales_invoices_by_customer = artech_engine.db.get_list(
			"Sales Invoice",
		...
		sales_invoices_by_name = artech_engine.db.get_list(
			"Sales Invoice",
		...
		pos_invoice_list = artech_engine.db.get_list(
			"POS Invoice",
			filters=get_invoice_filters("POS Invoice", status),
			fields=fields,
			page_length=limit,
		...
		sales_invoice_list = artech_engine.db.get_list(
			"Sales Invoice",
			filters=get_invoice_filters("Sales Invoice", status),
			fields=fields,
			page_length=limit,
...
def set_customer_info(fieldname: str, customer: str, value: str = ""):
	if fieldname == "loyalty_program":
		artech_engine.db.set_value("Customer", customer, "loyalty_program", value)

	contact = artech_engine.get_cached_value("Customer", customer, "customer_primary_contact")
	if not contact:
		contact = artech_engine.db.sql(
			"""
	...
	if not contact:
		new_contact = artech_engine.new_doc("Contact")
		new_contact.is_primary_contact = 1
		new_contact.first_name = customer
		new_contact.set("links", [{"link_doctype": "Customer", "link_name": customer}])
		new_contact.save()
		contact = new_contact.name
		artech_engine.db.set_value("Customer", customer, "customer_primary_contact", contact)

	contact_doc = artech_engine.get_doc("Contact", contact)
	if fieldname == "email_id":
		contact_doc.set("email_ids", [{"email_id": value, "is_primary": 1}])
		artech_engine.db.set_value("Customer", customer, "email_id", value)
	elif fieldname == "mobile_no":
		contact_doc.set("phone_nos", [{"phone": value, "is_primary_mobile_no": 1}])
		artech_engine.db.set_value("Customer", customer, "mobile_no", value)
	contact_doc.save()


@artech_engine.whitelist()
def get_pos_profile_data(pos_profile: str):
	pos_profile = artech_engine.get_doc("POS Profile", pos_profile)
...
def get_customer_recent_transactions(customer: str):
	sales_invoices = artech_engine.db.get_list(
		"Sales Invoice",
		filters={
			"customer": customer,
			"docstatus": 1,
	...
	pos_invoices = artech_engine.db.get_list(
		"POS Invoice",
