artech.PointOfSale.ItemDetails = class {
	...
	async toggle_item_details_section(item) {
		...
		if (item && current_item_changed) {
			...
			this.item_meta = artech_engine.get_meta(this.doctype);
			this.name = item.name;
			this.item_row = item;
	...
	validate_serial_batch_item() {
		...
		if ((serialized && no_bundle_selected) || (batched && no_bundle_selected)) {
			...
			artech_engine.utils.play_sound("cancel");
		...
		if (!this.hide_images && image) {
			this.$item_image.html(
				`<img
					...
					alt="${artech_engine.get_abbr(item_name)}"
					style="object-fit: cover;">`
			);
		} else {
			this.$item_image.html(`<div class="item-abbr">${artech_engine.get_abbr(item_name)}</div>`);
		}
	}
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			this[`${fieldname}_control`] = artech_engine.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function () {
	...
	bind_custom_control_change_event() {
			...
			this.rate_control.df.onchange = function () {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, "rate", this.value).then(() => {
						const item_row = artech_engine.get_doc(me.doctype, me.name);
		...
		artech_engine.model.on(`${frm_doctype} Item`, "*", (fieldname, value, item_row) => {
	...
	async auto_update_batch_no() {
		if (this.serial_no_control && this.batch_no_control) {
			...
			const serials_with_batch_no = await artech_engine.db.get_list("Serial No", {
	...
	bind_auto_serial_fetch_event() {
		this.$form_container.on("click", ".auto-fetch-btn", () => {
			...
			let numbers = artech_engine.call({
				method: "artech.stock.doctype.serial_no.serial_no.auto_fetch_serial_number",
				args: {
			...
			numbers.then((data) => {
				...
				if (!records_length) {
					...
					artech_engine.msgprint(__("Fetched only {0} available serial numbers.", [records_length]));
