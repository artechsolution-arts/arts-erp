artech.PointOfSale.PastOrderSummary = class {
	...
	init_email_print_dialog() {
		const email_dialog = new artech_engine.ui.Dialog({
		...
		const print_dialog = new artech_engine.ui.Dialog({
	...
	get_upper_section_html(doc) {
		const { status } = doc;
		let indicator_color = "";
		const is_customer_naming_by_customer_name = artech_engine.sys_defaults.cust_master_name !== "Customer Name";
	...
	async get_item_html(doc, item_data) {
		...
		async function get_returned_qty() {
			const r = await artech_engine.call({
				method: "artech.controllers.sales_and_purchase_return.get_invoice_item_returned_qty",
				args: {
					doctype: doc.doctype,
					invoice: doc.name,
	...
	print_receipt() {
		...
		artech_engine.utils.print(
			this.doc.doctype,
			this.doc.name,
			frm.pos_print_format,
			this.doc.letter_head,
			this.doc.language || artech_engine.boot.lang
		);
	}
	...
	send_email() {
		...
		artech_engine.call({
			method: "artech_engine.core.doctype.communication.email.make",
			args: {
				...
				send_email: 1,
				print_format,
				sender_full_name: artech_engine.user.full_name(),
				_lang: doc.language,
			},
			callback: (r) => {
				if (!r.exc) {
					artech_engine.utils.play_sound("email");
					if (r.message["emails_not_sent_to"]) {
						artech_engine.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								artech_engine.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					} else {
					...
					artech_engine.msgprint(__("There were errors while sending email. Please try again."));
				}
			},
		});
	}
	...
	attach_document_info(doc) {
		artech_engine.db.get_value("Customer", this.doc.customer, "email_id").then(({ message }) => {
	...
	async is_invoice_returnable(doctype, invoice) {
		const r = await artech_engine.call({
			method: "artech.controllers.sales_and_purchase_return.is_invoice_returnable",
			args: {
				doctype: doctype,
				invoice: invoice,
			},
