...
artech.PointOfSale.ItemSelector = class {
	...
	async get_parent_item_group() {
		const r = await artech_engine.call({
			method: "artech.selling.page.point_of_sale.point_of_sale.get_parent_item_group",
			args: {
	...
	async load_items_data() {
		...
		if (!this.price_list) {
			const res = await artech_engine.db.get_value("POS Profile", this.pos_profile, "selling_price_list");
	...
	get_items({ start = 0, page_length = 40, search_term = "" }) {
		...
		return artech_engine.call({
			method: "artech.selling.page.point_of_sale.point_of_sale.get_items",
			freeze: true,
	...
	get_item_html(item) {
		...
		function get_item_image_html() {
			...
			if (item_image) {
				...
				return `<div class="item-qty-pill">
						...
						<div class="item-display abbr">${artech_engine.get_abbr(item.item_name)}</div>`;
			}
		}
		...
		return `<div class="item-wrapper"
				...
				<div class="item-detail">
					<div class="item-name">
						${!me.hide_images ? artech_engine.ellipsis(item.item_name, 18) : item.item_name}
					</div>
					${
						!me.hide_images
							? `<div class="item-rate">
	...
	make_search_bar() {
		...
		this.search_field = artech_engine.ui.form.make_control({
			df: {
				label: __("Search"),
				fieldtype: "Data",
		...
		this.item_group_field = artech_engine.ui.form.make_control({
			df: {
				label: __("Item Group"),
				fieldtype: "Link",
				options: "Item Group",
				...
				get_query: function () {
					return {
						query: "artech.selling.page.point_of_sale.point_of_sale.item_group_query",
						filters: {
		...
		this.search_field.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Clear")}">
					${artech_engine.utils.icon("close", "sm")}
				</a>
			</span>`
		);
	...
	attach_shortcuts() {
		...
		artech_engine.ui.keys.on("enter", () => {
