...
artech.PointOfSale.Payment = class {
	...
	make_invoice_field_dialog() {
		...
		me.addl_dlg = new artech_engine.ui.Dialog({
	...
	on_numpad_clicked($btn, from_numpad = true) {
		...
		const number_format_details = get_number_format_info(artech_engine.sys_defaults.number_format);
		const precision = artech_engine.sys_defaults.currency_precision || number_format_details.precision;
		this.numpad_value = "0";
		...
		valid_input && artech_engine.utils.play_sound("numpad-touch");
	...
	bind_events() {
		...
		artech_engine.ui.form.on("POS Invoice", "contact_mobile", (frm) => {
		...
		artech_engine.ui.form.on("POS Invoice", "coupon_code", (frm) => {
			this.bind_coupon_code_event(frm);
		});

		artech_engine.ui.form.on("Sales Invoice", "coupon_code", (frm) => {
			this.bind_coupon_code_event(frm);
		});
		...
		this.$component.on("click", ".submit-order-btn", () => {
			...
			if (
				...
				artech_engine.show_alert({ message, indicator: "orange" });
		...
		artech_engine.ui.form.on("POS Invoice", "loyalty_amount", (frm) => {
		...
		artech_engine.ui.form.on("Sales Invoice", "loyalty_amount", (frm) => {
			this.bind_loyalty_amount_event(frm);
		});

		artech_engine.ui.form.on("Sales Invoice Payment", "amount", (frm, cdt, cdn) => {
	...
	setup_listener_for_payments() {
		artech_engine.realtime.on("process_phone_payment", (data) => {
			const doc = this.events.get_frm().doc;
			...
			artech_engine.msgprint({ message: message, title: title });
		});
	}
	...
	attach_shortcuts() {
		...
		artech_engine.ui.keys.on("ctrl+enter", () => {
	...
	toggle_remarks_control() {
		if (this.$remarks.find(".artech_engine-control").length) {
			this.$remarks.html("+ Add Remark");
		} else {
			this.$remarks.html("");
			this[`remark_control`] = artech_engine.ui.form.make_control({
				df: {
					label: __("Remark"),
					fieldtype: "Data",
					onchange: function () {},
	...
	render_payment_mode_dom() {
		...
		payments.forEach((p) => {
			const mode = this.sanitize_mode_of_payment(p.mode_of_payment);
			const me = this;
			this[`${mode}_control`] = artech_engine.ui.form.make_control({
				df: {
					label: p.mode_of_payment,
					fieldtype: "Currency",
					placeholder: __("Enter {0} amount.", [__(p.mode_of_payment)]),
					onchange: function () {
						const current_value = artech_engine.model.get_value(p.doctype, p.name, "amount");
	...
	render_loyalty_points_payment_mode() {
		...
		this["loyalty-amount_control"] = artech_engine.ui.form.make_control({
			df: {
