artech.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new artech.PointOfSale.NumberPad({
	...
	bind_events() {
		...
		artech_engine.ui.form.on("POS Invoice", "paid_amount", (frm) => {
			// called when discount is applied
			this.update_totals_section(frm);
		});

		artech_engine.ui.form.on("Sales Invoice", "paid_amount", (frm) => {
			// called when discount is applied
			this.update_totals_section(frm);
		});
	}
	...
	attach_shortcuts() {
		for (let row of this.number_pad.keys) {
			for (let btn of row) {
				...
				let shortcut_key = `ctrl+${artech_engine.scrub(String(btn))[0]}`;
				...
				const fieldname = this.number_pad.fieldnames[btn]
					...
					: typeof btn === "string"
					? artech_engine.scrub(btn)
					: btn;

				let shortcut_label = shortcut_key.split("+").map(artech_engine.utils.to_title_case).join("+");
				shortcut_label = artech_engine.utils.is_mac() ? shortcut_label.replace("Ctrl", "⌘") : shortcut_label;
				this.$numpad_section
					.find(`.numpad-btn[data-button-value="${fieldname}"]`)
					.attr("title", shortcut_label);

				artech_engine.ui.keys.on(`${shortcut_key}`, () => {
		...
		const ctrl_label = artech_engine.utils.is_mac() ? "⌘" : "Ctrl";
		this.$component.find(".checkout-btn").attr("title", `${ctrl_label}+Enter`);
		artech_engine.ui.keys.add_shortcut({
			shortcut: "ctrl+enter",
		...
		artech_engine.ui.keys.on("ctrl+e", () => {
		...
		artech_engine.ui.keys.on("escape", () => {
			const item_cart_visible = this.$component.is(":visible");
	...
	make_customer_selector() {
		...
		this.customer_field = artech_engine.ui.form.make_control({
			df: {
				label: __("Customer"),
				fieldtype: "Link",
				options: "Customer",
				...
				onchange: function () {
					if (this.value) {
						...
						artech_engine.model.set_value(frm.doc.doctype, frm.doc.name, "customer", this.value);
			...
			return new Promise((resolve) => {
				artech_engine.db
					...
					.then(({ message }) => {
						...
						if (loyalty_program) {
							artech_engine.call({
								method: "artech.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details_with_points",
	...
	show_discount_control() {
		...
		this.discount_field = artech_engine.ui.form.make_control({
			df: {
				label: __("Discount"),
				fieldtype: "Data",
	...
	get_customer_image() {
		...
		if (image) {
			...
			return `<div class="customer-image customer-abbr">${artech_engine.get_abbr(customer)}</div>`;
		}
	}
	...
	update_totals_section(frm) {
		...
		const grand_total = cint(artech_engine.sys_defaults.disable_rounded_total)
			? frm.doc.grand_total
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_description_html() {
			if (item_data.description) {
				...
				item_data.description = artech_engine.ellipsis(item_data.description, 45);
				...
				return `
					<div class="item-image">
						<img
							...
							src="${image}" alt="${artech_engine.get_abbr(item_name)}"">
					</div>`;
			} else {
				return `<div class="item-image item-abbr">${artech_engine.get_abbr(item_name)}</div>`;
			}
		}
	}
	...
	render_customer_fields() {
		...
		const me = this;
		dfs.forEach((df) => {
			this[`customer_${df.fieldname}_field`] = artech_engine.ui.form.make_control({
				df: df,
		...
		function handle_customer_field_change() {
			...
			if (this.value && current_value != this.value && this.df.fieldname != "loyalty_points") {
				artech_engine.call({
					method: "artech.selling.page.point_of_sale.point_of_sale.set_customer_info",
					args: {
					...
					callback: (r) => {
						if (!r.exc) {
							...
							artech_engine.utils.play_sound("submit");
						}
					},
				});
			}
	...
	fetch_customer_transactions() {
		artech_engine
			.call({
				method: "artech.selling.page.point_of_sale.point_of_sale.get_customer_recent_transactions",
			...
			.then((res) => {
				...
				res.forEach((invoice) => {
					const posting_datetime = artech_engine.datetime.str_to_user(
					...
					transaction_container.append(
						...
						<div class="invoice-total-status">
							<div class="invoice-total">
								${format_currency(invoice.grand_total, invoice.currency, artech_engine.sys_defaults.currency_precision) || 0}
							</div>
