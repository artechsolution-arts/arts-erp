...
artech_engine.pages["sales-funnel"].on_page_load = function (wrapper) {
	...
	wrapper.sales_funnel = new artech.SalesFunnel(wrapper);

	artech_engine.breadcrumbs.add("Selling");
};

artech.SalesFunnel = class SalesFunnel {
	constructor(wrapper) {
		var me = this;
	...
	setup(wrapper) {
		...
		(this.company_field = wrapper.page.add_field({
			...
			change: function () {
				me.company = this.value || artech_engine.defaults.get_user_default("company");
				me.get_data();
			},
		})),
			(this.elements = {
		...
		this.company = artech_engine.defaults.get_user_default("company");
		this.options = {
			from_date: artech_engine.datetime.add_months(artech_engine.datetime.get_today(), -1),
			to_date: artech_engine.datetime.get_today(),
			chart: "sales_funnel",
		};
		...
		$.each(this.options, function (k, v) {
			if (["from_date", "to_date"].includes(k)) {
				me.elements[k].val(artech_engine.datetime.str_to_user(v));
			} else {
				me.elements[k].val(v);
			}
			...
			me.elements[k].on("change", function () {
				if (["from_date", "to_date"].includes(k)) {
					me.options[k] =
						artech_engine.datetime.user_to_str($(this).val()) != "Invalid date"
							? artech_engine.datetime.user_to_str($(this).val())
	...
	get_data(btn) {
		var me = this;
		if (!this.company) {
			artech_engine.throw(__("Please Select a Company."));
		}

		const method_map = {
			sales_funnel: "artech.selling.page.sales_funnel.sales_funnel.get_funnel_data",
			opp_by_utm_source: "artech.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_source",
			opp_by_utm_campaign: "artech.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_campaign",
			opp_by_utm_medium: "artech.selling.page.sales_funnel.sales_funnel.get_opp_by_utm_medium",
			sales_pipeline: "artech.selling.page.sales_funnel.sales_funnel.get_pipeline_data",
		};
		artech_engine.call({
	...
	render_chart(title) {
		let me = this;
		let currency = artech_engine.defaults.get_default("currency");

		let chart_data = me.options.data ? me.options.data : null;

		const parent = me.elements.funnel_wrapper[0];
		this.chart = new artech_engine.Chart(parent, {
			title: title,
			height: 400,
			data: chart_data,
			type: "bar",
