...
from artech.accounts.report.utils import convert


def validate_filters(from_date, to_date, company):
	if from_date and to_date and (from_date >= to_date):
		artech_engine.throw(_("To Date must be greater than From Date"))
...
def get_funnel_data(from_date: str, to_date: str, company: str):
	...
	active_leads = artech_engine.db.sql(
	...
	opportunities = artech_engine.db.sql(
	...
	quotations = artech_engine.db.sql(
	...
	converted = artech_engine.db.sql(
...
def get_opp_by(by_field, from_date, to_date, company):
	...
	opportunities = artech_engine.get_all(
		"Opportunity",
		filters=[
	...
	if opportunities:
		default_currency = artech_engine.get_cached_value("Global Defaults", "None", "default_currency")
