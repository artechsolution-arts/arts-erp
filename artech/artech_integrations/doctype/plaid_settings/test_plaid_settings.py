...
from artech_engine.utils.response import json_handler

from artech.artech_integrations.doctype.plaid_settings.plaid_settings import (
	add_account_subtype,
	add_account_type,
	add_bank_accounts,
...
class TestPlaidSettings(ArtechTestSuite):
	def test_plaid_disabled(self):
		artech_engine.db.set_single_value("Plaid Settings", "enabled", 0)
		self.assertTrue(get_plaid_configuration() == "disabled")

	def test_add_account_type(self):
		add_account_type("brokerage")
		self.assertEqual(artech_engine.get_doc("Bank Account Type", "brokerage").name, "brokerage")

	def test_add_account_subtype(self):
		add_account_subtype("loan")
		self.assertEqual(artech_engine.get_doc("Bank Account Subtype", "loan").name, "loan")

	def test_new_transaction(self):
		if not artech_engine.db.exists("Bank", "Citi"):
			artech_engine.get_doc({"doctype": "Bank", "bank_name": "Citi"}).insert()
		...
		bank = json.dumps(artech_engine.get_doc("Bank", "Citi").as_dict(), default=json_handler)
		...
		self.assertTrue(len(artech_engine.get_all("Bank Transaction")) == 1)
