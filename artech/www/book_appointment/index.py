...
from artech_engine.utils.data import get_system_timezone
...
def get_context(context):
	is_enabled = artech_engine.db.get_single_value("Appointment Booking Settings", "enable_scheduling")
	if is_enabled:
		return context
	else:
		artech_engine.redirect_to_message(
			_("Appointment Scheduling Disabled"),
			_("Appointment Scheduling has been disabled for this site"),
			http_status_code=302,
			indicator_color="red",
		)
		raise artech_engine.Redirect


@artech_engine.whitelist(allow_guest=True)
def get_appointment_settings():
	settings = artech_engine.get_cached_value(
...
def get_appointment_slots(date: str, timezone: str):
	...
	# Database queries
	settings = artech_engine.get_doc("Appointment Booking Settings")
	holiday_list = artech_engine.get_doc("Holiday List", settings.holiday_list)
...
def create_appointment(date: str, time: str, tz: str, contact: str):
	...
	appointment = artech_engine.new_doc("Appointment")
...
def check_availabilty(timeslot, settings):
	return artech_engine.db.count("Appointment", {"scheduled_time": timeslot}) < settings.number_of_agents
