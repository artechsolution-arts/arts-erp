...
async function get_global_variables() {
	...
	window.appointment_settings = (
		await artech_engine.call({
			method: "artech.www.book_appointment.index.get_appointment_settings",
		})
	).message;
	window.timezones = (
		await artech_engine.call({
			method: "artech.www.book_appointment.index.get_timezones",
		})
	).message;
}
...
function hide_next_button() {
	...
	next_button.onclick = () => artech_engine.msgprint(__("Please select a date and time"));
}
...
function on_date_or_timezone_select() {
	...
	if (date_picker.value === "") {
		clear_time_slots();
		hide_next_button();
		artech_engine.throw(__("Please select a date"));
	}
...
async function get_time_slots(date, timezone) {
	let slots = (
		await artech_engine.call({
			method: "artech.www.book_appointment.index.get_appointment_slots",
			args: {
				date: date,
				timezone: timezone,
			},
...
async function submit() {
	...
	let appointment = artech_engine.call({
		method: "artech.www.book_appointment.index.create_appointment",
		args: {
		...
		callback: (response) => {
			if (response.message.status == "Unverified") {
				artech_engine.show_alert(__("Please check your email to confirm the appointment"));
			} else {
				artech_engine.show_alert(__("Appointment Created Successfully"));
			}
			setTimeout(() => {
				let redirect_url = "/";
		...
		error: (err) => {
			artech_engine.show_alert(__("Something went wrong please try again"));
			button.disabled = false;
		},
	});
}
