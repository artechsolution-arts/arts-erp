...
from artech_engine.contacts.doctype.contact.contact import get_contact_with_phone_number
from artech_engine.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from artech_engine.model.document import Document

from artech.crm.doctype.lead.lead import get_lead_with_phone_number
from artech.crm.doctype.utils import get_scheduled_employees_for_popup, strip_number
...
class CallLog(Document):
	...
	if TYPE_CHECKING:
		from artech_engine.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def on_update(self):
		...
		if _is_call_missed(doc_before_save, self):
			artech_engine.publish_realtime(f"call_{self.id}_missed", self)
			self.trigger_call_popup()

		if _is_call_ended(doc_before_save, self):
			artech_engine.publish_realtime(f"call_{self.id}_ended", self)
	...
	def trigger_call_popup(self):
		...
		if artech_engine.conf.developer_mode:
			self.add_comment(
				text=f"""
		...
		for email in emails:
			artech_engine.publish_realtime("show_call_popup", self, user=email)
...
def add_call_summary_and_call_type(call_log: str, summary: str, call_type: str):
	doc = artech_engine.get_doc("Call Log", call_log)
	doc.type_of_call = call_type
	doc.save()
	doc.add_comment("Comment", artech_engine.bold(_("Call Summary")) + "<br><br>" + summary)
...
def get_employees_with_number(number):
	...
	employee_doc_name_and_emails = artech_engine.cache().hget("employees_with_number", number)
	if employee_doc_name_and_emails:
		return employee_doc_name_and_emails

	employee_doc_name_and_emails = artech_engine.get_all(
		"Employee",
		filters={"cell_number": ["like", f"%{number}%"], "user_id": ["!=", ""]},
		fields=["name", "user_id"],
	)

	artech_engine.cache().hset("employees_with_number", number, employee_doc_name_and_emails)
...
def link_existing_conversations(doc, state):
	...
	try:
		...
		for number in numbers:
			...
			if not number:
				continue
			logs = artech_engine.db.sql_list(
				"""
			...
			if logs:
				for log in logs:
					call_log = artech_engine.get_doc("Call Log", log)
		...
		artech_engine.log_error(title=_("Error during caller information update"))
...
def get_linked_call_logs(doctype, docname):
	...
	logs = artech_engine.get_all("Call Log", fields=["*"], filters={"name": ["in", logs]})
