...
def email_sending(project_name, frequency, date_start, date_end, progress, number_of_drafts, update):
	holiday = artech_engine.db.sql("""SELECT holiday_date FROM `tabHoliday` where holiday_date = CURRENT_DATE;""")
	msg = (
		"<p>Project Name: "
		+ project_name
		+ "</p><p>Frequency: "
	...
	msg += "</table>"
	if len(holiday) == 0:
		email = artech_engine.db.sql("""SELECT user from `tabProject User` WHERE parent = %s;""", project_name)
		for emails in email:
			artech_engine.sendmail(recipients=emails, subject=artech_engine._(project_name + " " + "Summary"), message=msg)
	else:
		pass
