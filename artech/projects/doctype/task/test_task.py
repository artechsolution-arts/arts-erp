...
from artech.projects.doctype.task.task import CircularReferenceError, ParentIsGroupError
from artech.tests.utils import ArtechTestSuite


class TestTask(ArtechTestSuite):
	def test_task_total_costing_and_billing_amount(self):
		...
		employee = make_employee("employee@artech_engine.io", company="_Test Company")
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(
			getdate(artech_engine.db.get_value("Task", task2.name, "exp_start_date")),
			getdate(add_days(nowdate(), 21)),
		)

		self.assertEqual(
			getdate(artech_engine.db.get_value("Task", task2.name, "exp_end_date")), getdate(add_days(nowdate(), 25))
		)

		self.assertEqual(
			getdate(artech_engine.db.get_value("Task", task3.name, "exp_start_date")),
			getdate(add_days(nowdate(), 26)),
		)

		self.assertEqual(
			getdate(artech_engine.db.get_value("Task", task3.name, "exp_end_date")), getdate(add_days(nowdate(), 30))
		)

	def test_close_assignment(self):
		if not artech_engine.db.exists("Task", "Test Close Assignment"):
			task = artech_engine.new_doc("Task")
			task.subject = "Test Close Assignment"
			task.insert()

		def assign():
			from artech_engine.desk.form import assign_to
	...
	def test_overdue(self):
		...
		from artech.projects.doctype.task.task import set_tasks_as_overdue

		set_tasks_as_overdue()

		self.assertEqual(artech_engine.db.get_value("Task", task.name, "status"), "Overdue")
...
def create_task(
	...
	if not artech_engine.db.exists("Task", subject):
		task = artech_engine.new_doc("Task")
		task.status = "Open"
		...
		task.project = (
			project or None if is_template else artech_engine.get_value("Project", {"project_name": "_Test Project"})
		)
		...
		task = artech_engine.get_doc("Task", subject)
