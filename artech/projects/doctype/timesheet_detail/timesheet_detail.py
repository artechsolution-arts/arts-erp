...
from artech_engine.utils import add_to_date, flt, get_datetime, time_diff_in_hours, time_diff_in_seconds
...
class TimesheetDetail(Document):
	...
	def update_cost(self, employee: str):
		...
		from artech.projects.doctype.timesheet.timesheet import get_activity_cost
		...
		exchange_rate = flt(artech_engine.get_value("Timesheet", self.parent, "exchange_rate")) or 1.0
	...
	def validate_dates(self):
		...
		if self.from_time and self.to_time and time_diff_in_hours(self.to_time, self.from_time) < 0:
			artech_engine.throw(_("To Time cannot be before from date"))
	...
	def validate_task_project(self):
		...
		if self.task and self.project:
			task_project = artech_engine.db.get_value("Task", self.task, "project")
			if task_project and task_project != self.project:
				artech_engine.throw(
					_("Row {0}: Task {1} does not belong to Project {2}").format(
						self.idx, artech_engine.bold(self.task), artech_engine.bold(self.project)
					)
				)
