...
from artech_engine.utils import add_to_date, now_datetime, nowdate
...
from artech.projects.doctype.timesheet.timesheet import OverlapError, make_sales_invoice
from artech.setup.doctype.employee.test_employee import make_employee
from artech.tests.utils import ArtechTestSuite


class TestTimesheet(ArtechTestSuite):
	def test_timesheet_post_update(self):
		...
		timesheet = artech_engine.new_doc("Timesheet")
		timesheet.append(
			"time_logs",
			{
				"task": task.name,
	...
	@ArtechTestSuite.change_settings("Projects Settings", {"fetch_timesheet_in_sales_invoice": 1})
	def test_timesheet_billing_based_on_project(self):
		...
		ts = artech_engine.get_doc("Timesheet", timesheet.name)
	...
	def test_timesheet_time_overlap(self):
		...
		self.assertRaises(artech_engine.ValidationError, timesheet.save)
		...
		self.assertRaises(artech_engine.ValidationError, timesheet.submit)
	...
	def test_per_billed_hours(self):
		...
		ts = artech_engine.new_doc("Timesheet")
	...
	def test_partial_billing_and_return(self):
		...
		timesheet_status = artech_engine.get_value("Timesheet", timesheet.name, "status")
...
def update_activity_type(activity_type):
	activity_type = artech_engine.get_doc("Activity Type", activity_type)
