...
from artech_engine.utils import flt, get_datetime, getdate
...
class OverWorkLoggedError(artech_engine.ValidationError):
	pass
...
class Timesheet(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.projects.doctype.timesheet_detail.timesheet_detail import TimesheetDetail
	...
	def validate_mandatory_fields(self):
		for data in self.time_logs:
			if not data.from_time and not data.to_time:
				artech_engine.throw(_("Row {0}: From Time and To Time is mandatory.").format(data.idx))

			if not data.activity_type and self.employee:
				artech_engine.throw(_("Row {0}: Activity Type is mandatory.").format(data.idx))

			if flt(data.hours) == 0.0:
				artech_engine.throw(_("Row {0}: Hours value must be greater than zero.").format(data.idx))
	...
	def update_task_and_project(self):
		...
		for data in self.time_logs:
			if data.task and data.task not in tasks:
				task = artech_engine.get_doc("Task", data.task)
		...
		for project in projects:
			project_doc = artech_engine.get_doc("Project", project)
	...
	def validate_overlap(self, data):
		settings = artech_engine.get_single("Projects Settings")
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		timelog = artech_engine.qb.DocType("Timesheet Detail")
...
def get_projectwise_timesheet_data(
	...
	return artech_engine.db.sql(query, filters, as_dict=1)


@artech_engine.whitelist()
def get_timesheet_detail_rate(timelog: str, currency: str):
	ts = artech_engine.qb.DocType("Timesheet")
	ts_detail = artech_engine.qb.DocType("Timesheet Detail")

	timelog_detail = (
		artech_engine.qb.from_(ts_detail)
		.inner_join(ts)
...
def make_sales_invoice(
	...
	target = artech_engine.new_doc("Sales Invoice")
	timesheet = artech_engine.get_doc("Timesheet", source_name)

	if not timesheet.total_billable_hours:
		artech_engine.throw(_("Invoice can't be made for zero billing hour"))

	if timesheet.total_billable_hours == timesheet.total_billed_hours:
		artech_engine.throw(_("Invoice already created for all billing hours"))
	...
	if customer:
		...
		default_price_list = artech_engine.get_value("Customer", customer, "default_price_list")
...
def get_activity_cost(
	...
	rate = artech_engine.db.get_values(
		"Activity Cost",
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="creation"):
	...
	contact = artech_engine.db.exists("Contact", {"user": user})
	if contact:
		# find customer
		contact = artech_engine.get_doc("Contact", contact)
		customer = contact.get_link_for("Customer")

	if customer:
		sales_invoices = artech_engine.get_all("Sales Invoice", filters={"customer": customer}, pluck="name")
		projects = artech_engine.get_all("Project", filters={"customer": customer}, pluck="name")

		# Return timesheet related data to web portal.
		table = artech_engine.qb.DocType("Timesheet")
		child_table = artech_engine.qb.DocType("Timesheet Detail")
		query = (
		...
		if conditions:
			query = query.where(artech_engine.qb.terms.Criterion.any(conditions))
