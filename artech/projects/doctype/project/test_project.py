...
from artech.projects.doctype.project_template.test_project_template import make_project_template
from artech.projects.doctype.task.test_task import create_task
from artech.selling.doctype.sales_order.sales_order import make_project as make_project_from_so
...
class TestProject(ArtechTestSuite):
	...
	def test_project_with_template_having_no_parent_and_depend_tasks(self):
		...
		artech_engine.db.sql(""" delete from tabTask where project = %s """, project_name)
		artech_engine.delete_doc("Project", project_name)
	...
	def test_project_template_having_parent_child_tasks(self):
		...
		if artech_engine.db.get_value("Project", {"project_name": project_name}, "name"):
			project_name = artech_engine.db.get_value("Project", {"project_name": project_name}, "name")
	...
	def test_project_with_template_tasks_having_common_name(self):
		...
		project_tasks = artech_engine.get_all(
...
def make_project(args):
	...
	if args.project_name and artech_engine.db.exists("Project", {"project_name": args.project_name}):
		return artech_engine.get_doc("Project", {"project_name": args.project_name})
...
def task_exists(subject):
	result = artech_engine.db.get_list("Task", filters={"subject": subject}, fields=["name"])
	if not len(result):
		return False
	return artech_engine.get_doc("Task", result[0].name)
