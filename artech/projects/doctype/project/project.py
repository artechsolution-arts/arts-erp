...
from artech_engine.query_builder.functions import Count, CurDate, Date, Sum, UnixTimestamp
from artech_engine.utils import add_days, flt, get_datetime, get_link_to_form, get_time, nowtime, today
from artech_engine.utils.user import is_website_user

from artech import get_default_company
from artech.controllers.queries import get_filters_cond
...
class Project(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.projects.doctype.project_user.project_user import ProjectUser
	...
	def copy_from_template(self):  # nosemgrep
		...
		"""
		if self.project_template and not artech_engine.db.get_all("Task", dict(project=self.name), limit=1):
			# has a template, and no loaded tasks, so lets create
			if not self.expected_start_date:
				# project starts today
				self.expected_start_date = today()

			template = artech_engine.get_doc("Project Template", self.project_template)
			...
			for task in template.tasks:
				template_task_details = artech_engine.get_doc("Task", task.task)
	...
	def dependency_mapping(self, template_tasks, project_tasks):
		for project_task in project_tasks:
			template_task = artech_engine.get_doc("Task", project_task.template_task)
	...
	def after_insert(self):
		self.copy_from_template()  # nosemgrep
		if self.sales_order:
			artech_engine.db.set_value("Sales Order", self.sales_order, "project", self.name)

	def on_trash(self):
		artech_engine.db.set_value("Sales Order", {"project": self.name}, "project", "")

	def update_percent_complete(self):
		if self.status == "Completed":
			if (
				len(artech_engine.get_all("Task", dict(project=self.name))) == 0
		...
		total = artech_engine.db.count("Task", dict(project=self.name))
		...
		if not total:
			...
			if self.percent_complete_method == "Task Weight" and total > 0:
				weight_sum = artech_engine.db.sql(
					"""select sum(task_weight) from tabTask where
					project=%s""",
					self.name,
				)[0][0]
				weighted_progress = artech_engine.db.sql(
				...
				for row in weighted_progress:
					pct_complete += row["progress"] * artech_engine.utils.safe_div(row["task_weight"], weight_sum)
	...
	def update_sales_amount(self):
		total_sales_amount = artech_engine.db.sql(
	...
	def get_billed_amount_from_parent(self):
		total_billed_amount = artech_engine.db.sql(
	...
	def after_rename(self, old_name, new_name, merge=False):
		if old_name == self.copied_from:
			artech_engine.db.set_value("Project", new_name, "copied_from", new_name)
...
def get_timeline_data(doctype: str, name: str) -> dict[int, int]:
	...
	timesheet_detail = artech_engine.qb.DocType("Timesheet Detail")

	return dict(
		artech_engine.qb.from_(timesheet_detail)
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="creation"):
	customers, suppliers = get_customers_suppliers("Project", artech_engine.session.user)

	ignore_permissions = False
	if is_website_user() and artech_engine.session.user != "Guest":
		if not filters:
			filters = []
...
def get_cost_center_name(project: str):
	return artech_engine.db.get_value("Project", project, "cost_center")
...
def create_duplicate_project(prev_doc: str, project_name: str):
	...
	if project_name == prev_doc.get("name"):
		artech_engine.throw(_("Use a name that is different from previous project name"))

	# change the copied doc name to new project name
	project = artech_engine.copy_doc(prev_doc)
	...
	task_list = artech_engine.get_all("Task", filters={"project": prev_doc.get("name")}, fields=["name"])

	# Create duplicate task for all the task
	for task in task_list:
		task = artech_engine.get_doc("Task", task)
		new_task = artech_engine.copy_doc(task)
...
def send_project_update_email_to_users(project):
	doc = artech_engine.get_doc("Project", project)

	if is_holiday(doc.holiday_list) or not doc.users:
		return

	project_update = artech_engine.get_doc(
		{
	...
	incoming_email_account = artech_engine.db.get_value(
...
def collect_project_status():
	for data in artech_engine.get_all("Project Update", {"date": today(), "sent": 0}):
		replies = artech_engine.get_all(
			"Communication",
		...
		for d in replies:
			doc = artech_engine.get_doc("Project Update", data.name)
			user_data = artech_engine.db.get_values(
			...
			doc.append(
				"users",
				{
					...
					"project_status": artech_engine.utils.md_to_html(
...
def send_project_status_email_to_users():
	...
	for d in artech_engine.get_all("Project Update", {"date": yesterday, "sent": 0}):
		doc = artech_engine.get_doc("Project Update", d.name)

		project_doc = artech_engine.get_doc("Project", doc.project)
...
def update_project_sales_billing():
	sales_update_frequency = artech_engine.get_single_value("Selling Settings", "sales_update_frequency")
	if sales_update_frequency == "Each Transaction":
		return
	elif sales_update_frequency == "Monthly" and artech_engine.utils.now_datetime().day != 1:
		return

	# Else simply fallback to Daily
	for project in artech_engine.get_all("Project", filters={"status": ["!=", "Cancelled"]}):
		artech_engine.get_doc("Project", project.name).save()


@artech_engine.whitelist()
def create_kanban_board_if_not_exists(project: str):
	from artech_engine.desk.doctype.kanban_board.kanban_board import quick_kanban_board

	project = artech_engine.get_doc("Project", project)
	if not artech_engine.db.exists("Kanban Board", project.project_name):
...
def set_project_status(project: str, status: str):
	...
	if status not in ("Completed", "Cancelled"):
		artech_engine.throw(_("Status must be Cancelled or Completed"))

	project = artech_engine.get_doc("Project", project)
	artech_engine.has_permission(doc=project, throw=True)

	for task in artech_engine.get_all("Task", dict(project=project.name)):
		artech_engine.db.set_value("Task", task.name, "status", status)
...
def get_holiday_list(company: str | None = None) -> str:
	if not company:
		company = get_default_company() or artech_engine.get_all("Company")[0].name

	holiday_list = artech_engine.get_cached_value("Company", company, "default_holiday_list")
	if not holiday_list:
		artech_engine.throw(
			_("Please set a default Holiday List for Company {0}").format(artech_engine.bold(get_default_company()))
		)
	return holiday_list


def get_users_email(doc):
	return [d.email for d in doc.users if artech_engine.db.get_value("User", d.user, "enabled")]
