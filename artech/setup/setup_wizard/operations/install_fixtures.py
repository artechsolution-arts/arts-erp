...
from artech_engine.desk.doctype.global_search_settings.global_search_settings import (
...
from artech.accounts.doctype.account.account import RootNotEditable
from artech.regional.address_template.setup import set_up_address_templates
...
def install(country=None):
	...
	response = artech_engine.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))
	...
	from artech.buying.doctype.supplier_scorecard.supplier_scorecard import make_default_records
...
def update_buying_defaults():
	buying_settings = artech_engine.get_doc("Buying Settings")
...
def add_uom_data():
	...
	for d in uoms:
		if d.get("category") and not artech_engine.db.exists("UOM Category", d.get("category")):
			artech_engine.get_doc({"doctype": "UOM Category", "category_name": d.get("category")}).db_insert()

		if not artech_engine.db.exists("UOM", d.get("uom_name")):
			doc = artech_engine.new_doc("UOM")
			doc.update(d)
	...
	uom_conversions = json.loads(
		open(
			artech_engine.get_app_path("artech", "setup", "setup_wizard", "data", "uom_conversion_data.json")
		).read()
	)
...
def add_sale_stages():
	...
	for sales_stage in records:
		artech_engine.get_doc(sales_stage).db_insert()
...
def install_defaults(args=None):  # nosemgrep
	...
	artech_engine.db.set_value("Currency", args.get("currency"), "enabled", 1)
	artech_engine.db.set_single_value("Stock Settings", "email_footer_address", args.get("company_name"))
...
def set_global_defaults(kwargs):
	global_defaults = artech_engine.get_doc("Global Defaults", "Global Defaults")
	company = artech_engine.db.get_value(
		"Company",
...
def create_bank_account(args, demo=False):
	...
	bank_account_group = artech_engine.db.get_value(
		"Account",
		{"account_type": "Bank", "is_group": 1, "root_type": "Asset", "company": company_name},
	)
	if bank_account_group:
		bank_account = artech_engine.get_doc(
			{
				"doctype": "Account",
		...
		try:
			...
			artech_engine.throw(artech_engine._("Bank account cannot be named as {0}").format(args.get("bank_account")))
