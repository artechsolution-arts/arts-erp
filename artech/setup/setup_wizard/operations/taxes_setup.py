...
def setup_taxes_and_charges(company_name: str, country: str):
	if not artech_engine.db.exists("Company", company_name):
		artech_engine.throw(_("Company {} does not exist yet. Taxes setup aborted.").format(company_name))
...
def from_detailed_data(company_name, data):
	...
	if (
		artech_engine.db.get_value("Company", company_name, "create_chart_of_accounts_based_on")
		== "Existing Company"
	):
		charts_company_name = artech_engine.db.get_value("Company", company_name, "existing_company")
	coa_name = artech_engine.db.get_value("Company", charts_company_name, "chart_of_accounts")
...
def update_regional_tax_settings(country, company):
	path = artech_engine.get_app_path("artech", "regional", artech_engine.scrub(country))
	if os.path.exists(path.encode("utf-8")):
		try:
			module_name = f"artech.regional.{artech_engine.scrub(country)}.setup.update_regional_tax_settings"
			artech_engine.get_attr(module_name)(country, company)
		except (ImportError, AttributeError):
			pass
		except Exception:
			# Log error and ignore if failed to setup regional tax settings
			artech_engine.log_error("Unable to setup regional tax settings")
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	if artech_engine.db.exists(doctype, {"title": template.get("title"), "company": company_name}):
		return
	...
	for tax_row in template.get("taxes"):
		...
		tax_row_defaults = {
			...
			"cost_center": artech_engine.db.get_value("Company", company_name, "cost_center"),
		}
	...
	doc = artech_engine.get_doc(template)
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = artech_engine.get_all(
		"Account", filters={"company": company_name, "root_type": root_type}, or_filters=or_filters
	)

	if existing_accounts:
		return artech_engine.get_doc("Account", existing_accounts[0].name)
	...
	doc = artech_engine.get_doc(account)
	doc.flags.ignore_links = True
	doc.flags.ignore_validate = True
...
def get_or_create_tax_group(company_name, root_type):
	...
	tax_group_name = artech_engine.db.get_value(
		"Account",
	...
	tax_group_account = artech_engine.get_doc(
		{
			"doctype": "Account",
			"company": company_name,
			"is_group": 1,
...
def make_tax_category(tax_category):
	...
	if not artech_engine.db.exists(doctype, tax_category["title"]):
		doc = artech_engine.get_doc(tax_category)
