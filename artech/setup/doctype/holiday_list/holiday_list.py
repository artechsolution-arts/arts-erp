...
from artech_engine.utils import DateTimeLikeObject, formatdate, getdate, today
...
class HolidayList(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.setup.doctype.holiday.holiday import Holiday
	...
	def get_local_holidays(self):
		...
		for holiday_date, holiday_name in country_holidays(
			...
			language=artech_engine.local.lang,
		).items():
			...
			if row.holiday_date in unique_dates:
				artech_engine.throw(
					_("Holiday Date {0} added multiple times").format(
						artech_engine.bold(formatdate(row.holiday_date))
					)
				)
...
def local_country_name(country_code: str) -> str:
	...
	return Locale.parse(artech_engine.local.lang, sep="-").territories.get(country_code, country_code)
