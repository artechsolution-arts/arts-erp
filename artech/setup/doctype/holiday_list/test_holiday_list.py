...
from artech.setup.doctype.holiday_list.holiday_list import local_country_name
from artech.tests.utils import ArtechTestSuite


class TestHolidayList(ArtechTestSuite):
	def test_holiday_list(self):
		...
		fetched_holiday_list = artech_engine.get_value("Holiday List", holiday_list.name)
		self.assertEqual(holiday_list.name, fetched_holiday_list)

	def test_weekly_off(self):
		holiday_list = artech_engine.new_doc("Holiday List")
...
def make_holiday_list(name, from_date=None, to_date=None, holiday_dates=None):
	...
	artech_engine.delete_doc_if_exists("Holiday List", name, force=1)
