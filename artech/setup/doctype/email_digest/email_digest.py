...
import artech_engine.desk.notifications
from dateutil.relativedelta import relativedelta
from artech_engine import _
from artech_engine.core.doctype.user.user import STANDARD_USERS
...
from artech.accounts.utils import get_balance_on, get_count_on, get_fiscal_year
...
class EmailDigest(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.setup.doctype.email_digest_recipient.email_digest_recipient import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.set_dates()
		self._accounts = {}
		self.currency = artech_engine.db.get_value("Company", self.company, "default_currency")

	@artech_engine.whitelist()
	def get_users(self):
		"""get list of users"""
		user_list = artech_engine.db.sql(
			"""
		...
		artech_engine.response["user_list"] = user_list
	...
	def get_msg_html(self):
		...
		from artech.setup.doctype.email_digest.quotes import get_random_quote
		...
		artech_engine.flags.ignore_account_permission = False

		# style
		return artech_engine.render_template(
			"artech/setup/doctype/email_digest/templates/default.html", context, is_path=True
		)
	...
	def get_notifications(self):
		...
		notifications = artech_engine.desk.notifications.get_notifications()
	...
	def get_calendar_events(self):
		...
		from artech_engine.desk.doctype.event.event import get_events
	...
	def get_todo_list(self, user_id=None):
		"""Get to-do list"""
		if not user_id:
			user_id = artech_engine.session.user

		todo_list = artech_engine.db.sql(
			"""select *
	...
	def get_issue_list(self, user_id=None):
		...
		meta = artech_engine.get_meta("Issue")
		role_permissions = artech_engine.permissions.get_role_permissions(meta, user_id)
		if not role_permissions.get("read"):
			return None

		issue_list = artech_engine.db.sql(
			"""select *
	...
	def get_project_list(self, user_id=None):
		...
		project_list = artech_engine.db.sql(
			"""select *
	...
	def set_accounting_cards(self, context):
		...
		for key in (
			...
			if self.get(key):
				...
				if card:
					card = artech_engine.safe_eval(card)

				else:
					card = artech_engine._dict(getattr(self, "get_" + key)())
	...
	def get_expenses_booked(self):
		...
		expense_account = artech_engine.db.get_all(
			"Account",
			fields=["name"],
	...
	def get_sales_orders_to_bill(self):
		...
		value, count = artech_engine.db.sql(
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		value, count, billed_value, delivered_value = artech_engine.db.sql(
	...
	def get_summary_of_pending_quotations(self, fieldname):
		...
		last_value = artech_engine.db.sql(
	...
	def get_purchase_orders_items_overdue_list(self):
		...
		purchase_order_list = artech_engine.db.sql(sql_po, as_dict=True)
		purchase_order_items_overdue_list = artech_engine.db.sql(sql_poi, as_dict=True)
...
def send():
	...
	for ed in artech_engine.db.sql(
		"""select name from `tabEmail Digest`
			where enabled=1 and docstatus<2""",
		as_list=1,
	):
		ed_obj = artech_engine.get_doc("Email Digest", ed[0])
...
def get_digest_msg(name: str):
	return artech_engine.get_doc("Email Digest", name).get_msg_html()
