...
def save_new_records(test_records):
	for record in test_records:
		...
		try:
			artech_engine.set_value(**kwargs)
		except artech_engine.DoesNotExistError:
			curr_exchange = artech_engine.new_doc(record.get("doctype"))
	...
	class PatchResponse:
		...
		def raise_for_status(self):
			if self.status_code != 200:
				raise artech_engine.DoesNotExistError
...
class TestCurrencyExchange(ArtechTestSuite):
	def setUp(self):
	...
	def test_exchange_rate(self, mock_get):
		artech_engine.db.set_single_value("Accounts Settings", "allow_stale", 1)
	...
	def test_exchange_rate_via_exchangerate_host(self, mock_get):
		...
		settings = artech_engine.get_single("Currency Exchange Settings")
	...
	def test_exchange_rate_strict(self, mock_get):
		...
		artech_engine.db.set_single_value("Accounts Settings", "allow_stale", 0)
		artech_engine.db.set_single_value("Accounts Settings", "stale_days", 1)
