...
artech.setup.EmployeeController = class EmployeeController extends artech_engine.ui.form.Controller {
	setup() {
		this.frm.fields_dict.user_id.get_query = function (doc, cdt, cdn) {
			return {
				query: "artech_engine.core.doctype.user.user.user_query",
...
artech_engine.ui.form.on("Employee", {
	setup: function (frm) {
		frm.make_methods = {
			...
			frm.add_custom_button(__("Create User"), () => {
				const dialog = new artech_engine.ui.Dialog({
					...
					primary_action: (values) => {
						if (!values.email) {
							artech_engine.msgprint(__("Email is required to create a user."));
							return;
						}

						artech_engine
							.call({
								method: "artech.setup.doctype.employee.employee.create_user",
								args: {
	...
	build_anniversary_content: function (frm) {
			...
			items.push(`
				<div class="form-sidebar-items milestone-item">
					<span class="form-sidebar-label">
						${artech_engine.utils.icon("cake", "sm")}
		...
		if (frm.events.is_work_anniversary(frm, today)) {
			...
			items.push(`
				<div class="form-sidebar-items milestone-item">
					<span class="form-sidebar-label">
						${artech_engine.utils.icon("briefcase", "sm")}
	...
	update_contact: function (frm) {
		var prefered_email_fieldname = artech_engine.model.scrub(frm.doc.prefered_contact_email) || "user_id";
...
cur_frm.cscript = new artech.setup.EmployeeController({
	frm: cur_frm,
});

artech_engine.tour["Employee"] = [
	{
