...
class TestEmployee(ArtechTestSuite):
	def test_employee_status_left(self):
		...
		employee1_doc = artech_engine.get_doc("Employee", employee1)
		employee2_doc = artech_engine.get_doc("Employee", employee2)
	...
	def test_user_has_employee(self):
		...
		self.assertTrue("Employee" in artech_engine.get_roles(user))
		employee_doc.user_id = ""
		employee_doc.save()
		self.assertTrue("Employee" not in artech_engine.get_roles(user))
	...
	def test_employee_user_permission(self):
		...
		artech_engine.set_user(employee1_doc.user_id)
		...
		employee_list = artech_engine.db.get_list("Employee", pluck="name", order_by="name")
	...
	def test_create_user_automatically(self):
		...
		user = artech_engine.db.get_value("User", "test_auto_user1@example.com")
		...
		user2 = artech_engine.db.get_value("User", "test_auto_user2@example.com")
...
def make_employee(user, company=None, **kwargs):
	if not artech_engine.db.get_value("User", user):
	...
	if not artech_engine.db.get_value("Employee", {"user_id": user}):
		employee = artech_engine.get_doc(
			{
				...
				"date_of_joining": "2013-01-01",
				"department": artech_engine.get_all("Department", fields="name")[0].name,
				"gender": "Female",
				"company_email": user,
		...
		employee = artech_engine.get_doc("Employee", {"employee_name": user})
		employee.update(kwargs)
