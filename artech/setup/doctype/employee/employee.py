...
from artech_engine import _, scrub, throw
from artech_engine.model.naming import set_name_by_naming_series
from artech_engine.permissions import (
	add_user_permission,
	get_doc_permissions,
	remove_user_permission,
)
from artech_engine.utils import cint, cstr, getdate, today, validate_email_address
from artech_engine.utils.nestedset import NestedSet

from artech.utilities.transaction_base import delete_events


class EmployeeUserDisabledError(artech_engine.ValidationError):
	pass


class InactiveEmployeeStatusError(artech_engine.ValidationError):
	pass
...
class Employee(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from artech.setup.doctype.employee_education.employee_education import EmployeeEducation
		from artech.setup.doctype.employee_external_work_history.employee_external_work_history import (
			EmployeeExternalWorkHistory,
		)
		from artech.setup.doctype.employee_internal_work_history.employee_internal_work_history import (
	...
	def validate(self):
		...
		if self.user_id:
			...
			existing_user_id = artech_engine.db.get_value("Employee", self.name, "user_id")
			if existing_user_id:
				user = artech_engine.get_doc("User", existing_user_id)
	...
	def validate_user_details(self):
		if self.user_id:
			data = artech_engine.db.get_value("User", self.user_id, ["enabled"], as_dict=1)
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = artech_engine.db.exists(
	...
	def update_user(self):
		...
		user = artech_engine.get_doc("User", self.user_id)
	...
	def set_preferred_email(self):
		preferred_email_field = artech_engine.scrub(self.prefered_contact_email)
		self.prefered_email = self.get(preferred_email_field) if preferred_email_field else None

	def validate_status(self):
		if self.status == "Left":
			reports_to = artech_engine.db.get_all(
				"Employee",
			...
			if reports_to:
				link_to_employees = [
					artech_engine.utils.get_link_to_form("Employee", employee.name, label=employee.employee_name)
	...
	def validate_for_enabled_user_id(self, enabled):
		if enabled is None:
			artech_engine.throw(_("User {0} does not exist").format(self.user_id))

		if self.status != "Active" and enabled or self.status == "Active" and enabled == 0:
			artech_engine.db.set_value("User", self.user_id, "enabled", not enabled)
	...
	def validate_preferred_email(self):
		if self.prefered_contact_email and not self.get(scrub(self.prefered_contact_email)):
			artech_engine.msgprint(_("Please enter {0}").format(self.prefered_contact_email))
	...
	def reset_employee_emails_cache(self):
		...
		if cell_number != prev_number or self.get("user_id") != prev_doc.get("user_id"):
			artech_engine.cache().hdel("employees_with_number", cell_number)
			artech_engine.cache().hdel("employees_with_number", prev_number)


def validate_employee_role(doc, method=None, ignore_emp_check=False):
	# called via User hook
	if not ignore_emp_check:
		if artech_engine.db.get_value("Employee", {"user_id": doc.name}):
			return

	user_roles = [d.role for d in doc.get("roles")]
	if "Employee" in user_roles:
		artech_engine.msgprint(_("User {0}: Removed Employee role as there is no mapped employee.").format(doc.name))
...
def get_holiday_list_for_employee(employee, raise_exception=True, as_on=None):
	hrms_override = artech_engine.get_hooks("employee_holiday_list")

	if hrms_override:
		return artech_engine.get_attr(hrms_override[-1])(employee, raise_exception, as_on)
	if employee:
		holiday_list, company = artech_engine.get_cached_value("Employee", employee, ["holiday_list", "company"])
	else:
		holiday_list = ""
...
def is_holiday(employee, date=None, raise_exception=True, only_non_weekly=False, with_description=False):
	...
	holidays = artech_engine.get_all("Holiday", fields=["description"], filters=filters, pluck="description")
...
def deactivate_sales_person(status: str | None = None, employee: str | None = None):
	if status == "Left":
		sales_person = artech_engine.db.get_value("Sales Person", {"Employee": employee})
		if sales_person:
			artech_engine.db.set_value("Sales Person", sales_person, "enabled", 0)


@artech_engine.whitelist()
def create_user(employee: str, email: str | None = None, create_user_permission: int = 0) -> str:
	emp = artech_engine.get_doc("Employee", employee)
	if emp.user_id:
		artech_engine.throw(_("Employee {0} already has a linked user").format(emp.name))

	if not email:
		artech_engine.throw(_("Email is required to create a user"))
...
def get_all_employee_emails(company):
	...
	employee_list = artech_engine.get_all(
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = artech_engine.db.get_value(
...
def get_children(
	...
	employees = artech_engine.get_list(doctype, fields=fields, filters=filters, order_by="name")

	for employee in employees:
		is_expandable = artech_engine.get_all(doctype, filters=[["reports_to", "=", employee.get("value")]])
...
def on_doctype_update():
	artech_engine.db.add_index("Employee", ["lft", "rgt"])
...
def get_contact_details(employee: str) -> dict:
	...
	"""
	if not employee:
		artech_engine.throw(msg=_("Employee is required"), title=_("Missing Parameter"))
...
def _get_contact_details(employee: str) -> dict:
	contact_data = artech_engine.db.get_value(
		"Employee",
		employee,
		[
			"employee_name",
	...
	if not contact_data:
		artech_engine.throw(msg=_("Employee {0} not found").format(employee), title=_("Not Found"))
