...
from artech_engine.desk.notifications import clear_notifications
from artech_engine.model.document import Document
from artech_engine.query_builder.functions import Max
from artech_engine.utils import cint, comma_and, create_batch, get_link_to_form
from artech_engine.utils.background_jobs import get_job, is_job_enqueued
...
def get_company_link_fields(doctype_name: str):
	...
	"""
	artech_engine.only_for("System Manager")
	if not doctype_name or not artech_engine.db.exists("DocType", doctype_name):
		return []
...
def _get_protected_doctypes_internal():
	...
	for doctype in PROTECTED_CORE_DOCTYPES:
		if artech_engine.db.exists("DocType", doctype):
			protected.append(doctype)

	singles = artech_engine.get_all("DocType", filters={"issingle": 1}, pluck="name")
...
class TransactionDeletionRecord(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.transaction_deletion_record_details.transaction_deletion_record_details import (
			TransactionDeletionRecordDetails,
		)
		from artech.setup.doctype.transaction_deletion_record_item.transaction_deletion_record_item import (
			TransactionDeletionRecordItem,
		)
		from artech.setup.doctype.transaction_deletion_record_to_delete.transaction_deletion_record_to_delete import (
	...
	def validate_to_delete_list(self):
		...
		for item in self.doctypes_to_delete:
			if not artech_engine.db.exists("DocType", item.doctype_name):
				artech_engine.throw(_("DocType {0} does not exist").format(item.doctype_name))
			...
			is_child_table = artech_engine.db.get_value("DocType", item.doctype_name, "istable")
			if is_child_table:
				artech_engine.throw(
					_(
			...
			is_virtual = artech_engine.db.get_value("DocType", item.doctype_name, "is_virtual")
			if is_virtual:
				artech_engine.throw(
					_(
	...
	def before_submit(self):
		if queued_docs := artech_engine.db.get_all(
			"Transaction Deletion Record",
		...
		if not self.doctypes_to_delete and not self.doctypes_to_be_ignored:
			artech_engine.throw(_("Please generate To Delete list before submitting"))
	...
	def _clear_deletion_cache(self):
		...
		for item in self.doctypes_to_delete:
			artech_engine.cache.delete_value(f"deletion_running_doctype:{item.doctype_name}")
	...
	def _get_to_delete_row_infos(self, doctype_name, company_field=None, company=None):
		...
		if company_field and company:
			doc_count = artech_engine.db.count(doctype_name, filters={company_field: company})
		else:
			doc_count = artech_engine.db.count(doctype_name)
	...
	def _get_company_link_fields(self, doctype_name):
		...
		Returns:
		        list: List of field names that link to Company DocType, ordered by field index
		"""
		company_fields = artech_engine.get_all(
			"DocField",
			filters={"parent": doctype_name, "fieldtype": "Link", "options": "Company"},
			pluck="fieldname",
			order_by="idx",
	...
	def generate_to_delete_list(self):
		...
		doctypes_with_company_field = artech_engine.get_all(
			"DocField",
		...
		for doctype_name in doctypes_with_company_field:
			...
			if artech_engine.db.exists("DocType", doctype_name):
				meta = artech_engine.get_meta(doctype_name)
		...
		for doctype_name in doctypes_with_company:
			...
			for company_field in company_fields:
				doc_count = artech_engine.db.count(doctype_name, {company_field: self.company})
	...
	def populate_doctype_details(
		...
		if not artech_engine.db.exists("DocType", doctype_name):
			artech_engine.throw(_("DocType {0} does not exist").format(doctype_name))

		is_child_table = artech_engine.db.get_value("DocType", doctype_name, "istable")
		if is_child_table:
			return {
				"child_doctypes": "",
				"document_count": 0,
	...
	def export_to_delete_template_method(self):
		...
		if not self.doctypes_to_delete:
			artech_engine.throw(_("Generate To Delete list first"))
		...
		artech_engine.response["result"] = output.getvalue()
		artech_engine.response["type"] = "csv"
		artech_engine.response[
			"doctype"
		] = f"deletion_template_{self.company}_{artech_engine.utils.now_datetime().strftime('%Y%m%d')}"
	...
	def import_to_delete_template_method(self, csv_content):
		...
		if "doctype_name" not in (reader.fieldnames or []):
			artech_engine.throw(_("Invalid CSV format. Expected column: doctype_name"))
		...
		for row in reader:
			...
			is_child = artech_engine.db.get_value("DocType", doctype_name, "istable")
			if is_child:
				skipped.append(_("{0}: Child table (auto-deleted with parent)").format(doctype_name))
				continue

			is_virtual = artech_engine.db.get_value("DocType", doctype_name, "is_virtual")
			if is_virtual:
		...
		if task and task in self.task_to_internal_method_map:
			...
			if self.process_in_single_transaction:
				...
				artech_engine.enqueue(
					"artech_engine.utils.background_jobs.run_doc_method",
			...
			if task := getattr(self, method, None):
				try:
					...
					if traceback:
						...
						artech_engine.db.set_value(self.doctype, self.name, "error_log", message)
					artech_engine.db.set_value(self.doctype, self.name, "status", "Failed")
					self._clear_deletion_cache()
	...
	def delete_lead_addresses(self):
		...
		if self.delete_leads_and_addresses_status == "Pending":
			...
			leads = artech_engine.db.get_all("Lead", filters={"company": self.company}, pluck="name")
			addresses = []
			if leads:
				addresses = artech_engine.db.get_all(
					"Dynamic Link", filters={"link_name": ("in", leads)}, pluck="parent"
				)
				if addresses:
					addresses = ["%s" % artech_engine.db.escape(addr) for addr in addresses]
	...
	def reset_company_values(self):
		...
		if self.reset_company_default_values_status == "Pending":
			...
			company_obj = artech_engine.get_doc("Company", self.company)
	...
	def delete_company_transactions(self):
		...
		if self.delete_transactions_status == "Pending":
			...
			for docfield in self.doctypes:
				if docfield.doctype_name != self.doctype and not docfield.done:
					if docfield.doctype_name in protected_doctypes:
						...
						artech_engine.log_error(error_msg, "Transaction Deletion Security")
						artech_engine.throw(
					...
					if company_field:
						...
						no_of_docs = artech_engine.db.count(docfield.doctype_name)

					if no_of_docs > 0:
						if company_field:
							reference_docs = artech_engine.get_all(
						...
						artech_engine.db.set_value(docfield.doctype, docfield.name, "no_of_docs", processed)
					else:
						naming_series = artech_engine.db.get_value("DocType", docfield.doctype_name, "autoname")
						if naming_series:
							if "#" in naming_series:
								self.update_naming_series(naming_series, docfield.doctype_name)
						artech_engine.db.set_value(docfield.doctype, docfield.name, "done", 1)

						to_delete_row = artech_engine.db.get_value(
			...
			pending_doctypes = artech_engine.db.get_all(
	...
	def get_doctypes_to_be_ignored_list(self):
		doctypes_to_be_ignored_list = artech_engine.get_all(
	...
	def get_doctypes_with_company_field(self, doctypes_to_be_ignored_list):
		docfields = artech_engine.get_all(
			"DocField",
			filters={
				"fieldtype": "Link",
				"options": "Company",
	...
	def get_all_child_doctypes(self):
		return artech_engine.get_all("DocType", filters={"istable": 1}, pluck="name")

	def get_number_of_docs_linked_with_specified_company(self, doctype, company_fieldname):
		return artech_engine.db.count(doctype, {company_fieldname: self.company})
	...
	def delete_child_tables(self, doctype, reference_doc_names):
		...
		for table in child_tables:
			artech_engine.db.delete(table, {"parent": ["in", reference_doc_names]})

	def delete_docs_linked_with_specified_company(self, doctype, reference_doc_names):
		artech_engine.db.delete(doctype, {"name": ("in", reference_doc_names)})
	...
	def update_naming_series(self, naming_series, doctype_name):
		...
		artech_engine.db.set_value("Series", prefix, "current", last, update_modified=False)
	...
	def delete_communications(self, doctype, reference_doc_names):
		...
		for batch in create_batch(communication_names, self.batch_size):
			artech_engine.delete_doc("Communication", batch, ignore_permissions=True)
...
def get_doctypes_to_be_ignored():
	...
	doctypes_to_be_ignored.extend(artech_engine.get_hooks("company_data_to_be_ignored") or [])
...
def export_to_delete_template(name: str):
	...
	artech_engine.only_for("System Manager")
	doc = artech_engine.get_doc("Transaction Deletion Record", name)
...
def process_import_template(transaction_deletion_record_name: str, file_url: str):
	...
	doc = artech_engine.get_doc("Transaction Deletion Record", transaction_deletion_record_name)
	doc.check_permission("write")

	if not file_url or ".." in file_url:
		artech_engine.throw(_("Invalid file URL"))

	try:
		file_doc = artech_engine.get_doc("File", {"file_url": file_url})
	except artech_engine.DoesNotExistError:
		artech_engine.throw(_("File not found"))

	if (
		file_doc.attached_to_doctype != "Transaction Deletion Record"
		or file_doc.attached_to_name != transaction_deletion_record_name
	):
		artech_engine.throw(_("File does not belong to this Transaction Deletion Record"))

	if not file_doc.file_name or not file_doc.file_name.lower().endswith(".csv"):
		artech_engine.throw(_("Only CSV files are allowed"))

	file_path = file_doc.get_full_path()

	if not os.path.isfile(file_path):
		artech_engine.throw(_("File not found on server"))
...
def is_deletion_doc_running(company: str | None = None, err_msg: str | None = None):
	...
	"""
	running_deletion_job = artech_engine.db.get_value(
		"Transaction Deletion Record",
...
def check_for_running_deletion_job(doc, method=None):
	...
	deletion_name = artech_engine.cache.get_value(f"deletion_running_doctype:{doc.doctype}")
	...
	if deletion_name:
		artech_engine.throw(
			...
			msg=_(
				...
				get_link_to_form("Transaction Deletion Record", deletion_name), artech_engine.bold(doc.doctype)
			),
		)
