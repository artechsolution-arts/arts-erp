...
artech_engine.ui.form.on("Transaction Deletion Record", {
	setup: function (frm) {
	...
	onload: function (frm) {
		if (frm.doc.docstatus == 0) {
			...
			artech_engine.call({
				method: "artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_protected_doctypes",
				callback: function (r) {
					if (r.message) {
			...
			artech_engine.call({
				method: "artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_doctypes_to_be_ignored",
				callback: function (r) {
		...
		if (frm.doc.docstatus === 0 && !frm.is_new()) {
			...
			frm.page.set_primary_action(__("Submit"), () => {
				if (!frm.doc.doctypes_to_delete || frm.doc.doctypes_to_delete.length === 0) {
					artech_engine.msgprint(__("Please generate the To Delete list before submitting"));
					return;
				}
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.doctypes_to_delete && frm.doc.doctypes_to_delete.length > 0) {
				frm.add_custom_button(
					...
					() => {
						open_url_post(
							"/api/method/artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.export_to_delete_template",
							{
								name: frm.doc.name,
							}
						);
				...
				frm.add_custom_button(__("Remove Zero Counts"), () => {
					...
					if (removed_count === 0) {
						artech_engine.msgprint(__("No rows with zero document count found"));
						return;
					}
			...
			frm.add_custom_button(
				...
				() => {
					new artech_engine.ui.FileUploader({
						...
						on_success: (file_doc) => {
							artech_engine.call({
								method: "artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.process_import_template",
								args: {
...
artech_engine.ui.form.on("Transaction Deletion Record To Delete", {
	doctype_name: function (frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (row.doctype_name) {
			// Fetch company fields for auto-selection (only if exactly 1 field exists)
			artech_engine.call({
				method: "artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_company_link_fields",
				args: {
					doctype_name: row.doctype_name,
				},
				callback: function (r) {
					if (r.message && r.message.length === 1 && !row.company_field) {
						artech_engine.model.set_value(cdt, cdn, "company_field", r.message[0]);
			...
			frm.call({
				...
				callback: function (r) {
					if (r.message) {
						...
						artech_engine.model.set_value(cdt, cdn, "child_doctypes", r.message.child_doctypes || "");
						artech_engine.model.set_value(cdt, cdn, "document_count", r.message.document_count || 0);
					}
				},
			});
		}
	...
	company_field: function (frm, cdt, cdn) {
		...
		if (row.doctype_name && row.company_field !== undefined) {
			...
			if (duplicates.length > 0) {
				...
				artech_engine.model.set_value(cdt, cdn, "company_field", "");
				return;
			}
