...
class TestTransactionDeletionRecord(ArtechTestSuite):
	def setUp(self):
	...
	def _clear_all_deletion_cache_flags(self):
		...
		cache_keys = artech_engine.cache.get_keys("deletion_running_doctype:*")
		if cache_keys:
			for key in cache_keys:
				...
				if "|" in key_str:
					...
					key_name = key_str
				artech_engine.cache.delete_value(key_name)
	...
	def test_doctypes_contain_company_field(self):
		...
		for doctype_row in tdr.doctypes_to_delete:
			...
			if doctype_row.company_field:
				field_found = False
				doctype_fields = artech_engine.get_meta(doctype_row.doctype_name).as_dict()["fields"]
	...
	def test_deletion_is_successful(self):
		...
		tasks_containing_company = artech_engine.get_all("Task", filters={"company": "Dunder Mifflin Paper Co"})
		self.assertEqual(tasks_containing_company, [])

	def test_company_transaction_deletion_request(self):
		"""Test creation via company deletion request method"""
		from artech.setup.doctype.company.company import create_transaction_deletion_request
	...
	def test_generate_to_delete_list(self):
		...
		tdr = artech_engine.new_doc("Transaction Deletion Record")
		tdr.company = company
		tdr.insert()
	...
	def test_csv_export_import(self):
		...
		csv_content = artech_engine.response.get("result")
		...
		tdr2 = artech_engine.new_doc("Transaction Deletion Record")
	...
	def test_cache_flag_management(self):
		...
		cached_value = artech_engine.cache.get_value("deletion_running_doctype:Task")
	...
	def test_check_for_running_deletion_blocks_save(self):
		...
		artech_engine.cache.set_value("deletion_running_doctype:Task", "TDR-00001", expires_in_sec=60)

		try:
			# Try to validate a new Task
			new_task = artech_engine.new_doc("Task")
			new_task.company = company
			new_task.subject = "Should be blocked"

			# Should throw error when cache flag exists
			with self.assertRaises(artech_engine.ValidationError) as context:
			...
			artech_engine.cache.delete_value("deletion_running_doctype:Task")
	...
	def test_check_for_running_deletion_allows_save_when_no_flag(self):
		...
		try:
			new_task.insert()
			# Cleanup
			artech_engine.delete_doc("Task", new_task.name)
		except artech_engine.ValidationError as e:
	...
	def test_only_one_deletion_allowed_globally(self):
		...
		tdr1 = artech_engine.new_doc("Transaction Deletion Record")
...
def create_task(company):
	task = artech_engine.get_doc({"doctype": "Task", "company": company, "subject": "Delete"})
	task.insert()
