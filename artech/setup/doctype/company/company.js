...
artech_engine.provide("artech.company");

artech_engine.ui.form.on("Company", {
	onload: function (frm) {
		if (frm.doc.__islocal && frm.doc.parent_company) {
			artech_engine.db.get_value("Company", frm.doc.parent_company, "is_group", (r) => {
				if (!r.is_group) {
	...
	date_of_commencement: function (frm) {
		if (frm.doc.date_of_commencement < frm.doc.date_of_incorporation) {
			artech_engine.throw(__("Date of Commencement should be greater than Date of Incorporation"));
		}
	...
	refresh: function (frm) {
		artech.company.setup_queries(frm);
		...
		if (!frm.is_new()) {
			...
			if (artech_engine.perm.has_perm("Cost Center", 0, "read")) {
				frm.add_custom_button(
					__("Cost Centers"),
					function () {
						artech_engine.set_route("Tree", "Cost Center", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (artech_engine.perm.has_perm("Account", 0, "read")) {
				frm.add_custom_button(
					__("Chart of Accounts"),
					function () {
						artech_engine.set_route("Tree", "Account", { company: frm.doc.name });
					},
					__("View")
				);
			}

			if (artech_engine.perm.has_perm("Sales Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Sales Tax Template"),
					function () {
						artech_engine.set_route("List", "Sales Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
			...
			if (artech_engine.perm.has_perm("Purchase Taxes and Charges Template", 0, "read")) {
				frm.add_custom_button(
					__("Purchase Tax Template"),
					function () {
						artech_engine.set_route("List", "Purchase Taxes and Charges Template", {
							company: frm.doc.name,
						});
					},
					__("View")
			...
			if (artech_engine.user.has_role("System Manager")) {
				if (frm.has_perm("write")) {
					frm.add_custom_button(
		...
		artech.company.set_chart_of_accounts_options(frm.doc);
	},
	...
	make_default_tax_template: function (frm) {
		frm.call({
			...
			freeze: true,
			callback: function () {
				artech_engine.msgprint(__("Default tax templates for sales, purchase and items are created."));
			},
		});
	},
	...
	delete_company_transactions: function (frm) {
		artech_engine.call({
			method: "artech.setup.doctype.transaction_deletion_record.transaction_deletion_record.is_deletion_doc_running",
			args: {
				company: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				if (!r.exc) {
					artech_engine.verify_password(function () {
						var d = artech_engine.prompt(
							{
								fieldtype: "Data",
							...
							function (data) {
								if (data.company_name !== frm.doc.name) {
									artech_engine.msgprint(__("Company name not same"));
									return;
								}
								artech_engine.call({
									method: "artech.setup.doctype.company.company.create_transaction_deletion_request",
									args: {
									...
									onerror: function () {
										artech_engine.msgprint(__("Wrong Password"));
									},
								});
							},
...
artech.company.set_chart_of_accounts_options = function (doc) {
	var selected_value = doc.chart_of_accounts;
	if (doc.country) {
		return artech_engine.call({
			method: "artech.accounts.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				country: doc.country,
				with_standard: true,
			},
...
artech.company.setup_queries = function (frm) {
	$.each(
		[
		...
		function (i, v) {
			artech.company.set_custom_query(frm, v);
		}
	);
...
artech.company.set_custom_query = function (frm, v) {
	var filters = {
		company: frm.doc.name,
		is_group: 0,
	};
