...
from artech_engine.cache_manager import clear_defaults_cache
...
from artech_engine.utils.nestedset import NestedSet, rebuild_tree

from artech.accounts.doctype.account.account import get_account_currency
from artech.accounts.doctype.financial_report_template.financial_report_template import (
	sync_financial_report_templates,
)
from artech.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
from artech.stock.utils import check_pending_reposting
...
class Company(NestedSet):
	...
	def validate_inventory_account_settings(self):
		...
		if (
			...
			and artech_engine.db.get_value("Stock Ledger Entry", {"is_cancelled": 0, "company": self.name}, "name")
	...
	def validate_abbr(self):
		...
		if not self.abbr.strip():
			artech_engine.throw(_("Abbreviation is mandatory"))

		if artech_engine.db.sql("select abbr from tabCompany where name!=%s and abbr=%s", (self.name, self.abbr)):
			artech_engine.throw(_("Abbreviation already used for another company"))
	...
	def validate_default_accounts(self):
		...
		for account in accounts:
			if self.get(account[1]):
				for_company, is_group, disabled = artech_engine.db.get_value(
					"Account", self.get(account[1]), ["company", "is_group", "disabled"]
				)

				if disabled:
					artech_engine.throw(_("Account {0} is disabled.").format(artech_engine.bold(self.get(account[1]))))

				if is_group:
					artech_engine.throw(
						_("{0}: {1} is a group account.").format(
							artech_engine.bold(account[0]), artech_engine.bold(self.get(account[1]))
						)
					)
				...
				if get_account_currency(self.get(account[1])) != self.default_currency:
					...
					).format(artech_engine.bold(account[0]))
					artech_engine.throw(error_message)

	def validate_advance_account_currency(self):
		if (
			self.default_advance_received_account
			and artech_engine.get_cached_value("Account", self.default_advance_received_account, "account_currency")
			!= self.default_currency
		):
			artech_engine.throw(
				_("'{0}' should be in company currency {1}.").format(
					artech_engine.bold(_("Default Advance Received Account")), artech_engine.bold(self.default_currency)
				)
			)

		if (
			self.default_advance_paid_account
			and artech_engine.get_cached_value("Account", self.default_advance_paid_account, "account_currency")
			!= self.default_currency
		):
			artech_engine.throw(
				_("'{0}' should be in company currency {1}.").format(
					artech_engine.bold(_("Default Advance Paid Account")), artech_engine.bold(self.default_currency)
				)
			)
	...
	def validate_currency(self):
		if self.is_new():
			return
		self.previous_default_currency = artech_engine.get_cached_value("Company", self.name, "default_currency")
		if (
			self.default_currency
	...
	def on_update(self):
		...
		if not artech_engine.db.sql(
			"""select name from tabAccount
				where company=%s and docstatus<2 limit 1""",
			self.name,
		):
			if not artech_engine.local.flags.ignore_chart_of_accounts:
				artech_engine.flags.country_change = True
				sync_financial_report_templates(self.chart_of_accounts, self.existing_company)
				self.create_default_accounts()
				self.create_default_warehouses()

		if not artech_engine.db.get_value("Cost Center", {"is_group": 0, "company": self.name}):
			self.create_default_cost_center()

		if artech_engine.flags.country_change:
			install_country_fixtures(self.name, self.country)
			self.create_default_tax_template()

		if not artech_engine.db.get_value("Department", {"company": self.name}):
		...
		if self.default_currency:
			artech_engine.db.set_value("Currency", self.default_currency, "enabled", 1)

		if (
			hasattr(artech_engine.local, "enable_perpetual_inventory")
			and self.name in artech_engine.local.enable_perpetual_inventory
		):
			artech_engine.local.enable_perpetual_inventory[self.name] = self.enable_perpetual_inventory

		if artech_engine.flags.parent_company_changed:
	...
	def create_default_warehouses(self):
		...
		for wh_detail in [
			...
			warehouse = artech_engine.get_doc(
				{
					"doctype": "Warehouse",
					"warehouse_name": wh_detail["warehouse_name"],
	...
	def create_default_accounts(self):
		from artech.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts

		artech_engine.local.flags.ignore_root_company_validation = True
		...
		self.db_set(
			...
			artech_engine.db.get_value("Account", {"company": self.name, "account_type": "Payable", "is_group": 0}),
		)
	...
	def create_default_departments(self):
		records = [
			# Department
			{
				...
				"is_group": 1,
				"parent_department": "",
				"__condition": lambda: not artech_engine.db.exists("Department", _("All Departments")),
			},
			{
				"doctype": "Department",
		...
		artech_engine.local.flags.ignore_update_nsm = True
		make_records(records)
		artech_engine.local.flags.ignore_update_nsm = False
	...
	def validate_coa_input(self):
		if self.create_chart_of_accounts_based_on == "Existing Company":
			...
			if not self.existing_company:
				artech_engine.throw(_("Please select Existing Company for creating Chart of Accounts"))
	...
	def validate_perpetual_inventory(self):
		...
		if (
			...
			if artech_engine.db.get_value("Stock Ledger Entry", {"is_cancelled": 0, "company": self.name}, "name"):
				artech_engine.throw(
					_(
			...
			if (
				...
				artech_engine.throw(
					_("Set default {0} account for non stock items").format(
						artech_engine.bold(_("Provisional Account"))
					)
				)
	...
	def check_country_change(self):
		artech_engine.flags.country_change = False

		if not self.is_new() and self.country != artech_engine.get_cached_value("Company", self.name, "country"):
			artech_engine.flags.country_change = True
	...
	def validate_parent_company(self):
		if self.parent_company:
			is_group = artech_engine.get_value("Company", self.parent_company, "is_group")

			if not is_group:
				artech_engine.throw(_("Parent Company must be a group company"))

	def set_reporting_currency(self):
		self.reporting_currency = self.default_currency
		if self.parent_company:
			parent_reporting_currency = artech_engine.db.get_value(
	...
	def set_default_accounts(self):
		...
		if not self.default_income_account:
			income_account = artech_engine.db.get_all(
				"Account",
		...
		if not self.write_off_account:
			write_off_acct = artech_engine.db.get_value(
		...
		if not self.exchange_gain_loss_account:
			exchange_gain_loss_acct = artech_engine.db.get_value(
		...
		if not self.disposal_account:
			disposal_acct = artech_engine.db.get_value(
				"Account",
	...
	def set_mode_of_payment_account(self):
		cash = artech_engine.db.get_value("Mode of Payment", {"type": "Cash"}, "name")
		if (
			cash
			and self.default_cash_account
			and not artech_engine.db.get_value("Mode of Payment Account", {"company": self.name, "parent": cash})
		):
			mode_of_payment = artech_engine.get_doc("Mode of Payment", cash, for_update=True)
	...
	def create_default_cost_center(self):
		...
		for cc in cc_list:
			...
			cc_doc = artech_engine.get_doc(cc)
	...
	def on_trash(self):
		...
		rec = artech_engine.db.sql("SELECT name from `tabGL Entry` where company = %s", self.name)
		if not rec:
			artech_engine.db.sql(
			...
			for doctype in ["Account", "Cost Center", "Budget", "Party Account"]:
				artech_engine.db.sql(f"delete from `tab{doctype}` where company = %s", self.name)

		if not artech_engine.db.get_value("Stock Ledger Entry", {"company": self.name}):
			artech_engine.db.sql("""delete from `tabWarehouse` where company=%s""", self.name)

		artech_engine.defaults.clear_default("company", value=self.name)
		for doctype in ["Mode of Payment Account", "Item Default"]:
			artech_engine.db.sql(f"delete from `tab{doctype}` where company = %s", self.name)

		# clear default accounts, warehouses from item
		warehouses = artech_engine.db.sql_list("select name from tabWarehouse where company=%s", self.name)
		if warehouses:
			artech_engine.db.sql(
		...
		# delete BOMs
		boms = artech_engine.db.sql_list("select name from tabBOM where company=%s", self.name)
		if boms:
			artech_engine.db.sql("delete from tabBOM where company=%s", self.name)
		...
		artech_engine.db.sql("delete from tabEmployee where company=%s", self.name)
		artech_engine.db.sql("delete from tabDepartment where company=%s", self.name)
		artech_engine.db.sql("delete from `tabTax Withholding Account` where company=%s", self.name)
		artech_engine.db.sql("delete from `tabTransaction Deletion Record` where company=%s", self.name)

		# delete tax templates
		artech_engine.db.sql("delete from `tabSales Taxes and Charges Template` where company=%s", self.name)
		artech_engine.db.sql("delete from `tabPurchase Taxes and Charges Template` where company=%s", self.name)
		artech_engine.db.sql("delete from `tabItem Tax Template` where company=%s", self.name)

		# delete Process Deferred Accounts if no GL Entry found
		if not artech_engine.db.get_value("GL Entry", {"company": self.name}):
			artech_engine.db.sql("delete from `tabProcess Deferred Accounting` where company=%s", self.name)

	def check_parent_changed(self):
		artech_engine.flags.parent_company_changed = False

		if not self.is_new() and self.parent_company != artech_engine.db.get_value(
			"Company", self.name, "parent_company"
		):
			artech_engine.flags.parent_company_changed = True
...
def install_country_fixtures(company, country):
	try:
		module_name = f"artech.regional.{artech_engine.scrub(country)}.setup.setup"
		artech_engine.get_attr(module_name)(company, False)
	except ImportError:
		pass
	except Exception:
		artech_engine.log_error("Unable to set country fixtures")
		artech_engine.throw(
			_("Failed to setup defaults for country {0}. Please contact support.").format(
				artech_engine.bold(country)
			)
		)
...
def update_company_current_month_sales(company):
	...
	si = artech_engine.qb.DocType("Sales Invoice")

	total_monthly_sales = (
		artech_engine.qb.from_(si)
	...
	# Fieldname in standard Artech is `total_monthly_sales`
...
def update_company_monthly_sales(company):
	...
	from artech_engine.utils.goal import get_monthly_results
	...
	artech_engine.db.set_value("Company", company, "sales_monthly_history", json.dumps(month_to_value_dict))
...
def update_transactions_annual_history(company, commit=False):
	...
	artech_engine.db.set_value("Company", company, "transactions_annual_history", json.dumps(transactions_history))

	if commit and not artech_engine.in_test:
		artech_engine.db.commit()


def cache_companies_monthly_sales_history():
	companies = [d["name"] for d in artech_engine.get_list("Company")]
...
def get_children(doctype: str, parent: str | None = None, company: str | None = None, is_root: bool = False):
	...
	return artech_engine.db.sql(
		...
		from
			`tabCompany` comp
		where
			ifnull(parent_company, "")={artech_engine.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def get_timeline_data(doctype, name):
	...
	history = artech_engine.get_cached_value("Company", name, "transactions_annual_history")
...
def create_transaction_deletion_request(company: str):
	...
	from artech.setup.doctype.transaction_deletion_record.transaction_deletion_record import (
		is_deletion_doc_running,
	)

	is_deletion_doc_running(company)

	tdr = artech_engine.get_doc({"doctype": "Transaction Deletion Record", "company": company})
	tdr.insert()
	...
	artech_engine.msgprint(
		_("Transaction Deletion Document {0} has been triggered for company {1}").format(
			get_link_to_form("Transaction Deletion Record", tdr.name), artech_engine.bold(company)
		)
	)
