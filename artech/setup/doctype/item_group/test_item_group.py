...
from artech_engine.utils.nestedset import (
...
class TestItemGroup(ArtechTestSuite):
	def setUp(self):
		self.load_test_records("Item Group")

	def test_basic_tree(self, records=None):
		min_lft = 1
		max_rgt = artech_engine.db.sql("select max(rgt) from `tabItem Group`")[0][0]

		if not records:
			records = self.globalTestRecords["Item Group"][2:]

		for item_group in records:
			lft, rgt, parent_item_group = artech_engine.db.get_value(
				"Item Group", item_group["item_group_name"], ["lft", "rgt", "parent_item_group"]
			)

			if parent_item_group:
				parent_lft, parent_rgt = artech_engine.db.get_value("Item Group", parent_item_group, ["lft", "rgt"])
			else:
				# root
	...
	def test_recursion(self):
		group_b = artech_engine.get_doc("Item Group", "_Test Item Group B")
	...
	def test_move_group_into_another(self):
		# before move
		old_lft, old_rgt = artech_engine.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = artech_engine.db.get_value("Item Group", "_Test Item Group C", ["lft", "rgt"])
	...
	def test_move_leaf_into_another_group(self):
		...
		group_b_3 = artech_engine.get_doc("Item Group", "_Test Item Group B - 3")
	...
	def test_delete_leaf(self):
		# for checking later
		parent_item_group = artech_engine.db.get_value("Item Group", "_Test Item Group B - 3", "parent_item_group")
		artech_engine.db.get_value("Item Group", parent_item_group, "rgt")

		ancestors = get_ancestors_of("Item Group", "_Test Item Group B - 3")
		ancestors = artech_engine.db.sql(
		...
		artech_engine.delete_doc("Item Group", "_Test Item Group B - 3")
		...
		for item_group in ancestors:
			new_lft, new_rgt = artech_engine.db.get_value("Item Group", item_group.name, ["lft", "rgt"])
			self.assertEqual(new_rgt, item_group.rgt - 2)

		# insert it back
		artech_engine.copy_doc(self.globalTestRecords["Item Group"][6]).insert()

		self.test_basic_tree()

	def test_delete_group(self):
		# cannot delete group with child, but can delete leaf
		self.assertRaises(NestedSetChildExistsError, artech_engine.delete_doc, "Item Group", "_Test Item Group B")

	def test_merge_groups(self):
		artech_engine.rename_doc("Item Group", "_Test Item Group B", "_Test Item Group C", merge=True)
		records_to_test = self.globalTestRecords["Item Group"][2:]
		del records_to_test[1]
		self.test_basic_tree(records=records_to_test)

		# insert Group B back
		artech_engine.copy_doc(self.globalTestRecords["Item Group"][3]).insert()
		self.test_basic_tree()

		# move its children back
		for name in artech_engine.db.sql_list(
			"""select name from `tabItem Group`
			where parent_item_group='_Test Item Group C'"""
		):
			doc = artech_engine.get_doc("Item Group", name)
	...
	def test_merge_leaves(self):
		artech_engine.rename_doc("Item Group", "_Test Item Group B - 2", "_Test Item Group B - 1", merge=True)
		records_to_test = self.globalTestRecords["Item Group"][2:]
		del records_to_test[3]
		self.test_basic_tree(records=records_to_test)

		# insert Group B - 2back
		artech_engine.copy_doc(self.globalTestRecords["Item Group"][5]).insert()
		self.test_basic_tree()
