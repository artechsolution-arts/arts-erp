...
from artech_engine.utils import add_days, flt, get_datetime_str, nowdate
...
def get_pegged_currencies():
	pegged_currencies = artech_engine.get_all(
...
def get_exchange_rate(
	...
	currency_settings = artech_engine.get_cached_doc("Accounts Settings")
	...
	if artech_engine.get_cached_value("Currency Exchange Settings", "Currency Exchange Settings", "disabled"):
		return 0.00
	...
	try:
		...
		if not value:
			...
			settings = artech_engine.get_cached_doc("Currency Exchange Settings")
			req_params = {
		...
		artech_engine.log_error("Unable to fetch exchange rate")
...
def _enable_all_roles_for_admin():
	...
	all_roles = set(artech_engine.get_all("Role", pluck="name"))
	admin_roles = set(
		artech_engine.db.get_values("Has Role", {"parent": "Administrator"}, fieldname="role", pluck="role")
	)
...
def set_defaults_for_tests():
	...
	artech_engine.db.set_single_value("Selling Settings", defaults)
	for key, value in defaults.items():
		artech_engine.db.set_default(key, value)
...
def welcome_email():
	site_name = get_default_company() or "Artech"
...
def identity(x, *args, **kwargs):
	...
	Use like this: `from artech.setup.utils import identity as _`
	"""
	return x
