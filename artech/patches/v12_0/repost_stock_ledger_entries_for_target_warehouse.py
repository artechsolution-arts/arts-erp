...
def execute():
	warehouse_perm = artech_engine.get_all(
		"User Permission",
	...
	for doctype in ["Sales Invoice", "Delivery Note"]:
		if not artech_engine.get_meta(doctype + " Item").get_field("target_warehouse").hidden:
			continue
		...
		if data:
			...
			for d in parent_names:
				doc = artech_engine.get_doc(doctype, d)
				if doc.docstatus != 1:
					continue
	...
	if artech_engine.get_meta("Sales Order Item").get_field("target_warehouse").hidden:
		artech_engine.db.sql(
