...
def execute():
	...
	artech_engine.reload_doc("Selling", "doctype", "Customer Credit Limit")
	artech_engine.reload_doc("Selling", "doctype", "Customer")
	artech_engine.reload_doc("Setup", "doctype", "Customer Group")

	if artech_engine.db.a_row_exists("Customer Credit Limit"):
		return
...
def move_credit_limit_to_child_table():
	...
	companies = artech_engine.get_all("Company", "name")
	for doctype in ("Customer", "Customer Group"):
		fields = ""
		if doctype == "Customer" and artech_engine.db.has_column(
			"Customer", "bypass_credit_limit_check_at_sales_order"
		):
			fields = ", bypass_credit_limit_check_at_sales_order"

		credit_limit_records = artech_engine.db.sql(
			f"""
		...
		for record in credit_limit_records:
			doc = artech_engine.get_doc(doctype, record.name)
			for company in companies:
				row = artech_engine._dict({"credit_limit": record.credit_limit, "company": company.name})
				if doctype == "Customer":
