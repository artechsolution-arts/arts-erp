...
def execute():
	if "tax_type" not in artech_engine.db.get_table_columns("Item Tax"):
		return
	old_item_taxes = {}
	item_tax_templates = {}

	artech_engine.reload_doc("accounts", "doctype", "item_tax_template_detail", force=1)
	artech_engine.reload_doc("accounts", "doctype", "item_tax_template", force=1)
	existing_templates = artech_engine.db.sql(
	...
	artech_engine.reload_doc("stock", "doctype", "item", force=1)
	artech_engine.reload_doc("stock", "doctype", "item_tax", force=1)
	artech_engine.reload_doc("selling", "doctype", "quotation_item", force=1)
	artech_engine.reload_doc("selling", "doctype", "sales_order_item", force=1)
	artech_engine.reload_doc("stock", "doctype", "delivery_note_item", force=1)
	artech_engine.reload_doc("accounts", "doctype", "sales_invoice_item", force=1)
	artech_engine.reload_doc("buying", "doctype", "supplier_quotation_item", force=1)
	artech_engine.reload_doc("buying", "doctype", "purchase_order_item", force=1)
	artech_engine.reload_doc("stock", "doctype", "purchase_receipt_item", force=1)
	artech_engine.reload_doc("accounts", "doctype", "purchase_invoice_item", force=1)
	artech_engine.reload_doc("accounts", "doctype", "accounts_settings", force=1)
	...
	for item_code in old_item_taxes.keys():
		...
		artech_engine.db.sql("delete from `tabItem Tax` where parent=%s and parenttype='Item'", item_code)
	...
	for dt in doctypes:
		for d in artech_engine.db.sql(
			...
			artech_engine.db.set_value(dt + " Item", d.name, "item_tax_template", item_tax_template_name)
...
def get_item_tax_template(
	...
	item_tax_template = artech_engine.new_doc("Item Tax Template")
	...
	for tax_type, tax_rate in item_tax_map.items():
		account_details = artech_engine.db.get_value(
		...
		if account_details:
			...
			if account_details.account_type not in (
				...
				artech_engine.db.set_value("Account", account_details.name, "account_type", "Chargeable")
		else:
			...
			if not account_name:
				...
				parent_account = artech_engine.get_value(
				...
				tax_type = artech_engine.db.get_value("Account", filters)
				if not tax_type:
					account = artech_engine.new_doc("Account")
					account.update(filters)
					try:
						account.insert()
						tax_type = account.name
					except artech_engine.DuplicateEntryError:
						tax_type = artech_engine.db.get_value(
							"Account", {"account_name": account_name, "company": company}, "name"
						)

		account_type = artech_engine.get_cached_value("Account", tax_type, "account_type")
...
def get_company(company_abbr, parenttype=None, parent=None):
	if parenttype and parent:
		company = artech_engine.get_cached_value(parenttype, parent, "company")
	else:
		company = artech_engine.db.get_value("Company", filters={"abbr": company_abbr})
