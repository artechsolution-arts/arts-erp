...
def execute():
	artech_engine.reload_doc("support", "doctype", "issue_priority")
	artech_engine.reload_doc("support", "doctype", "service_level_priority")
...
def set_issue_priority():
	...
	for priority in artech_engine.get_meta("Issue").get_field("priority").options.split("\n"):
		if priority and not artech_engine.db.exists("Issue Priority", priority):
			artech_engine.get_doc({"doctype": "Issue Priority", "name": priority}).insert(ignore_permissions=True)


def set_priority_for_issue():
	# Sets priority for Issues as Select field is changed to Link field.
	issue_priority = artech_engine.get_list("Issue", fields=["name", "priority"])
	artech_engine.reload_doc("support", "doctype", "issue")

	for issue in issue_priority:
		artech_engine.db.set_value("Issue", issue.name, "priority", issue.priority)
...
def set_priorities_service_level():
	...
	try:
		service_level_priorities = artech_engine.get_list(
			"Service Level",
			fields=[
				"name",
				"priority",
		...
		artech_engine.reload_doc("support", "doctype", "service_level")
		artech_engine.reload_doc("support", "doctype", "support_settings")
		...
		for service_level in service_level_priorities:
			if service_level:
				doc = artech_engine.get_doc("Service Level", service_level.name)
	...
	except artech_engine.db.TableMissingError:
		artech_engine.reload_doc("support", "doctype", "service_level")
...
def set_priorities_service_level_agreement():
	...
	try:
		service_level_agreement_priorities = artech_engine.get_list(
