...
logger = artech_engine.logger("patch", allow_site=True, file_count=50)


def execute():
	affected_purchase_receipts = artech_engine.db.sql(
	...
	artech_engine.reload_doc("stock", "doctype", "Purchase Receipt")
	artech_engine.reload_doc("stock", "doctype", "Purchase Receipt Item")

	for pr in affected_purchase_receipts:
		pr_name = pr[0]
		logger.info(f"purchase_receipt_status: patching PR - {pr_name}")

		pr_doc = artech_engine.get_doc("Purchase Receipt", pr_name)
