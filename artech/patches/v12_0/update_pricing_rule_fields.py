...
def execute():
	if not artech_engine.get_all("Pricing Rule", limit=1):
		return

	artech_engine.reload_doc("accounts", "doctype", "pricing_rule_detail")
	doctypes = {
	...
	for doctype, module in doctypes.items():
		...
		child_doc = artech_engine.scrub(doctype) + "_item"
		artech_engine.reload_doc(module, "doctype", child_doc, force=True)
	...
	for doctype, apply_on in {
		...
		artech_engine.reload_doc("accounts", "doctype", artech_engine.scrub(doctype))
