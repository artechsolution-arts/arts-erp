...
def execute():
	...
	artech_engine.delete_doc("Module Def", "HR", ignore_missing=True, force=True)
	artech_engine.delete_doc("Module Def", "Payroll", ignore_missing=True, force=True)

	artech_engine.delete_doc("Workspace", "HR", ignore_missing=True, force=True)
	artech_engine.delete_doc("Workspace", "Payroll", ignore_missing=True, force=True)
	...
	doctypes = artech_engine.get_all("DocType", {"module": ("in", ["HR", "Payroll"]), "custom": 0}, pluck="name")
	for doctype in doctypes:
		artech_engine.delete_doc("DocType", doctype, ignore_missing=True, force=True)

	artech_engine.delete_doc("DocType", "Salary Slip Loan", ignore_missing=True, force=True)
	artech_engine.delete_doc("DocType", "Salary Component Account", ignore_missing=True, force=True)
	...
	artech_engine.delete_doc("User Type", "Employee Self Service", ignore_missing=True, force=True)

	for dt in ["Web Form", "Dashboard", "Dashboard Chart", "Number Card"]:
		records = artech_engine.get_all(dt, {"module": ("in", ["HR", "Payroll"]), "is_standard": 1}, pluck="name")
