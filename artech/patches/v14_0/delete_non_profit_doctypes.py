...
def execute():
	artech_engine.delete_doc("Module Def", "Non Profit", ignore_missing=True, force=True)

	artech_engine.delete_doc("Workspace", "Non Profit", ignore_missing=True, force=True)

	print_formats = artech_engine.get_all("Print Format", {"module": "Non Profit", "standard": "Yes"}, pluck="name")
	...
	reports = artech_engine.get_all("Report", {"module": "Non Profit", "is_standard": "Yes"}, pluck="name")
	for report in reports:
		artech_engine.delete_doc("Report", report, ignore_missing=True, force=True)

	dashboards = artech_engine.get_all("Dashboard", {"module": "Non Profit", "is_standard": 1}, pluck="name")
	for dashboard in dashboards:
		artech_engine.delete_doc("Dashboard", dashboard, ignore_missing=True, force=True)

	doctypes = artech_engine.get_all("DocType", {"module": "Non Profit", "custom": 0}, pluck="name")
	...
	for record in custom_records:
		try:
			artech_engine.delete_doc(record["doctype"], record["name"], ignore_missing=True)
