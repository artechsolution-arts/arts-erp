...
def execute():
	artech_engine.reload_doc("accounts", "doctype", "overdue_payment")
	artech_engine.reload_doc("accounts", "doctype", "dunning")
	...
	all_dunnings = artech_engine.get_all("Dunning", filters=filters, pluck="name")

	for dunning_name in all_dunnings:
		dunning = artech_engine.get_doc("Dunning", dunning_name)
		...
		payment_schedules = artech_engine.get_all(
			"Payment Schedule",
...
def get_accounts_closing_date():
	...
	accounts_frozen_till = artech_engine.db.get_single_value(
		"Accounts Settings", "acc_frozen_upto"
	)  # always returns datetime.date

	period_closing_date = artech_engine.db.get_value(
