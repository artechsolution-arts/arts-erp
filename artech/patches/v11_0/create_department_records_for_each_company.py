...
def execute():
	...
	for doctype in ["department", "leave_period", "staffing_plan", "job_opening"]:
		artech_engine.reload_doc("hr", "doctype", doctype)
	artech_engine.reload_doc("Payroll", "doctype", "payroll_entry")

	companies = artech_engine.db.get_all("Company", fields=["name", "abbr"])
	departments = artech_engine.db.get_all("Department")
	comp_dict = {}
	...
	for department in departments:
		...
		for company in companies:
			copy_doc = artech_engine.copy_doc(department_doc)
...
def update_instructors(comp_dict):
	when_then = []
	emp_details = artech_engine.get_all("Employee", fields=["name", "company"])
