...
def execute():
	artech_engine.reload_doc("buying", "doctype", "purchase_order")
	artech_engine.reload_doc("buying", "doctype", "supplier_quotation")
	artech_engine.reload_doc("selling", "doctype", "sales_order")
	artech_engine.reload_doc("selling", "doctype", "quotation")
	artech_engine.reload_doc("stock", "doctype", "delivery_note")
	artech_engine.reload_doc("stock", "doctype", "purchase_receipt")
	artech_engine.reload_doc("accounts", "doctype", "sales_invoice")
	artech_engine.reload_doc("accounts", "doctype", "purchase_invoice")
	...
	for doctype in doctypes:
		total_qty = artech_engine.db.sql(
			f"""
			SELECT
		...
		for i in range(0, len(total_qty), batch_size):
			...
			for d in batch_transactions:
				values.append(f"({artech_engine.db.escape(d.parent)}, {d.qty})")
