...
def execute():
	...
	"""
	if not artech_engine.db.has_column("Item", "default_warehouse"):
		return
	...
	if len(companies) == 1 and not artech_engine.get_all("Item Default", limit=1):
		try:
		...
		item_details = artech_engine.db.sql(
		...
		for item_data in item_details:
			for d in [
				...
				if item_data.get(d[0]):
					company = artech_engine.get_value(d[1], item_data.get(d[0]), "company", cache=True)
