...
def execute():
	artech_engine.reload_doc("accounts", "doctype", "POS Invoice Merge Log")
	artech_engine.reload_doc("accounts", "doctype", "POS Closing Entry")
	if artech_engine.db.count("POS Invoice Merge Log"):
		...
		artech_engine.db.sql("""UPDATE `tabPOS Closing Entry` SET status = 'Submitted' where docstatus = 1""")
		artech_engine.db.sql("""UPDATE `tabPOS Closing Entry` SET status = 'Cancelled' where docstatus = 2""")
