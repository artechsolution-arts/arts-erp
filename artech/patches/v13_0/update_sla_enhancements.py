...
def execute():
	...
	if artech_engine.db.exists("DocType", "Service Level Agreement"):
		sla_details = artech_engine.db.get_all("Service Level Agreement", fields=["name", "service_level"])
		priorities = artech_engine.db.get_all(
		...
		artech_engine.reload_doc("support", "doctype", "pause_sla_on_status")
		artech_engine.reload_doc("support", "doctype", "service_level_priority")
		artech_engine.reload_doc("support", "doctype", "service_day")

		for entry in sla_details:
			values = artech_engine.db.get_value(
		...
		if artech_engine.db.exists("DocType", "Service Level"):
			service_levels = artech_engine.db.get_all(
				"Service Level", filters={"service_level": ("not in", sl)}, fields=["*"]
			)
			for entry in service_levels:
				sla = artech_engine.new_doc("Service Level Agreement")
				...
				if priority_list:
					sla = artech_engine.get_doc("Service Level Agreement", sla.name)
	...
	artech_engine.delete_doc_if_exists("DocType", "Service Level")
