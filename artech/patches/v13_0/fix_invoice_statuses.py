...
def execute():
	...
	if not artech_engine.db.exists("DocType", "Party Specific Item"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		invoices_to_update = artech_engine.get_all(
			doctype,
			fields=fields,
			filters={
				"docstatus": 1,
		...
		payment_schedule_items = artech_engine.get_all(
			"Payment Schedule",
		...
		for invoice in invoices_to_update.values():
			...
			doc = artech_engine.get_doc(invoice)
		...
		for status, docs in status_map.items():
			artech_engine.db.set_value(doctype, {"name": ("in", docs)}, "status", status, update_modified=False)
