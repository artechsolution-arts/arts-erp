...
def execute():
	if artech_engine.db.exists("DocType", "Issue"):
		issues = artech_engine.db.get_all(
			"Issue",
	...
	if artech_engine.db.exists("DocType", "Opportunity"):
		opportunities = artech_engine.db.get_all(
			"Opportunity", fields=["name", "mins_to_first_response"], order_by="creation desc"
		)
		artech_engine.reload_doctype("Opportunity", force=True)
		rename_field("Opportunity", "mins_to_first_response", "first_response_time")

		# change fieldtype to duration
		artech_engine.reload_doc("crm", "doctype", "opportunity", force=True)
		count = 0
	...
	for report in [
		...
		if artech_engine.db.exists("Report", report):
			artech_engine.delete_doc("Report", report, ignore_permissions=True)
