...
def execute():
	...
	for row in results:
		so_item = artech_engine.get_value("Material Request Item", row.material_request_item, "sales_order_item")
		artech_engine.db.set_value("Work Order", row.name, "sales_order_item", so_item)

		if so_item:
			wo = artech_engine.get_doc("Work Order", row.name)
