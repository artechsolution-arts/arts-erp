...
def execute():
	artech_engine.reload_doc("accounts", "doctype", "pos_payment_method")
	pos_profiles = artech_engine.get_all("POS Profile")

	for pos_profile in pos_profiles:
		payments = artech_engine.db.sql(
			"""
		...
		if payments:
			for payment_mode in payments:
				pos_payment_method = artech_engine.new_doc("POS Payment Method")
		...
		artech_engine.db.sql("""delete from `tabSales Invoice Payment` where parent=%s""", pos_profile.name)
