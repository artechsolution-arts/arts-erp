...
from artech_engine.utils import cint, cstr, flt
...
def execute():
	if not artech_engine.db.has_column("Work Order", "has_batch_no"):
		return

	artech_engine.reload_doc("manufacturing", "doctype", "manufacturing_settings")
	if cint(artech_engine.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")):
		return
	...
	work_orders = [d.name for d in artech_engine.get_all("Work Order", filters=filters, fields=fields)]
	...
	stock_entries = artech_engine.db.sql_list(
		"""
		SELECT
			se.name
		FROM
	...
	for stock_entry in stock_entries:
		doc = artech_engine.get_doc("Stock Entry", stock_entry)
...
def set_expense_account(doc):
	for row in doc.items:
		if row.is_finished_item and not row.expense_account:
			row.expense_account = artech_engine.get_cached_value("Company", doc.company, "stock_adjustment_account")
