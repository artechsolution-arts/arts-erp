...
def execute():
	artech_engine.reload_doc("automation", "doctype", "auto_repeat")
	...
	for module, doctypes in doctypes_to_rename.items():
		for doctype in doctypes:
			...
			if artech_engine.db.has_column(doctype, "subscription"):
				rename_field(doctype, "subscription", "auto_repeat")

	subscriptions = artech_engine.db.sql("select * from `tabSubscription`", as_dict=1)

	for doc in subscriptions:
		doc["doctype"] = "Auto Repeat"
		auto_repeat = artech_engine.get_doc(doc)
		auto_repeat.db_insert()

	artech_engine.db.sql("delete from `tabSubscription`")
...
def drop_columns_from_subscription():
	...
	for field in [
		...
		if field in artech_engine.db.get_table_columns("Subscription"):
			fields_to_drop["Subscription"].append(field)

	artech_engine.model.delete_fields(fields_to_drop, delete=1)
