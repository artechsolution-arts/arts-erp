# Copyright (c) 2015, Artech and Contributors


import artech_engine


def execute():
	stock_settings = artech_engine.get_doc("Stock Settings")
	if stock_settings.default_warehouse and not artech_engine.db.exists(
		"Warehouse", stock_settings.default_warehouse
	):
		stock_settings.default_warehouse = None
	if stock_settings.stock_uom and not artech_engine.db.exists("UOM", stock_settings.stock_uom):
		stock_settings.stock_uom = None
	stock_settings.flags.ignore_mandatory = True
	stock_settings.action_if_quality_inspection_is_not_submitted = "Stop"
	stock_settings.save()
