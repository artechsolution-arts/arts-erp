...
from artech.setup.install import create_marketing_campaign_custom_fields
...
def execute():
	...
	"""
	if not artech_engine.db.exists("DocType", "Lead Source") or not artech_engine.db.exists("DocType", "UTM Source"):
		return

	ls = artech_engine.qb.DocType("Lead Source")
	ms = artech_engine.qb.DocType("UTM Source")

	# Fetch all Lead Sources
	if lead_sources := artech_engine.qb.from_(ls).select(ls.source_name, ls.details).run(as_dict=True):
		# Prepare the insert query with IGNORE
		insert_query = artech_engine.qb.into(ms).ignore().columns(ms.name, ms.description)
	...
	artech_engine.delete_doc("DocType", "Lead Source", ignore_missing=True)

	campaign = artech_engine.qb.DocType("Campaign")
	create_marketing_campaign_custom_fields()
	marketing_campaign = artech_engine.qb.DocType("UTM Campaign")

	# Fetch all Campaigns
	if campaigns := (
		artech_engine.qb.from_(campaign).select(campaign.campaign_name, campaign.description).run(as_dict=True)
	):
		# Prepare the insert query with IGNORE
		insert_query = (
			artech_engine.qb.into(marketing_campaign)
			.ignore()
			.columns(
