...
from artech.stock.doctype.purchase_receipt.purchase_receipt import adjust_incoming_rate_for_pr


def execute():
	if not artech_engine.db.get_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate"):
		return

	for company in artech_engine.get_all("Company", pluck="name"):
		table = artech_engine.qb.DocType("Purchase Receipt Item")
		parent = artech_engine.qb.DocType("Purchase Receipt")
		query = (
		...
		accounting_period = artech_engine.get_all(
		...
		period_closing_voucher = artech_engine.get_all(
		...
		stock_frozen_upto = artech_engine.db.get_single_value("Stock Settings", "stock_frozen_upto")
		if stock_frozen_upto and getdate(stock_frozen_upto) > getdate(posting_date):
			posting_date = stock_frozen_upto

		fiscal_year = get_fiscal_year(artech_engine.utils.datetime.date.today(), raise_on_missing=False)
		...
		if result := query.run(as_dict=True):
			...
			precision = artech_engine.get_precision("Purchase Receipt Item", "amount")
			for item in result:
				adjusted_amt = 0.0
			...
			for pr in purchase_receipts:
				adjust_incoming_rate_for_pr(artech_engine.get_doc("Purchase Receipt", pr))
...
def get_billed_qty_against_purchase_receipt(pr_names):
	...
	return artech_engine._dict(invoice_data)
