...
from artech_engine import parse_json
from artech_engine.model.document import bulk_insert
...
def execute():
	for tax_doctype, doctypes in DOCTYPES_TO_PATCH.items():
		for doctype in doctypes:
			docnames = artech_engine.get_all(
				tax_doctype,
				filters={
...
def get_taxes_for_docs(parents, tax_doctype, doctype):
	tax = artech_engine.qb.DocType(tax_doctype)
...
def get_items_for_docs(parents, doctype):
	item = artech_engine.qb.DocType(f"{doctype} Item")
	additional_fields = []
...
def get_doc_details(parents, doctype):
	inv = artech_engine.qb.DocType(doctype)
	additional_fields = []
	if doctype in TAX_WITHHOLDING_DOCS:
		additional_fields.append(inv.base_tax_withholding_net_total)

	return (
		artech_engine.qb.from_(inv)
		.select(
			inv.name,
			inv.base_net_total,
			inv.company,
...
def compile_docs(doc_info, taxes, items, doctype, tax_doctype):
	...
	"""
	response = artech_engine._dict()
	for doc in doc_info:
		response[doc.name] = artech_engine._dict(**doc, taxes=[], items=[], doctype=doctype, tax_doctype=tax_doctype)
...
def delete_existing_tax_details(doc_names, doctype):
	...
	artech_engine.db.delete("Item Wise Tax Detail", {"parent": ["in", doc_names], "parenttype": doctype})
...
class ItemTax:
	def get_item_wise_tax_details(self, doc):
		...
		company_currency = artech_engine.get_cached_value("Company", doc.company, "default_currency")
		precision = artech_engine.get_precision(doc.tax_doctype, "tax_amount", currency=company_currency)

		tax_differences = artech_engine._dict()
		last_taxable_items = artech_engine._dict()
	...
	def _get_item_tax_rate(self, item, tax_row):
		...
		item_tax_rates = artech_engine.parse_json(item.item_tax_rate or {})
	...
	return artech_engine.get_doc(
		{
			...
			"name": artech_engine.generate_hash(),
			"idx": idx,
			"item_row": item.name,
			"tax_row": tax.name,
			"rate": rate,
	...
	try:
		...
		for item, tax_data in item_iterator.items():
			if isinstance(tax_data, list) and len(tax_data) >= 2:
				updated_tax_details[item] = artech_engine._dict(
