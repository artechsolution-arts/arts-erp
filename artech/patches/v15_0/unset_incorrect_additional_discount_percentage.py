...
from artech_engine import scrub
from artech_engine.model.meta import get_field_precision
from artech_engine.utils import flt
from semantic_version import Version

from artech.accounts.report.calculated_discount_mismatch.calculated_discount_mismatch import (
	AFFECTED_DOCTYPES,
...
def execute():
	# run this patch only if artech version before update is v15.64.0 or higher
	...
	for doctype in AFFECTED_DOCTYPES:
		...
		documents = artech_engine.get_all(
			doctype,
			fields=[
				"name",
		...
		precision = get_field_precision(artech_engine.get_meta(doctype).get_field("additional_discount_percentage"))
...
def should_run_patch():
	installed_app = artech_engine.db.get_value(
