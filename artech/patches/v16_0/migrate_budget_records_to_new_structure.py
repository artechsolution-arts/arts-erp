...
from artech_engine.utils import add_months, flt, get_first_day, get_last_day
...
def execute():
	...
	budget_names = artech_engine.db.get_list(
		"Budget",
...
def remove_old_property_setter():
	old_property_setter = artech_engine.db.get_value(
		"Property Setter",
		{
			"doc_type": "Budget",
	...
	if old_property_setter:
		artech_engine.delete_doc("Property Setter", old_property_setter, force=1)


def migrate_single_budget(budget_name):
	budget_doc = artech_engine.get_doc("Budget", budget_name)

	account_rows = artech_engine.get_all(
		"Budget Account",
	...
	artech_engine.db.delete("Budget Account", filters={"parent": budget_doc.name})
	...
	if budget_doc.docstatus == 1:
		...
		artech_engine.delete_doc("Budget", budget_name)


def get_percentage_allocations(budget_doc):
	if budget_doc.monthly_distribution:
		distribution_doc = artech_engine.get_cached_doc("Monthly Distribution", budget_doc.monthly_distribution)
