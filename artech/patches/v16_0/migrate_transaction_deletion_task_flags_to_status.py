...
def execute():
	...
	"""
	if not artech_engine.db.table_exists("tabTransaction Deletion Record"):
		return
	...
	records = artech_engine.db.get_all("Transaction Deletion Record", pluck="name")
	...
	for name in records or []:
		...
		for old_field, new_field in field_mapping.items():
			...
			current_value = artech_engine.db.get_value("Transaction Deletion Record", name, old_field)
		...
		if updates:
			artech_engine.db.set_value("Transaction Deletion Record", name, updates, update_modified=False)
