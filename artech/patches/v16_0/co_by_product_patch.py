...
def copy_doctypes():
	previous = artech_engine.db.auto_commit_on_many_writes
	artech_engine.db.auto_commit_on_many_writes = True
	try:
		insert_into_bom()
		insert_into_job_card()
		if artech_engine.db.has_table("Subcontracting Inward Order Scrap Item"):
			insert_into_subcontracting_inward()
	finally:
		artech_engine.db.auto_commit_on_many_writes = previous


def insert_into_bom():
	fields = ["item_code", "item_name", "stock_uom", "stock_qty", "rate"]
	data = artech_engine.get_all("BOM Scrap Item", {"docstatus": ("<", 2)}, ["parent", *fields])
	grouped_data = defaultdict(list)
	for item in data:
		grouped_data[item.parent].append(item)

	for parent, items in grouped_data.items():
		bom = artech_engine.get_doc("BOM", parent)
		for item in items:
			secondary_item = artech_engine.new_doc(
...
def bulk_insert(parent_doctype, old_doctype, new_doctype, old_fields, new_fields, new_values):
	data = artech_engine.get_all(old_doctype, {"docstatus": ("<", 2)}, ["parent", *old_fields])
	grouped_data = defaultdict(list)
	...
	for parent, items in grouped_data.items():
		parent_doc = artech_engine.get_doc(parent_doctype, parent)
		for item in items:
			secondary_item = artech_engine.new_doc(new_doctype, parent_doc=parent_doc, parentfield="secondary_items")
