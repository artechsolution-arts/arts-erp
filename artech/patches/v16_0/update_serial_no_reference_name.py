...
def execute():
	...
	if not artech_engine.db.has_column("Serial and Batch Bundle", "posting_date"):
		return

	sabb = artech_engine.qb.DocType("Serial and Batch Bundle")
	sabb_entry = artech_engine.qb.DocType("Serial and Batch Entry")
	serial_no = artech_engine.qb.DocType("Serial No").as_("sn")

	query = (
		artech_engine.qb.update(serial_no)
		.join(sabb_entry)
