...
from artech_engine.query_builder.functions import IfNull, Max, Sum
from artech_engine.utils import flt, now
...
def execute():
	...
	artech_engine.db.delete("Tax Withholding Entry", filters={"created_by_migration": 1})
...
def get_column_cache():
	return {
		"Supplier": {
			"pan": artech_engine.db.has_column("Supplier", "pan"),
			"tax_id": artech_engine.db.has_column("Supplier", "tax_id"),
		},
		"Customer": {
			"pan": artech_engine.db.has_column("Customer", "pan"),
			"tax_id": artech_engine.db.has_column("Customer", "tax_id"),
		},
		"Purchase Invoice": {
			"tax_withholding_category": artech_engine.db.has_column("Purchase Invoice", "tax_withholding_category"),
		},
		"Purchase Invoice Item": {
			"tax_withholding_category": artech_engine.db.has_column(
				"Purchase Invoice Item", "tax_withholding_category"
			),
		},
		"Sales Invoice": {
			"tax_withholding_category": artech_engine.db.has_column("Sales Invoice", "tax_withholding_category"),
		},
		"Sales Invoice Item": {
			"tax_withholding_category": artech_engine.db.has_column(
...
def get_tds_accounts():
	twa = artech_engine.qb.DocType("Tax Withholding Account")

	result = (
		artech_engine.qb.from_(twa).select(twa.account, twa.company, twa.parent.as_("category")).run(as_dict=True)
	)
...
def get_tax_rate_map():
	twr = artech_engine.qb.DocType("Tax Withholding Rate")
	twc = artech_engine.qb.DocType("Tax Withholding Category")

	rates = (
		artech_engine.qb.from_(twr)
		.join(twc)
...
def get_party_tax_id(party_type, party, column_cache, party_tax_id_cache):
	...
	if doctype_cols.get("pan"):
		tax_id = artech_engine.db.get_value(party_type, party, "pan")
	elif doctype_cols.get("tax_id"):
		tax_id = artech_engine.db.get_value(party_type, party, "tax_id")
...
def bulk_insert_entries(all_entries):
	...
	existing_names = set(artech_engine.get_all("Tax Withholding Entry", pluck="name"))
	...
	if values:
		artech_engine.db.bulk_insert("Tax Withholding Entry", fields, values, ignore_duplicates=True)
...
class PurchaseInvoiceMigrator:
	...
	def _fetch_data(self):
		pi = artech_engine.qb.DocType("Purchase Invoice")
		ptc = artech_engine.qb.DocType("Purchase Taxes and Charges")
		twv = artech_engine.qb.DocType("Tax Withheld Vouchers")
		at = artech_engine.qb.DocType("Advance Tax")
		pe = artech_engine.qb.DocType("Payment Entry")
		atc = artech_engine.qb.DocType("Advance Taxes and Charges")
...
def migrate_sales_invoices(tds_accounts, tax_rate_map, column_cache, party_tax_id_cache):
	...
	cust = artech_engine.qb.DocType("Customer")
	...
	for category, invoices in category_wise_invoices.items():
		artech_engine.db.set_value("Sales Invoice", {"name": ("in", invoices)}, {"apply_tds": 1})
...
def copy_category_to_items_for_purchase(column_cache):
	...
	parent = artech_engine.qb.DocType(parent_doctype)
	item = artech_engine.qb.DocType(item_doctype, alias="item")
...
def copy_category_to_items_for_sales(column_cache):
	...
	parent = artech_engine.qb.DocType(parent_doctype)
	item = artech_engine.qb.DocType(item_doctype, alias="item")
	customer = artech_engine.qb.DocType("Customer", alias="customer")
