...
def get_operating_cost_account(company):
	company_details = artech_engine.db.get_value(
...
def execute():
	...
	companies = artech_engine.get_all("Company", filters={"is_group": 0}, pluck="name")

	for component in components:
		component = _(component)
		if not artech_engine.db.exists("Workstation Operating Component", component):
			doc = artech_engine.new_doc("Workstation Operating Component")
	...
	workstations = artech_engine.get_all("Workstation", filters={"hour_rate": (">", 0.0)}, pluck="name") or []
	workstation_types = (
		artech_engine.get_all("Workstation Type", filters={"hour_rate": (">", 0.0)}, pluck="name") or []
	)
	...
	for workstation in workstations:
		doc = artech_engine.get_doc("Workstation", workstation)
	...
	for workstation_type in workstation_types:
		doc = artech_engine.get_doc("Workstation Type", workstation_type)
		for field, component in components_map.items():
			if doc.get(field):
				doc.append(
					"workstation_costs",
