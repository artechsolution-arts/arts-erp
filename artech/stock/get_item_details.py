...
from artech_engine.utils import add_days, add_months, cint, cstr, flt, get_link_to_form, getdate, parse_json

import artech
from artech import get_company_currency
from artech.accounts.doctype.pricing_rule.pricing_rule import (
...
from artech.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from artech.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from artech.stock.doctype.price_list.price_list import get_price_list_details

ItemDetails = artech_engine._dict
ItemDetailsCtx = artech_engine._dict
ItemPriceCtx = artech_engine._dict
...
@artech.normalize_ctx_input(ItemDetailsCtx)
def get_item_details(
	...
	item = artech_engine.get_cached_doc("Item", ctx.item_code)
	...
	if (
		...
		and artech_engine.get_single_value("Selling Settings", "fallback_to_default_price_list")
	):
		fallback_args = ctx.copy()
		fallback_args.price_list = artech_engine.get_single_value("Selling Settings", "selling_price_list")
	...
	if (
		artech_engine.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward")
...
def set_valuation_rate(out: ItemDetails | dict, ctx: ItemDetailsCtx):
	if artech_engine.db.exists("Product Bundle", {"name": ctx.item_code, "disabled": 0}, cache=True):
		valuation_rate = 0.0
		bundled_items = artech_engine.get_doc("Product Bundle", ctx.item_code)
...
def update_stock(ctx, out, doc=None):
	from artech.stock.doctype.batch.batch import get_available_batches
	from artech.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
	...
	if (
		...
		if doc and isinstance(doc, dict):
			doc = artech_engine._dict(doc)

		kwargs = artech_engine._dict(
			{
				"item_code": ctx.item_code,
				"warehouse": ctx.warehouse,
				"based_on": artech_engine.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
...
def get_item_code(barcode=None, serial_no=None):
	if barcode:
		item_code = artech_engine.db.get_value("Item Barcode", {"barcode": barcode}, fieldname=["parent"])
		if not item_code:
			artech_engine.throw(_("No Item with Barcode {0}").format(barcode))
	elif serial_no:
		item_code = artech_engine.db.get_value("Serial No", serial_no, "item_code")
		if not item_code:
			artech_engine.throw(_("No Item with Serial No {0}").format(serial_no))
...
def validate_item_details(ctx: ItemDetailsCtx, item):
	...
	from artech.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: ItemDetailsCtx, item, overwrite_warehouse=True) -> ItemDetails:
	...
	if item.variant_of and not item.taxes and artech_engine.db.exists("Item Tax", {"parent": item.variant_of}):
	...
	if ctx.doctype == "Material Request" and not ctx.material_request_type:
		ctx["material_request_type"] = artech_engine.db.get_value(
	...
	if item.is_fixed_asset:
		from artech.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
		...
		if is_cwip_accounting_enabled(item.asset_category):
			...
			from artech.assets.doctype.asset_category.asset_category import get_asset_category_account
	...
	if ctx.batch_no and item.name != artech_engine.get_cached_value("Batch", ctx.batch_no, "item"):
		ctx.batch_no = ""
	...
	if not item.is_stock_item and not out.expense_account:
		out.expense_account = artech_engine.get_cached_value("Company", ctx.company, "service_expense_account")
	...
	if ctx.doctype in purchase_doctypes and not artech_engine.db.get_single_value(
		"Buying Settings", "disable_last_purchase_rate"
	):
		from artech.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	for d in [
		...
		if not out[d[1]]:
			out[d[1]] = artech_engine.get_cached_value("Company", ctx.company, d[2]) if d[2] else None
	...
	meta = artech_engine.get_meta(child_doctype)
...
def get_item_warehouse_(ctx: ItemDetailsCtx, item, overwrite_warehouse, defaults=None):
	...
	if not warehouse:
		default_warehouse = artech_engine.get_single_value("Stock Settings", "default_warehouse")
		if (
			default_warehouse
			and artech_engine.get_cached_value("Warehouse", default_warehouse, "company") == ctx.company
		):
...
def get_barcode_data(items_list=None, item_code=None):
	...
	if not items_list and item_code:
		_dict_item_code = artech_engine._dict(
			{
				"item_code": item_code,
			}
		)

		items_list = [artech_engine._dict(_dict_item_code)]

	for item in items_list:
		barcodes = artech_engine.db.get_all("Item Barcode", filters={"parent": item.item_code}, fields="barcode")
...
def get_item_tax_info(
	...
	for item_code in item_codes:
		...
		item = artech_engine.get_cached_doc("Item", item_code[0])
		ctx: ItemDetailsCtx = {
			"company": doc.company,
...
def get_item_tax_template(
	...
	if not item:
		if not ctx.get("item_code"):
			artech_engine.throw(_("Item/Item Code required to get Item Tax Template."))
		else:
			item = artech_engine.get_cached_doc("Item", ctx.item_code)
...
def _get_item_tax_template_from_item_group(ctx, item_group, out=None):
	from artech_engine.utils.nestedset import get_ancestors_of

	ancestors = get_ancestors_of("Item Group", item_group)
	for group in [item_group, *ancestors]:
		group_doc = artech_engine.get_cached_doc("Item Group", group)
...
def _get_item_tax_template(
	...
	for tax in taxes:
		disabled, tax_company = artech_engine.get_cached_value(
...
def get_item_tax_map(*, doc: str | dict | Document, tax_template: str | None = None, as_json: bool = True):
	...
	if tax_template:
		template = artech_engine.get_cached_doc("Item Tax Template", tax_template)
		for d in template.taxes:
			if artech_engine.get_cached_value("Account", d.tax_type, "company") == doc.get("company"):
...
def get_default_inventory_account(ctx: ItemDetailsCtx, item, item_group, brand):
	if not artech_engine.get_cached_value("Company", ctx.company, "enable_item_wise_inventory_account"):
		return None
...
def get_default_expense_account(ctx: ItemDetailsCtx, item, item_group, brand):
	if ctx.get("doctype") in ["Sales Invoice", "Delivery Note"]:
		...
		if not expense_account:
			expense_account = artech_engine.get_cached_value("Company", ctx.company, "default_expense_account")
...
def get_default_deferred_account(ctx: ItemDetailsCtx, item, fieldname=None):
	if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
		return (
			...
			or artech_engine.get_cached_value("Company", ctx.company, "default_" + fieldname)
		)
	else:
		return None
...
def get_default_cost_center(ctx: ItemDetailsCtx, item=None, item_group=None, brand=None, company=None):
	...
	if ctx.get("project"):
		cost_center = artech_engine.db.get_value("Project", ctx.get("project"), "cost_center", cache=True)
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"artech.stock.get_item_details.{method}"
			data = artech_engine.get_attr(path)(ctx.get("item_code"), company)
	...
	if company and cost_center and artech_engine.get_cached_value("Cost Center", cost_center, "company") != company:
		return None

	if not cost_center and company:
		cost_center = artech_engine.get_cached_value("Company", company, "cost_center")
...
def get_price_list_rate(ctx: ItemDetailsCtx, item_doc, out: ItemDetails = None):
	...
	meta = artech_engine.get_meta(ctx.parenttype or ctx.doctype)
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		# insert in database
		if price_list_rate is None or artech_engine.get_cached_value(
		...
		if artech_engine.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
			return out

		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from artech.stock.doctype.item.item import get_last_purchase_details
...
def insert_item_price(ctx: ItemDetailsCtx):
	...
	stock_settings = artech_engine.get_cached_doc("Stock Settings")

	if (
		not artech_engine.db.get_value("Price List", ctx.price_list, "currency", cache=True) == ctx.currency
		or not stock_settings.auto_insert_price_list_rate_if_missing
		or not artech_engine.has_permission("Item Price", "write")
	):
		return
	...
	item_prices = artech_engine.get_all(
		"Item Price",
		filters={
			"item_code": ctx.item_code,
	...
	if item_price and item_price.name:
		...
		if is_price_valid_for_transaction:
			artech_engine.db.set_value("Item Price", item_price.name, "price_list_rate", price_list_rate)
			...
			item_price = artech_engine.new_doc(
				"Item Price",
				item_code=ctx.item_code,
...
def get_item_price(
	...
	:param pctx: dict (or artech_engine._dict) with mandatory fields price_list, uom
	        optional fields transaction_date, customer, supplier
	:param item_code: str, Item Doctype field item_code
	"""
	pctx: ItemPriceCtx = artech_engine._dict(pctx)

	ip = artech_engine.qb.DocType("Item Price")
	query = (
		artech_engine.qb.from_(ip)
		...
		.orderby(ip.valid_from, order=artech_engine.qb.desc)
		.orderby(IfNull(ip.batch_no, ""), order=artech_engine.qb.desc)
		.orderby(ip.uom, order=artech_engine.qb.desc)
		.limit(1)
	)
...
def check_packing_list(price_list_rate_name, desired_qty, item_code):
	...
	flag = True
	if packing_unit := artech_engine.db.get_value("Item Price", price_list_rate_name, "packing_unit", cache=True):
...
def validate_conversion_rate(ctx: ItemDetailsCtx, meta):
	from artech.controllers.accounts_controller import validate_conversion_rate

	company_currency = artech_engine.get_cached_value("Company", ctx.company, "default_currency")
	...
	ctx.conversion_rate = flt(
		ctx.conversion_rate,
		get_field_precision(meta.get_field("conversion_rate"), artech_engine._dict({"fields": ctx})),
	)

	if ctx.price_list:
		if not ctx.plc_conversion_rate and ctx.price_list_currency == artech_engine.db.get_value(
		...
		if not ctx.price_list_currency:
			...
			if meta.get_field("plc_conversion_rate"):
				ctx.plc_conversion_rate = flt(
					ctx.plc_conversion_rate,
					get_field_precision(meta.get_field("plc_conversion_rate"), artech_engine._dict({"fields": ctx})),
				)
...
def get_party_item_code(ctx: ItemDetailsCtx, item_doc, out: ItemDetails):
	if ctx.transaction_type == "selling" and ctx.customer:
		...
		if customer_item_code:
			...
			customer_group = artech_engine.get_cached_value("Customer", ctx.customer, "customer_group")
...
def get_tax_withholding_category(ctx: ItemDetailsCtx, item_doc, out: ItemDetails):
	...
	if item_doc.get(field):
		...
		tax_withholding_category = artech_engine.get_cached_value(
...
def get_pos_profile_item_details_(ctx: ItemDetailsCtx, company, pos_profile=None, update_data=False):
	...
	if not artech_engine.flags.pos_profile and not pos_profile:
		pos_profile = artech_engine.flags.pos_profile = get_pos_profile(company, ctx.pos_profile)
...
def get_pos_profile(company: str, pos_profile: str | None = None, user: str | None = None):
	if pos_profile:
		return artech_engine.get_cached_doc("POS Profile", pos_profile)

	if not user:
		user = artech_engine.session["user"]

	pf = artech_engine.qb.DocType("POS Profile")
	pfu = artech_engine.qb.DocType("POS Profile User")

	query = (
		artech_engine.qb.from_(pf)
		.left_join(pfu)
...
def get_conversion_factor(item_code: str | None, uom: str):
	item = artech_engine.get_cached_value("Item", item_code, ["variant_of", "stock_uom"], as_dict=True)
	...
	parent = artech_engine.qb.DocType("Item")
	child = artech_engine.qb.DocType("UOM Conversion Detail")
	query = (
...
def get_projected_qty(item_code: str, warehouse: str):
	return {
		"projected_qty": artech_engine.db.get_value(
...
def get_bin_details(
	...
	if warehouse:
		from artech_engine.query_builder.functions import Coalesce, Sum
...
def get_batch_qty(batch_no: str, warehouse: str, item_code: str):
	from artech.stock.doctype.batch import batch
...
def apply_price_list_on_item(ctx, doc=None):
	item_doc = artech_engine.get_cached_doc("Item", ctx.item_code)
...
def get_default_bom(item_code: str | None = None):
	def _get_bom(item):
		bom = artech_engine.get_all("BOM", dict(item=item, is_active=True, is_default=True, docstatus=1), limit=1)
	...
	template_item = artech_engine.db.get_value("Item", item_code, "variant_of")
...
def get_valuation_rate(item_code: str, company: str, warehouse: str | None = None):
	if artech_engine.get_cached_value("Warehouse", warehouse, "is_group"):
	...
	if item.get("is_stock_item"):
		...
		pi_item = artech_engine.qb.DocType("Purchase Invoice Item")
		valuation_rate = (
			artech_engine.qb.from_(pi_item)
...
def get_blanket_order_details(ctx: ItemDetailsCtx):
	...
	if ctx.item_code:
		bo = artech_engine.qb.DocType("Blanket Order")
		bo_item = artech_engine.qb.DocType("Blanket Order Item")

		query = (
			artech_engine.qb.from_(bo)
			.from_(bo_item)
