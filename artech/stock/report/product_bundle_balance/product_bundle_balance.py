...
from artech_engine.query_builder.functions import IfNull, Max
...
def execute(filters=None):
	...
	for parent_item in parent_items:
		...
		for child_item in required_items:
			child_item_balance = stock_balance.get(child_item.item_code, artech_engine._dict())
		...
		for warehouse, company in warehouse_company_map.items():
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, artech_engine._dict()).get(
					warehouse, artech_engine._dict()
				)
				child_row = {
					"indent": 1,
...
def get_items(filters):
	pb_details = artech_engine._dict()
	item_details = artech_engine._dict()

	item = artech_engine.qb.DocType("Item")
	pb = artech_engine.qb.DocType("Product Bundle")
	...
	if parent_items:
		item = artech_engine.qb.DocType("Item")
		pb = artech_engine.qb.DocType("Product Bundle")
		pbi = artech_engine.qb.DocType("Product Bundle Item")

		child_item_details = (
			artech_engine.qb.from_(pbi)
			.inner_join(pb)
...
def get_stock_balance(filters, items):
	...
	stock_balance = artech_engine._dict()
	for d in sle:
		stock_balance.setdefault(d.item_code, artech_engine._dict())[d.warehouse] = d
...
def get_stock_ledger_entries(filters, items):
	...
	if filters.get("warehouse"):
		warehouse_details = artech_engine.db.get_value(
