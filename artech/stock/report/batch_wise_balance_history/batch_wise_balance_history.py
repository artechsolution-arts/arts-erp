...
def execute(filters=None):
	...
	sle_count = artech_engine.db.estimate_count("Stock Ledger Entry")
...
def get_stock_ledger_entries_for_batch_no(filters):
	if not filters.get("from_date"):
		artech_engine.throw(_("'From Date' is required"))
	if not filters.get("to_date"):
		artech_engine.throw(_("'To Date' is required"))
...
def get_stock_ledger_entries_for_batch_bundle(filters):
	sle = artech_engine.qb.DocType("Stock Ledger Entry")
	batch_package = artech_engine.qb.DocType("Serial and Batch Entry")
...
def get_item_details(filters):
	item_map = {}
	for d in (artech_engine.qb.from_("Item").select("name", "item_name", "description", "stock_uom")).run(as_dict=1):
