...
class ReportData:
	...
	def get_data(self):
		...
		if self.filters.get("traceability_direction") in ["Forward", "Both"]:
			data = self.get_serial_no_batches()
			batch_details = artech_engine._dict({})
			for row in data:
	...
	def parse_batch_details(self, sabb_data_details, data, direction, indent=0):
		for key in sabb_data_details:
			...
			if sabb.reference_doctype == "Purchase Receipt":
				row["supplier"] = artech_engine.db.get_value(
					"Purchase Receipt",
					sabb.reference_name,
					"supplier",
				)
			elif sabb.reference_doctype == "Stock Entry":
				row["work_order"] = artech_engine.db.get_value(
					"Stock Entry",
					sabb.reference_name,
					"work_order",
				)
			elif sabb.reference_doctype == "Delivery Note":
				row["customer"] = artech_engine.db.get_value(
					"Delivery Note",
					sabb.reference_name,
					"customer",
				)
	...
	def prepare_source_data(self, data):
		source_data = artech_engine._dict({})
		for row in data:
			...
			if key not in source_data:
				row["raw_materials"] = artech_engine._dict({})
				source_data[key] = row

		return source_data

	def get_data_from_sabb(self, row):
		sabb = artech_engine.qb.DocType("Serial and Batch Bundle")
		sabb_entry = artech_engine.qb.DocType("Serial and Batch Entry")

		query = (
			artech_engine.qb.from_(sabb)
	...
	def set_backward_data(self, sabb_data, qty=None):
		...
		if "raw_materials" not in sabb_data:
			sabb_data.raw_materials = artech_engine._dict({})
		...
		for material in materials:
			...
			if material.serial_no or material.batch_no:
				...
				if sub_key not in sabb_data.raw_materials:
					sabb_data.raw_materials[sub_key] = artech_engine._dict(
						{
	...
	def get_serial_no_batches(self, name=None):
		...
		doctype = artech_engine.qb.DocType(self.doctype_name)
	...
	def get_sabb_entries(self, value, type_of_transaction=None):
		...
		SABB = artech_engine.qb.DocType("Serial and Batch Bundle")
		SABE = artech_engine.qb.DocType("Serial and Batch Entry")
	...
	def process_manufacture_or_repack_entry(self, row, batch_details):
		ste = artech_engine.db.get_value("Stock Entry", row.reference_name, ["purpose", "work_order"], as_dict=True)
				...
				fg_item.update(
					{
						...
						"raw_materials": artech_engine._dict(
