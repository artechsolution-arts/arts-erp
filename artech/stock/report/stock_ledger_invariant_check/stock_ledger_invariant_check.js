...
artech_engine.query_reports["Stock Ledger Invariant Check"] = {
	filters: [
		{
			fieldname: "item_code",
			fieldtype: "Link",
	...
	onload(report) {
		report.page.add_inner_button(__("Create Reposting Entry"), () => {
			...
			if (!selected_rows.length) {
				...
				artech_engine.throw(__("Please select only one row to create a Reposting Entry"));
			} else {
				artech_engine.confirm(__(message), () => {
					artech_engine.call({
						method: "artech.stock.report.stock_ledger_invariant_check.stock_ledger_invariant_check.create_reposting_entries",
						args: {
							rows: selected_rows,
							item_code: artech_engine.query_report.get_filter_values().item_code,
							warehouse: artech_engine.query_report.get_filter_values().warehouse,
						},
					});
				});
			}
