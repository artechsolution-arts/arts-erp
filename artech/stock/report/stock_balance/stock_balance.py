...
from artech_engine.utils import add_days, cint, date_diff, flt, getdate
...
from artech.stock.doctype.stock_closing_entry.stock_closing_entry import StockClosing
from artech.stock.doctype.warehouse.warehouse import apply_warehouse_filter
from artech.stock.report.stock_ageing.stock_ageing import FIFOSlots, get_average_age
from artech.stock.utils import add_additional_uom_columns
...
class StockBalanceReport:
	...
	def set_company_currency(self) -> None:
		if self.filters.get("company"):
			self.company_currency = artech.get_company_currency(self.filters.get("company"))
		else:
			self.company_currency = artech_engine.db.get_single_value("Global Defaults", "default_currency")

	def run(self):
		self.float_precision = cint(artech_engine.db.get_default("float_precision")) or 3

		self.item_warehouse_map = artech_engine._dict({})
	...
	def prepare_opening_stock(self) -> None:
		...
		for entry in opening_entries:
			...
			self.item_warehouse_map[key] = artech_engine._dict(
				{
	...
	def prepare_item_warehouse_map_for_current_period(self):
		...
		_system_settings = artech_engine.get_cached_doc("System Settings")
	...
	def prepare_stock_reco_voucher_wise_count(self):
		self.stock_reco_voucher_wise_count = artech_engine._dict()
		...
		for row in data:
			...
			sr_item = artech_engine.db.get_value(
	...
	def initialize_data(self, group_by_key, entry):
		self.item_warehouse_map[group_by_key] = artech_engine._dict(
			{
				"item_code": entry.item_code,
				"warehouse": entry.warehouse,
				"item_group": entry.item_group,
	...
	def get_itemwise_conversion_factor(self):
		...
		table = artech_engine.qb.DocType("UOM Conversion Detail")
		query = (
	...
	def get_variant_values_for(self):
		...
		attribute_info = artech_engine.get_all(
	...
	def get_opening_vouchers(self):
		...
		sr = artech_engine.qb.DocType("Stock Reconciliation")

		vouchers_data = (
			artech_engine.qb.from_(
				(
...
def get_variants_attributes() -> list[str]:
	...
	return artech_engine.get_all("Item Attribute", pluck="name")
