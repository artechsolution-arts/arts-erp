...
def get_data(item):
	...
	variant_results = artech_engine.db.get_all(
		"Item", fields=["name"], filters={"variant_of": ["=", item], "disabled": 0}
	)

	if not variant_results:
		artech_engine.msgprint(_("There aren't any item variants for the selected item"))
		return []
	else:
	...
	attributes = artech_engine.db.get_all(
	...
	for item_dict in variant_dicts:
		...
		for attribute in attribute_list:
			...
			if attr_dict and attr_dict.get(attribute):
				item_dict[artech_engine.scrub(attribute)] = attr_val_map.get(name).get(attribute)
...
def get_columns(item):
	...
	for entry in item_doc.attributes:
		columns.append(
			{
				"fieldname": artech_engine.scrub(entry.attribute),
...
def get_open_sales_orders_count(variants_list):
	open_sales_orders = artech_engine.db.get_list(
		"Sales Order",
...
def get_stock_details_map(variant_list):
	stock_details = artech_engine.db.get_all(
		"Bin",
		fields=[
...
def get_buying_price_map(variant_list):
	buying = artech_engine.db.get_all(
		"Item Price",
		fields=[
...
def get_selling_price_map(variant_list):
	selling = artech_engine.db.get_all(
		"Item Price",
		fields=[
			{"AVG": "price_list_rate", "as": "avg_rate"},
			"item_code",
...
def get_attribute_values_map(variant_list):
	attribute_list = artech_engine.db.get_all(
		"Item Variant Attribute",
