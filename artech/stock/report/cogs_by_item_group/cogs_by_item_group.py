...
from artech_engine.utils import date_diff

from artech.accounts.report.general_ledger.general_ledger import get_gl_entries
...
SVDList = list[artech_engine._dict]
...
def update_filters_with_account(filters: Filters) -> None:
	account = artech_engine.get_value("Company", filters.get("company"), "default_expense_account")
...
def get_stock_value_difference_list(filtered_entries: FilteredEntries) -> SVDList:
	...
	svd_list = artech_engine.get_list(
		"Stock Ledger Entry",
...
def get_row(name: str, value: float, is_bold: int, indent: int) -> Row:
	item_group = name
	if is_bold:
		item_group = artech_engine.bold(item_group)
	return artech_engine._dict(item_group=item_group, cogs_debit=value, indent=indent)
...
def get_item_groups_dict() -> ItemGroupsDict:
	item_groups_list = artech_engine.get_all("Item Group", fields=("name", "is_group", "lft", "rgt"))
