...
from artech_engine.utils import cint, date_diff, flt, get_datetime
...
def check_and_replace_valuations_if_moving_average(
	...
	if item_valuation_method == "Moving Average" or (
		...
		and artech_engine.get_cached_value("Company", company, "valuation_method") == "Moving Average"
	):
...
class FIFOSlots:
	...
	def generate(self) -> dict:
		...
		"""
		from artech.stock.serial_batch_bundle import get_serial_nos_from_bundle

		stock_ledger_entries = self.sle

		bundle_wise_serial_nos = artech_engine._dict({})
	...
	def __get_bundle_wise_serial_nos(self) -> dict:
		...
		entry = artech_engine.qb.DocType("Serial and Batch Entry")
	...
	def __get_item_query(self) -> str:
		...
		item = artech_engine.qb.from_("Item").select(
			"name",
			"item_name",
			"description",
			"stock_uom",
	...
	def __get_warehouse_conditions(self, sle, sle_query) -> str:
		warehouse = artech_engine.qb.DocType("Warehouse")
		lft, rgt = artech_engine.db.get_value("Warehouse", self.filters.get("warehouse"), ["lft", "rgt"])

		warehouse_results = (
			artech_engine.qb.from_(warehouse)
			.select("name")
