...
def validate_warehouse(filters):
	...
	if not artech_engine.db.exists("Warehouse", {"name": warehouse, "company": company}):
		artech_engine.throw(_("Warehouse: {0} does not belong to {1}").format(warehouse, company))
...
def get_data(warehouse, show_disabled_items):
	...
	serial_item_list = artech_engine.get_all(
		"Item",
		filters=filters,
	...
	for item in serial_item_list:
		total_serial_no = artech_engine.db.count(
			"Serial No",
		...
		# artech_engine.db.get_value returns null if no record exist.
		if not actual_qty:
			actual_qty = 0
