...
def fix_sabb_entries(selected_rows: str | list):
	if isinstance(selected_rows, str):
		selected_rows = artech_engine.parse_json(selected_rows)

	for row in selected_rows:
		doc = artech_engine.get_doc("Serial and Batch Bundle", row.get("name"))
		if doc.is_cancelled == 0 and not artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		elif doc.is_cancelled == 1 and artech_engine.db.get_value(
			"Stock Ledger Entry",
			{"serial_and_batch_bundle": doc.name, "is_cancelled": 0},
			"name",
		):
	...
	artech_engine.msgprint(_("Selected Serial and Batch Bundle entries have been fixed."))
