...
from artech_engine.utils import flt, get_datetime, today
...
def get_batchwise_data_from_stock_ledger(filters):
	batchwise_data = artech_engine._dict({})
...
def get_batchwise_data_from_serial_batch_bundle(batchwise_data, filters):
	table = artech_engine.qb.DocType("Stock Ledger Entry")
	ch_table = artech_engine.qb.DocType("Serial and Batch Entry")
	batch = artech_engine.qb.DocType("Batch")
...
def get_query_based_on_filters(query, batch, table, filters):
	...
	if filters.warehouse:
		lft, rgt = artech_engine.db.get_value("Warehouse", filters.warehouse, ["lft", "rgt"])
