...
from artech_engine.utils import cint, flt, get_datetime
...
from artech.stock.doctype.stock_reconciliation.stock_reconciliation import get_stock_balance_for
from artech.stock.doctype.warehouse.warehouse import apply_warehouse_filter
...
def execute(filters=None):
	...
	precision = cint(artech_engine.db.get_single_value("System Settings", "float_precision"))
	bundle_details = {}
...
def get_serial_batch_bundle_details(sl_entries, filters=None):
	...
	_bundle_details = artech_engine._dict({})
	batch_entries = artech_engine.get_all(
...
def get_serial_and_batch_bundles(filters):
	SBB = artech_engine.qb.DocType("Serial and Batch Bundle")
	SBE = artech_engine.qb.DocType("Serial and Batch Entry")

	query = (
		artech_engine.qb.from_(SBE)
		.inner_join(SBB)
...
def get_items(filters):
	...
	query = artech_engine.qb.from_(item).select(item.name)
	conditions = []
...
def get_item_details(items, sl_entries, include_uom):
	...
	if include_uom:
		ucd = artech_engine.qb.DocType("UOM Conversion Detail")
		query = (
			query.left_join(ucd)
...
def get_opening_balance_from_batch(filters, columns, sl_entries):
	...
	opening_data = artech_engine.get_all(
		"Stock Ledger Entry",
		fields=[
	...
	sabb_table = artech_engine.qb.DocType("Serial and Batch Entry")
	query = (
...
def get_opening_balance(filters, columns, sl_entries, inv_dimension_wise_value=None):
	...
	project = None
	if filters.get("project") and not artech_engine.get_all(
	...
	for sle in list(sl_entries):
		if (
			...
			and artech_engine.db.get_value("Stock Reconciliation", sle.voucher_no, "purpose") == "Opening Stock"
		):
			last_entry = sle
...
def get_item_group_condition(item_group, item_table=None):
	item_group_details = artech_engine.db.get_value("Item Group", item_group, ["lft", "rgt"], as_dict=1)
	if item_group_details:
		if item_table:
			ig = artech_engine.qb.DocType("Item Group")
			return item_table.item_group.isin(
				artech_engine.qb.from_(ig)
				.select(ig.name)
				.where(
...
def get_opening_balance_for_inv_dimension(filters, inv_dimension_wise_value):
	...
	sl_doctype = artech_engine.qb.DocType("Stock Ledger Entry")

	query = (
		artech_engine.qb.from_(sl_doctype)
		.select(
...
def get_inv_dimension_wise_value(filters) -> list:
	inv_dimension_key = artech_engine._dict({})
	for dimension in get_inventory_dimensions():
