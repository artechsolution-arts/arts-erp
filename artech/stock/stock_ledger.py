...
import artech
from artech.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from artech.stock.utils import (
...
from artech.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero


class NegativeStockError(artech_engine.ValidationError):
	pass


class SerialNoExistsInFutureTransaction(artech_engine.ValidationError):
	pass
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from artech.controllers.stock_controller import future_sle_exists
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			is_stock_item = artech_engine.get_cached_value("Item", args.get("item_code"), "is_stock_item")
			if is_stock_item:
				bin_name = get_or_make_bin(args.get("item_code"), args.get("warehouse"))
				args.reserved_stock = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_stock"))
...
def validate_cancellation(kargs):
	if kargs[0].get("is_cancelled"):
		repost_entry = artech_engine.db.get_value(
		...
		if repost_entry:
			...
			if repost_entry.status == "Queued":
				doc = artech_engine.get_doc("Repost Item Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def set_as_cancel(voucher_type, voucher_no):
	artech_engine.db.sql(
		...
		(now(), artech_engine.session.user, voucher_type, voucher_no),
	)


def make_entry(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	args["doctype"] = "Stock Ledger Entry"
	sle = artech_engine.get_doc(args)
...
def update_args_in_repost_item_valuation(
	...
	if doc.reposting_data_file:
		...
		# artech_engine.delete_doc("File", file_name, ignore_permissions=True, delete_permanently=True)
	...
	doc.db_set(
		{
			"current_index": index,
			"items_to_be_repost": artech_engine.as_json(items_to_be_repost),
...
def create_json_gz_file(data, doc, file_name=None) -> str:
	encoded_content = artech_engine.safe_encode(artech_engine.as_json(data))
	compressed_content = gzip.compress(encoded_content)

	if not file_name:
		return create_file(doc, compressed_content)
	else:
		file_doc = artech_engine.get_doc("File", file_name)
		if "/artech_engine_s3_attachment." in file_doc.file_url:
			file_doc.delete()
			return create_file(doc, compressed_content)
...
def validate_item_warehouse(args):
	for field in ["item_code", "warehouse", "posting_date", "posting_time"]:
		if args.get(field) in [None, ""]:
			validation_msg = f"The field {artech_engine.unscrub(field)} is required for the reposting"
			artech_engine.throw(_(validation_msg))
...
def get_items_to_be_repost(voucher_type=None, voucher_no=None, doc=None, reposting_data=None):
	...
	if not items_to_be_repost and voucher_type and voucher_no:
		items_to_be_repost = artech_engine.db.get_all(
			"Stock Ledger Entry",
...
def get_item_wh_wise_last_posted_sle_from_reposting_data(doc, reposting_data=None):
	...
	if reposting_data and reposting_data.item_wh_wise_last_posted_sle:
		return artech_engine._dict(reposting_data.item_wh_wise_last_posted_sle)
...
def get_reposting_data(file_path) -> dict:
	...
	attached_file = artech_engine.get_doc("File", file_name)
...
class update_entries_after:
	...
	def __init__(
		...
		self.args = artech_engine._dict(args)
		if self.args.sle_id:
			self.args["name"] = self.args.sle_id

		self.prev_sle_dict = artech_engine._dict({})
		self.company = artech_engine.get_cached_value("Warehouse", self.args.warehouse, "company")
		self.set_precision()
	...
	def set_precision(self):
		self.flt_precision = cint(artech_engine.db.get_default("float_precision")) or 2
		self.currency_precision = get_field_precision(
			artech_engine.get_meta("Stock Ledger Entry").get_field("stock_value")
		)
	...
	def initialize_previous_data(self, args):
		...
		"""
		self.data.setdefault(args.warehouse, artech_engine._dict())
		...
		if previous_sle:
			...
			self.prev_sle_dict[(args.get("item_code"), args.get("warehouse"))] = artech_engine._dict(
				{
	...
	def get_item_wh_wise_last_posted_sle(self):
		if self.args and self.args.get("item_wh_wise_last_posted_sle"):
			...
			for key, sle in self.args.get("item_wh_wise_last_posted_sle").items():
				_sles[artech_engine.safe_eval(key)] = artech_engine._dict(sle)

			return _sles

		return {
			(self.args.item_code, self.args.warehouse): artech_engine._dict(
				{
	...
	def update_item_wh_wise_last_posted_sle(self, sle):
		if not self._sles:
			self.item_wh_wise_last_posted_sle = artech_engine._dict()
			return

		self.item_wh_wise_last_posted_sle[(sle.item_code, sle.warehouse)] = artech_engine._dict(
			{
	...
	def has_stock_reco_with_serial_batch(self, sle):
		if (
			...
			and artech_engine.db.get_value(sle.voucher_type, sle.voucher_no, "set_posting_time") == 1
		):
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or artech_engine._dict(
	...
	def process_sle(self, sle):
		...
		if not self.prev_sle_dict.get(key):
			self.prev_sle_dict[key] = artech_engine._dict(
				{
					"qty_after_transaction": 0.0,
					"valuation_rate": 0.0,
					"stock_value": 0.0,
		...
		if sle.serial_and_batch_bundle:
			...
			sle.batch_no
			and artech_engine.db.get_value("Batch", sle.batch_no, "use_batchwise_valuation", cache=True)
			and not self.args.get("sle_id")
		):
			# Only run in reposting
		...
		sle.modified = now()
		artech_engine.get_doc(sle).db_update()
		...
		if not cint(artech.is_perpetual_inventory_enabled(sle.company)):
			return
	...
	def get_serialized_values(self, sle):
		from artech.stock.serial_batch_bundle import SerialNoValuation
	...
	def reset_actual_qty_for_stock_reco(self, sle):
		doc = artech_engine.get_doc("Stock Reconciliation", sle.voucher_no)
		...
		if sle.actual_qty < 0:
			...
			sle.actual_qty = (
				flt(artech_engine.db.get_value("Stock Reconciliation Item", sle.voucher_detail_no, "current_qty"))
				* -1
			)
			...
			if abs(sle.actual_qty) == 0.0:
				...
				if sle.serial_and_batch_bundle:
					...
					sabb_doc = artech_engine.get_doc("Serial and Batch Bundle", sle.serial_and_batch_bundle)
					sabb_doc.voucher_detail_no = None
					sabb_doc.voucher_no = None
					sabb_doc.cancel()

	def calculate_valuation_for_serial_batch_bundle(self, sle):
		if not artech_engine.db.exists("Serial and Batch Bundle", sle.serial_and_batch_bundle):
			return

		if sle.actual_qty < 0 and (
			sle.voucher_type in ["Stock Reconciliation", "Asset Capitalization"]
			or not artech_engine.db.get_value(sle.voucher_type, sle.voucher_no, "is_return")
		):
			doc = artech_engine._dict({})
			self.update_serial_batch_no_valuation(sle, doc, prev_sle=self.wh_data)
		else:
			doc = artech_engine.get_doc("Serial and Batch Bundle", sle.serial_and_batch_bundle)
			doc.set_incoming_rate(
				save=True, allow_negative_stock=self.allow_negative_stock, prev_sle=self.wh_data
			)
			doc.calculate_qty_and_amount(save=True)

		if stock_queue := artech_engine.get_all(
	...
	def update_serial_batch_no_valuation(self, sle, doc, prev_sle=None):
		from artech.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
	...
	def has_landed_cost_based_on_pi(self, sle):
		if sle.voucher_type == "Purchase Receipt" and artech_engine.db.get_single_value(
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		if sle.voucher_type == "Stock Entry":
			...
			rate = artech_engine.db.get_value("Stock Entry Detail", sle.voucher_detail_no, "valuation_rate")
		# Sales and Purchase Return
			...
			if artech_engine.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				...
				# check in item table
				item_code, incoming_rate = artech_engine.db.get_value(
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		artech_engine.db.set_value("Stock Entry Detail", sle.voucher_detail_no, "basic_rate", outgoing_rate)
	...
	def is_manufacture_entry_with_sabb(self, sle):
		if (
			...
			purpose = artech_engine.get_cached_value("Stock Entry", sle.voucher_no, "purpose")
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no, voucher_detail_no):
		stock_entry = artech_engine.get_lazy_doc("Stock Entry", voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		item_code = artech_engine.db.get_value(sle.voucher_type + " Item", sle.voucher_detail_no, "item_code")
		if item_code == sle.item_code:
			artech_engine.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if artech_engine.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			if sle.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
				details = artech_engine.get_cached_value(
					sle.voucher_type,
					sle.voucher_no,
		...
		if artech_engine.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = artech_engine.get_lazy_doc(sle.voucher_type, sle.voucher_no)
			doc.update_valuation_rate(reset_outgoing_rate=False)
			for d in doc.items + doc.supplied_items:
				d.db_update()

	def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
		if artech_engine.db.exists("Subcontracting Receipt Item", sle.voucher_detail_no):
			artech_engine.db.set_value("Subcontracting Receipt Item", sle.voucher_detail_no, "rate", outgoing_rate)
		else:
			artech_engine.db.set_value(
		...
		scr = artech_engine.get_lazy_doc("Subcontracting Receipt", sle.voucher_no, for_update=True)
	...
	def update_rate_on_stock_reconciliation(self, sle):
		if not sle.serial_no and not sle.batch_no:
			sr = artech_engine.get_lazy_doc("Stock Reconciliation", sle.voucher_no, for_update=True)
	...
	def get_incoming_value_for_serial_nos(self, sle, serial_nos):
		...
		all_serial_nos = artech_engine.get_all(
		...
		for serial_no in invalid_serial_nos:
			incoming_rate = artech_engine.db.sql(
				"""
	...
	def is_return_purchase_entry(self, sle):
		if sle.voucher_type in ["Purchase Invoice", "Purchase Receipt"]:
			return artech_engine.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return")

		return False

	def update_batched_values(self, sle):
		from artech.stock.serial_batch_bundle import BatchNoValuation
		...
		if actual_qty > 0:
			...
			new_sle.qty = new_sle.actual_qty
			new_sle.batch_nos = artech_engine._dict({new_sle.batch_no: new_sle})
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if ref_item_dt:
			return artech_engine.db.get_value(ref_item_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=artech.get_company_currency(sle.company),
			company=sle.company,
		)
	...
	def get_sle_before_datetime(self, args):
		...
		sle = sle[0] if sle else artech_engine._dict()
		return sle
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in self.exceptions.items():
			...
			) in artech_engine.local.flags.currently_saving:
				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					artech_engine.bold(abs(deficiency)),
					artech_engine.get_desk_link("Item", exceptions[0]["item_code"], show_title_with_name=True),
					artech_engine.get_desk_link("Warehouse", warehouse),
				)
			else:
				msg = _(
					...
					artech_engine.get_desk_link(exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]),
				)
				...
				if self.reserved_stock:
					...
					if allowed_qty > 0:
						msg = "{} As {} units are reserved for other sales orders, you are allowed to consume only {} units.".format(
							msg, artech_engine.bold(self.reserved_stock), artech_engine.bold(allowed_qty)
						)
					else:
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				artech_engine.throw(message, NegativeStockError, title=_("Insufficient Stock"))
			else:
	...
	def update_bin_data(self, sle):
		...
		artech_engine.db.set_value("Bin", bin_name, values_to_update)
	...
	def update_bin(self):
		...
		for (item_code, warehouse), data in self.prev_sle_dict.items():
			...
			artech_engine.db.set_value("Bin", bin_name, updated_values, update_modified=True)
...
def get_sle_against_current_voucher(kwargs):
	...
	doctype = artech_engine.qb.DocType("Stock Ledger Entry")
...
def get_previous_sle_of_current_voucher(args, operator="<", exclude_current_voucher=False):
	...
	sle = artech_engine.db.sql(  # nosemgrep
		f"""
	...
	return sle[0] if sle else artech_engine._dict()
...
def get_stock_ledger_entries(
	...
	if check_serial_no and previous_sle.get("serial_no"):
		# conditions += " and serial_no like {}".format(artech_engine.db.escape('%{0}%'.format(previous_sle.get("serial_no"))))
		...
		"""
			...
			artech_engine.db.escape(serial_no),
			artech_engine.db.escape(f"{serial_no}\n%"),
			artech_engine.db.escape(f"%\n{serial_no}"),
			artech_engine.db.escape(f"%\n{serial_no}\n%"),
		)
...
def get_valuation_rate(
	...
	if not company:
		company = artech_engine.get_cached_value("Warehouse", warehouse, "company")

	if warehouse and batch_no and artech_engine.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
			...
			sle=artech_engine._dict(
				{
					...
					"posting_datetime": artech_engine.get_value(
	...
	if last_valuation_rate := artech_engine.db.sql(  # nosemgrep
	...
	if fallbacks:
		...
		if rate := (
			artech_engine.db.get_value("Item", item_code, "valuation_rate")
			or artech_engine.db.get_value("Item", item_code, "standard_rate")
			or artech_engine.db.get_value(
	...
	if (
		...
		and cint(artech.is_perpetual_inventory_enabled(company))
	):
		...
		solutions += (
			...
			+ " {} ".format(artech_engine.bold(_("after")))
		...
		artech_engine.throw(msg=msg, title=_("Valuation Rate Missing"))
...
def update_qty_in_future_sle(args, allow_negative_stock=False):
	...
	artech_engine.db.sql(  # nosemgrep
		f"""
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	...
	if (
		...
		and artech_engine.db.get_value("Stock Reconciliation Item", args.voucher_detail_no, "qty") > 0
	):
		return
	...
	if is_negative_with_precision(neg_sle):
		message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
			...
			artech_engine.get_desk_link("Item", args.item_code, show_title_with_name=True),
			artech_engine.get_desk_link("Warehouse", args.warehouse),
			neg_sle[0]["posting_date"],
			neg_sle[0]["posting_time"],
			artech_engine.get_desk_link(neg_sle[0]["voucher_type"], neg_sle[0]["voucher_no"]),
		)
	...
	if args.batch_no:
		...
		if is_negative_with_precision(neg_batch_sle, is_batch=True):
			message = _(
				...
				artech_engine.get_desk_link("Batch", args.batch_no),
				artech_engine.get_desk_link("Warehouse", args.warehouse),
				neg_batch_sle[0]["posting_date"],
				neg_batch_sle[0]["posting_time"],
				artech_engine.get_desk_link(neg_batch_sle[0]["voucher_type"], neg_batch_sle[0]["voucher_no"]),
			)
			artech_engine.throw(message, NegativeStockError, title=_("Insufficient Stock for Batch"))
...
def is_negative_with_precision(neg_sle, is_batch=False):
	...
	precision = cint(artech_engine.db.get_default("float_precision")) or 2
...
def get_future_sle_with_negative_qty(sle_args):
	return artech_engine.db.sql(  # nosemgrep
		"""
		select
...
def validate_reserved_stock(kwargs):
	...
	if diff < 0 and abs(diff) > 0.0001:
		msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
			abs(diff),
			artech_engine.get_desk_link("Item", kwargs.item_code, show_title_with_name=True),
			artech_engine.get_desk_link("Warehouse", kwargs.warehouse),
			nowdate(),
			nowtime(),
		)
		artech_engine.throw(msg, title=_("Reserved Stock"))
	...
	if reserved_serial_nos_details := get_sre_reserved_serial_nos_details(item_code, warehouse, serial_nos):
		if common_serial_nos := list(set(serial_nos).intersection(set(reserved_serial_nos_details.keys()))):
			...
			msg += _("Example: Serial No {0} reserved in {1}.").format(
				artech_engine.bold(common_serial_nos[0]),
				artech_engine.get_desk_link(
					"Stock Reservation Entry", reserved_serial_nos_details[common_serial_nos[0]]
				),
			)
			artech_engine.throw(msg, title=_("Reserved Serial No."))
	...
	if reserved_batches_map := get_sre_reserved_batch_nos_details(item_code, warehouse, batch_nos):
		...
		for batch_no in batch_nos:
			...
			if diff < 0 and abs(diff) > 0.0001:
				msg = _("{0} units of {1} needed in {2} on {3} {4} to complete this transaction.").format(
					abs(diff),
					artech_engine.get_desk_link("Batch", batch_no),
					artech_engine.get_desk_link("Warehouse", warehouse),
					nowdate(),
					nowtime(),
				)
				artech_engine.throw(msg, title=_("Reserved Stock for Batch"))


def is_negative_stock_allowed(*, item_code: str | None = None) -> bool:
	if artech_engine.get_cached_doc("Stock Settings").allow_negative_stock:
		return True
	if item_code and cint(artech_engine.get_cached_value("Item", item_code, "allow_negative_stock")):
		return True
	return False
...
def get_incoming_rate_for_inter_company_transfer(sle) -> float:
	...
	reference_name = artech_engine.get_cached_value(sle.voucher_type + " Item", sle.voucher_detail_no, field)

	if reference_name:
		rate = artech_engine.get_cached_value(
			doctype,
			reference_name,
			"incoming_rate",
		)
	...
	lcv_amount = artech_engine.db.get_value(
...
def is_transfer_stock_entry(voucher_no):
	purpose = artech_engine.get_cached_value("Stock Entry", voucher_no, "purpose")
...
def is_repack_entry(stock_entry_id):
	return artech_engine.get_cached_value("Stock Entry", stock_entry_id, "purpose") == "Repack"
