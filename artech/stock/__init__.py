...
def get_warehouse_account_map(company=None):
	company_warehouse_account_map = company and artech_engine.flags.setdefault("warehouse_account_map", {}).get(
		company
	)
	warehouse_account_map = artech_engine.flags.warehouse_account_map

	if not warehouse_account_map or not company_warehouse_account_map or artech_engine.in_test:
		warehouse_account = artech_engine._dict()

		filters = {}
		if company:
			filters["company"] = company
			artech_engine.flags.setdefault("warehouse_account_map", {}).setdefault(company, {})
		...
		for d in artech_engine.get_all(
			...
			if d.account:
				d.account_currency = artech_engine.db.get_value("Account", d.account, "account_currency", cache=True)
				warehouse_account.setdefault(d.name, d)
		if company:
			artech_engine.flags.warehouse_account_map[company] = warehouse_account
		else:
			artech_engine.flags.warehouse_account_map = warehouse_account

	return artech_engine.flags.warehouse_account_map.get(company) or artech_engine.flags.warehouse_account_map
...
def get_company_default_inventory_account(company):
	return artech_engine.get_cached_value("Company", company, "default_inventory_account")
