...
from artech_engine.utils import cint, cstr, flt, get_link_to_form, getdate, new_line_sep, nowdate
...
from artech.stock.stock_balance import get_indented_qty, update_bin_qty
...
class MaterialRequest(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.material_request_item.material_request_item import MaterialRequestItem
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = artech_engine.db.sql(
					"""select sum(qty)
				...
				actual_so_qty = artech_engine.db.sql(
	...
	def validate(self):
		...
		if not self.buying_price_list:
			self.buying_price_list = artech_engine.defaults.get_defaults().buying_price_list
	...
	def validate_pp_qty(self):
		...
		if items_from_pp:
			...
			table = artech_engine.qb.DocType("Material Request Plan Item")
			query = (
	...
	def on_submit(self):
		...
		if self.material_request_type == "Purchase":
			...
			if artech_engine.db.exists("Budget", {"applicable_on_material_request": 1, "docstatus": 1}):
	...
	def check_modified_date(self):
		mod_db = artech_engine.db.sql("""select modified from `tabMaterial Request` where name = %s""", self.name)
		date_diff = artech_engine.db.sql(f"""select TIMEDIFF('{mod_db[0][0]}', '{cstr(self.modified)}')""")

		if date_diff and date_diff[0][0]:
			artech_engine.throw(_("{0} {1} has been modified. Please refresh.").format(_(self.doctype), self.name))
	...
	def get_mr_items_ordered_qty(self, mr_items):
		...
		if self.material_request_type in ("Material Issue", "Material Transfer", "Customer Provided"):
			doctype = artech_engine.qb.DocType("Stock Entry Detail")
		...
		if doctype and qty_field:
			...
			mr_items_ordered_qty = artech_engine._dict(query.run())
	...
	def update_completed_qty(self, mr_items=None, update_modified=True):
		...
		mr_qty_allowance = artech_engine.db.get_single_value("Stock Settings", "mr_qty_allowance")
		...
		for d in self.get("items"):
			...
			if d.name in mr_items:
				...
				artech_engine.db.set_value(d.doctype, d.name, "ordered_qty", d.ordered_qty)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (
				...
				and d.warehouse
				and artech_engine.db.get_value("Item", d.item_code, "is_stock_item") == 1
			):
	...
	def update_requested_qty_in_production_plan(self, cancel=False):
		...
		for d in self.get("items"):
			if d.production_plan and d.material_request_plan_item:
				requested_qty = artech_engine.get_value(
		...
		for production_plan in production_plans:
			doc = artech_engine.get_doc("Production Plan", production_plan)
			doc.set_status()
...
def update_status(name: str, status: str):
	material_request = artech_engine.get_doc("Material Request", name)
...
def make_purchase_order(
	...
	is_subcontracted = (
		artech_engine.db.get_value("Material Request", source_name, "material_request_type") == "Subcontracting"
	)
...
def get_material_requests_based_on_supplier(
	...
	if not supplier_items:
		artech_engine.throw(_("{0} is not the default supplier for any items.").format(supplier))
...
def make_stock_entry(source_name: str, target_doc: str | Document | None = None):
	...
	def set_missing_values(source, target):
		...
		if source.job_card:
			job_card_details = artech_engine.get_all(
...
def raise_work_orders(material_request: str, company: str):
	...
	errors = []
	work_orders = []
	default_wip_warehouse = artech_engine.get_cached_value("Company", company, "default_wip_warehouse")

	for d in mr.items:
		if (d.stock_qty - d.ordered_qty) > 0:
			if artech_engine.db.exists("BOM", {"item": d.item_code, "is_default": 1, "is_active": 1}) or (
				(variant_of := artech_engine.get_value("Item", d.item_code, "variant_of"))
				and artech_engine.db.exists("BOM", {"item": variant_of, "is_default": 1, "is_active": 1})
			):
		...
		if len(work_orders) > 1:
			msgprint(
				_("The following {0} were created: {1}").format(
					artech_engine.bold(_("Work Orders")), "<br>" + ", ".join(work_orders_list)
				)
			)
		else:
			msgprint(
				_("The {0} {1} created successfully").format(
					artech_engine.bold(_("Work Order")), work_orders_list[0]
				)
			)
