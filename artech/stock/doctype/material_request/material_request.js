...
artech_engine.ui.form.on("Material Request", {
	setup: function (frm) {
	...
	schedule_date(frm) {
		if (frm.doc.schedule_date) {
			frm.doc.items.forEach((d) => {
				artech_engine.model.set_value(d.doctype, d.name, "schedule_date", frm.doc.schedule_date);
			});
		}
	},
	...
	onload: function (frm) {
		...
		if (!frm.doc.buying_price_list) {
			frm.doc.buying_price_list = artech_engine.defaults.get_default("buying_price_list");
		}
	},
	...
	refresh: function (frm) {
		...
		artech.buying.prevent_past_schedule_dates(frm);
	...
	set_from_warehouse: function (frm) {
		if (frm.doc.material_request_type == "Material Transfer" && frm.doc.set_from_warehouse) {
			frm.doc.items.forEach((d) => {
				artech_engine.model.set_value(d.doctype, d.name, "from_warehouse", frm.doc.set_from_warehouse);
			});
		}
	},
	...
	make_custom_buttons: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.status != "Stopped") {
			let precision = artech_engine.defaults.get_default("float_precision");
	...
	update_status: function (frm, stop_status) {
		artech_engine.call({
			method: "artech.stock.doctype.material_request.material_request.update_status",
			...
			args: {
				ctx: {
					...
					buying_price_list: frm.doc.buying_price_list
						...
						: artech_engine.defaults.get_default("buying_price_list"),
					currency: artech_engine.defaults.get_default("Currency"),
					name: frm.doc.name,
					qty: item.qty || 1,
			...
			callback: function (r) {
				...
				if (!r.exc) {
					...
					if (d.price_list_rate != r.message.price_list_rate) {
						...
						artech_engine.model.set_value(d.doctype, d.name, "rate", d.price_list_rate);
					}
					...
					callback: function (r) {
						if (!r.message) {
							...
							$.each(r.message, function (i, item) {
								var d = artech_engine.model.add_child(cur_frm.doc, "Material Request Item", "items");
	...
	make_purchase_order: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.material_request.material_request.make_purchase_order",
			frm: frm,
	...
	make_request_for_quotation: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.material_request.material_request.make_request_for_quotation",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	make_supplier_quotation: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.material_request.material_request.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_in_transit_stock_entry(frm) {
		artech_engine.prompt(
			...
			(values) => {
				artech_engine.call({
					method: "artech.stock.doctype.material_request.material_request.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.material_request.material_request.create_pick_list",
			frm: frm,
		});
	},
	...
	raise_work_orders: function (frm) {
		artech_engine.call({
			method: "artech.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
artech_engine.ui.form.on("Material Request Item", {
	qty: function (frm, doctype, name) {
		const item = locals[doctype][name];
		if (flt(item.qty) < flt(item.min_order_qty)) {
			artech_engine.msgprint(__("Warning: Material Requested Qty is less than Minimum Order Qty"));
		}
	...
	rate(frm, doctype, name) {
		...
		artech_engine.model.set_value(doctype, name, "amount", item.amount);
	...
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
artech.buying.MaterialRequestController = class MaterialRequestController extends (
	...
	qty(doc, cdt, cdn) {
		...
		artech_engine.model.set_value(cdt, cdn, "amount", row.amount);
		refresh_field("amount", row.name, row.parentfield);
	}
};

// for backward compatibility: combine new and previous states
extend_cscript(cur_frm.cscript, new artech.buying.MaterialRequestController({ frm: cur_frm }));

function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		artech.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
