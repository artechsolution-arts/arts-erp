...
from artech_engine.utils import flt, today
...
class TestMaterialRequest(ArtechTestSuite):
	...
	def test_make_purchase_order(self):
		mr = artech_engine.copy_doc(self.globalTestRecords["Material Request"][0]).insert()

		self.assertRaises(artech_engine.ValidationError, make_purchase_order, mr.name)

		mr = artech_engine.get_doc("Material Request", mr.name)
		mr.submit()
	...
	def test_make_subcontracted_purchase_order(self):
		...
		artech_engine.db.set_value("Item", mr.items[0].item_code, "is_sub_contracted_item", 1)

		raw_materials = ["Raw Material Item 1", "Raw Material Item 2"]
		for item in raw_materials:
			create_item(item)

		artech_engine.new_doc("UOM").update({"uom_name": "Test UOM"}).save()
	...
	def test_make_supplier_quotation(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_supplier_quotation, mr.name)
	...
	def test_make_stock_entry(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_stock_entry, mr.name)
	...
	def test_partial_make_stock_entry(self):
		from artech.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry as _make_stock_entry
	...
	def test_cannot_stop_cancelled_material_request(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(artech_engine.ValidationError, mr.update_status, "Stopped")
	...
	def test_cannot_submit_cancelled_mr(self):
		...
		mr.load_from_db()
		mr.cancel()
		self.assertRaises(artech_engine.ValidationError, mr.submit)
	...
	def test_cannot_change_cancelled_mr(self):
		...
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Draft")
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Stopped")
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Ordered")
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Issued")
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Transferred")
		self.assertRaises(artech_engine.InvalidStatusError, mr.update_status, "Pending")
	...
	def test_cannot_delete_submitted_mr(self):
		...
		self.assertRaises(artech_engine.ValidationError, mr.delete)
	...
	def test_completed_qty_for_purchase(self):
		...
		po = artech_engine.copy_doc(po_doc)
		po.insert()
		po.load_from_db()
		mr.update_status("Stopped")
		self.assertRaises(artech_engine.InvalidStatusError, po.submit)
		po.db_set("docstatus", 1)
		self.assertRaises(artech_engine.InvalidStatusError, po.cancel)
	...
	def test_completed_qty_for_transfer(self):
		...
		# check for stopped status of Material Request
		se = artech_engine.copy_doc(se_doc)
		se.insert()
		mr.update_status("Stopped")
		self.assertRaises(artech_engine.InvalidStatusError, se.submit)
		...
		se.submit()
		mr.update_status("Stopped")
		self.assertRaises(artech_engine.InvalidStatusError, se.cancel)
	...
	def test_over_transfer_qty_allowance(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "mr_qty_allowance", 20)
		...
		se = artech_engine.copy_doc(se_doc)
		se.insert()
		self.assertRaises(artech_engine.ValidationError)
		se.items[0].qty = 12
		se.submit()
	...
	def test_incorrect_mapping_of_stock_entry(self):
		...
		# check for stopped status of Material Request
		se = artech_engine.copy_doc(se_doc)
		self.assertRaises(artech_engine.MappingMismatchError, se.insert)
	...
	def _get_requested_qty(self, item_code, warehouse):
		return flt(
			artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty")
		)
	...
	def test_completed_qty_for_issue(self):
		...
		mr.material_request_type = "Material Issue"
		mr.submit()
		artech_engine.db.value_cache.clear()
	...
	def test_material_request_type_manufacture(self):
		mr = artech_engine.copy_doc(self.globalTestRecords["Material Request"][1]).insert()
		mr = artech_engine.get_doc("Material Request", mr.name)
		mr.submit()
		completed_qty = mr.items[0].ordered_qty
		requested_qty = artech_engine.db.sql(
		...
		po = artech_engine.get_doc("Work Order", prod_order[0])
		...
		new_requested_qty = artech_engine.db.sql(
			"""select indented_qty from `tabBin` where \
			item_code= %s and warehouse= %s """,
			(mr.items[0].item_code, mr.items[0].warehouse),
		)[0][0]
	...
	def test_requested_qty_multi_uom(self):
		...
		wo = artech_engine.get_doc("Work Order", work_order[0])
		wo.qty = 50
	...
	def test_multi_uom_for_purchase(self):
		...
		if not artech_engine.db.get_value("UOM Conversion Detail", {"parent": item.item_code, "uom": "Kg"}):
			item_doc = artech_engine.get_doc("Item", item.item_code)
	...
	def test_auto_email_users_with_company_user_permissions(self):
		from artech.stock.reorder_item import get_email_list
		...
		for company, user in comapnywise_users.items():
			...
			if not artech_engine.db.exists(
				...
				perm_doc = artech_engine.get_doc(
					{
		...
		comapnywise_mr_list = artech_engine._dict({})
	...
	def test_get_remaining_qty_from_sales_order(self):
		...
		from artech.selling.doctype.sales_order.sales_order import make_material_request
		from artech.selling.doctype.sales_order.test_sales_order import make_sales_order
	...
	def test_pending_qty_in_pick_list(self):
		...
		from artech.stock.doctype.pick_list.pick_list import create_stock_entry
		...
		se = artech_engine.get_doc(se_data)
	...
	def test_mr_status_with_partial_and_excess_end_transit(self):
		...
		self.assertRaises(artech_engine.ValidationError, end_transit_2.submit)
...
def get_in_transit_warehouse(company):
	...
	in_transit_warehouse = artech_engine.db.exists("Warehouse", {"warehouse_type": "Transit", "company": company})
