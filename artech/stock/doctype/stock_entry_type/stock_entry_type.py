...
class StockEntryType(Document):
	...
	def validate_standard_type(self):
		if self.is_standard and self.name not in [
			...
			artech_engine.throw(f"Stock Entry Type {self.name} cannot be set as standard")
...
class ManufactureEntry:
	...
	def make_stock_entry(self):
		self.stock_entry = artech_engine.new_doc("Stock Entry")
	...
	def prepare_source_warehouse(self):
		...
		if self.skip_material_transfer:
			if not self.backflush_from_wip_warehouse:
				self.source_wh = artech_engine._dict(
	...
	def add_raw_materials(self):
		if self.job_card:
			...
			available_serial_batches = artech_engine._dict({})
			...
			for item_code, _dict in item_dict.items():
				...
				if backflush_based_on != "BOM" and not artech_engine.db.get_value(
	...
	def get_transferred_serial_batches(self):
		...
		for row in stock_entry_data:
			...
			if key not in available_serial_batches:
				available_serial_batches[key] = artech_engine._dict(
					{
			...
			sabb_data = artech_engine.get_all(
		...
		for item, item_details in item_dict.items():
			for d in [
				...
				if not item_details.get(d[1]) or (company_in_record and self.company != company_in_record):
					item_dict[item][d[1]] = (
						artech_engine.get_cached_value("Company", self.company, d[2]) if d[2] else None
					)
