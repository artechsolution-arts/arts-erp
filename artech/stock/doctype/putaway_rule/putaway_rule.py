...
from artech_engine.utils import cint, cstr, floor, flt, nowdate
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = artech_engine.db.exists(
	...
	def validate_priority(self):
		if self.priority < 1:
			artech_engine.throw(_("Priority cannot be lesser than 1."), title=_("Invalid Priority"))

	def validate_warehouse_and_company(self):
		company = artech_engine.db.get_value("Warehouse", self.warehouse, "company")
		if company != self.company:
			artech_engine.throw(
				_("Warehouse {0} does not belong to Company {1}.").format(
					artech_engine.bold(self.warehouse), artech_engine.bold(self.company)
				),
				title=_("Invalid Warehouse"),
			)

	def validate_capacity(self):
		stock_uom = artech_engine.db.get_value("Item", self.item_code, "stock_uom")
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			artech_engine.throw(
				...
				).format(self.item_code, artech_engine.bold(balance_qty), stock_uom),
				title=_("Insufficient Capacity"),
			)

		if not self.capacity:
			artech_engine.throw(_("Capacity must be greater than 0"), title=_("Invalid"))
...
def get_available_putaway_capacity(rule: str):
	stock_capacity, item_code, warehouse = artech_engine.db.get_value(
...
def apply_putaway_rule(
	...
	for item in items:
		...
		uom_must_be_whole_number = artech_engine.db.get_value("UOM", item.uom, "must_be_whole_number")
	...
	if updated_table and _items_changed(items, updated_table, doctype):
		artech_engine.msgprint(_("Applied putaway rules."), alert=True)
...
def _items_changed(old, new, doctype: str) -> bool:
	...
	old = [artech_engine._dict(item) if isinstance(item, dict) else item for item in old]
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		item_link = artech_engine.utils.get_link_to_form("Item", entry[0])
		formatted_item_rows += f"""
			<td>{item_link}</td>
			<td>{artech_engine.bold(entry[1])}</td>
		</tr>"""
	...
	artech_engine.msgprint(msg, title=_("Insufficient Capacity"), is_minimizable=True, wide=True)
