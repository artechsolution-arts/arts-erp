...
artech.landed_cost_taxes_and_charges.setup_triggers("Landed Cost Voucher");
artech.stock.LandedCostVoucher = class LandedCostVoucher extends artech.stock.StockController {
	refresh() {
		...
		if (this.frm.doc.company) {
			let company_currency = artech_engine.get_doc(":Company", this.frm.doc.company).default_currency;
	...
	get_items_from_purchase_receipts() {
		...
		if (!this.frm.doc.purchase_receipts.length) {
			artech_engine.msgprint(__("Please enter Purchase Receipt first"));
		} else {
...
cur_frm.script_manager.make(artech.stock.LandedCostVoucher);
...
artech_engine.ui.form.on("Landed Cost Voucher", {
	setup(frm) {
	...
	setup_queries(frm) {
		...
		frm.set_query("vendor_invoice", "vendor_invoices", (doc, cdt, cdn) => {
			return {
				query: "artech.stock.doctype.landed_cost_voucher.landed_cost_voucher.get_vendor_invoices",
				filters: {
					company: doc.company,
				},
			};
...
artech_engine.ui.form.on("Landed Cost Purchase Receipt", {
...
artech_engine.ui.form.on("Landed Cost Vendor Invoice", {
