...
from artech_engine.utils import add_days, add_to_date, flt, now, nowtime, today
...
from artech.accounts.utils import update_gl_entries_after
...
class TestLandedCostVoucher(ArtechTestSuite):
	def setUp(self):
	...
	def test_landed_cost_voucher(self):
		...
		last_sle = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		pr1 = make_purchase_receipt(
			...
			qty=500,
			rate=80,
			posting_date=add_days(artech_engine.utils.nowdate(), -2),
		)
		...
		last_sle = artech_engine.db.get_value(  # SLE of second PR
			"Stock Ledger Entry",
			{
		...
		last_sle_after_landed_cost = artech_engine.db.get_value(  # SLE of second PR after LCV's effect
			"Stock Ledger Entry",
			{
				"voucher_type": pr2.doctype,
				"voucher_no": pr2.name,
	...
	def test_lcv_validates_company(self):
		from artech import is_perpetual_inventory_enabled
		from artech.accounts.doctype.account.test_account import create_account
		from artech.stock.doctype.landed_cost_voucher.landed_cost_voucher import (
		...
		company_doc = artech_engine.get_doc("Company", company_a)
		...
		artech_engine.db.set_value("Company", company_a, "enable_perpetual_inventory", epi)
		artech_engine.local.enable_perpetual_inventory = {}
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = artech_engine.db.get_value(
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		artech_engine.db.set_value("Item", "_Test Serialized Item", "serial_no_series", "SNJJ.###")
	...
	def test_multi_currency_lcv(self):
		from artech.setup.doctype.currency_exchange.test_currency_exchange import save_new_records
	...
	def test_asset_lcv(self):
		...
		if not artech_engine.db.exists("Item", "Macbook Pro"):
		...
		self.assertEqual(artech_engine.db.get_value("Asset", assets[0].name, "net_purchase_amount"), 50080)
	...
	def test_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not artech_engine.db.exists("Batch", "BATCH-TLCVSNO-0001"):
		...
		for row in pr.items:
			...
			self.assertEqual(
				row.valuation_rate,
				artech_engine.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "avg_rate"),
			)
	...
	def test_do_not_validate_landed_cost_voucher_with_serial_batch_for_legacy_pr(self):
		...
		if not artech_engine.db.exists("Batch", "BATCH-TDVLCVSNO-0001"):
			batch_doc = artech_engine.get_doc(
				{
					"doctype": "Batch",
					"item": batch_item,
		...
		stock_ledger_entries = artech_engine.get_all("Stock Ledger Entry", filters={"voucher_no": pr.name})
		for sle in stock_ledger_entries:
			doc = artech_engine.get_doc("Stock Ledger Entry", sle.name)
		...
		stock_ledger_entries = artech_engine.get_all("Stock Ledger Entry", filters={"voucher_no": dn.name})
		for sle in stock_ledger_entries:
			doc = artech_engine.get_doc("Stock Ledger Entry", sle.name)
			if doc.item_code == sn_item:
				doc.db_set("serial_no", ", ".join(serial_nos))
	...
	def test_lcv_for_work_order_scr(self):
		...
		company = "_Test Company with perpetual inventory"
		warehouse = artech_engine.get_value("Warehouse", {"company": company}, "name")
		wip_warehouse = artech_engine.get_value(
			"Warehouse",
		...
		se = artech_engine.get_doc(make_stock_entry_for_wo(wo_order.name, "Material Transfer for Manufacture", 10))
		se.submit()

		se = artech_engine.get_doc(make_stock_entry_for_wo(wo_order.name, "Manufacture", 10))
		se.submit()
...
def make_landed_cost_voucher(**args):
	...
	ref_doc = artech_engine.get_doc(args.receipt_document_type, args.receipt_document)
