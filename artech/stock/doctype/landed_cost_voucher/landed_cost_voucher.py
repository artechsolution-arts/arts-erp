...
class IncorrectCompanyValidationError(artech_engine.ValidationError):
	pass
...
class LandedCostVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.landed_cost_item.landed_cost_item import LandedCostItem
		from artech.stock.doctype.landed_cost_purchase_receipt.landed_cost_purchase_receipt import (
		...
		from artech.stock.doctype.landed_cost_vendor_invoice.landed_cost_vendor_invoice import (
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				...
				for d in pr_items:
					...
					item.cost_center = d.cost_center or artech.get_default_cost_center(self.company)
	...
	def validate_line_items(self):
		for d in self.get("items"):
			if (
				...
				and not artech_engine.db.exists(
				...
				artech_engine.throw(
					_("Row {0}: {2} Item {1} does not exist in {2} {3}").format(
						d.idx,
						artech_engine.bold(d.purchase_receipt_item),
						d.receipt_document_type,
						artech_engine.bold(d.receipt_document),
					),
	...
	def check_mandatory(self):
		if not self.get("purchase_receipts"):
			artech_engine.throw(_("Please enter Receipt Document"))

	def validate_receipt_documents(self):
		receipt_documents = []

		for d in self.get("purchase_receipts"):
			docstatus, company = artech_engine.get_cached_value(
				d.receipt_document_type, d.receipt_document, ["docstatus", "company"]
			)
			if docstatus != 1:
				msg = f"Row {d.idx}: {d.receipt_document_type} {artech_engine.bold(d.receipt_document)} must be submitted"
				artech_engine.throw(_(msg), title=_("Invalid Document"))
	...
	def validate_expense_accounts(self):
		...
		for t in self.taxes:
			company = artech_engine.get_cached_value("Account", t.expense_account, "company")
			...
			if company != self.company:
				artech_engine.throw(
					_(
						...
						t.idx,
						artech_engine.bold(t.expense_account),
						artech_engine.bold(company),
						artech_engine.bold(self.company),
					),
	...
	def set_applicable_charges_on_item(self):
		if self.get("taxes") and self.distribute_charges_based_on != "Distribute Manually":
			...
			total_charges = 0.0
			item_count = 0
			based_on_field = artech_engine.scrub(self.distribute_charges_based_on)
	...
	def validate_applicable_charges_for_item(self):
		...
		precision = get_field_precision(
			artech_engine.get_meta("Landed Cost Item").get_field("applicable_charges"),
			currency=artech_engine.get_cached_value("Company", self.company, "default_currency"),
		)
	...
	def update_landed_cost(self):
		for d in self.get("purchase_receipts"):
			doc = artech_engine.get_doc(d.receipt_document_type, d.receipt_document)
...
def get_pr_items(purchase_receipt):
	...
	if purchase_receipt.receipt_document_type == "Stock Entry":
		pr_item = artech_engine.qb.DocType("Stock Entry Detail")
	else:
		pr_item = artech_engine.qb.DocType(purchase_receipt.receipt_document_type + " Item")

	query = (
		artech_engine.qb.from_(pr_item)
		.inner_join(item)
...
def get_vendor_invoices(
	...
	if not artech_engine.has_permission("Purchase Invoice", "read"):
		return []

	if txt and txt.lower().startswith(("select", "delete", "update")):
		artech_engine.throw(_("Invalid search query"), title=_("Invalid Query"))

	query = get_vendor_invoice_query(filters)

	if txt:
		query = query.where(artech_engine.qb.DocType(doctype).name.like(f"%{txt}%"))
...
def get_vendor_invoice_query(filters):
	doctype = artech_engine.qb.DocType("Purchase Invoice")
	child_doctype = artech_engine.qb.DocType("Purchase Invoice Item")
	item = artech_engine.qb.DocType("Item")
	...
	query = (
		...
		.having(artech_engine.qb.Field("unclaimed_amount") > 0)
	)
