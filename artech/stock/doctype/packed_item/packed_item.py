...
from artech.stock.get_item_details import ItemDetailsCtx, get_item_details, get_price_list_rate
...
class PackedItem(Document):
	...
	def set_actual_and_projected_qty(self):
		...
		_bin = artech_engine.db.get_value(
			"Bin",
...
def make_packing_list(doc):
	...
	set_price_from_children = artech_engine.get_single_value("Selling Settings", "editable_bundle_item_rates")
...
def is_product_bundle(item_code: str) -> bool:
	return bool(artech_engine.db.exists("Product Bundle", {"new_item_code": item_code, "disabled": 0}))
...
def get_product_bundle_items(item_code):
	product_bundle = artech_engine.qb.DocType("Product Bundle")
	product_bundle_item = artech_engine.qb.DocType("Product Bundle Item")

	query = (
		artech_engine.qb.from_(product_bundle_item)
		.join(product_bundle)
...
def update_packed_item_stock_data(main_item_row, pi_row, packing_item, item_data, doc):
	...
	pi_row.use_serial_batch_fields = artech_engine.get_single_value("Stock Settings", "use_serial_batch_fields")


def update_packed_item_with_pick_list_info(main_item_row, pi_row):
	pl_row = artech_engine.db.get_value(
		"Pick List Item",
		{
...
def update_packed_item_price_data(pi_row, item_data, doc):
	...
	item_doc = artech_engine.get_cached_doc("Item", pi_row.item_code)
...
def get_packed_item_bin_qty(item, warehouse):
	bin_data = artech_engine.db.get_values(
		"Bin",
...
def on_doctype_update():
	artech_engine.db.add_index("Packed Item", ["item_code", "warehouse"])
		...
		row.update(
			{
				...
				"conversion_rate": 1,
				"currency": artech_engine.defaults.get_defaults().currency,
			}
		)
