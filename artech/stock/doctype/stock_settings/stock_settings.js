artech_engine.ui.form.on("Stock Settings", {
	...
	enable_serial_and_batch_no_for_item(frm) {
		if (frm.doc.enable_serial_and_batch_no_for_item) {
			artech_engine.msgprint(__("After save, please refresh the page to apply the changes."));
		}
	},
	...
	auto_insert_price_list_rate_if_missing(frm) {
		...
		frm.set_value(
			...
			cint(artech_engine.defaults.get_default("editable_price_list_rate")) ? "Price List Rate" : "Rate"
		);
	},
	update_price_list_based_on(frm) {
		if (
			frm.doc.update_price_list_based_on === "Price List Rate" &&
			!cint(artech_engine.defaults.get_default("editable_price_list_rate"))
		) {
			const dialog = artech_engine.warn(
