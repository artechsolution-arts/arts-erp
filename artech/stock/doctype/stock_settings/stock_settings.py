...
class StockSettings(Document):
	...
	def validate(self):
		for key in [
			...
			artech_engine.db.set_default(key, self.get(key, ""))
	...
	def validate_do_not_use_batchwise_valuation(self):
		...
		if not artech_engine.get_all("Serial and Batch Bundle", filters={"docstatus": 1}, limit=1, pluck="name"):
			return
		...
		if doc_before_save.do_not_use_batchwise_valuation and not self.do_not_use_batchwise_valuation:
			artech_engine.throw(
				_("Cannot disable {0} as it may lead to incorrect stock valuation.").format(
					artech_engine.bold(_("Do Not Use Batchwise Valuation"))
				)
			)
	...
	def validate_serial_and_batch_no_settings(self):
		...
		if (
			...
			if artech_engine.get_all("Serial and Batch Bundle", filters={"docstatus": 1}, limit=1, pluck="name"):
				artech_engine.throw(
					_(
	...
	def validate_warehouses(self):
		...
		for field in warehouse_fields:
			if artech_engine.db.get_value("Warehouse", self.get(field), "is_group"):
				artech_engine.throw(
					_(
						"Group Warehouses cannot be used in transactions. Please change the value of {0}"
					).format(artech_engine.bold(self.meta.get_field(field).label)),
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) and not int(self.db_get("clean_description_html") or 0):
			...
			artech_engine.enqueue(
				"artech.stock.doctype.stock_settings.stock_settings.clean_all_descriptions",
			...
			if self.allow_negative_stock and self.enable_stock_reservation:
				artech_engine.throw(
					_("As {0} is enabled, you can not enable {1}.").format(
						artech_engine.bold(_("Stock Reservation")), artech_engine.bold(_("Allow Negative Stock"))
					)
				)
		...
		if self.has_value_changed("enable_stock_reservation"):
			...
			if self.enable_stock_reservation:
				...
				if self.allow_negative_stock:
					artech_engine.throw(
						_("As {0} is enabled, you can not enable {1}.").format(
							artech_engine.bold(_("Allow Negative Stock")), artech_engine.bold(_("Stock Reservation"))
						)
					)
			...
			# Enable -> Disable
				...
				has_reserved_stock = artech_engine.db.exists(
	...
	def validate_auto_insert_price_list_rate_if_missing(self):
		if (
			...
			selling_meta = artech_engine.get_meta("Selling Settings")
			artech_engine.msgprint(
				_(
					...
					artech_engine.bold(_("Selling Settings")),
				)
			)
	...
	def make_property_setter_for_precision(doctypes):
		for doctype in doctypes:
			if property_name := artech_engine.db.exists(
				...
				artech_engine.db.set_value("Property Setter", property_name, "value", 9)
				continue
...
def clean_all_descriptions():
	for item in artech_engine.get_all("Item", ["name", "description"]):
		if item.description:
			clean_description = clean_html(item.description)
			if item.description != clean_description:
				artech_engine.db.set_value("Item", item.name, "description", clean_description)
