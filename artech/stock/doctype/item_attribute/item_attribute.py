...
class ItemAttributeIncrementError(artech_engine.ValidationError):
	pass
...
class ItemAttribute(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.item_attribute_value.item_attribute_value import ItemAttributeValue
	...
	def set_enabled_disabled_in_items(self):
		...
		if not db_value or db_value.disabled != self.disabled:
			item_variant_table = artech_engine.qb.DocType("Item Variant Attribute")
			query = (
				artech_engine.qb.update(item_variant_table)
	...
	def validate_numeric(self):
		if self.numeric_values:
			...
			if self.from_range is None or self.to_range is None:
				artech_engine.throw(_("Please specify from/to range"))

			elif flt(self.from_range) >= flt(self.to_range):
				artech_engine.throw(_("From Range has to be less than To Range"))

			if not self.increment:
				artech_engine.throw(_("Increment cannot be 0"), ItemAttributeIncrementError)
		else:
	...
	def validate_duplication(self):
		...
		for d in self.item_attribute_values:
			...
			if d.abbr.lower() in map(str.lower, abbrs):
				artech_engine.throw(_("Abbreviation: {0} must appear only once").format(d.abbr.title()))
			abbrs.append(d.abbr)
