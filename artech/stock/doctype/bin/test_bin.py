...
from artech.stock.utils import _create_bin
from artech.tests.utils import ArtechTestSuite


class TestBin(ArtechTestSuite):
	def test_concurrent_inserts(self):
		...
		bin1 = artech_engine.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		bin1.insert()

		bin2 = artech_engine.get_doc(doctype="Bin", item_code=item_code, warehouse=warehouse)
		with self.assertRaises(artech_engine.UniqueValidationError):
			bin2.insert()
	...
	def test_index_exists(self):
		indexes = artech_engine.db.sql("show index from tabBin where Non_unique = 0", as_dict=1)
