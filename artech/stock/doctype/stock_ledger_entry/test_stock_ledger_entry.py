...
from artech_engine.core.page.permission_manager.permission_manager import reset
from artech_engine.query_builder.functions import Timestamp
from artech_engine.utils import add_days, add_to_date, flt, today
...
from artech.stock.doctype.stock_ledger_entry.stock_ledger_entry import BackDatedStockTransaction
...
class TestStockLedgerEntry(ArtechTestSuite, StockTestMixin):
	def setUp(self):
		create_items()
		reset("Stock Entry")
	...
	def test_item_cost_reposting(self):
		...
		target_wh_sle = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": "_Test Item for Reposting",
				"warehouse": "Finished Goods - _TC",
		...
		finished_item_sle = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_back_dated_entry_not_allowed(self):
		...
		try:
			user = artech_engine.get_doc("User", "test@example.com")
	...
	def test_fifo_dependent_consumption(self):
		...
		for rate in rates[1:]:
			row = artech_engine.copy_doc(receipt.items[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		for _ in rates[1:]:
			row = artech_engine.copy_doc(transfer.items[0], ignore_no_copy=False)
	...
	def test_fifo_multi_item_repack_consumption(self):
		...
		for _ in rates[1:]:
			row = artech_engine.copy_doc(repack.items[0], ignore_no_copy=False)
	...
	def test_tie_breaking(self):
		from artech.stock.doctype.repost_item_valuation.repost_item_valuation import repost_entries

		artech_engine.flags.dont_execute_stock_reposts = True
		self.addCleanup(artech_engine.flags.pop, "dont_execute_stock_reposts")
	...
	@ArtechTestSuite.change_settings("System Settings", {"float_precision": 3, "currency_precision": 2})
	...
	@ArtechTestSuite.change_settings("System Settings", {"float_precision": 4})
...
def create_repack_entry(**args):
	...
	repack = artech_engine.new_doc("Stock Entry")
...
def create_product_bundle_item(new_item_code, packed_items):
	if not artech_engine.db.exists("Product Bundle", new_item_code):
		item = artech_engine.new_doc("Product Bundle")
