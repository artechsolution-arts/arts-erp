...
from artech_engine.utils import add_days, cint, flt, formatdate, get_datetime, getdate

from artech.accounts.utils import get_fiscal_year
from artech.controllers.item_variant import ItemTemplateCannotHaveStock
from artech.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
from artech.stock.serial_batch_bundle import SerialBatchBundle


class StockFreezeError(artech_engine.ValidationError):
	pass


class BackDatedStockTransaction(artech_engine.ValidationError):
	pass


class InventoryDimensionNegativeStockError(artech_engine.ValidationError):
	pass
...
class StockLedgerEntry(Document):
	...
	def validate_inventory_dimension_negative_stock(self):
		...
		flt_precision = cint(artech_engine.db.get_default("float_precision")) or 2
	...
	def throw_validation_error(self, diff, dimensions):
		msg = _(
			...
			abs(diff),
			artech_engine.get_desk_link("Item", self.item_code),
			artech_engine.get_desk_link("Warehouse", self.warehouse),
			artech_engine.bold(
				", ".join([f"{dimension}: {values.get('value')}" for dimension, values in dimensions.items()])
			),
			self.posting_date,
			self.posting_time,
			artech_engine.get_desk_link(self.voucher_type, self.voucher_no),
		)
	...
	def on_submit(self):
		...
		if artech_engine.in_test and artech_engine.flags.ignore_serial_batch_bundle_validation:
			return
	...
	def validate_mandatory(self):
		...
		if self.voucher_type != "Stock Reconciliation" and not self.actual_qty:
			artech_engine.throw(_("Actual Qty is mandatory"))

	def validate_serial_batch_no_bundle(self):
		if self.is_cancelled == 1:
			return

		item_detail = artech_engine.get_cached_value(
			"Item",
			self.item_code,
	...
	def throw_error_message(self, message, exception=artech_engine.ValidationError):
		artech_engine.throw(_(message), exception)
	...
	def check_stock_frozen_date(self):
		...
		if stock_settings.stock_frozen_upto:
			if (
				...
				and stock_settings.stock_auth_role not in artech_engine.get_roles()
			):
				artech_engine.throw(
		...
		if stock_frozen_upto_days:
			...
			if older_than_x_days_ago and stock_settings.stock_auth_role not in artech_engine.get_roles():
				artech_engine.throw(
	...
	def validate_batch(self):
		if self.batch_no and self.voucher_type != "Stock Entry":
			...
			expiry_date = artech_engine.db.get_value("Batch", self.batch_no, "expiry_date")
			if expiry_date:
	...
	def validate_and_set_fiscal_year(self):
		if not self.fiscal_year:
			...
			from artech.accounts.utils import validate_fiscal_year
	...
	def block_transactions_against_group_warehouse(self):
		from artech.stock.utils import is_group_warehouse

		is_group_warehouse(self.warehouse)

	def validate_with_last_transaction_posting_time(self):
		authorized_role = artech_engine.get_single_value(
			"Stock Settings", "role_allowed_to_create_edit_back_dated_transactions"
		)
		if authorized_role:
			authorized_users = get_users(authorized_role)
			if authorized_users and artech_engine.session.user not in authorized_users:
				last_transaction_time = artech_engine.db.sql(
					"""
...
def on_doctype_update():
	artech_engine.db.add_index("Stock Ledger Entry", ["voucher_no", "voucher_type"])
	artech_engine.db.add_index("Stock Ledger Entry", ["item_code", "warehouse", "posting_datetime", "creation"])
