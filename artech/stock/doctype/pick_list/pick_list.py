...
from artech_engine.model.mapper import map_child_doc
from artech_engine.query_builder import Case
from artech_engine.query_builder.custom import GROUP_CONCAT
from artech_engine.query_builder.functions import Coalesce, Locate, Replace, Sum
from artech_engine.utils import cint, floor, flt, get_link_to_form
...
from artech.stock.get_item_details import get_company_total_stock, get_conversion_factor
...
class MissingWarehouseValidationError(artech_engine.ValidationError):
	pass


class IncorrectWarehouseValidationError(artech_engine.ValidationError):
	pass
...
class PickList(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.pick_list_item.pick_list_item import PickListItem
	...
	def onload(self) -> None:
		if artech_engine.get_cached_value("Stock Settings", None, "enable_stock_reservation"):
	...
	def validate_warehouses(self):
		for location in self.locations:
			...
			company = artech_engine.get_cached_value("Warehouse", location.warehouse, "company")
			...
			if company != self.company:
				artech_engine.throw(
					_(
						...
						location.idx,
						artech_engine.bold(location.warehouse),
	...
	def check_serial_no_status(self):
		...
		for row in self.get("locations"):
			...
			validated_serial_nos = artech_engine.get_all(
				"Serial No",
	...
	def validate_sales_order_percentage(self):
		...
		for location in self.get("locations"):
			if (
				...
				and artech_engine.db.get_value("Sales Order", location.sales_order, "per_picked", cache=True) == 100
			):
				artech_engine.throw(
	...
	def validate_sales_order(self):
		...
		if so_list:
			for so in so_list:
				so_doc = artech_engine.get_doc("Sales Order", so)
		...
		if batches:
			...
			query = (
				...
				.where(
					...
					& (batch.expiry_date <= artech_engine.utils.nowdate())
	...
	def make_bundle_using_old_serial_batch_fields(self):
		...
		for row in self.locations:
			...
			if not row.use_serial_batch_fields and (row.serial_no or row.batch_no):
				artech_engine.throw(_("Please enable Use Old Serial / Batch Fields to make_bundle"))
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": artech_engine._dict({row.batch_no: row.stock_qty}) if row.batch_no else None,
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.locations:
			if (
				...
				and artech_engine.db.get_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "docstatus")
				== 1
			):
	...
	def remove_serial_and_batch_bundle(self):
		for row in self.locations:
			if row.serial_and_batch_bundle:
				artech_engine.delete_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def update_packed_items_qty(self, packed_items):
		...
		if doc_updates:
			artech_engine.db.bulk_update("Packed Item", doc_updates, update_modified=False)
	...
	def update_sales_order_item_qty(self, so_items):
		...
		if doc_updates:
			artech_engine.db.bulk_update("Sales Order Item", doc_updates, update_modified=False)
	...
	def update_sales_order_picking_status(self) -> None:
		...
		for sales_order in sales_orders:
			artech_engine.get_doc("Sales Order", sales_order, for_update=True).update_picking_status()
	...
	def validate_picked_qty(self, data):
		over_delivery_receipt_allowance = 100 + flt(
			artech_engine.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
		)
	...
	def set_item_locations(self, save: bool = False):
		...
		self.item_location_map = artech_engine._dict()

		from_warehouses = [self.parent_warehouse] if self.parent_warehouse else []

		if self.work_order:
			root_warehouse = artech_engine.db.get_value(
		...
		item_warehouse_dict = artech_engine._dict()
		if self.work_order:
			item_warehouse_list = artech_engine.get_all(
				"Work Order Item",
		...
		updated_locations = artech_engine._dict()
	...
	def aggregate_item_qty(self):
		...
		for item in locations:
			...
			if not item.item_code:
				artech_engine.throw(f"Row #{item.idx}: Item Code is Mandatory")
			if not cint(
				artech_engine.get_cached_value("Item", item.item_code, "is_stock_item")
			) and not artech_engine.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0}):
				continue
	...
	def validate_for_qty(self):
		if self.purpose == "Material Transfer for Manufacture" and (
			...
			artech_engine.throw(_("Qty of Finished Goods Item should be greater than 0."))
	...
	def update_bundle_picked_qty(self):
		...
		for so_row, value in product_bundles.items():
			...
			already_picked = artech_engine.db.get_value(item_table, so_row, "picked_qty", for_update=True)
			artech_engine.db.set_value(
				item_table,
				so_row,
				"picked_qty",
	...
	def get_picked_items_details(self, items):
		picked_items = artech_engine._dict()
		...
		for item_data in items_data:
			...
			if key not in picked_items[item_data.item_code]:
				picked_items[item_data.item_code][key] = artech_engine._dict(
					{
						"picked_qty": 0,
						"serial_no": [],
		...
		for row in self.locations:
			if flt(row.picked_qty) > 0:
				...
				if key not in picked_items[row.item_code]:
					picked_items[row.item_code][key] = artech_engine._dict(
						{
							"picked_qty": 0,
							"serial_no": [],
	...
	def _get_pick_list_items(self, items):
		pi = artech_engine.qb.DocType("Pick List")
		pi_item = artech_engine.qb.DocType("Pick List Item")
		query = (
	...
	def _get_product_bundles(self) -> dict[str, str]:
		...
		for item in self.locations:
			...
			product_bundles[item.sales_order_item] = artech_engine._dict(
				{
					"item_code": artech_engine.db.get_value(
						"Sales Order Item",
	...
	def _get_product_bundle_qty_map(self, bundles) -> dict[str, dict[str, float]]:
		...
		for data in bundles:
			bundle = artech_engine.get_last_doc("Product Bundle", {"new_item_code": data.item_code, "disabled": 0})
	...
	def _compute_picked_qty_for_bundle(self, bundle_row, bundle_items) -> int:
		...
		precision = artech_engine.get_precision("Stock Ledger Entry", "qty_after_transaction")
		possible_bundles = {}
		for item in self.locations:
...
def update_pick_list_status(pick_list):
	if pick_list:
		doc = artech_engine.get_doc("Pick List", pick_list)
...
def validate_item_locations(pick_list):
	if not pick_list.locations:
		artech_engine.throw(_("Add items in the Item Locations table"))
...
def get_items_with_location_and_quantity(item_doc, item_location_map, docstatus):
	...
	precision = artech_engine.get_precision("Pick List Item", "qty")
	while flt(remaining_stock_qty, precision) > 0 and available_locations:
		item_location = available_locations.pop(0)
		item_location = artech_engine._dict(item_location)

		stock_qty = remaining_stock_qty if item_location.qty >= remaining_stock_qty else item_location.qty
		qty = stock_qty / (item_doc.conversion_factor or 1)

		uom_must_be_whole_number = artech_engine.get_cached_value("UOM", item_doc.uom, "must_be_whole_number")
...
def get_available_item_locations(
	...
	has_serial_no = artech_engine.get_cached_value("Item", item_code, "has_serial_no")
	has_batch_no = artech_engine.get_cached_value("Item", item_code, "has_batch_no")
...
def get_available_item_locations_for_serial_and_batched_item(
	...
	if locations:
		sn = artech_engine.qb.DocType("Serial No")
...
def get_available_item_locations_for_serialized_item(
	...
	warehouse_serial_nos_map = artech_engine._dict()
...
def get_available_item_locations_for_batched_item(
	...
	warehouse_wise_batches = artech_engine._dict()
...
def create_delivery_note(source_name: str, target_doc: str | Document | None = None):
	pick_list = artech_engine.get_doc("Pick List", source_name)
	...
	if len(delivery_notes) == 1:
		...
		from artech_engine.utils import comma_and

		doc_list = [get_link_to_form("Delivery Note", p.name) for p in delivery_notes]
		artech_engine.msgprint(_("{0} created").format(comma_and(doc_list)))


def create_dn_wo_so(pick_list, delivery_note=None):
	if not delivery_note:
		delivery_note = artech_engine.new_doc("Delivery Note")
...
def create_dn_for_pick_lists(
	...
	if sales_order_arg:
		...
		if customer_arg:
			sales_orders = artech_engine.get_all(
				"Sales Order",
	...
	if not sales_order_arg and not all(item.sales_order for item in pick_list.locations):
		if isinstance(delivery_note, str):
			delivery_note = artech_engine.get_doc(artech_engine.parse_json(delivery_note))
...
def map_pl_locations(pick_list, item_mapper, delivery_note, sales_order=None):
	for location in pick_list.locations:
		...
		if location.sales_order_item:
			sales_order_item = artech_engine.get_doc("Sales Order Item", location.sales_order_item)
		else:
	...
	delivery_note.company = pick_list.company
	if sales_order:
		delivery_note.customer = artech_engine.get_value("Sales Order", sales_order, "customer")
...
def add_product_bundles_to_delivery_note(
	...
	for so_row, value in product_bundles.items():
		sales_order_item = artech_engine.get_doc("Sales Order Item", so_row)
...
def create_stock_entry(pick_list: str):
	pick_list = artech_engine.get_doc(json.loads(pick_list))
	validate_item_locations(pick_list)

	if stock_entry_exists(pick_list.get("name")):
		return artech_engine.msgprint(_("Stock Entry has been already created against this Pick List"))
...
def get_item_details(
	...
	details = artech_engine.db.get_value("Item", item_code, "stock_uom", as_dict=1)
...
def update_delivery_note_item(source, target, delivery_note):
	cost_center = artech_engine.db.get_value("Project", delivery_note.project, "cost_center")
	if not cost_center:
...
def stock_entry_exists(pick_list_name):
	return artech_engine.db.exists("Stock Entry", {"pick_list": pick_list_name})


def update_stock_entry_based_on_work_order(pick_list, stock_entry):
	work_order = artech_engine.get_doc("Work Order", pick_list.get("work_order"))
	...
	if work_order.bom_no:
		stock_entry.inspection_required = artech_engine.db.get_value("BOM", work_order.bom_no, "inspection_required")

	is_wip_warehouse_group = artech_engine.db.get_value("Warehouse", work_order.wip_warehouse, "is_group")
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = artech_engine.get_value(
...
def get_rejected_warehouses():
	if not hasattr(artech_engine.local, "rejected_warehouses"):
		artech_engine.local.rejected_warehouses = []

	if not artech_engine.local.rejected_warehouses:
		artech_engine.local.rejected_warehouses = artech_engine.get_all(
			"Warehouse", filters={"is_rejected_warehouse": 1}, pluck="name"
		)

	return artech_engine.local.rejected_warehouses


@artech_engine.whitelist()
def get_pick_list_query(doctype: Any, txt: str, searchfield: Any, start: int, page_len: int, filters: dict):
	artech_engine.has_permission("Pick List", throw=True)

	if not filters.get("company"):
		artech_engine.throw(_("Please select a Company"))

	PICK_LIST = artech_engine.qb.DocType("Pick List")
	PICK_LIST_ITEM = artech_engine.qb.DocType("Pick List Item")
	SALES_ORDER = artech_engine.qb.DocType("Sales Order")

	query = (
		artech_engine.qb.from_(PICK_LIST)
		.join(PICK_LIST_ITEM)
	...
	if txt:
		meta = artech_engine.get_meta("Pick List")
