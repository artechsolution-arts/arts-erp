...
from artech_engine import _dict

from artech.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
from artech.selling.doctype.sales_order.sales_order import create_pick_list
...
from artech.stock.doctype.packed_item.test_packed_item import create_product_bundle
from artech.stock.doctype.pick_list.pick_list import create_delivery_note, create_dn_for_pick_lists
...
class TestPickList(ArtechTestSuite):
	def test_pick_list_picks_warehouse_for_each_item(self):
		...
		pick_list = artech_engine.get_doc(
			{
				"doctype": "Pick List",
				"company": "_Test Company",
	...
	def test_pick_list_shows_serial_no_for_serialized_item(self):
		...
		stock_reconciliation = artech_engine.get_doc(
			{
				"doctype": "Stock Reconciliation",
	...
	def test_pick_list_shows_batch_no_for_batched_item(self):
		...
		item = artech_engine.db.exists("Item", {"item_name": "Batched Item"})
		if not item:
			...
			item = artech_engine.get_doc("Item", {"item_name": "Batched Item"})
	...
	def test_pick_list_warehouse_for_batched_item(self):
		...
		batch_company = artech_engine.get_doc(
			{
				"doctype": "Company",
		...
		batch_warehouse = artech_engine.get_doc(
			{
				"doctype": "Warehouse",
		...
		batch_item = artech_engine.db.exists("Item", "Batch Warehouse Item")
		if not batch_item:
			...
			batch_item = artech_engine.get_doc("Item", "Batch Warehouse Item")
	...
	def test_pick_list_warehouse_validation(self):
		...
		temp_warehouse = artech_engine.get_doc(
	...
	def test_pick_list_for_items_from_multiple_sales_orders(self):
		...
		sales_order = artech_engine.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
				"items": [
					{
						"item_code": item_code,
						"qty": 10,
						"delivery_date": artech_engine.utils.today(),
						"warehouse": "_Test Warehouse - _TC",
					}
				],
			}
	...
	def test_multiple_dn_creation(self):
		sales_order_1 = artech_engine.get_doc(
			{
				"doctype": "Sales Order",
				"customer": "_Test Customer",
				"company": "_Test Company",
		...
		sales_order_2 = artech_engine.get_doc(
			{
				"doctype": "Sales Order",
		...
		for dn in artech_engine.get_all(
			"Delivery Note",
			filters={"against_pick_list": pick_list.name, "customer": "_Test Customer"},
			fields=["name"],
		):
			for dn_item in artech_engine.get_doc("Delivery Note", dn.name).get("items"):
		...
		pick_list_1 = artech_engine.get_doc(
			{
				"doctype": "Pick List",
				"company": "_Test Company",
				"purpose": "Delivery",
	...
	def test_picklist_reserved_qty_validation(self):
		...
		if not artech_engine.db.exists("Item", test_stock_item):
			create_item(
	...
	def test_picklist_for_batch_item(self):
		...
		make_stock_entry(
			...
			qty=50,
			basic_rate=100,
			batches=artech_engine._dict({"PICKLT-000001": 30, "PICKLT-000002": 20}),
		)
	...
	def test_pick_list_warehouse_for_work_order(self):
		...
		from artech.manufacturing.doctype.work_order.work_order import create_pick_list, make_work_order
	...
	def test_validate_picked_qty_with_manual_option(self):
		...
		self.assertRaises(artech_engine.ValidationError, pl.save)
	...
	def test_over_allowance_picking(self):
		...
		self.assertRaises(artech_engine.ValidationError, pl_doc.submit)

		artech_engine.db.set_single_value("Stock Settings", "over_picking_allowance", 50)

		pl_doc.reload()
		pl_doc.submit()

		artech_engine.db.set_single_value("Stock Settings", "over_picking_allowance", 0)

	@ArtechTestSuite.change_settings("Selling Settings", {"allow_multiple_items": 1})
	def test_ignore_pricing_rule_in_pick_list(self):
		artech_engine.flags.print_stmt = False
		warehouse = "_Test Warehouse - _TC"
		item = make_item(
			properties={
				"is_stock_item": 1,
		...
		pricing_rule = artech_engine.get_doc(
			{
		...
		pricing_rule.save()
		artech_engine.flags.print_stmt = True
	...
	def test_multiple_pick_lists_delivery_note(self):
		from artech.stock.doctype.pick_list.pick_list import create_dn_for_pick_lists
	...
	def test_packed_item_multiple_times_in_so(self):
		artech_engine.db.delete("Item Price")
		warehouse_1 = "_Test Warehouse - _TC"
		warehouse_2 = "_Test Warehouse 2 - _TC"
		warehouse = "_Test Warehouse - _TC"
		item_1 = make_item(properties={"is_stock_item": 0}).name
	...
	def test_creating_dn_from_so_with_different_addresses(self):
		...
		customer_shipping_address_1 = artech_engine.get_doc(
			{
				"doctype": "Address",
		...
		customer_shipping_address_2 = artech_engine.get_doc(
			{
				"doctype": "Address",
				"address_title": "Customer Shipping Address",
				"address_type": "Shipping",
		...
		pick_list = artech_engine.new_doc("Pick List")
		map_docs(
			"artech.selling.doctype.sales_order.sales_order.create_pick_list",
		...
		create_delivery_note(pick_list.name)
		delivery_notes = artech_engine.get_all(
		...
		for delivery_note in delivery_notes:
			doc = artech_engine.get_doc("Delivery Note", delivery_note)
			if len(doc.items) == 1:
