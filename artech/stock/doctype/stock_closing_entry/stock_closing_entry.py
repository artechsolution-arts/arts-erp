...
from artech_engine.utils import add_days, get_date_str, get_link_to_form, nowtime, parse_json
...
class StockClosingEntry(Document):
	...
	def validate_duplicate(self):
		table = artech_engine.qb.DocType("Stock Closing Entry")
	...
	def remove_stock_closing(self):
		table = artech_engine.qb.DocType("Stock Closing Balance")
		artech_engine.qb.from_(table).delete().where(table.stock_closing_entry == self.name).run()
	...
	def create_stock_closing_balance_entries(self):
		...
		for key in entries:
			...
			new_doc = artech_engine.new_doc("Stock Closing Balance")
			new_doc.update(row)
	...
	def get_prepared_data(self):
		if attachments := get_attachments(self.doctype, self.name):
			...
			attached_file = artech_engine.get_doc("File", attachment.name)
...
def prepare_closing_stock_balance(name):
	doc = artech_engine.get_doc("Stock Closing Entry", name)
...
class StockClosing:
	...
	def get_stock_closing_entries(self):
		...
		closing_stock = artech_engine._dict()
	...
	def get_entries(self, doctype, fields, filters, **kwargs):
		...
		query = artech_engine.qb.from_(table).select(*fields).orderby(table.posting_datetime)
	...
	def get_last_stock_closing_entry(self):
		...
		return entries[0] if entries else artech_engine._dict()
	...
	def get_stock_closing_balance(self, kwargs, for_batch=False):
		...
		table = artech_engine.qb.DocType("Stock Closing Balance")
		query = artech_engine.qb.from_(table).select("*").where(table.stock_closing_entry == stock_closing_entry)
