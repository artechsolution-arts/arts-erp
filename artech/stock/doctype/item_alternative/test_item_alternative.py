...
class TestItemAlternative(ArtechTestSuite):
	def setUp(self):
		super().setUp()
		make_items()
	...
	def test_alternative_item_for_subcontract_rm(self):
		...
		reserved_qty_for_sub_contract = artech_engine.db.get_value(
			"Bin",
		...
		doc = artech_engine.get_doc("Stock Entry", se.name)
		...
		doc.save()
		doc.submit()
		after_transfer_reserved_qty_for_sub_contract = artech_engine.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_sub_contract",
		)
		...
		scr = artech_engine.get_doc("Subcontracting Receipt", scr.name)
		status = False
	...
	def test_alternative_item_for_production_rm(self):
		...
		reserved_qty_for_production = artech_engine.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)

		ste = artech_engine.get_doc(make_stock_entry(pro_order.name, "Material Transfer for Manufacture", 5))
		ste.insert()
		...
		ste.submit()
		reserved_qty_for_production_after_transfer = artech_engine.db.get_value(
			"Bin",
			{"item_code": "Test FG A RW 1", "warehouse": "_Test Warehouse - _TC"},
			"reserved_qty_for_production",
		)
		...
		ste1 = artech_engine.get_doc(make_stock_entry(pro_order.name, "Manufacture", 5))
...
def make_items():
	...
	if artech_engine.db.exists("Item", "Test FG A RW 1"):
		doc = artech_engine.get_doc("Item", "Test FG A RW 1")
		doc.allow_alternative_item = 1
		doc.save()

	if artech_engine.db.exists("Item", "Test Finished Goods - A"):
		doc = artech_engine.get_doc("Item", "Test Finished Goods - A")
		doc.is_sub_contracted_item = 1
		doc.save()

	if not artech_engine.db.get_value("BOM", {"item": "Test Finished Goods - A", "docstatus": 1}):
		make_bom(item="Test Finished Goods - A", raw_materials=["Test FG A RW 1", "Test FG A RW 2"])

	if not artech_engine.db.get_value("Warehouse", {"warehouse_name": "Test Supplier Warehouse"}):
