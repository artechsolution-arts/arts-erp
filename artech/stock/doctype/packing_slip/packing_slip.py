...
class PackingSlip(StatusUpdater):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.packing_slip_item.packing_slip_item import PackingSlipItem
	...
	def validate_delivery_note(self):
		...
		if cint(artech_engine.db.get_value("Delivery Note", self.delivery_note, "docstatus")) != 0:
			artech_engine.throw(
	...
	def validate_case_nos(self):
		...
		if cint(self.from_case_no) <= 0:
			artech_engine.throw(_("The 'From Package No.' field must neither be empty nor it's value less than 1."))
		elif not self.to_case_no:
			self.to_case_no = self.from_case_no
		elif cint(self.to_case_no) < cint(self.from_case_no):
			artech_engine.throw(_("'To Package No.' cannot be less than 'From Package No.'"))
		else:
			ps = artech_engine.qb.DocType("Packing Slip")
			res = (
	...
	def validate_items(self):
		for item in self.items:
			if item.qty <= 0:
				artech_engine.throw(_("Row {0}: Qty must be greater than 0.").format(item.idx))
			...
			DocType = artech_engine.qb.DocType("Delivery Note Item" if item.dn_detail else "Packed Item")
			remaining_qty = artech_engine.db.get_value(
			...
			if remaining_qty is None:
				...
				artech_engine.throw(
					_("Row {0}: Qty cannot be greater than {1} for the Item {2}.").format(
						item.idx, artech_engine.bold(remaining_qty), artech_engine.bold(item.item_code)
					)
				)
	...
	def set_missing_values(self):
		...
		for item in self.items:
			weight_per_unit, weight_uom = artech_engine.db.get_value(
