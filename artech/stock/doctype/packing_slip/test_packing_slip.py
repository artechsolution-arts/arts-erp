...
from artech.stock.doctype.delivery_note.delivery_note import make_packing_slip
...
class TestPackingSlip(ArtechTestSuite):
	def test_packing_slip(self):
		...
		for item in dn.items:
			if not artech_engine.db.exists("Product Bundle", {"new_item_code": item.item_code}):
		...
		self.assertRaises(artech_engine.exceptions.ValidationError, ps3.save)
		...
		dn.load_from_db()
		self.assertRaises(artech_engine.exceptions.ValidationError, dn.submit)
