...
from artech_engine.utils import add_days, flt, now_datetime, nowdate

import artech
from artech.stock.doctype.delivery_trip.delivery_trip import (
...
class TestDeliveryTrip(ArtechTestSuite):
	def setUp(self):
		super().setUp()
	...
	def test_delivery_trip_notify_customers(self):
		...
		outgoing = artech_engine.get_doc(
			{
...
def create_address(driver):
	if not artech_engine.db.exists("Address", {"address_title": "_Test Address for Driver"}):
		...
		artech_engine.db.set_value("Driver", driver.name, "address", address.name)

		return address

	return artech_engine.get_doc("Address", {"address_title": "_Test Address for Driver"})


def create_driver():
	if not artech_engine.db.exists("Driver", {"full_name": "Newton Scmander"}):
		driver = artech_engine.get_doc(
			{
				"doctype": "Driver",
	...
	return artech_engine.get_doc("Driver", {"full_name": "Newton Scmander"})


def create_delivery_notification():
	if not artech_engine.db.exists("Email Template", "Delivery Notification"):
		dispatch_template = artech_engine.get_doc(
			{
	...
	delivery_settings = artech_engine.get_single("Delivery Settings")
	delivery_settings.dispatch_template = "Delivery Notification"
	delivery_settings.save()


def create_vehicle():
	if not artech_engine.db.exists("Vehicle", "JB 007"):
...
def create_delivery_trip(driver, address, contact=None, company=None):
	...
	delivery_trip = artech_engine.get_doc(
		{
			"doctype": "Delivery Trip",
			"company": company or artech.get_default_company(),
