...
from artech_engine.utils import cint, get_datetime, get_link_to_form
...
class DeliveryTrip(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.delivery_stop.delivery_stop import DeliveryStop
	...
	def __init__(self, *args, **kwargs):
		...
		self.default_distance_uom = (
			artech_engine.db.get_single_value("Global Defaults", "default_distance_unit") or "Meter"
		)
		self.uom_conversion_factor = artech_engine.db.get_value(
	...
	def validate(self):
		if self._action == "submit" and not self.driver:
			artech_engine.throw(_("A driver must be set to submit."))
	...
	def validate_stop_addresses(self):
		for stop in self.delivery_stops:
			if not stop.customer_address:
				stop.customer_address = get_address_display(artech_engine.get_doc("Address", stop.address).as_dict())

	def validate_delivery_note_not_draft(self):
		delivery_notes = list(set(stop.delivery_note for stop in self.delivery_stops if stop.delivery_note))
		draft_delivery_notes = artech_engine.get_all(
			"Delivery Note",
	...
	def update_delivery_notes(self, delete=False):
		...
		for delivery_note in delivery_notes:
			note_doc = artech_engine.get_doc("Delivery Note", delivery_note)
		...
		artech_engine.msgprint(_("Delivery Notes {0} updated").format(", ".join(delivery_notes_updated)))
		...
		for route in route_list:
			...
			if directions:
				...
				for leg in legs:
					...
					stop_delay = artech_engine.db.get_single_value("Delivery Settings", "stop_delay")
	...
	def form_route_list(self, optimize):
		...
		if not self.driver_address:
			artech_engine.throw(_("Cannot Calculate Arrival Time as Driver Address is Missing."))

		home_address = get_address_display(artech_engine.get_doc("Address", self.driver_address).as_dict())
	...
	def get_directions(self, route, optimize):
		...
		"""
		if not artech_engine.db.get_single_value("Google Settings", "api_key"):
			artech_engine.throw(_("Enter API key in Google Settings."))

		import googlemaps

		try:
			maps_client = googlemaps.Client(key=artech_engine.db.get_single_value("Google Settings", "api_key"))
		except Exception as e:
			artech_engine.throw(e)
...
def get_default_contact(out, name):
	contact_persons = artech_engine.db.sql(
		"""
			SELECT parent,
...
def get_default_address(out, name):
	shipping_addresses = artech_engine.db.sql(
		"""
			SELECT parent,
...
def get_contact_display(contact: str):
	contact_info = artech_engine.db.get_value(
...
def notify_customers(delivery_trip: str):
	delivery_trip = artech_engine.get_doc("Delivery Trip", delivery_trip)

	context = delivery_trip.as_dict()

	if delivery_trip.driver:
		context.update(artech_engine.db.get_value("Driver", delivery_trip.driver, "cell_number", as_dict=1))
	...
	for stop in delivery_trip.delivery_stops:
		...
		if contact_info and contact_info.email_id:
			...
			dispatch_template_name = artech_engine.db.get_single_value("Delivery Settings", "dispatch_template")
			dispatch_template = artech_engine.get_doc("Email Template", dispatch_template_name)

			artech_engine.sendmail(
				recipients=contact_info.email_id,
				subject=dispatch_template.subject,
				message=artech_engine.render_template(dispatch_template.response, context),
	...
	if email_recipients:
		artech_engine.msgprint(_("Email sent to {0}").format(", ".join(email_recipients)))
		delivery_trip.db_set("email_notification_sent", 1)
	else:
		artech_engine.msgprint(_("No contacts with email IDs found."))


def get_attachments(delivery_stop):
	if not (
		artech_engine.db.get_single_value("Delivery Settings", "send_with_attachment")
		and delivery_stop.delivery_note
	):
		return []

	dispatch_attachment = artech_engine.db.get_single_value("Delivery Settings", "dispatch_attachment")
	attachments = artech_engine.attach_print(
		"Delivery Note",
...
def get_driver_email(driver: str):
	employee = artech_engine.db.get_value("Driver", driver, "employee")
	email = artech_engine.db.get_value("Employee", employee, "prefered_email")
	return {"email": email}
