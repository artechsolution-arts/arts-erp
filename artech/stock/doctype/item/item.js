artech_engine.provide("artech.item");

const SALES_DOCTYPES = ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice"];
const PURCHASE_DOCTYPES = ["Purchase Order", "Purchase Receipt", "Purchase Invoice"];

artech_engine.ui.form.on("Item", {
	...
	toggle_has_serial_batch_fields(frm) {
		...
		if (hide_fields) {
			...
			wrapper.find(".go-to-settings").on("click", function () {
				artech_engine.set_route("Form", "Stock Settings");
			});
		}
	},
	...
	refresh: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frm.add_custom_button(
				...
				function () {
					...
					artech_engine.set_route("query-report", "Stock Balance");
				},
				__("View")
			);
			frm.add_custom_button(
			...
			frm.add_custom_button(
				...
				function () {
					...
					artech_engine.set_route("query-report", "Stock Projected Qty");
				},
				__("View")
			);
			...
			frm.toggle_display(
				["opening_stock"],
				artech_engine.model.can_create("Stock Entry") && artech_engine.model.can_write("Stock Entry")
			);
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.add_custom_button(
				__("Show Variants"),
				function () {
					artech_engine.set_route("List", "Item", { variant_of: frm.doc.name });
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.set_route("Form", "Item Variant Settings");
				},
				__("View")
			);
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.set_route("query-report", "Item Variant Details", { item: frm.doc.name });
				},
				__("View")
			);
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						artech.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						artech.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						artech.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		if (artech_engine.defaults.get_default("item_naming_by") != "Naming Series" || frm.doc.variant_of) {
			frm.toggle_display("naming_series", false);
		} else {
			artech.toggle_naming_series();
		}

		artech.item.edit_prices_button(frm);
		artech.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			artech.item.make_dashboard(frm);
		}

		frm.add_custom_button(__("Duplicate"), function () {
			var new_item = artech_engine.model.copy_doc(frm.doc);
			...
			artech_engine.set_route("Form", "Item", new_item.name);
		});
		...
		frm.toggle_display(["standard_rate"], artech_engine.model.can_create("Item Price"));
	},

	validate: function (frm) {
		artech.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		artech_engine.call({
			method: "artech.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
	...
	page_name: artech_engine.utils.warn_page_name_change,
...
artech_engine.ui.form.on("Item Reorder", {
	...
	warehouse_group(frm, cdt, cdn) {
		...
		if (!row.warehouse_group) {
			artech_engine.throw(__("Please select the Warehouse first"));
		}
	},
});

artech_engine.ui.form.on("Item Customer Detail", {
	customer_items_add: function (frm, cdt, cdn) {
		artech_engine.model.set_value(cdt, cdn, "customer_group", "");
	},
	...
	customer_group: function (frm, cdt, cdn) {
		if (set_customer_group(frm, cdt, cdn)) {
			artech_engine.msgprint(__("Changing Customer Group for the selected Customer is not allowed."));
		}
	},
});
...
var set_customer_group = function (frm, cdt, cdn) {
	...
	artech_engine.model.with_doc("Customer", row.customer_name, function () {
		var customer = artech_engine.model.get_doc("Customer", row.customer_name);
	...
	let banner_html = `
		<div class="custom-serial-batch-banner ${hiddenClass}">
			<div class="banner-content">
				<span class="banner-icon">${artech_engine.utils.icon("solid-warning", "lg", "", "padding-bottom:2px")}</span>
...
$.extend(artech.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict["item_defaults"].grid.get_field("income_account").get_query = function (
			...
			const row = locals[cdt][cdn];
			return {
				query: "artech.controllers.queries.get_income_account",
				filters: { company: row.company },
			};
		};
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "artech.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			artech_engine.require("item-dashboard.bundle.js", function () {
				const section = frm.dashboard.add_section("", __("Stock Levels"));
				artech.item.item_dashboard = new artech.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "artech.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				artech.item.item_dashboard.refresh();
			});
		}
	},
	...
	edit_prices_button: function (frm) {
		frm.add_custom_button(
			__("Add / Edit Prices"),
			function () {
				artech_engine.set_route("List", "Item Price", { item_code: frm.doc.name });
			},
			__("Actions")
		);
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			artech_engine.call({
				method: "artech.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_fields_from_attribute_values(attr_dict) {
			...
			att_key.forEach((name, i) => {
				...
				fields.push({
					fieldtype: "Data",
					placeholder: "Search",
					fieldname: `search_${artech_engine.scrub(name)}`,
					onchange: function (e) {
		...
		function make_and_show_dialog(fields) {
			me.multiple_variant_dialog = new artech_engine.ui.Dialog({
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				artech_engine.call({
					method: "artech.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			$($(me.multiple_variant_dialog.$wrapper.find(".form-column")).find(".artech_engine-control")).css(
				"margin-bottom",
				"0px"
			);
			...
			if (!row.disabled) {
				if (row.numeric_values) {
					...
					desc = __("Min Value: {0}, Max Value: {1}, in Increments of: {2}", [
						artech_engine.format(row.from_range, { fieldtype: "Float" }, { always_show_decimals: true }),
						artech_engine.format(row.to_range, { fieldtype: "Float" }, { always_show_decimals: true }),
						artech_engine.format(row.increment, { fieldtype: "Float" }, { always_show_decimals: true }),
					]);
				} else {
					fieldtype = "Data";
					desc = "";
			...
			artech_engine.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						artech_engine.msgprint_dialog = artech_engine.msgprint(
						...
						artech_engine.msgprint_dialog.hide_on_page_refresh = true;
						artech_engine.msgprint_dialog.$wrapper.find(".variant-click").on("click", function () {
							d.hide();
						});
					} else {
						d.hide();
						artech_engine.call({
							method: "artech.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					artech_engine.call({
						method: "artech.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
artech_engine.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.uom) {
			artech_engine.call({
				method: "artech.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
				...
				callback: function (r) {
					if (!r.exc && r.message) {
						artech_engine.model.set_value(cdt, cdn, "conversion_factor", r.message);
					}
				},
			});
		}
...
artech_engine.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, Artech will make a stock ledger entry for each transaction of this item."
		),
	},
	{
...
function open_form(frm, doctype, child_doctype, parentfield) {
	artech_engine.model.with_doctype(doctype, () => {
		let new_doc = artech_engine.model.get_new_doc(doctype);

		let new_child_doc = artech_engine.model.add_child(new_doc, child_doctype, parentfield);
		...
		artech_engine.run_serially([
			...
			() => {
				artech_engine.flags.ignore_company_party_validation = true;
				artech_engine.model.trigger("item_code", frm.doc.name, new_child_doc);
			},
		]);
	});
}
