...
from artech_engine.model.naming import NamingSeries
from artech_engine.query_builder import Interval
from artech_engine.query_builder.functions import Count, CurDate, UnixTimestamp
...
from artech_engine.utils.html_utils import clean_html
from pypika import Order

import artech
from artech.controllers.item_variant import (
...
class DuplicateReorderRows(artech_engine.ValidationError):
	pass


class StockExistsForTemplate(artech_engine.ValidationError):
	pass


class InvalidBarcode(artech_engine.ValidationError):
	pass


class DataValidationError(artech_engine.ValidationError):
	pass
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from artech.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from artech.stock.doctype.item_default.item_default import ItemDefault
		from artech.stock.doctype.item_reorder.item_reorder import ItemReorder
		from artech.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from artech.stock.doctype.item_tax.item_tax import ItemTax
		from artech.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from artech.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
	...
	def autoname(self):
		if artech_engine.db.get_default("item_naming_by") == "Naming Series":
			if self.variant_of:
				if not self.item_code:
					template_item_name = artech_engine.db.get_value("Item", self.variant_of, "item_name")
	...
	def after_insert(self):
		...
		if self.standard_rate:
			for default in self.item_defaults or [artech_engine._dict()]:
	...
	def validate(self):
		...
		if not self.is_new():
			self.old_item_group = artech_engine.db.get_value(self.doctype, self.name, "item_group")
	...
	def validate_description(self):
		...
		if (
			cint(artech_engine.get_single_value("Stock Settings", "clean_description_html"))
			...
			if (
				...
				artech_engine.msgprint(
					_(
						...
						artech_engine.get_meta("Stock Settings").get_label("clean_description_html"),
	...
	def validate_customer_provided_part(self):
		if self.is_customer_provided_item:
			if self.is_purchase_item:
				artech_engine.throw(_('"Customer Provided Item" cannot be Purchase Item also'))
			if self.valuation_rate:
				artech_engine.throw(_('"Customer Provided Item" cannot have Valuation Rate'))
			self.default_material_request_type = "Customer Provided"

	def add_price(self, price_list=None):
		"""Add a new price"""
		if not price_list:
			price_list = artech_engine.get_single_value(
				"Selling Settings", "selling_price_list"
			) or artech_engine.db.get_value("Price List", _("Standard Selling"))
		if price_list:
			item_price = artech_engine.get_doc(
				{
					"doctype": "Item Price",
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": artech.get_default_currency(),
	...
	def set_opening_stock(self):
		...
		if not self.valuation_rate and not self.standard_rate and not self.is_customer_provided_item:
			artech_engine.throw(_("Valuation Rate is mandatory if Opening Stock entered"))

		from artech.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry

		# default warehouse, or Stores
		for default in self.item_defaults or [
			artech_engine._dict({"company": artech_engine.defaults.get_defaults().company})
		]:
			default_warehouse = default.default_warehouse or artech_engine.get_single_value(
				"Stock Settings", "default_warehouse"
			)
			if default_warehouse:
				warehouse_company = artech_engine.db.get_value("Warehouse", default_warehouse, "company")

			if not default_warehouse or warehouse_company != default.company:
				default_warehouse = artech_engine.db.get_value(
	...
	def validate_fixed_asset(self):
		if self.is_fixed_asset:
			if self.is_stock_item:
				artech_engine.throw(_("Fixed Asset Item must be a non-stock item."))

			if not self.asset_category:
				artech_engine.throw(_("Asset Category is mandatory for Fixed Asset item"))

			if self.stock_ledger_created():
				artech_engine.throw(_("Cannot be a fixed asset item as Stock Ledger is created."))
	...
	def validate_retain_sample(self):
		if self.retain_sample and not artech_engine.get_single_value("Stock Settings", "sample_retention_warehouse"):
			artech_engine.throw(_("Please select Sample Retention Warehouse in Stock Settings first"))
	...
	def update_template_tables(self):
		template = artech_engine.get_cached_doc("Item", self.variant_of)
	...
	def validate_item_type(self):
		if self.has_serial_no == 1 and self.is_stock_item == 0 and not self.is_fixed_asset:
			artech_engine.throw(_("'Has Serial No' can not be 'Yes' for non-stock item"))
	...
	def validate_naming_series(self):
		for field in ["serial_no_series", "batch_number_series"]:
			...
			if series and "#" in series and "." not in series:
				artech_engine.throw(
					_("Invalid naming series (. missing) for {0}").format(
						artech_engine.bold(self.meta.get_field(field).label)
					)
				)
			...
			if self.is_new() and series:
				...
				doctype = artech_engine.qb.DocType("Series")

				query = artech_engine.qb.from_(doctype).select(doctype.name).where(doctype.name.like(f"{prefix}%"))
				...
				if prefix_exists:
					artech_engine.msgprint(
						...
						).format(bold(artech_engine.unscrub(field)), bold(prefix)),
	...
	def check_for_active_boms(self):
		if self.default_bom:
			bom_item = artech_engine.db.get_value("BOM", self.default_bom, "item")
	...
	def validate_barcode(self):
		...
		if len(self.barcodes) > 0:
			for item_barcode in self.barcodes:
				options = artech_engine.get_meta("Item Barcode").get_options("barcode_type").split("\n")
				if item_barcode.barcode:
					duplicate = artech_engine.db.sql(
					...
					if item_barcode.barcode_type:
						barcode_type = convert_artech_to_barcodenumber(
	...
	def validate_warehouse_for_reorder(self):
		...
		_warehouse_before_save = artech_engine._dict()
		...
		for d in self.get("reorder_levels"):
			...
			if d.warehouse_reorder_level and not d.warehouse_reorder_qty:
				artech_engine.throw(_("Row #{0}: Please set reorder quantity").format(d.idx))
	...
	def has_submitted_assets(self):
		return bool(artech_engine.db.exists("Asset", {"item_code": self.name, "docstatus": 1}))
	...
	def on_trash(self):
		artech_engine.db.sql("""delete from tabBin where item_code=%s""", self.name)
		artech_engine.db.sql("delete from `tabItem Price` where item_code=%s", self.name)
		for variant_of in artech_engine.get_all("Item", filters={"variant_of": self.name}):
			artech_engine.delete_doc("Item", variant_of.name)

	def before_rename(self, old_name, new_name, merge=False):
		if self.item_name == old_name:
			artech_engine.db.set_value("Item", old_name, "item_name", new_name)
	...
	def after_rename(self, old_name, new_name, merge):
		...
		artech_engine.db.set_value("Item", new_name, "item_code", new_name)
	...
	def delete_old_bins(self, old_name):
		artech_engine.db.delete("Bin", {"item_code": old_name})

	def validate_duplicate_item_in_stock_reconciliation(self, old_name, new_name):
		records = artech_engine.db.sql(
		...
		msg = _("The items {0} and {1} are present in the following {2} :").format(
			artech_engine.bold(old_name), artech_engine.bold(new_name), document
		)
		...
		).format(artech_engine.bold(old_name))

		artech_engine.throw(_(msg), title=_("Cannot Merge"), exc=DataValidationError)

	def validate_properties_before_merge(self, new_name):
		# Validate properties before merging
		if not artech_engine.db.exists("Item", new_name):
			artech_engine.throw(_("Item {0} does not exist").format(new_name))

		field_list = ["stock_uom", "is_stock_item", "has_serial_no", "has_batch_no"]
		new_properties = [cstr(d) for d in artech_engine.db.get_value("Item", new_name, field_list)]
		...
		if new_properties != [cstr(self.get(field)) for field in field_list]:
			...
			artech_engine.throw(msg, title=_("Cannot Merge"), exc=DataValidationError)

	def validate_duplicate_product_bundles_before_merge(self, old_name, new_name):
		"Block merge if both old and new items have product bundles."
		old_bundle = artech_engine.get_value("Product Bundle", filters={"new_item_code": old_name, "disabled": 0})
		new_bundle = artech_engine.get_value("Product Bundle", filters={"new_item_code": new_name, "disabled": 0})

		if old_bundle and new_bundle:
			bundle_link = get_link_to_form("Product Bundle", old_bundle)
			old_name, new_name = artech_engine.bold(old_name), artech_engine.bold(new_name)
	...
	def set_last_purchase_rate(self, new_name):
		...
		artech_engine.db.set_value("Item", new_name, "last_purchase_rate", last_purchase_rate)

	def recalculate_bin_qty(self, new_name):
		from artech.stock.stock_balance import repost_stock

		existing_allow_negative_stock = artech_engine.get_single_value("Stock Settings", "allow_negative_stock")
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		repost_stock_for_warehouses = artech_engine.get_all(
			"Stock Ledger Entry",
			"warehouse",
		...
		artech_engine.db.delete("Bin", {"item_code": new_name})

		for warehouse in repost_stock_for_warehouses:
			repost_stock(new_name, warehouse)

		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", existing_allow_negative_stock)
	...
	def validate_item_defaults(self):
		...
		if len(companies) != len(self.item_defaults):
			artech_engine.throw(_("Cannot set multiple Item Defaults for a company."))
	...
	def update_defaults_from_item_group(self):
		...
		item_group = artech_engine.get_cached_doc("Item Group", self.item_group)
		if item_group.item_group_defaults:
			...
			defaults = artech_engine.defaults.get_defaults() or {}
			...
			if (
				...
				and defaults.company
				== artech_engine.get_cached_value("Warehouse", defaults.default_warehouse, "company")
			):
				self.append(
					"item_defaults",
	...
	def update_variants(self):
		if self.flags.dont_update_variants or artech_engine.db.get_single_value(
			"Item Variant Settings", "do_not_update_variants"
		):
			return
		if self.has_variants:
			variants = artech_engine.db.get_all("Item", fields=["item_code"], filters={"variant_of": self.name})
			if variants:
				if len(variants) <= 30:
					update_variants(variants, self, publish_progress=False)
					artech_engine.msgprint(_("Item Variants updated"))
				else:
					artech_engine.enqueue(
						"artech.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=artech_engine.in_test,
						timeout=600,
	...
	def validate_has_variants(self):
		...
		if not self.has_variants and self.has_value_changed("has_variants"):
			if artech_engine.db.exists("Item", {"variant_of": self.name}):
				artech_engine.throw(_("Item has variants."))
	...
	def validate_attributes_in_variants(self):
		...
		items = [item["name"] for item in artech_engine.get_all("Item", {"variant_of": self.name})]
		...
		item_attributes = artech_engine.get_all(
		...
		for docname, attr_list in not_included.items():
			link = f"<a href='/app/item/{docname}'>{artech_engine.bold(docname)}</a>"
		...
		artech_engine.throw(message, title=_("Variant Attribute Error"), is_minimizable=True, wide=True)
	...
	def validate_variant_based_on_change(self):
		if not self.is_new() and (
			self.variant_of or (self.has_variants and artech_engine.get_all("Item", {"variant_of": self.name}))
		):
			if self.variant_based_on != artech_engine.db.get_value("Item", self.name, "variant_based_on"):
				artech_engine.throw(_("Variant Based On cannot be changed"))

	def validate_uom(self):
		if not self.is_new():
			check_stock_uom_with_bin(self.name, self.stock_uom)
		if self.has_variants:
			for d in artech_engine.db.get_all("Item", filters={"variant_of": self.name}):
				check_stock_uom_with_bin(d.name, self.stock_uom)
		if self.variant_of:
			allow_different_uom = artech_engine.get_cached_value(
				"Item Variant Settings", "Item Variant Settings", "allow_different_uom"
			)
			if not allow_different_uom:
				template_uom = artech_engine.db.get_value("Item", self.variant_of, "stock_uom")
	...
	def validate_attributes(self):
		...
		if self.variant_based_on == "Item Attribute":
			...
			if not self.attributes:
				artech_engine.throw(_("Attribute table is mandatory"))
	...
	def cant_change(self):
		...
		values = artech_engine.db.get_value("Item", self.name, restricted_fields, as_dict=True)
		if not values:
			return

		if not values.get("valuation_method") and self.get("valuation_method"):
			values["valuation_method"] = (
				artech_engine.get_single_value("Stock Settings", "valuation_method") or "FIFO"
			)
		...
		if linked_doc := self._get_linked_submitted_documents(changed_fields):
			changed_field_labels = [artech_engine.bold(_(self.meta.get_label(f))) for f in changed_fields]
			msg = _(
			...
			if linked_doc and isinstance(linked_doc, dict):
				...
				msg += _("Example of a linked document: {0}").format(
					artech_engine.get_desk_link(linked_doc.doctype, linked_doc.docname)
				)

			artech_engine.throw(msg, title=_("Linked with submitted documents"))
	...
	def _get_linked_submitted_documents(self, changed_fields: list[str]) -> dict[str, str] | None:
		...
		for doctype in linked_doctypes:
			...
			if doctype in ("Product Bundle", "BOM"):
				...
				if linked_doc := artech_engine.db.get_value(doctype, filters, fieldname, as_dict=True):
				...
				if linked_doc := artech_engine.db.get_value(
					"Stock Ledger Entry",
			...
			elif linked_doc := artech_engine.db.get_value(
				doctype,
				filters,
	...
	def validate_auto_reorder_enabled_in_stock_settings(self):
		if self.reorder_levels:
			enabled = artech_engine.get_single_value("Stock Settings", "auto_indent")
			if not enabled:
				artech_engine.msgprint(
...
def convert_artech_to_barcodenumber(artech_number, barcode):
	if artech_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if artech_number in convert:
		return convert[artech_number]
...
def validate_end_of_life(item_code, end_of_life=None, disabled=None):
	if (not end_of_life) or (disabled is None):
		end_of_life, disabled = artech_engine.db.get_value("Item", item_code, ["end_of_life", "disabled"])
	...
	if disabled:
		artech_engine.throw(_("Item {0} is disabled").format(item_code))


def validate_is_stock_item(item_code, is_stock_item=None):
	if not is_stock_item:
		is_stock_item = artech_engine.db.get_value("Item", item_code, "is_stock_item")

	if is_stock_item != 1:
		artech_engine.throw(_("Item {0} is not a stock Item").format(item_code))


def validate_cancelled_item(item_code, docstatus=None):
	if docstatus is None:
		docstatus = artech_engine.db.get_value("Item", item_code, "docstatus")

	if docstatus == 2:
		artech_engine.throw(_("Item {0} is cancelled").format(item_code))
...
def get_purchase_voucher_details(doctype, item_code, document_name=None):
	parent_doc = artech_engine.qb.DocType(doctype)
	child_doc = artech_engine.qb.DocType(doctype + " Item")

	query = (
		artech_engine.qb.from_(parent_doc)
...
def check_stock_uom_with_bin(item, stock_uom):
	if stock_uom == artech_engine.db.get_value("Item", item, "stock_uom"):
		return

	ref_uom = artech_engine.db.get_value("Stock Ledger Entry", {"item_code": item}, "stock_uom")
	...
	artech_engine.db.sql("""update `tabBin` set stock_uom=%s where item_code=%s""", (stock_uom, item))


def get_item_defaults(item_code, company):
	item = artech_engine.get_cached_doc("Item", item_code)
	...
	for d in item.item_defaults:
		if d.company == company:
			if not d.get(fieldname):
				artech_engine.db.set_value(d.doctype, d.name, fieldname, value)
			return
...
def get_item_details(item_code: str, company: str | None = None):
	out = artech_engine._dict()
	if company:
		out = get_item_defaults(item_code, company) or artech_engine._dict()

	doc = artech_engine.get_cached_doc("Item", item_code)
...
def get_uom_conv_factor(uom: str | None, stock_uom: str | None):
	...
	exact_match = artech_engine.db.get_value(
		"UOM Conversion Factor", {"to_uom": to_uom, "from_uom": from_uom}, ["value"], as_dict=1
	)
	if exact_match:
		return exact_match.value

	inverse_match = artech_engine.db.get_value(
	...
	intermediate_match = artech_engine.db.sql(
		"""
...
def get_item_attribute(parent: str, attribute_value: str = ""):
	...
	if not artech_engine.has_permission("Item"):
		artech_engine.throw(_("No Permission"))
...
def update_variants(variants, template, publish_progress=True):
	...
	for count, d in enumerate(variants, start=1):
		variant = artech_engine.get_doc("Item", d)
		copy_attributes_to_variant(template, variant)
		variant.save()
		if publish_progress:
			artech_engine.publish_progress(count / total * 100, title=_("Updating Variants..."))
	...
	for item_default in item_defaults:
		for doctype, field in [
			...
			if item_default.get(field):
				company = artech_engine.db.get_value(doctype, item_default.get(field), "company", cache=True)
				if company and company != item_default.company:
					artech_engine.throw(
						_("Row #{}: {} {} doesn't belong to Company {}. Please select valid {}.").format(
							item_default.idx,
							doctype,
							artech_engine.bold(item_default.get(field)),
							artech_engine.bold(item_default.company),
							artech_engine.bold(artech_engine.unscrub(field)),
						),
...
def get_asset_naming_series():
	from artech.assets.doctype.asset.asset import get_asset_naming_series
...
def get_child_warehouses(warehouse):
	from artech.stock.doctype.warehouse.warehouse import get_child_warehouses
