...
from artech.stock.doctype.item.item import (
	DataValidationError,
	InvalidBarcode,
...
def make_item(item_code=None, properties=None, uoms=None, barcode=None):
	if not item_code:
		item_code = artech_engine.generate_hash(length=16)

	if artech_engine.db.exists("Item", item_code):
		return artech_engine.get_doc("Item", item_code)

	item = artech_engine.get_doc(
		{
			"doctype": "Item",
			"item_code": item_code,
			"item_name": item_code,
...
class TestItem(ArtechTestSuite):
	def setUp(self):
		super().setUp()
		artech_engine.flags.attribute_values = None

	def get_item(self, idx):
		item_code = self.globalTestRecords["Item"][idx].get("item_code")
		if not artech_engine.db.exists("Item", item_code):
			item = artech_engine.copy_doc(self.globalTestRecords["Item"][idx])
			item.insert()
		else:
			item = artech_engine.get_doc("Item", item_code)
		return item

	def make_bin(self, records):
		for x in records:
			x = artech_engine._dict(x)
			bin = qb.DocType("Bin")
			filters = {
	...
	def test_get_item_details(self):
		...
		currency = artech_engine.get_cached_value("Company", company, "default_currency")
	...
	def test_get_asset_item_details(self):
		from artech.assets.doctype.asset.test_asset import create_fixed_asset_item

		artech_engine.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", 0)
		...
		artech_engine.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", "1")
		details = get_item_details(
			ItemDetailsCtx(
				{
					"item_code": "Macbook Pro",
	...
	def test_item_defaults(self):
		artech_engine.delete_doc_if_exists("Item", "Test Item With Defaults", force=1)
		make_item(
	...
	def test_item_default_validations(self):
		with self.assertRaises(artech_engine.ValidationError) as ve:
			make_item(
				"Bad Item defaults",
				{
					"item_group": "_Test Item Group",
	...
	def test_item_attribute_change_after_variant(self):
		artech_engine.delete_doc_if_exists("Item", "_Test Variant Item-L", force=1)

		variant = create_variant("_Test Variant Item", {"Test Size": "Large"})
		variant.save()

		attribute = artech_engine.get_doc("Item Attribute", "Test Size")
	...
	def test_copy_fields_from_template_to_variants(self):
		artech_engine.delete_doc_if_exists("Item", "_Test Variant Item-XL", force=1)
		...
		if not artech_engine.db.get_value(
			...
			item_attribute = artech_engine.get_doc("Item Attribute", "Test Size")
		...
		variant = artech_engine.get_doc("Item", "_Test Variant Item-XL")
	...
	def test_make_item_variant_with_numeric_values(self):
		# cleanup
		for d in artech_engine.db.get_all("Item", filters={"variant_of": "_Test Numeric Template Item"}):
			artech_engine.delete_doc_if_exists("Item", d.name)

		artech_engine.delete_doc_if_exists("Item", "_Test Numeric Template Item")
		artech_engine.delete_doc_if_exists("Item Attribute", "Test Item Length")
	...
	def test_item_merging(self):
		old = create_item(artech_engine.generate_hash(length=20)).name
		new = create_item(artech_engine.generate_hash(length=20)).name
		...
		artech_engine.rename_doc("Item", old, new, merge=True)

		self.assertFalse(artech_engine.db.exists("Item", old))

		self.assertTrue(artech_engine.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse - _TC"}))
		self.assertTrue(
			artech_engine.db.get_value("Bin", {"item_code": new, "warehouse": "_Test Warehouse 1 - _TC"})
		)
	...
	def test_item_merging_with_product_bundle(self):
		...
		with self.assertRaises(DataValidationError):
			artech_engine.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		bundle1.delete()
		artech_engine.rename_doc("Item", "Test Item Bundle Item 1", "Test Item Bundle Item 2", merge=True)

		self.assertFalse(artech_engine.db.exists("Item", "Test Item Bundle Item 1"))

	def test_uom_conversion_factor(self):
		if artech_engine.db.exists("Item", "Test Item UOM"):
			artech_engine.delete_doc("Item", "Test Item UOM")
	...
	def test_item_variant_by_manufacturer(self):
		...
		for manufacturer in ["DFSS", "DASA", "ASAAS"]:
			if not artech_engine.db.exists("Manufacturer", manufacturer):
				m_doc = artech_engine.new_doc("Manufacturer")
				m_doc.short_name = manufacturer
				m_doc.insert()

		self.assertFalse(artech_engine.db.exists("Item Manufacturer", {"manufacturer": "DFSS"}))
		variant = get_variant(template, manufacturer="DFSS", manufacturer_part_no="DFSS-123")

		item_manufacturer = artech_engine.db.exists(
			"Item Manufacturer", {"manufacturer": "DFSS", "item_code": variant.name}
		)
		self.assertTrue(item_manufacturer)

		artech_engine.delete_doc("Item Manufacturer", item_manufacturer)

	def test_stock_exists_against_template_item(self):
		stock_item = artech_engine.get_all("Stock Ledger Entry", fields=["item_code"], limit=1)
		if stock_item:
	...
	def test_add_item_barcode(self):
		...
		if artech_engine.db.exists("Item", item_code):
			artech_engine.delete_doc("Item", item_code)
		...
		self.assertRaises(artech_engine.UniqueValidationError, item_doc.save)
	...
	def test_index_creation(self):
		...
		indices = artech_engine.db.sql("show index from tabItem", as_dict=1)
	...
	def test_check_stock_uom_with_bin(self):
		...
		item = artech_engine.get_doc("Item", "_Test Item")
		item.stock_uom = "Gram"
		self.assertRaises(artech_engine.ValidationError, item.save)

	def test_check_stock_uom_with_bin_no_sle(self):
		from artech.stock.stock_balance import update_bin_qty
		...
		try:
			item.save()
		except artech_engine.ValidationError as e:
	...
	def test_erasure_of_old_conversions(self):
		...
		item.stock_uom = "Gram"
		item.append("uoms", artech_engine._dict(uom="Box", conversion_factor=2))
		item.save()
		item.reload()
		item.stock_uom = "Nos"
		item.save()
	...
	def test_validate_stock_item(self):
		self.assertRaises(artech_engine.ValidationError, validate_is_stock_item, "_Test Non Stock Item")
	...
	@ArtechTestSuite.change_settings("Stock Settings", {"item_naming_by": "Naming Series"})
	def test_autoname_series(self):
		item = artech_engine.new_doc("Item")
	...
	def test_item_dashboard(self):
		from artech.stock.dashboard.item_dashboard import get_data
	...
	def test_customer_codes_length(self):
		...
		for _row in range(3):
			item.append("customer_items", {"ref_code": artech_engine.generate_hash("", 120)})
		item.save()
	...
	def test_update_is_stock_item(self):
		...
		pb = artech_engine.new_doc("Product Bundle")
	...
	def test_search_fields_for_item(self):
		from artech.controllers.queries import item_query
	...
	def test_group_warehouse_for_reorder_item(self):
		...
		warehouse_doc = artech_engine.get_doc("Warehouse", warehouse)
		...
		self.assertRaises(artech_engine.ValidationError, item_doc.save)

	def test_variant_uom_mismatch_throws_error(self):
		artech_engine.db.set_single_value("Item Variant Settings", "allow_different_uom", 0)

		template_item = artech_engine.get_doc(
			{
				"doctype": "Item",
	...
	def test_opening_stock_for_serial_batch(self):
		...
		for item_code, properties in items.items():
			...
			serial_and_batch_bundle = artech_engine.db.get_value(
				"Stock Entry Detail", {"docstatus": 1, "item_code": item_code}, "serial_and_batch_bundle"
			)
			self.assertTrue(serial_and_batch_bundle)

			sabb_qty = artech_engine.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "total_qty")
			self.assertEqual(sabb_qty, properties["opening_stock"])


def set_item_variant_settings(fields):
	doc = artech_engine.get_doc("Item Variant Settings")
	doc.set("fields", fields)
	doc.save()


def make_item_variant():
	if not artech_engine.db.exists("Item", "_Test Variant Item-S"):
