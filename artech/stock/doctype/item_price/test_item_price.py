...
from artech.stock.get_item_details import ItemDetailsCtx, get_price_list_rate_for
from artech.tests.utils import ArtechTestSuite


class TestItemPrice(ArtechTestSuite):
	def setUp(self):
	...
	def test_duplicate_item(self):
		doc = artech_engine.copy_doc(self.globalTestRecords["Item Price"][0])
		self.assertRaises(ItemPriceDuplicateItem, doc.save)

	def test_addition_of_new_fields(self):
		# Based on https://github.com/artech_engine/artech/issues/8456
		...
		doc_fields = artech_engine.copy_doc(self.globalTestRecords["Item Price"][1]).__dict__.keys()

		for test_field in test_fields_existance:
			self.assertTrue(test_field in doc_fields)

	def test_dates_validation_error(self):
		doc = artech_engine.copy_doc(self.globalTestRecords["Item Price"][1])
	...
	def test_price_in_a_qty(self):
		...
		doc = artech_engine.copy_doc(self.globalTestRecords["Item Price"][2])
	...
	def test_prices_at_invalid_date(self):
		...
		doc = artech_engine.copy_doc(self.globalTestRecords["Item Price"][3])
