...
from artech_engine.query_builder.functions import Cast_


class ItemPriceDuplicateItem(artech_engine.ValidationError):
	pass
...
class ItemPrice(Document):
	...
	def validate_item(self):
		if not artech_engine.db.exists("Item", self.item_code):
			artech_engine.throw(_("Item {0} not found.").format(self.item_code))

		if self.uom and not artech_engine.db.exists(
			"UOM Conversion Detail", {"parenttype": "Item", "parent": self.item_code, "uom": self.uom}
		):
			artech_engine.throw(_("UOM {0} not found in Item {1}").format(self.uom, self.item_code))

	def update_price_list_details(self):
		if self.price_list:
			price_list_details = artech_engine.db.get_value(
				"Price List", {"name": self.price_list, "enabled": 1}, ["buying", "selling", "currency"]
			)

			if not price_list_details:
				link = artech_engine.utils.get_link_to_form("Price List", self.price_list)
				artech_engine.throw(f"The price list {link} does not exist or is disabled")

			self.buying, self.selling, self.currency = price_list_details

	def update_item_details(self):
		if self.item_code:
			self.item_name, self.item_description = artech_engine.db.get_value(
				"Item", self.item_code, ["item_name", "description"]
			)

	def validate_item_template(self):
		if artech_engine.get_cached_value("Item", self.item_code, "has_variants"):
			msg = f"Item Price cannot be created for the template item {bold(self.item_code)}"

			artech_engine.throw(_(msg))

	def check_duplicates(self):
		item_price = artech_engine.qb.DocType("Item Price")

		query = (
			artech_engine.qb.from_(item_price)
