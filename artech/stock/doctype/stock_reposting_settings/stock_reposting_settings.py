...
from artech_engine.utils import add_to_date, get_datetime, get_time_str, time_diff_in_hours
...
class StockRepostingSettings(Document):
	...
	def convert_to_item_wh_reposting(self):
		...
		for ledger in get_stock_ledgers(vouchers):
			...
			elif artech_engine.utils.getdate(item_warehouses.get(key)) > artech_engine.utils.getdate(ledger.posting_date):
				item_warehouses[key] = ledger.posting_date
		...
		for row in reposting_data:
			artech_engine.db.set_value("Repost Item Valuation", row.name, "status", "Skipped")

		self.db_set("item_based_reposting", 1)
		artech_engine.msgprint(_("Item Warehouse based reposting has been enabled."))
