artech_engine.ui.form.on("Quick Stock Balance", {
	setup: (frm) => {
		frm.set_query("item", () => {
	...
	check_warehouse_and_date: (frm) => {
		artech_engine.msgprint(__("Please enter Warehouse and Date"));
		frm.doc.item = "";
		frm.refresh();
	},
	...
	item: (frm) => {
		artech_engine.flags.last_updated_element = "item";
		frm.trigger("get_stock_and_item_details");
		frm.trigger("make_custom_stock_report_button");
	},

	item_barcode: (frm) => {
		artech_engine.flags.last_updated_element = "item_barcode";
		frm.trigger("get_stock_and_item_details");
		frm.trigger("make_custom_stock_report_button");
	},
	...
	get_stock_and_item_details: (frm) => {
		if (!(frm.doc.warehouse && frm.doc.date)) {
			...
			if (artech_engine.flags.last_updated_element === "item") {
				filters = { ...filters, ...{ item: frm.doc.item } };
			} else {
				filters = { ...filters, ...{ barcode: frm.doc.item_barcode } };
			}
			artech_engine.call({
				method: "artech.stock.doctype.quick_stock_balance.quick_stock_balance.get_stock_item_details",
				args: filters,
				callback: (r) => {
					if (r.message) {
