...
from artech_engine.utils import add_days, cstr, flt, getdate, nowdate, nowtime, today
...
from artech.stock.doctype.delivery_note.delivery_note import (
	make_delivery_trip,
	make_sales_invoice,
)
from artech.stock.doctype.delivery_trip.test_delivery_trip import create_driver
from artech.stock.doctype.item.test_item import make_item
from artech.stock.doctype.purchase_receipt.test_purchase_receipt import get_gl_entries
...
from artech.stock.stock_ledger import get_previous_sle
from artech.tests.utils import ArtechTestSuite


class TestDeliveryNote(ArtechTestSuite):
	def setUp(self):
	...
	def test_over_billing_against_dn(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_sales_invoice, dn.name)
		...
		self.assertRaises(artech_engine.ValidationError, artech_engine.get_doc(si).insert)

	def test_delivery_note_no_gl_entry(self):
		artech_engine.db.get_value("Warehouse", "_Test Warehouse - _TC", "company")
		...
		sle = artech_engine.get_doc("Stock Ledger Entry", {"voucher_type": "Delivery Note", "voucher_no": dn.name})
	...
	def test_delivery_note_gl_entry_packing_item(self):
		artech_engine.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_serialize_status(self):
		...
		serial_no = artech_engine.get_doc(
			{
				"doctype": "Serial No",
	...
	def test_delivery_note_return_against_denormalized_serial_no(self):
		from artech.stock.doctype.delivery_note.delivery_note import make_sales_return
		from artech.stock.doctype.serial_no.serial_no import get_serial_nos

		artech_engine.flags.ignore_serial_batch_bundle_validation = True
		...
		for sn in serial_nos:
			if not artech_engine.db.exists("Serial No", sn):
				sn_doc = artech_engine.get_doc(
					{
						"doctype": "Serial No",
						"item_code": sn_item,
						"serial_no": sn,
		...
		artech_engine.flags.ignore_serial_batch_bundle_validation = False
	...
	def test_sales_return_for_non_bundled_items_partial(self):
		...
		returned = artech_engine.get_doc("Delivery Note", dn1.name)
	...
	def test_sales_return_against_serial_batch_bundle(self):
		...
		for row in return_dn.items:
			total_qty = artech_engine.db.get_value(
	...
	def test_delivery_note_internal_transfer_serial_no_status(self):
		...
		for serial_no in serial_nos:
			sn = artech_engine.db.get_value("Serial No", serial_no, ["status", "warehouse"], as_dict=1)
	...
	def test_delivery_of_bundled_items_to_target_warehouse(self):
		...
		stock_value_difference1 = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Delivery Note",
				"voucher_no": dn.name,
	...
	def test_closed_delivery_note(self):
		from artech.stock.doctype.delivery_note.delivery_note import update_delivery_note_status
		...
		self.assertEqual(artech_engine.db.get_value("Delivery Note", dn.name, "Status"), "Closed")
	...
	def test_sales_order_reference_validation(self):
		...
		self.assertRaises(artech_engine.ValidationError, dn.save)
		dn.reload()
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"delete_linked_ledger_entries": True})
	...
	def test_dn_billing_status_case4(self):
		# SO -> SI -> DN
		from artech.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note
	...
	def test_dn_billing_status_case5(self):
		...
		# SO -> DN
		from artech.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
	...
	def test_delivery_trip(self):
		...
		self.assertRaisesRegex(
			artech_engine.exceptions.ValidationError,
	...
	def test_delivery_note_bundle_with_batched_item(self):
		...
		reposting_entries = artech_engine.get_all("Repost Item Valuation", filters={"docstatus": 1})
		for entry in reposting_entries:
			doc = artech_engine.get_doc("Repost Item Valuation", entry.name)
			doc.cancel()
			doc.delete()

		artech_engine.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 1)

		dn.reload()
		dn.delete()

		bundle = artech_engine.db.get_value("Serial and Batch Bundle", {"voucher_detail_no": packed_name}, "name")
		self.assertFalse(bundle)

		artech_engine.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		artech_engine.db.set_single_value("Accounts Settings", "delete_linked_ledger_entries", 0)
		...
		artech_engine.get_doc(
			{
				...
				"title": artech_engine.generate_hash(),
				"apply_on": "Item Code",
	...
	def test_internal_transfer_precision_gle(self):
		...
		self.assertFalse(artech_engine.db.exists("GL Entry", {"voucher_no": dn.name, "voucher_type": dn.doctype}))
	...
	def test_reserve_qty_on_sales_return(self):
		artech_engine.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 0)
		self.reserved_qty_check()

	def test_dont_reserve_qty_on_sales_return(self):
		artech_engine.db.set_single_value("Selling Settings", "dont_reserve_sales_order_qty_on_sales_return", 1)
		self.reserved_qty_check()

	def reserved_qty_check(self):
		from artech.controllers.sales_and_purchase_return import make_return_doc
		from artech.selling.doctype.sales_order.sales_order import make_delivery_note
		from artech.stock.stock_balance import get_reserved_qty

		dont_reserve_qty = artech_engine.db.get_single_value(
		...
		returned = artech_engine.get_doc("Delivery Note", dn_return.name)
		returned.update_prevdoc_status()
	...
	def test_serial_no_status(self):
		...
		dn.reload()
		self.assertEqual(artech_engine.db.get_value("Serial No", serial_no, "status"), "Delivered")
	...
	def test_sales_return_valuation_for_moving_average_case2(self):
		...
		artech_engine.flags.print_debug_messages = False
		item_code = make_item(
	...
	def test_batch_with_non_stock_uom(self):
		...
		if not artech_engine.db.exists("UOM Conversion Detail", {"parent": item.name, "uom": "Kg"}):
		...
		delivered_batch_qty = artech_engine.db.get_value(
	...
	def test_delivery_note_legacy_serial_no_valuation(self):
		...
		sle_data = artech_engine.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se_doc.name, "voucher_type": "Stock Entry"},
		)[0]

		sle_doc = artech_engine.get_doc("Stock Ledger Entry", sle_data.name)
		...
		for sn in serial_nos:
			sn_doc = artech_engine.get_doc("Serial No", sn)
			sn_doc.db_set(
				{
					"status": "Active",
					"warehouse": warehouse,
	...
	def test_partial_sales_return_batch_no_for_batched_item_in_dn(self):
		...
		returned_batch_no = get_batch_from_bundle(dn_return.items[0].serial_and_batch_bundle)
		self.assertEqual(batch_no, returned_batch_no)
		sabb_qty = artech_engine.db.get_value(
	...
	def test_same_posting_date_and_posting_time(self):
		...
		self.assertRaises(artech_engine.ValidationError, dn5.submit)
	...
	def test_warranty_expiry_date_for_serial_item(self):
		...
		for row in serial_nos:
			sn = artech_engine.get_doc("Serial No", row)
	...
	def test_auto_set_serial_batch_for_draft_dn(self):
		artech_engine.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)
		artech_engine.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", "FIFO")
	...
	def test_packed_item_serial_no_status(self):
		...
		for row in dn.packed_items:
			...
			for row in doc.entries:
				status = artech_engine.db.get_value("Serial No", row.serial_no, "status")
	...
	def test_sales_return_for_product_bundle(self):
		...
		for row in dn.packed_items:
			...
			if row.item_code not in serial_batch_map:
				serial_batch_map[row.item_code] = artech_engine._dict(
					{
						"serial_nos": [],
	...
	def test_negative_stock_with_higher_precision(self):
		original_flt_precision = artech_engine.db.get_default("float_precision")
		artech_engine.db.set_single_value("System Settings", "float_precision", 7)
		...
		self.assertRaises(artech_engine.ValidationError, dn.submit)

		artech_engine.db.set_single_value("System Settings", "float_precision", original_flt_precision)
	...
	def test_different_rate_for_same_serial_nos(self):
		...
		sabb = artech_engine.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		...
		sabb = artech_engine.get_doc("Serial and Batch Bundle", dn1.items[0].serial_and_batch_bundle)
		for entry in sabb.entries:
			self.assertEqual(entry.incoming_rate, 200)

		doc = artech_engine.new_doc("Repost Item Valuation")
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"validate_selling_price": 1})
...
def create_delivery_note(**args):
	dn = artech_engine.new_doc("Delivery Note")
	...
	if not args.use_serial_batch_fields and (args.get("batch_no") or args.get("serial_no")):
		...
		if args.get("batches"):
			batches = artech_engine._dict(args.batches)
