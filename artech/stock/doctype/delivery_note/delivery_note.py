...
from artech_engine.contacts.doctype.contact.contact import get_default_contact
...
from artech.accounts.party import get_due_date
from artech.controllers.accounts_controller import get_taxes_and_charges, merge_taxes
...
class DeliveryNote(SellingController):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.delivery_note_item.delivery_note_item import DeliveryNoteItem
	...
	def before_print(self, settings=None):
		...
		item_meta = artech_engine.get_meta("Delivery Note Item")
		print_hide_fields = {
	...
	def set_actual_qty(self):
		for d in self.get("items"):
			if d.item_code and d.warehouse:
				actual_qty = artech_engine.db.sql(
	...
	def so_required(self):
		...
		if artech_engine.get_single_value("Selling Settings", "so_required") == "Yes":
			for d in self.get("items"):
				if not d.against_sales_order:
					artech_engine.throw(_("Sales Order required for Item {0}").format(d.item_code))
	...
	def set_serial_and_batch_bundle_from_pick_list(self):
		...
		for item in self.items:
			...
			if item.pick_list_item and not item.serial_and_batch_bundle:
				...
				bundle_id = artech_engine.db.get_value("Serial and Batch Bundle", filters, "name")
			...
			if missing_label and missing_label != "No Label":
				errors.append(
					_("The field {0} in row {1} is not set").format(
						artech_engine.bold(_(missing_label)), artech_engine.bold(item.idx)
					)
				)

		if errors:
			artech_engine.throw("<br>".join(errors), title=error_title)
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if not d["warehouse"] and artech_engine.get_cached_value("Item", d["item_code"], "is_stock_item") == 1:
				artech_engine.throw(_("Warehouse required for stock Item {0}").format(d["item_code"]))

	def update_current_stock(self):
		if self.get("_action") and self._action != "update_after_submit":
			for d in self.get("items"):
				d.actual_qty = artech_engine.db.get_value(
					"Bin", {"item_code": d.item_code, "warehouse": d.warehouse}, "actual_qty"
				)

			for d in self.get("packed_items"):
				bin_qty = artech_engine.db.get_value(
					"Bin",
			...
			if not item.warehouse:
				...
				if item.warehouse not in reserved_warehouses:
					msg = _("Row #{0}: Stock is reserved for item {1} in warehouse {2}.").format(
						...
						artech_engine.bold(reserved_warehouses[0])
						if len(reserved_warehouses) == 1
						else _("{0} and {1}").format(
							artech_engine.bold(", ".join(reserved_warehouses[:-1])),
							artech_engine.bold(reserved_warehouses[-1]),
						),
					)
					artech_engine.throw(msg, title=_("Stock Reservation Warehouse Mismatch"))
	...
	def validate_packed_qty(self):
		...
		if artech_engine.db.exists("Packing Slip", {"docstatus": 1, "delivery_note": self.name}):
				...
				if (
					...
					artech_engine.throw(
						_("Row {0}: Packed Qty must be equal to {1} Qty.").format(
							item.idx, artech_engine.bold(item.doctype)
						)
					)
	...
	def check_next_docstatus(self):
		...
		if submit_rv:
			artech_engine.throw(_("Sales Invoice {0} has already been submitted").format(submit_rv[0][0]))

		submit_in = artech_engine.db.sql(
			"""select t1.name
		...
		if submit_in:
			artech_engine.throw(_("Installation Note {0} has already been submitted").format(submit_in[0][0]))
	...
	def cancel_packing_slips(self):
		...
		if res:
			for r in res:
				ps = artech_engine.get_doc("Packing Slip", r[0])
				ps.cancel()
			artech_engine.msgprint(_("Packing Slip(s) cancelled"))
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else artech_engine.get_lazy_doc("Delivery Note", dn)
	...
	def make_return_invoice(self):
		try:
			...
			credit_note_link = artech_engine.utils.get_link_to_form("Sales Invoice", return_invoice.name)

			artech_engine.msgprint(_("Credit Note {0} has been created automatically").format(credit_note_link))
		except Exception:
			artech_engine.throw(
				_(
	...
	def get_product_bundle_list(self):
		...
		return artech_engine.db.get_all(
			"Product Bundle",
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si = artech_engine.qb.DocType("Sales Invoice").as_("si")
	si_item = artech_engine.qb.DocType("Sales Invoice Item").as_("si_item")
	sum_amount = Sum(si_item.amount).as_("amount")

	billed_against_so = (
		artech_engine.qb.from_(si_item)
		.join(si)
	...
	dn = artech_engine.qb.DocType("Delivery Note").as_("dn")
	dn_item = artech_engine.qb.DocType("Delivery Note Item").as_("dn_item")
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_against_dn = artech_engine.db.sql(
...
def get_invoiced_qty_map(delivery_note):
	...
	invoiced_qty_map = artech_engine._dict(
		(
			artech_engine.qb.from_(sii)
...
def get_returned_qty_map(delivery_note):
	...
	returned_qty_map = artech_engine._dict(
		(
			artech_engine.qb.from_(dni)
			.join(dn)
...
def make_sales_invoice(
	...
	doc = artech_engine.get_doc("Delivery Note", source_name)
	...
	def set_missing_values(source, target):
		...
		if len(target.get("items")) == 0:
			artech_engine.throw(_("All these items have already been Invoiced/Returned"))
	...
	if not doc.is_return:
		...
		if (
			...
			payment_terms_template = artech_engine.db.get_value(doctype, so, "payment_terms_template")
...
def make_delivery_trip(
	...
	if not target_doc:
		target_doc = artech_engine.new_doc("Delivery Trip")
		...
		source_name,
		{
			...
			"Delivery Note Item": {
				...
				"condition": lambda item: (
					not artech_engine.db.exists("Product Bundle", {"new_item_code": item.item_code, "disabled": 0})
					and flt(item.packed_qty) < flt(item.qty)
				),
			},
			"Packed Item": {
...
def make_shipment(source_name: str, target_doc: str | Document | None = None):
	def postprocess(source, target):
		user = artech_engine.db.get_value(
			"User", artech_engine.session.user, ["email", "full_name", "phone", "mobile_no"], as_dict=1
		)
		...
		target.pickup_contact_person = artech_engine.session.user

		contact_person = source.contact_person or get_default_contact("Customer", source.customer)
		if contact_person:
			contact = artech_engine.db.get_value(
...
def update_delivery_note_status(docname: str, status: str):
	dn = artech_engine.get_lazy_doc("Delivery Note", docname)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if not target.get("items"):
			artech_engine.throw(_("All items have already been received"))
