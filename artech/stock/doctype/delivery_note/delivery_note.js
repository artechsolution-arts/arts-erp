...
artech_engine.ui.form.on("Delivery Note", {
	setup: function (frm) {
		...
		artech.queries.setup_warehouse_query(frm);
		...
		frm.set_query("expense_account", "items", function (doc, cdt, cdn) {
			if (artech.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
	...
	print_without_amount: function (frm) {
		artech.stock.delivery_note.set_print_hide(frm.doc);
	},
	...
	refresh: function (frm) {
		if (
			...
			artech_engine.model.can_create("Sales Invoice")
		) {
		...
		if (
			...
			artech_engine.model.can_create("Purchase Receipt")
		) {
			...
			if (internal) {
				...
				frm.add_custom_button(
					...
					function () {
						artech_engine.model.open_mapped_doc({
							method: "artech.stock.doctype.delivery_note.delivery_note.make_inter_company_purchase_receipt",
							frm: frm,
						});
					},
					__("Create")
...
artech_engine.ui.form.on("Delivery Note Item", {
...
artech.stock.DeliveryNoteController = class DeliveryNoteController extends (
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			artech_engine.model.can_read("Sales Order") &&
		...
		if (
			...
			doc.status != "Closed" &&
			this.frm.has_perm("write") &&
			artech_engine.model.can_read("Pick List") &&
			this.frm.doc.docstatus === 0
		) {
			this.frm.add_custom_button(
				__("Pick List"),
				function () {
					...
					artech.utils.map_current_doc({
						method: "artech.stock.doctype.pick_list.pick_list.create_dn_for_pick_lists",
						...
						get_query_method: "artech.stock.doctype.pick_list.pick_list.get_pick_list_query",
						size: "extra-large",
						allow_child_item_selection: true,
		...
		if (!doc.is_return && doc.status != "Closed") {
			if (doc.docstatus == 1 && artech_engine.model.can_create("Shipment")) {
				this.frm.add_custom_button(
					__("Shipment"),
					function () {
						me.make_shipment();
			...
			if (
				...
				doc.docstatus == 1 &&
				artech_engine.model.can_create("Installation Note")
			) {
				this.frm.add_custom_button(
			...
			if (doc.docstatus == 1 && doc.status != "Completed" && artech_engine.model.can_create("Delivery Trip")) {
				this.frm.add_custom_button(
					__("Delivery Trip"),
					function () {
			...
			if (
				...
				artech_engine.model.can_create("Packing Slip")
			) {
				this.frm.add_custom_button(
					__("Packing Slip"),
					function () {
		...
		artech.stock.delivery_note.set_print_hide(doc, dt, dn);
	}

	make_shipment() {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.delivery_note.delivery_note.make_shipment",
			frm: this.frm,
		});
	}
	...
	make_installation_note() {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.delivery_note.delivery_note.make_installation_note",
			frm: this.frm,
		});
	}
	...
	make_sales_return() {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.delivery_note.delivery_note.make_sales_return",
			frm: this.frm,
		});
	}
	...
	update_status(status) {
		...
		artech_engine.call({
			method: "artech.stock.doctype.delivery_note.delivery_note.update_delivery_note_status",
...
extend_cscript(cur_frm.cscript, new artech.stock.DeliveryNoteController({ frm: cur_frm }));
...
artech_engine.ui.form.on("Delivery Note", {
	...
	unhide_account_head: function (frm) {
		...
		var aii_enabled = artech.is_perpetual_inventory_enabled(frm.doc.company);
		frm.fields_dict["items"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	},
});

artech.stock.delivery_note.set_print_hide = function (doc, cdt, cdn) {
	var dn_fields = artech_engine.meta.docfield_map["Delivery Note"];
	var dn_item_fields = artech_engine.meta.docfield_map["Delivery Note Item"];
...
artech_engine.tour["Delivery Note"] = [
	{
		fieldname: "customer",
		title: __("Customer"),
