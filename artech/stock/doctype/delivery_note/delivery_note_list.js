artech_engine.listview_settings["Delivery Note"] = {
	add_fields: [
		"customer",
		"customer_name",
		"base_grand_total",
	...
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				artech_engine.new_doc("Delivery Trip").then(() => {
		...
		if (artech_engine.model.can_create("Delivery Trip")) {
		...
		if (artech_engine.model.can_create("Sales Invoice")) {
			doclist.page.add_action_item(__("Sales Invoice"), () => {
				artech.bulk_transaction_processing.create(doclist, "Delivery Note", "Sales Invoice");
			});
		}

		if (artech_engine.model.can_create("Packing Slip")) {
			doclist.page.add_action_item(__("Packaging Slip From Delivery Note"), () => {
				artech.bulk_transaction_processing.create(doclist, "Delivery Note", "Packing Slip");
			});
		}
	},
};
