...
from artech_engine import ValidationError, _
from artech_engine.model.naming import make_autoname
from artech_engine.query_builder.functions import Coalesce
from artech_engine.utils import cint, cstr, getdate, nowdate, safe_json_loads
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = artech_engine.db.get_value("Serial No", self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				artech_engine.throw(_("Item Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				artech_engine.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
	...
	def on_trash(self):
		sl_entries = artech_engine.db.sql(
...
def get_new_serial_number(series):
	...
	if artech_engine.db.exists("Serial No", sr_no):
...
def get_serial_nos_from_sle_list(bundles):
	...
	query = artech_engine.qb.from_(table).select(table.parent, table.serial_no).where(table.parent.isin(bundles))
...
def update_maintenance_status():
	serial_nos = artech_engine.db.sql(
	...
	for serial_no in serial_nos:
		doc = artech_engine.get_doc("Serial No", serial_no[0])
		doc.set_maintenance_status()
		artech_engine.db.set_value("Serial No", doc.name, "maintenance_status", doc.maintenance_status)
...
def auto_fetch_serial_number(
	...
	filters = artech_engine._dict({"item_code": item_code, "warehouse": warehouse})
...
def get_pos_reserved_serial_nos(filters: str | dict):
	...
	POSInvoiceItem = artech_engine.qb.DocType("POS Invoice Item")
	query = (
		artech_engine.qb.from_(POSInvoice)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	serial_no = artech_engine.qb.DocType("Serial No")

	query = (
		artech_engine.qb.from_(serial_no)
...
def on_doctype_update():
	artech_engine.db.add_index("Serial No", ["item_code", "warehouse"])
