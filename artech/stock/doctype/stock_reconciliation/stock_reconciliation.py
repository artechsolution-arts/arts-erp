...
from artech_engine import _, bold, json, msgprint
from artech_engine.utils import add_to_date, cint, cstr, flt, now
from artech_engine.utils.data import DateTimeLikeObject

import artech
from artech.accounts.utils import get_company_default
from artech.controllers.stock_controller import StockController, create_repost_item_valuation_entry
from artech.stock.doctype.batch.batch import get_available_batches, get_batch_qty
...
from artech.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import StockReconciliationItem
from artech.stock.utils import get_combine_datetime, get_incoming_rate, get_stock_balance


class OpeningEntryAccountError(artech_engine.ValidationError):
	pass


class EmptyStockReconciliationItemsError(artech_engine.ValidationError):
	pass
...
class StockReconciliation(StockController):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.stock_reconciliation_item.stock_reconciliation_item import (
	...
	def validate(self):
		...
		if not self.expense_account:
			self.expense_account = artech_engine.get_cached_value(
				"Company", self.company, "stock_adjustment_account"
			)
		if not self.cost_center:
			self.cost_center = artech_engine.get_cached_value("Company", self.company, "cost_center")
				...
				sn_doc = SerialBatchCreation(
					{
						...
						"batches": artech_engine._dict({row.batch_no: row.current_qty}) if row.batch_no else None,
	...
	def set_current_serial_and_batch_bundle(self, voucher_detail_no=None, save=False) -> None:
		...
		for item in self.items:
			...
			if not item.reconcile_all_serial_batch and item.serial_and_batch_bundle:
				...
				if save:
					...
					if (
						...
						sabb_doc = artech_engine.get_doc(
			...
			if not item.current_serial_and_batch_bundle:
				serial_and_batch_bundle = artech_engine.get_doc(
					{
			...
			if save:
				sle_creation = artech_engine.db.get_value(
	...
	def has_change_in_serial_batch(self, row) -> bool:
		...
		if diff:
			...
			artech_engine.delete_doc("Serial and Batch Bundle", bundle)
	...
	def set_new_serial_and_batch_bundle(self):
		for item in self.items:
			...
			if item.current_serial_and_batch_bundle and not item.serial_and_batch_bundle:
				current_doc = artech_engine.get_doc("Serial and Batch Bundle", item.current_serial_and_batch_bundle)

				item.qty = abs(current_doc.total_qty)
				item.valuation_rate = abs(current_doc.avg_rate)

				bundle_doc = artech_engine.copy_doc(current_doc)
				...
				bundle_doc = artech_engine.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def update_existing_serial_and_batch_bundle(self, item):
		batch_details = artech_engine.get_all(
			"Serial and Batch Entry",
		...
		if batch_details and len(batch_details) == 1:
			...
			artech_engine.db.set_value("Serial and Batch Entry", batch.name, update_values)

	def remove_items_with_no_change(self):
		from artech.stock.stock_ledger import get_stock_value_difference
		...
		if not items:
			...
			artech_engine.msgprint(_("Removed items with no change in quantity or value."))
	...
	def validate_data(self):
		...
		default_currency = artech_engine.db.get_default("currency")
		...
		for row in self.items:
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = artech_engine.db.get_value(
						"Item Price",
					...
					if buying_rate:
						...
						row.valuation_rate = artech_engine.get_value("Item", row.item_code, "valuation_rate")
		...
		if self.validation_messages:
			...
			raise artech_engine.ValidationError(self.validation_messages)
	...
	def validate_item(self, item_code, row):
		...
		def validate_serial_batch_items():
			has_batch_no, has_serial_no = artech_engine.get_value(
				"Item", item_code, ["has_batch_no", "has_serial_no"]
			)
			if row.use_serial_batch_fields and self.purpose == "Stock Reconciliation":
				if has_batch_no and not row.batch_no:
					raise artech_engine.ValidationError(_("Please enter Batch No"))
				if has_serial_no and not row.serial_no:
					raise artech_engine.ValidationError(_("Please enter Serial No"))
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_and_batch_bundle:
					artech_engine.throw(
						...
						).format(row.idx, artech_engine.bold(row.item_code))
					)
		...
		if sl_entries:
			if not allow_negative_stock:
				allow_negative_stock = cint(
					artech_engine.db.get_single_value("Stock Settings", "allow_negative_stock")
				)
	...
	def update_valuation_rate_for_serial_nos(self, row, serial_nos):
		...
		for d in serial_nos:
			artech_engine.db.set_value("Serial No", d, "purchase_rate", valuation_rate)
		...
		data = artech_engine._dict(
			{
				...
				"actual_qty": 0,
				"company": self.company,
				"stock_uom": artech_engine.db.get_value("Item", row.item_code, "stock_uom"),
	...
	def make_sle_on_cancel(self):
		...
		if sl_entries:
			...
			sl_entries.reverse()
			allow_negative_stock = cint(artech_engine.db.get_single_value("Stock Settings", "allow_negative_stock"))
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)
	...
	def validate_expense_account(self):
		...
		if not self.expense_account:
			artech_engine.throw(_("Please enter Expense Account"))
		elif self.purpose == "Opening Stock" or not artech_engine.db.sql(
			"""select name from `tabStock Ledger Entry` limit 1"""
		):
			if artech_engine.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				artech_engine.throw(
					_(
	...
	def set_zero_value_for_customer_provided_items(self):
		...
		for d in self.get("items"):
			is_customer_item = artech_engine.get_cached_value("Item", d.item_code, "is_customer_provided_item")
	...
	def recalculate_current_qty(self, voucher_detail_no, sle_creation, add_new_sle=False):
		for row in self.items:
			...
			if add_new_sle and not artech_engine.db.get_value(
				"Stock Ledger Entry",
	...
	def add_missing_stock_ledger_entry(self, row, voucher_detail_no, sle_creation):
		...
		new_sle = artech_engine.get_doc(self.get_sle_for_items(row))
	...
	def get_current_qty_for_serial_or_batch(self, row, sle_creation):
		doc = artech_engine.get_doc("Serial and Batch Bundle", row.current_serial_and_batch_bundle)
		current_qty = 0.0
...
def get_item_and_warehouses(item_code, warehouse):
	...
	if artech_engine.get_cached_value("Warehouse", warehouse, "is_group"):
		...
		for ch_warehouse in childrens:
			items.append(artech_engine._dict({"item_code": item_code, "warehouse": ch_warehouse}))
	else:
		items = [artech_engine._dict({"item_code": item_code, "warehouse": warehouse})]
...
def get_itemwise_batch(warehouse, posting_date, company, item_code=None):
	from artech.stock.report.batch_wise_balance_history.batch_wise_balance_history import execute
...
def get_stock_balance_for(
	...
	artech_engine.has_permission("Stock Reconciliation", "write", throw=True)

	item_dict = artech_engine.get_cached_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
	...
	if not item_dict:
		...
		artech_engine.throw(msg, title=_("Missing"))

	serial_nos = None
	has_serial_no = bool(item_dict.get("has_serial_no"))
	has_batch_no = bool(item_dict.get("has_batch_no"))

	use_serial_batch_fields = artech_engine.db.get_single_value("Stock Settings", "use_serial_batch_fields")
