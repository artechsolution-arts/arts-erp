...
from artech_engine.utils import add_days, cstr, flt, nowdate, nowtime

from artech.accounts.utils import get_stock_and_account_balance
...
from artech.stock.stock_ledger import get_previous_sle, update_entries_after
...
class TestStockReconciliation(ArtechTestSuite, StockTestMixin):
	def setUp(self):
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", 1)
		artech_engine.local.future_sle = {}
		artech_engine.flags.pop("dont_execute_stock_reposts", None)
	...
	def test_stock_reco_for_serialized_item(self):
		...
		serial_nos = artech_engine.get_doc(
		...
		sn_doc = artech_engine.get_doc("Serial and Batch Bundle", sr.items[0].serial_and_batch_bundle)
		...
		for d in to_delete_records:
			stock_doc = artech_engine.get_doc("Stock Reconciliation", d)
			stock_doc.cancel()
	...
	def test_stock_reco_for_serial_and_batch_item(self):
		...
		self.assertEqual(artech_engine.db.get_value("Serial No", serial_nos[0], "batch_no"), batch_no)

		sr.cancel()

		self.assertEqual(artech_engine.db.get_value("Serial No", serial_nos[0], "warehouse"), None)
	...
	def test_stock_reco_for_serial_and_batch_item_with_future_dependent_entry(self):
		...
		self.assertEqual(artech_engine.db.get_value("Serial No", reco_serial_no, "batch_no"), batch_no)
		self.assertTrue(artech_engine.db.get_value("Serial No", reco_serial_no, "warehouse"))

		# Check if Serial No from Stock Entry is Unlinked and Inactive
		self.assertFalse(artech_engine.db.get_value("Serial No", serial_no_2, "warehouse"))
	...
	def test_backdated_stock_reco_qty_reposting(self):
		...
		artech_engine.flags.dont_execute_stock_reposts = True

		def assertBalance(doc, qty_after_transaction):
			sle_balance = artech_engine.db.get_value(
	...
	def test_backdated_stock_reco_future_negative_stock(self):
		...
		pr1_balance = artech_engine.db.get_value(
			"Stock Ledger Entry", {"voucher_no": pr1.name, "is_cancelled": 0}, "qty_after_transaction"
		)
		dn2_balance = artech_engine.db.get_value(
	...
	def test_backdated_stock_reco_cancellation_future_negative_stock(self):
		...
		dn_balance = artech_engine.db.get_value(
		...
		repost_exists = bool(
			artech_engine.db.exists("Repost Item Valuation", {"voucher_no": sr.name, "status": "Queued"})
		)
	...
	def test_intermediate_sr_bin_update(self):
		...
		old_bin_qty = artech_engine.db.get_value(
		...
		new_bin_qty = artech_engine.db.get_value(
			"Bin", {"item_code": item_code, "warehouse": warehouse}, "actual_qty"
		)
	...
	def test_valid_batch(self):
		...
		self.assertRaises(artech_engine.ValidationError, doc.save)
	...
	def test_serial_no_cancellation(self):
		...
		active_sr_no = artech_engine.get_all(
	...
	def test_serial_no_creation_and_inactivation(self):
		...
		if not artech_engine.db.exists("Serial No", "SR-CREATED-SR-NO"):
	...
	def test_serial_no_batch_no_item(self):
		...
		sl_entry = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_stock_reco_entry(self):
		...
		posting_datetime = artech_engine.db.get_value("Serial and Batch Bundle", stock_reco_sabb, "posting_datetime")
		self.assertEqual(
		...
		self.assertFalse(artech_engine.db.exists("Repost Item Valuation", {"voucher_no": stock_reco.name}))

		self.assertRaises(artech_engine.ValidationError, stock_reco.cancel)
	...
	def test_make_stock_zero_for_serial_batch_item(self):
		...
		for item_code in [serial_item, batch_item, serial_batch_item]:
			...
			serial_batch_bundle = artech_engine.get_all(
				"Stock Ledger Entry",
	...
	def test_not_reconcile_all_batch(self):
		...
		for qty in [10, 20, 30]:
			...
			batches.append(artech_engine._dict({"batch_no": batch_no, "qty": qty}))
		...
		doc = artech_engine.get_doc("Serial and Batch Bundle", current_sabb)
		...
		for row in artech_engine.get_all("Repost Item Valuation", filters={"voucher_no": sr.name}):
			rdoc = artech_engine.get_doc("Repost Item Valuation", row.name)
			rdoc.cancel()
			rdoc.delete()
	...
	def test_not_reconcile_all_serial_nos(self):
		...
		doc = artech_engine.get_doc("Serial and Batch Bundle", sabb)
		for row in doc.entries:
	...
	def test_stock_reco_with_legacy_batch(self):
		...
		if not artech_engine.db.exists("Batch", batch_id):
			batch_doc = artech_engine.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_id,
		...
		batch_doc = artech_engine.get_doc("Batch", batch_id)
	...
	def test_stock_reco_for_negative_batch(self):
		...
		doc = artech_engine.get_doc("Stock Ledger Entry", sles[0].name)
		doc.db_set(
			{
				"actual_qty": -20,
		...
		sabb_doc = artech_engine.get_doc("Serial and Batch Bundle", doc.serial_and_batch_bundle)
	...
	def test_stock_reco_recalculate_qty_for_backdated_entry(self):
		...
		batch_no = get_batch_from_bundle(sr.items[0].serial_and_batch_bundle)
		stock_ledgers = artech_engine.get_all(
			"Stock Ledger Entry",
		...
		stock_ledger = artech_engine.get_all(
			"Stock Ledger Entry",
			filters={"voucher_no": se.name, "is_cancelled": 0},
	...
	def test_serial_no_backdated_stock_reco(self):
		...
		status = artech_engine.get_value(
			"Serial No",
			serial_no,
			"status",
		)
	...
	def test_sabb_cancel_on_stock_reco_cancellation(self):
		...
		for sabb in [serial_and_batch_bundle, current_serial_and_batch_bundle]:
			docstatus = artech_engine.db.get_value(
	...
	def test_stock_reco_with_opening_stock_with_diff_inventory(self):
		from artech.stock.doctype.inventory_dimension.test_inventory_dimension import (
			create_inventory_dimension,
		)

		if artech_engine.db.exists("DocType", "Plant"):
			return

		doctype = artech_engine.get_doc(
			{
				"doctype": "DocType",
				"name": "Plant",
				"module": "Stock",
		...
		sr = artech_engine.new_doc("Stock Reconciliation")
		...
		sle_count = artech_engine.db.count(
			"Stock Ledger Entry",
	...
	def test_serial_no_status_with_backdated_stock_reco(self):
		...
		self.assertEqual(artech_engine.get_value("Serial No", serial_no, "status"), "Delivered")
...
def create_stock_reconciliation(**args):
	...
	sr.expense_account = args.expense_account or (
		(
			artech_engine.get_cached_value("Company", sr.company, "stock_adjustment_account")
			or artech_engine.get_cached_value(
				"Account", {"account_type": "Stock Adjustment", "company": sr.company}, "name"
			)
		)
		if artech_engine.get_all("Stock Ledger Entry", {"company": sr.company})
		else artech_engine.get_cached_value("Account", {"account_type": "Temporary", "company": sr.company}, "name")
	)
	sr.cost_center = (
		args.cost_center
		or artech_engine.get_cached_value("Company", sr.company, "cost_center")
		or artech_engine.get_cached_value("Cost Center", filters={"is_group": 0, "company": sr.company})
	)
...
def set_valuation_method(item_code, valuation_method):
	...
	artech_engine.db.set_value("Item", item_code, "valuation_method", valuation_method)

	for warehouse in artech_engine.get_all(
