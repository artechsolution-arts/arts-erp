...
import artech_engine.query_builder
import artech_engine.query_builder.functions
from artech_engine import _, _dict, bold
from artech_engine.model.document import Document
from artech_engine.model.naming import make_autoname
from artech_engine.query_builder.functions import Concat_ws, Locate, Sum
...
from artech_engine.utils.csvutils import build_csv_response

from artech.stock.doctype.purchase_receipt_item.purchase_receipt_item import PurchaseReceiptItem
from artech.stock.serial_batch_bundle import (
	BatchNoValuation,
	SerialNoValuation,
	get_batches_from_bundle,
)
from artech.stock.serial_batch_bundle import get_serial_nos as get_serial_nos_from_bundle
from artech.stock.valuation import FIFOValuation


class SerialNoExistsInFutureTransactionError(artech_engine.ValidationError):
	pass


class BatchNegativeStockError(artech_engine.ValidationError):
	pass


class SerialNoDuplicateError(artech_engine.ValidationError):
	pass


class SerialNoWarehouseError(artech_engine.ValidationError):
	pass
...
class SerialandBatchBundle(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.serial_and_batch_entry.serial_and_batch_entry import SerialandBatchEntry
	...
	def autoname(self):
		if artech_engine.get_single_value(
			...
			if not self.naming_series:
				artech_engine.throw(_("Naming Series is mandatory"))
			...
			try:
				self.name = artech_engine.generate_hash(length=20)
	...
	def validate(self):
		...
		if self.docstatus == 1:
			if not self.flags.ignore_validate_serial_batch or artech_engine.in_test:
	...
	def validate_allow_to_set_serial_batch(self):
		if not artech_engine.db.get_single_value("Stock Settings", "enable_serial_and_batch_no_for_item"):
			artech_engine.throw(
				_(
	...
	def validate_serial_no_status(self):
		...
		invalid_serial_nos = artech_engine.get_all(
			"Serial No",
			filters={
				"name": ("in", serial_nos),
	...
	def validate_voucher_detail_no(self):
		...
		if self.voucher_type == "POS Invoice":
			if not artech_engine.db.exists("POS Invoice Item", self.voucher_detail_no):
				artech_engine.throw(
		...
		elif not artech_engine.db.exists("Stock Ledger Entry", {"voucher_detail_no": self.voucher_detail_no}):
			if self.voucher_type in ["Delivery Note", "Sales Invoice"] and artech_engine.db.exists(
	...
	def allow_existing_serial_nos(self):
		...
		if artech_engine.get_single_value("Stock Settings", "allow_existing_serial_no"):
			return

		if self.voucher_type not in ["Purchase Receipt", "Purchase Invoice", "Stock Entry"]:
			return

		if self.voucher_type == "Stock Entry" and artech_engine.get_cached_value(
	...
	def set_batch_no(self):
		if self.has_serial_no and self.has_batch_no:
			...
			serial_no_batch = artech_engine._dict(
	...
	def validate_serial_nos_inventory(self):
		...
		available_serial_nos = get_available_serial_nos(artech_engine._dict(kwargs))
	...
	def validate_serial_nos_duplicate(self):
		...
		if (
			...
			and artech_engine.get_cached_value("Stock Entry", self.voucher_no, "purpose")
	...
	def throw_error_message(self, message, exception=artech_engine.ValidationError):
		artech_engine.throw(_(message), exception, title=_("Error"))
	...
	def set_valuation_rate_for_return_entry(self, return_against, row, save=False, prev_sle=None):
		if valuation_details := self.get_valuation_rate_for_return_entry(return_against):
			from artech.stock.utils import get_valuation_method
			...
			if not self.has_serial_no and valuation_method == "FIFO":
				non_batchwise_batches = artech_engine.get_all(
					"Batch",
					filters={
			...
			for row in self.entries:
				...
				if artech_engine.flags.through_repost_item_valuation and not valuation_rate:
	...
	def validate_returned_serial_batch_no(self, return_against, row, original_inv_details):
		if artech_engine.flags.through_repost_item_valuation and not artech_engine.in_test:
			return
	...
	def get_valuation_rate_for_return_entry(self, return_against):
		from artech.controllers.sales_and_purchase_return import get_warehouses_for_return

		if not self.voucher_detail_no:
			return {}

		valuation_details = artech_engine._dict(
			{
		...
		return_against_voucher_detail_no = artech_engine.db.get_value(
	...
	def validate_negative_batch(self, batch_no, available_qty):
		if available_qty < 0 and not self.is_stock_reco_for_valuation_adjustment(available_qty):
			...
			artech_engine.throw(_(msg), BatchNegativeStockError)
	...
	def is_stock_reco_for_valuation_adjustment(self, available_qty):
		if (
			...
			and (
				abs(artech_engine.db.get_value("Stock Reconciliation Item", self.voucher_detail_no, "qty"))
				== abs(available_qty)
			)
		):
			return True
	...
	def get_return_against(self, parent=None):
		...
		if (
			...
			voucher_details = artech_engine.db.get_value(
	...
	def set_incoming_rate_for_inward_transaction(self, row=None, save=False, prev_sle=None):
		...
		if self.voucher_type == "Subcontracting Receipt":
			if not self.voucher_detail_no:
				return
			elif artech_engine.db.exists("Subcontracting Receipt Supplied Item", self.voucher_detail_no):
				valuation_field = "rate"
		...
		if not rate and self.voucher_detail_no and self.voucher_no:
			rate = artech_engine.db.get_value(child_table, self.voucher_detail_no, valuation_field)

		is_packed_item = False
		if rate is None and child_table in ["Delivery Note Item", "Sales Invoice Item"]:
			rate = artech_engine.db.get_value(
				"Packed Item",
			...
			if rate is None:
				rate = artech_engine.db.get_value("Packed Item", self.voucher_detail_no, "incoming_rate")
		...
		set_valuation_rate_for_rejected_materials = artech_engine.db.get_single_value(
			"Buying Settings", "set_valuation_rate_for_rejected_materials"
		)

		precision = artech_engine.get_precision("Serial and Batch Entry", "incoming_rate")
		for d in self.entries:
	...
	def set_serial_and_batch_values(self, parent, row, qty_field=None):
		...
		if parent.doctype in [
			...
			return_ref_field = artech_engine.scrub(parent.doctype) + "_item"
	...
	def validate_voucher_no(self):
		...
		if (
			self.docstatus == 1
			and self.voucher_no
			and not artech_engine.db.exists(self.voucher_type, self.voucher_no)
		):
		...
		if (
			self.docstatus == 1
			and artech_engine.get_cached_value(self.voucher_type, self.voucher_no, "docstatus") != 1
		):
	...
	def check_future_entries_exists(self, is_cancelled=False):
		...
		parent = artech_engine.qb.DocType("Serial and Batch Bundle")
		child = artech_engine.qb.DocType("Serial and Batch Entry")
		...
		if future_entries:
			...
			artech_engine.throw(_(msg), title=_(title), exc=SerialNoExistsInFutureTransactionError)
	...
	def validate_quantity(self, row, qty_field=None):
		...
		if abs(abs(flt(self.total_qty, precision)) - abs(flt(qty, precision))) > 0.01:
			total_qty = artech_engine.format_value(abs(flt(self.total_qty)), "Float", row)
			set_qty = artech_engine.format_value(abs(flt(row.get(qty_field))), "Float", row)
			self.throw_error_message(
	...
	def calculate_outgoing_rate(self):
		...
		if self.voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
			return artech_engine.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type in ["Sales Invoice", "Delivery Note"]:
			return not artech_engine.get_cached_value(self.voucher_type, self.voucher_no, "is_return")
		elif self.voucher_type == "Stock Entry":
			return artech_engine.get_cached_value(self.voucher_type, self.voucher_no, "purpose") in [
				"Material Receipt"
			]
	...
	def validate_serial_batch_no(self, serial_batches):
		correct_batches = artech_engine._dict(
			artech_engine.get_all(
				"Serial No",
	...
	def validate_incorrect_serial_nos(self, serial_nos):
		incorrect_serial_nos = artech_engine.get_all(
			"Serial No",
	...
	def validate_incorrect_batch_nos(self, batch_nos):
		incorrect_batch_nos = artech_engine.get_all(
		...
		for d in data:
			if self.has_serial_no:
				...
				if d.serial_and_batch_bundle:
					...
					batches = artech_engine._dict({d.batch_no: d.stock_qty})
	...
	def get_orignal_document_data(self):
		...
		return artech_engine.get_all(child_doc, fields=fields, filters={"name": self.returned_against})
	...
	def delink_serial_and_batch_bundle(self):
		sles = artech_engine.get_all("Stock Ledger Entry", filters={"serial_and_batch_bundle": self.name})

		for sle in sles:
			artech_engine.db.set_value("Stock Ledger Entry", sle.name, "serial_and_batch_bundle", None)
	...
	def delink_refernce_from_voucher(self):
		...
		if (
			...
			and self.voucher_detail_no
			and not artech_engine.db.exists("Subcontracting Receipt Item", self.voucher_detail_no)
		):
			self.voucher_type = "Subcontracting Receipt Supplied"

		vouchers = artech_engine.get_all(
			self.child_table,
			fields=fields,
			filters={"docstatus": 0},
		...
		if not vouchers and self.voucher_type == "Delivery Note":
			if artech_engine.db.exists("Packed Item", self.voucher_detail_no):
				artech_engine.db.set_value("Packed Item", self.voucher_detail_no, "serial_and_batch_bundle", None)
			else:
				packed_items = artech_engine.get_all(
					"Packed Item",
					filters={
				...
				for packed_item in packed_items:
					artech_engine.db.set_value("Packed Item", packed_item, "serial_and_batch_bundle", None)

			return

		for voucher in vouchers:
			if voucher.get("current_serial_and_batch_bundle"):
				artech_engine.db.set_value(self.child_table, voucher.name, "current_serial_and_batch_bundle", None)
			elif voucher.get("rejected_serial_and_batch_bundle"):
				artech_engine.db.set_value(self.child_table, voucher.name, "rejected_serial_and_batch_bundle", None)

			artech_engine.db.set_value(self.child_table, voucher.name, "serial_and_batch_bundle", None)
	...
	def delink_reference_from_batch(self):
		...
		for batch in batches:
			artech_engine.db.set_value("Batch", batch.name, {"reference_name": None, "reference_doctype": None})

	def validate_serial_and_batch_data(self):
		if not self.voucher_no:
			artech_engine.throw(_("Voucher No is mandatory"))
	...
	def set_source_document_no(self):
		...
		if self.has_serial_no:
			serial_nos = [d.serial_no for d in self.entries if d.serial_no]
			sn_table = artech_engine.qb.DocType("Serial No")
			(
				artech_engine.qb.update(sn_table)
		...
		if self.has_batch_no:
			...
			batch_table = artech_engine.qb.DocType("Batch")
			(
				artech_engine.qb.update(batch_table)
	...
	def throw_negative_batch(self, batch_no, available_qty, precision, posting_datetime=None):
		...
		if artech_engine.db.get_single_value("Stock Settings", "allow_negative_stock_for_batch"):
			return
	...
	def get_batchwise_available_qty(self):
		...
		available_qty = artech_engine._dict({})
	...
	def validate_voucher_no_docstatus(self):
		...
		if self.voucher_type == "Delivery Note" and not artech_engine.db.exists(
			"Delivery Note Item", self.voucher_detail_no
		):
			child_doctype = "Packed Item"

		elif self.voucher_type == "Sales Invoice" and not artech_engine.db.exists(
			"Sales Invoice Item", self.voucher_detail_no
		):
			child_doctype = "Packed Item"

		elif self.voucher_type == "Subcontracting Receipt" and not artech_engine.db.exists(
			"Subcontracting Receipt Item", self.voucher_detail_no
		):
			child_doctype = "Subcontracting Receipt Supplied Item"

		if (
			artech_engine.db.get_value(self.voucher_type, self.voucher_no, "docstatus") == 1
			and self.voucher_detail_no
			and artech_engine.db.exists(child_doctype, self.voucher_detail_no)
		):
	...
	def delete_serial_batch_entries(self):
		SBBE = artech_engine.qb.DocType("Serial and Batch Entry")

		artech_engine.qb.from_(SBBE).delete().where(SBBE.parent == self.name).run()
...
def get_serial_batch_from_csv(item_code, file_path):
	...
	try:
		file = artech_engine.get_doc("File", {"file_url": file_path})
	except artech_engine.DoesNotExistError:
		artech_engine.msgprint(
			_("File '{0}' not found").format(artech_engine.bold(file_path)),
			alert=True,
			indicator="red",
	...
	if file.file_type != "CSV":
		artech_engine.msgprint(
			_("{0} is not a CSV file.").format(artech_engine.bold(file.file_name)),
			alert=True,
			indicator="red",
...
def make_serial_nos(item_code, serial_nos):
	item = artech_engine.get_cached_value(
	...
	existing_serial_nos = artech_engine.get_all("Serial No", filters={"name": ("in", serial_nos)})
	...
	artech_engine.msgprint(_("Serial Nos are created successfully"), alert=True)


def make_batch_nos(item_code, batch_nos):
	item = artech_engine.get_cached_value("Item", item_code, ["description", "item_code"], as_dict=1)
	batch_nos = [d.get("batch_no") for d in batch_nos if d.get("batch_no")]

	existing_batches = artech_engine.get_all("Batch", filters={"name": ("in", batch_nos)})
	...
	for batch_no in batch_nos:
		if artech_engine.db.exists("Batch", batch_no):
			continue
	...
	artech_engine.db.bulk_insert("Batch", fields=fields, values=set(batch_nos_details))

	artech_engine.msgprint(_("Batch Nos are created successfully"), alert=True)
...
def get_reference_serial_and_batch_bundle(child_row):
	...
	if field:
		return artech_engine.get_cached_value(child_row.doctype, child_row.get(field), "serial_and_batch_bundle")
...
def add_serial_batch_ledgers(
	...
	if isinstance(child_row, str):
		child_row = artech_engine._dict(parse_json(child_row))
	...
	if artech_engine.db.exists("Serial and Batch Bundle", bundle):
...
def create_serial_batch_no_ledgers(
	...
	if (
		...
		and artech_engine.get_cached_value("Item", child_row.item_code, "has_batch_no")
	):
	...
	if do_not_save:
		artech_engine.db.set_value(child_row.doctype, child_row.name, "serial_and_batch_bundle", doc.name)

	artech_engine.msgprint(_("Serial and Batch Bundle created"), alert=True)

	return doc


def combine_datetime(date, time=None):
	from artech.stock.utils import get_combine_datetime
...
def update_serial_batch_no_ledgers(bundle, entries, child_row, parent_doc, warehouse=None) -> object:
	doc = artech_engine.get_doc("Serial and Batch Bundle", bundle)
	...
	artech_engine.msgprint(_("Serial and Batch Bundle updated"), alert=True)
...
def update_serial_or_batch(bundle_id: str, serial_no: str | None = None, batch_no: str | None = None):
	if batch_no and not serial_no:
		if qty := artech_engine.db.get_value(
	...
	doc = artech_engine.get_cached_doc("Serial and Batch Bundle", bundle_id)
...
def get_serial_and_batch_ledger(**kwargs):
	...
	sle_table = artech_engine.qb.DocType("Stock Ledger Entry")
	serial_batch_table = artech_engine.qb.DocType("Serial and Batch Entry")

	query = (
		artech_engine.qb.from_(sle_table)
...
def get_serial_nos_based_on_filters(filters, fields, order_by, kwargs):
	doctype = artech_engine.qb.DocType("Serial No")

	order_by_column = getattr(doctype, order_by)
	query = artech_engine.qb.from_(doctype).limit(cint(kwargs.qty) or 10000000).for_update()

	if kwargs.based_on == "LIFO":
		query = query.orderby(order_by_column, order=artech_engine.query_builder.Order.desc)
	else:
...
def get_serial_nos_from_sre(kwargs):
	table = artech_engine.qb.DocType("Stock Reservation Entry")
	child_table = artech_engine.qb.DocType("Serial and Batch Entry")
	query = (
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(child_table.creation, order=artech_engine.query_builder.Order.desc)
	else:
...
def get_reserved_serial_nos_for_pos(kwargs):
	from artech.controllers.sales_and_purchase_return import get_returned_serial_nos
...
def get_reserved_serial_nos_for_sre(kwargs) -> list:
	...
	sb_entry = artech_engine.qb.DocType("Serial and Batch Entry")
	query = (
		artech_engine.qb.from_(sre)
		.inner_join(sb_entry)
...
def get_reserved_batches_for_pos(kwargs) -> dict:
	...
	pos_batches = artech_engine._dict()
	POS_Invoice = artech_engine.qb.DocType("POS Invoice")
	POS_Invoice_Item = artech_engine.qb.DocType("POS Invoice Item")

	pos_invoices = (
		artech_engine.qb.from_(POS_Invoice)
	...
	if ids:
		for d in get_serial_batch_ledgers(kwargs.item_code, docstatus=1, name=ids):
			...
			if key not in pos_batches:
				pos_batches[key] = artech_engine._dict(
					{
						"qty": d.qty,
...
def get_reserved_batches_for_sre(kwargs) -> dict:
	...
	reserved_batches_details = artech_engine._dict()
	if data:
		reserved_batches_details = artech_engine._dict(
			{(d.batch_no, d.warehouse): artech_engine._dict({"warehouse": d.warehouse, "qty": d.qty}) for d in data}
		)
...
def get_auto_batch_nos(kwargs):
	...
	picked_batches = artech_engine._dict()
	...
	if not kwargs.consider_negative_batches:
		precision = artech_engine.get_precision("Stock Ledger Entry", "actual_qty")
...
def get_batch_nos_from_sre(kwargs):
	...
	query = (
		...
		.orderby(child_table.batch_no, order=artech_engine.query_builder.Order.asc)
	)
...
def get_batches_to_be_considered(sales_order_name):
	parent = artech_engine.qb.DocType("Stock Reservation Entry")
	child = artech_engine.qb.DocType("Serial and Batch Entry")
...
def get_available_batches(kwargs):
	stock_ledger_entry = artech_engine.qb.DocType("Stock Ledger Entry")
	batch_ledger = artech_engine.qb.DocType("Serial and Batch Entry")
	batch_table = artech_engine.qb.DocType("Batch")

	query = (
		artech_engine.qb.from_(stock_ledger_entry)
	...
	if kwargs.based_on == "LIFO":
		query = query.orderby(batch_table.creation, order=artech_engine.qb.desc)
	elif kwargs.based_on == "Expiry":
...
def get_voucher_wise_serial_batch_from_bundle(**kwargs) -> dict[str, dict]:
	...
	for row in data:
		...
		if kwargs.get("get_subcontracted_item"):
			...
			subcontracted_item_code = artech_engine.get_cached_value(doctype, row.voucher_detail_no, field_name)
		...
		if key not in group_by_voucher:
			group_by_voucher.setdefault(
				key,
				artech_engine._dict({"serial_nos": [], "batch_nos": defaultdict(float), "item_row": row}),
			)
...
def get_picked_batches(kwargs) -> dict[str, dict]:
	...
	table = artech_engine.qb.DocType("Serial and Batch Bundle")
	child_table = artech_engine.qb.DocType("Serial and Batch Entry")
	pick_list_table = artech_engine.qb.DocType("Pick List")
	...
	for row in data:
		...
		if key not in picked_batches:
			picked_batches[key] = artech_engine._dict(
				{
					"qty": row.qty,
					"warehouse": row.warehouse,
				}
...
def get_ledgers_from_serial_batch_bundle(**kwargs) -> list[artech_engine._dict]:
	bundle_table = artech_engine.qb.DocType("Serial and Batch Bundle")
	serial_batch_table = artech_engine.qb.DocType("Serial and Batch Entry")

	query = (
		artech_engine.qb.from_(bundle_table)
		.inner_join(serial_batch_table)
...
def get_stock_ledgers_for_serial_nos(kwargs):
	...
	stock_ledger_entry = artech_engine.qb.DocType("Stock Ledger Entry")
	serial_batch_entry = artech_engine.qb.DocType("Serial and Batch Entry")
...
def get_batch_no_from_serial_no(serial_no: str):
	return artech_engine.get_cached_value("Serial No", serial_no, "batch_no")
...
def is_serial_batch_no_exists(
	...
	if serial_no and not artech_engine.db.exists("Serial No", serial_no):
		if type_of_transaction != "Inward":
			artech_engine.throw(_("Serial No {0} does not exists").format(serial_no))

		make_serial_no(serial_no, item_code)

	if batch_no and not artech_engine.db.exists("Batch", batch_no):
		if type_of_transaction != "Inward":
			artech_engine.throw(_("Batch No {0} does not exists").format(batch_no))

		make_batch_no(batch_no, item_code)


def make_serial_no(serial_no, item_code):
	serial_no_doc = artech_engine.new_doc("Serial No")
...
def make_batch_no(batch_no, item_code):
	batch_doc = artech_engine.new_doc("Batch")
...
def is_duplicate_serial_no(bundle_id: str, serial_no: str):
	return artech_engine.db.exists("Serial and Batch Entry", {"parent": bundle_id, "serial_no": serial_no})
