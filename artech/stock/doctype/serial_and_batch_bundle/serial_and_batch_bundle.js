...
artech_engine.ui.form.on("Serial and Batch Bundle", {
	setup(frm) {
		frm.trigger("set_queries");
	},

	before_submit(frm) {
		artech_engine.throw(__("The user cannot submit the Serial and Batch Bundle manually"));
	},
	...
	validate_prompt_data(frm, prompt_data) {
		if (prompt_data.using_csv_file && !prompt_data.csv_file) {
			artech_engine.throw(__("Please attach CSV file"));
		}
	...
	toggle_fields(frm) {
		let show_naming_series_field =
			artech_engine.user_defaults.set_serial_and_batch_bundle_naming_based_on_naming_series;
		...
		if (frm.doc.has_serial_no) {
			frm.doc.entries.forEach((row) => {
				if (Math.abs(row.qty) !== 1) {
					artech_engine.model.set_value(row.doctype, row.name, "qty", 1);
				}
			});
		}
	...
	set_queries(frm) {
		frm.set_query("item_code", () => {
			return {
				query: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.item_query",
			};
		});
...
artech_engine.ui.form.on("Serial and Batch Entry", {
	entries_add(frm, cdt, cdn) {
		if (frm.doc.warehouse) {
			artech_engine.model.set_value(cdt, cdn, "warehouse", frm.doc.warehouse);
		}
	},
});
