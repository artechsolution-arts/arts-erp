...
from artech_engine.utils import flt, nowtime, today
...
class TestSerialandBatchBundle(ArtechTestSuite):
	...
	def test_old_serial_no_valuation(self):
		...
		if not artech_engine.db.exists("Serial No", serial_no_id):
	...
	def test_batch_not_belong_to_serial_no(self):
		...
		self.assertRaises(artech_engine.exceptions.ValidationError, doc.save)
	...
	def test_auto_delete_draft_serial_and_batch_bundle(self):
		...
		ste.delete()
		self.assertFalse(artech_engine.db.exists("Serial and Batch Bundle", bundle_doc.name))
	...
	def test_serial_and_batch_bundle_company(self):
		...
		for serial_no in ["TT-SER-VAL-00001", "TT-SER-VAL-00002", "TT-SER-VAL-00003"]:
			entries.append(artech_engine._dict({"serial_no": serial_no, "qty": 1}))
	...
	def test_auto_cancel_serial_and_batch(self):
		...
		docstatus = artech_engine.db.get_value("Serial and Batch Bundle", bundle, "docstatus")
	...
	def test_batch_duplicate_entry(self):
		...
		self.assertTrue(artech_engine.db.exists("Batch", batch_id))
		use_batchwise_valuation = artech_engine.db.get_value("Batch", batch_id, "use_batchwise_valuation")
	...
	def test_serial_no_duplicate_entry(self):
		...
		self.assertTrue(artech_engine.db.exists("Serial No", serial_no_id))
	...
	def test_duplicate_serial_and_batch_bundle(self):
		...
		self.assertRaises(artech_engine.exceptions.ValidationError, pr2.save)
	...
	def test_auto_create_serial_and_batch_bundle_for_outward_for_batch_item(self):
		...
		if not artech_engine.db.exists("Batch", "ACSBBO-TACSB-00001"):
			artech_engine.get_doc(
				{
					"doctype": "Batch",
		...
		self.assertRaises(artech_engine.ValidationError, dispatch.submit)
	...
	def test_voucher_detail_no(self):
		...
		if not artech_engine.db.exists("Batch", "TST-ACSBBO-TACSB-00001"):
			artech_engine.get_doc(
				{
					"doctype": "Batch",
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"posting_time": nowtime(),
				"qty": 10,
				"batches": artech_engine._dict({"TST-ACSBBO-TACSB-00001": 10}),
				"type_of_transaction": "Inward",
				"do_not_submit": True,
			}
		)
		...
		bundle_doc = artech_engine.get_doc("Serial and Batch Bundle", bundle_doc.name)
		...
		self.assertRaises(artech_engine.ValidationError, bundle_doc.submit)
	...
	def test_reference_voucher_on_cancel(self):
		...
		serial_no = get_serial_nos_from_bundle(se.items[0].serial_and_batch_bundle)[0]
		self.assertEqual(artech_engine.get_value("Serial No", serial_no, "reference_name"), se.name)

		se.cancel()
		self.assertIsNone(artech_engine.get_value("Serial No", serial_no, "reference_name"))

		se1 = artech_engine.copy_doc(se, ignore_no_copy=False)
		se1.items[0].serial_no = serial_no
		se1.submit()

		self.assertEqual(artech_engine.get_value("Serial No", serial_no, "reference_name"), se1.name)
	...
	def test_stock_queue_for_return_entry_with_non_batchwise_valuation(self):
		...
		return_sle = artech_engine.db.get_value(
			"Stock Ledger Entry",
