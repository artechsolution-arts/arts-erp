...
from artech.stock.doctype.item.test_item import (
	create_item,
	make_item,
	make_item_variant,
	set_item_variant_settings,
)
from artech.stock.doctype.material_request.material_request import (
	make_in_transit_stock_entry,
)
from artech.stock.doctype.material_request.test_material_request import (
...
from artech.stock.doctype.serial_no.serial_no import *
from artech.stock.doctype.stock_entry.stock_entry import FinishedGoodError, make_stock_in_entry
from artech.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from artech.stock.doctype.stock_ledger_entry.stock_ledger_entry import StockFreezeError
...
from artech.stock.stock_ledger import NegativeStockError, get_previous_sle
...
class TestStockEntry(ArtechTestSuite):
	def setUp(self):
		self.load_test_records("Stock Entry")
		artech_engine.local.flags.dont_execute_stock_reposts = False
	...
	def test_fifo(self):
		...
		self.assertEqual([[1, 10]], artech_engine.safe_eval(sle.stock_queue))
		...
		self.assertEqual([[-2, 10]], artech_engine.safe_eval(sle.stock_queue))

		# move stock to positive
		make_stock_entry(item_code=item_code, target=warehouse, qty=3, basic_rate=20)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]
		self.assertEqual([[1, 20]], artech_engine.safe_eval(sle.stock_queue))

		# incoming entry with diff rate
		make_stock_entry(item_code=item_code, target=warehouse, qty=1, basic_rate=30)
		sle = get_sle(item_code=item_code, warehouse=warehouse)[0]

		self.assertEqual([[1, 20], [1, 30]], artech_engine.safe_eval(sle.stock_queue))

		artech_engine.db.set_default("allow_negative_stock", 0)
	...
	def test_auto_material_request_for_variant(self):
		...
		make_item_variant()
		template = artech_engine.get_doc("Item", "_Test Variant Item")
	...
	def _test_auto_material_request(
		...
		variant = artech_engine.get_doc("Item", item_code)

		projected_qty, actual_qty = artech_engine.db.get_value(
		...
		projected_qty = (
			artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "projected_qty") or 0
		)

		artech_engine.db.set_single_value("Stock Settings", "auto_indent", 1)
		...
		from artech.stock.reorder_item import reorder_item

		mr_list = reorder_item()

		artech_engine.db.set_single_value("Stock Settings", "auto_indent", 0)
	...
	def test_material_receipt_gl_entry(self):
		company = artech_engine.db.get_value("Warehouse", "Stores - TCP1", "company")
	...
	def test_repack_multiple_fg(self):
		...
		repack = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][3])
	...
	def test_repack_with_additional_costs(self):
		...
		default_expense_account = artech_engine.get_value("Company", company, "default_expense_account")
	...
	def test_serial_no_not_reqd(self):
		se = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][0])
		...
		self.assertRaises(artech_engine.ValidationError, bundle_id.make_serial_and_batch_bundle)
	...
	def test_serial_no_transfer_in(self):
		...
		for serial_no in serial_nos:
			if not artech_engine.db.exists("Serial No", serial_no):
				doc = artech_engine.new_doc("Serial No")
		...
		self.assertTrue(artech_engine.db.get_value("Serial No", "ABCD1", "warehouse"))
		self.assertTrue(artech_engine.db.get_value("Serial No", "EFGH1", "warehouse"))

		se.cancel()
		self.assertFalse(artech_engine.db.get_value("Serial No", "ABCD1", "warehouse"))
	...
	def test_serial_by_series(self):
		...
		self.assertTrue(artech_engine.db.exists("Serial No", serial_nos[0]))
		self.assertTrue(artech_engine.db.exists("Serial No", serial_nos[1]))
	...
	def test_serial_move(self):
		...
		artech_engine.flags.use_serial_and_batch_fields = True
		...
		se.insert()
		se.submit()
		self.assertTrue(artech_engine.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC")

		se.cancel()
		self.assertTrue(artech_engine.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC")
		artech_engine.flags.use_serial_and_batch_fields = False
	...
	def test_serial_cancel(self):
		...
		se.cancel()
		self.assertFalse(artech_engine.db.get_value("Serial No", serial_no, "warehouse"))
	...
	def test_serial_batch_item_stock_entry(self):
		...
		item = artech_engine.db.exists("Item", {"item_name": "Batched and Serialised Item"})
		if not item:
			...
			item = artech_engine.get_doc("Item", {"item_name": "Batched and Serialised Item"})
		...
		batch_in_serial_no = artech_engine.db.get_value("Serial No", serial_no, "batch_no")
		...
		batch_in_serial_no = artech_engine.db.get_value("Serial No", serial_no, "batch_no")
		self.assertEqual(artech_engine.db.get_value("Serial No", serial_no, "warehouse"), None)

	def test_warehouse_company_validation(self):
		artech_engine.db.get_value("Warehouse", "_Test Warehouse 2 - _TC1", "company")
		artech_engine.get_doc("User", "test2@example.com").add_roles(
			"Sales User", "Sales Manager", "Stock User", "Stock Manager"
		)
		with self.set_user("test2@example.com"):
			from artech.stock.utils import InvalidWarehouseCompany

			st1 = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][0])
	...
	def test_warehouse_user(self):
		...
		test_user = artech_engine.get_doc("User", "test@example.com")
		...
		with self.set_user("test@example.com"):
			st1.get("items")[0].t_warehouse = "_Test Warehouse 2 - _TC1"
			self.assertRaises(artech_engine.PermissionError, st1.insert)
	...
	def test_freeze_stocks(self):
		artech_engine.db.set_single_value("Stock Settings", "stock_auth_role", "")

		# test freeze_stocks_upto
		artech_engine.db.set_single_value("Stock Settings", "stock_frozen_upto", add_days(nowdate(), 5))
		se = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][0]).insert()
		self.assertRaises(StockFreezeError, se.submit)

		artech_engine.db.set_single_value("Stock Settings", "stock_frozen_upto", "")

		# test freeze_stocks_upto_days
		artech_engine.db.set_single_value("Stock Settings", "stock_frozen_upto_days", -1)
		se = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][0])
		...
		se.insert()
		self.assertRaises(StockFreezeError, se.submit)
		artech_engine.db.set_single_value("Stock Settings", "stock_frozen_upto_days", 0)
	...
	def test_work_order(self):
		...
		bom_no, bom_operation_cost = artech_engine.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "is_default": 1, "docstatus": 1}, ["name", "operating_cost"]
		)

		work_order = artech_engine.new_doc("Work Order")
		work_order.update(
			{
				"company": "_Test Company",
	...
	@ArtechTestSuite.change_settings("Manufacturing Settings", {"material_consumption": 1})
	def test_work_order_manufacture_with_material_consumption(self):
		from artech.manufacturing.doctype.work_order.work_order import (
			make_stock_entry as _make_stock_entry,
		)

		bom_no = artech_engine.db.get_value("BOM", {"item": "_Test FG Item", "is_default": 1, "docstatus": 1})
		...
		s = artech_engine.get_doc(_make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		...
		s = artech_engine.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		s.save()
		rm_cost = 0
		for d in s.get("items"):
			if d.s_warehouse:
	...
	def test_variant_work_order(self):
		bom_no = artech_engine.db.get_value("BOM", {"item": "_Test Variant Item", "is_default": 1, "docstatus": 1})
		...
		from artech.manufacturing.doctype.work_order.work_order import make_stock_entry

		stock_entry = artech_engine.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		stock_entry.insert()
	...
	def test_nagative_stock_for_batch(self):
		...
		ste = artech_engine.new_doc("Stock Entry")
		...
		self.assertRaises(artech_engine.ValidationError, ste.submit)
	...
	def test_quality_check_for_secondary_item(self):
		...
		bom_name = artech_engine.db.get_value("BOM Secondary Item", {"docstatus": 1}, "parent")
		production_item = artech_engine.db.get_value("BOM", bom_name, "item")
		...
		work_order.update(
			{
				...
				"bom_no": bom_name,
				"qty": 1.0,
				"stock_uom": artech_engine.db.get_value("Item", production_item, "stock_uom"),
				"skip_transfer": 1,
			}
		)
		...
		stock_entry = artech_engine.get_doc(_make_stock_entry(work_order.name, "Manufacture", 1))
		for row in stock_entry.items:
			...
			if row.type or row.is_legacy_scrap_item:
				...
				row.uom = artech_engine.db.get_value("Item", secondary_item, "stock_uom")
				row.stock_uom = artech_engine.db.get_value("Item", secondary_item, "stock_uom")
	...
	def test_quality_check(self):
		...
		repack.insert()
		self.assertRaises(artech_engine.ValidationError, repack.submit)
	...
	def test_zero_incoming_rate(self):
		...
		value_diff = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = artech_engine.db.get_value(
			"GL Entry",
	...
	def test_conversion_factor_change(self):
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", 1)
		repack_entry = artech_engine.copy_doc(self.globalTestRecords["Stock Entry"][3])
	...
	def test_additional_cost_distribution_manufacture(self):
		se = artech_engine.get_doc(
			doctype="Stock Entry",
			purpose="Manufacture",
			additional_costs=[artech_engine._dict(base_amount=100)],
			items=[
				artech_engine._dict(item_code="RM", basic_amount=10),
				artech_engine._dict(item_code="FG", basic_amount=20, t_warehouse="X", is_finished_item=1),
				artech_engine._dict(item_code="scrap", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_additional_cost_distribution_non_manufacture(self):
		se = artech_engine.get_doc(
			...
			additional_costs=[artech_engine._dict(base_amount=100)],
			items=[
				artech_engine._dict(item_code="RECEIVED_1", basic_amount=20, t_warehouse="X"),
				artech_engine._dict(item_code="RECEIVED_2", basic_amount=30, t_warehouse="X"),
			],
		)
	...
	def test_independent_manufacture_entry(self):
		...
		se = artech_engine.get_doc(
			...
			items=[
				...
				artech_engine._dict(item_code="_Test FG Item", qty=4, t_warehouse="_Test Warehouse 1 - _TC"),
			],
		)
		artech_engine.db.set_value("Company", "_Test Company", "stock_adjustment_account", "Stock Adjustment - _TC")
	...
	def test_future_negative_sle_batch(self):
		from artech.stock.doctype.batch.test_batch import TestBatch
		...
		with self.assertRaises(artech_engine.ValidationError):
			make_stock_entry(
				item_code=item_code,
				qty=1,
	...
	def test_multi_batch_value_diff(self):
		...
		receipt.append(
			"items", artech_engine.copy_doc(receipt.items[0], ignore_no_copy=False).update({"basic_rate": 20})
		)
		receipt.save()
		receipt.submit()
		receipt.load_from_db()

		batches = artech_engine._dict(
	...
	def test_transfer_qty_validation(self):
		...
		self.assertRaises(artech_engine.ValidationError, se.save)

	def test_mapped_stock_entry(self):
		"Check if rate and stock details are populated in mapped SE given warehouse."
		from artech.stock.doctype.purchase_receipt.purchase_receipt import make_stock_entry
	...
	@ArtechTestSuite.change_settings("Stock Reposting Settings", {"item_based_reposting": 0})
	def test_reposting_for_depedent_warehouse(self):
		from artech.stock.doctype.repost_item_valuation.repost_item_valuation import repost_sl_entries
		from artech.stock.doctype.warehouse.test_warehouse import create_warehouse
		...
		repost_name = artech_engine.db.get_value(
			"Repost Item Valuation", {"voucher_no": receipt2.name, "docstatus": 1}, "name"
		)

		doc = artech_engine.get_doc("Repost Item Valuation", repost_name)
	...
	def test_batch_expiry(self):
		from artech.controllers.stock_controller import BatchExpiredError
		from artech.stock.doctype.batch.test_batch import make_new_batch
		...
		batch = make_new_batch(
			batch_id=artech_engine.generate_hash("", 5), item_code=item_doc.name, expiry_date=add_days(today(), -1)
		)
	...
	def test_negative_stock_reco(self):
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", 0)
		...
		self.assertRaises(artech_engine.ValidationError, sr_doc.submit)
	...
	def test_negative_batch(self):
		...
		self.assertRaises(artech_engine.ValidationError, se1.cancel)
	...
	def test_auto_reorder_level(self):
		...
		if not artech_engine.db.exists("Item Reorder", {"parent": item_doc.name}):
			item_doc.append(
				"reorder_levels",
				{
	...
	def test_use_serial_and_batch_fields(self):
		...
		for serial_no in serial_nos:
			self.assertTrue(artech_engine.db.exists("Serial No", serial_no))
			self.assertEqual(artech_engine.db.get_value("Serial No", serial_no, "status"), "Active")
		...
		for serial_no in serial_nos:
			self.assertTrue(artech_engine.db.exists("Serial No", serial_no))
			self.assertEqual(artech_engine.db.get_value("Serial No", serial_no, "status"), "Consumed")
	...
	def test_serial_batch_bundle_type_of_transaction(self):
		...
		artech_engine.db.set_value("Serial and Batch Bundle", sbb, "type_of_transaction", "Inward")
		self.assertRaises(artech_engine.ValidationError, se.submit)
	...
	def test_use_batch_wise_valuation_for_moving_average_item(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "do_not_use_batchwise_valuation", 0)
	...
	def test_periodic_accounting_entries(self):
		...
		warehouse = artech_engine.db.get_value("Warehouse", {"company": company, "is_group": 0}, "name")
	...
	def test_prevent_reuse_delivered_serial_no_in_repack(self):
		...
		se.save()
		status = artech_engine.db.get_value("Serial No", delivered_serial_no, "status")
	...
	def test_transferred_qty_in_material_transfer(self):
		...
		if not artech_engine.db.get_value("UOM Conversion Detail", {"parent": item_code, "uom": "Box"}):
			item_doc = artech_engine.get_doc("Item", item_code)
	...
	def test_sample_retention_stock_entry(self):
		from artech.stock.doctype.stock_entry.stock_entry import move_sample_to_retention_warehouse
		...
		source_sabb = artech_engine.get_doc(
		...
		sample_entry = artech_engine.get_doc(
			move_sample_to_retention_warehouse(material_receipt.company, material_receipt.items)
		)
		sample_entry.submit()
		target_sabb = artech_engine.get_doc("Serial and Batch Bundle", sample_entry.items[0].serial_and_batch_bundle)
	...
	@ArtechTestSuite.change_settings("Manufacturing Settings", {"material_consumption": 0})
	...
	def test_validation_as_per_bom_with_continuous_raw_material_consumption(self):
		from artech.manufacturing.doctype.production_plan.test_production_plan import make_bom
		from artech.manufacturing.doctype.work_order.work_order import make_stock_entry as _make_stock_entry
		from artech.manufacturing.doctype.work_order.work_order import make_work_order
		...
		artech_engine.get_doc(_make_stock_entry(work_order.name, "Material Consumption for Manufacture", 5)).submit()
		artech_engine.get_doc(_make_stock_entry(work_order.name, "Manufacture", 5)).submit()
	...
	def test_qi_creation_with_naming_rule_company_condition(self):
		...
		"""
		from artech.controllers.stock_controller import make_quality_inspections
		from artech.stock.doctype.warehouse.test_warehouse import create_warehouse
		...
		qc_naming_rule = artech_engine.new_doc(
		...
		# delete naming rule
		artech_engine.delete_doc("Document Naming Rule", qc_naming_rule.name)

	def test_co_by_product(self):
		from artech.manufacturing.doctype.production_plan.test_production_plan import make_bom

		artech_engine.set_value("UOM", "Nos", "must_be_whole_number", 0)
...
def initialize_records_for_future_negative_sle_test(
	...
	from artech.stock.doctype.batch.test_batch import TestBatch, make_new_batch
	from artech.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
		create_stock_reconciliation,
	)
	from artech.stock.doctype.warehouse.test_warehouse import create_warehouse
