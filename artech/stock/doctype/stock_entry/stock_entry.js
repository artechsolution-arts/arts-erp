...
artech.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");

artech_engine.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		frm.set_query("source_warehouse_address", function () {
			return {
				query: "artech.controllers.queries.get_warehouse_address",
				filters: {
		...
		frm.set_query("batch_no", "items", function (doc, cdt, cdn) {
			...
			if (!item.item_code) {
				artech_engine.throw(__("Please enter Item Code to get Batch Number"));
			} else {
				if (
					[
					...
					filters = {
						...
						posting_date: frm.doc.posting_date || artech_engine.datetime.nowdate(),
					};
				} else {
					filters = {
						item_code: item.item_code,
		...
		frm.set_query("project", "items", function (doc) {
			return {
				query: "artech.controllers.queries.get_project_name",
				filters: {
					company: doc.company,
				},
			};
		...
		artech_engine.db.get_single_value("Stock Settings", "disable_serial_no_and_batch_selector").then((value) => {
			if (value) {
		...
		if (frm.doc.purpose == "Receive from Customer") {
			frm.set_query("against_fg", "items", function () {
				return {
					query: "artech.controllers.subcontracting_inward_controller.get_fg_reference_names",
					filters: {
	...
	setup_quality_inspection: function (frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			frm.add_custom_button(
				...
				() => {
					let transaction_controller = new artech.TransactionController({ frm: frm });
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "artech.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	source_stock_entry: async function (frm) {
		...
		let available_qty = await artech_engine.xcall(
			"artech.manufacturing.doctype.work_order.work_order.get_disassembly_available_qty",
	...
	refresh: function (frm) {
		...
		artech.toggle_serial_batch_fields(frm);
		...
		if (!frm.doc.docstatus && !frm.doc.subcontracting_inward_order) {
			...
			frm.add_custom_button(
				__("Material Request"),
				function () {
					artech_engine.model.with_doctype("Material Request", function () {
						var mr = artech_engine.model.get_new_doc("Material Request");
						...
						items.forEach(function (item) {
							var mr_item = artech_engine.model.add_child(mr, "items");
							...
							mr_item.required_date = artech_engine.datetime.nowdate();
						});
						artech_engine.set_route("Form", "Material Request", mr.name);
					});
				},
				__("Create")
			);
		...
		if (frm.doc.items) {
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__("Alternate Item"), () => {
					artech.utils.select_alternate_items({
						frm: frm,
						child_docname: "items",
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					artech_engine.model.open_mapped_doc({
						method: "artech.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
			...
			if (frm.doc.per_transferred > 0) {
				frm.add_custom_button(
					...
					function () {
						...
						artech_engine.set_route("List", "Stock Entry");
					},
					__("View")
				);
			}
						...
						artech_engine.prompt(
							...
							async (data) => {
								if (frm.doc.work_order) {
									let stock_entry = await artech_engine.xcall(
										"artech.manufacturing.doctype.work_order.work_order.make_stock_entry",
										{
									...
									if (stock_entry) {
										artech_engine.model.sync(stock_entry);
										artech_engine.set_route("Form", stock_entry.doctype, stock_entry.name);
									}
								} else {
									let se = artech_engine.model.get_new_doc("Stock Entry");
									...
									artech_engine.set_route("Form", "Stock Entry", se.name);
								}
							},
							__("Disassemble"),
							__("Create")
		...
		if (frm.doc.docstatus === 0 && !frm.doc.subcontracting_inward_order) {
			frm.add_custom_button(
				...
				function () {
					artech.utils.map_current_doc({
						method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = artech.utils.map_current_doc({
						method: "artech.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.call({
						method: "artech.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	cost_center(frm, cdt, cdn) {
		artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	validate_purpose_consumption: function (frm) {
		artech_engine
			.call({
				method: "artech.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
				if (cint(r.message) == 0 && frm.doc.purpose == "Material Consumption for Manufacture") {
					frm.set_value("purpose", "Manufacture");
					artech_engine.throw(__("Material Consumption is not set in Manufacturing Settings."));
				}
			});
	},
	...
	set_basic_rate: function (frm, cdt, cdn) {
		...
		if (item.item_code || item.serial_no) {
			artech_engine.call({
				...
				callback: function (r) {
					artech_engine.model.set_value(cdt, cdn, "basic_rate", r.message || 0.0);
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			artech_engine.call({
				method: "artech.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
				...
				callback: function (r) {
					if (!r.exc) {
						...
						fields.forEach((field) => {
							artech_engine.model.set_value(cdt, cdn, field, r.message[field] || 0.0);
						});
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			if (!frm.doc.company) artech_engine.throw(__("Company field is required"));
			artech_engine.call({
				method: "artech.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						artech_engine.throw(__("BOM does not contain any stock item"));
					} else {
						artech.utils.remove_empty_first_row(frm, "items");
						$.each(r.message, function (i, item) {
							let d = artech_engine.model.add_child(cur_frm.doc, "Stock Entry Detail", "items");
	...
	calculate_total_additional_costs: function (frm) {
		const total_additional_costs = artech_engine.utils.sum(
	...
	source_warehouse_address: function (frm) {
		artech.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		artech.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	set_transit_warehouse: function (frm) {
		if (
			...
			artech_engine.db.get_value(dt, dn, "default_in_transit_warehouse", (r) => {
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) artech.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			artech.utils.map_current_doc({
				method: "artech.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
artech_engine.ui.form.on("Stock Entry Detail", {
	...
	s_warehouse(frm, cdt, cdn) {
		...
		let item = artech_engine.get_doc(cdt, cdn);
		if (item.s_warehouse) {
			artech_engine.model.set_value(cdt, cdn, "allow_zero_valuation_rate", 0);
		}
	},
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return artech_engine.call({
				method: "artech.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						artech_engine.model.set_value(cdt, cdn, r.message);
					}
				},
			});
		}
					...
					if (r.message) {
						...
						$.each(r.message, function (k, v) {
							if (v) {
								...
								if (k != "barcode") {
									artech_engine.model.set_value(cdt, cdn, k, v); // qty and it's subsequent fields weren't triggered
								} else {
									d.barcode = v;
								}
							}
						...
						if (no_batch_serial_number_value && !artech_engine.flags.hide_serial_batch_dialog) {
							if (!artech_engine.flags.dialog_set) {
								artech_engine.flags.dialog_set = true;
							}
							artech.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		artech.stock.select_batch_and_serial_no(frm, child);
	},
});
...
artech_engine.ui.form.on("Landed Cost Taxes and Charges", {
...
artech.stock.StockEntry = class StockEntry extends artech.stock.StockController {
	setup() {
		var me = this;

		this.barcode_scanner = new artech.utils.BarcodeScanner({
			frm: this.frm,
			warehouse_field: (doc) => {
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return artech.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (artech.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		artech_engine.dynamic_link = { doc: this.frm.doc, fieldname: "supplier", doctype: "Supplier" };
		this.frm.set_query("supplier_address", artech.queries.address_query);
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 1 && artech.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		artech.hide_company(this.frm);
		artech.utils.add_item(this.frm);
	...
	company() {
		if (this.frm.doc.company) {
			var company_doc = artech_engine.get_doc(":Company", this.frm.doc.company);
			if (company_doc.default_letter_head) {
				this.frm.set_value("letter_head", company_doc.default_letter_head);
			}
			this.frm.trigger("toggle_display_account_head");

			artech.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "artech.accounts.utils.get_company_default",
			args: {
	...
	clean_up() {
		...
		if (
			...
			artech_engine.model.remove_from_locals("Work Order", this.frm.doc.work_order);
		}
	}
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						artech.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return artech_engine.call({
			method: "artech.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	add_excise_button() {
		if (artech_engine.boot.sysdefaults.country === "India")
			this.frm.add_custom_button(
				__("Excise Invoice"),
				function () {
					var excise = artech_engine.model.make_new_doc_and_get_name("Journal Entry");
					excise = locals["Journal Entry"][excise];
					excise.voucher_type = "Excise Entry";
					artech_engine.set_route("Form", "Journal Entry", excise.name);
				},
				__("Create")
			);
	}
	...
	items_add(doc, cdt, cdn) {
		var row = artech_engine.get_doc(cdt, cdn);
		...
		if (cint(artech_engine.user_defaults?.use_serial_batch_fields)) {
			artech_engine.model.set_value(row.doctype, row.name, "use_serial_batch_fields", 1);
		}
	}
	...
	set_warehouse_in_children(child_table, warehouse_field, warehouse) {
		let transaction_controller = new artech.TransactionController();
		transaction_controller.autofill_warehouse(child_table, warehouse_field, warehouse);
	}

	items_on_form_rendered(doc, grid_row) {
		artech.setup_serial_or_batch_no();
	}
	...
	toggle_related_fields(doc) {
		...
		if (artech.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		artech.utils.get_party_details(this.frm, null, null, null);
	}
};

artech.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/artech/js/utils/serial_no_batch_selector.js";

	artech_engine.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
		if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
			item.has_serial_no = r.message.has_serial_no;
			item.has_batch_no = r.message.has_batch_no;
			item.type_of_transaction = item.s_warehouse ? "Outward" : "Inward";

			new artech.SerialBatchPackageSelector(frm, item, (r) => {
				if (r) {
					artech_engine.model.set_value(item.doctype, item.name, {
...
artech.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	artech_engine.db.get_doc("BOM", bom_no).then((bom) => {
		const { name, items } = bom;
		artech.stock.bom = { name, items: {} };
		items.forEach((item) => {
			artech.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (artech.stock.bom && artech.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new artech.stock.StockEntry({ frm: cur_frm }));
