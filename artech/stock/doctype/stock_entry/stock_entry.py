...
from artech_engine import _, bold
...
import artech
from artech.accounts.general_ledger import process_gl_map
from artech.accounts.utils import get_account_currency
from artech.buying.utils import check_on_hold_or_closed_status
from artech.controllers.taxes_and_totals import init_landed_taxes_and_totals
from artech.manufacturing.doctype.bom.bom import (
	add_additional_cost,
	get_op_cost_from_sub_assemblies,
	get_secondary_items_from_sub_assemblies,
	validate_bom_no,
)
from artech.setup.doctype.brand.brand import get_brand_defaults
from artech.setup.doctype.item_group.item_group import get_item_group_defaults
from artech.stock.doctype.batch.batch import get_batch_qty
from artech.stock.doctype.item.item import get_item_defaults
from artech.stock.doctype.serial_no.serial_no import get_serial_nos
from artech.stock.doctype.stock_reconciliation.stock_reconciliation import (
...
from artech.stock.serial_batch_bundle import (
	SerialBatchCreation,
	get_empty_batches_based_work_order,
	get_serial_or_batch_items,
)
from artech.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from artech.stock.utils import get_bin, get_combine_datetime, get_incoming_rate


class FinishedGoodError(artech_engine.ValidationError):
	pass


class IncorrectValuationRateError(artech_engine.ValidationError):
	pass


class DuplicateEntryForWorkOrderError(artech_engine.ValidationError):
	pass


class OperationsNotCompleteError(artech_engine.ValidationError):
	pass


class MaxSampleAlreadyRetainedError(artech_engine.ValidationError):
	pass


from artech.controllers.stock_controller import StockController
from artech.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from artech.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def __init__(self, *args, **kwargs):
		...
		if self.purchase_order:
			self.subcontract_data = artech_engine._dict(
				{
	...
	def before_insert(self):
		if self.subcontracting_order and artech_engine.get_cached_value(
			"Subcontracting Order", self.subcontracting_order, "reserve_stock"
		):
			self.set_serial_batch_from_reserved_entry()

	def before_validate(self):
		from artech.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate(self):
		self.pro_doc = artech_engine._dict()
		if self.work_order:
			self.pro_doc = artech_engine.get_doc("Work Order", self.work_order)
	...
	def validate_raw_materials_exists(self):
		...
		if artech_engine.db.get_single_value("Manufacturing Settings", "material_consumption"):
			return
	...
	def _set_serial_batch_for_disassembly_from_stock_entry(self):
		from artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import (
			get_voucher_wise_serial_batch_from_bundle,
		)

		source_fg_qty = flt(artech_engine.db.get_value("Stock Entry", self.source_stock_entry, "fg_completed_qty"))
	...
	def set_job_card_data(self):
		if self.job_card and not self.work_order:
			data = artech_engine.db.get_value(
	...
	def validate_job_card_fg_item(self):
		...
		job_card = artech_engine.db.get_value(
	...
	def validate_job_card_item(self):
		...
		if cint(artech_engine.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")):
			return
		...
		for row in self.items:
			...
			artech_engine.throw(_(msg))

	def validate_work_order_status(self):
		pro_doc = artech_engine.get_doc("Work Order", self.work_order)
		if pro_doc.status == "Completed":
			artech_engine.throw(_("Cannot cancel transaction for Completed Work Order."))
	...
	def validate_purpose(self):
		...
		if self.purpose not in valid_purposes:
			artech_engine.throw(_("Purpose must be one of {0}").format(comma_or(valid_purposes)))
	...
	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in artech_engine.get_all(
				...
				artech_engine.delete_doc("Stock Entry", d.name)
	...
	def delink_asset_repair_sabb(self):
		...
		for row in self.items:
			if row.serial_and_batch_bundle:
				voucher_detail_no = artech_engine.db.get_value(
					"Asset Repair Consumed Item",
					{"parent": self.asset_repair, "serial_and_batch_bundle": row.serial_and_batch_bundle},
					"name",
				)

				doc = artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
				doc.db_set(
					{
	...
	def set_transfer_qty(self):
		...
		for item in self.get("items"):
			if not flt(item.conversion_factor):
				artech_engine.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		if self.work_order and not artech_engine.db.get_value(
		...
		for project in projects:
			...
			additional_costs = artech_engine.db.sql(
			...
			project = artech_engine.get_doc("Project", project)
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			...
			if item.item_code not in stock_items:
				artech_engine.throw(_("{0} is not a stock Item").format(item.item_code))
			...
			if self.purpose == "Subcontracting Delivery":
				item.expense_account = artech_engine.get_value("Company", self.company, "default_expense_account")
	...
	def validate_fg_completed_qty(self):
		...
		precision = artech_engine.get_precision("Stock Entry Detail", "qty")
		...
		for d in self.items:
			...
			if fg_completed_qty != (flt(fg_item_qty, precision) + flt(self.process_loss_qty, precision)):
				artech_engine.throw(
					...
					).format(artech_engine.bold(self.process_loss_qty), artech_engine.bold(d.item_code))
				)

	def validate_difference_account(self):
		if not cint(artech.is_perpetual_inventory_enabled(self.company)):
			return
		...
		for d in self.get("items"):
			...
			acc_details = artech_engine.get_cached_value(
				"Account",
				d.expense_account,
		...
		for d in self.get("items"):
			...
			if self.purpose in source_mandatory and not d.s_warehouse:
				if self.from_warehouse:
					...
					artech_engine.throw(_("Source warehouse is mandatory for row {0}").format(d.idx))

			if self.purpose in target_mandatory and not d.t_warehouse:
				if self.to_warehouse:
					d.t_warehouse = self.to_warehouse
				else:
					artech_engine.throw(_("Target warehouse is mandatory for row {0}").format(d.idx))
			...
			if cstr(d.s_warehouse) == cstr(d.t_warehouse) and self.purpose not in [
				...
				artech_engine.throw(_("Source and target warehouse cannot be same for row {0}").format(d.idx))

			if not (d.s_warehouse or d.t_warehouse):
				artech_engine.throw(_("At least one warehouse is mandatory"))
	...
	def validate_work_order(self):
		if self.purpose in (
			...
			if (
				...
				and self.work_order
				and artech_engine.get_cached_value("Work Order", self.work_order, "track_semi_finished_goods") != 1
			):
				if not self.fg_completed_qty:
					artech_engine.throw(_("For Quantity (Manufactured Qty) is mandatory"))
	...
	def validate_source_stock_entry(self):
		...
		if self.work_order:
			source_wo = artech_engine.db.get_value("Stock Entry", self.source_stock_entry, "work_order")
		...
		from artech.manufacturing.doctype.work_order.work_order import get_disassembly_available_qty
	...
	def check_if_operations_completed(self):
		...
		prod_order = artech_engine.get_doc("Work Order", self.work_order)
		allowance_percentage = flt(
			artech_engine.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order")
		)
		...
		for d in prod_order.get("operations"):
			...
			if flt(total_completed_qty, self.precision("fg_completed_qty")) > flt(
				...
				job_card = artech_engine.db.get_value("Job Card", {"operation_id": d.name}, "name")
				if not job_card:
					artech_engine.throw(
				...
				artech_engine.throw(
					_(
						...
						d.idx,
						artech_engine.bold(d.operation),
						artech_engine.bold(total_completed_qty),
						work_order_link,
						job_card_link,
					),
	...
	def check_duplicate_entry_for_work_order(self):
		other_ste = [
			t[0]
			for t in artech_engine.db.get_values(
				"Stock Entry",
				{
		...
		if other_ste:
			production_item, qty = artech_engine.db.get_value(
				"Work Order", self.work_order, ["production_item", "qty"]
			)
			args = [*other_ste, production_item]
			fg_qty_already_entered = artech_engine.db.sql(
	...
	def set_actual_qty(self):
		from artech.stock.stock_ledger import is_negative_stock_allowed
			...
			if (
				...
				artech_engine.throw(
					_(
						...
						d.idx,
						artech_engine.bold(d.s_warehouse),
						formatdate(self.posting_date),
						format_time(self.posting_time),
						artech_engine.bold(d.item_code),
					)
					+ "<br><br>"
					+ _("Available quantity is {0}, you need {1}").format(
						artech_engine.bold(flt(d.actual_qty, d.precision("actual_qty"))), artech_engine.bold(d.transfer_qty)
					),
					NegativeStockError,
	...
	def validate_component_and_quantities(self):
		...
		if not artech_engine.db.get_single_value("Manufacturing Settings", "validate_components_quantities_per_bom"):
			return
			...
			if matched_item := self.get_matched_items(item_code):
				...
				artech_engine.throw(
					_("According to the BOM {0}, the Item '{1}' is missing in the stock entry.").format(
						get_link_to_form("BOM", self.bom_no), artech_engine.bold(item_code)
					),
	...
	def validate_same_source_target_warehouse_during_material_transfer(self):
		...
		Raises:
		artech_engine.ValidationError: If warehouses are same and no inventory dimensions differ
		"""

		if artech_engine.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
	...
	def get_consumed_items(self):
		...
		parent = artech_engine.qb.DocType("Stock Entry")
		child = artech_engine.qb.DocType("Stock Entry Detail")

		query = (
			artech_engine.qb.from_(parent)
			.join(child)
	...
	def set_basic_rate(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		for d in self.get("items"):
			...
			if d.allow_zero_valuation_rate and d.basic_rate and self.purpose != "Receive from Customer":
				...
				if self.bom_no:
					d.basic_rate *= artech_engine.get_value("BOM", self.bom_no, "cost_allocation_per") / 100
			elif d.type and d.bom_secondary_item:
				cost_allocation_per = artech_engine.get_value(
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=artech.get_company_currency(self.company),
					company=self.company,
		...
		if items:
			...
			if len(items) > 1:
				...
				).format(", ".join(artech_engine.bold(item) for item in items))
			else:
				message = _(
					"Item rate has been updated to zero as Allow Zero Valuation Rate is checked for item {0}"
				).format(artech_engine.bold(items[0]))

			artech_engine.msgprint(message, alert=True)
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0) -> float:
		settings = artech_engine.get_single("Manufacturing Settings")
				...
				if artech_engine.db.exists(
					...
					for item in self.items:
						if not item.is_finished_item and not item.type and not item.is_legacy_scrap_item:
							label = artech_engine.get_meta(settings.doctype).get_label(
							...
							artech_engine.throw(
								_(
									...
									item.idx,
									artech_engine.bold(label),
									artech_engine.bold(_("Manufacture")),
									artech_engine.bold(_("Material Consumption for Manufacture")),
								)
							)
					...
					if artech_engine.db.exists(
						...
						artech_engine.throw(
							_("Only one {0} entry can be created against the Work Order {1}").format(
								artech_engine.bold(_("Manufacture")), artech_engine.bold(self.work_order)
							)
						)

					SE = artech_engine.qb.DocType("Stock Entry")
					SE_ITEM = artech_engine.qb.DocType("Stock Entry Detail")

					outgoing_items_cost = (
						artech_engine.qb.from_(SE)
						.left_join(SE_ITEM)
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = artech_engine.get_cached_value(
	...
	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = artech_engine.get_cached_value("Stock Entry Type", self.stock_entry_type, "purpose")
	...
	def make_serial_and_batch_bundle_for_outward(self):
		...
		for row in self.items:
			...
			if row.serial_and_batch_bundle and abs(row.transfer_qty) != abs(
				artech_engine.get_cached_value("Serial and Batch Bundle", row.serial_and_batch_bundle, "total_qty")
			):
				bundle_doc = SerialBatchCreation(
					{
						"item_code": row.item_code,
			...
			elif not row.serial_and_batch_bundle and artech_engine.get_single_value(
	...
	def set_serial_batch_fields_for_subcontracting_inward(self):
		serial_nos, batch_nos = artech_engine._dict(), artech_engine._dict()
		for row in self.items:
			if self.purpose in [
				"Return Raw Material to Customer",
				"Subcontracting Delivery",
	...
	def validate_subcontract_order(self):
		...
		backflush_raw_materials_based_on = artech_engine.db.get_single_value(
			"Buying Settings", "backflush_raw_materials_of_subcontract_based_on"
		)

		qty_allowance = flt(artech_engine.db.get_single_value("Buying Settings", "over_transfer_allowance"))
		...
		if backflush_raw_materials_based_on == "BOM":
			subcontract_order = artech_engine.get_doc(
				self.subcontract_data.order_doctype, self.get(self.subcontract_data.order_field)
			)
			for se_item in self.items:
				item_code = se_item.original_item or se_item.item_code
				precision = cint(artech_engine.db.get_default("float_precision")) or 3
				required_qty = sum(
					[
						flt(d.required_qty)
				...
				if not required_qty:
					...
					if se_item.allow_alternative_item:
						original_item_code = artech_engine.get_value(
				...
				se = artech_engine.qb.DocType("Stock Entry")
				se_detail = artech_engine.qb.DocType("Stock Entry Detail")
				...
				if flt(total_supplied + se_item.transfer_qty - total_returned, precision) > flt(
					...
					order_rm_detail = artech_engine.db.get_value(
				...
				if not row.subcontracted_item:
					artech_engine.throw(
						_("Row {0}: Subcontracted Item is mandatory for the raw material {1}").format(
							row.idx, artech_engine.bold(row.item_code)
						)
					)
	...
	def validate_purchase_order(self):
		if self.purpose == "Send to Subcontractor" and self.get("purchase_order"):
			is_old_subcontracting_flow = artech_engine.db.get_value(
	...
	def get_finished_item(self):
		finished_item = None
		if self.work_order:
			finished_item = artech_engine.db.get_value("Work Order", self.work_order, "production_item")
		elif self.bom_no:
			finished_item = artech_engine.db.get_value("BOM", self.bom_no, "item")
	...
	def validate_finished_goods(self):
		...
		if self.work_order:
			wo_details = artech_engine.db.get_value("Work Order", self.work_order, ["production_item", "qty"])
			if wo_details:
	...
	def validate_serial_batch_bundle_type(self, serial_and_batch_bundle):
		if (
			artech_engine.db.get_value("Serial and Batch Bundle", serial_and_batch_bundle, "type_of_transaction")
			!= "Outward"
		):
			artech_engine.throw(
				_(
		...
		for d in self.get("items"):
			if cstr(d.s_warehouse):
				...
				if sle.serial_and_batch_bundle and self.docstatus == 2:
					bundle_id = artech_engine.get_cached_value(
						"Serial and Batch Bundle",
						{
	...
	def get_gl_entries(self, inventory_account_map):
		...
		return process_gl_map(gl_entries, from_repost=artech_engine.flags.through_repost_item_valuation)
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			...
			if msg:
				artech_engine.throw(_(msg), title=title)

		if self.job_card:
			job_doc = artech_engine.get_doc("Job Card", self.job_card)
			if self.purpose != "Manufacture":
	...
	def reserve_stock_for_subcontracting(self):
		if self.purpose == "Send to Subcontractor" and artech_engine.get_value(
			"Subcontracting Order", self.subcontracting_order, "reserve_stock"
		):
			items = {}
			for item in self.items:
				if item.sco_rm_detail in items:
					...
					items[item.sco_rm_detail] = artech_engine._dict(
						{
			...
			artech_engine.get_doc("Subcontracting Order", self.subcontracting_order).reserve_raw_materials(
	...
	def is_stock_reserve_for_work_order(self):
		if (
			...
			and artech_engine.get_cached_value("Work Order", self.work_order, "reserve_stock")
		):
			return True
	...
	def get_item_details(self, args: ItemDetailsCtx | None = None, for_update: bool = False):
		...
		item_default = artech_engine.qb.DocType("Item Default")
		...
		if not ret.get("expense_account"):
			ret["expense_account"] = artech_engine.get_cached_value(
		...
		for company_field, field in {
			...
			if not ret.get(field):
				ret[field] = artech_engine.get_cached_value("Company", self.company, company_field)
		...
		if (
			...
			subcontract_items = artech_engine.get_all(
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == "Material Transfer":
			doc = artech_engine.get_doc("Stock Entry", self.outgoing_stock_entry)

			if doc.per_transferred == 100:
				artech_engine.throw(_("Goods are already received against the outward entry {0}").format(doc.name))
	...
	def _add_items_for_disassembly_from_stock_entry(self):
		source_fg_qty = artech_engine.db.get_value("Stock Entry", self.source_stock_entry, "fg_completed_qty")
	...
	def _add_items_for_disassembly_from_work_order(self):
		wo_produced_qty = artech_engine.db.get_value("Work Order", self.work_order, "produced_qty")

		wo_produced_qty = flt(wo_produced_qty)
		if wo_produced_qty <= 0:
			artech_engine.throw(_("Work Order {0} has no produced qty").format(self.work_order))

		disassemble_qty = flt(self.fg_completed_qty)
		if disassemble_qty <= 0:
			artech_engine.throw(_("Disassemble Qty cannot be less than or equal to 0."))
	...
	def _add_items_for_disassembly_from_bom(self):
		if not self.bom_no or not self.fg_completed_qty:
			artech_engine.throw(_("BOM and Finished Good Quantity is mandatory for Disassembly"))
		...
		if secondary_items:
			...
			if self.work_order:
				wo_values = artech_engine.db.get_value(
	...
	def get_items_from_manufacture_stock_entry(self):
		SE = artech_engine.qb.DocType("Stock Entry")
		SED = artech_engine.qb.DocType("Stock Entry Detail")
		query = artech_engine.qb.from_(SED).join(SE).on(SED.parent == SE.name).where(SE.docstatus == 1)
	...
	def get_items(self):
		...
		if not self.posting_date or not self.posting_time:
			artech_engine.throw(_("Posting date and posting time is mandatory"))
		...
		if self.bom_no:
			...
			if self.purpose in [
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					and artech_engine.db.get_single_value("Manufacturing Settings", "material_consumption") == 1
				):
					self.get_unconsumed_raw_materials()

				else:
					if not self.fg_completed_qty:
						artech_engine.throw(_("Manufacturing Quantity is mandatory"))
					...
					if (
						...
						# Get Subcontract Order Supplied Items Details
						parent = artech_engine.qb.DocType(self.subcontract_data.order_doctype)
						child = artech_engine.qb.DocType(self.subcontract_data.order_supplied_items_field)
						...
						item_wh = artech_engine._dict(item_wh)
			...
			if self.work_order and self.purpose == "Manufacture":
				work_order = artech_engine.get_doc("Work Order", self.work_order)
	...
	def set_serial_batch_from_reserved_entry(self):
		if self.work_order and artech_engine.get_cached_value("Work Order", self.work_order, "reserve_stock"):
			skip_transfer = artech_engine.get_cached_value("Work Order", self.work_order, "skip_transfer")
				...
				if batches := details.get("batch_no"):
					for batch_no, qty in batches.items():
						...
						if d.batch_no and original_qty > 0:
							new_row = artech_engine.copy_doc(d)
							new_row.name = None
	...
	def get_backflush_based_on(self):
		from artech.manufacturing.doctype.bom.bom import get_backflush_based_on
	...
	def get_available_reserved_materials(self):
		...
		itemwise_serial_batch_qty = artech_engine._dict()

		for d in reserved_entries:
			key = (d.item_code, d.warehouse)
			if key not in itemwise_serial_batch_qty:
				itemwise_serial_batch_qty[key] = artech_engine._dict(
					{
						"serial_no": [],
	...
	def get_reserved_materials(self):
		...
		serial_batch_doc = artech_engine.qb.DocType("Serial and Batch Entry")
	...
	def set_process_loss_qty(self):
		...
		if not self.process_loss_percentage and not self.process_loss_qty:
			self.process_loss_percentage = artech_engine.get_cached_value(
	...
	def load_items_from_bom(self):
		if self.work_order:
			...
			item_code = artech_engine.db.get_value("BOM", self.bom_no, "item")
		...
		if not expense_account:
			expense_account = artech_engine.get_cached_value("Company", self.company, "stock_adjustment_account")
	...
	def add_batchwise_finished_good(self, batches, args, item):
		qty = flt(self.fg_completed_qty)
		row = artech_engine._dict({"batches_to_be_consume": defaultdict(float)})
	...
	def get_bom_raw_materials(self, qty):
		from artech.manufacturing.doctype.bom.bom import get_bom_items_as_dict
		...
		for item in item_dict.values():
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = artech_engine.db.get_value(
					"Work Order", self.work_order, "allow_alternative_item"
				)

			skip_transfer, from_wip_warehouse = (
				artech_engine.get_value("Work Order", self.work_order, ["skip_transfer", "from_wip_warehouse"])
				if self.work_order
				else [None, None]
			)

			item.from_warehouse = (
				artech_engine.get_value(
					"Work Order Item",
	...
	def get_secondary_items(self, qty):
		...
		if (
			...
			and self.work_order
			and artech_engine.get_cached_value("Work Order", self.work_order, "use_multi_level_bom")
		):
	...
	def set_secondary_items_from_job_card(self):
		...
		for row in self.get_secondary_items_from_job_card():
			...
			item_dict[row.item_code] = artech_engine._dict(
				{
					"uom": row.stock_uom,
					"from_warehouse": "",
					"qty": row.stock_qty,
	...
	def get_secondary_items_from_job_card(self):
		...
		job_card_secondary_item = artech_engine.qb.DocType("Job Card Secondary Item")

		other = (
			artech_engine.qb.from_(job_card)
			.select(
		...
		for row in other:
			...
			if cint(artech_engine.get_cached_value("UOM", row.stock_uom, "must_be_whole_number")):
				row.stock_qty = artech_engine.utils.ceil(row.stock_qty)
	...
	def get_used_secondary_items(self):
		...
		StockEntry = artech_engine.qb.DocType("Stock Entry")
		StockEntryDetail = artech_engine.qb.DocType("Stock Entry Detail")
		data = (
			artech_engine.qb.from_(StockEntry)
	...
	def get_unconsumed_raw_materials(self):
		wo = artech_engine.get_doc("Work Order", self.work_order)
		wo_items = artech_engine.get_all(
			"Work Order Item",
			filters={"parent": self.work_order},
	...
	def add_transfered_raw_materials_in_items(self) -> None:
		...
		wo_data = artech_engine.db.get_value(
			"Work Order",
			self.work_order,
		...
		for _key, row in available_materials.items():
			...
			if cint(artech_engine.get_cached_value("UOM", item.stock_uom, "must_be_whole_number")):
				qty = artech_engine.utils.ceil(qty)
	...
	def update_item_in_stock_entry_detail(self, row, item, qty) -> None:
		...
		use_serial_batch_fields = artech_engine.get_single_value("Stock Settings", "use_serial_batch_fields")
	...
	def get_serial_nos_based_on_transferred_batch(batch_no, serial_nos) -> list:
		serial_nos = artech_engine.get_all(
			"Serial No",
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		transfer_extra_materials_percentage = flt(
			artech_engine.db.get_single_value("Manufacturing Settings", "transfer_extra_materials_percentage")
		)
		...
		if not len(item_dict):
			artech_engine.msgprint(_("""All items have already been transferred for this Work Order."""))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		"""
		item_dict, job_card_items = artech_engine._dict(), []
		work_order = artech_engine.get_doc("Work Order", self.work_order)
		...
		if not artech_engine.db.get_value("Warehouse", work_order.wip_warehouse, "is_group"):
	...
	def get_job_card_item_codes(self, job_card=None):
		...
		job_card_items = artech_engine.get_all(
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		...
		for d in item_dict:
			...
			stock_uom = item_row.get("stock_uom") or artech_engine.db.get_value("Item", d, "stock_uom")
	...
	def validate_with_material_request(self):
		for item in self.get("items"):
			...
			if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
				parent_se = artech_engine.get_value(
					"Stock Entry Detail",
					item.ste_detail,
			...
			if material_request:
				mreq_item = artech_engine.db.get_value(
				...
				if mreq_item.item_code != item.item_code:
					artech_engine.throw(
						...
						artech_engine.MappingMismatchError,
					)
	...
	def validate_batch(self):
		if self.purpose in [
			...
			for item in self.get("items"):
				if item.batch_no:
					disabled = artech_engine.db.get_value("Batch", item.batch_no, "disabled")
					if disabled == 0:
						expiry_date = artech_engine.db.get_value("Batch", item.batch_no, "expiry_date")
						if expiry_date:
	...
	def update_subcontract_order_supplied_items(self):
		if self.get(self.subcontract_data.order_field) and (
			...
			# Get Subcontract Order Supplied Items Details
			order_supplied_items = artech_engine.db.get_all(
				self.subcontract_data.order_supplied_items_field,
			...
			for row in order_supplied_items:
				...
				artech_engine.db.set_value(self.subcontract_data.order_supplied_items_field, row.name, item)
	...
	def update_transferred_qty(self):
		if self.purpose == "Material Transfer" and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = artech_engine.get_all(
					"Stock Entry Detail",
				...
				if d.docstatus == 1:
					transfer_qty = artech_engine.get_value("Stock Entry Detail", d.ste_detail, "transfer_qty")
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = {} and name = {}) THEN {}
					""".format(
					artech_engine.db.escape(data[0]),
					artech_engine.db.escape(data[1]),
					transferred_qty,
				)
	...
	def set_material_request_transfer_status(self, status):
		...
		if self.outgoing_stock_entry:
			parent_se = artech_engine.get_value("Stock Entry", self.outgoing_stock_entry, "add_to_transit")
		...
		for item in self.items:
			...
			if self.purpose == "Material Transfer" and material_request not in material_requests:
				if self.outgoing_stock_entry and parent_se:
					material_request = artech_engine.get_value(
			...
			if material_request and material_request not in material_requests:
				...
				artech_engine.db.set_value("Material Request", material_request, "transfer_status", status)

	def set_serial_no_batch_for_finished_good(self):
		if not (
			(self.pro_doc.has_serial_no or self.pro_doc.has_batch_no)
			and artech_engine.db.get_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order")
		):
			return
		...
		for d in self.items:
			if (
				...
				if serial_nos:
					row = artech_engine._dict({"serial_nos": serial_nos[0 : cint(d.qty)]})
	...
	def update_subcontracting_order_status(self):
		if self.subcontracting_order and self.purpose in ["Send to Subcontractor", "Material Transfer"]:
			from artech.subcontracting.doctype.subcontracting_order.subcontracting_order import (
	...
	def update_pick_list_status(self):
		from artech.stock.doctype.pick_list.pick_list import update_pick_list_status
...
def move_sample_to_retention_warehouse(company: str, items: str | list):
	...
	retention_warehouse = artech_engine.get_single_value("Stock Settings", "sample_retention_warehouse")
	stock_entry = artech_engine.new_doc("Stock Entry")
...
def make_stock_in_entry(source_name: str, target_doc: str | Document | None = None):
	def set_missing_values(source, target):
		...
		if not artech_engine.get_single_value("Stock Settings", "use_serial_batch_fields"):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request:
			add_to_transit = artech_engine.db.get_value("Stock Entry", source_name, "add_to_transit")
			if add_to_transit:
				warehouse = artech_engine.get_value(
...
def get_work_order_details(work_order: str, company: str):
	work_order = artech_engine.get_doc("Work Order", work_order)
...
def get_consumed_operating_cost(wo_name, bom_no):
	table = artech_engine.qb.DocType("Stock Entry")
	child_table = artech_engine.qb.DocType("Landed Cost Taxes and Charges")
	query = (
		artech_engine.qb.from_(child_table)
		.join(table)
...
def get_operating_cost_per_unit(work_order=None, bom_no=None):
	...
	if work_order:
		if (
			bom_no
			and artech_engine.db.get_single_value(
				"Manufacturing Settings", "set_op_cost_and_secondary_items_from_sub_assemblies"
			)
			and artech_engine.get_cached_value("Work Order", work_order.name, "use_multi_level_bom")
		):
	...
	if not operating_cost_per_unit and bom_no:
		bom = artech_engine.db.get_value("BOM", bom_no, ["operating_cost", "quantity"], as_dict=1)
		if bom.quantity:
...
def get_valuation_rate_for_finished_good_entry(work_order):
	work_order_qty = flt(
		artech_engine.get_cached_value("Work Order", work_order, "material_transferred_for_manufacturing")
	)

	field = "(SUM(total_outgoing_value) / %s) as valuation_rate" % (work_order_qty)

	stock_data = artech_engine.get_all(
		"Stock Entry",
		fields=field,
		filters={
			"docstatus": 1,
...
def get_uom_details(item_code: str, uom: str, qty: float | None):
	...
	if not conversion_factor:
		artech_engine.msgprint(_("UOM conversion factor required for UOM: {0} in Item: {1}").format(uom, item_code))
		ret = {"uom": ""}
	else:
		ret = {
...
def get_expired_batch_items():
	from artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
...
def get_expired_batches():
	...
	expired_batches = artech_engine._dict()
	for row in data:
...
def validate_sample_quantity(item_code: str, sample_quantity: int, qty: float, batch_no: str | None = None):
	...
	max_retain_qty = artech_engine.get_value("Item", item_code, "sample_quantity")
...
def get_supplied_items(
	...
	for row in artech_engine.get_all("Stock Entry", fields=fields, filters=filters):
		...
		if key not in supplied_item_details:
			supplied_item_details.setdefault(
				key, artech_engine._dict({"supplied_qty": 0, "returned_qty": 0, "total_supplied_qty": 0})
			)
...
def get_items_from_subcontract_order(source_name: str, target_doc: str | Document | None = None):
	from artech.controllers.subcontracting_controller import make_rm_stock_entry

	if isinstance(target_doc, str):
		target_doc = artech_engine.get_doc(json.loads(target_doc))
...
def get_stock_entry_data(work_order, stock_entry_doc=None):
	...
	stock_entry = artech_engine.qb.DocType("Stock Entry")
	stock_entry_detail = artech_engine.qb.DocType("Stock Entry Detail")

	data = (
		artech_engine.qb.from_(stock_entry)
...
def create_serial_and_batch_bundle(parent_doc, row, child, type_of_transaction=None):
	item_details = artech_engine.get_cached_value(
	...
	if row.serial_nos and row.batches_to_be_consume:
		...
		precision = artech_engine.get_precision("Serial and Batch Entry", "qty")
		doc.has_batch_no = 1
		for batch_no, qty in row.batches_to_be_consume.items():
