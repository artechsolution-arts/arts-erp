...
from artech_engine.utils import cint, flt, nowdate, nowtime, parse_json

from artech.stock.utils import get_or_make_bin, get_stock_balance
...
class StockReservationEntry(Document):
		...
		if entries:
			...
			artech_engine.throw(
				_(
					...
					", ".join([artech_engine.bold(entry.name) for entry in entries]),
					", ".join([artech_engine.bold(wo.name) for wo in work_orders]),
				)
			)
	...
	def validate_amended_doc(self) -> None:
		...
		if self.amended_from:
			msg = _("Cannot amend {0} {1}, please create a new one instead.").format(
				self.doctype, artech_engine.bold(self.amended_from)
			)
	...
	def validate_group_warehouse(self) -> None:
		...
		if artech_engine.get_cached_value("Warehouse", self.warehouse, "is_group"):
			msg = _("Stock cannot be reserved in group warehouse {0}.").format(artech_engine.bold(self.warehouse))
			artech_engine.throw(msg, title=_("Invalid Warehouse"))

	def validate_uom_is_integer(self) -> None:
		"""Validates `Reserved Qty` with Stock UOM."""

		if cint(artech_engine.db.get_value("UOM", self.stock_uom, "must_be_whole_number", cache=True)):
	...
	def auto_reserve_serial_and_batch(self, based_on: str | None = None) -> None:
		...
		if (
			...
			and artech_engine.get_single_value("Stock Settings", "auto_reserve_serial_and_batch")
			...
			from artech.stock.serial_batch_bundle import get_serial_nos_batch
			...
			kwargs = artech_engine._dict(
				{
					...
					"based_on": based_on
					or artech_engine.get_single_value("Stock Settings", "pick_serial_and_batch_based_on"),
				}
			)
			...
			if serial_nos:
				serial_no_wise_batch = artech_engine._dict({})
	...
	def validate_reservation_based_on_serial_and_batch(self) -> None:
		...
		if self.reservation_based_on == "Serial and Batch":
			allow_partial_reservation = artech_engine.get_single_value("Stock Settings", "allow_partial_reservation")
			...
			if self.has_serial_no:
				...
				if not available_serial_nos:
					msg = _("Stock not available for Item {0} in Warehouse {1}.").format(
						artech_engine.bold(self.item_code), artech_engine.bold(self.warehouse)
					)
					artech_engine.throw(msg)
				...
				if self.has_serial_no:
					...
					if key not in available_serial_nos:
						msg = _(
							...
							entry.idx,
							artech_engine.bold(entry.serial_no),
							artech_engine.bold(self.item_code),
							_("Batch {0} and Warehouse").format(artech_engine.bold(entry.batch_no))
							if self.has_batch_no
							else _("Warehouse"),
							artech_engine.bold(self.warehouse),
							artech_engine.bold(_("Stock Reservation Entry")),
						)
					...
					if entry.serial_no in selected_serial_nos:
						msg = _("Row #{0}: Serial No {1} is already selected.").format(
							entry.idx, artech_engine.bold(entry.serial_no)
						)
						artech_engine.throw(msg)
					else:
					...
					if cint(artech_engine.db.get_value("Batch", entry.batch_no, "disabled")):
						msg = _(
							"Row #{0}: Stock cannot be reserved for Item {1} against a disabled Batch {2}."
						).format(entry.idx, artech_engine.bold(self.item_code), artech_engine.bold(entry.batch_no))
						artech_engine.throw(msg)
					...
					if available_qty_to_reserve <= 0:
						msg = _(
							...
							entry.idx,
							artech_engine.bold(self.item_code),
							artech_engine.bold(entry.batch_no),
							artech_engine.bold(self.warehouse),
						)
						artech_engine.throw(msg)
					...
					if entry.qty > available_qty_to_reserve:
						if allow_partial_reservation:
							...
							msg = _(
								...
								entry.idx,
								artech_engine.bold(available_qty_to_reserve),
								artech_engine.bold(self.item_code),
								artech_engine.bold(entry.batch_no),
								artech_engine.bold(self.warehouse),
							)
					...
					if entry.batch_no in selected_batch_nos:
						msg = _("Row #{0}: Batch No {1} is already selected.").format(
							entry.idx, artech_engine.bold(entry.batch_no)
						)
						artech_engine.throw(msg)
					else:
	...
	def update_reserved_qty_in_voucher(
		...
		if item_doctype:
			...
			if self.voucher_type == "Production Plan" and artech_engine.db.exists(
	...
	def update_reserved_stock_in_bin(self) -> None:
		...
		bin_doc = artech_engine.get_cached_doc("Bin", bin_name)
	...
	def update_status(self, status: str | None = None, update_modified: bool = True) -> None:
		...
		artech_engine.db.set_value(self.doctype, self.name, "status", status, update_modified=update_modified)
	...
	def validate_with_allowed_qty(self, qty_to_be_reserved: float) -> None:
		...
		if self.voucher_type == "Sales Order":
			delivered_qty, conversion_factor = artech_engine.db.get_value(
		...
		if self.get("_action") != "submit" and self.voucher_type == "Sales Order" and allowed_qty <= 0:
			msg = _("Item {0} is already reserved/delivered against Sales Order {1}.").format(
				artech_engine.bold(self.item_code), artech_engine.bold(self.voucher_no)
			)

			if self.docstatus == 1:
				self.cancel()
				return artech_engine.msgprint(msg)
			else:
				artech_engine.throw(msg)
		...
		if qty_to_be_reserved > allowed_qty:
			...
			""".format(
				artech_engine.bold(allowed_qty),
				self.stock_uom,
				artech_engine.bold(self.item_code),
				self.voucher_type,
				artech_engine.bold(self.voucher_no),
				actual_qty,
...
def validate_stock_reservation_settings(voucher: object) -> None:
	...
	if not artech_engine.get_single_value("Stock Settings", "enable_stock_reservation"):
		msg = _("Please enable {0} in the {1}.").format(
			artech_engine.bold(_("Stock Reservation")),
			artech_engine.bold(_("Stock Settings")),
		)
		artech_engine.throw(msg)
...
def get_sre_reserved_qty_details_for_voucher(voucher_type: str, voucher_no: str) -> dict:
	...
	return artech_engine._dict(data)
...
def get_ssb_bundle_for_voucher(sre: dict) -> object:
	...
	if sb_entries:
		bundle = artech_engine.new_doc("Serial and Batch Bundle")
...
class StockReservation:
	...
	def cancel_stock_reservation_entries(self, names=None) -> None:
		...
		if sre_names:
			for sre_name in sre_names:
				sre_doc = artech_engine.get_doc("Stock Reservation Entry", sre_name.name)
				sre_doc.cancel()

		if sre_names and names:
			artech_engine.msgprint(
				_("Stock has been unreserved for work order {0}.").format(artech_engine.bold(self.doc.name)),
				alert=True,
			)
	...
	def make_stock_reservation_entries(self):
		...
		child_doctype = artech_engine.scrub(self.doc.doctype + " Item")

		is_sre_created = False
		for item in items:
			sre = artech_engine.new_doc("Stock Reservation Entry")
			if isinstance(item, dict):
				item = artech_engine._dict(item)
			...
			if (
				...
				artech_engine.throw(
					_("Source Warehouse is mandatory for the Item {0}.").format(artech_engine.bold(item.item_code))
				)
	...
	def set_serial_batch(self, sre, serial_batch_bundles):
		bundle_details = artech_engine.get_all(
	...
	def throw_stock_not_exists_error(self, idx, item_code, warehouse):
		artech_engine.msgprint(
			_("Row #{0}: Stock not available to reserve for the Item {1} in Warehouse {2}.").format(
				idx, artech_engine.bold(item_code), artech_engine.bold(warehouse)
			),
	...
	def transfer_reservation_entries_to(
		...
		entries_to_reserve = artech_engine._dict()
		...
		for entry in [
			...
			if not available_qty:
				artech_engine.msgprint(
					_("No available quantity to reserve for item {0} in warehouse {1}").format(
						artech_engine.bold(entry.item_code), artech_engine.bold(entry.warehouse)
					),
					alert=True,
				)
				continue
			...
			sre.has_serial_no = artech_engine.get_value("Item", sre.item_code, "has_serial_no")
			sre.has_batch_no = artech_engine.get_value("Item", sre.item_code, "has_batch_no")
			sre.insert()
			sre.submit()
	...
	def make_stock_reservation_entry(self, row):
		...
		if row.serial_nos:
			for serial_no in row.serial_nos:
				batch_no = None
				if row.batches:
					batch_no = artech_engine.db.get_value("Serial No", serial_no, "batch_no")
	...
	def get_reserved_entries(self, doctype, docnames, against_fg_item=None):
		...
		sre = artech_engine.qb.DocType("Stock Reservation Entry")
		sabb_entry = artech_engine.qb.DocType("Serial and Batch Entry")
	...
	def get_items_to_reserve(self, docnames, from_doctype, to_doctype):
		field = artech_engine.scrub(from_doctype)
		...
		doctype = artech_engine.qb.DocType(to_doctype)
		child_doctype = artech_engine.qb.DocType(to_doctype + child_table_suffix)
...
def create_stock_reservation_entries_for_so_items(
	...
	from artech.selling.doctype.sales_order.sales_order import get_unreserved_qty

	if not from_voucher_type and (
		sales_order.get("_action") == "submit"
		and sales_order.set_warehouse
		and cint(artech_engine.get_cached_value("Warehouse", sales_order.set_warehouse, "is_group"))
	):
		return artech_engine.msgprint(
			_("Stock cannot be reserved in the group warehouse {0}.").format(
				artech_engine.bold(sales_order.set_warehouse)
			)
		)
	...
	if items_details:
		for item in items_details:
			so_item = artech_engine.get_doc("Sales Order Item", item.get("sales_order_item"))
	...
	for item in items if items_details else sales_order.get("items"):
		...
		is_stock_item, has_serial_no, has_batch_no = artech_engine.get_cached_value(
		...
		if artech_engine.get_cached_value("Warehouse", item.warehouse, "is_group"):
			artech_engine.msgprint(
				_("Row #{0}: Stock cannot be reserved in group warehouse {1}.").format(
					item.idx, artech_engine.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="yellow",
			)
		...
		if available_qty_to_reserve <= 0:
			artech_engine.msgprint(
				_("Row #{0}: Stock not available to reserve for the Item {1} in Warehouse {2}.").format(
					item.idx, artech_engine.bold(item.item_code), artech_engine.bold(item.warehouse)
				),
				title=_("Stock Reservation"),
				indicator="orange",
			)
		...
		if qty_to_be_reserved < unreserved_qty:
			if not from_voucher_type and (
				...
				msg = _("Row #{0}: Only {1} available to reserve for the Item {2}").format(
					item.idx,
					artech_engine.bold(str(qty_to_be_reserved / item.conversion_factor) + " " + item.uom),
					artech_engine.bold(item.item_code),
				)
				artech_engine.msgprint(msg, title=_("Stock Reservation"), indicator="orange")
			...
			if not allow_partial_reservation:
				if qty_to_be_reserved == flt(item.get("qty_to_reserve")):
					...
					artech_engine.msgprint(msg, title=_("Partial Stock Reservation"), indicator="yellow")
	...
	if sre_count and notify:
		artech_engine.msgprint(_("Stock Reservation Entries Created"), alert=True, indicator="green")
...
def cancel_stock_reservation_entries(
	...
	if sre_list:
		for sre in sre_list:
			artech_engine.get_doc("Stock Reservation Entry", sre).cancel()

		if notify:
			msg = _("Stock Reservation Entries Cancelled")
			artech_engine.msgprint(msg, alert=True, indicator="red")
...
def update_serial_batch_delivered_qty(row, name, is_cancelled=False):
	if row.serial_nos:
		doctype = artech_engine.qb.DocType("Serial and Batch Entry")
		query = (
			artech_engine.qb.update(doctype)
