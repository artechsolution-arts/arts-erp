...
class TestQualityInspection(ArtechTestSuite):
	def setUp(self):
		super().setUp()
		create_item("_Test Item with QA")
		artech_engine.db.set_value("Item", "_Test Item with QA", "inspection_required_before_delivery", 1)
	...
	def test_qa_for_delivery(self):
		...
		artech_engine.db.set_value("Quality Inspection", qa.name, "status", "Accepted")
		dn.reload()
		dn.submit()
	...
	def test_make_quality_inspections_from_linked_document(self):
		...
		for qi in quality_inspections:
			artech_engine.delete_doc("Quality Inspection", qi)
		dn.delete()
	...
	def test_rejected_qi_validation(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Stop")
		se.reload()
		self.assertRaises(
			QualityInspectionRejectedError, se.submit
		)  # when blocked in Stock settings, block rejected QI

		artech_engine.db.set_single_value("Stock Settings", "action_if_quality_inspection_is_rejected", "Warn")
		se.reload()
	...
	@ArtechTestSuite.change_settings("System Settings", {"number_format": "#.###,##"})
	def test_diff_number_format(self):
		self.assertEqual(artech_engine.db.get_default("number_format"), "#.###,##")  # sanity check
...
def create_quality_inspection(**args):
	...
	qa = artech_engine.new_doc("Quality Inspection")
	...
	qa.sample_size = 1
	qa.inspected_by = artech_engine.session.user
...
def create_quality_inspection_parameter(parameter):
	if not artech_engine.db.exists("Quality Inspection Parameter", parameter):
