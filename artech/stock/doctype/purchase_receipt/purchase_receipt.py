...
from artech_engine.query_builder.functions import Abs, CombineDatetime, Sum
from artech_engine.utils import cint, flt, get_datetime, getdate, nowdate
...
from artech.controllers.accounts_controller import merge_taxes
from artech.controllers.buying_controller import BuyingController
from artech.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
...
class PurchaseReceipt(BuyingController):
	...
	def validate_provisional_expense_account(self):
		provisional_accounting_for_non_stock_items = cint(
			artech_engine.db.get_value("Company", self.company, "enable_provisional_accounting_for_non_stock_items")
		)
		...
		if (
			...
			for d in self.get("items"):
				if not d.purchase_order:
					artech_engine.throw(_("Purchase Order number required for Item {0}").format(d.item_code))

	def validate_items_quality_inspection(self):
		for item in self.get("items"):
			if item.quality_inspection:
				qi = artech_engine.db.get_value(
					"Quality Inspection",
				...
				if qi.reference_type != self.doctype or qi.reference_name != self.name:
					msg = f"""Row #{item.idx}: Please select a valid Quality Inspection with Reference Type
						{artech_engine.bold(self.doctype)} and Reference Name {artech_engine.bold(self.name)}."""
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = artech_engine.db.get_value("Purchase Order Item", po_detail, ["qty", "warehouse"])
	...
	def update_received_qty_if_from_pp(self):
		...
		if items_from_po:
			table = artech_engine.qb.DocType("Purchase Order Item")
			subquery = (
	...
	def check_next_docstatus(self):
		submit_rv = artech_engine.db.sql(
			"""select t1.name
		...
		if submit_rv:
			artech_engine.throw(_("Purchase Invoice {0} is already submitted").format(self.submit_rv[0][0]))
	...
	def on_cancel(self):
		...
		if submitted:
			artech_engine.throw(_("Purchase Invoice {0} is already submitted").format(submitted[0][0]))
	...
	def make_item_gl_entries(self, gl_entries, inventory_account_map=None):
		...
		def validate_account(account_type):
			artech_engine.throw(_("{0} account not found while submitting purchase receipt").format(account_type))
		...
		def make_stock_received_but_not_billed_entry(item):
			if (
				...
				and not artech_engine.db.get_single_value(
			...
			if item.get("rejected_qty") and artech_engine.db.get_single_value(
				"Buying Settings", "set_valuation_rate_for_rejected_materials"
			):
		...
		def make_divisional_loss_gl_entry(item, outgoing_amount):
			...
			if divisional_loss:
				...
				cost_center = item.cost_center or artech_engine.get_cached_value(
		...
		for d in self.get("items"):
			...
			if (
				...
				if not (
					(artech.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
				...
				not artech_engine.db.get_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials")
			...
			if d.rejected_qty and artech_engine.db.get_single_value(
				"Buying Settings", "set_valuation_rate_for_rejected_materials"
			):
				stock_asset_rbnb = (
					self.get_company_default("asset_received_but_not_billed")
	...
	def add_provisional_gl_entry(
		...
		if reverse:
			...
			amount = item_amount
			expense_account = artech_engine.db.get_value(
	...
	def update_billing_status(self, update_modified=True):
		...
		for pr in set(updated_pr):
			pr_doc = self if (pr == self.name) else artech_engine.get_lazy_doc("Purchase Receipt", pr)
	...
	def reserve_stock_for_sales_order(self):
		if (
			self.is_return
			or not artech_engine.get_single_value("Stock Settings", "enable_stock_reservation")
			or not artech_engine.get_single_value("Stock Settings", "auto_reserve_stock_for_sales_order_on_purchase")
		):
			return
		...
		if so_items_details_map:
			...
			for so, items_details in so_items_details_map.items():
				so_doc = artech_engine.get_lazy_doc("Sales Order", so)
	...
	def reserve_stock_for_production_plan(self):
		if self.is_return or not artech_engine.get_single_value("Stock Settings", "enable_stock_reservation"):
			return
	...
	def get_production_plan_references(self):
		production_plan_references = artech_engine._dict()
	...
	def enable_recalculate_rate_in_sles(self):
		rejected_warehouses = artech_engine.get_all(
		...
		(
			artech_engine.qb.update(sle_table)
...
def get_purchase_receipts_against_po_details(po_details):
	...
	purchase_receipt = artech_engine.qb.DocType("Purchase Receipt")
	purchase_receipt_item = artech_engine.qb.DocType("Purchase Receipt Item")

	query = (
		artech_engine.qb.from_(purchase_receipt)
...
def get_billed_amount_against_pr(pr_items):
	...
	purchase_invoice_item = artech_engine.qb.DocType("Purchase Invoice Item")

	query = (
		artech_engine.qb.from_(purchase_invoice_item)
...
def get_billed_amount_against_po(po_items):
	...
	purchase_invoice = artech_engine.qb.DocType("Purchase Invoice")
	purchase_invoice_item = artech_engine.qb.DocType("Purchase Invoice Item")
...
def update_billing_percentage(pr_doc, update_modified=True, adjust_incoming_rate=False):
	...
	over_billing_allowance, role_allowed_to_over_bill = artech_engine.get_single_value(
	...
	billed_qty_amt = artech_engine._dict()
	...
	for item in pr_doc.items:
		...
		amount = item.amount
		if artech_engine.db.get_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice"):
		...
		if adjust_incoming_rate:
			...
			if (
				...
				and role_allowed_to_over_bill not in artech_engine.get_roles()
			):
				artech_engine.throw(
					_("Over Billing Allowance exceeded for Purchase Receipt Item {0} ({1}) by {2}%").format(
						item.name, artech_engine.bold(item.item_code), per_over_billed - over_billing_allowance
					)
				)
...
def get_billed_qty_amount_against_purchase_receipt(pr_doc):
	...
	parent_table = artech_engine.qb.DocType("Purchase Invoice")
	table = artech_engine.qb.DocType("Purchase Invoice Item")
	query = (
		artech_engine.qb.from_(parent_table)
		.inner_join(table)
...
def get_billed_qty_amount_against_purchase_order(pr_doc):
	...
	invoice_data_po_based = artech_engine._dict()
	if po_names:
		parent_table = artech_engine.qb.DocType("Purchase Invoice")
		table = artech_engine.qb.DocType("Purchase Invoice Item")
...
def make_purchase_invoice(
	...
	from artech.accounts.party import get_payment_terms_template

	doc = artech_engine.get_doc("Purchase Receipt", source_name)
	returned_qty_map = get_returned_qty_map(source_name)
	invoiced_qty_map = get_invoiced_qty_map(source_name)

	def set_missing_values(source, target):
		if len(target.get("items")) == 0:
			artech_engine.throw(_("All items have already been Invoiced/Returned"))

		doc = artech_engine.get_doc(target)
...
def get_invoiced_qty_map(purchase_receipt):
	...
	for pr_detail, qty in artech_engine.db.sql(
...
def get_returned_qty_map(purchase_receipt):
	...
	pr = artech_engine.qb.DocType("Purchase Receipt")
	pr_item = artech_engine.qb.DocType("Purchase Receipt Item")
	...
	return artech_engine._dict(query) if query else artech_engine._dict()
...
def update_purchase_receipt_status(docname: str, status: str):
	pr = artech_engine.get_lazy_doc("Purchase Receipt", docname, check_permission="submit")
...
def make_lcv(doctype: str, docname: str):
	landed_cost_voucher = artech_engine.new_doc("Landed Cost Voucher")

	details = artech_engine.db.get_value(doctype, docname, ["supplier", "company", "base_grand_total"], as_dict=1)
