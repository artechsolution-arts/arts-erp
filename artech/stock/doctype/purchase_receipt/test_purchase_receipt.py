...
from artech_engine.utils import add_days, cint, cstr, flt, get_datetime, getdate, nowtime, today
from pypika import functions as fn

import artech
import artech.controllers
import artech.controllers.status_updater
from artech.accounts.doctype.account.test_account import get_inventory_account
from artech.buying.doctype.supplier.test_supplier import create_supplier
from artech.controllers.accounts_controller import InvalidQtyError
from artech.controllers.buying_controller import QtyMismatchError
from artech.stock import get_warehouse_account_map
from artech.stock.doctype.item.test_item import create_item, make_item
from artech.stock.doctype.material_request.material_request import make_purchase_order
from artech.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice
...
class TestPurchaseReceipt(ArtechTestSuite):
	def setUp(self):
		artech_engine.local.future_sle = {}
	...
	def test_purchase_receipt_skips_validation(self):
		...
		mr = artech_engine.new_doc("Material Request")
		...
		mr.insert()
		mr.submit()
		artech_engine.db.set_value("Item", item.name, "over_delivery_receipt_allowance", 200)
		po = make_purchase_order(mr.name)
		po.supplier = "_Test Supplier"
		po.items[0].qty = 300
		po.save()
		po.submit()
		artech_engine.db.set_value("Item", item.name, "over_delivery_receipt_allowance", 20)
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		sl_entry = artech_engine.db.get_all(
			"Stock Ledger Entry",
		...
		sl_entry_cancelled = artech_engine.db.get_all(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": pr.name},
			["actual_qty"],
			order_by="creation",
	...
	def test_make_purchase_invoice(self):
		from artech.accounts.doctype.payment_entry.test_payment_entry import create_payment_term
		...
		template = artech_engine.db.get_value(
			"Payment Terms Template", "_Test Payment Terms Template For Purchase Invoice"
		)
		artech_engine.db.set_value("Supplier", "_Test Supplier", "payment_terms", template)

		pr = make_purchase_receipt(do_not_save=True)
		self.assertRaises(artech_engine.ValidationError, make_purchase_invoice, pr.name)
		pr.submit()
		...
		self.assertRaises(artech_engine.ValidationError, artech_engine.get_doc(pi).submit)
	...
	def test_purchase_receipt_no_gl_entry(self):
		...
		existing_bin_qty, existing_bin_stock_value = artech_engine.db.get_value(
			"Bin",
		...
		stock_value_difference = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
		...
		current_bin_stock_value = artech_engine.db.get_value(
	...
	def test_batched_serial_no_purchase(self):
		item = artech_engine.db.exists("Item", {"item_name": "Batched Serialized Item"})
		if not item:
			...
			item = artech_engine.get_doc("Item", {"item_name": "Batched Serialized Item"})

		pr = make_purchase_receipt(item_code=item.name, qty=5, rate=500)

		self.assertTrue(artech_engine.db.get_value("Batch", {"item": item.name, "reference_name": pr.name}))
	...
	def test_duplicate_serial_nos(self):
		...
		item = artech_engine.db.exists("Item", {"item_name": "Test Serialized Item 123"})
		if not item:
			...
			item = artech_engine.get_doc("Item", {"item_name": "Test Serialized Item 123"})

		# First make purchase receipt
		pr = make_purchase_receipt(item_code=item.name, qty=2, rate=500)
		pr.load_from_db()

		bundle_id = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_receipt_gl_entry(self):
		...
		self.assertEqual(cint(artech.is_perpetual_inventory_enabled(pr.company)), 1)
	...
	def test_serial_no_warehouse(self):
		...
		self.assertEqual(
			artech_engine.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"), pr.get("items")[0].warehouse
		)

		pr.cancel()
		self.assertFalse(artech_engine.db.get_value("Serial No", pr_row_1_serial_no, "warehouse"))

	def test_rejected_warehouse_filter(self):
		pr = artech_engine.copy_doc(self.globalTestRecords["Purchase Receipt"][0])
		...
		self.assertRaises(artech_engine.ValidationError, pr.save)
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(
				artech_engine.db.get_value("Serial No", serial_no, "warehouse"), pr.get("items")[0].warehouse
			)
		...
		for serial_no in rejected_serial_nos:
			self.assertEqual(
				artech_engine.db.get_value("Serial No", serial_no, "warehouse"),
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		returned = artech_engine.get_doc("Purchase Receipt", return_pr.name)
	...
	def test_purchase_return_for_rejected_qty(self):
		from artech.stock.doctype.warehouse.test_warehouse import get_warehouse

		rejected_warehouse = "_Test Rejected Warehouse - TCP1"
		if not artech_engine.db.exists("Warehouse", rejected_warehouse):
			get_warehouse(
				company="_Test Company with perpetual inventory",
				abbr=" - TCP1",
		...
		actual_qty = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_serialized_items(self):
		def _check_serial_no_values(serial_no, field_values):
			serial_no = artech_engine.get_doc("Serial No", serial_no)
	...
	def test_purchase_return_for_multi_uom(self):
		...
		if not artech_engine.db.exists("Item", item_code):
	...
	def test_closed_purchase_receipt(self):
		from artech.stock.doctype.purchase_receipt.purchase_receipt import (
		...
		self.assertEqual(artech_engine.db.get_value("Purchase Receipt", pr.name, "status"), "Closed")
	...
	def test_pr_billing_status(self):
		...
		3. PO -> PR2.
		"""
		from artech.buying.doctype.purchase_order.purchase_order import (
			make_purchase_invoice as make_purchase_invoice_from_po,
		)
		from artech.buying.doctype.purchase_order.purchase_order import make_purchase_receipt
		from artech.buying.doctype.purchase_order.test_purchase_order import create_purchase_order

		artech_engine.flags.print_test_messages = False
		# Qty: 10, Rate: 500
		...
		artech_engine.flags.print_test_messages = True
	...
	def test_serial_no_against_purchase_receipt(self):
		...
		if not artech_engine.db.exists("Serial No", serial_no[0]):
			artech_engine.get_doc(
				{
					"doctype": "Serial No",
					"item_code": item_code,
		...
		voucher_no = artech_engine.db.get_value("Serial and Batch Bundle", bundle_id, "voucher_no")
	...
	def test_auto_asset_creation(self):
		...
		if not artech_engine.db.exists("Item", asset_item):
			asset_category = artech_engine.get_all("Asset Category")
		...
		assets = artech_engine.db.get_all("Asset", filters={"purchase_receipt": pr.name})

		self.assertEqual(len(assets), 3)

		location = artech_engine.db.get_value("Asset", assets[0].name, "location")
		self.assertEqual(location, "Test Location")

		pr.cancel()

	def test_purchase_return_with_submitted_asset(self):
		from artech.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_return

		pr = make_purchase_receipt(item_code="Test Asset Item", qty=1)

		asset = artech_engine.get_doc("Asset", {"purchase_receipt": pr.name})
		asset.available_for_use_date = artech_engine.utils.nowdate()
		...
		self.assertRaises(artech_engine.exceptions.ValidationError, pr_return.submit)
	...
	def test_purchase_receipt_cost_center(self):
		...
		if not artech_engine.db.exists("Location", "Test Location"):
			artech_engine.get_doc({"doctype": "Location", "location_name": "Test Location"}).insert()
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		from artech.buying.doctype.purchase_order.test_purchase_order import (
			create_pr_against_po,
			create_purchase_order,
		)

		artech_engine.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 0)
	...
	def test_stock_transfer_from_purchase_receipt(self):
		from artech.stock.doctype.delivery_note.delivery_note import make_inter_company_purchase_receipt
		from artech.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
	...
	def test_lcv_for_internal_transfer(self):
		from artech.stock.doctype.delivery_note.delivery_note import make_inter_company_purchase_receipt
		from artech.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from artech.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import (
		...
		sle_entries = artech_engine.get_all(
			"Stock Ledger Entry",
		...
		inward_sabb = artech_engine.get_all(
		...
		original_cost = artech_engine.db.get_value("Serial and Batch Bundle", inward_sabb[0], "total_amount")
		...
		new_inward_sabb = artech_engine.get_all(
			"Serial and Batch Bundle",
			filters={
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		new_cost = artech_engine.db.get_value("Serial and Batch Bundle", new_inward_sabb[0], "total_amount")
	...
	def test_stock_transfer_from_purchase_receipt_with_valuation(self):
		from artech.stock.doctype.delivery_note.delivery_note import make_inter_company_purchase_receipt
		from artech.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from artech.stock.doctype.stock_reconciliation.test_stock_reconciliation import (
			create_stock_reconciliation,
		)
		from artech.stock.get_item_details import get_valuation_rate
		from artech.stock.utils import get_stock_balance
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from artech.buying.doctype.purchase_order import purchase_order, test_purchase_order
	...
	def test_purchase_receipt_with_exchange_rate_difference(self):
		from artech.accounts.doctype.purchase_invoice.purchase_invoice import (
			make_purchase_receipt as create_purchase_receipt,
		)
		from artech.accounts.doctype.purchase_invoice.test_purchase_invoice import (
			make_purchase_invoice as create_purchase_invoice,
		)
		from artech.accounts.party import add_party_account
		...
		exchange_gain_loss_account = artech_engine.db.get_value("Company", pr.company, "exchange_gain_loss_account")

		# fetching the latest GL Entry with exchange gain and loss account account
		amount = artech_engine.db.get_value(
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"automatically_fetch_payment_terms": 1})
	def test_payment_terms_are_fetched_when_creating_purchase_invoice(self):
		from artech.accounts.doctype.payment_entry.test_payment_entry import (
			create_payment_terms_template,
		)
		from artech.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice
		from artech.buying.doctype.purchase_order.test_purchase_order import (
			create_purchase_order,
			make_pr_against_po,
		)
		from artech.selling.doctype.sales_order.test_sales_order import (
	...
	@ArtechTestSuite.change_settings("Stock Settings", {"allow_negative_stock": 1})
	def test_neg_to_positive(self):
		from artech.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
	...
	def test_backdated_transaction_for_internal_transfer(self):
		...
		dn_value = artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		pr_value = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_receipt(
		...
		stk_ledger = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_backdated_transaction_for_internal_transfer_in_trasit_warehouse_for_purchase_invoice(
		...
		from artech.accounts.doctype.sales_invoice.sales_invoice import (
		...
		si_value = artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		pi_value = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_batch_expiry_for_purchase_receipt(self):
		...
		artech_engine.db.set_value("Batch", batch_no, "expiry_date", add_days(today(), -1))
	...
	def test_disable_last_purchase_rate(self):
		from artech.stock.get_item_details import ItemDetailsCtx, get_item_details
		...
		artech_engine.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 1)
		...
		artech_engine.db.set_single_value("Buying Settings", "disable_last_purchase_rate", 0)
	...
	def test_validate_received_qty_for_internal_pr(self):
		...
		self.assertRaises(artech_engine.ValidationError, pr.submit)

		# Step 5: Test Over Receipt Allowance
		artech_engine.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 50)
		...
		artech_engine.db.set_single_value("Stock Settings", "over_delivery_receipt_allowance", 0)
	...
	def test_internal_pr_gl_entries(self):
		...
		stock_account_value = artech_engine.db.get_value(
			"GL Entry",
			{
		...
		stock_diff = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
	...
	def test_purchase_receipt_with_backdated_landed_cost_voucher(self):
		...
		available_qty = artech_engine.db.get_value(
			"Bin",
		...
		sle = artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		qty_after_transaction = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{"voucher_type": "Purchase Receipt", "voucher_no": new_pr.name, "is_cancelled": 0},
	...
	def test_purchase_receipt_provisional_accounting(self):
		...
		amount = artech_engine.db.get_value("GL Entry", {"docstatus": 1, "voucher_no": pr.name}, [{"SUM": "debit"}])
	...
	def test_use_serial_batch_fields_for_serial_nos(self):
		...
		sbb_doc = artech_engine.get_doc("Serial and Batch Bundle", pr.items[0].serial_and_batch_bundle)
		...
		serial_no_status = artech_engine.db.get_value("Serial No", "SNU-TSFISI-000015", "status")
		...
		doc = artech_engine.get_doc("Serial and Batch Bundle", dn.items[0].serial_and_batch_bundle)
		for row in doc.entries:
			self.assertTrue(row.serial_no in new_serial_nos)

		for sn in new_serial_nos:
			serial_no_status = artech_engine.db.get_value("Serial No", sn, "status")
			self.assertTrue(serial_no_status != "Active")
	...
	def test_pr_billed_amount_against_return_entry(self):
		from artech.accounts.doctype.purchase_invoice.purchase_invoice import make_debit_note
		from artech.stock.doctype.purchase_receipt.purchase_receipt import (
	...
	def test_valuation_taxes_lcv_repost_after_billing(self):
		...
		old_perpetual_inventory = artech.is_perpetual_inventory_enabled("_Test Company")
		artech_engine.local.enable_perpetual_inventory["_Test Company"] = 1
		...
		artech_engine.local.enable_perpetual_inventory["_Test Company"] = old_perpetual_inventory
	...
	def test_purchase_receipt_with_use_serial_batch_field_for_rejected_qty(self):
		...
		if not artech_engine.db.exists("Batch", batch_no):
			artech_engine.get_doc(
				{
					"doctype": "Batch",
					"batch_id": batch_no,
		...
		for serial_no in serial_nos:
			if not artech_engine.db.exists("Serial No", serial_no):
				artech_engine.get_doc(
					{
						"doctype": "Serial No",
		...
		for row in inter_transfer_pr_return.items:
			...
			if row.item_code == serial_item_doc.name:
				...
				for sn in serial_nos:
					serial_no_details = artech_engine.db.get_value(
	...
	def test_internal_transfer_with_serial_batch_items_without_use_serial_batch_fields(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "use_serial_batch_fields", 0)
	...
	def test_purchase_receipt_bill_for_rejected_quantity_in_purchase_invoice(self):
		...
		return_pr.items[0].purchase_receipt_item = pr.items[0].name
		return_pr.submit()
		old_value = artech_engine.db.get_single_value(
		...
		artech_engine.db.set_single_value("Buying Settings", "bill_for_rejected_quantity_in_purchase_invoice", 1)
		pi = make_purchase_invoice(pr.name)
		pi.submit()
	...
	def test_zero_valuation_rate_for_batched_item(self):
		...
		sabb_doc = artech_engine.get_doc("Serial and Batch Bundle", se.items[0].serial_and_batch_bundle)
	...
	def test_manufacturing_and_expiry_date_for_batch(self):
		...
		batch_no = get_batch_from_bundle(pr.items[0].serial_and_batch_bundle)
		batch = artech_engine.get_doc("Batch", batch_no)
	...
	def test_tax_account_heads_on_lcv_and_item_repost(self):
		...
		company_doc = artech_engine.get_doc("Company", test_company)
		...
		repost_doc = artech_engine.get_doc(
			{
	...
	def create_lcv(self, receipt_document_type, receipt_document, company, expense_account, charges=50):
		ref_doc = artech_engine.get_doc(receipt_document_type, receipt_document)

		lcv = artech_engine.new_doc("Landed Cost Voucher")
		lcv.company = company
	...
	def test_do_not_use_batchwise_valuation_rate(self):
		...
		valuation_rate = artech_engine.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
		self.assertEqual(valuation_rate, 100.0)

		doc = artech_engine.get_doc("Stock Settings")
	...
	def test_internal_transfer_for_batch_items_with_cancel_use_serial_batch_fields(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "use_serial_batch_fields", 1)
		artech_engine.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 0)
	...
	def test_sles_with_same_posting_datetime_and_creation(self):
		from artech.stock.doctype.stock_entry.test_stock_entry import make_stock_entry
		from artech.stock.report.stock_balance.stock_balance import execute
		...
		sle_doc = artech_engine.get_doc("Stock Ledger Entry", sle)
	...
	def test_purchase_receipt_return_valuation_without_use_serial_batch_field(self):
		...
		for row in return_entry.items:
			if row.item_code == batch_item:
				bundle_data = artech_engine.get_all(
	...
	def test_do_not_allow_to_inward_same_serial_no_multiple_times(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "allow_existing_serial_no", 0)
		...
		status = artech_engine.db.get_value("Serial No", serial_no, "status")
		...
		self.assertRaises(artech_engine.exceptions.ValidationError, pr.submit)
	...
	def test_seral_no_return_validation(self):
		...
		sn_return.save()
		self.assertRaises(artech_engine.ValidationError, sn_return.submit)
	...
	def test_batch_no_return_validation(self):
		...
		batch_return.save()
		self.assertRaises(artech_engine.ValidationError, batch_return.submit)
	...
	def test_recreate_stock_ledgers(self):
		...
		for row in sles:
			artech_engine.db.delete("Stock Ledger Entry", {"name": row})
	...
	def test_validate_recreate_stock_ledgers_for_sn_item(self):
		...
		self.assertRaises(artech_engine.ValidationError, repost_doc.save)
	...
	def test_internal_pr_qty_change_only_single_batch(self):
		...
		def get_sabb_qty(sabb):
			return artech_engine.get_value("Serial and Batch Bundle", sabb, "total_qty")
		...
		self.assertRaises(artech.controllers.status_updater.OverAllowanceError, pr.submit)
	...
	def test_valuation_rate_for_rejected_materials(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials", 1)
		...
		stock_received_but_not_billed_account = artech_engine.get_value(
			"Company",
			company,
			"stock_received_but_not_billed",
		)

		rejected_item_cost = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		srbnb_cost = artech_engine.db.get_value(
			"GL Entry",
			{
				"voucher_type": "Purchase Receipt",
				"voucher_no": pr.name,
		...
		artech_engine.db.set_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials", 0)
	...
	def test_valuation_rate_for_rejected_materials_without_accepted_materials(self):
		...
		gl_entry = artech_engine.get_all(
			"GL Entry", filters={"debit": (">", 0), "voucher_no": pr.name}, pluck="name"
		)

		stock_value_diff = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_purchase_expense_account(self):
		...
		if not artech_engine.db.exists("Account", expense_account):
			artech_engine.get_doc(
				{
					"doctype": "Account",
		...
		if not artech_engine.db.exists("Account", expense_contra_account):
			artech_engine.get_doc(
				{
					"doctype": "Account",
		...
		item_doc = artech_engine.get_doc("Item", item)
		item_doc.append(
			"item_defaults",
			{
				"company": company,
	...
	def test_repost_gl_entries(self):
		...
		if not artech_engine.db.exists("Account", account):
			artech_engine.get_doc(
				{
					"doctype": "Account",
	...
	@ArtechTestSuite.change_settings("Stock Settings", {"allow_negative_stock": 0})
	def test_multiple_transactions_with_same_posting_datetime(self):
		from artech.stock.doctype.delivery_note.test_delivery_note import create_delivery_note
		from artech.stock.stock_ledger import NegativeStockError
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"over_billing_allowance": 100})
	def test_set_lcv_from_pi_created_against_po(self):
		...
		pr.items[0].qty = 5
		item = artech_engine.copy_doc(pr.items[0])
		item.qty = 2
	...
	def test_purchase_return_with_and_without_return_against_rejected_qty(self):
		...
		company = artech_engine.db.get_value("Warehouse", warehouse, "company")
	...
	def test_do_not_use_batchwise_valuation_with_fifo(self):
		...
		doc = artech_engine.new_doc("Batch")
		doc.update(
			{
		...
		stock_queue = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": item_code,
				"warehouse": warehouse,
		...
		stock_queue = artech_engine.parse_json(stock_queue)
		...
		ste_details = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"item_code": item_code,
				"warehouse": warehouse,
		...
		stock_queue = artech_engine.parse_json(ste_details.stock_queue)
		...
		self.assertEqual(artech_engine.parse_json(stock_queue), [[20, 0.0]])
	...
	def test_negative_stock_error_for_purchase_return(self):
		...
		self.assertRaises(artech_engine.ValidationError, return_pr.submit)
	...
	def test_internal_purchase_receipt_incoming_rate_with_lcv(self):
		...
		repost_doc = artech_engine.new_doc("Repost Item Valuation")
		repost_doc.update(
			{
				"based_on": "Transaction",
	...
	def test_purchase_return_from_different_warehouse(self):
		...
		sabb = artech_engine.get_doc("Serial and Batch Bundle", make_purchase_entry.items[0].serial_and_batch_bundle)
	...
	def test_different_exchange_rate_in_pr_and_pi(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 1)
...
def prepare_data_for_internal_transfer():
	from artech.accounts.doctype.sales_invoice.test_sales_invoice import create_internal_supplier
	from artech.selling.doctype.customer.test_customer import create_internal_customer
	...
	if not artech_engine.db.get_value("Company", company, "unrealized_profit_loss_account"):
		...
		artech_engine.db.set_value("Company", company, "unrealized_profit_loss_account", account)
...
def get_gl_entries(voucher_type, voucher_no, skip_cancelled=False, as_dict=True):
	...
	gl_query = (
		...
		.orderby(gl.account, order=artech_engine.qb.desc)
	)
	if skip_cancelled:
...
def make_purchase_receipt(**args):
	...
	artech_engine.db.set_single_value("Buying Settings", "allow_multiple_items", 1)
	pr = artech_engine.new_doc("Purchase Receipt")
	args = artech_engine._dict(args)
	...
	uom = args.uom or artech_engine.db.get_value("Item", item_code, "stock_uom") or "_Test UOM"

	bundle_id = None
	if not args.use_serial_batch_fields and (args.get("batch_no") or args.get("serial_no")):
		batches = {}
		if args.get("batch_no"):
			batches = artech_engine._dict({args.batch_no: qty})
	...
	pr.append(
		"items",
		{
			...
			"uom": uom,
			"cost_center": args.cost_center or artech_engine.get_cached_value("Company", pr.company, "cost_center"),
	...
	if args.get_multiple_items:
		...
		company_cost_center = artech_engine.get_cached_value("Company", pr.company, "cost_center")
