...
artech_engine.ui.form.on("Warehouse", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			...
			if ("Stock Balance" in artech_engine.boot.user.all_reports) {
				frm.add_custom_button(
					__("Stock Balance"),
					function () {
		...
		if (
			...
			"General Ledger" in artech_engine.boot.user.all_reports
		) {
			frm.add_custom_button(
...
function convert_to_group_or_ledger(frm) {
	artech_engine.call({
		method: "artech.stock.doctype.warehouse.warehouse.convert_to_group_or_ledger",
		args: {
