...
from artech.stock.doctype.warehouse.warehouse import convert_to_group_or_ledger, get_children
from artech.tests.utils import ArtechTestSuite


class TestWarehouse(ArtechTestSuite):
	def test_parent_warehouse(self):
		parent_warehouse = artech_engine.get_doc("Warehouse", "_Test Warehouse Group - _TC")
		self.assertEqual(parent_warehouse.is_group, 1)

	def test_warehouse_hierarchy(self):
		p_warehouse = artech_engine.get_doc("Warehouse", "_Test Warehouse Group - _TC")

		child_warehouses = artech_engine.db.sql(
	...
	def test_naming(self):
		...
		wh = artech_engine.get_doc(doctype="Warehouse", warehouse_name=warehouse_name, company=company).insert()
	...
	def test_unlinking_warehouse_from_item_defaults(self):
		...
		for warehouse in warehouse_ids:
			artech_engine.delete_doc("Warehouse", warehouse)

		# Check Item existance
		for item in item_names:
			self.assertTrue(bool(artech_engine.db.exists("Item", item)), f"{item} doesn't exist")
	...
	def test_group_non_group_conversion(self):
		warehouse = artech_engine.get_doc("Warehouse", create_warehouse("TestGroupConversion"))
		...
		# chid exists
		self.assertRaises(artech_engine.ValidationError, convert_to_group_or_ledger, warehouse.name)
		artech_engine.delete_doc("Warehouse", child)
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	warehouse_id = artech.encode_company_abbr(warehouse_name, company)
	if not artech_engine.db.exists("Warehouse", warehouse_id):
		w = artech_engine.new_doc("Warehouse")
...
def get_warehouse(**args):
	...
	if artech_engine.db.exists("Warehouse", args.warehouse_name + " - " + args.abbr):
		return artech_engine.get_doc("Warehouse", args.warehouse_name + " - " + args.abbr)
	else:
		w = artech_engine.get_doc(
			{
...
def get_warehouse_account(warehouse_name, company, company_abbr=None):
	...
	if not artech_engine.db.exists("Account", warehouse_name + " - " + company_abbr):
...
def get_group_stock_account(company, company_abbr=None):
	group_stock_account = artech_engine.db.get_value(
