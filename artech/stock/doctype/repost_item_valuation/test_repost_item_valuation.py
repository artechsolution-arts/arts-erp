...
from artech_engine.utils import add_days, add_to_date, now, nowdate, today

from artech.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from artech.accounts.utils import repost_gle_for_stock_vouchers
from artech.controllers.stock_controller import create_item_wise_repost_entries
...
from artech.stock.doctype.repost_item_valuation.repost_item_valuation import (
...
from artech.stock.utils import PendingRepostingError
from artech.tests.utils import ArtechTestSuite


class TestRepostItemValuation(ArtechTestSuite, StockTestMixin):
	def test_repost_time_slot(self):
		repost_settings = artech_engine.get_doc("Stock Reposting Settings")
	...
	def test_clear_old_logs(self):
		...
		logs = artech_engine.get_all("Repost Item Valuation", filters={"status": "Skipped"})
	...
	def test_deduplication(self):
		...
		riv1 = artech_engine.get_doc(riv_args)
		riv1.flags.dont_run_in_test = True
		riv1.submit()
		_assert_status(riv1, "Queued")

		# newer than existing duplicate - riv1
		riv2 = artech_engine.get_doc(riv_args.update({"posting_date": "2021-01-03"}))
		...
		riv3 = artech_engine.get_doc(riv_args.update({"posting_date": "2021-01-01"}))
		...
		riv4 = artech_engine.get_doc(riv_args.update({"warehouse": "Stores - _TC"}))
	...
	def test_stock_freeze_validation(self):
		...
		stock_settings = artech_engine.get_doc("Stock Settings")
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		riv = artech_engine.get_last_doc(
			"Repost Item Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
		self.assertRaises(artech_engine.ValidationError, riv.cancel)
	...
	def test_queue_progress_serialization(self):
		...
		self.assertEqual(
			[["a", "b"], ["c", "d"]],
			sorted(artech_engine.parse_json(artech_engine.as_json(set([("a", "b"), ("c", "d")])))),
		)

	def test_gl_repost_progress(self):
		from artech.accounts import utils
	...
	def test_duplicate_ple_on_repost(self):
		...
		ple_entries = artech_engine.db.get_list(
	...
	def test_account_freeze_validation(self):
		...
		self.assertRaises(artech_engine.ValidationError, riv.save)
	...
	def test_create_repost_entry_for_cancelled_document(self):
		...
		self.assertFalse(artech_engine.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

		pr.load_from_db()

		pr.cancel()
		self.assertTrue(pr.docstatus == 2)
		self.assertTrue(artech_engine.db.exists("Repost Item Valuation", {"voucher_no": pr.name}))

	def test_repost_item_valuation_for_closing_stock_balance(self):
		from artech.stock.doctype.stock_closing_entry.stock_closing_entry import (
			prepare_closing_stock_balance,
		)

		doc = artech_engine.new_doc("Stock Closing Entry")
		...
		riv = artech_engine.new_doc("Repost Item Valuation")
		riv.update(
			{
				"item_code": "_Test Item",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_remove_attached_file(self):
		...
		if docname := artech_engine.db.exists("Repost Item Valuation", {"voucher_no": pr1.name}):
			self.assertFalse(
