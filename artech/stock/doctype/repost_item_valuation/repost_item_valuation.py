...
from artech_engine.desk.form.load import get_attachments
from artech_engine.exceptions import QueryDeadlockError, QueryTimeoutError
from artech_engine.model.document import Document
from artech_engine.query_builder import DocType, Interval
from artech_engine.query_builder.functions import CombineDatetime, Max, Now
from artech_engine.utils import cint, get_link_to_form, get_weekday, getdate, now, nowtime
from artech_engine.utils.user import get_users_with_role
from rq.timeouts import JobTimeoutException

import artech
from artech.accounts.general_ledger import validate_accounting_period
from artech.accounts.utils import get_future_stock_vouchers, repost_gle_for_stock_vouchers
from artech.stock.stock_ledger import (
...
class RepostItemValuation(Document):
	...
	def set_default_posting_time(self):
		...
		if not self.posting_date:
			artech_engine.throw(_("Posting date is required"))
	...
	def validate_update_stock(self):
		if (
			...
			update_stock = artech_engine.get_value(self.voucher_type, self.voucher_no, "update_stock")
			if not update_stock:
				msg = _(
	...
	def validate_recreate_stock_ledgers(self):
		...
		if serial_batch_items := artech_engine.get_all(
	...
	def validate_period_closing_voucher(self):
		...
		if year_end_date and getdate(self.posting_date) <= getdate(year_end_date):
			date = artech_engine.format(year_end_date, "Date")
		...
		if closing_stock and closing_stock[0].name:
			...
			to_date = artech_engine.format(closing_stock[0].posting_date, "Date")
			artech_engine.throw(
	...
	def get_max_period_closing_date(company):
		table = artech_engine.qb.DocType("Period Closing Voucher")
	...
	def validate_accounts_freeze(self):
		acc_frozen_till_date = artech_engine.db.get_value("Company", self.company, "accounts_frozen_till_date")
		frozen_accounts_modifier = artech_engine.db.get_value(
		...
		if getdate(self.posting_date) <= getdate(acc_frozen_till_date):
			if frozen_accounts_modifier and artech_engine.session.user in get_users_with_role(
				frozen_accounts_modifier
			):
				artech_engine.msgprint(_("Caution: This might alter frozen accounts."))
				return
			artech_engine.throw(_("You cannot repost item valuation before {}").format(acc_frozen_till_date))
	...
	def set_company(self):
		if self.based_on == "Transaction":
			self.company = artech_engine.get_cached_value(self.voucher_type, self.voucher_no, "company")
		elif self.warehouse:
			self.company = artech_engine.get_cached_value("Warehouse", self.warehouse, "company")
	...
	def clear_attachment(self):
		if attachments := get_attachments(self.doctype, self.name):
			...
			artech_engine.delete_doc("File", attachment.name, ignore_permissions=True)
	...
	def on_submit(self):
		...
		Exceptions:
		        ...
		        2. global flag artech_engine.flags.dont_execute_stock_reposts is set
		...
		if not artech_engine.in_test:
			return
		if self.flags.dont_run_in_test or artech_engine.flags.dont_execute_stock_reposts:
			return
	...
	def check_pending_repost_against_cancelled_transaction(self):
		...
		artech_engine.throw(msg, title=_("Pending processing"))
	...
	def skipped_similar_reposts(self):
		repost_entries = artech_engine.get_all(
		...
		for entry in repost_entries:
			if (
				artech_engine.db.get_value("Repost Item Valuation", entry.reposting_reference, "status")
				== "Completed"
			):
				artech_engine.db.set_value("Repost Item Valuation", entry.name, "status", "Skipped")
	...
	def recreate_stock_ledger_entries(self):
		...
		if self.based_on == "Transaction" and self.recreate_stock_ledgers:
			doc = artech_engine.get_doc(self.voucher_type, self.voucher_no)
...
def bulk_restart_reposting(names: str):
	...
	for name in names:
		doc = artech_engine.get_doc("Repost Item Valuation", name)
		if doc.status != "Failed":
			continue

		doc.restart_reposting()

	artech_engine.msgprint(_("Repost Item Valuation restarted for selected failed records."))


def on_doctype_update():
	artech_engine.db.add_index("Repost Item Valuation", ["warehouse", "item_code"], "item_warehouse")


def repost(doc):
	try:
		artech_engine.flags.through_repost_item_valuation = True
		if not artech_engine.db.exists("Repost Item Valuation", doc.name):
			return

		# This is to avoid TooManyWritesError in case of large reposts
		artech_engine.db.MAX_WRITES_PER_TRANSACTION *= 4
		...
		message = artech_engine.message_log.pop() if artech_engine.message_log else ""
		...
		if status == "Failed":
			outgoing_email_account = artech_engine.get_cached_value(
...
def remove_attached_file(docname):
	if file_name := artech_engine.db.get_value(
		"File", {"attached_to_name": docname, "attached_to_doctype": "Repost Item Valuation"}, "name"
	):
		artech_engine.delete_doc("File", file_name, ignore_permissions=True, delete_permanently=True, force=True)
...
def repost_gl_entries(doc):
	if not cint(artech.is_perpetual_inventory_enabled(doc.company)):
		return
	...
	enable_separate_reposting_for_gl = artech_engine.db.get_single_value(
...
def _get_directly_dependent_vouchers(doc):
	...
	if doc.based_on == "Transaction":
		ref_doc = artech_engine.get_lazy_doc(doc.voucher_type, doc.voucher_no)
...
def notify_error_to_stock_managers(doc, traceback):
	...
	artech_engine.sendmail(recipients=recipients, subject=subject, message=message)


def get_recipients():
	role = (
		artech_engine.db.get_single_value("Stock Reposting Settings", "notify_reposting_error_to_role")
		or "Stock Manager"
	)
...
def run_parallel_reposting():
	...
	if not artech_engine.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		return
	...
	no_of_parallel_reposting = (
		artech_engine.db.get_single_value("Stock Reposting Settings", "no_of_parallel_reposting") or 4
	)

	riv_entries = get_repost_item_valuation_entries()

	rq_jobs = artech_engine.get_all(
		"RQ Job",
		fields=["arguments"],
		filters={
			"status": ("like", "%started%"),
			"job_name": "artech.stock.doctype.repost_item_valuation.repost_item_valuation.execute_reposting_entry",
		},
	)
...
def repost_entries():
	...
	if artech_engine.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		return
...
def execute_reposting_entry(name):
	...
	if (
		...
		and artech_engine.db.get_value("Repost Item Valuation", doc.reposting_reference, "status")
...
def get_repost_item_valuation_entries():
	doctype = artech_engine.qb.DocType("Repost Item Valuation")
	...
	query = (
		...
		.orderby(CombineDatetime(doctype.posting_date, doctype.posting_time), order=artech_engine.qb.asc)
		.orderby(doctype.creation, order=artech_engine.qb.asc)
		.orderby(doctype.status, order=artech_engine.qb.asc)
	)
...
def in_configured_timeslot(repost_settings=None, current_time=None):
	...
	if repost_settings is None:
		repost_settings = artech_engine.get_cached_doc("Stock Reposting Settings")
...
def execute_repost_item_valuation():
	...
	method = "artech.stock.doctype.repost_item_valuation.repost_item_valuation.repost_entries"
	if artech_engine.db.get_single_value("Stock Reposting Settings", "enable_parallel_reposting"):
		method = "artech.stock.doctype.repost_item_valuation.repost_item_valuation.run_parallel_reposting"

	if name := artech_engine.db.get_value(
		"Scheduled Job Type",
		{"method": method},
		"name",
	):
		artech_engine.get_doc("Scheduled Job Type", name).enqueue(force=True)
...
def make_reposting_for_accounting_ledgers(transactions, company, repost_doc):
	...
	for voucher_type, voucher_no in transactions:
		...
		try:
			new_repost_doc = artech_engine.new_doc("Repost Item Valuation")
...
def get_existing_reposting_only_gl_entries(reposting_reference):
	existing_reposting = artech_engine.get_all(
		"Repost Item Valuation",
		filters={
