...
from artech_engine.exceptions import ValidationError
from artech_engine.utils import cint, flt
from artech_engine.utils.data import add_to_date, getdate
...
class TestBatch(ArtechTestSuite):
	def test_item_has_batch_enabled(self):
		self.assertRaises(
			ValidationError,
			artech_engine.get_doc({"doctype": "Batch", "name": "_test Batch", "item": "_Test Item"}).save,
		)
	...
	def make_batch_item(cls, item_name=None):
		...
		if not artech_engine.db.exists("Item", item_name):
			return make_item(item_name, dict(has_batch_no=1, create_new_batch=1, is_stock_item=1))

		return artech_engine.get_doc("Item", item_name)

	def test_purchase_receipt(self, batch_qty=100):
		"""Test automated batch creation from Purchase Receipt"""
		self.make_batch_item("ITEM-BATCH-1")

		receipt = artech_engine.get_doc(
	...
	def test_batch_stock_levels(self, batch_qty=100):
			...
			SerialBatchCreation(
				{
					...
					"batches": artech_engine._dict({batch_no: 20}),
		...
		receipt2 = artech_engine.get_doc(
			doctype="Purchase Receipt",
			supplier="_Test Supplier",
			company="_Test Company",
			items=[
		...
		batchwise_qty = artech_engine._dict({})
	...
	def test_batch_qty_on_pos_creation(self):
		...
		from artech.accounts.doctype.pos_invoice.test_pos_invoice import create_pos_invoice
		...
		invoice_type = artech_engine.db.get_single_value("POS Settings", "invoice_type")
		session_user = artech_engine.session.user
		...
		try:
			...
			artech_engine.db.set_single_value("POS Settings", "invoice_type", invoice_type)
			# Set user to session user
			artech_engine.set_user(session_user)
	...
	def test_delivery_note(self):
			...
			SerialBatchCreation(
				{
					...
					"batches": artech_engine._dict({batch_no: batch_qty}),
		...
		delivery_note = artech_engine.get_doc(
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 5000,
				"avg_rate": 10,
				"batches": artech_engine._dict({batch_no: 5000}),
				"type_of_transaction": "Outward",
				"company": receipt.company,
			}
		)
	...
	def test_batch_split(self):
		...
		from artech.stock.doctype.batch.batch import split_batch
	...
	def test_ignore_reserved_qty(self):
		...
		batch = artech_engine.get_doc("Batch", batch_id)
	...
	def test_total_batch_qty(self):
		...
		existing_batch_qty = flt(artech_engine.db.get_value("Batch", "B100", "batch_qty"))
		stock_entry = self.make_new_batch_and_entry("ITEM-BATCH-3", "B100", "_Test Warehouse - _TC")

		current_batch_qty = flt(artech_engine.db.get_value("Batch", "B100", "batch_qty"))
	...
	def make_new_batch_and_entry(cls, item_name, batch_name, warehouse):
		...
		if not artech_engine.db.exists("Batch", batch_name):
			batch = artech_engine.get_doc(doctype="Batch", item=item_name, batch_id=batch_name).insert(
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 90,
				"avg_rate": 10,
				"batches": artech_engine._dict({batch_name: 90}),
				"type_of_transaction": "Inward",
				"company": "_Test Company",
				"do_not_submit": 1,
			}
	...
	def test_batch_name_with_naming_series(self):
		stock_settings = artech_engine.get_single("Stock Settings")
		use_naming_series = cint(stock_settings.use_naming_series)

		if not use_naming_series:
			artech_engine.set_value("Stock Settings", "Stock Settings", "use_naming_series", 1)
		...
		if not use_naming_series:
			artech_engine.set_value("Stock Settings", "Stock Settings", "use_naming_series", 0)

	def make_new_batch(self, item_name=None, batch_id=None, do_not_insert=0):
		batch = artech_engine.new_doc("Batch")
	...
	def test_batch_wise_item_price(self):
		if not artech_engine.db.get_value("Item", "_Test Batch Price Item"):
	...
	def test_basic_batch_wise_valuation(self, batch_qty=100):
		...
		for batch, qty in consumption_plan:
			...
			sle = artech_engine.get_last_doc("Stock Ledger Entry", {"is_cancelled": 0, "voucher_no": se.name})
...
def create_batch(item_code, rate, create_item_price_for_batch):
	...
	batch = artech_engine.db.get_value("Batch", {"item": item_code, "reference_name": pi.name})
...
def make_new_batch(**args):
	...
	if artech_engine.db.exists("Batch", args.batch_id):
		batch = artech_engine.get_doc("Batch", args.batch_id)
	else:
		batch = artech_engine.get_doc(
			{
				"doctype": "Batch",
