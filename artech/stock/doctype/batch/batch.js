artech_engine.ui.form.on("Batch", {
	setup: (frm) => {
		frm.set_query("item", () => {
			return {
	...
	item: (frm) => {
		// artech_engine.db.get_value('Item', {name: frm.doc.item}, 'has_expiry_date', (r) => {
		...
		artech_engine.db.get_value(
			...
			(r) => {
				if (r.has_expiry_date && r.shelf_life_in_days) {
					...
					frm.set_value(
						"expiry_date",
						artech_engine.datetime.add_days(frm.doc.manufacturing_date, r.shelf_life_in_days)
					);
				...
				callback: (r) => {
					...
					rows.find(".btn-move").on("click", function () {
						...
						artech_engine.prompt(
							...
							(data) => {
								artech_engine.call({
									method: "artech.stock.doctype.stock_entry.stock_entry_utils.make_stock_entry",
									args: {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
					...
					rows.find(".btn-split").on("click", function () {
						...
						artech_engine.prompt(
							...
							(data) => {
								artech_engine
									.xcall("artech.stock.doctype.batch.batch.split_batch", {
										item_code: frm.doc.item,
										batch_no: frm.doc.name,
										qty: data.qty,
...
artech_engine.ui.form.on("Batch", "manufacturing_date", function (frm) {
	artech_engine.db.get_value("Item", { name: frm.doc.item }, ["shelf_life_in_days", "has_expiry_date"], (r) => {
		if (r.has_expiry_date && r.shelf_life_in_days) {
			// Calculate expiry date based on shelf_life_in_days
			frm.set_value(
				"expiry_date",
