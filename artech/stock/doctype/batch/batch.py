...
from artech_engine.model.naming import make_autoname, revert_series_if_last
from artech_engine.query_builder.functions import CurDate, Sum
from artech_engine.utils import cint, flt, get_link_to_form
from artech_engine.utils.data import DateTimeLikeObject, add_days
from artech_engine.utils.jinja import render_template


class UnableToSelectBatchError(artech_engine.ValidationError):
	pass
...
def get_name_from_hash():
	...
	temp = None
	while not temp:
		temp = artech_engine.generate_hash()[:7].upper()
		if artech_engine.db.exists("Batch", temp):
			temp = None
...
def batch_uses_naming_series():
	...
	:return: bool
	"""
	use_naming_series = cint(artech_engine.get_single_value("Stock Settings", "use_naming_series"))
...
def _get_batch_prefix():
	...
	"""
	naming_series_prefix = artech_engine.get_single_value("Stock Settings", "naming_series_prefix")
...
class Batch(Document):
	...
	def autoname(self):
		...
		create_new_batch, batch_number_series = artech_engine.db.get_value(
			"Item", self.item, ["create_new_batch", "batch_number_series"]
		)

		if not create_new_batch:
			artech_engine.throw(_("Batch ID is mandatory"), artech_engine.MandatoryError)
		...
		while not self.batch_id:
			...
			if artech_engine.db.exists("Batch", self.batch_id):
				self.batch_id = None

		self.name = self.batch_id

	def onload(self):
		self.image = artech_engine.db.get_value("Item", self.item, "image")
	...
	def item_has_batch_enabled(self):
		if artech_engine.db.get_value("Item", self.item, "has_batch_no") == 0:
			artech_engine.throw(_("The selected item cannot have Batch"))
	...
	def recalculate_batch_qty(self):
		...
		artech_engine.msgprint(_("Batch Qty updated to {0}").format(batch_qty), alert=True)

	def set_batchwise_valuation(self):
		from artech.stock.utils import get_valuation_method

		if self.is_new():
			if get_valuation_method(self.item) == "Moving Average" and artech_engine.get_single_value(
	...
	def set_expiry_date(self):
		has_expiry_date, shelf_life_in_days = artech_engine.db.get_value(
		...
		if not self.expiry_date and has_expiry_date and shelf_life_in_days:
			if (
				...
				self.manufacturing_date = artech_engine.db.get_value(
		...
		if has_expiry_date and not self.expiry_date:
			artech_engine.throw(
				msg=_("Please set {0} for Batched Item {1}, which is used to set {2} on Submit.").format(
					artech_engine.bold(_("Shelf Life in Days")),
					get_link_to_form("Item", self.item),
					artech_engine.bold(_("Batch Expiry Date")),
				),
...
def get_batches_by_oldest(item_code: str, warehouse: str):
	...
	batches_dates = [[batch, artech_engine.get_value("Batch", batch.batch_no, "expiry_date")] for batch in batches]
...
def split_batch(batch_no: str, item_code: str, warehouse: str, qty: float, new_batch_id: str | None = None):
	...
	batch = artech_engine.get_doc(doctype="Batch", item=item_code, batch_id=new_batch_id).insert()
	qty = flt(qty)
	...
	from_bundle_id = make_batch_bundle(
		item_code=item_code,
		warehouse=warehouse,
		batches=artech_engine._dict({batch_no: qty}),
		company=company,
	...
	to_bundle_id = make_batch_bundle(
		item_code=item_code,
		warehouse=warehouse,
		batches=artech_engine._dict({batch.name: qty}),
		company=company,
...
def make_batch_bundle(
	...
	from artech_engine.utils import nowtime, today

	from artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import combine_datetime
...
def get_batches(item_code, warehouse, qty=1, throw=False, serial_no=None):
	...
	if serial_no and artech_engine.get_cached_value("Item", item_code, "has_batch_no"):
...
def validate_serial_no_with_batch(serial_nos, item_code):
	if artech_engine.get_cached_value("Serial No", serial_nos[0], "item_code") != item_code:
		artech_engine.throw(
	...
	artech_engine.throw(_("There is no batch found against the {0}: {1}").format(message, serial_no_link))


def make_batch(kwargs):
	if artech_engine.db.get_value("Item", kwargs.item, "has_batch_no"):
		kwargs.doctype = "Batch"
		return artech_engine.get_doc(kwargs).insert().name
...
def get_pos_reserved_batch_qty(filters: dict | str):
	...
	p = artech_engine.qb.DocType("POS Invoice").as_("p")
	item = artech_engine.qb.DocType("POS Invoice Item").as_("item")
	sum_qty = artech_engine.query_builder.functions.Sum(item.stock_qty).as_("qty")

	reserved_batch_qty = (
		artech_engine.qb.from_(p)
		.from_(item)
		.select(sum_qty)
		.where(
			(p.name == item.parent)
...
def get_batch_no(bundle_id):
	from artech.stock.serial_batch_bundle import get_batch_nos
