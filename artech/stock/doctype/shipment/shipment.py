...
from artech_engine.utils import flt, get_time
...
class Shipment(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.stock.doctype.shipment_delivery_note.shipment_delivery_note import ShipmentDeliveryNote
		from artech.stock.doctype.shipment_parcel.shipment_parcel import ShipmentParcel
	...
	def on_submit(self):
		if not self.shipment_parcel:
			artech_engine.throw(_("Please enter Shipment Parcel information"))
		if self.value_of_goods == 0:
			artech_engine.throw(_("Value of goods cannot be 0"))
	...
	def validate_weight(self):
		for parcel in self.shipment_parcel:
			if flt(parcel.weight) <= 0:
				artech_engine.throw(_("Parcel weight cannot be 0"))
	...
	def validate_pickup_time(self):
		if self.pickup_from and self.pickup_to and get_time(self.pickup_to) < get_time(self.pickup_from):
			artech_engine.throw(_("Pickup To time should be greater than Pickup From time"))
