...
from artech.stock.doctype.delivery_note.delivery_note import make_shipment
from artech.tests.utils import ArtechTestSuite


class TestShipment(ArtechTestSuite):
	...
	def test_get_total_weight(self):
		shipment = artech_engine.new_doc("Shipment")
		shipment.extend(
			"shipment_parcel",
			[
...
def get_shipment_customer_contact(customer_name):
	...
	contacts = artech_engine.get_all("Contact", fields=["name"], filters={"name": customer_name})
	if len(contacts):
		return contacts[0]
	else:
...
def get_shipment_customer_address(customer_name):
	...
	customer_address = artech_engine.get_all("Address", fields=["name"], filters={"address_title": address_title})
...
def get_shipment_customer():
	...
	customer = artech_engine.get_all("Customer", fields=["name"], filters={"name": customer_name})
	if len(customer):
		return customer[0]
	else:
...
def get_shipment_company_address(company_name):
	...
	addresses = artech_engine.get_all("Address", fields=["name"], filters={"address_title": address_title})
	if len(addresses):
		return addresses[0]
	else:
...
def get_shipment_company():
	return artech_engine.get_doc("Company", "_Test Company")
...
def create_shipment_address(address_title, company_name, postal_code):
	address = artech_engine.new_doc("Address")
...
def create_customer_contact(fname, lname):
	customer = artech_engine.new_doc("Contact")
...
def create_material_receipt(item, company):
	...
	stock = artech_engine.new_doc("Stock Entry")
