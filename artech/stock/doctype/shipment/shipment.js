artech_engine.ui.form.on("Shipment", {
	...
	onload: function (frm) {
		frm.set_query("delivery_address_name", () => {
			let delivery_to = `delivery_${artech_engine.model.scrub(frm.doc.delivery_to_type)}`;
		...
		frm.set_query("pickup_address_name", () => {
			let pickup_from = `pickup_${artech_engine.model.scrub(frm.doc.pickup_from_type)}`;
			return frm.events.address_query(
				frm,
	...
	pickup_from_type: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			frm.set_value("pickup_company", artech_engine.defaults.get_default("company"));
	...
	delivery_to_type: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			frm.set_value("delivery_company", artech_engine.defaults.get_default("company"));
	...
	delivery_address_name: function (frm) {
		if (frm.doc.delivery_to_type == "Company") {
			artech.utils.get_address_display(frm, "delivery_address_name", "delivery_address", true);
		} else {
			artech.utils.get_address_display(frm, "delivery_address_name", "delivery_address", false);
		}
	},
	pickup_address_name: function (frm) {
		if (frm.doc.pickup_from_type == "Company") {
			artech.utils.get_address_display(frm, "pickup_address_name", "pickup_address", true);
		} else {
			artech.utils.get_address_display(frm, "pickup_address_name", "pickup_address", false);
		}
	},
	get_contact_display: function (frm, contact_name, contact_type) {
		artech_engine.call({
			method: "artech_engine.contacts.doctype.contact.contact.get_contact_details",
	...
	pickup_contact_person: function (frm) {
		if (frm.doc.pickup_contact_person) {
			artech_engine.call({
				method: "artech.stock.doctype.shipment.shipment.get_company_contact",
			...
			if (frm.doc.pickup_from_type === "Company") {
				artech_engine.call({
					method: "artech.stock.doctype.shipment.shipment.get_company_contact",
					args: { user: artech_engine.session.user },
					callback: function ({ message }) {
						const r = message;
						let contact_display = `${r.first_name} ${r.last_name}`;
						if (r.email) {
	...
	set_company_contact: function (frm, delivery_type) {
		artech_engine.db.get_value(
			"User",
			{ name: artech_engine.session.user },
				...
				if (!(r.full_name && r.email && (r.phone || r.mobile_no))) {
					...
					artech_engine.throw(
						__("Full Name, Email or Phone/Mobile of the user are mandatory to continue.") +
							...
							` <a href="/app/user/${artech_engine.session.user}">${artech_engine.session.user}</a>`
					);
				}
		...
		frm.set_value("pickup_contact_person", artech_engine.session.user);
	},
	...
	set_address_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		artech_engine.call({
			method: "artech.stock.doctype.shipment.shipment.get_address_name",
			args: {
	...
	set_contact_name: function (frm, ref_doctype, ref_docname, delivery_type) {
		artech_engine.call({
			method: "artech.stock.doctype.shipment.shipment.get_contact_name",
			args: {
				ref_doctype: ref_doctype,
				docname: ref_docname,
			},
	...
	add_template: function (frm) {
		if (frm.doc.parcel_template) {
			artech_engine.model.with_doc("Shipment Parcel Template", frm.doc.parcel_template, () => {
				let parcel_template = artech_engine.model.get_doc(
					"Shipment Parcel Template",
					frm.doc.parcel_template
				);
				let row = artech_engine.model.add_child(frm.doc, "Shipment Parcel", "shipment_parcel");
	...
	pickup_date: function (frm) {
		if (frm.doc.pickup_date < artech_engine.datetime.get_today()) {
			artech_engine.throw(__("Pickup Date cannot be before this day"));
		}
	},
...
artech_engine.ui.form.on("Shipment Delivery Note", {
