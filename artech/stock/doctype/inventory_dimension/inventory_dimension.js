...
artech_engine.ui.form.on("Inventory Dimension", {
	...
	set_query_on_fields(frm) {
		...
		frm.set_query("document_type", () => {
			return {
				query: "artech.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_documents",
			};
		});
	},
	...
	set_parent_fields(frm) {
		if (frm.doc.apply_to_all_doctypes) {
			...
			artech_engine.call({
				method: "artech.stock.doctype.inventory_dimension.inventory_dimension.get_parent_fields",
				args: {
	...
	delete_dimension(frm) {
		...
		artech_engine.confirm(__(msg, [frm.doc.name.bold()]), () => {
			artech_engine.call({
				method: "artech.stock.doctype.inventory_dimension.inventory_dimension.delete_dimension",
				args: {
					dimension: frm.doc.name,
				},
				callback: function () {
					artech_engine.set_route("List", "Inventory Dimension");
				},
			});
		});
	},
