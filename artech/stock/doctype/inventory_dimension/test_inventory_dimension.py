...
from artech_engine.utils import nowdate, nowtime
...
from artech.stock.doctype.stock_ledger_entry.stock_ledger_entry import InventoryDimensionNegativeStockError
from artech.stock.doctype.warehouse.test_warehouse import create_warehouse
from artech.tests.utils import ArtechTestSuite


class TestInventoryDimension(ArtechTestSuite):
	...
	def test_delete_inventory_dimension(self):
		...
		custom_field = artech_engine.db.get_value(
	...
	def test_inventory_dimension(self):
		...
		sle_data = artech_engine.db.get_value(
		...
		sle_shelf = artech_engine.db.get_value("Stock Ledger Entry", {"voucher_no": outward.name}, "shelf")
	...
	def test_inventory_dimension_for_purchase_receipt_and_delivery_note(self):
		...
		sle_rack = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_check_standard_dimensions(self):
		...
		self.assertFalse(
			artech_engine.db.get_value("Custom Field", {"fieldname": "project", "dt": "Stock Ledger Entry"}, "name")
		)
	...
	def test_for_purchase_sales_and_stock_transaction(self):
		...
		if not artech_engine.db.exists("Store", "Rejected Store"):
			artech_engine.get_doc({"doctype": "Store", "store_name": "Rejected Store"}).insert(
	...
	def test_validate_negative_stock_for_inventory_dimension(self):
		...
		site_name = artech_engine.get_all(
		...
		artech_engine.clear_cache(doctype="Inventory Dimension")
...
def create_inventory_dimension(**args):
	...
	if artech_engine.db.exists("Inventory Dimension", args.dimension_name):
		return artech_engine.get_doc("Inventory Dimension", args.dimension_name)

	doc = artech_engine.new_doc("Inventory Dimension")
	doc.update(args)
...
def prepare_data_for_internal_transfer():
	...
	for store in ["Inter Transfer Store 1", "Inter Transfer Store 2", "Inter Transfer Store 3"]:
		if not artech_engine.db.exists("Store", store):
			artech_engine.get_doc({"doctype": "Store", "store_name": store}).insert(ignore_permissions=True)
	...
	cost_center = artech_engine.db.get_value("Company", company, "cost_center") or artech_engine.db.get_value(
		"Cost Center", {"company": company}, "name"
	)

	expene_account = artech_engine.db.get_value(
		"Company", company, "stock_adjustment_account"
	) or artech_engine.db.get_value("Account", {"company": company, "account_type": "Expense Account"}, "name")
	...
	return artech_engine._dict(
		{
			...
			"store_warehouse": artech_engine.db.get_value(
