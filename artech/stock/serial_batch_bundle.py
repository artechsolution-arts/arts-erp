...
from artech_engine.model.naming import NamingSeries, parse_naming_series
from artech_engine.query_builder.functions import Max, Sum
from artech_engine.utils import add_days, cint, cstr, flt, get_link_to_form, getdate, now
from pypika import Order

from artech.stock.deprecated_serial_batch import (
	DeprecatedBatchNoValuation,
	DeprecatedSerialNoValuation,
)
from artech.stock.valuation import round_off_if_near_zero
...
class SerialBatchBundle:
	...
	def set_item_details(self):
		...
		self.item_details = artech_engine.get_cached_value("Item", self.sle.item_code, fields, as_dict=1)
	...
	def is_material_transfer(self):
		...
		if (
			...
			and artech_engine.get_cached_value("Stock Entry", self.sle.voucher_no, "purpose") in allowed_types
		):
			return True

	def make_serial_batch_no_bundle_for_material_transfer(self):
		from artech.controllers.stock_controller import make_bundle_for_material_transfer

		bundle = artech_engine.db.get_value(
	...
	def validate_actual_qty(self, sn_doc):
		...
		if not condition and self.sle.actual_qty:
			...
			artech_engine.throw(_(msg), title=_("Incorrect Type of Transaction"))
	...
	def validate_item(self):
		...
		if self.sle.actual_qty > 0:
			...
			if (
				...
				and not artech_engine.get_single_value("Stock Settings", "naming_series_prefix")
			):
			...
			if not artech_engine.get_single_value(
		...
		if msg:
			...
			artech_engine.throw(_(error_msg))
	...
	def set_serial_and_batch_bundle(self, sn_doc):
		...
		if sn_doc.is_rejected:
			...
			if self.sle.actual_qty < 0 and self.is_material_transfer():
				...
				ste_detail = artech_engine.db.get_value(
					"Stock Entry Detail",
					self.sle.voucher_detail_no,
			...
			artech_engine.db.set_value(doctype, name, values_to_update)
	...
	def is_packed_entry(self):
		if self.sle.voucher_type in ["Delivery Note", "Sales Invoice"]:
			...
			if item_code != self.sle.item_code:
				return artech_engine.db.get_value("Item", item_code, "is_stock_item") == 0
	...
	def process_batch_no(self):
		if (
			...
			and (
				...
				or (
					artech_engine.get_single_value(
						"Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"
					)
	...
	def validate_item_and_warehouse(self):
		if self.sle.serial_and_batch_bundle and not artech_engine.db.exists(
	...
	def delink_serial_and_batch_bundle(self):
		...
		artech_engine.db.set_value(self.child_doctype, self.sle.voucher_detail_no, update_values)
	...
	def post_process(self):
		...
		if self.sle.serial_and_batch_bundle:
			docstatus = artech_engine.get_cached_value(
	...
	def cancel_serial_and_batch_bundle(self):
		...
		doc = artech_engine.get_cached_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle)
		if doc.docstatus == 1:
			doc.cancel()

	def is_pos_or_asset_repair_transaction(self):
		voucher_type = artech_engine.get_cached_value(
	...
	def submit_serial_and_batch_bundle(self):
		doc = artech_engine.get_doc("Serial and Batch Bundle", self.sle.serial_and_batch_bundle)
	...
	def set_warehouse_and_status_in_serial_nos(self):
		from artech.stock.doctype.serial_no.serial_no import get_serial_nos as get_parsed_serial_nos
	...
	def update_serial_no_status_warehouse(self, sle, serial_nos):
		...
		if sle.voucher_type in ["Sales Invoice", "Delivery Note"] and sle.actual_qty < 0:
			customer = artech_engine.get_cached_value(sle.voucher_type, sle.voucher_no, "customer")
		...
		if status == "Delivered":
			warranty_period = artech_engine.get_cached_value("Item", sle.item_code, "warranty_period")
			if warranty_period:
	...
	def update_serial_no_status_for_stock_reco(self, serial_nos):
		for serial_no in serial_nos:
			sle_doctype = artech_engine.qb.DocType("Stock Ledger Entry")
			sn_table = artech_engine.qb.DocType("Serial and Batch Entry")

			query = (
				artech_engine.qb.from_(sle_doctype)
				.inner_join(sn_table)
...
def get_serial_nos(serial_and_batch_bundle, serial_nos=None):
	...
	serial_nos = artech_engine.get_all("Serial and Batch Entry", filters=filters, order_by="idx", pluck="serial_no")
...
def get_batches_from_bundle(serial_and_batch_bundle, batches=None):
	...
	return artech_engine._dict(entries)
...
def get_serial_or_batch_nos(bundle):
	...
	bundle_data = artech_engine.get_cached_value(
	...
	data = artech_engine.get_all("Serial and Batch Entry", fields=fields, filters={"parent": bundle})
...
class SerialNoValuation(DeprecatedSerialNoValuation):
	...
	def calculate_stock_value_change(self):
		if flt(self.sle.actual_qty) > 0:
			self.stock_value_change = artech_engine.get_cached_value(
	...
	def get_serial_no_wise_incoming_rate(self, serial_nos):
		...
		return artech_engine._dict(result) if result else artech_engine._dict({})
...
def is_rejected(voucher_type, voucher_detail_no, warehouse):
	if voucher_type in ["Purchase Receipt", "Purchase Invoice"]:
		return warehouse == artech_engine.get_cached_value(
...
class BatchNoValuation(DeprecatedBatchNoValuation):
	...
	def prepare_batches(self):
		...
		) == "Moving Average" and artech_engine.get_single_value("Stock Settings", "do_not_use_batchwise_valuation"):
...
class SerialBatchCreation:
	...
	def set_item_details(self):
		...
		item_details = artech_engine.get_cached_value("Item", self.item_code, fields, as_dict=1)
	...
	def duplicate_package(self):
		...
		package = artech_engine.get_doc("Serial and Batch Bundle", id)
		new_package = artech_engine.copy_doc(package, ignore_no_copy=False)
	...
	def make_serial_and_batch_bundle(
		...
		doc = artech_engine.new_doc("Serial and Batch Bundle")
		...
		if doc.voucher_no and artech_engine.get_cached_value(doc.voucher_type, doc.voucher_no, "docstatus") == 2:
			doc.voucher_no = ""
	...
	def update_serial_and_batch_entries(
		...
		doc = artech_engine.get_doc("Serial and Batch Bundle", self.serial_and_batch_bundle)
	...
	def validate_qty(self, doc):
		if doc.type_of_transaction == "Outward" and self.actual_qty and doc.total_qty:
			...
			if required_qty - total_qty > 0:
				...
				artech_engine.throw(msg, title=_("Insufficient Stock"))
	...
	def set_auto_serial_batch_entries_for_inward(self):
		...
		if self.has_serial_no:
			...
			self.batches = artech_engine._dict({self.batch_no: abs(self.actual_qty)})

	def make_serial_no_if_not_exists(self):
		non_exists_serial_nos = []
		for row in self.serial_nos:
			if not artech_engine.db.exists("Serial No", row):
	...
	def create_batch(self):
		...
		if hasattr(self, "is_rejected") and self.is_rejected:
			...
			if bundle:
				if batch_no := artech_engine.db.get_value("Serial and Batch Entry", {"parent": bundle}, "batch_no"):
					return batch_no
	...
	def get_auto_created_serial_nos(self):
		...
		posting_date = artech_engine.db.get_value(
			voucher_type,
			voucher_no,
			"posting_date",
		)
...
def get_serial_or_batch_items(items):
	serial_or_batch_items = artech_engine.get_all(
		"Item",
...
def update_batch_qty(voucher_type, voucher_no, docstatus, via_landed_cost_voucher=False):
	...
	precision = artech_engine.get_precision("Batch", "batch_qty")
	for batch, qty in batches.items():
		...
		artech_engine.db.set_value("Batch", batch, "batch_qty", current_qty)


def get_batch_current_qty(batch):
	doctype = artech_engine.qb.DocType("Batch")
	query = artech_engine.qb.from_(doctype).select(doctype.batch_qty).where(doctype.name == batch).for_update()
	batch_qty = query.run()
...
def get_batchwise_qty(voucher_type, voucher_no):
	...
	return artech_engine._dict(batches)


def get_serial_batch_list_from_item(item):
	serial_list, batch_list = [], []
	if item.serial_and_batch_bundle:
		table = artech_engine.qb.DocType("Serial and Batch Entry")
		query = (
