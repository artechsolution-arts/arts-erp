...
artech.stock.ItemDashboard = class ItemDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(artech_engine.render_template("item_dashboard")).appendTo(this.parent);
		...
		this.content.on("click", ".btn-edit", function () {
			...
			artech_engine.db.get_value(
				...
				"name",
				(r) => {
					artech_engine.set_route("Form", "Putaway Rule", r.name);
				}
			);
		});
		...
		function handle_move_add(element, action) {
			...
			if (disable_quick_entry) {
				...
				if (action === "Add") {
					...
					artech.stock.move_item(item, null, warehouse, actual_qty, rate, stock_uom, function () {
						me.refresh();
					});
				} else {
					artech.stock.move_item(item, warehouse, null, actual_qty, null, stock_uom, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(item, warehouse, entry_type, stock_uom) {
			artech_engine.model.with_doctype("Stock Entry", function () {
				var doc = artech_engine.model.get_new_doc("Stock Entry");
				if (entry_type) {
					doc.stock_entry_type = entry_type;
				}

				var row = artech_engine.model.add_child(doc, "items");
				row.item_code = item;
				row.uom = stock_uom;
	...
	render(data) {
		...
		if (context.data.length > 0) {
			...
			$(artech_engine.render_template(this.template, context)).appendTo(this.result);
		} else {
	...
	get_item_dashboard_data(data, max_count, show_item) {
		...
		let can_write = 0;
		if (artech_engine.boot.user.can_write.indexOf("Stock Entry") >= 0) {
			can_write = 1;
		}
	...
	get_capacity_dashboard_data(data) {
		...
		let can_write = 0;
		if (artech_engine.boot.user.can_write.indexOf("Putaway Rule") >= 0) {
			can_write = 1;
		}
...
artech.stock.move_item = function (item, source, target, actual_qty, rate, stock_uom, callback) {
	...
	dialog.set_primary_action(__("Create Stock Entry"), function () {
		if (flt(dialog.get_value("qty")) <= 0) {
			artech_engine.msgprint(__("Quantity must be greater than zero"));
			return;
		}

		if (source && dialog.get_value("qty") > actual_qty) {
			artech_engine.msgprint(__("Quantity must be less than or equal to {0}", [actual_qty]));
			return;
		}

		if (dialog.get_value("source") === dialog.get_value("target")) {
			artech_engine.msgprint(__("Source and target warehouse must be different"));
			return;
		}

		artech_engine.model.with_doctype("Stock Entry", function () {
			let doc = artech_engine.model.get_new_doc("Stock Entry");
			doc.from_warehouse = dialog.get_value("source");
			doc.to_warehouse = dialog.get_value("target");
			doc.stock_entry_type = doc.from_warehouse ? "Material Transfer" : "Material Receipt";
			let row = artech_engine.model.add_child(doc, "items");
