...
from artech_engine.utils import add_days, cint, escape_html, flt, nowdate
...
def reorder_item():
	...
	if not (artech_engine.db.a_row_exists("Company") and artech_engine.db.a_row_exists("Fiscal Year")):
		return

	if cint(artech_engine.db.get_single_value("Stock Settings", "auto_indent")):
		return _reorder_item()


def _reorder_item():
	material_requests = {"Purchase": {}, "Transfer": {}, "Material Issue": {}, "Manufacture": {}}
	warehouse_company = artech_engine._dict(
	...
	default_company = (
		artech.get_default_company() or artech_engine.db.sql("""select name from tabCompany limit 1""")[0][0]
	)
...
def get_items_for_reorder() -> dict[str, list]:
	reorder_table = artech_engine.qb.DocType("Item Reorder")
	item_table = artech_engine.qb.DocType("Item")

	query = (
		artech_engine.qb.from_(reorder_table)
	...
	itemwise_reorder = artech_engine._dict({})
	for d in data:
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in artech_engine.db.sql(
		...
		while warehouse_doc.parent_warehouse:
			...
			warehouse_doc = artech_engine.get_doc("Warehouse", warehouse_doc.parent_warehouse)
...
def create_material_request(material_requests):
	...
	company_wise_mr = artech_engine._dict({})
		...
		for company in material_requests[request_type]:
			try:
				...
				for d in items:
					...
					must_be_whole_number = artech_engine.db.get_value("UOM", uom, "must_be_whole_number", cache=True)
	...
	if company_wise_mr:
		if getattr(artech_engine.local, "reorder_email_notify", None) is None:
			artech_engine.local.reorder_email_notify = cint(
				artech_engine.db.get_single_value("Stock Settings", "reorder_email_notify")
			)

		if artech_engine.local.reorder_email_notify:
...
def send_email_notification(company_wise_mr):
	...
	for company, mr_list in company_wise_mr.items():
		...
		msg = artech_engine.render_template("templates/emails/reorder_item.html", {"mr_list": mr_list})

		artech_engine.sendmail(recipients=email_list, subject=_("Auto Material Requests Generated"), message=msg)


def get_email_list(company):
	users = get_comapny_wise_users(company)
	user_table = artech_engine.qb.DocType("User")
	role_table = artech_engine.qb.DocType("Has Role")

	query = (
		artech_engine.qb.from_(user_table)
...
def get_comapny_wise_users(company):
	...
	if parent_company := artech_engine.db.get_value("Company", company, "parent_company"):
		companies.append(parent_company)

	users = artech_engine.get_all(
		"User Permission",
...
def notify_errors(exceptions_list):
	subject = _("[Important] [Artech] Auto Reorder Errors")
	content = (
	...
	from artech_engine.email import sendmail_to_system_managers
