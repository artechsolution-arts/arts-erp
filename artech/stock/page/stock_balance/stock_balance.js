artech_engine.pages["stock-balance"].on_page_load = function (wrapper) {
	...
	page.warehouse_field = page.add_field({
		...
		fieldtype: "Link",
		options: "Warehouse",
		default: artech_engine.route_options && artech_engine.route_options.warehouse,
		change: function () {
	...
	page.item_field = page.add_field({
		...
		fieldtype: "Link",
		options: "Item",
		default: artech_engine.route_options && artech_engine.route_options.item_code,
		change: function () {
			page.item_dashboard.start = 0;
			page.item_dashboard.refresh();
		},
	...
	page.item_group_field = page.add_field({
		...
		fieldtype: "Link",
		options: "Item Group",
		default: artech_engine.route_options && artech_engine.route_options.item_group,
		change: function () {
			page.item_dashboard.start = 0;
			page.item_dashboard.refresh();
		},
	...
	page.sort_selector = new artech_engine.ui.SortSelector({
	...
	artech_engine.require("item-dashboard.bundle.js", function () {
		page.item_dashboard = new artech.stock.ItemDashboard({
			parent: page.main,
			page_length: 20,
