...
class StockTestMixin:
	...
	def assertGLEs(self, doc, expected_gles, gle_filters=None, order_by=None):
		...
		actual_gles = artech_engine.get_all(
			"GL Entry",
			fields=["*"],
			filters=filters,
...
class TestStockUtilities(ArtechTestSuite, StockTestMixin):
	def test_barcode_scanning(self):
		...
		batch = artech_engine.get_doc(doctype="Batch", item=batch_item.name).insert()
		...
		serial = artech_engine.get_doc(
			doctype="Serial No",
			item_code=serial_item.name,
			serial_no=artech_engine.generate_hash(),
	...
	def test_barcode_scanning_of_warehouse(self):
		...
		warehouse_2 = artech_engine.get_doc(
			{
				"doctype": "Warehouse",
