...
class TestFIFOValuation(ArtechTestSuite):
	def setUp(self):
...
class TestLIFOValuation(ArtechTestSuite):
	def setUp(self):
...
class TestLIFOValuationSLE(ArtechTestSuite):
	...
	def assertStockQueue(self, se, expected_queue):
		sle_name = artech_engine.db.get_value(
			"Stock Ledger Entry", {"voucher_no": se.name, "is_cancelled": 0, "voucher_type": "Stock Entry"}
		)
		sle = artech_engine.get_doc("Stock Ledger Entry", sle_name)
