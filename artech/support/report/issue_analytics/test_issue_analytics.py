...
from artech.support.doctype.issue.test_issue import create_customer, make_issue
from artech.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from artech.support.report.issue_analytics.issue_analytics import execute
from artech.tests.utils import ArtechTestSuite

months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]


class TestIssueAnalytics(ArtechTestSuite):
	def setUp(self):
...
def create_issue_types():
	for entry in ["Bug", "Service Request", "Discomfort"]:
		if not artech_engine.db.exists("Issue Type", entry):
			artech_engine.get_doc({"doctype": "Issue Type", "__newname": entry}).insert()
