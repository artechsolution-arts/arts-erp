...
class IssueSummary:
	...
	def get_issues(self):
		...
		self.entries = artech_engine.db.get_all(
			"Issue",
			fields=[
	...
	def get_summary_data(self):
		self.issue_summary_data = artech_engine._dict()
			...
			if self.filters.based_on == "Assigned To":
				if d._assign:
					for entry in json.loads(d._assign):
						self.issue_summary_data.setdefault(entry, artech_engine._dict()).setdefault(status, 0.0)
						self.issue_summary_data.setdefault(entry, artech_engine._dict()).setdefault(
				...
				self.issue_summary_data.setdefault(value, artech_engine._dict()).setdefault(status, 0.0)
				self.issue_summary_data.setdefault(value, artech_engine._dict()).setdefault(agreement_status, 0.0)
				self.issue_summary_data.setdefault(value, artech_engine._dict()).setdefault("total_issues", 0.0)
	...
	def get_metrics_data(self):
		...
		if issues:
			if self.filters.based_on == "Assigned To":
				assignment_map = artech_engine._dict()
				for d in self.entries:
					if d._assign:
						for entry in json.loads(d._assign):
				...
				for entry in data:
					...
					for metric in metrics_list:
						self.issue_summary_data.setdefault(value, artech_engine._dict()).setdefault(metric, 0.0)
