...
class WarrantyClaim(TransactionBase):
	...
	def validate(self):
		if session["user"] != "Guest" and not self.customer:
			artech_engine.throw(_("Customer is required"))

		if (
			self.status == "Closed"
			and not self.resolution_date
			and artech_engine.db.get_value("Warranty Claim", self.name, "status") != "Closed"
		):
			self.resolution_date = now_datetime()

	def on_cancel(self):
		lst = artech_engine.db.sql(
			"""select t1.name
		...
		if lst:
			...
			artech_engine.throw(_("Cancel Material Visit {0} before cancelling this Warranty Claim").format(lst1))
		else:
...
def make_maintenance_visit(source_name: str, target_doc: str | Document | None = None):
	from artech_engine.model.mapper import get_mapped_doc, map_child_doc
	...
	if not visit:
		...
		source_doc = artech_engine.get_doc("Warranty Claim", source_name)
