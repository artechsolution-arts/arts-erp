...
from artech_engine.utils import flt, get_datetime

from artech.support.doctype.service_level_agreement.test_service_level_agreement import (
	create_service_level_agreements_for_issues,
)
from artech.tests.utils import ArtechTestSuite


class TestSetUp(ArtechTestSuite):
	def setUp(self):
		artech_engine.db.sql("delete from `tabService Level Agreement`")
		artech_engine.db.sql("delete from `tabService Level Priority`")
		artech_engine.db.sql("delete from `tabSLA Fulfilled On Status`")
		artech_engine.db.sql("delete from `tabPause SLA On Status`")
		artech_engine.db.sql("delete from `tabService Day`")
		artech_engine.db.set_single_value("Support Settings", "track_service_level_agreement", 1)
		create_service_level_agreements_for_issues()
...
class TestIssue(TestSetUp):
	def test_response_time_and_resolution_time_based_on_different_sla(self):
		...
		artech_engine.flags.current_time = get_datetime("2019-03-04 15:00")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_hold_time_on_replied(self):
		...
		artech_engine.flags.current_time = get_datetime("2020-03-04 4:15")
		issue.reload()
		issue.status = "Replied"
		issue.save()

		self.assertEqual(issue.on_hold_since, artech_engine.flags.current_time)
		self.assertFalse(issue.sla_resolution_by)

		creation = get_datetime("2020-03-04 5:00")
		artech_engine.flags.current_time = get_datetime("2020-03-04 5:00")
		create_communication(issue.name, "test@example.com", "Received", creation)
		...
		artech_engine.flags.current_time = get_datetime("2020-03-04 5:05")
		issue.reload()
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_close_after_on_hold(self):
		artech_engine.flags.current_time = get_datetime("2021-11-01 19:00")

		issue = make_issue(artech_engine.flags.current_time, index=1)
		create_communication(issue.name, "test@example.com", "Received", artech_engine.flags.current_time)

		# send a reply within SLA
		artech_engine.flags.current_time = get_datetime("2021-11-02 11:00")
		create_communication(issue.name, "test@admin.com", "Sent", artech_engine.flags.current_time)
		...
		artech_engine.flags.current_time = get_datetime("2021-11-22 01:00")
		issue.status = "Closed"
		issue.save()
	...
	def test_issue_open_after_closed(self):
		...
		artech_engine.flags.current_time = get_datetime("2021-11-01 13:00")
		issue = make_issue(
			artech_engine.flags.current_time, index=1, issue_type="Critical"
		...
		# Replied on → 2 pm
		artech_engine.flags.current_time = get_datetime("2021-11-01 14:00")
		create_communication(issue.name, "test@admin.com", "Sent", artech_engine.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		...
		self.assertEqual(issue.on_hold_since, artech_engine.flags.current_time)
		self.assertEqual(issue.first_responded_on, artech_engine.flags.current_time)

		# Customer Replied → 3 pm
		artech_engine.flags.current_time = get_datetime("2021-11-01 15:00")
		create_communication(issue.name, "test@example.com", "Received", artech_engine.flags.current_time)
		issue.reload()
		...
		artech_engine.flags.current_time = get_datetime("2021-11-01 16:00")
		create_communication(issue.name, "test@admin.com", "Sent", artech_engine.flags.current_time)
		issue.reload()
		issue.status = "Replied"
		issue.save()
		...
		artech_engine.flags.current_time = get_datetime("2021-11-01 22:00")
		issue.status = "Closed"
		issue.save()
		# Hold Time + 6 Hrs
		...
		self.assertEqual(issue.agreement_status, "Fulfilled")
		self.assertEqual(issue.sla_resolution_date, artech_engine.flags.current_time)

		# Customer Open → 3 am i.e after resolution by is crossed
		artech_engine.flags.current_time = get_datetime("2021-11-02 03:00")
		create_communication(issue.name, "test@example.com", "Received", artech_engine.flags.current_time)
		issue.reload()
		...
		artech_engine.flags.current_time = get_datetime("2021-11-02 04:00")
		issue.status = "Closed"
		issue.save()
		self.assertEqual(issue.sla_resolution_by, get_datetime("2021-11-02 07:00"))
		self.assertEqual(issue.agreement_status, "Fulfilled")
	...
	def test_recording_of_assignment_on_first_reponse_failure(self):
		from artech_engine.desk.form.assign_to import add as add_assignment
		...
		artech_engine.flags.current_time = get_datetime("2021-11-02 15:00")
		create_communication(issue.name, "test@admin.com", "Sent", artech_engine.flags.current_time)

		# assert if a new timeline item has been added
		# to record the assignment
		comment = artech_engine.db.exists(
			"Comment",
			{
...
def create_issue_and_communication(issue_creation, first_responded_on):
	...
	artech_engine.flags.current_time = first_responded_on
...
def make_issue(creation=None, customer=None, index=0, priority=None, issue_type=None):
	if issue_type and not artech_engine.db.exists("Issue Type", issue_type):
		doc = artech_engine.new_doc("Issue Type")
		doc.name = issue_type
		doc.insert()

	issue = artech_engine.get_doc(
		{
			"doctype": "Issue",
...
def create_customer(name, customer_group, territory):
	...
	if not artech_engine.db.exists("Customer", {"customer_name": name}):
...
def create_customer_group(customer_group):
	if not artech_engine.db.exists("Customer Group", {"customer_group_name": customer_group}):
		artech_engine.get_doc({"doctype": "Customer Group", "customer_group_name": customer_group}).insert(
			ignore_permissions=True
		)


def create_territory(territory):
	if not artech_engine.db.exists("Territory", {"territory_name": territory}):
		artech_engine.get_doc(
			{
				"doctype": "Territory",
...
def create_communication(reference_name, sender, sent_or_received, creation):
	communication = artech_engine.get_doc(
		{
