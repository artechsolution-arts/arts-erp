artech_engine.ui.form.on("Issue", {
	onload: function (frm) {
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.model.open_mapped_doc({
						method: "artech.support.doctype.issue.issue.make_task",
						frm: frm,
					});
				},
				__("Create")
	...
	reset_service_level_agreement: function (frm) {
		let reset_sla = new artech_engine.ui.Dialog({
			...
			primary_action: (values) => {
				...
				artech_engine.call(
					"artech.support.doctype.service_level_agreement.service_level_agreement.reset_service_level_agreement",
					{
						reason: values.reason,
						user: artech_engine.session.user_email,
					...
					() => {
						...
						frm.refresh();
						artech_engine.msgprint(__("Service Level Agreement was reset."));
					}
				);
			},
		});
	...
	timeline_refresh: function (frm) {
		if (!frm.timeline.wrapper.find(".btn-split-issue").length) {
			let split_issue_btn = $(`
				<a class="action-btn btn-split-issue" title="${__("Split Issue")}">
					${artech_engine.utils.icon("branch", "sm")}
				</a>
			`);
		...
		// if (artech_engine.model.can_create("Help Article")) {
		...
		// 			${artech_engine.utils.icon('solid-info', 'sm')}
		// 		</a>
		// 	`);
		...
		// 			const doc = artech_engine.model.get_new_doc("Help Article");
		// 			doc.title = frm.doc.subject;
		// 			doc.content = content;
		// 			artech_engine.set_route("Form", "Help Article", doc.name);
		// 		});
		// 	}
