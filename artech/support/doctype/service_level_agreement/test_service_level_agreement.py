...
from artech.support.doctype.issue_priority.test_issue_priority import make_priorities
from artech.support.doctype.service_level_agreement.service_level_agreement import (
	get_service_level_agreement_fields,
)
from artech.tests.utils import ArtechTestSuite


class TestServiceLevelAgreement(ArtechTestSuite):
	...
	def test_custom_field_creation_for_sla_on_standard_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				artech_engine.db.exists("Custom Field", {"dt": doctype, "fieldname": field.get("fieldname")})
			)
	...
	def test_docfield_creation_for_sla_on_custom_dt(self):
		...
		for field in sla_fields:
			self.assertTrue(
				artech_engine.db.exists("DocField", {"fieldname": field.get("fieldname"), "parent": doctype.name})
			)
	...
	def test_sla_application(self):
		...
		artech_engine.flags.current_time = datetime.datetime(2019, 3, 4, 15, 0)
		lead.reload()
	...
	def test_hold_time(self):
		...
		artech_engine.flags.current_time = datetime.datetime(2020, 3, 4, 4, 15)
		lead.reload()
		lead.status = "Replied"
		lead.save()

		lead.reload()
		self.assertEqual(lead.on_hold_since, artech_engine.flags.current_time)

		artech_engine.flags.current_time = datetime.datetime(2020, 3, 4, 5, 5)
		lead.reload()
		lead.status = "Converted"
		lead.save()
	...
	def test_failed_sla_for_response_only(self):
		...
		artech_engine.flags.current_time = datetime.datetime(2019, 3, 4, 16, 5)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_fulfilled_sla_for_response_only(self):
		...
		artech_engine.flags.current_time = datetime.datetime(2019, 3, 4, 15, 30)
		lead.reload()
		lead.status = "Replied"
		lead.save()
	...
	def test_service_level_agreement_filters(self):
		...
		applied_sla = artech_engine.db.get_value("Lead", lead.name, "service_level_agreement")
		self.assertFalse(applied_sla)

		source = artech_engine.new_doc(doctype="UTM Source")
...
def get_service_level_agreement(
	...
	service_level_agreement = artech_engine.get_doc("Service Level Agreement", filters)
...
def create_service_level_agreement(
	...
	service_level_agreement = artech_engine._dict(
		{
			...
			"entity": entity,
			"start_date": artech_engine.utils.getdate(),
			"end_date": artech_engine.utils.add_to_date(artech_engine.utils.getdate(), days=100),
	...
	sla = artech_engine.db.exists("Service Level Agreement", filters)
	if sla:
		artech_engine.delete_doc("Service Level Agreement", sla, force=1)

	return artech_engine.get_doc(service_level_agreement).insert(ignore_permissions=True, ignore_if_duplicate=True)
...
def create_customer():
	...
	if not artech_engine.db.exists("Customer", "_Test Customer"):
		customer.insert(ignore_permissions=True)
		return customer.name
	else:
		return artech_engine.db.exists("Customer", "_Test Customer")


def create_customer_group():
	customer_group = artech_engine.get_doc(
		{"doctype": "Customer Group", "customer_group_name": "_Test SLA Customer Group"}
	)

	if not artech_engine.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"}):
		customer_group.insert()
		return customer_group.name
	else:
		return artech_engine.db.exists("Customer Group", {"customer_group_name": "_Test SLA Customer Group"})


def create_territory():
	territory = artech_engine.get_doc(
		{
			"doctype": "Territory",
	...
	if not artech_engine.db.exists("Territory", {"territory_name": "_Test SLA Territory"}):
		territory.insert()
		return territory.name
	else:
		return artech_engine.db.exists("Territory", {"territory_name": "_Test SLA Territory"})
...
def make_holiday_list():
	holiday_list = artech_engine.db.exists("Holiday List", "__Test Holiday List")
	if not holiday_list:
...
def create_custom_doctype():
	if not artech_engine.db.exists("DocType", "Test SLA on Custom Dt"):
		...
		return artech_engine.get_doc("DocType", "Test SLA on Custom Dt")
