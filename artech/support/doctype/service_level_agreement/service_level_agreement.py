...
from artech_engine.core.utils import get_parent_doc
...
from artech_engine.utils.safe_exec import get_safe_globals

from artech.support.doctype.issue.issue import calculate_first_response_time, get_holidays
...
class ServiceLevelAgreement(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.support.doctype.pause_sla_on_status.pause_sla_on_status import PauseSLAOnStatus
		from artech.support.doctype.service_day.service_day import ServiceDay
		from artech.support.doctype.service_level_priority.service_level_priority import (
			ServiceLevelPriority,
		)
		from artech.support.doctype.sla_fulfilled_on_status.sla_fulfilled_on_status import (
	...
	def check_priorities(self):
		...
		if len(set(priorities)) != len(priorities):
			...
			artech_engine.throw(_("Priority {0} has been repeated.").format(repeated_priority))

		# set default priority from priorities
		try:
			self.default_priority = next(d.priority for d in self.priorities if d.default_priority)
		except Exception:
			artech_engine.throw(_("Select a Default Priority."))
	...
	def check_support_and_resolution(self):
		...
		if len(set(support_days)) != len(support_days):
			...
			artech_engine.throw(_("Workday {0} has been repeated.").format(repeated_days))

	def validate_doc(self):
		if (
			self.enabled
			and self.document_type == "Issue"
			and not artech_engine.db.get_single_value("Support Settings", "track_service_level_agreement")
		):
			artech_engine.throw(
				_("{0} is not enabled in {1}").format(
					artech_engine.bold(_("Track Service Level Agreement")),
					get_link_to_form("Support Settings", "Support Settings"),
				)
			)

		if self.default_service_level_agreement and artech_engine.db.exists(
		...
		if (
			...
			artech_engine.throw(
				_("Service Level Agreement for {0} {1} already exists.").format(
					artech_engine.bold(self.entity_type), artech_engine.bold(self.entity)
				)
			)

	def validate_selected_doctype(self):
		invalid_doctypes = list(artech_engine.model.core_doctypes_list)
		invalid_doctypes.extend(["Cost Center", "Company"])
		valid_document_types = artech_engine.get_all(
			"DocType",
			{
				"issingle": 0,
				"istable": 0,
		...
		if self.document_type not in valid_document_types:
			artech_engine.throw(msg=_("Please select valid document type."), title=_("Invalid Document Type"))

	def validate_status_field(self):
		meta = artech_engine.get_meta(self.document_type)
		if not meta.get_field("status"):
			artech_engine.throw(
				_(
					"The Document Type {0} must have a Status field to configure Service Level Agreement"
				).format(artech_engine.bold(self.document_type))
			)

	def validate_condition(self):
		temp_doc = artech_engine.new_doc(self.document_type)
		if self.condition:
			try:
				artech_engine.safe_eval(self.condition, None, get_context(temp_doc))
			except Exception:
				artech_engine.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def get_service_level_agreement_priority(self, priority):
		priority = artech_engine.get_doc("Service Level Priority", {"priority": priority, "parent": self.name})
	...
	def before_insert(self):
		...
		meta = artech_engine.get_meta(self.document_type, cached=False)
	...
	def create_docfields(self, meta, service_level_agreement_fields):
		...
		artech_engine.get_doc("DocType", self.document_type).save(ignore_permissions=True)
	...
	def reset_field_properties(self, field, field_dt, sla_field):
		field = artech_engine.get_doc(field_dt, {"fieldname": field.fieldname})
...
def check_agreement_status():
	service_level_agreements = artech_engine.get_all(
		"Service Level Agreement",
	...
	for service_level_agreement in service_level_agreements:
		doc = artech_engine.get_doc("Service Level Agreement", service_level_agreement.name)
		if doc.end_date and getdate(doc.end_date) < getdate(artech_engine.utils.getdate()):
			artech_engine.db.set_value("Service Level Agreement", service_level_agreement.name, "enabled", 0)


def get_active_service_level_agreement_for(doc):
	if not artech_engine.db.get_single_value("Support Settings", "track_service_level_agreement"):
		return
	...
	default_sla = artech_engine.get_all(
		"Service Level Agreement",
	...
	agreements = artech_engine.get_all(
		"Service Level Agreement",
		filters=filters,
	...
	for agreement in agreements:
		...
		if not condition or (condition and artech_engine.safe_eval(condition, None, get_context(doc))):
...
def get_context(doc):
	return {
		"doc": doc.as_dict(),
		"nowdate": nowdate,
		"artech_engine": artech_engine._dict(utils=get_safe_globals().get("artech_engine").get("utils")),
	}


def get_customer_group(customer):
	customer_groups = []
	customer_group = artech_engine.db.get_value("Customer", customer, "customer_group") if customer else None
	if customer_group:
...
def get_customer_territory(customer):
	...
	customer_territory = artech_engine.db.get_value("Customer", customer, "territory") if customer else None
...
def get_service_level_agreement_filters(doctype: str, name: str, customer: str | None = None):
	...
	return {
		"priority": [
			priority.priority
			for priority in artech_engine.get_all(
				"Service Level Priority", filters={"parent": name}, fields=["priority"]
			)
		],
		"service_level_agreements": [
			d.name for d in artech_engine.get_all("Service Level Agreement", filters=filters, or_filters=or_filters)
		],
	}
...
def get_documents_with_active_service_level_agreement():
	sla_doctypes = artech_engine.cache.get_value("doctypes_with_active_sla")
...
def set_documents_with_active_service_level_agreement():
	try:
		...
		artech_engine.cache.set_value("doctypes_with_active_sla", active)
	except (artech_engine.DoesNotExistError, artech_engine.db.TableMissingError):
...
def handle_status_change(doc, apply_sla_for_resolution):
	now_time = artech_engine.flags.current_time or now_datetime(doc.get("owner"))
	prev_status = artech_engine.db.get_value(doc.doctype, doc.name, "status")
...
def get_fulfillment_statuses(service_level_agreement):
	return [
		entry.status
		for entry in artech_engine.db.get_all(
...
def get_allotted_seconds(parameter, service_level):
	...
	if parameter == "response":
		...
		artech_engine.throw(_("{0} parameter is invalid").format(parameter))
...
def get_support_days(service_level):
	...
	for service in service_level.get("support_and_resolution"):
		support_days[service.workday] = artech_engine._dict(
			{
...
def change_service_level_agreement_and_priority(self):
	if (
		...
		and artech_engine.db.exists("Issue", self.name)
		and artech_engine.db.get_single_value("Support Settings", "track_service_level_agreement")
	):
		if self.priority != artech_engine.db.get_value("Issue", self.name, "priority"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
			artech_engine.msgprint(_("Priority has been changed to {0}.").format(self.priority))

		if self.service_level_agreement != artech_engine.db.get_value("Issue", self.name, "service_level_agreement"):
			self.set_response_and_resolution_time(
				priority=self.priority, service_level_agreement=self.service_level_agreement
			)
...
def get_response_and_resolution_duration(doc):
	sla = artech_engine.get_doc("Service Level Agreement", doc.service_level_agreement)
...
def reset_service_level_agreement(doctype: str, docname: str, reason: str, user: str):
	if not artech_engine.db.get_single_value("Support Settings", "allow_resetting_service_level_agreement"):
		artech_engine.throw(_("Allow Resetting Service Level Agreement from Support Settings."))
...
def on_communication_update(doc, status):
	...
	for_resolution = artech_engine.db.get_value(
...
def get_tz(user):
	return artech_engine.db.get_value("User", user, "time_zone") or get_system_timezone()
...
def get_sla_doctypes():
	doctypes = []
	data = artech_engine.get_all("Service Level Agreement", {"enabled": 1}, ["document_type"], distinct=1)
