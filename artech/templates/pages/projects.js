artech_engine.ready(function () {
	...
	$(".file-size").each(function () {
		$(this).text(artech_engine.form.formatters.FileSize($(this).text()));
	});
	...
	var reload_items = function (item_status, item, $btn) {
		$.ajax({
			...
			dataType: "json",
			data: {
				cmd: "artech.templates.pages.projects.get_" + item + "_html",
				project: "{{ doc.name }}",
	...
	var close_item = function (item, item_name) {
		...
		artech_engine.call({
			btn: this,
			type: "POST",
			method: "artech.templates.pages.projects.set_" + item + "_status",
			args: args,
			callback: function (r) {
				if (r.exc) {
					if (r._server_messages) artech_engine.msgprint(r._server_messages);
				} else {
					$(this).remove();
				}
			},
