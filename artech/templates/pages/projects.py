...
def get_context(context):
	project_user = artech_engine.db.get_value(
		"Project User",
		{"parent": artech_engine.form_dict.project, "user": artech_engine.session.user},
		["user", "view_attachments", "hide_timesheets"],
		as_dict=True,
	)
	if artech_engine.session.user != "Administrator" and (not project_user or artech_engine.session.user == "Guest"):
		raise artech_engine.PermissionError

	context.no_cache = 1
	context.show_sidebar = True
	project = artech_engine.get_doc("Project", artech_engine.form_dict.project)

	project.has_permission("read")

	project.tasks = get_tasks(
		project.name, start=0, item_status="open", search=artech_engine.form_dict.get("search")
	)

	if project_user and not project_user.hide_timesheets:
		project.timesheets = get_timesheets(project.name, start=0, search=artech_engine.form_dict.get("search"))
...
def get_task_html(project: str, start: int = 0, item_status: str | None = None):
	return artech_engine.render_template(
		"artech/templates/includes/projects/project_tasks.html",
		{
			"doc": {
				"name": project,
...
def get_timesheets(project, start=0, search=None):
	...
	timesheets = artech_engine.get_all(
		"Timesheet Detail",
		filters=filters,
...
def get_timesheet_html(project: str, start: int = 0):
	return artech_engine.render_template(
		"artech/templates/includes/projects/project_timesheets.html",
