...
from artech_engine.core.utils import html2text
from artech_engine.utils import sanitize_html
from artech_engine.utils.global_search import search
from jinja2 import utils


def get_context(context):
	context.no_cache = 1
	if artech_engine.form_dict.q:
		query = str(utils.escape(sanitize_html(artech_engine.form_dict.q)))
		context.title = _("Help Results for")
		context.query = query

		context.route = "/search_help"
		d = artech_engine._dict()
...
def get_help_results_sections(text: str):
	out = []
	settings = artech_engine.get_doc("Support Settings", "Support Settings")
