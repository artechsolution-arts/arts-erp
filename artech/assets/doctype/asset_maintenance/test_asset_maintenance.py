...
from artech_engine.utils import add_days, get_last_day, nowdate
...
class TestAssetMaintenance(ArtechTestSuite):
	def setUp(self):
		set_depreciation_settings_in_company()
		self.pr = make_purchase_receipt(
			item_code="Photocopier", qty=1, rate=100000.0, location="Test Location"
		)
		self.asset_name = artech_engine.db.get_value("Asset", {"purchase_receipt": self.pr.name}, "name")
		self.asset_doc = artech_engine.get_doc("Asset", self.asset_name)
	...
	def test_create_asset_maintenance_with_log(self):
		...
		asset_maintenance = artech_engine.get_doc(
			{
		...
		asset_maintenance_log = artech_engine.db.get_value(
			"Asset Maintenance Log",
			{"asset_maintenance": asset_maintenance.name, "task_name": "Change Oil"},
			"name",
		)

		asset_maintenance_log_doc = artech_engine.get_doc("Asset Maintenance Log", asset_maintenance_log)
