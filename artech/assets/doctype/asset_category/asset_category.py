...
from artech_engine.utils import cint, get_link_to_form
...
class AssetCategory(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.asset_category_account.asset_category_account import AssetCategoryAccount
	...
	def validate_finance_books(self):
		for d in self.finance_books:
			for field in ("Total Number of Depreciations", "Frequency of Depreciation"):
				if cint(d.get(artech_engine.scrub(field))) < 1:
					artech_engine.throw(
						_("Row {0}: {1} must be greater than 0").format(d.idx, field), artech_engine.MandatoryError
					)
	...
	def validate_account_currency(self):
		...
		for d in self.accounts:
			company_currency = artech_engine.get_value("Company", d.get("company_name"), "default_currency")
			for type_of_account in account_types:
				if d.get(type_of_account):
					account_currency = artech_engine.get_value("Account", d.get(type_of_account), "account_currency")
		...
		for d in invalid_accounts:
			artech_engine.throw(
				_("Row #{}: Currency of {} - {} doesn't matches company currency.").format(
					d.idx, artech_engine.bold(artech_engine.unscrub(d.type)), artech_engine.bold(d.account)
				),
		...
		for d in self.accounts:
			for fieldname in account_type_map.keys():
				if d.get(fieldname):
					...
					selected_key_type = artech_engine.db.get_value("Account", selected_account, key_to_match)
					...
					if selected_key_type not in expected_key_types:
						artech_engine.throw(
							_(
								...
								d.idx,
								artech_engine.unscrub(key_to_match),
								artech_engine.bold(selected_account),
								artech_engine.bold(" or ".join(expected_key_types)),
							),
							title=_("Invalid Account"),
						)
	...
	def validate_duplicate_rows(self):
		...
		if len(companies) != len(self.accounts):
			artech_engine.throw(_("Cannot set multiple account rows for the same company"))

	def validate_cwip_accounts(self):
		if self.enable_cwip_accounting:
			missing_cwip_accounts_for_company = []
			for d in self.accounts:
				if not d.capital_work_in_progress_account and not artech_engine.get_cached_value(
			...
			if missing_cwip_accounts_for_company:
				...
				artech_engine.throw(msg, title=_("Missing Account"))
	...
	def validate_depreciation_accounts(self):
		...
		def validate_company_accounts(company, acc_row=None):
			default_accounts = artech_engine.get_cached_value(
				"Company",
				company,
		...
		companies_with_assets = artech_engine.db.get_all(
			"Asset",
			{
		...
		if error_msg:
			...
			artech_engine.throw(msg, title=_("Missing Accounts"))


def get_asset_category_account(
	fieldname, item=None, asset=None, account=None, asset_category=None, company=None
):
	if item and artech_engine.db.get_value("Item", item, "is_fixed_asset"):
		asset_category = artech_engine.db.get_value("Item", item, ["asset_category"])

	elif not asset_category or not company:
		if account:
			if artech_engine.db.get_value("Account", account, "account_type") != "Fixed Asset":
				account = None

		if not account:
			asset_details = artech_engine.db.get_value("Asset", asset, ["asset_category", "company"])
