...
from artech_engine.utils import cstr, get_datetime, get_link_to_form
...
class AssetMovement(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.asset_movement_item.asset_movement_item import AssetMovementItem
	...
	def validate_asset(self, d):
		status, company = artech_engine.db.get_value("Asset", d.asset, ["status", "company"])
		if self.purpose == "Transfer" and status in ("Draft", "Scrapped", "Sold"):
			artech_engine.throw(_("{0} asset cannot be transferred").format(status))

		if company != self.company:
			artech_engine.throw(_("Asset {0} does not belong to company {1}").format(d.asset, self.company))
	...
	def validate_transaction_date(self, d):
		previous_movement_date = artech_engine.db.get_value(
			"Asset Movement",
		...
		if previous_movement_date and get_datetime(previous_movement_date) > get_datetime(
			...
			artech_engine.throw(_("Transaction date can't be earlier than previous movement date"))
	...
	def validate_location(self, d):
		if self.purpose in ["Transfer", "Transfer and Issue"]:
			current_location = artech_engine.db.get_value("Asset", d.asset, "location")
			...
			if not d.target_location:
				artech_engine.throw(_("Target Location is required for transferring Asset {0}").format(d.asset))
			if d.source_location == d.target_location:
				artech_engine.throw(_("Source and Target Location cannot be same"))

		if self.purpose == "Receipt":
			if not d.target_location:
				artech_engine.throw(_("Target Location is required while receiving Asset {0}").format(d.asset))
			if d.to_employee and artech_engine.db.get_value("Employee", d.to_employee, "company") != self.company:
				artech_engine.throw(
	...
	def validate_employee(self, d):
		if self.purpose == "Transfer and Issue":
			if not d.from_employee:
				artech_engine.throw(_("From Employee is required while issuing Asset {0}").format(d.asset))

		if d.from_employee:
			current_custodian = artech_engine.db.get_value("Asset", d.asset, "custodian")
		...
		if not d.to_employee:
			artech_engine.throw(_("Employee is required while issuing Asset {0}").format(d.asset))
	...
	def get_latest_location_and_custodian(self, asset):
		...
		latest_movement_entry = artech_engine.db.sql(
			f"""
	...
	def update_asset_location_and_custodian(self, asset_id, location, employee):
		asset = artech_engine.get_doc("Asset", asset_id)

		if cstr(employee) != asset.custodian:
			artech_engine.db.set_value("Asset", asset_id, "custodian", cstr(employee))
		if location and location != asset.location:
			artech_engine.db.set_value("Asset", asset_id, "location", location)
