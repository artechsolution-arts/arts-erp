...
class TestAssetMovement(ArtechTestSuite):
	def setUp(self):
	...
	def test_movement(self):
		...
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "location"), "Test Location 2")
		...
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "location"), "Test Location")
		...
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "location"), "Test Location 2")
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "custodian"), employee)
	...
	def test_last_movement_cancellation(self):
		...
		movement = artech_engine.get_doc({"doctype": "Asset Movement", "reference_name": pr.name})
		self.assertRaises(artech_engine.ValidationError, movement.cancel)
	...
	def test_movement_transaction_date(self):
		...
		asset_creation_date = artech_engine.db.get_value(
			"Asset Movement",
		...
		self.assertRaises(artech_engine.ValidationError, asset_movement.save)
...
def create_asset_movement(**args):
	...
	movement = artech_engine.new_doc("Asset Movement")
	movement.update(
		{
			"assets": args.assets,
