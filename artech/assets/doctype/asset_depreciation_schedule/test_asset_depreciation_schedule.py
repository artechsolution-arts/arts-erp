...
from artech_engine.utils import cstr, flt, getdate
...
from artech.assets.doctype.asset_repair.test_asset_repair import create_asset_repair
from artech.assets.doctype.asset_value_adjustment.test_asset_value_adjustment import (
	make_asset_value_adjustment,
)
from artech.tests.utils import ArtechTestSuite


class TestAssetDepreciationSchedule(ArtechTestSuite):
	def test_throw_error_if_another_asset_depr_schedule_exist(self):
		...
		second_asset_depr_schedule = artech_engine.get_doc(
			{"doctype": "Asset Depreciation Schedule", "asset": asset.name, "finance_book": None}
		)

		self.assertRaises(artech_engine.ValidationError, second_asset_depr_schedule.insert)

	def test_daily_prorata_based_depr_on_sl_method(self):
		artech_engine.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 0)
		asset = create_asset(
	...
	def test_schedule_for_slm_for_existing_asset_daily_pro_rata_enabled(self):
		artech_engine.db.set_single_value("Accounts Settings", "calculate_depr_using_total_days", 1)
		asset = create_asset(
			calculate_depreciation=1,
			depreciation_method="Straight Line",
