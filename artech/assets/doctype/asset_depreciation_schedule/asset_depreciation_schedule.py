...
from artech.assets.doctype.asset_depreciation_schedule.deppreciation_schedule_controller import (
...
class AssetDepreciationSchedule(DepreciationScheduleController):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.depreciation_schedule.depreciation_schedule import DepreciationSchedule
	...
	def validate_another_asset_depr_schedule_does_not_exist(self):
		...
		asset_depr_schedule = artech_engine.db.exists(
	...
	def validate_asset(self):
		asset = artech_engine.get_doc("Asset", self.asset)
	...
	def cancel_depreciation_entries(self):
		for d in self.get("depreciation_schedule"):
			if d.journal_entry:
				je_status = artech_engine.db.get_value("Journal Entry", d.journal_entry, "docstatus")
				if je_status == 0:
					artech_engine.throw(
						_(
	...
	def get_finance_book_row(self, fb_row=None):
		...
		asset_finance_book_name = artech_engine.db.get_value(
			doctype="Asset Finance Book",
			filters=[["parent", "=", self.asset], finance_book_filter],
		)
		self.fb_row = artech_engine.get_doc("Asset Finance Book", asset_finance_book_name)
...
def make_draft_asset_depr_schedule(asset_doc, row):
	asset_depr_schedule_doc = artech_engine.new_doc("Asset Depreciation Schedule")
	...
	for row in asset_doc.get("finance_books"):
		...
		if current_schedule:
			if current_schedule.docstatus == 1:
				new_schedule = artech_engine.copy_doc(current_schedule)
...
def get_temp_depr_schedule_doc(asset_doc, fb_row, disposal_date=None, updated_depr_schedule=None):
	...
	temp_schedule_doc = artech_engine.copy_doc(current_schedule)
...
def get_asset_shift_factors_map():
	return dict(artech_engine.db.get_all("Asset Shift Factor", ["shift_name", "shift_factor"], as_list=True))
...
def get_asset_depr_schedule_doc(asset_name: str, status: str | None = None, finance_book: str | None = None):
	...
	asset_depr_schedule_doc = artech_engine.get_doc("Asset Depreciation Schedule", asset_depr_schedule)
...
def get_asset_depr_schedule_name(asset_name, status=None, finance_book=None):
	...
	depreciation_schedules = artech_engine.get_all(
