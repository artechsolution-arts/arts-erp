...
from artech.assets.doctype.asset_depreciation_schedule.depreciation_methods import (
	StraightLineMethod,
	WDVMethod,
)
...
class DepreciationScheduleController(StraightLineMethod, WDVMethod):
	...
	def get_total_pending_days_or_years(self):
		if cint(artech_engine.get_single_value("Accounts Settings", "calculate_depr_using_total_days")):
			last_depr_date = self.get_last_booked_depreciation_date()
			if last_depr_date:
		...
		if flt(self.depreciation_amount, self.asset_doc.precision("net_purchase_amount")) <= 0:
			artech_engine.throw(
				_("Net Purchase Amount {0} cannot be depreciated over {1} cycles.").format(
					artech_engine.bold(self.asset_doc.net_purchase_amount),
					artech_engine.bold(self.fb_row.total_number_of_depreciations),
				)
			)
	...
	def add_depr_schedule_row(self, row_idx):
		...
		if self.shift_based:
			shift = (
				...
				else artech_engine.get_cached_value("Asset Shift Factor", {"default": 1}, "shift_name")
			)
