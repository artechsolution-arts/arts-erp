...
from artech_engine.utils import getdate, nowdate, today

from artech.assets.doctype.asset_maintenance.asset_maintenance import calculate_next_due_date
...
class AssetMaintenanceLog(Document):
	...
	def validate(self):
		...
		if self.maintenance_status == "Completed" and not self.completion_date:
			artech_engine.throw(_("Please select Completion Date for Completed Asset Maintenance Log"))

		if self.maintenance_status != "Completed" and self.completion_date:
			artech_engine.throw(_("Please select Maintenance Status as Completed or remove Completion Date"))

	def on_submit(self):
		if self.maintenance_status not in ["Completed", "Cancelled"]:
			artech_engine.throw(_("Maintenance Status has to be Cancelled or Completed to Submit"))
		self.update_maintenance_task()

	def update_maintenance_task(self):
		asset_maintenance_doc = artech_engine.get_doc("Asset Maintenance Task", self.task)
		...
		if self.maintenance_status == "Cancelled":
			...
			asset_maintenance_doc.save()
		asset_maintenance_doc = artech_engine.get_doc("Asset Maintenance", self.asset_maintenance)
		asset_maintenance_doc.save()
...
def update_asset_maintenance_log_status():
	...
	(
		artech_engine.qb.update(AssetMaintenanceLog)
...
def get_maintenance_tasks(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	asset_maintenance_tasks = artech_engine.db.get_values(
