...
from artech_engine.utils.nestedset import NestedSet, update_nsm
...
class Location(NestedSet):
	...
	def update_ancestor_location_features(self):
		...
		for ancestor in self.get_ancestors():
			ancestor_doc = artech_engine.get_doc("Location", ancestor)
...
def get_children(doctype: str, parent: str | None = None, location: str | None = None, is_root: bool = False):
	...
	return artech_engine.db.sql(
		...
		from
			`tabLocation` comp
		where
			ifnull(parent_location, "")={artech_engine.db.escape(parent)}
		""",
		as_dict=1,
	)
...
def on_doctype_update():
	artech_engine.db.add_index("Location", ["lft", "rgt"])
