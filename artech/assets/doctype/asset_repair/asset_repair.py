...
from artech_engine.utils import DateTimeLikeObject, cint, flt, get_link_to_form, getdate, time_diff_in_hours
...
from artech.assets.doctype.asset.asset import get_asset_account
...
class AssetRepair(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.asset_repair_consumed_item.asset_repair_consumed_item import (
			AssetRepairConsumedItem,
		)
		from artech.assets.doctype.asset_repair_purchase_invoice.asset_repair_purchase_invoice import (
	...
	def validate(self):
		self.asset_doc = artech_engine.get_lazy_doc("Asset", self.asset)
		self.validate_asset()
		self.validate_dates()
	...
	def validate_duplicate_purchase_invoices(self):
		...
		if duplicates:
			duplicate_links = "".join(
				[
					f"<li>{_('Row #{0}:').format(idx)} {get_link_to_form('Purchase Invoice', pi)} - {artech_engine.bold(account)}</li>"
		...
		if row.expense_account not in valid_accounts:
			artech_engine.throw(
				_(
					...
					row.idx,
					artech_engine.bold(row.expense_account),
		...
		if flt(row.repair_cost) > available_amount:
			artech_engine.throw(
				_(
					...
					row.idx,
					artech_engine.bold(artech_engine.format_value(row.repair_cost, {"fieldtype": "Currency"})),
					artech_engine.bold(artech_engine.format_value(available_amount, {"fieldtype": "Currency"})),
					get_link_to_form("Purchase Invoice", row.purchase_invoice),
					artech_engine.bold(row.expense_account),
				)
			)
	...
	def update_status(self):
		if self.repair_status == "Pending" and self.asset_doc.status != "Out of Order":
			artech_engine.db.set_value("Asset", self.asset, "status", "Out of Order")
	...
	def on_cancel(self):
		self.asset_doc = artech_engine.get_doc("Asset", self.asset)
		if self.get("capitalize_repair_cost"):
			self.update_asset_value()
	...
	def after_delete(self):
		artech_engine.get_lazy_doc("Asset", self.asset).set_status()

	def check_repair_status(self):
		if self.repair_status == "Pending" and self.docstatus == 1:
			artech_engine.throw(_("Please update Repair Status."))
	...
	def validate_serial_no(self, stock_item):
		if not stock_item.serial_and_batch_bundle and artech_engine.get_cached_value(
			"Item", stock_item.item_code, "has_serial_no"
		):
			msg = f"Serial No Bundle is mandatory for Item {stock_item.item_code}"
			artech_engine.throw(_(msg), title=_("Missing Serial No Bundle"))
	...
	def get_gl_entries_for_consumed_items(self, gl_entries, fixed_asset_account):
		...
		stock_entry_name = artech_engine.db.get_value("Stock Entry", {"asset_repair": self.name}, "name")
		stock_entry_items = artech_engine.get_all(
		...
		if not artech.is_perpetual_inventory_enabled(self.company):
			default_expense_account = artech_engine.get_cached_value(
				"Company", self.company, "default_expense_account"
			)
			if not default_expense_account:
				artech_engine.throw(_("Please set default Expense Account in Company {0}").format(self.company))
...
def _get_expense_accounts_for_purchase_invoice(purchase_invoice: str) -> list[str]:
	...
	"""
	pi_items = artech_engine.db.get_all(
...
def get_unallocated_repair_cost(
	...
	artech_engine.has_permission("Purchase Invoice", "read", purchase_invoice, throw=True)
...
def get_allocated_repair_cost(
	...
	query = (
		artech_engine.qb.from_(asset_repair_pi)
