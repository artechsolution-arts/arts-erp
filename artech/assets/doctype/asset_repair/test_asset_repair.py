...
from artech_engine.utils import add_days, add_months, flt, get_first_day, nowdate, nowtime, today
...
from artech.assets.doctype.asset.test_asset import (
	create_asset,
...
class TestAssetRepair(ArtechTestSuite):
	def setUp(self):
	...
	def test_asset_status(self):
		...
		self.assertRaises(artech_engine.ValidationError, asset_repair.save)
	...
	def test_update_status(self):
		...
		asset_repair.save()
		asset_status = artech_engine.db.get_value("Asset", asset_repair.asset, "status")
	...
	def test_decrease_stock_quantity(self):
		...
		stock_entry = artech_engine.get_last_doc("Stock Entry")
	...
	def test_serialized_item_consumption(self):
		...
		asset_repair.repair_status = "Completed"
		self.assertRaises(artech_engine.ValidationError, asset_repair.submit)
	...
	def test_repair_cost_exceeds_available_amount(self):
		...
		asset_repair2 = artech_engine.new_doc("Asset Repair")
		...
		self.assertRaises(artech_engine.ValidationError, asset_repair2.save)

	def test_gl_entries_with_perpetual_inventory(self):
		company = "_Test Company with perpetual inventory"
		set_depreciation_settings_in_company(company)

		asset_category = artech_engine.get_doc("Asset Category", "Computers")
		...
		stock_entry_expense_account = (
			artech_engine.get_doc("Stock Entry", {"asset_repair": asset_repair.name}).get("items")[0].expense_account
		)
	...
	def test_gl_entries_with_periodical_inventory(self):
		artech_engine.db.set_value("Company", "_Test Company", "default_expense_account", "Cost of Goods Sold - _TC")
		asset_repair = create_asset_repair(
			capitalize_repair_cost=1,
			stock_consumption=1,
			increase_in_asset_life=1,
...
def create_asset_repair(**args):
	...
	if args.submit:
		...
		asset_repair.cost_center = artech_engine.db.get_value("Company", asset.company, "cost_center")
