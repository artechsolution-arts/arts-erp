...
from artech_engine.query_builder.functions import Max, Min
...
from artech.accounts.doctype.journal_entry.journal_entry import make_reverse_journal_entry
...
def post_depreciation_entries(date=None):
	...
	if not cint(artech_engine.get_single_value("Accounts Settings", "book_asset_depreciation_entry_automatically")):
		return
...
def book_depreciation_entries(date):
	...
	for data in depreciable_assets_data:
		...
		try:
			...
			error_log = artech_engine.log_error(e)
...
def get_depreciable_assets_data(date):
	a = artech_engine.qb.DocType("Asset")
	ads = artech_engine.qb.DocType("Asset Depreciation Schedule")
	ds = artech_engine.qb.DocType("Depreciation Schedule")

	res = (
		artech_engine.qb.from_(ads)
		.join(a)
		.on(ads.asset == a.name)
		.join(ds)
...
def get_companies_with_frozen_limits():
	...
	for d in artech_engine.get_all(
		...
		if (
			d.role_allowed_for_frozen_entries not in artech_engine.get_roles()
			and artech_engine.session.user != "Administrator"
		):
...
def make_depreciation_entry(
	...
	artech_engine.has_permission("Journal Entry", throw=True)
	date = date or today()

	depr_schedule_doc = artech_engine.get_doc("Asset Depreciation Schedule", depr_schedule_name)
	asset = artech_engine.get_doc("Asset", depr_schedule_doc.asset)
...
def get_credit_and_debit_accounts(accumulated_depreciation_account, depreciation_expense_account):
	root_type = artech_engine.get_value("Account", depreciation_expense_account, "root_type")
	...
	if root_type == "Expense":
		...
		artech_engine.throw(_("Depreciation Expense Account should be an Income or Expense Account."))
...
def set_depr_entry_posting_status_for_failed_assets(failed_asset_names):
	for asset_name in failed_asset_names:
		artech_engine.db.set_value("Asset", asset_name, "depr_entry_posting_status", "Failed")


def notify_depr_entry_posting_error(failed_asset_names, error_log_names):
	user_role = artech_engine.db.get_single_value("Accounts Settings", "role_to_notify_on_depreciation_failure")
...
def validate_asset_for_scrap(asset, scrap_date):
	if asset.docstatus != 1:
		artech_engine.throw(_("Asset {0} must be submitted").format(asset.name))
	elif asset.status in ("Cancelled", "Sold", "Scrapped", "Capitalized"):
		artech_engine.throw(_("Asset {0} cannot be scrapped, as it is already {1}").format(asset.name, asset.status))
...
def validate_scrap_date(asset, scrap_date):
	if scrap_date > getdate():
		artech_engine.throw(_("Future date is not allowed"))
	elif scrap_date < getdate(asset.purchase_date):
		artech_engine.throw(_("Scrap date cannot be before purchase date"))
	...
	if asset.calculate_depreciation:
		...
		if (
			...
			artech_engine.throw(_("Asset cannot be scrapped before the last depreciation entry."))


def get_last_depreciation_date(asset_name):
	depreciation = artech_engine.qb.DocType("Asset Depreciation Schedule")
	depreciation_schedule = artech_engine.qb.DocType("Depreciation Schedule")

	last_depreciation_date = (
		artech_engine.qb.from_(depreciation)
...
def create_journal_entry_for_scrap(asset, scrap_date):
	depreciation_series = artech_engine.get_cached_value("Company", asset.company, "series_for_depreciation_entry")
...
def create_reverse_depreciation_entry(asset_name, journal_entry):
	...
	artech_engine.flags.is_reverse_depr_entry = True
...
def update_value_after_depreciation_on_asset_restore(schedule, row, journal_entry):
	artech_engine.db.set_value("Depreciation Schedule", schedule.name, "journal_entry", None, update_modified=False)
...
def get_depreciation_accounts(asset_category, company):
	...
	accounts = artech_engine.db.get_value(
	...
	if not fixed_asset_account:
		artech_engine.throw(_("Please set Fixed Asset Account in Asset Category {0}").format(asset_category))

	if not non_depreciable_category:
		accounts = artech_engine.get_cached_value(
...
def get_disposal_account_and_cost_center(company: str):
	disposal_account, depreciation_cost_center = artech_engine.get_cached_value(
		"Company", company, ["disposal_account", "depreciation_cost_center"]
	)

	if not disposal_account:
		artech_engine.throw(_("Please set 'Gain/Loss Account on Asset Disposal' in Company {0}").format(company))
	if not depreciation_cost_center:
		artech_engine.throw(_("Please set 'Asset Depreciation Cost Center' in Company {0}").format(company))
