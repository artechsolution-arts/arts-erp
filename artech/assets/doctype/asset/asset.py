...
from artech.accounts.general_ledger import make_reverse_gl_entries
...
class Asset(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.asset_finance_book.asset_finance_book import AssetFinanceBook
	...
	def create_asset_depreciation_schedule(self):
		...
		for fb_row in self.get("finance_books"):
			...
			if not existing_schedule:
				new_schedule = artech_engine.new_doc("Asset Depreciation Schedule")
	...
	def before_submit(self):
		if self.asset_type == "Composite Asset" and not has_active_capitalization(self.name):
			if self.split_from and has_active_capitalization(self.split_from):
				return
			artech_engine.throw(_("Please capitalize this asset before submitting."))
	...
	def get_linked_item(self, purchase_doc_type, purchase_doc):
		purchase_doc = artech_engine.get_doc(purchase_doc_type, purchase_doc)
	...
	def validate_asset_and_reference(self):
		if self.purchase_invoice or self.purchase_receipt:
			...
			reference_doc = artech_engine.get_doc(reference_doc, reference_name)
		...
		if self.asset_type == "Existing Asset" and self.purchase_invoice:
			artech_engine.throw(_("Purchase Invoice cannot be made against an existing asset {0}").format(self.name))
	...
	def validate_item(self):
		...
		if not item:
			artech_engine.throw(_("Item {0} does not exist").format(self.item_code))
		elif item.disabled:
			artech_engine.throw(_("Item {0} has been disabled").format(self.item_code))
		elif not item.is_fixed_asset:
			artech_engine.throw(_("Item {0} must be a Fixed Asset Item").format(self.item_code))
		elif item.is_stock_item:
			artech_engine.throw(_("Item {0} must be a non-stock item").format(self.item_code))

	def validate_cost_center(self):
		if self.cost_center:
			cost_center_company, cost_center_is_group = artech_engine.db.get_value(
				"Cost Center", self.cost_center, ["company", "is_group"]
			)
			if cost_center_company != self.company:
				artech_engine.throw(
					_("Cost Center {} doesn't belong to Company {}").format(
						artech_engine.bold(self.cost_center), artech_engine.bold(self.company)
					),
			...
			if cost_center_is_group:
				artech_engine.throw(
					...
					).format(artech_engine.bold(self.cost_center)),
					title=_("Invalid Cost Center"),
				)

		else:
			if not artech_engine.get_cached_value("Company", self.company, "depreciation_cost_center"):
				artech_engine.throw(
					_(
	...
	def validate_in_use_date(self):
		if not self.available_for_use_date and self.asset_type != "Composite Component":
			artech_engine.throw(_("Available for use date is required"))
	...
	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = artech_engine.get_cached_value("Item", self.item_code, "asset_category")
	...
	def validate_category(self):
		non_depreciable_category = artech_engine.db.get_value(
	...
	def validate_asset_values(self):
		...
		if not flt(self.net_purchase_amount) and self.asset_type != "Composite Asset":
			artech_engine.throw(_("Net Purchase Amount is mandatory"), artech_engine.MandatoryError)
		...
		if is_cwip_accounting_enabled(self.asset_category):
			...
			if (
				...
				and not artech_engine.db.get_value("Purchase Invoice", self.purchase_invoice, "update_stock")
			):
				artech_engine.throw(
		...
		if not self.calculate_depreciation:
			...
			if not self.finance_books:
				artech_engine.throw(_("Enter depreciation details"))
			if self.is_fully_depreciated:
				artech_engine.throw(_("Depreciation cannot be calculated for fully depreciated assets"))

		if self.asset_type == "Existing Asset":
			return

		if self.available_for_use_date and getdate(self.available_for_use_date) < getdate(self.purchase_date):
			artech_engine.throw(_("Available-for-use Date should be after purchase date"))
	...
	def validate_linked_purchase_documents(self):
		...
		for fieldname, doctype in [
			...
			if artech_engine.db.get_value(doctype, purchase_doc, "docstatus") == 0:
				artech_engine.throw(
	...
	def validate_asset_qty_with_purchase_doc(self, doctype, purchase_doc):
		...
		existing_asset_qty = (
			artech_engine.qb.from_(Asset)
			.select(IfNull(Sum(Asset.asset_quantity), 0))
			.where((Asset.item_code == self.item_code) & (Asset.name != self.name) & (Asset.docstatus != 2))
			.where(asset_filter)
		).run()[0][0]

		PurchaseDoc = artech_engine.qb.DocType(doctype)
		PurchaseDocItems = artech_engine.qb.DocType(f"{doctype} Item")

		purchased_qty = (
			artech_engine.qb.from_(PurchaseDoc)
	...
	def validate_gross_and_purchase_amount(self):
		...
		if self.net_purchase_amount and self.net_purchase_amount != self.purchase_amount:
			...
			artech_engine.throw(error_message, title=_("Invalid Net Purchase Amount"))
	...
	def make_asset_movement(self):
		...
		if reference_docname:
			posting_date, posting_time = artech_engine.db.get_value(
		...
		asset_movement = artech_engine.get_doc(
			{
	...
	def validate_asset_finance_books(self, row):
		...
		if flt(row.expected_value_after_useful_life) < 0:
			artech_engine.throw(_("Row {0}: Expected Value After Useful Life cannot be negative").format(row.idx))
	...
	def validate_opening_depreciation_values(self, row):
		...
		if self.opening_accumulated_depreciation:
			if not self.opening_number_of_booked_depreciations:
				artech_engine.throw(_("Please set opening number of booked depreciations"))
		else:
			self.opening_number_of_booked_depreciations = 0
	...
	def validate_total_number_of_depreciations_and_frequency(self, row):
		...
		if row.frequency_of_depreciation <= 0:
			artech_engine.throw(_("Row #{0}: Frequency of Depreciation must be greater than zero").format(row.idx))
	...
	def validate_cancellation(self):
		...
		if self.status not in ("Submitted", "Partially Depreciated", "Fully Depreciated"):
			artech_engine.throw(_("Asset cannot be cancelled, as it is already {0}").format(self.status))

	def cancel_movement_entries(self):
		movements = artech_engine.db.sql(
		...
		for movement in movements:
			movement = artech_engine.get_doc("Asset Movement", movement.get("name"))
			movement.cancel()
		...
		if self.calculate_depreciation:
			for row in self.get("finance_books"):
				...
				for d in depr_schedule or []:
					if d.journal_entry:
						artech_engine.get_doc("Journal Entry", d.journal_entry).cancel()
		else:
			depr_entries = self.get_manual_depreciation_entries()

			for depr_entry in depr_entries or []:
				artech_engine.get_doc("Journal Entry", depr_entry.name).cancel()
	...
	def get_default_finance_book_idx(self):
		if not self.get("default_finance_book") and self.company:
			self.default_finance_book = artech.get_default_finance_book(self.company)
	...
	def validate_make_gl_entry(self):
		...
		if asset_bought_with_invoice:
			...
			expense_booked = artech_engine.db.sql(query, (purchase_document, fixed_asset_account), as_dict=1)
			if expense_booked:
				# if expense is already booked from invoice then do not make gl entries regardless of cwip enabled/disabled
				return False

			cwip_booked = artech_engine.db.sql(query, (purchase_document, cwip_account), as_dict=1)
			if cwip_booked:
	...
	def get_purchase_document(self):
		asset_bought_with_invoice = self.purchase_invoice and artech_engine.db.get_value(
		...
		if not fixed_asset_account:
			artech_engine.throw(
				_("Set {0} in asset category {1} for company {2}").format(
					artech_engine.bold(_("Fixed Asset Account")),
					artech_engine.bold(self.asset_category),
	...
	def check_asset_capitalization_gl_entries(self):
		if self.asset_type == "Composite Asset":
			result = artech_engine.db.get_value(
	...
	def get_depreciation_rate(self, args: str | dict | Document, on_validate: bool = False):
		...
		rate_field_precision = artech_engine.get_single_value("System Settings", "float_precision") or 2
...
def has_gl_entries(doctype, docname, target_account):
	...
	gl_entries = (
		artech_engine.qb.from_(gl_entry)
...
def update_maintenance_status():
	...
	for asset in assets:
		asset = artech_engine.get_doc("Asset", asset.name)
		if artech_engine.db.exists("Asset Repair", {"asset_name": asset.name, "repair_status": "Pending"}):
			asset.set_status("Out of Order")
		elif artech_engine.db.exists("Asset Maintenance Task", {"parent": asset.name, "next_due_date": today()}):
...
def make_post_gl_entry():
	asset_categories = artech_engine.db.get_all("Asset Category", fields=["name", "enable_cwip_accounting"])

	for asset_category in asset_categories:
		if cint(asset_category.enable_cwip_accounting):
			assets = artech_engine.db.sql_list(
			...
			for asset in assets:
				doc = artech_engine.get_doc("Asset", asset)
				doc.make_gl_entries()


def get_asset_naming_series():
	meta = artech_engine.get_meta("Asset")
...
def make_sales_invoice(asset: str, item_code: str, company: str, sell_qty: int, serial_no: str | None = None):
	asset_doc = artech_engine.get_doc("Asset", asset)
	si = artech_engine.new_doc("Sales Invoice")
	si.company = company
	si.currency = artech_engine.get_cached_value("Company", company, "default_currency")
...
def create_asset_maintenance(
	...
	asset_maintenance = artech_engine.new_doc("Asset Maintenance")
...
def create_asset_repair(
	...
	asset_repair = artech_engine.new_doc("Asset Repair")
...
def create_asset_capitalization(
	...
	asset_capitalization = artech_engine.new_doc("Asset Capitalization")
...
def create_asset_value_adjustment(
	...
	asset_value_adjustment = artech_engine.new_doc("Asset Value Adjustment")
...
def get_item_details(
	...
	asset_category_doc = artech_engine.get_cached_doc("Asset Category", asset_category)
	books = []
...
def get_asset_account(account_name, asset=None, asset_category=None, company=None):
	...
	if not account:
		account = artech_engine.get_cached_value("Company", company, account_name)

	if not account:
		if not asset_category:
			artech_engine.throw(_("Set {0} in company {1}").format(account_name.replace("_", " ").title(), company))
		else:
			artech_engine.throw(
...
def make_journal_entry(asset_name: str):
	asset = artech_engine.get_doc("Asset", asset_name)
	(
		fixed_asset_account,
	...
	depreciation_cost_center, depreciation_series = artech_engine.get_cached_value(
...
def make_asset_movement(
	...
	if len(assets) == 0:
		artech_engine.throw(_("At least one asset has to be selected."))

	asset_movement = artech_engine.new_doc("Asset Movement")
	asset_movement.purpose = purpose
	for asset in assets:
		asset = artech_engine.get_doc("Asset", asset.get("name"))
...
def is_cwip_accounting_enabled(asset_category):
	return cint(artech_engine.db.get_value("Asset Category", asset_category, "enable_cwip_accounting"))
...
def has_active_capitalization(asset: str):
	active_capitalizations = artech_engine.db.count(
...
def get_values_from_purchase_doc(
	...
	purchase_doc = artech_engine.get_doc(doctype, purchase_doc_name)
	matching_items = [item for item in purchase_doc.items if item.item_code == item_code]

	if not matching_items:
		artech_engine.throw(_(f"Selected {doctype} does not contain the Item Code {item_code}"))
...
def split_asset(asset_name: str, split_qty: int):
	...
	existing_asset = artech_engine.get_doc("Asset", asset_name)
...
def validate_split_quantity(existing_asset, split_qty):
	if split_qty >= existing_asset.asset_quantity:
		artech_engine.throw(_("Split Quantity must be less than Asset Quantity"))
...
def process_asset_split(existing_asset, split_qty, splitted_asset=None, is_new_asset=False):
	...
	new_asset = artech_engine.copy_doc(existing_asset) if is_new_asset else splitted_asset
...
def create_new_depr_schedule(current_depr_schedule_doc, existing_asset, new_asset, is_new_asset, fb_row):
	"""Create a new depreciation schedule based on the current one."""
	new_depr_schedule_doc = artech_engine.copy_doc(current_depr_schedule_doc)
...
def add_reference_in_jv_on_split(entry_name, new_asset_name, old_asset_name, depreciation_amount):
	...
	journal_entry = artech_engine.get_doc("Journal Entry", entry_name)
	entries_to_add = []
...
def adjust_existing_accounts(journal_entry, old_asset_name, depreciation_amount, entries_to_add):
	...
	for account in journal_entry.get("accounts"):
		if account.reference_name == old_asset_name:
			entries_to_add.append(artech_engine.copy_doc(account).as_dict())
