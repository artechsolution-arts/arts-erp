...
artech_engine.provide("artech.asset");
artech_engine.provide("artech.accounts.dimensions");

artech_engine.ui.form.on("Asset", {
	onload: function (frm) {
	...
	setup: function (frm) {
		...
		frm.make_methods = {
			"Asset Movement": () => {
				artech_engine.call({
					method: "artech.assets.doctype.asset.asset.make_asset_movement",
					freeze: true,
					args: {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				query: "artech.controllers.queries.get_purchase_receipts",
				filters: { item_code: doc.item_code },
			};
		});
		frm.set_query("purchase_invoice", (doc) => {
			return {
				query: "artech.controllers.queries.get_purchase_invoices",
				filters: { item_code: doc.item_code },
			};
		});
	...
	before_submit: function (frm) {
		if (frm.doc.asset_type == "Composite Asset" && !frm.has_active_capitalization) {
			artech_engine.throw(__("Please capitalize this asset before submitting."));
		}
	},
	...
	refresh: async function (frm) {
		...
		artech_engine.ui.form.trigger("Asset", "asset_type");
		...
		if (frm.doc.docstatus == 1) {
			...
			if (["Submitted", "Partially Depreciated", "Fully Depreciated"].includes(frm.doc.status)) {
				...
				frm.add_custom_button(
					__("Transfer Asset"),
					function () {
						artech.asset.transfer_asset(frm);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(__("Restore Asset"), function () {
					artech.asset.restore_asset(frm);
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.asset_type == "Composite Asset") {
				artech_engine.call({
					method: "artech.assets.doctype.asset.asset.has_active_capitalization",
					args: {
						asset: frm.doc.name,
					},
					callback: function (r) {
	...
	set_dynamic_labels: function (frm) {
		frm.set_currency_labels(["net_purchase_amount"], artech.get_currency(frm.doc.company));
	},
	...
	should_show_accounting_ledger: async function (frm) {
		...
		const asset_category = await artech_engine.db.get_value(
			"Asset Category",
	...
	make_journal_entry: function (frm) {
		artech_engine.call({
			method: "artech.assets.doctype.asset.asset.make_journal_entry",
			args: {
	...
	render_depreciation_schedule_view: function (frm, asset_depr_schedule_doc) {
		...
		asset_depr_schedule_doc.depreciation_schedule.forEach((sch) => {
			const row = [
				sch["idx"],
				artech_engine.format(sch["schedule_date"], { fieldtype: "Date" }),
				artech_engine.format(sch["depreciation_amount"], {
					fieldtype: "Currency",
					options: "Company:company:default_currency",
				}),
				artech_engine.format(sch["accumulated_depreciation_amount"], {
					fieldtype: "Currency",
					options: "Company:company:default_currency",
				}),
				sch["journal_entry"] || "",
		...
		let datatable = new artech_engine.DataTable(wrapper.get(0), {
			columns: columns,
			data: data,
			layout: "fluid",
	...
	setup_chart_and_depr_schedule_view: async function (frm) {
		...
		var x_intervals = [artech_engine.format(frm.doc.purchase_date, { fieldtype: "Date" })];
		var asset_values = [frm.doc.net_purchase_amount];

		if (frm.doc.calculate_depreciation) {
			if (frm.doc.opening_accumulated_depreciation) {
				var depreciation_date = artech_engine.datetime.add_months(
					frm.doc.finance_books[0].depreciation_start_date,
					-1 * frm.doc.finance_books[0].frequency_of_depreciation
				);
				x_intervals.push(artech_engine.format(depreciation_date, { fieldtype: "Date" }));
				asset_values.push(
					flt(
			...
			let asset_depr_schedule_doc = (
				await artech_engine.call(
					"artech.assets.doctype.asset_depreciation_schedule.asset_depreciation_schedule.get_asset_depr_schedule_doc",
					{
						asset_name: frm.doc.name,
						status: "Active",
			...
			$.each(asset_depr_schedule_doc.depreciation_schedule || [], function (i, v) {
				x_intervals.push(artech_engine.format(v.schedule_date, { fieldtype: "Date" }));
				var asset_value = flt(
			...
			if (frm.doc.opening_accumulated_depreciation) {
				x_intervals.push(artech_engine.format(frm.doc.creation.split(" ")[0], { fieldtype: "Date" }));
				asset_values.push(
					flt(
						frm.doc.net_purchase_amount - frm.doc.opening_accumulated_depreciation,
						precision("net_purchase_amount")
			...
			$.each(depr_entries || [], function (i, v) {
				x_intervals.push(artech_engine.format(v.posting_date, { fieldtype: "Date" }));
		...
		if (["Scrapped", "Sold"].includes(frm.doc.status)) {
			x_intervals.push(artech_engine.format(frm.doc.disposal_date, { fieldtype: "Date" }));
			asset_values.push(0);
		}
	...
	set_finance_book: function (frm) {
		artech_engine.call({
			method: "artech.assets.doctype.asset.asset.get_item_details",
			args: {
	...
	create_asset_maintenance: function (frm) {
		artech_engine.call({
			...
			method: "artech.assets.doctype.asset.asset.create_asset_maintenance",
			callback: function (r) {
				var doclist = artech_engine.model.sync(r.message);
				artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_repair: function (frm) {
		artech_engine.call({
			...
			method: "artech.assets.doctype.asset.asset.create_asset_repair",
			callback: function (r) {
				var doclist = artech_engine.model.sync(r.message);
				artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	create_asset_capitalization: function (frm) {
		artech_engine.call({
			...
			method: "artech.assets.doctype.asset.asset.create_asset_capitalization",
			callback: function (r) {
				var doclist = artech_engine.model.sync(r.message);
				artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
			},
	...
	sell_asset: function (frm) {
		const make_sales_invoice = (sell_qty) => {
			artech_engine.call({
				method: "artech.assets.doctype.asset.asset.make_sales_invoice",
				args: {
					asset: frm.doc.name,
					item_code: frm.doc.item_code,
					company: frm.doc.company,
		...
		dialog.set_primary_action(__("Sell"), function () {
			...
			if (sell_qty <= 0) {
				artech_engine.throw(__("Sell quantity must be greater than zero"));
			}

			if (sell_qty > asset_qty) {
				artech_engine.throw(__("Sell quantity cannot exceed the asset quantity"));
			}
	...
	split_asset: function (frm) {
		...
		dialog.set_primary_action(__("Split"), function () {
			...
			artech_engine.call({
				...
				method: "artech.assets.doctype.asset.asset.split_asset",
				callback: function (r) {
					let doclist = artech_engine.model.sync(r.message);
					artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
				},
			});
	...
	create_asset_value_adjustment: function (frm) {
		artech_engine.call({
			...
			method: "artech.assets.doctype.asset.asset.create_asset_value_adjustment",
			freeze: 1,
			callback: function (r) {
				var doclist = artech_engine.model.sync(r.message);
				artech_engine.set_route("Form", doclist[0].doctype, doclist[0].name);
	...
	set_values_from_purchase_doc: (frm, doctype) => {
		artech_engine.call({
			method: "artech.assets.doctype.asset.asset.get_values_from_purchase_doc",
			args: {
	...
	set_depreciation_rate: function (frm, row) {
		if (
			...
			artech_engine.call({
				...
				callback: function (r) {
					if (r.message) {
						artech_engine.flags.dont_change_rate = true;
	...
	set_salvage_value_percentage_or_expected_value_after_useful_life: function (
		...
		if (expected_value_after_useful_life_changed) {
			artech_engine.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = true;
			...
			artech_engine.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life = false;
...
artech_engine.ui.form.on("Asset Finance Book", {
	...
	expected_value_after_useful_life: function (frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (!artech_engine.flags.from_set_salvage_value_percentage_or_expected_value_after_useful_life) {
	...
	rate_of_depreciation: function (frm, cdt, cdn) {
		if (!artech_engine.flags.dont_change_rate) {
			artech_engine.model.set_value(cdt, cdn, "expected_value_after_useful_life", 0);
		}

		artech_engine.flags.dont_change_rate = false;
	},

	depreciation_start_date: function (frm, cdt, cdn) {
		const book = locals[cdt][cdn];
		if (frm.doc.available_for_use_date && book.depreciation_start_date < frm.doc.available_for_use_date) {
			artech_engine.msgprint(__("Depreciation Posting Date cannot be before Available-for-use Date"));
...
artech.asset.scrap_asset = function (frm) {
	var scrap_dialog = new artech_engine.ui.Dialog({
		...
		primary_action(values) {
			artech_engine.call({
				...
				method: "artech.assets.doctype.asset.depreciation.scrap_asset",
				callback: function (r) {
					frm.reload_doc();
					scrap_dialog.hide();
				},
...
artech.asset.restore_asset = function (frm) {
	artech_engine.confirm(__("Do you really want to restore this scrapped asset?"), function () {
		artech_engine.call({
			args: {
				asset_name: frm.doc.name,
			},
			method: "artech.assets.doctype.asset.depreciation.restore_asset",
			callback: (r) => frm.reload_doc(),
		});
	});
};

artech.asset.transfer_asset = function (frm) {
	artech_engine.call({
		method: "artech.assets.doctype.asset.asset.make_asset_movement",
		freeze: true,
		args: {
