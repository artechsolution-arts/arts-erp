...
from artech_engine.utils.data import add_to_date
...
from artech.assets.doctype.asset.asset import (
	make_sales_invoice,
	split_asset,
...
class AssetSetup(ArtechTestSuite):
	def setUp(self):
...
class TestAsset(AssetSetup):
	...
	def test_net_purchase_amount_is_mandatory(self):
		...
		self.assertRaises(artech_engine.MandatoryError, asset.save)
	...
	def test_pr_or_pi_mandatory_if_not_existing_asset(self):
		...
		self.assertRaises(artech_engine.ValidationError, asset.save)
	...
	def test_validate_item(self):
		asset = create_asset(item_code="MacBook Pro", do_not_save=1)
		item = artech_engine.get_doc("Item", "MacBook Pro")
	...
	def test_purchase_asset(self):
		...
		asset_name = artech_engine.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
	...
	def test_is_fixed_asset_set(self):
		...
		doc = artech_engine.new_doc("Purchase Invoice")
	...
	def test_scrap_asset(self):
		...
		asset_depreciation = artech_engine.db.get_value(
			"Asset Depreciation Schedule", {"asset": asset.name, "docstatus": 1}, "name"
		)
		last_booked_depreciation_date = artech_engine.db.get_value(
		...
		self.assertRaises(artech_engine.ValidationError, scrap_asset, asset.name, scrap_date=before_purchase_date)
		self.assertRaises(artech_engine.ValidationError, scrap_asset, asset.name, scrap_date=future_date)
		self.assertRaises(
			artech_engine.ValidationError, scrap_asset, asset.name, scrap_date=before_last_booked_depreciation_date
		)
	...
	def test_gle_made_by_asset_sale(self):
		...
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "status"), "Sold")
		...
		si.cancel()
		self.assertEqual(artech_engine.db.get_value("Asset", asset.name, "status"), "Partially Depreciated")
	...
	def test_asset_splitting(self):
		...
		jv = artech_engine.get_doc("Journal Entry", journal_entry)
	...
	def test_cwip_accounting(self):
		...
		asset = artech_engine.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
	...
	def test_asset_cwip_toggling_cases(self):
		cwip = artech_engine.db.get_value("Asset Category", "Computers", "enable_cwip_accounting")
		...
		artech_engine.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", "")
		...
		asset = artech_engine.db.get_value("Asset", {"purchase_invoice": pi.name, "docstatus": 0}, "name")
		asset_doc = artech_engine.get_doc("Asset", asset)
		...
		artech_engine.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		asset = artech_engine.db.get_value("Asset", {"purchase_receipt": pr.name, "docstatus": 0}, "name")
		asset_doc = artech_engine.get_doc("Asset", asset)
		asset_doc.available_for_use_date = nowdate()
		asset_doc.calculate_depreciation = 0
		...
		artech_engine.db.set_value("Asset Category", "Computers", "enable_cwip_accounting", cwip)
		artech_engine.db.set_value("Asset Category Account", name, "capital_work_in_progress_account", cwip_acc)
		artech_engine.db.get_value("Company", "_Test Company", "capital_work_in_progress_account", cwip_acc)
	...
	def test_asset_splitting_for_non_existing_asset(self):
		...
		artech_engine.db.set_value("Item", asset_item, "is_grouped_asset", 1)
		...
		artech_engine.db.set_value("Item", asset_item, "is_grouped_asset", 0)
	...
	def test_depreciation_accounts_is_set_for_depreciable_assets(self):
		company_depreciation_accounts = artech_engine.db.get_value(
			"Company",
			"_Test Company",
			[
		...
		if artech_engine.db.exists("Asset Category", asset_category_name):
			filters = {
			...
			asset_category_account = artech_engine.db.get_value(
		...
		try:
			...
			artech_engine.db.set_value("Company", "_Test Company", company_depreciation_accounts)
...
class TestDepreciationMethods(AssetSetup):
	def setUp(self):
		artech_engine.db.set_single_value("System Settings", "float_precision", 2)
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_number_of_depreciations(self):
		...
		self.assertRaises(artech_engine.ValidationError, asset_2.save)
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_expense_account(self):
		...
		je = artech_engine.get_doc("Journal Entry", get_depr_schedule(asset.name, "Active")[0].journal_entry)
		accounting_entries = [
	...
	def test_depr_entry_posting_when_depr_expense_account_is_an_income_account(self):
		depr_expense_account = artech_engine.get_doc("Account", "_Test Depreciations - _TC")
	...
	def test_depreciation_entry_cancellation(self):
		...
		artech_engine.get_doc("Journal Entry", depr_entry).cancel()
	...
	def test_gle_made_by_depreciation_entries(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "series_for_depreciation_entry", "DEPR-")
		post_depreciation_entries(date="2021-01-01")
		asset.load_from_db()
	...
	def test_asset_cost_center(self):
		...
		self.assertRaises(artech_engine.ValidationError, asset.submit)
	...
	def test_manual_depreciation_with_incorrect_jv_voucher_type(self):
		...
		self.assertRaises(artech_engine.ValidationError, jv.insert)
	...
	def test_split_asset_created_via_capitalization(self):
		...
		from artech.assets.doctype.asset_capitalization.test_asset_capitalization import (
		...
		target_asset = artech_engine.get_doc("Asset", asset_capitalization.target_asset)
...
def set_depreciation_settings_in_company(company=None):
	...
	company = artech_engine.get_doc("Company", company)
	...
	artech_engine.db.set_single_value("Accounts Settings", "book_asset_depreciation_entry_automatically", 1)


def enable_cwip_accounting(asset_category, enable=1):
	artech_engine.db.set_value("Asset Category", asset_category, "enable_cwip_accounting", enable)
