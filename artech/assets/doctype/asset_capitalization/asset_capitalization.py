...
import artech
from artech.assets.doctype.asset.asset import get_asset_value_after_depreciation
...
class AssetCapitalization(StockController):
	...
	if TYPE_CHECKING:
		...
		from artech.assets.doctype.asset_capitalization_asset_item.asset_capitalization_asset_item import (
			AssetCapitalizationAssetItem,
		)
		from artech.assets.doctype.asset_capitalization_service_item.asset_capitalization_service_item import (
			AssetCapitalizationServiceItem,
		)
		from artech.assets.doctype.asset_capitalization_stock_item.asset_capitalization_stock_item import (
	...
	def validate_target_item(self):
		target_item = artech_engine.get_cached_doc("Item", self.target_item_code)

		if not target_item.is_fixed_asset:
			artech_engine.throw(_("Target Item {0} must be a Fixed Asset item").format(target_item.name))
	...
	def validate_target_asset(self):
		if self.target_asset:
			...
			if not target_asset.asset_type == "Composite Asset":
				artech_engine.throw(_("Target Asset {0} needs to be composite asset").format(target_asset.name))
			...
			if target_asset.docstatus == 1:
				artech_engine.throw(_("Target Asset {0} cannot be submitted").format(target_asset.name))
			elif target_asset.docstatus == 2:
				artech_engine.throw(_("Target Asset {0} cannot be cancelled").format(target_asset.name))
	...
	def validate_consumed_stock_item(self):
		for d in self.stock_items:
			if d.item_code:
				item = artech_engine.get_cached_doc("Item", d.item_code)

				if not item.is_stock_item:
					artech_engine.throw(_("Row #{0}: Item {1} is not a stock item").format(d.idx, d.item_code))

				if flt(d.stock_qty) <= 0:
					artech_engine.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))
		...
		for d in self.asset_items:
			if d.asset:
				...
				if asset.docstatus == 0:
					artech_engine.throw(_("Row #{0}: Consumed Asset {1} cannot be Draft").format(d.idx, asset.name))
		...
		for d in self.service_items:
			if d.item_code:
				...
				if item.is_stock_item or item.is_fixed_asset:
					artech_engine.throw(_("Row #{0}: Item {1} is not a service item").format(d.idx, d.item_code))

				if flt(d.qty) <= 0:
					artech_engine.throw(_("Row #{0}: Qty must be a positive number").format(d.idx))

				if flt(d.rate) <= 0:
					artech_engine.throw(_("Row #{0}: Amount must be a positive number").format(d.idx))

				self.validate_item(item)

			if not d.cost_center:
				d.cost_center = artech_engine.get_cached_value("Company", self.company, "cost_center")
	...
	def get_target_account(self):
		...
		asset_category = artech_engine.get_cached_value("Asset", self.target_asset, "asset_category")
	...
	def get_gl_entries_for_consumed_asset_items(self, gl_entries, target_account, target_against, precision):
		...
		for item in self.asset_items:
			asset = artech_engine.get_doc("Asset", item.asset)
	...
	def get_composite_component_value(self):
		...
		for item in self.asset_items:
			asset = artech_engine.db.get_value("Asset", item.asset, ["asset_type"], as_dict=True)
	...
	def update_target_asset(self):
		...
		asset_doc = artech_engine.get_doc("Asset", self.target_asset)
...
def get_target_asset_details(asset: str | None = None, company: str | None = None):
	...
	# Get Asset Details
	asset_details = artech_engine._dict()
	if asset:
		asset_details = artech_engine.db.get_value("Asset", asset, ["asset_name", "item_code"], as_dict=1)
		if not asset_details:
			artech_engine.throw(_("Asset {0} does not exist").format(asset))
...
def get_consumed_stock_item_details(ctx: ItemDetailsCtx):
	...
	if ctx.item_code and out.warehouse:
		incoming_rate_args = artech_engine._dict(
			{
...
@artech_engine.whitelist()
@artech.normalize_ctx_input(ItemDetailsCtx)
def get_warehouse_details(ctx: ItemDetailsCtx) -> artech_engine._dict:
	out = artech_engine._dict()
...
@artech_engine.whitelist()
@artech.normalize_ctx_input(ItemDetailsCtx)
def get_consumed_asset_details(ctx: ItemDetailsCtx) -> artech_engine._dict:
	out = artech_engine._dict()

	asset_details = artech_engine._dict()
	if ctx.asset:
		asset_details = artech_engine.db.get_value(
			"Asset", ctx.asset, ["asset_name", "item_code", "item_name"], as_dict=1
		)
		if not asset_details:
			artech_engine.throw(_("Asset {0} does not exist").format(ctx.asset))
	...
	# Cost Center
	if asset_details.item_code:
		item = artech_engine.get_cached_doc("Item", asset_details.item_code)
		item_defaults = get_item_defaults(item.name, ctx.company)
		item_group_defaults = get_item_group_defaults(item.name, ctx.company)
		brand_defaults = get_brand_defaults(item.name, ctx.company)
		out.cost_center = get_default_cost_center(ctx, item_defaults, item_group_defaults, brand_defaults)
...
@artech_engine.whitelist()
@artech.normalize_ctx_input(ItemDetailsCtx)
def get_service_item_details(ctx: ItemDetailsCtx) -> artech_engine._dict:
	out = artech_engine._dict()
...
def process_stock_item(d):
	stock_capitalized = artech_engine.db.exists(
	...
	stock_item_data = artech_engine._dict(d)
