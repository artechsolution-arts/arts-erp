...
from artech_engine.utils import cint, flt, now_datetime
...
class TestAssetCapitalization(ArtechTestSuite):
	def setUp(self):
		set_depreciation_settings_in_company()
	...
	def test_capitalization_with_periodical_inventory(self):
		...
		# Test General Ledger Entries
		default_expense_account = artech_engine.db.get_value("Company", company, "default_expense_account")
		expected_gle = {
	...
	def test_capitalize_composite_component(self):
		...
		consumed_asset_name = artech_engine.db.get_value("Asset", {"purchase_receipt": pr.name}, "name")
		consumed_asset_doc = artech_engine.get_doc("Asset", consumed_asset_name)
...
def create_asset_capitalization(**args):
	...
	now = now_datetime()
	target_asset = artech_engine.get_doc("Asset", args.target_asset) if args.target_asset else artech_engine._dict()
				...
				artech_engine._dict(
					{
						...
						"company": artech_engine.get_cached_value("Warehouse", source_warehouse, "company"),
...
def create_depreciation_asset(**args):
	...
	asset = artech_engine.new_doc("Asset")
