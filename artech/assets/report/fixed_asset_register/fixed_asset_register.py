...
from artech_engine.utils import add_months, cstr, flt, formatdate, getdate, nowdate, today
...
def get_data(filters):
	...
	group_by = artech_engine.scrub(filters.get("group_by"))
	...
	assets_record = artech_engine.db.get_all("Asset", filters=conditions, fields=fields)
...
def get_conditions(filters):
	...
	date_field = artech_engine.scrub(filters.date_based_on or "Purchase Date")
...
def prepare_chart_data(data, filters):
	...
	if filters.filter_based_on not in ("Date Range", "Fiscal Year"):
		...
		date_field = artech_engine.scrub(filters.date_based_on)
	...
	for d in period_list:
		labels_values_map.setdefault(
			d.get("label"), artech_engine._dict({"asset_value": 0, "depreciated_amount": 0})
		)
...
def get_assets_linked_to_fb(filters):
	afb = artech_engine.qb.DocType("Asset Finance Book")

	query = artech_engine.qb.from_(afb).select(
		afb.parent,
	)
	...
	if filters.include_default_book_assets:
		...
		if filters.finance_book and company_fb and cstr(filters.finance_book) != cstr(company_fb):
			artech_engine.throw(_("To use a different finance book, please uncheck 'Include Default FB Assets'"))
...
def get_asset_depreciation_amount_map(filters, finance_book):
	...
	aca = artech_engine.qb.DocType("Asset Category Account")
...
def get_group_by_data(
	...
	assets = artech_engine.db.get_all("Asset", filters=conditions, fields=fields)
		...
		return [
			{
				...
				"fieldtype": "Link",
				"fieldname": artech_engine.scrub(filters.get("group_by")),
