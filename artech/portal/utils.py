...
def set_default_role(doc, method):
	...
	if artech_engine.flags.setting_role or artech_engine.flags.in_migrate:
		return

	roles = artech_engine.get_roles(doc.name)

	contact_name = artech_engine.get_value("Contact", dict(email_id=doc.email))
	if contact_name:
		contact = artech_engine.get_doc("Contact", contact_name)
		for link in contact.links:
			artech_engine.flags.setting_role = True
...
def create_customer_or_supplier():
	...
	if artech_engine.db.get_value("User", user, "user_type") != "Website User":
		return

	user_roles = artech_engine.get_roles()
	portal_settings = artech_engine.get_single("Portal Settings")
	...
	fullname = artech_engine.utils.get_fullname(user)
...
def create_party(doctype, fullname):
	party = artech_engine.new_doc(doctype)
...
def party_exists(doctype, user):
	...
	contact_name = artech_engine.db.get_value("Contact", {"email_id": user})
	if contact_name:
		contact = artech_engine.get_doc("Contact", contact_name)
