class VisualPlantFloor {
	...
	prepare_data() {
		artech_engine.call({
			method: "artech.manufacturing.doctype.workstation.workstation.get_workstations",
			args: {
	...
	add_filter() {
		this.plant_floor = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Plant Floor",
		...
		this.workstation_type = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Link",
		...
		this.workstation = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Link",
				options: "Workstation",
		...
		this.workstation_status = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Select",
				options: "\nProduction\nOff\nIdle\nProblem\nMaintenance\nSetup",
	...
	render_workstations() {
		...
		let template = artech_engine.render_template("visual_plant_floor_template", {
	...
	update_status(data) {
		...
		if (data.status_image) {
			...
			workstation_card$
				.find(".workstation-image-container")
				.append(
					`<div class="workstation-image-cls workstation-abbr" style="margin:6px; height:82px">${artech_engine.get_abbr(
						data.name,
						2
					)}</div>`
				);
...
artech_engine.ui.VisualPlantFloor = VisualPlantFloor;
