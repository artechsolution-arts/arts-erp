...
artech_engine.provide("artech.stock.utils");

$.extend(artech, {
	get_currency: function (company) {
		if (!company && cur_frm) company = cur_frm.doc.company;
		if (company)
			return artech_engine.get_doc(":Company", company)?.default_currency || artech_engine.boot.sysdefaults.currency;
		else return artech_engine.boot.sysdefaults.currency;
	},

	get_presentation_currency_list: () => {
		const docs = artech_engine.boot.docs;
	...
	toggle_serial_batch_fields(frm) {
		let hide_fields = cint(artech_engine.user_defaults?.enable_serial_and_batch_no_for_item) === 0 ? 1 : 0;
		if (!hide_fields) {
			return;
		}
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (artech.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", artech.last_selected_company);
			}
		}
	},
	...
	stale_rate_allowed: () => {
		return cint(artech_engine.boot.sysdefaults.allow_stale);
	},
	...
	route_to_adjustment_jv: (args) => {
		artech_engine.model.with_doctype("Journal Entry", () => {
			// route to adjustment Journal Entry to handle Account Balance and Stock Value mismatch
			let journal_entry = artech_engine.model.get_new_doc("Journal Entry");

			args.accounts.forEach((je_account) => {
				let child_row = artech_engine.model.add_child(journal_entry, "accounts");
			...
			artech_engine.set_route("Form", "Journal Entry", journal_entry.name);
		});
	},

	route_to_pending_reposts: (args) => {
		artech_engine.set_route("List", "Repost Item Valuation", args);
	},
});
...
$.extend(artech.utils, {
		...
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			...
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					artech.utils.add_indicator_for_multicompany(frm, info);
				});
		...
		if (bundle_ids?.length) {
			frm.add_custom_button(
				...
				() => {
					...
					artech_engine.set_route("query-report", "Serial and Batch Summary");
				},
				__("View")
			);
		}
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return artech_engine.call({
				method: "artech.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	make_bank_account: function (doctype, docname) {
		artech_engine.new_doc("Bank Account", {
			party_type: doctype,
			party: docname,
		});
	},
	...
	add_dimensions: function (report_name, index) {
		let filters = artech_engine.query_reports[report_name].filters;
					...
					if (!found) {
						filters.splice(index, 0, {
							...
							get_data: function (txt) {
								return artech_engine.db.get_link_options(dimension["document_type"], txt);
							},
						});
					}
				});
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		artech_engine.call({
			method: "artech.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
							...
							filters.splice(index, 0, {
								...
								depends_on:
									report_name === "Stock Balance"
										...
										: "",
								get_data: function (txt) {
									return artech_engine.db.get_link_options(dimension["doctype"], txt);
								},
							});
						} else {
	...
	make_subscription: function (doctype, docname) {
		artech_engine.call({
			method: "artech_engine.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
			args: {
				doctype: doctype,
				docname: docname,
			},
	...
	get_tree_options: function (option) {
		...
		let unscrub_option = artech_engine.model.unscrub(option);
		let user_permission = artech_engine.defaults.get_user_permissions();
		let options;
	...
	get_tree_default: function (option) {
		...
		if (options.includes(artech_engine.defaults.get_default(option))) {
			return artech_engine.defaults.get_default(option);
		} else {
			return options[0];
		}
	},
	overrides_parent_value_in_all_rows: function (doc, dt, dn, table_fieldname, fieldname, parent_fieldname) {
		if (doc[parent_fieldname]) {
			...
			artech_engine.refresh_field(table_fieldname);
		}
	},
	create_new_doc: function (doctype, update_fields) {
		artech_engine.model.with_doctype(doctype, function () {
			var new_doc = artech_engine.model.get_new_doc(doctype);
			for (let [key, value] of Object.entries(update_fields)) {
				new_doc[key] = value;
			}
			artech_engine.ui.form.make_quick_entry(doctype, null, null, new_doc);
		});
	},

	// check if payments app is installed on site, if not warn user.
	check_payments_app: () => {
		if (artech_engine.boot.versions && !artech_engine.boot.versions.payments) {
			const marketplace_link =
				'<a href="https://artech_enginecloud.com/marketplace/apps/payments">Marketplace</a>';
			const github_link = '<a href="https://github.com/artech_engine/payments/">GitHub</a>';
	...
	pick_serial_and_batch_bundle(frm, cdt, cdn, type_of_transaction, warehouse_field) {
		let item_row = artech_engine.get_doc(cdt, cdn);
		item_row.type_of_transaction = type_of_transaction;

		artech_engine.db.get_value("Item", item_row.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		if (!artech_engine.boot.setup_complete) {
			return;
		}
		const today = artech_engine.datetime.get_today();
		if (!date) {
			date = today;
		}
		...
		let fiscal_year = "";
		if (
			artech_engine.boot.current_fiscal_year &&
			date >= artech_engine.boot.current_fiscal_year[1] &&
			date <= artech_engine.boot.current_fiscal_year[2]
		) {
			if (with_dates) fiscal_year = artech_engine.boot.current_fiscal_year;
			else fiscal_year = artech_engine.boot.current_fiscal_year[0];
		} else if (today != date) {
			artech_engine.call({
				method: "artech.accounts.utils.get_fiscal_year",
	...
	set_letter_head: function (frm) {
		if (frm.fields_dict.letter_head) {
			artech_engine.db.get_value("Company", frm.doc.company, "default_letter_head").then((res) => {
...
artech.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
	...
	const dialog = new artech_engine.ui.Dialog({
					...
					},
					{
						...
						onchange: function () {
							...
							if (item_code && warehouse) {
								artech_engine.call({
									method: "artech.stock.utils.get_latest_stock_qty",
									args: {
										item_code: item_code,
										warehouse: warehouse,
									},
						...
						get_query: (e) => {
							return {
								query: "artech.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
		...
		primary_action: function () {
			...
			alternative_items.forEach((d) => {
				let row = artech_engine.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				...
				artech_engine.model.set_value(row.doctype, row.name, "qty", qty);
				artech_engine.model.set_value(row.doctype, row.name, opts.original_item_field, d.item_code);
...
artech.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	const cannot_add_row = typeof opts.cannot_add_row === "undefined" ? true : opts.cannot_add_row;
	const child_docname = typeof opts.cannot_add_row === "undefined" ? "items" : opts.child_docname;
	const child_meta = artech_engine.get_meta(`${frm.doc.doctype} Item`);
	...
	let dialog = new artech_engine.ui.Dialog({
		...
		update_items: function () {
			...
			artech_engine.call({
				method: "artech.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
artech.utils.map_current_doc = function (opts) {
	function _map() {
		if ($.isArray(cur_frm.doc.items) && cur_frm.doc.items.length > 0) {
			...
			var items_doctype = artech_engine.meta.get_docfield(cur_frm.doctype, "items").options;

			// find the link fieldname from items table for the given
			// source_doctype
			var link_fieldname = null;
			artech_engine.get_meta(items_doctype).fields.forEach(function (d) {
			...
			if (already_set) {
				opts.source_name.forEach(function (src) {
					artech_engine.model.with_doc(opts.source_doctype, src, function (r) {
						var source_doc = artech_engine.model.get_doc(opts.source_doctype, src);
		...
		return artech_engine.call({
			...
			type: "POST",
			method: "artech_engine.model.mapper.map_docs",
			args: {
				method: opts.method,
	...
	if (opts.source_doctype) {
		...
		if (["Purchase Receipt", "Delivery Note", "Purchase Invoice"].includes(opts.source_doctype)) {
			let target_meta = artech_engine.get_meta(cur_frm.doc.doctype);
		...
		const d = new artech_engine.ui.form.MultiSelectDialog({
			...
			action: function (selections, args) {
				...
				if (values.length === 0) {
					artech_engine.msgprint(__("Please select {0}", [opts.source_doctype]));
					return;
				}
...
artech_engine.form.link_formatters["Item"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "item_name");
};

artech_engine.form.link_formatters["Employee"] = function (value, doc, df) {
	return add_link_title(value, doc, df, "employee_name");
};

artech_engine.form.link_formatters["Project"] = function (value, doc, df) {
...
$(document).on("app_ready", function () {
	if (!artech_engine.datetime.is_timezone_same()) {
		$.each(
			[
				"Stock Reconciliation",
				"Stock Entry",
			...
			function (i, d) {
				artech_engine.ui.form.on(d, "onload", function (frm) {
					cur_frm.set_df_property("posting_time", "description", artech_engine.sys_defaults.time_zone);
				});
			}
		);
	}
...
// Show SLA dashboard
$(document).on("app_ready", function () {
	$.each(artech_engine.boot.service_level_agreement_doctypes, function (_i, d) {
		artech_engine.ui.form.on(d, {
			onload: function (frm) {
				if (!frm.doc.service_level_agreement) return;

				artech_engine.call({
					method: "artech.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
			...
			refresh: function (frm) {
				if (
					...
					artech_engine.call({
						method: "artech_engine.client.get",
						args: {
...
function get_time_left(timestamp, agreement_status) {
	const diff = moment(timestamp).diff(artech_engine.datetime.system_datetime(true));
...
$.extend(artech.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new artech.utils.BarcodeScanner({ frm: frm });
