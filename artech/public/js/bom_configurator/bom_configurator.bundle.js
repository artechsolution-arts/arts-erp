class BOMConfigurator {
	...
	make() {
		...
		artech_engine.views.trees["BOM Configurator"] = new artech_engine.views.TreeView(options);
		let node = artech_engine.views.trees["BOM Configurator"].tree.root_node;
		artech_engine.views.trees["BOM Configurator"].tree.show_toolbar(node);
		artech_engine.views.trees["BOM Configurator"].tree.load_children(node, true);
		this.tree_view = artech_engine.views.trees["BOM Configurator"];
	}

	bind_events() {
		artech_engine.views.trees["BOM Configurator"].events = {
			frm: this.frm,
	...
	tree_options() {
		return {
			...
			get_tree_nodes: "artech.manufacturing.doctype.bom_creator.bom_creator.get_children",
	...
	tree_methods() {
		let frm_obj = this;
		let view = artech_engine.views.trees["BOM Configurator"];
		...
		return {
			...
			onrender(node) {
				...
				amount = artech_engine.format(amount, { fieldtype: "Currency", currency: frm_obj.frm.doc.currency });
			...
			toolbar:
				this.frm?.doc.docstatus === 0
					? [
							{
								label: __(artech_engine.utils.icon("edit", "sm") + " BOM"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("add", "sm") + " Raw Material"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("add", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("add", "sm") + " Phantom Item"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("move", "sm") + " Sub Assembly"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("move", "sm") + " Phantom Item"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
							...
							{
								label: __(artech_engine.utils.icon("delete", "sm") + " Item"),
								click: function (node) {
									let view = artech_engine.views.trees["BOM Configurator"];
	...
	add_item(node, view) {
		artech_engine.prompt(
			...
			(data) => {
				...
				artech_engine.call({
					method: "artech.manufacturing.doctype.bom_creator.bom_creator.add_item",
					args: {
	...
	add_sub_assembly(node, view, phantom = false) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			if (dialog.operation && !dialog.workstation_type && !dialog.workstation) {
				artech_engine.throw(__("Either Workstation or Workstation Type is mandatory"));
			}
			...
			artech_engine.call({
				method: "artech.manufacturing.doctype.bom_creator.bom_creator.add_sub_assembly",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
					fg_reference_id: node.data.name || this.frm.doc.name,
				...
				...[
					{ fieldtype: "Section Break" },
					{
						...
						get_query() {
							...
							if (doc.routing) {
								return {
									query: "artech.manufacturing.doctype.routing.routing.get_operations",
									filters: {
										routing: doc.routing,
									},
								};
	...
	convert_to_sub_assembly(node, view, phantom = false) {
		...
		dialog.set_primary_action(__("Add"), () => {
			...
			bom_item.items.forEach((d) => {
				if (!d.item_code) {
					artech_engine.throw(__("Item is mandatory in Raw Materials table."));
				}
			});
	...
	delete_node(node, view) {
		artech_engine.confirm(__("Are you sure you want to delete this Item?"), () => {
			artech_engine.call({
				method: "artech.manufacturing.doctype.bom_creator.bom_creator.delete_node",
				args: {
					parent: node.data.parent_id,
					fg_item: node.data.value,
	...
	edit_bom(node, view) {
		...
		this.frm.edit_bom_dialog = artech_engine.prompt(
			fields,
			(data) => {
				...
				artech_engine.call({
					method: "artech.manufacturing.doctype.bom_creator.bom_creator.edit_bom_creator",
					args: {
						doctype: doctype,
						docname: docname,
						data: data,
	...
	load_tree(response, node) {
		...
		artech_engine.views.trees["BOM Configurator"].tree.load_children(node);
		...
		while (node) {
			...
			total_amount = artech_engine.format(total_amount, {
				fieldtype: "Currency",
...
artech_engine.ui.BOMConfigurator = BOMConfigurator;
