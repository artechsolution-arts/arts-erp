class CallPopup {
	...
	make() {
		artech_engine.utils.play_sound("incoming-call");
		...
		artech_engine.utils.bind_actions_with_object(this.dialog.$body, this);
	...
	close_modal() {
		this.dialog.hide();
		delete artech.call_popup;
	}

	call_ended(call_log, missed) {
		artech_engine.utils.play_sound("call-disconnect");
	...
	setup_listener() {
		artech_engine.realtime.on(`call_${this.call_log.id}_ended`, (call_log) => {
			this.call_ended(call_log);
		});

		artech_engine.realtime.on(`call_${this.call_log.id}_missed`, (call_log) => {
			this.call_ended(call_log, true);
		});
	}

	clear_listeners() {
		artech_engine.realtime.off(`call_${this.call_log.id}_ended`);
		artech_engine.realtime.off(`call_${this.call_log.id}_missed`);
	}

	setup_call_details() {
		this.caller_info = $(`<div></div>`);
		this.call_details = new artech_engine.ui.FieldGroup({
			fields: [
				{
					fieldname: "name",
					label: "Name",
				...
				},
				{
					...
					click: () => artech_engine.set_route("Form", "Contact", this.get_contact_link().link_name),
				...
				},
				{
					...
					click: () => artech_engine.new_doc("Lead", { mobile_no: this.caller_number }),
					depends_on: () => !this.get_caller_name(),
				},
				{
				...
				},
				{
					...
					click: () => {
						...
						artech_engine
							.xcall(
								"artech.telephony.doctype.call_log.call_log.add_call_summary_and_call_type",
								{
	...
	create_new_customer() {
		...
		const new_customer = artech_engine.model.get_new_doc("Customer");
		new_customer.mobile_no = this.caller_number;
		artech_engine.set_route("Form", new_customer.doctype, new_customer.name);
	}

	create_new_contact() {
		// TODO: fix new_doc, it should accept child table values
		const new_contact = artech_engine.model.get_new_doc("Contact");
		const phone_no = artech_engine.model.add_child(new_contact, "Contact Phone", "phone_nos");
		phone_no.phone = this.caller_number;
		phone_no.is_primary_mobile_no = 1;
		artech_engine.set_route("Form", new_contact.doctype, new_contact.name);
	}
}

$(document).on("app_ready", function () {
	artech_engine.realtime.on("show_call_popup", (call_log) => {
		let call_popup = artech.call_popup;
		if (call_popup && call_log.name === call_popup.call_log.name) {
			call_popup.update_call_log(call_log);
			call_popup.dialog.show();
		} else {
			artech.call_popup = new CallPopup(call_log);
		}
	});
});
