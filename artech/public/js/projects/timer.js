artech_engine.provide("artech.timesheet");

artech.timesheet.timer = function (frm, row, timestamp = 0) {
	...
	artech.timesheet.control_timer(frm, dialog, row, timestamp);
	dialog.show();
};

artech.timesheet.control_timer = function (frm, dialog, row, timestamp = 0) {
	...
	$btn_start.click(function (e) {
		if (!initialized) {
			...
			row = artech_engine.model.add_child(frm.doc, "Timesheet Detail", "time_logs");
			row.activity_type = args.activity_type;
			row.from_time = artech_engine.datetime.get_datetime_as_string();
			...
			if (row.expected_hours) {
				...
				row.to_time = artech_engine.datetime.get_datetime_as_string(d);
			}
	...
	$btn_complete.click(function () {
		...
		grid_row.doc.to_time = artech_engine.datetime.get_datetime_as_string();
		grid_row.refresh();
		frm.dirty();
		frm.save();
		reset();
	...
	function updateStopwatch(increment) {
		...
		if (cur_dialog && cur_dialog.get_value("expected_hours") > 0) {
			if (flag && currentIncrement >= cur_dialog.get_value("expected_hours") * 3600) {
				artech_engine.utils.play_sound("alert");
				artech_engine.msgprint(__("Timer exceeded the given hours."));
				flag = false;
			}
		}
