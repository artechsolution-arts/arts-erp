artech_engine.provide("artech.setup");

artech_engine.pages["setup-wizard"].on_page_load = function (wrapper) {
	if (artech_engine.sys_defaults.company) {
		artech_engine.set_route("desk");
		return;
	}
};

artech_engine.setup.on("before_load", function () {
	if (
		artech_engine.boot.setup_wizard_completed_apps?.length &&
		artech_engine.boot.setup_wizard_completed_apps.includes("artech")
	) {
		return;
	}

	artech.setup.slides_settings.map(artech_engine.setup.add_slide);
});

artech.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		validate: function () {
			...
			if ((this.values.company_name || "").toLowerCase() == "company") {
				artech_engine.msgprint(__("Company Name cannot be Company"));
				return false;
			}
		...
		validate_fy_dates: function () {
			...
			if (invalid || start_greater_than_end) {
				artech_engine.msgprint(__("Please enter valid Financial Year Start and End Dates"));
				return false;
			}
		...
		set_fy_dates: function (slide) {
			var country = artech_engine.wizard.values.country || artech_engine.defaults.get_default("country");

			if (country) {
				let fy = artech.setup.fiscal_years[country];
				...
				if (year_start_date > artech_engine.datetime.get_today()) {
					next_year = current_year;
					current_year -= 1;
				}
		...
		load_chart_of_accounts: function (slide) {
			let country = artech_engine.wizard.values.country || artech_engine.defaults.get_default("country");
		...
		bind_events: function (slide) {
			...
			slide.get_input("fy_start_date").on("change", function () {
				...
				var year_end_date = artech_engine.datetime.add_days(artech_engine.datetime.add_months(start_date, 12), -1);
			...
			slide
				...
				.val(artech_engine.boot.sysdefaults.company_name || "")
				.trigger("change");
			...
			slide
				...
				.on("change", function () {
					...
					if (abbr.length > 10) {
						artech_engine.msgprint(__("Company Abbreviation cannot have more than 5 characters"));
						abbr = abbr.slice(0, 10);
					}
					slide.get_field("company_abbr").set_value(abbr);
				})
				.val(artech_engine.boot.sysdefaults.company_abbr || "")
				.trigger("change");
		},
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new artech_engine.ui.Tree({
				parent: $(dialog.body),
				label: parent,
				expandable: true,
				method: "artech.accounts.utils.get_coa",
				args: {
			...
			const buttons = $(form_container).find(".artech_engine-control");
