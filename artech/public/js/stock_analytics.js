...
artech.StockAnalytics = class StockAnalytics extends artech.StockGridReport {
			...
			tree_grid: {
				...
				formatter: function (item) {
					if (!item.is_group) {
						return repl(
							"<a \
							onclick='artech_engine.cur_grid_report.show_stock_ledger(\"%(value)s\")'>\
							%(value)s</a>",
							{
								value: item.name,
							}
	...
	prepare_balances() {
		var me = this;
		var from_date = artech_engine.datetime.str_to_obj(this.from_date);
		var to_date = artech_engine.datetime.str_to_obj(this.to_date);
		var data = artech_engine.report_dump.data["Stock Ledger Entry"];
		...
		for (var i = 0, j = data.length; i < j; i++) {
			...
			var posting_datetime = artech_engine.datetime.str_to_obj(sl.posting_datetime);
			...
			if (me.is_default("warehouse") ? true : me.warehouse == sl.warehouse) {
				...
				if (me.value_or_qty != "Quantity") {
					...
					var valuation_method = item.valuation_method
						...
						: artech_engine.sys_defaults.valuation_method;
