artech_engine.provide("artech.financial_statements");

function get_filter_value(filter_name) {
	// not warn when the filter is missing
	return artech_engine.query_report.get_filter_value(filter_name, false);
}

artech.financial_statements = {
	filters: get_filters(),
	baseData: null,
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		// Growth/Margin
		if (artech.financial_statements._is_special_view(column, data))
			return artech.financial_statements._format_special_view(...report_params);

		if (get_filter_value("report_template"))
			return artech.financial_statements._format_custom_report(...report_params);
		else return artech.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = artech.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = artech.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return artech.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return artech.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_parse_column_info: function (fieldname, data) {
		...
		return {
			isAccount: baseName === artech.financial_statements.name_field,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"artech.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return artech.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return artech.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == artech.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"artech.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	open_general_ledger: function (data) {
		...
		let filters = artech_engine.query_report.filters;
		...
		filters.forEach((f) => {
			if (f.df.fieldtype == "MultiSelectList") {
				if (f.df.fieldname in artech_engine.route_options) return;
				let value = f.get_value();
				if (value && value.length > 0) {
					artech_engine.route_options[f.df.fieldname] = value;
				}
			}
		});
		...
		if (["Payable", "Receivable"].includes(data.account_type)) {
			...
			artech_engine.route_options["party_account"] = data.account;
			artech_engine.route_options["report_date"] = data.year_end_date;
		}

		artech_engine.set_route("query-report", report);
	},
	tree: true,
	name_field: "account",
	...
	onload: function (report) {
		...
		artech.financial_statements.filters = get_filters();

		let fiscal_year = artech.utils.get_fiscal_year(artech_engine.datetime.get_today());
		var filters = report.get_values();

		if (!filters.period_start_date || !filters.period_end_date) {
			artech_engine.model.with_doc("Fiscal Year", fiscal_year, function (r) {
				var fy = artech_engine.model.get_doc("Fiscal Year", fiscal_year);
				artech_engine.query_report.set_filter_value({
		...
		if (report.page) {
			...
			report.page.add_custom_menu_item(views_menu, __("Balance Sheet"), function () {
				var filters = report.get_values();
				artech_engine.set_route("query-report", "Balance Sheet", {
			...
			report.page.add_custom_menu_item(views_menu, __("Profit and Loss"), function () {
				var filters = report.get_values();
				artech_engine.set_route("query-report", "Profit and Loss Statement", {
					company: filters.company,
					filter_based_on: filters.filter_based_on,
					period_start_date: filters.period_start_date,
					period_end_date: filters.period_end_date,
			...
			report.page.add_custom_menu_item(views_menu, __("Cash Flow Statement"), function () {
				var filters = report.get_values();
				artech_engine.set_route("query-report", "Cash Flow", {
					company: filters.company,
					filter_based_on: filters.filter_based_on,
					period_start_date: filters.period_start_date,
					period_end_date: filters.period_end_date,
...
function get_filters() {
	let filters = [
		...
		},
		{
			...
			on_change: function () {
				...
				artech_engine.query_report.toggle_filter_display("to_fiscal_year", filter_based_on === "Date Range");
				artech_engine.query_report.toggle_filter_display(
					"period_start_date",
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: artech.get_presentation_currency_list(),
	...
	let fiscal_year = artech.utils.get_fiscal_year(artech_engine.datetime.get_today(), false, false);
	if (fiscal_year) {
