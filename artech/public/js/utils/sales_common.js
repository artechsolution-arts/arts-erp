...
artech_engine.provide("artech.selling");

artech.sales_common = {
	setup_selling_controller: function () {
		artech.selling.SellingController = class SellingController extends artech.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], artech.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", artech.queries.contact_query);
				me.frm.set_query("company_contact_person", artech.queries.company_contact_query);
				me.frm.set_query("customer_address", artech.queries.address_query);
				me.frm.set_query("shipping_address_name", artech.queries.address_query);
				me.frm.set_query("dispatch_address_name", artech.queries.dispatch_address_query);
				me.frm.set_query("company_address", artech.queries.company_address_query);
			...
			refresh() {
				...
				artech_engine.dynamic_link = { doc: this.frm.doc, fieldname: "customer", doctype: "Customer" };
			...
			set_default_company_address() {
				if (!artech_engine.meta.has_field(this.frm.doc.doctype, "company_address")) return;
				var me = this;
				if (this.frm.doc.company) {
					artech_engine.call({
						method: "artech.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			set_default_company_contact_person() {
				if (!artech_engine.meta.has_field(this.frm.doc.doctype, "company_contact_person")) {
					return;
				}
			...
			customer_address() {
				artech.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				artech.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				artech.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				artech.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			total_commission() {
				artech_engine.model.round_floats_in(this.frm.doc, [
			...
			allocated_percentage(doc, cdt, cdn) {
				var sales_person = artech_engine.get_doc(cdt, cdn);
			...
			warehouse(doc, cdt, cdn) {
				if (doc.docstatus === 0 && doc.is_return && !doc.return_against) {
					artech_engine.model.set_value(cdt, cdn, "incoming_rate", 0.0);
				}
			...
			set_actual_qty(doc, cdt, cdn) {
				...
				if (row.item_code && row.warehouse && sales_doctypes.includes(doc.doctype)) {
					artech_engine.call({
						...
						callback(r) {
							if (r.message) {
								artech_engine.model.set_value(cdt, cdn, "actual_qty", r.message.actual_qty);
							}
						},
					});
				}
			...
			toggle_editable_price_list_rate() {
				var df = artech_engine.meta.get_docfield(
					this.frm.doc.doctype + " Item",
					"price_list_rate",
					this.frm.doc.name
				);
				var editable_price_list_rate = cint(artech_engine.defaults.get_default("editable_price_list_rate"));

				if (df && editable_price_list_rate) {
					const parent_field = artech_engine.meta.get_parentfield(
			...
			calculate_commission() {
				...
				if (this.frm.doc.commission_rate > 100) {
					...
					artech_engine.throw(
						`${__(
							artech_engine.meta.get_label(this.frm.doc.doctype, "commission_rate", this.frm.doc.name)
			...
			calculate_contribution() {
				...
				$.each(this.frm.doc.doctype.sales_team || [], function (i, sales_person) {
					artech_engine.model.round_floats_in(sales_person);
				...
				if ((doc.packed_items || []).length) {
					...
					if (["Delivery Note", "Sales Invoice"].includes(doc.doctype)) {
						var help_msg =
							...
							"</div>";
						artech_engine.meta.get_docfield(doc.doctype, "product_bundle_help", doc.name).options =
							help_msg;
					}
				} else {
			...
			project(doc, cdt, cdn) {
				if (!cdt || !cdn) {
					if (this.frm.doc.project) {
						$.each(this.frm.doc["items"] || [], function (i, item) {
							if (!item.project) {
								artech_engine.model.set_value(item.doctype, item.name, "project", doc.project);
							}
						});
					}
				} else {
					const item = artech_engine.get_doc(cdt, cdn);
					if (item.project) {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						artech_engine.call({
							method: "artech.projects.doctype.project.project.get_cost_center_name",
...
artech.pre_sales = {
	set_as_lost: function (doctype) {
		artech_engine.ui.form.on(doctype, {
