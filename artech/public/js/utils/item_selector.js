artech.ItemSelector = class ItemSelector {
	constructor(opts) {
		$.extend(this, opts);
		...
		if (!this.item_query) {
			this.item_query = artech.queries.item().query;
		}
	...
	add_item(item_code) {
		...
		$.each(this.frm.doc.items || [], (i, d) => {
			if (d[this.item_field] === item_code) {
				artech_engine.model.set_value(d.doctype, d.name, "qty", d.qty + 1);
				artech_engine.show_alert({ message: __("Added {0} ({1})", [item_code, d.qty]), indicator: "green" });
				added = true;
				return false;
			}
		});
		...
		if (!added) {
			...
			artech_engine.run_serially([
				...
				() => artech_engine.model.set_value(d.doctype, d.name, this.item_field, item_code),
				() => artech_engine.timeout(0.1),
				() => {
					artech_engine.model.set_value(d.doctype, d.name, "qty", 1);
					artech_engine.show_alert({ message: __("Added {0} ({1})", [item_code, 1]), indicator: "green" });
				},
			]);
		}
	}
	...
	render_items() {
		...
		var me = this;
		artech_engine.link_search("Item", args, function (results) {
			$.each(results, function (i, d) {
				if (!d.image) {
					d.abbr = artech_engine.get_abbr(d.item_name);
					d.color = artech_engine.get_palette(d.item_name);
				}
			});
			me.dialog.results.html(artech_engine.render_template("item_selector", { data: results }));
		});
	}
};
