...
artech.accounts.dimensions = {
	...
	setup_filters(frm, doctype) {
		...
		if (this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
				artech_engine.model.with_doctype(dimension["document_type"], () => {
					let parent_fields = [];
					artech_engine.meta.get_docfields(doctype).forEach((df) => {
						...
						if (artech_engine.meta.has_field(doctype, dimension["fieldname"])) {
	...
	setup_child_filters(frm, doctype, parentfield, dimension) {
		...
		if (artech_engine.meta.has_field(doctype, dimension)) {
			artech_engine.model.with_doctype(doctype, () => {
				...
				frm.set_query(dimension, parentfield, function (doc, cdt, cdn) {
					...
					return artech.queries.get_filtered_dimensions(row, fields, dimension, doc.company);
				});
			});
		}
	},
	...
	setup_account_filters(frm, dimension, fields) {
		frm.set_query(dimension, function (doc) {
			return artech.queries.get_filtered_dimensions(doc, fields, dimension, doc.company);
		});
	},
	...
	update_dimension(frm, doctype) {
		...
		this.accounting_dimensions.forEach((dimension) => {
			...
			(frm.doc.items || frm.doc.accounts || []).forEach((row) => {
				artech_engine.model.set_value(row.doctype, row.name, dimension["fieldname"], default_dimension);
			});
		});
	},

	copy_dimension_from_first_row(frm, cdt, cdn, fieldname) {
		if (artech_engine.meta.has_field(frm.doctype, fieldname) && this.accounting_dimensions) {
			this.accounting_dimensions.forEach((dimension) => {
