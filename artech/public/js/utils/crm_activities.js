artech.utils.CRMActivities = class CRMActivities {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		if (!$(this.all_activities_wrapper).find(".form-footer").length) {
			...
			// remove artech_engine-control class to avoid absolute position for action-btn
			$(this.all_activities_wrapper).removeClass("artech_engine-control");
		...
		// open activities
		artech_engine.call({
			method: "artech.crm.utils.get_open_activities",
			args: {
			...
			callback: (r) => {
				if (!r.exc) {
					var activities_html = artech_engine.render_template("crm_activities", {
	...
	create_task() {
		...
		let _create_task = () => {
			...
			let composer = new artech_engine.views.InteractionComposer(args);
	...
	async update_status(input_field, doctype) {
		...
		if (completed) {
			await artech_engine.db.set_value(doctype, docname, "status", "Closed");
			this.refresh();
		}
	}
};

artech.utils.CRMNotes = class CRMNotes {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	refresh() {
		...
		let notes_html = artech_engine.render_template("crm_notes", {
			notes: notes,
		});
