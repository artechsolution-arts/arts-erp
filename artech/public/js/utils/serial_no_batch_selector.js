artech.SerialBatchPackageSelector = class SerialNoBatchBundleUpdate {
		...
		if (qty > 0) {
			this.dialog.set_value("qty", qty).then(() => {
				if (this.item.serial_no && !this.item.serial_and_batch_bundle) {
					...
					artech_engine.model.set_value(this.item.doctype, this.item.name, "serial_no", "");
				} else if (this.item.batch_no && !this.item.serial_and_batch_bundle) {
					this.dialog.set_value("scan_batch_no", this.item.batch_no);
					artech_engine.model.set_value(this.item.doctype, this.item.name, "batch_no", "");
				}
	...
	get_dialog_fields() {
		...
		fields.push({
			...
			get_query: () => {
				return {
					query: "artech.controllers.queries.warehouse_query",
					filters: [
	...
	set_serial_nos_from_range() {
		...
		const serial_nos = artech.stock.utils.get_serial_range(serial_no_range, "::");
	...
	create_serial_nos() {
		...
		if (!upload_serial_nos) {
			artech_engine.throw(__("Please enter Serial Nos"));
		}

		artech_engine.call({
			method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.create_serial_nos",
			args: {
				item_code: this.item.item_code,
	...
	download_csv_file() {
		...
		const method = `/api/method/artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(
			JSON.stringify(csvFileData)
		)}`;
		const w = window.open(artech_engine.urllib.get_full_url(method));
		if (!w) {
	...
	upload_csv_file() {
		...
		artech_engine.call({
			method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file",
			args: {
				item_code: this.item.item_code,
				file_path: file_path,
			},
	...
	get_batch_qty(batch_no, callback) {
		...
		artech_engine.call({
			method: "artech.stock.doctype.batch.batch.get_batch_qty",
			args: {
				batch_no: batch_no,
				warehouse: warehouse,
				item_code: this.item.item_code,
	...
	get_auto_data() {
		...
		if (qty) {
			artech_engine.call({
				method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data",
				args: {
					item_code: this.item.item_code,
					warehouse: warehouse,
	...
	scan_barcode_data() {
		...
		if (scan_serial_no || scan_batch_no) {
			artech_engine.call({
				method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.is_serial_batch_no_exists",
				args: {
					item_code: this.item.item_code,
	...
	update_serial_batch_no() {
		...
		if (scan_serial_no) {
			...
			if (existing_row?.length) {
				artech_engine.throw(__("Serial No {0} already exists", [scan_serial_no]));
			}
			...
			if (!this.item.has_batch_no) {
				...
				artech_engine.call({
					method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_batch_no_from_serial_no",
					args: {
						serial_no: scan_serial_no,
					},
					callback: (r) => {
	...
	update_bundle_entries() {
		...
		if ((entries && !entries.length) || !entries) {
			artech_engine.throw(__("Please add atleast one Serial No / Batch No"));
		}

		if (!warehouse) {
			artech_engine.throw(__("Please select a Warehouse"));
		}

		if (this.item?.is_rejected && this.item.rejected_warehouse === this.item.warehouse) {
			artech_engine.throw(__("Rejected Warehouse and Accepted Warehouse cannot be same."));
		}

		artech_engine
			.call({
				method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.add_serial_batch_ledgers",
				args: {
					entries: entries,
					child_row: this.item,
					doc: this.frm.doc,
	...
	edit_full_form() {
		...
		if (!bundle_id) {
			let _new = artech_engine.model.get_new_doc("Serial and Batch Bundle", null, null, true);
		...
		artech_engine.set_route("Form", "Serial and Batch Bundle", bundle_id);
		this.dialog.hide();
	}
	...
	render_data() {
		if (this.bundle || (this.frm.doc.is_return && this.frm.doc.return_against)) {
			artech_engine
				.call({
					method: "artech.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_serial_batch_ledgers",
					args: {
						item_code: this.item.item_code,
						name: this.bundle,
