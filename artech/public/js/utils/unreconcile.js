...
artech.accounts.unreconcile_payment = {
	add_unreconcile_btn(frm) {
		if (frm.doc.docstatus == 1) {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.unreconcile_payment.unreconcile_payment.doc_has_references",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						frm.add_custom_button(
							__("UnReconcile"),
							function () {
								artech.accounts.unreconcile_payment.build_unreconcile_dialog(frm);
							},
							__("Actions")
						);
					}
	...
	build_unreconcile_dialog(frm) {
		if (
			...
			// get linked payments
			artech_engine.call({
				method: "artech.accounts.doctype.unreconcile_payment.unreconcile_payment.get_linked_payments_for_doc",
				args: {
						...
						let d = new artech_engine.ui.Dialog({
							...
							primary_action(values) {
								...
								if (selected_allocations.length > 0) {
									let selection_map =
										artech.accounts.unreconcile_payment.build_selection_map(
											frm,
											selected_allocations
										);
									artech.accounts.unreconcile_payment.create_unreconcile_docs(
										selection_map,
										frm
									);
									d.hide();
								} else {
									artech_engine.msgprint(__("No Selection"));
								}
							},
						});
	...
	create_unreconcile_docs(selection_map, frm) {
		artech_engine.call({
			method: "artech.accounts.doctype.unreconcile_payment.unreconcile_payment.create_unreconcile_doc_for_selection",
			args: {
			...
			callback: function (r) {
				...
				artech_engine.show_alert({ message: __("Unreconciled successfully"), indicator: "green" });
			},
		});
	},
};
