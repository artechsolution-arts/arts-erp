...
artech.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (artech_engine.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!artech.utils.validate_mandatory(
				frm,
...
artech.utils.add_item = function (frm) {
	if (frm.is_new()) {
		var prev_route = artech_engine.get_prev_route();
		if (prev_route[1] === "Item" && !(frm.doc.items && frm.doc.items.length)) {
			...
			// set item
			artech_engine.model.set_value(item.doctype, item.name, "item_code", prev_route[2]);
		}
	}
};

artech.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
artech.utils.set_taxes_from_address = function (
	frm,
	...
	artech_engine.call({
		method: "artech.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					artech.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
artech.utils.set_taxes = function (frm, triggered_from_field) {
	if (artech_engine.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!artech.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	if (!frm.doc.company) {
		artech_engine.throw(__("Kindly select the company first"));
	}

	artech_engine.call({
		method: "artech.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
artech.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
artech.utils.get_employee_contact_details = function (frm) {
	...
	artech_engine.call({
		method: "artech.setup.doctype.employee.employee.get_contact_details",
...
artech.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
		refresh_field(trigger_on);
		artech_engine.throw({ message: __("Please enter {0} first", [label]), title: __("Mandatory") });
		return false;
	}
	return true;
};

artech.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		artech_engine.call({
			method: "artech.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
		...
		artech_engine.msgprint(__("Select company first"));
	}
};
