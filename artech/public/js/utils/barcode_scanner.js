artech.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		this.frm = opts.frm;
		// artech_engine.flags.trigger_from_barcode_scanner is used for custom scripts
		...
		// see https://artech_engineframework.com/docs/v14/user/en/python-api/hooks#sounds
		...
		// }
		this.scan_api = opts.scan_api || "artech.stock.utils.scan_barcode";
		this.has_last_scanned_warehouse = artech_engine.meta.has_field(this.frm.doctype, "last_scanned_warehouse");
	}
	...
	update_table(data) {
		return new Promise((resolve, reject) => {
			...
			artech_engine.flags.trigger_from_barcode_scanner = true;
			...
			if (!row) {
				...
				row = artech_engine.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
	...
	revert_selector_flag() {
		...
		artech_engine.flags.trigger_from_barcode_scanner = false;
	}
	...
	set_item(row, item_code, barcode, batch_no, serial_no) {
		return new Promise((resolve) => {
			const increment = async (value = 1) => {
				...
				await artech_engine.model.set_value(row.doctype, row.name, item_data);
				return value;
			};

			if (this.prompt_qty) {
				artech_engine.prompt(__("Please enter quantity for item {0}", [item_code]), ({ value }) => {
	...
	prepare_item_for_scan(row, item_code, barcode, batch_no, serial_no) {
		var me = this;
		this.dialog = new artech_engine.ui.Dialog({
		...
		this.dialog.set_primary_action(__("Update"), () => {
			...
			artech_engine.model.set_value(row.doctype, row.name, item_data);
	...
	validate_duplicate_serial_no(serial_no) {
		...
		if (serial_nos.includes(serial_no)) {
			artech_engine.throw(__("Serial No {0} already scanned", [serial_no]));
		}
	}
	...
	add_child_for_remaining_qty(prev_row) {
		if (this.remaining_qty && this.remaining_qty > 0) {
			let cur_grid = this.frm.fields_dict[this.items_table_name].grid;
			let row = artech_engine.model.add_child(this.frm.doc, cur_grid.doctype, this.items_table_name);
			...
			if (this.qty_field == "qty" && artech_engine.meta.has_field(row.doctype, "stock_qty")) {
	...
	async set_serial_no(row, serial_no) {
		if (serial_no && artech_engine.meta.has_field(row.doctype, this.serial_no_field)) {
			...
			await artech_engine.model.set_value(row.doctype, row.name, this.serial_no_field, new_serial_nos);
		}
	}

	async set_barcode_uom(row, uom) {
		if (uom && artech_engine.meta.has_field(row.doctype, this.uom_field)) {
			await artech_engine.model.set_value(row.doctype, row.name, this.uom_field, uom);
		}
	}

	async set_batch_no(row, batch_no) {
		if (batch_no && artech_engine.meta.has_field(row.doctype, this.batch_no_field)) {
			await artech_engine.model.set_value(row.doctype, row.name, this.batch_no_field, batch_no);
		}
	}

	async set_barcode(row, barcode) {
		if (barcode && artech_engine.meta.has_field(row.doctype, this.barcode_field)) {
			await artech_engine.model.set_value(row.doctype, row.name, this.barcode_field, barcode);
		}
	}
	...
	async set_warehouse(row) {
		...
		if (!warehouse_field || !artech_engine.meta.has_field(row.doctype, warehouse_field)) return;

		await artech_engine.model.set_value(row.doctype, row.name, warehouse_field, last_scanned_warehouse);
	}
	...
	get_row_to_modify_on_scan(item_code, batch_no, uom, barcode, default_warehouse) {
		...
		let is_batch_no_scan = batch_no && artech_engine.meta.has_field(cur_grid.doctype, this.batch_no_field);
		let check_max_qty = this.max_qty_field && artech_engine.meta.has_field(cur_grid.doctype, this.max_qty_field);

		const warehouse_field = this.has_last_scanned_warehouse && this.get_warehouse_field();
		const has_warehouse_field =
			warehouse_field && artech_engine.meta.has_field(cur_grid.doctype, warehouse_field);
	...
	setup_last_scanned_warehouse() {
		...
		this.frm.set_df_property("last_scanned_warehouse", "formatter", function (value, df, options, doc) {
			const link_formatter = artech_engine.form.get_formatter(df.fieldtype);
			...
			const clear_btn = `
				<a class="btn-clear-last-scanned-warehouse" title="${__("Clear Last Scanned Warehouse")}">
					${artech_engine.utils.icon("close", "xs", "es-icon")}
				</a>
			`;
	...
	handle_warehouse_scan(data) {
		...
		const warehouse_field_label = artech_engine.meta.get_label(cur_grid.doctype, warehouse_field);
	...
	play_success_sound() {
		this.success_sound && artech_engine.utils.play_sound(this.success_sound);
	}

	play_fail_sound() {
		this.fail_sound && artech_engine.utils.play_sound(this.fail_sound);
	}
	...
	show_alert(msg, indicator, duration = 3) {
		artech_engine.show_alert({ message: msg, indicator: indicator }, duration);
	}
};
