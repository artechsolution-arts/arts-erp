artech_engine.provide("artech.stock_reservation");

$.extend(artech.stock_reservation, {
	make_entries(frm, table_name) {
		artech.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = artech.stock_reservation.get_parms(frm, table_name);

		artech.stock_reservation.dialog = new artech_engine.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: artech.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				artech.stock_reservation.reserve_stock(frm, table_name, parms);
			},
		});

		artech.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"artech.manufacturing.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"artech.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = artech.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = artech.stock_reservation.dialog;
		...
		let table_fields = [
			{ fieldtype: "Section Break" },
			{
				...
				fields: [
					{
						fieldname: artech_engine.scrub(parms.child_doctype),
						fieldtype: "Link",
	...
	render_items(frm, parms) {
		...
		let dialog = artech.stock_reservation.dialog;
		let field = artech_engine.scrub(parms.child_doctype);
	...
	unreserve_stock(frm) {
		artech.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					artech.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
					artech_engine.msgprint(__("No reserved stock to unreserve."));
				}
			}
		});
	},
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: artech.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				artech.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "artech.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"artech.manufacturing.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
