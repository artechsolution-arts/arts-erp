...
artech.taxes_and_totals = class TaxesAndTotals extends artech.payments {
	setup() {
	...
	apply_pricing_rule_on_item(item) {
		...
		artech_engine.model.set_value(item.doctype, item.name, "rate", item_rate);
	}
	...
	calculate_discount_amount() {
		if (artech_engine.meta.get_docfield(this.frm.doc.doctype, "discount_amount")) {
	...
	validate_conversion_rate() {
		...
		var conversion_rate_label = artech_engine.meta.get_label(
		...
		if (!this.frm.doc.conversion_rate) {
			if (this.frm.doc.currency == company_currency) {
				...
				artech_engine.throw(err_message);
			}
		}
	}
	...
	calculate_item_values() {
		...
		if (!this.discount_amount_applied) {
			for (const item of this.frm.doc.items || []) {
				artech_engine.model.round_floats_in(item);
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function (i, tax) {
			...
			if (!this.discount_amount_applied) {
				artech.accounts.taxes.validate_taxes_and_charges(tax.doctype, tax.name);
				artech.accounts.taxes.validate_inclusive_tax(tax, this.frm);
			}
			artech_engine.model.round_floats_in(tax);
		});
	}

	fetch_round_off_accounts() {
		let me = this;
		artech_engine.flags.round_off_applicable_accounts = [];

		if (me.frm.doc.company) {
			artech_engine.call({
				method: "artech.controllers.taxes_and_totals.get_round_off_applicable_accounts",
				args: {
					company: me.frm.doc.company,
					account_list: artech_engine.flags.round_off_applicable_accounts,
				},
				callback(r) {
					if (r.message) {
						artech_engine.flags.round_off_applicable_accounts.push(...r.message);
					}
				},
			});
		}
		...
		artech_engine.call({
			method: "artech.controllers.taxes_and_totals.get_rounding_tax_settings",
			callback: function (r) {
				artech_engine.flags.round_off_settings = r.message;
			},
		});
	}
	...
	calculate_net_total() {
		...
		artech_engine.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
	}
	...
	calculate_shipping_charges() {
		...
		artech_engine.model.round_floats_in(this.frm.doc, ["total", "base_total", "net_total", "base_net_total"]);
		if (artech_engine.meta.get_docfield(this.frm.doc.doctype, "shipping_rule", this.frm.doc.name)) {
	...
	add_taxes_from_item_tax_template(item_tax_map) {
		...
		if (item_tax_map && cint(artech_engine.defaults.get_default("add_taxes_from_item_tax_template"))) {
			...
			$.each(item_tax_map, function (tax, rate) {
				...
				if (!found) {
					let child = artech_engine.model.add_child(me.frm.doc, "taxes");
	...
	calculate_taxes() {
		...
		$.each(this.frm._items || [], function (n, item) {
			...
			$.each(doc.taxes, function (i, tax) {
				...
				if (artech_engine.flags.round_row_wise_tax) {
	...
	round_off_totals(tax) {
		if (artech_engine.flags.round_off_applicable_accounts.includes(tax.account_head)) {
	...
	adjust_grand_total_for_inclusive_tax() {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				var non_inclusive_tax_amount = artech_engine.utils.sum(
	...
	calculate_totals() {
		...
		artech_engine.model.round_floats_in(this.frm.doc, ["grand_total", "base_grand_total"]);
	...
	set_rounded_total() {
		...
		if (artech_engine.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total", this.frm.doc.name)) {
			disable_rounded_total = this.frm.doc.disable_rounded_total;
		} else if (artech_engine.sys_defaults.disable_rounded_total) {
			disable_rounded_total = artech_engine.sys_defaults.disable_rounded_total;
		}

		if (artech_engine.meta.get_docfield(this.frm.doc.doctype, "rounded_total", this.frm.doc.name)) {
	...
	_cleanup() {
		...
		if (items && items.length) {
			if (!artech_engine.meta.get_docfield(items[0].doctype, "item_tax_amount", this.frm.doctype)) {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (
				!artech_engine.meta.get_docfield(
	...
	set_discount_amount() {
		if (this.frm.doc.additional_discount_percentage) {
			this.frm.doc.discount_amount = flt(
				(flt(this.frm.doc[artech_engine.scrub(this.frm.doc.apply_discount_on)]) *
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			if (!this.frm.doc.apply_discount_on) artech_engine.throw(__("Please select Apply Discount On"));
	...
	calculate_total_advance(update_paid_amount) {
		var total_allocated_amount = artech_engine.utils.sum(
	...
	calculate_outstanding_amount(update_paid_amount) {
		...
		artech_engine.model.round_floats_in(this.frm.doc, ["grand_total", "total_advance", "write_off_amount"]);
		...
		if (["Sales Invoice", "POS Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
			...
			artech_engine.model.round_floats_in(this.frm.doc, ["paid_amount"]);
	...
	async set_total_amount_to_default_mop() {
		...
		*/
		...
		if (this.frm.doc.return_against) {
			let { message: return_against_mop } = await artech_engine.call({
				method: "artech.controllers.sales_and_purchase_return.get_payment_data",
				args: {
		...
		if (
			...
			$.each(this.frm.doc["payments"] || [], function (index, data) {
				if (data.default && payment_status && total_amount_to_pay > 0) {
					...
					artech_engine.model.set_value(data.doctype, data.name, "base_amount", base_amount);
					artech_engine.model.set_value(data.doctype, data.name, "amount", amount);
					payment_status = false;
				} else if (me.frm.doc.paid_amount) {
					artech_engine.model.set_value(data.doctype, data.name, "amount", 0.0);
				}
			});
		}
	}
