...
artech.buying = {
	setup_buying_controller: function () {
		artech.buying.BuyingController = class BuyingController extends artech.TransactionController {
			setup() {
				super.setup();
			...
			onload(doc, cdt, cdn) {
				...
				if (
					...
					artech_engine.meta.has_field(this.frm.doc.doctype, "disable_rounded_total")
				) {
					var df = artech_engine.meta.get_docfield(this.frm.doc.doctype, "disable_rounded_total");
					var disable = cint(df.default) || cint(artech_engine.sys_defaults.disable_rounded_total);
				...
				if (this.frm.get_field("shipping_address")) {
					this.frm.set_query("shipping_address", () => {
						...
						} else return artech.queries.company_address_query(this.frm.doc);
					});
				}

				if (this.frm.get_field("dispatch_address")) {
					this.frm.set_query("dispatch_address", () => {
						return artech.queries.address_query(this.frm.doc);
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query("supplier", artech.queries.supplier);
				me.frm.set_query("contact_person", artech.queries.contact_query);
				me.frm.set_query("supplier_address", artech.queries.address_query);

				me.frm.set_query("billing_address", artech.queries.company_address_query);
				artech.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function (doc, cdt, cdn) {
					...
					return {
						query: "artech.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				artech.utils.get_party_details(this.frm, null, null, function () {
					me.apply_price_list();
				});
			}
			...
			company() {
				super.company();
				if (!artech_engine.meta.has_field(this.frm.doc.doctype, "billing_address")) return;

				artech_engine.call({
					method: "artech.setup.doctype.company.company.get_billing_shipping_address",
					args: {
					...
					callback: (r) => {
						...
						if (artech_engine.meta.has_field(this.frm.doc.doctype, "shipping_address")) {
							this.frm.set_value("shipping_address", r.message.shipping_address || "");
						}
					},
				});
				artech.utils.set_letter_head(this.frm);
			}

			supplier_address() {
				artech.utils.get_address_display(this.frm);
				artech.utils.set_taxes_from_address(
					this.frm,
					"supplier_address",
					"supplier_address",
					"supplier_address"
			...
			calculate_received_qty(doc, cdt, cdn) {
				var item = artech_engine.get_doc(cdt, cdn);
				artech_engine.model.round_floats_in(item, ["qty", "rejected_qty"]);
				...
				artech_engine.model.set_value(cdt, cdn, "received_qty", received_qty);
				artech_engine.model.set_value(cdt, cdn, "received_stock_qty", received_stock_qty);
			}
					...
					if (item[fieldnames[i]] < 0) {
						artech_engine.msgprint(
							__("Row #{0}: {1} can not be negative for item {2}", [
								item.idx,
								__(artech_engine.meta.get_label(cdt, fieldnames[i], cdn)),
								item.item_code,
							])
						);
			...
			warehouse(doc, cdt, cdn) {
				...
				if (item.item_code && item.warehouse) {
					return this.frm.call({
						method: "artech.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(
					__("Product Bundle"),
					function () {
						artech.buying.get_items_from_product_bundle(me.frm);
					},
					__("Get Items From")
				);
			}
			...
			shipping_address() {
				var me = this;
				artech.utils.get_address_display(
					this.frm,
					"shipping_address",
			...
			update_auto_repeat_reference(doc) {
				if (doc.auto_repeat) {
					artech_engine.call({
						method: "artech_engine.automation.doctype.auto_repeat.auto_repeat.update_reference",
						args: {
			...
			manufacturer(doc, cdt, cdn) {
				...
				if (row.manufacturer) {
					artech_engine.call({
						method: "artech.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
						...
						callback: function (r) {
							if (r.message) {
								artech_engine.model.set_value(cdt, cdn, "manufacturer_part_no", r.message);
							}
						},
					});
				}
			...
			add_serial_batch_bundle(doc, cdt, cdn) {
				...
				artech_engine.db.get_value("Item", item.item_code, fields).then((r) => {
					if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
						...
						new artech.SerialBatchPackageSelector(me.frm, item, (r) => {
							if (r) {
								...
								artech_engine.model.set_value(item.doctype, item.name, update_values);
							}
						});
					}
				});
...
artech.buying.link_to_mrs = function (frm) {
	artech_engine.call({
		method: "artech.buying.utils.get_linked_material_requests",
		args: {
			...
			for (let item of frm.doc.items) {
				...
				(r.message[0] || []).forEach(function (d) {
					if (
						...
						if (qty > 0) {
							artech_engine.msgprint("Splitting " + qty + " units of " + d.item_code);
							var newrow = artech_engine.model.add_child(frm.doc, item.doctype, "items");
							item_length++;
...
artech.buying.get_default_bom = function (frm) {
	$.each(frm.doc["items"] || [], function (i, d) {
		if (d.item_code && d.bom === "") {
			return artech_engine.call({
				type: "GET",
				method: "artech.stock.get_item_details.get_default_bom",
				args: {
					item_code: d.item_code,
				},
				callback: function (r) {
					if (r) {
						artech_engine.model.set_value(d.doctype, d.name, "bom", r.message);
					}
				},
			});
		}
...
artech.buying.get_items_from_product_bundle = function (frm) {
	var dialog = new artech_engine.ui.Dialog({
		...
		primary_action(args) {
			...
			return artech_engine.call({
				type: "GET",
				method: "artech.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
				...
				callback: function (r) {
					...
					if (!r.exc && r.message) {
						...
						for (var i = 0; i < r.message.length; i++) {
							...
							if (artech_engine.meta.get_docfield(d.doctype, "price_list_rate", d.name)) {
...
artech.buying.prevent_past_schedule_dates = function (frm) {
