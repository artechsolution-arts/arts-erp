...
// get tax rate
artech_engine.provide("artech.taxes");

artech.accounts.taxes = {
			...
			onload: function (frm) {
				if (frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function (doc) {
						...
						return {
							query: "artech.controllers.queries.tax_account_query",
							filters: {
		...
		artech_engine.ui.form.on(doctype, {
			account_head: function (frm, cdt, cdn) {
				...
				if (!d.charge_type && d.account_head) {
					artech_engine.msgprint(__("Please select Charge Type first"));
					artech_engine.model.set_value(cdt, cdn, "account_head", "");
				} else if (d.account_head) {
					artech_engine.call({
						type: "GET",
						method: "artech.controllers.accounts_controller.get_tax_rate",
						args: { account_head: d.account_head },
						callback: function (r) {
							if (d.charge_type !== "Actual") {
								artech_engine.model.set_value(cdt, cdn, "rate", r.message.tax_rate || 0);
							}
							artech_engine.model.set_value(cdt, cdn, "description", r.message.account_name);
						},
					});
				}
			},
			...
			included_in_print_rate: function (frm, cdt, cdn) {
				let tax = artech_engine.get_doc(cdt, cdn);
				try {
					me.validate_taxes_and_charges(cdt, cdn);
	...
	validate_inclusive_tax: function (tax, frm) {
		...
		if (cint(tax.included_in_print_rate)) {
			if (tax.charge_type == "Actual") {
				...
				artech_engine.throw(__("Valuation type charges can not marked as Inclusive"));
			}
		}
	},
};

artech.accounts.payment_triggers = {
...
artech.accounts.pos = {
	setup: function (doctype) {
		artech_engine.ui.form.on(doctype, {
			mode_of_payment: function (frm, cdt, cdn) {
				...
				artech.accounts.pos.get_payment_mode_account(frm, d.mode_of_payment, function (account) {
					artech_engine.model.set_value(cdt, cdn, "account", account);
				});
			},
		});
	},
	...
	get_payment_mode_account: function (frm, mode_of_payment, callback) {
		...
		return artech_engine.call({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
