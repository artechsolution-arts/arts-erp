...
artech.TransactionController = class TransactionController extends artech.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		this.barcode_scanner = new artech.utils.BarcodeScanner({ frm: this.frm });

		this.set_fields_onload_for_line_item();
		this.frm.ignore_doctypes_on_cancel_all = ["Serial and Batch Bundle"];

		artech_engine.flags.hide_serial_batch_dialog = true;
		artech_engine.ui.form.on(this.frm.doctype + " Item", "rate", function (frm, cdt, cdn) {
			var item = artech_engine.get_doc(cdt, cdn);
			var has_margin_field = artech_engine.meta.has_field(cdt, "margin_type");

			artech_engine.model.round_floats_in(item, ["rate", "price_list_rate"]);
			...
			if (item.item_code && item.rate) {
				artech_engine.call({
					method: "artech.stock.get_item_details.get_item_tax_template",
					args: {
						ctx: {
					...
					callback: function (r) {
						...
						artech_engine.model.set_value(cdt, cdn, "item_tax_template", item_tax_template);
					},
				});
			}
		});

		artech_engine.ui.form.on(this.frm.cscript.tax_table, "rate", function (frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		artech_engine.ui.form.on(this.frm.cscript.tax_table, "tax_amount", function (frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		artech_engine.ui.form.on(this.frm.cscript.tax_table, "row_id", function (frm, cdt, cdn) {
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		artech_engine.ui.form.on(this.frm.cscript.tax_table, "included_in_print_rate", function (frm, cdt, cdn) {
			cur_frm.cscript.set_dynamic_labels();
			cur_frm.cscript.calculate_taxes_and_totals();
		});

		artech_engine.ui.form.on(this.frm.doctype, "apply_discount_on", function (frm) {
		...
		artech_engine.ui.form.on(this.frm.doctype, "additional_discount_percentage", function (frm) {
			if (!frm.doc.apply_discount_on) {
				artech_engine.msgprint(__("Please set 'Apply Additional Discount On'"));
				return;
			}
			...
			var total = flt(frm.doc[artech_engine.model.scrub(frm.doc.apply_discount_on)]);
		...
		artech_engine.ui.form.on(this.frm.doctype, "discount_amount", function (frm) {
		...
		artech_engine.ui.form.on("Tax Withholding Entry", "taxable_amount", function (frm, cdt, cdn) {
			me.calculate_withholding_amount(frm, cdt, cdn);
		});

		artech_engine.ui.form.on("Tax Withholding Entry", "tax_rate", function (frm, cdt, cdn) {
			me.calculate_withholding_amount(frm, cdt, cdn);
		});

		artech_engine.ui.form.on(this.frm.doctype + " Item", {
			items_add: function (frm, cdt, cdn) {
				...
				if (
					item.docstatus === 0 &&
					artech_engine.meta.has_field(item.doctype, "use_serial_batch_fields") &&
					cint(artech_engine.user_defaults?.use_serial_batch_fields) === 1
				) {
					artech_engine.model.set_value(item.doctype, item.name, "use_serial_batch_fields", 1);
				}

				artech.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, "items");
			},
		});
		...
		if (this.frm.fields_dict["items"].grid.get_field("uom")) {
			this.frm.set_query("uom", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "artech.controllers.queries.get_item_uom_query",
					filters: {
		...
		if (artech_engine.meta.get_docfield(this.frm.doc.doctype, "pricing_rules")) {
		...
		if (this.frm.fields_dict["items"].grid.get_field("blanket_order")) {
			this.frm.set_query("blanket_order", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "artech.controllers.queries.get_blanket_orders",
					filters: {
						company: doc.company,
	...
	use_serial_batch_fields(frm, cdt, cdn) {
		...
		if (!item.use_serial_batch_fields) {
			artech_engine.model.set_value(cdt, cdn, "serial_no", "");
			artech_engine.model.set_value(cdt, cdn, "batch_no", "");
			artech_engine.model.set_value(cdt, cdn, "rejected_serial_no", "");
		}
	}
	...
	toggle_enable_for_stock_uom(field) {
		artech_engine.call({
			method: "artech.stock.doctype.stock_settings.stock_settings.get_enable_stock_uom_editing",
			callback: (r) => {
				if (r.message) {
	...
	onload() {
		...
		if (this.frm.doc.__islocal) {
			var currency = artech_engine.defaults.get_user_default("currency");
			...
			return artech_engine.run_serially([
	...
	setup_quality_inspection() {
		...
		if (
			...
			artech_engine.model.can_create("Quality Inspection") &&
			show_qc_button
		) {
	...
	make_payment_request() {
		...
		artech_engine.call({
			method: "artech.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	make_payment_request_with_schedule = async function () {
		let frm = this.frm;
		const { message: schedules } = await artech_engine.call({
			method: "artech.accounts.doctype.payment_request.payment_request.get_available_payment_schedules",
			args: {
				reference_doctype: frm.doctype,
				reference_name: frm.doc.name,
			},
		});
		const value = await artech_engine.db.get_single_value(
			"Accounts Settings",
		...
		if (!schedules || !schedules.length) {
			artech_engine.msgprint(__("No pending payment schedules available."));
			return;
		}
		schedules.forEach((schedule) => (schedule.__checked = 1));

		const dialog = new artech_engine.ui.Dialog({
			...
			primary_action: async () => {
				...
				const payment_request_type = ["Sales Order", "Sales Invoice"].includes(this.frm.doc.doctype)
					? "Inward"
					: "Outward";
				const { message: pr_name } = await artech_engine.call({
					method: "artech.accounts.doctype.payment_request.payment_request.make_payment_request",
					args: {
						dt: me.frm.doc.doctype,
						dn: me.frm.doc.name,
				...
				artech_engine.model.sync(pr_name);
				artech_engine.set_route("Form", "Payment Request", pr_name.name);
			},
		});
	...
	onload_post_render() {
		if (
			...
			artech_engine.after_ajax(() => this.apply_default_taxes());
		} else if (
			this.frm.doc.__islocal &&
			...
			artech_engine.after_ajax(() => this.calculate_taxes_and_totals());
		}
		if (artech_engine.meta.get_docfield(this.frm.doc.doctype + " Item", "item_code")) {
	...
	refresh() {
		...
		artech.hide_company(this.frm);
		this.set_dynamic_labels();
		this.setup_sms();
		this.setup_quality_inspection();
		this.validate_has_items();
		artech.utils.view_serial_batch_nos(this.frm);
		this.set_route_options_for_new_doc();
		artech.toggle_serial_batch_fields(this.frm);
	}
	...
	scan_barcode() {
		artech_engine.flags.dialog_set = false;
		this.barcode_scanner.process_scan();
	}

	barcode(doc, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (row.barcode && !artech_engine.flags.trigger_from_barcode_scanner) {
	...
	apply_default_taxes() {
		var me = this;
		var taxes_and_charges_field = artech_engine.meta.get_docfield(
			me.frm.doc.doctype,
			"taxes_and_charges",
			me.frm.doc.name
		);
		...
		if (taxes_and_charges_field) {
			return artech_engine.call({
				method: "artech.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	setup_sms() {
		...
		if (
			artech_engine.boot.sms_gateway_enabled &&
	...
	calculate_withholding_amount(frm, cdt, cdn) {
		...
		let row = artech_engine.get_doc(cdt, cdn);
		...
		artech_engine.model.set_value(cdt, cdn, "withholding_amount", withholding_amount);
	}

	send_sms() {
		var sms_man = new artech.SMSManager(this.frm.doc);
	}
	...
	item_code(doc, cdt, cdn) {
		...
		if (!artech_engine.boot.sysdefaults.use_legacy_js_reactivity) {
			var item = artech_engine.get_doc(cdt, cdn);
		...
		if (item.item_code || item.serial_no) {
			if (!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "artech.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						ctx: {
							...
							artech_engine.run_serially([
								...
								async () => {
									...
									const fetch_valuation_rate_for_internal_transactions =
										await artech_engine.db.get_single_value(
											"Accounts Settings",
	...
	show_batch_dialog_if_required(item) {
		...
		artech_engine.run_serially([
			() => {
				if (show_batch_dialog && !artech_engine.flags.trigger_from_barcode_scanner)
					return artech_engine.db
						.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"])
						.then((r) => {
							if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
								item.has_serial_no = r.message.has_serial_no;
								item.has_batch_no = r.message.has_batch_no;
								artech_engine.flags.hide_serial_batch_dialog = false;
							} else {
			...
			() => {
				...
				if (show_batch_dialog && !artech_engine.flags.hide_serial_batch_dialog)
					return artech_engine.db
			...
			() => {
				if (show_batch_dialog && !artech_engine.flags.hide_serial_batch_dialog && !artech_engine.flags.dialog_set) {
					if (item.has_batch_no && item.has_serial_no) {
						item.batch_no = undefined;
					}

					artech_engine.flags.dialog_set = true;
					artech.show_serial_batch_selector(
						me.frm,
						item,
						(item) => {
						...
						undefined,
						!artech_engine.flags.hide_serial_batch_dialog
					);
				} else {
					artech_engine.flags.dialog_set = false;
				}
	...
	price_list_rate(doc, cdt, cdn) {
		var item = artech_engine.get_doc(cdt, cdn);
		artech_engine.model.round_floats_in(item, ["price_list_rate", "discount_percentage"]);
	...
	margin_rate_or_amount(doc, cdt, cdn) {
		...
		let item = artech_engine.get_doc(cdt, cdn);
		this.apply_pricing_rule_on_item(item);
		this.calculate_taxes_and_totals();
	...
	margin_type(doc, cdt, cdn) {
		...
		let item = artech_engine.get_doc(cdt, cdn);
		if (!item.margin_type) {
			artech_engine.model.set_value(cdt, cdn, "margin_rate_or_amount", 0);
		} else {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		artech_engine.call({
			method: "artech.stock.utils.get_incoming_rate",
			args: {
				args: item_args,
			},
			callback: function (r) {
				artech_engine.model.set_value(item.doctype, item.name, "rate", r.message * item.conversion_factor);
			},
		});
	}
	...
	refresh_serial_batch_bundle_field() {
		artech_engine.route_hooks.after_submit = (frm_obj) => {
			frm_obj.reload_doc();
		};
	}
	...
	update_qty(cdt, cdn) {
		...
		artech_engine.model.set_value(item.doctype, item.name, "stock_qty", valid_serial_nos.length);
	}
	...
	async confirm_posting_date_change() {
		if (!artech_engine.meta.has_field(this.frm.doc.doctype, "set_posting_time")) return;
		if (this.frm.doc.set_posting_time) return;
		if (artech_engine.datetime.get_today() == this.frm.doc.posting_date) return;

		let is_confirmation_reqd = await artech_engine.db.get_single_value(
			"Accounts Settings",
		...
		return new Promise((resolve, reject) => {
			artech_engine.confirm(
				...
				() => {
					this.frm.doc.posting_date = artech_engine.datetime.get_today();
	...
	company() {
		...
		var get_party_currency = function () {
			...
			if (me.frm.doc.doctype == "Quotation" && me.frm.doc.quotation_to == "Customer") {
				...
				party_type = artech_engine.meta.has_field(me.frm.doc.doctype, "supplier") ? "Supplier" : "Customer";
		...
		var set_terms = function () {
			if (artech_engine.meta.has_field(me.frm.doc.doctype, "tc_name") && !me.frm.doc.tc_name) {
				var company_doc = artech_engine.get_doc(":Company", me.frm.doc.company);
		...
		var set_party_account = function (set_pricing) {
			if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
				...
				var party = me.frm.doc[artech_engine.model.scrub(party_type)];
				if (
					party &&
					me.frm.doc.company &&
					...
					return artech_engine.call({
						method: "artech.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (
			artech_engine.meta.get_docfield(this.frm.doctype, "shipping_address") &&
			...
			if (!is_drop_ship) {
				artech.utils.get_shipping_address(this.frm, function () {
		...
		if (this.frm.doc.company) {
			artech.last_selected_company = this.frm.doc.company;
		}
	}
	...
	transaction_date() {
		if (this.frm.doc.transaction_date) {
			...
			artech_engine.ui.form.trigger(this.frm.doc.doctype, "currency");
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if (
				...
				return artech_engine.call({
					method: "artech.accounts.party.get_due_date",
					args: {
					...
					callback: function (r, rt) {
						if (r.message) {
							...
							artech_engine.ui.form.trigger(me.frm.doc.doctype, "currency");
	...
	cost_center(doc, cdt, cdn) {
		artech.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
	}
	...
	due_date(doc, cdt, cdn) {
		...
		if (
			...
			if (doc.payment_terms_template) {
				to_clear.push(__(artech_engine.meta.get_label(cdt, "payment_terms_template")));
			}

			if (doc.payment_schedule?.length) {
				to_clear.push(__(artech_engine.meta.get_label(cdt, "payment_schedule")));
			}

			artech_engine.confirm(
				__(
					"For the new {0} to take effect, would you like to clear the current {1}?",
					[__(artech_engine.meta.get_label(cdt, "due_date")), artech_engine.utils.comma_and(to_clear)],
	...
	get_company_currency() {
		return artech.get_currency(this.frm.doc.company);
	}

	contact_person() {
		artech.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", artech.stale_rate_allowed() ? 0 : 1);
	}
	...
	get_exchange_rate(transaction_date, from_currency, to_currency, callback) {
		...
		return artech_engine.call({
			method: "artech.setup.utils.get_exchange_rate",
			args: {
	...
	uom(doc, cdt, cdn) {
		...
		if (item.item_code && item.uom) {
			return this.frm.call({
				method: "artech.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom,
				},
				callback: function (r) {
					if (!r.exc) {
						artech_engine.model.set_value(cdt, cdn, "conversion_factor", r.message.conversion_factor);
						me.apply_price_list(item, true);
					}
				},
			});
	...
	conversion_factor(doc, cdt, cdn, dont_fetch_price_list_rate) {
		if (artech_engine.meta.get_docfield(cdt, "stock_qty", cdn)) {
			var item = artech_engine.get_doc(cdt, cdn);
			artech_engine.model.round_floats_in(item, ["qty", "conversion_factor"]);
			...
			if (artech_engine.flags.dont_fetch_price_list_rate) {
				return;
			}

			if (!dont_fetch_price_list_rate && artech_engine.meta.has_field(doc.doctype, "price_list_currency")) {
		...
		if (this.frm.get_field("items").grid.fields_map.conversion_factor) {
			this.frm.fields_dict.items.grid.toggle_enable(
				...
				item.uom != item.stock_uom &&
					!artech_engine.meta.get_docfield(cur_frm.fields_dict.items.grid.doctype, "conversion_factor")
						.read_only
					? true
					: false
			);
	...
	service_stop_date(frm, cdt, cdn) {
		...
		if (child.service_stop_date) {
			...
			if (stop_date < start_date) {
				artech_engine.model.set_value(cdt, cdn, "service_stop_date", "");
				artech_engine.throw(__("Service Stop Date cannot be before Service Start Date"));
			} else if (stop_date > end_date) {
				artech_engine.model.set_value(cdt, cdn, "service_stop_date", "");
				artech_engine.throw(__("Service Stop Date cannot be after Service End Date"));
			}
		}
	}
	...
	service_start_date(frm, cdt, cdn) {
		...
		if (child.service_start_date) {
			artech_engine.call({
				method: "artech.stock.get_item_details.calculate_service_end_date",
				args: { ctx: child },
				callback: function (r) {
					artech_engine.model.set_value(cdt, cdn, "service_end_date", r.message.service_end_date);
				},
			});
		}
	}
	...
	change_form_labels(company_currency) {
		...
		if (this.frm.doc.doctype && artech_engine.meta.get_docfield(this.frm.doc.doctype, "net_total")) {
			this.frm.toggle_display("net_total", show);
		}

		if (this.frm.doc.doctype && artech_engine.meta.get_docfield(this.frm.doc.doctype, "base_net_total")) {
	...
	change_grid_labels(company_currency) {
		...
		if (this.frm.doc.operations && this.frm.doc.operations.length > 0) {
			...
			$.each(["base_operating_cost", "base_hour_rate"], function (i, fname) {
				if (artech_engine.meta.get_docfield(item_grid.doctype, fname))
	...
	update_payment_schedule_grid_labels(company_currency) {
		...
		if (this.frm.doc.payment_schedule && this.frm.doc.payment_schedule.length > 0) {
			...
			$.each(["base_payment_amount", "base_outstanding", "base_paid_amount"], function (i, fname) {
				if (artech_engine.meta.get_docfield(schedule_grid.doctype, fname))
	...
	batch_no(frm, cdt, cdn) {
		...
		if (row.batch_no) {
			...
			artech_engine.call({
				method: "artech.stock.get_item_details.get_batch_based_item_price",
				args: {
					pctx: params,
					item_code: row.item_code,
				},
	...
	toggle_item_grid_columns(company_currency) {
		...
		$.each(["net_rate", "net_amount"], function (i, fname) {
			if (artech_engine.meta.get_docfield(item_grid.doctype, fname)) item_grid.set_column_disp(fname, show);
		});
	...
	ignore_pricing_rule() {
		if (this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "artech.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules) {
			let me = this;
			return this.frm.call({
				method: "artech.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "artech.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	_set_values_for_item_list(children) {
		...
		for (const child of children) {
			const existing_pricing_rule = artech_engine.model.get_value(child.doctype, child.name, "pricing_rules");

			for (const [key, value] of Object.entries(child)) {
				if (!["doctype", "name"].includes(key)) {
					if (key === "price_list_rate") {
						artech_engine.model.set_value(child.doctype, child.name, "rate", value);
					}

					if (key === "pricing_rules") {
						artech_engine.model.set_value(child.doctype, child.name, key, value);
					}
			...
			artech_engine.model.round_floats_in(artech_engine.get_doc(child.doctype, child.name), [
				"price_list_rate",
				"discount_percentage",
			]);
			...
			if (!this.frm.doc.ignore_pricing_rule && existing_pricing_rule && !child.pricing_rules) {
				this.apply_price_list(artech_engine.get_doc(child.doctype, child.name));
			} else if (!child.pricing_rules) {
				this.remove_pricing_rule(artech_engine.get_doc(child.doctype, child.name));
			}
	...
	apply_rule_on_other_items(args) {
		...
		for (var k in args) {
			...
			if (data && data.apply_rule_on_other_items && JSON.parse(data.apply_rule_on_other_items)) {
				fields.push(artech_engine.scrub(data.pricing_rule_for));
					...
					if (JSON.parse(data.apply_rule_on_other_items).includes(d[data.apply_rule_on])) {
						for (var k in data) {
							if (
								...
								artech_engine.model.set_value(d.doctype, d.name, k, data[k]);
							}
						}
					}
				});
	...
	apply_product_discount(args) {
		...
		args.free_item_data.forEach(async (pr_row) => {
			...
			if (
				...
				row_to_modify = artech_engine.model.add_child(this.frm.doc, this.frm.doc.doctype + " Item", "items");
			} else if (items) {
			...
			if (this.frm.doc.hasOwnProperty("is_pos") && this.frm.doc.is_pos) {
				let r = await artech_engine.db.get_value("POS Profile", this.frm.doc.pos_profile, "cost_center");
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `artech.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm
			.call({
				method: "artech.stock.get_item_details.apply_price_list",
	...
	validate_company_and_party() {
		...
		if (artech_engine.flags.ignore_company_party_validation) {
			return valid;
		}

		$.each(["company", "customer"], function (i, fieldname) {
			if (
				artech_engine.meta.has_field(me.frm.doc.doctype, fieldname) &&
				...
				if (!me.frm.doc[fieldname]) {
					artech_engine.msgprint(
						__("Please specify") +
							": " +
							__(artech_engine.meta.get_label(me.frm.doc.doctype, fieldname, me.frm.doc.name)) +
							". " +
	...
	get_terms() {
		...
		artech.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function (r) {
			if (!r.exc) {
	...
	taxes_and_charges() {
		...
		if (this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "artech.controllers.accounts_controller.get_taxes_and_charges",
				args: {
					master_doctype: artech_engine.meta.get_docfield(
						this.frm.doc.doctype,
						"taxes_and_charges",
						this.frm.doc.name
					).options,
	...
	tax_category() {
		...
		artech_engine.run_serially([
			...
			() => artech.utils.set_taxes(this.frm, "tax_category"),
		]);
	}
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "artech.stock.get_item_details.get_item_tax_info",
				args: {
					doc: me.frm.doc,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if (item.item_tax_template) {
			return this.frm.call({
				method: "artech.stock.get_item_details.get_item_tax_map",
				args: {
					doc: me.frm.doc,
	...
	is_recurring() {
		...
		if (this.frm.doc.is_recurring && this.frm.doc.__islocal) {
			artech_engine.msgprint(__("Please set recurring after saving"));
		...
		if (this.frm.doc.is_recurring) {
			...
			var owner_email =
				this.frm.doc.owner == "Administrator"
					? artech_engine.user_info("Administrator").email
					: this.frm.doc.owner;
			...
			this.frm.doc.repeat_on_day_of_month = artech_engine.datetime
	...
	from_date() {
		...
		if (this.frm.doc.from_date) {
			...
			if (months) {
				var to_date = artech_engine.datetime.add_months(this.frm.doc.from_date, months);
				this.frm.doc.to_date = artech_engine.datetime.add_days(to_date, -1);
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new artech.ItemSelector({frm: this.frm});
		// }
	}
	...
	make_quality_inspection() {
		...
		const dialog = new artech_engine.ui.Dialog({
			...
			primary_action: function () {
				...
				artech_engine.call({
					method: "artech.controllers.stock_controller.make_quality_inspections",
					args: {
						company: me.frm.doc.company,
						doctype: me.frm.doc.doctype,
					...
					callback: function (r) {
						if (r.message.length > 0) {
							if (r.message.length === 1) {
								artech_engine.set_route("Form", "Quality Inspection", r.message[0]);
							} else {
								artech_engine.route_options = {
									reference_type: me.frm.doc.doctype,
									reference_name: me.frm.doc.name,
								};
								artech_engine.set_route("List", "Quality Inspection");
							}
						}
						dialog.hide();
					},
		...
		artech_engine.call({
			method: "artech.controllers.stock_controller.check_item_quality_inspection",
			args: {
	...
	get_method_for_payment() {
		let method = "artech.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if (this.frm.doc.__onload && this.frm.doc.__onload.make_payment_via_journal_entry) {
			if (["Sales Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
				method =
					"artech.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			} else {
				method =
					"artech.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			artech_engine.throw(__("Please enter Item Code to get batch no"));
		} else if (
			...
			let filters = {
				item_code: item.item_code,
				posting_date: me.frm.doc.posting_date || artech_engine.datetime.nowdate(),
			};
			...
			return {
				query: "artech.controllers.queries.get_batch_no",
				filters: filters,
			};
		}
	}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			...
			return {
				query: "artech.controllers.queries.get_tax_template",
				filters: filters,
			};
		}
	}
	...
	payment_terms_template() {
		...
		if (doc.payment_terms_template && doc.doctype !== "Delivery Note" && !doc.is_return) {
			...
			artech_engine.call({
				method: "artech.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if (row.payment_term) {
			artech_engine.call({
				method: "artech.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	against_blanket_order(doc, cdt, cdn) {
		...
		if (!item.against_blanket_order) {
			artech_engine.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order", null);
			artech_engine.model.set_value(this.frm.doctype + " Item", item.name, "blanket_order_rate", 0.0);
		}
	}
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype == "Sales Order" || item.parenttype == "Purchase Order")) {
			artech_engine.call({
				method: "artech.stock.get_item_details.get_blanket_order_details",
				args: {
					ctx: {
						item_code: item.item_code,
				...
				callback: function (r) {
					if (!r.message) {
						artech_engine.throw(__("Invalid Blanket Order for the selected Customer and Item"));
					} else {
						artech_engine.run_serially([
							() =>
								artech_engine.model.set_value(
	...
	autofill_warehouse(child_table, warehouse_field, warehouse) {
		if (warehouse && child_table && child_table.length) {
			...
			$.each(child_table || [], function (i, item) {
				artech_engine.model.set_value(doctype, item.name, warehouse_field, warehouse);
			});
		}
	}
	...
	setup_accounting_dimension_triggers() {
		artech_engine.call({
			method: "artech.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
				if (r.message && r.message[0]) {
					let dimensions = r.message[0].map((d) => d.fieldname);
					dimensions.forEach((dim) => {
						// nosemgrep: artech_engine-semgrep-rules.rules.artech_engine-cur-frm-usage
						cur_frm.cscript[dim] = function (doc, cdt, cdn) {
							artech.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", dim);
						};
					});
				}
			},
...
artech.show_serial_batch_selector = function (frm, item_row, callback, on_close, show_dialog) {
	...
	new artech.SerialBatchPackageSelector(frm, item_row, (r) => {
		if (r) {
			let update_values = {
			...
			artech_engine.model.set_value(item_row.doctype, item_row.name, update_values);
		}
	});
};

artech.apply_putaway_rule = (frm, purpose = null) => {
	if (!frm.doc.company) {
		artech_engine.throw({ message: __("Please select a Company first."), title: __("Mandatory") });
	}
	if (!frm.doc.items.length) return;

	artech_engine.call({
		method: "artech.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
			company: frm.doc.company,
...
artech.set_unit_price_items_note = (frm) => {
