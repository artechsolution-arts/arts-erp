...
artech.accounts.bank_reconciliation.DataTableManager = class DataTableManager {
	constructor(opts) {
		Object.assign(this, opts);
		this.dialog_manager = new artech.accounts.bank_reconciliation.DialogManager(
			this.company,
			this.bank_account,
	...
	make_dt() {
		const me = this;
		artech_engine.call({
			method: "artech.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_bank_transactions",
			args: {
	...
	format_row(row) {
		return [
			row["date"],
			row["party_type"],
			artech_engine.form.formatters.Link(row["party"], { options: row["party_type"] }),
			row["description"],
			row["deposit"],
			row["withdrawal"],
	...
	get_datatable() {
		...
		this.datatable = new artech_engine.DataTable(this.$reconciliation_tool_dt.get(0), datatable_options);
