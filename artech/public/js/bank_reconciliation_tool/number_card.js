...
artech.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new artech.accounts.NumberCard(summary);
...
artech.accounts.NumberCard = class NumberCard {
	constructor(options) {
		this.$card = artech_engine.utils.build_summary_item(options);
	}
