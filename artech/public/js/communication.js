artech_engine.ui.form.on("Communication", {
	refresh: (frm) => {
	...
	setup_custom_buttons: (frm) => {
		...
		if (frm.doc.reference_doctype !== "Issue") {
			frm.add_custom_button(
				__("Issue"),
				() => {
					artech_engine.confirm(__(confirm_msg, [__("Issue")]), () => {
		...
		if (!["Lead", "Opportunity"].includes(frm.doc.reference_doctype)) {
			frm.add_custom_button(
				__("Lead"),
				() => {
					artech_engine.confirm(__(confirm_msg, [__("Lead")]), () => {
			...
			frm.add_custom_button(
				__("Opportunity"),
				() => {
					artech_engine.confirm(__(confirm_msg, [__("Opportunity")]), () => {
	...
	make_lead_from_communication: (frm) => {
		return artech_engine.call({
			method: "artech.crm.doctype.lead.lead.make_lead_from_communication",
			args: {
	...
	make_issue_from_communication: (frm) => {
		return artech_engine.call({
			method: "artech.support.doctype.issue.issue.make_issue_from_communication",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
	...
	make_opportunity_from_communication: (frm) => {
		...
		artech_engine.prompt(
			...
			(data) => {
				artech_engine.call({
					method: "artech.crm.doctype.opportunity.opportunity.make_opportunity_from_communication",
					args: {
						communication: frm.doc.name,
