...
function handle_route_event() {
	const route = artech_engine.get_route();
...
function fetch_company_details(current_doctype, current_docname) {
	artech_engine.call({
		method: "artech.controllers.accounts_controller.get_missing_company_details",
...
function save_company_details(dialog, data, values, current_doctype) {
	artech_engine.call({
		method: "artech.controllers.accounts_controller.update_company_master_and_address",
		args: {
			name: data.name,
		...
		callback() {
			dialog.hide();
			artech_engine.msgprint(__("Updating details."));
