...
from artech_engine.utils.data import add_days, formatdate, today

from artech.maintenance.doctype.maintenance_schedule.maintenance_schedule import (
...
class TestMaintenanceSchedule(ArtechTestSuite):
	def setUp(self):
	...
	def make_sales_person(cls):
		...
		cls.sales_person = []
		for x in records:
			if not artech_engine.db.exists("Sales Person", {"sales_person_name": x.get("sales_person_name")}):
				cls.sales_person.append(artech_engine.get_doc(x).insert())
			else:
				cls.sales_person.append(
					artech_engine.get_doc("Sales Person", {"sales_person_name": x.get("sales_person_name")})
				)
	...
	def test_make_schedule(self):
		...
		visit = artech_engine.new_doc("Maintenance Visit")
		...
		visit.save()
		visit.submit()
		ms = artech_engine.get_doc("Maintenance Schedule", ms.name)
...
def make_serial_item_with_serial(self, item_code):
	...
	active_serials = artech_engine.db.get_all("Serial No", {"status": "Active", "item_code": item_code})
...
def make_maintenance_schedule(**args):
	ms = artech_engine.new_doc("Maintenance Schedule")
