...
from artech_engine.utils import add_days, cint, cstr, date_diff, formatdate, getdate
...
from artech.utilities.transaction_base import TransactionBase, delete_events
...
class MaintenanceSchedule(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from artech.maintenance.doctype.maintenance_schedule_detail.maintenance_schedule_detail import (
			MaintenanceScheduleDetail,
		)
		from artech.maintenance.doctype.maintenance_schedule_item.maintenance_schedule_item import (
	...
	def on_submit(self):
		...
		for d in self.get("items"):
			...
			if d.sales_person and d.sales_person not in email_map:
				sp = artech_engine.get_doc("Sales Person", d.sales_person)
				try:
			...
			scheduled_date = artech_engine.db.get_all(
			...
			for key in scheduled_date:
				description = artech_engine._("Reference: {0}, Item Code: {1} and Customer: {2}").format(
					self.name, d.item_code, self.customer
				)
				event = artech_engine.get_doc(
					{
						"doctype": "Event",
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		employee = artech_engine.db.get_value("Sales Person", sales_person, "employee")
		if employee:
			holiday_list = get_holiday_list_for_employee(employee)
		else:
			holiday_list = artech_engine.get_cached_value("Company", self.company, "default_holiday_list")

		holidays = artech_engine.db.sql_list(
	...
	def validate_serial_no_bundle(self):
		...
		voucher_nos = artech_engine.get_all(
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			...
			if not sr_details:
				artech_engine.throw(_("Serial No {0} not found").format(serial_no))

			if sr_details.get("item_code") != item_code:
				artech_engine.throw(
					_("Serial No {0} does not belong to Item {1}").format(
						artech_engine.bold(serial_no), artech_engine.bold(item_code)
					),
					title=_("Invalid"),
				)
	...
	def get_pending_data(self, data_type: str, s_date: str | None = None, item_name: str | None = None):
		if data_type == "date":
			dates = ""
			if not item_name:
				artech_engine.throw(_("Item Name is required."))
			...
			if not s_date:
				artech_engine.throw(_("Scheduled Date is required."))
			for schedule in self.schedules:
...
def get_serial_nos_from_schedule(item_code: str, schedule: str):
	serial_nos = artech_engine.db.get_value(
...
def make_maintenance_visit(
	...
	def update_serial(source, target, parent):
		if source.item_reference:
			if sbb := artech_engine.db.get_value(
				"Maintenance Schedule Item", source.item_reference, "serial_and_batch_bundle"
			):
				serial_nos = artech_engine.get_doc("Serial and Batch Bundle", sbb).get_serial_nos()
