...
from artech_engine.utils import add_days, flt, formatdate, getdate

from artech.accounts.doctype.account_closing_balance.account_closing_balance import (
	make_closing_entries,
)
from artech.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
)
from artech.accounts.utils import get_account_currency, get_fiscal_year
...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_start_and_end_date(self):
		self.fy_start_date, self.fy_end_date = artech_engine.db.get_value(
		...
		if getdate(self.period_start_date) != getdate(valid_start_date):
			artech_engine.throw(_("Period Start Date must be {0}").format(formatdate(valid_start_date)))

		if getdate(self.period_start_date) > getdate(self.period_end_date):
			artech_engine.throw(_("Period Start Date cannot be greater than Period End Date"))

		if getdate(self.period_end_date) > getdate(self.fy_end_date):
			artech_engine.throw(_("Period End Date cannot be greater than Fiscal Year End Date"))
	...
	def check_if_previous_year_closed(self):
		...
		previous_fiscal_year_closed = artech_engine.db.exists(
		...
		gle_exists_in_previous_year = artech_engine.db.exists(
			"GL Entry",
			{
		...
		artech_engine.throw(_("Previous Year is not closed, please close it first"))
	...
	def check_closing_account_type(self):
		closing_account_type = artech_engine.get_cached_value("Account", self.closing_account_head, "root_type")
	...
	def check_closing_account_currency(self):
		...
		company_currency = artech_engine.get_cached_value("Company", self.company, "default_currency")
		if account_currency != company_currency:
			artech_engine.throw(_("Currency of the Closing Account must be {0}").format(company_currency))

	def on_submit(self):
		self.db_set("gle_processing_status", "In Progress")
		if artech_engine.get_single_value("Accounts Settings", "use_legacy_controller_for_pcv"):
			self.make_gl_entries()
		else:
			ppcv = artech_engine.get_doc({"doctype": "Process Period Closing Voucher", "parent_pcv": self.name})
			ppcv.save().submit()
	...
	def on_cancel(self):
		...
		if not artech_engine.get_single_value("Accounts Settings", "use_legacy_controller_for_pcv"):
	...
	def cancel_process_pcv_docs(self):
		ppcvs = artech_engine.db.get_all("Process Period Closing Voucher", {"parent_pcv": self.name, "docstatus": 1})
		for x in ppcvs:
			artech_engine.get_doc("Process Period Closing Voucher", x.name).cancel()

	def on_trash(self):
		super().on_trash()
		ppcvs = artech_engine.db.get_all(
			"Process Period Closing Voucher", {"parent_pcv": self.name, "docstatus": ["in", [1, 2]]}
		)
		for x in ppcvs:
			artech_engine.delete_doc("Process Period Closing Voucher", x.name, force=True, ignore_permissions=True)

	def make_gl_entries(self):
		if artech_engine.db.estimate_count("GL Entry") > 100_000:
		...
		gl_entry = artech_engine._dict(
			{
				...
				"account_currency": artech_engine.db.get_value(
	...
	def get_account_balances_based_on_dimensions(self, report_type):
		...
		acc_bal_dict = artech_engine._dict()
		gl_entries = []
	...
	def set_account_balance_dict(self, gle, acc_bal_dict):
		...
		acc_bal_dict.setdefault(key, artech_engine._dict()).setdefault(
			gle.account,
			artech_engine._dict(
				{
	...
	def get_closing_entry(self, account, balances, dimensions):
		closing_entry = artech_engine._dict(
			{
				"company": self.company,
	...
	def is_first_period_closing_voucher(self):
		first_pcv = artech_engine.db.get_value(
			"Period Closing Voucher",
...
def process_gl_and_closing_entries(doc):
	...
	try:
		...
		artech_engine.db.set_value(doc.doctype, doc.name, "gle_processing_status", "Completed")
	except Exception as e:
		artech_engine.db.rollback()
		artech_engine.log_error(title=_("Period Closing Voucher {0} GL Entry Processing Failed").format(doc.name))
...
def process_cancellation(voucher_type, voucher_no):
	...
	try:
		...
		artech_engine.db.set_value("Period Closing Voucher", voucher_no, "gle_processing_status", "Completed")
	except Exception as e:
		artech_engine.db.rollback()
...
def delete_closing_entries(voucher_no):
	closing_balance = artech_engine.qb.DocType("Account Closing Balance")
	artech_engine.qb.from_(closing_balance).delete().where(
...
def get_period_start_end_date(fiscal_year: str, company: str):
	fy_start_date, fy_end_date = artech_engine.db.get_value(
...
def get_previous_closed_period_in_current_year(fiscal_year, company):
	prev_closed_period_end_date = artech_engine.db.get_value(
		"Period Closing Voucher",
		filters={
			"company": company,
