...
artech_engine.ui.form.on("Budget", {
	onload: function (frm) {
		...
		artech_engine.db.get_single_value("Accounts Settings", "use_legacy_budget_controller").then((value) => {
			if (value) {
	...
	revise_budget_action: function (frm) {
		artech_engine.confirm(
			...
			function () {
				artech_engine.call({
					method: "artech.accounts.doctype.budget.budget.revise_budget",
					args: { budget_name: frm.doc.name },
					callback: function (r) {
						if (r.message) {
							artech_engine.msgprint(__("New revised budget created successfully"));
							artech_engine.set_route("Form", "Budget", r.message);
						}
					},
				});
			},
			function () {
				artech_engine.msgprint(__("Revision cancelled"));
			}
		);
	},
});

artech_engine.ui.form.on("Budget Distribution", {
	amount(frm, cdt, cdn) {
