...
from artech_engine.utils import add_months, flt, fmt_money, get_last_day, getdate
from artech_engine.utils.data import get_first_day
...
class BudgetError(artech_engine.ValidationError):
	pass


class DuplicateBudgetError(artech_engine.ValidationError):
	pass
...
class Budget(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.budget_distribution.budget_distribution import BudgetDistribution
	...
	def validate(self):
		if not self.get(artech_engine.scrub(self.budget_against)):
			artech_engine.throw(_("{0} is mandatory").format(self.budget_against))
	...
	def validate_budget_amount(self):
		if self.budget_amount <= 0:
			artech_engine.throw(_("Budget Amount can not be {0}.").format(self.budget_amount))
	...
	def validate_fiscal_year_company(self, fiscal_year, company):
		linked_companies = artech_engine.get_all(
			"Fiscal Year Company", filters={"parent": fiscal_year}, pluck="company"
		)
		if linked_companies and company not in linked_companies:
			artech_engine.throw(_("Fiscal Year {0} is not available for Company {1}.").format(fiscal_year, company))

	def set_fiscal_year_dates(self):
		if self.from_fiscal_year:
			self.budget_start_date = artech_engine.get_cached_value(
				"Fiscal Year", self.from_fiscal_year, "year_start_date"
			)
		if self.to_fiscal_year:
			self.budget_end_date = artech_engine.get_cached_value(
				"Fiscal Year", self.to_fiscal_year, "year_end_date"
			)

		if self.budget_start_date > self.budget_end_date:
			artech_engine.throw(_("From Fiscal Year cannot be greater than To Fiscal Year"))

	def validate_duplicate(self):
		budget_against_field = artech_engine.scrub(self.budget_against)
		...
		existing_budget = artech_engine.db.sql(
			f"""
			SELECT name, account
			FROM `tabBudget`
			WHERE
	...
	def validate_account(self):
		if not self.account:
			artech_engine.throw(_("Account is mandatory"))
		...
		if account_details.is_group:
			artech_engine.throw(_("Budget cannot be assigned against Group Account {0}").format(self.account))
		elif account_details.company != self.company:
			artech_engine.throw(_("Account {0} does not belong to company {1}").format(self.account, self.company))
	...
	def validate_applicable_for(self):
		if self.applicable_on_material_request and not (
			...
			artech_engine.throw(_("Please enable Applicable on Booking Actual Expenses"))
		...
		params = artech_engine._dict(
			{
				...
				"budget_against_field": artech_engine.scrub(self.budget_against),
				"budget_against_doctype": artech_engine.unscrub(self.budget_against),
			}
		)

		params[params.budget_against_field] = self.get(params.budget_against_field)

		if artech_engine.get_cached_value("DocType", params.budget_against_doctype, "is_tree"):
			params.is_tree = True
		else:
		...
		if actual_spent > self.budget_amount:
			artech_engine.throw(
				_(
					...
					artech_engine.bold(self.budget_start_date),
					artech_engine.bold(self.budget_end_date),
					artech_engine.bold(artech_engine.utils.fmt_money(actual_spent)),
					artech_engine.bold(artech_engine.utils.fmt_money(self.budget_amount)),
				),
...
def validate_expense_against_budget(params, expense_amount=0):
	params = artech_engine._dict(params)
	if not artech_engine.db.count("Budget", cache=True):
		return
	...
	budget_exists = artech_engine.db.sql(
		"""
		select name
		from `tabBudget`
		where company = %s
	...
	if params.get("company"):
		artech_engine.flags.exception_approver_role = artech_engine.get_cached_value(
	...
	for dimension in default_dimensions + get_accounting_dimensions(as_list=False):
		...
		if (
			...
			and params.account
			and (artech_engine.get_cached_value("Account", params.account, "root_type") == "Expense")
		):
			doctype = dimension.get("document_type")

			if artech_engine.get_cached_value("DocType", doctype, "is_tree"):
				lft, rgt = artech_engine.get_cached_value(doctype, params.get(budget_against), ["lft", "rgt"])
				condition = f"""and exists(select name from `tab{doctype}`
					where lft<={lft} and rgt>={rgt} and name=b.{budget_against})"""  # nosec
				params.is_tree = True
			else:
				condition = f"and b.{budget_against}={artech_engine.db.escape(params.get(budget_against))}"
				params.is_tree = False

			params.budget_against_field = budget_against
			params.budget_against_doctype = doctype

			budget_records = artech_engine.db.sql(
				f"""
				SELECT
					b.name,
...
def compare_expense_with_budget(params, budget_amount, action_for, action, budget_against, amount=0):
	...
	if total_expense > budget_amount:
		...
		currency = artech_engine.get_cached_value("Company", params.company, "default_currency")
		msg = _msg.format(
			_(action_for),
			artech_engine.bold(params.account),
			artech_engine.unscrub(params.budget_against_field),
			artech_engine.bold(budget_against),
			artech_engine.bold(fmt_money(budget_amount, currency=currency)),
			artech_engine.bold(fmt_money(diff, currency=currency)),
		)

		msg += get_expense_breakup(params, currency, budget_against)

		if artech_engine.flags.exception_approver_role and artech_engine.flags.exception_approver_role in artech_engine.get_roles(
			artech_engine.session.user
		):
			action = "Warn"

		if action == "Stop":
			artech_engine.throw(msg, BudgetError, title=_("Budget Exceeded"))
		else:
			artech_engine.msgprint(msg, indicator="orange", title=_("Budget Exceeded"))


def get_expense_breakup(params, currency, budget_against):
	msg = "<hr> {} - <ul>".format(_("Total Expenses booked through"))

	common_filters = artech_engine._dict(
		{
			params.budget_against_field: budget_against,
	...
	from_date = artech_engine.get_cached_value("Fiscal Year", params.from_fiscal_year, "year_start_date")
	to_date = artech_engine.get_cached_value("Fiscal Year", params.to_fiscal_year, "year_end_date")
	...
	msg += (
		"<li>"
		+ artech_engine.utils.get_link_to_report(
			"General Ledger",
			label=_("Actual Expenses"),
			filters=gl_filters,
		)
		+ " - "
		+ artech_engine.bold(fmt_money(params.actual_expense, currency=currency))
		+ "</li>"
	)
	...
	msg += (
		...
		+ " - "
		+ artech_engine.bold(fmt_money(params.requested_amount, currency=currency))
		+ "</li>"
	)
	...
	msg += (
		...
		+ " - "
		+ artech_engine.bold(fmt_money(params.ordered_amount, currency=currency))
		+ "</li></ul>"
	)
...
def get_other_condition(params, for_doc):
	...
	start_date = artech_engine.get_cached_value("Fiscal Year", params.from_fiscal_year, "year_start_date")
	end_date = artech_engine.get_cached_value("Fiscal Year", params.to_fiscal_year, "year_end_date")
...
def get_actual_expense(params):
	if not params.budget_against_doctype:
		params.budget_against_doctype = artech_engine.unscrub(params.budget_against_field)
	...
	if params.is_tree:
		lft_rgt = artech_engine.db.get_value(
...
def get_accumulated_monthly_budget(budget_name, posting_date):
	...
	bd = artech_engine.qb.DocType("Budget Distribution")
	b = artech_engine.qb.DocType("Budget")

	result = (
		artech_engine.qb.from_(bd)
		.join(b)
		.on(bd.parent == b.name)
...
def get_item_details(params):
	...
	if params.item_code:
		item_defaults = artech_engine.db.get_value(
			"Item Default",
...
def get_expense_cost_center(doctype, params):
	if doctype == "Item Group":
		return artech_engine.db.get_value(
			"Item Default",
			{"parent": params.get(artech_engine.scrub(doctype)), "company": params.get("company")},
			["buying_cost_center", "expense_account"],
		)
	else:
		return artech_engine.db.get_value(
			doctype, params.get(artech_engine.scrub(doctype)), ["cost_center", "default_expense_account"]
		)


def get_fiscal_year_date_range(from_fiscal_year, to_fiscal_year):
	from_year = artech_engine.get_cached_value(
		"Fiscal Year", from_fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	to_year = artech_engine.get_cached_value(
...
def revise_budget(budget_name: str):
	old_budget = artech_engine.get_doc("Budget", budget_name)

	if old_budget.docstatus == 1:
		old_budget.cancel()

	new_budget = artech_engine.copy_doc(old_budget)
