...
from artech_engine.utils import flt, now_datetime, nowdate

from artech.accounts.doctype.budget.budget import (
	BudgetError,
...
class TestBudget(ArtechTestSuite):
	def setUp(self):
		artech_engine.db.set_single_value("Accounts Settings", "use_legacy_budget_controller", False)
		self.company = "_Test Company"
		self.fiscal_year = artech_engine.db.get_value("Fiscal Year", {}, "name")
	...
	def test_monthly_budget_crossed_ignore(self):
		...
		self.assertTrue(
			artech_engine.db.get_value("GL Entry", {"voucher_type": "Journal Entry", "voucher_no": jv.name})
		)
	...
	def test_monthly_budget_crossed_stop1(self):
		...
		artech_engine.db.set_value("Budget", budget.name, "action_if_accumulated_monthly_budget_exceeded", "Stop")
	...
	def test_exception_approver_role(self):
		...
		artech_engine.db.set_value("Company", budget.company, "exception_budget_approver_role", "Accounts User")

		jv.submit()
		self.assertEqual(artech_engine.db.get_value("Journal Entry", jv.name, "docstatus"), 1)
		jv.cancel()

		artech_engine.db.set_value("Company", budget.company, "exception_budget_approver_role", "")
	...
	def test_monthly_budget_crossed_stop2(self):
		...
		project = artech_engine.get_value("Project", {"project_name": "_Test Project"})
		accumulated_limit = get_accumulated_monthly_budget(
			budget.name,
			nowdate(),
		)
	...
	def test_monthly_budget_against_parent_group_cost_center(self):
		...
		if not artech_engine.db.exists("Cost Center", cost_center):
	...
	def test_create_revised_budget(self):
		...
		revised_budget = artech_engine.get_doc("Budget", revised_name)
		...
		old_budget = artech_engine.get_doc("Budget", budget.name)
	...
	def test_duplicate_budget_validation(self):
		...
		new_budget = artech_engine.new_doc("Budget")
...
def set_total_expense_zero(posting_date, budget_against_field=None, budget_against_CC=None):
	if budget_against_field == "project":
		budget_against = artech_engine.db.get_value("Project", {"project_name": "_Test Project"})
	else:
	...
	args.budget_against_doctype = artech_engine.unscrub(budget_against_field)

	if artech_engine.get_cached_value("DocType", args.budget_against_doctype, "is_tree"):
		args.is_tree = True
	else:
		args.is_tree = False
...
def make_budget(**args):
	...
	if budget_against == "Project":
		project = artech_engine.get_value("Project", {"project_name": "_Test Project"})
		budget_list = artech_engine.get_all(
			"Budget",
			filters={
				"project": project,
				"account": "_Test Account Cost for Goods Sold - _TC",
	...
	for name in budget_list:
		doc = artech_engine.get_doc("Budget", name)
		if doc.docstatus == 1:
			doc.cancel()
		artech_engine.delete_doc("Budget", name, force=True, ignore_missing=True)

	budget = artech_engine.new_doc("Budget")

	if budget_against == "Project":
		budget.project = artech_engine.get_value("Project", {"project_name": "_Test Project"})
	else:
