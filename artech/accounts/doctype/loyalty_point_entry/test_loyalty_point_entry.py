...
class TestLoyaltyPointEntry(ArtechTestSuite):
	def setUp(self):
		# Create test records
		self.loyalty_program_name = "Test Single Loyalty"
		self.customer_name = "Test Loyalty Customer"
		customer = artech_engine.get_doc("Customer", self.customer_name)
	...
	def test_add_loyalty_points(self):
		...
		doc = artech_engine.get_last_doc("Loyalty Point Entry")
	...
	def test_add_loyalty_points_with_discretionary_reason(self):
		...
		entry = artech_engine.get_doc("Loyalty Point Entry", doc.name)
