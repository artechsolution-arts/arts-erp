...
from artech.accounts.doctype.journal_entry.journal_entry import StockAccountInvalidTransaction
from artech.exceptions import InvalidAccountCurrency
from artech.selling.doctype.customer.test_customer import make_customer, set_credit_limit
from artech.tests.utils import ArtechTestSuite


class TestJournalEntry(ArtechTestSuite):
	def setUp(self):
	...
	def test_journal_entry_with_against_jv(self):
		jv_invoice = artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][2])
		base_jv = artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][0])
	...
	def test_jv_against_purchase_order(self):
		...
		base_jv = artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][1])
	...
	def jv_against_voucher_testcase(self, base_jv, test_voucher):
		...
		submitted_voucher = artech_engine.get_doc(test_voucher.doctype, test_voucher.name)
	...
	def advance_paid_testcase(self, base_jv, test_voucher, dr_or_cr):
		...
		advance_paid = artech_engine.db.sql(
	...
	def cancel_against_voucher_testcase(self, test_voucher):
		if test_voucher.doctype == "Journal Entry":
			...
			submitted_voucher = artech_engine.get_doc(test_voucher.doctype, test_voucher.name)
			self.assertRaises(artech_engine.LinkExistsError, submitted_voucher.cancel)
	...
	def test_jv_with_project(self):
		...
		if not artech_engine.db.exists("Project", {"project_name": "Journal Entry Project"}):
			...
			project_name = artech_engine.get_value("Project", {"project_name": "_Test Project"})
	...
	def check_gl_entries(self):
		...
		query = artech_engine.qb.from_(gl)
	...
	def test_credit_limit_for_customer(self):
		...
		jv.save()
		self.assertRaises(artech_engine.ValidationError, jv.submit)
