...
artech_engine.provide("artech.journal_entry");

artech_engine.ui.form.on("Journal Entry", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				function () {
					return artech.journal_entry.reverse_journal_entry(frm);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), function () {
				return artech.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		artech.journal_entry.toggle_fields_based_on_currency(frm);
		...
		artech.accounts.unreconcile_payment.add_unreconcile_btn(frm);

		if (frm.doc.voucher_type !== "Exchange Gain Or Loss") {
			$.each(frm.doc.accounts || [], function (i, row) {
				artech.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
			});
		}
	},
			...
			if (payment_entry_references.length > 0) {
				...
				artech_engine.throw(
					__("Rows: {0} have 'Payment Entry' as reference_type. This should not be set manually.", [
						artech_engine.utils.comma_and(rows),
					])
				);
			}
		}
	...
	make_inter_company_journal_entry: function (frm) {
		...
		d.set_primary_action(__("Create"), function () {
			...
			artech_engine.call({
				...
				method: "artech.accounts.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
			...
			callback: function (r) {
				if (r.message) {
					$.each(frm.doc.accounts || [], function (i, jvd) {
						artech_engine.model.set_value(jvd.doctype, jvd.name, "cost_center", r.message.cost_center);
					});
				}
			},
		});
		...
		if (
			...
			if (["Bank Entry", "Cash Entry"].includes(frm.doc.voucher_type)) {
				return artech_engine.call({
					type: "GET",
					method: "artech.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
						account_type:
	...
	from_template: function (frm) {
		if (frm.doc.from_template) {
			artech_engine.db.get_doc("Journal Entry Template", frm.doc.from_template).then((doc) => {
				artech_engine.model.clear_table(frm.doc, "accounts");
				frm.set_value({
					company: doc.company,
...
var update_jv_details = function (doc, r) {
	$.each(r, function (i, d) {
		var row = artech_engine.model.add_child(doc, "Journal Entry Account", "accounts");
		const {
			idx,
			name,
			owner,
		...
		artech_engine.model.set_value(row.doctype, row.name, fields);
	});
	refresh_field("accounts");
};

artech.accounts.JournalEntry = class JournalEntry extends artech_engine.ui.form.Controller {
	onload() {
		this.load_defaults();
		this.setup_queries();
		artech.accounts.dimensions.setup_dimension_filters(this.frm, this.frm.doctype);
	}

	load_defaults() {
		//this.frm.show_print_first = true;
		if (this.frm.doc.__islocal && this.frm.doc.company) {
			artech_engine.model.set_default_values(this.frm.doc);
			$.each(this.frm.doc.accounts || [], function (i, jvd) {
				artech_engine.model.set_default_values(jvd);
			});
			var posting_date = this.frm.doc.posting_date;
			if (!this.frm.doc.amended_from)
				this.frm.set_value("posting_date", posting_date || artech_engine.datetime.get_today());
		}
	}
	...
	setup_queries() {
		...
		me.frm.set_query("account", "accounts", function (doc, cdt, cdn) {
			return artech.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("party_type", "accounts", function (doc, cdt, cdn) {
			...
			return {
				query: "artech.setup.doctype.party_type.party_type.get_party_type",
				filters: {
					account: row.account,
				},
			};
		...
		me.frm.set_query("reference_name", "accounts", function (doc, cdt, cdn) {
			var jvd = artech_engine.get_doc(cdt, cdn);

			// journal entry
			if (jvd.reference_type === "Journal Entry") {
				artech_engine.model.validate_missing(jvd, "account");
				return {
					query: "artech.accounts.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party,
					},
			...
			if (["Sales Order", "Purchase Order"].includes(jvd.reference_type)) {
				...
				artech_engine.model.validate_missing(jvd, "party_type");
				artech_engine.model.validate_missing(jvd, "party");
	...
	reference_name(doc, cdt, cdn) {
		var d = artech_engine.get_doc(cdt, cdn);
	...
	get_outstanding(doctype, docname, company, child) {
		...
		return artech_engine.call({
			method: "artech.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args },
			callback: function (r) {
				if (r.message) {
					$.each(r.message, function (field, value) {
						artech_engine.model.set_value(child.doctype, child.name, field, value);
					});
				}
			},
		});
	...
	accounts_add(doc, cdt, cdn) {
		...
		artech.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "accounts");
	}
};

cur_frm.script_manager.make(artech.accounts.JournalEntry);
...
artech_engine.ui.form.on("Journal Entry Account", {
	party: function (frm, cdt, cdn) {
		var d = artech_engine.get_doc(cdt, cdn);
		if (!d.account && d.party_type && d.party) {
			if (!frm.doc.company) artech_engine.throw(__("Please select Company"));
			return frm.call({
				method: "artech.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
				child: d,
				args: {
					company: frm.doc.company,
	...
	account: function (frm, dt, dn) {
		artech.journal_entry.set_account_details(frm, dt, dn);
	},

	debit_in_account_currency: function (frm, cdt, cdn) {
		artech.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function (frm, cdt, cdn) {
		var company_currency = artech_engine.get_doc(":Company", frm.doc.company).default_currency;
		var row = locals[cdt][cdn];

		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			artech_engine.model.set_value(cdt, cdn, "exchange_rate", 1);
		}

		artech.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
});

artech_engine.ui.form.on("Journal Entry Account", "accounts_remove", function (frm) {
	frm.cscript.update_totals(frm.doc);
});

$.extend(artech.journal_entry, {
	...
	set_exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
	...
	quick_entry: function (frm) {
		...
		var dialog = new artech_engine.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					options: "Account",
					get_query: function () {
						return artech.journal_entry.account_query(frm);
					},
				},
				{
					fieldtype: "Link",
		...
		dialog.set_primary_action(__("Save"), function () {
			...
			artech_engine.model.set_value(debit_row.doctype, debit_row.name, "account", values.debit_account);
			artech_engine.model.set_value(
				debit_row.doctype,
				debit_row.name,
			...
			artech_engine.model.set_value(credit_row.doctype, credit_row.name, "account", values.credit_account);
			artech_engine.model.set_value(
				credit_row.doctype,
				credit_row.name,
	...
	account_query: function (frm) {
		...
		if (!frm.doc.multi_currency) {
			$.extend(filters, {
				account_currency: [
					"in",
					[artech_engine.get_doc(":Company", frm.doc.company).default_currency, null],
				],
			});
		}
	...
	reverse_journal_entry: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: frm,
		});
	},
});
...
$.extend(artech.journal_entry, {
	set_account_details: function (frm, dt, dn) {
		var d = locals[dt][dn];
		if (d.account) {
			if (!frm.doc.company) artech_engine.throw(__("Please select Company first"));
			if (!frm.doc.posting_date) artech_engine.throw(__("Please select Posting Date first"));

			return artech_engine.call({
				method: "artech.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
				args: {
					account: d.account,
				...
				callback: function (r) {
					if (r.message) {
						$.extend(d, r.message);
						artech.journal_entry.set_amount_on_last_row(frm, dt, dn);
						artech.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field("accounts");
					}
				},
			});
		} else {
			artech.journal_entry.clear_fields(frm, dt, dn);
		}
	},
