...
from artech_engine import _, msgprint, scrub
from artech_engine.core.doctype.submission_queue.submission_queue import queue_submission
from artech_engine.model.document import Document
from artech_engine.utils import comma_and, cstr, flt, fmt_money, formatdate, get_link_to_form, nowdate

import artech
from artech.accounts.deferred_revenue import get_deferred_booking_accounts
...
from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
...
from artech.setup.utils import get_exchange_rate as _get_exchange_rate


class StockAccountInvalidTransaction(artech_engine.ValidationError):
	pass
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate(self):
		...
		if not artech_engine.flags.is_reverse_depr_entry:
	...
	def validate_advance_accounts(self):
		...
		advance_accounts.add(
			artech_engine.get_cached_value("Company", self.company, "default_advance_received_account")
		)
		advance_accounts.add(artech_engine.get_cached_value("Company", self.company, "default_advance_paid_account"))
		if advance_accounts_used := journal_accounts & advance_accounts:
			artech_engine.msgprint(
				...
				).format(artech_engine.bold(comma_and(advance_accounts_used)))
			)
	...
	def before_submit(self):
		...
		if not artech_engine.flags.in_import:
	...
	def get_balance_for_periodic_accounting(self):
		...
		for account in stock_accounts:
			...
			if difference_value == 0:
				artech_engine.msgprint(
					_("No difference found for stock account {0}").format(artech_engine.bold(account)),
					alert=True,
				)
				continue
	...
	def validate_company_for_periodic_accounting(self):
		if artech.is_perpetual_inventory_enabled(self.company):
			artech_engine.throw(
				_(
		...
		if not self.periodic_entry_difference_account:
			artech_engine.throw(_("Please select Periodic Accounting Entry Difference Account"))
	...
	def get_stock_accounts_for_periodic_accounting(self):
		...
		if not self.stock_asset_account:
			artech_engine.throw(_("Please select Stock Asset Account"))
	...
	def validate_inter_company_accounts(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			doc = artech_engine.db.get_value(
				"Journal Entry",
				self.inter_company_journal_entry_reference,
				["company", "total_debit", "total_credit"],
				as_dict=True,
			)
			account_currency = artech_engine.get_cached_value("Company", self.company, "default_currency")
			previous_account_currency = artech_engine.get_cached_value("Company", doc.company, "default_currency")
			if account_currency == previous_account_currency:
				...
				if (flt(self.total_credit, credit_precision) != flt(doc.total_debit, debit_precision)) or (
					...
					artech_engine.throw(_("Total Credit/ Debit Amount should be same as linked Journal Entry"))
	...
	def validate_depr_account_and_depr_entry_voucher_type(self):
		for d in self.get("accounts"):
			if d.account_type == "Depreciation":
				...
				if artech_engine.get_cached_value("Account", d.account, "root_type") != "Expense":
					artech_engine.throw(_("Account {0} should be of type Expense").format(d.account))

	def validate_stock_accounts(self):
		if (
			not artech.is_perpetual_inventory_enabled(self.company)
	...
	def update_asset_on_depreciation(self):
		...
		for d in self.get("accounts"):
			if (
				...
				and artech_engine.get_cached_value("Account", d.account, "root_type") == "Expense"
				and d.debit
			):
				asset = artech_engine.get_cached_doc("Asset", d.reference_name)
	...
	def update_journal_entry_link_on_depr_schedule(self, asset, je_row):
		...
		for d in depr_schedule or []:
			if (
				...
				artech_engine.db.set_value("Depreciation Schedule", d.name, "journal_entry", self.name)
		...
		if self.voucher_type == "Asset Disposal":
			...
			for d in self.get("accounts"):
				if (
					...
					asset_doc = artech_engine.get_doc("Asset", d.reference_name)
	...
	def update_invoice_discounting(self):
		...
		for inv_disc in invoice_discounting_list:
			inv_disc_doc = artech_engine.get_doc("Invoice Discounting", inv_disc)
			status = None
	...
	def unlink_advance_entry_reference(self):
		for d in self.get("accounts"):
			if d.is_advance == "Yes" and d.reference_type in ("Sales Invoice", "Purchase Invoice"):
				doc = artech_engine.get_doc(d.reference_type, d.reference_name)
	...
	def unlink_asset_reference(self):
		for d in self.get("accounts"):
			if (
				...
				asset = artech_engine.get_doc("Asset", d.reference_name)
				...
				journal_entry_for_scrap = artech_engine.db.get_value(
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			artech_engine.db.set_value("Journal Entry", self.name, "inter_company_journal_entry_reference", "")

	def has_asset_adjustment_entry(self):
		if self.flags.get("via_asset_value_adjustment"):
			return

		asset_value_adjustment = artech_engine.db.get_value(
		...
		if asset_value_adjustment:
			artech_engine.throw(
				...
				).format(artech_engine.utils.get_link_to_form("Asset Value Adjustment", asset_value_adjustment))
			)

	def unlink_asset_adjustment_entry(self):
		AssetValueAdjustment = artech_engine.qb.DocType("Asset Value Adjustment")
		(
			artech_engine.qb.update(AssetValueAdjustment)
	...
	def validate_party(self):
		for d in self.get("accounts"):
			account_type = artech_engine.get_cached_value("Account", d.account, "account_type")
			...
			if account_type in ["Receivable", "Payable"]:
				if (
					...
					d.party_type
					and artech_engine.db.get_value("Party Type", d.party_type, "account_type") != account_type
					and d.party_type
	...
	def check_credit_limit(self):
		...
		if customers:
			...
			customer_details = artech_engine._dict(
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				...
				if d.is_advance == "Yes":
					if d.party_type == "Customer" and flt(d.debit) > 0:
						artech_engine.throw(_("Row {0}: Advance against Customer must be credit").format(d.idx))
					elif d.party_type == "Supplier" and flt(d.credit) > 0:
						artech_engine.throw(_("Row {0}: Advance against Supplier must be debit").format(d.idx))
	...
	def validate_against_jv(self):
		for d in self.get("accounts"):
			if d.reference_type == "Journal Entry":
				account_root_type = artech_engine.get_cached_value("Account", d.account, "root_type")
				if (
				...
				if d.reference_name == self.name:
					artech_engine.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = artech_engine.db.sql(
	...
	def validate_reference_doc(self):
		...
		for d in self.get("accounts"):
			...
			if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
				...
				against_voucher = artech_engine.db.get_value(
					d.reference_type, d.reference_name, [scrub(dt) for dt in field_dict.get(d.reference_type)]
				)

				if not against_voucher:
					artech_engine.throw(_("Row {0}: Invalid reference {1}").format(d.idx, d.reference_name))
	...
	def validate_orders(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				order = artech_engine.get_doc(reference_type, reference_name)

				if order.docstatus != 1:
					artech_engine.throw(_("{0} {1} is not submitted").format(reference_type, reference_name))

				if flt(order.per_billed) >= 100:
					artech_engine.throw(_("{0} {1} is fully billed").format(reference_type, reference_name))

				if cstr(order.status) == "Closed":
					artech_engine.throw(_("{0} {1} is closed").format(reference_type, reference_name))
	...
	def validate_invoices(self):
		...
		for reference_name, total in self.reference_totals.items():
			...
			if reference_type in ("Sales Invoice", "Purchase Invoice") and self.voucher_type not in [
				...
				invoice = artech_engine.get_doc(reference_type, reference_name)
	...
	def set_against_account(self):
		...
		if self.voucher_type in ("Deferred Revenue", "Deferred Expense"):
			for d in self.get("accounts"):
				...
				d.against_account = artech_engine.db.get_value(d.reference_type, d.reference_name, field)
		else:
			for d in self.get("accounts"):
				if flt(d.debit) > 0:
		...
		if not (self.voucher_type == "Exchange Gain Or Loss" and self.multi_currency):
			for d in self.get("accounts"):
				if not flt(d.debit) and not flt(d.credit):
					artech_engine.throw(_("Row {0}: Both Debit and Credit values cannot be zero").format(d.idx))
	...
	def set_total_debit_credit(self):
		...
		for d in self.get("accounts"):
			if d.debit and d.credit:
				artech_engine.throw(_("You cannot credit and debit same account at the same time"))
	...
	def validate_multi_currency(self):
		...
		if alternate_currency:
			if not self.multi_currency:
				artech_engine.throw(_("Please check Multi Currency option to allow accounts with other currency"))
	...
	def set_exchange_rate(self):
		for d in self.get("accounts"):
			...
			if not d.exchange_rate:
				artech_engine.throw(_("Row {0}: Exchange Rate is mandatory").format(d.idx))
	...
	def create_remarks(self):
		...
		if self.cheque_no:
			if self.cheque_date:
				...
				msgprint(_("Please enter Reference date"), raise_exception=artech_engine.MandatoryError)
	...
	def set_print_format_fields(self):
		...
		for d in self.get("accounts"):
			...
			elif artech_engine.get_cached_value("Account", d.account, "account_type") in ["Bank", "Cash"]:
				bank_amount += flt(d.debit_in_account_currency) or flt(d.credit_in_account_currency)
				bank_account_currency = d.account_currency

		if party_type and pay_to_recd_from:
			self.pay_to_recd_from = artech_engine.db.get_value(
		...
		for d in self.get("accounts"):
			if d.debit or d.credit or (self.voucher_type == "Exchange Gain Or Loss"):
				...
				if account_type in ["Receivable", "Payable"] and self.party_not_required:
					artech_engine.flags.party_not_required = True
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		if gl_map:
			...
			artech_engine.flags.party_not_required = False
			if cancel:
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": artech.get_default_cost_center(self.company),
						},
					)
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			if artech_engine.db.get_value("Stock Entry", self.stock_entry, "docstatus") != 1:
				artech_engine.throw(_("Stock Entry {0} is not submitted").format(self.stock_entry))
	...
	def validate_empty_accounts_table(self):
		if not self.get("accounts"):
			artech_engine.throw(_("Accounts table cannot be blank."))
...
def get_default_bank_cash_account(
	...
	*,
	...
	from artech.accounts.doctype.sales_invoice.sales_invoice import get_bank_cash_account
	...
	if not account:
		...
		if account_type == "Bank":
			account = artech_engine.get_cached_value("Company", company, "default_bank_account")
			if not account:
				account_list = artech_engine.get_all(
			...
			account = artech_engine.get_cached_value("Company", company, "default_cash_account")
			if not account:
				account_list = artech_engine.get_all(
	...
	if account:
		account_details = artech_engine.get_cached_value(
		...
		return artech_engine._dict(result)
	else:
...
def get_payment_entry_against_order(
	...
	ref_doc = artech_engine.get_doc(dt, dn)

	if flt(ref_doc.per_billed, 2) > 0:
		artech_engine.throw(_("Can only make payment against unbilled {0}").format(dt))
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or artech_engine.get_cached_value(
	...
	party_row = je.append(
		"accounts",
		{
			...
			"account_type": artech_engine.get_cached_value("Account", args.get("party_account"), "account_type"),
...
def get_against_jv(
	...
	if not artech_engine.db.has_column("Journal Entry", searchfield):
		return []

	JournalEntry = artech_engine.qb.DocType("Journal Entry")
	JournalEntryAccount = artech_engine.qb.DocType("Journal Entry Account")

	query = (
		artech_engine.qb.from_(JournalEntry)
		...
		.orderby(JournalEntry.name, order=artech_engine.qb.desc)
		.limit(page_len)
		.offset(start)
	)
...
def get_outstanding(args: str | dict):
	if not artech_engine.has_permission("Account"):
		artech_engine.msgprint(_("No Permission"), raise_exception=1)

	if isinstance(args, str):
		args = json.loads(args)

	company_currency = artech.get_company_currency(args.get("company"))
	due_date = None

	if args.get("doctype") == "Journal Entry":
		condition = " and party=%(party)s" if args.get("party") else ""

		against_jv_amount = artech_engine.db.sql(
			f"""
		...
		invoice = artech_engine.db.get_value(
			args["doctype"],
			args["docname"],
...
def get_party_account_and_currency(company: str, party_type: str, party: str):
	...
	return {
		"account": account,
		"account_currency": artech_engine.get_cached_value("Account", account, "account_currency"),
	}
...
def get_account_details_and_party_type(
	...
	company_currency = artech.get_company_currency(company)
	account_details = artech_engine.get_cached_value(
	...
	grid_values = {
		...
		"bank_account": (
			artech_engine.db.get_value("Bank Account", {"account": account, "company": company}) or None
		),
...
def get_exchange_rate(
	...
	if not account_details:
		artech_engine.throw(_("Please select correct account"))
	...
	if account_currency != company_currency:
		if reference_type in ("Sales Invoice", "Purchase Invoice") and reference_name:
			exchange_rate = artech_engine.db.get_value(reference_type, reference_name, "conversion_rate")
...
def make_reverse_journal_entry(source_name: str, target_doc: str | Document | None = None):
	existing_reverse = artech_engine.db.exists("Journal Entry", {"reversal_of": source_name, "docstatus": 1})
	if existing_reverse:
		artech_engine.throw(
