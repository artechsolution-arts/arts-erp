...
from artech_engine.utils import add_days, add_years, flt, getdate, nowdate, today
...
class TestPaymentReconciliation(ArtechTestSuite):
	def setUp(self):
		self.create_company()
		self.create_item()
	...
	def create_company(self):
		company = None
		if artech_engine.db.exists("Company", "_Test Payment Reconciliation"):
			company = artech_engine.get_doc("Company", "_Test Payment Reconciliation")
		else:
		...
		# create bank account
		if artech_engine.db.exists("Account", "HDFC - _PR"):
			self.bank = "HDFC - _PR"
		else:
			bank_acc = artech_engine.get_doc(
				{
					"doctype": "Account",
					"account_name": "HDFC",
	...
	def create_cost_center(self):
		...
		self.main_cc = artech_engine.get_doc("Cost Center", get_default_cost_center(self.company))

		cc_exists = artech_engine.db.get_list("Cost Center", filters={"cost_center_name": cc_name})
		if cc_exists:
			self.sub_cc = artech_engine.get_doc("Cost Center", cc_exists[0].name)
		else:
			sub_cc = artech_engine.new_doc("Cost Center")
	...
	def test_difference_amount_via_journal_entry(self):
		...
		pr.reconcile()
		total_credit_amount = artech_engine.db.get_all(
			"Journal Entry Account",
	...
	def test_no_difference_amount_for_base_currency_accounts(self):
		...
		exc_gain_loss_journals = artech_engine.db.get_all(
			"Journal Entry Account",
	...
	def test_reconciliation_purchase_invoice_against_return(self):
		...
		pi_return = artech_engine.get_doc(pi.as_dict())
		pi_return.name = None
		...
		# Should not raise artech_engine.exceptions.ValidationError: Total Debit must be equal to Total Credit.
		pr.reconcile()
	...
	def test_rounding_of_unallocated_amount(self):
		...
		# Should not raise artech_engine.exceptions.ValidationError: Payment Entry has been modified after you pulled it. Please pull it again.
		pr.reconcile()
	...
	def test_advance_reverse_payment_against_payment_for_supplier(self):
		...
		pl_entries = artech_engine.db.get_all(
			"Payment Ledger Entry",
			filters={"voucher_no": pe.name},
	...
	def test_advance_payment_reconciliation_date_for_older_date(self):
		old_settings = artech_engine.db.get_value(
			"Company",
			self.company,
			[
		...
		artech_engine.db.set_value("Payment Entry Reference", pay.references[1].name, "reconcile_effect_on", None)
		pay.reload()
		pay.cancel()
		...
		artech_engine.db.set_value("Company", self.company, old_settings)
	...
	def test_reconciliation_on_closed_period_payment(self):
		...
		first_fy_start_date = artech_engine.db.get_value(
		...
		je_1 = artech_engine.new_doc("Journal Entry")
	...
	def test_partial_advance_payment_with_closed_fiscal_year(self):
		...
		"""
		default_settings = artech_engine.db.get_value(
			"Company",
			self.company,
			[
				"book_advance_payments_in_separate_party_account",
		...
		artech_engine.db.set_value("Company", self.company, default_settings)
...
def create_fiscal_year(company, year_start_date, year_end_date):
	...
	if not fy_docname:
		...
		fy_doc = artech_engine.get_doc("Fiscal Year", fy_docname)
		if not artech_engine.db.exists("Fiscal Year Company", {"parent": fy_docname, "company": company}):
