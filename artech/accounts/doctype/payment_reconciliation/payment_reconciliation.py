...
from artech_engine.query_builder import Case, Criterion
from artech_engine.query_builder.custom import ConstantColumn
from artech_engine.utils import flt, fmt_money, get_link_to_form, getdate, nowdate, today

import artech
from artech.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from artech.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation import (
	is_any_doc_running,
)
...
from artech.controllers.accounts_controller import get_advance_payment_entries_for_regional
...
class PaymentReconciliation(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.payment_reconciliation_allocation.payment_reconciliation_allocation import (
			PaymentReconciliationAllocation,
		)
		from artech.accounts.doctype.payment_reconciliation_invoice.payment_reconciliation_invoice import (
			PaymentReconciliationInvoice,
		)
		from artech.accounts.doctype.payment_reconciliation_payment.payment_reconciliation_payment import (
	...
	def get_jv_entries(self):
		...
		account_type = artech.get_party_account_type(self.party_type)
	...
	def get_return_invoices(self):
		...
		conditions.append(doc[artech_engine.scrub(self.party_type)] == self.party)
	...
	def get_dr_or_cr_notes(self):
		...
		if artech.get_party_account_type(self.party_type) == "Receivable":
	...
	def get_difference_amount(self, payment_entry, invoice, allocated_amount):
		party_account_defaults = artech_engine.get_cached_value(
			"Account", self.receivable_payable_account, ["account_type", "account_currency"], as_dict=True
		)
		allocated_amount_precision = get_field_precision(
			artech_engine.get_meta("Payment Reconciliation Allocation").get_field("allocated_amount")
		)
		difference_amount_precision = get_field_precision(
			artech_engine.get_meta("Payment Reconciliation Allocation").get_field("difference_amount")
		)
		difference_amount = 0
		if party_account_defaults.get("account_currency") != artech_engine.get_cached_value(
	...
	def is_auto_process_enabled(self):
		return artech_engine.get_single_value("Accounts Settings", "auto_reconcile_payments")
	...
	def allocate_entries(self, args: dict):
		...
		exc_gain_loss_posting_date = artech_engine.db.get_single_value(
			"Accounts Settings", "exchange_gain_loss_posting_date", cache=True
		)
		invoice_exchange_map = self.get_invoice_exchange_map(args.get("invoices"), args.get("payments"))
		default_exchange_gain_loss_account = artech_engine.get_cached_value(
	...
	@artech_engine.whitelist()
	def reconcile(self):
		if artech_engine.get_single_value("Accounts Settings", "auto_reconcile_payments"):
	...
	def get_payment_details(self, row, dr_or_cr):
		payment_details = artech_engine._dict(
			{
	...
	def check_mandatory_to_fetch(self):
		for fieldname in ["company", "party_type", "party", "receivable_payable_account"]:
			if not self.get(fieldname):
				artech_engine.throw(_("Please select {0} first").format(_(self.meta.get_label(fieldname))))

	def validate_entries(self):
		if not self.get("invoices"):
			artech_engine.throw(_("No records found in the Invoices table"))

		if not self.get("payments"):
			artech_engine.throw(_("No records found in the Payments table"))
	...
	def get_invoice_exchange_map(self, invoices, payments):
		...
		invoice_exchange_map = artech_engine._dict()

		if sales_invoices:
			sales_invoice_map = artech_engine._dict(
		...
		if purchase_invoices:
			purchase_invoice_map = artech_engine._dict(
				artech_engine.db.get_all(
					"Purchase Invoice",
		...
		if journals:
			...
			journals_map = artech_engine._dict(
				artech_engine.db.get_all(
	...
	def validate_allocation(self):
		unreconciled_invoices = artech_engine._dict()
		...
		if not invoices_to_reconcile:
			artech_engine.throw(_("No records found in Allocation table"))

	def build_dimensions_filter_conditions(self):
		ple = qb.DocType("Payment Ledger Entry")
		for x in self.dimensions:
			dimension = x.fieldname
			if self.get(dimension) and artech_engine.db.has_column("Payment Ledger Entry", dimension):
...
def reconcile_dr_cr_note(dr_cr_notes, company, active_dimensions=None):
	for inv in dr_cr_notes:
		if (
			abs(artech_engine.db.get_value(inv.voucher_type, inv.voucher_no, "outstanding_amount"))
				...
				"accounts": [
					{
						...
						"cost_center": inv.cost_center or artech.get_default_cost_center(company),
...
def get_queries_for_dimension_filters(company: str | None = None):
	...
	for d in get_dimensions()[0]:
		filters = {}
		meta = artech_engine.get_meta(d.document_type)
