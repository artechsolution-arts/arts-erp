...
artech.accounts.PaymentReconciliationController = class PaymentReconciliationController extends (
	...
	onload() {
		const default_company = artech_engine.defaults.get_default("company");
		...
		this.frm.set_query("receivable_payable_account", () => {
			return {
				filters: {
					...
					is_group: 0,
					account_type: artech_engine.boot.party_account_types[this.frm.doc.party_type],
			...
			this.frm.call({
				...
				callback: (r) => {
					if (r.message) {
						this.frm
							.call({
								method: "artech.accounts.doctype.process_payment_reconciliation.process_payment_reconciliation.is_any_doc_running",
								args: {
									for_filter: {
										company: this.frm.doc.company,
							...
							.then((r) => {
								if (r.message) {
									let doc_link = artech_engine.utils.get_form_link(
	...
	set_query_for_dimension_filters() {
		artech_engine.call({
			method: "artech.accounts.doctype.payment_reconciliation.payment_reconciliation.get_queries_for_dimension_filters",
			args: {
				company: this.frm.doc.company,
			},
			callback: (r) => {
		...
		return this.frm.call({
			...
			callback: () => {
				if (!(this.frm.doc.payments.length || this.frm.doc.invoices.length)) {
					...
					artech_engine.throw({ message: __("No Outstanding Invoices found for this party") });
				} else if (!this.frm.doc.payments.length) {
					artech_engine.throw({ message: __("No Unreconciled Payments found for this party") });
				}
				this.frm.refresh();
			},
		});
...
artech_engine.ui.form.on("Payment Reconciliation Allocation", {
...
extend_cscript(cur_frm.cscript, new artech.accounts.PaymentReconciliationController({ frm: cur_frm }));
