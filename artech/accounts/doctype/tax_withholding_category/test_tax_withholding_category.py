...
class TestTaxWithholdingCategory(ArtechTestSuite):
	def setUp(self):
	...
	def test_tds_withholding_group_different_rates(self):
		...
		for group_name in ["Individual", "Company"]:
			if not artech_engine.db.exists("Tax Withholding Group", group_name):
				artech_engine.get_doc({"doctype": "Tax Withholding Group", "group_name": group_name}).insert()
		...
		if not artech_engine.db.exists("Tax Withholding Category", "TDS Group Rate Category"):
		...
		artech_engine.db.set_value("Supplier", "Test TDS Supplier5", "tax_withholding_group", "Individual")
		...
		artech_engine.db.set_value("Supplier", "Test TDS Supplier6", "tax_withholding_group", "Company")
	...
	def test_tds_calculation_on_net_total_partial_tds(self):
		...
		item_code = artech_engine.db.get_value("Item", {"item_name": "TDS Item"}, "name")
	...
	def set_previous_fy_and_tax_category(self):
		...
		if prev_fiscal_year := get_fiscal_year(add_days(fiscal_year[1], -10)):
			self.prev_fy = artech_engine.get_doc("Fiscal Year", prev_fiscal_year[0])
			...
			self.prev_fy = artech_engine.get_doc(
				{
					"doctype": "Fiscal Year",
	...
	def test_tds_with_multi_currency_invoice(self):
		...
		self.validate_tax_withholding_entries("Purchase Invoice", pi.name, pi_expected)
		self.cleanup_invoices(invoices)
		artech_engine.db.set_value("Supplier", "_Test Supplier USD", "tax_withholding_category", "")
	...
	def test_journal_entry_negative_amount_debit_note(self):
		...
		jv2 = artech_engine.new_doc("Journal Entry")
	...
	def test_tax_withholding_entry_status_determination(self):
		...
		from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import (
			TaxWithholdingEntry,
		)

		# Entry with only taxable fields (Under Withheld)
		entry = artech_engine._dict(
			docstatus=1,
			withholding_name="",
			under_withheld_reason="",
			taxable_name="PI-001",
		...
		# Cancelled entry
		entry = artech_engine._dict(docstatus=2, withholding_name="", under_withheld_reason="", taxable_name="")
	...
	def test_invalid_withholding_amount_validation(self):
		...
		self.assertRaisesRegex(artech_engine.ValidationError, "Withholding Amount.*does not match", pi.save)

	def test_tax_id_is_set_in_all_generated_entries_from_party_doctype(self):
		self.setup_party_with_category("Supplier", "Test TDS Supplier3", "New TDS Category")
		artech_engine.db.set_value("Supplier", "Test TDS Supplier3", "tax_id", "ABCTY1234D")
	...
	def test_threshold_considers_two_parties_with_same_tax_id_with_overrided_hook(self):
		...
		with patch(
			"artech.accounts.doctype.tax_withholding_category.tax_withholding_category.get_tax_id_for_party",
...
def create_purchase_invoice(**args):
	...
	item = artech_engine.db.get_value("Item", {"item_name": "TDS Item"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	item = artech_engine.db.get_value("Item", {"item_name": "TCS Item"}, "name")
...
def create_records():
	...
	for name in [
		...
		if artech_engine.db.exists("Supplier", name):
			continue
	...
	# create item
	if not artech_engine.db.exists("Item", "TDS Item"):
		artech_engine.get_doc(
			{
				"doctype": "Item",
				"item_code": "TDS Item",
	...
	if not artech_engine.db.exists("Item", "TCS Item"):
		artech_engine.get_doc(
			{
				"doctype": "Item",
				"item_code": "TCS Item",
	...
	# create tds account
	if not artech_engine.db.exists("Account", "TDS - _TC"):
		artech_engine.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
	...
	# create tcs account
	if not artech_engine.db.exists("Account", "TCS - _TC"):
		artech_engine.get_doc(
			{
				"doctype": "Account",
				"company": "_Test Company",
...
def create_tax_withholding_category(
	...
	if not artech_engine.db.exists("Tax Withholding Category", category_name):
		...
		doc = artech_engine.get_doc("Tax Withholding Category", category_name)
...
def create_lower_deduction_certificate(
	...
	fiscal_year = get_fiscal_year(today(), company="_Test Company")
	if not artech_engine.db.exists("Lower Deduction Certificate", certificate_no):
		artech_engine.get_doc(
			{
