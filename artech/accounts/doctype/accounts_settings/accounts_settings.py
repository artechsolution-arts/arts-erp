...
class AccountsSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.repost_allowed_types.repost_allowed_types import RepostAllowedTypes
	...
	def validate_and_sync_auto_reconcile_config(self):
		if self.has_value_changed("auto_reconciliation_job_trigger"):
			if (
				...
				artech_engine.throw(_("Cron Interval should be between 1 and 59 Min"))

		if self.has_value_changed("reconciliation_queue_size"):
			if cint(self.reconciliation_queue_size) < 5 or cint(self.reconciliation_queue_size) > 100:
				artech_engine.throw(_("Queue Size should be between 5 and 100"))

	def validate_auto_tax_settings(self):
		if self.add_taxes_from_item_tax_template and self.add_taxes_from_taxes_and_charges_template:
			artech_engine.throw(
				_("You cannot enable both the settings '{0}' and '{1}'.").format(
					artech_engine.bold(_(self.meta.get_label("add_taxes_from_item_tax_template"))),
					artech_engine.bold(_(self.meta.get_label("add_taxes_from_taxes_and_charges_template"))),
				),
	...
	def update_property_for_accounting_dimension(self):
		...
		from artech.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import get_child_docs
	...
	def drop_ar_sql_procedures(self):
		from artech.accounts.report.accounts_receivable.accounts_receivable import InitSQLProceduresForAR

		artech_engine.db.sql(f"drop procedure if exists {InitSQLProceduresForAR.init_procedure_name}")
		artech_engine.db.sql(f"drop procedure if exists {InitSQLProceduresForAR.allocate_procedure_name}")


def toggle_accounting_dimension_sections(hide):
	accounting_dimension_doctypes = artech_engine.get_hooks("accounting_dimension_doctypes")
...
def toggle_subscription_sections(hide):
	subscription_doctypes = artech_engine.get_hooks("subscription_doctypes")
...
def set_allow_on_submit_for_dimension_fields(doctypes):
	for dt in doctypes:
		meta = artech_engine.get_meta(dt)
		for dimension in get_accounting_dimensions():
			df = meta.get_field(dimension)
			if df and not df.allow_on_submit:
				artech_engine.db.set_value("Custom Field", dt + "-" + dimension, "allow_on_submit", 1)
