...
class POSOpeningEntry(StatusUpdater):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.pos_opening_entry_detail.pos_opening_entry_detail import (
	...
	def validate_pos_profile_and_cashier(self):
		if not artech_engine.db.exists("POS Profile", self.pos_profile):
			artech_engine.throw(_("POS Profile {} does not exist.").format(self.pos_profile))

		pos_profile_company, pos_profile_disabled = artech_engine.db.get_value(
			"POS Profile", self.pos_profile, ["company", "disabled"]
		)

		if pos_profile_disabled:
			artech_engine.throw(_("POS Profile {} is disabled.").format(artech_engine.bold(self.pos_profile)))
		...
		if not cint(artech_engine.db.get_value("User", self.user, "enabled")):
			artech_engine.throw(_("User {} is disabled. Please select valid user/cashier").format(self.user))
	...
	def check_user_already_assigned(self):
		if artech_engine.db.exists("POS Opening Entry", {"user": self.user, "status": "Open"}):
			artech_engine.throw(
	...
	def check_poe_is_cancellable(self):
		from artech.accounts.doctype.pos_closing_entry.pos_closing_entry import get_invoices

		invoices = get_invoices(
			self.period_start_date, artech_engine.utils.get_datetime(), self.pos_profile, self.user
		)
