...
class TestPOSOpeningEntry(ArtechTestSuite):
	def setUp(self):
	...
	def test_multiple_pos_opening_entry_for_multiple_pos_profiles(self):
		...
		artech_engine.set_user(cashier_user.name)
	...
	def test_cancel_pos_opening_entry_with_invoice(self):
		...
		self.assertRaises(artech_engine.ValidationError, opening_entry.cancel)


def create_opening_entry(pos_profile, user, get_obj=False):
	entry = artech_engine.new_doc("POS Opening Entry")
	entry.pos_profile = pos_profile.name
	entry.user = user
	entry.company = pos_profile.company
	entry.period_start_date = artech_engine.utils.get_datetime()

	balance_details = []
	for d in pos_profile.payments:
		balance_details.append(artech_engine._dict({"mode_of_payment": d.mode_of_payment}))
