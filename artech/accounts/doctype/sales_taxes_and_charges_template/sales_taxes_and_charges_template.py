...
class SalesTaxesandChargesTemplate(Document):
	...
	def autoname(self):
		if self.company and self.title:
			abbr = artech_engine.get_cached_value("Company", self.company, "abbr")
			self.name = f"{self.title} - {abbr}"

	def set_missing_values(self):
		for data in self.taxes:
			if data.charge_type == "On Net Total" and flt(data.rate) == 0.0:
				data.rate = artech_engine.get_cached_value("Account", data.account_head, "tax_rate")


def valdiate_taxes_and_charges_template(doc):
	# default should not be disabled
	# if not doc.is_default and not artech_engine.get_all(doc.doctype, filters={"is_default": 1}):
	# 	doc.is_default = 1
...
def validate_disabled(doc):
	if doc.is_default and doc.disabled:
		artech_engine.throw(_("Disabled template must not be default template"))
...
def validate_for_tax_category(doc):
	...
	if artech_engine.db.exists(
		...
		artech_engine.throw(
			...
			).format(artech_engine.bold(doc.tax_category))
		)
