...
from artech.assets.doctype.asset.depreciation import get_disposal_account_and_cost_center
...
class SalesInvoiceItem(Document):
	...
	def validate_cost_center(self, company: str):
		cost_center_company = artech_engine.get_cached_value("Cost Center", self.cost_center, "company")
		if cost_center_company != company:
			artech_engine.throw(
				_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
					artech_engine.bold(self.idx), artech_engine.bold(self.cost_center), artech_engine.bold(company)
				)
			)
