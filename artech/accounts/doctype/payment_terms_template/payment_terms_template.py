...
class PaymentTermsTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.payment_terms_template_detail.payment_terms_template_detail import (
	...
	def validate_invoice_portion(self):
		...
		if flt(total_portion, 2) != 100.00:
			artech_engine.msgprint(_("Combined invoice portion must equal 100%"), raise_exception=1, indicator="red")

	def validate_terms(self):
		terms = []
		for term in self.terms:
			if self.allocate_payment_based_on_payment_terms and not term.payment_term:
				artech_engine.throw(_("Row {0}: Payment Term is mandatory").format(term.idx))
