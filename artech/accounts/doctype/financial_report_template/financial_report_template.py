...
from artech.accounts.doctype.account_category.account_category import import_account_categories
from artech.accounts.doctype.financial_report_template.financial_report_validation import TemplateValidator
...
class FinancialReportTemplate(Document):
	...
	def _export_template(self):
		from artech_engine.modules.utils import export_module_json
	...
	def _delete_template(self):
		if not self.module or not artech_engine.conf.developer_mode:
			return

		module_path = artech_engine.get_module_path(self.module)
		dir_path = os.path.join(module_path, "financial_report_template", artech_engine.scrub(self.name))
	...
	def _export_account_categories(self):
		...
		if not self.module or not artech_engine.conf.developer_mode or artech_engine.flags.in_import:
			return
		...
		if category_names:
			db_categories = artech_engine.get_all(
				"Account Category",
...
def sync_financial_report_templates(chart_of_accounts=None, existing_company=None):
	from artech.accounts.doctype.account.chart_of_accounts.chart_of_accounts import get_chart
	...
	# Allow regional templates to completely override Artech
	...
	installed_apps = artech_engine.get_installed_apps()

	for app in installed_apps:
		if disable_default_financial_report_template and app == "artech":
			continue
...
def _sync_templates_for(app_name):
	...
	for module_name in artech_engine.local.app_modules.get(app_name) or []:
		module_path = artech_engine.get_module_path(module_name)
	...
	artech_engine.flags.in_import = True

	for template_path in templates:
		with open(template_path) as f:
			template_data = artech_engine._dict(artech_engine.parse_json(f.read()))

		template_name = template_data.get("name")

		if not artech_engine.db.exists("Financial Report Template", template_name):
			doc = artech_engine.get_doc(template_data)
