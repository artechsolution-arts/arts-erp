...
from artech_engine.database.operator_map import OPERATOR_MAP
from artech_engine.query_builder import Case
from artech_engine.query_builder.functions import Sum
from artech_engine.utils import cstr, date_diff, flt, getdate
from artech_engine.utils.xlsxutils import XLSXMetadata, XLSXStyleBuilder
...
from artech.accounts.doctype.financial_report_row.financial_report_row import FinancialReportRow
from artech.accounts.doctype.financial_report_template.financial_report_template import (
	FinancialReportTemplate,
)
from artech.accounts.doctype.financial_report_template.financial_report_validation import (
	AccountFilterValidator,
	CalculationFormulaValidator,
	DependencyValidator,
)
from artech.accounts.report.financial_statements import (
	get_columns,
	get_cost_centers_with_children,
	get_period_list,
)
from artech.accounts.utils import get_children, get_currency_precision
	...
	def _validate_filters(self, filters: dict[str, Any]) -> None:
		...
		for filter_key in required_filters:
			if not filters.get(filter_key):
				artech_engine.throw(_("Missing required filter: {0}").format(filter_key))

		if filters.get("presentation_currency"):
			artech_engine.msgprint(_("Currency filters are currently unsupported in Custom Financial Report."))
		...
		if (view := filters.get("selected_view")) and view not in supported_views:
			artech_engine.msgprint(_("{0} view is currently unsupported in Custom Financial Report.").format(view))

	def _initialize_context(self, filters: dict[str, Any]) -> ReportContext:
		template_name = filters.get("report_template")
		template = artech_engine.get_doc("Financial Report Template", template_name)

		if not template:
			artech_engine.throw(_("Financial Report Template {0} not found").format(template_name))

		if template.disabled:
			artech_engine.throw(_("Financial Report Template {0} is disabled").format(template_name))
...
class DataCollector:
	...
	def __init__(self, filters: dict[str, Any], periods: list[dict]):
		...
		self.account_fields = {field.fieldname for field in artech_engine.get_meta("Account").fields}
...
class FinancialQueryBuilder:
	...
	def _get_opening_balances(self, accounts: list[str]) -> dict[str, dict[str, dict[str, float]]]:
		...
		"""
		if artech_engine.get_single_value("Accounts Settings", "ignore_account_closing_balance"):
			return self._get_opening_balances_from_gl(accounts)

		first_period_start = getdate(self.periods[0]["from_date"])
		last_closing_voucher = artech_engine.db.get_all(
	...
	def _get_closing_balances(self, account_names: list[str], closing_voucher: str) -> dict[str, float]:
		...
		acb_table = artech_engine.qb.DocType("Account Closing Balance")

		query = (
			artech_engine.qb.from_(acb_table)
			.select(
				acb_table.account,
	...
	def _get_gap_movements(self, account_names: list[str], from_date: str, to_date: str) -> dict[str, float]:
		gl_table = artech_engine.qb.DocType("GL Entry")

		query = (
			artech_engine.qb.from_(gl_table)
	...
	def _get_gl_movements(self, account_names: list[str]) -> list[dict]:
		...
		if not artech_engine.get_single_value("Accounts Settings", "ignore_is_opening_check_for_reporting"):
	...
	def _apply_standard_filters(self, query, table, doctype: str = "GL Entry"):
		...
		if self.filters.get("include_default_book_entries"):
			default_book = artech_engine.get_cached_value("Company", self.filters.company, "default_finance_book")
		...
		for dimension in dimensions:
			if self.filters.get(dimension.fieldname):
				if artech_engine.get_cached_value("DocType", dimension.document_type, "is_tree"):
...
class FilterExpressionParser:
	...
	def build_condition(self, report_row, table):
		...
		if not errors.is_valid:
			...
			artech_engine.log_error(f"Filter validation errors found:\n{'<br><br>'.join(error_messages)}")
			return None
		...
		try:
			...
			artech_engine.log_error(f"Invalid filter formula syntax: {filter_formula} - {e}")
			return None
		except Exception as e:
			artech_engine.log_error(f"Failed to build condition from formula: {filter_formula} - {e}")
			return None
...
class FormulaFieldUpdater:
	...
	def update_in_rows(self, rows: list) -> dict[str, dict[str, str]]:
		...
		if updated_rows:
			artech_engine.db.bulk_update("Financial Report Row", updated_rows, update_modified=False)
...
def get_filtered_accounts(company: str, account_rows: str | list):
	artech_engine.has_permission("Financial Report Template", ptype="read", throw=True)

	if isinstance(account_rows, str):
		account_rows = json.loads(account_rows, object_hook=artech_engine._dict)
...
def get_children_accounts(
	...
	```
	"""
	artech_engine.has_permission(doctype, ptype="read", throw=True)
	...
	if isinstance(filtered_accounts, str):
		filtered_accounts = artech_engine.parse_json(filtered_accounts)
...
class RowProcessor:
	...
	def _process_api_row(self, row) -> RowData:
		...
		try:
			values = artech_engine.call(api_path, filters=self.context.filters, periods=self.period_list, row=row)
			...
			artech_engine.log_error(f"Custom API Error: {api_path} - {e!s}")
...
class FormulaCalculator:
	...
	def evaluate_formula(self, report_row: dict[str, Any]) -> list[float]:
		...
		if validation_result.issues:
			...
			artech_engine.log_error(f"Formula validation errors found:\n{messages}")
	...
	def _evaluate_for_period(self, formula: str, period_index: int, negation_factor: int) -> float:
		...
		try:
			...
			result = artech_engine.safe_eval(formula, context)
			return flt(result * negation_factor, self.precision)

		except ZeroDivisionError:
			artech_engine.log_error(f"Division by zero in formula: {formula}")
			return 0.0
		except Exception as e:
			artech_engine.log_error(f"Formula evaluation error: {formula} - {e!s}")
			return 0.0
		...
		for row_data in rows:
			if row_data.row.data_source == "Column Break":
				...
				if section_header and current_rows:
					...
					section_header = RowData(row=artech_engine._dict({"data_source": "Blank Line"}))
	...
	for row_idx, row in metadata.row_map.items():
		...
		for col_idx, col in metadata.column_map.items():
			...
			if (
				...
				and cell_fieldtype in artech_engine.model.numeric_fieldtypes
