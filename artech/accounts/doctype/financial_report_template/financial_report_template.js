artech_engine.ui.form.on("Financial Report Template", {
	refresh(frm) {
		...
		frm.add_custom_button(__("View Report"), function () {
			artech_engine.set_route("query-report", frm.doc.report_type, {
	...
	after_save(frm) {
		if (!frm.doc.rows || frm.doc.rows.length === 0) {
			artech_engine.msgprint(__("At least one row is required for a financial report template"));
		}
	},
});

artech_engine.ui.form.on("Financial Report Row", {
...
function set_up_filters_editor(frm, cdt, cdn) {
	...
	artech_engine.model.with_doctype(ACCOUNT, () => {
		const filter_group = new artech_engine.ui.FilterGroup({
			parent: wrapper,
			doctype: ACCOUNT,
			on_change: () => {
				...
				artech_engine.model.set_value(cdt, cdn, "calculation_formula", JSON.stringify(current));
			},
		});
...
function show_accounts_tree(template_rows, has_selection) {
	...
	if (account_rows.length === 0) {
		artech_engine.show_alert(__("No <strong>Account Data</strong> row found"));
		return;
	}
...
async function refresh_tree_view(dialog, account_rows) {
	...
	const { message: filtered_accounts } = await artech_engine.call({
		method: "artech.accounts.doctype.financial_report_template.financial_report_engine.get_filtered_accounts",
	...
	const tree = new FilteredTree({
		...
		label: company,
		root_value: company,
		method: "artech.accounts.doctype.financial_report_template.financial_report_engine.get_children_accounts",
...
class FilteredTree extends artech_engine.ui.Tree {
	...
	if (data_source === "Account Data") {
		...
		description_html = `
			<div ${container_style}>
				...
				<ul ${list_style}>
					<li><code>artech.custom.financial_apis.get_custom_revenue</code></li>
