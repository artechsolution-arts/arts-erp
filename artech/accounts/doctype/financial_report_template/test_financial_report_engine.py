...
from artech.accounts.doctype.financial_report_template.test_financial_report_template import (
	FinancialReportTemplateTestCase,
)
from artech.accounts.doctype.journal_entry.test_journal_entry import make_journal_entry
from artech.accounts.utils import get_currency_precision, get_fiscal_year
...
class TestFilterExpressionParser(FinancialReportTemplateTestCase):
	...
	def test_build_condition_accepts_document_instance(self):
		...
		row_doc = artech_engine.get_doc(row_obj)
		cond = parser.build_condition(row_doc, account_table)
		self.assertIsNotNone(cond)

		# Also accepts plain artech_engine._dict object
...
class TestFinancialQueryBuilder(FinancialReportTemplateTestCase):
	def test_fetch_balances_with_journal_entries(self):
		...
		try:
			...
			accounts = [
				artech_engine._dict({"name": cash_account, "account_name": "Cash", "account_number": "1001"}),
				artech_engine._dict({"name": bank_account, "account_name": "Bank", "account_number": "1002"}),
			]
	...
	def test_opening_balance_from_previous_period_closing(self):
		...
		pcv = None
		jv_2024 = None
		original_pcv_setting = artech_engine.db.get_single_value(
		...
		try:
			...
			closing_account = artech_engine.db.get_value(
				"Account",
				{
					"company": company,
					"root_type": "Liability",
