...
class ValidationResult:
	...
	def notify_user(self) -> None:
		...
		if warnings:
			artech_engine.msgprint(warnings, title=_("Warnings"), indicator="orange")

		if errors:
			artech_engine.throw(errors, title=_("Errors"))
...
class CalculationFormulaValidator(Validator):
	...
	def _test_formula_evaluation(self, formula: str, available_codes: list[str]) -> str | None:
		try:
			...
			result = artech_engine.safe_eval(formula, eval_globals=None, eval_locals=context)
...
class AccountFilterValidator(Validator):
	...
	def __init__(self, account_fields: set | None = None):
		self.account_meta = artech_engine.get_meta("Account")
...
class FormulaValidator(Validator):
	...
	def _validate_custom_api(self, row) -> ValidationResult:
		...
		try:
			...
			module = artech_engine.get_module(module_path)
