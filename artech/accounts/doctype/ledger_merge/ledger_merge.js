...
artech_engine.ui.form.on("Ledger Merge", {
	setup: function (frm) {
		artech_engine.realtime.on("ledger_merge_refresh", ({ ledger_merge }) => {
			if (ledger_merge !== frm.doc.name) return;
			artech_engine.model.clear_doc(frm.doc.doctype, frm.doc.name);
			artech_engine.model.with_doc(frm.doc.doctype, frm.doc.name).then(() => {
				frm.refresh();
			});
		});

		artech_engine.realtime.on("ledger_merge_progress", (data) => {
		...
		frm.set_query("account", function (doc) {
			if (!doc.company) artech_engine.throw(__("Please set Company"));
			if (!doc.root_type) artech_engine.throw(__("Please set Root Type"));
			return {
				filters: {
		...
		frm.set_query("account", "merge_accounts", function (doc) {
			if (!doc.company) artech_engine.throw(__("Please set Company"));
			if (!doc.root_type) artech_engine.throw(__("Please set Root Type"));
			if (!doc.account) artech_engine.throw(__("Please set Account"));
			let acc = [doc.account];
...
artech_engine.ui.form.on("Ledger Merge Accounts", {
