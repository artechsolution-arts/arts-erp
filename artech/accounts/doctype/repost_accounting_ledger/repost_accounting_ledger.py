...
from artech_engine.desk.form.linked_with import get_child_tables_of_doctypes
...
class RepostAccountingLedger(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.repost_accounting_ledger_items.repost_accounting_ledger_items import (
		...
		if self.vouchers:
			...
			for vtype in self._allowed_types:
				if names := [x.voucher_no for x in self.vouchers if x.voucher_type == vtype]:
					latest_voucher = artech_engine.db.get_all(
						vtype,
					...
					if latest_voucher and latest_pcv[0] >= latest_voucher:
						artech_engine.throw(_("Cannot Resubmit Ledger entries for vouchers in Closed fiscal year."))
	...
	def get_existing_ledger_entries(self):
		...
		self.gles = artech_engine._dict({})

		for gle in existing_gles:
			self.gles.setdefault((gle.voucher_type, gle.voucher_no), artech_engine._dict({})).setdefault(
				"existing", []
			).append(gle.update({"old": True}))

	def generate_preview_data(self):
		artech_engine.flags.through_repost_accounting_ledger = True
		self.gl_entries = []
		self.get_existing_ledger_entries()
		for x in self.vouchers:
			doc = artech_engine.get_doc(x.voucher_type, x.voucher_no)
	...
	def generate_preview(self):
		...
		if not self.vouchers:
			artech_engine.msgprint(_("Add vouchers to generate preview."))
			return
		...
		rendered_page = artech_engine.render_template(
			"artech/accounts/doctype/repost_accounting_ledger/repost_accounting_ledger.html",
	...
	def on_submit(self):
		if len(self.vouchers) > 5:
			...
			artech_engine.enqueue(
				method="artech.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger.start_repost",
				account_repost_doc=self.name,
				is_async=True,
				job_name=job_name,
				enqueue_after_commit=True,
			)
			artech_engine.msgprint(_("Repost has started in the background"))
		else:
...
def start_repost(account_repost_doc: str | None = None) -> None:
	...
	if account_repost_doc:
		repost_doc = artech_engine.get_doc("Repost Accounting Ledger", account_repost_doc)
		...
		if repost_doc.docstatus == 1:
			...
			for x in repost_doc.vouchers:
				...
				if doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
					...
					doc.make_gl_entries()
				elif doc.doctype in artech_engine.get_hooks("repost_allowed_doctypes"):
...
def validate_docs_for_deferred_accounting(sales_docs, purchase_docs):
	docs_with_deferred_revenue = artech_engine.db.get_all(
		"Sales Invoice Item",
	...
	docs_with_deferred_expense = artech_engine.db.get_all(
	...
	if disallowed_types := voucher_types.difference(allowed_types):
		...
		artech_engine.throw(
			_(
				...
				artech_engine.bold(comma_and(list(disallowed_types))),
				message,
				artech_engine.bold("Allowed Doctype"),
				artech_engine.utils.get_link_to_form("Accounts Settings"),
			)
		)
...
def get_repost_allowed_types(
	...
	if allowed_types := artech_engine.db.get_all(
		"Repost Allowed Types",
		filters=filters,
		fields=["document_type"],
		as_list=1,
