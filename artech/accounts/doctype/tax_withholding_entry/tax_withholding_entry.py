...
import artech
from artech.accounts.utils import get_advance_payment_doctypes
...
class TaxWithholdingEntry(Document):
	...
	def _adjust_against_old_entries(self, field_type: str) -> set:
		...
		# old entries
		old_entries = artech_engine.get_all(
			DOCTYPE,
			filters={
		...
		for old_entry_data in old_entries:
			old_entry = artech_engine.get_doc(DOCTYPE, **old_entry_data)
			...
			if old_amount <= amount_we_can_match:
				# complete adjustment
				artech_engine.db.set_value(DOCTYPE, old_entry.name, values_to_update)
				...
				artech_engine.db.set_value(DOCTYPE, old_entry.name, balance_values)
				...
				new_entry = artech_engine.copy_doc(old_entry)
	...
	def _handle_return_invoice_cancellation(self, filters):
		...
		for entry in old_entries:
			...
			new_entry = artech_engine.copy_doc(artech_engine.get_doc(DOCTYPE, **entry))
			values_to_update = {
...
from artech.accounts.doctype.tax_withholding_category.tax_withholding_category import (
...
class TaxWithholdingController:
	...
	def _check_historical_threshold_status(self, category):
		entry = artech_engine.qb.DocType(DOCTYPE)
		result = artech_engine._dict(
	...
	def _get_linked_payments(self):
		references = artech_engine._dict()
	...
	def _create_tax_row(self, account_head, tax_amount):
		cost_center = self.doc.cost_center or artech.get_default_cost_center(self.doc.company)
...
class PaymentTaxWithholding(TaxWithholdingController):
	...
	def _get_open_entries_for_category(self, category):
		...
		current_entry = artech_engine._dict(
			{
				**self._create_default_entry(category),
				"taxable_amount": category.taxable_amount,
				"taxable_doctype": "",
...
class JournalTaxWithholding(TaxWithholdingController):
	...
	def _calculate_net_total(self):
		from artech.accounts.report.general_ledger.general_ledger import get_account_type_map
	...
	def update_tax_rows(self):
		...
		if self.has_multiple_parties:
			artech_engine.throw(_("Cannot apply TDS against multiple parties in one entry"))
	...
	def _create_or_update_tds_row(self, account_head, tax_amount):
		...
		company_currency = artech_engine.get_cached_value("Company", self.doc.company, "default_currency")
		...
		if not tax_row:
			tax_row = self.doc.append(
				"accounts",
				{
					...
					or artech.get_default_cost_center(self.doc.company),
					"credit": 0,
					"credit_in_account_currency": 0,
					"debit": 0,
					"debit_in_account_currency": 0,
...
def _reset_idx(docs_to_reset_idx):
	...
	if updates:
		artech_engine.db.bulk_update(DOCTYPE, updates, update_modified=False)
