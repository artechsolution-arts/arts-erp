...
class CurrencyExchangeSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.currency_exchange_settings_details.currency_exchange_settings_details import (
			CurrencyExchangeSettingsDetails,
		)
		from artech.accounts.doctype.currency_exchange_settings_result.currency_exchange_settings_result import (
	...
	def validate(self):
		...
		if artech_engine.in_test or artech_engine.flags.in_install or artech_engine.flags.in_setup_wizard:
			return
			...
			if not self.access_key:
				artech_engine.throw(
					_("Access Key is required for Service Provider: {0}").format(
						artech_engine.bold(self.service_provider)
					)
				)
	...
	def validate_parameters(self):
		...
		try:
			...
			artech_engine.throw("Error: " + str(e))
	...
	def validate_result(self, response, value):
		try:
			...
			artech_engine.throw(_("Invalid result key. Response:") + " " + response.text)
		if not isinstance(value, int | float):
			artech_engine.throw(_("Returned exchange rate is neither integer not float."))
