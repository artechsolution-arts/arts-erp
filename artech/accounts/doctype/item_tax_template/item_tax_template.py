...
class ItemTaxTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.item_tax_template_detail.item_tax_template_detail import (
	...
	def validate_tax_accounts(self):
		...
		for d in self.get("taxes"):
			if d.tax_type:
				account_type, account_company = artech_engine.get_cached_value(
				...
				if account_company != self.company:
					artech_engine.throw(
						_("Item Tax Row {0}: Account must belong to Company - {1}").format(
							d.idx, artech_engine.bold(self.company)
						)
					)
				...
				if account_type not in [
					...
					if d.tax_type in check_list:
						artech_engine.throw(_("{0} entered twice in Item Tax").format(d.tax_type))
					else:
