artech_engine.ui.form.on("Bank Statement Import", {
	...
	setup(frm) {
		artech_engine.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			artech_engine.model.clear_doc("Bank Statement Import", frm.doc.name);
			artech_engine.model.with_doc("Bank Statement Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		artech_engine.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", artech_engine.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				artech_engine.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = artech_engine.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		...
		artech_engine.call({
			method: "artech.accounts.doctype.bank_statement_import.bank_statement_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button(__("Report Error"), () => {
							...
							artech_engine.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template() {
		let method = "/api/method/artech_engine.core.doctype.data_import.data_import.download_template";
	...
	toggle_submit_after_import(frm) {
		...
		if (doctype) {
			artech_engine.model.with_doctype(doctype, () => {
				let meta = artech_engine.get_meta(doctype);
	...
	// method: 'artech_engine.core.doctype.data_import.data_import.get_preview_from_template',
	...
	show_import_preview(frm, preview_data) {
		...
		artech_engine.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new artech_engine.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/artech.accounts.doctype.bank_statement_import.bank_statement_import.download_errored_template",
			{
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/artech.accounts.doctype.bank_statement_import.bank_statement_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		artech_engine.call({
			method: "artech.accounts.doctype.bank_statement_import.bank_statement_import.get_import_logs",
			args: {
				docname: frm.doc.name,
			},
			callback: function (r) {
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${artech_engine.utils.get_form_link(
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = artech_engine.dom.get_unique_id();
							html = `${messages}
