...
from artech_engine.core.doctype.data_import.data_import import DataImport
from artech_engine.core.doctype.data_import.importer import Importer, ImportFile
from artech_engine.utils.background_jobs import enqueue
from artech_engine.utils.file_manager import get_file, save_file
from artech_engine.utils.xlsxutils import ILLEGAL_CHARACTERS_RE, handle_html
...
class BankStatementImport(DataImport):
	...
	def validate(self):
		...
		if (
			...
			column_to_field_map = {}
			bank = artech_engine.get_doc("Bank", self.bank)
	...
	def start_import(self):
		preview = artech_engine.get_doc("Bank Statement Import", self.name).get_preview_from_template(
			self.import_file, self.google_sheets_url
		)

		if "Bank Account" not in json.dumps(preview["columns"]):
			artech_engine.throw(_("Please add the Bank Account column"))
		...
		run_now = artech_engine.in_test or artech_engine.conf.developer_mode
...
def convert_mt940_to_csv(data_import: str, mt940_file_path: str):
	doc = artech_engine.get_doc("Bank Statement Import", data_import)

	_file_doc, content = get_file(mt940_file_path)

	is_mt940 = is_mt940_format(content)
	if not is_mt940:
		artech_engine.throw(_("The uploaded file does not appear to be in valid MT940 format."))

	if is_mt940 and not doc.import_mt940_fromat:
		artech_engine.throw(_("MT940 file detected. Please enable 'Import MT940 Format' to proceed."))
	...
	try:
		...
		artech_engine.throw(_("Failed to parse MT940 format. Error: {0}").format(str(e)))

	if not transactions:
		artech_engine.throw(_("Parsed file is not in valid MT940 format or contains no transactions."))
	...
	filename = f"{artech_engine.utils.now_datetime().strftime('%Y%m%d%H%M%S')}_converted_mt940.csv"
...
def get_preview_from_template(
	...
	return artech_engine.get_doc("Bank Statement Import", data_import).get_preview_from_template(
...
def form_start_import(data_import: str):
	job_id = artech_engine.get_doc("Bank Statement Import", data_import).start_import()
	return job_id is not None


@artech_engine.whitelist()
def download_errored_template(data_import_name: str):
	data_import = artech_engine.get_doc("Bank Statement Import", data_import_name)
	data_import.export_errored_rows()


@artech_engine.whitelist()
def download_import_log(data_import_name: str):
	return artech_engine.get_doc("Bank Statement Import", data_import_name).download_import_log()
...
def start_import(data_import, bank_account, import_file_path, google_sheets_url, bank, template_options):
	...
	data_import = artech_engine.get_doc("Bank Statement Import", data_import)
	...
	try:
		...
		artech_engine.flags.in_import = False

	artech_engine.publish_realtime("data_import_refresh", {"data_import": data_import.name})


def update_mapping_db(bank, template_options):
	"""Update bank transaction mapping database with template options."""
	bank = artech_engine.get_doc("Bank", bank)
...
def get_import_status(docname: str):
	...
	data_import = artech_engine.get_doc("Bank Statement Import", docname)
	import_status["status"] = data_import.status

	logs = artech_engine.get_all(
		"Data Import Log",
...
def get_import_logs(docname: str):
	artech_engine.has_permission("Bank Statement Import", throw=True)
...
def upload_bank_statement(**args):
	...
	bsi = artech_engine.new_doc("Bank Statement Import")
