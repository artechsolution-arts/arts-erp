...
from artech.accounts.utils import run_ledger_health_checks
from artech.tests.utils import ArtechTestSuite


class TestLedgerHealth(ArtechTestSuite, AccountsTestMixin):
	def setUp(self):
		self.create_company()
	...
	def configure_monitoring_tool(self):
		monitor_settings = artech_engine.get_doc("Ledger Health Monitor")
	...
	def test_debit_credit_mismatch(self):
		...
		gle = artech_engine.db.get_all(
			"GL Entry", filters={"voucher_no": self.je.name, "account": self.income_account}
		)[0]
		artech_engine.db.set_value("GL Entry", gle.name, "credit", 8000)
	...
	def test_gl_and_pl_mismatch(self):
		...
		ple = artech_engine.db.get_all("Payment Ledger Entry", filters={"voucher_no": self.je.name})[0]
		artech_engine.db.set_value("Payment Ledger Entry", ple.name, "amount", 11000)
