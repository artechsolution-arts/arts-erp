...
from artech_engine.utils import date_diff, flt, get_first_day, get_last_day, getdate

from artech.utilities.product import get_price
...
class SubscriptionPlan(Document):
	...
	def validate_interval_count(self):
		if self.billing_interval_count < 1:
			artech_engine.throw(_("Billing Interval Count cannot be less than 1"))
...
def get_plan_rate(
	...
	plan = artech_engine.get_doc("Subscription Plan", plan)
	if plan.price_determination == "Fixed Rate":
		return plan.cost * prorate_factor

	elif plan.price_determination == "Based On Price List":
		if customer:
			customer_group = artech_engine.db.get_value("Customer", customer, "customer_group")
		else:
			customer_group = None
		...
		prorate = artech_engine.db.get_single_value("Subscription Settings", "prorate")
