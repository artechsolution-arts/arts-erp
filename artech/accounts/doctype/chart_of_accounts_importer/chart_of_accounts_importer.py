...
from artech_engine.desk.form.linked_with import get_linked_fields
from artech_engine.model.document import Document
from artech_engine.utils import cint, cstr
from artech_engine.utils.csvutils import UnicodeWriter
from artech_engine.utils.xlsxutils import (
...
def validate_columns(data):
	if not data:
		artech_engine.throw(_("No data found. Seems like you uploaded a blank file"))
...
def validate_company(company: str):
	parent_company, allow_account_creation_against_child_company = artech_engine.get_cached_value(
		"Company", company, ["parent_company", "allow_account_creation_against_child_company"]
	)

	if parent_company and (not allow_account_creation_against_child_company):
		msg = _("{} is a child company.").format(artech_engine.bold(company)) + " "
		msg += _("Please import accounts against parent company or enable {} in company master.").format(
			artech_engine.bold(_("Allow Account Creation Against Child Company"))
		)
		artech_engine.throw(msg, title=_("Wrong Company"))

	if artech_engine.db.get_all("GL Entry", {"company": company}, "name", limit=1):
		return False
...
def get_file(file_name):
	file_doc = artech_engine.get_doc("File", {"file_url": file_name})
...
def generate_data_from_csv(file_doc, as_dict=False):
	...
	with open(file_path) as in_file:
		...
		for row in csv_reader:
			if as_dict:
				data.append({artech_engine.scrub(header): row[index] for index, header in enumerate(headers)})
			else:
...
def build_forest(data):
				...
				if not parent_account_list and parent_account:
					artech_engine.throw(
						_("The parent account {0} does not exists in the uploaded template").format(
							artech_engine.bold(parent_account)
						)
					)
	...
	if error_messages:
		artech_engine.throw("<br>".join(error_messages))
...
def build_response_as_excel(writer):
	filename = artech_engine.generate_hash("", 10)
	with open(filename, "wb") as f:
		f.write(cstr(writer.getvalue()).encode("utf-8"))
	f = open(filename)
	reader = csv.reader(f)

	from artech_engine.utils.xlsxutils import make_xlsx
	...
	artech_engine.response["filename"] = "coa_importer_template.xlsx"
	artech_engine.response["filecontent"] = xlsx_file.getvalue()
	artech_engine.response["type"] = "binary"
...
def download_template(file_type: str, template_type: str, company: str):
	...
	if file_type == "CSV":
		# download csv file
		artech_engine.response["result"] = cstr(writer.getvalue())
		artech_engine.response["type"] = "csv"
		artech_engine.response["doctype"] = "Chart of Accounts Importer"
	else:
...
def get_sample_template(writer, company):
	currency = artech_engine.db.get_value("Company", company, "default_currency")
...
def validate_accounts(file_doc: Document, extension: str):
	...
	for account in accounts:
		...
		if "parent_account" not in account:
			...
			artech_engine.throw(msg, title=_("Parent Account Missing"))
...
def validate_missing_roots(roots):
	...
	if missing:
		artech_engine.throw(_("Please add Root Account for - {0}").format(" , ".join(missing)))
...
def unset_existing_data(company):
	...
	artech_engine.db.set_value("Company", company, update_values, update_values)
	...
	for doctype in [
		...
		artech_engine.qb.from_(dt).where(dt.company == company).delete().run()
...
def set_default_accounts(company):
	...
	company.update(
		{
			"default_receivable_account": artech_engine.db.get_value(
				"Account", {"company": company.name, "account_type": "Receivable", "is_group": 0}
			),
			"default_payable_account": artech_engine.db.get_value(
				"Account", {"company": company.name, "account_type": "Payable", "is_group": 0}
			),
			"default_provisional_account": artech_engine.db.get_value(
				"Account",
