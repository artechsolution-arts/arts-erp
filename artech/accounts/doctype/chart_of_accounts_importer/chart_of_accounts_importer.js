artech_engine.ui.form.on("Chart of Accounts Importer", {
	onload: function (frm) {
		...
		var d = new artech_engine.ui.Dialog({
			...
			primary_action: function () {
				...
				if (!data.template_type) {
					artech_engine.throw(__("Please select <b>Template Type</b> to download template"));
				}

				open_url_post(
					"/api/method/artech.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.download_template",
					{
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.validate_company",
				args: {
	...
	frm.page
		.set_primary_action(__("Import"), function () {
			return artech_engine.call({
				method: "artech.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.import_coa",
				args: {
...
var validate_coa = function (frm) {
	if (frm.doc.import_file) {
		...
		return artech_engine.call({
			method: "artech.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer.get_coa",
			args: {
				file_name: frm.doc.import_file,
				parent: parent,
...
var generate_tree_preview = function (frm) {
	...
	return new artech_engine.ui.Tree({
