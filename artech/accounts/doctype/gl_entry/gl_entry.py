...
from artech_engine.model.naming import set_name_from_naming_options
from artech_engine.utils import create_batch, flt, fmt_money, now
...
from artech.accounts.utils import OUTSTANDING_DOCTYPES, get_account_currency, get_fiscal_year
from artech.exceptions import InvalidAccountCurrency, ReportingCurrencyExchangeNotFoundError
...
class GLEntry(Document):
	...
	def autoname(self):
		...
		"""
		self.name = artech_engine.generate_hash(txt="", length=10)
	...
	def on_update(self):
		...
		if not self.flags.from_repost and self.voucher_type != "Period Closing Voucher":
			...
			if (
				...
				and artech_engine.get_cached_value("Journal Entry", self.voucher_no, "voucher_type")
				== "Exchange Gain Or Loss"
			):
				return

			if artech_engine.get_cached_value("Account", self.account, "account_type") not in [
				"Receivable",
				"Payable",
			]:
				...
				if (
					...
					and not artech_engine.flags.is_reverse_depr_entry
				):
	...
	def check_mandatory(self):
		...
		for k in mandatory:
			if not self.get(k):
				artech_engine.throw(_("{0} is required").format(_(self.meta.get_label(k))))

		if not self.is_cancelled and not (self.party_type and self.party):
			account_type = artech_engine.get_cached_value("Account", self.account, "account_type")

			if not artech_engine.flags.party_not_required:  # skipping validation if party is not required
	...
	def pl_must_have_cost_center(self):
		...
		if artech_engine.get_cached_value("Account", self.account, "report_type") == "Profit and Loss":
			...
			artech_engine.throw(msg, title=_("Missing Cost Center"))

	def validate_dimensions_for_pl_and_bs(self):
		account_type = artech_engine.get_cached_value("Account", self.account, "report_type")
	...
	def check_pl_account(self):
		if (
			self.is_opening == "Yes"
			and artech_engine.get_cached_value("Account", self.account, "report_type") == "Profit and Loss"
			and not self.is_cancelled
		):
			artech_engine.throw(
	...
	def validate_cost_center(self):
		...
		is_group, company = artech_engine.get_cached_value("Cost Center", self.cost_center, ["is_group", "company"])
		...
		if self.voucher_type != "Period Closing Voucher" and is_group:
			artech_engine.throw(
				...
				).format(self.voucher_type, self.voucher_no, artech_engine.bold(self.cost_center))
			)
	...
	def set_amount_in_reporting_currency(self):
		default_currency, reporting_currency = artech_engine.get_cached_value(
...
def validate_balance_type(account, adv_adj=False):
	if not adv_adj and account:
		balance_must_be = artech_engine.get_cached_value("Account", account, "balance_must_be")
		if balance_must_be:
			balance = artech_engine.db.sql(
...
def update_outstanding_amt(
	...
	if party_type and party:
		party_condition = " and party_type={} and party={}".format(
			artech_engine.db.escape(party_type), artech_engine.db.escape(party)
		)
	else:
		party_condition = ""

	if against_voucher_type == "Sales Invoice":
		party_account = artech_engine.get_cached_value(against_voucher_type, against_voucher, "debit_to")
		account_condition = f"and account in ({artech_engine.db.escape(account)}, {artech_engine.db.escape(party_account)})"
	else:
		account_condition = f" and account = {artech_engine.db.escape(account)}"
	...
	if against_voucher_type in OUTSTANDING_DOCTYPES:
		ref_doc = artech_engine.get_doc(against_voucher_type, against_voucher)

		# Didn't use db_set for optimization purpose
		ref_doc.outstanding_amount = bal
		artech_engine.db.set_value(against_voucher_type, against_voucher, "outstanding_amount", bal)

		ref_doc.set_status(update=True)


def validate_frozen_account(company, account, adv_adj=None):
	frozen_account = artech_engine.get_cached_value("Account", account, "freeze_account")
	if frozen_account == "Yes" and not adv_adj:
		...
		if not role_allowed_for_frozen_entries:
			artech_engine.throw(_("Account {0} is frozen").format(account))
		elif role_allowed_for_frozen_entries not in artech_engine.get_roles():
			artech_engine.throw(_("Not authorized to edit frozen Account {0}").format(account))


def update_against_account(voucher_type, voucher_no):
	entries = artech_engine.db.get_all(
		"GL Entry",
	...
	if not entries:
		return
	company_currency = artech.get_company_currency(entries[0].company)
	precision = get_field_precision(artech_engine.get_meta("GL Entry").get_field("debit"), currency=company_currency)
	...
	for d in entries:
		...
		if d.against != new_against:
			artech_engine.db.set_value("GL Entry", d.name, "against", new_against)


def on_doctype_update():
	artech_engine.db.add_index("GL Entry", ["voucher_type", "voucher_no"])
	artech_engine.db.add_index("GL Entry", ["posting_date", "company"])
	artech_engine.db.add_index("GL Entry", ["party_type", "party"])
...
def rename_temporarily_named_docs(doctype):
	...
	docs_to_rename = artech_engine.get_all(doctype, {"to_rename": "1"}, order_by="creation", limit=50000)
	autoname = artech_engine.get_meta(doctype).autoname
	...
	for batch in create_batch(docs_to_rename, 100):
		for doc in batch:
			...
			for hook_type in ("on_gle_rename", "on_sle_rename"):
				for hook in artech_engine.get_hooks(hook_type):
					artech_engine.call(hook, newname=newname, oldname=oldname)
