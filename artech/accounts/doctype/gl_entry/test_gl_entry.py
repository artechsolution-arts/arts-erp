...
from artech_engine.model.naming import parse_naming_series

from artech.accounts.doctype.gl_entry.gl_entry import rename_gle_sle_docs
...
class TestGLEntry(ArtechTestSuite):
	def test_round_off_entry(self):
		artech_engine.db.set_value("Company", "_Test Company", "round_off_account", "_Test Write Off - _TC")
		artech_engine.db.set_value("Company", "_Test Company", "round_off_cost_center", "_Test Cost Center - _TC")
		...
		round_off_entry = artech_engine.db.sql(
	...
	def test_rename_entries(self):
		...
		rename_gle_sle_docs()
		naming_series = parse_naming_series(parts=artech_engine.get_meta("GL Entry").autoname.split(".")[:-1])
		...
		old_naming_series_current_value = artech_engine.db.sql(
			"SELECT current from tabSeries where name = %s", naming_series
		)[0][0]

		rename_gle_sle_docs()

		new_gl_entries = artech_engine.get_all(
			"GL Entry",
			fields=["name", "to_rename"],
			filters={"voucher_type": "Journal Entry", "voucher_no": je.name},
			order_by="creation",
		...
		new_naming_series_current_value = artech_engine.db.sql(
			"SELECT current from tabSeries where name = %s", naming_series
		)[0][0]
