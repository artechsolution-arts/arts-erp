...
class InvoiceDiscounting(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.discounted_invoice.discounted_invoice import DiscountedInvoice
	...
	def validate_mandatory(self):
		if self.docstatus == 1 and not (self.loan_start_date and self.loan_period):
			artech_engine.throw(_("Loan Start Date and Loan Period are mandatory to save the Invoice Discounting"))

	def validate_invoices(self):
		discounted_invoices = [
			record.sales_invoice
			for record in artech_engine.get_all(
		...
		for record in self.invoices:
			if record.sales_invoice in discounted_invoices:
				artech_engine.throw(
					_("Row({0}): {1} is already discounted in {2}").format(
						record.idx, artech_engine.bold(record.sales_invoice), artech_engine.bold(record.parent)
					)
				)

			actual_outstanding = artech_engine.db.get_value(
			...
			if record.outstanding_amount > actual_outstanding:
				artech_engine.throw(
					...
					).format(record.idx, artech_engine.bold(actual_outstanding), artech_engine.bold(record.sales_invoice))
				)
	...
	def set_status(self, status=None, cancel=0):
		if status:
			...
			for d in self.invoices:
				artech_engine.get_doc("Sales Invoice", d.sales_invoice).set_status(
	...
	def update_sales_invoice(self):
		for d in self.invoices:
			if self.docstatus == 1:
				...
				discounted_invoice = artech_engine.db.exists(
					{"doctype": "Discounted Invoice", "sales_invoice": d.sales_invoice, "docstatus": 1}
				)
				is_discounted = 1 if discounted_invoice else 0
			artech_engine.db.set_value("Sales Invoice", d.sales_invoice, "is_discounted", is_discounted)
	...
	def make_gl_entries(self):
		...
		for d in self.invoices:
			inv = artech_engine.db.get_value("Sales Invoice", d.sales_invoice, invoice_fields, as_dict=1)

			if d.outstanding_amount:
				outstanding_in_company_currency = flt(
					d.outstanding_amount * inv.conversion_rate, d.precision("outstanding_amount")
				)
				ar_credit_account_currency = artech_engine.get_cached_value(
	...
	def close_loan(self):
		...
		if getdate(self.loan_end_date) > getdate(nowdate()):
			for d in self.invoices:
				outstanding_amount = artech_engine.db.get_value(
...
def get_party_account_based_on_invoice_discounting(sales_invoice):
	party_account = None
	invoice_discounting = artech_engine.db.sql(
		"""
