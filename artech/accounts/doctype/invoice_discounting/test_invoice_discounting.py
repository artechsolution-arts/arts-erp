...
from artech.accounts.doctype.journal_entry.journal_entry import get_payment_entry_against_invoice
...
class TestInvoiceDiscounting(ArtechTestSuite):
	def setUp(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "default_bank_account", self.bank_account)
	...
	def test_on_close_after_loan_period_after_inv_payment(self):
		...
		je_on_payment = artech_engine.get_doc(get_payment_entry_against_invoice("Sales Invoice", inv.name))
...
def create_invoice_discounting(invoices, **args):
	...
	inv_disc = artech_engine.new_doc("Invoice Discounting")
