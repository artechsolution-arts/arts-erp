...
from artech_engine.query_builder.functions import Count, Max, Min, Sum
...
class ProcessPeriodClosingVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.process_period_closing_voucher_detail.process_period_closing_voucher_detail import (
	...
	def generate_pcv_dates(self):
		...
		pcv = artech_engine.get_doc("Period Closing Voucher", self.parent_pcv)
...
def start_pcv_processing(docname: str):
	if artech_engine.db.get_value("Process Period Closing Voucher", docname, "status") in ["Queued", "Running"]:
		artech_engine.db.set_value("Process Period Closing Voucher", docname, "status", "Running")
		if normal_balances := artech_engine.db.get_all(
			...
			if not is_scheduler_inactive():
				for x in normal_balances:
					...
					artech_engine.enqueue(
						method="artech.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.process_individual_date",
						queue="long",
						timeout="3600",
						is_async=True,
			...
			artech_engine.db.set_value("Process Period Closing Voucher", docname, "status", "Completed")
...
def pause_pcv_processing(docname: str):
	...
	if queued_dates := artech_engine.db.get_all(
		"Process Period Closing Voucher Detail",
		filters={"parent": docname, "status": "Queued"},
		pluck="name",
	):
...
def resume_pcv_processing(docname: str):
	...
	if paused_dates := artech_engine.db.get_all(
		"Process Period Closing Voucher Detail",
	...
	gl_entry = artech_engine._dict(
		{
			...
			"account_currency": artech_engine.db.get_value("Account", pcv.closing_account_head, "account_currency"),
...
def schedule_next_date(docname: str):
	if to_process := artech_engine.db.get_all(
		"Process Period Closing Voucher Detail",
		filters={"parent": docname, "status": "Queued"},
		fields=["processing_date", "report_type", "parentfield"],
		order_by="parentfield, idx, processing_date",
...
def get_consolidated_gles(balances, report_type) -> list:
	...
	for x in balances:
		if x.report_type == report_type:
			closing_balances = [artech_engine._dict(gle) for gle in artech_engine.json.loads(x.closing_balance)]
...
def get_gl_entries(docname):
	...
	"""
	ppcv = artech_engine.get_doc("Process Period Closing Voucher", docname)
	...
	artech_engine.db.set_value(
		"Process Period Closing Voucher", docname, "p_l_closing_balance", artech_engine.json.dumps(json_dict)
	)

	# build gl map
	pcv = artech_engine.get_doc("Period Closing Voucher", ppcv.parent_pcv)
...
def calculate_balance_sheet_balance(docname):
	...
	artech_engine.db.set_value(
		"Process Period Closing Voucher", docname, "bs_closing_balance", artech_engine.json.dumps(json_dict)
	)
...
def summarize_and_post_ledger_entries(docname):
	...
	pcv_name = artech_engine.db.get_value("Process Period Closing Voucher", docname, "parent_pcv")
	pcv = artech_engine.get_doc("Period Closing Voucher", pcv_name)
	...
	artech_engine.db.set_value("Period Closing Voucher", pcv.name, "gle_processing_status", "Completed")
	artech_engine.db.set_value("Process Period Closing Voucher", docname, "status", "Completed")
...
def build_dimension_wise_balance_dict(gl_entries):
	dimension_balances = artech_engine._dict()
	for x in gl_entries:
		dimension_key = get_dimension_key(x)
		dimension_balances.setdefault(dimension_key, artech_engine._dict()).setdefault(
			x.account,
			artech_engine._dict(
				{
...
def process_individual_date(docname: str, date, report_type, parentfield):
	current_date_status = artech_engine.db.get_value(
		"Process Period Closing Voucher Detail",
	...
	pcv_name = artech_engine.db.get_value("Process Period Closing Voucher", docname, "parent_pcv")
	company = artech_engine.db.get_value("Period Closing Voucher", pcv_name, "company")
