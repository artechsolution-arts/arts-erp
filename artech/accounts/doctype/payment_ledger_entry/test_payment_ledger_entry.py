...
class TestPaymentLedgerEntry(ArtechTestSuite):
	def setUp(self):
	...
	def create_company(self):
		...
		# create bank account
		if artech_engine.db.exists("Account", "HDFC - _PL"):
			self.bank = "HDFC - _PL"
		else:
	...
	def test_multi_payment_unlink_on_invoice_cancellation(self):
		...
		entries = artech_engine.db.get_list(
			"Payment Ledger Entry",
		...
		si.delete()
		self.assertRaises(artech_engine.DoesNotExistError, artech_engine.get_doc, si.doctype, si.name)
	...
	def test_advance_payment_unlink_on_order_cancellation(self):
		...
		# with references removed, deletion should be possible
		so.delete()
		self.assertRaises(artech_engine.DoesNotExistError, artech_engine.get_doc, so.doctype, so.name)
