...
class BankTransaction(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.bank_transaction_payments.bank_transaction_payments import (
	...
	def validate_currency(self):
		...
		if self.currency and self.bank_account:
			if account := artech_engine.get_cached_value("Bank Account", self.bank_account, "account"):
				...
				if self.currency != account_currency:
					artech_engine.throw(
						_(
							...
							artech_engine.bold(self.bank_account),
							artech_engine.bold(account_currency),
						)
					)
	...
	def before_submit(self):
		...
		if artech_engine.get_single_value("Accounts Settings", "enable_party_matching"):
			self.auto_set_party()
	...
	def add_payment_entries(self, vouchers):
		...
		if 0.0 >= self.unallocated_amount:
			artech_engine.throw(_("Bank Transaction {0} is already fully reconciled").format(self.name))
	...
	def allocate_payment_entries(self):
		...
		gl_bank_account = artech_engine.db.get_value("Bank Account", self.bank_account, "account")
		...
		for payment_entry in list(self.payment_entries):
			...
			if allocable_amount < 0:
				artech_engine.throw(_("Voucher {0} is over-allocated by {1}").format(allocable_amount))
	...
	def clear_linked_payment_entry(self, payment_entry, clearance_date=None):
		...
		artech_engine.db.set_value(doctype, docname, "clearance_date", clearance_date)

	def update_linked_bank_transaction(self, bank_transaction_name, allocated_amount=None):
		"""For when a second bank transaction has fixed another, e.g. refund"""

		bt = artech_engine.get_doc(self.doctype, bank_transaction_name)
		if allocated_amount:
			bt.append(
				"payment_entries",
				{
	...
	def auto_set_party(self):
		from artech.accounts.doctype.bank_transaction.auto_match_party import AutoMatchParty
		...
		try:
			...
			artech_engine.log_error(title=_("Error in party matching for Bank Transaction {0}").format(self.name))
	...
	def validate_included_fee(self):
		...
		if self.included_fee and self.withdrawal:
			if self.included_fee > self.withdrawal:
				artech_engine.throw(_("Included fee is bigger than the withdrawal itself."))
	...
	def handle_excluded_fee(self):
		...
		if flt(self.deposit) > 0 and (flt(self.deposit) - excluded_fee) < 0:
			artech_engine.throw(_("The Excluded Fee is bigger than the Deposit it is deducted from."))
...
def get_doctypes_for_bank_reconciliation():
	...
	return artech_engine.get_hooks("bank_reconciliation_doctypes")
...
def get_clearance_details(transaction, payment_entry, bt_allocations, gl_entries, gl_bank_account):
	...
	if payment_entry.payment_document == "Bank Transaction":
		bt = artech_engine.db.get_value(
			"Bank Transaction",
			payment_entry.payment_entry,
			("unallocated_amount", "bank_account"),
			as_dict=True,
		)
		bt_bank_account = artech_engine.db.get_value("Bank Account", bt.bank_account, "account")
...
def get_related_bank_gl_entries(docs):
	# nosemgrep: artech_engine-semgrep-rules.rules.artech_engine-using-db-sql
	if not docs:
		return {}

	result = artech_engine.db.sql(
		"""
        SELECT
...
def remove_from_bank_transaction(doctype, docname):
	...
	for bt_name in get_reconciled_bank_transactions(doctype, docname):
		bt = artech_engine.get_doc("Bank Transaction", bt_name)
