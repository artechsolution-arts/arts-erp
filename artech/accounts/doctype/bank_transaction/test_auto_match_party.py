...
from artech.accounts.doctype.bank_transaction.test_bank_transaction import create_bank_account
...
class TestAutoMatchParty(ArtechTestSuite):
	def setUp(self):
		create_bank_account()
		artech_engine.db.set_single_value("Accounts Settings", "enable_party_matching", 1)
		artech_engine.db.set_single_value("Accounts Settings", "enable_fuzzy_matching", 1)
...
def create_supplier_for_match(supplier_name="John Doe & Co.", iban=None, account_no=None):
	if artech_engine.db.exists("Supplier", {"supplier_name": supplier_name}):
	...
	if not artech_engine.db.exists("Bank", "TestBank"):
		bank = artech_engine.new_doc("Bank")
		bank.bank_name = "TestBank"
		bank.insert(ignore_if_duplicate=True)

	if not artech_engine.db.exists("Bank Account", supplier.name + " - " + "TestBank"):
		bank_account = artech_engine.new_doc("Bank Account")
...
def create_bank_transaction(
	...
	doc = artech_engine.new_doc("Bank Transaction")
	doc.update(
		{
