...
from artech_engine import utils
from artech_engine.model.docstatus import DocStatus

from artech.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool import (
	get_linked_payments,
	reconcile_vouchers,
)
...
class TestBankTransaction(ArtechTestSuite):
	def setUp(self):
		make_pos_profile()

		# generate and use a uniq hash identifier for 'Bank Account' and it's linked GL 'Account' to avoid validation error
		uniq_identifier = artech_engine.generate_hash(length=10)
	...
	# This test checks if Artech is able to provide a linked payment for a bank transaction based on the amount of the bank transaction.
	def test_linked_payments(self):
		bank_transaction = artech_engine.get_doc(
			"Bank Transaction",
	...
	def test_reconcile(self):
		...
		payment = artech_engine.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1700))
		vouchers = json.dumps(
			[
				{
		...
		unallocated_amount = artech_engine.db.get_value(
			"Bank Transaction", bank_transaction.name, "unallocated_amount"
		)
		self.assertTrue(unallocated_amount == 0)

		clearance_date = artech_engine.db.get_value("Payment Entry", payment.name, "clearance_date")
	...
	# Check if Artech can correctly filter a linked payments based on the debit/credit amount
	...
	def test_already_reconciled(self):
		...
		payment = artech_engine.get_doc("Payment Entry", dict(party="Mr G", paid_amount=1200))
		vouchers = json.dumps(
			[
				{
					"payment_doctype": "Payment Entry",
	...
	def test_clear_sales_invoice(self):
		...
		payment = artech_engine.get_doc("Sales Invoice", dict(customer="Fayva", status=["=", "Paid"]))
		vouchers = json.dumps(
			[
				{
		...
		self.assertEqual(
			artech_engine.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount"), 0
		)
		self.assertTrue(
			artech_engine.db.get_value("Sales Invoice Payment", dict(parent=payment.name), "clearance_date")
			is not None
		)
...
def add_vouchers(gl_account="_Test Bank - _TC"):
	...
	mode_of_payment = artech_engine.get_doc({"doctype": "Mode of Payment", "name": "Wire Transfer"})
