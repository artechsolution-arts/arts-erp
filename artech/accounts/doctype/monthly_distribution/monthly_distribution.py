...
from artech_engine.utils import add_months, flt
...
class MonthlyDistribution(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.monthly_distribution_percentage.monthly_distribution_percentage import (
	...
	def validate(self):
		...
		if flt(total, 2) != 100.0:
			artech_engine.throw(_("Percentage Allocation should be equal to 100%") + f" ({flt(total, 2)!s}%)")


def get_periodwise_distribution_data(distribution_id, period_list, periodicity):
	doc = artech_engine.get_doc("Monthly Distribution", distribution_id)
