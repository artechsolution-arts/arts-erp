...
class Dunning(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.overdue_payment.overdue_payment import OverduePayment
	...
	def validate_same_currency(self):
		...
		for row in self.overdue_payments:
			invoice_currency = artech_engine.get_value("Sales Invoice", row.sales_invoice, "currency")
	...
	def set_dunning_level(self):
		for row in self.overdue_payments:
			past_dunnings = artech_engine.get_all(
				"Overdue Payment",
				filters={
...
def update_linked_dunnings(doc, previous_outstanding_amount):
	...
	dunnings = [artech_engine.get_doc("Dunning", dunning.name) for dunning in dunnings]
	invoices = set()
...
def get_linked_dunnings_as_per_state(sales_invoice, state):
	dunning = artech_engine.qb.DocType("Dunning")
	overdue_payment = artech_engine.qb.DocType("Overdue Payment")

	return (
		artech_engine.qb.from_(dunning)
...
def get_dunning_letter_text(dunning_type: str, doc: str | dict, language: str | None = None) -> dict:
	...
	letter_text = None
	if language:
		letter_text = artech_engine.db.get_value(
	...
	return {
		"body_text": artech_engine.render_template(letter_text.body_text, doc),
		"closing_text": artech_engine.render_template(letter_text.closing_text, doc),
