artech_engine.ui.form.on("Dunning", {
	setup: function (frm) {
		...
		frm.set_query("company_address", artech.queries.company_address_query);
	...
	refresh: function (frm) {
		...
		artech_engine.dynamic_link = { doc: frm.doc, fieldname: "customer", doctype: "Customer" };
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			if (frm.fields_dict.currency) {
				const company_currency = artech.get_currency(frm.doc.company);
			...
			const company_doc = artech_engine.get_doc(":Company", frm.doc.company);
	...
	conversion_rate: function (frm) {
		if (frm.doc.currency === artech.get_currency(frm.doc.company)) {
			frm.set_value("conversion_rate", 1.0);
		}

		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("conversion_rate", "read_only", artech.stale_rate_allowed() ? 0 : 1);
	},
	customer_address: function (frm) {
		artech.utils.get_address_display(frm, "customer_address");
	},
	company_address: function (frm) {
		artech.utils.get_address_display(frm, "company_address");
	},
	...
	get_dunning_letter_text: function (frm) {
		if (frm.doc.dunning_type) {
			artech_engine.call({
				method: "artech.accounts.doctype.dunning.dunning.get_dunning_letter_text",
				args: {
	...
	calculate_overdue_days: function (frm) {
		frm.doc.overdue_payments.forEach((row) => {
			if (frm.doc.posting_date && row.due_date) {
				...
				artech_engine.model.set_value(row.doctype, row.name, "overdue_days", overdue_days);
			}
		});
	},
	calculate_interest: function (frm) {
		frm.doc.overdue_payments.forEach((row) => {
			...
			artech_engine.model.set_value(row.doctype, row.name, "interest", interest);
		});
	},
	...
	make_payment_entry: function (frm) {
		return artech_engine.call({
			method: "artech.accounts.doctype.payment_entry.payment_entry.get_payment_entry",
			args: {
				dt: frm.doc.doctype,
				dn: frm.doc.name,
			},
...
artech_engine.ui.form.on("Overdue Payment", {
