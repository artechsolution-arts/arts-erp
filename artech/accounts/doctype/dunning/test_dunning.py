...
from artech_engine.model import mapper
...
class TestDunning(ArtechTestSuite):
	...
	def test_dunning_with_payment_entry(self):
		...
		for overdue_payment in dunning.overdue_payments:
			outstanding_amount = artech_engine.get_value(
	...
	def test_dunning_resolution_from_credit_note(self):
		...
		credit_note = artech_engine.copy_doc(sales_invoice)
	...
	def test_dunning_not_affected_by_standalone_credit_note(self):
		...
		credit_note = artech_engine.get_doc("Sales Invoice", credit_note.name)
		credit_note.submit()
...
def create_dunning(overdue_days, dunning_type_name=None):
	...
	if dunning_type_name:
		dunning_type = artech_engine.get_doc("Dunning Type", dunning_type_name)
...
def create_dunning_type(title, fee, interest, is_default):
	...
	if artech_engine.db.exists("Dunning Type", f"{title} - _TC"):
		return

	dunning_type = artech_engine.new_doc("Dunning Type")
...
def get_income_account(company):
	return (
		artech_engine.get_value("Company", company, "default_income_account")
		or artech_engine.get_all(
			"Account",
...
def create_payment_terms_template_for_dunning():
	...
	if not artech_engine.db.exists("Payment Terms Template", "_Test 50-50 for Dunning"):
