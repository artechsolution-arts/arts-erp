...
from artech.setup.doctype.customer_group.customer_group import get_parent_customer_groups


class IncorrectCustomerGroup(artech_engine.ValidationError):
	pass


class IncorrectSupplierType(artech_engine.ValidationError):
	pass


class ConflictingTaxRule(artech_engine.ValidationError):
	pass
...
class TaxRule(Document):
	...
	def validate_tax_template(self):
		...
		if not (self.sales_tax_template or self.purchase_tax_template):
			artech_engine.throw(_("Tax Template is mandatory."))
	...
	def validate_filters(self):
		...
		query = (
			artech_engine.qb.from_(TaxRule).select(TaxRule.name, TaxRule.priority).where(TaxRule.name != self.name)
		)
...
def get_party_details(party: str | None, party_type: str, args: dict | None = None):
	...
	if args:
		if args.get("billing_address"):
			billing_address = artech_engine.get_doc("Address", args.get("billing_address"))
		if args.get("shipping_address"):
			shipping_address = artech_engine.get_doc("Address", args.get("shipping_address"))
	else:
		...
		if billing_address_name:
			billing_address = artech_engine.get_doc("Address", billing_address_name)
		if shipping_address_name:
			shipping_address = artech_engine.get_doc("Address", shipping_address_name)
...
def get_tax_template(posting_date, args):
	...
	conditions.append(
		"ifnull(tax_category, '') = {}".format(artech_engine.db.escape(cstr(args.get("tax_category")), False))
	)
	...
	for key, value in args.items():
		if key == "use_for_shopping_cart":
			...
			conditions.append(f"ifnull({key}, '') in ('', {artech_engine.db.escape(cstr(value))})")

	tax_rule = artech_engine.db.sql(
	...
	if artech_engine.db.get_value(doctype, tax_template, "disabled") == 1:
		return None
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = ["%s" % (artech_engine.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)]
	if customer_groups:
