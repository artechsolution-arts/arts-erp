					...
					account = artech_engine.get_doc(
						{
							...
							if custom_chart
							else artech_engine.get_cached_value("Company", company, "default_currency"),
							"tax_rate": child.get("tax_rate"),
						}
					)

					if root_account or artech_engine.local.flags.allow_unverified_charts:
...
def get_chart(chart_template: str | None, existing_company: str | None = None):
	...
	if existing_company:
		...
		from artech.accounts.doctype.account.chart_of_accounts.verified import (
		...
		for folder in folders:
			...
			for fname in os.listdir(path):
				fname = artech_engine.as_unicode(fname)
...
def get_charts_for_country(country: str, with_standard: bool = False):
	...
	def _get_chart_name(content):
		if content:
			...
			) or artech_engine.local.flags.allow_unverified_charts:
				charts.append(content["name"])

	country_code = artech_engine.get_cached_value("Country", country, "code")
	if country_code:
		folders = ("verified",)
		if artech_engine.local.flags.allow_unverified_charts:
			folders = ("verified", "unverified")
...
def get_account_tree_from_existing_company(existing_company):
	all_accounts = artech_engine.get_all(
		"Account",
