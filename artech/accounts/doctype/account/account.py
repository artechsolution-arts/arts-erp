...
from artech_engine.utils import add_to_date, cint, cstr, pretty_date
from artech_engine.utils.nestedset import NestedSet, get_ancestors_of, get_descendants_of

import artech


class RootNotEditable(artech_engine.ValidationError):
	pass


class BalanceMismatchError(artech_engine.ValidationError):
	pass


class InvalidAccountMergeError(artech_engine.ValidationError):
	pass
...
class Account(NestedSet):
	...
	def on_update(self):
		if artech_engine.local.flags.ignore_update_nsm:
			return
		else:
			super().on_update()

	def onload(self):
		role_allowed_for_frozen_entries = artech_engine.db.get_value(
			"Company", self.company, "role_allowed_for_frozen_entries"
		)
		if not role_allowed_for_frozen_entries or role_allowed_for_frozen_entries in artech_engine.get_roles():
			self.set_onload("can_freeze_account", True)

	def autoname(self):
		from artech.accounts.utils import get_autoname_with_number

		self.name = get_autoname_with_number(self.account_number, self.account_name, self.company)

	def validate(self):
		if artech_engine.local.flags.allow_unverified_charts:
			return
	...
	def validate_parent_child_account_type(self):
		if self.parent_account:
			if self.account_type in [
				...
				parent_account_type = artech_engine.db.get_value("Account", self.parent_account, ["account_type"])
	...
	def validate_parent(self):
		...
		if self.parent_account:
			par = artech_engine.get_cached_value(
	...
	def validate_receivable_payable_account_type(self):
		...
		if (
			...
			if artech_engine.db.get_all("GL Entry", filters={"account": self.name, "is_cancelled": 0}, limit=1):
				msg = _(
					"There are ledger entries against this account. Changing {0} to non-{1} in live system will cause incorrect output in 'Accounts {2}' report"
				).format(
					artech_engine.bold(_("Account Type")), doc_before_save.account_type, doc_before_save.account_type
				)
	...
	def validate_root_details(self):
		...
		if not self.parent_account and not cint(self.is_group):
			throw(_("The root account {0} must be a group").format(artech_engine.bold(self.name)))

	def validate_root_company_and_sync_account_to_children(self):
		# ignore validation while creating new compnay or while syncing to child companies
		if artech_engine.local.flags.ignore_root_company_validation or self.flags.ignore_root_company_validation:
			return
		ancestors = get_root_company(self.company)
		if ancestors:
			if artech_engine.get_cached_value(
			...
			if not artech_engine.db.get_value(
				...
				artech_engine.throw(_("Please add the account to root level Company - {}").format(ancestors[0]))
		elif self.parent_account:
			descendants = get_descendants_of("Company", self.company)
			if not descendants:
				return
			parent_acc_name_map = {}
			parent_acc_name, parent_acc_number = artech_engine.get_cached_value(
			...
			for d in artech_engine.db.get_values(
	...
	def validate_default_accounts_in_company(self):
		...
		company_default_accounts = artech_engine.db.get_value(
			...
			if company_default_accounts.get(d) == self.name:
				throw(
					msg.format(
						...
						artech_engine.bold(default_account_fields.get(d)),
	...
	def validate_frozen_accounts_modifier(self):
		...
		if not role_allowed_for_frozen_entries or role_allowed_for_frozen_entries not in artech_engine.get_roles():
	...
	def validate_account_currency(self):
		...
		if not self.account_currency:
			self.account_currency = artech_engine.get_cached_value("Company", self.company, "default_currency")
			self.currency_explicitly_specified = False

		gl_currency = artech_engine.db.get_value(
			"GL Entry", {"account": self.name, "is_cancelled": 0}, "account_currency"
		)

		if gl_currency and self.account_currency != gl_currency:
			if artech_engine.db.get_value("GL Entry", {"account": self.name}):
				artech_engine.throw(_("Currency can not be changed after making entries using some other currency"))
	...
	def validate_account_number(self, account_number=None):
		...
		if account_number:
			account_with_same_number = artech_engine.db.get_value(
				"Account",
	...
	def create_account_for_child_company(self, parent_acc_name_map, descendants, parent_acc_name):
		for company in descendants:
			company_bold = artech_engine.bold(company)
			parent_acc_name_bold = artech_engine.bold(parent_acc_name)
			...
			if artech_engine.get_cached_value(
			...
			) and not artech_engine.get_cached_value("Account", parent_acc_name_map[company], "is_group"):
				msg = _(
				...
				artech_engine.throw(msg, title=_("Invalid Parent Account"))
			...
			child_account = artech_engine.db.get_value("Account", filters, "name")
			if not child_account:
				doc = artech_engine.copy_doc(self)
				...
				doc.update(
					{
						...
						else artech.get_company_currency(company),
						"parent_account": parent_acc_name_map[company],
					}
				)

				doc.save()
				artech_engine.msgprint(_("Account {0} is added in the child company {1}").format(doc.name, company))
			elif child_account:
				# update the parent company's value in child companies
				doc = artech_engine.get_doc("Account", child_account)
	...
	def check_gle_exists(self):
		return artech_engine.db.get_value("GL Entry", {"account": self.name})
...
def get_account_currency(account):
	...
	def generator():
		account_currency, company = artech_engine.get_cached_value(
			"Account", account, ["account_currency", "company"]
		)
		if not account_currency:
			account_currency = artech_engine.get_cached_value("Company", company, "default_currency")

		return account_currency

	return artech_engine.local_cache("account_currency", account, generator)


def on_doctype_update():
	artech_engine.db.add_index("Account", ["lft", "rgt"])


def get_account_autoname(account_number, account_name, company):
	# first validate if company exists
	company = artech_engine.get_cached_value("Company", company, ["abbr", "name"], as_dict=True)
	if not company:
		artech_engine.throw(_("Company {0} does not exist").format(company))
...
def update_account_number(
	...
	_ensure_idle_system()
	account = artech_engine.get_cached_doc("Account", name)
	if not account:
		return
	...
	allow_independent_account_creation = artech_engine.get_cached_value(
		"Company", account.company, "allow_account_creation_against_child_company"
	)

	if ancestors and not allow_independent_account_creation:
		for ancestor in ancestors:
			old_name = artech_engine.db.get_value(
				"Account",
			...
			if old_name and not from_descendant:
				...
				message = _("Account {0} exists in parent company {1}.").format(
					artech_engine.bold(old_acc_name), artech_engine.bold(ancestor)
				)
				message += "<br>"
				message += _("Renaming it is only allowed via parent company {0}, to avoid mismatch.").format(
					artech_engine.bold(ancestor)
				)
				message += "<br><br>"
				message += _("To overrule this, enable '{0}' in company {1}").format(
					allow_child_account_creation, artech_engine.bold(account.company)
				)

				artech_engine.throw(message, title=_("Rename Not Allowed"))

	account.validate_account_number(account_number)
	if account_number:
		artech_engine.db.set_value("Account", name, "account_number", account_number.strip())
	else:
		artech_engine.db.set_value("Account", name, "account_number", "")
	artech_engine.db.set_value("Account", name, "account_name", account_name.strip())
	...
	if name != new_name:
		artech_engine.rename_doc("Account", name, new_name, force=1)
		return new_name
...
def merge_account(old: str, new: str):
	...
	new_account = artech_engine.get_cached_doc("Account", new)
	old_account = artech_engine.get_cached_doc("Account", old)
	...
	if old_account.is_group and new_account.parent_account == old:
		new_account.db_set("parent_account", artech_engine.get_cached_value("Account", old, "parent_account"))

	artech_engine.rename_doc("Account", old, new, merge=1, force=1)
...
def sync_update_account_number_in_child(
	...
	for d in artech_engine.db.get_values("Account", filters=filters, fieldname=["company", "name"], as_dict=True):
...
def _ensure_idle_system():
	...
	try:
		...
		last_gl_update = artech_engine.db.get_value("GL Entry", {}, "modified", for_update=True, wait=False)
	except artech_engine.QueryTimeoutError:
