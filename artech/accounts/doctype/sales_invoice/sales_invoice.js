...
artech.accounts.taxes.setup_tax_validations("Sales Invoice");
artech.accounts.payment_triggers.setup("Sales Invoice");
artech.accounts.pos.setup("Sales Invoice");
artech.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
artech.sales_common.setup_selling_controller();
artech.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	artech.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		artech.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		artech.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	...
	make_invoice_discounting() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	on_submit(doc, dt, dn) {
		...
		super.on_submit();
		if (artech_engine.get_route()[0] != "Form") {
			return;
		}

		doc.items.forEach((row) => {
			if (row.delivery_note) artech_engine.model.clear_doc("Delivery Note", row.delivery_note);
		});
	}
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				artech.utils.map_current_doc({
					method: "artech.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				...
				artech.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "artech.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
					allow_child_item_selection: true,
	...
	customer() {
		...
		artech.utils.get_party_details(
			this.frm,
			"artech.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			artech_engine.call({
				method: "artech.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	write_off_outstanding_amount_automatically() {
		if (cint(this.frm.doc.write_off_outstanding_amount_automatically)) {
			artech_engine.model.round_floats_in(this.frm.doc, ["grand_total", "paid_amount"]);
	...
	make_sales_return() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			artech_engine.call({
				method: artech.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
					company: frm.doc.company,
				},
				callback: function (r, rt) {
					artech_engine.model.set_value(cdt, cdn, "income_account", r.message[0]);
					artech_engine.model.set_value(cdt, cdn, "cost_center", r.message[1]);
				},
			});
		}
	}
	...
	set_pos_data() {
		if (this.frm.doc.is_pos) {
			...
			if (!this.frm.doc.company) {
				...
				artech_engine.msgprint(__("Please specify Company to proceed"));
			} else {
				var me = this;
				...
				return this.frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							artech_engine.model.set_default_values(me.frm.doc);
							me.set_dynamic_labels();
	...
	currency() {
		...
		if (this.frm.doc.timesheets) {
			this.frm.doc.timesheets.forEach((d) => {
				let row = artech_engine.get_doc(d.doctype, d.name);
...
extend_cscript(cur_frm.cscript, new artech.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	artech_engine.model.open_mapped_doc({
		method: "artech.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	artech.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	artech.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};
...
artech_engine.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "artech.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "artech.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	hide_fields: function (frm) {
		...
		if (cint(doc.is_pos) == 1) {
			...
			for (var i in parent_fields) {
				var docfield = artech_engine.meta.docfield_map[doc.doctype][parent_fields[i]];
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			artech_engine.call({
				method: "artech.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	set_loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			if (frm.doc.grand_total && remaining_amount < loyalty_amount) {
				...
				artech_engine.throw(__("You can only redeem max {0} points in this order.", [redeemable_points]));
			}
	...
	add_timesheet_item: function (frm, item_code, timesheets) {
		...
		artech_engine.model.set_value(row.doctype, row.name, "item_code", item_code);
	...
	async get_timesheet_data(frm, kwargs) {
		return artech_engine
			.call({
				method: "artech.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
...
artech_engine.ui.form.on("Sales Invoice Timesheet", {
	timesheets_remove(frm) {
		frm.trigger("calculate_timesheet_totals");
	},
});

artech_engine.ui.form.on("Sales Invoice Payment", {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	artech_engine.call({
		method: "artech.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
		callback: function (r) {
			if (!r.exc && r.message) {
				artech_engine.model.set_value(cdt, cdn, "billing_amount", r.message);
			}
		},
	});
};
...
var select_loyalty_program = function (frm, loyalty_programs) {
	...
	dialog.set_primary_action(__("Set Loyalty Program"), function () {
		dialog.hide();
		return artech_engine.call({
			method: "artech_engine.client.set_value",
			args: {
				doctype: "Customer",
