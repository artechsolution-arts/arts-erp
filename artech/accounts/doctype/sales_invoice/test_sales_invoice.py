...
from artech_engine.model.dynamic_links import get_dynamic_link_map
from artech_engine.utils import add_days, cint, flt, format_date, getdate, nowdate, today

import artech
from artech.accounts.doctype.account.test_account import create_account, get_inventory_account
from artech.accounts.doctype.mode_of_payment.test_mode_of_payment import (
	set_default_account_for_mode_of_payment,
)
from artech.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from artech.accounts.doctype.purchase_invoice.purchase_invoice import WarehouseMissingError
from artech.accounts.doctype.purchase_invoice.test_purchase_invoice import (
	unlink_payment_on_cancel_of_invoice,
)
from artech.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_transaction
from artech.accounts.utils import PaymentEntryUnlinkError
from artech.assets.doctype.asset.depreciation import post_depreciation_entries
...
from artech.controllers.accounts_controller import InvalidQtyError, update_invoice_status
from artech.controllers.taxes_and_totals import get_itemised_tax_breakup_data
from artech.exceptions import InvalidAccountCurrency, InvalidCurrency
from artech.selling.doctype.customer.test_customer import get_customer_dict
from artech.stock.doctype.delivery_note.delivery_note import make_sales_invoice
...
from artech.stock.get_item_details import get_item_tax_map
from artech.stock.utils import get_incoming_rate, get_stock_balance
from artech.tests.utils import ArtechTestSuite


class TestSalesInvoice(ArtechTestSuite):
	def setUp(self):
		...
		from artech.stock.doctype.stock_ledger_entry.test_stock_ledger_entry import create_items

		create_items(["_Test Internal Transfer Item"], uoms=[{"uom": "Box", "conversion_factor": 10}])
		create_internal_parties()
		setup_accounts()
		mode_of_payment = artech_engine.get_doc("Mode of Payment", "Bank Draft")
		...
		for company in artech_engine.get_all("Company", pluck="name"):
			artech_engine.db.set_value("Company", company, "accounts_frozen_till_date", None)
	...
	def test_invalid_rate_without_override(self):
		from artech_engine import ValidationError

		from artech.accounts.doctype.sales_invoice.sales_invoice import make_inter_company_purchase_invoice
	...
	def make(self):
		w = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		w.is_pos = 0
		w.insert()
		w.submit()
		return w
	...
	def test_timestamp_change(self):
		...
		w2 = artech_engine.get_doc(w.doctype, w.name)
		...
		time.sleep(1)
		self.assertRaises(artech_engine.TimestampMismatchError, w2.save)

	def test_sales_invoice_change_naming_series(self):
		si = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][2])
		si.insert()
		si.naming_series = "TEST-"

		self.assertRaises(artech_engine.CannotChangeConstantError, si.save)

		si = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][1])
		si.insert()
		si.naming_series = "TEST-"
	...
	def test_payment_entry_unlink_against_invoice(self):
		from artech.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry

		si = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][0])
		si.is_pos = 0
		si.insert()
		si.submit()
		...
		si = artech_engine.get_doc("Sales Invoice", si.name)
		self.assertRaises(artech_engine.LinkExistsError, si.cancel)
		unlink_payment_on_cancel_of_invoice()

	@ArtechTestSuite.change_settings("Accounts Settings", {"unlink_payment_on_cancellation_of_invoice": 1})
	...
	def test_sales_invoice_discount_amount(self):
		si = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][3])
	...
	def test_discount_amount_gl_entry(self):
		artech_engine.db.set_value("Company", "_Test Company", "round_off_account", "Round Off - _TC")
		si = artech_engine.copy_doc(self.globalTestRecords["Sales Invoice"][3])
		si.discount_amount = 104.94
		si.append(
			"taxes",
		...
		gl_entries = artech_engine.db.sql(
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_multiple_items": True})
	...
	def test_inclusive_rate_validations(self):
		...
		self.assertRaises(artech_engine.ValidationError, si.insert)
	...
	def test_sales_invoice_calculation_export_currency_with_tax_inclusive_price(self):
		...
		if artech_engine.db.get_value("Currency", "USD", "smallest_currency_fraction_value") < 0.01:
	...
	def test_payment(self):
		w = self.make()
		jv = artech_engine.get_doc(artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][0]))
		jv.get("accounts")[0].reference_type = w.doctype
		jv.get("accounts")[0].reference_name = w.name
		jv.insert()
		jv.submit()

		self.assertEqual(artech_engine.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 162.0)
		...
		jv.cancel()
		self.assertEqual(artech_engine.db.get_value("Sales Invoice", w.name, "outstanding_amount"), 562.0)
	...
	def test_outstanding_on_cost_center_allocation(self):
		...
		from artech.accounts.doctype.cost_center_allocation.test_cost_center_allocation import (
	...
	def test_pos_returns_with_repayment(self):
		from artech.accounts.doctype.sales_invoice.sales_invoice import make_sales_return
	...
	def test_pos_with_no_gl_entry_for_change_amount(self):
		artech_engine.db.set_single_value("POS Settings", "post_change_gl_entries", 0)
		...
		artech_engine.db.set_single_value("POS Settings", "post_change_gl_entries", 1)
	...
	def validate_pos_gl_entry(self, si, pos, cash_amount, validate_without_change_gle=False):
		...
		artech_engine.db.sql("delete from `tabPOS Profile`")

	def test_bin_details_of_packed_item(self):
		from artech.selling.doctype.product_bundle.test_product_bundle import make_product_bundle
		from artech.stock.doctype.item.test_item import make_item

		# test Update Items with product bundle
		if not artech_engine.db.exists("Item", "_Test Product Bundle Item New"):
		...
		bin_details = artech_engine.db.get_value(
			"Bin",
	...
	def test_pos_si_without_payment(self):
		...
		self.assertRaises(artech_engine.ValidationError, si.submit)
	...
	def _insert_delivery_note(self):
		dn = artech_engine.copy_doc(self.globalTestRecords["Delivery Note"][0])
	...
	def test_sales_invoice_with_advance(self):
		jv = artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][0])
		jv.insert()
		jv.submit()
	...
	def test_serialized(self):
		from artech.stock.doctype.stock_entry.test_stock_entry import make_serialized_item
		...
		self.assertFalse(artech_engine.db.get_value("Serial No", serial_nos[0], "warehouse"))
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(
			artech_engine.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC"
		)
	...
	def test_return_sales_invoice(self):
		...
		incoming_rate, stock_value_difference = artech_engine.db.get_value(
			"Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = artech_engine.db.get_value(
			"GL Entry",
		...
		party_credited = artech_engine.db.get_value(
			"GL Entry",
			{
		...
		self.assertEqual(artech_engine.db.get_value("Sales Invoice", si1.name, "outstanding_amount"), -1000)
		self.assertEqual(artech_engine.db.get_value("Sales Invoice", si.name, "outstanding_amount"), 2500)
	...
	def test_zero_qty_return_invoice_with_stock_effect(self):
		...
		self.assertRaises(artech_engine.ValidationError, cr_note.save)
	...
	def test_gle_made_when_asset_is_returned(self):
		...
		disposal_account = artech_engine.get_cached_value("Company", "_Test Company", "disposal_account")

		# Asset value is 100,000 but it was sold for 90,000, so there should be a loss of 10,000
		loss_for_si = artech_engine.get_all(
			"GL Entry",
			filters={"voucher_no": si.name, "account": disposal_account},
			fields=["credit", "debit"],
		)[0]

		loss_for_return_si = artech_engine.get_all(
			"GL Entry",
	...
	def test_incoming_rate_for_stand_alone_credit_note(self):
		...
		incoming_rate = artech_engine.db.get_value(
			"Stock Ledger Entry", {"voucher_no": return_si.name}, "incoming_rate"
		)
		debit_amount = artech_engine.db.get_value(
	...
	def test_outstanding_amount_after_advance_jv_cancellation(self):
		...
		jv = artech_engine.get_doc("Journal Entry", jv.name)
		jv.cancel()
	...
	def test_multiple_uom_in_selling(self):
		item_price = artech_engine.new_doc("Item Price")
	...
	def test_item_wise_tax_breakup(self):
		artech_engine.flags.country = "United States"
		...
		artech_engine.flags.country = None
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"sales_update_frequency": "Each Transaction"})
	def test_company_monthly_sales(self):
		...
		existing_current_month_sales = artech_engine.db.get_value("Company", company, "total_monthly_sales")

		si = create_sales_invoice()
		current_month_sales = artech_engine.db.get_value("Company", company, "total_monthly_sales")
	...
	def test_rounding_adjustment_2(self):
		...
		round_off_account = artech_engine.get_cached_value("Company", "_Test Company", "round_off_account")
		expected_values = {
	...
	def test_rounding_adjustment_3(self):
		...
		round_off_gle = artech_engine.db.get_value(
			"GL Entry",
	...
	def test_sales_invoice_with_shipping_rule(self):
		from artech.accounts.doctype.shipping_rule.test_shipping_rule import create_shipping_rule
	...
	def test_credit_note(self):
		...
		si_doc = artech_engine.get_doc("Sales Invoice", si.name)
	...
	def test_deferred_revenue_missing_account(self):
		...
		self.assertRaises(artech_engine.ValidationError, si.save)
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		# setup
		old_negative_stock = artech_engine.db.get_single_value("Stock Settings", "allow_negative_stock")
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", 1)

		old_perpetual_inventory = artech.is_perpetual_inventory_enabled("_Test Company 1")
		artech_engine.local.enable_perpetual_inventory["_Test Company 1"] = 1
		...
		# tear down
		artech_engine.local.enable_perpetual_inventory["_Test Company 1"] = old_perpetual_inventory
		artech_engine.db.set_single_value("Stock Settings", "allow_negative_stock", old_negative_stock)
	...
	def test_internal_transfer_gl_precision_issues(self):
		...
		if get_stock_balance("_Test Internal Transfer Item", "Stores - TCP1", "2022-04-10"):
			create_stock_reconciliation(
				...
				if artech_engine.get_all("Stock Ledger Entry")
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"enable_discount_accounting": 1})
	...
	def test_additional_discount_for_sales_invoice_with_discount_accounting_enabled(self):
		from artech.accounts.doctype.repost_accounting_ledger.test_repost_accounting_ledger import (
	...
	def test_sales_invoice_against_supplier(self):
		from artech.accounts.doctype.opening_invoice_creation_tool.test_opening_invoice_creation_tool import (
			make_customer,
		)
		from artech.accounts.doctype.party_link.party_link import create_party_link
		...
		artech_engine.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 1)
		...
		party_link.delete()
		artech_engine.db.set_single_value("Accounts Settings", "enable_common_party_accounting", 0)
	...
	def test_sales_invoice_against_supplier_usd_with_dimensions(self):
		...
		cust_doc = artech_engine.get_doc("Customer", customer)
		cust_doc.default_currency = "USD"
		cust_doc.save()
		# create a supplier
		supplier = create_supplier(supplier_name="_Test Common Supplier USD").name
		supp_doc = artech_engine.get_doc("Supplier", supplier)
		...
		if not artech_engine.get_all("Accounting Dimension", filters={"document_type": "Department"}):
			dim = artech_engine.get_doc(
				{
			...
			dim = artech_engine.get_doc(
				...
				artech_engine.get_all("Accounting Dimension", filters={"document_type": "Department"})[0],
			)
			dim.disabled = False
	...
	def test_sales_invoice_cancel_with_common_party_advance_jv(self):
		...
		jv_status = artech_engine.db.get_value("Journal Entry", jv, "docstatus")
	...
	def test_sales_commission(self):
		...
		artech_engine.db.set_value("Item", si.get("items")[0].item_code, "grant_commission", 1)
		artech_engine.db.set_value("Item", si.get("items")[1].item_code, "grant_commission", 0)
	...
	def test_sales_invoice_submission_post_account_freezing_date(self):
		artech_engine.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", add_days(getdate(), 1))
		si = create_sales_invoice(do_not_save=True)
		...
		si.submit()
		artech_engine.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", None)

	@ArtechTestSuite.change_settings("Accounts Settings", {"over_billing_allowance": 0})
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_multiple_items": 1})
	def test_over_billing_case_against_delivery_note(self):
		...
		si = make_sales_invoice(dn.name)
		item_copy = artech_engine.copy_doc(si.items[0])
		si.save()
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "accounts_frozen_till_date", getdate("2019-01-31"))
	...
	def test_sales_invoice_with_disabled_account(self):
		try:
			...
			account = artech_engine.get_doc("Account", account_name)
			account.disabled = 1
			account.save()
	...
	def test_gain_loss_with_advance_entry(self):
		...
		je_type = artech_engine.get_cached_value("Journal Entry", journals[0], "voucher_type")
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"customer_group": None, "territory": None})
	def test_sales_invoice_without_customer_group_and_territory(self):
		# create a customer
		if not artech_engine.db.exists("Customer", "_Test Simple Customer"):
			customer_dict = get_customer_dict("_Test Simple Customer")
			customer_dict.pop("customer_group")
			customer_dict.pop("territory")
			customer = artech_engine.get_doc(customer_dict).insert(ignore_permissions=True)
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_negative_rates_for_items": 0})
	...
	def test_partial_allocation_on_advance_as_liability(self):
		...
		pr = artech_engine.get_doc("Payment Reconciliation")
		pr.company = company
		...
		pr.allocate_entries(artech_engine._dict({"invoices": invoices, "payments": payments}))
	...
	def test_loyalty_points_redemption_with_shopping_cart(self):
		from artech.accounts.doctype.loyalty_program.test_loyalty_program import (
			create_sales_invoice_record,
		)
		from artech.selling.doctype.sales_order.sales_order import make_sales_invoice
		from artech.selling.doctype.sales_order.test_sales_order import make_sales_order

		# Set up loyalty program
		loyalty_customer = artech_engine.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name
		artech_engine.db.set_value("Customer", loyalty_customer, "loyalty_program", "Test Single Loyalty")
		...
		from artech_engine.model.trace import traced_field_context
		...
		loyalty_account = artech_engine.db.get_value("Loyalty Program", "Test Single Loyalty", "expense_account")
		expected_gl_entries = [
	...
	def test_taxes_merging_from_delivery_note(self):
		...
		from artech_engine.model.mapper import map_docs

		map_docs(
			method="artech.stock.doctype.delivery_note.delivery_note.make_sales_invoice",
	...
	def _create_opening_roundoff_account(self, company_name):
		liability_root = artech_engine.db.get_all(
			"Account",
		...
		if acc := artech_engine.db.exists(
			"Account",
			{
			...
			artech_engine.db.set_value("Company", company_name, "round_off_for_opening", acc)
		else:
			...
			acc.save()
			artech_engine.db.set_value("Company", company_name, "round_off_for_opening", acc.name)
	...
	def test_ledger_entries_on_opening_invoice_with_rounding_loss_by_inclusive_tax(self):
		...
		si.submit()
		actual = artech_engine.db.get_all(
			"GL Entry",
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"enable_common_party_accounting": True})
	...
	def test_total_billed_amount(self):
		...
		project = artech_engine.new_doc("Project")
		...
		doc = artech_engine.get_doc("Project", project.name)
	...
	def test_prevents_fully_returned_invoice_with_zero_quantity(self):
		from artech.controllers.sales_and_purchase_return import StockOverReturnError, make_return_doc
	...
	def test_pos_sales_invoice_creation_during_pos_invoice_mode(self):
		...
		with self.change_settings("POS Settings", {"invoice_type": "POS Invoice"}):
			...
			self.assertRaises(artech_engine.ValidationError, pos.insert)
	...
	def test_stand_alone_credit_note_valuation(self):
		...
		stock_ledger_entry = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Sales Invoice",
				"voucher_no": si.name,
	...
	def test_non_batchwise_valuation_for_moving_average(self):
		...
		change_in_value = artech_engine.db.get_value(
			"Stock Ledger Entry",
			{
				"voucher_type": "Sales Invoice",
				"voucher_no": si.name,
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"set_zero_rate_for_expired_batch": True})
	...
	def test_inter_company_transaction_cost_center(self):
		...
		cost_center = artech_engine.db.get_value("Company", "_Test Company 1", "cost_center")
		artech_engine.db.set_value("Company", "_Test Company 1", "cost_center", None)
		...
		artech_engine.db.set_value("Company", "_Test Company 1", "cost_center", cost_center)
...
def create_sales_invoice(**args):
	si = artech_engine.new_doc("Sales Invoice")
...
def create_internal_parties():
	...
	artech_engine.db.set_value("Company", "_Test Company", "unrealized_profit_loss_account", account)
...
def create_internal_supplier(supplier_name, represents_company, allowed_to_interact_with):
	if not artech_engine.db.exists("Supplier", supplier_name):
		supplier = artech_engine.get_doc(
			{
		...
		supplier_name = artech_engine.db.exists("Supplier", supplier_name)
