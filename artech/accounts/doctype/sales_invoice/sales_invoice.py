...
from artech_engine.model.utils import get_fetch_values
from artech_engine.query_builder import Case
from artech_engine.utils import add_days, cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
from artech_engine.utils.data import comma_and

import artech
from artech.accounts.deferred_revenue import validate_service_stop_date
from artech.accounts.doctype.loyalty_program.loyalty_program import (
...
from artech.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import (
	validate_docs_for_deferred_accounting,
	validate_docs_for_voucher_types,
)
from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import SalesTaxWithholding
from artech.accounts.general_ledger import get_round_off_account_and_cost_center
from artech.accounts.party import get_due_date, get_party_account, get_party_details
from artech.accounts.utils import (
	get_account_currency,
	update_voucher_outstanding,
)
from artech.assets.doctype.asset.asset import split_asset
from artech.assets.doctype.asset.depreciation import (
	depreciate_asset,
...
from artech.assets.doctype.asset_activity.asset_activity import add_asset_activity
from artech.controllers.accounts_controller import validate_account_head
from artech.controllers.selling_controller import SellingController
from artech.projects.doctype.timesheet.timesheet import get_projectwise_timesheet_data
from artech.setup.doctype.company.company import update_company_current_month_sales
from artech.stock.doctype.delivery_note.delivery_note import update_billed_amount_based_on_so

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class PartialPaymentValidationError(artech_engine.ValidationError):
	pass
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.item_wise_tax_detail.item_wise_tax_detail import ItemWiseTaxDetail
		from artech.accounts.doctype.payment_schedule.payment_schedule import PaymentSchedule
		from artech.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from artech.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from artech.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from artech.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from artech.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
			SalesInvoiceTimesheet,
		)
		from artech.accounts.doctype.sales_taxes_and_charges.sales_taxes_and_charges import (
			SalesTaxesandCharges,
		)
		from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import TaxWithholdingEntry
		from artech.selling.doctype.sales_team.sales_team import SalesTeam
		from artech.stock.doctype.packed_item.packed_item import PackedItem
	...
	def onload(self):
		super().onload()
		if self.customer:
			tax_withholding_category, tax_withholding_group = artech_engine.get_cached_value(
	...
	def validate(self):
		...
		if self.is_return and not self.return_against and self.timesheets:
			artech_engine.throw(_("Direct return is not allowed for Timesheet."))
	...
	def validate_fixed_asset(self):
		...
		for d in self.get("items"):
			if d.is_fixed_asset:
				if d.asset:
					if not self.is_return:
						asset_status = artech_engine.db.get_value("Asset", d.asset, "status")
						if self.update_stock:
							artech_engine.throw(_("'Update Stock' cannot be checked for fixed asset sale"))
							...
							artech_engine.throw(_("Row #{0}: Asset {1} is already sold").format(d.idx, d.asset))
	...
	def on_submit(self):
		...
		if artech_engine.get_single_value("Selling Settings", "sales_update_frequency") == "Each Transaction":
		...
		if (
			...
			against_si_doc = artech_engine.get_doc("Sales Invoice", self.return_against)
	...
	def validate_pos_return(self):
		...
		if self.is_pos and self.is_return:
			...
			if total_amount_in_payments < invoice_total:
				artech_engine.throw(_("Total payments amount can't be greater than {}").format(-invoice_total))

	def validate_pos_paid_amount(self):
		if len(self.payments) == 0 and self.is_pos and flt(self.grand_total) > 0:
			artech_engine.throw(_("At least one mode of payment is required for POS invoice."))
	...
	def check_if_consolidated_invoice(self):
		...
		if self.doctype == "Sales Invoice" and self.is_consolidated:
			...
			pos_closing_entry = artech_engine.get_all(
			...
			if pos_closing_entry and pos_closing_entry[0]:
				msg = _("To cancel a {} you need to cancel the POS Closing Entry {}.").format(
					artech_engine.bold(_("Consolidated Sales Invoice")),
	...
	def check_if_created_using_pos_and_pos_closing_entry_generated(self):
		if self.doctype == "Sales Invoice" and self.is_created_using_pos and self.pos_closing_entry:
			pos_closing_entry_docstatus = artech_engine.db.get_value(
	...
	def check_credit_limit(self):
		from artech.selling.doctype.customer.customer import check_credit_limit

		validate_against_credit_limit = False
		bypass_credit_limit_check_at_sales_order = artech_engine.db.get_value(
	...
	def unlink_sales_invoice_from_timesheets(self):
		for row in self.timesheets:
			timesheet = artech_engine.get_doc("Timesheet", row.time_sheet)
	...
	def cancel_pos_invoice_credit_note_generated_during_sales_invoice_mode(self):
		pos_invoices = artech_engine.get_all(
			"POS Invoice", filters={"consolidated_invoice": self.name}, pluck="name"
		)
		if pos_invoices:
			for pos_invoice in pos_invoices:
				pos_invoice_doc = artech_engine.get_doc("POS Invoice", pos_invoice)
	...
	def set_missing_values(self, for_validate: bool = False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = artech_engine.db.get_value(
		...
		if not print_format and not cint(artech_engine.db.get_value("Print Format", "POS Invoice", "disabled")):
	...
	def reset_mode_of_payments(self):
		if self.pos_profile:
			pos_profile = artech_engine.get_cached_doc("POS Profile", self.pos_profile)
	...
	def update_time_sheet(self, sales_invoice):
		for d in self.timesheets:
			if d.time_sheet:
				timesheet = artech_engine.get_doc("Timesheet", d.time_sheet)
	...
	def update_billed_qty_in_scio(self):
		...
		table = artech_engine.qb.DocType("Subcontracting Inward Order Received Item")
		...
		if data:
			...
			artech_engine.qb.update(table).set(table.billed_qty, case_expr).where(
	...
	def validate_time_sheets_are_submitted(self):
		...
		for data in self.timesheets:
			...
			if data.time_sheet and data.timesheet_detail:
				if sales_invoice := artech_engine.db.get_value(
					"Timesheet Detail", data.timesheet_detail, "sales_invoice"
				):
					artech_engine.throw(
						_("Row {0}: Sales Invoice {1} is already created for {2}").format(
							data.idx, artech_engine.bold(sales_invoice), artech_engine.bold(data.time_sheet)
						)
					)

			if data.time_sheet:
				status = artech_engine.db.get_value("Timesheet", data.time_sheet, "status")
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.account_for_change_amount:
			self.account_for_change_amount = artech_engine.get_cached_value(
		...
		pos = {}
		if self.pos_profile:
			pos = artech_engine.get_doc("POS Profile", self.pos_profile)
		...
		if pos:
			...
			if self.customer:
				customer_price_list, customer_group = artech_engine.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group"]
				)
				customer_group_price_list = artech_engine.get_value(
			...
			if self.tc_name and not self.terms:
				self.terms = artech_engine.db.get_value("Terms and Conditions", self.tc_name, "terms")
	...
	def get_company_abbr(self):
		return artech_engine.db.sql("select abbr from tabCompany where name=%s", self.company)[0][0]
	...
	def validate_debit_to_acc(self):
		...
		account = artech_engine.get_cached_value(
			"Account", self.debit_to, ["account_type", "report_type", "account_currency"], as_dict=True
		)

		if not account:
			artech_engine.throw(_("Debit To is required"), title=_("Account Missing"))

		if account.report_type != "Balance Sheet":
			msg = (
				_("Please ensure {} account is a Balance Sheet account.").format(artech_engine.bold(_("Debit To")))
				+ " "
			)
			msg += _(
				"You can change the parent account to a Balance Sheet account or select a different account."
			)
			artech_engine.throw(msg, title=_("Invalid Account"))

		if self.customer and account.account_type != "Receivable":
			msg = (
				_("Please ensure {} account {} is a Receivable account.").format(
					artech_engine.bold(_("Debit To")), artech_engine.bold(self.debit_to)
				)
				+ " "
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(artech_engine.get_single_value("Selling Settings", "maintain_same_sales_rate"))
	...
	def force_set_against_income_account(self):
		self.set_against_income_account()
		artech_engine.db.set_value(self.doctype, self.name, "against_income_account", self.against_income_account)
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.items():
			if artech_engine.get_single_value("Selling Settings", value[0]) == "Yes":
				if artech_engine.get_value("Customer", self.customer, value[0]):
					continue
	...
	def validate_pos(self):
		if self.is_return:
			...
			if abs(flt(self.paid_amount)) + abs(flt(self.write_off_amount)) - abs(
				...
				artech_engine.throw(_("Paid amount + Write Off Amount can not be greater than Grand Total"))

	def validate_created_using_pos(self):
		if self.is_created_using_pos and not self.pos_profile:
			artech_engine.throw(_("POS Profile is mandatory to mark this invoice as POS Transaction."))

		self.invoice_type_in_pos = artech_engine.db.get_single_value("POS Settings", "invoice_type")
		if self.invoice_type_in_pos == "POS Invoice" and not self.is_return:
			artech_engine.throw(_("Transactions using Sales Invoice in POS are disabled."))

		self.validate_pos_opening_entry()

	def validate_full_payment(self):
		allow_partial_payment = artech_engine.db.get_value("POS Profile", self.pos_profile, "allow_partial_payment")
	...
	def validate_pos_opening_entry(self):
		opening_entries = artech_engine.get_all(
			"POS Opening Entry",
		...
		if not opening_entries:
			artech_engine.throw(
				...
				msg=_("No open POS Opening Entry found for POS Profile {0}.").format(
					artech_engine.bold(self.pos_profile)
				),
			)
		...
		if artech_engine.utils.get_date_str(opening_entries[0].get("period_start_date")) != artech_engine.utils.today():
			artech_engine.throw(
	...
	def validate_warehouse(self):
		...
		for d in self.get_item_list():
			if (
				not d.warehouse
				and d.item_code
				and artech_engine.get_cached_value("Item", d.item_code, "is_stock_item")
			):
				artech_engine.throw(_("Warehouse required for stock Item {0}").format(d.item_code))
	...
	def validate_subcontracted_sales_order(self):
		if self.has_subcontracted:
			...
			if not all(
				...
				artech_engine.throw(_("All linked Sales Orders must be subcontracted."))
				...
				if item.stock_qty > max_qty:
					artech_engine.throw(
						_("Row #{0}: Stock quantity {1} ({2}) for item {3} cannot exceed {4}").format(
							...
							artech_engine.bold(max_qty),
						)
					)

	def validate_write_off_account(self):
		if flt(self.write_off_amount) and not self.write_off_account:
			self.write_off_account = artech_engine.get_cached_value("Company", self.company, "write_off_account")
	...
	def set_billing_hours_and_amount(self):
		if not self.project:
			for timesheet in self.timesheets:
				ts_doc = artech_engine.get_doc("Timesheet", timesheet.time_sheet)
	...
	def is_auto_fetch_timesheet_enabled(self):
		return artech_engine.db.get_single_value("Projects Settings", "fetch_timesheet_in_sales_invoice")
	...
	def get_warehouse(self):
		user_pos_profile = artech_engine.db.sql(
			"""select name, warehouse from `tabPOS Profile`
			where ifnull(user,'') = %s and company = %s""",
			(artech_engine.session["user"], self.company),
		)
		warehouse = user_pos_profile[0][1] if user_pos_profile else None

		if not warehouse:
			global_pos_profile = artech_engine.db.sql(
				"""select name, warehouse from `tabPOS Profile`
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if (
				d.sales_order
				and artech_engine.db.get_value("Sales Order", d.sales_order, "docstatus", cache=True) != 1
			):
				artech_engine.throw(_("Sales Order {0} is not submitted").format(d.sales_order))

			if (
				d.delivery_note
				and artech_engine.db.get_value("Delivery Note", d.delivery_note, "docstatus", cache=True) != 1
			):
	...
	def depreciate_asset_on_sale(self):
		...
		for d in self.get("items"):
			if d.asset:
				asset = artech_engine.get_doc("Asset", d.asset)
	...
	def restore_asset(self):
		...
		for d in self.get("items"):
			if d.asset:
				asset = artech_engine.get_cached_doc("Asset", d.asset)
	...
	def update_asset(self):
		...
		def _update_asset(asset, disposal_date, note, asset_status=None):
			artech_engine.db.set_value("Asset", d.asset, "disposal_date", disposal_date)
	...
	def get_disposal_date(self):
		if self.is_return:
			disposal_date = artech_engine.db.get_value("Sales Invoice", self.return_against, "posting_date")
		else:
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from artech.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = artech.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
	...
	def get_gl_entries(self, inventory_account_map=None):
		from artech.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and artech.is_perpetual_inventory_enabled(self.company):
			gl_entries += super().get_gl_entries()

	def get_gl_entries_for_fixed_asset(self, item, gl_entries):
		asset = artech_engine.get_cached_doc("Asset", item.asset)
	...
	def make_pos_gl_entries(self, gl_entries):
		if cint(self.is_pos):
			skip_change_gl_entries = not cint(
				artech_engine.get_single_value("POS Settings", "post_change_gl_entries")
			)
	...
	def get_gle_for_change_amount(self) -> list[dict]:
		...
		if not self.account_for_change_amount:
			artech_engine.throw(_("Please set Account for Change Amount"), title=_("Mandatory Field"))
	...
	def make_write_off_gl_entry(self, gl_entries):
		...
		if (
			...
			default_cost_center = artech_engine.get_cached_value("Company", self.company, "cost_center")
				...
				if not round_off_for_opening:
					artech_engine.throw(
						_(
							...
							artech_engine.bold(self.rounding_adjustment),
							artech_engine.bold("Round Off for Opening"),
							get_link_to_form("Company", self.company),
							artech_engine.bold("Disable Rounded Total"),
						)
					)
				else:
	...
	def update_billing_status_in_dn(self, update_modified=True):
		...
		for d in self.get("items"):
			if d.dn_detail:
				billed_amt = artech_engine.db.sql(
		...
		for dn in set(updated_delivery_notes):
			artech_engine.get_doc("Delivery Note", dn).update_billing_percentage(update_modified=update_modified)
	...
	def update_project(self):
		...
		for p in unique_projects:
			project = artech_engine.get_doc("Project", p)
	...
	def verify_payment_amount_is_positive(self):
		for entry in self.payments:
			if entry.amount < 0:
				artech_engine.throw(_("Row #{0} (Payment Table): Amount must be positive").format(entry.idx))

	def verify_payment_amount_is_negative(self):
		for entry in self.payments:
			if entry.amount > 0:
				artech_engine.throw(_("Row #{0} (Payment Table): Amount must be negative").format(entry.idx))
	...
	def delete_loyalty_point_entry(self):
		...
		if against_lp_entry:
			...
			artech_engine.db.sql("""delete from `tabLoyalty Point Entry` where invoice=%s""", (self.name))
			# Set loyalty program
			self.set_loyalty_program_tier()
	...
	def set_loyalty_program_tier(self):
		...
		customer = artech_engine.get_doc("Customer", self.customer)
		customer.db_set("loyalty_program_tier", lp_details.tier_name)

	def get_returned_amount(self):
		from artech_engine.query_builder.functions import Sum

		doc = artech_engine.qb.DocType(self.doctype)
		returned_amount = (
			artech_engine.qb.from_(doc)
	...
	def apply_loyalty_points(self):
		from artech.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
	...
	def set_status(self, update=False, status=None, update_modified=True):
		...
		if not status:
			if self.docstatus == 2:
				...
				elif self.is_return == 0 and artech_engine.db.get_value(
...
def get_discounting_status(sales_invoice):
	...
	invoice_discounting_list = artech_engine.db.sql(
		"""
		select status
...
def validate_inter_company_party(doctype, party, company, inter_company_reference):
	...
	if inter_company_reference:
		doc = artech_engine.get_doc(ref_doc, inter_company_reference)
		ref_party = doc.supplier if doctype in ["Sales Invoice", "Sales Order"] else doc.customer
		if artech_engine.db.get_value(partytype, {"represents_company": doc.company}, "name") != party:
			artech_engine.throw(_("Invalid {0} for Inter Company Transaction.").format(_(partytype)))
		if artech_engine.get_cached_value(ref_partytype, ref_party, "represents_company") != company:
			artech_engine.throw(_("Invalid Company for Inter Company Transaction."))

	elif artech_engine.db.get_value(partytype, {"name": party, internal: 1}, "name") == party:
		companies = artech_engine.get_all(
...
def update_linked_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		artech_engine.db.set_value(doctype, inter_company_reference, ref_field, name)
...
def unlink_inter_company_doc(doctype, name, inter_company_reference):
	...
	if inter_company_reference:
		artech_engine.db.set_value(doctype, name, ref_field, "")
		artech_engine.db.set_value(ref_doc, inter_company_reference, ref_field, "")


def get_list_context(context=None):
	from artech.controllers.website_list_for_contact import get_list_context
...
def get_inter_company_details(doc, doctype):
	if doctype in ["Sales Invoice", "Sales Order", "Delivery Note"]:
		...
		company = artech_engine.get_cached_value("Supplier", doc.supplier, "represents_company")
...
def get_internal_party(parties, link_doctype, doc):
	if len(parties) == 1:
		...
		if doc.get("company_address") or doc.get("shipping_address"):
			party = artech_engine.db.get_value(
				"Dynamic Link",
				{
...
def validate_inter_company_transaction(doc, doctype):
	...
	valid_price_list = artech_engine.db.get_value("Price List", {"name": price_list, "buying": 1, "selling": 1})
	if not valid_price_list and not doc.is_internal_transfer():
		artech_engine.throw(_("Selected Price List should have buying and selling fields checked."))

	party = details.get("party")
	if not party:
		partytype = "Supplier" if doctype in ["Sales Invoice", "Sales Order"] else "Customer"
		artech_engine.throw(_("No {0} found for Inter Company Transactions.").format(partytype))

	company = details.get("company")
	default_currency = artech_engine.get_cached_value("Company", company, "default_currency")
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		source_doc = artech_engine.get_doc(doctype, source_name)
	...
	def update_details(source_doc, target_doc, source_parent):
		...
		if target_doc.doctype in ["Purchase Invoice", "Purchase Order"]:
			currency = artech_engine.db.get_value("Supplier", details.get("party"), "default_currency")
			...
			currency = artech_engine.db.get_value("Customer", details.get("party"), "default_currency")
			target_doc.company = details.get("company")
...
def get_received_items(reference_name, doctype, reference_fieldname):
	...
	target_doctypes = artech_engine.get_all(
		doctype,
		filters=filters,
		as_list=True,
	)
	...
	received_items_map = artech_engine._dict(
		artech_engine.get_all(
			doctype + " Item",
	...
	for item in doc.get("items"):
		...
		if doc.update_stock:
			...
			if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
				item.warehouse = artech_engine.db.get_value(
...
def update_pr_items(doc, sales_item_map, purchase_item_map, parent_child_map, warehouse_map):
	for item in doc.get("items"):
		...
		if not item.warehouse and item.get("purchase_order") and item.get("purchase_order_item"):
			item.warehouse = artech_engine.db.get_value("Purchase Order Item", item.purchase_order_item, "warehouse")


def get_delivery_note_details(internal_reference):
	si_item_details = artech_engine.get_all(
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_item_details = artech_engine.get_all(
		doctype,
...
def get_loyalty_programs(customer: str):
	...
	from artech.selling.doctype.customer.customer import get_loyalty_programs

	customer = artech_engine.get_doc("Customer", customer)
...
def create_invoice_discounting(source_name: str, target_doc: str | Document | None = None):
	invoice = artech_engine.get_doc("Sales Invoice", source_name)
	invoice_discounting = artech_engine.new_doc("Invoice Discounting")
...
def update_multi_mode_option(doc, pos_profile):
	...
	if invalid_modes:
		...
		artech_engine.throw(msg.format(", ".join(invalid_modes)), title=_("Missing Account"))
...
def create_dunning(
	...
	def postprocess_dunning(source, target):
		from artech.accounts.doctype.dunning.dunning import get_dunning_letter_text

		dunning_type = artech_engine.db.exists("Dunning Type", {"is_default": 1, "company": source.company})
		if dunning_type:
			dunning_type = artech_engine.get_doc("Dunning Type", dunning_type)
...
def check_if_return_invoice_linked_with_payment_entry(self):
	...
	if not artech_engine.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
		return
	...
	payment_entries = artech_engine.db.sql_list(
		"""
		SELECT
			t1.name
		FROM
	...
	if payment_entries:
		for payment in payment_entries:
			payment_entry = artech_engine.get_doc("Payment Entry", payment)
