...
from artech_engine.utils import add_days, flt, format_date, getdate


class MainCostCenterCantBeChild(artech_engine.ValidationError):
	pass


class InvalidMainCostCenter(artech_engine.ValidationError):
	pass


class InvalidChildCostCenter(artech_engine.ValidationError):
	pass


class WrongPercentageAllocation(artech_engine.ValidationError):
	pass


class InvalidDateError(artech_engine.ValidationError):
	pass
...
class CostCenterAllocation(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.cost_center_allocation_percentage.cost_center_allocation_percentage import (
	...
	def validate_total_allocation_percentage(self):
		...
		if total_percentage != 100:
			artech_engine.throw(_("Total percentage against cost centers should be 100"), WrongPercentageAllocation)

	def validate_from_date_based_on_existing_gle(self):
		# Check if GLE exists against the main cost center
		# If exists ensure from date is set after posting date of last GLE

		last_gle_date = artech_engine.db.get_value(
			"GL Entry",
	...
	def validate_backdated_allocation(self):
		...
		future_allocation = artech_engine.db.get_value(
		...
		if future_allocation:
			artech_engine.msgprint(
				_(
					...
					artech_engine.bold(future_allocation.name),
					artech_engine.bold(format_date(future_allocation.valid_from)),
					artech_engine.bold(format_date(add_days(future_allocation.valid_from, -1))),
				),
				title=_("Warning!"),
				indicator="orange",
				alert=1,
	...
	def validate_main_cost_center(self):
		...
		parent = artech_engine.db.get_value(
	...
	def validate_child_cost_centers(self):
		...
		main_cost_centers = [
			d.main_cost_center
			for d in artech_engine.get_all("Cost Center Allocation", {"docstatus": 1}, "main_cost_center")
		]
	...
	def clear_cache(self):
		artech_engine.clear_cache(doctype="Cost Center")
