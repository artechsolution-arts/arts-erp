...
from artech.accounts.doctype.subscription.subscription import get_prorata_factor
from artech.tests.utils import ArtechTestSuite


class TestSubscription(ArtechTestSuite):
	def setUp(self):
		make_plans()
		create_parties()
		reset_settings()
	...
	def test_create_subscription_trial_with_wrong_dates(self):
		...
		self.assertRaises(artech_engine.ValidationError, subscription.save)
	...
	def test_subscription_cancel_after_grace_period(self):
		settings = artech_engine.get_single("Subscription Settings")
	...
	def test_restart_active_subscription(self):
		subscription = create_subscription()
		self.assertRaises(artech_engine.ValidationError, subscription.restart_subscription)
	...
	def test_subscription_with_follow_calendar_months(self):
		subscription = artech_engine.new_doc("Subscription")
	...
	def test_multi_currency_subscription(self):
		...
		artech_engine.db.set_value("Customer", party, "default_currency", "USD")
		subscription = create_subscription(
			start_date="2018-01-01",
			generate_invoice_at="Beginning of the current subscription period",
		...
		currency = artech_engine.db.get_value("Sales Invoice", subscription.invoices[0].name, "currency")
	...
	def test_subscription_recovery(self):
		...
		self.assertEqual(len(subscription.invoices), 2)
		self.assertEqual(
			getdate(artech_engine.db.get_value("Sales Invoice", subscription.invoices[0].name, "from_date")),
			getdate("2021-01-01"),
		)
		self.assertEqual(
			getdate(artech_engine.db.get_value("Sales Invoice", subscription.invoices[1].name, "from_date")),
			getdate("2021-02-01"),
		)
	...
	def test_subscription_auto_cancellation_uneven_cycle(self):
		...
		self.assertRaises(artech_engine.ValidationError, subscription.process, posting_date=add_days(start_date, 7))
...
def create_plan(**kwargs):
	if not artech_engine.db.exists("Subscription Plan", kwargs.get("plan_name")):
		plan = artech_engine.new_doc("Subscription Plan")
...
def create_parties():
	if not artech_engine.db.exists("Supplier", "_Test Supplier"):
		supplier = artech_engine.new_doc("Supplier")
		supplier.supplier_name = "_Test Supplier"
		supplier.supplier_group = "All Supplier Groups"
		supplier.insert()

	if not artech_engine.db.exists("Customer", "_Test Subscription Customer"):
	...
	if not artech_engine.db.exists("Customer", "_Test Subscription Customer Multi Currency"):
		customer = artech_engine.new_doc("Customer")
		customer.customer_name = "Test Subscription Customer Multi Currency"
		customer.default_currency = "USD"
		customer.insert()

	if not artech_engine.db.exists("Customer", "_Test Subscription Customer John Doe"):
		customer = artech_engine.new_doc("Customer")
