...
from artech_engine.utils.data import (
	add_days,
	add_months,
	add_to_date,
	cint,
...
from artech import get_default_company, get_default_cost_center
...
class InvoiceCancelled(artech_engine.ValidationError):
	pass


class InvoiceNotCancelled(artech_engine.ValidationError):
	pass
...
class Subscription(Document):
	...
	def validate_plans_billing_cycle(billing_cycle_data: list[dict[str, str]]) -> None:
		...
		if billing_cycle_data and len(billing_cycle_data) != 1:
			artech_engine.throw(_("You can only have Plans with the same billing cycle in a Subscription"))
	...
	def get_billing_cycle_and_interval(self) -> list[dict[str, str]]:
		...
		subscription_plan = artech_engine.qb.DocType("Subscription Plan")
		billing_info = (
			artech_engine.qb.from_(subscription_plan)
	...
	def get_status_for_past_grace_period(self) -> str:
		cancel_after_grace = cint(artech_engine.get_value("Subscription Settings", None, "cancel_after_grace"))
		status = "Unpaid"
	...
	def is_past_grace_period(self, posting_date: DateTimeLikeObject | None = None) -> bool:
		...
		grace_period = cint(artech_engine.get_value("Subscription Settings", None, "grace_period"))
	...
	def validate_party_billing_currency(self):
		...
		"""
		if self.party:
			party_billing_currency = artech_engine.get_cached_value(
				self.party_type, self.party, "default_currency"
			) or artech_engine.get_cached_value("Company", self.company, "default_currency")

			plans = [x.plan for x in self.plans]
			subscription_plan_currencies = artech_engine.db.get_all(
			...
			if unsupported_plans:
				unsupported_plans = [
					...
					).format(artech_engine.bold(party_billing_currency)),
					*unsupported_plans,
				]

				artech_engine.throw(
					unsupported_plans, artech_engine.ValidationError, "Unsupported Subscription Plans", as_list=True
				)
	...
	def validate_trial_period(self) -> None:
		...
		if self.trial_period_start and self.trial_period_end:
			if getdate(self.trial_period_end) < getdate(self.trial_period_start):
				artech_engine.throw(_("Trial Period End Date Cannot be before Trial Period Start Date"))

		if self.trial_period_start and not self.trial_period_end:
			artech_engine.throw(_("Both Trial Period Start Date and Trial Period End Date must be set"))

		if self.trial_period_start and getdate(self.trial_period_start) > getdate(self.start_date):
			artech_engine.throw(_("Trial Period Start date cannot be after Subscription Start Date"))
	...
	def validate_to_follow_calendar_months(self) -> None:
		...
		if not self.end_date:
			artech_engine.throw(_("Subscription End Date is mandatory to follow calendar months"))

		if billing_info[0]["billing_interval"] != "Month":
			artech_engine.throw(_("Billing Interval in Subscription Plan must be Month to follow calendar months"))
	...
	def create_invoice(
		...
		invoice = artech_engine.new_doc(self.invoice_document_type)
		...
		invoice.currency = artech_engine.db.get_value("Subscription Plan", {"name": self.plans[0].plan}, "currency")
	...
	def get_items_from_plans(self, plans: list[dict[str, str]], prorate: int = 0) -> list[dict]:
		...
		party = self.party
		for plan in plans:
			plan_doc = artech_engine.get_doc("Subscription Plan", plan.plan)
			...
			deferred = artech_engine.db.get_value("Item", item_code, deferred_field)
	...
	def get_current_invoice(self) -> Document | None:
		...
		if invoice:
			return artech_engine.get_doc(self.invoice_document_type, invoice[0])
	...
	def cancel_subscription(self) -> None:
		...
		if self.status == "Cancelled":
			artech_engine.throw(_("subscription is already cancelled."), InvoiceCancelled)
	...
	def restart_subscription(self, posting_date: DateTimeLikeObject | None = None) -> None:
		...
		"""
		if self.status != "Cancelled":
			artech_engine.throw(_("You cannot restart a Subscription that is not cancelled."), InvoiceNotCancelled)
	...
	def force_fetch_subscription_updates(self):
		...
		if nowdate() < self.current_invoice_start:
			artech_engine.msgprint(_("Subscription for Future dates cannot be processed."))
			return
...
def is_prorate() -> int:
	return cint(artech_engine.db.get_single_value("Subscription Settings", "prorate"))
...
def process_all(subscription: list, posting_date: DateTimeLikeObject | None = None) -> None:
	...
	for subscription_name in subscription:
		try:
			subscription = artech_engine.get_doc("Subscription", subscription_name)
