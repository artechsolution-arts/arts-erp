...
from artech_engine.utils import add_months, nowdate

from artech.accounts.doctype.accounting_period.accounting_period import (
...
class TestAccountingPeriod(ArtechTestSuite):
	...
	def test_accounting_period_exempted_role(self):
		...
		# Create users
		users = artech_engine.get_all("User", filters={"email": ["like", "test%"]}, limit=1)
		user = None

		if users[0].name:
			user = artech_engine.get_doc("User", users[0].name)
		else:
		...
		artech_engine.clear_cache(user=user.name)

		artech_engine.set_user(user.name)
...
def create_accounting_period(**args):
	...
	accounting_period = artech_engine.new_doc("Accounting Period")
