...
class TestFiscalYear(ArtechTestSuite):
	def test_extra_year(self):
		if artech_engine.db.exists("Fiscal Year", "_Test Fiscal Year 2000"):
			artech_engine.delete_doc("Fiscal Year", "_Test Fiscal Year 2000")
		...
		self.assertRaises(artech_engine.exceptions.InvalidDates, fy.insert)

	def test_company_fiscal_year_overlap(self):
		for name in ["_Test Global FY 2001", "_Test Company FY 2001"]:
			if artech_engine.db.exists("Fiscal Year", name):
				artech_engine.delete_doc("Fiscal Year", name)

		global_fy = artech_engine.new_doc("Fiscal Year")
		global_fy.year = "_Test Global FY 2001"
		global_fy.year_start_date = "2001-04-01"
		global_fy.year_end_date = "2002-03-31"
		global_fy.insert()

		company_fy = artech_engine.new_doc("Fiscal Year")
		...
		company_fy.insert()
		self.assertTrue(artech_engine.db.exists("Fiscal Year", global_fy.name))
		self.assertTrue(artech_engine.db.exists("Fiscal Year", company_fy.name))
