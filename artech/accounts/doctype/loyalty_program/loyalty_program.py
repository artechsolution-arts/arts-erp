...
class LoyaltyProgram(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.loyalty_program_collection.loyalty_program_collection import (
...
def get_loyalty_details(
	...
	LoyaltyPointEntry = artech_engine.qb.DocType("Loyalty Point Entry")

	query = (
		artech_engine.qb.from_(LoyaltyPointEntry)
		.select(
...
def get_loyalty_program_details_with_points(
	...
	loyalty_program = artech_engine.get_doc("Loyalty Program", loyalty_program)
...
def get_loyalty_program_details(
	...
	lp_details = artech_engine._dict()

	if not loyalty_program:
		loyalty_program = artech_engine.db.get_value("Customer", customer, "loyalty_program")

		if not loyalty_program and not silent:
			artech_engine.throw(_("Customer isn't enrolled in any Loyalty Program"))
		elif silent and not loyalty_program:
			return artech_engine._dict({"loyalty_programs": None})

	if not company:
		company = artech_engine.db.get_default("company") or artech_engine.get_all("Company")[0].name
...
def get_redeemption_factor(loyalty_program: str | None = None, customer: str | None = None):
	...
	if not loyalty_program:
		customer_loyalty_program = artech_engine.db.get_value("Customer", customer, "loyalty_program")
		loyalty_program = customer_loyalty_program
	if loyalty_program:
		return artech_engine.db.get_value("Loyalty Program", loyalty_program, "conversion_factor")
	else:
		artech_engine.throw(_("Customer isn't enrolled in any Loyalty Program"))
...
def validate_loyalty_points(ref_doc, points_to_redeem):
	...
	if hasattr(ref_doc, "loyalty_program") and ref_doc.loyalty_program:
		...
		loyalty_program = artech_engine.db.get_value("Customer", ref_doc.customer, ["loyalty_program"])

	if (
		loyalty_program
		and artech_engine.db.get_value("Loyalty Program", loyalty_program, ["company"]) != ref_doc.company
	):
		artech_engine.throw(_("The Loyalty Program isn't valid for the selected company"))
	...
	if loyalty_program and points_to_redeem:
		...
		if points_to_redeem > loyalty_program_details.loyalty_points:
			artech_engine.throw(_("You don't have enough Loyalty Points to redeem"))
		...
		if loyalty_amount > total_amount:
			artech_engine.throw(_("You can't redeem Loyalty Points having more value than the Total Amount."))
