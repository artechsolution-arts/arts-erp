...
from artech_engine.utils import cint, flt, getdate, today

from artech.accounts.doctype.loyalty_program.loyalty_program import (
	get_loyalty_details,
	get_loyalty_program_details_with_points,
)
from artech.accounts.party import get_dashboard_info
from artech.tests.utils import ArtechTestSuite


class TestLoyaltyProgram(ArtechTestSuite):
	def test_loyalty_points_earned_single_tier(self):
		...
		customer = artech_engine.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		artech_engine.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = artech_engine.get_doc("Customer", {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = artech_engine.get_doc(
	...
	def test_cancel_sales_invoice(self):
		...
		si.cancel()
		lpe = artech_engine.db.exists("Loyalty Point Entry", lpe.name)
	...
	def test_sales_invoice_return(self):
		...
		earned_points = get_points_earned(si_original)
		lpe_original = artech_engine.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		si_original = artech_engine.get_doc("Sales Invoice", lpe_original.invoice)

		earned_points = get_points_earned(si_original)
		lpe_after_return = artech_engine.get_doc(
			"Loyalty Point Entry",
			{
				"invoice_type": "Sales Invoice",
				"invoice": si_original.name,
		...
		for d in [si_return, si_original]:
			try:
				d.cancel()
			except artech_engine.TimestampMismatchError:
				artech_engine.get_doc("Sales Invoice", d.name).cancel()

	def test_loyalty_points_for_dashboard(self):
		doc = artech_engine.get_doc("Customer", "Test Loyalty Customer")
		company_wise_info = get_dashboard_info("Customer", doc.name, doc.loyalty_program)

		for d in company_wise_info:
			self.assertTrue(d.get("loyalty_points"))

	@unittest.mock.patch("artech.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_details")
	def test_tier_selection(self, mock_get_loyalty_details):
		# Create a new loyalty program with multiple tiers
		loyalty_program = artech_engine.get_doc(
			{
...
def get_points_earned(self):
	def get_returned_amount():
		returned_amount = artech_engine.db.sql(
			"""
	...
	return artech_engine.get_doc(
		{
			...
			"customer": artech_engine.get_doc("Customer", {"customer_name": "Test Loyalty Customer"}).name,
			"company": "_Test Company",
			"due_date": today(),
			"posting_date": today(),
			"currency": "INR",
			...
			"items": [
				{
					...
					"item_code": artech_engine.get_doc("Item", {"item_name": "Loyal Item"}).name,
					"qty": qty,
					"rate": 10000,
