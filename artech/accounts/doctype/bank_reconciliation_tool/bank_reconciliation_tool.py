...
from artech_engine.utils import cint, create_batch, flt

from artech import get_default_cost_center
from artech.accounts.doctype.bank_transaction.bank_transaction import get_total_allocated_amount
...
from artech.accounts.utils import get_account_currency, get_balance_on
...
def get_bank_transactions(
	...
	transactions = artech_engine.get_list(
		"Bank Transaction",
		fields=[
			"date",
			"deposit",
...
def get_account_balance(bank_account: str, till_date: str | date, company: str):
	...
	artech_engine.has_permission("Bank Account", "read", bank_account, throw=True)
	account = artech_engine.db.get_value("Bank Account", bank_account, "account")
...
def update_bank_transaction(
	...
	bank_transaction = artech_engine.get_doc("Bank Transaction", bank_transaction_name)
...
def create_journal_entry_bts(
	...
	bank_transaction = artech_engine.db.get_values(
		"Bank Transaction",
		bank_transaction_name,
		fieldname=["name", "deposit", "withdrawal", "bank_account", "currency"],
		as_dict=True,
	)[0]
	company_account = artech_engine.get_value("Bank Account", bank_transaction.bank_account, "account")
	account_type = artech_engine.db.get_value("Account", second_account, "account_type")
	...
	company = artech_engine.get_value("Account", company_account, "company")
	company_default_currency = artech_engine.get_cached_value("Company", company, "default_currency")
	company_account_currency = artech_engine.get_cached_value("Account", company_account, "account_currency")
	second_account_currency = artech_engine.get_cached_value("Account", second_account, "account_currency")
...
def create_payment_entry_bts(
	...
	bank_account = artech_engine.get_cached_value("Bank Account", bank_transaction.bank_account, "account")
	company = artech_engine.get_cached_value("Account", bank_account, "company")
	party_account = get_party_account(party_type, party, company)

	bank_currency = bank_transaction.currency
	party_currency = artech_engine.get_cached_value("Account", party_account, "account_currency")
	...
	if len(bank_transactions) > 10:
		for bank_transaction_batch in create_batch(bank_transactions, 1000):
			artech_engine.enqueue(
				method="artech.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.start_auto_reconcile",
				queue="long",
		...
		artech_engine.msgprint(_("Auto Reconciliation has started in the background"))
	else:
...
def start_auto_reconcile(
	...
	artech_engine.flags.auto_reconcile_vouchers = True
	...
	artech_engine.msgprint(title=_("Auto Reconciliation"), msg=alert_message, indicator=indicator)

	artech_engine.flags.auto_reconcile_vouchers = False
...
def reconcile_vouchers(bank_transaction_name: str, vouchers: str):
	...
	transaction = artech_engine.get_doc("Bank Transaction", bank_transaction_name)
...
def get_linked_payments(
	...
	transaction = artech_engine.get_doc("Bank Transaction", bank_transaction_name)
	bank_account = artech_engine.db.get_values(
...
def get_queries(
	...
	for method_name in artech_engine.get_hooks("get_matching_queries"):
		queries.extend(
			artech_engine.get_attr(method_name)(
				bank_account,
				company,
				transaction,
				document_types,
...
def get_bt_matching_query(exact_match, transaction):
	...
	bt = artech_engine.qb.DocType("Bank Transaction")

	field = "deposit" if transaction.withdrawal > 0.0 else "withdrawal"
	amount_equality = getattr(bt, field) == transaction.unallocated_amount
	amount_rank = artech_engine.qb.terms.Case().when(amount_equality, 1).else_(0)
	amount_condition = amount_equality if exact_match else getattr(bt, field) > 0.0

	ref_rank = artech_engine.qb.terms.Case().when(bt.reference_number == transaction.reference_number, 1).else_(0)
	unallocated_rank = (
		artech_engine.qb.terms.Case().when(bt.unallocated_amount == transaction.unallocated_amount, 1).else_(0)
	)

	party_condition = (
		(bt.party_type == transaction.party_type) & (bt.party == transaction.party) & bt.party.isnotnull()
	)
	party_rank = artech_engine.qb.terms.Case().when(party_condition, 1).else_(0)

	query = (
		artech_engine.qb.from_(bt)
		.select(
...
def get_pe_matching_query(
	...
	ref_rank = artech_engine.qb.terms.Case().when(ref_condition, 1).else_(0)
	...
	if artech_engine.flags.auto_reconcile_vouchers is True:
...
def get_je_matching_query(
	...
	ref_rank = artech_engine.qb.terms.Case().when(subquery.reference_no == transaction.reference_number, 1).else_(0)
...
def get_pi_matching_query(exact_match, currency, common_filters):
	...
	query = (
		artech_engine.qb.from_(purchase_invoice)
		.select(
