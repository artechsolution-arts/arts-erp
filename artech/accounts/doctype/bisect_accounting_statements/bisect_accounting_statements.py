...
from artech_engine.utils.data import guess_date_format
...
class BisectAccountingStatements(Document):
		...
		if getdate(self.from_date) > getdate(self.to_date):
			artech_engine.throw(
				_("From Date: {0} cannot be greater than To date: {1}").format(
					artech_engine.bold(self.from_date), artech_engine.bold(self.to_date)
				)
			)

	def bfs(self, from_date: datetime, to_date: datetime):
		# Make Root node
		node = artech_engine.new_doc("Bisect Nodes")
		node.root = None
		...
		while period_queue:
			...
			if delta.days == 0:
				...
				left_node = artech_engine.new_doc("Bisect Nodes")
				...
				right_node = artech_engine.new_doc("Bisect Nodes")
	...
	def build_tree(self):
		artech_engine.db.delete("Bisect Nodes")
		...
		root = artech_engine.db.get_all("Bisect Nodes", filters={"root": ["is", "not set"]})[0]
	...
	def get_report_summary(self):
		...
		pl_summary = artech_engine.get_doc("Report", "Profit and Loss Statement")
		self.p_l_summary = pl_summary.execute_script_report(filters=filters)[5]
		bs_summary = artech_engine.get_doc("Report", "Balance Sheet")
	...
	def update_node(self):
		current_node = artech_engine.get_doc("Bisect Nodes", self.current_node)
	...
	def current_node_has_summary_info(self):
		"Assertion method"
		return artech_engine.db.get_value("Bisect Nodes", self.current_node, "generated")
	...
	def bisect_left(self):
		if self.current_node is not None:
			cur_node = artech_engine.get_doc("Bisect Nodes", self.current_node)
			if cur_node.left_child is not None:
				lft_node = artech_engine.get_doc("Bisect Nodes", cur_node.left_child)
				...
				artech_engine.msgprint(_("No more children on Left"))
	...
	def bisect_right(self):
		if self.current_node is not None:
			...
			if cur_node.right_child is not None:
				rgt_node = artech_engine.get_doc("Bisect Nodes", cur_node.right_child)
				...
				artech_engine.msgprint(_("No more children on Right"))
	...
	def move_up(self):
		if self.current_node is not None:
			...
			if cur_node.root is not None:
				root = artech_engine.get_doc("Bisect Nodes", cur_node.root)
				...
				artech_engine.msgprint(_("Reached Root"))
