...
from artech.selling.page.point_of_sale.point_of_sale import get_items
...
class TestPOSClosingEntry(ArtechTestSuite):
	def setUp(self):
	...
	def test_cancelling_of_pos_closing_entry(self):
		...
		self.assertRaises(artech_engine.ValidationError, pos_inv1.cancel)

		si_doc = artech_engine.get_doc("Sales Invoice", pos_inv1.consolidated_invoice)
		self.assertRaises(artech_engine.ValidationError, si_doc.cancel)

		pcv_doc.load_from_db()
		pcv_doc.cancel()

		cancelled_invoice = artech_engine.db.get_value(
			"POS Invoice Merge Log", {"pos_closing_entry": pcv_doc.name}, "consolidated_invoice"
		)
		docstatus = artech_engine.db.get_value("Sales Invoice", cancelled_invoice, "docstatus")
	...
	def test_pos_closing_for_required_accounting_dimension_in_pos_profile(self):
		...
		location = artech_engine.get_doc("Accounting Dimension", "Location")
		location.dimension_defaults[0].mandatory_for_bs = True
		location.save()

		pos_profile = make_pos_profile(do_not_insert=1, do_not_set_accounting_dimension=1)

		self.assertRaises(artech_engine.ValidationError, pos_profile.insert)

		pos_profile.location = "Block 1"
		pos_profile.insert()
		self.assertTrue(artech_engine.db.exists("POS Profile", pos_profile.name))
		...
		accounting_dimension_department = artech_engine.get_doc("Accounting Dimension", {"name": "Department"})
		...
		pcv_doc.flags.in_test = True
		self.assertRaises(artech_engine.ValidationError, pcv_doc.submit)

		accounting_dimension_department = artech_engine.get_doc(
	...
	def test_merging_into_sales_invoice_for_batched_item(self):
		artech_engine.flags.print_message = False
		...
		piv_merge = artech_engine.db.get_value("POS Invoice Merge Log", {"pos_closing_entry": pcv_doc.name}, "name")

		self.assertTrue(piv_merge)
		piv_merge_doc = artech_engine.get_doc("POS Invoice Merge Log", piv_merge)
		...
		batch_qty = artech_engine.db.get_value("Batch", batch_no, "batch_qty")
		self.assertEqual(batch_qty, 0.0)

		batch_qty_with_pos = get_batch_qty(batch_no, "_Test Warehouse - _TC", item_code)
		self.assertEqual(batch_qty_with_pos, 0.0)

		artech_engine.flags.print_message = True
	...
	@ArtechTestSuite.change_settings("POS Settings", {"invoice_type": "Sales Invoice"})
	...
	def test_sales_invoice_in_pos_invoice_mode(self):
		...
		with self.change_settings("POS Settings", {"invoice_type": "Sales Invoice"}):
			...
			self.assertRaises(artech_engine.ValidationError, pos_inv.save)
		...
		with self.change_settings("POS Settings", {"invoice_type": "POS Invoice"}):
			...
			self.assertRaises(artech_engine.ValidationError, pos_si3.save)
	...
	def test_pos_invoice_in_sales_invoice_mode(self):
		...
		with self.change_settings("POS Settings", {"invoice_type": "POS Invoice"}):
			...
			self.assertRaises(artech_engine.ValidationError, pos_sinv.save)
		...
		with self.change_settings("POS Settings", {"invoice_type": "Sales Invoice"}):
			...
			self.assertRaises(artech_engine.ValidationError, pos_inv3.save)
			...
			pos_rinv2_si = artech_engine.get_doc("Sales Invoice", pos_rinv2.consolidated_invoice)
