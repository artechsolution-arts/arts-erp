...
from artech_engine.query_builder import functions as fn
...
from artech.controllers.status_updater import StatusUpdater
...
class POSClosingEntry(StatusUpdater):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.pos_closing_entry_detail.pos_closing_entry_detail import (
			POSClosingEntryDetail,
		)
		from artech.accounts.doctype.pos_closing_entry_taxes.pos_closing_entry_taxes import (
			POSClosingEntryTaxes,
		)
		from artech.accounts.doctype.pos_invoice_reference.pos_invoice_reference import POSInvoiceReference
		from artech.accounts.doctype.sales_invoice_reference.sales_invoice_reference import (
	...
	def set_posting_date_and_time(self):
		if self.posting_date:
			self.posting_date = artech_engine.utils.nowdate()
		if self.posting_time:
			self.posting_time = artech_engine.utils.nowtime()

	def fetch_invoice_type(self):
		self.invoice_type = artech_engine.db.get_single_value("POS Settings", "invoice_type")

	def validate_pos_opening_entry(self):
		if artech_engine.db.get_value("POS Opening Entry", self.pos_opening_entry, "status") != "Open":
			artech_engine.throw(_("Selected POS Opening Entry should be open."), title=_("Invalid Opening Entry"))
	...
	def validate_invoice_mode(self):
		...
		if self.invoice_type == "Sales Invoice":
			if len(self.pos_invoices) != 0:
				artech_engine.throw(_("POS Invoices can't be added when Sales Invoice is enabled"))
	...
	def validate_duplicate_pos_invoices(self):
		...
		for key, value in pos_occurences.items():
			if len(value) > 1:
				error_list.append(
					_("{0} is added multiple times on rows: {1}").format(artech_engine.bold(key), artech_engine.bold(value))
				)

		if error_list:
			artech_engine.throw(error_list, title=_("Duplicate POS Invoices found"), as_list=True)

	def validate_pos_invoices(self):
		invalid_rows = []
		for d in self.pos_invoices:
			invalid_row = {"idx": d.idx}
			pos_invoice = artech_engine.db.get_values(
				"POS Invoice",
				d.pos_invoice,
			...
			if pos_invoice.pos_profile != self.pos_profile:
				invalid_row.setdefault("msg", []).append(
					_("POS Profile doesn't match {}").format(artech_engine.bold(self.pos_profile))
				)
			if pos_invoice.docstatus != 1:
				invalid_row.setdefault("msg", []).append(_("POS Invoice is not submitted"))
			if pos_invoice.owner != self.user:
				invalid_row.setdefault("msg", []).append(
					_("POS Invoice isn't created by user {}").format(artech_engine.bold(self.owner))
				)
		...
		artech_engine.throw(error_list, title=_("Invalid POS Invoices"), as_list=True)
	...
	def validate_duplicate_sales_invoices(self):
		...
		if error_list:
			artech_engine.throw(error_list, title=_("Duplicate Sales Invoices found"), as_list=True)

	def validate_sales_invoices(self):
		invalid_rows = []
		for d in self.sales_invoices:
			invalid_row = {"idx": d.idx}
			sales_invoice = artech_engine.db.get_values(
				"Sales Invoice",
				d.sales_invoice,
				[
					"pos_profile",
			...
			if sales_invoice.owner != self.user:
				invalid_row.setdefault("msg", []).append(
					_("Sales Invoice isn't created by user {}").format(artech_engine.bold(self.owner))
				)
		...
		artech_engine.throw(error_list, title=_("Invalid Sales Invoices"), as_list=True)
	...
	def update_opening_entry(self, for_cancel=False):
		opening_entry = artech_engine.get_doc("POS Opening Entry", self.pos_opening_entry)
	...
	def check_pce_is_cancellable(self):
		if artech_engine.db.exists("POS Opening Entry", {"pos_profile": self.pos_profile, "status": "Open"}):
			artech_engine.throw(
				...
				).format(artech_engine.bold(self.pos_profile)),
			)
...
def get_cashiers(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	cashiers_list = artech_engine.get_all("POS Profile User", filters=filters, fields=["user"], as_list=1)
	return [c for c in cashiers_list]


@artech_engine.whitelist()
def get_invoices(start: str | datetime, end: str | datetime, pos_profile: str, user: str):
	invoice_doctype = artech_engine.db.get_single_value("POS Settings", "invoice_type")
...
def get_payments(invoices):
	...
	query = (
		artech_engine.qb.from_(SalesInvoicePayment)
		.where(
...
def get_taxes(invoices):
	...
	query = (
		artech_engine.qb.from_(SalesInvoiceTaxesCharges)
		.where(
...
def make_closing_entry_from_opening(opening_entry):
	closing_entry = artech_engine.new_doc("POS Closing Entry")
	closing_entry.pos_opening_entry = opening_entry.name
	closing_entry.period_start_date = opening_entry.period_start_date
	closing_entry.period_end_date = artech_engine.utils.get_datetime()
	...
	sales_invoices = []
	taxes = [
		artech_engine._dict({"account_head": tx.account_head, "amount": tx.tax_amount}) for tx in data.get("taxes")
	]
	payments = [
		artech_engine._dict(
			{
	...
	for d in data.get("invoices"):
		...
		invoice_data = artech_engine._dict(
			{
				invoice: d.name,
...
def build_invoice_query(invoice_doctype, user, pos_profile, start, end):
	...
	query = (
		artech_engine.qb.from_(InvoiceDocType)
		.select(
			InvoiceDocType.name,
