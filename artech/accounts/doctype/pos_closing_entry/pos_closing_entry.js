artech_engine.ui.form.on("POS Closing Entry", {
	onload: async function (frm) {
		...
		if (frm.doc.docstatus === 0 && !frm.doc.amended_from)
			frm.set_value("period_end_date", artech_engine.datetime.now_datetime());

		artech_engine.realtime.on("closing_process_complete", async function (data) {
	...
	refresh: function (frm) {
		if (frm.doc.docstatus == 1 && frm.doc.status == "Failed") {
			...
			$("#jump_to_error").on("click", (e) => {
				e.preventDefault();
				artech_engine.utils.scroll_to(cur_frm.get_field("error_message").$wrapper, true, 30);
			});
	...
	pos_opening_entry(frm) {
		if (
			...
			artech_engine.run_serially([
				() => artech_engine.dom.freeze(__("Loading Invoices! Please Wait...")),
	...
	get_invoices(frm) {
		return artech_engine.call({
			method: "artech.accounts.doctype.pos_closing_entry.pos_closing_entry.get_invoices",
			args: {
				start: artech_engine.datetime.get_datetime_as_string(frm.doc.period_start_date),
				end: artech_engine.datetime.get_datetime_as_string(frm.doc.period_end_date),
...
artech_engine.ui.form.on("POS Closing Entry Detail", {
	closing_amount: (frm, cdt, cdn) => {
		const row = locals[cdt][cdn];
		artech_engine.model.set_value(cdt, cdn, "difference", flt(row.closing_amount - row.expected_amount));
	},
});
