...
artech_engine.ui.form.on("Cost Center", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.doc.__islocal) {
			frm.add_custom_button(__("Chart of Cost Centers"), function () {
				artech_engine.set_route("Tree", "Cost Center");
			});

			frm.add_custom_button(__("Budget"), function () {
				artech_engine.set_route("List", "Budget", { cost_center: frm.doc.name });
			});
		}
	},
	update_cost_center_number: function (frm) {
		var d = new artech_engine.ui.Dialog({
			...
			primary_action: function () {
				...
				artech_engine.call({
					method: "artech.accounts.utils.update_cost_center",
					args: {
					...
					callback: function (r) {
						...
						if (!r.exc) {
							if (r.message) {
								artech_engine.set_route("Form", "Cost Center", r.message);
							} else {
	...
	parent_cost_center(frm) {
		if (!frm.doc.company) {
			artech_engine.msgprint(__("Please enter company name first"));
		}
	},
