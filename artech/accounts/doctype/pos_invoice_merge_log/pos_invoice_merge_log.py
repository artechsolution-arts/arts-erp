...
from artech_engine.utils import cint, flt, get_time, getdate, nowdate, nowtime
...
class POSInvoiceMergeLog(Document):
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			status, docstatus, is_return, return_against = artech_engine.db.get_value(
				"POS Invoice", d.pos_invoice, ["status", "docstatus", "is_return", "return_against"]
			)

			bold_pos_invoice = artech_engine.bold(d.pos_invoice)
			bold_status = artech_engine.bold(status)
			if docstatus != 1:
				artech_engine.throw(
			...
			if (
				...
				bold_return_against = artech_engine.bold(return_against)
				return_against_status = artech_engine.db.get_value("POS Invoice", return_against, "status")
	...
	def on_submit(self):
		pos_invoice_docs = [artech_engine.get_cached_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices]
	...
	def distinguish_return_pos_invoices(self, data, sales_invoice_doc=None):
		...
		for doc in data:
			sales_invoices_of_return_against = artech_engine.db.get_value(
	...
	def merge_pos_invoice_into(self, invoice, data):
		...
		for doc in data:
			old_new_item_map = artech_engine._dict()
			old_new_tax_map = artech_engine._dict()
		...
		invoice.disable_rounded_total = cint(
			artech_engine.db.get_value("POS Profile", invoice.pos_profile, "disable_rounded_total")
		)
			...
			if not dimension_value and (dimension.mandatory_for_pl or dimension.mandatory_for_bs):
				artech_engine.throw(
					_("Please set Accounting Dimension {} in {}").format(
						artech_engine.bold(dimension.label),
						artech_engine.get_desk_link("POS Profile", invoice.pos_profile),
					)
				)
	...
	def get_new_sales_invoice(self):
		sales_invoice = artech_engine.new_doc("Sales Invoice")
	...
	def serial_and_batch_bundle_reference_for_pos_invoice(self):
		for d in self.pos_invoices:
			pos_invoice = artech_engine.get_doc("POS Invoice", d.pos_invoice)
	...
	def cancel_linked_invoices(self):
		...
		for si_name in invoices:
			if not si_name:
				continue
			si = artech_engine.get_doc("Sales Invoice", si_name)
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = artech_engine.db.get_all(
		"POS Invoice",
		filters=filters,
		fields=[
			"name as pos_invoice",
...
def split_invoices_by_accounting_dimension(pos_invoices):
	...
	for invoice in pos_invoices:
		...
		accounting_dimensions = artech_engine.db.get_value(
...
def consolidate_pos_invoices(pos_invoices=None, closing_entry=None):
	...
	if artech_engine.in_test and not invoices:
...
def unconsolidate_pos_invoices(closing_entry):
	merge_logs = artech_engine.get_all(
...
def split_invoices(invoices):
	...
	special_invoices = []
	pos_return_docs = [
		artech_engine.get_cached_doc("POS Invoice", d.pos_invoice)
		for d in invoices
	...
	for pos_invoice in pos_return_docs:
		for item in pos_invoice.items:
			...
			return_against_is_consolidated = (
				artech_engine.db.get_value("POS Invoice", pos_invoice.return_against, "status", cache=True)
				== "Consolidated"
			)
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	...
	if artech_engine.in_test:
		...
		try:
			...
			message_log = artech_engine.message_log.pop() if artech_engine.message_log else str(e)
			...
			artech_engine.publish_realtime("closing_process_complete", user=artech_engine.session.user)


def cancel_merge_logs(merge_logs, closing_entry=None):
	def merge_cancel_and_close():
		for log in merge_logs:
			merge_log = artech_engine.get_doc("POS Invoice Merge Log", log)
	...
	if artech_engine.flags.in_test:
...
def enqueue_job(job, **kwargs):
	...
	if not is_job_enqueued(job_id):
		...
		artech_engine.msgprint(msg, alert=1)


def check_scheduler_status():
	if is_scheduler_inactive() and not artech_engine.in_test:
		artech_engine.throw(_("Scheduler is inactive. Cannot enqueue job."), title=_("Scheduler Inactive"))
