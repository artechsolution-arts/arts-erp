...
from artech_engine.utils import cint, flt, fmt_money, getdate
...
class BankClearance(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.bank_clearance_detail.bank_clearance_detail import (
			BankClearanceDetail,
		)
	...
	def get_payment_entries(self):
		...
		if not self.account:
			artech_engine.throw(_("Account is mandatory to get payment entries"))

		entries = []

		# get entries from all the apps
		precision = cint(artech_engine.db.get_default("currency_precision")) or 2
		for method_name in artech_engine.get_hooks("get_payment_entries_for_bank_clearance"):
			entries += (
		...
		default_currency = artech.get_default_currency()
	...
	def update_clearance_date(self):
		...
		for d in entries_to_update:
			if d.payment_document == "Sales Invoice":
				old_clearance_date = artech_engine.db.get_value(
				...
				if d.clearance_date or old_clearance_date:
					...
					sales_invoice = artech_engine.get_lazy_doc("Sales Invoice", d.payment_entry)
				...
				payment_entry = artech_engine.get_lazy_doc(d.payment_document, d.payment_entry)
...
def get_payment_entries_for_bank_clearance(
	...
	journal_entry_account = artech_engine.qb.DocType("Journal Entry Account")

	journal_entry_query = (
		artech_engine.qb.from_(journal_entry_account)
	...
	if include_pos_transactions:
		si_payment = artech_engine.qb.DocType("Sales Invoice Payment")
		si = artech_engine.qb.DocType("Sales Invoice")

		pos_sales_invoices_query = (
			artech_engine.qb.from_(si_payment)
			.inner_join(si)
