...
from artech_engine.model.mapper import map_child_doc, map_doc
from artech_engine.query_builder.functions import IfNull, Sum
from artech_engine.utils import cint, flt, get_link_to_form, getdate, nowdate
from artech_engine.utils.nestedset import get_descendants_of

from artech.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
...
from artech.accounts.party import get_due_date, get_party_account
from artech.controllers.queries import item_query as _item_query
from artech.controllers.sales_and_purchase_return import get_sales_invoice_item_from_consolidated_invoice
...
class ProductBundleStockValidationError(artech_engine.ValidationError):
	pass
...
class POSInvoice(SalesInvoice):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.pos_invoice_item.pos_invoice_item import POSInvoiceItem
	...
	def validate(self):
		if not self.customer:
			artech_engine.throw(_("Please select Customer first"))

		if not cint(self.is_pos):
			artech_engine.throw(
				_("POS Invoice should have the field {0} checked.").format(artech_engine.bold(_("Include Payment")))
			)
	...
	def on_submit(self):
		...
		if not self.is_return and self.loyalty_program:
			...
			against_psi_doc = artech_engine.get_doc("POS Invoice", self.return_against)
	...
	def before_cancel(self):
		if (
			...
			and artech_engine.db.get_value("Sales Invoice", self.consolidated_invoice, "docstatus") == 1
		):
	...
	def clear_unallocated_mode_of_payments(self):
		...
		sip = artech_engine.qb.DocType("Sales Invoice Payment")
		artech_engine.qb.from_(sip).delete().where(sip.parent == self.name).where(sip.amount == 0).run()
	...
	def create_return_sales_invoice(self):
		return_sales_invoice = artech_engine.new_doc("Sales Invoice")
		...
		return_sales_invoice.return_against = artech_engine.db.get_value(
	...
	def delink_serial_and_batch_bundle(self):
		for row in self.items:
			if row.serial_and_batch_bundle:
				...
				artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle).cancel()
				row.db_set("serial_and_batch_bundle", None)

	def submit_serial_batch_bundle(self, table_name):
		for item in self.get(table_name):
			if item.serial_and_batch_bundle:
				doc = artech_engine.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
	...
	def check_phone_payments(self):
		for pay in self.payments:
			if pay.type == "Phone" and pay.amount >= 0:
				paid_amt = artech_engine.db.get_value(
					"Payment Request",
					filters=dict(
	...
	def validate_stock_availablility(self):
		...
		if self.docstatus.is_draft() and not artech_engine.db.get_value(
		...
		for d in self.get("items"):
			if not d.serial_and_batch_bundle:
				if artech_engine.db.exists("Product Bundle", d.item_code):
					(
						availability,
						is_stock_item,
				...
				if isinstance(availability, list):
						...
						if flt(item["available"]) < flt(item["required"]):
							error_msgs.append(
								_("<li>Packed Item {0}: Required {1}, Available {2}</li>").format(
									...
									artech_engine.bold(flt(item["required"], 2)),
									artech_engine.bold(flt(item["available"], 2)),
								)
							)
					...
					if error_msgs:
						artech_engine.throw(
							_(
								...
								artech_engine.bold(d.warehouse),
								"<br>".join(error_msgs),
							),
					...
					item_code, warehouse = artech_engine.bold(d.item_code), artech_engine.bold(d.warehouse)
					if is_stock_item and flt(availability) <= 0:
						...
						artech_engine.throw(
							_("Row #{0}: Item {1} in warehouse {2}: Available {3}, Needed {4}.").format(
								...
								item_code,
								warehouse,
								artech_engine.bold(flt(availability, 2)),
								artech_engine.bold(flt(d.stock_qty, 2)),
							),
	...
	def validate_is_pos_using_sales_invoice(self):
		...
		if self.invoice_type_in_pos == "Sales Invoice" and not self.is_return:
			artech_engine.throw(_("Sales Invoice mode is activated in POS. Please create Sales Invoice instead."))
	...
	def validate_serialised_or_batched_item(self):
		...
		if error_msg:
			artech_engine.throw(error_msg, title=_("Serial / Batch Bundle Missing"), as_list=True)
		...
		for d in self.get("items"):
			if d.get("qty") > 0:
				artech_engine.throw(
					...
					).format(d.idx, artech_engine.bold(d.item_code)),
					title=_("Invalid Item"),
				)
			if d.get("serial_no"):
				serial_nos = get_serial_nos(d.serial_no)
				for sr in serial_nos:
					serial_no_exists = artech_engine.db.sql(
						"""
						SELECT name
					...
					if not serial_no_exists:
						bold_return_against = artech_engine.bold(self.return_against)
						bold_serial_no = artech_engine.bold(sr)
						artech_engine.throw(
							_(
	...
	def validate_change_account(self):
		if (
			...
			and artech_engine.get_cached_value("Account", self.account_for_change_amount, "company") != self.company
		):
			artech_engine.throw(
	...
	def validate_change_amount(self):
		...
		if flt(self.change_amount) and not self.account_for_change_amount:
			artech_engine.msgprint(_("Please enter Account for Change Amount"), raise_exception=1)
	...
	def validate_company_with_pos_company(self):
		if self.company != artech_engine.db.get_value("POS Profile", self.pos_profile, "company"):
			artech_engine.throw(
				_("Company {} does not match with POS Profile Company {}").format(
					self.company, artech_engine.db.get_value("POS Profile", self.pos_profile, "company")
				)
			)
	...
	def validate_loyalty_transaction(self):
		if self.redeem_loyalty_points and (
			...
			expense_account, cost_center = artech_engine.db.get_value(
		...
		if not status:
			if self.docstatus == 2:
				...
				if self.consolidated_invoice:
					...
					and artech_engine.db.get_value(
	...
	def set_pos_fields(self, for_validate=False):
		...
		if not self.pos_profile:
			...
			if not pos_profile:
				artech_engine.throw(_("No POS Profile found. Please create a New POS Profile first"))
			self.pos_profile = pos_profile.get("name")

		profile = {}
		if self.pos_profile:
			profile = artech_engine.get_doc("POS Profile", self.pos_profile)
		...
		if profile:
			...
			if self.customer:
				customer_price_list, customer_group, customer_currency = artech_engine.db.get_value(
					"Customer", self.customer, ["default_price_list", "customer_group", "default_currency"]
				)
				customer_group_price_list = artech_engine.get_cached_value(
	...
	def set_missing_values(self, for_validate: bool | None = False):
		...
		if not self.debit_to:
			...
			self.party_account_currency = artech_engine.get_cached_value(
	...
	def create_payment_request(self):
		for pay in self.payments:
			if pay.type == "Phone":
				if pay.amount <= 0:
					artech_engine.throw(_("Payment amount cannot be less than or equal to 0"))

				if not self.contact_mobile:
					artech_engine.throw(_("Please enter the phone number first"))
	...
	def get_new_payment_request(self, mop):
		payment_gateway_account = artech_engine.db.get_value(
	...
	def get_existing_payment_request(self, pay):
		...
		pr = artech_engine.db.get_value("Payment Request", filters=filters)
		if pr:
			return artech_engine.get_doc("Payment Request", pr)

	@artech_engine.whitelist()
	def update_payments(self, payments: list):
		if self.status == "Consolidated":
			artech_engine.throw(_("Create Payment Entry for Consolidated POS Invoices."))

		paid_amount = flt(self.paid_amount)
		total = flt(self.rounded_total) or flt(self.grand_total)

		if paid_amount >= total:
			artech_engine.throw(title=_("Invoice Paid"), msg=_("This invoice has already been paid."))
		...
		for d in payments:
			idx += 1
			payment = create_payments_on_invoice(self, idx, artech_engine._dict(d))
		...
		pi = artech_engine.qb.DocType("POS Invoice")
		query = (
			artech_engine.qb.update(pi)
...
def get_stock_availability(item_code: str | None, warehouse: str):
	if artech_engine.db.get_value("Item", item_code, "is_stock_item"):
		is_stock_item = True
		...
		is_stock_item = True
		if artech_engine.db.exists("Product Bundle", {"name": item_code, "disabled": 0}):
...
def get_product_bundle_stock_availability(item_code, warehouse, item_qty):
	is_stock_item = True
	bundle = artech_engine.get_doc("Product Bundle", item_code)
	availabilities = []
	for bundle_item in bundle.items:
		if artech_engine.get_value("Item", bundle_item.item_code, "is_stock_item"):
...
def get_bundle_availability(bundle_item_code, warehouse):
	product_bundle = artech_engine.get_doc("Product Bundle", bundle_item_code)
	...
	for item in product_bundle.items:
		...
		if bundle_bin_qty > max_available_bundles and artech_engine.get_value(
...
def get_pos_reserved_qty_from_table(child_table, item_code, warehouse):
	...
	"""
	p_inv = artech_engine.qb.DocType("POS Invoice")
	p_item = artech_engine.qb.DocType(child_table)

	qty_column = "qty" if child_table == "Packed Item" else "stock_qty"

	reserved_qty = (
		artech_engine.qb.from_(p_inv)
		.from_(p_item)
...
def make_merge_log(invoices: str | list):
	...
	if len(invoices) == 0:
		artech_engine.throw(_("At least one invoice has to be selected."))

	merge_log = artech_engine.new_doc("POS Invoice Merge Log")
	merge_log.posting_date = getdate(nowdate())
	for inv in invoices:
		inv_data = artech_engine.db.get_values(
...
def item_query(
	...
	if pos_profile := filters.get("pos_profile")[1]:
		pos_profile = artech_engine.get_cached_doc("POS Profile", pos_profile)
...
def create_payments_on_invoice(doc, idx, payment_details):
	...
	payment = artech_engine.new_doc("Sales Invoice Payment")
	payment.idx = idx
