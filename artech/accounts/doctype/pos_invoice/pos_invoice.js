...
artech.accounts.pos.setup("POS Invoice");
artech.selling.POSInvoiceController = class POSInvoiceController extends artech.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		if (doc.__islocal && doc.is_pos && artech_engine.get_route_str() !== "point-of-sale") {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "artech.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
	...
	make_sales_return() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	async fetch_pos_payment_methods() {
		...
		const pos_profile_doc = await artech_engine.db.get_doc("POS Profile", pos_profile);
		const data = [];
...
extend_cscript(cur_frm.cscript, new artech.selling.POSInvoiceController({ frm: cur_frm }));

artech_engine.ui.form.on("POS Invoice", {
	...
	request_for_payment: function (frm) {
		if (!frm.doc.contact_mobile) {
			artech_engine.throw(__("Please enter mobile number first."));
		}
		frm.dirty();
		frm.save().then(() => {
			artech_engine.dom.freeze(__("Waiting for payment..."));
			artech_engine
				.call({
				...
				.fail(() => {
					...
					artech_engine.msgprint(__("Payment request failed"));
				})
				.then(({ message }) => {
					...
					setTimeout(() => {
						artech_engine.db
							...
							.then(({ message }) => {
								...
								} else if (artech_engine.dom.freeze_count != 0) {
									artech_engine.dom.unfreeze();
									frm.reload_doc();
