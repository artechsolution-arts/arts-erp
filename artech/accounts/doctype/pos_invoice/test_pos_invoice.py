...
from artech.accounts.doctype.pos_invoice.pos_invoice import make_sales_return
from artech.accounts.doctype.pos_profile.test_pos_profile import make_pos_profile
from artech.accounts.doctype.sales_invoice.sales_invoice import PartialPaymentValidationError
...
class POSInvoiceTestMixin(ArtechTestSuite):
	def setUp(self):
		from artech.accounts.doctype.pos_closing_entry.test_pos_closing_entry import init_user_and_profile
...
class TestPOSInvoice(POSInvoiceTestMixin):
	...
	def test_change_naming_series(self):
		...
		self.assertRaises(artech_engine.CannotChangeConstantError, inv.save)
	...
	def test_partial_pos_returns(self):
		...
		bundle_id = artech_engine.get_doc(
	...
	def test_without_payment(self):
		...
		inv.payments = []
		self.assertRaises(artech_engine.ValidationError, inv.insert)
	...
	def test_serialized_item_transaction(self):
		...
		pos2.insert()
		self.assertRaises(artech_engine.ValidationError, pos2.submit)
	...
	def test_value_error_on_serial_no_validation(self):
		...
		artech_engine.db.set_value("POS Invoice", pos.name, "docstatus", 2)
	...
	def test_loyalty_points(self):
		...
		artech_engine.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Single Loyalty")
	...
	def test_pos_batch_reservation(self):
		...
		batches = get_auto_batch_nos(
			artech_engine._dict({"item_code": "_BATCH ITEM Test For Reserve", "warehouse": "_Test Warehouse - _TC"})
		)
	...
	def test_pos_batch_reservation_with_return_qty(self):
		...
		batches = get_auto_batch_nos(
			artech_engine._dict({"item_code": "_Batch Item Reserve Return", "warehouse": "_Test Warehouse - _TC"})
		)
	...
	def test_pos_batch_item_qty_validation(self):
		...
		item = artech_engine.get_doc("Item", "_BATCH ITEM")
		...
		sn_doc = SerialBatchCreation(
			{
				...
				"qty": 2,
				"avg_rate": 300,
				"batches": artech_engine._dict({"TestBatch 01": 2}),
	...
	def test_ignore_pricing_rule(self):
		from artech.accounts.doctype.pricing_rule.test_pricing_rule import make_pricing_rule
	...
	def test_delivered_serial_no_case(self):
		from artech.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
	...
	def test_bundle_stock_availability_validation(self):
		from artech.accounts.doctype.pos_invoice.pos_invoice import ProductBundleStockValidationError
		from artech.accounts.doctype.pos_invoice_merge_log.test_pos_invoice_merge_log import (
			init_user_and_profile,
		)
		...
		if not artech_engine.db.exists("Item", sub_item_a):
			create_item(
		...
		if not artech_engine.db.exists("Item", sub_item_b):
			create_item(
		...
		if not artech_engine.db.exists("Item", bundle_item):
			create_item(
				item_code=bundle_item,
				is_stock_item=0,
			)

		if not artech_engine.db.exists("Product Bundle", bundle_item):
...
def create_pos_invoice(**args):
	...
	pos_inv = artech_engine.new_doc("POS Invoice")
	pos_inv.update(args)
	...
	pos_inv.posting_date = args.posting_date or artech_engine.utils.nowdate()
