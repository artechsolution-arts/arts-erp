...
class AccountCategory(Document):
	...
	def after_rename(self, old_name, new_name, merge):
		...
		row = artech_engine.qb.DocType("Financial Report Row")
		rows = artech_engine._dict(
			artech_engine.qb.from_(row)
...
def import_account_categories(template_path: str):
	...
	with open(categories_file) as f:
		categories = json.load(f, object_hook=artech_engine._dict)
...
def create_account_categories(categories: list[dict]):
	...
	existing_categories = set(artech_engine.get_all(DOCTYPE, pluck="name"))
	new_categories = []
