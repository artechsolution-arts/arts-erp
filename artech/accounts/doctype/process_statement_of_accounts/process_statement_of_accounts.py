...
from artech_engine.utils import add_days, add_months, format_date, getdate, today
from artech_engine.utils.jinja import validate_template
from artech_engine.utils.pdf import get_pdf
from artech_engine.www.printview import get_print_style

from artech import get_company_currency
from artech.accounts.party import get_party_account_currency
from artech.accounts.report.accounts_receivable.accounts_receivable import execute as get_ar_soa
from artech.accounts.report.accounts_receivable_summary.accounts_receivable_summary import (
	execute as get_ageing,
)
from artech.accounts.report.general_ledger.general_ledger import execute as get_soa
...
class ProcessStatementOfAccounts(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.process_statement_of_accounts_cc.process_statement_of_accounts_cc import (
			ProcessStatementOfAccountsCC,
		)
		from artech.accounts.doctype.process_statement_of_accounts_customer.process_statement_of_accounts_customer import (
			ProcessStatementOfAccountsCustomer,
		)
		from artech.accounts.doctype.psoa_cost_center.psoa_cost_center import PSOACostCenter
		from artech.accounts.doctype.psoa_project.psoa_project import PSOAProject
	...
	def validate(self):
		...
		if not self.customers:
			artech_engine.throw(_("Customers not selected."))
		...
		if self.print_format:
			...
			if not pf:
				artech_engine.throw(title=_("Invalid Print Format"), msg=_("Selected Print Format does not exist."))
			if pf.print_format_type != "Jinja":
				artech_engine.throw(title=_("Invalid Print Format"), msg=_("Print Format Type should be Jinja."))
	...
	def validate_account(self):
		...
		if self.company != artech_engine.get_cached_value("Account", self.account, "company"):
			artech_engine.throw(
	...
	def validate_company_for_table(self, doctype):
		field = artech_engine.scrub(doctype)
		...
		invalid_values = artech_engine.db.get_all(
			doctype, filters={"name": ["in", values], "company": ["!=", self.company]}, pluck="name"
		)

		if invalid_values:
			msg = _("<p>Following {0}s doesn't belong to Company {1} :</p>").format(
				doctype, artech_engine.bold(self.company)
			)

			msg += (
				"<ul>"
				+ "".join(_("<li>{}</li>").format(artech_engine.bold(row)) for row in invalid_values)
				+ "</ul>"
			)
...
def get_statement_dict(doc, get_statement_dict=False):
	...
	for entry in doc.customers:
		...
		tax_id = artech_engine.get_doc("Customer", entry.customer).tax_id
...
def set_ageing(doc, entry):
	ageing_filters = artech_engine._dict(
		{
			"company": doc.company,
...
def get_html(doc, filters, entry, col, res, ageing):
	base_template_path = "artech_engine/www/printview.html"
	template_path = "artech/accounts/doctype/process_statement_of_accounts/process_statement_of_accounts_accounts_receivable.html"
	if doc.report == "General Ledger":
		template_path = (
			"artech/accounts/doctype/process_statement_of_accounts/process_statement_of_accounts.html"
		)

	process_soa_html = artech_engine.get_hooks("process_soa_html")
	# fetching custom print format for Process Statement of Accounts
	if process_soa_html and process_soa_html.get(doc.report):
		template_path = process_soa_html[doc.report][-1]

	if doc.print_format:
		custom_html, custom_css = artech_engine.db.get_value("Print Format", doc.print_format, ["html", "css"])
		template_path = f"<style>{custom_css}</style> {custom_html}"

	if doc.letter_head:
		from artech_engine.www.printview import get_letter_head

		letter_head = get_letter_head(doc, 0)
	html = artech_engine.render_template(
		template_path,
		{
			"filters": filters,
			"data": res,
			...
			"terms_and_conditions": artech_engine.db.get_value(
...
def get_customers_based_on_territory_or_customer_group(customer_collection, collection_name):
	...
	collection = artech_engine.get_doc(customer_collection, collection_name)
	selected = [
		customer.name
		for customer in artech_engine.get_list(
			customer_collection,
...
def get_customers_based_on_sales_person(sales_person):
	lft, rgt = artech_engine.db.get_value("Sales Person", sales_person, ["lft", "rgt"])
	...
	sales_person_records = artech_engine._dict()
	for d in records:
...
def get_recipients_and_cc(customer, doc):
	...
	if doc.cc_to != "":
		try:
			cc = [artech_engine.get_value("User", user.cc, "email") for user in doc.cc_to]
		except Exception:
			pass
...
def get_context(customer, doc):
	...
	return {
		...
		"artech_engine": artech_engine.utils,
	}
...
def fetch_customers(customer_collection: str, collection_name: str, primary_mandatory: str | int):
	...
	if customer_collection == "Sales Person":
		...
		if not bool(customers):
			artech_engine.throw(_("No Customers found with selected options."))
	else:
		if customer_collection == "Sales Partner":
			customers = artech_engine.get_list(
				"Customer",
				fields=["name", "customer_name", "email_id"],
...
def get_customer_emails(customer_name: str, primary_mandatory: str | int, billing_and_primary: bool = True):
	...
	artech_engine.has_permission("Customer", "read", customer_name, throw=True)

	billing_email = artech_engine.db.sql(
		"""
		SELECT
			email.email_id
		FROM
	...
	if len(billing_email) == 0 or (billing_email[0][0] is None):
		if billing_and_primary:
			artech_engine.throw(_("No billing email found for customer: {0}").format(customer_name))
		else:
			return ""

	if billing_and_primary:
		primary_email = artech_engine.get_value("Customer", customer_name, "email_id")
		if primary_email is None and int(primary_mandatory):
			artech_engine.throw(_("No primary email found for customer: {0}").format(customer_name))
...
def download_statements(document_name: str):
	doc = artech_engine.get_doc("Process Statement Of Accounts", document_name)
	doc.check_permission("read")
	report = get_report_pdf(doc)
	if report:
		artech_engine.local.response.filename = doc.name + ".pdf"
		artech_engine.local.response.filecontent = report
...
def send_emails(document_name: str, from_scheduler: bool = False, posting_date: str | None = None):
	...
	if report:
		for customer, report_pdf in report.items():
			...
			filename = artech_engine.render_template(doc.pdf_name, context)
			...
			subject = artech_engine.render_template(doc.subject, context)
			message = artech_engine.render_template(doc.body, context)

			if doc.sender:
				sender_email = artech_engine.db.get_value("Email Account", doc.sender, "email_id")
			else:
				sender_email = artech_engine.session.user
		...
		if doc.enable_auto_email and from_scheduler:
			...
			doc.add_comment("Comment", "Emails sent on: " + artech_engine.utils.format_datetime(artech_engine.utils.now()))
			if doc.report == "General Ledger":
				artech_engine.db.set_value(doc.doctype, doc.name, "to_date", new_to_date)
				artech_engine.db.set_value(doc.doctype, doc.name, "from_date", new_from_date)
			else:
				artech_engine.db.set_value(doc.doctype, doc.name, "posting_date", new_to_date)
		return True
	else:
		return False
...
@artech_engine.whitelist()
def send_auto_email():
	selected = artech_engine.get_list(
