...
from artech.accounts.doctype.process_statement_of_accounts.process_statement_of_accounts import (
	get_statement_dict,
	send_emails,
)
...
class TestProcessStatementOfAccounts(ArtechTestSuite, AccountsTestMixin):
	def setUp(self):
		artech_engine.db.set_single_value("Selling Settings", "validate_selling_price", 0)
		letterhead = artech_engine.get_doc("Letter Head", "Company Letterhead - Grey")
	...
	def test_process_soa_for_ar(self):
		...
		expected_summary = artech_engine._dict(
			range1=100,
			range2=0,
			range3=0,
			range4=0,
...
def create_process_soa(**args):
	...
	artech_engine.delete_doc_if_exists("Process Statement Of Accounts", args.name)
	process_soa = artech_engine.new_doc("Process Statement Of Accounts")
	soa_dict = artech_engine._dict(
		name=args.name,
