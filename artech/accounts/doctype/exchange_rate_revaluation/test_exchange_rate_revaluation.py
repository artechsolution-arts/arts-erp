...
from artech_engine.utils import add_days, flt, today
...
class TestExchangeRateRevaluation(ArtechTestSuite, AccountsTestMixin):
	def setUp(self):
		self.create_company()
	...
	def test_01_revaluation_of_forex_balance(self):
		...
		gl = DocType("GL Entry")
		acc_balance = artech_engine.db.get_all(
			"GL Entry",
	...
	def test_02_accounts_only_with_base_currency_balance(self):
		...
		je = artech_engine.db.get_all("Journal Entry Account", filters={"reference_name": si.name}, pluck="parent")[
			0
		]
		...
		err_journals = err.make_jv_entries()
		je = artech_engine.get_doc("Journal Entry", err_journals.get("zero_balance_jv"))
		je = je.submit()
	...
	def test_03_accounts_only_with_account_currency_balance(self):
		...
		"""
		precision = artech_engine.db.get_single_value("System Settings", "currency_precision")
	...
	def test_04_get_account_details_function(self):
		...
		from artech.accounts.doctype.exchange_rate_revaluation.exchange_rate_revaluation import (
			get_account_details,
		)
