...
def create_or_update_cheque_print_format(template_name: str):
	if not artech_engine.db.exists("Print Format", template_name):
		cheque_print = artech_engine.new_doc("Print Format")
		cheque_print.update(
			{
		...
		cheque_print = artech_engine.get_doc("Print Format", template_name)

	doc = artech_engine.get_doc("Cheque Print Template", template_name)
...
<div style="position: relative; top:{starting_position_from_top_edge}cm">
	<div style="width:{cheque_width}cm;height:{cheque_height}cm;">
		...
		<span style="top:{date_dist_from_top_edge}cm; left:{date_dist_from_left_edge}cm;
			position: absolute;">
			{{{{ artech_engine.utils.formatdate(doc.reference_date) or '' }}}}
		</span>
		...
		<span style="top:{amt_in_words_from_top_edge}cm; left:{amt_in_words_from_left_edge}cm;
			...
			line-height:{amt_in_words_line_spacing}cm; word-wrap: break-word;">
				{{{{artech_engine.utils.money_in_words(doc.base_paid_amount or doc.base_received_amount)}}}}
		</span>
	...
	artech_engine.db.set_value("Cheque Print Template", template_name, "has_print_format", 1)
