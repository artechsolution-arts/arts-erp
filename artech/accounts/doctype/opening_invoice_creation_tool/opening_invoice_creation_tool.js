artech_engine.ui.form.on("Opening Invoice Creation Tool", {
	setup: function (frm) {
		...
		artech_engine.realtime.on("opening_invoice_creation_progress", (data) => {
				...
				setTimeout(
					() => {
						...
						if (frm.doc.invoice_type == "Sales") {
							artech_engine.msgprint(__("Opening Sales Invoices have been created."));
						} else {
							artech_engine.msgprint(__("Opening Purchase Invoices have been created."));
						}
					},
					1500,
					data.title
	...
	company: function (frm) {
		if (frm.doc.company) {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.get_temporary_opening_account",
				args: {
	...
	invoice_type: function (frm) {
		$.each(frm.doc.invoices, (idx, row) => {
			...
			artech_engine.model.set_value(row.doctype, row.name, "party", "");
			artech_engine.model.set_value(row.doctype, row.name, "party_name", "");
		});
		frm.clear_table("invoices");
		frm.refresh_fields();
		frm.trigger("update_party_labels");
	...
	make_dashboard: function (frm) {
		...
		if (!$.isEmptyObject(opening_invoices_summary)) {
			let section = frm.dashboard.add_section(
				artech_engine.render_template("opening_invoice_creation_tool_dashboard", {
			...
			section.on("click", ".invoice-link", function () {
				...
				artech_engine.set_route("List", doctype, { is_opening: "Yes", company: company, docstatus: 1 });
			});
			frm.dashboard.show();
		}
	},
...
artech_engine.ui.form.on("Opening Invoice Creation Tool Item", {
	party: function (frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (!row.party) {
			artech_engine.model.set_value(cdt, cdn, "party_name", "");
			return;
		}
		...
		artech_engine.db.get_value(party_type, row.party, name_field, (r) => {
			artech_engine.model.set_value(cdt, cdn, "party_name", r?.[name_field] || "");
		});
	},
