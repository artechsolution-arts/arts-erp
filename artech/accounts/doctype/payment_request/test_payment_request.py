...
from artech.accounts.doctype.payment_entry.test_payment_entry import create_payment_terms_template
...
class TestPaymentRequest(ArtechTestSuite):
	def setUp(self):
		for payment_gateway in payment_gateways:
			if not artech_engine.db.get_value("Payment Gateway", payment_gateway["gateway"], "name"):
				artech_engine.get_doc(payment_gateway).insert(ignore_permissions=True)
		...
		for method in payment_method:
			if not artech_engine.db.get_value(
				...
				artech_engine.get_doc(method).insert(ignore_permissions=True)

		send_email = patch(
			"artech.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email",
			return_value=None,
		)
		...
		_get_payment_gateway_controller = patch(
			"artech.accounts.doctype.payment_request.payment_request._get_payment_gateway_controller",
		)
	...
	def test_payment_request_linkings(self):
		...
		pr = make_payment_request(
			dt="Sales Order",
			dn=so_inr.name,
			recipient_id="saurabh@artech.com",
	...
	def test_payment_entry(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "write_off_account", "_Test Write Off - _TC")
		artech_engine.db.set_value("Company", "_Test Company", "cost_center", "_Test Cost Center - _TC")
		...
		so_inr = artech_engine.get_doc("Sales Order", so_inr.name)
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@artech.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
		...
		pr2.grand_total = 900
		self.assertRaises(artech_engine.ValidationError, pr2.save)
	...
	def test_conversion_on_foreign_currency_accounts(self):
		...
		pr = make_payment_request(dt=po_doc.doctype, dn=po_doc.name, recipient_id="nabin@artech.com")
		pr = artech_engine.get_doc(pr).save().submit()
	...
	def test_payment_request_on_unreconcile(self):
		...
		pe = pr.create_payment_entry()
		unreconcile = artech_engine.get_doc(
			{
