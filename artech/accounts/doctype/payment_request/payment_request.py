...
from artech_engine.utils import flt, nowdate
...
from artech.accounts.doctype.payment_entry.payment_entry import (
	get_payment_entry,
)
from artech.accounts.doctype.subscription_plan.subscription_plan import get_plan_rate
from artech.accounts.party import get_party_account, get_party_bank_account
from artech.accounts.utils import get_account_currency, get_advance_payment_doctypes, get_currency_precision
from artech.utilities import payment_app_import_guard
...
class PaymentRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.payment_reference.payment_reference import PaymentReference
		from artech.accounts.doctype.subscription_plan_detail.subscription_plan_detail import (
	...
	def validate_against_payment_reference(self):
		...
		if flt(expected, self.precision("grand_total")) != flt(self.grand_total):
			artech_engine.throw(_("Grand Total must match sum of Payment References"))
		...
		for r in self.payment_reference:
			...
			if r.payment_schedule in seen:
				artech_engine.throw(_("Duplicate Payment Schedule selected"))

			seen.add(r.payment_schedule)

	def validate_reference_document(self):
		if not self.reference_doctype or not self.reference_name:
			artech_engine.throw(_("To create a Payment Request reference document is required"))
	...
	def validate_payment_request_amount(self):
		...
		ref_doc = artech_engine.get_doc(self.reference_doctype, self.reference_name)
		if not hasattr(ref_doc, "order_type") or ref_doc.order_type != "Shopping Cart":
			ref_amount = get_amount(ref_doc, self.payment_account)
			if not ref_amount:
				artech_engine.throw(_("Payment Entry is already created"))
	...
	def validate_currency(self):
		ref_doc = artech_engine.get_doc(self.reference_doctype, self.reference_name)
		if self.payment_account and ref_doc.currency != artech_engine.get_cached_value(
			"Account", self.payment_account, "account_currency"
		):
			artech_engine.throw(_("Transaction currency must be same as Payment Gateway currency"))

	def validate_subscription_details(self):
		if self.is_a_subscription:
			amount = 0
			for subscription_plan in self.subscription_plans:
				payment_gateway = artech_engine.db.get_value(
	...
	def before_submit(self):
		if (
			...
			invoice = artech_engine.get_value(
				self.reference_doctype,
				self.reference_name,
	...
	def get_request_amount(self):
		data_of_completed_requests = artech_engine.get_all(
			"Integration Request",
			filters={
	...
	def get_payment_url(self):
		if self.reference_doctype != "Fees":
			...
			data.update({"company": artech_engine.defaults.get_defaults().company})
		...
		return controller.get_payment_url(
			**{
				...
				"reference_doctype": "Payment Request",
				"reference_docname": self.name,
				"payer_email": self.email_to or artech_engine.session.user,
	...
	def create_payment_entry(self, submit=True):
		"""create entry"""
		artech_engine.flags.ignore_account_permission = True
	...
	def get_message(self):
		...
		context = {
			"doc": artech_engine.get_doc(self.reference_doctype, self.reference_name),
			"payment_url": self.payment_url,
			"payment_request": self,
		}

		if self.message:
			return artech_engine.render_template(self.message, context)
	...
	def check_if_payment_entry_exists(self):
		if self.status == "Paid":
			if artech_engine.get_all(
				"Payment Entry Reference",
				filters={"reference_name": self.reference_name, "docstatus": ["<", 2]},
				fields=["parent"],
				limit=1,
			):
				artech_engine.throw(_("Payment Entry already exists"), title=_("Error"))
...
def make_payment_request(**args):
	...
	if args.dt not in ALLOWED_DOCTYPES_FOR_PAYMENT_REQUEST:
		artech_engine.throw(_("Payment Requests cannot be created against: {0}").format(artech_engine.bold(args.dt)))

	if args.dn and not isinstance(args.dn, str):
		artech_engine.throw(_("Invalid parameter. 'dn' should be of type str"))

	artech_engine.has_permission("Payment Request", "create", throw=True)
	artech_engine.has_permission(args.dt, "read", args.dn, throw=True)

	ref_doc = args.ref_doc or artech_engine.get_doc(args.dt, args.dn)
	if not args.get("company"):
		args.company = ref_doc.company

	gateway_account = get_gateway_details(args) or artech_engine._dict()
	...
	def validate_and_calculate_grand_total(grand_total, existing_payment_request_amount):
		...
		if not grand_total:
			artech_engine.throw(_("Payment Request is already created"))
		return grand_total
	...
	if existing_payment_request_amount:
		if args.order_type == "Shopping Cart":
			...
			grand_total = validate_and_calculate_grand_total(grand_total, existing_payment_request_amount)
	draft_payment_request = artech_engine.db.get_value(
		"Payment Request",
		{"reference_doctype": ref_doc.doctype, "reference_name": ref_doc.name, "docstatus": 0},
	)

	if draft_payment_request:
		pr = artech_engine.get_doc("Payment Request", draft_payment_request)
		...
		pr = artech_engine.new_doc("Payment Request")
		...
		if artech_engine.db.get_single_value("Accounts Settings", "create_pr_in_draft_status", cache=True):
	...
	if args.order_type == "Shopping Cart":
		...
		artech_engine.local.response["location"] = pr.get_payment_url()
...
def get_irequest_status(payment_requests: None | list = None) -> list:
	IR = artech_engine.qb.DocType("Integration Request")
	res = []
	if payment_requests:
		res = (
			artech_engine.qb.from_(IR)
			.select(IR.name)
...
def cancel_old_payment_requests(ref_dt, ref_dn):
	...
	if res := (
		...
		if get_irequest_status([x.name for x in res]):
			artech_engine.throw(_("Another Payment Request is already processed"))
		else:
			for x in res:
				doc = artech_engine.get_doc("Payment Request", x.name)
				doc.flags.ignore_permissions = True
				doc.cancel()

				if ireqs := get_irequests_of_payment_request(doc.name):
					for ireq in ireqs:
						artech_engine.db.set_value("Integration Request", ireq.name, "status", "Cancelled")
...
def get_print_format_list(ref_doctype: str):
	...
	print_format_list.extend(
		[p.name for p in artech_engine.get_all("Print Format", filters={"doc_type": ref_doctype})]
	)
...
def resend_payment_email(docname: str):
	return artech_engine.get_doc("Payment Request", docname).send_email()


@artech_engine.whitelist()
def make_payment_entry(docname: str):
	doc = artech_engine.get_doc("Payment Request", docname)
...
def update_payment_requests_as_per_pe_references(references=None, cancel=False):
	...
	precision = artech_engine.get_precision("Payment Entry Reference", "allocated_amount")
	referenced_payment_requests = artech_engine.get_all(
		"Payment Request",
	...
	if doc_updates:
		artech_engine.db.bulk_update("Payment Request", doc_updates)
...
def get_subscription_details(reference_doctype: str, reference_name: str):
	if reference_doctype == "Sales Invoice":
		subscriptions = artech_engine.db.sql(
		...
		for subscription in subscriptions:
			plans = artech_engine.get_doc("Subscription", subscription.sub_name).plans
			for plan in plans:
...
def validate_payment(doc, method=None):
	if doc.reference_doctype != "Payment Request" or (
		artech_engine.db.get_value(doc.reference_doctype, doc.reference_docname, "status") != "Paid"
	):
		return
...
def get_open_payment_requests_query(
	...
	open_payment_requests = artech_engine.get_list(
		"Payment Request",
		filters=filters,
...
def get_available_payment_schedules(reference_doctype: str, reference_name: str):
	ref_doc = artech_engine.get_doc(reference_doctype, reference_name)
...
def get_existing_payment_references(reference_name):
	PR = artech_engine.qb.DocType("Payment Request")
	PRF = artech_engine.qb.DocType("Payment Reference")
