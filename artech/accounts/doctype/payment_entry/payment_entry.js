...
artech.accounts.taxes.setup_tax_validations("Payment Entry");
artech.accounts.taxes.setup_tax_filters("Advance Taxes and Charges");

artech_engine.ui.form.on("Payment Entry", {
	onload: function (frm) {
	...
	setup: function (frm) {
		frm.set_query("paid_from", function () {
			...
			var account_types = ["Pay", "Internal Transfer"].includes(frm.doc.payment_type)
				? ["Bank", "Cash"]
				: [artech_engine.boot.party_account_types[frm.doc.party_type]];
		...
		frm.set_query("party_type", function () {
			...
			return {
				filters: {
					name: ["in", Object.keys(artech_engine.boot.party_account_types)],
				},
			};
		});
		...
		frm.set_query("payment_term", "references", function (frm, cdt, cdn) {
			...
			if (
				...
				return {
					query: "artech.controllers.queries.get_payment_terms_for_references",
					filters: {
		...
		frm.set_query("payment_request", "references", function (doc, cdt, cdn) {
			...
			return {
				query: "artech.accounts.doctype.payment_request.payment_request.get_open_payment_requests_query",
				filters: {
	...
	refresh: function (frm) {
		artech.hide_company(frm);
		frm.events.hide_unhide_fields(frm);
		frm.events.set_dynamic_labels(frm);
		frm.events.show_general_ledger(frm);
		artech.accounts.ledger_preview.show_accounting_ledger_preview(frm);
		if (
			frm.doc.references &&
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.set_route("List", "Journal Entry", {
		...
		artech_engine.flags.allocate_payment_amount = true;
	},
	...
	hide_unhide_fields: function (frm) {
		var company_currency = frm.doc.company
			? artech_engine.get_doc(":Company", frm.doc.company)?.default_currency
			: "";
	...
	mode_of_payment: function (frm) {
		artech.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
	...
	party_type: function (frm) {
		let party_types = Object.keys(artech_engine.boot.party_account_types);
		if (frm.doc.party_type && !party_types.includes(frm.doc.party_type)) {
			frm.set_value("party_type", "");
			artech_engine.throw(__("Party can only be one of {0}", [party_types.join(", ")]));
		}
	...
	party: function (frm) {
		...
		if (frm.doc.payment_type && frm.doc.party_type && frm.doc.party && frm.doc.company) {
			if (!frm.doc.posting_date) {
				artech_engine.msgprint(__("Please select Posting Date before selecting Party"));
				frm.set_value("party", "");
				return;
			}

			artech.utils.get_employee_contact_details(frm);

			frm.set_party_account_based_on_party = true;

			let company_currency = artech_engine.get_doc(":Company", frm.doc.company).default_currency;

			return artech_engine.call({
				method: "artech.accounts.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
					party_type: frm.doc.party_type,
					party: frm.doc.party,
	...
	apply_tds: function (frm) {
		if (!frm.doc.apply_tds) {
			...
			artech_engine.db.get_value(frm.doc.party_type, frm.doc.party, "tax_withholding_category", (values) => {
	...
	set_account_currency_and_balance: function (frm, account, currency_field, callback_function) {
		...
		if (frm.doc.posting_date && account) {
			artech_engine.call({
				method: "artech.accounts.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					account: account,
					date: frm.doc.posting_date,
					cost_center: frm.doc.cost_center,
	...
	source_exchange_rate: function (frm) {
		...
		frm.set_df_property("source_exchange_rate", "read_only", artech.stale_rate_allowed() ? 0 : 1);
	},
	...
	target_exchange_rate: function (frm) {
		...
		// Make read only if Accounts Settings doesn't allow stale rates
		frm.set_df_property("target_exchange_rate", "read_only", artech.stale_rate_allowed() ? 0 : 1);
	},
	...
	get_outstanding_invoices_or_orders: function (frm, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		let fields = [
			{ fieldtype: "Section Break", label: __("Posting Date") },
			{
				...
				default: artech_engine.datetime.add_days(today, -30),
			},
	...
	validate_filters_data: function (frm, filters) {
		...
		for (let key in fields) {
			...
			if (filters[from_field] && !filters[to_field]) {
				artech_engine.throw(__("Error: {0} is mandatory field", [to_field.replace(/_/g, " ")]));
	...
	get_outstanding_documents: function (frm, filters, get_outstanding_invoices, get_orders_to_be_billed) {
		...
		artech_engine.flags.allocate_payment_amount = filters["allocate_payment_amount"];

		return artech_engine.call({
			method: "artech.accounts.doctype.payment_entry.payment_entry.get_outstanding_reference_documents",
			args: {
				args: args,
			},
			callback: function (r, rt) {
	...
	allocate_party_amount_against_ref_docs: async function (frm, paid_amount, paid_amount_change) {
		await frm.call("allocate_amount_to_references", {
			...
			allocate_payment_amount: artech_engine.flags.allocate_payment_amount ?? false,
		});
	...
	set_difference_amount: function (frm) {
		...
		var total_deductions = artech_engine.utils.sum(
	...
	check_mandatory_to_fetch: function (frm) {
		$.each(["Company", "Party Type", "Party", "payment_type"], function (i, field) {
			if (!frm.doc[artech_engine.model.scrub(field)]) {
				artech_engine.msgprint(__("Please select {0} first", [field]));
				return false;
			}
		});
	},
	...
	validate_reference_document: function (frm, row) {
		var _validate = function (i, row) {
			...
			if (
				...
				artech_engine.model.set_value(row.doctype, row.name, "reference_doctype", null);
			...
			if (
				...
				artech_engine.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				artech_engine.msgprint(
					__(
	...
	set_exchange_gain_loss_deduction: async function (frm) {
		...
		await artech_engine.after_ajax();
		...
		if (!row) {
			const company_defaults = artech_engine.get_doc(":Company", frm.doc.company);
			const account =
	...
	bank_account: function (frm) {
		...
		if (frm.doc.bank_account && ["Pay", "Receive"].includes(frm.doc.payment_type)) {
			artech_engine.call({
				method: "artech.accounts.doctype.bank_account.bank_account.get_bank_account_details",
				args: {
	...
	after_save: function (frm) {
		const { matched_payment_requests } = artech_engine.last_response;
...
artech_engine.ui.form.on("Payment Entry Reference", {
	...
	reference_name: function (frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return artech_engine.call({
				method: "artech.accounts.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
					reference_doctype: row.reference_doctype,
					reference_name: row.reference_name,
					party_account_currency:
				...
				callback: function (r, rt) {
					if (r.message) {
						$.each(r.message, function (field, value) {
							artech_engine.model.set_value(cdt, cdn, field, value);
						});
						...
						artech_engine.model.set_value(cdt, cdn, "allocated_amount", allocated_amount);
						frm.refresh_fields();
					}
				},
			});
...
artech_engine.ui.form.on("Advance Taxes and Charges", {
	rate: function (frm) {
		frm.events.apply_taxes(frm);
		frm.events.set_unallocated_amount(frm);
	},
...
artech_engine.ui.form.on("Payment Entry Deduction", {
	before_deductions_remove: function (doc, cdt, cdn) {
		const row = artech_engine.get_doc(cdt, cdn);
		if (row.is_exchange_gain_loss && row.amount) {
			artech_engine.throw(__("Cannot delete Exchange Gain/Loss row"));
		}
	},
...
function get_company_defaults(company) {
	return artech_engine.call({
		method: "artech.accounts.doctype.payment_entry.payment_entry.get_company_defaults",
		args: {
			company: company,
		},
	});
...
function prompt_for_missing_account(frm, account) {
	return new Promise((resolve) => {
		const dialog = artech_engine.prompt(
			{
				label: __(artech_engine.unscrub(account)),
				fieldname: account,
				fieldtype: "Link",
				options: "Account",
				get_query: () => ({
...
function get_deduction_amount_precision() {
	return artech_engine.meta.get_field_precision(artech_engine.meta.get_field("Payment Entry Deduction", "amount"));
}
