...
from artech_engine import ValidationError, _, qb, scrub, throw
from artech_engine.model.document import Document
from artech_engine.model.meta import get_field_precision
from artech_engine.query_builder import Tuple
from artech_engine.query_builder.functions import Count
from artech_engine.utils import cint, comma_or, flt, getdate, nowdate
from artech_engine.utils.data import comma_and, fmt_money, get_link_to_form
from pypika.functions import Coalesce, Sum

import artech
from artech.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from artech.accounts.doctype.bank_account.bank_account import (
	get_bank_account_details,
	get_default_company_bank_account,
	get_party_bank_account,
)
from artech.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import PaymentTaxWithholding
...
from artech.controllers.accounts_controller import (
	AccountsController,
...
class PaymentEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.advance_taxes_and_charges.advance_taxes_and_charges import (
			AdvanceTaxesandCharges,
		)
		from artech.accounts.doctype.payment_entry_deduction.payment_entry_deduction import (
			PaymentEntryDeduction,
		)
		from artech.accounts.doctype.payment_entry_reference.payment_entry_reference import (
	...
	def on_submit(self):
		if self.difference_amount:
			artech_engine.throw(_("Difference Amount must be zero"))
	...
	def set_liability_account(self):
		...
		account_type = artech_engine.get_value(
		...
		if not liability_account:
			throw(
				_("Please set default {0} in Company {1}").format(
					artech_engine.bold(artech_engine.get_meta("Company").get_label(fieldname)), artech_engine.bold(self.company)
				)
			)
		...
		artech_engine.msgprint(
			_(
				...
				artech_engine.bold(self.party_account),
				artech_engine.bold(liability_account),
			),
			alert=True,
		)
	...
	def validate_allocated_amount(self):
		...
		for d in self.get("references"):
			if (flt(d.allocated_amount)) > 0 and flt(d.allocated_amount) > flt(d.outstanding_amount):
				artech_engine.throw(fail_message.format(d.idx))
	...
	def term_based_allocation_enabled_for_reference(
		...
		if (
			...
			if template := artech_engine.db.get_value(reference_doctype, reference_name, "payment_terms_template"):
	...
	def validate_allocated_amount_with_latest_data(self):
		...
		vouchers = [artech_engine._dict({"voucher_type": x[0], "voucher_no": x[1]}) for x in uniq_vouchers]
		...
		for d in latest_references:
			d = artech_engine._dict(d)
			latest_lookup.setdefault((d.voucher_type, d.voucher_no), artech_engine._dict())[d.payment_term] = d

		for idx, d in enumerate(self.get("references"), start=1):
			latest = latest_lookup.get((d.reference_doctype, d.reference_name)) or artech_engine._dict()
			...
			if (
				...
				artech_engine.throw(
					...
					).format(artech_engine.bold(d.reference_name), artech_engine.bold(idx))
				)
	...
	def delink_advance_entry_references(self):
		for reference in self.references:
			if reference.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				doc = artech_engine.get_lazy_doc(reference.reference_doctype, reference.reference_name)
	...
	def set_missing_values(self):
		if self.payment_type == "Internal Transfer":
			...
			if not self.party_type:
				artech_engine.throw(_("Party Type is mandatory"))

			if not self.party:
				artech_engine.throw(_("Party is mandatory"))

			_party_name = "title" if self.party_type == "Shareholder" else self.party_type.lower() + "_name"

			if artech_engine.db.has_column(self.party_type, _party_name):
				self.party_name = artech_engine.db.get_value(self.party_type, self.party, _party_name)
			else:
				self.party_name = artech_engine.db.get_value(self.party_type, self.party, "name")
	...
	def validate_payment_type(self):
		if self.payment_type not in ("Receive", "Pay", "Internal Transfer"):
			artech_engine.throw(_("Payment Type must be one of Receive, Pay and Internal Transfer"))

	def validate_party_details(self):
		if self.party and not artech_engine.db.exists(self.party_type, self.party):
			artech_engine.throw(_("{0} {1} does not exist").format(_(self.party_type), self.party))
	...
	def validate_mandatory(self):
		for field in ("paid_amount", "received_amount", "source_exchange_rate", "target_exchange_rate"):
			if not self.get(field):
				artech_engine.throw(_("{0} is mandatory").format(_(self.meta.get_label(field))))
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				...
				if not artech_engine.db.exists(d.reference_doctype, d.reference_name):
					artech_engine.throw(_("{0} {1} does not exist").format(d.reference_doctype, d.reference_name))

				ref_doc = artech_engine.get_lazy_doc(d.reference_doctype, d.reference_name)
				...
				if d.reference_doctype in artech_engine.get_hooks("invoice_doctypes"):
					if self.party_type == "Customer":
						ref_party_account = (
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice"):
				outstanding_amount, is_return = artech_engine.get_cached_value(
		...
		for reference_doctype, references in no_oustanding_refs.items():
			artech_engine.msgprint(
				_(
					...
					artech_engine.bold(comma_and([d.reference_name for d in references])),
					_(reference_doctype),
				)
				+ "<br><br>"
	...
	def validate_journal_entry(self):
		for d in self.get("references"):
			if d.allocated_amount and d.reference_doctype == "Journal Entry":
				je_accounts = artech_engine.db.sql(
	...
	def update_payment_schedule(self, cancel=0):
		...
		for ref in self.get("references"):
			...
			if not invoice_paid_amount_map.get(key):
				payment_schedule = artech_engine.get_all(
					"Payment Schedule",
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.items(), 1):
			if not invoice_paid_amount_map.get(key):
				artech_engine.throw(_("Payment term {0} not used in {1}").format(key[0], key[1]))
			...
			conversion_rate = artech_engine.db.get_value(key[2], {"name": key[1]}, "conversion_rate")
			base_paid_amount_precision = get_field_precision(
				artech_engine.get_meta("Payment Schedule").get_field("base_paid_amount")
			)
			base_outstanding_precision = get_field_precision(
				artech_engine.get_meta("Payment Schedule").get_field("base_outstanding")
			)
	...
	def get_allocated_amount_in_transaction_currency(
		...
		* PE in INR (accounting done in base currency)
		"""
		ref_currency, ref_exchange_rate = artech_engine.db.get_value(
	...
	def set_total_in_words(self):
		from artech_engine.utils import money_in_words
	...
	def validate_received_amount(self):
		if self.paid_from_account_currency == self.paid_to_account_currency:
			if self.paid_amount < self.received_amount:
				artech_engine.throw(_("Received Amount cannot be greater than Paid Amount"))
	...
	def set_exchange_gain_loss(self):
		...
		if not exchange_gain_loss_row:
			values = artech_engine.get_cached_value(
			...
			for fieldname, value in values.items():
				...
				label = _(artech_engine.get_meta("Company").get_label(fieldname))
				return artech_engine.msgprint(
	...
	def set_title(self):
		if artech_engine.flags.in_import and self.title:
	...
	def validate_transaction_reference(self):
		...
		bank_account_type = artech_engine.get_cached_value("Account", bank_account, "account_type")

		if bank_account_type == "Bank":
			if not self.reference_no or not self.reference_date:
				artech_engine.throw(_("Reference No and Reference Date is mandatory for Bank transaction"))
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		...
		merge_entries = artech_engine.get_single_value("Accounts Settings", "merge_similar_account_heads")
	...
	def add_party_gl_entries(self, gl_entries):
		...
		party_account_type = artech_engine.db.get_value("Party Type", self.party_type, "account_type")
		...
		for d in self.get("references"):
			...
			if d.reference_doctype == "Sales Invoice" and not cost_center:
				cost_center = artech_engine.db.get_value(d.reference_doctype, d.reference_name, "cost_center")
	...
	def add_advance_gl_for_reference(self, gl_entries, invoice):
		...
		if invoice.reconcile_effect_on:
			...
			artech_engine.db.set_value("Payment Entry Reference", invoice.name, "reconcile_effect_on", posting_date)
	...
	def add_tax_gl_entries(self, gl_entries):
		for d in self.get("taxes"):
			...
			if account_currency != self.company_currency:
				artech_engine.throw(_("Currency for {0} must be {1}").format(d.account_head, self.company_currency))
	...
	def add_deductions_gl_entries(self, gl_entries):
		for d in self.get("deductions"):
			...
			if account_currency != self.company_currency:
				artech_engine.throw(_("Currency for {0} must be {1}").format(d.account, self.company_currency))
	...
	def calculate_deductions(self, tax_details):
		return {
			...
			"cost_center": artech_engine.get_cached_value("Company", self.company, "cost_center"),
	...
	def set_matched_unset_payment_requests_to_response(self):
		...
		And set to `artech_engine.response` to show in the frontend for allocation.
		"""
		if not self.references:
			return
		...
		artech_engine.response["matched_payment_requests"] = matched_payment_requests
...
def get_matched_payment_request_of_references(references=None):
	...
	matched_prs = (
		artech_engine.qb.from_(subquery)
		.select(
...
def get_outstanding_of_references_with_payment_term(references=None):
	...
	PS = artech_engine.qb.DocType("Payment Schedule")

	response = (
		artech_engine.qb.from_(PS)
...
def get_outstanding_reference_documents(args: str | dict, validate: bool = False):
	...
	company_currency = artech_engine.get_cached_value("Company", args.get("company"), "default_currency")

	# Get positive outstanding sales /purchase invoices
	condition = ""
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = " and voucher_type={} and voucher_no={}".format(
			artech_engine.db.escape(args["voucher_type"]), artech_engine.db.escape(args["voucher_no"])
		)
	...
	if args.get("company"):
		condition += " and company = {}".format(artech_engine.db.escape(args.get("company")))
	...
	if args.get("get_outstanding_invoices"):
		...
		for d in outstanding_invoices:
			...
			if party_account_currency != company_currency:
				if d.voucher_type in artech_engine.get_hooks("invoice_doctypes"):
					d["exchange_rate"] = artech_engine.db.get_value(d.voucher_type, d.voucher_no, "conversion_rate")
				elif d.voucher_type == "Journal Entry":
					d["exchange_rate"] = get_exchange_rate(
						party_account_currency, company_currency, d.posting_date
					)
			if d.voucher_type in ("Purchase Invoice"):
				d["bill_no"] = artech_engine.db.get_value(d.voucher_type, d.voucher_no, "bill_no")
		...
		if not validate:
			artech_engine.msgprint(
				_(
					...
					_(ref_document_type), _(args.get("party_type")).lower(), artech_engine.bold(args.get("party"))
				)
			)
...
def split_refdocs_based_on_payment_terms(refdocs, company) -> list:
	...
	for entry in refdocs:
		if entry.voucher_type in ["Sales Invoice", "Purchase Invoice", "Sales Order", "Purchase Order"]:
			if payment_term_template := artech_engine.db.get_value(
				...
				if len(split_rows) > 1:
					artech_engine.msgprint(
						_("Splitting {0} {1} into {2} rows as per Payment Terms").format(
							_(entry.voucher_type), artech_engine.bold(entry.voucher_no), len(split_rows)
						),
						alert=True,
					)
...
def get_currency_data(outstanding_refdocs: list, company: str | None = None) -> dict:
	...
	exc_rates = artech_engine._dict()
	company_currency = artech_engine.db.get_value("Company", company, "default_currency") if company else None
	...
	for doctype in ["Sales Invoice", "Purchase Invoice", "Sales Order", "Purchase Order"]:
		...
		for x in artech_engine.db.get_all(
			...
			exc_rates[x.name] = artech_engine._dict(
...
def get_party_details(company: str, party_type: str, party: str, date: str, cost_center: str | None = None):
	...
	if not artech_engine.db.exists(party_type, party):
		artech_engine.throw(_("{0} {1} does not exist").format(_(party_type), party))

	party_account = get_party_account(party_type, party, company)
	account_currency = get_account_currency(party_account)
	_party_name = "title" if party_type == "Shareholder" else party_type.lower() + "_name"
	party_name = artech_engine.db.get_value(party_type, party, _party_name)
...
def get_account_details(account: str, date: str | date, cost_center: str | None = None):
	artech_engine.has_permission("Payment Entry", throw=True)

	# to check if the passed account is accessible under reference doctype Payment Entry
	account_list = artech_engine.get_list("Account", {"name": account}, reference_doctype="Payment Entry", limit=1)

	# There might be some user permissions which will allow account under certain doctypes
	# except for Payment Entry, only in such case we should throw permission error
	if not account_list:
		artech_engine.throw(_("Account: {0} is not permitted under Payment Entry").format(account))

	return artech_engine._dict(
		{
			"account_currency": get_account_currency(account),
			"account_type": artech_engine.get_cached_value("Account", account, "account_type"),
		}
	)
...
def get_company_defaults(company: str):
	...
	return artech_engine.get_cached_value("Company", company, fields, as_dict=1)
...
def get_reference_details(
	...
	ref_doc = artech_engine.get_lazy_doc(reference_doctype, reference_name)
	company_currency = ref_doc.get("company_currency") or artech.get_company_currency(ref_doc.company)
...
def get_payment_entry(
	...
	doc = artech_engine.get_doc(dt, dn)
	over_billing_allowance = artech_engine.get_single_value("Accounts Settings", "over_billing_allowance")
	if dt in ("Sales Order", "Purchase Order") and flt(doc.per_billed, 2) >= (100.0 + over_billing_allowance):
		artech_engine.throw(_("Can only make payment against unbilled {0}").format(_(dt)))
	...
	if party_type in ["Customer", "Supplier"] and not bank:
		...
		if party_bank_account:
			account = artech_engine.db.get_value("Bank Account", party_bank_account, "account")
	...
	pe = artech_engine.new_doc("Payment Entry")
	...
	pe.paid_from_account_type = artech_engine.db.get_value("Account", pe.paid_from, "account_type")
	pe.paid_to_account_type = artech_engine.db.get_value("Account", pe.paid_to, "account_type")
	pe.paid_amount = paid_amount
	pe.received_amount = received_amount
	pe.letter_head = doc.get("letter_head")
	pe.bank_account = artech_engine.db.get_value(
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		artech_engine.msgprint(_("{0} is on hold till {1}").format(doc.name, doc.release_date))
	else:
		if doc.doctype in (
			"Sales Invoice",
			"Purchase Invoice",
			"Purchase Order",
			"Sales Order",
		) and artech_engine.get_cached_value(
	...
	if party_account and bank:
		if discount_amount:
			...
			if artech_engine.get_single_value("Accounts Settings", "book_tax_discount_loss"):
...
def get_open_payment_requests_for_references(references=None):
	...
	response = (
		...
		.orderby(Coalesce(PR.transaction_date, PR.creation), order=artech_engine.qb.asc)
	).run(as_dict=True)
	...
	while row_number <= len(references):
		...
		if pr_outstanding_amount == allocated_amount:
			...
			while allocated_amount:
				...
				new_row = artech_engine.copy_doc(row)
...
def set_paid_amount_and_received_amount(
	...
	if party_account_currency == bank.account_currency:
		...
		if bank_amount:
			...
			company_currency = artech_engine.get_cached_value("Company", doc.get("company"), "default_currency")
...
def apply_early_payment_discount(paid_amount, received_amount, doc, party_account_currency, reference_date):
	...
	if doc.doctype in eligible_for_payments and has_payment_schedule:
		...
		if total_discount:
			...
			money = artech_engine.utils.fmt_money(total_discount, currency=currency)
			artech_engine.msgprint(_("Discount of {} applied as per Payment Term").format(money), alert=1)
...
def set_pending_discount_loss(pe, doc, discount_amount, base_total_discount_loss, party_account_currency):
	...
	if discount_amount > 0.0:
		...
		book_tax_loss = artech_engine.get_single_value("Accounts Settings", "book_tax_discount_loss")
		account_type = "round_off_account" if book_tax_loss else "default_discount_account"

		pe.append(
			"deductions",
			{
				"account": artech_engine.get_cached_value("Company", pe.company, account_type),
				"cost_center": pe.cost_center
				or artech_engine.get_cached_value("Company", pe.company, "cost_center"),
...
def add_income_discount_loss(pe, doc, total_discount_percent) -> float:
	...
	pe.append(
		"deductions",
		{
			"account": artech_engine.get_cached_value("Company", pe.company, "default_discount_account"),
			"cost_center": pe.cost_center or artech_engine.get_cached_value("Company", pe.company, "cost_center"),
...
def get_paid_amount(dt, dn, party_type, party, account, due_date):
	...
	paid_amount = artech_engine.db.sql(
		f"""
