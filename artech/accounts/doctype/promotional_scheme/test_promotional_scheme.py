...
from artech.accounts.doctype.promotional_scheme.promotional_scheme import TransactionExists
...
class TestPromotionalScheme(ArtechTestSuite):
	def setUp(self):
		if artech_engine.db.exists("Promotional Scheme", "_Test Scheme"):
			artech_engine.delete_doc("Promotional Scheme", "_Test Scheme")

	def test_promotional_scheme(self):
		ps = make_promotional_scheme(applicable_for="Customer", customer="_Test Customer")
		price_rules = artech_engine.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name", "creation"],
			filters={"promotional_scheme": ps.name},
		)
		self.assertTrue(len(price_rules), 1)
		price_doc_details = artech_engine.db.get_value(
		...
		artech_engine.delete_doc("Promotional Scheme", ps.name)
		price_rules = artech_engine.get_all(
			"Pricing Rule",
			fields=["promotional_scheme_id", "name"],
			filters={"promotional_scheme": ps.name},
	...
	def test_promotional_scheme_without_applicable_for(self):
		...
		price_rules = artech_engine.get_all("Pricing Rule", filters={"promotional_scheme": ps.name})
	...
	def test_min_max_amount_configuration(self):
		...
		price_rules_data = artech_engine.db.get_value(
	...
	def test_pricing_rule_for_product_discount_slabs(self):
		...
		ps.save()
		pr = artech_engine.get_doc("Pricing Rule", {"promotional_scheme_id": ps.product_discount_slabs[0].name})
	...
	def test_validation_on_recurse_with_mixed_condition(self):
		...
		self.assertRaises(artech_engine.ValidationError, ps.save)


def make_promotional_scheme(**args):
	args = artech_engine._dict(args)

	ps = artech_engine.new_doc("Promotional Scheme")
	...
	if args.applicable_for:
		...
		ps.append(
			artech_engine.scrub(args.applicable_for),
			{artech_engine.scrub(args.applicable_for): args.get(artech_engine.scrub(args.applicable_for))},
		)
