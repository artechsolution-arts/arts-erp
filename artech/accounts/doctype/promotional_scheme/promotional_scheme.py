...
class TransactionExists(artech_engine.ValidationError):
	pass
...
class PromotionalScheme(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.campaign_item.campaign_item import CampaignItem
		from artech.accounts.doctype.customer_group_item.customer_group_item import CustomerGroupItem
		from artech.accounts.doctype.customer_item.customer_item import CustomerItem
		from artech.accounts.doctype.pricing_rule_brand.pricing_rule_brand import PricingRuleBrand
		from artech.accounts.doctype.pricing_rule_item_code.pricing_rule_item_code import (
			PricingRuleItemCode,
		)
		from artech.accounts.doctype.pricing_rule_item_group.pricing_rule_item_group import (
			PricingRuleItemGroup,
		)
		from artech.accounts.doctype.promotional_scheme_price_discount.promotional_scheme_price_discount import (
			PromotionalSchemePriceDiscount,
		)
		from artech.accounts.doctype.promotional_scheme_product_discount.promotional_scheme_product_discount import (
			PromotionalSchemeProductDiscount,
		)
		from artech.accounts.doctype.sales_partner_item.sales_partner_item import SalesPartnerItem
		from artech.accounts.doctype.supplier_group_item.supplier_group_item import SupplierGroupItem
		from artech.accounts.doctype.supplier_item.supplier_item import SupplierItem
		from artech.accounts.doctype.territory_item.territory_item import TerritoryItem
	...
	def validate(self):
		if not self.selling and not self.buying:
			artech_engine.throw(_("Either 'Selling' or 'Buying' must be selected"), title=_("Mandatory"))
		if not (self.price_discount_slabs or self.product_discount_slabs):
			artech_engine.throw(_("Price or product discount slabs are required"))
	...
	def validate_applicable_for(self):
		if self.applicable_for:
			applicable_for = artech_engine.scrub(self.applicable_for)

			if not self.get(applicable_for):
				msg = f"The field {artech_engine.bold(self.applicable_for)} is required"
	...
	def validate_pricing_rules(self):
		...
		for doc in invalid_pricing_rule:
			artech_engine.delete_doc("Pricing Rule", doc)
	...
	def get_invalid_pricing_rules(self):
		pr = artech_engine.qb.DocType("Pricing Rule")
		conditions = []
		...
		return artech_engine.qb.from_(pr).select(pr.name).where(Criterion.all(conditions)).run(pluck=True)
	...
	def update_pricing_rules(self, pricing_rules):
		...
		for doc in docs:
			...
			if doc.get("__islocal"):
				...
				doc.save()
				artech_engine.msgprint(_("Pricing Rule {0} is updated").format(doc.name))

		if count:
			artech_engine.msgprint(_("New {0} pricing rules are created").format(count))

	def on_trash(self):
		for rule in artech_engine.get_all("Pricing Rule", {"promotional_scheme": self.name}):
			artech_engine.delete_doc("Pricing Rule", rule.name)


def raise_for_transaction_exists(name):
	msg = f"""You can't change the {artech_engine.bold(_('Applicable For'))}
		because transactions are present against the Promotional Scheme {artech_engine.bold(name)}. """
	msg += "Kindly disable this Promotional Scheme and create new for new Applicable For."

	artech_engine.throw(_(msg), TransactionExists)
...
def _get_pricing_rules(doc, child_doc, discount_fields, rules=None):
	...
	applicable_for = artech_engine.scrub(doc.get("applicable_for"))
...
def get_pricing_rule_docname(
	...
	docname = artech_engine.get_all("Pricing Rule", fields=fields, filters=filters)
...
def prepare_pricing_rule(
	...
	if docname:
		pr = artech_engine.get_doc("Pricing Rule", docname)
	else:
		pr = artech_engine.new_doc("Pricing Rule")
...
def set_args(args, pr, doc, child_doc, discount_fields, child_doc_fields):
	...
	for field in ["items", "item_groups", "brands"]:
		...
		apply_on = artech_engine.scrub(doc.get("apply_on"))
