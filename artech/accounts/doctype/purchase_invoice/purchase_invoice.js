...
artech.accounts.payment_triggers.setup("Purchase Invoice");
artech.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
artech.accounts.taxes.setup_tax_validations("Purchase Invoice");
artech.buying.setup_buying_controller();

artech.accounts.PurchaseInvoice = class PurchaseInvoice extends artech.buying.BuyingController {
	setup(doc) {
	...
	refresh(doc) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			artech_engine.model.with_doc("Supplier", me.frm.doc.supplier, function () {
				var supplier = artech_engine.model.get_doc("Supplier", me.frm.doc.supplier);
	...
	unblock_invoice() {
		const me = this;
		artech_engine.call({
			method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	can_change_release_date(date) {
		const diff = artech_engine.datetime.get_diff(date, artech_engine.datetime.nowdate());
		if (diff < 0) {
			artech_engine.throw(__("New release date should be in the future"));
			return false;
		} else {
			return true;
		}
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			artech_engine.call({
				method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return artech_engine.call({
			method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	on_submit() {
		...
		$.each(this.frm.doc["items"] || [], function (i, row) {
			if (row.purchase_receipt) artech_engine.model.clear_doc("Purchase Receipt", row.purchase_receipt);
		});
	}

	make_debit_note() {
		artech_engine.model.open_mapped_doc({
			method: "artech.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(artech.accounts.PurchaseInvoice);
...
artech_engine.ui.form.on("Purchase Invoice", {
	setup: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.per_received > 0) {
			frm.add_custom_button(
				...
				() => {
					...
					artech_engine.set_route("List", "Purchase Receipt", "List");
				},
				__("View")
			);
		}
