...
from artech_engine.utils import cint, cstr, flt, formatdate, get_link_to_form, getdate, nowdate
...
from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import PurchaseTaxWithholding
...
from artech.accounts.utils import get_account_currency, get_fiscal_year, update_voucher_outstanding
from artech.assets.doctype.asset.asset import is_cwip_accounting_enabled
...
from artech.controllers.accounts_controller import merge_taxes, validate_account_head
from artech.controllers.buying_controller import BuyingController
from artech.stock.doctype.purchase_receipt.purchase_receipt import (
	update_billed_amount_based_on_po,
)


class WarehouseMissingError(artech_engine.ValidationError):
	pass
...
class PurchaseInvoice(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.purchase_invoice_advance.purchase_invoice_advance import (
			PurchaseInvoiceAdvance,
		)
		from artech.accounts.doctype.purchase_invoice_item.purchase_invoice_item import PurchaseInvoiceItem
		from artech.accounts.doctype.purchase_taxes_and_charges.purchase_taxes_and_charges import (
			PurchaseTaxesandCharges,
		)
		from artech.accounts.doctype.tax_withholding_entry.tax_withholding_entry import TaxWithholdingEntry
		from artech.buying.doctype.purchase_receipt_item_supplied.purchase_receipt_item_supplied import (
	...
	def validate_release_date(self):
		if self.release_date and getdate(nowdate()) >= getdate(self.release_date):
			artech_engine.throw(_("Release date must be in the future"))

	def validate_cash(self):
		if not self.cash_bank_account and flt(self.paid_amount):
			artech_engine.throw(_("Cash or Bank Account is mandatory for making payment entry"))
	...
	def validate_credit_to_acc(self):
		...
		if account.report_type != "Balance Sheet":
			artech_engine.throw(
				...
				).format(artech_engine.bold(_("Credit To"))),
		...
		if self.supplier and account.account_type != "Payable":
			artech_engine.throw(
				...
				).format(artech_engine.bold(_("Credit To")), artech_engine.bold(self.credit_to)),
				title=_("Invalid Account"),
			)
	...
	def validate_with_previous_doc(self):
		...
		if (
			cint(artech_engine.db.get_single_value("Buying Settings", "maintain_same_rate"))
	...
	def validate_item_code(self):
		for d in self.get("items"):
			if not d.item_code:
				artech_engine.msgprint(_("Item Code required at Row No {0}").format(d.idx), raise_exception=True)
	...
	def set_expense_account(self, for_validate=False):
		...
		for item in self.get("items"):
			...
			if (
				...
				and (
					not item.po_detail
					or not artech_engine.db.get_value("Purchase Order Item", item.po_detail, "delivered_by_supplier")
				)
			):
				if self.update_stock and item.warehouse and (not item.from_warehouse):
					...
					if for_validate and item.expense_account and item.expense_account != _inv_dict["account"]:
						msg = _(
							...
							item.idx,
							artech_engine.bold(_inv_dict["account"]),
							artech_engine.bold(item.expense_account),
							artech_engine.bold(item.warehouse),
						)
						artech_engine.msgprint(msg, title=_("Expense Head Changed"))
					item.expense_account = _inv_dict["account"]
				else:
					# check if 'Stock Received But Not Billed' account is credited in Purchase receipt or not
					if item.purchase_receipt:
						negative_expense_booked_in_pr = artech_engine.db.sql(
						...
						if negative_expense_booked_in_pr:
							if (
								...
								msg = _(
									...
									item.idx,
									artech_engine.bold(stock_not_billed_account),
									artech_engine.bold(item.purchase_receipt),
								)
								artech_engine.msgprint(msg, title=_("Expense Head Changed"))
						...
						if (
							...
							msg = _(
								...
								item.idx, artech_engine.bold(stock_not_billed_account), artech_engine.bold(item.item_code)
							)
							msg += "<br>"
							msg += _(
				...
				if not item.pr_detail and item.po_detail:
					receipt_item = artech_engine.get_cached_value(
						"Purchase Receipt Item",
						{
				...
				if item.pr_detail:
					...
					arbnb_booked_in_pr = artech_engine.db.get_value(
						"GL Entry",
						{
	...
	def force_set_against_expense_account(self):
		self.set_against_expense_account()
		artech_engine.db.set_value(self.doctype, self.name, "against_expense_account", self.against_expense_account)

	def po_required(self):
		if (
			artech_engine.db.get_single_value("Buying Settings", "po_required") == "Yes"
			and not self.is_internal_transfer()
			and not artech_engine.get_value(
				"Supplier", self.supplier, "allow_purchase_invoice_creation_without_purchase_order"
			)
		):
			for d in self.get("items"):
				if not d.purchase_order:
					msg = _("Purchase Order Required for item {}").format(artech_engine.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase order please set {0} as {1} in {2}"
					).format(
						artech_engine.bold(_("Purchase Order Required")),
						artech_engine.bold(_("No")),
	...
	def pr_required(self):
		stock_items = self.get_stock_items()
		if artech_engine.db.get_single_value("Buying Settings", "pr_required") == "Yes":
			if artech_engine.get_value(
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_items:
					msg = _("Purchase Receipt Required for item {}").format(artech_engine.bold(d.item_code))
					msg += "<br><br>"
					msg += _(
						"To submit the invoice without purchase receipt please set {0} as {1} in {2}"
					).format(
						artech_engine.bold(_("Purchase Receipt Required")),
						artech_engine.bold(_("No")),
						get_link_to_form("Buying Settings", "Buying Settings", "Buying Settings"),
					)
	...
	def check_prev_docstatus(self):
		for d in self.get("items"):
			if d.purchase_order:
				submitted = artech_engine.db.sql(
					"select name from `tabPurchase Order` where docstatus = 1 and name = %s", d.purchase_order
				)
				if not submitted:
					artech_engine.throw(_("Purchase Order {0} is not submitted").format(d.purchase_order))
			if d.purchase_receipt:
				...
				if not submitted:
					artech_engine.throw(_("Purchase Receipt {0} is not submitted").format(d.purchase_receipt))
	...
	def on_submit(self):
		...
		if artech_engine.db.get_single_value("Buying Settings", "project_update_frequency") == "Each Transaction":
			self.update_project()
	...
	def get_gl_entries(self, inventory_account_map=None):
		self.auto_accounting_for_stock = artech.is_perpetual_inventory_enabled(self.company)
	...
	def check_asset_cwip_enabled(self):
		...
		for item in self.get("items"):
			if item.item_code and item.is_fixed_asset:
				asset_category = artech_engine.get_cached_value("Item", item.item_code, "asset_category")
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if self.update_stock:
			stock_ledger_entries = artech_engine.get_all(
				"Stock Ledger Entry",
		...
		adjust_incoming_rate = artech_engine.db.get_single_value(
		...
		for item in self.get("items"):
			if flt(item.base_net_amount) or (self.get("update_stock") and item.valuation_rate):
				if item.item_code:
					artech_engine.get_cached_value("Item", item.item_code, "asset_category")
	...
	def get_provisional_accounts(self):
		...
		provisional_gl_entries = artech_engine.get_all(
			"GL Entry",
			filters={
				"voucher_type": "Purchase Receipt",
	...
	def make_provisional_gl_entry(self, gl_entries, item):
		if item.purchase_receipt:
			...
			if pr_item.get("has_provisional_entry"):
				purchase_receipt_doc = artech_engine.get_cached_doc("Purchase Receipt", item.purchase_receipt)
	...
	def update_net_purchase_amount_for_linked_assets(self, item):
		assets = artech_engine.db.get_all(
			"Asset",
			filters={
						...
						self.get_gl_dict(
							{
								...
								"credit_in_transaction_currency": flt(
									...
									artech_engine.get_precision("Purchase Invoice Item", "item_tax_amount"),
								),
								"remarks": self.remarks or _("Accounting Entry for Stock"),
							},
							item=tax,
	...
	def update_project(self):
		...
		pj = artech_engine.qb.DocType("Project")
		for proj, value in projects.items():
			res = artech_engine.qb.from_(pj).select(pj.total_purchase_cost).where(pj.name == proj).for_update().run()
			current_purchase_cost = res and res[0][0] or 0
			# artech_engine.db.set_value("Project", proj, "total_purchase_cost", current_purchase_cost + value)
			project_doc = artech_engine.get_lazy_doc("Project", proj)
	...
	def validate_supplier_invoice(self):
		if self.bill_no:
			if cint(artech_engine.get_single_value("Accounts Settings", "check_supplier_invoice_uniqueness")):
				fiscal_year = get_fiscal_year(self.posting_date, company=self.company, as_dict=True)

				pi = artech_engine.db.sql(
	...
	def update_billing_status_in_pr(self, update_modified=True):
		...
		for pr in set(updated_pr):
			from artech.stock.doctype.purchase_receipt.purchase_receipt import update_billing_percentage

			pr_doc = artech_engine.get_lazy_doc("Purchase Receipt", pr)
	...
	def get_pr_details_billed_amt(self):
		...
		if pr_details:
			doctype = artech_engine.qb.DocType("Purchase Invoice Item")
			query = (
			...
			pr_details_billed_amt = artech_engine._dict(query.run(as_list=1))
...
def get_purchase_document_details(doc):
	...
	exchange_rate_map = artech_engine._dict(
	...
	net_rate_map = artech_engine._dict(
		artech_engine.get_all(child_doctype, filters={"name": ("in", items)}, fields=["name", "net_rate"], as_list=1)
	)
...
def change_release_date(name: str, release_date: str | None = None):
	if artech_engine.db.exists("Purchase Invoice", name):
		pi = artech_engine.get_lazy_doc("Purchase Invoice", name)
...
def make_purchase_receipt(
	...
	def update_item(obj, target, source_parent):
		from artech.controllers.sales_and_purchase_return import get_returned_qty_map_for_row
