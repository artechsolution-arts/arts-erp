...
from artech_engine.utils import add_days, cint, flt, getdate, nowdate, today
...
from artech.buying.doctype.purchase_order.purchase_order import get_mapped_purchase_invoice
from artech.buying.doctype.purchase_order.purchase_order import make_purchase_invoice as make_pi_from_po
...
from artech.controllers.accounts_controller import InvalidQtyError, get_payment_terms
...
from artech.stock.doctype.material_request.test_material_request import make_material_request
from artech.stock.doctype.purchase_receipt.purchase_receipt import (
	make_purchase_invoice as create_purchase_invoice_from_receipt,
)
from artech.stock.doctype.purchase_receipt.test_purchase_receipt import (
	get_taxes,
...
from artech.stock.doctype.stock_entry.test_stock_entry import get_qty_after_transaction
from artech.stock.tests.test_utils import StockTestMixin
from artech.tests.utils import ArtechTestSuite


class TestPurchaseInvoice(ArtechTestSuite, StockTestMixin):
	def setUp(self):
	...
	def test_update_received_qty_in_material_request(self):
		from artech.buying.doctype.purchase_order.purchase_order import make_purchase_invoice
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		pi = artech_engine.copy_doc(self.globalTestRecords["Purchase Invoice"][0])
		self.assertTrue(not cint(artech.is_perpetual_inventory_enabled(pi.company)))
		pi.insert()
		pi.submit()
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		self.assertTrue(cint(artech.is_perpetual_inventory_enabled(pi.company)), 1)

		self.check_gle_for_pi(pi.name)

	def test_terms_added_after_save(self):
		pi = artech_engine.copy_doc(self.globalTestRecords["Purchase Invoice"][1])
		pi.insert()
	...
	def test_payment_entry_unlink_against_purchase_invoice(self):
		...
		pi_doc = artech_engine.get_doc("Purchase Invoice", pi_doc.name)
		pi_doc.load_from_db()
		self.assertTrue(pi_doc.status, "Paid")

		self.assertRaises(artech_engine.LinkExistsError, pi_doc.cancel)
		unlink_payment_on_cancel_of_invoice()

	def test_purchase_invoice_for_blocked_supplier(self):
		supplier = artech_engine.get_doc("Supplier", "_Test Supplier")
		supplier.on_hold = 1
		supplier.save()

		self.assertRaises(artech_engine.ValidationError, make_purchase_invoice)
	...
	def test_purchase_invoice_blocked_invoice_must_be_in_future(self):
		...
		self.assertRaises(artech_engine.ValidationError, pi.save)
		pi.release_date = ""
		pi.save()
	...
	def test_purchase_invoice_temporary_blocked(self):
		...
		self.assertRaises(artech_engine.ValidationError, pe.save)
	...
	def test_purchase_invoice_with_exchange_rate_difference(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
		...
		exchange_gain_loss_account = artech_engine.db.get_value("Company", pi.company, "exchange_gain_loss_account")
	...
	def test_purchase_invoice_change_naming_series(self):
		...
		self.assertRaises(artech_engine.CannotChangeConstantError, pi.save)
	...
	def test_purchase_invoice_with_advance(self):
		jv = artech_engine.copy_doc(self.globalTestRecords["Journal Entry"][1])
		jv.insert()
		jv.submit()
	...
	def test_total_purchase_cost_for_project(self):
		if not artech_engine.db.exists("Project", {"project_name": "_Test Project for Purchase"}):
			project = make_project({"project_name": "_Test Project for Purchase"})
		else:
			project = artech_engine.get_doc("Project", {"project_name": "_Test Project for Purchase"})

		existing_purchase_cost = artech_engine.db.sql(
		...
		self.assertEqual(
			artech_engine.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(
			artech_engine.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost
		)
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		pi = make_purchase_invoice(
			update_stock=1,
			posting_date=artech_engine.utils.nowdate(),
			posting_time=artech_engine.utils.nowtime(),
	...
	def test_auto_batch(self):
		item_code = artech_engine.db.get_value("Item", {"has_batch_no": 1, "create_new_batch": 1}, "name")
		...
		self.assertTrue(artech_engine.db.get_value("Batch", {"item": item_code, "reference_name": pi.name}))

	def test_update_stock_and_purchase_return(self):
		actual_qty_0 = get_qty_after_transaction()

		pi = make_purchase_invoice(
			update_stock=1, posting_date=artech_engine.utils.nowdate(), posting_time=artech_engine.utils.nowtime()
		)
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(
			artech_engine.db.get_value("Serial No", rejected_serial_no, "warehouse"),
	...
	def test_outstanding_amount_after_advance_payment_entry_cancelation(self):
		...
		# added to avoid Document has been modified exception
		pe = artech_engine.get_doc("Payment Entry", pe.name)
		pe.cancel()
	...
	def test_duplicate_due_date_in_terms(self):
		...
		self.assertRaises(artech_engine.ValidationError, pi.insert)

	def test_debit_note(self):
		from artech.accounts.doctype.payment_entry.test_payment_entry import get_payment_entry
		from artech.accounts.doctype.sales_invoice.test_sales_invoice import get_outstanding_amount
		...
		pi_doc = artech_engine.get_doc("Purchase Invoice", pi.name)
	...
	def test_deferred_expense_via_journal_entry(self):
		...
		acc_settings = artech_engine.get_doc("Accounts Settings", "Accounts Settings")
		...
		gl_entries = gl_entries = artech_engine.db.sql(
	...
	def test_gain_loss_with_advance_entry(self):
		unlink_enabled = artech_engine.db.get_single_value(
			"Accounts Settings", "unlink_payment_on_cancellation_of_invoice"
		)

		artech_engine.db.set_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice", 1)

		original_account = artech_engine.db.get_value("Company", "_Test Company", "exchange_gain_loss_account")
		artech_engine.db.set_value(
			"Company", "_Test Company", "exchange_gain_loss_account", "Exchange Gain/Loss - _TC"
		)

		pay = artech_engine.get_doc(
			{
				"doctype": "Payment Entry",
				"company": "_Test Company",
				"payment_type": "Pay",
		...
		total_debit_amount = artech_engine.db.get_all(
		...
		jea_parent = artech_engine.db.get_all(
			"Journal Entry Account",
			filters={
		...
		self.assertEqual(
			artech_engine.db.get_value("Journal Entry", jea_parent.parent, "voucher_type"), "Exchange Gain Or Loss"
		)
		...
		jea_parent_2 = artech_engine.db.get_all(
			"Journal Entry Account",
			filters={
				"account": creditors_account,
				"docstatus": 1,
		...
		self.assertEqual(
			artech_engine.db.get_value("Journal Entry", jea_parent_2.parent, "voucher_type"),
		...
		self.assertEqual(artech_engine.db.get_value("Journal Entry", jea_parent.parent, "docstatus"), 2)

		pi_2.reload()
		pi_2.cancel()

		self.assertEqual(artech_engine.db.get_value("Journal Entry", jea_parent_2.parent, "docstatus"), 2)
		...
		artech_engine.db.set_value("Company", "_Test Company", "exchange_gain_loss_account", original_account)

	@ArtechTestSuite.change_settings("Accounts Settings", {"unlink_payment_on_cancellation_of_invoice": 1})
	def test_purchase_invoice_advance_taxes(self):
		from artech.accounts.doctype.payment_entry.payment_entry import get_payment_entry

		artech_engine.db.set_single_value("Accounts Settings", "merge_similar_account_heads", 1)
		...
		tds_account_args = {
			...
			"account_type": "Tax",
			"parent_account": artech_engine.db.get_value(
	...
	def test_provisional_accounting_entry_for_over_billing(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "maintain_same_rate", 0)

		# Configure Accounts Settings to allow 300% over billing
		artech_engine.db.set_single_value("Accounts Settings", "over_billing_allowance", 300)
	...
	def test_adjust_incoming_rate(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "maintain_same_rate", 1)
	...
	def test_item_less_defaults(self):
		pi = artech_engine.new_doc("Purchase Invoice")
	...
	def test_batch_expiry_for_purchase_invoice(self):
		...
		artech_engine.db.set_value("Batch", batch_no, "expiry_date", add_days(nowdate(), -1))
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"supplier_group": None})
	def test_purchase_invoice_without_supplier_group(self):
		# Create a Supplier
		test_supplier_name = "_Test Supplier Without Supplier Group"
		if not artech_engine.db.exists("Supplier", test_supplier_name):
	...
	def test_debit_note_with_account_mismatch(self):
		...
		self.assertRaises(artech_engine.ValidationError, dr_note.save)
	...
	def test_make_pr_and_pi_from_po(self):
		...
		pr_gl_entries = artech_engine.db.sql(
			"""select account, debit, credit
		...
		pi_gl_entries = artech_engine.db.sql(
			"""select account, debit, credit
			from `tabGL Entry` where voucher_type='Purchase Invoice' and voucher_no=%s
			order by account asc""",
			pi.name,
	...
	def test_opening_invoice_rounding_adjustment_validation(self):
		...
		pi.is_opening = "Yes"
		pi.save()
		self.assertRaises(artech_engine.ValidationError, pi.submit)
	...
	def test_pr_pi_over_billing(self):
		...
		artech_engine.db.set_single_value("Buying Settings", "set_landed_cost_based_on_purchase_invoice_rate", 0)
		artech_engine.db.set_single_value("Accounts Settings", "over_billing_allowance", 20)
		pi.cancel()
		pi = make_purchase_invoice_from_pr(pr.name)
		pi.items[0].rate = 12
...
def toggle_provisional_accounting_setting(**args):
	args = artech_engine._dict(args)
	company = artech_engine.get_doc("Company", args.company or "_Test Company")
