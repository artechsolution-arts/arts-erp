...
class PricingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.pricing_rule_brand.pricing_rule_brand import PricingRuleBrand
		from artech.accounts.doctype.pricing_rule_item_code.pricing_rule_item_code import PricingRuleItemCode
		from artech.accounts.doctype.pricing_rule_item_group.pricing_rule_item_group import (
	...
	def validate_duplicate_apply_on(self):
		if self.apply_on != "Transaction":
			...
			apply_on_field = artech_engine.scrub(self.apply_on)
			values = [d.get(apply_on_field) for d in self.get(apply_on_table) if d.get(apply_on_field)]
			if len(values) != len(set(values)):
				artech_engine.throw(_("Duplicate {0} found in the table").format(self.apply_on))

	def validate_mandatory(self):
		if self.has_priority and not self.priority:
			throw(_("Priority is mandatory"), artech_engine.MandatoryError, _("Please Set Priority"))
		...
		for apply_on, field in apply_on_dict.items():
			if self.apply_on == apply_on and len(self.get(field) or []) < 1:
				throw(_("{0} is not added in the table").format(apply_on), artech_engine.MandatoryError)

		tocheck = artech_engine.scrub(self.get("applicable_for", ""))
		if tocheck and not self.get(tocheck):
			throw(_("{0} is required").format(_(self.meta.get_label(tocheck))), artech_engine.MandatoryError)

		if self.apply_rule_on_other:
			o_field = "other_" + artech_engine.scrub(self.apply_rule_on_other)
			if not self.get(o_field) and o_field in other_fields:
				artech_engine.throw(
					_("For the 'Apply Rule On Other' condition the field {0} is mandatory").format(
						artech_engine.bold(self.apply_rule_on_other)
					)
				)

		if self.price_or_product_discount == "Price" and not self.rate_or_discount:
			throw(_("Rate or Discount is required for the price discount."), artech_engine.MandatoryError)

		if self.apply_discount_on_rate:
			if not self.priority:
				throw(
					_("As the field {0} is enabled, the field {1} is mandatory.").format(
						artech_engine.bold(_("Apply Discount on Discounted Rate")),
						artech_engine.bold(_("Priority")),
					)
				)
			...
			if self.priority and cint(self.priority) == 1:
				throw(
					...
					).format(artech_engine.bold(_("Apply Discount on Discounted Rate")), artech_engine.bold(_("Priority")))
				)
	...
	def cleanup_fields_value(self):
		for logic_field in ["apply_on", "applicable_for", "rate_or_discount"]:
			fieldname = artech_engine.scrub(self.get(logic_field) or "")
			...
			for f in options:
				...
				scrubbed_f = artech_engine.scrub(f)
		...
		apply_rule_on_other = artech_engine.scrub(self.apply_rule_on_other or "")
	...
	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.get("items"):
			for d in self.items:
				max_discount = artech_engine.get_cached_value("Item", d.item_code, "max_discount")
				if max_discount and flt(self.discount_percentage) > flt(max_discount):
					throw(_("Max discount allowed for item: {0} is {1}%").format(d.item_code, max_discount))

	def validate_price_list_with_currency(self):
		if self.currency and self.for_price_list:
			price_list_currency = artech_engine.db.get_value("Price List", self.for_price_list, "currency", True)
	...
	def validate_dates(self):
		if self.is_cumulative and not (self.valid_from and self.valid_upto):
			artech_engine.throw(_("Valid from and valid upto fields are mandatory for the cumulative"))
	...
	def validate_condition(self):
		if (
			...
			artech_engine.throw(_("Invalid condition expression"))

	def validate_mixed_with_recursion(self):
		if self.mixed_conditions and self.is_recursive:
			artech_engine.throw(_("Recursive Discounts with Mixed condition is not supported by the system"))
...
def apply_pricing_rule(args: str | dict, doc: str | dict | Document | None = None):
	...
	query_items = artech_engine.get_all(
		"Item",
...
def update_pricing_rule_uom(pricing_rule, args):
	...
	apply_on_field = artech_engine.scrub(pricing_rule.apply_on)
...
def get_pricing_rule_for_item(args, doc=None, for_validate=False):
	...
	if args.ignore_pricing_rule or not args.item_code:
		if artech_engine.db.exists(args.doctype, args.name) and args.get("pricing_rules"):
	...
	if pricing_rules:
		...
		for pricing_rule in pricing_rules:
			...
			if isinstance(pricing_rule, str):
				pricing_rule = artech_engine.get_cached_doc("Pricing Rule", pricing_rule)
			...
			if pricing_rule.get("coupon_code_based") == 1:
				if not args.coupon_code:
					continue
				coupon_code = artech_engine.db.get_value(
				...
				item_details.update(
					{
						...
						"apply_rule_on": (
							artech_engine.scrub(pricing_rule.apply_rule_on_other)
							if pricing_rule.apply_rule_on_other
							else artech_engine.scrub(pricing_rule.get("apply_on"))
						),
					}
				)
		...
		item_details.pricing_rules = artech_engine.as_json([d.pricing_rule for d in rules])
...
def update_args_for_pricing_rule(args):
	if not (args.item_group and args.brand):
		item = artech_engine.get_cached_value("Item", args.item_code, ("item_group", "brand"))
		if not item:
			return
		...
		if not args.item_group:
			artech_engine.throw(_("Item Group not mentioned in item master for item {0}").format(args.item_code))

	if args.transaction_type == "selling":
		if args.customer and not (args.customer_group and args.territory):
			if args.quotation_to and args.quotation_to != "Customer":
				customer = artech_engine._dict()
			else:
				customer = artech_engine.get_cached_value("Customer", args.customer, ["customer_group", "territory"])
		...
		args.supplier_group = artech_engine.get_cached_value("Supplier", args.supplier, "supplier_group")
...
def apply_price_discount_rule(pricing_rule, item_details, args):
	...
	if pricing_rule.rate_or_discount == "Rate":
		...
		# TODO https://github.com/artech_engine/artech/pull/23636 solve this in some other way.
	...
	for apply_on in ["Discount Amount", "Discount Percentage"]:
		...
		field = artech_engine.scrub(apply_on)
...
def remove_pricing_rule_for_item(
	...
	item_details: str | artech_engine._dict,
	...
	if isinstance(item_details, str):
		...
		item_details = artech_engine._dict(item_details)

	for d in get_applied_pricing_rules(pricing_rules):
		if not d or not artech_engine.db.exists("Pricing Rule", d):
			continue
		pricing_rule = artech_engine.get_cached_doc("Pricing Rule", d)
...
def set_transaction_type(pricing_ctx: artech_engine._dict) -> None:
...
def get_item_uoms(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	if filters.get("apply_on") != "Item Code":
		field = artech_engine.scrub(filters.get("apply_on"))
		items = [d.name for d in artech_engine.db.get_all("Item", filters={field: filters.get("value")})]
