...
from artech_engine.utils import cint, flt, fmt_money, get_link_to_form, getdate, today

from artech.setup.doctype.item_group.item_group import get_child_item_groups
...
class MultiplePricingRuleConflict(artech_engine.ValidationError):
	pass
...
def get_pricing_rules(args, doc=None):
	...
	if not artech_engine.db.count("Pricing Rule", cache=True):
		return
	...
	if doc:
		for pricing_rule in pricing_rules:
			if pricing_rule.condition:
				try:
					if artech_engine.safe_eval(pricing_rule.condition, None, doc.as_dict()):
...
def _get_pricing_rules(apply_on, args, values):
	apply_on_field = artech_engine.scrub(apply_on)
			...
			if args.get("uom", None):
				item_conditions += (
					" and ({child_doc}.uom={item_uom} or IFNULL({child_doc}.uom, '')='')".format(
						child_doc=child_doc, item_uom=artech_engine.db.escape(args.get("uom"))
					)
				)
			if "variant_of" not in args:
				args.variant_of = artech_engine.get_cached_value("Item", args.item_code, "variant_of")
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	field = artech_engine.scrub(parenttype)
	condition = ""
	if args.get(field):
		if not artech_engine.flags.tree_conditions:
			artech_engine.flags.tree_conditions = {}
		key = (parenttype, args.get(field))
		if key in artech_engine.flags.tree_conditions:
			return artech_engine.flags.tree_conditions[key]

		try:
			lft, rgt = artech_engine.db.get_value(parenttype, args.get(field), ["lft", "rgt"])
		except TypeError:
			artech_engine.throw(_("Invalid {0}").format(args.get(field)))

		parent_groups = artech_engine.db.sql_list(
		...
		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = f"parent_{artech_engine.scrub(parenttype)}"
			root_name = artech_engine.db.get_list(
				parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table, field=field, parent_groups=", ".join(artech_engine.db.escape(d) for d in parent_groups)
			)

			artech_engine.flags.tree_conditions[key] = condition
...
def get_other_conditions(conditions, values, args):
	...
	date = (
		...
		or artech_engine.get_value(args.get("doctype"), args.get("name"), "posting_date", ignore=True)
	)
	if date:
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if pricing_rules:
		...
		pr_doc = artech_engine.get_cached_doc("Pricing Rule", pricing_rules[0].name)
		...
		if pricing_rules[0].mixed_conditions and doc:
			...
			items = [args.get(artech_engine.scrub(pr_doc.get("apply_on")))]
...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	apply_on = artech_engine.scrub(pr_doc.get("apply_on"))
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=None):
	...
	date_field = (
		"transaction_date" if artech_engine.get_meta(doctype).has_field("transaction_date") else "posting_date"
	)
...
def apply_pricing_rule_on_transaction(doc):
	...
	pricing_rules = artech_engine.db.sql(
			...
			if d.price_or_product_discount == "Price":
				...
				for field in ["additional_discount_percentage", "discount_amount"]:
					...
					if (
						...
						artech_engine.msgprint(_("User has not applied rule on the invoice {0}").format(doc.name))
					else:
						if not d.coupon_code_based:
							doc.set(field, d.get(pr_field))
						elif doc.get("coupon_code"):
							# coupon code based pricing rule
							coupon_code_pricing_rule = artech_engine.db.get_value(
				...
				item_details = artech_engine._dict({"parenttype": doc.doctype, "free_item_data": []})
...
def apply_pricing_rule_for_free_items(doc, pricing_rule_args):
	if pricing_rule_args:
		...
		for free_item in args.values():
			if doc.is_new() or not artech_engine.get_value(
...
def get_pricing_rule_items(pr_doc, other_items=False) -> list:
	...
	if pr_doc.apply_rule_on_other and other_items:
		apply_on = artech_engine.scrub(pr_doc.apply_rule_on_other)
...
def validate_coupon_code(coupon_name):
	coupon = artech_engine.get_doc("Coupon Code", coupon_name)
	if coupon.valid_from and coupon.valid_from > getdate(today()):
		artech_engine.throw(_("Sorry, this coupon code's validity has not started"))
	elif coupon.valid_upto and coupon.valid_upto < getdate(today()):
		artech_engine.throw(_("Sorry, this coupon code's validity has expired"))
	elif coupon.maximum_use and coupon.used >= coupon.maximum_use:
		artech_engine.throw(_("Sorry, this coupon code is no longer valid"))
