artech_engine.ui.form.on("Process Deferred Accounting", {
	setup: function (frm) {
	...
	end_date: function (frm) {
		if (frm.doc.end_date && frm.doc.end_date < frm.doc.start_date) {
			artech_engine.throw(__("End date cannot be before start date"));
		}
	},

	onload: function (frm) {
		if (frm.doc.posting_date && frm.doc.docstatus === 0) {
			frm.set_value("start_date", artech_engine.datetime.add_months(frm.doc.posting_date, -1));
