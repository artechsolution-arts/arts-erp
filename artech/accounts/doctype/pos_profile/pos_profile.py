...
from artech_engine.utils import get_link_to_form, now
...
class POSProfile(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.pos_customer_group.pos_customer_group import POSCustomerGroup
		from artech.accounts.doctype.pos_item_group.pos_item_group import POSItemGroup
		from artech.accounts.doctype.pos_payment_method.pos_payment_method import POSPaymentMethod
		from artech.accounts.doctype.pos_profile_user.pos_profile_user import POSProfileUser
			...
			if (
				...
				artech_engine.throw(
					_(
						...
						artech_engine.bold(acc_dim.label),
					),
	...
	def validate_disabled(self):
		...
		if (
			...
			and artech_engine.db.exists("POS Opening Entry", {"pos_profile": self.name, "status": "Open"})
		):
			artech_engine.throw(
	...
	def validate_all_link_fields(self):
		...
		for link_dt, dn_list in accounts.items():
			for link_dn in dn_list:
				if link_dn and not artech_engine.db.exists(
					{"doctype": link_dt, "company": self.company, "name": link_dn}
				):
					artech_engine.throw(_("{0} does not belong to Company {1}").format(link_dn, self.company))
	...
	def validate_payment_methods(self):
		if not self.payments:
			artech_engine.throw(_("Payment methods are mandatory. Please add at least one payment method."))

		default_mode = [d.default for d in self.payments if d.default]
		if not default_mode:
			artech_engine.throw(_("Please select a default mode of payment"))

		if len(default_mode) > 1:
			artech_engine.throw(_("You can only select one mode of payment as default"))
	...
	def set_defaults(self, include_current_pos=True):
		artech_engine.defaults.clear_default("is_pos")
		...
		pos_view_users = artech_engine.db.sql_list(
			f"""select pfu.user
		...
		for user in pos_view_users:
			if user:
				artech_engine.defaults.set_user_default("is_pos", 1, user)
			else:
				artech_engine.defaults.set_global_default("is_pos", 1)
...
def get_item_groups(pos_profile):
	...
	if pos_profile.get("item_groups"):
		...
		for data in pos_profile.get("item_groups"):
			item_groups.extend(
				[
					"%s" % artech_engine.db.escape(d.name)
	...
	if not item_groups and permitted_item_groups:
		item_groups = ["%s" % artech_engine.db.escape(d) for d in permitted_item_groups]
...
def get_permitted_nodes(group_type):
	...
	for node in permitted_nodes:
		if artech_engine.db.get_value(group_type, node, "is_group"):
...
def get_child_nodes(group_type, root):
	lft, rgt = artech_engine.db.get_value(group_type, root, ["lft", "rgt"])
...
def pos_profile_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	...
	company = filters.get("company") or artech_engine.defaults.get_user_default("company")
	...
	pos_profile = artech_engine.db.sql(
		"""select pf.name
		from
