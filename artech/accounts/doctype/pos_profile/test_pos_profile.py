...
from artech.accounts.doctype.pos_profile.pos_profile import (
	get_child_nodes,
)
from artech.stock.get_item_details import get_pos_profile
from artech.tests.utils import ArtechTestSuite


class TestPOSProfile(ArtechTestSuite):
	def test_pos_profile(self):
		...
		if pos_profile:
			doc = artech_engine.get_doc("POS Profile", pos_profile.get("name"))
	...
	def test_disabled_pos_profile_creation(self):
		...
		pos_profile = artech_engine.get_doc("POS Profile", "_Test POS Profile 001")
	...
	def test_disabled_pos_profile_after_opening(self):
		...
		if pos_profile:
			...
			self.assertRaises(artech_engine.ValidationError, pos_profile.save)
	...
	def test_disabled_pos_profile_after_completing_session(self):
		...
		from artech.accounts.doctype.pos_closing_entry.test_pos_closing_entry import init_user_and_profile
		from artech.accounts.doctype.pos_opening_entry.test_pos_opening_entry import (
...
def make_pos_profile(**args):
	artech_engine.db.sql("delete from `tabPOS Payment Method`")
	artech_engine.db.sql("delete from `tabPOS Profile`")

	args = artech_engine._dict(args)

	pos_profile = artech_engine.get_doc(
		{
			...
			"customer_group": artech_engine.db.get_value("Customer Group", {"is_group": 0}, "name"),
	...
	mode_of_payment = artech_engine.get_doc("Mode of Payment", "Cash")
	company = args.company or "_Test Company"
	default_account = args.income_account or "Sales - _TC"

	if not artech_engine.db.get_value("Mode of Payment Account", {"company": company, "parent": "Cash"}):
	...
	if not artech_engine.db.exists("POS Profile", args.name or "_Test POS Profile"):
