...
class OverlappingConditionError(artech_engine.ValidationError):
	pass


class FromGreaterThanToError(artech_engine.ValidationError):
	pass


class ManyBlankToValuesError(artech_engine.ValidationError):
	pass
...
class ShippingRule(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.shipping_rule_condition.shipping_rule_condition import (
			ShippingRuleCondition,
		)
		from artech.accounts.doctype.shipping_rule_country.shipping_rule_country import (
			ShippingRuleCountry,
		)
	...
	def add_shipping_rule_to_tax_table(self, doc, shipping_amount):
		...
		if self.shipping_rule_type == "Selling":
			...
			if doc.meta.get_field("taxes").options != "Sales Taxes and Charges":
				artech_engine.throw(_("Shipping rule only applicable for Selling"))
			...
			if doc.meta.get_field("taxes").options != "Purchase Taxes and Charges":
				artech_engine.throw(_("Shipping rule only applicable for Buying"))
