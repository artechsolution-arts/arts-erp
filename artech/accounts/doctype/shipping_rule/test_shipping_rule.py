...
from artech.accounts.doctype.shipping_rule.shipping_rule import (
...
class TestShippingRule(ArtechTestSuite):
	def setUp(self):
		self.load_test_records("Shipping Rule")

	def test_from_greater_than_to(self):
		shipping_rule = artech_engine.copy_doc(self.globalTestRecords["Shipping Rule"][0])
...
def create_shipping_rule(shipping_rule_type, shipping_rule_name):
	if artech_engine.db.exists("Shipping Rule", shipping_rule_name):
		return artech_engine.get_doc("Shipping Rule", shipping_rule_name)

	sr = artech_engine.new_doc("Shipping Rule")
