...
class UnreconcilePayment(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.unreconcile_payment_entries.unreconcile_payment_entries import (
	...
	def validate(self):
		...
		if self.voucher_type not in self.supported_types:
			artech_engine.throw(_("Only {0} are supported").format(comma_and(self.supported_types)))
	...
	def on_submit(self):
		...
		for alloc in self.allocations:
			doc = artech_engine.get_doc(alloc.reference_doctype, alloc.reference_name)
			...
			artech_engine.db.set_value("Unreconcile Payment Entries", alloc.name, "unlinked", True)
...
def create_unreconcile_doc_for_selection(selections: str | None = None):
	if selections:
		...
		for row in selections:
			unrecon = artech_engine.new_doc("Unreconcile Payment")
