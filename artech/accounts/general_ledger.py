...
from artech_engine.utils import cint, flt, formatdate, get_link_to_form, getdate, now
...
from artech.accounts.doctype.accounting_dimension_filter.accounting_dimension_filter import (
	get_dimension_filter_map,
)
from artech.accounts.doctype.accounting_period.accounting_period import ClosedAccountingPeriod
from artech.accounts.doctype.budget.budget import validate_expense_against_budget
from artech.accounts.utils import create_payment_ledger_entry, is_immutable_ledger_enabled
from artech.controllers.budget_controller import BudgetValidation
from artech.exceptions import InvalidAccountDimensionError, MandatoryAccountDimensionError
...
def make_gl_entries(
	...
	if gl_map:
		if (
			not cancel
			and not cint(artech_engine.get_single_value("Accounts Settings", "use_legacy_budget_controller"))
...
def get_accounting_dimensions_for_offsetting_entry(gl_map, company):
	acc_dimension = artech_engine.qb.DocType("Accounting Dimension")
	dimension_detail = artech_engine.qb.DocType("Accounting Dimension Detail")

	acc_dimensions = (
		artech_engine.qb.from_(acc_dimension)
	...
	for acc_dimension in acc_dimensions:
		...
		acc_dimension.account_currency = artech_engine.get_cached_value(
...
def validate_disabled_accounts(gl_map):
	...
	disabled_accounts = artech_engine.get_all(
		"Account",
	...
	if used_disabled_accounts:
		account_list = "<br>"
		account_list += ", ".join([artech_engine.bold(d) for d in used_disabled_accounts])
		artech_engine.throw(
...
def validate_accounting_period(gl_map):
	accounting_periods = artech_engine.db.sql(
		""" SELECT
	...
	if accounting_periods:
		if accounting_periods[0].exempted_role:
			...
			if exempted_roles in artech_engine.get_roles():
				return
		artech_engine.throw(
			_(
				"You cannot create or cancel any accounting entries with in the closed Accounting Period {0}"
			).format(artech_engine.bold(accounting_periods[0].name)),
...
def distribute_gl_based_on_cost_center_allocation(gl_map, precision=None, from_repost=False):
	round_off_account, default_currency = artech_engine.get_cached_value(
		"Company", gl_map[0].company, ["round_off_account", "default_currency"]
	)
	if not precision:
		precision = get_field_precision(
			artech_engine.get_meta("GL Entry").get_field("debit"),
...
def get_cost_center_allocation_data(company, posting_date, cost_center):
	cost_center_allocation = artech_engine.db.get_value(
	...
	records = artech_engine.db.get_all(
...
def merge_similar_entries(gl_map, precision=None):
	...
	company = gl_map[0].company if gl_map else artech.get_default_company()
	company_currency = artech.get_company_currency(company)

	if not precision:
		precision = get_field_precision(
			artech_engine.get_meta("GL Entry").get_field("debit"), currency=company_currency
		)
	...
	merged_gl_map = filter(
		...
		or (
			...
			and artech_engine.get_cached_value("Journal Entry", x.voucher_no, "voucher_type")
...
def make_entry(args, adv_adj, update_outstanding, from_repost=False):
	gle = artech_engine.new_doc("GL Entry")
	gle.update(args)
...
def validate_cwip_accounts(gl_map):
	...
	cwip_enabled = any(
		...
		for ac in artech_engine.db.get_all("Asset Category", "enable_cwip_accounting")
	)
	if cwip_enabled:
		cwip_accounts = [
			d[0]
...
def process_debit_credit_difference(gl_map):
	precision = get_field_precision(
		artech_engine.get_meta("GL Entry").get_field("debit"),
		currency=artech_engine.get_cached_value("Company", gl_map[0].company, "default_currency"),
	)
	...
	if abs(debit_credit_diff) > allowance:
		if not (
			...
			and artech_engine.get_cached_value("Journal Entry", voucher_no, "voucher_type")
			== "Exchange Gain Or Loss"
		):
...
def make_round_off_gle(gl_map, debit_credit_diff, trx_cur_debit_credit_diff, precision):
	...
	round_off_gle = artech_engine._dict()
		...
		if not round_off_for_opening:
			artech_engine.throw(
				_("Please set '{0}' in Company: {1}").format(
					artech_engine.bold("Round Off for Opening"), get_link_to_form("Company", gl_map[0].company)
				)
			)
...
def update_accounting_dimensions(round_off_gle):
	...
	meta = artech_engine.get_meta(round_off_gle["voucher_type"])
	...
	if dimensions and has_all_dimensions:
		dimension_values = artech_engine.db.get_value(
		...
		report_type = artech_engine.get_cached_value("Account", round_off_gle.account, "report_type")
...
def get_round_off_account_and_cost_center(company, voucher_type, voucher_no, use_company_default=False):
	round_off_account, round_off_cost_center, round_off_for_opening = artech_engine.get_cached_value(
		"Company", company, ["round_off_account", "round_off_cost_center", "round_off_for_opening"]
	) or [None, None, None]

	# Use expense account as fallback
	if not round_off_account:
		round_off_account = artech_engine.get_cached_value("Company", company, "default_expense_account")

	meta = artech_engine.get_meta(voucher_type)

	# Give first preference to parent cost center for round off GLE
	if not use_company_default and meta.has_field("cost_center"):
		parent_cost_center = artech_engine.db.get_value(voucher_type, voucher_no, "cost_center")
	...
	if not round_off_account:
		artech_engine.throw(
			_("Please mention '{0}' in Company: {1}").format(
				artech_engine.bold("Round Off Account"), get_link_to_form("Company", company)
			)
		)
	...
	if not round_off_cost_center:
		artech_engine.throw(
			_("Please mention '{0}' in Company: {1}").format(
				artech_engine.bold("Round Off Cost Center"), get_link_to_form("Company", company)
			)
		)
	...
	if gl_entries:
			...
			if not immutable_ledger_enabled:
				...
				if not all(gle_names):
					...
					artech_engine.db.sql(
						...
						(now(), artech_engine.session.user, tuple(gle_names)),
					)
		...
		for entry in gl_entries:
			...
			if immutable_ledger_enabled:
				...
				new_gle["posting_date"] = artech_engine.form_dict.get("posting_date") or getdate()
			elif posting_date:
...
def check_freezing_date(posting_date, company, adv_adj=False):
	...
	"""
	if not adv_adj:
		acc_frozen_till_date = artech_engine.db.get_value("Company", company, "accounts_frozen_till_date")
		if acc_frozen_till_date:
			...
			if getdate(posting_date) <= getdate(acc_frozen_till_date) and (
				frozen_accounts_modifier not in artech_engine.get_roles() or artech_engine.session.user == "Administrator"
			):
				artech_engine.throw(
...
def validate_against_pcv(is_opening, posting_date, company):
	if is_opening and artech_engine.db.exists("Period Closing Voucher", {"docstatus": 1, "company": company}):
		artech_engine.throw(
			_("Opening Entry can not be created after Period Closing Voucher is created."),
			title=_("Invalid Opening Entry"),
		)

	last_pcv_date = artech_engine.db.get_value(
	...
	if last_pcv_date and getdate(posting_date) <= getdate(last_pcv_date):
		...
		artech_engine.throw(message, title=_("Period Closed"))
			...
			if value["is_mandatory"] and not gl_entry.get(dimension):
				artech_engine.throw(
					_("{0} is mandatory for account {1}").format(
						artech_engine.bold(artech_engine.unscrub(dimension)), artech_engine.bold(gl_entry.account)
					),
				...
				if gl_entry.get(dimension) and gl_entry.get(dimension) not in value["allowed_dimensions"]:
					artech_engine.throw(
						_("Invalid value {0} for {1} against account {2}").format(
							artech_engine.bold(gl_entry.get(dimension)),
							artech_engine.bold(artech_engine.unscrub(dimension)),
							artech_engine.bold(gl_entry.account),
						),
