...
from artech_engine import _, msgprint, qb, scrub
from artech_engine.contacts.doctype.address.address import get_company_address, get_default_address
from artech_engine.core.doctype.user_permission.user_permission import get_permitted_documents
from artech_engine.model.utils import get_fetch_values
from artech_engine.query_builder.functions import Abs, Date, Sum
...
from artech.exceptions import InvalidAccountCurrency, PartyDisabled, PartyFrozen
from artech.utilities.regional import temporary_flag

try:
	from artech_engine.contacts.doctype.address.address import render_address as _render_address
except ImportError:
	# Older artech_engine versions where this function is not available
	from artech_engine.contacts.doctype.address.address import get_address_display as _render_address
...
class DuplicatePartyAccountError(artech_engine.ValidationError):
	pass
...
def get_party_details(
	...
	if not artech_engine.db.exists(party_type, party):
		artech_engine.throw(_("{0}: {1} does not exists").format(party_type, party))
...
def _get_party_details(
	...
	party_details = artech_engine._dict(
		set_account_and_due_date(party, account, party_type, company, posting_date, bill_date, doctype)
	)
	party = party_details[party_type.lower()]
	party = artech_engine.get_doc(party_type, party)

	if not ignore_permissions:
		ptype = "select" if artech_engine.only_has_select_perm(party_type) else "read"
		artech_engine.has_permission(party_type, ptype, party, throw=True)
	...
	if not party_details.get("tax_category") and pos_profile:
		party_details["tax_category"] = artech_engine.get_value("POS Profile", pos_profile, "tax_category")
...
def complete_contact_details(party_details):
	contact_details = artech_engine._dict()

	if party_details.party_type == "Employee":
		from artech.setup.doctype.employee.employee import _get_contact_details as get_employee_contact
		...
		contact_details = artech_engine.db.get_value(
			"Contact",
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		return artech_engine.get_cached_value("Customer Group", party.customer_group, "default_price_list")
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list and len(price_list) == 1:
		...
		customer_price_list = artech_engine.get_value("Customer", party.name, "default_price_list")

		if customer_price_list:
			price_list = customer_price_list
		else:
			pos_price_list = artech_engine.get_value("POS Profile", pos, "selling_price_list")
			price_list = pos_price_list or given_price_list
	else:
		price_list = get_default_price_list(party) or given_price_list

	if price_list:
		party_details.price_list_currency = artech_engine.db.get_value(
...
def get_party_account(
	...
	if not party and party_type in ["Customer", "Supplier"]:
		...
		return artech_engine.get_cached_value("Company", company, default_account_name)
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		group = artech_engine.get_cached_value(party_type, party, scrub(party_group_doctype))
		account = artech_engine.db.get_value(
			"Party Account",
	...
	if not account and party_type in ["Customer", "Supplier"]:
		...
		account = artech_engine.get_cached_value("Company", company, default_account_name)
	...
	if not account:
		account_type = artech_engine.get_cached_value("Party Type", party_type, "account_type")
...
def get_party_bank_account(party_type: str, party: str):
	return artech_engine.db.get_value("Bank Account", {"party_type": party_type, "party": party, "is_default": 1})


def get_party_account_currency(party_type, party, company):
	def generator():
		party_account = get_party_account(party_type, party, company)
		return artech_engine.get_cached_value("Account", party_account, "account_currency")

	return artech_engine.local_cache("party_account_currency", (party_type, party, company), generator)
...
def get_party_gle_currency(party_type, party, company):
	...
	return artech_engine.local_cache(
...
def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = artech_engine.db.sql(
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		artech_engine.throw(
			_(
				...
				artech_engine.bold(party_type),
				artech_engine.bold(party),
				artech_engine.bold(existing_gle_currency),
...
def validate_party_accounts(doc):
	...
	for account in doc.get("accounts"):
		...
		party_account_currency = artech_engine.get_cached_value("Account", account.account, "account_currency")
		if artech_engine.db.get_default("Company"):
			company_default_currency = artech_engine.get_cached_value(
				"Company", artech_engine.db.get_default("Company"), "default_currency"
			)
		else:
			company_default_currency = artech_engine.get_cached_value("Company", account.company, "default_currency")
	...
	if (bill_date or posting_date) and party:
		...
		if template_name:
			...
			if party_type == "Supplier":
				supplier_group = artech_engine.get_cached_value(party_type, party, "supplier_group")
				template_name = artech_engine.get_cached_value("Supplier Group", supplier_group, "payment_terms")
				if template_name:
...
def get_due_date_from_template(template_name, posting_date, bill_date):
	...
	template = artech_engine.get_doc("Payment Terms Template", template_name)
...
def validate_due_date(posting_date, due_date, bill_date=None, template_name=None, doctype=None):
	if getdate(due_date) < getdate(posting_date):
		...
		artech_engine.throw(_("Due Date cannot be before {0}").format(doctype_date))
	else:
...
def validate_due_date_with_template(posting_date, due_date, bill_date, template_name, doctype=None):
	...
	if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
		if artech_engine.get_single_value("Accounts Settings", "credit_controller") in artech_engine.get_roles():
			...
			artech_engine.throw(_("Due Date cannot be after {0}").format(formatdate(default_due_date)))
...
def get_address_tax_category(
	...
	addr_tax_category_from = artech_engine.get_single_value(
		"Accounts Settings", "determine_address_tax_category_from"
	)
	if addr_tax_category_from == "Shipping Address":
		if shipping_address:
			tax_category = artech_engine.db.get_value("Address", shipping_address, "tax_category") or tax_category
	else:
		if billing_address:
			tax_category = artech_engine.db.get_value("Address", billing_address, "tax_category") or tax_category
...
def set_taxes(
	...
	from artech.accounts.doctype.tax_rule.tax_rule import get_party_details, get_tax_template

	args = {artech_engine.scrub(party_type): party, "company": company}
	...
	if party_type in ("Customer", "Lead", "Prospect", "CRM Deal"):
		...
		if party_type in ["Lead", "Prospect", "CRM Deal"]:
			args["customer"] = None
			del args[artech_engine.scrub(party_type)]
	else:
...
def get_payment_terms_template(party_name: str, party_type: str, company: str | None = None):
	...
	if party_type == "Customer":
		customer = artech_engine.get_cached_value(
			"Customer", party_name, fieldname=["payment_terms", "customer_group"], as_dict=1
		)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = artech_engine.get_cached_value("Customer Group", customer.customer_group, "payment_terms")
	else:
		supplier = artech_engine.get_cached_value(
			"Supplier", party_name, fieldname=["payment_terms", "supplier_group"], as_dict=1
		)
		template = supplier.payment_terms
		if not template and supplier.supplier_group:
			template = artech_engine.get_cached_value("Supplier Group", supplier.supplier_group, "payment_terms")

	if not template and company:
		template = artech_engine.get_cached_value("Company", company, fieldname="payment_terms")
	return template


def validate_party_frozen_disabled(company, party_type, party_name):
	if artech_engine.flags.ignore_party_validation:
		return

	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			party = artech_engine.get_cached_value(party_type, party_name, ["is_frozen", "disabled"], as_dict=True)
			if party.disabled:
				artech_engine.throw(_("{0} {1} is disabled").format(party_type, party_name), PartyDisabled)
			elif party.get("is_frozen"):
				role_allowed_for_frozen_entries = artech_engine.get_cached_value(
					"Company", company, "role_allowed_for_frozen_entries"
				)
				if role_allowed_for_frozen_entries not in artech_engine.get_roles():
					artech_engine.throw(_("{0} {1} is frozen").format(party_type, party_name), PartyFrozen)

		elif party_type == "Employee":
			if artech_engine.db.get_value("Employee", party_name, "status") != "Active":
				artech_engine.msgprint(_("{0} {1} is not active").format(party_type, party_name), alert=True)
...
def validate_account_party_type(self):
	...
	if self.party_type and self.party:
		account_type = artech_engine.get_cached_value("Account", self.account, "account_type")
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	company_wise_grand_total = artech_engine.get_all(
		doctype,
		filters={
			"docstatus": 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = artech_engine._dict(
	...
	company_wise_billing_this_year = artech_engine._dict()
	...
	company_wise_total_unpaid = artech_engine._dict(
	...
	for d in companies:
		company_default_currency = artech_engine.get_cached_value("Company", d.company, "default_currency")
...
def get_party_shipping_address(doctype: str, name: str) -> str | None:
	...
	:return: String
	"""
	shipping_addresses = artech_engine.get_all(
		"Address",
		filters=[
...
def get_partywise_advanced_payment_amount(
	...
	acc = artech_engine.qb.DocType("Account")
	...
	if invoice_doctypes := artech_engine.get_hooks("invoice_doctypes"):
	...
	data = artech_engine._dict(data or {})
...
def add_party_account(party_type, party, company, account):
	doc = artech_engine.get_doc(party_type, party)
...
def render_address(address, check_permissions=True):
	return artech_engine.call(_render_address, address, check_permissions=check_permissions)


def validate_party_currency_before_merging(party_type, old_party, new_party):
	for company in artech_engine.get_all("Company"):
