...
from artech_engine.utils import flt, getdate, now_datetime, nowdate

import artech
from artech.accounts.doctype.account.account import get_root_company
from artech.accounts.report.financial_statements import (
	filter_accounts,
	filter_out_zero_value_rows,
	set_gl_entries_by_account,
)
from artech.accounts.report.trial_balance.trial_balance import (
...
from artech.accounts.report.utils import get_rate_as_at
...
def validate_companies(filters):
	...
	lft, rgt = artech_engine.db.get_value("Company", root_company, fieldname=["lft", "rgt"])

	company_subtree = artech_engine.db.get_all(
		"Company",
		{"lft": [">=", lft], "rgt": ["<=", rgt]},
		"name",
		order_by="lft",
...
def get_data(filters) -> list[list]:
	...
	for company in filters.company:
		company_filter = artech_engine._dict(filters)
...
def get_company_wise_tb_data(filters, reporting_currency, ignore_reporting_currency):
	...
	default_currency = artech.get_company_currency(filters.company)
...
def get_reporting_currency(filters):
	reporting_currency = artech_engine.get_cached_value("Company", filters.company[0], "reporting_currency")
	default_currency = None
	for company in filters.company:
		company_default_currency = artech.get_company_currency(company)
