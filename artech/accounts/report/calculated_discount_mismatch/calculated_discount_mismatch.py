...
from artech_engine.query_builder import Order, Tuple
from artech_engine.utils.formatters import format_value
...
def get_data():
	...
	VERSION = artech_engine.qb.DocType("Version")

	versions = (
		artech_engine.qb.from_(VERSION)
	...
	discount_amount_field_map = {
		doctype: artech_engine.get_meta(doctype).get_field("discount_amount") for doctype in AFFECTED_DOCTYPES
	}
...
def get_transactions_with_discount_percentage(doctype):
	transactions = artech_engine.get_all(
		doctype,
		fields=[
			"name",
			"currency",
