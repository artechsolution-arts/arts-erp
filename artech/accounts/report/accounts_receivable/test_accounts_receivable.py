...
from artech_engine.utils import add_days, flt, getdate, today
...
from artech.accounts.report.accounts_receivable.accounts_receivable import execute
...
class TestAccountsReceivable(ArtechTestSuite, AccountsTestMixin):
	def setUp(self):
		self.create_company()
	...
	def test_exchange_revaluation_for_party(self):
		...
		company_doc = artech_engine.get_doc("Company", self.company)
		...
		je = artech_engine.get_doc("Journal Entry", err_journals.get("revaluation_jv"))
		je = je.submit()
	...
	def test_sales_person(self):
		sales_person = artech_engine.get_doc(
	...
	def test_customer_group_filter(self):
		self.create_sales_invoice()
		cus_group = artech_engine.db.get_value("Customer", self.customer, "customer_group")
		filters = {
			"company": self.company,
			"report_date": today(),
			"range": "30, 60, 90, 120",
	...
	def test_report_output_if_party_is_missing(self):
		...
		if not artech_engine.db.get_value("Account", filters={"account_name": acc_name, "company": self.company}):
			additional_receivable_acc = artech_engine.get_doc(
				{
					"doctype": "Account",
	...
	def test_payment_terms_template_filters(self):
		from artech.controllers.accounts_controller import get_payment_terms

		payment_term1 = artech_engine.get_doc(
			{"doctype": "Payment Term", "payment_term_name": "_Test 50% on 15 Days"}
		).insert()
		payment_term2 = artech_engine.get_doc(
