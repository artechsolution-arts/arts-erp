...
from artech_engine import _, qb, query_builder, scrub
from artech_engine.database.schema import get_definition
from artech_engine.query_builder import Criterion
from artech_engine.query_builder.functions import Date, Substring, Sum
from artech_engine.utils import cint, cstr, flt, getdate, nowdate

from artech.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimension_with_children,
)
from artech.accounts.report.financial_statements import get_cost_centers_with_children
...
class ReceivablePayableReport:
	def __init__(self, filters=None):
		...
		self.ple_fetch_method = (
			artech_engine.get_single_value("Accounts Settings", "receivable_payable_fetch_method")
			or "Buffered Cursor"
		)  # Fail Safe
		self.advance_payment_doctypes = artech_engine.get_hooks(
			"advance_payment_receivable_doctypes"
		) + artech_engine.get_hooks("advance_payment_payable_doctypes")

	def run(self, args):
		self.filters.update(args)
		self.set_defaults()
		self.party_naming_by = artech_engine.db.get_single_value(args.get("naming_by")[0], args.get("naming_by")[1])
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def set_defaults(self):
		if not self.filters.get("company"):
			self.filters.company = artech_engine.db.get_single_value("Global Defaults", "default_company")
		self.company_currency = artech_engine.get_cached_value(
	...
	def fetch_ple_in_unbuffered_cursor(self):
		self.ple_entries = []
		with artech_engine.db.unbuffered_cursor():
	...
	def fetch_ple_in_sql_procedures(self):
		...
		"""
		artech_engine.db.sql(build_balance)

		balances = artech_engine.db.sql(
			f"""select
			name,
			voucher_type,
			voucher_no,
	...
	def build_delivery_note_map(self):
		if self.invoices and self.filters.show_delivery_notes:
			self.delivery_notes = artech_engine._dict()

			# delivery note link inside sales invoice
			# nosemgrep
			si_against_dn = artech_engine.db.sql(
				"""
			...
			# nosemgrep
			dn_against_si = artech_engine.db.sql(
				"""
	...
	def get_invoice_details(self):
		self.invoice_details = artech_engine._dict()
		if self.account_type == "Receivable":
			# nosemgrep
			si_list = artech_engine.get_list(
				"Sales Invoice",
				filters={
			...
			if self.filters.show_sales_person:
				# nosemgrep
				sales_team = artech_engine.db.sql(
					"""
		...
		if self.account_type == "Payable":
			# nosemgrep
			invoices = artech_engine.get_list(
				"Purchase Invoice",
				filters={
					"posting_date": ("<=", self.filters.report_date),
					"company": self.filters.company,
		...
		# nosemgrep
		journal_entries = artech_engine.get_list(
			"Journal Entry",
			filters={
				"posting_date": ("<=", self.filters.report_date),
				"company": self.filters.company,
	...
	def get_payment_terms(self, row):
		...
		# nosemgrep
		payment_terms_details = artech_engine.db.sql(
			f"""
			select
		...
		original_row = artech_engine._dict(row)
		row.payment_terms = []
		...
		company_currency = artech_engine.get_value("Company", self.filters.get("company"), "default_currency")
		...
		for d in payment_terms_details:
			term = artech_engine._dict(original_row)
		...
		for key in ("paid", "credit_note"):
			if row[key] > 0:
				if not additional_row:
					additional_row = artech_engine._dict(row)
	...
	def get_future_payments(self):
		if self.filters.show_future_payments:
			self.future_payments = artech_engine._dict()
	...
	def get_return_entries(self):
		...
		self.return_entries = artech_engine._dict(
	...
	def prepare_ple_query(self):
		...
		if self.filters.get("show_remarks"):
			if remarks_length := artech_engine.get_single_value(
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("sales_person"):
			lft, rgt = artech_engine.db.get_value("Sales Person", self.filters.get("sales_person"), ["lft", "rgt"])
			...
			self.sales_person_records = artech_engine._dict()
			for d in records:
	...
	def get_hierarchical_filters(self, doctype, key):
		lft, rgt = artech_engine.db.get_value(doctype, self.filters.get(key), ["lft", "rgt"])
	...
	def get_party_details(self, party):
		if party not in self.party_details:
			if self.account_type == "Receivable":
				...
				self.party_details[party] = artech_engine.db.get_value(
					"Customer",
					party,
					fields,
					as_dict=True,
...
def get_party_group_with_children(party, party_groups):
	...
	for d in party_groups:
		if artech_engine.db.exists(group_dtype, d):
			lft, rgt = artech_engine.db.get_value(group_dtype, d, ["lft", "rgt"])
			children = artech_engine.get_all(
				group_dtype, filters={"lft": [">=", lft], "rgt": ["<=", rgt]}, pluck="name"
			)
			all_party_groups += children
		else:
			artech_engine.throw(_("{0}: {1} does not exist").format(group_dtype, d))
...
class InitSQLProceduresForAR:
	...
	def __init__(self):
		existing_procedures = artech_engine.db.get_routines()

		if self.init_procedure_name not in existing_procedures:
			artech_engine.db.sql(self.init_procedure_sql)

		if self.allocate_procedure_name not in existing_procedures:
			artech_engine.db.sql(self.allocate_procedure_sql)

		artech_engine.db.sql(f"drop table if exists `{self._voucher_balance_name}`")
		artech_engine.db.sql(self._voucher_balance_definition)

		artech_engine.db.sql(f"drop table if exists `{self._row_def_table_name}`")
		artech_engine.db.sql(self._row_def_table_definition)
