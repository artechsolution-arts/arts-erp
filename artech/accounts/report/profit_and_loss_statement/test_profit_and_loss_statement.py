...
from artech_engine.desk.query_report import export_query
...
from artech.accounts.report.profit_and_loss_statement.profit_and_loss_statement import execute
from artech.accounts.test.accounts_mixin import AccountsTestMixin
from artech.tests.utils import ArtechTestSuite


class TestProfitAndLossStatement(ArtechTestSuite, AccountsTestMixin):
	def setUp(self):
		self.create_company()
	...
	def get_fiscal_year(self):
		active_fy = artech_engine.db.get_all(
			"Fiscal Year",
			filters={"disabled": 0, "year_start_date": ("<=", today()), "year_end_date": (">=", today())},
		)[0]
		return artech_engine.get_doc("Fiscal Year", active_fy.name)
	...
	def test_p_and_l_export(self):
		...
		filters = self.get_report_filters()
		artech_engine.local.form_dict = artech_engine._dict(
			{
		...
		export_query()
		contents = artech_engine.response["filecontent"].decode()
		sales_account = artech_engine.db.get_value("Company", self.company, "default_income_account")
	...
	def test_accumulate_filter(self):
		...
		_x = artech_engine.db.get_all(
			"Fiscal Year",
			filters={"disabled": 0, "year_start_date": ("<=", find_for), "year_end_date": (">=", find_for)},
		)[0]
		prev_fy = artech_engine.get_doc("Fiscal Year", _x.name)
