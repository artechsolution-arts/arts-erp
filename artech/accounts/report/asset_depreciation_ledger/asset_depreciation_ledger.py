...
def get_data(filters):
	data = []
	depreciation_accounts = artech_engine.db.sql_list(
	...
	company_fb = artech_engine.get_cached_value("Company", filters.get("company"), "default_finance_book")
	...
	for d in gl_entries:
		...
		if asset_data:
			if not asset_data.get("accumulated_depreciation_amount"):
				...
				query = (
					artech_engine.qb.from_(DepreciationSchedule)
			...
			row = artech_engine._dict(asset_data)
			row.update(
				{
...
def get_assets_details(assets):
	...
	for d in artech_engine.get_all("Asset", fields=fields, filters={"name": ("in", assets)}):
