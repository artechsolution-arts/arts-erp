...
from artech_engine.utils import add_days, flt, get_first_day, get_last_day, nowdate

from artech.accounts.doctype.sales_invoice.sales_invoice import make_delivery_note, make_sales_return
from artech.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice
from artech.accounts.report.gross_profit.gross_profit import execute
...
class TestGrossProfit(ArtechTestSuite):
	def setUp(self):
		self.create_company()
		self.create_item()
		self.create_bundle()
	...
	def test_invoice_without_only_delivery_note(self):
		...
		expected_entry_without_dn = {
			...
			"customer": self.customer,
			"posting_date": artech_engine.utils.datetime.date.fromisoformat(nowdate()),
			"item_code": self.item,
			"item_name": self.item,
	...
	def test_crnote_against_invoice_with_multiple_instances_of_same_item(self):
		...
		sinv.append("items", artech_engine.copy_doc(sinv.items[0], ignore_no_copy=False))
		sinv = sinv.save().submit()
	...
	def test_drop_ship(self):
		...
		from artech.selling.doctype.sales_order.sales_order import make_purchase_order, make_sales_invoice
		from artech.selling.doctype.sales_order.test_sales_order import make_sales_order
...
def make_sales_person(sales_person_name="_Test Sales Person"):
	if not artech_engine.db.exists("Sales Person", {"sales_person_name": sales_person_name}):
		sales_person_doc = artech_engine.get_doc(
			{
		...
		sales_person_doc = artech_engine.get_doc("Sales Person", {"sales_person_name": sales_person_name})
