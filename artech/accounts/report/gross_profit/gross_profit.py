...
from artech_engine import _, qb, scrub
from artech_engine.query_builder import Case, Order
from artech_engine.query_builder.functions import Coalesce
from artech_engine.utils import cint, flt, formatdate
...
from artech.stock.report.stock_ledger.stock_ledger import get_item_group_condition
...
def execute(filters=None):
	...
	filters.currency = artech_engine.get_cached_value("Company", filters.company, "default_currency")

	gross_profit_data = GrossProfitGenerator(filters)

	data = []

	group_wise_columns = artech_engine._dict(
		{
			"invoice": [
				"invoice_or_item",
				"customer",
		...
		artech_engine._dict(
			{
				...
				"gross_profit_%": flt(
					...
					cint(artech_engine.db.get_default("currency_precision")) or 3,
				)
...
def get_data_when_not_grouped_by_invoice(gross_profit_data, filters, group_wise_columns, data):
	...
	currency_precision = cint(artech_engine.db.get_default("currency_precision")) or 3
...
def get_columns(group_wise_columns, filters):
	...
	column_map = artech_engine._dict(
		{
			"parent": {
...
class GrossProfitGenerator:
	def __init__(self, filters=None):
		...
		self.filters = artech_engine._dict(filters)
	...
	def process(self):
		...
		self.currency_precision = cint(artech_engine.db.get_default("currency_precision")) or 3
		self.float_precision = cint(artech_engine.db.get_default("float_precision")) or 2
		...
		for row in reversed(self.si_list):
			...
			product_bundles = []
			if row.update_stock:
				product_bundles = self.product_bundles.get(row.parenttype, {}).get(row.parent, artech_engine._dict())
			elif row.dn_detail:
				product_bundles = self.product_bundles.get("Delivery Note", {}).get(
					row.delivery_note, artech_engine._dict()
				)
	...
	def get_returned_invoice_items(self):
		returned_invoices = artech_engine.db.sql(
			"""
			select
		...
		self.returned_invoices = artech_engine._dict()
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, artech_engine._dict()).setdefault(
				inv.item_code, []
			).append(inv)
	...
	def get_buying_amount(self, row, item_code):
		...
		if (
			...
			and row.so_detail
			and (
				po_details := artech_engine.get_all(
					"Purchase Order Item",
	...
	def get_buying_amount_from_so_dn(self, sales_order, so_detail, item_code):
		...
		delivery_note_item = artech_engine.qb.DocType("Delivery Note Item")

		query = (
			artech_engine.qb.from_(delivery_note_item)
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		query = query.orderby(purchase_invoice.posting_date, order=artech_engine.qb.desc).limit(1)
	...
	def prepare_invoice_query(self):
		SalesInvoice = artech_engine.qb.DocType("Sales Invoice")
		SalesInvoiceItem = artech_engine.qb.DocType("Sales Invoice Item")
		Item = artech_engine.qb.DocType("Item")
		SalesTeam = artech_engine.qb.DocType("Sales Team")
		PaymentSchedule = artech_engine.qb.DocType("Payment Schedule")
	...
	def apply_common_filters(self, query, SalesInvoice, SalesInvoiceItem, SalesTeam, Item):
		...
		if self.filters.sales_person:
			query = query.where(
				ExistsCriterion(
					artech_engine.qb.from_(SalesTeam)
					.select(1)
					.where(
		...
		if self.filters.cost_center:
			self.filters.cost_center = artech_engine.parse_json(self.filters.get("cost_center"))
			self.filters.cost_center = get_cost_centers_with_children(self.filters.cost_center)
			query = query.where(SalesInvoiceItem.cost_center.isin(self.filters.cost_center))

		if self.filters.project:
			self.filters.project = artech_engine.parse_json(self.filters.get("project"))
		...
		for dim in get_accounting_dimensions(as_list=False) or []:
			if self.filters.get(dim.fieldname):
				if artech_engine.get_cached_value("DocType", dim.document_type, "is_tree"):
		...
		if self.filters.warehouse:
			lft, rgt = artech_engine.db.get_value("Warehouse", self.filters.warehouse, ["lft", "rgt"])
			WH = artech_engine.qb.DocType("Warehouse")
			query = query.where(
				SalesInvoiceItem.warehouse.isin(
					artech_engine.qb.from_(WH).select(WH.name).where((WH.lft >= lft) & (WH.rgt <= rgt))
				)
			)
	...
	def load_product_bundle(self):
		...
		for d in pki_query.run(as_dict=True):
			self.product_bundles.setdefault(d.parenttype, artech_engine._dict()).setdefault(
				d.parent, artech_engine._dict()
			).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = artech_engine.db.sql_list(
