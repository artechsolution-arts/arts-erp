...
from artech.accounts.report.deferred_revenue_and_expense.deferred_revenue_and_expense import (
...
class TestDeferredRevenueAndExpense(ArtechTestSuite, AccountsTestMixin):
	maxDiff = None
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"book_deferred_entries_based_on": "Months"})
	def test_deferred_revenue(self):
		...
		# execute report
		fiscal_year = artech_engine.get_doc("Fiscal Year", get_fiscal_year(date="2021-05-01")[0])
	...
	def test_deferred_expense(self):
		...
		pi = make_purchase_invoice(
			...
			is_return=False,
			update_stock=False,
			posting_date=artech_engine.utils.datetime.date(2021, 5, 1),
			parent_cost_center=self.cost_center,
			cost_center=self.cost_center,
			do_not_save=True,
			rate=300,
	...
	def test_zero_amount(self):
		...
		pi = make_purchase_invoice(
			...
			is_return=False,
			update_stock=False,
			posting_date=artech_engine.utils.datetime.date(2021, 12, 30),
			parent_cost_center=self.cost_center,
			cost_center=self.cost_center,
			do_not_save=True,
			rate=3910,
		...
		# execute report
		fiscal_year = artech_engine.get_doc("Fiscal Year", get_fiscal_year(date="2022-01-31")[0])
		self.filters = artech_engine._dict(
			{
				"company": self.company,
				"filter_based_on": "Date Range",
