...
from artech_engine.query_builder import Column, functions
from artech_engine.utils import add_days, date_diff, flt, get_first_day, get_last_day, getdate, rounded

from artech.accounts.report.financial_statements import get_period_list
...
class Deferred_Item:
	...
	def report_data(self):
		...
		"""
		ret_data = artech_engine._dict({"name": self.item_name})
	...
	def make_dummy_gle(self, name, date, amount):
		"""
		return - artech_engine._dict() of a dummy gle entry
		"""
	...
	def calculate_item_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			...
			self.period_total.append(artech_engine._dict({"key": period.key, "total": period_sum, "actual": actual}))
...
class Deferred_Invoice:
	def __init__(self, invoice, items, filters, period_list):
		...
		items - list : artech_engine._dict() with item details. Refer Deferred_Item for required fields
		"""
		self.name = invoice
	...
	def calculate_invoice_revenue_expense_for_period(self):
		...
		for period in self.period_list:
			self.period_total.append(artech_engine._dict({"key": period.key, "total": 0, "actual": 0}))
	...
	def report_data(self):
		...
		"""
		ret_data = []
		inv_total = artech_engine._dict({"name": self.name, "amount": self.amount_total})
...
class Deferred_Revenue_and_Expense_Report:
	def __init__(self, filters=None):
		...
		if not filters:
			fiscal_year = artech_engine.get_doc("Fiscal Year", get_fiscal_year(date=getdate()))
		...
		self.filters.book_deferred_entries_based_on = artech_engine.db.get_singles_value(
	...
	def generate_report_data(self):
		...
		# add total row
		if self.filters.type == "Revenue":
			total_row = artech_engine._dict({"name": "Total Deferred Income"})
		elif self.filters.type == "Expense":
			total_row = artech_engine._dict({"name": "Total Deferred Expense"})
