...
def get_account_type_based_gl_data(company, filters=None):
	...
	if filters.include_default_book_entries:
		company_fb = artech_engine.get_cached_value("Company", company, "default_finance_book")
		cond = """ AND (finance_book in ({}, {}, '') OR finance_book IS NULL)
			""".format(
			artech_engine.db.escape(filters.finance_book),
			artech_engine.db.escape(company_fb),
		)
	else:
		cond = " AND (finance_book in (%s, '') OR finance_book IS NULL)" % (
			artech_engine.db.escape(cstr(filters.finance_book))
		)
	...
	gl_sum = artech_engine.db.sql_list(
		f"""
...
def get_opening_range_using_fiscal_year(company, period_list):
	...
	earliest_fy = (
		artech_engine.qb.from_(FiscalYear)
	...
	if not earliest_fy:
		artech_engine.throw(_("Not able to find the earliest Fiscal Year for the given company."))
