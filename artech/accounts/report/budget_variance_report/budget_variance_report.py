...
from artech_engine.utils import add_months, flt, formatdate
...
from artech.controllers.trends import get_period_date_ranges
...
def get_budget_records(filters, dimensions):
	budget_against_field = artech_engine.scrub(filters["budget_against"])
...
def get_actual_transactions(dimension_name, filters):
	budget_against = artech_engine.scrub(filters.get("budget_against"))
	cost_center_filter = ""

	if filters.get("budget_against") == "Cost Center" and dimension_name:
		cc_lft, cc_rgt = artech_engine.db.get_value("Cost Center", dimension_name, ["lft", "rgt"])
		cost_center_filter = f"""
			and lft >= "{cc_lft}"
			and rgt <= "{cc_rgt}"
		"""

	actual_transactions = artech_engine.db.sql(
		f"""
			select
				gl.account,
				gl.debit,
	...
	for dimension, accounts in budget_map.items():
		for account, fiscal_year_map in accounts.items():
			...
			for period in periods:
				...
				row[artech_engine.scrub(budget_label)] = display_budget
				row[artech_engine.scrub(actual_label)] = display_actual
				row[artech_engine.scrub(variance_label)] = display_budget - display_actual
	...
	for year in fiscal_year:
		for from_date, to_date in get_period_date_ranges(filters["period"], year[0]):
			if filters["period"] == "Yearly":
				...
				for label in labels:
					columns.append(
						{"label": label, "fieldtype": "Float", "fieldname": artech_engine.scrub(label), "width": 150}
					)
			else:
				for label in [
...
def get_fiscal_years(filters):
	fiscal_year = artech_engine.db.sql(
		"""
			select
				name
			from
	...
	if filters.get("budget_against") and filters.get("budget_against") not in dimensions:
		artech_engine.throw(
			...
			msg=_("{0} is not a valid Accounting Dimension.").format(
				artech_engine.bold(filters.get("budget_against"))
			),
		)
