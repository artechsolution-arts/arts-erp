...
from artech_engine.query_builder import Criterion, Tuple
...
class PartyLedgerSummaryReport:
	...
	def get_party_details(self):
		...
		"""
		self.parties = []
		self.party_details = artech_engine._dict()
	...
	def get_data(self):
		...
		self.party_data = artech_engine._dict({})
	...
	def get_return_invoices(self):
		...
		self.return_invoices = artech_engine.get_all(doctype, filters=filters, pluck="name")
	...
	def get_party_adjustment_amounts(self):
		...
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"
		round_off_account = artech_engine.get_cached_value("Company", self.filters.company, "round_off_account")
		...
		for voucher_gl_entries in adjustment_voucher_entries.values():
			...
			for gle in voucher_gl_entries:
				...
				elif artech_engine.get_cached_value("Account", gle.account, "account_type") == account_type:
