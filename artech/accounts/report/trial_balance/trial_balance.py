...
from artech_engine.utils import add_days, cstr, flt, formatdate, getdate
...
def validate_filters(filters):
	if not filters.fiscal_year:
		artech_engine.throw(_("Fiscal Year {0} is required").format(filters.fiscal_year))

	fiscal_year = artech_engine.get_cached_value(
		"Fiscal Year", filters.fiscal_year, ["year_start_date", "year_end_date"], as_dict=True
	)
	if not fiscal_year:
		artech_engine.throw(_("Fiscal Year {0} does not exist").format(filters.fiscal_year))
	else:
...
def get_data(filters):
	...
	company_currency = filters.presentation_currency or artech.get_company_currency(filters.company)
...
def get_opening_balance(
	...
	closing_balance = artech_engine.qb.DocType(doctype)
	accounts = artech_engine.db.get_all("Account", filters={"report_type": report_type}, pluck="name")

	opening_balance = (
		artech_engine.qb.from_(closing_balance)
		.select(
	...
	if artech_engine.db.count("Finance Book"):
