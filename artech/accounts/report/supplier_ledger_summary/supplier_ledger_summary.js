...
artech_engine.query_reports["Supplier Ledger Summary"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			on_change: () => {
				...
				if (party) {
					artech_engine.db.get_value("Supplier", party, ["tax_id", "supplier_name"], function (value) {
						artech_engine.query_report.set_filter_value("tax_id", value["tax_id"]);
						artech_engine.query_report.set_filter_value("supplier_name", value["supplier_name"]);
					});
				} else {
					artech_engine.query_report.set_filter_value("tax_id", "");
					artech_engine.query_report.set_filter_value("supplier_name", "");
				}
			},
		},
		{
...
artech.utils.add_dimensions("Supplier Ledger Summary", 11);
