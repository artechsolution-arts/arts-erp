...
artech_engine.query_reports["General Ledger"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			fieldtype: "Data",
			on_change: function () {
				artech_engine.query_report.set_filter_value("categorize_by", "Categorize by Voucher (Consolidated)");
			},
		},
		{
		...
		},
		{
			...
			options: Object.keys(artech_engine.boot.party_account_types),
			on_change: function () {
		...
		},
		{
			...
			on_change: function () {
				...
				var parties = artech_engine.query_report.get_filter_value("party");

				if (!party_type || parties.length === 0 || parties.length > 1) {
					artech_engine.query_report.set_filter_value("party_name", "");
					artech_engine.query_report.set_filter_value("tax_id", "");
					return;
				} else {
					var party = parties[0];
					var fieldname = artech.utils.get_party_name(party_type) || "name";
					artech_engine.db.get_value(party_type, party, fieldname, function (value) {
						artech_engine.query_report.set_filter_value("party_name", value[fieldname]);
					});

					if (party_type === "Customer" || party_type === "Supplier") {
						artech_engine.db.get_value(party_type, party, "tax_id", function (value) {
...
artech.utils.add_dimensions("General Ledger", 15);
