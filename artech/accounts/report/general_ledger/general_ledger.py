...
from artech_engine import _, _dict
from artech_engine.query_builder import Criterion
from artech_engine.utils import cstr, getdate

from artech import get_company_currency, get_default_company
...
from artech.accounts.report.utils import convert_to_presentation_currency, get_currency
...
def execute(filters=None):
	...
	if filters and filters.get("print_in_account_currency") and not filters.get("account"):
		artech_engine.throw(_("Select an account to print in account currency"))

	for acc in artech_engine.db.sql("""select name, is_group from tabAccount""", as_dict=1):
		account_details.setdefault(acc.name, acc)

	if filters.get("party"):
		filters.party = artech_engine.parse_json(filters.get("party"))
...
def validate_filters(filters, account_details):
	...
	if not filters.get("from_date") and not filters.get("to_date"):
		artech_engine.throw(
			_("{0} and {1} are mandatory").format(artech_engine.bold(_("From Date")), artech_engine.bold(_("To Date")))
		)

	if filters.get("account"):
		filters.account = artech_engine.parse_json(filters.get("account"))
		for account in filters.account:
			if not account_details.get(account):
				artech_engine.throw(_("Account {0} does not exists").format(account))
	...
	if filters.get("account") and filters.get("categorize_by") == "Categorize by Account":
		...
		for account in filters.account:
			if account_details[account].is_group == 0:
				artech_engine.throw(_("Can not filter based on Child Account, if grouped by Account"))

	if filters.get("voucher_no") and filters.get("categorize_by") in ["Categorize by Voucher"]:
		artech_engine.throw(_("Can not filter based on Voucher No, if grouped by Voucher"))

	if filters.from_date > filters.to_date:
		artech_engine.throw(_("From Date must be before To Date"))

	if filters.get("project"):
		filters.project = artech_engine.parse_json(filters.get("project"))

	if filters.get("cost_center"):
		filters.cost_center = artech_engine.parse_json(filters.get("cost_center"))
...
def validate_party(filters):
	...
	if party and party_type:
		for d in party:
			if not artech_engine.db.exists(party_type, d):
				artech_engine.throw(_("Invalid {0}: {1}").format(party_type, d))


def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
		filters["company_currency"] = artech_engine.get_cached_value("Company", filters.company, "default_currency")
		...
		if filters.get("account"):
			...
			gle_currency = artech_engine.db.get_value(
				"GL Entry",
			...
			if gle_currency:
				...
				account_currency = (
					...
					else artech_engine.get_cached_value(filters.party_type, filters.party[0], "default_currency")
				)
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters.get("show_remarks"):
		if remarks_length := artech_engine.get_single_value("Accounts Settings", "general_ledger_remarks_length"):
...
def get_conditions(filters):
	...
	ignore_is_opening = artech_engine.get_single_value("Accounts Settings", "ignore_is_opening_check_for_reporting")
	...
	if filters.get("ignore_err"):
		err_journals = artech_engine.db.get_all(
			"Journal Entry",
			filters={
	...
	if filters.get("ignore_cr_dr_notes"):
		system_generated_cr_dr_journals = artech_engine.db.get_all(
			"Journal Entry",
			filters={
				"company": filters.get("company"),
				"docstatus": 1,
...
def get_party_name_map():
	...
	customers = artech_engine.get_all("Customer", fields=["name", "customer_name"])
	party_map["Customer"] = {c.name: c.customer_name for c in customers}

	suppliers = artech_engine.get_all("Supplier", fields=["name", "supplier_name"])
	party_map["Supplier"] = {s.name: s.supplier_name for s in suppliers}

	employees = artech_engine.get_all("Employee", fields=["name", "employee_name"])
...
def get_accounts_with_children(accounts):
	...
	doctype = artech_engine.qb.DocType("Account")
	accounts_data = (
	...
	return artech_engine.qb.from_(doctype).select(doctype.name).where(Criterion.any(conditions)).run(pluck=True)
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map):
	...
	immutable_ledger = artech_engine.get_single_value("Accounts Settings", "enable_immutable_ledger")
...
def get_account_type_map(company):
	account_type_map = artech_engine._dict(
		artech_engine.get_all("Account", fields=["name", "account_type"], filters={"company": company}, as_list=1)
	)
...
def get_columns(filters):
	...
	if (
		...
		artech_engine.throw(
			_(
				f'Presentation Currency cannot be {artech_engine.bold(filters["presentation_currency"])} , When {artech_engine.bold("Show Credit / Debit in Company Currency")} is enabled.'
			)
		)
	...
	supplier_master_name = artech_engine.db.get_single_value("Buying Settings", "supp_master_name")
	customer_master_name = artech_engine.db.get_single_value("Selling Settings", "cust_master_name")
