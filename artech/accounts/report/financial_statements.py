...
from artech_engine.utils import add_days, add_months, cint, cstr, flt, formatdate, get_first_day, getdate
...
from artech.accounts.utils import get_fiscal_year, get_zero_cutoff
...
def get_period_list(
	...
	for i in range(cint(math.ceil(months / months_to_add))):
		period = artech_engine._dict({"from_date": start_date})
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
	from_year_start_date = artech_engine.get_cached_value("Fiscal Year", from_fiscal_year, "year_start_date")
	to_year_end_date = artech_engine.get_cached_value("Fiscal Year", to_fiscal_year, "year_end_date")
...
def validate_fiscal_year(fiscal_year, from_fiscal_year, to_fiscal_year):
	if not fiscal_year.get("year_start_date") or not fiscal_year.get("year_end_date"):
		artech_engine.throw(_("Start Year and End Year are mandatory"))

	if getdate(fiscal_year.get("year_end_date")) < getdate(fiscal_year.get("year_start_date")):
		artech_engine.throw(_("End Year cannot be before Start Year"))


def validate_dates(from_date, to_date):
	if not from_date or not to_date:
		artech_engine.throw(_("From Date and To Date are mandatory"))

	if to_date < from_date:
		artech_engine.throw(_("To Date cannot be less than From Date"))
...
def get_data(
	...
	for root in artech_engine.db.sql(
...
def set_gl_entries_by_account(
	...
	# For balance sheet
	ignore_closing_balances = artech_engine.get_single_value("Accounts Settings", "ignore_account_closing_balance")
	if not from_date and not ignore_closing_balances:
		last_period_closing_voucher = artech_engine.db.get_all(
...
def get_accounting_entries(
	...
	gl_entry = artech_engine.qb.DocType(doctype)
	query = (
...
def get_account_filter_query(root_lft, root_rgt, root_type, gl_entry):
	...
	exists_query = (
		artech_engine.qb.from_(acc).select(acc.name).where(acc.name == gl_entry.account).where(acc.is_group == 0)
	)
...
def apply_additional_conditions(doctype, query, from_date, ignore_closing_entries, filters):
	...
	if filters:
		...
		if filters.get("include_default_book_entries"):
			company_fb = artech_engine.get_cached_value("Company", filters.company, "default_finance_book")
...
def get_cost_centers_with_children(cost_centers):
	...
	for d in cost_centers:
		if artech_engine.db.exists("Cost Center", d):
			lft, rgt = artech_engine.db.get_value("Cost Center", d, ["lft", "rgt"])
			children = artech_engine.get_all("Cost Center", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
			all_cost_centers += [c.name for c in children]
		else:
			artech_engine.throw(_("Cost Center: {0} does not exist").format(d))
