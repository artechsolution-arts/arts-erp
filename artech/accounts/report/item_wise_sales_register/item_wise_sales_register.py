...
from artech_engine.utils.xlsxutils import handle_html
from pypika.terms import Bracket, LiteralValue, Order

from artech.accounts.report.sales_register.sales_register import get_mode_of_payments
from artech.accounts.report.utils import get_values_for_columns
from artech.selling.report.item_wise_sales_history.item_wise_sales_history import (
...
def apply_conditions(query, si, sii, sip, filters, additional_conditions=None):
	...
	if filters.get("warehouse"):
		if artech_engine.db.get_value("Warehouse", filters.get("warehouse"), "is_group"):
			lft, rgt = artech_engine.db.get_all(
				"Warehouse", filters={"name": filters.get("warehouse")}, fields=["lft", "rgt"], as_list=True
			)[0]
			warehouses = artech_engine.db.get_all("Warehouse", {"lft": (">", lft), "rgt": ("<", rgt)}, pluck="name")
	...
	if filters.get("item_group"):
		if artech_engine.db.get_value("Item Group", filters.get("item_group"), "is_group"):
...
def apply_order_by_conditions(doctype, query, filters):
	...
	invoice_item = artech_engine.qb.DocType(f"{doctype} Item")
	...
	if not filters.get("group_by"):
		...
		filter_field = artech_engine.scrub(filters.get("group_by"))
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	sii = artech_engine.qb.DocType("Sales Invoice Item")
	...
	if additional_query_columns:
		for column in additional_query_columns:
			if column.get("_doctype"):
				table = artech_engine.qb.DocType(column.get("_doctype"))
...
def get_tax_accounts(
	...
	tax = artech_engine.qb.DocType("Item Wise Tax Detail")
	taxes_and_charges = artech_engine.qb.DocType(tax_doctype)
	...
	precision = artech_engine.get_precision(tax_doctype, "tax_amount", currency=company_currency) or 2
	tax_columns = {}
	itemised_tax = {}
	...
	for row in tax_details:
		...
		if not scrubbed_description:
			scrubbed_description = artech_engine.scrub(description)
...
def get_display_value(filters, group_by_field, item):
	if filters.get("group_by") == "Item":
		...
		party = artech_engine.scrub(filters.get("group_by"))
...
def get_group_by_and_display_fields(filters):
	if filters.get("group_by") == "Item":
		...
		group_by_field = artech_engine.scrub(filters.get("group_by"))
		subtotal_display_field = "item_code"
