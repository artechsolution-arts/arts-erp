...
from artech_engine.query_builder.functions import IfNull, Round
...
def get_ordered_to_be_billed_data(args, filters=None):
	...
	precision = (
		get_field_precision(
			artech_engine.get_meta(child_tab).get_field("billed_amt"), currency=get_default_currency()
		)
		or 2
	)

	doctype = artech_engine.qb.DocType(doctype)
	child_doctype = artech_engine.qb.DocType(child_tab)
