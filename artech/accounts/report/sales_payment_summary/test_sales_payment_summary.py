...
from artech.accounts.report.sales_payment_summary.sales_payment_summary import (
...
class TestSalesPaymentSummary(ArtechTestSuite):
	def test_get_mode_of_payments(self):
		...
		payment_entries = artech_engine.get_all(
			"Payment Entry",
			filters={"mode_of_payment": "Cash", "docstatus": 1},
			fields=["name", "docstatus"],
		)
		for payment_entry in payment_entries:
			pe = artech_engine.get_doc("Payment Entry", payment_entry.name)
			pe.cancel()
	...
	return artech_engine.get_doc(
		{
			...
			"customer": artech_engine.get_doc("Customer", {"customer_name": "Prestiga-Biz"}).name,
			...
			"items": [
				{
					...
					"item_code": artech_engine.get_doc("Item", {"item_name": "Consulting"}).name,
					"qty": qty,
					"rate": 10000,
