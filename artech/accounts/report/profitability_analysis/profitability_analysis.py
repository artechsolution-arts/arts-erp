...
def execute(filters=None):
	if filters.get("based_on") == "Accounting Dimension" and not filters.get("accounting_dimension"):
		artech_engine.throw(_("Select Accounting Dimension."))
...
def get_accounts_data(based_on, company):
	if based_on == "Cost Center":
		...
		return artech_engine.get_all("Project", fields=["name"], filters={"company": company}, order_by="name")
	else:
		filters = {}
		doctype = artech_engine.unscrub(based_on)
		has_company = artech_engine.db.has_column(doctype, "company")

		if has_company:
			filters.update({"company": company})

		return artech_engine.get_all(doctype, fields=["name"], filters=filters, order_by="name")
