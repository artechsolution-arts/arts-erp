...
from artech_engine.utils import flt, formatdate, get_datetime_str, get_table_name
...
from artech.accounts.doctype.fiscal_year.fiscal_year import get_from_and_to_date
...
def convert_to_presentation_currency(gl_entries, currency_info, filters=None):
	...
	if filters and isinstance(filters.get("account"), list):
		...
		gain_loss_account = artech_engine.db.get_value("Company", filters.company, "exchange_gain_loss_account")
...
def get_invoiced_item_gross_margin(
	...
	from artech.accounts.report.gross_profit.gross_profit import GrossProfitGenerator

	sales_invoice = sales_invoice or artech_engine.form_dict.get("sales_invoice")
	item_code = item_code or artech_engine.form_dict.get("item_code")
	company = company or artech_engine.get_cached_value("Sales Invoice", sales_invoice, "company")
...
def get_party_details(party_type, party_list):
	party_details = {}
	party = artech_engine.qb.DocType(party_type)
	query = artech_engine.qb.from_(party).select(party.name, party.tax_id).where(party.name.isin(party_list))
...
def get_taxes_query(invoice_list, doctype, parenttype):
	taxes = artech_engine.qb.DocType(doctype)

	query = (
		artech_engine.qb.from_(taxes)
...
def get_journal_entries(filters, args):
	...
	journal_account = artech_engine.qb.DocType("Journal Entry Account")
	query = (
...
def apply_common_conditions(filters, query, doctype, child_doctype=None, payments=False):
	...
	if child_doctype:
		child_doc = artech_engine.qb.DocType(child_doctype)
...
def get_advance_taxes_and_charges(invoice_list):
	adv_taxes = artech_engine.qb.DocType("Advance Taxes and Charges")
	return (
		artech_engine.qb.from_(adv_taxes)
		.select(
			adv_taxes.parent,
