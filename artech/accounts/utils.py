...
from artech_engine import _, qb, throw
from artech_engine.desk.reportview import build_match_conditions
from artech_engine.model.meta import get_field_precision
from artech_engine.model.naming import determine_consecutive_week_number
from artech_engine.query_builder import AliasedQuery, Case, Criterion, Field, Table
from artech_engine.query_builder.functions import Count, IfNull, Max, Round, Sum
from artech_engine.query_builder.utils import DocType
...
from artech_engine.utils.caching import site_cache
...
# imported to enable artech.accounts.utils.get_account_currency
from artech.accounts.doctype.account.account import get_account_currency
from artech.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from artech.stock import get_warehouse_account_map
from artech.stock.utils import get_stock_value_on

if TYPE_CHECKING:
	from artech.stock.doctype.repost_item_valuation.repost_item_valuation import RepostItemValuation


class FiscalYearError(artech_engine.ValidationError):
	pass


class PaymentEntryUnlinkError(artech_engine.ValidationError):
	pass
...
def get_fiscal_years(
	...
	if raise_on_missing:
		...
		if company:
			error_msg = _("""{0} for {1}""").format(error_msg, artech_engine.bold(company))

		if verbose == 1:
			artech_engine.msgprint(error_msg)
...
def _get_fiscal_years(company=None):
	fiscal_years = artech_engine.cache().hget("fiscal_years", company) or []
	...
	if not fiscal_years:
		...
		query = (
			artech_engine.qb.from_(FY).select(FY.name, FY.year_start_date, FY.year_end_date).where(FY.disabled == 0)
		)

		if company:
			FYC = DocType("Fiscal Year Company")
			query = query.where(
				ExistsCriterion(artech_engine.qb.from_(FYC).select(FYC.name).where(FYC.parent == FY.name)).negate()
				| ExistsCriterion(
					artech_engine.qb.from_(FYC)
					.select(FYC.company)
		...
		artech_engine.cache().hset("fiscal_years", company, fiscal_years)
	return fiscal_years
...
def get_balance_on(
	...
	if not account and artech_engine.form_dict.get("account"):
		account = artech_engine.form_dict.get("account")
	if not date and artech_engine.form_dict.get("date"):
		date = artech_engine.form_dict.get("date")
	if not party_type and artech_engine.form_dict.get("party_type"):
		party_type = artech_engine.form_dict.get("party_type")
	if not party and artech_engine.form_dict.get("party"):
		party = artech_engine.form_dict.get("party")
	if not cost_center and artech_engine.form_dict.get("cost_center"):
		cost_center = artech_engine.form_dict.get("cost_center")

	cond = ["is_cancelled=0"]
	if start_date:
		cond.append("posting_date >= %s" % artech_engine.db.escape(cstr(start_date)))
	if date:
		cond.append("posting_date <= %s" % artech_engine.db.escape(cstr(date)))
	else:
	...
	if cost_center and report_type == "Profit and Loss":
		cc = artech_engine.get_lazy_doc("Cost Center", cost_center)
		if cc.is_group:
			cond.append(
				f""" exists (
			...
			cond.append(f"""gle.cost_center = {artech_engine.db.escape(cost_center)} """)

	if account:
		if not (artech_engine.flags.ignore_account_permission or ignore_account_permission):
		...
		if acc.is_group:
			...
			if acc.account_currency == artech_engine.get_cached_value("Company", acc.company, "default_currency"):
				in_account_currency = False
		else:
			cond.append(f"""gle.account = {artech_engine.db.escape(account)} """)

	if account_type:
		accounts = artech_engine.db.get_all(
			"Account",
		...
		cond.append(
			...
			gle.account in (%s)
		"""
			% (", ".join([artech_engine.db.escape(account) for account in accounts]))
		)

	if party_type and party:
		cond.append(
			f"""gle.party_type = {artech_engine.db.escape(party_type)} and gle.party = {artech_engine.db.escape(party)} """
		)

	default_finance_book = None
	if company:
		cond.append("""gle.company = %s """ % (artech_engine.db.escape(company)))
		default_finance_book = artech_engine.get_cached_value("Company", company, "default_finance_book")

	if finance_book:
		if default_finance_book and include_default_fb_balances:
			cond.append(
				f"""(gle.finance_book IN (
						{artech_engine.db.escape(finance_book)},
						{artech_engine.db.escape(default_finance_book)}
					) OR gle.finance_book IS NULL
				)"""
			)
		else:
			cond.append(f"(gle.finance_book = {artech_engine.db.escape(finance_book)} OR gle.finance_book IS NULL)")

	elif default_finance_book and include_default_fb_balances:
		# No finance book passed → fall back to default
		cond.append(
			f"""(
				gle.finance_book = {artech_engine.db.escape(default_finance_book)}
				OR gle.finance_book IS NULL
			)"""
		)
...
def get_count_on(account, fieldname, date):
	...
	if account:
		...
		if not artech_engine.flags.ignore_account_permission:
			acc.check_permission("read")
		...
		for gle in entries:
			if fieldname not in ("invoiced_amount", "payables"):
				...
				if (
					...
					payment_amount = artech_engine.db.sql(
						f"""
...
@artech_engine.whitelist()
def add_ac(args: artech_engine._dict | None = None):
	...
	ac = artech_engine.new_doc("Account")
...
@artech_engine.whitelist()
def add_cc(args: artech_engine._dict | None = None):
	from artech_engine.desk.treeview import make_tree_args
	...
	if args.parent_cost_center == args.company:
		args.parent_cost_center = "{} - {}".format(
			args.parent_cost_center, artech_engine.get_cached_value("Company", args.company, "abbr")
		)
...
def reconcile_against_document(
	...
	for key, entries in reconciled_entries.items():
		...
		doc = artech_engine.get_doc(voucher_type, voucher_no)
		artech_engine.flags.ignore_party_validation = True
		...
		if voucher_type == "Payment Entry" and doc.book_advance_payments_in_separate_party_account:
			...
			from artech.accounts.general_ledger import process_debit_credit_difference
		...
		artech_engine.flags.ignore_party_validation = False
...
def check_if_advance_entry_modified(args):
	...
	if args.voucher_type == "Journal Entry":
		...
		precision = artech_engine.get_precision("Payment Entry", "unallocated_amount")
...
def validate_allocated_amount(args):
	precision = args.get("precision") or artech_engine.db.get_single_value("System Settings", "currency_precision")
...
def update_reference_in_journal_entry(d, journal_entry, do_not_save=False):
	...
	[
		...
		for field in artech_engine.get_meta("Journal Entry Account").get_fieldnames_with_value()
	]
...
def update_reference_in_payment_entry(
	...
	if not skip_ref_details_update_for_pe:
		reference_exchange_details = artech_engine._dict()
	...
	payment_entry.make_exchange_gain_loss_journal(
		artech_engine._dict({"difference_posting_date": d.difference_posting_date}), dimensions_dict
	)
...
def get_reconciliation_effect_date(against_voucher_type, against_voucher, company, posting_date):
	reconciliation_takes_effect_on = artech_engine.get_cached_value(
	...
	if reconciliation_takes_effect_on == "Advance Payment Date":
		...
		reconcile_on = artech_engine.db.get_value(against_voucher_type, against_voucher, date_field)
...
def cancel_exchange_gain_loss_journal(
	...
	if parent_doc.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
		...
		for doc in gain_loss_journals:
			gain_loss_je = artech_engine.get_doc("Journal Entry", doc)
...
def get_linked_exchange_gain_loss_journal(referenced_dt: str, referenced_dn: str, je_docstatus: int) -> list:
	...
	"""
	gain_loss_journals = []
	if journals := artech_engine.db.get_all(
		...
		gain_loss_journals = artech_engine.db.get_all(
			"Journal Entry",
			{
				"name": ["in", journals],
...
def cancel_common_party_journal(self):
	...
	if not artech_engine.get_single_value("Accounts Settings", "enable_common_party_accounting"):
		return
	...
	journal_entry = artech_engine.db.get_value(
		"Journal Entry Account",
		filters={
	...
	common_party_journal = artech_engine.db.get_value(
		"Journal Entry",
		filters={
			"name": journal_entry,
	...
	common_party_je = artech_engine.get_doc("Journal Entry", common_party_journal)
...
def update_accounting_ledgers_after_reference_removal(
	...
	gle_update_query = (
		...
		.set(gle.modified_by, artech_engine.session.user)
	...
	ple_update_query = (
		...
		.set(ple.modified_by, artech_engine.session.user)
		.where(
	...
	adv_ple = (
		...
		.set(adv.modified_by, artech_engine.session.user)
...
def remove_ref_from_advance_section(ref_doc: object = None, payment_name: str | None = None):
	...
	if ref_doc.doctype in ("Sales Invoice", "Purchase Invoice"):
		...
		artech_engine.db.delete(child_table, {"name": ("in", row_names)})
...
def remove_ref_doc_link_from_jv(
	...
	if linked_jv:
		update_query = (
			...
			.set(jea.modified_by, artech_engine.session.user)
		...
		artech_engine.msgprint(_("Journal Entries {0} are un-linked").format("\n".join(linked_jv)))
...
def remove_ref_doc_link_from_pe(
	...
	from artech.accounts.doctype.payment_request.payment_request import (
	...
	(
		...
		.set(per.modified_by, artech_engine.session.user)
	...
	for pe in linked_pe:
		try:
			pe_doc = artech_engine.get_doc("Payment Entry", pe)
			pe_doc.set_amounts()
			...
			artech_engine.throw(msg, exc=PaymentEntryUnlinkError, title=_("Payment Unlink Error"))
		...
		(
			...
			.set(pay.modified_by, artech_engine.session.user)
			.where(pay.name == pe)
			.run()
		)
	artech_engine.msgprint(_("Payment Entries {0} are un-linked").format("\n".join(linked_pe)))


@artech_engine.whitelist()
def get_company_default(company: str, fieldname: str, ignore_validation: bool = False):
	value = artech_engine.get_cached_value("Company", company, fieldname)

	if not ignore_validation and not value:
		throw(
			_("Please set default {0} in Company {1}").format(
				_(artech_engine.get_meta("Company").get_label(fieldname)), company
			)
		)
...
def get_currency_precision():
	precision = cint(artech_engine.db.get_default("currency_precision"))
	if not precision:
...
def get_fraction_units(currency: str) -> int:
	...
	fraction_units = artech_engine.db.get_value("Currency", currency, "fraction_units")
...
def get_held_invoices(party_type, party):
	...
	if party_type == "Supplier":
		held_invoices = artech_engine.db.sql(
...
def get_outstanding_invoices(
	...
	precision = artech_engine.get_precision("Sales Invoice", "outstanding_amount") or 2

	if account:
		root_type, account_type = artech_engine.get_cached_value(
			"Account", account[0], ["root_type", "account_type"]
		)
		party_account_type = "Receivable" if root_type == "Asset" else "Payable"
		party_account_type = account_type or party_account_type
	else:
		party_account_type = artech.get_party_account_type(party_type)
...
def get_account_name(account_type=None, root_type=None, is_group=None, account_currency=None, company=None):
	...
	return artech_engine.db.get_value(
		"Account",
		{
			...
			"account_currency": account_currency or artech_engine.defaults.get_defaults().currency,
			"company": company or artech_engine.defaults.get_defaults().company,
		},
		"name",
	)
...
def get_companies():
	...
	return [d.name for d in artech_engine.get_list("Company", fields=["name"], order_by="name")]
...
def get_children(
	...
	from artech.accounts.report.financial_statements import sort_accounts
	...
	acc = artech_engine.get_list(doctype, fields=fields, filters=filters)
...
def get_account_balances_coa(company: str, include_default_fb_balances: bool = False):
	...
	account_list = (
		artech_engine.qb.from_(Account)
	...
	get_ledger_balances_query = (
		artech_engine.qb.from_(GLEntry)
		.select(
			GLEntry.account,
...
def create_payment_gateway_account(gateway, payment_channel="Email", company=None):
	from artech.setup.setup_wizard.operations.install_fixtures import create_bank_account

	if not company:
		company = artech_engine.get_cached_value("Global Defaults", "Global Defaults", "default_company")
		if not company:
			return

	# NOTE: we translate Payment Gateway account name because that is going to be used by the end user
	bank_account = artech_engine.db.get_value(
		"Account",
	...
	if not bank_account:
		artech_engine.msgprint(_("Payment Gateway Account not created, please create one manually."))
		return
...
def update_cost_center(
	...
	if cost_center_number:
		artech_engine.db.set_value("Cost Center", docname, "cost_center_number", cost_center_number.strip())
	else:
		artech_engine.db.set_value("Cost Center", docname, "cost_center_number", "")

	artech_engine.db.set_value("Cost Center", docname, "cost_center_name", cost_center_name.strip())

	new_name = get_autoname_with_number(cost_center_number, cost_center_name, company)
	if docname != new_name:
		artech_engine.rename_doc("Cost Center", docname, new_name, force=1, merge=merge)
		return new_name
...
def validate_field_number(doctype_name, docname, number_value, company, field_name):
	...
	if number_value:
		...
		doctype_with_same_number = artech_engine.db.get_value(doctype_name, filters)
...
def get_autoname_with_number(number_value, doc_title, company):
	...
	company_abbr = artech_engine.get_cached_value("Company", company, "abbr")
...
def parse_naming_series_variable(doc, variable):
	if variable in ["FY", "TFY"]:
		...
		if doc:
			company = doc.get("company") or artech_engine.db.get_default("company")
		else:
			company = artech_engine.db.get_default("company")

		return artech_engine.db.get_value("Company", company, "abbr") if company else ""
		...
		if doc and doc.doctype in ["Batch", "Serial No"] and doc.reference_doctype and doc.reference_name:
			doc = artech_engine.get_doc(doc.reference_doctype, doc.reference_name)
		...
		date = (
			...
			if doc and artech_engine.get_single_value("Global Defaults", "use_posting_datetime_for_naming_documents")
			else now_datetime()
		)
...
def get_coa(doctype: str, parent: str, is_root: bool | None = None, chart: str | None = None):
	...
	chart = chart if chart else artech_engine.flags.chart
	artech_engine.flags.chart = chart
...
def repost_gle_for_stock_vouchers(
	...
	from artech.accounts.general_ledger import toggle_debit_credit_if_negative
	...
	precision = get_field_precision(artech_engine.get_meta("GL Entry").get_field("debit")) or 2
	...
	for stock_vouchers_chunk in create_batch(stock_vouchers, GL_REPOSTING_CHUNK):
		...
		for voucher_type, voucher_no in stock_vouchers_chunk:
			...
			voucher_obj = artech_engine.get_lazy_doc(voucher_type, voucher_no)
...
def get_future_stock_vouchers(posting_date, posting_time, for_warehouses=None, for_items=None, company=None):
	...
	future_stock_vouchers = artech_engine.db.sql(
...
def get_stock_accounts(company, voucher_type=None, voucher_no=None, accounts=None):
	stock_accounts = [
		d.name
		for d in artech_engine.db.get_all("Account", {"account_type": "Stock", "company": company, "is_group": 0})
	]
	...
	if accounts:
		...
		if voucher_type == "Journal Entry":
			stock_accounts = [
				d.account
				for d in artech_engine.db.get_all(
					"Journal Entry Account",
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	account_table = artech_engine.qb.DocType("Account")
	query = (
		artech_engine.qb.from_(account_table)
...
def check_and_delete_linked_reports(report):
	...
	icons = artech_engine.get_all("Desktop Icon", fields=["name"], filters={"_report": report})
	if icons:
		for icon in icons:
...
def create_err_and_its_journals(company: dict) -> None:
	err = artech_engine.new_doc("Exchange Rate Revaluation")
	...
	if err.accounts:
		...
		if company.submit_err_jv:
			...
			jv and artech_engine.get_doc("Journal Entry", jv).submit()
...
def _auto_create_exchange_rate_revaluation_for(frequency: str) -> None:
	...
	"""
	companies = artech_engine.db.get_all(
		"Company",
	...
	if companies:
		for company in companies:
			artech_engine.enqueue(
				"artech.accounts.utils.create_err_and_its_journals",
				company=company,
				queue="long",
			)
...
def create_payment_ledger_entry(
	...
	if gl_entries:
		...
		for entry in ple_map:
			ple = artech_engine.get_doc(entry)

			if cancel:
				delink_original_entry(ple, partial_cancel=partial_cancel)
				if is_immutable_ledger_enabled():
					ple.delinked = 0
					ple.posting_date = artech_engine.form_dict.get("posting_date") or getdate()
...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	from artech.accounts.doctype.dunning.dunning import update_linked_dunnings

	if not voucher_type or not voucher_no:
		return

	if voucher_type in get_advance_payment_doctypes():
		ref_doc = artech_engine.get_lazy_doc(voucher_type, voucher_no)
	...
	vouchers = [artech_engine._dict({"voucher_type": voucher_type, "voucher_no": voucher_no})]
	common_filter = []
	common_filter.append(ple.party_type == party_type)
	common_filter.append(ple.party == party)
...
def create_gain_loss_journal(
	...
	party_account_currency = artech_engine.get_cached_value("Account", party_account, "account_currency")

	if not gain_loss_account:
		artech_engine.throw(_("Please set default Exchange Gain/Loss Account in Company {}").format(company))
	gain_loss_account_currency = get_account_currency(gain_loss_account)
	company_currency = artech_engine.get_cached_value("Company", company, "default_currency")

	if gain_loss_account_currency != company_currency:
		artech_engine.throw(_("Currency for {0} must be {1}").format(gain_loss_account, company_currency))

	journal_account = artech_engine._dict(
		{
			...
			"exchange_rate": 0,
			"cost_center": cost_center or artech.get_default_cost_center(company),
			"project": project,
...
def get_party_types_from_account_type(account_type):
	return artech_engine.db.get_all("Party Type", {"account_type": account_type}, pluck="name")
...
def get_advance_payment_doctypes(payment_type=None):
	...
	"""
	if payment_type:
		return artech_engine.get_hooks(f"advance_payment_{payment_type}_doctypes") or []

	return (artech_engine.get_hooks("advance_payment_receivable_doctypes") or []) + (
		artech_engine.get_hooks("advance_payment_payable_doctypes") or []
	)


def run_ledger_health_checks():
	health_monitor_settings = artech_engine.get_doc("Ledger Health Monitor")
	if health_monitor_settings.enable_health_monitor:
		...
		if health_monitor_settings.debit_credit_mismatch:
			for x in health_monitor_settings.companies:
				...
				voucher_wise = artech_engine.get_doc("Report", "Voucher-wise Balance")
				res = voucher_wise.execute_script_report(filters=filters)
				for x in res[1]:
					doc = artech_engine.new_doc("Ledger Health")
		...
		if health_monitor_settings.general_and_payment_ledger_mismatch:
			for x in health_monitor_settings.companies:
				...
				gl_pl_comparison = artech_engine.get_doc("Report", "General and Payment Ledger Comparison")
...
def sync_auto_reconcile_config(auto_reconciliation_job_trigger: int = 15):
	auto_reconciliation_job_trigger = auto_reconciliation_job_trigger or artech_engine.get_single_value(
	...
	sch_event = artech_engine.get_doc(
		"Scheduler Event", {"scheduled_against": "Process Payment Reconciliation", "method": method}
	)
	if artech_engine.db.get_value("Scheduled Job Type", {"method": method}):
		artech_engine.get_doc(
			"Scheduled Job Type",
			{
				"method": method,
...
def build_qb_match_conditions(doctype, user=None) -> list:
	...
	criterion = []
	apply_strict_user_permissions = artech_engine.get_system_settings("apply_strict_user_permissions")
...
def is_immutable_ledger_enabled():
	return artech_engine.get_single_value("Accounts Settings", "enable_immutable_ledger")
