artech_engine.pages["bom-comparison-tool"].on_page_load = function (wrapper) {
	...
	new artech.BOMComparisonTool(page);
};

artech.BOMComparisonTool = class BOMComparisonTool {
	constructor(page) {
		this.page = page;
		this.make_form();
	}
	...
	make_form() {
		this.form = new artech_engine.ui.FieldGroup({
			fields: [
				{
					label: __("BOM 1"),
					fieldname: "name1",
	...
	fetch_and_render() {
		...
		artech_engine
			.call("artech.manufacturing.doctype.bom.bom.get_bom_diff", {
				bom1: name1,
				bom2: name2,
			})
			.then((r) => {
				let diff = r.message;
				artech_engine.model.with_doctype("BOM", () => {
	...
	render(doctype, name1, name2, diff) {
		let change_html = (title, doctype, changed) => {
			let values_changed = this.get_changed_values(doctype, changed)
				.map((change) => {
					...
					return `
						<tr>
							<td>${artech_engine.meta.get_label(doctype, fieldname)}</td>
							<td>${value1}</td>
							<td>${value2}</td>
						</tr>
					`;
		...
		let table_changes = Object.keys(row_changes_by_fieldname)
			.map((fieldname) => {
				...
				let df = artech_engine.meta.get_docfield(doctype, fieldname);
				...
				let html = changes
					.map((change) => {
						...
						<tr>
							${th}
							<td>${artech_engine.meta.get_label(child_doctype, fieldname)}</td>
							<td>${value1}</td>
							<td>${value2}</td>
						</tr>
					`;
		...
		let get_added_removed_html = (title, grouped_items) => {
			return Object.keys(grouped_items)
				.map((fieldname) => {
					...
					let df = artech_engine.meta.get_docfield(doctype, fieldname);
					let fields = artech_engine.meta.get_docfields(df.options).filter((df) => df.in_list_view);
