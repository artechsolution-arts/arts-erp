...
class ProductionPlanReport:
	...
	def get_open_orders(self):
		...
		parent = artech_engine.qb.DocType(doctype)
		query = None
		...
		if doctype == "Work Order":
			...
			child = artech_engine.qb.DocType(f"{doctype} Item")
			query = (
				artech_engine.qb.from_(parent)
				.from_(child)
				.select(
	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			for d in self.orders:
				bom_no = d.bom_no or artech_engine.get_cached_value("Item", d.production_item, "default_bom")
			...
			bom_item = artech_engine.qb.DocType(bom_item_doctype)
	...
	def get_purchase_details(self):
		...
		purchased_items = artech_engine.get_all(
			"Purchase Order Item",
			fields=[
				"item_code",
