...
from artech.tests.utils import ArtechTestSuite, ReportFilters, ReportName, execute_script_report


class TestManufacturingReports(ArtechTestSuite):
	def setUp(self):
		self.setup_default_filters()

	def setup_default_filters(self):
		self.last_bom = artech_engine.get_last_doc("BOM").name
		...
		if artech_engine.db.a_row_exists("Production Plan"):
			self.REPORT_FILTER_TEST_CASES.append(
				("Production Plan Summary", {"production_plan": artech_engine.get_last_doc("Production Plan").name})
			)
