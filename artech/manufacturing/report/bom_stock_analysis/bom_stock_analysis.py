...
from artech_engine.query_builder.functions import Floor, IfNull, Sum
from artech_engine.utils import flt, fmt_money
...
def fmt_qty(value):
	...
	return artech_engine.utils.fmt_money(value, precision=2, currency=None)


def fmt_rate(value):
	"""Format a currency rate for display as a string."""
	currency = artech_engine.defaults.get_global_default("currency")
	return artech_engine.utils.fmt_money(value, precision=2, currency=currency)
...
def get_bom_data(filters):
	...
	bom_item = artech_engine.qb.DocType(bom_item_table)
...
def get_manufacturer_records():
	...
	manufacture_details = artech_engine._dict()
...
def get_producible_fg_items(filters):
	...
	if not warehouse:
		artech_engine.throw(_("Warehouse is required to get producible FG Items"))

	warehouse_details = artech_engine.db.get_value("Warehouse", warehouse, ["lft", "rgt"], as_dict=1)
