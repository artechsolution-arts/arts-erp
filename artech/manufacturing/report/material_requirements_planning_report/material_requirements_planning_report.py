...
class MaterialRequirementsPlanningReport:
	...
	def add_non_planned_orders(self, items):
		_adhoc_so_details = artech_engine._dict({})
		...
		if self.filters.get("warehouse"):
			...
			if artech_engine.db.get_value("Warehouse", self.filters.get("warehouse"), "is_group"):
	...
	def get_item_wise_bin_details(self):
		...
		_bin_details = artech_engine._dict({})
	...
	def update_mps_data_with_bin_details(self, bin_details):
		...
		sales_order_items = artech_engine.get_all(
			"Sales Order Item",
	...
	def get_detailed_view_chart_data(self, data):
		chart_data = artech_engine._dict({})
		i = 0
		...
		for row in sorted_data:
			...
			if delivery_date not in chart_data:
				i += 1
				chart_data[delivery_date] = artech_engine._dict(
					{
						"demand": 0.0,
						"supply": 0.0,
					}
			...
			for d in data:
				if getdate(d.delivery_date) >= getdate(row.from_date) and getdate(d.delivery_date) <= getdate(
					...
					if row.from_date not in chart_data:
						...
						labels.append(label)
						chart_data[row.from_date] = artech_engine._dict(
							{
								"demand": 0.0,
								"supply": 0.0,
							}
	...
	def get_bucket_view_data(self, data):
		...
		item_wise_data = artech_engine._dict({})
		for item in data:
			if item.item_code not in item_wise_data:
				item_wise_data[item.item_code] = artech_engine._dict(
					{
		...
		for row in item_wise_data:
			new_data.append(artech_engine._dict(item_wise_data[row]))
	...
	def get_detailed_view_data(self):
		...
		for row in sorted_data:
			rm_details = self.item_rm_details.get(row.item_code) or artech_engine._dict({})
			...
			if i != 0:
				data.append(artech_engine._dict({}))
	...
	def get_work_order_data(self):
		wo_details = artech_engine._dict({})
	...
	def get_purchase_order_data(self):
		po_details = artech_engine._dict({})

		parent_doctype = artech_engine.qb.DocType("Purchase Order")
		doctype = artech_engine.qb.DocType("Purchase Order Item")
	...
	def get_sales_order_data(self):
		...
		so_details = artech_engine._dict({})

		parent_doctype = artech_engine.qb.DocType("Sales Order")
		doctype = artech_engine.qb.DocType("Sales Order Item")
	...
	def get_packed_items_sales_order(self):
		parent_doctype = artech_engine.qb.DocType("Sales Order")
		so_item = artech_engine.qb.DocType("Sales Order Item")
		doctype = artech_engine.qb.DocType("Packed Item")
	...
	def get_subcontracted_data(self):
		parent_doctype = artech_engine.qb.DocType("Subcontracting Order")
		doctype = artech_engine.qb.DocType("Subcontracting Order Item")
	...
	def get_mps_data(self):
		doctype = artech_engine.qb.DocType("Master Production Schedule")
		child_doctype = artech_engine.qb.DocType("Master Production Schedule Item")
	...
	def get_items_from_mps(self, mps_data):
		...
		if self.filters.mps:
			sales_forecasts = artech_engine.get_all(
			...
			if sales_forecasts:
				sales_forecast_items = artech_engine.get_all(
					"Sales Forecast Item",
	...
	def get_raw_materials_data(self, items):
		item_wise_rm_details = artech_engine._dict()
		for item_code in items:
			if item_code not in item_wise_rm_details:
				item_wise_rm_details[item_code] = artech_engine.db.get_value(
					"Item",
					item_code,
	...
	def get_raw_materials(self, bom_no, indent=0):
		...
		raw_materials = artech_engine.get_all(
			"BOM",
	...
	def get_sales_forecast_data(self):
		forecast_doc = artech_engine.qb.DocType("Sales Forecast")
		doctype = artech_engine.qb.DocType("Sales Forecast Item")

		query = (
			artech_engine.qb.from_(forecast_doc)
			.inner_join(doctype)
		...
		if self.filters.mps:
			forecast = artech_engine.db.get_value("Master Production Schedule", self.filters.mps, "sales_forecast")
...
def get_item_details(item_code, company):
	...
	) or artech_engine._dict({"safety_stock": 0})

	default_data = artech_engine.db.get_value(
		"Item Default",
...
def get_item_lead_time(item_code, type_of_material):
	doctype = artech_engine.qb.DocType("Item Lead Time")

	query = artech_engine.qb.from_(doctype).where(doctype.item_code == item_code)
...
def get_item_capacity(item_code, bucket_size):
	capacity = artech_engine.db.get_value(
		"Item Lead Time",
		item_code,
		"capacity_per_day",
	)
...
def make_order(selected_rows: str | list, company: str, warehouse: str | None = None, mps: str | None = None):
	if not artech_engine.has_permission("Purchase Order", "create"):
		artech_engine.throw(_("Not permitted to make Purchase Orders"), artech_engine.PermissionError)

	if isinstance(selected_rows, str):
		selected_rows = parse_json(selected_rows)

	if not artech_engine.db.exists("Company", company):
	...
	for (supplier, release_date), items in purchase_orders.items():
		...
		if len(po.items) > 0:
			...
			artech_engine.msgprint(
				_("Purchase Order {0} created").format(artech_engine.bold(po.name)),
				alert=True,
			)
...
def make_work_orders(work_orders, company, warehouse=None, mps=None):
	for item in work_orders:
		...
		wo.insert()
		artech_engine.msgprint(
			_("Work Order {0} created").format(artech_engine.bold(wo.name)),
			alert=True,
		)
...
def get_item_uom(item_code):
	return artech_engine.get_cached_value("Item", item_code, ["stock_uom", "purchase_uom"], as_dict=True)


@artech_engine.request_cache
def is_whole_number(uom):
	return artech_engine.get_cached_value("UOM", uom, "must_be_whole_number")
