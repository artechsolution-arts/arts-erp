...
from artech_engine.utils import cint, cstr, date_diff, today

from artech.manufacturing.doctype.bom_update_log.bom_updation_utils import (
	get_leaf_boms,
...
class BOMMissingError(artech_engine.ValidationError):
	pass
...
class BOMUpdateLog(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.bom_update_batch.bom_update_batch import BOMUpdateBatch
	...
	def validate_same_bom(self):
		if cstr(self.current_bom) == cstr(self.new_bom):
			artech_engine.throw(_("Current BOM and New BOM can not be same"))

	def validate_bom_items(self):
		current_bom_item = artech_engine.db.get_value("BOM", self.current_bom, "item")
		new_bom_item = artech_engine.db.get_value("BOM", self.new_bom, "item")

		if current_bom_item != new_bom_item:
			artech_engine.throw(_("The selected BOMs are not for the same item"))

	def validate_bom_cost_update_in_progress(self):
		"If another Cost Updation Log is still in progress, dont make new ones."

		wip_log = artech_engine.get_all(
			"BOM Update Log",
		...
		if wip_log and date_diff(today(), wip_log[0].modified) < 1:
			log_link = artech_engine.utils.get_link_to_form("BOM Update Log", wip_log[0].name)
			artech_engine.throw(
	...
	def on_submit(self):
		if self.update_type == "Replace BOM":
			...
			artech_engine.enqueue(
				method="artech.manufacturing.doctype.bom_update_log.bom_update_log.run_replace_bom_job",
				doc=self,
				boms=boms,
				timeout=40000,
			...
			artech_engine.enqueue(
				method="artech.manufacturing.doctype.bom_update_log.bom_update_log.process_boms_cost_level_wise",
				queue="long",
				update_doc=self,
				now=artech_engine.in_test,
				enqueue_after_commit=True,
...
def run_replace_bom_job(
	...
	try:
		...
		boms = artech_engine._dict(boms or {})
...
def queue_bom_cost_jobs(current_boms_list: list[str], update_doc: "BOMUpdateLog", current_level: int) -> None:
	...
	while current_boms_list:
		...
		artech_engine.enqueue(
			method="artech.manufacturing.doctype.bom_update_log.bom_updation_utils.update_cost_in_level",
			doc=update_doc,
...
def resume_bom_cost_update_jobs():
	...
	in_progress_logs = artech_engine.db.get_all(
		"BOM Update Log",
	...
	for log in in_progress_logs:
		...
		bom_batches = artech_engine.db.get_all(
			"BOM Update Batch",
		...
		if status == "Completed":
			artech_engine.db.delete("BOM Update Batch", {"parent": log.name})

		if parent_boms:  # there is a next level to process
			process_boms_cost_level_wise(
				update_doc=artech_engine.get_doc("BOM Update Log", log.name), parent_boms=parent_boms
			)
