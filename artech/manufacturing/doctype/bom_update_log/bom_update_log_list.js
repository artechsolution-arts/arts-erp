artech_engine.listview_settings["BOM Update Log"] = {
	add_fields: ["status"],
	get_indicator: (doc) => {
		let status_map = {
			Queued: "orange",
	...
	onload: () => {
		if (!artech_engine.model.can_write("Log Settings")) {
			return;
		}
		...
		artech_engine.require("logtypes.bundle.js", () => {
			artech_engine.utils.logtypes.show_log_retention_message(cur_list.doctype);
		});
	},
};
