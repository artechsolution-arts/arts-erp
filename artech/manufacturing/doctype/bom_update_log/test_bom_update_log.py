...
from artech.manufacturing.doctype.bom_update_log.bom_update_log import (
	BOMMissingError,
	resume_bom_cost_update_jobs,
)
from artech.manufacturing.doctype.bom_update_tool.bom_update_tool import (
	enqueue_replace_bom,
	enqueue_update_cost,
)
...
class TestBOMUpdateLog(ArtechTestSuite):
	"Test BOM Update Tool Operations via BOM Update Log."

	def setUp(self):
		self.load_test_records("BOM")
		bom_doc = artech_engine.copy_doc(self.globalTestRecords["BOM"][0])
		bom_doc.items[1].item_code = "_Test Item"
		bom_doc.insert()

		self.boms = artech_engine._dict(
	...
	def test_bom_update_log_validate(self):
		...
		with self.assertRaises(artech_engine.ValidationError):
			enqueue_replace_bom(boms=artech_engine._dict(current_bom=self.boms.new_bom, new_bom=self.boms.new_bom))

		with self.assertRaises(artech_engine.ValidationError):
			enqueue_replace_bom(boms=artech_engine._dict(current_bom=self.boms.new_bom, new_bom="Dummy BOM"))
	...
	def test_bom_replace_for_root_bom(self):
		...
		exploded_items = artech_engine.get_all(
		...
		old_bom = artech_engine.db.get_value("BOM", {"item": "B-Item E"}, "name")
		bom_tree = {"B-Item E": {"B-Item G": {}}}

		new_bom = create_nested_bom(bom_tree, prefix="")
		enqueue_replace_bom(boms=artech_engine._dict(current_bom=old_bom, new_bom=new_bom.name))
...
def remove_bom(item_code):
	boms = artech_engine.get_all("BOM", fields=["docstatus", "name"], filters={"item": item_code})

	for row in boms:
		if row.docstatus == 1:
			artech_engine.get_doc("BOM", row.name).cancel()

		artech_engine.delete_doc("BOM", row.name)
