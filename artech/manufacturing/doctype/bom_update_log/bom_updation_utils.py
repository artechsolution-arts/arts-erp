...
if TYPE_CHECKING:
	from artech.manufacturing.doctype.bom_update_log.bom_update_log import BOMUpdateLog
...
def replace_bom(boms: dict, log_name: str) -> None:
	...
	artech_engine.cache().delete_key("bom_children")
	parent_boms = get_ancestor_boms(new_bom)

	for bom in parent_boms:
		bom_obj = artech_engine.get_doc("BOM", bom)
...
def update_cost_in_level(doc: "BOMUpdateLog", bom_list: list[str], batch_name: int | str) -> None:
	...
	try:
		status = artech_engine.db.get_value("BOM Update Log", doc.name, "status")
		if status == "Failed":
			return

		update_cost_in_boms(bom_list=bom_list)  # main updation logic

		bom_batch = artech_engine.qb.DocType("BOM Update Batch")
		(
			artech_engine.qb.update(bom_batch)
...
def get_ancestor_boms(new_bom: str, bom_list: list | None = None) -> list:
	...
	for d in parents:
		if new_bom == d.parent:
			artech_engine.throw(_("BOM recursion: {0} cannot be child of {1}").format(new_bom, d.parent))
...
def update_new_bom_in_bom_items(unit_cost: float, current_bom: str, new_bom: str) -> None:
	bom_item = artech_engine.qb.DocType("BOM Item")
	(
		artech_engine.qb.update(bom_item)
...
def get_bom_unit_cost(bom_name: str) -> float:
	...
	new_bom_unitcost = (
		artech_engine.qb.from_(bom).select(bom.total_cost / bom.quantity).where(bom.name == bom_name).run()
	)

	return artech_engine.utils.flt(new_bom_unitcost[0][0])
...
def update_cost_in_boms(bom_list: list[str]) -> None:
	...
	for index, bom in enumerate(bom_list):
		bom_doc = artech_engine.get_doc("BOM", bom, for_update=True)
		bom_doc.calculate_cost(save_updates=True, update_hour_rate=True)
		bom_doc.db_update()

		if (index % 50 == 0) and not artech_engine.in_test:
			artech_engine.db.commit()  # nosemgrep
...
def set_values_in_log(log_name: str, values: dict[str, Any], commit: bool = False) -> None:
	...
	bom_update_log = artech_engine.qb.DocType("BOM Update Log")
	query = artech_engine.qb.update(bom_update_log).where(bom_update_log.name == log_name)
