...
from artech_engine.utils import flt, random_string
from artech_engine.utils.data import add_to_date, now, today

from artech.manufacturing.doctype.job_card.job_card import (
...
from artech.manufacturing.doctype.work_order.work_order import WorkOrder, make_work_order
...
class TestJobCard(ArtechTestSuite):
	def setUp(self):
	...
	def make_bom_for_jc_tests(self):
		bom = artech_engine.copy_doc(self.globalTestRecords["BOM"][2])
	...
	def test_quality_inspection_mandatory_check(self):
		...
		final_bom = artech_engine.new_doc(
			"BOM",
			item=final.name,
			quantity=1,
			with_operations=1,
		...
		work_order.submit()
		job_card = artech_engine.get_all("Job Card", filters={"work_order": work_order.name, "operation": "Cutting"})
		job_card_doc = artech_engine.get_doc("Job Card", job_card[0].name)
		job_card_doc.append(
			"time_logs",
			{
		...
		self.assertRaises(artech_engine.ValidationError, job_card_doc.submit)
	...
	def test_job_card_operations(self):
		...
		if job_cards:
			...
			artech_engine.db.set_value("Job Card", job_card.name, "operation_row_number", job_card.operation_id)

			doc = artech_engine.get_doc("Job Card", job_card.name)
	...
	def test_job_card_with_different_work_station(self):
		...
		if job_card:
			workstation = artech_engine.db.get_value(
			...
			completed_qty = artech_engine.db.get_value(
	...
	def test_job_card_overlap(self):
		...
		jc1 = artech_engine.get_last_doc("Job Card", {"work_order": self.work_order.name})
		jc2 = artech_engine.get_last_doc("Job Card", {"work_order": wo2.name})

		employee = artech_engine.db.get_all("Employee", {"first_name": "_Test Employee"})[0].name
	...
	def test_job_card_overlap_with_capacity(self):
		...
		workstation = make_workstation(workstation_name=random_string(5)).name
		artech_engine.db.set_value("Workstation", workstation, "production_capacity", 1)
		...
		artech_engine.db.set_value("Workstation", workstation, "production_capacity", 2)
		jc2.load_from_db()
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		job_card_name = artech_engine.db.get_value("Job Card", {"work_order": self.work_order.name})
		job_card = artech_engine.get_doc("Job Card", job_card_name)
	...
	@ArtechTestSuite.change_settings("Manufacturing Settings", {"job_card_excess_transfer": 1})
	def test_job_card_excess_material_transfer(self):
		...
		job_card = artech_engine.get_last_doc("Job Card", {"work_order": self.work_order.name})
	...
	@ArtechTestSuite.change_settings("Manufacturing Settings", {"job_card_excess_transfer": 0})
	...
	def test_job_card_excess_material_transfer_with_no_reference(self):
		...
		self.assertRaises(artech_engine.ValidationError, transfer_entry_1.insert)
	...
	def test_job_card_material_transfer_correctness(self):
		...
		job_card_name = artech_engine.db.get_value(
	...
	def test_corrective_costing(self):
		...
		corrective_action = artech_engine.get_doc(
			doctype="Operation", is_corrective_operation=1, name=artech_engine.generate_hash()
		).insert()
	...
	def test_if_corrective_jc_ops_cost_is_added_to_manufacture_stock_entry(self):
		...
		job_card = artech_engine.get_last_doc("Job Card", {"work_order": wo.name})
		...
		artech_engine.get_doc(stock_entry).submit()

		from artech.manufacturing.doctype.work_order.work_order import make_job_card
	...
	def test_job_card_statuses(self):
		...
		jc = artech_engine.new_doc("Job Card")
	...
	def test_job_card_material_request_and_bom_details(self):
		from artech.stock.doctype.material_request.material_request import make_stock_entry

		create_bom_with_multiple_operations()
		work_order = make_wo_with_transfer_against_jc()

		job_card_name = artech_engine.db.get_value("Job Card", {"work_order": work_order.name}, "name")
	...
	def test_job_card_proccess_qty_and_completed_qty(self):
		from artech.manufacturing.doctype.routing.test_routing import (
			create_routing,
			setup_bom,
			setup_operations,
		)
		...
		first_job_card = artech_engine.get_all(
			"Job Card",
		...
		jc = artech_engine.get_doc("Job Card", first_job_card)
		...
		second_job_card = artech_engine.get_all(
			"Job Card",
		...
		jc2 = artech_engine.get_doc("Job Card", second_job_card)
		...
		self.assertRaises(artech_engine.ValidationError, jc2.save)
		...
		s = artech_engine.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 10))
		s.submit()
	...
	def test_op_cost_calculation(self):
		...
		s = artech_engine.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 4))
		s.submit()
		...
		job_card = artech_engine.get_last_doc("Job Card", {"work_order": wo_doc.name})
		job_card.append(
			"time_logs",
			{
		...
		s = artech_engine.get_doc(make_stock_entry_for_wo(wo_doc.name, "Manufacture", 6))
		self.assertEqual(s.additional_costs[0].amount, 8)

	def test_co_by_product_for_sfg_flow(self):
		from artech.manufacturing.doctype.operation.test_operation import make_operation

		artech_engine.db.set_value("UOM", "Nos", "must_be_whole_number", 0)
		...
		job_card = artech_engine.get_doc(
			"Job Card",
		...
		manufacturing_entry = artech_engine.get_doc(job_card.make_stock_entry_for_semi_fg_item())
	...
	def test_secondary_items_without_sfg(self):
		for row in artech_engine.get_doc("BOM", self.work_order.bom_no).items:
			make_stock_entry(
				item_code=row.item_code,
		...
		s = artech_engine.get_doc(make_stock_entry_for_wo(self.work_order.name, "Manufacture"))
		s.submit()
...
def create_bom_with_multiple_operations():
	...
	test_record = artech_engine.get_test_records("BOM")[2]
	bom_doc = artech_engine.get_doc(test_record)
