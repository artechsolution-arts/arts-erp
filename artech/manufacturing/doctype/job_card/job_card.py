...
from artech_engine.query_builder.functions import IfNull, Max, Min, Sum
...
from artech.controllers.stock_controller import (
	QualityInspectionNotSubmittedError,
	QualityInspectionRejectedError,
)
from artech.manufacturing.doctype.bom.bom import add_additional_cost, get_bom_items_as_dict
from artech.manufacturing.doctype.manufacturing_settings.manufacturing_settings import (
	get_mins_between_operations,
)
from artech.manufacturing.doctype.workstation_type.workstation_type import get_workstations
from artech.subcontracting.doctype.subcontracting_bom.subcontracting_bom import (
...
class OperationMismatchError(artech_engine.ValidationError):
	pass


class OperationSequenceError(artech_engine.ValidationError):
	pass


class JobCardCancelError(artech_engine.ValidationError):
	pass


class JobCardOverTransferError(artech_engine.ValidationError):
	pass
...
class JobCard(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.job_card_item.job_card_item import JobCardItem
		from artech.manufacturing.doctype.job_card_operation.job_card_operation import JobCardOperation
		from artech.manufacturing.doctype.job_card_scheduled_time.job_card_scheduled_time import (
			JobCardScheduledTime,
		)
		from artech.manufacturing.doctype.job_card_secondary_item.job_card_secondary_item import (
			JobCardSecondaryItem,
		)
		from artech.manufacturing.doctype.job_card_time_log.job_card_time_log import JobCardTimeLog
	...
	def onload(self):
		excess_transfer = artech_engine.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
	...
	def has_stock_entry(self):
		return artech_engine.db.exists("Stock Entry", {"job_card": self.name, "docstatus": ["!=", 2]})
	...
	def set_manufactured_qty(self):
		...
		table = artech_engine.qb.DocType(table_name)
		query = artech_engine.qb.from_(table).where((table.job_card == self.name) & (table.docstatus == 1))

		if self.is_subcontracted:
			query = query.select(Sum(table.qty))
		else:
			child = artech_engine.qb.DocType(child_name)
			query = (
				query.join(child)
	...
	def validate_job_card_qty(self):
		...
		wo_qty = flt(artech_engine.get_cached_value("Work Order", self.work_order, "qty"))

		completed_qty = flt(artech_engine.db.get_value("Work Order Operation", self.operation_id, "completed_qty"))
		...
		job_card_qty = artech_engine.get_all(
			"Job Card",
	...
	def set_secondary_items(self):
		...
		for item_code, values in items_dict.items():
			values = artech_engine._dict(values)
			secondary_item = {
				"item_code": item_code,
		...
		if self.get("time_logs"):
			for d in self.get("time_logs"):
				if d.to_time and get_datetime(d.from_time) > get_datetime(d.to_time):
					artech_engine.throw(_("Row {0}: From time must be less than to time").format(d.idx))
	...
	def get_overlap_for(self, args, open_job_cards=None):
		...
		if self.workstation:
			production_capacity = (
				artech_engine.get_cached_value("Workstation", self.workstation, "production_capacity") or 1
			)
	...
	def get_time_logs(self, args, doctype, open_job_cards=None):
		...
		jc = artech_engine.qb.DocType("Job Card")
		jctl = artech_engine.qb.DocType(doctype)
	...
	def get_open_job_cards(self, employee, workstation=None):
		jc = artech_engine.qb.DocType("Job Card")
		jctl = artech_engine.qb.DocType("Job Card Time Log")

		query = (
			artech_engine.qb.from_(jc)
			.left_join(jctl)
	...
	def check_workstation_time(self, row):
		workstation_doc = artech_engine.get_cached_doc("Workstation", self.workstation)
		if not workstation_doc.working_hours or cint(
			artech_engine.db.get_single_value("Manufacturing Settings", "allow_overtime")
		):
	...
	def get_required_items(self):
		...
		doc = artech_engine.get_doc("Work Order", self.get("work_order"))
			...
			if (
				...
				self.append(
					"items",
					{
						...
						"uom": artech_engine.db.get_value("Item", d.item_code, "stock_uom"),
	...
	def validate_inspection(self):
		action_submit, action_reject = artech_engine.get_single_value(
			"Stock Settings",
			["action_if_quality_inspection_is_not_submitted", "action_if_quality_inspection_is_rejected"],
		)

		bom_inspection_required = artech_engine.get_value("BOM", self.bom_no, "inspection_required")
		operation_inspection_required = artech_engine.get_value(
		...
		if bom_inspection_required and operation_inspection_required:
			...
			qa_status, docstatus = artech_engine.get_value(
	...
	def validate_job_card(self):
		...
		if self.work_order and artech_engine.get_cached_value("Work Order", self.work_order, "status") == "Stopped":
			artech_engine.throw(
		...
		elif artech_engine.db.get_single_value("Manufacturing Settings", "enforce_time_logs"):
			for row in self.time_logs:
	...
	def update_semi_finished_good_details(self):
		if self.operation_id:
			...
			artech_engine.db.set_value("Work Order Operation", self.operation_id, "completed_qty", qty)
			if (
				self.finished_good
				and artech_engine.get_cached_value("Work Order", self.work_order, "production_item")
				== self.finished_good
			):
				_wo_doc = artech_engine.get_doc("Work Order", self.work_order)
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty:
			...
			).format(artech_engine.bold(_("Job Card")), artech_engine.bold(self.name), artech_engine.bold(self.production_item))

			second_part_msg = _(
				"Kindly cancel the Manufacturing Entries first against the work order {0}."
			).format(artech_engine.bold(get_link_to_form("Work Order", self.work_order)))
	...
	def update_work_order_data(self, for_quantity, process_loss_qty, time_in_mins, wo):
		workstation_hour_rate = artech_engine.get_value("Workstation", self.workstation, "hour_rate")
		jc = artech_engine.qb.DocType("Job Card")
		jctl = artech_engine.qb.DocType("Job Card Time Log")
	...
	def set_consumed_qty_in_job_card_item(self, ste_doc):
		...
		itemwise_consumed_qty = artech_engine._dict(query.run(as_list=True))
		...
		for row in ste_doc.items:
			...
			artech_engine.db.set_value("Job Card Item", row.job_card_item, "consumed_qty", consumed_qty)
	...
	def set_transferred_qty_in_job_card_item(self, ste_doc):
		def _get_job_card_items_transferred_qty(ste_doc):
			...
			if job_card_items:
				...
				job_card_items_transferred_qty = artech_engine._dict(query.run(as_list=True))
		...
		def _validate_over_transfer(row, transferred_qty):
			...
			required_qty = artech_engine.db.get_value("Job Card Item", row.job_card_item, "required_qty")
			...
			if is_excess:
				artech_engine.throw(
					_(
						...
						row.idx, artech_engine.bold(required_qty), artech_engine.bold(row.item_code), ste_doc.job_card
					),
		...
		allow_excess = artech_engine.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")
		...
		for row in ste_doc.items:
			...
			artech_engine.db.set_value("Job Card Item", row.job_card_item, "transferred_qty", flt(transferred_qty))
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.work_order and not artech_engine.get_cached_value(
	...
	def set_transferred_qty_in_work_order(self):
		doc = artech_engine.get_doc("Work Order", self.work_order)
	...
	def validate_operation_id(self):
		if (
			...
			and self.operation
			and self.work_order
			and artech_engine.get_cached_value("Work Order Operation", self.operation_row_number, "name")
			!= self.operation_id
		):
	...
	def validate_work_order(self):
		if self.is_work_order_closed():
			artech_engine.throw(_("You can't make any changes to Job Card since Work Order is closed."))
	...
	def is_work_order_closed(self):
		if self.work_order:
			status = artech_engine.get_value("Work Order", self.work_order, "status")
	...
	def update_status_in_workstation(self, status):
		...
		artech_engine.db.set_value("Workstation", self.workstation, "status", status)
	...
	def update_workstation_status(self):
		...
		if not job_cards:
			artech_engine.db.set_value("Workstation", self.workstation, "status", "Off")

		for row in job_cards:
			artech_engine.db.set_value("Workstation", self.workstation, "status", status_map.get(row.status))
			return
	...
	def make_stock_entry_for_semi_fg_item(self, auto_submit: bool = False):
		...
		from artech.stock.doctype.stock_entry_type.stock_entry_type import ManufactureEntry
		...
		ste = ManufactureEntry(
			{
				...
				"project": artech_engine.db.get_value("Work Order", self.work_order, "project"),
			}
		)

		ste.make_stock_entry()
		ste.stock_entry.flags.ignore_mandatory = True
		wo_doc = artech_engine.get_doc("Work Order", self.work_order)
		add_additional_cost(ste.stock_entry, wo_doc, self)

		ste.stock_entry.pro_doc = artech_engine.get_doc("Work Order", self.work_order)
...
def make_time_log(kwargs: str | dict):
	...
	kwargs = artech_engine._dict(kwargs)
	doc = artech_engine.get_doc("Job Card", kwargs.job_card_id)
...
def get_operations(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	if not filters.get("work_order"):
		artech_engine.msgprint(_("Please select a Work Order first."))
		return []
...
def make_stock_entry(source_name: str, target_doc: Document | str | None = None):
	...
	def set_missing_values(source, target):
		if source.finished_good and not source.target_warehouse:
			artech_engine.throw(_("Please set the Target Warehouse in the Job Card"))

		if not source.skip_material_transfer or source.backflush_from_wip_warehouse:
			if not source.wip_warehouse:
				artech_engine.throw(_("Please set the WIP Warehouse in the Job Card"))
		...
		wo_allows_alternate_item = artech_engine.db.get_value(
			"Work Order", target.work_order, "allow_alternative_item"
		)
		for item in target.items:
			item.allow_alternative_item = int(
				wo_allows_alternate_item
				and artech_engine.get_cached_value("Item", item.item_code, "allow_alternative_item")
			)
...
def get_job_details(start: Any, end: Any, filters: str | dict | None = None):
	...
	from artech_engine.desk.reportview import get_filters_cond

	conditions = get_filters_cond("Job Card", filters, [])

	job_cards = artech_engine.db.sql(
