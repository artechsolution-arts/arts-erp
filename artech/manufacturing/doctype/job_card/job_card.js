...
artech_engine.ui.form.on("Job Card", {
	setup: function (frm) {
		frm.set_query("operation", function () {
			return {
				query: "artech.manufacturing.doctype.job_card.job_card.get_operations",
				filters: {
	...
	refresh: function (frm) {
		...
		if (
			...
			if (
				...
				frm.add_custom_button(__("Start Job"), () => {
					let from_time = artech_engine.datetime.now_datetime();
				...
				frm.add_custom_button(__("Resume Job"), () => {
					frm.call({
						...
						doc: frm.doc,
						args: {
							start_time: artech_engine.datetime.now_datetime(),
						},
						callback() {
							frm.reload_doc();
						},
					...
					if (!frm.doc.is_paused) {
						frm.add_custom_button(__("Pause Job"), () => {
							frm.call({
								...
								doc: frm.doc,
								args: {
									end_time: artech_engine.datetime.now_datetime(),
								},
								callback() {
									frm.reload_doc();
								},
		...
		if (frm.doc.work_order) {
			artech_engine.db.get_value("Work Order", frm.doc.work_order, "transfer_material_against").then((r) => {
	...
	complete_job_card(frm) {
		...
		frm.job_completion_dialog = artech_engine.prompt(
			fields,
			(data) => {
				if (data.qty <= 0) {
					artech_engine.throw(__("Quantity should be greater than 0"));
				}
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				artech_engine.model.open_mapped_doc({
					method: "artech.manufacturing.doctype.job_card.job_card.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card: function (frm, operation, for_operation) {
		artech_engine.call({
			method: "artech.manufacturing.doctype.job_card.job_card.make_corrective_job_card",
			args: {
	...
	operation: function (frm) {
		...
		if (frm.doc.operation && frm.doc.work_order) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.job_card.job_card.get_operation_details",
				args: {
					work_order: frm.doc.work_order,
	...
	make_time_log: function (frm, args) {
		...
		artech_engine.call({
			method: "artech.manufacturing.doctype.job_card.job_card.make_time_log",
			args: {
				args: args,
			},
			freeze: true,
	...
	get_current_time(frm) {
		...
		frm.doc.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += get_seconds_diff(artech_engine.datetime.now_datetime(), d.from_time);
			}
		});
	...
	make_material_request: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.manufacturing.doctype.job_card.job_card.make_material_request",
			frm: frm,
	...
	make_stock_entry: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.manufacturing.doctype.job_card.job_card.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
	...
	source_warehouse(frm) {
		if (frm.doc.source_warehouse) {
			frm.doc.items.forEach((d) => {
				artech_engine.model.set_value(d.doctype, d.name, "source_warehouse", frm.doc.source_warehouse);
			});
		}
	},
});

artech_engine.ui.form.on("Job Card Time Log", {
	completed_qty: function (frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (!row.completed_qty) {
			artech_engine.model.set_value(row.doctype, row.name, {
				time_in_mins: 0,
				to_time: "",
			});
		}
