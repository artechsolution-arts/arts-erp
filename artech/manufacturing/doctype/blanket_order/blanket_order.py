...
class BlanketOrder(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.blanket_order_item.blanket_order_item import BlanketOrderItem
	...
	def validate_duplicate_items(self):
		...
		for item in self.items:
			if item.item_code in item_list:
				artech_engine.throw(_("Note: Item {0} added multiple times").format(artech_engine.bold(item.item_code)))
			item_list.append(item.item_code)

	def update_ordered_qty(self):
		ref_doctype = "Sales Order" if self.blanket_order_type == "Selling" else "Purchase Order"

		trans = artech_engine.qb.DocType(ref_doctype)
		trans_item = artech_engine.qb.DocType(f"{ref_doctype} Item")

		item_ordered_qty = artech_engine._dict(
			(
				artech_engine.qb.from_(trans_item)
				.from_(trans)
	...
	def validate_item_qty(self):
		for d in self.items:
			if flt(d.qty) < 0:
				artech_engine.throw(_("Row {0}: Quantity cannot be negative.").format(d.idx))


@artech_engine.whitelist()
def make_order(source_name: str):
	doctype = artech_engine.flags.args.doctype
	...
	if order_doc.doctype in ("Sales Order", "Purchase Order"):
		...
		if order_data:
			...
			for bo_name, item_data in order_data.items():
				bo_doc = artech_engine.get_doc("Blanket Order", bo_name)
