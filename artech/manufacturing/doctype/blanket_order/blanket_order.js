...
artech_engine.ui.form.on("Blanket Order", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		if (frm.doc.customer && frm.doc.docstatus === 1 && frm.doc.to_date > artech_engine.datetime.get_today()) {
			frm.add_custom_button(
				__("Sales Order"),
				function () {
					artech_engine.model.open_mapped_doc({
						method: "artech.manufacturing.doctype.blanket_order.blanket_order.make_order",
						frm: frm,
						args: {
							doctype: "Sales Order",
						},
	...
	tc_name: function (frm) {
		artech.utils.get_terms(frm.doc.tc_name, frm.doc, function (r) {
			if (!r.exc) {
