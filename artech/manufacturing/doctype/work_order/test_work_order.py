...
from artech_engine.tests import timeout
from artech_engine.utils import add_days, add_months, add_to_date, cint, flt, now, nowdate, nowtime, today

from artech.manufacturing.doctype.job_card.job_card import JobCardCancelError
from artech.manufacturing.doctype.job_card.job_card import make_stock_entry as make_stock_entry_from_jc
from artech.manufacturing.doctype.production_plan.test_production_plan import make_bom
from artech.manufacturing.doctype.work_order.work_order import (
	CapacityError,
	ItemHasVariantError,
	OverProductionError,
...
from artech.stock.doctype.item.test_item import create_item, make_item
from artech.stock.doctype.serial_and_batch_bundle.test_serial_and_batch_bundle import (
	get_batch_from_bundle,
	get_serial_nos_from_bundle,
	make_serial_batch_bundle,
)
from artech.stock.doctype.serial_no.serial_no import get_serial_nos
from artech.stock.doctype.stock_entry import test_stock_entry
...
class TestWorkOrder(ArtechTestSuite):
	def setUp(self):
	...
	def check_planned_qty(self):
		...
		planned1 = artech_engine.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
		...
		# from stores to wip
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 4))
		...
		# from wip to fg
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 4))
		s.insert()
		s.submit()

		self.assertEqual(artech_engine.db.get_value("Work Order", wo_order.name, "produced_qty"), 4)

		planned2 = artech_engine.db.get_value(
			"Bin", {"item_code": "_Test FG Item", "warehouse": "_Test Warehouse 1 - _TC"}, "planned_qty"
		)
	...
	def test_over_production(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 7))
		s.insert()
	...
	def test_reserved_qty_for_partial_completion(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		s.submit()
	...
	def test_production_item(self):
		...
		artech_engine.db.set_value("Item", "_Test FG Item", "end_of_life", "2000-1-1")

		self.assertRaises(artech_engine.ValidationError, wo_order.save)

		artech_engine.db.set_value("Item", "_Test FG Item", "end_of_life", None)
		artech_engine.db.set_value("Item", "_Test FG Item", "disabled", 1)

		self.assertRaises(artech_engine.ValidationError, wo_order.save)

		artech_engine.db.set_value("Item", "_Test FG Item", "disabled", 0)
	...
	def test_reserved_qty_for_production_on_stock_entry(self):
		...
		s = artech_engine.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 2))
		...
		s = artech_engine.get_doc(make_stock_entry(self.wo_order.name, "Manufacture", 2))
	...
	def test_backflush_qty_for_overpduction_manufacture(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)

		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 60))
		s.submit()
		cancel_stock_entry.append(s.name)
		...
		s1 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 50))
		s1.submit()
		...
		for ste in cancel_stock_entry:
			doc = artech_engine.get_doc("Stock Entry", ste)
			doc.cancel()
	...
	def test_reserved_qty_for_stopped_production(self):
		...
		s = artech_engine.get_doc(make_stock_entry(self.wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_secondary_material_qty(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		for d in s.get("items"):
			d.s_warehouse = "Stores - _TC"
		s.insert()
		s.submit()

		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		s.insert()
		s.submit()

		wo_order_details = artech_engine.db.get_value(
	...
	def test_allow_overproduction(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 3))
		s.insert()
		self.assertRaises(StockOverProductionError, s.submit)
	...
	def test_work_order_with_non_stock_item(self):
		...
		if not artech_engine.db.get_value("Item Price", {"item_code": "_Test FG Non Stock Item"}):
		...
		if not artech_engine.db.get_value("BOM", {"item": fg_item, "docstatus": 1}):
			bom = make_bom(
				item=fg_item,
				rate=1000,
		...
		se = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se.insert()
		se.submit()

		ste = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		ste.insert()
	...
	def test_job_card(self):
		...
		ste = artech_engine.get_doc(make_stock_entry(work_order.name, "Material Transfer for Manufacture", 1))
		ste.submit()
		...
		ste1 = artech_engine.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		ste1.submit()
	...
	def test_work_order_material_transferred_qty_with_process_loss(self):
		...
		if not artech_engine.db.exists("Workstation", workstation_name):
			doc = artech_engine.new_doc("Workstation")
			doc.workstation_name = workstation_name
			doc.save()

		if not artech_engine.db.exists("Operation", operation_name):
			doc = artech_engine.new_doc("Operation")
		...
		for row in job_cards:
			...
			doc = artech_engine.get_doc("Job Card", row.name)
			for row in doc.scheduled_time_logs:
				doc.append(
					"time_logs",
					{
	...
	def test_capcity_planning(self):
		...
		data = artech_engine.get_cached_value(
			"BOM",
		...
		if data:
			...
			artech_engine.db.set_single_value("Manufacturing Settings", {"capacity_planning_for_days": 30})
	...
	def test_planned_qty_updates_after_closing_work_order(self):
		...
		planned_before = (
			artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": fg_warehouse}, "planned_qty")
			or 0
		)
		...
		self.assertEqual(artech_engine.db.get_value("Work Order", wo.name, "status"), "Closed")
	...
	def test_work_order_with_non_transfer_item(self):
		artech_engine.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		...
		if not artech_engine.db.get_value("BOM", {"item": fg_item}):
			make_bom(item=fg_item, raw_materials=["_Test FG Item", "_Test FG Item 1"])

		wo = make_wo_order_test_record(production_item=fg_item)
		ste = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		ste.insert()
		ste.submit()
		self.assertEqual(len(ste.items), 1)
		ste1 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_operation_time_with_batch_size(self):
		...
		bom_name = artech_engine.db.get_value("BOM", {"item": fg_item, "is_active": 1, "with_operations": 1}, "name")
	...
	@ArtechTestSuite.change_settings("Global Defaults", {"default_company": "_Test Company"})
	def test_batch_size_for_fg_item(self):
		fg_item = "Test Batch Size Item For BOM 3"
		rm1 = "Test Batch Size Item RM 1 For BOM 3"

		artech_engine.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 0)
		...
		artech_engine.db.set_single_value("Manufacturing Settings", "make_serial_no_batch_from_work_order", 1)
		ste1 = artech_engine.get_doc(make_stock_entry(work_order.name, "Manufacture", 1))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
		...
		batches = artech_engine.get_all("Batch", filters={"reference_name": work_order.name})
		self.assertEqual(len(batches), 3)
		batches = [batch.name for batch in batches]

		ste1 = artech_engine.get_doc(make_stock_entry(work_order.name, "Manufacture", 10))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
				self.assertEqual(row.qty, 10)

				bundle_id = artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		remaining_batches = []
		ste1 = artech_engine.get_doc(make_stock_entry(work_order.name, "Manufacture", 20))
		for row in ste1.get("items"):
			if row.is_finished_item:
				self.assertEqual(row.item_code, fg_item)
	...
	def test_partial_material_consumption(self):
		artech_engine.db.set_single_value("Manufacturing Settings", "material_consumption", 1)
		...
		ste1 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		ste1.submit()
		ste_cancel_list.append(ste1)

		ste3 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Consumption for Manufacture", 2))
		...
		artech_engine.db.set_single_value("Manufacturing Settings", "material_consumption", 0)
	...
	def test_extra_material_transfer(self):
		...
		ste3 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
		...
		ste2 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 2))
	...
	def test_make_stock_entry_for_customer_provided_item(self):
		...
		if not artech_engine.db.exists("BOM", {"item": finished_item}):
		...
		ste = artech_engine.get_doc(make_stock_entry(wo.name, purpose="Material Transfer for Manufacture"))
		ste.insert()
	...
	def test_valuation_rate_missing_on_make_stock_entry(self):
		...
		if not artech_engine.db.get_value("BOM", {"item": item_name}):
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture"))
		self.assertRaises(artech_engine.ValidationError, stock_entry.save)

	def test_wo_completion_with_pl_bom(self):
		from artech.manufacturing.doctype.bom.test_bom import (
		...
		if not artech_engine.db.exists("BOM", bom_no):
		...
		se = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", qty))
		se.get("items")[0].s_warehouse = "Stores - _TC"
		se.get("items")[0].t_warehouse = wip_warehouse
		se.insert()
		se.submit()

		se = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", qty))
		se.insert()
		se.submit()
	...
	def test_job_card_secondary_item(self):
		...
		for job_card in job_cards:
			if job_card.docstatus == 1:
				artech_engine.get_doc("Job Card", job_card.name).cancel()

			artech_engine.delete_doc("Job Card Time Log", job_card.name)
		...
		if not artech_engine.db.get_value("BOM", {"item": item}):
			bom = make_bom(
		...
		job_card = artech_engine.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		update_job_card(job_card)

		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		...
		job_card = artech_engine.db.get_value("Job Card", {"work_order": wo_order.name, "docstatus": 0}, "name")
	...
	def test_close_work_order(self):
		...
		job_cards = artech_engine.db.get_value("Job Card", {"work_order": wo_order.name}, "name")

		if len(job_cards) == len(bom.operations):
			for jc in job_cards:
				job_card_doc = artech_engine.get_doc("Job Card", jc)
				job_card_doc.append(
					"time_logs",
	...
	def test_partial_manufacture_entries(self):
		...
		sm = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 100))
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 90))
		...
		s1 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		...
		s2 = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
	...
	@ArtechTestSuite.change_settings("Manufacturing Settings", {"make_serial_no_batch_from_work_order": 1})
	def test_auto_batch_creation(self):
		from artech.manufacturing.doctype.bom.test_bom import create_nested_bom

		fg_item = artech_engine.generate_hash(length=20)
		child_item = artech_engine.generate_hash(length=20)

		bom_tree = {fg_item: {child_item: {}}}

		create_nested_bom(bom_tree, prefix="")

		item = artech_engine.get_doc("Item", fg_item)
		item.has_batch_no = 1
		...
		except artech_engine.MandatoryError:
		...
		try:
			...
			for row in stock_entry.items:
				if row.item_code == fg_item:
					...
					sn_doc = artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
	...
	def get_serial_nos_for_fg(self, work_order):
		serial_nos = []
		for row in artech_engine.get_all("Serial No", filters={"work_order": work_order}):
	...
	def test_manufacture_entry_mapped_idx_with_exploded_bom(self):
		...
		ste_manu = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
	...
	def test_work_order_multiple_material_transfer(self):
		...
		transfer_entry = artech_engine.get_doc(
		...
		final_transfer_entry = artech_engine.get_doc(  # transfer last RM with For Quantity = 0
	...
	def test_backflushed_batch_raw_materials_based_on_transferred(self):
		...
		transferred_ste_doc = artech_engine.get_doc(
			...
			artech_engine._dict(
				{
					...
					"batches": artech_engine._dict({batch_no: 4}),
		...
		manufacture_ste_doc1 = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 1))
		...
		manufacture_ste_doc2 = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 2))
		...
		bundle_doc = artech_engine.get_doc(
	...
	def test_backflushed_serial_no_raw_materials_based_on_transferred(self):
		...
		# Second Manufacture stock entry
		manufacture_ste_doc2 = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 3))
	...
	def test_backflushed_serial_no_batch_raw_materials_based_on_transferred(self):
		...
		for row in ste_doc.items:
			bundle_doc = artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		bundle_doc = artech_engine.get_doc("Serial and Batch Bundle", bundle)
		for d in bundle_doc.entries:
			self.assertTrue(d.serial_no)
			self.assertTrue(d.batch_no)
			batch_no = artech_engine.get_cached_value("Serial No", d.serial_no, "batch_no")
			self.assertEqual(d.batch_no, batch_no)
	...
	def test_backflushed_batch_raw_materials_based_on_transferred_autosabb(self):
		...
		manufacture_ste_doc = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 4))
	...
	def test_non_consumed_material_return_against_work_order(self):
		...
		ste_doc = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Material Transfer for Manufacture", 5))
		for row in ste_doc.items:
			row.qty += 2
			row.transfer_qty += 2
			test_stock_entry.make_stock_entry(
		...
		ste_doc = artech_engine.get_doc(make_stock_entry(wo_doc.name, "Manufacture", 5))
		for row in ste_doc.items:
	...
	def test_job_card_extra_qty(self):
		...
		job_card = artech_engine.db.get_value("Job Card", {"work_order": wo_order.name}, "name")
		job_card_doc = artech_engine.get_doc("Job Card", job_card)
		...
		job_card2 = artech_engine.copy_doc(job_card_doc)
		job_card2.append(
			"time_logs",
			{
				"from_time": row.from_time,
		...
		self.assertRaises(artech_engine.ValidationError, job_card2.save)

		artech_engine.db.set_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order", 100)
	...
	def test_operating_cost_account(self):
		...
		company = "_Test Company"
		if not artech_engine.db.exists("Account", operating_cost_account):
			artech_engine.get_doc(
				{
					"doctype": "Account",
		...
		artech_engine.db.set_value("Company", company, "default_operating_cost_account", operating_cost_account)

		for item in ["TEST RM OP COST Item 1", "TEST FG OP COST Item"]:
			if not artech_engine.db.exists("Item", item):
		...
		if not artech_engine.db.exists("Workstation", workstation):
		...
		job_cards = artech_engine.get_all("Job Card", filters={"work_order": wo.name})
		for job_card in job_cards:
			job_card_doc = artech_engine.get_doc("Job Card", job_card.name)
		...
		se_doc = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		se_doc.save()
	...
	def test_set_op_cost_and_secondary_items_from_sub_assemblies(self):
		...
		se_doc = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
		se_doc.save()
	...
	def test_get_rm_cost_from_consumption_entry(self):
		from artech.stock.doctype.stock_entry.test_stock_entry import (
		...
		mte = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		mte.items[0].s_warehouse = "Stores - _TC"
		mte.insert().submit()

		mce = artech_engine.get_doc(make_stock_entry(wo.name, "Material Consumption for Manufacture", 10))
		mce.insert().submit()

		me = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		me.insert().submit()
	...
	def test_partial_material_consumption_with_batch(self):
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10))
		stock_entry.submit()
		stock_entry.reload()

		batch_no = get_batch_from_bundle(stock_entry.items[0].serial_and_batch_bundle)

		stock_entry = artech_engine.get_doc(
	...
	def test_disassemby_order(self):
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
		for row in stock_entry.items:
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Disassemble", 1))
		stock_entry.save()
	...
	def test_disassembly_order_with_qty_from_wo_behavior(self):
		...
		se_for_material_tranfer_mfr = artech_engine.get_doc(
		...
		se_for_manufacture = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", wo.qty))
		se_for_manufacture.submit()

		# Disassembly via WO required_items path (no source_stock_entry)
		disassemble_qty = 4
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Disassemble", disassemble_qty))
		...
		disassemble_qty_2 = 2
		stock_entry_2 = artech_engine.get_doc(
			make_stock_entry(
	...
	def test_disassembly_with_multiple_manufacture_entries(self):
		...
		se_for_material_transfer = artech_engine.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", wo.qty)
		)
		...
		se_manufacture1 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		se_manufacture1.submit()

		# Second Manufacture Entry - 7 units
		se_manufacture2 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 7))
		...
		manufacture_entries = artech_engine.get_all(
			"Stock Entry",
			filters={
				"work_order": wo.name,
				"purpose": "Manufacture",
	...
	def test_disassembly_auto_sets_source_stock_entry(self):
		...
		se_transfer = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", wo.qty))
		for item in se_transfer.items:
			item.s_warehouse = wo.wip_warehouse
		se_transfer.save()
		se_transfer.submit()

		se_manufacture = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", wo.qty))
		se_manufacture.submit()

		# Disassemble without specifying source_stock_entry
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Disassemble", 3))
		stock_entry.save()
	...
	def test_disassembly_batch_tracked_items(self):
		from artech.stock.doctype.batch.batch import make_batch
		from artech.stock.doctype.stock_entry.test_stock_entry import (
			make_stock_entry as make_stock_entry_test_record,
		)
		...
		fg_batch_1 = make_batch(artech_engine._dict(item=fg_item))
		fg_batch_2 = make_batch(artech_engine._dict(item=fg_item))

		# Manufacture entry 1 — 3 FG using batch_1 RM/FG
		se_manufacture_1 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		...
		se_manufacture_2 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
	...
	def test_disassembly_serial_tracked_items(self):
		from artech_engine.model.naming import make_autoname
		...
		series = artech_engine.db.get_value("Item", fg_item, "serial_no_series")
	...
	def test_components_alternate_item_for_bom_based_manufacture_entry(self):
		artech_engine.db.set_single_value("Manufacturing Settings", "backflush_raw_materials_based_on", "BOM")
		artech_engine.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 1)
		...
		transfer_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()
		...
		manufacture_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 10))
		...
		artech_engine.db.set_single_value("Manufacturing Settings", "validate_components_quantities_per_bom", 0)
	...
	def test_components_qty_for_bom_based_manufacture_entry(self):
		...
		self.assertRaises(artech_engine.ValidationError, transfer_entry.save)
		...
		self.assertRaises(artech_engine.ValidationError, manufacture_entry.save)
	...
	def test_serial_no_status_for_stock_entry(self):
		...
		ste = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 4))
		...
		for row in serial_nos:
			status = artech_engine.db.get_value("Serial No", row, "status")
			self.assertEqual(status, "Consumed")

	def test_stock_reservation_for_serialized_raw_material(self):
		from artech.stock.doctype.stock_entry.stock_entry_utils import (
			make_stock_entry as make_stock_entry_test_record,
		)
		...
		self.assertTrue(artech_engine.get_all("Stock Reservation Entry", filters={"voucher_no": wo.name}))
		...
		self.assertFalse(artech_engine.get_all("Stock Reservation Entry", filters={"voucher_no": wo1.name}))

		transfer_entry = artech_engine.get_doc(make_stock_entry(wo1.name, "Material Transfer for Manufacture", 10))
		transfer_entry.save()

		self.assertRaises(artech_engine.ValidationError, transfer_entry.submit)
	...
	def test_auto_stock_reservation_for_batched_raw_material(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "auto_reserve_serial_and_batch", 1)
		...
		itemwise_batches = artech_engine._dict()
		for row in bom.exploded_items:
		...
		for row in artech_engine.get_all("Stock Reservation Entry", filters={"voucher_no": wo.name}):
			reservation_entry = artech_engine.get_doc("Stock Reservation Entry", row.name)
	...
	def test_work_order_valuation_auto_pick(self):
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 5))
		...
		original_value = artech_engine.db.get_single_value(
			"Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"
		)
		original_based_on = artech_engine.db.get_single_value("Stock Settings", "pick_serial_and_batch_based_on")

		artech_engine.db.set_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward", 1)
		artech_engine.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", "Expiry")

		stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 5))
		stock_entry.items[0].use_serial_batch_fields = 1
		stock_entry.submit()
		stock_entry.reload()
		...
		artech_engine.db.set_single_value("Stock Settings", "pick_serial_and_batch_based_on", original_based_on)

	def test_operations_time_planning_calculation(self):
		from artech.manufacturing.doctype.routing.test_routing import create_routing, setup_operations
		...
		workstation = artech_engine.get_doc("Workstation", "Test Workstation A")
	...
	def test_allow_additional_material_transfer(self):
		...
		artech_engine.db.set_single_value("Manufacturing Settings", "transfer_extra_materials_percentage", 50)
		wo_order = make_wo_order_test_record(planned_start_date=now(), qty=2)
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 2))
		stock_entry.insert()
		stock_entry.submit()
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 1))
		stock_entry.insert()
		stock_entry.submit()

		wo_order.reload()
		self.assertEqual(wo_order.material_transferred_for_manufacturing, 3)
		artech_engine.db.set_single_value("Manufacturing Settings", "transfer_extra_materials_percentage", 0)
	...
	def test_req_qty_clamping_in_manufacture_entry(self):
		...
		se_transfer_1 = artech_engine.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", 6, wip_warehouse)
		)
		se_transfer_1.insert()
		se_transfer_1.submit()

		stock_entry_1 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 6, fg_warehouse))
		...
		se_transfer_2 = artech_engine.get_doc(
			make_stock_entry(wo.name, "Material Transfer for Manufacture", 20, wip_warehouse)
		)
		se_transfer_2.insert()
		se_transfer_2.submit()

		stock_entry_2 = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 20, fg_warehouse))
	...
	def test_overproduction_allowed_qty(self):
		...
		mt_stock_entry = artech_engine.get_doc(
			make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 10)
		)
		mt_stock_entry.submit()

		fg_stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 10))
	...
	def test_reserved_serial_batch(self):
		...
		original_auto_reserve = artech_engine.db.get_single_value("Stock Settings", "auto_reserve_serial_and_batch")
		original_backflush = artech_engine.db.get_single_value(
		...
		# Transfer 5 qty
		mt_stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 5))
		mt_stock_entry.submit()
		...
		# Manufacture 2 qty
		fg_stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 2))
		fg_stock_entry.submit()
		...
		# Manufacture 3 qty
		fg_stock_entry = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 3))
		fg_stock_entry.submit()
		...
		artech_engine.db.set_single_value("Stock Settings", "auto_reserve_serial_and_batch", original_auto_reserve)
	...
	def test_phantom_bom_item_not_in_additional_cost(self):
		...
		for item_code, rate in [
			...
			if not artech_engine.db.get_value("Item Price", {"item_code": item_code, "price_list": price_list}):
				artech_engine.get_doc(
					{
						"doctype": "Item Price",
						"item_code": item_code,
		...
		phantom_bom = artech_engine.get_doc(
			{
				"doctype": "BOM",
				"item": phantom_item,
				"is_default": 1,
		...
		# Transfer materials
		se_transfer = artech_engine.get_doc(make_stock_entry(wo.name, "Material Transfer for Manufacture", 1))
		se_transfer.insert()
		se_transfer.submit()

		# Manufacture
		se_manufacture = artech_engine.get_doc(make_stock_entry(wo.name, "Manufacture", 1))
	...
	def test_phantom_bom_explosion(self):
		from artech.manufacturing.doctype.bom.test_bom import create_tree_for_phantom_bom_tests
	...
	def test_reserved_qty_for_pp_with_extra_material_transfer(self):
		...
		s = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 1))
	...
	def test_backflush_based_on_in_bom(self):
		...
		backflush_based_on = artech_engine.db.get_single_value(
			"Manufacturing Settings", "backflush_raw_materials_based_on"
		)
		...
		backflush_based_on = artech_engine.db.get_value("BOM", wo_order.bom_no, "backflush_based_on")
		self.assertEqual(backflush_based_on, "Material Transferred for Manufacture")

		material_transfer_entry = artech_engine.get_doc(
		...
		material_transfer_entry.append(
			"items",
			{
				...
				"item_group": artech_engine.get_value("Item", raw_material_2, "item_group"),
				"uom": artech_engine.get_value("Item", raw_material_2, "stock_uom"),
				"conversion_factor": 1,
		...
		manufacture_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 1))
		manufacture_entry.save()
...
def get_reserved_entries(voucher_no, warehouse=None):
	doctype = artech_engine.qb.DocType("Stock Reservation Entry")
	sabb = artech_engine.qb.DocType("Serial and Batch Entry")
...
def make_stock_in_entries_and_get_batches(rm_item, source_warehouse, wip_warehouse):
	...
	for qty, rate in ((5, 100), (5, 200)):
		...
		batch_no = get_batch_from_bundle(stock_entry.items[0].serial_and_batch_bundle)
		batch_doc = artech_engine.get_doc("Batch", batch_no)
...
def make_operation(**kwargs):
	kwargs = artech_engine._dict(kwargs)

	operation_doc = artech_engine.get_doc(
		{
			"doctype": "Operation",
...
def make_workstation(**kwargs):
	...
	workstation_doc = artech_engine.get_doc(
		{
			"doctype": "Workstation",
...
def prepare_boms_for_sub_assembly_test():
	if not artech_engine.db.exists("BOM", {"item": "Test Final SF Item 1"}):
		bom = make_bom(
	...
	if not artech_engine.db.exists("BOM", {"item": "Test Final SF Item 2"}):
		bom = make_bom(
	...
	if not artech_engine.db.exists("BOM", {"item": "Test Final FG Item"}):
		bom = make_bom(
...
def prepare_data_for_workstation_type_check():
	from artech.manufacturing.doctype.operation.test_operation import make_operation
	from artech.manufacturing.doctype.workstation.test_workstation import make_workstation
	from artech.manufacturing.doctype.workstation_type.test_workstation_type import (
	...
	if not artech_engine.db.exists("BOM", {"item": fg_item.name}):
		bom_doc = make_bom(
			item=fg_item.name,
			source_warehouse="Stores - _TC",
		...
		for index, operation in enumerate(operations):
			if not artech_engine.db.exists("BOM Operation", {"parent": bom_doc.name, "operation": operation}):
				bom_doc.append(
					"operations",
					{
						"operation": operation,
...
def update_job_card(job_card, jc_qty=None, days=None):
	employee = artech_engine.db.get_value("Employee", {"status": "Active"}, "name")
	job_card_doc = artech_engine.get_doc("Job Card", job_card)
	job_card_doc.set(
		"secondary_items",
		[
...
def get_secondary_item_details(bom_no):
	secondary_items = {}
	for item in artech_engine.db.sql(
...
def allow_overproduction(fieldname, percentage):
	doc = artech_engine.get_doc("Manufacturing Settings")
...
def make_wo_order_test_record(**args):
	...
	wo_order = artech_engine.new_doc("Work Order")
	wo_order.production_item = args.production_item or args.item or args.item_code or "_Test FG Item"
	wo_order.bom_no = args.bom_no or artech_engine.db.get_value(
