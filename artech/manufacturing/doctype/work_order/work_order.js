...
artech_engine.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("bom_no", function () {
			if (frm.doc.production_item) {
				return {
					query: "artech.controllers.queries.bom",
					filters: { item: cstr(frm.doc.production_item) },
				};
			} else {
				artech_engine.msgprint(__("Please enter Production Item first"));
			}
		});
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "artech.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
		...
		frm.set_query("sales_order", function () {
			if (frm.doc.production_item) {
				return {
					query: "artech.manufacturing.doctype.work_order.work_order.query_sales_order",
					filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			artech.work_order.set_default_warehouse(frm);
		}

		if (frm.doc.docstatus == 0 && frm.doc.bom_no) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.work_order.work_order.check_if_scrap_warehouse_mandatory",
				args: {
	...
	refresh: function (frm) {
		...
		artech.work_order.set_custom_buttons(frm);
		frm.set_intro("");
	...
	create_stock_return_entry: function (frm) {
		artech_engine.call({
			method: "artech.manufacturing.doctype.work_order.work_order.make_stock_return_entry",
			args: {
				work_order: frm.doc.name,
			},
			callback: function (r) {
				if (!r.exc) {
					let doc = artech_engine.model.sync(r.message);
		...
		const dialog = artech_engine.prompt(
			...
			function () {
				...
				if (selected_rows.length == 0) {
					artech_engine.msgprint(__("Please select atleast one operation to create Job Card"));
					return;
				}
				artech_engine.call({
					method: "artech.manufacturing.doctype.work_order.work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		artech.work_order
			...
			.then((data) => {
				if (flt(data.qty) <= 0) {
					artech_engine.msgprint(__("Disassemble Qty cannot be less than or equal to <b>0</b>."));
					return;
				}
				return artech_engine.xcall("artech.manufacturing.doctype.work_order.work_order.make_stock_entry", {
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						...
						artech.in_production_item_onchange = true;
						...
						artech.in_production_item_onchange = false;
					}
				},
			});
		}
	...
	project: function (frm) {
		if (!artech.in_production_item_onchange && !frm.doc.bom_no) {
	...
	additional_operating_cost: function (frm) {
		artech.work_order.calculate_cost(frm.doc);
		artech.work_order.calculate_total_cost(frm);
	},

	on_submit() {
		artech_engine.route_hooks.after_submit = (frm) => {
			frm.reload_doc();
		};
	},
});

artech_engine.ui.form.on("Work Order Item", {
	...
	source_warehouse: function (frm, cdt, cdn) {
		...
		if (!row.item_code) {
			artech_engine.throw(__("Please set the Item Code first"));
	...
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			artech_engine.call({
				method: "artech.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company,
				},
...
artech_engine.ui.form.on("Work Order Operation", {
		...
		if (d.workstation) {
			artech_engine.call({
				...
				callback: function (data) {
					artech_engine.model.set_value(d.doctype, d.name, "hour_rate", data.message.hour_rate);
					artech.work_order.calculate_cost(frm.doc);
					artech.work_order.calculate_total_cost(frm);
				},
			});
...
artech.work_order = {
	set_custom_buttons: function (frm) {
		...
		if (doc.docstatus === 1 && !["Closed", "Completed"].includes(doc.status)) {
			frm.add_custom_button(
				__("Close"),
				function () {
					artech_engine.confirm(__("Once the Work Order is Closed. It can't be resumed."), () => {
						artech.work_order.change_work_order_status(frm, "Closed");
					});
				},
				__("Status")
			);
			if (doc.status != "Stopped" && doc.status != "Completed") {
				frm.add_custom_button(
					__("Stop"),
					function () {
						artech.work_order.change_work_order_status(frm, "Stopped");
					},
					__("Status")
				);
				...
				frm.add_custom_button(
					__("Re-open"),
					function () {
						artech.work_order.change_work_order_status(frm, "Resumed");
					},
					__("Status")
				);
			}

			artech.work_order.setup_stock_reservation(frm);
				...
				if (show_start_btn) {
					...
					if (pending_to_transfer && frm.doc.status != "Stopped") {
						...
						frm.add_custom_button(__("Create Pick List"), function () {
							artech.work_order.create_pick_list(frm);
						});

						var start_btn = frm.add_custom_button(__("Start"), function () {
							artech.work_order.make_se(frm, "Material Transfer for Manufacture");
						});
						...
						if (counter > 0) {
							frm.add_custom_button(
								...
								function () {
									...
									artech.work_order.make_consumption_se(
										frm,
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if (flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing)) {
							...
							let finish_btn = frm.add_custom_button(__("Finish"), function () {
								artech.work_order.make_se(frm, "Manufacture");
							});
	...
	setup_stock_reservation(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (
				...
				frm.add_custom_button(
					__("Reserve"),
					() => artech.stock_reservation.make_entries(frm, "required_items"),
			...
			if (frm.events.has_reserved_stock(frm)) {
				if (frm.doc.skip_transfer || frm.doc.material_transferred_for_manufacturing < frm.doc.qty) {
					frm.add_custom_button(
						__("Unreserve"),
						() => artech.stock_reservation.unreserve_stock(frm),
						__("Stock Reservation")
					);
				}
				...
				frm.add_custom_button(
					__("Reserved Stock"),
					() => artech.stock_reservation.show_reserved_stock(frm, "required_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	set_default_warehouse: function (frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				args: {
					company: frm.doc.company,
				},
				callback: function (r) {
	...
	show_disassembly_prompt: function (frm) {
		...
		return new Promise((resolve, reject) => {
			frm.disassembly_prompt = artech_engine.prompt(
				fields,
				(data) => resolve(data),
				__("Disassemble"),
				__("Create")
	...
	show_prompt_for_qty_input: function (frm, purpose, qty, additional_transfer_entry) {
		...
		return new Promise((resolve, reject) => {
			frm.qty_prompt = artech_engine.prompt(
				fields,
				(data) => {
					max += (frm.doc.qty * (frm.doc.__onload.overproduction_percentage || 0.0)) / 100;

					if (data.qty > max) {
						artech_engine.msgprint(__("Quantity must not be more than {0}", [max]));
						reject();
					}
	...
	make_se: function (frm, purpose, qty, is_additional_transfer_entry) {
		if (qty) {
			artech_engine
				.xcall("artech.manufacturing.doctype.work_order.work_order.make_stock_entry", {
					work_order_id: frm.doc.name,
					purpose: purpose,
					qty: qty,
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		this.show_prompt_for_qty_input(frm, purpose)
			.then((data) => {
				return artech_engine.xcall("artech.manufacturing.doctype.work_order.work_order.create_pick_list", {
					source_name: frm.doc.name,
					for_qty: data.qty,
				});
			})
			.then((pick_list) => {
				artech_engine.model.sync(pick_list);
				artech_engine.set_route("Form", pick_list.doctype, pick_list.name);
			});
	},
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		artech_engine.call({
			method: "artech.manufacturing.doctype.work_order.work_order.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
	...
	change_work_order_status: function (frm, status) {
		...
		artech_engine.call({
			method: `artech.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
