...
artech_engine.ui.form.on("Master Production Schedule", {
	refresh(frm) {
	...
	set_indicator_for_item(frm) {
		frm.set_indicator_formatter("item_code", function (doc) {
			if (doc.order_release_date < artech_engine.datetime.get_today()) {
				return "orange";
			} else if (doc.order_release_date > artech_engine.datetime.get_today()) {
				return "blue";
			} else {
				return "green";
			}
	...
	set_custom_buttons(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("View MRP"), () => {
				...
				artech_engine.set_route("query-report", "Material Requirements Planning Report", {
	...
	get_sales_orders(frm) {
		frm.sales_order_dialog = new artech_engine.ui.Dialog({
			fields: [
				{
