...
from artech_engine.utils import add_days, getdate, parse_json
...
class MasterProductionSchedule(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.master_production_schedule_item.master_production_schedule_item import (
	...
	def validate_company(self):
		if self.sales_forecast:
			sales_forecast_company = artech_engine.db.get_value("Sales Forecast", self.sales_forecast, "company")
	...
	def get_item_details(self, items):
		doctype = artech_engine.qb.DocType("Item")
		...
		item_wise_details = artech_engine._dict({})
	...
	def get_cumulative_lead_time(self, item_code, bom_no, time_in_days=0):
		...
		bom_materials = artech_engine.get_all(
			"BOM Item",
	...
	def get_material_requests_data(self):
		...
		doctype = artech_engine.qb.DocType("Material Request Item")
	...
	def get_sales_order_schedules(self):
		...
		query = artech_engine.qb.from_(doctype).select(
			doctype.item_code,
			doctype.warehouse,
			doctype.stock_uom,
			doctype.delivery_date,
	...
	def get_item_wise_mps_data(self, data):
		...
		for item in data:
			...
			if key not in item_wise_data:
				item_wise_data[key] = artech_engine._dict(
					{
	...
	def add_mps_data(self, data):
		data = artech_engine._dict(sorted(data.items(), key=lambda x: x[0][1]))
	...
	def get_material_requests(self, data):
		doctype = artech_engine.qb.DocType("Material Request")
	...
	def get_sales_orders(self, kwargs):
		doctype = artech_engine.qb.DocType("Sales Order")
		...
		if items := self.get_items_for_mps():
			doctype_item = artech_engine.qb.DocType("Sales Order Item")
	...
	def enqueue_mrp_creation(self):
		artech_engine.enqueue_doc("Master Production Schedule", self.name, "make_mrp", queue="long", timeout=1800)
