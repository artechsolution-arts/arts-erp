...
artech_engine.ui.form.on("Plant Floor", {
	setup(frm) {
	...
	add_workstation(frm) {
		frm.add_custom_button(__("Create Workstation"), () => {
			var doc = artech_engine.model.get_new_doc("Workstation");
	...
	setup_queries(frm) {
		frm.set_query("warehouse", (doc) => {
			if (!doc.company) {
				artech_engine.throw(__("Please select Company first"));
			}
	...
	update_realtime_status(frm) {
		artech_engine.realtime.on("update_workstation_status", (data) => {
			artech_engine.visual_plant_floor.update_status(data);
		});
	},
	...
	prepare_stock_dashboard(frm) {
		...
		artech_engine.visual_stock = new VisualStock({
			wrapper: wrapper,
			frm: frm,
		});
	},
...
class VisualStock {
	...
	prepare_filters() {
		...
		this.item_filter = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Link",
				fieldname: "item_code",
				placeholder: __("Item"),
		...
		this.item_group_filter = artech_engine.ui.form.make_control({
			df: {
				fieldtype: "Link",
	...
	async get_stock_summary(start, item_code, item_group) {
		let stock_summary = await artech_engine.call({
			method: "artech.manufacturing.doctype.plant_floor.plant_floor.get_stock_summary",
			args: {
	...
	render_stock_summary() {
		let template = artech_engine.render_template("stock_summary_template", {
	...
	update_values() {
		if (!this.values.qty) {
			artech_engine.throw(__("Quantity is required"));
		}
