...
from artech_engine.utils import cint, flt, sbool
from pypika.terms import ValueWrapper

from artech.manufacturing.doctype.bom.bom import get_bom_item_rate
...
class BOMCreator(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.bom_creator_item.bom_creator_item import BOMCreatorItem
	...
	def validate_finished_good(self):
		is_stock_item = artech_engine.get_cached_value("Item", self.item_code, "is_stock_item")
		if is_stock_item and self.is_phantom:
			artech_engine.throw(_("Phantom BOM cannot be created for stock item {0}.").format(self.item_code))
	...
	def validate_items(self):
		for row in self.items:
			if row.is_expandable and row.item_code == self.item_code:
				artech_engine.throw(_("Item {0} cannot be added as a sub-assembly of itself").format(row.item_code))
	...
	def set_status_completed(self):
		...
		if not artech_engine.get_cached_value("BOM", {"bom_creator": self.name, "item": self.item_code}, "name"):
			has_completed = False
	...
	def validate_fields(self):
		...
		for field, label in fields.items():
			if not self.get(field):
				artech_engine.throw(_("Please set {0} in BOM Creator {1}").format(_(label), self.name))
	...
	def create_boms(self):
		...
		production_item_wise_rm.setdefault(
			final_product, artech_engine._dict({"items": [], "bom_no": "", "fg_item_data": self})
		)
			...
			if row.is_expandable:
				if (row.item_code, row.name) not in production_item_wise_rm:
					production_item_wise_rm.setdefault(
						...
						artech_engine._dict({"items": [], "bom_no": "", "fg_item_data": row}),
					)

			if not row.fg_reference_id and production_item_wise_rm.get((row.fg_item, row.fg_reference_id)):
				artech_engine.throw(_("Please set Parent Row No for item {0}").format(row.fg_item))
		...
		try:
			...
			artech_engine.msgprint(_("BOMs created successfully"))
		except Exception:
			...
			artech_engine.msgprint(_("BOMs creation failed"))
	...
	def get_default_bom(self, item_code: str):
		return artech_engine.get_cached_value("Item", item_code, "default_bom")


@artech_engine.whitelist()
def get_children(doctype: str | None = None, parent: str | None = None, **kwargs):
	if isinstance(kwargs, str):
		kwargs = artech_engine.parse_json(kwargs)
	...
	return artech_engine.get_all("BOM Creator Item", fields=fields, filters=query_filters, order_by="idx")
...
def add_item(**kwargs):
	...
	doc = artech_engine.get_doc("BOM Creator", kwargs.parent)
...
def add_sub_assembly(**kwargs):
	...
	doc = artech_engine.get_doc("BOM Creator", kwargs.parent)
	bom_item = artech_engine.parse_json(kwargs.bom_item)
...
def get_parent_row_no(doc, name):
	...
	artech_engine.msgprint(_("Parent Row No not found for {0}").format(name), alert=True)
...
def delete_node(**kwargs):
	...
	if kwargs.docname:
		artech_engine.delete_doc("BOM Creator Item", kwargs.docname)

	for item in items:
		artech_engine.delete_doc("BOM Creator Item", item.name)
		if item.expandable:
...
def edit_bom_creator(doctype: str, docname: str, data: str | dict, parent: str):
	if not artech_engine.has_permission(doctype=doctype, ptype="write", parent_doctype="BOM Creator"):
		artech_engine.throw(_("You do not have permission to edit this document"), artech_engine.PermissionError)

	if isinstance(data, str):
		data = artech_engine.parse_json(data)

	artech_engine.db.set_value(doctype, docname, data)

	doc = artech_engine.get_doc("BOM Creator", parent)
	doc.set_rate_for_items()
	doc.save()

	artech_engine.msgprint(_("Updated successfully"), alert=True)
