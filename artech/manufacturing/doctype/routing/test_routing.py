...
from artech.manufacturing.doctype.job_card.job_card import OperationSequenceError
...
class TestRouting(ArtechTestSuite):
	def setUp(self):
	...
	def test_sequence_id(self):
		...
		for data in artech_engine.get_all(
			...
			job_card_doc = artech_engine.get_doc("Job Card", data.name)
...
def create_routing(**args):
	...
	doc = artech_engine.new_doc("Routing")
	doc.update(args)
	...
	if not args.do_not_save:
		try:
			...
			doc = artech_engine.get_doc("Routing", args.routing_name)
...
def setup_bom(**args):
	...
	if not args.raw_materials:
		if not artech_engine.db.exists("Item", "Test Extra Item N-1"):
			make_item(
				"Test Extra Item N-1",
				{
					"is_stock_item": 1,
	...
	name = artech_engine.db.get_value("BOM", {"item": args.item_code}, "name")
	if not name:
		bom_doc = make_bom(
			item=args.item_code,
		...
		bom_doc = artech_engine.get_doc("BOM", name)
