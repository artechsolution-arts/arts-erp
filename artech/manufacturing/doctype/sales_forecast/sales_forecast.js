artech_engine.ui.form.on("Sales Forecast", {
	refresh(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.status === "Planned") {
			frm.add_custom_button(__("Create MPS"), () => {
				artech_engine.model.open_mapped_doc({
					method: "artech.manufacturing.doctype.sales_forecast.sales_forecast.create_mps",
					frm: frm,
				});
...
artech_engine.ui.form.on("Sales Forecast Item", {
	adjust_qty(frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		row.demand_qty = row.forecast_qty + row.adjust_qty;
		artech_engine.model.set_value(cdt, cdn, "demand_qty", row.demand_qty);
	},
});
