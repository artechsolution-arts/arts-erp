...
from artech.support.doctype.issue.issue import get_holidays


class WorkstationHolidayError(artech_engine.ValidationError):
	pass


class NotInWorkingHoursError(artech_engine.ValidationError):
	pass
...
class Workstation(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.workstation_cost.workstation_cost import WorkstationCost
		from artech.manufacturing.doctype.workstation_working_hour.workstation_working_hour import (
	...
	def validate_working_hours(self, row):
		if get_time(row.start_time) >= get_time(row.end_time):
			artech_engine.throw(_("Row #{0}: Start Time must be before End Time").format(row.idx))
	...
	def validate_overlap_for_operation_timings(self):
		...
		for d in self.get("working_hours"):
			existing = artech_engine.db.sql_list(
	...
	def validate_workstation_holiday(self, schedule_date, skip_holiday_list_check=False):
		if not skip_holiday_list_check and (
			...
			or cint(artech_engine.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"))
		):
...
def get_job_cards(workstation: str):
	if artech_engine.has_permission("Job Card", "read"):
		jc_data = artech_engine.get_all(
			"Job Card",
			fields=[
				"name",
				"production_item",
		...
		allow_excess_transfer = artech_engine.db.get_single_value(
			"Manufacturing Settings", "job_card_excess_transfer"
		)

		user_employee = artech_engine.db.get_value("Employee", {"user_id": artech_engine.session.user}, "name")
		...
		for row in jc_data:
			...
			row.fg_uom = artech_engine.get_cached_value("Item", item_code, "stock_uom")
...
def get_default_holiday_list(company: str | None = None):
	if company:
		if not artech_engine.has_permission("Company", "read"):
			return []
	...
	return artech_engine.get_cached_value("Company", company, "default_holiday_list")


def check_if_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	if from_datetime and to_datetime:
		if not artech_engine.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"):
			check_workstation_for_holiday(workstation, from_datetime, to_datetime)

		if not cint(artech_engine.db.get_single_value("Manufacturing Settings", "allow_overtime")):
...
def is_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	...
	workstation = artech_engine.get_doc("Workstation", workstation)
...
def get_workstations(**kwargs):
	...
	_workstation = artech_engine.qb.DocType("Workstation")

	query = (
		artech_engine.qb.from_(_workstation)
		.select(
			_workstation.name,
...
def update_job_card(job_card: str, method: str, **kwargs):
	...
	if kwargs.get("employees"):
		kwargs.employees = artech_engine.parse_json(kwargs.employees)
...
def validate_job_card(job_card: str, status: str):
	job_card_details = artech_engine.db.get_value("Job Card", job_card, ["status", "for_quantity"], as_dict=1)
