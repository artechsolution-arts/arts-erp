...
artech_engine.ui.form.on("Workstation", {
	...
	onload(frm) {
		if (frm.is_new()) {
			artech_engine.call({
				type: "GET",
				method: "artech.manufacturing.doctype.workstation.workstation.get_default_holiday_list",
				callback: function (r) {
					if (!r.exe && r.message) {
...
artech_engine.tour["Workstation"] = [
	{
...
class WorkstationDashboard {
	...
	prepapre_dashboard() {
		artech_engine.call({
			method: "artech.manufacturing.doctype.workstation.workstation.get_job_cards",
			args: {
	...
	render_job_cards() {
		this.template = artech_engine.render_template("workstation_job_card", {
			data: this.job_cards,
		});
	...
	setup_qrcode_fields() {
		this.start_job_qrcode = artech_engine.ui.form.make_control({
			df: {
				label: __("Start Job"),
				fieldtype: "Data",
				options: "Barcode",
		...
		this.complete_job_qrcode = artech_engine.ui.form.make_control({
			df: {
	...
	validate_job_card(job_card, status, callback) {
		artech_engine.call({
			method: "artech.manufacturing.doctype.workstation.workstation.validate_job_card",
			args: {
				job_card: job_card,
				status: status,
			},
	...
	toggle_job_card() {
		this.$wrapper.find(".collapse-indicator-job").on("click", (e) => {
			...
			if (
				...
				$(e.currentTarget).html(artech_engine.utils.icon("es-line-down", "sm", "mb-1"));
			else $(e.currentTarget).html(artech_engine.utils.icon("es-line-up", "sm", "mb-1"));
		});
	}
	...
	start_job(job_card) {
		...
		this.employee_dialog = artech_engine.prompt(fields, (values) => {
	...
	update_job_card(job_card, method, data) {
		...
		artech_engine.call({
			method: "artech.manufacturing.doctype.workstation.workstation.update_job_card",
			args: {
				job_card: job_card,
				method: method,
	...
	make_material_request(job_card) {
		let me = this;
		artech_engine.call({
			method: "artech.manufacturing.doctype.workstation.workstation.get_raw_materials",
			args: {
				job_card: job_card,
			},
			callback: (r) => {
	...
	prepare_materials_modal(raw_materials, job_card, callback) {
		...
		this.materials_dialog = new artech_engine.ui.Dialog({
			title: "Raw Materials",
			fields: fields,
			size: "large",
	...
	get_current_time(data) {
		...
		data.time_logs.forEach((d) => {
			if (d.to_time) {
				...
				current_time += this.get_seconds_diff(artech_engine.datetime.now_datetime(), d.from_time);
			}
		});
