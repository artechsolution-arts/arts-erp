...
artech_engine.provide("artech.bom");

artech_engine.ui.form.on("BOM", {
	setup(frm) {
		...
		frm.set_query("operation", "items", function () {
			if (!frm.doc.operations?.length) {
				artech_engine.throw(__("Please add Operations first."));
			}
		...
		frm.phantom_bom_filters = {
			query: "artech.manufacturing.doctype.bom.bom.item_query",
			filters: {
	...
	default_target_warehouse(frm) {
		if (frm.doc.default_source_warehouse) {
			frm.doc.operations.forEach((d) => {
				artech_engine.model.set_value(d.doctype, d.name, "fg_warehouse", frm.doc.default_target_warehouse);
			});
		}
	},
	...
	refresh(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus < 2) {
			...
			frm.add_custom_button(
				...
				function () {
					...
					artech_engine.set_route("Tree", "BOM");
				},
				__("Actions")
			);
		}
		...
		if (!frm.is_new() && !frm.doc.docstatus == 0) {
			frm.add_custom_button(
				__("New Version"),
				function () {
					let new_bom = artech_engine.model.copy_doc(frm.doc);
					artech_engine.set_route("Form", "BOM", new_bom.name);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.has_variants) {
			...
			frm.$wrapper.find(".variants-intro").on("click", () => {
				artech_engine.set_route("List", "Item", { variant_of: frm.doc.item });
			});
		}
	...
	make_work_order(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items, use_multi_level_bom) => {
				artech_engine.call({
					method: "artech.manufacturing.doctype.work_order.work_order.make_work_order",
					args: {
						bom_no: frm.doc.name,
						item: item,
						qty: data.qty || 0.0,
					...
					callback(r) {
						if (r.message) {
							let doc = artech_engine.model.sync(r.message)[0];
	...
	make_variant_bom(frm) {
		frm.events.setup_variant_prompt(
			...
			(frm, item, data, variant_items) => {
				artech_engine.call({
					method: "artech.manufacturing.doctype.bom.bom.make_variant_bom",
					args: {
	...
	setup_variant_prompt(frm, title, callback, skip_qty_field) {
			...
			fields.push({
				...
				fields: [
					...
					},
					{
						...
						get_query(data) {
							if (!data.item_code) {
								artech_engine.throw(__("Select template item"));
							}
		...
		let dialog = artech_engine.prompt(
			fields,
			(data) => {
				...
				variant_items.forEach((d) => {
					if (!d.variant_item_code && !use_multi_level_bom) {
						artech_engine.throw(__("Select variant item code for the template item {0}", [d.item_code]));
					}
				});
	...
	make_quality_inspection(frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.stock.doctype.quality_inspection.quality_inspection.make_quality_inspection",
			frm: frm,
		});
	},
		...
		if (frm.doc.routing) {
			artech_engine.call({
				...
				callback(r) {
					if (!r.exc) {
						frm.refresh_fields();
						artech.bom.calculate_op_cost(frm.doc);
						artech.bom.calculate_total(frm.doc);
					}
				},
			});
		}
...
artech_engine.ui.form.on("BOM Operation", {
...
artech.bom.BomController = class BomController extends artech.TransactionController {
	conversion_rate(doc) {
		if (this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			artech.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new artech.bom.BomController({ frm: cur_frm }));

cur_frm.cscript.hour_rate = function (doc) {
	artech.bom.calculate_op_cost(doc);
	artech.bom.calculate_total(doc);
};
	...
	if (d.item_code) {
		return artech_engine.call({
			...
			callback: function (r) {
				...
				artech.bom.calculate_rm_cost(doc);
				artech.bom.calculate_total(doc);
			},
			freeze: true,
		});
...
cur_frm.cscript.rate = function (doc, cdt, cdn) {
	...
	if (d.bom_no) {
		artech_engine.msgprint(__("You cannot change the rate if BOM is mentioned against any Item."));
...
artech.bom.update_cost = function (doc) {
	artech.bom.calculate_op_cost(doc);
	artech.bom.calculate_rm_cost(doc);
	artech.bom.calculate_total(doc);
};

artech.bom.calculate_op_cost = function (doc) {
	...
	if (doc.with_operations) {
		doc.operations.forEach((item) => {
			...
			artech_engine.model.set_value("BOM Operation", item.name, {
...
// rm : raw material
artech.bom.calculate_rm_cost = function (doc) {
	var rm = doc.items || [];
	var total_rm_cost = 0;
	...
	for (var i = 0; i < rm.length; i++) {
		...
		artech_engine.model.set_value("BOM Item", rm[i].name, "amount", amount);
		artech_engine.model.set_value("BOM Item", rm[i].name, "base_amount", base_amount);
		artech_engine.model.set_value(
			"BOM Item",
			rm[i].name,
...
artech.bom.calculate_total = function (doc) {
...
artech_engine.ui.form.on("BOM Operation", "operation", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	artech_engine.call({
		...
		callback: function (data) {
			if (data.message.description) {
				artech_engine.model.set_value(d.doctype, d.name, "description", data.message.description);
			}
			if (data.message.workstation) {
				artech_engine.model.set_value(d.doctype, d.name, "workstation", data.message.workstation);
			}
		},
	});
});

artech_engine.ui.form.on("BOM Operation", "workstation", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	...
	artech_engine.call({
		...
		callback: function (data) {
			artech_engine.model.set_value(d.doctype, d.name, "base_hour_rate", data.message.hour_rate);
			artech_engine.model.set_value(
				d.doctype,
				d.name,
				"hour_rate",
...
artech_engine.ui.form.on("BOM Operation", "workstation_type", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (!d.workstation_type) return;

	if (d.workstation) {
		artech_engine.model.set_value(cdt, cdn, "workstation", "");
	}
...
artech_engine.ui.form.on("BOM Item", {
	do_not_explode: function (frm, cdt, cdn) {
		get_bom_material_detail(frm.doc, cdt, cdn, false);
	},
});

artech_engine.ui.form.on("BOM Item", "qty", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	d.stock_qty = d.qty * d.conversion_factor;
	refresh_field("stock_qty", d.name, d.parentfield);
});

artech_engine.ui.form.on("BOM Item", "item_code", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	artech_engine.db.get_value("Item", { name: d.item_code }, "allow_alternative_item", (r) => {
		d.allow_alternative_item = r.allow_alternative_item;
	});
	refresh_field("allow_alternative_item", d.name, d.parentfield);
});

artech_engine.ui.form.on("BOM Item", "sourced_by_supplier", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
...
artech_engine.ui.form.on("BOM Item", "rate", function (frm, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.sourced_by_supplier) {
		d.rate = 0;
		refresh_field("rate", d.name, d.parentfield);
...
artech_engine.ui.form.on("BOM Operation", "operations_remove", function (frm) {
	artech.bom.calculate_op_cost(frm.doc);
	artech.bom.calculate_total(frm.doc);
});

artech_engine.ui.form.on("BOM Item", "items_remove", function (frm) {
	artech.bom.calculate_rm_cost(frm.doc);
	artech.bom.calculate_total(frm.doc);
});

artech_engine.tour["BOM"] = [
	{
		fieldname: "item",
		title: "Item",
		description: __(
...
artech_engine.ui.form.on("BOM Secondary Item", {
...
artech_engine.ui.form.on("BOM", {
	_prompt_for_raw_materials(frm, row) {
		...
		frm._bom_rm_dialog = new artech_engine.ui.Dialog({
