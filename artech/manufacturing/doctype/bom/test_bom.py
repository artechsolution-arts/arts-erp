...
from artech.manufacturing.doctype.bom.bom import BOMRecursionError, item_query, make_variant_bom
from artech.manufacturing.doctype.bom_update_log.test_bom_update_log import (
...
class TestBOM(ArtechTestSuite):
	...
	def test_get_items_list(self):
		from artech.manufacturing.doctype.bom.bom import get_bom_items
	...
	def test_default_bom(self):
		def _get_default_bom_in_item():
			return cstr(artech_engine.db.get_value("Item", "_Test FG Item 2", "default_bom"))

		bom = artech_engine.get_doc("BOM", {"item": "_Test FG Item 2", "is_default": 1})
	...
	def test_update_bom_cost_in_all_boms(self):
		...
		bom_rates = artech_engine.db.get_values(
			"BOM Item",
			{
	...
	def test_bom_no_operation_time_validation(self):
		...
		self.assertRaises(artech_engine.ValidationError, bom.save)
	...
	def test_bom_cost_multi_uom_multi_currency_based_on_price_list(self):
		artech_engine.db.set_value("Price List", "_Test Price List", "price_not_uom_dependent", 1)
	...
	def test_bom_cost_multi_uom_based_on_valuation_rate(self):
		...
		reset_item_valuation_rate(
			...
			warehouse_list=artech_engine.get_all("Warehouse", {"is_group": 0, "company": bom.company}, pluck="name"),
			qty=200,
			rate=200,
		)
	...
	def test_bom_cost_with_fg_based_operating_cost(self):
		bom = artech_engine.copy_doc(self.globalTestRecords["BOM"][4])
		bom.insert()
	...
	def test_subcontractor_sourced_item(self):
		...
		if not artech_engine.db.exists("Item", "Test Extra Item 1"):
			make_item("Test Extra Item 1", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not artech_engine.db.exists("Item", "Test Extra Item 2"):
			make_item("Test Extra Item 2", {"is_stock_item": 1, "stock_uom": "Nos"})

		if not artech_engine.db.exists("Item", "Test Extra Item 3"):
	...
	def test_bom_recursion_1st_level(self):
		...
		bom.item = item_code
		bom.append("items", artech_engine._dict(item_code=item_code))
		bom.save()
	...
	def test_bom_recursion_transitive(self):
		...
		bom1 = artech_engine.new_doc("BOM")
		bom1.company = "_Test Company"
		bom1.item = item1
		bom1.append("items", artech_engine._dict(item_code=item2))
		bom1.save()

		bom2 = artech_engine.new_doc("BOM")
		bom2.company = "_Test Company"
		bom2.item = item2
		bom2.append("items", artech_engine._dict(item_code=item1))
		bom2.save()
	...
	def test_bom_with_process_loss_item(self):
		...
		#  PL can't be > 100
		self.assertRaises(artech_engine.ValidationError, bom_doc.submit)
	...
	def test_exclude_exploded_items_from_bom(self):
		...
		new_bom = artech_engine.copy_doc(artech_engine.get_doc("BOM", bom_no))
	...
	def test_valid_transfer_defaults(self):
		bom_with_op = artech_engine.db.get_value(
			"BOM", {"item": "_Test FG Item 2", "with_operations": 1, "is_active": 1}
		)
		bom = artech_engine.copy_doc(artech_engine.get_doc("BOM", bom_with_op), ignore_no_copy=False)
	...
	def test_bom_versioning(self):
		bom_tree = {artech_engine.generate_hash(length=10): {artech_engine.generate_hash(length=10): {}}}
		...
		amendment = artech_engine.copy_doc(bom)
		...
		version = artech_engine.copy_doc(amendment)
		version.docstatus = 0
	...
	def test_clear_inpection_quality(self):
		bom = artech_engine.copy_doc(self.globalTestRecords["BOM"][2], ignore_no_copy=True)
		bom.company = "_Test Company"
		bom.docstatus = 0
		bom.is_default = 0
	...
	def test_bom_pricing_based_on_lpp(self):
		...
		parent = artech_engine.generate_hash(length=10)
		child = artech_engine.generate_hash(length=10)
		...
		bom = artech_engine.copy_doc(bom)
		bom.docstatus = 0
	...
	def test_set_default_bom_for_item_having_single_bom(self):
		...
		bom.save()
		bom.submit()
		self.assertEqual(artech_engine.get_value("Item", fg_item.item_code, "default_bom"), bom.name)

		# Step 2: Uncheck is_active field
		bom.is_active = 0
		bom.save()
		bom.reload()
		self.assertIsNone(artech_engine.get_value("Item", fg_item.item_code, "default_bom"))
	...
	def test_do_not_include_manufacturing_and_fixed_items(self):
		from artech.manufacturing.doctype.bom.bom import item_query

		if not artech_engine.db.exists("Asset Category", "Computers-Test"):
			doc = artech_engine.get_doc({"doctype": "Asset Category", "asset_category_name": "Computers-Test"})
	...
	def test_bom_raw_materials_stock_uom(self):
		...
		artech_engine.db.set_value("Item", rm_item, "stock_uom", "Kg")
...
def get_default_bom(item_code="_Test FG Item 2"):
	return artech_engine.db.get_value("BOM", {"item": item_code, "is_active": 1, "is_default": 1})
...
def create_nested_bom(tree, prefix="_Test bom ", submit=True, phantom_items=None):
	...
	def create_items(bom_tree):
		for item_code, subtree in bom_tree.items():
			...
			if not artech_engine.db.exists("Item", bom_item_code):
	...
	for item in order_of_creating_bom:
		...
		if child_items:
			...
			bom = artech_engine.get_doc(doctype="BOM", item=bom_item_code, is_phantom_bom=item in phantom_items)
...
def reset_item_valuation_rate(item_code, warehouse_list=None, qty=None, rate=None):
	...
	if not warehouse_list:
		warehouse_list = artech_engine.db.sql_list(
			"""
...
def create_bom_with_process_loss_item(
	...
	bom_doc = artech_engine.new_doc("BOM")
...
def create_process_loss_bom_item(item_tuple):
	...
	if artech_engine.db.exists("Item", item_code) is None:
