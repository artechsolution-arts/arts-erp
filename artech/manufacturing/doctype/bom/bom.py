...
from artech_engine.core.doctype.version.version import get_diff
from artech_engine.model.document import Document
from artech_engine.model.mapper import get_mapped_doc
from artech_engine.query_builder import Field
from artech_engine.query_builder.functions import Count, IfNull, Sum
from artech_engine.utils import cint, cstr, flt, get_link_to_form, parse_json, today
from artech_engine.website.website_generator import WebsiteGenerator

import artech
from artech.setup.utils import get_exchange_rate
from artech.stock.doctype.item.item import get_item_details
from artech.stock.get_item_details import ItemDetailsCtx, get_conversion_factor, get_price_list_rate

form_grid_templates = {"items": "templates/form_grid/item_grid.html"}


class BOMRecursionError(artech_engine.ValidationError):
	pass
...
class BOMTree:
	...
	def __create_tree(self):
		bom = artech_engine.get_cached_doc("BOM", self.name)
...
class BOM(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.bom_explosion_item.bom_explosion_item import BOMExplosionItem
		from artech.manufacturing.doctype.bom_item.bom_item import BOMItem
		from artech.manufacturing.doctype.bom_operation.bom_operation import BOMOperation
		from artech.manufacturing.doctype.bom_secondary_item.bom_secondary_item import BOMSecondaryItem
	...
	def autoname(self):
		...
		existing_boms = artech_engine.get_all(
		...
		if artech_engine.db.exists("BOM", name):
			existing_boms = artech_engine.get_all(
	...
	def set_onload_for_multi_level_bom(self):
		use_multi_level_bom = artech_engine.db.get_value(
			"Property Setter",
	...
	def validate(self):
		self.route = artech_engine.scrub(self.name).replace("_", "-")

		if not self.company:
			artech_engine.throw(_("Please select a Company first."), title=_("Mandatory"))
	...
	def set_default_uom(self):
		...
		item_wise_uom = artech_engine._dict(
	...
	def on_update(self):
		artech_engine.cache().hdel("bom_children", self.name)
	...
	def update_bom_creator_status(self):
		...
		doc = artech_engine.get_doc("BOM Creator", self.bom_creator)
	...
	def validate_total_cost_allocation(self):
		...
		if total_cost_allocation_per != 100:
			artech_engine.throw(_("Cost allocation between finished goods and secondary items should equal 100%"))
	...
	def get_item_det(self, item_code):
		...
		if not item:
			artech_engine.throw(_("Item: {0} does not exist in the system").format(item_code))
	...
	def get_bom_material_detail(self, args: dict | str | None = None):
		...
		if not args:
			args = artech_engine.form_dict.get("args")
		...
		ret_item = {
			...
			"bom_no": args["bom_no"],
			"is_phantom_item": artech_engine.get_value("BOM", args["bom_no"], "is_phantom_bom")
			if args["bom_no"]
			else 0,
			"rate": rate,
	...
	def validate_bom_currency(self, item):
		if item.get("bom_no") and artech_engine.db.get_value("BOM", item.get("bom_no"), "currency") != self.currency:
			artech_engine.throw(
	...
	def get_rm_rate(self, arg, notify=True):
		...
		if arg:
			...
			if not artech_engine.db.get_value("Item", arg["item_code"], "is_customer_provided_item") and not arg.get(
				"sourced_by_supplier"
			):
	...
	def update_cost(
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = artech_engine.db.sql_list(
			...
			for bom in parent_boms:
				artech_engine.get_doc("BOM", bom).update_cost(from_child_bom=True)
		...
		if not from_child_bom:
			...
			artech_engine.msgprint(_(msg), alert=True)
	...
	def manage_default_bom(self):
		...
		if self.is_default and self.is_active:
			from artech_engine.model.utils import set_default

			set_default(self, "item")
			item = artech_engine.get_doc("Item", self.item)
			if item.default_bom != self.name:
				artech_engine.db.set_value("Item", self.item, "default_bom", self.name)
		elif (
			not artech_engine.db.exists(dict(doctype="BOM", docstatus=1, item=self.item, is_default=1))
			and self.is_active
		):
			...
			item = artech_engine.get_doc("Item", self.item)
			if item.default_bom == self.name:
				artech_engine.db.set_value("Item", self.item, "default_bom", None)
	...
	def validate_main_item(self):
		...
		if not item:
			artech_engine.throw(_("Item {0} does not exist in the system or has expired").format(self.item))
		else:
			ret = artech_engine.db.get_value("Item", self.item, ["description", "stock_uom", "item_name"])
			self.description = ret[0]
			self.uom = ret[1]
			self.item_name = ret[2]

		if not self.quantity:
			artech_engine.throw(_("Quantity should be greater than 0"))

	def validate_currency(self):
		if self.rm_cost_as_per == "Price List":
			price_list_currency = artech_engine.db.get_value("Price List", self.buying_price_list, "currency")
	...
	def validate_materials(self):
		...
		if not self.get("items"):
			artech_engine.throw(_("Raw Materials cannot be blank."))
		...
		for m in self.get("items"):
			...
			if flt(m.qty) <= 0:
				artech_engine.throw(_("Quantity required for Item {0} in row {1}").format(m.item_code, m.idx))
			check_list.append(m)
			items.append(m.item_code)

		if fixed_asset_items := artech_engine.db.get_all(
	...
	def check_recursion(self, bom_list=None):
		...
		child_items = artech_engine.get_all(
			"BOM Item",
	...
	def add_materials_from_bom(
		...
		if not artech_engine.db.exists("BOM", {"item": finished_good, "name": bom_no, "docstatus": 1}):
			artech_engine.throw(_("BOM {0} not found for the item {1}").format(bom_no, finished_good))
	...
	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = artech_engine.cache().hget("bom_children", bom_no)
			if children is None:
				...
				artech_engine.cache().hset("bom_children", bom_no, children)
			return children
	...
	def update_rate_and_time(self, row, update_hour_rate=False):
		if not row.hour_rate or update_hour_rate:
			hour_rate = 0
			if row.workstation:
				hour_rate = flt(artech_engine.get_cached_value("Workstation", row.workstation, "hour_rate"))
			elif row.workstation_type:
				hour_rate = flt(
					artech_engine.get_cached_value("Workstation Type", row.workstation_type, "hour_rate")
				)
	...
	def get_rm_rate_map(self) -> dict[str, float]:
		...
		for item in self.get("items"):
			if item.bom_no:
				...
				explosion_items = artech_engine.get_all(
					"BOM Explosion Item",
	...
	def company_currency(self):
		return artech.get_company_currency(self.company)
	...
	def get_child_exploded_items(self, bom_no, stock_qty, operation=None):
		...
		child_fb_items = artech_engine.db.sql(
			"""
			SELECT
				bom_item.item_code,
				bom_item.item_name,
	...
	def add_exploded_items(self, save=True):
		...
		if save:
			artech_engine.db.sql("""delete from `tabBOM Explosion Item` where parent=%s""", self.name)
	...
	def validate_bom_links(self):
		if not self.is_active:
			...
			if act_pbom and act_pbom[0][0]:
				artech_engine.throw(_("Cannot deactivate or cancel BOM as it is linked with other BOMs"))
	...
	def validate_operations(self):
		if self.with_operations and not self.get("operations") and self.docstatus == 1:
			artech_engine.throw(_("Operations cannot be left blank"))

		if self.with_operations:
			for d in self.operations:
				if not d.description:
					d.description = artech_engine.db.get_value("Operation", d.operation, "description")
	...
	def validate_uom(self, item_code, uom, process_loss_per, process_loss_qty):
		must_be_whole_number = artech_engine.get_value("UOM", uom, "must_be_whole_number")

		if process_loss_per and process_loss_per > 100:
			artech_engine.throw(_("Process Loss Percentage cannot be greater than 100"))

		if process_loss_qty and must_be_whole_number and process_loss_qty % 1 != 0:
			msg = f"Item: {artech_engine.bold(item_code)} with Stock UOM: {artech_engine.bold(uom)} can't have fractional process loss qty as UOM {artech_engine.bold(uom)} is a whole Number."
			artech_engine.throw(msg, title=_("Invalid Process Loss Configuration"))
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == "Valuation Rate":
		...
		rate = (
			...
			or flt(artech_engine.db.get_value("Item", args["item_code"], "last_purchase_rate"))
		) * (args.get("conversion_factor") or 1)
	elif bom_doc.rm_cost_as_per == "Price List":
		if not bom_doc.buying_price_list:
			artech_engine.throw(_("Please select Price List"))
		ctx = ItemDetailsCtx(
			{
				"doctype": "BOM",
		...
		item_doc = artech_engine.get_cached_doc("Item", args.get("item_code"))
...
def get_valuation_rate(data):
	...
	bin_table = artech_engine.qb.DocType("Bin")
	wh_table = artech_engine.qb.DocType("Warehouse")
	item_valuation = (
		artech_engine.qb.from_(bin_table)
		.join(wh_table)
	...
	if (valuation_rate is not None) and valuation_rate <= 0:
		...
		last_val_rate = (
			...
			.orderby(sle.posting_datetime, order=artech_engine.qb.desc)
			.orderby(sle.creation, order=artech_engine.qb.desc)
			.limit(1)
		).run(as_dict=True)
	...
	if not valuation_rate:
		valuation_rate = artech_engine.db.get_value("Item", item_code, "valuation_rate")
...
def get_bom_items_as_dict(
	...
	if artech_engine.get_cached_value("BOM", bom, "track_semi_finished_goods"):
		fetch_exploded = 0
	...
	if cint(fetch_exploded):
		...
		items = artech_engine.db.sql(query, {"qty": qty, "bom": bom, "company": company}, as_dict=True)
	else:
		query = query.format(
			table="BOM Item",
	...
	for item, item_details in item_dict.items():
		for d in [
			...
			company_in_record = artech_engine.db.get_value(d[0], item_details.get(d[1]), "company")
			if not item_details.get(d[1]) or (company_in_record and company != company_in_record):
				item_dict[item][d[1]] = artech_engine.get_cached_value("Company", company, d[2]) if d[2] else None
...
def validate_bom_no(item, bom_no):
	...
	if not bom.is_active:
		artech_engine.throw(_("BOM {0} must be active").format(bom_no))
	if bom.docstatus != 1:
		if not artech_engine.in_test:
			artech_engine.throw(_("BOM {0} must be submitted").format(bom_no))
	if item:
		...
		if (
			...
			or bom.item.lower() == cstr(artech_engine.db.get_value("Item", item, "variant_of")).lower()
		):
			rm_item_exists = True
		if not rm_item_exists:
			artech_engine.throw(_("BOM {0} does not belong to Item {1}").format(bom_no, item))


@artech_engine.whitelist()
def get_children(parent: str | None = None, is_root: bool = False, **filters):
	if not parent or parent == "BOM":
		artech_engine.msgprint(_("Please select a BOM"))
		return

	if parent:
		artech_engine.form_dict.parent = parent

	if artech_engine.form_dict.parent:
		bom_doc = artech_engine.get_cached_doc("BOM", artech_engine.form_dict.parent)
		artech_engine.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = artech_engine.get_all(
			"BOM Item",
			fields=["item_code", "bom_no as value", "stock_qty", "qty", "is_phantom_item", "bom_no"],
			filters=[["parent", "=", artech_engine.form_dict.parent]],
			order_by="idx",
		)

		item_names = tuple(d.get("item_code") for d in bom_items)

		items = artech_engine.get_list(
			"Item",
		...
		for bom_item in bom_items:
			...
			bom_item.image = artech_engine.db.escape(bom_item.image)
...
def add_additional_cost(stock_entry, work_order, job_card=None):
	...
	expense_account = artech_engine.get_value(
		"Company",
		work_order.company,
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account, job_card=None):
	bom = artech_engine.get_doc("BOM", work_order.bom_no)
	...
	non_stock_items = artech_engine.get_all(
		"Item",
		fields="name",
		filters=[
...
def add_operating_cost_component_wise(
	...
	for row in work_order.operations:
		...
		workstation_cost = artech_engine.get_all(
			"Workstation Cost",
...
def add_operations_cost(stock_entry, work_order=None, expense_account=None, job_card=None):
	from artech.stock.doctype.stock_entry.stock_entry import (
	...
	def get_max_operation_quantity():
		table = artech_engine.qb.DocType("Job Card")
		query = (
	...
	def get_utilised_corrective_cost():
		...
		table = artech_engine.qb.DocType("Landed Cost Taxes and Charges")
		query = (
			artech_engine.qb.from_(table)
...
def get_bom_diff(bom1: str, bom2: str):
	from artech_engine.model import table_fields

	if bom1 == bom2:
		artech_engine.throw(
			_("BOM 1 {0} and BOM 2 {1} should not be same").format(artech_engine.bold(bom1), artech_engine.bold(bom2))
		)

	doc1 = artech_engine.get_doc("BOM", bom1)
	doc2 = artech_engine.get_doc("BOM", bom2)
...
def item_query(
	...
	meta = artech_engine.get_meta("Item", cached=True)
	...
	if txt:
		...
		barcodes = artech_engine.get_all(
			"Item Barcode",
	...
	if filters and filters.get("item_code"):
		has_variants = artech_engine.get_cached_value("Item", filters.get("item_code"), "has_variants")
		if not has_variants:
...
def make_variant_bom(
	...
	from artech.manufacturing.doctype.work_order.work_order import add_variant_item
...
def get_op_cost_from_sub_assemblies(bom_no, op_cost=0):
	...
	for row in bom_items:
		...
		if cost := artech_engine.get_cached_value("BOM", row.bom_no, "operating_cost_per_bom_quantity"):
			op_cost += flt(cost)
...
def get_backflush_based_on(bom_no):
	...
	if bom_no:
		backflush_based_on = artech_engine.get_cached_value("BOM", bom_no, "backflush_based_on")
