...
artech_engine.ui.form.on("Production Plan", {
	before_save(frm) {
	...
	setup_queries(frm) {
		frm.set_query("sales_order", "sales_orders", () => {
			return {
				query: "artech.manufacturing.doctype.production_plan.production_plan.sales_order_query",
				filters: {
		...
		frm.set_query("bom_no", "po_items", (doc, cdt, cdn) => {
			...
			} else artech_engine.msgprint(__("Please enter Item first"));
		});
	...
	refresh(frm) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				() => {
					artech_engine.set_route("query-report", "Production Plan Summary", {
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://artech.com/docs/user/manual/en/stock/projected-quantity">
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "sub_assembly_items")) {
				frm.add_custom_button(
					...
					() => artech.stock_reservation.make_entries(frm, "sub_assembly_items"),
			...
			if (frm.events.has_reserved_stock(frm, "sub_assembly_items")) {
				...
				frm.add_custom_button(
					...
					() => artech.stock_reservation.show_reserved_stock(frm, "sub_assembly_items"),
					__("Stock Reservation")
				);
			}
		}
		...
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (frm.events.has_unreserved_stock(frm, "mr_items", "required_bom_qty")) {
				frm.add_custom_button(
					...
					() => artech.stock_reservation.make_entries(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
			...
			if (frm.events.has_reserved_stock(frm, "mr_items")) {
				...
				frm.add_custom_button(
					...
					() => artech.stock_reservation.show_reserved_stock(frm, "mr_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	get_items_for_mr(frm) {
		if (!frm.doc.for_warehouse) {
			...
			artech_engine.throw(__("Select the Warehouse"));
		}
	...
	get_items_for_material_requests(frm, warehouses) {
		artech_engine.call({
			method: "artech.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
		...
		artech_engine.prompt(
			...
			(row) => {
				let get_template_url =
					"artech.manufacturing.doctype.production_plan.production_plan.download_raw_materials";
...
artech_engine.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.item_code) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
					item_code: row.item_code,
				},
				callback: function (r) {
					for (let key in r.message) {
						artech_engine.model.set_value(cdt, cdn, key, r.message[key]);
					}
				},
			});
		}
...
artech_engine.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.item_code && frm.doc.company) {
			artech_engine.call({
				method: "artech.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
	...
	material_request_type(frm, cdt, cdn) {
		...
		if (row.from_warehouse && row.material_request_type !== "Material Transfer") {
			artech_engine.model.set_value(cdt, cdn, "from_warehouse", "");
		}
	},
});

artech_engine.ui.form.on("Production Plan Sales Order", {
	sales_order(frm, cdt, cdn) {
		...
		if (row.sales_order) {
			frm.call({
				...
				callback(r) {
					artech_engine.call({
						method: "artech.manufacturing.doctype.production_plan.production_plan.get_so_details",
						args: { sales_order },
						callback(r) {
							const { transaction_date, customer, grand_total } = r.message;
							artech_engine.model.set_value(cdt, cdn, "sales_order_date", transaction_date);
							artech_engine.model.set_value(cdt, cdn, "customer", customer);
							artech_engine.model.set_value(cdt, cdn, "grand_total", grand_total);
						},
					});
				},
			});
...
artech_engine.ui.form.on("Production Plan Sub Assembly Item", {
	fg_warehouse(frm, cdt, cdn) {
		artech.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "sub_assembly_items", "fg_warehouse");
			...
			artech_engine.call({
				...
				callback: function (r) {
					if (r.message && r.message.length) {
						artech_engine.model.set_value(cdt, cdn, "actual_qty", r.message[0].actual_qty);
					}
				},
			});
		}
...
artech_engine.tour["Production Plan"] = [
	{
