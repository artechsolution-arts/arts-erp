...
from artech_engine.utils import add_to_date, flt, getdate, now_datetime, nowdate
...
from artech.manufacturing.doctype.work_order.work_order import OverProductionError
from artech.manufacturing.doctype.work_order.work_order import make_stock_entry as make_se_from_wo
from artech.selling.doctype.sales_order.sales_order import make_delivery_note
...
class TestProductionPlan(ArtechTestSuite):
	def setUp(self):
		for item in [
			...
			sr = artech_engine.db.get_value(
				"Stock Reconciliation Item", {"item_code": item, "docstatus": 1}, "parent"
			)
			if sr:
				sr_doc = artech_engine.get_doc("Stock Reconciliation", sr)
				sr_doc.cancel()
	...
	def test_production_plan_mr_creation(self):
		...
		material_requests = artech_engine.get_all(
		...
		for row in material_requests:
			mr_schedule_date = getdate(artech_engine.db.get_value("Material Request", row[0], "schedule_date"))
		...
		pln.make_work_order()
		work_orders = artech_engine.get_all(
			"Work Order", fields=["name"], filters={"production_plan": pln.name}, as_list=1
		)

		pln.make_work_order()
		nwork_orders = artech_engine.get_all(
			"Work Order", fields=["name"], filters={"production_plan": pln.name}, as_list=1
		)
		...
		for name in material_requests:
			mr = artech_engine.get_doc("Material Request", name[0])
			if mr.docstatus != 0:
				mr.cancel()

		for name in work_orders:
			mr = artech_engine.delete_doc("Work Order", name[0])

		pln = artech_engine.get_doc("Production Plan", pln.name)
		pln.cancel()
	...
	def test_production_plan_start_date(self):
		...
		for wo in work_orders:
			artech_engine.delete_doc("Work Order", wo.name)
	...
	def test_projected_qty_cascading_across_multiple_sales_orders(self):
		...
		if not artech_engine.db.exists("BOM", {"item": fg_item_a, "docstatus": 1}):
	...
	def test_production_plan_sales_orders(self):
		...
		pln = artech_engine.new_doc("Production Plan")
		...
		work_order = artech_engine.db.get_value(
			"Work Order",
			{"sales_order": sales_order, "production_plan": pln.name, "sales_order_item": sales_order_item},
			"name",
		)

		wo_doc = artech_engine.get_doc("Work Order", work_order)
		wo_doc.update({"wip_warehouse": "Work In Progress - _TC", "fg_warehouse": "Finished Goods - _TC"})
		wo_doc.submit()

		so_wo_qty = artech_engine.db.get_value("Sales Order Item", sales_order_item, "work_order_qty")
	...
	def test_donot_allow_to_make_multiple_pp_against_same_so(self):
		...
		pln.get_so_items()
		self.assertRaises(artech_engine.ValidationError, pln.save)
	...
	def test_so_based_bill_of_material(self):
		...
		pln1 = artech_engine.new_doc("Production Plan")
		...
		pln2 = artech_engine.new_doc("Production Plan")
	...
	def test_production_plan_combine_items(self):
		...
		for plan_reference in pln.prod_plan_references:
			...
			so_wo_qty = artech_engine.db.get_value(
		...
		for so_item in so_items:
			so_wo_qty = artech_engine.db.get_value("Sales Order Item", so_item, "work_order_qty")
	...
	def test_production_plan_subassembly_default_supplier(self):
		...
		item_doc = artech_engine.get_doc("Item", "Test Motherboard")
		company = "_Test Company"
	...
	def test_production_plan_for_subcontracting_po(self):
		...
		from artech.manufacturing.doctype.bom.test_bom import create_nested_bom
		from artech.subcontracting.doctype.subcontracting_bom.test_subcontracting_bom import (
			create_subcontracting_bom,
		)

		def make_purchase_receipt_from_po(po_doc):
			from artech.buying.doctype.purchase_order.purchase_order import make_subcontracting_order
			...
			from artech.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt import (
		...
		item_doc = artech_engine.get_doc("Item", fg_item)
		company = "_Test Company"
		...
		po = artech_engine.db.get_value("Purchase Order Item", {"production_plan": plan.name}, "parent")
		po_doc = artech_engine.get_doc("Purchase Order", po)
		...
		po_doc.cancel()
		po_doc = artech_engine.copy_doc(po_doc)
		...
		self.assertEqual(
			artech_engine.db.count("Purchase Order Item", {"production_plan": plan.name, "docstatus": 1}), 2
	...
	def test_sales_order_references_for_sub_assembly_items(self):
		...
		production_plan = artech_engine.new_doc("Production Plan")
		...
		po_items = artech_engine.get_all(
			"Purchase Order Item",
			{
	...
	def test_production_plan_for_mr_items(self):
		...
		mr_item = artech_engine.db.get_value("Material Request Item", {"production_plan": plan.name}, "parent")
		mr_doc = artech_engine.get_doc("Material Request", mr_item)
		mr_doc.submit()
		plan.reload()
		plan.make_material_request()

		# Test 1 : No more MRs should be created as quantity from Production Plan is fulfilled
		self.assertEqual(artech_engine.db.count("Material Request Item", {"production_plan": plan.name}), 2)
		...
		self.assertRaises(artech_engine.ValidationError, mr_doc.submit)
	...
	def test_production_plan_combine_subassembly(self):
		...
		artech_engine.db.set_value("BOM Item", parent_bom_2.items[0].name, "bom_no", subassembly_bom)
		...
		plan.append(
			...
			{  # Add Green-Car to Prod Plan
				"use_multi_level_bom": 1,
				"item_code": "Green-Car",
				"bom_no": artech_engine.db.get_value("Item", "Green-Car", "default_bom"),
				"planned_qty": 1,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = artech_engine.db.get_value(
			"Material Request Item",
			{"production_plan": production_plan.name, "item_code": "CUST-0987"},
			"parent",
		)
		mr = artech_engine.get_doc("Material Request", material_request)
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		if not artech_engine.db.get_value("BOM", {"item": "Test BOM 3"}):
			make_bom(item="Test BOM 3", raw_materials=["Test RM BOM 1"], rm_qty=3)

		if not artech_engine.db.get_value("BOM", {"item": "Test BOM 2"}):
			make_bom(item="Test BOM 2", raw_materials=["Test BOM 3"], rm_qty=3)

		if not artech_engine.db.get_value("BOM", {"item": "Test BOM 1"}):
		...
		pln.append(
			...
			{
				"item_code": item_code,
				"bom_no": artech_engine.db.get_value("BOM", {"item": "Test BOM 1"}),
				"planned_qty": 3,
			},
		)
	...
	def test_get_sales_order_with_variant(self):
		...
		if not artech_engine.db.exists("Item", {"item_code": "PIV"}):
		...
		if not artech_engine.db.exists("BOM", {"item": "PIV"}):
			parent_bom = make_bom(item="PIV", raw_materials=[rm_item.item_code])
		else:
			parent_bom = artech_engine.get_doc("BOM", {"item": "PIV"})

		if not artech_engine.db.exists("Item", {"item_code": "PIV-RED"}):
			variant = create_variant("PIV", {"Colour": "Red"})
			variant.save()
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		else:
			variant = artech_engine.get_doc("Item", "PIV-RED")
		if not artech_engine.db.exists("BOM", {"item": "PIV-RED"}):
			variant_bom = make_bom(item=variant.item_code, raw_materials=[rm_item.item_code])
		...
		so.cancel()
		artech_engine.delete_doc("Sales Order", so.name)
		variant_bom.cancel()
		artech_engine.delete_doc("BOM", variant_bom.name)
	...
	def test_get_sales_order_items_for_product_bundle(self):
		...
		from artech.manufacturing.doctype.work_order.test_work_order import (
		...
		if not artech_engine.db.exists("Warehouse", fg_warehouse):
		...
		manufacture_entry = artech_engine.get_doc(create_stock_entry(work_order.name, "Manufacture"))
	...
	def test_multiple_work_order_for_production_plan_item(self):
		...
		def create_work_order(item, pln, qty):
			...
			for _key, item in items_data.items():
				...
				wo_doc = artech_engine.get_doc("Work Order", wo_name)
				wo_doc.update(
		...
		wo1 = artech_engine.get_doc("Work Order", wo_list[0])
		wo1.cancel()
		pln.reload()
		self.assertEqual(pln.po_items[0].ordered_qty, 2)

		# Cancel 2nd Work Order
		wo2 = artech_engine.get_doc("Work Order", wo_list[1])
		wo2.cancel()
		pln.reload()
	...
	def test_production_plan_pending_qty_with_sales_order(self):
		...
		se = artech_engine.get_doc(make_se_from_wo(wo.name, "Manufacture", 1))
		se.submit()
	...
	def test_qty_based_status(self):
		pp = artech_engine.new_doc("Production Plan")
		pp.po_items = [artech_engine._dict(planned_qty=5, produce_qty=4)]
		self.assertFalse(pp.all_items_completed())

		pp.po_items = [
			artech_engine._dict(planned_qty=5, produce_qty=10),
			artech_engine._dict(planned_qty=5, produce_qty=4),
		]
		self.assertFalse(pp.all_items_completed())

	def test_production_plan_planned_qty(self):
		# Case 1: When Planned Qty is non-integer and UOM is integer.
		from artech.utilities.transaction_base import UOMMustBeIntegerError
	...
	def test_temporary_name_relinking(self):
		...
		for _ in range(10):
			po_item = pp.append(
				"po_items",
				{
					"name": artech_engine.generate_hash(length=10),
					"temporary_name": artech_engine.generate_hash(length=10),
				},
			)
		...
		pp.append("sub_assembly_items", {"production_plan_item": artech_engine.generate_hash(length=16)})
		pp._rename_temporary_references()
	...
	def test_produced_qty_for_multi_level_bom_item(self):
		...
		pln.make_work_order()
		work_orders = artech_engine.get_all("Work Order", filters={"production_plan": pln.name}, pluck="name")
		sa_wo = fg_wo = None
		...
		se = artech_engine.get_doc(make_se_from_wo(sa_wo, "Manufacture"))
		se.submit()

		se = artech_engine.get_doc(make_se_from_wo(fg_wo, "Manufacture"))
		se.submit()
	...
	def test_material_request_item_for_purchase_uom(self):
		...
		if not artech_engine.db.exists("UOM Conversion Detail", {"parent": bom_item, "uom": "Nos"}):
			doc = artech_engine.get_doc("Item", bom_item)
		...
		for row in artech_engine.get_all(
			"Material Request Item",
			filters={"production_plan": pln.name},
	...
	def test_reserved_qty_for_production_plan_for_material_requests(self):
		from artech.stock.utils import get_or_make_bin

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		before_qty = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))

		pln = create_production_plan(item_code="Test Production Item 1")

		bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
		after_qty = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_reserved_qty_for_production_plan_for_work_order(self):
		...
		work_orders = []
		for row in artech_engine.get_all("Work Order", filters={"production_plan": pln.name}, fields=["name"]):
			wo_doc = artech_engine.get_doc("Work Order", row.name)
		...
		if rm_work_order:
			s = artech_engine.get_doc(make_se_from_wo(rm_work_order.name, "Material Transfer for Manufacture", 1))
			s.submit()
			bin_name = get_or_make_bin("Raw Material Item 1", "_Test Warehouse - _TC")
			after_qty = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_reserved_qty_for_production_plan_for_material_requests_with_multi_UOM(self):
		...
		for row in pln.mr_items:
			...
			reserved_qty = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
			self.assertEqual(reserved_qty - before_qty, 100.0)

		pln.submit_material_request = 1
		pln.make_work_order()

		for work_order in artech_engine.get_all(
			"Work Order",
			fields=["name"],
			filters={"production_plan": pln.name},
		):
			wo_doc = artech_engine.get_doc("Work Order", work_order.name)
			wo_doc.source_warehouse = "_Test Warehouse - _TC"
			wo_doc.wip_warehouse = "_Test Warehouse 1 - _TC"
			wo_doc.fg_warehouse = "_Test Warehouse - _TC"
			wo_doc.submit()

		reserved_qty_after_mr = flt(artech_engine.db.get_value("Bin", bin_name, "reserved_qty_for_production_plan"))
	...
	def test_transfer_and_purchase_mrp_for_purchase_uom(self):
		...
		parent_bom = create_nested_bom(bom_tree, prefix="")
		if not artech_engine.db.exists("UOM Conversion Detail", {"parent": "Test RM Item INK", "uom": "Kg"}):
			doc = artech_engine.get_doc("Item", "Test RM Item INK")
			doc.purchase_uom = "Kg"
	...
	def test_mr_qty_for_complex_bom(self):
		...
		def set_bom_qty(item_code, qtys):
			...
			bom = artech_engine.get_doc("BOM", {"item": item_code})
	...
	def test_reserve_sub_assembly_items(self):
		...
		for d in work_orders:
			wo_doc = artech_engine.get_doc("Work Order", d.name)
			...
			if d.production_item == "Frame Assembly":
				...
				se_doc = artech_engine.get_doc(make_se_from_wo(wo_doc.name, "Manufacture", 2))
				se_doc.submit()
	...
	def test_calculation_of_sub_assembly_items(self):
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": artech_engine.db.get_value("Item", "_Test FG Item 2", "default_bom"),
				"planned_qty": 50,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": artech_engine.db.get_value("Item", "_Test FG Item 3", "default_bom"),
				"planned_qty": 10,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
		...
		plan.append(
			"po_items",
			{
				...
				"bom_no": artech_engine.db.get_value("Item", "_Test FG Item 4", "default_bom"),
				"planned_qty": 10,
				"planned_start_date": now_datetime(),
				"stock_uom": "Nos",
				"warehouse": "_Test Warehouse - _TC",
	...
	def test_stock_reservation_against_production_plan(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "enable_stock_reservation", 1)
		...
		work_orders = artech_engine.get_all("Work Order", filters={"production_plan": plan.name}, pluck="name")
		...
		artech_engine.db.set_single_value("Stock Settings", "enable_stock_reservation", 0)
	...
	def test_stock_reservation_of_serial_nos_against_production_plan(self):
		...
		for item_code in [
			...
			doc = artech_engine.get_doc("Item", item_code)
			doc.has_serial_no = 1
		...
		serial_nos_res_for_pp = artech_engine.get_all(
		...
		for wo_name in list(set(work_orders)):
			...
			sre = StockReservation(wo_doc)
			reserved_entries = sre.get_reserved_entries("Work Order", wo_doc.name)
			serial_nos_res_for_wo = artech_engine.get_all(
				"Serial and Batch Entry",
				filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
				pluck="serial_no",
			)
	...
	def test_stock_reservation_of_batch_nos_against_production_plan(self):
		...
		reserved_entries = sre.get_reserved_entries("Production Plan", plan.name)
		self.assertTrue(len(reserved_entries) == 9)
		batches_reserved_for_pp = artech_engine.get_all(
			"Serial and Batch Entry",
			filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
			pluck="batch_no",
		)
		...
		for wo_name in list(set(work_orders)):
			...
			sre = StockReservation(wo_doc)
			reserved_entries = sre.get_reserved_entries("Work Order", wo_doc.name)
			batches_reserved_for_wo = artech_engine.get_all(
				"Serial and Batch Entry",
				filters={"parent": ("in", [x.name for x in reserved_entries]), "docstatus": 1},
				pluck="batch_no",
			)
	...
	def test_production_plan_for_partial_sub_assembly_items(self):
		...
		artech_engine.flags.test_print = False
		...
		work_order = artech_engine.db.get_value("Work Order", {"production_plan": plan.name, "docstatus": 0}, "name")
		wo_doc = artech_engine.get_doc("Work Order", work_order)
	...
	def test_phantom_bom_explosion_across_multiple_po_items(self):
		...
		plan = artech_engine.new_doc("Production Plan")
	...
	if not args.get("sales_order"):
		pln.append(
			"po_items",
			{
				...
				"bom_no": artech_engine.db.get_value("Item", args.item_code, "default_bom"),
...
def make_bom(**args):
	...
	if args.set_as_default_bom and not args.do_not_save and not args.do_not_submit:
		artech_engine.set_value("Item", args.item, "default_bom", bom.name)
