...
from artech_engine import _, msgprint
...
from artech.manufacturing.doctype.bom.bom import get_children as get_bom_children
from artech.manufacturing.doctype.bom.bom import validate_bom_no
from artech.manufacturing.doctype.work_order.work_order import get_item_details
...
from artech.stock.get_item_details import get_conversion_factor
...
class ProductionPlan(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.manufacturing.doctype.material_request_plan_item.material_request_plan_item import (
			MaterialRequestPlanItem,
		)
		from artech.manufacturing.doctype.production_plan_item.production_plan_item import ProductionPlanItem
		from artech.manufacturing.doctype.production_plan_item_reference.production_plan_item_reference import (
			ProductionPlanItemReference,
		)
		from artech.manufacturing.doctype.production_plan_material_request.production_plan_material_request import (
			ProductionPlanMaterialRequest,
		)
		from artech.manufacturing.doctype.production_plan_material_request_warehouse.production_plan_material_request_warehouse import (
			ProductionPlanMaterialRequestWarehouse,
		)
		from artech.manufacturing.doctype.production_plan_sales_order.production_plan_sales_order import (
			ProductionPlanSalesOrder,
		)
		from artech.manufacturing.doctype.production_plan_sub_assembly_item.production_plan_sub_assembly_item import (
	...
	def validate_sales_orders(self, sales_order: str | None = None):
		...
		if not data and sales_orders:
			...
			artech_engine.throw(msg, title=title)
	...
	def validate_data(self):
		for d in self.get("po_items"):
			if not d.bom_no:
				artech_engine.throw(_("Please select BOM for Item in Row {0}").format(d.idx))
			else:
				validate_bom_no(d.item_code, d.bom_no)

			if not flt(d.planned_qty):
				artech_engine.throw(_("Please enter Planned Qty for Item {0} at row {1}").format(d.item_code, d.idx))
	...
	def get_open_sales_orders(self):
		...
		if open_so:
			...
			artech_engine.msgprint(_("Sales orders are not available for production"))
	...
	def get_pending_material_requests(self):
		...
		mr = artech_engine.qb.DocType("Material Request")
		mr_item = artech_engine.qb.DocType("Material Request Item")

		pending_mr_query = (
			artech_engine.qb.from_(mr)
			.from_(mr_item)
	...
	def get_bom_item_condition(self):
		...
		has_bom = artech_engine.db.exists({"doctype": "BOM", "item": self.item_code, "docstatus": 1})

		if not has_bom:
			bom = artech_engine.qb.DocType("BOM")
			template_item = artech_engine.db.get_value("Item", self.item_code, ["variant_of"])
	...
	def get_so_items(self):
		...
		if not self.get("sales_orders") or not self.get_so_mr_list("sales_order", "sales_orders"):
			artech_engine.throw(_("Please fill the Sales Orders table"), title=_("Sales Orders Required"))

		so_list = self.get_so_mr_list("sales_order", "sales_orders")

		bom = artech_engine.qb.DocType("BOM")
		so_item = artech_engine.qb.DocType("Sales Order Item")

		items_subquery = artech_engine.qb.from_(bom).select(bom.name).where(bom.is_active == 1)
		items_query = (
			artech_engine.qb.from_(so_item)
			.select(
				so_item.parent,
				so_item.item_code,
				so_item.warehouse,
		...
		if self.item_code and artech_engine.db.exists("Item", self.item_code):
		...
		pi = artech_engine.qb.DocType("Packed Item")
	...
	def get_mr_items(self):
		...
		if not self.get("material_requests") or not self.get_so_mr_list(
			...
			artech_engine.throw(_("Please fill the Material Requests table"), title=_("Material Requests Required"))
		...
		items_query = (
			artech_engine.qb.from_(mr_item)
			.select(
				mr_item.parent,
				mr_item.name,
				mr_item.item_code,
	...
	def add_reference_to_raw_materials(self):
		for item in self.mr_items:
			if reference := next(
				...
				and item.from_bom
				and item.main_item_code != artech_engine.get_cached_value("BOM", item.from_bom, "item")
			):
				artech_engine.throw(
					_(
	...
	def get_so_wise_planned_qty(sales_orders):
		so_wise_planned_qty = artech_engine._dict()
	...
	def update_bin_qty(self):
		for d in self.mr_items:
			if d.warehouse:
				...
				bin = artech_engine.get_doc("Bin", bin_name, for_update=True)
	...
	def delete_draft_work_order(self):
		for d in artech_engine.get_all(
			...
			artech_engine.delete_doc("Work Order", d.name)
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			item_details = {
				...
				"source_warehouse": artech_engine.get_value("BOM", d.bom_no, "default_source_warehouse"),
			...
			if not item_details["project"] and d.sales_order:
				item_details["project"] = artech_engine.get_cached_value("Sales Order", d.sales_order, "project")
	...
	def make_work_order(self):
		from artech.manufacturing.doctype.work_order.work_order import get_default_warehouse
		...
		if not wo_list:
			artech_engine.msgprint(_("No Work Orders were created"))

		if not po_list:
			artech_engine.msgprint(_("No Purchase Orders were created"))
	...
	def make_work_order_for_subassembly_items(self, wo_list, subcontracted_po, default_warehouses):
		for row in self.sub_assembly_items:
			...
			work_order_data = {
				"source_warehouse": artech_engine.get_value("BOM", row.bom_no, "default_source_warehouse"),
	...
	def make_subcontracted_purchase_order(self, subcontracted_po, purchase_orders):
		...
		for supplier, po_list in subcontracted_po.items():
			po = artech_engine.new_doc("Purchase Order")
	...
	def show_list_created_message(self, doctype, doc_list=None):
		...
		artech_engine.flags.mute_messages = False
		if doc_list:
	...
	def create_work_order(self, item):
		...
		wo = artech_engine.new_doc("Work Order")
		wo.update(item)
	...
	def validate_mr_subcontracted(self):
		for row in self.mr_items:
			if row.material_request_type == "Subcontracting":
				if not artech_engine.db.get_value("Item", row.item_code, "is_sub_contracted_item"):
					artech_engine.throw(
	...
	def make_material_request(self):
		...
		for item in self.mr_items:
			...
			item_doc = artech_engine.get_cached_doc("Item", item.item_code)
			...
			if key not in material_request_map:
				...
				material_request_map[key] = artech_engine.new_doc("Material Request")
			...
			material_request.append(
				"items",
				{
					...
					"project": artech_engine.db.get_value("Sales Order", item.sales_order, "project")
					if item.sales_order
					else None,
				},
			)
	...
	def get_sub_assembly_items(self, manufacturing_type: str | None = None):
		...
		bin_details = artech_engine._dict()

		track_semi_finished_goods = True
		for row in self.po_items:
			if self.skip_available_sub_assembly_item and not self.sub_assembly_warehouse:
				artech_engine.throw(_("Row #{0}: Please select the Sub Assembly Warehouse").format(row.idx))

			if not row.item_code:
				artech_engine.throw(_("Row #{0}: Please select Item Code in Assembly Items").format(row.idx))

			if not row.bom_no:
				artech_engine.throw(_("Row #{0}: Please select the BOM No in Assembly Items").format(row.idx))

			if artech_engine.db.get_value("BOM", row.bom_no, "track_semi_finished_goods"):
		...
		if (
			...
			artech_engine.msgprint(message, title=_("Note"))
	...
	def set_sub_assembly_items_based_on_level(self, row, bom_data, manufacturing_type=None):
		...
		is_group_warehouse = artech_engine.db.get_value("Warehouse", self.sub_assembly_warehouse, "is_group")
	...
	def set_default_supplier_for_subcontracting_order(self):
		...
		default_supplier = artech_engine._dict(
	...
	def all_items_completed(self):
		...
		wo_status = artech_engine.get_all(
			"Work Order",
			filters={
				"production_plan": self.name,
...
def download_raw_materials(doc: str | dict | Document, warehouses: str | list | None = None):
	if isinstance(doc, str):
		doc = artech_engine._dict(json.loads(doc))
...
def get_exploded_items(item_details, company, bom_no, include_non_stock_items, planned_qty=1, doc=None):
	bei = artech_engine.qb.DocType("BOM Explosion Item")
	bom = artech_engine.qb.DocType("BOM")
	item = artech_engine.qb.DocType("Item")
	item_default = artech_engine.qb.DocType("Item Default")
	item_uom = artech_engine.qb.DocType("UOM Conversion Detail")

	data = (
		artech_engine.qb.from_(bei)
		.join(bom)
...
def get_subitems(
	...
	bom_item = artech_engine.qb.DocType("BOM Item")
	bom = artech_engine.qb.DocType("BOM")
	item = artech_engine.qb.DocType("Item")
	item_default = artech_engine.qb.DocType("Item Default")
	item_uom = artech_engine.qb.DocType("UOM Conversion Detail")
	...
	items = (
		artech_engine.qb.from_(bom_item)
		.join(bom)
...
def get_material_request_items(
	...
	if row["purchase_uom"] != row["stock_uom"]:
		if not (row["conversion_factor"] or artech_engine.flags.show_qty_in_stock_uom):
			artech_engine.throw(
	...
	if artech_engine.db.get_value("UOM", row["purchase_uom"], "must_be_whole_number"):
		required_qty = ceil(required_qty)

	if include_safety_stock:
		required_qty += flt(row["safety_stock"])

	item_details = artech_engine.get_cached_value("Item", row.item_code, ["purchase_uom", "stock_uom"], as_dict=1)
...
def get_sales_orders(self):
	...
	open_so_subquery1 = artech_engine.qb.from_(bom).select(bom.name).where(bom.is_active == 1)

	open_so_subquery2 = (
		artech_engine.qb.from_(pi)
		.select(pi.name)
		.where(
			(pi.parent == so.name)
			& (pi.parent_item == so_item.item_code)
...
def get_bin_details(
	...
	if isinstance(row, str):
		row = artech_engine._dict(json.loads(row))

	bin = artech_engine.qb.DocType("Bin")
	wh = artech_engine.qb.DocType("Warehouse")

	subquery = artech_engine.qb.from_(wh).select(wh.name).where(wh.company == company)
	...
	if warehouse:
		lft, rgt = artech_engine.db.get_value("Warehouse", warehouse, ["lft", "rgt"])
...
def get_warehouse_list(warehouses):
	...
	for row in warehouses:
		child_warehouses = artech_engine.db.get_descendants("Warehouse", row.get("warehouse"))
		if child_warehouses:
...
def get_items_for_material_requests(
	doc: str | artech_engine._dict | Document,
	...
	if doc.get("sub_assembly_items"):
		for sa_row in doc.sub_assembly_items:
			sa_row = artech_engine._dict(sa_row)
	...
	so_item_details = artech_engine._dict()
	...
	for data in po_items:
		...
		if data.get("bom") or data.get("bom_no"):
			...
			if not planned_qty:
				artech_engine.throw(_("For row {0}: Enter Planned Qty").format(data.get("idx")))
			...
			item_master = artech_engine.get_doc("Item", data["item_code"]).as_dict()
			purchase_uom = item_master.purchase_uom or item_master.stock_uom
			conversion_factor = (
				get_uom_conversion_factor(item_master.name, purchase_uom) if item_master.purchase_uom else 1.0
			)

			item_details[item_master.item_code] = artech_engine._dict(
				{
		...
		qty_precision = artech_engine.get_precision("Material Request Plan Item", "quantity")

		for key, details in item_details.items():
			details.qty = flt(details.qty, qty_precision)
			so_item_details.setdefault(sales_order, artech_engine._dict())
	...
	if not mr_items:
		to_enable = artech_engine.bold(_("Ignore Existing Projected Quantity"))
		warehouse = artech_engine.bold(doc.get("for_warehouse"))
		message = (
			_(
...
def get_materials_from_other_locations(item, warehouses, new_mr_items, company):
	from artech.stock.doctype.pick_list.pick_list import get_available_item_locations

	stock_uom, purchase_uom = artech_engine.db.get_value(
	...
	precision = artech_engine.get_precision("Material Request Plan Item", "quantity")
	if flt(required_qty, precision) > 0:
		required_qty = required_qty

		if artech_engine.db.get_value("UOM", purchase_uom, "must_be_whole_number"):
			required_qty = ceil(required_qty)
...
def get_sub_assembly_items(
	...
	precision = artech_engine.get_precision("Production Plan Sub Assembly Item", "qty")
	for d in data:
		if d.expandable:
			parent_item_code = artech_engine.get_cached_value("BOM", bom_no, "item")
...
def get_reserved_qty_for_production_plan(item_code, warehouse):
	from artech.manufacturing.doctype.work_order.work_order import get_reserved_qty_for_production

	table = artech_engine.qb.DocType("Production Plan")
	child = artech_engine.qb.DocType("Material Request Plan Item")
...
def get_raw_materials_of_sub_assembly_items(
	...
	bei = artech_engine.qb.DocType("BOM Item")
	bom = artech_engine.qb.DocType("BOM")
	item = artech_engine.qb.DocType("Item")
	item_default = artech_engine.qb.DocType("Item Default")
	item_uom = artech_engine.qb.DocType("UOM Conversion Detail")
...
def sales_order_query(
	...
	artech_engine.has_permission("Production Plan", throw=True)

	if not filters:
		filters = {}

	so_table = artech_engine.qb.DocType("Sales Order")
	table = artech_engine.qb.DocType("Sales Order Item")

	query = (
		artech_engine.qb.from_(so_table)
		.join(table)
...
def get_reserved_qty_for_sub_assembly(item_code, warehouse):
	table = artech_engine.qb.DocType("Production Plan")
	child = artech_engine.qb.DocType("Production Plan Sub Assembly Item")
...
def make_stock_reservation_entries(
	...
	if isinstance(doc, str):
		doc = parse_json(doc)
		doc = artech_engine.get_doc("Production Plan", doc.get("name"))
