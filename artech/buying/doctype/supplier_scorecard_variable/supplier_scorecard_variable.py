...
class VariablePathNotFound(artech_engine.ValidationError):
	pass
	...
	def validate_path_exists(self):
		if "." in self.path:
			try:
				...
				artech_engine.throw(_("Could not find path for " + self.path), VariablePathNotFound)
...
def get_item_workdays(scorecard):
	"""Gets the number of days in this period"""
	supplier = artech_engine.get_doc("Supplier", scorecard.supplier)
	total_item_days = artech_engine.db.sql(
		"""
			SELECT
...
def get_cost_of_on_time_shipments(scorecard):
	...
	total_delivered_on_time_costs = artech_engine.db.sql(
		"""
			SELECT
...
def get_total_days_late(scorecard):
	...
	supplier = artech_engine.get_doc("Supplier", scorecard.supplier)
	total_delivered_late_days = artech_engine.db.sql(
		"""
			SELECT
	...
	total_missed_late_days = artech_engine.db.sql(
		"""
			SELECT
...
def get_on_time_shipments(scorecard):
	...
	# Look up all PO Items with delivery dates between our dates
	total_items_delivered_on_time = artech_engine.db.sql(
		"""
			SELECT
				COUNT(pr_item.qty)
			FROM
...
def get_rfq_response_days(scorecard):
	...
	supplier = artech_engine.get_doc("Supplier", scorecard.supplier)
	total_sq_days = artech_engine.db.sql(
		"""
			SELECT
