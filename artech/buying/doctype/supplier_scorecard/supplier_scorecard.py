...
from artech_engine.utils import add_days, add_years, get_last_day, getdate, nowdate

from artech.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import (
...
class SupplierScorecard(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.buying.doctype.supplier_scorecard_scoring_criteria.supplier_scorecard_scoring_criteria import (
			SupplierScorecardScoringCriteria,
		)
		from artech.buying.doctype.supplier_scorecard_scoring_standing.supplier_scorecard_scoring_standing import (
	...
	def calculate_total_score(self):
		scorecards = artech_engine.db.sql(
			"""
			SELECT
				scp.name
			FROM
		...
		for scp in scorecards:
			my_sc = artech_engine.get_doc("Supplier Scorecard Period", scp.name)
		...
		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.supplier_score)) and (
				...
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					...
					artech_engine.db.set_value("Supplier", self.supplier, fieldname, self.get(fieldname))


@artech_engine.whitelist()
def get_timeline_data(doctype: str, name: str):
	# Get a list of all the associated scorecards
	scs = artech_engine.get_doc(doctype, name)
	out = {}
	timeline_data = {}
	scorecards = artech_engine.db.sql(
		"""
	...
	for sc in scorecards:
		start_date, end_date, total_score = artech_engine.db.get_value(
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			artech_engine.get_doc("Supplier Scorecard", sc.name).save()


@artech_engine.whitelist()
def make_all_scorecards(docname: str):
	sc = artech_engine.get_doc("Supplier Scorecard", docname)
	supplier = artech_engine.get_doc("Supplier", sc.supplier)
