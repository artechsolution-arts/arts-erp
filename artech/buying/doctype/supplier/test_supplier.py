...
from artech.exceptions import PartyDisabled
from artech.tests.utils import ArtechTestSuite


class TestSupplier(ArtechTestSuite):
	def test_get_supplier_group_details(self):
		doc = artech_engine.new_doc("Supplier Group")
		...
		doc.save()
		s_doc = artech_engine.new_doc("Supplier")
	...
	def test_supplier_default_payment_terms(self):
		...
		artech_engine.db.set_value("Supplier", "_Test Supplier With Template 1", "payment_terms", "")
		...
		artech_engine.db.set_value("Supplier Group", "_Test Supplier Group", "payment_terms", "")
		artech_engine.db.set_value("Supplier", "_Test Supplier", "payment_terms", "")
	...
	def test_supplier_disabled(self):
		artech_engine.db.set_value("Supplier", "_Test Supplier", "disabled", 1)
		...
		artech_engine.db.set_value("Supplier", "_Test Supplier", "disabled", 0)

		po.save()

	def test_supplier_country(self):
		# Test that country field exists in Supplier DocType
		supplier = artech_engine.get_doc("Supplier", "_Test Supplier with Country")
	...
	def test_party_details_tax_category(self):
		...
		artech_engine.delete_doc_if_exists("Address", "_Test Address With Tax Category-Billing")
...
def create_supplier(**args):
	...
	if not args.supplier_name:
		args.supplier_name = artech_engine.generate_hash()

	if artech_engine.db.exists("Supplier", args.supplier_name):
		return artech_engine.get_doc("Supplier", args.supplier_name)
	...
	if args.get("party_account"):
		doc.append(
			"accounts",
			{
				"company": artech_engine.db.get_value("Account", args.get("party_account"), "company"),
...
class TestSupplierPortal(ArtechTestSuite):
	def test_portal_user_can_access_supplier_data(self):
		supplier = create_supplier()

		user = artech_engine.generate_hash() + "@example.com"
