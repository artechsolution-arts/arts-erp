...
artech_engine.ui.form.on("Supplier", {
	setup: function (frm) {
		...
		frm.set_query("supplier_primary_contact", function (doc) {
			return {
				query: "artech.buying.doctype.supplier.supplier.get_supplier_primary",
				filters: {
					supplier: doc.name,
					type: "Contact",
				},
	...
	refresh: function (frm) {
		if (artech_engine.defaults.get_default("supp_master_name") != "Naming Series") {
		...
		if (frm.doc.__islocal) {
			...
			frm.add_custom_button(
				__("Accounts Payable"),
				function () {
					artech_engine.set_route("query-report", "Accounts Payable", {
						party_type: "Supplier",
						party: frm.doc.name,
					});
				},
