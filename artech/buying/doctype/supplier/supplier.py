...
class Supplier(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from artech.buying.doctype.customer_number_at_supplier.customer_number_at_supplier import (
	...
	def autoname(self):
		supp_master_name = artech_engine.defaults.get_global_default("supp_master_name")
	...
	def _add_supplier_role(self, portal_user):
		...
		user_doc = artech_engine.get_doc("User", portal_user.user)
		roles = {r.role for r in user_doc.roles}

		if "Supplier" in roles:
			return

		if "System Manager" not in artech_engine.get_roles():
		...
		artech_engine.msgprint(_("Added Supplier Role to User {0}.").format(artech_engine.bold(user_doc.name)), alert=True)

	def validate(self):
		self.flags.is_new_doc = self.is_new()

		# validation for Naming Series mandatory field...
		if artech_engine.defaults.get_global_default("supp_master_name") == "Naming Series":
	...
	def get_supplier_group_details(self):
		doc = artech_engine.get_doc("Supplier Group", self.supplier_group)
		self.payment_terms = ""
		self.accounts = []
	...
	def validate_internal_supplier(self):
		...
		internal_supplier = artech_engine.db.get_value(
			"Supplier",
			{
	...
	def create_primary_contact(self):
		from artech.selling.doctype.customer.customer import make_contact
	...
	def create_primary_address(self):
		...
		from artech.selling.doctype.customer.customer import make_address
	...
	def after_rename(self, olddn, newdn, merge=False):
		if artech_engine.defaults.get_global_default("supp_master_name") == "Supplier Name":
...
def get_supplier_primary(
	...
	type_doctype = artech_engine.qb.DocType(type)
	dynamic_link = artech_engine.qb.DocType("Dynamic Link")

	query = (
		artech_engine.qb.from_(type_doctype)
		.join(dynamic_link)
