...
from artech.accounts.party import get_party_account, get_party_account_currency
from artech.buying.utils import check_on_hold_or_closed_status, validate_for_items
...
from artech.stock.doctype.item.item import get_item_defaults, get_last_purchase_details
from artech.stock.stock_balance import get_ordered_qty, update_bin_qty
...
class PurchaseOrder(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from artech.buying.doctype.purchase_order_item.purchase_order_item import PurchaseOrderItem
		from artech.buying.doctype.purchase_order_item_supplied.purchase_order_item_supplied import (
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not artech_engine.db.get_single_value("Buying Settings", "allow_zero_qty_in_purchase_order"):
			return
	...
	def validate_with_previous_doc(self):
		...
		if cint(artech_engine.db.get_single_value("Buying Settings", "maintain_same_rate")):
			self.validate_rate_with_reference_doc(
				[["Supplier Quotation", "supplier_quotation", "supplier_quotation_item"]]
			)

	def validate_supplier(self):
		prevent_po = artech_engine.db.get_value("Supplier", self.supplier, "prevent_pos")
		if prevent_po:
			standing = artech_engine.db.get_value("Supplier Scorecard", self.supplier, "status")
			if standing:
				artech_engine.throw(
		...
		warn_po = artech_engine.db.get_value("Supplier", self.supplier, "warn_pos")
		if warn_po:
			standing = artech_engine.db.get_value("Supplier Scorecard", self.supplier, "status")
	...
	def validate_minimum_order_qty(self):
		...
		itemwise_min_order_qty = artech_engine._dict(
		...
		itemwise_qty = artech_engine._dict()
			...
			if not self.is_old_subcontracting_flow:
				for item in self.items:
					if not item.fg_item:
						...
						elif not item.bom and not artech_engine.get_value("Item", item.fg_item, "default_bom"):
							artech_engine.throw(
	...
	def get_schedule_dates(self):
		for d in self.get("items"):
			if d.material_request_item and not d.schedule_date:
				d.schedule_date = artech_engine.db.get_value(
		...
		for d in self.get("items"):
			if d.item_code:
				...
				if last_purchase_details:
					...
					item_last_purchase_rate = artech_engine.get_cached_value(
	...
	def check_modified_date(self):
		mod_db = artech_engine.db.sql("select modified from `tabPurchase Order` where name = %s", self.name)
		date_diff = artech_engine.db.sql(f"select '{mod_db[0][0]}' - '{cstr(self.modified)}' ")
	...
	def update_delivered_qty_in_sales_order(self):
		...
		for so_name in sales_orders_to_update:
			so = artech_engine.get_lazy_doc("Sales Order", so_name)
	...
	def update_ordered_qty_in_so_for_removed_items(self, removed_items):
		...
		for item in removed_items:
			prev_ordered_qty = flt(
				artech_engine.get_cached_value("Sales Order Item", item.get("sales_order_item"), "ordered_qty")
			)
	...
	def auto_create_subcontracting_order(self):
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			if artech_engine.db.get_single_value("Buying Settings", "auto_create_subcontracting_order"):
	...
	def update_subcontracting_order_status(self):
		...
		if self.is_subcontracted and not self.is_old_subcontracting_flow:
			sco = artech_engine.db.get_value("Subcontracting Order", {"purchase_order": self.name, "docstatus": 1})
...
def item_last_purchase_rate(name, conversion_rate, item_code, conversion_factor=1.0):
	...
	if last_purchase_details:
		...
		item_last_purchase_rate = artech_engine.get_cached_value("Item", item_code, "last_purchase_rate")
		if item_last_purchase_rate:
...
def close_or_unclose_purchase_orders(names: str, status: str):
	if not artech_engine.has_permission("Purchase Order", "write"):
		artech_engine.throw(_("Not permitted"), artech_engine.PermissionError)

	names = json.loads(names)
	for name in names:
		po = artech_engine.get_lazy_doc("Purchase Order", name)
		if po.docstatus == 1:
			if status == "Closed":
...
def make_purchase_receipt(
	...
	has_unit_price_items = artech_engine.db.get_value("Purchase Order", source_name, "has_unit_price_items")
...
def make_purchase_invoice_from_portal(purchase_order_name: str):
	...
	if artech_engine.session.user not in artech_engine.get_all("Portal User", {"parent": doc.supplier}, pluck="user"):
		artech_engine.throw(_("Not Permitted"), artech_engine.PermissionError)
	doc.save()
	if not artech_engine.in_test:
		artech_engine.db.commit()
	artech_engine.response["type"] = "redirect"
	artech_engine.response.location = "/purchase-invoices/" + doc.name
...
def get_mapped_purchase_invoice(source_name, target_doc=None, ignore_permissions=False, args=None):
	...
	def update_item(obj, target, source_parent):
		...
		target.cost_center = (
			obj.cost_center
			or artech_engine.db.get_value("Project", obj.project, "cost_center")
...
def update_status(status: str, name: str):
	po = artech_engine.get_lazy_doc("Purchase Order", name, check_permission="submit")
	po.update_status(status)
...
def make_subcontracting_order(
	...
	if not is_po_fully_subcontracted(source_name):
		...
		if (save or submit) and artech_engine.has_permission(target_doc.doctype, "create"):
			target_doc.save()

			if submit and artech_engine.has_permission(target_doc.doctype, "submit", target_doc):
				try:
					target_doc.submit()
		...
		artech_engine.throw(_("This Purchase Order has been fully subcontracted."))
	...
	def post_process(source_doc, target_doc):
		...
		for idx, item in enumerate(target_doc.items):
			...
			if not target_doc.supplier_warehouse:
				...
				target_doc.supplier_warehouse = artech_engine.get_cached_value(
		...
		target_doc.reserve_stock = artech_engine.get_single_value(
			"Stock Settings", "auto_reserve_stock"
		) or artech_engine.get_value("Production Plan", target_doc.production_plan, "reserve_stock")
