...
from artech_engine.utils import add_days, flt, getdate, nowdate
...
from artech.accounts.party import get_due_date_from_template
...
from artech.controllers.accounts_controller import InvalidQtyError, update_child_qty_rate
...
class TestPurchaseOrder(ArtechTestSuite):
	@ArtechTestSuite.change_settings("Buying Settings", {"allow_multiple_items": 1})
	def test_purchase_order_qty(self):
		...
		self.assertRaises(artech_engine.NonNegativeError, po.save)
	...
	def test_make_purchase_receipt(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_purchase_receipt, po.name)
		po.submit()
	...
	def test_ordered_qty_against_pi_with_update_stock(self):
		...
		artech_engine.db.set_value("Item", "_Test Item", "over_delivery_receipt_allowance", 0)
		artech_engine.db.set_value("Item", "_Test Item", "over_billing_allowance", 0)
		artech_engine.db.set_single_value("Accounts Settings", "over_billing_allowance", 0)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			artech_engine.ValidationError, update_child_qty_rate, "Purchase Order", trans_item, po.name
		)
	...
	def test_purchase_order_invoice_receipt_workflow(self):
		from artech.accounts.doctype.purchase_invoice.purchase_invoice import make_purchase_receipt
	...
	def test_make_purchase_invoice(self):
		...
		self.assertRaises(artech_engine.ValidationError, make_pi_from_po, po.name)
		...
		artech_engine.get_doc(
			{
				...
				"currency": artech_engine.get_cached_value("Company", "_Test Company", "default_currency"),
				"conversion_factor": 1,
	...
	def test_po_for_blocked_supplier_all(self):
		...
		self.assertRaises(artech_engine.ValidationError, create_purchase_order)
	...
	@ArtechTestSuite.change_settings("Accounts Settings", {"automatically_fetch_payment_terms": 0})
	def test_terms_are_not_copied_if_automatically_fetch_payment_terms_is_unchecked(self):
		...
		artech_engine.db.set_value("Company", "_Test Company", "payment_terms", "_Test Payment Term Template 1")
		pi = make_pi_from_po(po.name)
		pi.save()

		self.assertEqual(pi.get("payment_terms_template"), "_Test Payment Term Template 1")
		artech_engine.db.set_value("Company", "_Test Company", "payment_terms", "")
	...
	def test_advance_payment_entry_unlink_against_purchase_order(self):
		...
		po_doc = artech_engine.get_doc("Purchase Order", po_doc.name)
		po_doc.cancel()

		pe_doc = artech_engine.get_doc("Payment Entry", pe.name)
		pe_doc.cancel()

	def create_account(self, account_name, company, currency, parent):
		if not artech_engine.db.get_value("Account", filters={"account_name": account_name, "company": company}):
			...
			account = artech_engine.get_doc("Account", {"account_name": account_name, "company": company})
	...
	def test_advance_paid_upon_payment_entry_cancellation(self):
		...
		supplier_doc = artech_engine.get_doc("Supplier", supplier)
	...
	def test_po_optional_blanket_order(self):
		...
		po_doc = artech_engine.get_doc("Purchase Order", po.get("name"))
	...
	def test_internal_transfer_flow(self):
		...
		artech_engine.db.set_single_value("Selling Settings", "maintain_same_sales_rate", 1)
	...
	def test_variant_item_po(self):
		...
		self.assertRaises(artech_engine.ValidationError, po.save)

	def test_update_items_for_subcontracting_purchase_order(self):
		from artech.controllers.tests.test_subcontracting_controller import (
		...
		self.assertRaises(artech_engine.ValidationError, update_items, po=po, qty=30)
	...
	def test_new_sc_flow(self):
		...
		self.assertRaises(artech_engine.ValidationError, sco.save)
		...
		self.assertRaises(artech_engine.ValidationError, make_subcontracting_order, po.name)

	@ArtechTestSuite.change_settings("Buying Settings", {"auto_create_subcontracting_order": 1})
	def test_auto_create_subcontracting_order(self):
		...
		self.assertTrue(artech_engine.db.get_value("Subcontracting Order", {"purchase_order": po.name}))
	...
	def test_purchase_order_advance_payment_status(self):
		...
		po = create_purchase_order()
		self.assertEqual(artech_engine.db.get_value(po.doctype, po.name, "advance_payment_status"), "Not Initiated")
		...
		po.reload()
		self.assertEqual(artech_engine.db.get_value(po.doctype, po.name, "advance_payment_status"), "Initiated")

		pe = get_payment_entry(po.doctype, po.name).save().submit()

		pr.reload()
		self.assertEqual(pr.status, "Paid")
		self.assertEqual(artech_engine.db.get_value(po.doctype, po.name, "advance_payment_status"), "Fully Paid")
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"allow_zero_qty_in_purchase_order": 1})
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"maintain_same_rate": 0})
...
def create_purchase_order(**args):
	...
	po.currency = args.currency or artech_engine.get_cached_value("Company", po.company, "default_currency")
...
def get_ordered_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "ordered_qty"))


def get_requested_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(artech_engine.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty"))
