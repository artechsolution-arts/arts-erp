...
artech.accounts.taxes.setup_tax_validations("Purchase Order");
artech.buying.setup_buying_controller();

artech_engine.ui.form.on("Purchase Order", {
	setup: function (frm) {
	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "artech.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
						...
						callback: function (r) {
							if (r && r.message) {
								const doc = artech_engine.model.sync(r.message);
...
artech_engine.ui.form.on("Purchase Order Item", {
...
artech.buying.PurchaseOrderController = class PurchaseOrderController extends (
	...
	make_stock_entry() {
		artech_engine.call({
			method: "artech.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_invoice() {
		artech_engine.model.open_mapped_doc({
			method: "artech.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
			frm: this.frm,
		});
	}
	...
	make_subcontracting_order() {
		artech_engine.model.open_mapped_doc({
			method: "artech.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
			frm: this.frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				artech.utils.map_current_doc({
					method: "artech.buying.doctype.supplier_quotation.supplier_quotation.make_purchase_order",
							...
							(r.message[0] || []).forEach(function (d) {
								if (
									...
									if (qty > 0) {
										...
										var new_row = artech_engine.model.add_child(
											me.frm.doc,
...
extend_cscript(cur_frm.cscript, new artech.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	artech_engine.call({
		method: "artech.buying.doctype.purchase_order.purchase_order.update_status",
...
artech_engine.ui.form.on("Purchase Order", "is_subcontracted", function (frm) {
	if (frm.doc.is_old_subcontracting_flow) {
