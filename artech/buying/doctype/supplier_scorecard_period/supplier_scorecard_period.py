...
import artech.buying.doctype.supplier_scorecard_variable.supplier_scorecard_variable as variable_functions
from artech.buying.doctype.supplier_scorecard_criteria.supplier_scorecard_criteria import (
	get_variables,
)
...
class SupplierScorecardPeriod(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.buying.doctype.supplier_scorecard_scoring_variable.supplier_scorecard_scoring_variable import (
	...
	def calculate_weighted_score(self, weighing_function):
		try:
			weighed_score = artech_engine.safe_eval(
...
def make_supplier_scorecard(source_name, target_doc=None):
	def update_criteria_fields(obj, target, source_parent):
		target.max_score, target.formula = artech_engine.db.get_value(
