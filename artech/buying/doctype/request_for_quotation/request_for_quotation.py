...
from artech_engine.core.doctype.communication.email import make
...
from artech_engine.utils import get_url
from artech_engine.utils.print_format import download_pdf
from artech_engine.utils.user import get_user_fullname

from artech.accounts.party import get_party_account_currency, get_party_details
from artech.buying.utils import validate_for_items
from artech.controllers.buying_controller import BuyingController
from artech.stock.doctype.material_request.material_request import set_missing_values
...
class RequestforQuotation(BuyingController):
	...
	if TYPE_CHECKING:
		...
		from artech.buying.doctype.request_for_quotation_item.request_for_quotation_item import (
			RequestforQuotationItem,
		)
		from artech.buying.doctype.request_for_quotation_supplier.request_for_quotation_supplier import (
	...
	def set_has_unit_price_items(self):
		...
		"""
		if not artech_engine.db.get_single_value("Buying Settings", "allow_zero_qty_in_request_for_quotation"):
			return
	...
	def validate_duplicate_supplier(self):
		...
		if len(supplier_list) != len(set(supplier_list)):
			artech_engine.throw(_("Same supplier has been entered multiple times"))

	def validate_supplier_list(self):
		for d in self.suppliers:
			prevent_rfqs = artech_engine.db.get_value("Supplier", d.supplier, "prevent_rfqs")
			if prevent_rfqs:
				standing = artech_engine.db.get_value("Supplier Scorecard", d.supplier, "status")
				artech_engine.throw(
					_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(
						d.supplier, standing
					)
				)
			warn_rfqs = artech_engine.db.get_value("Supplier", d.supplier, "warn_rfqs")
			if warn_rfqs:
				standing = artech_engine.db.get_value("Supplier Scorecard", d.supplier, "status")
	...
	def update_email_id(self):
		for rfq_supplier in self.suppliers:
			if not rfq_supplier.email_id:
				rfq_supplier.email_id = artech_engine.db.get_value("Contact", rfq_supplier.contact, "email_id")

	def validate_email_id(self, args):
		if not args.email_id:
			artech_engine.throw(
				_("Row {0}: For Supplier {1}, Email Address is Required to send an email").format(
					args.idx, artech_engine.bold(args.supplier)
				)
			)
	...
	def get_link(self):
		...
		route = artech_engine.db.get_value(
			"Portal Menu Item", {"reference_doctype": "Request for Quotation"}, ["route"]
		)
		if not route:
			artech_engine.throw(_("Please add Request for Quotation to the sidebar in Portal Settings."))
	...
	def update_supplier_part_no(self, supplier):
		...
		for item in self.items:
			item.supplier_part_no = artech_engine.db.get_value(
	...
	def update_supplier_contact(self, rfq_supplier, link):
		...
		if artech_engine.db.exists("User", rfq_supplier.email_id):
			user = artech_engine.get_doc("User", rfq_supplier.email_id)
		else:
	...
	def link_supplier_contact(self, rfq_supplier, user):
		...
		if rfq_supplier.contact:
			contact = artech_engine.get_doc("Contact", rfq_supplier.contact)
		else:
			contact = artech_engine.new_doc("Contact")
	...
	def update_user_in_supplier(self, supplier, user):
		...
		if not artech_engine.db.exists("Portal User", {"parent": supplier, "user": user}):
	...
	def supplier_rfq_mail(self, data, update_password_link, rfq_link, preview=False):
		full_name = get_user_fullname(artech_engine.session["user"])
		if full_name == "Guest":
		...
		if data.get("contact"):
			contact = artech_engine.get_doc("Contact", data.get("contact"))
		...
		fixed_procurement_email = artech_engine.db.get_single_value("Buying Settings", "fixed_email")
		if fixed_procurement_email:
			sender = artech_engine.db.get_value("Email Account", fixed_procurement_email, "email_id")
		else:
			sender = artech_engine.session.user not in STANDARD_USERS and artech_engine.session.user or None

		message_template = self.mfs_html if self.use_html else self.message_for_supplier
		# nosemgrep: artech_engine-semgrep-rules.rules.security.artech_engine-ssti
		rendered_message = artech_engine.render_template(message_template, doc_args)

		subject_source = (
			self.subject
			or artech_engine.get_value("Email Template", self.email_template, "subject")
			or _("Request for Quotation")
		)
		rendered_subject = artech_engine.render_template(subject_source, doc_args)
		if preview:
			return {
		...
		if self.send_document_print:
			supplier_language = artech_engine.db.get_value("Supplier", data.supplier, "language")
			system_language = artech_engine.db.get_single_value("System Settings", "language")
			attachments.append(
	...
	def send_email(self, data, sender, subject, message, attachments):
		...
		artech_engine.msgprint(_("Email Sent to Supplier {0}").format(data.supplier))
	...
	def update_rfq_supplier_status(self, sup_name=None):
		for supplier in self.suppliers:
			if sup_name is None or supplier.supplier == sup_name:
				...
				for item in self.items:
					sqi_count = artech_engine.db.sql(
						"""
						SELECT
...
def send_supplier_emails(rfq_name: str):
	...
	rfq = artech_engine.get_doc("Request for Quotation", rfq_name)
	if rfq.docstatus == 1:
		rfq.send_to_supplier()


def check_portal_enabled(reference_doctype):
	if not artech_engine.db.get_value("Portal Menu Item", {"reference_doctype": reference_doctype}, "enabled"):
		artech_engine.throw(
			_(
...
def make_supplier_quotation_from_rfq(
	...
	def postprocess(source, target_doc):
		if for_supplier:
			...
			target_doc.buying_price_list = args.buying_price_list or artech_engine.db.get_single_value(
...
def create_supplier_quotation(doc: str | Document | dict):
	...
	try:
		sq_doc = artech_engine.get_doc(
			{
				...
				or artech_engine.db.get_single_value("Buying Settings", "buying_price_list"),
			}
		)
		...
		sq_doc.save()
		artech_engine.msgprint(_("Supplier Quotation {0} Created").format(sq_doc.name))
		return sq_doc.name
	except Exception:
		return None
	...
	args.update(
		{
			...
			"supplier_part_no": artech_engine.db.get_value(
...
def get_pdf(
	...
	doc = artech_engine.get_doc("Request for Quotation", name)
	if supplier:
...
def get_item_from_material_requests_based_on_supplier(
	...
	mr_items_list = artech_engine.db.sql(
		"""
		SELECT
...
def get_supplier_tag():
	...
	tags = list(set(tag.tag for tag in artech_engine.get_all("Tag Link", filters=filters, fields=["tag"]) if tag))
...
def get_rfq_containing_supplier(
	...
	rfq = artech_engine.qb.DocType("Request for Quotation")
	rfq_supplier = artech_engine.qb.DocType("Request for Quotation Supplier")

	query = (
		artech_engine.qb.from_(rfq)
		.from_(rfq_supplier)
		.select(rfq.name)
		.distinct()
