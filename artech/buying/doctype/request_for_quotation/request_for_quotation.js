...
artech_engine.ui.form.on("Request for Quotation", {
	setup: function (frm) {
	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					artech_engine.call({
						method: "artech.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
					...
					artech_engine.prompt(
						[
							...
							},
							{
								...
								default: artech_engine.get_meta("Request for Quotation").default_print_format || "",
								get_query: () => {
									return {
										filters: {
							...
							},
							{
								...
								fieldname: "language",
								options: "Language",
								default: artech_engine.boot.lang,
							},
							{
								fieldtype: "Link",
								label: "Letter Head",
						...
						(data) => {
							var w = window.open(
								artech_engine.urllib.get_full_url(
									"/api/method/artech.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
											supplier: data.supplier,
											print_format: data.print_format || "Standard",
											language: data.language || artech_engine.boot.lang,
	...
	show_supplier_quotation_comparison(frm) {
		...
		artech_engine.set_route("query-report", "Supplier Quotation Comparison");
	},
		...
		var dialog = new artech_engine.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return artech_engine.call({
					type: "GET",
					method: "artech.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
artech_engine.ui.form.on("Request for Quotation Item", {
	items_add(frm, cdt, cdn) {
		if (frm.doc.schedule_date) {
			artech_engine.model.set_value(cdt, cdn, "schedule_date", frm.doc.schedule_date);
		}
	},
});
artech_engine.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		artech_engine.call({
			method: "artech.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
			callback: function (r) {
				if (r.message) {
					artech_engine.model.set_value(cdt, cdn, "contact", r.message.contact_person);
					artech_engine.model.set_value(cdt, cdn, "email_id", r.message.contact_email);
				}
			},
		});
	},
...
artech.buying.RequestforQuotationController = class RequestforQuotationController extends (
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
				...
				function () {
					...
					var dialog = new artech_engine.ui.Dialog({
						...
						primary_action: (args) => {
							...
							artech.utils.map_current_doc({
								method: "artech.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(
				...
				function () {
					artech.buying.link_to_mrs(me.frm);
				},
				__("Tools")
			);
		...
		var dialog = new artech_engine.ui.Dialog({
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							artech_engine
								.call({
									method: "artech.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
			...
			primary_action: (args) => {
				...
				if (args.search_type === "Tag" && args.tag) {
					return artech_engine.call({
						type: "GET",
						method: "artech_engine.desk.doctype.tag.tag.get_tagged_docs",
						args: {
							doctype: "Supplier",
							tag: "%" + args.tag + "%",
						},
...
extend_cscript(cur_frm.cscript, new artech.buying.RequestforQuotationController({ frm: cur_frm }));
