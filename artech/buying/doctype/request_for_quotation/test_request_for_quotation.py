...
from artech.buying.doctype.request_for_quotation.request_for_quotation import (
	create_supplier_quotation,
	get_pdf,
	make_supplier_quotation_from_rfq,
)
from artech.controllers.accounts_controller import InvalidQtyError
from artech.crm.doctype.opportunity.opportunity import make_request_for_quotation as make_rfq
from artech.crm.doctype.opportunity.test_opportunity import make_opportunity
from artech.stock.doctype.item.test_item import make_item
from artech.templates.pages.rfq import check_supplier_has_docname_access
from artech.tests.utils import ArtechTestSuite


class TestRequestforQuotation(ArtechTestSuite):
	...
	def test_make_supplier_quotation_with_taxes(self):
		...
		tax_template = artech_engine.new_doc("Purchase Taxes and Charges Template")
		...
		tax_rule = artech_engine.new_doc("Tax Rule")
	...
	def test_make_supplier_quotation_with_special_characters(self):
		artech_engine.delete_doc_if_exists("Supplier", "_Test Supplier '1", force=1)
	...
	def test_make_supplier_quotation_from_portal(self):
		...
		supplier_quotation_doc = artech_engine.get_doc("Supplier Quotation", supplier_quotation_name)
	...
	def test_make_multi_uom_supplier_quotation(self):
		...
		supplier_quotation_name = create_supplier_quotation(rfq)
		supplier_quotation = artech_engine.get_doc("Supplier Quotation", supplier_quotation_name)
	...
	def test_get_pdf(self):
		...
		self.assertEqual(artech_engine.local.response.type, "pdf")

	def test_portal_user_with_new_supplier(self):
		supplier_doc = artech_engine.get_doc(
			{
				"doctype": "Supplier",
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"allow_zero_qty_in_request_for_quotation": 1})
	...
	def test_supplier_quotation_from_zero_qty_rfq_in_portal(self):
		...
		sq = artech_engine.get_doc("Supplier Quotation", sq_name)
		self.assertEqual(len(sq.items), 1)
		self.assertEqual(sq.items[0].qty, 0)
		self.assertEqual(sq.items[0].item_code, rfq.items[0].item_code)
...
def make_request_for_quotation(**args):
	...
	rfq = artech_engine.new_doc("Request for Quotation")
