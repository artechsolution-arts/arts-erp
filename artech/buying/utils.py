...
from artech_engine.utils import cint, cstr, flt, getdate

from artech.stock.doctype.item.item import get_last_purchase_details, validate_end_of_life
	...
	for d in doc.get("items"):
		...
		if last_purchase_details and (
			...
			# Conversion factor should not be mandatory for non itemized items
				...
				artech_engine.throw(_("UOM Conversion factor is required in row {0}").format(d.idx))

		# update last purchsae rate
		artech_engine.db.set_value("Item", d.item_code, "last_purchase_rate", flt(last_purchase_rate))
...
def validate_for_items(doc) -> None:
	...
	if (
		...
		and not cint(artech_engine.db.get_single_value("Buying Settings", "allow_multiple_items") or 0)
	):
		artech_engine.throw(_("Same item cannot be entered multiple times."))
...
def validate_item_and_get_basic_data(row) -> dict:
	item = artech_engine.db.get_values(
		"Item",
	...
	if not item:
		artech_engine.throw(_("Row #{0}: Item {1} does not exist").format(row.idx, artech_engine.bold(row.item_code)))
	...
	if item.is_stock_item == 1 and row.qty and not row.warehouse and not row.get("delivered_by_supplier"):
		artech_engine.throw(
			_("Row #{1}: Warehouse is mandatory for stock Item {0}").format(
				artech_engine.bold(row.item_code), row.idx
			)
		)


def check_on_hold_or_closed_status(doctype, docname) -> None:
	status = artech_engine.db.get_value(doctype, docname, "status")

	if status in ("Closed", "On Hold"):
		artech_engine.throw(_("{0} {1} status is {2}").format(doctype, docname, status), artech_engine.InvalidStatusError)
...
def get_linked_material_requests(items: str):
	...
	mr_list = []
	for item in items:
		material_request = artech_engine.db.sql(
