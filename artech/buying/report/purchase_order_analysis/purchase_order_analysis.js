...
artech_engine.query_reports["Purchase Order Analysis"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			get_data: function (txt) {
				...
				const from_date = artech_engine.query_report.get_filter_value("from_date");
				const to_date = artech_engine.query_report.get_filter_value("to_date");
				if (from_date && to_date) filters["transaction_date"] = ["between", [from_date, to_date]];

				return artech_engine.db.get_link_options("Purchase Order", txt, filters);
			},
		},
		{
			fieldname: "status",
