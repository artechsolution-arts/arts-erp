...
def get_data(filters):
	sq = artech_engine.qb.DocType("Supplier Quotation")
	sq_item = artech_engine.qb.DocType("Supplier Quotation Item")

	query = (
		artech_engine.qb.from_(sq_item)
		.from_(sq)
		.select(
			sq_item.parent,
			sq_item.item_code,
...
def prepare_data(supplier_quotation_data, filters):
	...
	float_precision = cint(artech_engine.db.get_default("float_precision")) or 2
...
def prepare_chart_data(suppliers, qty_list, supplier_qty_price_map):
	...
	dataset = []
	currency_symbol = artech_engine.db.get_value("Currency", artech_engine.db.get_default("currency"), "symbol")
	for qty in qty_list:
		datapoints = {
...
def get_columns(filters):
	currency = artech_engine.get_cached_value("Company", filters.get("company"), "default_currency")
