...
from artech_engine.utils import comma_and, flt, get_link_to_form
...
class SubcontractingInwardOrder(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from artech.subcontracting.doctype.subcontracting_inward_order_item.subcontracting_inward_order_item import (
			SubcontractingInwardOrderItem,
		)
		from artech.subcontracting.doctype.subcontracting_inward_order_received_item.subcontracting_inward_order_received_item import (
			SubcontractingInwardOrderReceivedItem,
		)
		from artech.subcontracting.doctype.subcontracting_inward_order_secondary_item.subcontracting_inward_order_secondary_item import (
			SubcontractingInwardOrderSecondaryItem,
		)
		from artech.subcontracting.doctype.subcontracting_inward_order_service_item.subcontracting_inward_order_service_item import (
	...
	def update_subcontracted_quantity_in_so(self):
		for service_item in self.service_items:
			doc = artech_engine.get_doc("Sales Order Item", service_item.sales_order_item)
	...
	def validate_customer_warehouse(self):
		if artech_engine.get_cached_value("Warehouse", self.customer_warehouse, "customer") != self.customer:
			artech_engine.throw(
				_("Customer Warehouse {0} does not belong to Customer {1}.").format(
					artech_engine.bold(self.customer_warehouse), artech_engine.bold(self.customer)
				)
			)
	...
	def populate_items_table(self):
		...
		for si in self.service_items:
			if si.fg_item:
				...
				so_item = artech_engine.get_doc("Sales Order Item", si.sales_order_item)
				...
				si.fg_item_qty = flt(
					available_qty / conversion_factor, artech_engine.get_precision("Sales Order Item", "qty")
				)
				...
				items.append(
					{
						...
						"expected_delivery_date": artech_engine.get_cached_value(
	...
	def set_is_customer_provided_item(self):
		for item in self.get("received_items"):
			item.is_customer_provided_item = artech_engine.get_cached_value(
	...
	def get_production_items(self):
		...
		for d in self.items:
			...
			item_details = {
				...
				"project": artech_engine.get_cached_value("Sales Order", self.sales_order, "project"),
			...
			qty = min(
				int(qty) if artech_engine.get_cached_value("UOM", d.stock_uom, "must_be_whole_number") else qty,
	...
	def make_rm_stock_entry_inward(self, target_doc: Document | str | None = None):
		def calculate_qty_as_per_bom(rm_item):
			...
			stock_qty = artech_engine.get_value(
			...
			qty = flt(
				...
				artech_engine.get_precision("Subcontracting Inward Order Received Item", "required_qty"),
			)
	...
	def make_subcontracting_delivery(self, target_doc: Document | str | None = None):
		...
		allow_over = artech_engine.get_single_value("Selling Settings", "allow_delivery_of_overproduced_qty")
		...
		if (
			artech_engine.get_single_value("Selling Settings", "deliver_secondary_items")
...
def update_subcontracting_inward_order_status(scio: str | Document, status: str | None = None):
	if isinstance(scio, str):
		scio = artech_engine.get_doc("Subcontracting Inward Order", scio)
