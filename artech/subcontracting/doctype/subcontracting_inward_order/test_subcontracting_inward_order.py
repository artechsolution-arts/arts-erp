...
from artech.manufacturing.doctype.work_order.work_order import make_stock_entry as make_stock_entry_from_wo
from artech.selling.doctype.sales_order.sales_order import make_subcontracting_inward_order
...
class IntegrationTestSubcontractingInwardOrder(ArtechTestSuite):
	"""
	...
	def test_customer_provided_item_cost_field(self):
		...
		rm_in = artech_engine.new_doc("Stock Entry").update(scio.make_rm_stock_entry_inward())
		rm_in.save()
	...
	def test_customer_provided_item_cost_with_multiple_receipts(self):
		...
		rm_in_1 = artech_engine.new_doc("Stock Entry").update(scio.make_rm_stock_entry_inward())
		...
		rm_in_2 = artech_engine.new_doc("Stock Entry").update(scio.make_rm_stock_entry_inward())
	...
	def test_add_extra_item_during_manufacture(self):
		...
		so, scio = create_so_scio()
		artech_engine.new_doc("Stock Entry").update(scio.make_rm_stock_entry_inward()).submit()

		scio.reload()
		wo = artech_engine.get_doc("Work Order", scio.make_work_order()[0])
		wo.skip_transfer = 1
		next(
		...
		manufacture = artech_engine.new_doc("Stock Entry").update(make_stock_entry_from_wo(wo.name, "Manufacture"))
		manufacture.save()
	...
	def test_work_order_creation_qty(self):
		new_bom = artech_engine.copy_doc(artech_engine.get_doc("BOM", "BOM-Basic FG Item-001"))
		new_bom.items = new_bom.items[:3]
		new_bom.items[1].qty = 2
		new_bom.items[2].qty = 3
		new_bom.submit()
		sc_bom = artech_engine.get_doc("Subcontracting BOM", "SB-0001")
	...
	def test_rm_return(self):
		...
		scio.reload()
		rm_return = artech_engine.new_doc("Stock Entry").update(scio.make_rm_return())
		rm_return.submit()
	...
	def test_subcontracting_delivery(self):
		...
		scio.reload()
		delivery = artech_engine.new_doc("Stock Entry").update(scio.make_subcontracting_delivery())
		...
		self.assertRaises(artech_engine.ValidationError, delivery.submit)
	...
	def test_fg_item_fields(self):
		...
		scio.reload()
		fg_return = artech_engine.new_doc("Stock Entry").update(scio.make_subcontracting_return())
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"allow_delivery_of_overproduced_qty": 1})
	...
	def test_over_production_delivery(self):
		...
		artech_engine.db.set_single_value("Selling Settings", "allow_delivery_of_overproduced_qty", 0)
		delivery.cancel()
		scio.reload()
		delivery = artech_engine.new_doc("Stock Entry").update(scio.make_subcontracting_delivery())
	...
	@ArtechTestSuite.change_settings("Selling Settings", {"deliver_secondary_items": 1})
	def test_secondary_items_delivery(self):
		new_bom = artech_engine.copy_doc(artech_engine.get_doc("BOM", "BOM-Basic FG Item-001"))
		new_bom.secondary_items.append(
			artech_engine.new_doc("BOM Secondary Item", item_code="Basic RM 2", qty=1, type="Scrap")
		)
		new_bom.submit()
		sc_bom = artech_engine.get_doc("Subcontracting BOM", "SB-0001")
		sc_bom.finished_good_bom = new_bom.name
		...
		artech_engine.new_doc("Stock Entry").update(make_stock_entry_from_wo(wo.name, "Manufacture")).submit()
		...
		artech_engine.db.set_single_value("Selling Settings", "deliver_secondary_items", 0)
		delivery = artech_engine.new_doc("Stock Entry").update(scio.make_subcontracting_delivery())
	...
	def test_self_rm_billed_qty(self):
		...
		artech_engine.new_doc("Stock Entry").update(make_stock_entry_from_wo(wo.name, "Manufacture")).submit()
		scio.reload()
		artech_engine.new_doc("Stock Entry").update(scio.make_subcontracting_delivery()).submit()
		scio.reload()
...
def make_service_items():
	from artech.controllers.tests.test_subcontracting_controller import make_service_item
...
def make_bom_for_subcontracted_items():
	...
	for item_code, raw_materials in boms.items():
		if not artech_engine.db.exists("BOM", {"item": item_code}):
			make_bom(
...
def make_subcontracting_boms():
	...
	for subcontracting_bom in subcontracting_boms:
		if not artech_engine.db.exists("Subcontracting BOM", {"finished_good": subcontracting_bom["finished_good"]}):
