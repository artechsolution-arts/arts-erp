...
class SubcontractingBOM(Document):
	...
	def validate_finished_good(self):
		disabled, is_stock_item, default_bom, is_sub_contracted_item = artech_engine.db.get_value(
			"Item",
			self.finished_good,
		...
		if disabled:
			artech_engine.throw(_("Finished Good {0} is disabled.").format(artech_engine.bold(self.finished_good)))
		if not is_stock_item:
			artech_engine.throw(_("Finished Good {0} must be a stock item.").format(artech_engine.bold(self.finished_good)))
		if not default_bom:
			artech_engine.throw(
				_("Finished Good {0} does not have a default BOM.").format(artech_engine.bold(self.finished_good))
			)
		if not is_sub_contracted_item:
			artech_engine.throw(
				_("Finished Good {0} must be a sub-contracted item.").format(artech_engine.bold(self.finished_good))
			)

	def validate_service_item(self):
		disabled, is_stock_item = artech_engine.db.get_value(
			"Item", self.service_item, ["disabled", "is_stock_item"]
		)

		if disabled:
			artech_engine.throw(_("Service Item {0} is disabled.").format(artech_engine.bold(self.service_item)))
		if is_stock_item:
			artech_engine.throw(
				_("Service Item {0} must be a non-stock item.").format(artech_engine.bold(self.service_item))
			)

	def validate_is_active(self):
		if self.is_active:
			if sb := artech_engine.db.exists(
				"Subcontracting BOM",
				...
				artech_engine.throw(
					_("There is already an active Subcontracting BOM {0} for the Finished Good {1}.").format(
						artech_engine.bold(sb), artech_engine.bold(self.finished_good)
					)
				)
...
def get_subcontracting_boms_for_finished_goods(fg_items: str | list):
	if fg_items:
		...
		if subcontracting_boms := artech_engine.get_all("Subcontracting BOM", filters=filters, fields=["*"]):
			if isinstance(fg_items, list):
...
def get_subcontracting_boms_for_service_item(service_item: str) -> dict:
	if service_item:
		...
		Subcontracting_boms = artech_engine.db.get_all("Subcontracting BOM", filters=filters, fields=["*"])
