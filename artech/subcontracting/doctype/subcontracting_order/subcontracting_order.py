...
class SubcontractingOrder(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from artech.subcontracting.doctype.subcontracting_order_item.subcontracting_order_item import (
			SubcontractingOrderItem,
		)
		from artech.subcontracting.doctype.subcontracting_order_service_item.subcontracting_order_service_item import (
			SubcontractingOrderServiceItem,
		)
		from artech.subcontracting.doctype.subcontracting_order_supplied_item.subcontracting_order_supplied_item import (
	...
	def onload(self):
		self.set_onload(
			...
			artech_engine.db.get_single_value("Buying Settings", "over_transfer_allowance"),
		)
		self.set_onload(
		...
		self.set_onload(
			"backflush_based_on",
			artech_engine.get_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on"),
		)
	...
	def validate_purchase_order_for_subcontracting(self):
		if self.purchase_order:
			po = artech_engine.get_doc("Purchase Order", self.purchase_order)

			if not po.is_subcontracted:
				artech_engine.throw(_("Please select a valid Purchase Order that is configured for Subcontracting."))

			if po.is_old_subcontracting_flow:
				artech_engine.throw(_("Please select a valid Purchase Order that has Service Items."))
			...
			artech_engine.throw(_("Please select a Subcontracting Purchase Order."))
	...
	def validate_service_items(self):
		...
		for service_item in self.service_items:
			if artech_engine.get_value("Item", service_item.item_code, "is_stock_item"):
				artech_engine.throw(_("Service Item {0} must be a non-stock item.").format(service_item.item_code))
	...
	def populate_items_table(self):
		...
		for si in self.service_items:
			if si.fg_item:
				item = artech_engine.get_doc("Item", si.fg_item)

				qty, subcontracted_qty, fg_item_qty, production_plan_sub_assembly_item = artech_engine.db.get_value(
					"Purchase Order Item",
				...
				si.fg_item_qty = flt(
					available_qty / conversion_factor, artech_engine.get_precision("Purchase Order Item", "qty")
				)
		...
		if self.reserve_stock:
			...
			for item in items or self.supplied_items:
				...
				if stock_entry:
					...
					if production_plan_sub_assembly_item := fg_item.production_plan_sub_assembly_item:
						from_voucher_detail_no, reserved_qty = artech_engine.get_value(
			...
			if is_transfer:
				...
				if sre.make_stock_reservation_entries():
					artech_engine.msgprint(_("Stock Reservation Entries created"), alert=True, indicator="blue")
...
def make_subcontracting_receipt(source_name: str, target_doc: Document | str | None = None):
	items = artech_engine.flags.args.get("items") if artech_engine.flags.args else None
...
def get_mapped_subcontracting_receipt(source_name, target_doc=None, items=None):
	def update_item(source, target, source_parent):
		...
		if process_loss_per := artech_engine.get_value("BOM", source.bom, "process_loss_percentage"):
...
def update_subcontracting_order_status(sco: str | Document, status: str | None = None):
	if isinstance(sco, str):
		sco = artech_engine.get_doc("Subcontracting Order", sco)
