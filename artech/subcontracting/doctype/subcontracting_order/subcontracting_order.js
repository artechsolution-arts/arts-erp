...
artech.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
artech_engine.ui.form.on("Subcontracting Order Item", {
	qty(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		artech_engine.model.set_value(cdt, cdn, "amount", row.qty * row.rate);
		...
		artech_engine.model.set_value(service_item.doctype, service_item.name, "fg_item_qty", row.qty);
		artech_engine.model.set_value(
			service_item.doctype,
			service_item.name,
			"amount",
...
artech_engine.ui.form.on("Subcontracting Order", {
	setup: (frm) => {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", artech.queries.address_query);

		frm.set_query("billing_address", artech.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return artech.queries.company_address_query(frm.doc);
		});
	},
	...
	refresh: function (frm) {
		artech_engine.dynamic_link = { doc: frm.doc, fieldname: "supplier", doctype: "Supplier" };
	...
	update_subcontracting_order_status(frm, status) {
		artech_engine.call({
			method: "artech.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
			...
			const dialog = new artech_engine.ui.Dialog({
				...
				primary_action: () => {
					...
					if (values.some((i) => !i.qty || i.qty == 0)) {
						artech_engine.throw(__("Quantity is mandatory for the selected items."));
					} else {
...
artech.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt(items) {
		artech_engine.model.open_mapped_doc({
			method: "artech.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
			args: { items: items || [] },
			freeze: true,
...
extend_cscript(cur_frm.cscript, new artech.buying.SubcontractingOrderController({ frm: cur_frm }));
