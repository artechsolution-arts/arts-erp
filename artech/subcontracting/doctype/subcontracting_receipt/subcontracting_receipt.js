...
artech.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Receipt");

artech_engine.ui.form.on("Subcontracting Receipt", {
	setup: (frm) => {
				...
				frm.add_custom_button(
					...
					() => {
						artech_engine.model.open_mapped_doc({
							method: "artech.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_purchase_receipt",
							frm: frm,
							freeze: true,
			...
			frm.add_custom_button(
				...
				() => {
					const make_standard_return = () => {
						artech_engine.model.open_mapped_doc({
							method: "artech.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return",
							frm: frm,
						});
					};
						...
						artech_engine.prompt(
							...
							function (values) {
								if (values.return_for_rejected_warehouse) {
									artech_engine.call({
										method: "artech.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return_against_rejected_warehouse",
										args: {
...
artech_engine.ui.form.on("Subcontracting Receipt Item", {
	item_code(frm) {
		set_missing_values(frm);
	},
...
artech_engine.ui.form.on("Subcontracting Receipt Supplied Item", {
	consumed_qty(frm) {
		set_missing_values(frm);
	},
