...
from artech.stock.get_item_details import get_default_cost_center, get_default_expense_account
from artech.stock.stock_ledger import get_valuation_rate


class BOMQuantityError(artech_engine.ValidationError):
	pass
...
class SubcontractingReceipt(SubcontractingController):
	...
	if TYPE_CHECKING:
		...
		from artech.subcontracting.doctype.subcontracting_receipt_item.subcontracting_receipt_item import (
			SubcontractingReceiptItem,
		)
		from artech.subcontracting.doctype.subcontracting_receipt_supplied_item.subcontracting_receipt_supplied_item import (
	...
	def validate(self):
		...
		if getdate(self.posting_date) > getdate(nowdate()):
			artech_engine.throw(_("Posting Date cannot be future date"))

		super().validate()

		if self.is_new() and self.get("_action") == "save" and not artech_engine.in_test:
	...
	def update_job_card(self):
		for row in self.get("items"):
			if row.job_card:
				doc = artech_engine.get_doc("Job Card", row.job_card)
	...
	def set_service_expense_account(self, default_expense_account):
		for row in self.get("items"):
			if not row.service_expense_account and row.purchase_order_item:
				service_item = artech_engine.db.get_value(
	...
	def get_manufactured_qty(self, job_card):
		table = artech_engine.qb.DocType("Subcontracting Receipt Item")
		query = (
	...
	def set_items_bom(self):
		if self.is_return:
			for item in self.items:
				if not item.bom:
					item.bom = artech_engine.db.get_value(
	...
	def get_secondary_items(self, recalculate_rate: bool | None = False):
		...
		for item in list(self.items):
			if item.bom:
				bom = artech_engine.get_doc("BOM", item.bom)
	...
	def set_available_qty_for_consumption(self):
		...
		sco_supplied_item = artech_engine.qb.DocType("Subcontracting Order Supplied Item")
		for item in self.get("items"):
			supplied_items = (
				artech_engine.qb.from_(sco_supplied_item)
				.select(
		...
		for item in self.get("items") or []:
			...
			if item.bom:
				...
				item.amount = (
					flt(item.received_qty)
					* flt(item.rate)
					* (artech_engine.get_value("BOM", item.bom, "cost_allocation_per") / 100)
				)
	...
	def validate_bom_required_qty(self):
		...
		) and not (artech_engine.db.get_single_value("Buying Settings", "validate_consumed_qty")):
			return
		...
		for row in self.items:
			...
			is_allow_alternative_item = artech_engine.db.get_value("BOM", row.bom, "allow_alternative_item")
			...
			for bom_item in self._get_materials_from_bom(
				...
				if diff < 0:
					msg = _(
						...
						artech_engine.bold(abs(diff)),
						artech_engine.bold(bom_item.stock_uom),
						artech_engine.bold(bom_item.rm_item_code),
					)
	...
	def update_status(self, status=None, update_modified=False):
		if not status:
			...
			if self.is_return:
				artech_engine.get_doc("Subcontracting Receipt", self.return_against).update_status(
	...
	def get_gl_entries(self, inventory_account_map=None):
		...
		if not artech.is_perpetual_inventory_enabled(self.company):
			return []
	...
	def make_item_gl_entries(self, gl_entries, inventory_account_map=None):
		...
		supplied_items_details = artech_engine._dict()
		for item in self.supplied_items:
	...
	def auto_create_purchase_receipt(self):
		if artech_engine.db.get_single_value("Buying Settings", "auto_create_purchase_receipt"):
...
def make_purchase_receipt(
	...
	if isinstance(source_name, str):
		source_doc = artech_engine.get_doc("Subcontracting Receipt", source_name)
	else:
		...
		target.update(
			{
				...
				"currency": artech_engine.get_cached_value("Company", target.company, "default_currency"),
			}
		)
...
def add_po_items_to_pr(scr_doc, target_doc):
	...
	for (item_code, po_name), fg_qty in fg_items.items():
		po_doc = artech_engine.get_doc("Purchase Order", po_name)
