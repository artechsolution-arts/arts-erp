...
from artech_engine.utils import add_days, cint, flt, nowtime, today
...
class TestSubcontractingReceipt(ArtechTestSuite):
	def setUp(self):
		make_subcontracted_items()
		make_raw_materials()
		make_service_items()
	...
	def test_subcontracting_over_receipt(self):
		...
		"""
		from artech.controllers.subcontracting_controller import (
		...
		ste = artech_engine.get_doc(make_subcontract_transfer_entry(sco.name, rm_items))
		...
		scr1.submit()
		self.assertRaises(artech_engine.ValidationError, scr2.submit)
	...
	def test_subcontracting_receipt_over_return(self):
		...
		args = artech_engine._dict(scr_name=scr1.name, qty=-15)
	...
	def test_subcontracting_receipt_gl_entry(self):
		...
		self.assertEqual(cint(artech.is_perpetual_inventory_enabled(scr.company)), 1)
	...
	def test_subcontracting_receipt_for_service_expense_account(self):
		...
		service_item_doc = artech_engine.get_doc("Item", "Subcontracted Service Item 10")
	...
	@ArtechTestSuite.change_settings("Stock Settings", {"use_serial_batch_fields": 0})
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"allow_multiple_items": True})
		...
		bundle_doc = make_serial_batch_bundle(
			{
				...
				"qty": -1,
				"batches": artech_engine._dict({rm_batch_no: 1}),
	...
	def test_subcontracting_receipt_valuation_for_fg_with_auto_created_serial_batch_bundle(self):
		...
		for row in scr.supplied_items:
			self.assertEqual(row.rate, 300.00)
			self.assertTrue(row.serial_and_batch_bundle)
			auto_created_serial_batch = artech_engine.db.get_value(
				"Stock Ledger Entry",
				{"voucher_no": scr.name, "voucher_detail_no": row.name},
		...
		valuation_rate = artech_engine.db.get_value(
			"Stock Ledger Entry",
	...
	def test_quality_inspection_for_subcontracting_receipt(self):
		from artech.stock.doctype.quality_inspection.test_quality_inspection import (
		...
		artech_engine.db.set_value("Item", fg_item, "inspection_required_before_purchase", 1)

		# ValidationError should be raised as Quality Inspection is not created/linked
		self.assertRaises(artech_engine.ValidationError, scr1.submit)
		...
		artech_engine.db.set_value("Item", fg_item, "inspection_required_before_purchase", 0)
		...
		batch_doc = artech_engine.get_doc(
			{
				"doctype": "Batch",
				"item": fg_item,
				"batch_id": artech_engine.generate_hash(length=10),
			}
		).insert(ignore_permissions=True)

		serial_batch_bundle = artech_engine.get_doc(
			{
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"auto_create_purchase_receipt": 1})
	def test_auto_create_purchase_receipt(self):
		...
		pr_details = artech_engine.get_all(
			"Purchase Receipt",
		...
		pr_qty = artech_engine.db.get_value("Purchase Receipt Item", {"parent": pr_details[0]["name"]}, "qty")
		...
		rm_items = [
			{
				...
				"warehouse": "_Test Warehouse - _TC",
				"rate": 100,
				"stock_uom": artech_engine.get_cached_value("Item", rm_item1, "stock_uom"),
				"use_serial_batch_fields": 1,
			},
		]
		se = artech_engine.get_doc(make_rm_stock_entry(sco.name, rm_items))
	...
	def test_bom_required_qty_validation_based_on_transfer(self):
		...
		set_backflush_based_on("Material Transferred for Subcontract")
		artech_engine.db.set_single_value("Buying Settings", "validate_consumed_qty", 1)
	...
	@ArtechTestSuite.change_settings("Buying Settings", {"over_transfer_allowance": 20})
	@ArtechTestSuite.change_settings("Stock Settings", {"over_delivery_receipt_allowance": 20})
	def test_over_receipt(self):
		...
		doc = artech_engine.get_doc(ste_dict)
		self.assertEqual(doc.items[0].qty, 0)
		doc.items[0].qty = 2
		doc.submit()

		artech_engine.flags["args"] = {"items": [{"name": sco.items[0].name, "qty": 2}]}
		scr = make_subcontracting_receipt(sco.name)
		self.assertEqual(scr.items[0].qty, 2)
		scr.submit()
		artech_engine.flags["args"].pop("items", None)
