...
from artech_engine.core.doctype.report.report import get_report_module_dotted_path
from artech_engine.custom.doctype.custom_field.custom_field import create_custom_fields
from artech_engine.tests.utils import load_test_records_for
from artech_engine.utils import now_datetime, today
...
def execute_script_report(
	...
	test_filters = []
	report_execute_fn = artech_engine.get_attr(get_report_module_dotted_path(module, report_name) + ".execute")
	report_filters = artech_engine._dict(default_filters).copy().update(filters)
...
def if_lending_app_not_installed(function):
	...
	def wrapper(*args, **kwargs):
		if "lending" not in artech_engine.get_installed_apps():
			return function(*args, **kwargs)
		return
...
class BootStrapTestData:
	...
	def make_presets(self):
		from artech_engine.desk.page.setup_wizard.install_fixtures import update_genders, update_salutations

		from artech.setup.setup_wizard.operations.install_fixtures import (
			add_uom_data,
			get_preset_records,
			get_sale_stages,
		)
		...
		for x in records:
			...
			if not artech_engine.db.exists(dt, dn):
				doc = artech_engine.get_doc(x)
				doc.insert()

		add_uom_data()
		# add sale stages
		for sales_stage in get_sale_stages():
			if not artech_engine.db.exists("Sales Stage", {"stage_name": sales_stage.get("stage_name")}):
				artech_engine.get_doc(sales_stage).insert()

		from artech.buying.doctype.supplier_scorecard.supplier_scorecard import (
		...
		for x in get_default_scorecard_variables():
			...
			if not artech_engine.db.exists("Supplier Scorecard Variable", {"name": x.get("variable_label")}):
				artech_engine.get_doc(x).insert()
		for x in get_default_scorecard_standing():
			x["doctype"] = "Supplier Scorecard Standing"
			if not artech_engine.db.exists("Supplier Scorecard Standing", {"name": x.get("standing_name")}):
				artech_engine.get_doc(x).insert()
	...
	def update_system_settings(self):
		system_settings = artech_engine.get_doc("System Settings")
	...
	def update_support_settings(self):
		support_settings = artech_engine.get_doc("Support Settings")
	...
	def update_stock_settings(self):
		...
		stock_settings.default_warehouse = artech_engine.db.get_value("Warehouse", {"warehouse_name": _("Stores")})
	...
	def make_records(self, key, records):
		...
		for x in records:
			...
			if not artech_engine.db.exists(doctype, filters):
				artech_engine.get_doc(x).insert()
	...
	def make_test_account(self):
		...
		for company, abbr in [
			...
			for account_name, parent_account, is_group, account_type, currency in records:
				if not artech_engine.db.exists("Account", {"account_name": account_name, "company": company}):
					...
					self.test_accounts.append(
						artech_engine.get_doc("Account", {"account_name": account_name, "company": company})
					)
	...
	def make_custom_doctype(self):
		if not artech_engine.db.exists("DocType", "Shelf"):
			artech_engine.get_doc(
				{
					"doctype": "DocType",
					"name": "Shelf",
		...
		if not artech_engine.db.exists("DocType", "Rack"):
			artech_engine.get_doc(
				{
					"doctype": "DocType",
					"name": "Rack",
		...
		if not artech_engine.db.exists("DocType", "Pallet"):
			artech_engine.get_doc(
				{
					"doctype": "DocType",
					"name": "Pallet",
		...
		if not artech_engine.db.exists("DocType", "Inv Site"):
			artech_engine.get_doc(
				{
					"doctype": "DocType",
					"name": "Inv Site",
			...
			if not artech_engine.db.exists("DocType", "Store"):
				artech_engine.get_doc(
					{
						"doctype": "DocType",
						"name": "Store",
			...
			if not artech_engine.db.exists("DocType", "Order Assignment"):
				artech_engine.get_doc(
					{
						"doctype": "DocType",
...
class ArtechTestSuite(unittest.TestCase):
	@classmethod
	...
	def set_user(self, user: str):
		try:
			old_user = artech_engine.session.user
			artech_engine.set_user(user)
			yield
		finally:
			artech_engine.set_user(old_user)


@ArtechTestSuite.registerAs(staticmethod)
@contextmanager
def change_settings(doctype, settings_dict=None, /, **settings) -> None:
	...
	settings = artech_engine.get_doc(doctype)
