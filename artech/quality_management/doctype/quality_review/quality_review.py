...
class QualityReview(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.quality_management.doctype.quality_review_objective.quality_review_objective import (
	...
	def validate(self):
		...
		if not self.reviews:
			for d in artech_engine.get_doc("Quality Goal", self.goal).objectives:
...
def review():
	day = artech_engine.utils.getdate().day
	weekday = artech_engine.utils.getdate().strftime("%A")
	month = artech_engine.utils.getdate().strftime("%B")

	for goal in artech_engine.get_list("Quality Goal", fields=["name", "frequency", "date", "weekday"]):
...
def create_review(goal):
	goal = artech_engine.get_doc("Quality Goal", goal)

	review = artech_engine.get_doc({"doctype": "Quality Review", "goal": goal.name, "date": artech_engine.utils.getdate()})
