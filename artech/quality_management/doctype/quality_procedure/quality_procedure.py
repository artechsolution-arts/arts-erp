...
class QualityProcedure(NestedSet):
	...
	if TYPE_CHECKING:
		...
		from artech.quality_management.doctype.quality_procedure_process.quality_procedure_process import (
	...
	def check_for_incorrect_child(self):
		for process in self.processes:
			if process.procedure:
				...
				parent_quality_procedure = artech_engine.db.get_value(
				...
				if parent_quality_procedure and parent_quality_procedure != self.name:
					artech_engine.throw(
						_("{0} already has a Parent Procedure {1}.").format(
							artech_engine.bold(process.procedure), artech_engine.bold(parent_quality_procedure)
						),
	...
	def add_child_to_parent(self):
		...
		if self.parent_quality_procedure:
			parent = artech_engine.get_doc("Quality Procedure", self.parent_quality_procedure)
		...
		if old_doc := self.get_doc_before_save():
			if old_parent := old_doc.parent_quality_procedure:
				if self.parent_quality_procedure != old_parent:
					parent = artech_engine.get_doc("Quality Procedure", old_parent)
...
def get_children(
	...
	if parent:
		parent_procedure = artech_engine.get_doc("Quality Procedure", parent)
		# return the list in order
		return [
			dict(
				value=d.procedure,
				expandable=artech_engine.db.get_value("Quality Procedure", d.procedure, "is_group"),
			)
...
def add_node():
	...
	return artech_engine.get_doc(args).insert()
