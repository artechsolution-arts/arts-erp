...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Artech app. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Artech version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
from artech_engine.deprecation_dumpster import Color, _deprecated, colorize


# we use Warning because DeprecationWarning has python default filters which would exclude them from showing
# see also artech_engine.__init__ enabling them when a dev_server
class ArtechDeprecationError(Warning):
...
class ArtechDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingArtechDeprecationWarning(ArtechDeprecationWarning):
...
warnings.simplefilter("error", ArtechDeprecationError)
warnings.simplefilter("ignore", PendingArtechDeprecationWarning)


class V15ArtechDeprecationWarning(ArtechDeprecationError):
	pass


class V16ArtechDeprecationWarning(ArtechDeprecationWarning):
	pass


class V17ArtechDeprecationWarning(PendingArtechDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}ArtechDeprecationWarning"
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: artech_engine.utils.make_esc  (fully qualified)
...
### Party starts here
@deprecated(
	"artech.controllers.taxes_and_totals.get_itemised_taxable_amount",
	"2024-11-07",
	"v17",
...
def taxes_and_totals_get_itemised_taxable_amount(items):
	...
	itemised_taxable_amount = artech_engine._dict()
	for item in items:
...
@deprecated(
	"artech.stock.get_pos_profile_item_details",
	"2024-11-19",
	"v16",
	"Use artech.stock.get_pos_profile_item_details_ with a flipped signature",
)
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from artech.stock.get_item_details import get_pos_profile_item_details_

	return get_pos_profile_item_details_(ctx, company, pos_profile=pos_profile, update_data=update_data)


@deprecated(
	"artech.stock.get_item_warehouse",
	"2024-11-19",
	"v16",
	"Use artech.stock.get_item_warehouse_ with a flipped signature",
)
