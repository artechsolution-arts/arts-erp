...
from artech_engine.utils import formatdate, get_link_to_form

from artech import get_region
from artech.accounts.report.item_wise_sales_register.item_wise_sales_register import get_tax_details_query
...
class VATAuditReport:
	...
	def validate_company_region(self):
		if self.filters.company and get_region(self.filters.company) != "South Africa":
			artech_engine.throw(
				...
				).format(artech_engine.bold(self.filters.company))
			)

	def get_sa_vat_accounts(self):
		self.sa_vat_accounts = artech_engine.get_all(
			"South Africa VAT Account", filters={"parent": self.filters.company}, pluck="account"
		)
		if not self.sa_vat_accounts and not artech_engine.in_test and not artech_engine.flags.in_migrate:
			link_to_settings = get_link_to_form(
				"South Africa VAT Settings", "", label="South Africa VAT Settings"
			)
			artech_engine.throw(_("Please set VAT Accounts in {0}").format(link_to_settings))

	def get_invoice_data(self, doctype):
		self.invoices = artech_engine._dict()
		invoice_doctype = artech_engine.qb.DocType(doctype)
		...
		query = (
			artech_engine.qb.from_(invoice_doctype)
			.select(
			...
			.orderby(invoice_doctype.posting_date, order=artech_engine.qb.desc)
		)
	...
	def get_invoice_items(self, doctype):
		self.invoice_items = artech_engine._dict()
		item_doctype = artech_engine.qb.DocType(doctype + " Item")
		self.invoice_items = artech_engine._dict(
	...
	def get_items_based_on_tax_rate(self, doctype):
		self.items_based_on_tax_rate = artech_engine._dict()
		self.tax_doctype = (
			"Purchase Taxes and Charges" if doctype == "Purchase Invoice" else "Sales Taxes and Charges"
		)

		taxes_and_charges = artech_engine.qb.DocType(self.tax_doctype)
		item_wise_tax = artech_engine.qb.DocType("Item Wise Tax Detail")
	...
	def get_data(self, doctype):
		...
		for rate, section in consolidated_data.items():
			rate = int(rate)
			label = artech_engine.bold(section_name + "- " + "Rate" + " " + str(rate) + "%")
			...
			total = {
				"posting_date": artech_engine.bold(_("Total")),
