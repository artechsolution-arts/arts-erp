...
from artech.regional.report.vat_audit_report.vat_audit_report import execute
from artech.tests.utils import ArtechTestSuite


class TestVATAuditReport(ArtechTestSuite):
	def setUp(self):
...
def make_company(company_name, abbr):
	...
	if not artech_engine.db.get_value("Cost Center", {"is_group": 0, "company": company.name}):
...
def set_sa_vat_accounts():
	if not artech_engine.db.exists("South Africa VAT Settings", "_Test Company SA VAT"):
...
def make_customer():
	if not artech_engine.db.exists("Customer", "_Test SA Customer"):
		artech_engine.get_doc(
			{
				"doctype": "Customer",
...
def make_supplier():
	if not artech_engine.db.exists("Supplier", "_Test SA Supplier"):
		artech_engine.get_doc(
			{
				"doctype": "Supplier",
