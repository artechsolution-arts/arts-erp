...
def append_vat_on_sales(data, filters):
	...
	append_data(
		...
		artech_engine.format((-1) * get_tourist_tax_return_total(filters), "Currency"),
		artech_engine.format((-1) * get_tourist_tax_return_tax(filters), "Currency"),
	)
	...
	append_data(
		...
		artech_engine.format(get_reverse_charge_total(filters), "Currency"),
		artech_engine.format(get_reverse_charge_tax(filters), "Currency"),
	)

	append_data(data, "4", _("Zero Rated"), artech_engine.format(get_zero_rated_total(filters), "Currency"), "-")

	append_data(data, "5", _("Exempt Supplies"), artech_engine.format(get_exempt_total(filters), "Currency"), "-")
...
def standard_rated_expenses_emiratewise(data, filters):
	...
	for emirate, amount, vat in total_emiratewise:
		amounts_by_emirate[emirate] = {
			...
			"raw_amount": amount,
			"raw_vat_amount": vat,
			"amount": artech_engine.format(amount, "Currency"),
			"vat_amount": artech_engine.format(vat, "Currency"),
		}
	...
	for no, emirate in enumerate(emirates, 97):
		if emirate in amounts_by_emirate:
			...
			append_data(
				...
				artech_engine.format(0, "Currency"),
				artech_engine.format(0, "Currency"),
			)
...
def append_vat_on_expenses(data, filters):
	...
	append_data(
		...
		artech_engine.format(get_standard_rated_expenses_total(filters), "Currency"),
		artech_engine.format(get_standard_rated_expenses_tax(filters), "Currency"),
	)
	append_data(
		data,
		"10",
		_("Supplies subject to the reverse charge provision"),
		artech_engine.format(get_reverse_charge_recoverable_total(filters), "Currency"),
		artech_engine.format(get_reverse_charge_recoverable_tax(filters), "Currency"),
	)
