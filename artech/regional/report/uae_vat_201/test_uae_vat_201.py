...
from artech.regional.report.uae_vat_201.uae_vat_201 import (
	get_exempt_total,
...
from artech.stock.doctype.warehouse.test_warehouse import get_warehouse_account
from artech.tests.utils import ArtechTestSuite


class TestUaeVat201(ArtechTestSuite):
	def setUp(self):
		set_vat_accounts()
...
def set_vat_accounts():
	if not artech_engine.db.exists("UAE VAT Settings", "_Test Company UAE VAT"):
		vat_accounts = artech_engine.get_all(
			"Account",
			fields=["name"],
...
def make_customer():
	if not artech_engine.db.exists("Customer", "_Test UAE Customer"):
...
def make_supplier():
	if not artech_engine.db.exists("Supplier", "_Test UAE Supplier"):
		artech_engine.get_doc(
			{
...
def create_warehouse(warehouse_name, properties=None, company=None):
	...
	if not artech_engine.db.exists("Warehouse", warehouse_id):
		warehouse = artech_engine.new_doc("Warehouse")
