...
class SouthAfricaVATSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.accounts.doctype.south_africa_vat_account.south_africa_vat_account import (
	...
	def validate_company_region(self):
		if self.company and get_region(self.company) != "South Africa":
			artech_engine.throw(_("Company {0} is not in South Africa.").format(artech_engine.bold(self.company)))
