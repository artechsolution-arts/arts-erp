...
from artech_engine.utils import flt, money_in_words, round_based_on_smallest_currency_fraction
...
def update_itemised_tax_data(doc):
	...
	meta = artech_engine.get_meta(doc.items[0].doctype)
	...
	def determine_if_export(doc):
		...
		company_country = artech_engine.get_cached_value("Company", doc.company, "country")
		customer_country = artech_engine.db.get_value("Address", doc.customer_address, "country")
	...
	for row in doc.items:
		...
		if not tax_rate or row.get("is_zero_rated"):
			row.is_zero_rated = is_export or artech_engine.get_cached_value("Item", row.item_code, "is_zero_rated")
...
def get_tax_accounts(company):
	...
	tax_accounts_dict = artech_engine._dict()
	tax_accounts_list = artech_engine.get_all("UAE VAT Account", filters={"parent": company}, fields=["Account"])

	if not tax_accounts_list and not artech_engine.in_test:
		artech_engine.throw(_('Please set Vat Accounts for Company: "{0}" in UAE VAT Settings').format(company))
...
def update_grand_total_for_rcm(doc, method):
	...
	country = artech_engine.get_cached_value("Company", doc.company, "country")
...
def update_totals(vat_tax, base_vat_tax, doc):
	...
	doc.base_in_words = money_in_words(doc.base_grand_total, artech.get_company_currency(doc.company))
