...
class TestUnitedStates(ArtechTestSuite):
	def test_irs_1099_custom_field(self):
		if not artech_engine.db.exists("Supplier", "_US 1099 Test Supplier"):
			make_irs_1099_supplier()
			supplier = artech_engine.get_doc("Supplier", "_US 1099 Test Supplier")
			self.assertEqual(supplier.irs_1099, 1)

	def test_irs_1099_report(self):
		make_irs_1099_supplier()
		make_payment_entry_to_irs_1099_supplier()
		filters = artech_engine._dict({"fiscal_year": "_Test Fiscal Year 2016", "company": "_Test Company 1"})
...
def make_irs_1099_supplier():
	doc = artech_engine.new_doc("Supplier")
