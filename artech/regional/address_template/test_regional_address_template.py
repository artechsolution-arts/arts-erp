...
from artech.regional.address_template.setup import get_address_templates, update_address_template
from artech.tests.utils import ArtechTestSuite


def ensure_country(country):
	if artech_engine.db.exists("Country", country):
		return artech_engine.get_doc("Country", country)
	else:
		c = artech_engine.get_doc({"doctype": "Country", "country_name": country})
		c.insert()
		return c


class TestRegionalAddressTemplate(ArtechTestSuite):
	...
	def test_create_address_template(self):
		...
		doc = artech_engine.get_doc("Address Template", country.name)
		self.assertEqual(doc.template, "TEST")

	def test_update_address_template(self):
		"""Update an existing Address Template."""
		country = ensure_country("Germany")
		if not artech_engine.db.exists("Address Template", country.name):
