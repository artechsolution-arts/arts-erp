...
def update_address_template(country, html, is_default=0):
	...
	if not artech_engine.db.exists("Country", country):
		artech_engine.log_error(f"Country {country} for regional Address Template does not exist.")
		return

	if artech_engine.db.exists("Address Template", country):
		artech_engine.db.set_value("Address Template", country, "template", html)
		artech_engine.db.set_value("Address Template", country, "is_default", is_default)
	else:
