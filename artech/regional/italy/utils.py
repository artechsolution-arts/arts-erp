...
from artech_engine.utils.file_manager import remove_file

from artech.controllers.taxes_and_totals import get_itemised_tax
...
def export_invoices(filters: str | None = None):
	artech_engine.has_permission("Sales Invoice", throw=True)
	...
	zip_filename = "{}-einvoices.zip".format(artech_engine.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	company = artech_engine.get_doc("Company", invoice.company)
	...
	company_address = artech_engine.get_doc("Address", invoice.company_address)
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended = get_unamended_name(
				artech_engine.get_doc("Sales Invoice", invoice.return_against)
			)
		else:
			invoice.type_of_document = "TD01"  # Sales Invoice (Fattura)

	# set customer information
	invoice.customer_data = artech_engine.get_doc("Customer", invoice.customer)
	customer_address = artech_engine.get_doc("Address", invoice.customer_address)
	invoice.customer_address_data = customer_address

	if invoice.shipping_address_name:
		invoice.shipping_address_data = artech_engine.get_doc("Address", invoice.shipping_address_name)
...
def download_zip(files, output_filename):
	...
	with zipfile.ZipFile(zip_stream, "w", zipfile.ZIP_DEFLATED) as zip_file:
		for file in files:
			file_path = artech_engine.utils.get_files_path(file.file_name, is_private=file.is_private)

			zip_file.write(file_path, arcname=file.file_name)

	artech_engine.local.response.filename = output_filename
	artech_engine.local.response.filecontent = zip_stream.getvalue()
	artech_engine.local.response.type = "download"
	zip_stream.close()


def get_invoice_summary(items, taxes, item_wise_tax_details):
	summary_data = artech_engine._dict()
...
def sales_invoice_validate(doc):
	...
	company_fiscal_regime = artech_engine.get_cached_value("Company", doc.company, "fiscal_regime")
	...
	doc.company_tax_id = artech_engine.get_cached_value("Company", doc.company, "tax_id")
	doc.company_fiscal_code = artech_engine.get_cached_value("Company", doc.company, "fiscal_code")
	...
	customer = artech_engine.get_doc("Customer", doc.customer)
	...
	if not doc.customer_address:
		artech_engine.throw(_("Please set the Customer Address"), title=_("E-Invoicing Information Missing"))
	else:
	...
	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = artech_engine.get_cached_value(
...
def sales_invoice_on_submit(doc, method):
	...
	for schedule in doc.payment_schedule:
		...
		elif not artech_engine.db.get_value("Mode of Payment", schedule.mode_of_payment, "mode_of_payment_code"):
			artech_engine.throw(
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	item_meta = artech_engine.get_meta("Sales Invoice Item")

	invoice_xml = artech_engine.render_template(
		"artech/regional/italy/e-invoice.xml",
...
def generate_single_invoice(docname: str):
	doc = artech_engine.get_doc("Sales Invoice", docname)
	artech_engine.has_permission("Sales Invoice", doc=doc, throw=True)
...
def get_e_invoice_attachments(invoices):
	...
	attachments = artech_engine.get_all(
		"File",
...
def validate_address(address_name):
	...
	data = artech_engine.get_cached_value("Address", address_name, fields, as_dict=1) or {}
...
def get_progressive_name_and_number(doc, replace=False):
	...
	progressive_name = artech_engine.model.naming.make_autoname(company_tax_id + "_.#####")
