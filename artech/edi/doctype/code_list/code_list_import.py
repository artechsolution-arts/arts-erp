...
from artech_engine.utils.file_manager import save_file
...
def import_genericode():
	try:
		...
		artech_engine.throw(_("The uploaded file does not match the selected Code List."))
...
def get_uploaded_genericode_file() -> tuple[bytes, str | None]:
	uploaded_data = artech_engine.local.uploaded_file
	file_name = artech_engine.local.uploaded_filename
	if uploaded_data and file_name:
		return uploaded_data, file_name

	file_url = artech_engine.local.uploaded_file_url
	if not file_url:
		raise artech_engine.ValidationError(_("No file uploaded or URL provided."))
...
def import_genericode_content(
	...
	if artech_engine.db.exists(doctype, docname):
		code_list = artech_engine.get_doc(doctype, docname)
		if code_list.name != name:
			raise CodeListSelectionMismatchError
	else:
		# Create a new Code List document with the extracted name
		code_list = artech_engine.new_doc(doctype)
		code_list.name = name
...
def process_genericode_import(
	...
	from artech.edi.doctype.common_code.common_code import import_genericode
