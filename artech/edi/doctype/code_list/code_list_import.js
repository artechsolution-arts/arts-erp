artech_engine.provide("artech.edi");

artech.edi.import_genericode = function (listview_or_form) {
	...
	new artech_engine.ui.FileUploader({
		method: "artech.edi.doctype.code_list.code_list_import.import_genericode",
		doctype: doctype,
		docname: docname,
...
function show_column_selection_dialog(context) {
	...
	let d = new artech_engine.ui.Dialog({
		...
		primary_action(values) {
			...
			artech_engine
				.xcall("artech.edi.doctype.code_list.code_list_import.process_genericode_import", {
				...
				.then((count) => {
					artech_engine.msgprint(__("Import completed. {0} common codes created.", [count]));
				});
			d.hide();
		},
	});
