...
from artech_engine.utils.data import get_link_to_form
from lxml import etree

from artech.edi.doctype.code_list.code_list_import import parse_genericode_content
...
class CommonCode(Document):
	...
	def validate_distinct_references(self):
		...
		for link in self.applies_to:
			existing_links = artech_engine.get_all(
				"Common Code",
				filters=[
					["name", "!=", self.name],
...
def import_genericode(code_list: str, file_name: str, column_map: dict, filters: dict | None = None):
	...
	for i, xml_element in enumerate(elements, start=1):
		common_code: CommonCode = artech_engine.new_doc("Common Code")
		common_code.code_list = code_list
		common_code.from_genericode(column_map, xml_element)
		common_code.save()
		artech_engine.publish_progress(i / total_elements * 100, title=_("Importing Common Codes"))

	return total_elements


def on_doctype_update():
	artech_engine.db.add_index("Common Code", ["code_list", "common_code"])
