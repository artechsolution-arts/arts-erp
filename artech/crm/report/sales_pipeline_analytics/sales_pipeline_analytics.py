...
from artech_engine.query_builder.custom import Month, MonthName, Quarter
...
class SalesPipelineAnalytics:
	...
	def validate_filters(self):
		if not self.filters.from_date:
			artech_engine.throw(_("From Date is mandatory"))

		if not self.filters.to_date:
			artech_engine.throw(_("To Date is mandatory"))
	...
	def get_fields(self):
		...
		opp = artech_engine.qb.DocType("Opportunity")
		...
		if self.filters.get("based_on") == "Number":
			self.query_result = (
				query.select(
					...
					artech_engine.query_builder.functions.Count("*").as_("count"),
					self.duration,
				)
	...
	def get_periodic_data(self):
		self.periodic_data = artech_engine._dict()
				...
				if self.filters.get("assigned_to"):
					for user in assigned_to:
						if self.filters.get("assigned_to") == user:
							value = user
							self.periodic_data.setdefault(value, artech_engine._dict()).setdefault(period, 0)
	...
	def get_default_currency(self):
		...
		return artech_engine.db.get_value("Company", company, ["default_currency"])

	def get_currency_rate(self, from_currency, to_currency):
		cacheobj = artech_engine.cache()
