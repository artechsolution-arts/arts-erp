...
from artech.crm.report.sales_pipeline_analytics.sales_pipeline_analytics import execute
from artech.tests.utils import ArtechTestSuite


class TestSalesPipelineAnalytics(ArtechTestSuite):
	def setUp(self):
		create_opportunity()
...
def create_opportunity():
	doc = artech_engine.db.exists({"doctype": "Opportunity", "party_name": "_Test NC"})
	if not doc:
		doc = artech_engine.new_doc("Opportunity")
		doc.opportunity_from = "Customer"
		customer_name = artech_engine.db.get_value("Customer", {"customer_name": "_Test NC"}, ["customer_name"])
