...
class Prospect(CRMNote):
	...
	if TYPE_CHECKING:
		...
		from artech.crm.doctype.prospect_lead.prospect_lead import ProspectLead
		from artech.crm.doctype.prospect_opportunity.prospect_opportunity import ProspectOpportunity
	...
	def link_with_lead_contact_and_address(self):
		for row in self.leads:
			links = artech_engine.get_all(
				"Dynamic Link",
				filters={"link_doctype": "Lead", "link_name": row.lead},
				fields=["parent", "parenttype"],
			)
			for link in links:
				linked_doc = artech_engine.get_doc(link["parenttype"], link["parent"])
				exists = False
...
def make_customer(source_name: str, target_doc: str | Document | None = None):
	def set_missing_values(source, target):
		...
		target.customer_group = source.customer_group or artech_engine.db.get_default("Customer Group")
