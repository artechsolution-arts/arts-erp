...
from artech_engine.utils import add_days, getdate, today
...
class EmailCampaign(Document):
	...
	def set_date(self):
		if getdate(self.start_date) < getdate(today()):
			artech_engine.throw(_("Start Date cannot be before the current date"))

		# set the end date as start date + max(send after days) in campaign schedule
		campaign = artech_engine.get_cached_doc("Campaign", self.campaign_name)
	...
	def validate_lead(self):
		lead_email_id = artech_engine.db.get_value("Lead", self.recipient, "email_id")
		if not lead_email_id:
			lead_name = artech_engine.db.get_value("Lead", self.recipient, "lead_name")
			artech_engine.throw(_("Please set an email id for the Lead {0}").format(lead_name))

	def validate_email_campaign_already_exists(self):
		email_campaign_exists = artech_engine.db.exists(
			"Email Campaign",
			{
...
def send_email_to_leads_or_contacts():
	...
	email_campaigns = artech_engine.get_all(
		"Email Campaign",
	...
	for email_campaign in email_campaigns:
		try:
			campaign = artech_engine.get_cached_doc("Campaign", email_campaign.campaign_name)
...
def send_mail(entry, email_campaign):
	...
	if campaign_for == "Email Group":
		recipient_list = artech_engine.get_all(
			"Email Group Member",
			filters={"email_group": recipient, "unsubscribed": 0},
			pluck="email",
		)
	else:
		email_id = artech_engine.db.get_value(campaign_for, recipient, "email_id")
		if not email_id:
			artech_engine.log_error(
				title=_("Email Campaign Error"),
	...
	email_template = artech_engine.get_cached_doc("Email Template", entry.get("email_template"))
	sender = artech_engine.db.get_value("User", sender_user, "email") if sender_user else None

	# Build context for template rendering
	if campaign_for != "Email Group":
		context = {"doc": artech_engine.get_doc(campaign_for, recipient)}
	else:
		# For email groups, use the email group document as context
		context = {"doc": artech_engine.get_doc("Email Group", recipient)}

	# Render template
	subject = artech_engine.render_template(email_template.get("subject"), context)
	content = artech_engine.render_template(email_template.response_, context)
	...
	try:
		...
		artech_engine.log_error(title="Email Campaign Failed.")
...
def unsubscribe_recipient(unsubscribe, method):
	...
	email_campaign = artech_engine.get_doc("Email Campaign", unsubscribe.reference_name)
	...
	if email_campaign.email_campaign_for == "Email Group":
		...
		# For Lead or Contact
		artech_engine.db.set_value("Email Campaign", email_campaign.name, "status", "Unsubscribed")
...
def set_email_campaign_status():
	...
	for name in email_campaigns:
		email_campaign = artech_engine.get_doc("Email Campaign", name)
