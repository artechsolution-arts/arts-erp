...
artech.LeadController = class LeadController extends artech_engine.ui.form.Controller {
	setup() {
	...
	refresh() {
		...
		if (!this.frm.is_new()) {
			artech_engine.contacts.render_address_and_contact(this.frm);
		} else {
			artech_engine.contacts.clear_address_and_contact(this.frm);
		}
	...
	add_lead_to_prospect(frm) {
		artech_engine.prompt(
			...
			function (data) {
				artech_engine.call({
					method: "artech.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
	...
	async make_opportunity() {
		...
		let existing_prospect = (
			await artech_engine.db.get_value(
				"Prospect Lead",
				{
					lead: frm.doc.name,
				},
		...
		if (fields.length) {
			const d = new artech_engine.ui.Dialog({
	...
	make_prospect() {
		const me = this;
		artech_engine.model.with_doctype("Prospect", function () {
			let prospect = artech_engine.model.get_new_doc("Prospect");
			...
			let leads_row = artech_engine.model.add_child(prospect, "leads");
			leads_row.lead = me.frm.doc.name;

			artech_engine.set_route("Form", "Prospect", prospect.name);
		});
	}
...
if (cur_frm) {
	extend_cscript(cur_frm.cscript, new artech.LeadController({ frm: cur_frm }));
}
