...
from artech_engine.utils import random_string, today

from artech.crm.doctype.lead.lead import make_opportunity
from artech.crm.utils import get_linked_prospect
from artech.tests.utils import ArtechTestSuite


class TestLead(ArtechTestSuite):
	def test_make_customer(self):
		from artech.crm.doctype.lead.lead import make_customer

		lead = artech_engine.db.get_all("Lead", {"lead_name": "_Test Lead"})[0].name

		artech_engine.delete_doc_if_exists("Customer", "_Test Lead")
	...
	def test_make_customer_from_organization(self):
		...
		lead = artech_engine.db.get_all("Lead", {"lead_name": "_Test Lead 1"})[0].name
		customer = make_customer(lead)
		self.assertEqual(customer.doctype, "Customer")
		self.assertEqual(customer.lead_name, lead)
	...
	def test_create_lead_and_unlinking_dynamic_links(self):
		...
		address_1 = artech_engine.get_doc(
			{
				"doctype": "Address",
				"address_type": "Billing",
				"address_line1": "Baner",
		...
		lead_doc.delete()
		address_1.reload()
		self.assertEqual(artech_engine.db.exists("Lead", lead_doc.name), None)
	...
	def test_opportunity_from_lead(self):
		...
		self.assertTrue(
			artech_engine.db.get_value("ToDo", {"reference_type": "Opportunity", "reference_name": opportunity.name})
		)
...
def create_event(subject, starts_on, reference_type, reference_name):
	event = artech_engine.new_doc("Event")
...
def create_todo(description, reference_type, reference_name):
	todo = artech_engine.new_doc("ToDo")
...
def make_lead(**args):
	...
	lead_doc = artech_engine.get_doc(
		{
			"doctype": "Lead",
