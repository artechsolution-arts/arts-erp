...
class AppointmentBookingSettings(Document):
	...
	if TYPE_CHECKING:
		from artech_engine.automation.doctype.assignment_rule_user.assignment_rule_user import AssignmentRuleUser
		from artech_engine.types import DF

		from artech.crm.doctype.appointment_booking_slots.appointment_booking_slots import (
	...
	def validate_from_and_to_time(self, from_time, to_time, record):
		if from_time > to_time:
			...
			artech_engine.throw(_(err_msg))

	def duration_is_divisible(self, from_time, to_time):
		timedelta = to_time - from_time
		if timedelta.total_seconds() % (self.appointment_duration * 60):
			artech_engine.throw(_("The difference between from time and To Time must be a multiple of Appointment"))
