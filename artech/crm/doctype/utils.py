...
def get_last_interaction(contact: str | None = None, lead: str | None = None):
	...
	if contact:
		...
		if query_condition:
			...
			last_communication = artech_engine.db.sql(
				f"""
				SELECT `name`, `content`
	...
	if lead:
		last_communication = artech_engine.get_all(
			"Communication",
...
def get_scheduled_employees_for_popup(communication_medium):
	...
	now_time = artech_engine.utils.nowtime()
	weekday = artech_engine.utils.get_weekday()

	available_employee_groups = artech_engine.get_all(
