...
artech_engine.provide("artech.crm");
artech.pre_sales.set_as_lost("Opportunity");
artech.sales_common.setup_selling_controller();

artech_engine.ui.form.on("Opportunity", {
	setup: function (frm) {
	...
	validate: function (frm) {
		if (frm.doc.status == "Lost" && !frm.doc.lost_reasons.length) {
			...
			artech_engine.throw(__("Lost Reasons are required in case opportunity is Lost."));
		}
	},
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			...
			artech.utils.map_current_doc({
				method: "artech.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		artech.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: artech.utils.get_contact_details,
	...
	set_contact_link: function (frm) {
		if (frm.doc.opportunity_from == "Customer" && frm.doc.party_name) {
			artech_engine.dynamic_link = { doc: frm.doc, fieldname: "party_name", doctype: "Customer" };
		} else if (frm.doc.opportunity_from == "Lead" && frm.doc.party_name) {
			artech_engine.dynamic_link = { doc: frm.doc, fieldname: "party_name", doctype: "Lead" };
		} else if (frm.doc.opportunity_from == "Prospect" && frm.doc.party_name) {
			artech_engine.dynamic_link = { doc: frm.doc, fieldname: "party_name", doctype: "Prospect" };
		}
	},
	...
	make_supplier_quotation: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
artech_engine.ui.form.on("Opportunity Item", {
	calculate: function (frm, cdt, cdn) {
		let row = artech_engine.get_doc(cdt, cdn);
		artech_engine.model.set_value(cdt, cdn, "amount", flt(row.qty) * flt(row.rate));
		artech_engine.model.set_value(cdt, cdn, "base_rate", flt(frm.doc.conversion_rate) * flt(row.rate));
		artech_engine.model.set_value(cdt, cdn, "base_amount", flt(frm.doc.conversion_rate) * flt(row.amount));
...
artech.crm.Opportunity = class Opportunity extends artech_engine.ui.form.Controller {
	onload() {
		if (!this.frm.doc.status) {
			this.frm.set_value("status", "Open");
		}
		if (!this.frm.doc.company && artech_engine.defaults.get_user_default("Company")) {
			this.frm.set_value("company", artech_engine.defaults.get_user_default("Company"));
		}
		if (!this.frm.doc.currency) {
			this.frm.set_value("currency", artech_engine.defaults.get_user_default("Currency"));
		}
	...
	setup_queries() {
		...
		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", artech.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", artech.queries["customer"]);
	...
	create_quotation() {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		artech_engine.model.open_mapped_doc({
			method: "artech.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}
...
extend_cscript(cur_frm.cscript, new artech.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return artech_engine.call({
			method: "artech.crm.doctype.opportunity.opportunity.get_item_details",
			args: { item_code: d.item_code },
			callback: function (r, rt) {
				if (r.message) {
					$.each(r.message, function (k, v) {
						artech_engine.model.set_value(cdt, cdn, k, v);
					});
