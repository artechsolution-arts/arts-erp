...
from artech_engine.query_builder.functions import Now
from artech_engine.utils import flt, get_fullname

from artech.crm.utils import (
	CRMNote,
	copy_comments,
	link_communications,
	link_open_events,
...
class Opportunity(TransactionBase, CRMNote):
	...
	if TYPE_CHECKING:
		...
		from artech.crm.doctype.opportunity_item.opportunity_item import OpportunityItem
		from artech.crm.doctype.opportunity_lost_reason_detail.opportunity_lost_reason_detail import (
	...
	def onload(self):
		ref_doc = artech_engine.get_doc(self.opportunity_from, self.party_name)
	...
	def after_insert(self):
		if self.opportunity_from == "Lead":
			...
			if artech_engine.db.get_single_value("CRM Settings", "carry_forward_communication_and_comments"):
	...
	def map_fields(self):
		for field in self.meta.get_valid_columns():
			if not self.get(field) and artech_engine.db.field_exists(self.opportunity_from, field):
				try:
					value = artech_engine.db.get_value(self.opportunity_from, self.party_name, field)
	...
	def update_prospect(self):
		...
		if self.opportunity_from == "Prospect" and self.party_name:
			...
			prospect_name = artech_engine.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")

		if prospect_name:
			prospect = artech_engine.get_doc("Prospect", prospect_name)
	...
	def make_new_lead_if_required(self):
		...
		if (not self.get("party_name")) and self.contact_email:
			...
			customer = (
				artech_engine.qb.from_(dynamic_link)
				.join(contact)
				.on(
			...
			lead_name = artech_engine.db.get_value("Lead", {"email_id": self.contact_email})
			if not lead_name:
	...
	def declare_enquiry_lost(
		...
		if not self.has_active_quotation():
			...
			artech_engine.throw(_("Cannot declare as lost, because Quotation has been made."))
	...
	def has_lost_quotation(self):
		lost_quotation = artech_engine.db.sql(
			"""
			select name
			from `tabQuotation`
			where docstatus=1
	...
	def validate_cust_name(self):
		if self.party_name:
			if self.opportunity_from == "Customer":
				...
				customer_name = artech_engine.db.get_value("Prospect Lead", {"lead": self.party_name}, "parent")
	...
	def validate_item_details(self):
		...
		for d in self.items:
			...
			item = artech_engine.db.get_value("Item", d.item_code, item_fields, as_dict=True)
...
def make_quotation(source_name: str, target_doc: str | Document | None = None):
	def set_missing_values(source, target):
		from artech.controllers.accounts_controller import get_default_taxes_and_charges

		quotation = artech_engine.get_doc(target)

		company_currency = artech_engine.get_cached_value("Company", quotation.company, "default_currency")
...
def set_multiple_status(names: str | list[str], status: str):
	...
	for name in names:
		opp = artech_engine.get_doc("Opportunity", name)
		opp.status = status
		opp.save()
...
def auto_close_opportunity():
	...
	auto_close_after_days = artech_engine.db.get_single_value("CRM Settings", "close_opportunity_after_days") or 15

	table = artech_engine.qb.DocType("Opportunity")
	opportunities = (
	...
	for opportunity in opportunities:
		doc = artech_engine.get_doc("Opportunity", opportunity)
		doc.status = "Closed"
...
def make_opportunity_from_communication(
	...
	from artech.crm.doctype.lead.lead import make_lead_from_communication
