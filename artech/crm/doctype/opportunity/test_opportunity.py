...
from artech_engine.utils import now_datetime, random_string, today

from artech.crm.doctype.lead.lead import make_customer
from artech.crm.doctype.lead.test_lead import make_lead
from artech.crm.doctype.opportunity.opportunity import make_quotation
from artech.crm.utils import get_linked_communication_list
from artech.tests.utils import ArtechTestSuite


class TestOpportunity(ArtechTestSuite):
	@classmethod
	def make_opportunities(cls):
		...
		cls.opportunities = []
		for x in records:
			if not artech_engine.db.exists("Opportunity", {"name": x.get("name")}):
				cls.opportunities.append(artech_engine.get_doc(x).insert())
			else:
				cls.opportunities.append(artech_engine.get_doc("Opportunity", {"party_name": x.get("party_name")}))
	...
	def test_opportunity_status(self):
		...
		doc = artech_engine.get_doc("Opportunity", doc.name)
	...
	def test_make_new_lead_if_required(self):
		...
		self.assertEqual(artech_engine.db.get_value("Lead", opp_doc.party_name, "email_id"), opp_doc.contact_email)
	...
	def test_carry_forward_of_email_and_comments(self):
		artech_engine.db.set_single_value("CRM Settings", "carry_forward_communication_and_comments", 1)
		lead_doc = make_lead()
		...
		opportunity_comment_count = artech_engine.db.count(
...
def make_opportunity_from_lead(company):
	...
	opp_doc = artech_engine.get_doc(args).insert(ignore_permissions=True)
