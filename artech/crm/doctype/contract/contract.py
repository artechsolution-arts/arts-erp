...
class Contract(Document):
	...
	if TYPE_CHECKING:
		...
		from artech.crm.doctype.contract_fulfilment_checklist.contract_fulfilment_checklist import (
	...
	def set_missing_values(self):
		if not self.party_full_name:
			...
			if res := artech_engine.db.get_value(self.party_type, self.party_name, field):
				self.party_full_name = res

	def before_submit(self):
		self.signed_by_company = artech_engine.session.user
	...
	def validate_dates(self):
		if self.end_date and self.end_date < self.start_date:
			artech_engine.throw(_("End Date cannot be before Start Date."))
...
def update_status_for_contracts():
	...
	contracts = artech_engine.get_all(
		"Contract",
	...
	for contract in contracts:
		...
		artech_engine.db.set_value("Contract", contract.get("name"), "status", status)
