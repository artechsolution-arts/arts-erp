...
from artech_engine.core.doctype.installed_applications.installed_applications import get_setup_wizard_completed_apps
...
def get_level(site_info):
	...
	for doctype, min_count in doctypes.items():
		count = artech_engine.db.count(doctype)
		if count > min_count:
			activation_level += 1
		sales_data.append({doctype: count})

	if "artech" in get_setup_wizard_completed_apps():
		activation_level += 1

	communication_number = artech_engine.db.count("Communication", dict(communication_medium="Email"))
	if communication_number > 10:
		activation_level += 1
	sales_data.append({"Communication": communication_number})

	# recent login
	if artech_engine.db.sql("select name from tabUser where last_login > date_sub(now(), interval 2 day) limit 1"):
		activation_level += 1
...
def get_help_messages():
	...
	domain = artech_engine.get_cached_value("Company", artech.get_default_company(), "domain")
	messages = []
	...
	for m in message_settings:
		if not m.domain or domain in m.domain:
			m.count = artech_engine.db.count(m.doctype)
			if m.count < m.target:
				messages.append(m)
