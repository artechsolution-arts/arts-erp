...
class Video(Document):
	...
	def set_youtube_statistics(self):
		api_key = artech_engine.db.get_single_value("Video Settings", "api_key")
...
def is_tracking_enabled():
	return artech_engine.db.get_single_value("Video Settings", "enable_youtube_tracking")
...
def update_youtube_data():
	...
	video_settings = artech_engine.get_cached_doc("Video Settings")
...
def get_id_from_url(url: str):
	...
	if not isinstance(url, str):
		artech_engine.throw(_("URL can only be a string"), title=_("Invalid URL"))
...
def batch_update_youtube_data():
	def get_youtube_statistics(video_ids):
		...
		try:
			...
			artech_engine.log_error("Unable to update YouTube statistics")
	...
	def set_youtube_data(entries):
		for entry in entries:
			...
			artech_engine.db.set_value("Video", video_id, stats)

	video_list = artech_engine.get_all("Video", fields=["youtube_video_id"])
