...
artech_engine.ui.form.on("Rename Tool", {
	refresh: function (frm) {
		...
		frm.page.set_primary_action(__("Rename"), function () {
			artech_engine.call({
				method: "artech.utilities.doctype.rename_tool.rename_tool.upload",
				args: {
		...
		artech_engine.db
			...
			.then((jobs) => {
				...
				for (const job of jobs) {
					if (job.job_name !== "artech_engine.model.rename_doc.bulk_rename") {
						continue;
					}
