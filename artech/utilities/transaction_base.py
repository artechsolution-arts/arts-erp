...
import artech_engine.share
from artech_engine import _
from artech_engine.utils import cint, flt, get_time, now_datetime
...
from artech.stock.get_item_details import NOT_APPLICABLE_TAX, get_item_details
from artech.stock.utils import get_incoming_rate


class UOMMustBeIntegerError(artech_engine.ValidationError):
	pass
...
class TransactionBase(StatusUpdater):
	def validate_posting_time(self):
		...
		if (artech_engine.flags.in_import or self.flags.from_restore) and self.posting_date:
		...
		if not getattr(self, "set_posting_time", None):
			...
			try:
				...
				artech_engine.throw(_("Invalid Posting Time"))
				...
				if ref_dn:
					if is_child:
						...
						if ref_dn not in item_ref_dn:
							...
							artech_engine.throw(_("Duplicate row {0} with same {1}").format(d.idx, key))
					elif ref_dn:
		...
		for reference_doctype, ref_dn_list in ref_doc.items():
			...
			for reference_name in ref_dn_list:
				...
				if not prevdoc_values:
					artech_engine.throw(_("Invalid reference {0} {1}").format(reference_doctype, reference_name))
			...
			for d in self.get("items"):
				if d.get(ref_link_field):
					...
					if abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
						if action == "Stop":
							if role_allowed_to_override not in artech_engine.get_roles():
								stop_actions.append(
	...
	def validate_currency_for_receivable_payable_and_advance_account(self):
		if self.doctype in ["Customer", "Supplier"]:
			...
			for x in self.accounts:
				company_default_currency = artech_engine.get_cached_value("Company", x.company, "default_currency")
				receivable_payable_account_currency = None
				advance_account_currency = None

				if x.account:
					receivable_payable_account_currency = artech_engine.get_cached_value(
						"Account", x.account, "account_currency"
					)

				if x.advance_account:
					advance_account_currency = artech_engine.get_cached_value(
				...
				if receivable_payable_account_currency and (
					...
					artech_engine.throw(
						_(
							...
							account_type,
							artech_engine.bold(x.account),
							artech_engine.bold(receivable_payable_account_currency),
							artech_engine.bold(self.default_currency),
							artech_engine.bold(company_default_currency),
						)
					)
				...
				if advance_account_currency and (
					...
					artech_engine.throw(
						_(
							...
							artech_engine.bold(x.advance_account),
							artech_engine.bold(self.default_currency),
							artech_engine.bold(company_default_currency),
						)
					)
	...
	def handle_internal_parties(self, item_obj: object, item_details: dict) -> None:
		fetch_valuation_rate_for_internal_transaction = cint(
			artech_engine.get_single_value("Accounts Settings", "fetch_valuation_rate_for_internal_transaction")
		)
		if (
	...
	def add_taxes_from_item_template(self, item_obj: object, item_details: dict) -> None:
		if item_details.item_tax_rate and artech_engine.get_single_value(
			"Accounts Settings", "add_taxes_from_item_tax_template"
		):
			item_tax_template = artech_engine.json.loads(item_details.item_tax_rate)
	...
	def conversion_factor(self, item_obj: object, item_details: dict) -> None:
		if artech_engine.get_meta(item_obj.doctype).has_field("stock_qty"):
			...
			if not artech_engine.flags.dont_fetch_price_list_rate and artech_engine.get_meta(self.doctype).has_field(
				"price_list_currency"
			):
	...
	def _apply_price_list(self, item_obj: object, reset_plc_conversion: bool) -> None:
		...
		from artech.stock.get_item_details import apply_price_list
...
def delete_events(ref_type, ref_name):
	...
	if events:
		artech_engine.delete_doc("Event", events, for_reload=True)
					...
					if abs(cint(qty) - flt(qty, precision)) > 0.0000001:
						artech_engine.throw(
							_(
								...
								artech_engine.bold(d.get(uom_field)),
							),
