...
from artech_engine.utils import comma_or, flt, get_link_to_form, getdate, now, nowdate, safe_div


class OverAllowanceError(artech_engine.ValidationError):
	pass


def validate_status(status, options):
	if status not in options:
		artech_engine.throw(_("Status must be one of {0}").format(comma_or(options)))
...
class StatusUpdater(Document):
			...
			if not s[1]:
				...
				if artech_engine.safe_eval(
					...
					None,
					{
						...
						"getdate": getdate,
						"nowdate": nowdate,
						"get_value": artech_engine.db.get_value,
					},
				):
					return {"status": s[0]}
	...
	def validate_qty(self):
		...
		for args in self.status_updater:
			...
			items_to_validate = []
			selling_negative_rate_allowed = artech_engine.get_single_value(
				"Selling Settings", "allow_negative_rates_for_items"
			)
			buying_negative_rate_allowed = artech_engine.get_single_value(
			...
			for d in self.get_all_children():
				if hasattr(d, "qty") and d.qty < 0 and not self.get("is_return"):
					artech_engine.throw(_("For an item {0}, quantity must be positive number").format(d.item_code))

				if hasattr(d, "qty") and d.qty > 0 and self.get("is_return"):
					artech_engine.throw(_("For an item {0}, quantity must be negative number").format(d.item_code))
					...
					if hasattr(d, "item_code") and hasattr(d, "rate") and flt(d.rate) < 0:
						artech_engine.throw(
							_(
								...
								artech_engine.bold(_("`Allow Negative rates for Items`")),
								get_link_to_form(
									"Selling Settings"
			...
			if items_to_validate:
				...
				if pp_sub_assembly_items:
					pp_subcontract_items = artech_engine.db.get_all(
	...
	def fetch_items_with_pending_qty(self, args, item_field, items):
		doctype = artech_engine.qb.DocType(args["target_dt"])
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = artech_engine.get_single_value(
			"Stock Settings", "role_allowed_to_over_deliver_receive"
		)
		role_allowed_to_over_bill = artech_engine.get_single_value("Accounts Settings", "role_allowed_to_over_bill")
		...
		if overflow_percent - allowance > 0.01:
			...
			if role not in artech_engine.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		artech_engine.throw(
			_(
				...
				artech_engine.bold(_(item["target_ref_field"].title())),
				artech_engine.bold(item["reduce_by"]),
				artech_engine.bold(_(args.get("target_dt"))),
				artech_engine.bold(_(self.doctype)),
				artech_engine.bold(item.get("item_code")),
			)
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if (
				...
				args["second_source_condition"] = artech_engine.db.sql(
	...
	def _calculate_target_parent_percentage(
		...
		child_records = artech_engine.get_all(
			target_dt,
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if update_data:
			target = artech_engine.get_lazy_doc(args["target_parent_dt"], args["name"])
	...
	def _update_modified(self, args, update_modified):
		...
		args["update_modified"] = ", modified = {}, modified_by = {}".format(
			artech_engine.db.escape(now()), artech_engine.db.escape(artech_engine.session.user)
		)

	def update_billing_status_for_zero_amount_refdoc(self, ref_dt):
		ref_fieldname = artech_engine.scrub(ref_dt)

		ref_docs = [item.get(ref_fieldname) for item in (self.get("items") or []) if item.get(ref_fieldname)]
		if not ref_docs:
			return

		zero_amount_refdocs = artech_engine.db.sql_list(
			f"""
			SELECT
				name
			from
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			...
			ref_doc = artech_engine.get_lazy_doc(ref_dt, ref_dn)
...
def get_allowance_for(
	...
	if item_allowance is None:
		item_allowance = {}
	if qty_or_amount == "qty":
		if item_allowance.get(item_code, artech_engine._dict()).get("qty"):
			return (
		...
		if item_allowance.get(item_code, artech_engine._dict()).get("amount"):
			return (
	...
	qty_allowance, over_billing_allowance = artech_engine.get_cached_value(
	...
	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance is None:
			global_qty_allowance = flt(
				artech_engine.get_cached_value("Stock Settings", None, "over_delivery_receipt_allowance")
			)
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance is None:
			global_amount_allowance = flt(
				artech_engine.get_cached_value("Accounts Settings", None, "over_billing_allowance")
			)
		over_billing_allowance = global_amount_allowance

	if qty_or_amount == "qty":
		allowance = qty_allowance
		item_allowance.setdefault(item_code, artech_engine._dict()).setdefault("qty", qty_allowance)
	else:
		allowance = over_billing_allowance
		item_allowance.setdefault(item_code, artech_engine._dict()).setdefault("amount", over_billing_allowance)
