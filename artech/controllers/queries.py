...
from artech_engine import qb, scrub
from artech_engine.desk.reportview import get_filters_cond, get_match_cond
from artech_engine.permissions import has_permission
from artech_engine.query_builder import Criterion, CustomFunction
from artech_engine.query_builder.functions import Concat, Locate, Sum
from artech_engine.utils import nowdate, today, unique
from pypika import Order

import artech
from artech.accounts.utils import build_qb_match_conditions
from artech.stock.get_item_details import ItemDetailsCtx, _get_item_tax_template
...
def employee_query(
	...
	if reference_doctype and ignore_user_permissions:
		ignore_permissions = has_ignored_field(reference_doctype, doctype) and has_permission(
			doctype,
			ptype="select" if artech_engine.only_has_select_perm(doctype) else "read",
		)
...
def has_ignored_field(reference_doctype, doctype):
	meta = artech_engine.get_meta(reference_doctype)
...
def lead_query(
	...
	searchfields = artech_engine.get_meta(doctype).get_search_fields()
...
def tax_account_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	doctype = "Account"
	company_currency = artech.get_company_currency(filters.get("company"))
...
def item_query(
	...
	meta = artech_engine.get_meta(doctype, cached=True)
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			item_rules_list = artech_engine.get_all(
				"Party Specific Item",
				filters={
					"party": ["!=", party],
	...
	description_cond = ""
	if artech_engine.db.estimate_count(doctype) < 50000:
...
def get_project_name(
	...
	searchfields = [
		x for x in artech_engine.get_meta(doctype).get_search_fields() if x not in ["customer", "status"]
	]
...
def get_delivery_notes_to_be_billed(
	...
	DeliveryNote = artech_engine.qb.DocType("Delivery Note")

	fields = get_fields(doctype, ["name", "customer", "posting_date"])

	original_dn = (
		artech_engine.qb.from_(DeliveryNote)
...
def get_batches_from_serial_and_batch_bundle(searchfields, txt, filters, start=0, page_len=100):
	bundle = artech_engine.qb.DocType("Serial and Batch Entry")
	stock_ledger_entry = artech_engine.qb.DocType("Stock Ledger Entry")
	batch_table = artech_engine.qb.DocType("Batch")
...
def get_account_list(
	...
	return artech_engine.desk.reportview.execute(
		doctype,
		filters=filter_list,
...
def warehouse_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: list):
	...
	meta = artech_engine.get_meta("Warehouse")
	...
	query = """select `tabWarehouse`.`{warehouse_field}`,
		...
		start=start,
		page_len=page_len,
		txt=artech_engine.db.escape(f"%{txt}%"),
	)

	return artech_engine.db.sql(query)
...
def get_batch_numbers(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	query = """select batch_id from `tabBatch`
			...
			and name like {txt}""".format(txt=artech_engine.db.escape(f"%{txt}%"))

	if filters and filters.get("item"):
		query += " and item = {item}".format(item=artech_engine.db.escape(filters.get("item")))

	return artech_engine.db.sql(query, filters)
...
def item_manufacturer_query(
	...
	item_manufacturers = artech_engine.get_all(
		"Item Manufacturer",
...
def get_purchase_receipts(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	query = """
		...
		and pr.name like {txt}""".format(txt=artech_engine.db.escape(f"%{txt}%"))

	if filters and filters.get("item_code"):
		query += " and pritem.item_code = {item_code}".format(
			item_code=artech_engine.db.escape(filters.get("item_code"))
		)
...
def get_purchase_invoices(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	query = """
		...
		and pi.name like {txt}""".format(txt=artech_engine.db.escape(f"%{txt}%"))
...
def get_tax_template(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	item_doc = artech_engine.get_cached_doc("Item", filters.get("item_code"))
	item_group = filters.get("item_group")
	company = filters.get("company")
	taxes = item_doc.taxes or []

	while item_group:
		item_group_doc = artech_engine.get_cached_doc("Item Group", item_group)
	...
	if not taxes:
		...
		if txt:
			...
			tax_template_doc = artech_engine.get_meta("Item Tax Template")
...
def get_payment_terms_for_references(
	...
	terms = []
	if filters:
		terms = artech_engine.db.get_all(
			"Payment Schedule",
...
def get_item_uom_query(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	if artech_engine.get_single_value("Stock Settings", "allow_uom_with_conversion_rate_defined_in_item"):
...
def get_warehouse_address(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters: dict):
	table = artech_engine.qb.DocType(doctype)
	child_table = artech_engine.qb.DocType("Dynamic Link")
