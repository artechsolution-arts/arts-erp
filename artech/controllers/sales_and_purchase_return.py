...
from artech_engine.query_builder.functions import Abs
from artech_engine.utils import cint, flt, format_datetime, get_datetime

import artech
from artech.stock.serial_batch_bundle import get_batches_from_bundle
from artech.stock.utils import get_combine_datetime, get_incoming_rate, get_valuation_method, getdate


class StockOverReturnError(artech_engine.ValidationError):
	pass
...
def validate_return_against(doc):
	if not artech_engine.db.exists(doc.doctype, doc.return_against):
		artech_engine.throw(_("Invalid {0}: {1}").format(doc.meta.get_label("return_against"), doc.return_against))
	else:
		ref_doc = artech_engine.get_doc(doc.doctype, doc.return_against)
...
def validate_returned_items(doc):
	valid_items = artech_engine._dict()
	...
	for d in doc.get("items"):
		...
		if doc.doctype in ["Purchase Receipt", "Purchase Invoice", "Sales Invoice", "POS Invoice"]:
			field = artech_engine.scrub(doc.doctype) + "_item"
			if d.get(field):
		...
		if d.item_code and (flt(d.qty) <= 0 or flt(d.get("received_qty")) <= 0):
			if key not in valid_items:
				...
				ref = valid_items.get(key, artech_engine._dict())
				...
				if (
					...
					and artech_engine.db.get_value("Item", d.item_code, "is_stock_item")
				):
					artech_engine.throw(_("Warehouse is mandatory"))
	...
	if not items_returned:
		artech_engine.throw(_("At least one item should be entered with negative quantity in return document"))
...
def validate_quantity(doc, key, args, ref, valid_items, already_returned_items):
	...
	field_precision = get_field_precision(
		artech_engine.get_meta(doc.doctype + " Item").get_field(
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				artech_engine.throw(_("{0} must be negative in return document").format(label))
...
def get_already_returned_items(doc):
	...
	field = (
		artech_engine.scrub(doc.doctype) + "_item"
...
def get_returned_qty_map_for_purchase_flow(return_against, supplier, row_name, doctype):
	...
	_return_map = artech_engine._dict({})

	for row in data:
		if row.warehouse and row.warehouse not in _return_map:
			_return_map[row.warehouse] = artech_engine._dict({"qty": 0, "stock_qty": 0})

		if row.rejected_warehouse and row.rejected_warehouse not in _return_map:
			_return_map[row.rejected_warehouse] = artech_engine._dict({"qty": 0, "stock_qty": 0})
...
def get_returned_qty_map_for_row(return_against, party, row_name, doctype):
	...
	reference_field = "dn_detail" if doctype == "Delivery Note" else artech_engine.scrub(child_doctype)
...
def make_return_doc(doctype: str, source_name: str, target_doc=None, return_against_rejected_qty=False):
	...
	company = artech_engine.db.get_value(doctype, source_name, "company")
	default_warehouse_for_sales_return = artech_engine.get_cached_value(
		"Company", company, "default_warehouse_for_sales_return"
	)

	if doctype == "Sales Invoice":
		inv_is_consolidated, inv_is_pos = artech_engine.db.get_value(
	...
	def set_missing_values(source, target):
		...
		if doctype == "Sales Invoice" or doctype == "POS Invoice":
			...
			if not doc.select_print_heading:
				doc.select_print_heading = artech_engine.get_cached_value("Print Heading", _("Credit Note"))

		elif doctype == "Purchase Invoice":
			# look for Print Heading "Debit Note"
			doc.select_print_heading = artech_engine.get_cached_value("Print Heading", _("Debit Note"))
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if doctype in ["Purchase Receipt", "Subcontracting Receipt"]:
			...
			wh_map = returned_qty_map.get(source_doc.warehouse) or artech_engine._dict()
			rejected_wh_map = returned_qty_map.get(source_doc.rejected_warehouse) or artech_engine._dict()
	...
	def get_returned_non_bundled_serial_nos(child_doc, parent_doc, serial_no_field="serial_no"):
		...
		return_ref_field = artech_engine.scrub(child_doc.doctype)
		...
		for row in artech_engine.get_all(parent_doc.doctype, fields=fields, filters=filters):
...
def get_rate_for_return(
	...
	if not return_against:
		return_against = artech_engine.get_cached_value(voucher_type, voucher_no, "return_against")
	...
	if voucher_type in ("Purchase Receipt", "Purchase Invoice", "Subcontracting Receipt"):
		...
		StockLedgerEntry = artech_engine.qb.DocType("Stock Ledger Entry")
		select_field = Abs(StockLedgerEntry.stock_value_difference / StockLedgerEntry.actual_qty)

	item_details = artech_engine.get_cached_value("Item", item_code, ["has_batch_no", "has_expiry_date"], as_dict=1)
	set_zero_rate_for_expired_batch = artech_engine.db.get_single_value(
	...
	if (
		...
		batch_no = artech_engine.db.get_value(f"{voucher_type} Item", voucher_detail_no, "batch_no")
	...
	rate = flt(artech_engine.db.get_value("Stock Ledger Entry", filters, select_field))
	if not (rate and return_against) and voucher_type in ["Sales Invoice", "Delivery Note"]:
		rate = artech_engine.db.get_value(f"{voucher_type} Item", voucher_detail_no, "incoming_rate")
...
def get_filters(
	...
	if item_row:
		...
		reference_voucher_detail_no = artech_engine.db.get_value(
...
def get_warehouses_for_return(voucher_type, name):
	warehouses = []
	warehouse_details = artech_engine.get_all(
		voucher_type + " Item",
...
def get_returned_serial_nos(child_doc, parent_doc, serial_no_field=None, ignore_voucher_detail_no=None):
	...
	from artech.stock.serial_batch_bundle import get_serial_nos
...
def available_serial_batch_for_return(field, doctype, reference_ids, is_rejected=False):
	...
	if not available_dict:
		artech_engine.throw(_("No Serial / Batches are available for return"))
...
def get_serial_batches_based_on_bundle(doctype, field, _bundle_ids):
	available_dict = artech_engine._dict({})
	batch_serial_nos = artech_engine.get_all(
	...
	for row in batch_serial_nos:
		...
		if artech_engine.get_cached_value(row.voucher_type, row.voucher_no, "is_return"):
			key = artech_engine.get_cached_value(row.voucher_type + " Item", row.voucher_detail_no, field)

		if doctype == "Packed Item":
			if key is None:
				key = artech_engine.get_cached_value(
					"Packed Item",
					{"parent_detail_docname": row.voucher_detail_no, "item_code": row.item_code},
					field,
				)
				if key is None:
					key = artech_engine.get_cached_value("Packed Item", row.voucher_detail_no, field)

				if row.voucher_type == "Delivery Note":
					key = artech_engine.get_cached_value("Delivery Note Item", key, "dn_detail")
				elif row.voucher_type == "Sales Invoice":
					key = artech_engine.get_cached_value("Sales Invoice Item", key, "sales_invoice_item")
		...
		if key not in available_dict:
			available_dict[key] = artech_engine._dict(
				{
					"qty": 0.0,
...
def get_serial_and_batch_bundle(field, doctype, reference_ids, is_rejected=False):
	...
	_bundle_ids = artech_engine.get_all(
		doctype,
		filters=filters,
		pluck=pluck_field,
	)
...
def get_filters_for_packed_item(field, reference_ids):
	...
	if dns := artech_engine.get_all("Delivery Note Item", filters=filters, pluck="name"):
		names.extend(dns)

	filters = {"docstatus": 1, "sales_invoice_item": ("in", reference_ids)}
	if sis := artech_engine.get_all("Sales Invoice Item", filters=filters, pluck="name"):
		names.extend(sis)
...
def filter_serial_batches(parent_doc, data, row, warehouse_field=None, qty_field=None):
	...
	filterd_serial_batch = artech_engine._dict(
		{
			"serial_nos": [],
			"batches": defaultdict(float),
	...
	if not child_doc.get(qty_field):
		artech_engine.throw(
			_("For the {0}, the quantity is required to make the return entry").format(
				artech_engine.bold(child_doc.item_code)
			)
		)
...
def get_payment_data(invoice: str):
	payment = artech_engine.db.get_all("Sales Invoice Payment", {"parent": invoice}, ["mode_of_payment", "amount"])
	return payment


@artech_engine.whitelist()
def get_invoice_item_returned_qty(doctype: str, invoice: str, customer: str, item_row_name: str):
	is_return, docstatus = artech_engine.db.get_value(doctype, invoice, ["is_return", "docstatus"])
...
def is_invoice_returnable(doctype: str, invoice: str):
	is_return, docstatus, customer = artech_engine.db.get_value(
		doctype, invoice, ["is_return", "docstatus", "customer"]
	)
	if is_return or docstatus == 0:
		return False

	invoice_item_qty = artech_engine.db.get_all(f"{doctype} Item", {"parent": invoice}, ["name", "qty"])
...
def get_sales_invoice_item_from_consolidated_invoice(return_against_pos_invoice, pos_invoice_item):
	try:
		...
		query = (
			artech_engine.qb.from_(SalesInvoice)
...
def is_batch_expired(batch_no, posting_date):
	...
	"""
	expiry_date = artech_engine.db.get_value("Batch", batch_no, "expiry_date")
	if not expiry_date:
		return
