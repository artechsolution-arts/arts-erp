...
from artech_engine import _, bold, qb, throw
from artech_engine.contacts.doctype.address.address import get_address_display
from artech_engine.model.workflow import get_workflow_name, is_transition_condition_satisfied
from artech_engine.query_builder import Criterion, DocType
...
from artech.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from artech.accounts.general_ledger import get_round_off_account_and_cost_center
from artech.accounts.party import (
...
from artech.accounts.utils import (
...
from artech.buying.utils import update_last_purchase_rate
from artech.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from artech.controllers.sales_and_purchase_return import validate_return
from artech.exceptions import InvalidCurrency
from artech.setup.utils import get_exchange_rate
from artech.stock.doctype.item.item import get_uom_conv_factor
from artech.stock.doctype.packed_item.packed_item import make_packing_list
from artech.stock.get_item_details import (
	NOT_APPLICABLE_TAX,
	ItemDetailsCtx,
...
from artech.utilities.regional import temporary_flag
from artech.utilities.transaction_base import TransactionBase


class AccountMissingError(artech_engine.ValidationError):
	pass


class InvalidQtyError(artech_engine.ValidationError):
	pass
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = artech.get_company_currency(self.company)
	...
	def onload(self):
		self.set_onload(
			...
			artech_engine.client_cache.get_doc("Accounts Settings").make_payment_via_journal_entry,
		)
	...
	def on_update(self):
		from artech.controllers.taxes_and_totals import process_item_wise_tax_details
	...
	def ensure_supplier_is_not_blocked(self):
		...
		if supplier_name:
			supplier = artech_engine.get_lazy_doc("Supplier", supplier_name)
	...
	def validate_against_voucher_outstanding(self):
		from artech_engine.model.meta import get_meta
		...
		if self.get("is_return") and self.return_against and not self.get("is_pos"):
			against_voucher_outstanding = artech_engine.get_value(
			...
			if self.get("update_outstanding_for_self"):
				msg = _(
					...
					artech_engine.bold(document_type),
					get_link_to_form(self.doctype, self.get("return_against")),
					artech_engine.bold(_("Update Outstanding for Self")),
				)
			...
			if invalid_advances := [x for x in self.advances if not x.reference_type or not x.reference_name]:
				artech_engine.throw(
					_(
						...
						artech_engine.bold(comma_and([x.idx for x in invalid_advances])),
						artech_engine.bold(_("Advance Payments")),
					)
				)
	...
	def validate_company_linked_addresses(self):
		...
		for field in address_fields:
			...
			if address and not artech_engine.db.exists(
				"Dynamic Link",
				{
					"parent": address,
					"parenttype": "Address",
	...
	def set_default_letter_head(self):
		if hasattr(self, "letter_head") and not self.letter_head:
			self.letter_head = artech_engine.db.get_value("Company", self.company, "default_letter_head")
	...
	def _remove_references_in_unreconcile(self):
		upe = artech_engine.qb.DocType("Unreconcile Payment Entries")
		rows = (
			artech_engine.qb.from_(upe)
		...
		if rows:
			references_map = artech_engine._dict()
			for x in rows:
				references_map.setdefault(x.parent, []).append(x.name)

			for doc, rows in references_map.items():
				unreconcile_doc = artech_engine.get_doc("Unreconcile Payment", doc)
				for row in rows:
		...
		unreconcile_docs = artech_engine.db.get_all("Unreconcile Payment", filters={"voucher_no": self.name})
		for x in unreconcile_docs:
			_doc = artech_engine.get_doc("Unreconcile Payment", x.name)
			if _doc.docstatus == 1:
				_doc.cancel()
			_doc.delete()
	...
	def _remove_references_in_repost_doctypes(self):
		...
		for _doctype in repost_doctypes:
			dt = artech_engine.qb.DocType(_doctype)

			cancelled_entries = (
				artech_engine.qb.from_(dt)
			...
			if rows:
				...
				for doc, rows in references_map.items():
					repost_doc = artech_engine.get_doc(doc[0], doc[1])
	...
	def on_trash(self):
		from artech.accounts.utils import delete_exchange_gain_loss_journal
		...
		if artech_engine.get_single_value("Accounts Settings", "delete_linked_ledger_entries"):
			...
			artech_engine.qb.from_(ple).delete().where(
			...
			gle = artech_engine.qb.DocType("GL Entry")
			artech_engine.qb.from_(gle).delete().where(
				(gle.voucher_type == self.doctype) & (gle.voucher_no == self.name)
			).run()
			sle = artech_engine.qb.DocType("Stock Ledger Entry")
			artech_engine.qb.from_(sle).delete().where(
	...
	def remove_serial_and_batch_bundle(self):
		bundles = artech_engine.get_all(
			"Serial and Batch Bundle",
			filters={"voucher_type": self.doctype, "voucher_no": self.name, "docstatus": ("!=", 1)},
		)

		for bundle in bundles:
			artech_engine.delete_doc("Serial and Batch Bundle", bundle.name)

		batches = artech_engine.get_all(
			"Batch", filters={"reference_doctype": self.doctype, "reference_name": self.name}
		)
		for row in batches:
			artech_engine.delete_doc("Batch", row.name)
	...
	def validate_company_in_accounting_dimension(self):
		...
		dimension_list = (
			artech_engine.qb.from_(accounting_dimension)
	...
	def validate_company(self, dimension_list, child=None):
		for dimension in dimension_list:
			if not child:
				dimension_value = self.get(artech_engine.scrub(dimension))
			else:
				dimension_value = child.get(artech_engine.scrub(dimension))

			if dimension_value:
				company = artech_engine.get_cached_value(dimension, dimension_value, "company")
				if company and company != self.company:
					artech_engine.throw(
						_("{0}: {1} does not belong to the Company: {2}").format(
							dimension, artech_engine.bold(dimension_value), self.company
						)
					)
			...
			if original_account != self.get(cr_dr_account_field):
				artech_engine.throw(
					_(
						...
						artech_engine.bold(_(self.meta.get_label(cr_dr_account_field), context=self.doctype)),
						artech_engine.bold(original_account),
						artech_engine.bold(self.return_against),
					)
				)
		...
		for item in self.get("items"):
			if item.get("enable_deferred_revenue") or item.get("enable_deferred_expense"):
				if not item.get(field_map.get(self.doctype)):
					default_deferred_account = artech_engine.get_cached_value(
	...
	def validate_auto_repeat_subscription_dates(self):
		if self.get("from_date") and self.get("to_date") and getdate(self.from_date) > getdate(self.to_date):
			artech_engine.throw(_("To Date cannot be before From Date"), title=_("Invalid Auto Repeat Date"))
	...
	def calculate_taxes_and_totals(self):
		from artech.controllers.taxes_and_totals import calculate_taxes_and_totals
			...
			if item.get(item_field) == self.get(party_account_field):
				artech_engine.throw(
					_("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
						item.idx,
						artech_engine.bold(artech_engine.unscrub(item_field)),
						item.get(item_field),
						artech_engine.bold(artech_engine.unscrub(party_account_field)),
	...
	def validate_inter_company_reference(self):
		...
		if self.is_internal_transfer():
			if not (
				...
				artech_engine.throw(msg, title=_("Internal Sales Reference Missing"))

			label = "Delivery Note Item" if self.doctype == "Purchase Receipt" else "Sales Invoice Item"

			field = artech_engine.scrub(label)

			for row in self.get("items"):
				if not row.get(field):
					msg = f"At Row {row.idx}: The field {bold(label)} is mandatory for internal transfer"
					artech_engine.throw(_(msg), title=_("Internal Transfer Reference Missing"))

	def validate_internal_transaction(self):
		if not cint(artech_engine.get_single_value("Accounts Settings", "maintain_same_internal_transaction_rate")):
			return
	...
	def validate_line_items(self, ref_dt, ref_dn_field, ref_link_field):
		action, role_allowed_to_override = artech_engine.get_cached_value(
			...
			if d.get(ref_link_field):
				...
				if ref_rate is not None and abs(flt(d.rate - ref_rate, d.precision("rate"))) >= 0.01:
					if action == "Stop":
						user_roles = [
							r["role"]
							for r in artech_engine.get_all(
								"Has Role", filters={"parent": artech_engine.session.user}, fields=["role"]
							)
						]
		...
		if stop_actions:
			artech_engine.throw(stop_actions, as_list=True)
	...
	def validate_due_date(self):
		...
		from artech.accounts.party import validate_due_date
		...
		if artech_engine.flags.in_import and getdate(self.due_date) < getdate(posting_date):
	...
	def set_price_list_currency(self, buying_or_selling):
		...
		if self.meta.get_field("currency"):
			...
			if self.meta.get_field(fieldname) and self.get(fieldname):
				self.price_list_currency = artech_engine.db.get_value("Price List", self.get(fieldname), "currency")
			...
			if (
				...
				and artech_engine.db.get_single_value("Buying Settings", "use_transaction_date_exchange_rate")
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or artech.get_default_cost_center(self.company),
						)
	...
	def apply_pricing_rule_on_items(self, item, pricing_rule_args):
		if not pricing_rule_args.get("validate_applied_rule", 0):
			...
			for pricing_rule in get_applied_pricing_rules(item.get("pricing_rules")):
				pricing_rule_doc = artech_engine.get_cached_doc("Pricing Rule", pricing_rule)
					...
					if item.get(field) < pricing_rule_doc.get(field):
						...
						artech_engine.msgprint(
							_("Row {0}: user has not applied the rule {1} on the item {2}").format(
								item.idx, artech_engine.bold(title), artech_engine.bold(item.item_code)
							)
						)
	...
	def set_taxes(self):
		...
		if (self.is_new() or self.is_pos_profile_changed()) and not self.get("taxes"):
			if self.company and not self.get("taxes_and_charges"):
				...
				self.taxes_and_charges = artech_engine.db.get_value(
	...
	def is_pos_profile_changed(self):
		if (
			...
			and self.is_pos
			and self.pos_profile != artech_engine.db.get_value("Sales Invoice", self.name, "pos_profile")
		):
			return True
	...
	def set_taxes_and_charges(self):
		...
		if artech_engine.get_single_value(
		...
		if artech_engine.get_single_value("Accounts Settings", "add_taxes_from_item_tax_template"):
	...
	def append_taxes_from_item_tax_template(self):
		if not artech_engine.get_single_value("Accounts Settings", "add_taxes_from_item_tax_template"):
			return
	...
	def validate_enabled_taxes_and_charges(self):
		...
		if self.taxes_and_charges and artech_engine.get_cached_value(
			taxes_and_charges_doctype, self.taxes_and_charges, "disabled"
		):
			artech_engine.throw(_("{0} '{1}' is disabled").format(taxes_and_charges_doctype, self.taxes_and_charges))

	def validate_tax_account_company(self):
		for d in self.get("taxes"):
			if d.account_head:
				tax_account_company = artech_engine.get_cached_value("Account", d.account_head, "company")
	...
	def get_gl_dict(self, args, account_currency=None, item=None):
		...
		dimension_dict = artech_engine._dict()
	...
	def get_voucher_subtype(self):
		...
		for method_name in artech_engine.get_hooks("voucher_subtypes"):
			voucher_subtype = artech_engine.get_attr(method_name)(self)
	...
	def validate_zero_qty_for_return_invoices_with_stock(self):
		...
		if rows:
			artech_engine.throw(
				...
				).format(artech_engine.bold(comma_and(["#" + str(x.idx) for x in rows])))
			)
			...
			if not flt(item.qty):
				artech_engine.throw(
					msg=_("Row #{0}: Quantity for Item {1} cannot be zero.").format(
						item.idx, artech_engine.bold(item.item_code)
					),
	...
	def clear_unallocated_advances(self, childtype, parentfield):
		...
		doctype = artech_engine.qb.DocType(childtype)
		artech_engine.qb.from_(doctype).delete().where(
	...
	def apply_shipping_rule(self):
		if self.shipping_rule:
			shipping_rule = artech_engine.get_doc("Shipping Rule", self.shipping_rule)
		...
		for fieldname in ("shipping_address_name", "shipping_address"):
			...
			if shipping_field and shipping_field.fieldtype == "Link":
				if self.get(fieldname):
					return artech_engine.get_doc("Address", self.get(fieldname))
	...
	def is_inclusive_tax(self):
		is_inclusive = cint(artech_engine.get_single_value("Accounts Settings", "show_inclusive_tax_in_print"))
	...
	def should_show_taxes_as_table_in_print(self):
		return cint(artech_engine.get_single_value("Accounts Settings", "show_taxes_as_table_in_print"))
	...
	def gain_loss_journal_already_booked(
		...
		"""
		if res := artech_engine.db.get_all(
			...
			if exc_vouchers := artech_engine.db.get_all(
				"Journal Entry",
	...
	def make_exchange_gain_loss_journal(
		...
		if self.docstatus == 1:
			if dimensions_dict is None:
				dimensions_dict = artech_engine._dict()
					...
					for arg in args:
						...
						if (
							...
							if not self.gain_loss_journal_already_booked(
								...
								posting_date = arg.get("difference_posting_date") or artech_engine.db.get_value(
			...
			if self.get("doctype") == "Payment Entry":
				...
				for d in gain_loss_to_book:
					...
					if d.exchange_gain_loss and (
						...
						gain_loss_account = artech_engine.get_cached_value(
	...
	def is_payable_account(self, reference_doctype, account):
		if reference_doctype == "Purchase Invoice" or (
			...
			and artech_engine.get_cached_value("Account", account, "account_type") == "Payable"
		):
			return True
		return False
	...
	def update_against_document_in_jv(self):
				...
				args = artech_engine._dict(
					{
						...
						"difference_account": artech_engine.get_cached_value(
							"Company", self.company, "exchange_gain_loss_account"
						),
		...
		if lst:
			from artech.accounts.utils import reconcile_against_document
	...
	def cancel_system_generated_credit_debit_notes(self):
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			journals = artech_engine.db.get_all(
				"Journal Entry",
				filters={
			...
			for x in journals:
				artech_engine.get_doc("Journal Entry", x).cancel()

	def on_cancel(self):
		from artech.accounts.doctype.bank_transaction.bank_transaction import (
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice", "Payment Entry", "Journal Entry"]:
			...
			if artech_engine.get_single_value("Accounts Settings", "unlink_payment_on_cancellation_of_invoice"):
				unlink_ref_doc_from_payment_entries(self)

		elif self.doctype in ["Sales Order", "Purchase Order"]:
			if artech_engine.get_single_value("Accounts Settings", "unlink_advance_payment_on_cancelation_of_order"):
				unlink_ref_doc_from_payment_entries(self)
	...
	def unlink_ref_doc_from_po(self):
		...
		if linked_po:
			...
			artech_engine.msgprint(_("Purchase Orders {0} are un-linked").format("\n".join(linked_po)))
	...
	def make_discount_gl_entries(self, gl_entries):
		enable_discount_accounting = cint(
			artech_engine.get_single_value("Selling Settings", "enable_discount_accounting")
		)
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or artech.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from artech.controllers.status_updater import get_allowance_for
		...
		role_allowed_to_overbill = artech_engine.get_single_value("Accounts Settings", "role_allowed_to_over_bill")
		is_overbilling_allowed = role_allowed_to_overbill in artech_engine.get_roles()
		...
		for row in ref_wise_billed_amount.values():
			...
			if overbill_amt > precision_allowance and not is_overbilling_allowed:
				if self.doctype != "Purchase Invoice" or not cint(
					artech_engine.db.get_single_value(
	...
	def get_reference_wise_billed_amt(self, ref_dt, item_ref_dn, based_on):
		...
		for item in self.items:
			...
			ref_wise_billed_amount.setdefault(
				key,
				artech_engine._dict(item_code=item.item_code, billed_amt=0.0, ref_amt=ref_amt, rows=[]),
			)
	...
	def get_already_billed_amount(self, reference_names, item_ref_dn, based_on):
		item_doctype = artech_engine.qb.DocType(self.items[0].doctype)
		based_on_field = artech_engine.qb.Field(based_on)
		join_field = artech_engine.qb.Field(item_ref_dn)

		return artech_engine._dict(
			(
				artech_engine.qb.from_(item_doctype)
	...
	def throw_overbill_exception(self, overbilled_items, precision):
		message = (
			...
			+ "".join(
				_("<li>Item {0} in row(s) {1} billed more than {2}</li>").format(
					artech_engine.bold(item.item_code),
					", ".join(str(x) for x in item.rows),
					artech_engine.bold(fmt_money(item.max_allowed_amt, precision=precision, currency=self.currency)),
				)
		...
		artech_engine.throw(_(message))

	def get_company_default(self, fieldname, ignore_validation=False):
		from artech.accounts.utils import get_company_default
	...
	def get_stock_items(self):
		...
		if item_codes:
			stock_items = artech_engine.db.get_values(
	...
	def get_asset_items(self):
		...
		item_codes = list(set(item.item_code for item in self.get("items")))
		if item_codes:
			asset_items = artech_engine.db.get_values(
	...
	def calculate_total_advance_from_ledger(self):
		adv = artech_engine.qb.DocType("Advance Payment Ledger Entry")
		return (
			qb.from_(adv)
	...
	def set_advance_payment_status(self):
		...
		PaymentRequest = artech_engine.qb.DocType("Payment Request")
		paid_amount = artech_engine.get_value(
	...
	def company_abbr(self):
		if not hasattr(self, "_abbr"):
			self._abbr = artech_engine.get_cached_value("Company", self.company, "abbr")
	...
	def raise_missing_debit_credit_account_error(self, party_type, party):
		...
		db_or_cr = (
			artech_engine.bold(_("Debit To")) if self.doctype == "Sales Invoice" else artech_engine.bold(_("Credit To"))
		)
		rec_or_pay = "Receivable" if self.doctype == "Sales Invoice" else "Payable"

		link_to_party = artech_engine.utils.get_link_to_form(party_type, party)
		link_to_company = artech_engine.utils.get_link_to_form("Company", self.company)

		message = _("{0} Account not found against Customer {1}.").format(db_or_cr, artech_engine.bold(party) or "")
		...
		artech_engine.throw(message, title=_("Account Missing"), exc=AccountMissingError)
	...
	def validate_party_account_currency(self):
		...
		allow_multi_currency_invoices_against_single_party_account = artech_engine.db.get_singles_value(
		...
		if (
			...
			artech_engine.throw(
				_("Party Account {0} currency ({1}) and document currency ({2}) should be same").format(
					artech_engine.bold(party_account), party_account_currency, self.currency
				)
			)
	...
	def delink_advance_entries(self, linked_doc_name):
		...
		for adv in self.advances:
			...
			if adv.reference_name == linked_doc_name:
				doctype = artech_engine.qb.DocType(self.doctype + " Advance")
				artech_engine.qb.from_(doctype).delete().where(doctype.name == adv.name).run()
	...
	def group_similar_items(self):
		...
		fields_to_group = artech_engine.get_hooks("fields_for_group_similar_items")
	...
	def set_payment_schedule(self):
		...
		if self.doctype in ("Sales Invoice", "Purchase Invoice", "Sales Order"):
			...
			automatically_fetch_payment_terms = cint(
				artech_engine.get_single_value("Accounts Settings", "automatically_fetch_payment_terms")
			)
			if self.doctype != "Sales Order":
		...
		allocate_payment_based_on_payment_terms = artech_engine.db.get_value(
	...
	def linked_order_has_payment_terms_template(self, po_or_so, doctype):
		return artech_engine.get_value(doctype, po_or_so, "payment_terms_template")

	def linked_order_has_payment_schedule(self, po_or_so):
		return artech_engine.get_all("Payment Schedule", filters={"parent": po_or_so})
	...
	def fetch_payment_terms_from_order(
		...
		"""
		po_or_so = artech_engine.get_cached_doc(po_or_so_doctype, po_or_so)
	...
	def is_rounded_total_disabled(self):
		if self.meta.get_field("disable_rounded_total"):
			...
			return artech_engine.db.get_single_value("Global Defaults", "disable_rounded_total")
	...
	def set_inter_company_account(self):
		...
		if self.is_internal_transfer() and not self.unrealized_profit_loss_account:
			unrealized_profit_loss_account = artech_engine.get_cached_value(
			...
			if not unrealized_profit_loss_account:
				...
				).format(artech_engine.bold(self.company))
	...
	def process_common_party_accounting(self):
		...
		if artech_engine.get_single_value("Accounts Settings", "enable_common_party_accounting"):
	...
	def create_advance_and_reconcile(self, party_link):
		...
		default_currency = artech.get_company_currency(self.company)
		...
		jv = artech_engine.new_doc("Journal Entry")
		...
		reconcilation_entry = artech_engine._dict()
		advance_entry = artech_engine._dict()
		...
		reconcilation_entry.cost_center = self.cost_center or artech.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or artech.get_default_cost_center(self.company)
	...
	def repost_accounting_entries(self):
		repost_ledger = artech_engine.new_doc("Repost Accounting Ledger")
	...
	def set_discount_amount_after_mapping(self, source_doc):
		...
		doctype_table = artech_engine.qb.DocType(self.doctype)
		item_table = artech_engine.qb.DocType(item_doctype)
		...
		query = (
			artech_engine.qb.from_(doctype_table)
		...
		if is_return:
			...
			item_meta = artech_engine.get_meta(item_doctype)
			...
			query = query.where(
				doctype_table.name.isin(
					artech_engine.qb.from_(item_table)
...
def get_tax_rate(account_head: str):
	return artech_engine.get_cached_value("Account", account_head, ["tax_rate", "account_name"], as_dict=True)
...
def get_default_taxes_and_charges(
	...
	if tax_template and company:
		tax_template_company = artech_engine.get_cached_value(master_doctype, tax_template, "company")
		if tax_template_company == company:
			return

	default_tax = artech_engine.db.get_value(master_doctype, {"is_default": 1, "company": company})
...
def get_taxes_and_charges(master_doctype: str, master_name: str | None = None):
	if not master_name:
		return
	from artech_engine.model import child_table_fields, default_fields

	tax_master = artech_engine.get_doc(master_doctype, master_name)
...
def validate_conversion_rate(currency, conversion_rate, conversion_rate_label, company):
	...
	company_currency = artech_engine.get_cached_value("Company", company, "default_currency")
...
def validate_account_head(idx: int, account: str, company: str, context: str | None = None) -> None:
	...
	if company != artech_engine.get_cached_value("Account", account, "company"):
		artech_engine.throw(
			_("Row {0}: The {3} Account {1} does not belong to the company {2}").format(
				idx, artech_engine.bold(account), artech_engine.bold(company), context or ""
			),
			title=_("Invalid Account"),
		)

	if artech_engine.get_cached_value("Account", account, "is_group"):
		artech_engine.throw(
			_(
				"You selected the account group {1} as {2} Account in row {0}. Please select a single account."
			).format(idx, artech_engine.bold(account), context or ""),
			title=_("Invalid Account"),
		)
...
def validate_cost_center(tax, doc):
	...
	company = artech_engine.get_cached_value("Cost Center", tax.cost_center, "company")

	if company != doc.company:
		artech_engine.throw(
			_("Row {0}: Cost Center {1} does not belong to Company {2}").format(
				tax.idx, artech_engine.bold(tax.cost_center), artech_engine.bold(doc.company)
			),
...
def validate_inclusive_tax(tax, doc):
	...
	if cint(getattr(tax, "included_in_print_rate", None)):
		if tax.charge_type == "Actual":
			...
			artech_engine.throw(_("Valuation type charges can not be marked as Inclusive"))
...
def get_advance_journal_entries(
	...
	journal_entry = artech_engine.qb.DocType("Journal Entry")
	journal_acc = artech_engine.qb.DocType("Journal Entry Account")
	q = (
		artech_engine.qb.from_(journal_entry)
...
def get_advance_payment_entries(
	...
	payment_entries = []
	payment_entry = artech_engine.qb.DocType("Payment Entry")
	...
	if order_list or against_all_orders:
		...
		payment_ref = artech_engine.qb.DocType("Payment Entry Reference")
...
def get_common_query(
	...
	account_type = artech_engine.db.get_value("Party Type", party_type, "account_type")
	payment_type = "Receive" if account_type == "Receivable" else "Pay"
	payment_entry = artech_engine.qb.DocType("Payment Entry")

	q = (
		artech_engine.qb.from_(payment_entry)
		.select(
...
def update_invoice_status():
	...
	today = getdate()
	payment_schedule = artech_engine.qb.DocType("Payment Schedule")
	for doctype in ("Sales Invoice", "Purchase Invoice"):
		invoice = artech_engine.qb.DocType(doctype)
		...
		payable_amount = (
			artech_engine.qb.from_(payment_schedule)
		...
		total_amount = artech_engine.qb.terms.Case().when(consider_base_amount, base_total).else_(total)
		...
		artech_engine.qb.update(invoice).set("status", status).where(conditions).run()
...
def get_payment_terms(
	...
	terms_doc = artech_engine.get_doc("Payment Terms Template", terms_template)

	schedule = []
	for d in terms_doc.get("terms"):
		d = artech_engine._dict(d.as_dict())
...
def get_payment_term_details(
	term: str | artech_engine._dict,
	posting_date: DateTimeLikeObject | None = None,
	grand_total: float | None = None,
	base_grand_total: float | None = None,
	bill_date: DateTimeLikeObject | None = None,
):
	term_details = artech_engine._dict()
	if isinstance(term, str):
		term = artech_engine.get_doc("Payment Term", term)
	else:
...
def get_supplier_block_status(party_name):
	...
	a `Supplier`
	"""
	supplier = artech_engine.get_doc("Supplier", party_name)
	info = {
...
def add_taxes_from_tax_template(child_item, parent_doc, db_insert=True):
	add_taxes_from_item_tax_template = artech_engine.get_single_value(
...
def set_order_defaults(parent_doctype, parent_doctype_name, child_doctype, child_docname, trans_item):
	...
	"""
	p_doc = artech_engine.get_doc(parent_doctype, parent_doctype_name)
	child_item = artech_engine.new_doc(child_doctype, parent_doc=p_doc, parentfield=child_docname)
	item = artech_engine.get_doc("Item", trans_item.get("item_code"))
	...
	if child_doctype == "Sales Order Item":
		...
		if not child_item.warehouse:
			artech_engine.throw(
				...
				).format(artech_engine.bold(item.item_code))
			)
...
def validate_child_on_delete(row, parent, ordered_item=None):
	...
	if parent.doctype == "Quotation":
		if ordered_item.get(row.name):
			artech_engine.throw(_("Cannot delete an item which has been ordered"))


def update_bin_on_delete(row, doctype):
	"""Update bin for deleted item (row)."""
	from artech.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def update_child_qty_rate(
	...
	from artech.buying.doctype.supplier_quotation.supplier_quotation import get_purchased_items
	from artech.selling.doctype.quotation.quotation import get_ordered_items

	def check_doc_permissions(doc, perm_type="create"):
		try:
			doc.check_permission(perm_type)
		except artech_engine.PermissionError:
	...
	def validate_workflow_conditions(doc):
		...
		workflow_doc = artech_engine.get_doc("Workflow", workflow)
	...
	def is_allowed_zero_qty():
		if parent_doctype == "Sales Order":
			return artech_engine.db.get_single_value("Selling Settings", "allow_zero_qty_in_sales_order") or False
		elif parent_doctype == "Purchase Order":
			return artech_engine.db.get_single_value("Buying Settings", "allow_zero_qty_in_purchase_order") or False
		return False
		...
		if not flt(new_data.get("qty")) and not is_allowed_zero_qty():
			artech_engine.throw(
				_("Row #{0}:Quantity for Item {1} cannot be zero.").format(
					new_data.get("idx"), artech_engine.bold(new_data.get("item_code"))
				),
				title=_("Invalid Qty"),
			)
		...
		if parent_doctype in qty_limits:
			...
			if flt(new_data.get("qty")) < flt(child_item.get(qty_field)):
				artech_engine.throw(
					...
					+ error_message.format(artech_engine.bold(new_data.get("item_code"))),
					title=_("Invalid Qty"),
				)
			...
			if qty_to_check:
				if not rate_unchanged:
					artech_engine.throw(
						...
						).format(artech_engine.bold(new_data.get("item_code")))
					)

				if flt(new_data.get("qty")) < qty_to_check:
					artech_engine.throw(_("Cannot reduce quantity than ordered or purchased quantity"))
	...
	def should_update_supplied_items(doc) -> bool:
		...
		if update_supplied_items and supplied_items_processed:
			artech_engine.throw(_("Item qty can not be updated as raw materials are already processed."))
	...
	def validate_fg_item_for_subcontracting(new_data, is_new):
		if is_new:
			if not new_data.get("fg_item"):
				...
				is_sub_contracted_item, default_bom = artech_engine.db.get_value(
				...
				if not is_sub_contracted_item:
					...
					artech_engine.throw(_("Default BOM not found for FG Item {0}").format(new_data["fg_item"]))

		if not new_data.get("fg_item_qty"):
			artech_engine.throw(_("Finished Good Item {0} Qty can not be zero").format(new_data["fg_item"]))
	...
	parent = artech_engine.get_doc(parent_doctype, parent_doctype_name)
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			check_doc_permissions(parent, "write")
			child_item = artech_engine.get_doc(parent_doctype + " Item", d.get("docname"))
		...
		if not rate_unchanged and not child_item.get("qty") and is_allowed_zero_qty():
			artech_engine.throw(_("Rate of '{}' items cannot be changed").format(artech_engine.bold(_("Unit Price"))))
	...
	artech_engine.get_cached_doc("Authorization Control").validate_approving_authority(
			...
			if parent.is_old_subcontracting_flow:
				...
				if not parent.can_update_items():
					artech_engine.throw(
						...
						).format(artech_engine.bold(parent.name))
					)
	...
	if parent_doctype == "Sales Order" and not parent.is_subcontracted:
		from artech.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
...
def update_gl_dict_with_app_based_fields(doc, gl_dict):
	for method in artech_engine.get_hooks("update_gl_dict_with_app_based_fields", default=[]):
		artech_engine.get_attr(method)(doc, gl_dict)


@artech_engine.whitelist()
def get_missing_company_details(doctype: str, docname: str):
	from artech_engine.contacts.doctype.address.address import get_address_display_list

	company = artech_engine.db.get_value(doctype, docname, "company")
	if doctype in ["Purchase Order", "Purchase Invoice"]:
		company_address = artech_engine.db.get_value(doctype, docname, "billing_address")
	elif doctype in ["Request for Quotation"]:
		company_address = artech_engine.db.get_value(doctype, docname, "shipping_address")
	else:
		company_address = artech_engine.db.get_value(doctype, docname, "company_address")

	company_details = artech_engine.get_value(
	...
	if not all(required_fields) and not artech_engine.has_permission("Company", "write", throw=False):
		artech_engine.msgprint(
			_(
	...
	if not company_address and not artech_engine.has_permission(doctype, "write", throw=False):
		artech_engine.msgprint(
			_(
	...
	if needs_new_company_address and not artech_engine.has_permission("Address", "create", throw=False):
		artech_engine.msgprint(
			_(
...
def update_company_master_and_address(current_doctype: str, name: str, company: str, details: dict | str):
	from artech_engine.utils import validate_email_address

	if not artech_engine.has_permission(current_doctype, "write", doc=name, throw=False):
		artech_engine.throw(
			_("You don't have permission to update this document. Please contact your System Manager."),
			title=_("Insufficient Permissions"),
		)

	if not artech_engine.has_permission("Company", "write", doc=company, throw=False):
		artech_engine.throw(
	...
	if isinstance(details, str):
		details = artech_engine.parse_json(details)
	...
	if company_fields_to_update:
		artech_engine.db.set_value("Company", company, company_fields_to_update)

	company_address = details.get("company_address")
	if details.get("address_line1"):
		if not artech_engine.has_permission("Address", "create", throw=False):
			artech_engine.throw(
				_(
		...
		address_doc = artech_engine.get_doc(
			{
				"doctype": "Address",
...
def update_doc_company_address(current_doctype, docname, company_address, details):
	...
	current_display = artech_engine.db.get_value(current_doctype, docname, display_field)
	...
	(
		artech_engine.qb.update(DocType)
