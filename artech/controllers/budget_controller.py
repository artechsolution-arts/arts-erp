...
from artech_engine.utils import fmt_money

from artech.accounts.doctype.budget.budget import BudgetError, get_accumulated_monthly_budget
...
class BudgetValidation:
	def __init__(self, doc: object | None = None, gl_map: list | None = None):
		...
		self.get_dimensions()
		self.exception_approver_role = artech_engine.get_cached_value(
	...
	def build_to_validate_map(self):
		self.to_validate = artech_engine._dict()
	...
	def get_child_nodes(self, budget_against, dimension):
		lft, rgt = artech_engine.db.get_all(
			budget_against, filters={"name": dimension}, fields=["lft", "rgt"], as_list=1
		)[0]
		return artech_engine.db.get_all(budget_against, filters={"lft": [">=", lft], "rgt": ["<=", rgt]}, as_list=1)
	...
	def build_budget_keys(self):
		...
		for _bud in self.get_budget_records():
			budget_against = artech_engine.scrub(_bud.budget_against)
			dimension = _bud.get(budget_against)

			if _bud.is_tree and artech_engine.get_cached_value(_bud.budget_against, dimension, "is_group"):
	...
	def get_dimensions(self):
		self.dimensions = []
		for _x in artech_engine.db.get_all("Accounting Dimension"):
			self.dimensions.append(artech_engine.get_lazy_doc("Accounting Dimension", _x.name))
	...
	def get_budget_records(self) -> list:
		...
		for x in _budgets:
			x.is_tree = artech_engine.get_meta(x.budget_against).is_tree
	...
	def warn(self, msg):
		artech_engine.msgprint(msg, _("Budget Exceeded"))

	def execute_action(self, action, msg):
		if self.exception_approver_role and self.exception_approver_role in artech_engine.get_roles(
		...
		if config.applies:
			...
			if annual_diff > 0:
				_msg = _(
					...
					artech_engine.bold(key[2]),
					artech_engine.bold(artech_engine.unscrub(key[0])),
					artech_engine.bold(key[1]),
					artech_engine.bold(fmt_money(budget_amt, currency=currency)),
					artech_engine.bold(fmt_money(annual_diff, currency=currency)),
				)
			...
			if monthly_diff > 0:
				_msg = _(
					...
					artech_engine.bold(artech_engine.unscrub(key[0])),
					artech_engine.bold(key[1]),
					artech_engine.bold(fmt_money(acc_monthly_budget, currency=currency)),
					artech_engine.bold(fmt_money(monthly_diff, currency=currency)),
				)
	...
	def handle_cumulative_overlimit_for_monthly(self, key, v_map):
		...
		if monthly_diff > 0:
			...
			_msg = _(
				...
				artech_engine.bold(artech_engine.unscrub(key[0])),
				artech_engine.bold(key[1]),
				artech_engine.bold(fmt_money(v_map.accumulated_monthly_budget, currency=currency)),
	...
	def handle_cumulative_overlimit_for_annual(self, key, v_map):
		...
		if total_diff > 0:
			...
			_msg = _(
				...
				artech_engine.bold(artech_engine.unscrub(key[0])),
				artech_engine.bold(key[1]),
				artech_engine.bold(fmt_money(v_map.budget_amount, currency=currency)),
				self.budget_applicable_for(v_map, current_amt),
				artech_engine.bold(fmt_money(total_diff, currency=currency)),
			)
