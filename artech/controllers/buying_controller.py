...
from artech_engine import ValidationError, _, msgprint
from artech_engine.contacts.doctype.address.address import render_address
from artech_engine.utils import cint, flt, format_date, get_link_to_form, getdate
from artech_engine.utils.data import nowtime

import artech
from artech.accounts.doctype.accounting_dimension.accounting_dimension import get_dimensions
from artech.accounts.doctype.budget.budget import validate_expense_against_budget
from artech.accounts.party import get_party_details
from artech.buying.utils import update_last_purchase_rate, validate_for_items
from artech.controllers.accounts_controller import get_taxes_and_charges
from artech.controllers.sales_and_purchase_return import get_rate_for_return
from artech.controllers.subcontracting_controller import SubcontractingController
from artech.stock.get_item_details import (
	NOT_APPLICABLE_TAX,
	get_conversion_factor,
	get_item_defaults,
)
from artech.stock.utils import get_incoming_rate
...
class BuyingController(SubcontractingController):
	...
	def validate(self):
		...
		if getattr(self, "supplier", None) and not self.supplier_name:
			self.supplier_name = artech_engine.db.get_value("Supplier", self.supplier, "supplier_name")
	...
	def onload(self):
		...
		self.set_onload(
			"backflush_based_on",
			artech_engine.db.get_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on"),
		)
		...
		if (
			self.get("company")
			and (
				default_buying_terms := artech_engine.get_value(
			...
			self.terms = artech_engine.get_value("Terms and Conditions", self.get("tc_name"), "terms")
	...
	def validate_posting_date_with_po(self):
		...
		msg = _("<p>Posting Date {0} cannot be before Purchase Order date for the following:</p><ul>").format(
			artech_engine.bold(format_date(self.posting_date))
		)
	...
	def create_package_for_transfer(self) -> None:
		...
		if self.is_internal_transfer() and (
			...
			bundle_ids = {}
			if ids:
				for bundle in artech_engine.get_all(
			...
			for item in self.get("items"):
				if item.get(field) and not item.serial_and_batch_bundle and bundle_ids.get(item.get(field)):
					...
					artech_engine.set_value("Serial and Batch Entry", sabe[0], "qty", item.qty)
	...
	def set_supplier_from_item_default(self):
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				supplier = artech_engine.db.get_value(
					"Item Default", {"parent": d.item_code, "company": self.company}, "default_supplier"
				)
				if supplier:
					self.supplier = supplier
				else:
					item_group = artech_engine.db.get_value("Item", d.item_code, "item_group")
					supplier = artech_engine.db.get_value(
	...
	def validate_asset_return(self):
		...
		if self.return_against:
			not_cancelled_asset = [
				d.name
				for d in artech_engine.db.get_all("Asset", {purchase_doc_field: self.return_against, "docstatus": 1})
			]
	...
	def set_gl_entry_for_purchase_expense(self, gl_entries):
		...
		for row in self.items:
			...
			if not details.purchase_expense_account:
				details.purchase_expense_account = artech_engine.get_cached_value(
			...
			if not details.purchase_expense_contra_account:
				details.purchase_expense_contra_account = artech_engine.get_cached_value(
			...
			self.add_gl_entry(
				...
				account_currency=artech_engine.get_cached_value(
	...
	def set_incoming_rate(self):
		...
		allow_at_arms_length_price = artech_engine.get_cached_value(
	...
	def set_sales_incoming_rate_for_internal_transfer(self):
		...
		for d in self.get("items"):
			if not d.get(artech_engine.scrub(ref_doctype)):
				posting_time = self.get("posting_time")
				if not posting_time:
				...
				d.sales_incoming_rate = flt(
					artech_engine.db.get_value(ref_doctype, d.get(artech_engine.scrub(ref_doctype)), field)
				...
				if item in self.sub_contracted_items and not item.bom:
					artech_engine.throw(
						_("Please select BOM in BOM field for Item {item_code}.").format(
							item_code=artech_engine.bold(item.item_code)
						)
					)
			...
			for row in self.get("supplied_items"):
				if not row.reserve_warehouse:
					artech_engine.throw(
						...
						).format(item_code=artech_engine.bold(row.rm_item_code))
					)
		else:
			for item in self.get("items"):
				if item.get("bom"):
	...
	def set_qty_as_per_stock_uom(self):
		allow_to_edit_stock_qty = artech_engine.get_single_value(
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty")):
				...
				).format(idx=d.idx, item_code=artech_engine.bold(d.item_code))
				artech_engine.throw(msg=message, title=_("Mismatch"), exc=QtyMismatchError)
			...
			if flt(item_row[fieldname]) < 0:
				artech_engine.throw(
					_("Row #{idx}: {field_label} can not be negative for item {item_code}.").format(
						idx=item_row["idx"],
						field_label=artech_engine.get_meta(item_row.doctype).get_label(fieldname),
						item_code=artech_engine.bold(item_row["item_code"]),
					)
				)
	...
	def check_for_on_hold_or_closed_status(self, ref_doctype, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = artech_engine.db.get_value(ref_doctype, d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold"):
					artech_engine.throw(
						_("{ref_doctype} {ref_name} is {status}.").format(
							ref_doctype=artech_engine.bold(_(ref_doctype)),
							ref_name=artech_engine.bold(d.get(ref_fieldname)),
							status=artech_engine.bold(_(status)),
						)
					)
	...
	def update_stock_ledger(self, allow_negative_stock=False, via_landed_cost_voucher=False):
		...
		for d in self.get("items"):
			...
			if flt(d.rejected_qty) != 0:
				...
				if artech_engine.db.get_single_value("Buying Settings", "set_valuation_rate_for_rejected_materials"):
	...
	def get_package_for_target_warehouse(
		...
		if via_landed_cost_voucher and item.get("warehouse"):
			if sabb := artech_engine.db.get_value(
	...
	def update_ordered_and_reserved_qty(self):
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				po_obj = artech_engine.get_lazy_doc("Purchase Order", po)

				if po_obj.status in ["Closed", "Cancelled"]:
					artech_engine.throw(
						_("{doctype} {name} is cancelled or closed.").format(
							doctype=artech_engine.bold(_("Purchase Order")),
							name=artech_engine.bold(po),
						),
						artech_engine.InvalidStatusError,
					)
	...
	def on_submit(self):
		...
		if self.doctype in [
			...
			"Purchase Receipt",
			"Purchase Invoice",
		] and not artech_engine.db.get_single_value("Buying Settings", "disable_last_purchase_rate"):
	...
	def validate_budget(self):
		if not artech_engine.get_single_value("Accounts Settings", "use_legacy_budget_controller"):
			from artech.controllers.budget_controller import BudgetValidation
			...
			if d.is_fixed_asset:
				...
				if item_data.get("auto_create_assets"):
						...
						if len(created_assets) > 5:
							...
							messages.append(
								_("{count} Assets created for {item_code}").format(
									count=len(created_assets), item_code=artech_engine.bold(d.item_code)
								)
							)
						else:
							assets_link = list(
								map(lambda d: artech_engine.utils.get_link_to_form("Asset", d), created_assets)
							)
							assets_link = artech_engine.bold(",".join(assets_link))

							if len(created_assets) == 1:
								msg = _("Asset {assets_link} created for {item_code}").format(
									assets_link=assets_link,
									item_code=artech_engine.bold(d.item_code),
								)
							else:
					...
					messages.append(
						...
						).format(item_code=artech_engine.bold(d.item_code))
					)
					alert = True

		for message in messages:
			artech_engine.msgprint(message, title="Success", indicator="green", alert=alert)
		...
		if not row.asset_location:
			artech_engine.throw(
				_("Row #{idx}: Please enter a location for the asset item {item_code}.").format(
					idx=row.idx,
					item_code=artech_engine.bold(row.item_code),
				)
			)

		item_data = artech_engine.get_cached_value(
			"Item", row.item_code, ["asset_naming_series", "asset_category"], as_dict=1
		)
		asset_quantity = row.qty if is_grouped_asset else 1
		purchase_amount = flt(row.valuation_rate) * asset_quantity

		asset = artech_engine.get_doc(
			{
				"doctype": "Asset",
				"item_code": row.item_code,
	...
	def update_fixed_asset(self, field, delete_asset=False):
		for d in self.get("items"):
			if d.is_fixed_asset:
				is_auto_create_enabled = artech_engine.db.get_value("Item", d.item_code, "auto_create_assets")
				assets = artech_engine.db.get_all("Asset", filters={field: self.name, "item_code": d.item_code})
				...
				for asset in assets:
					...
					if delete_asset and is_auto_create_enabled:
						...
						for movement in movements:
							artech_engine.delete_doc("Asset Movement", movement.name, force=1)
						artech_engine.delete_doc("Asset", asset.name, force=1)
						continue
					...
					if self.docstatus == 2:
						...
						if asset.docstatus == 1 and delete_asset:
							artech_engine.throw(
								...
								).format(asset_link=artech_engine.utils.get_link_to_form("Asset", asset.name))
							)
	...
	def delete_linked_asset(self):
		...
		asset_movement = artech_engine.db.get_value("Asset Movement", {"reference_name": self.name}, "name")
		artech_engine.delete_doc("Asset Movement", asset_movement, force=1)
...
def validate_item_type(doc, fieldname, message):
	...
	item_list = ", ".join(["%s" % artech_engine.db.escape(d) for d in items])
...
def get_purchase_expense_account(item_code, company):
	...
	return details or artech_engine._dict({})
