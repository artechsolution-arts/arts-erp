...
from artech_engine.core.doctype.user_permission.user_permission import add_user_permissions
from artech_engine.custom.doctype.property_setter.property_setter import make_property_setter

from artech.controllers import queries
...
class TestQueries(ArtechTestSuite):
	...
	def test_default_uoms(self):
		self.assertGreaterEqual(artech_engine.db.count("UOM", {"enabled": 1}), 10)
	...
	def test_employee_query_with_user_permissions(self):
		...
		with ArtechTestSuite.set_user(self, user.name):
			params = {
				"doctype": "Employee",
				"txt": "",
				"searchfield": "name",
