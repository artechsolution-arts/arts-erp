...
class TestItemWiseInventoryAccount(ArtechTestSuite):
	def setUp(self):
		self.company = make_company()
		self.company_abbr = artech_engine.db.get_value("Company", self.company, "abbr")
		self.default_warehouse = artech_engine.db.get_value(
			"Warehouse",
	...
	def test_item_account_for_delivery_note_entry(self):
		...
		dn = create_delivery_note(
			...
			warehouse=self.default_warehouse,
			company=self.company,
			cost_center=artech_engine.db.get_value("Company", self.company, "cost_center"),
			expense_account=artech_engine.db.get_value("Company", self.company, "default_expense_account"),
			do_not_submit=True,
		)
	...
	def test_item_group_account_for_purchase_receipt_entry(self):
		...
		inventory_account_dict = artech_engine._dict()
		for item_name, item_data in items.items():
			item_data = artech_engine._dict(item_data)
			make_item(
				item_name,
				properties=item_data,
			)

			item_group = artech_engine.get_doc("Item Group", item_data.item_group)
		...
		for item_group, account in inventory_account_dict.items():
			...
			sle_value = artech_engine.get_all(
				"Stock Ledger Entry",
				filters={
					"voucher_type": "Purchase Receipt",
					"voucher_no": pr.name,
	...
	def _make_item_group(self, item_name):
		if not artech_engine.db.exists("Item Group", item_name):
			item_group = artech_engine.get_doc(
				{
					"doctype": "Item Group",
		...
		return artech_engine.get_doc("Item Group", item_name)
	...
	def add_inventory_account(self, item, table_name=None):
		...
		if not artech_engine.db.exists("Account", account):
			account_doc = artech_engine.get_doc(
				{
					"doctype": "Account",
		...
		if not artech_engine.db.get_value("Item Default", {"parent": item.name, "company": self.company}):
			item.append(
				table_name,
				{
					"company": self.company,
	...
	def test_item_account_for_manufacture_entry(self):
		...
		wip_warehouse = artech_engine.db.get_value(
			"Warehouse",
			{"company": self.company, "is_group": 0, "warehouse_name": ("like", "%Work In Progress%")},
		)

		fg_warehouse = artech_engine.db.get_value(
			"Warehouse",
		...
		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Material Transfer for Manufacture", 5))
		stock_entry.submit()

		stock_entry = artech_engine.get_doc(make_stock_entry(wo_order.name, "Manufacture", 5))
		stock_entry.submit()
...
def make_company():
	...
	if artech_engine.db.exists("Company", company):
		return company
