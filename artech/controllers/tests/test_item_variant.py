...
from artech.controllers.item_variant import copy_attributes_to_variant, make_variant_item_code
from artech.stock.doctype.item.test_item import set_item_variant_settings
...
class TestItemVariant(ArtechTestSuite):
...
def make_item_variant():
	artech_engine.delete_doc_if_exists("Item", "_Test Variant Item-XSL", force=1)
...
def make_quality_inspection_template():
	...
	if artech_engine.db.exists("Quality Inspection Template", qc_template):
		return qc_template

	qc = artech_engine.new_doc("Quality Inspection Template")
