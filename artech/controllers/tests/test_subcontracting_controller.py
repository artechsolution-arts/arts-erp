...
class TestSubcontractingController(ArtechTestSuite):
	def setUp(self):
	...
	def test_item_with_batch_based_on_bom(self):
		...
		artech_engine.flags.add_debugger = True
		scr2 = make_subcontracting_receipt(sco.name)
		scr2.items[0].qty = 2
	...
	def test_incorrect_serial_no_components_based_on_material_transfer(self):
		...
		scr1 = make_subcontracting_receipt(sco.name)
		scr1.save()
		bundle = artech_engine.get_doc("Serial and Batch Bundle", scr1.supplied_items[0].serial_and_batch_bundle)
		original_serial_no = ""
		...
		self.assertRaises(artech_engine.ValidationError, bundle.save)
	...
	def test_co_by_product(self):
		...
		artech_engine.set_value("UOM", "Nos", "must_be_whole_number", 1)
	...
	for row in scr_doc.get("supplied_items"):
		if row.rm_item_code not in supplied_items:
			supplied_items.setdefault(
				row.rm_item_code, artech_engine._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def make_stock_in_entry(**args):
	...
	for row in args.rm_items:
		...
		if row.item_code not in items:
			items.setdefault(
				row.item_code, artech_engine._dict({"qty": 0, "serial_no": [], "batch_no": defaultdict(float)})
			)
...
def update_item_details(child_row, details):
	...
	if child_row.serial_and_batch_bundle:
		doc = artech_engine.get_doc("Serial and Batch Bundle", child_row.serial_and_batch_bundle)
...
def set_backflush_based_on(based_on):
	artech_engine.db.set_single_value("Buying Settings", "backflush_raw_materials_of_subcontract_based_on", based_on)


def get_subcontracting_order(**args):
	from artech.subcontracting.doctype.subcontracting_order.test_subcontracting_order import (
	...
	if args.get("po_name"):
		po = artech_engine.get_doc("Purchase Order", args.get("po_name"))
...
def make_subcontracted_item(**args):
	...
	if not artech_engine.db.exists("Item", args.item_code):
		make_item(
			args.item_code,
			{
				"is_stock_item": 1,
	...
	if not artech_engine.db.get_value("BOM", {"item": args.item_code}, "name"):
