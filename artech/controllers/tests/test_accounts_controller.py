...
from artech_engine.utils import add_days, getdate, nowdate
from artech_engine.utils.data import getdate as convert_to_date
...
def make_customer(customer_name, currency=None):
	if not artech_engine.db.exists("Customer", customer_name):
...
def make_supplier(supplier_name, currency=None):
	if not artech_engine.db.exists("Supplier", supplier_name):
		supplier = artech_engine.new_doc("Supplier")
...
class TestAccountsController(ArtechTestSuite):
	"""
	...
	def create_company(self):
		...
		if artech_engine.db.exists("Company", company_name):
			company = artech_engine.get_doc("Company", company_name)
		else:
			company = artech_engine.get_doc(
				{
					"doctype": "Company",
	...
	def create_account(self):
		...
		for x in accounts:
			if not artech_engine.db.get_value("Account", filters={"account_name": x.name, "company": self.company}):
				...
				acc = artech_engine.get_doc("Account", name)
			setattr(self, x.attribute_name, acc.name)

	def setup_advance_accounts_in_party_master(self):
		company = artech_engine.get_doc("Company", self.company)
		...
		supplier = artech_engine.get_doc("Supplier", self.supplier)
		supplier.append(
			"accounts",
			{
				"company": self.company,
	...
	def create_payment_reconciliation(self):
		pr = artech_engine.new_doc("Payment Reconciliation")
	...
	def test_15_gain_loss_on_different_posting_date(self):
		...
		self.assertEqual(
			artech_engine.db.get_value("Journal Entry", exc_je_for_si[0].parent, "posting_date"),
	...
	@ArtechTestSuite.change_settings("Stock Settings", {"allow_internal_transfer_at_arms_length_price": 1})
	def test_16_internal_transfer_at_arms_length_price(self):
		...
		artech_engine.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 0)
		si.items[0].rate = arms_length_price
		si.save()
		...
		artech_engine.db.set_single_value("Stock Settings", "allow_internal_transfer_at_arms_length_price", 1)
		pi = make_inter_company_purchase_invoice(si.name)
		pi.update_stock = 1
		pi.items[0].rate = arms_length_price
		pi.items[0].warehouse = target_warehouse
	...
	def test_17_gain_loss_posting_date_for_normal_payment(self):
		...
		self.assertEqual(
			getdate(nowdate()), artech_engine.db.get_value("Journal Entry", exc_je_for_pe[0].parent, "posting_date")
		)
		# Cancel Payment
		pe.reload()
		pe.cancel()
	...
	def test_18_fetch_taxes_based_on_taxes_and_charges_template(self):
		...
		if not artech_engine.db.exists("Sales Taxes and Charges Template", "_Test Tax - _TC"):
			doc = artech_engine.new_doc("Sales Taxes and Charges Template")
		...
		sinv = artech_engine.new_doc("Sales Invoice")
	...
	def test_40_cost_center_from_payment_entry(self):
		...
		cc = artech_engine.db.get_value("Company", self.company, "cost_center")
		artech_engine.db.set_value("Company", self.company, "cost_center", None)
		...
		artech_engine.db.set_value("Company", self.company, "cost_center", cc)
	...
	def test_difference_posting_date_in_pi_and_si(self):
		...
		jv = artech_engine.get_doc("Journal Entry", exc_je_for_si[0].parent)
		sales_invoice = artech_engine.get_doc("Sales Invoice", si.name)
		...
		journal_voucher = artech_engine.get_doc("Journal Entry", exc_je_for_pi[0].parent)
		purchase_invoice = artech_engine.get_doc("Purchase Invoice", pi.name)
	...
	def test_company_validation_in_dimension(self):
		...
		self.assertRaises(artech_engine.ValidationError, si_1.save)

	def test_party_billing_and_shipping_address(self):
		from artech.crm.doctype.prospect.test_prospect import make_address
	...
	def test_party_contact(self):
		from artech_engine.contacts.doctype.contact.test_contact import create_contact
	...
	@ArtechTestSuite.change_settings("Global Defaults", {"use_posting_datetime_for_naming_documents": 1})
	def test_document_naming_rule_based_on_posting_date(self):
		artech_engine.new_doc(
