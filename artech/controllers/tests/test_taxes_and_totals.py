...
class TestTaxesAndTotals(ArtechTestSuite):
	def test_regional_round_off_accounts(self):
		"""
		Regional overrides cannot extend the list in-place — the return
		value must be assigned back to artech_engine.flags.round_off_applicable_accounts.
		"""
		...
		with patch(
			"artech.controllers.taxes_and_totals.get_regional_round_off_accounts",
			mock_regional,
		):
			calculate_taxes_and_totals(so)

		self.assertIn(test_account, artech_engine.flags.round_off_applicable_accounts)
