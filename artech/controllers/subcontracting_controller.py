...
from artech.stock.serial_batch_bundle import SerialBatchCreation, get_serial_nos_from_bundle
...
class SubcontractingController(StockController):
	...
	def set_valuation_rate_for_rm(self):
		...
		if self.doctype == "Subcontracting Receipt":
			for row in self.supplied_items:
				...
				precision = artech_engine.get_precision("Subcontracting Receipt Supplied Item", "rate")
	...
	def validate_items(self):
		for item in self.items:
			is_stock_item, is_sub_contracted_item = artech_engine.get_value(
				"Item", item.item_code, ["is_stock_item", "is_sub_contracted_item"]
			)

			if not is_stock_item:
				artech_engine.throw(_("Row {0}: Item {1} must be a stock item.").format(item.idx, item.item_name))
			...
			if not item.get("type") and not item.get("is_legacy_scrap_item"):
				...
				if self.doctype != "Subcontracting Receipt":
					...
					order_item_field = artech_engine.scrub(order_item_doctype)
				...
				if item.bom:
					is_active, bom_item = artech_engine.get_value("BOM", item.bom, ["is_active", "item"])
	...
	def initialized_fields(self):
		self.available_materials = artech_engine._dict()
		self.__transferred_items = artech_engine._dict()
		self.alternative_item_details = artech_engine._dict()
	...
	def __get_received_items(self, doctype):
		...
		return artech_engine.get_all(f"{doctype}", fields=fields, filters=filters)
	...
	def __update_consumed_materials(self, doctype, return_consumed_items=False):
		...
		for row in consumed_materials:
			...
			consumed_bundles = voucher_bundle_data.get(bundle_key, artech_engine._dict())
	...
	def get_available_materials(self):
		...
		for row in transferred_items:
			...
			if not row.serial_no and not row.batch_no and voucher_bundle_data:
				...
				bundle_data = voucher_bundle_data.get(bundle_key, artech_engine._dict())
	...
	def __remove_serial_and_batch_bundle(self, item):
		if item.get("serial_and_batch_bundle"):
			artech_engine.delete_doc("Serial and Batch Bundle", item.serial_and_batch_bundle, force=True)
	...
	def _get_materials_from_bom(self, item_code, bom_no, exploded_item=0):
		...
		data = artech_engine.get_all("BOM", fields=fields, filters=filters, order_by=f"`tab{doctype}`.`idx`") or []
		to_remove = []
		for item in data:
	...
	def __update_reserve_warehouse(self, row, item):
		...
		elif artech_engine.get_cached_value("Item", row.rm_item_code, "is_customer_provided_item") and self.get(
			"customer_warehouse"
		):
	...
	def __get_batch_nos_for_bundle(self, qty, key):
		...
		precision = artech_engine.get_precision("Subcontracting Receipt Supplied Item", "consumed_qty")
	...
	def set_batch_for_supplied_items(self):
		from artech.stock.get_item_details import get_filtered_serial_nos
				...
				if batches:
					...
					for index, d in enumerate(batches):
						...
						if index == 0:
							...
							new_row.update(artech_engine.copy_doc(row).as_dict())
							new_row.update(
								{
									"consumed_qty": d.get("qty"),
	...
	def __get_qty_based_on_material_transfer(self, item_row, transfer_item):
		...
		if self.qty_to_be_received.get(key):
			...
			if transfer_item.serial_no or artech_engine.get_cached_value(
				"UOM", transfer_item.item_details.stock_uom, "must_be_whole_number"
			):
				return artech_engine.utils.ceil(qty)
	...
	def __set_rate_for_serial_and_batch_bundle(self):
		...
		for row in self.get(self.raw_material_table):
			...
			row.rate = artech_engine.get_cached_value(
	...
	def __validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.__transferred_items.get(key).get(
			...
			msg = f'The Batch No {artech_engine.bold(row.get("batch_no"))} has not supplied against the {self.subcontract_data.order_doctype} {link}'
			artech_engine.throw(_(msg), title=_("Incorrect Batch Consumed"))
	...
	def __validate_serial_no(self, row, key):
		if row.get("serial_and_batch_bundle") and self.__transferred_items.get(key).get("serial_no"):
			...
			if incorrect_sn:
				...
				artech_engine.throw(_(msg), title=_("Incorrect Serial Number Consumed"))
	...
	def update_ordered_and_reserved_qty(self):
		...
		for sco, sco_item_rows in sco_map.items():
			if sco and sco_item_rows:
				sco_doc = artech_engine.get_doc("Subcontracting Order", sco)
		...
		for item in self.get("supplied_items"):
			if item.reference_name == item_row_id:
				if (
					...
					and artech_engine.get_cached_value("Item", item.rm_item_code, "is_stock_item")
				):
	...
	def update_requested_qty(self):
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				mr_obj = artech_engine.get_doc("Material Request", mr)
...
def get_pending_subcontracted_quantity(doctype, name):
	table = artech_engine.qb.DocType(doctype)
	query = (
...
def make_rm_stock_entry(
	...
	if subcontract_order:
		subcontract_order = artech_engine.get_doc(order_doctype, subcontract_order)

		if not rm_items:
			if not subcontract_order.supplied_items:
				artech_engine.throw(_("No item available for transfer."))
		...
		if fg_item_code_list:
			...
			def post_process(source_doc, target_doc):
				...
				over_transfer_allowance = artech_engine.get_single_value(
							...
							if qty <= 0 and rm_item.get("total_supplied_qty"):
								per_transferred = (
									flt(
										...
										artech_engine.db.get_default("float_precision"),
									)
									* 100
								)
			...
			artech_engine.throw(_("No Items selected for transfer."))
...
def get_materials_from_supplier(
	...
	doc = artech_engine.get_cached_doc(order_doctype, subcontract_order)
