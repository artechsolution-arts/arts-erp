...
from artech_engine.model.document import Document, bulk_insert
from artech_engine.query_builder import functions
from artech_engine.utils import cint, flt, round_based_on_smallest_currency_fraction

import artech
from artech.accounts.doctype.journal_entry.journal_entry import get_exchange_rate
from artech.accounts.doctype.pricing_rule.utils import get_applied_pricing_rules
from artech.controllers.accounts_controller import (
	validate_conversion_rate,
	validate_inclusive_tax,
	validate_taxes_and_charges,
)
from artech.deprecation_dumpster import deprecated
...
class calculate_taxes_and_totals:
	def __init__(self, doc: Document):
		...
		artech_engine.flags.round_row_wise_tax = artech_engine.get_single_value("Accounts Settings", "round_row_wise_tax")
	...
	def validate_conversion_rate(self):
		...
		company_currency = artech.get_company_currency(self.doc.company)
	...
	def calculate_item_values(self):
		...
		if not self.discount_amount_applied:
			bill_for_rejected_quantity_in_purchase_invoice = artech_engine.get_single_value(
	...
	def calculate_shipping_charges(self):
		...
		if hasattr(self.doc, "shipping_rule") and self.doc.shipping_rule:
			shipping_rule = artech_engine.get_doc("Shipping Rule", self.doc.shipping_rule)
	...
	def round_off_totals(self, tax):
		if tax.account_head in artech_engine.flags.round_off_applicable_accounts:
	...
	def set_discount_amount(self):
		...
		if self.doc.get("is_return") and self.doc.get("return_against"):
			doctype = artech_engine.qb.DocType(self.doc.doctype)
	...
	def apply_discount_amount(self):
		if self.doc.discount_amount:
			if not self.doc.apply_discount_on:
				artech_engine.throw(_("Please select Apply Discount On"))
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice"]:
			...
			if (
				...
				write_off_limit = flt(
					artech_engine.db.get_value("POS Profile", self.doc.pos_profile, "write_off_limit")
				)
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		...
		if pending_amount > 0:
			default_mode_of_payment = artech_engine.db.get_value(
				"POS Payment Method",
...
def get_itemised_tax_breakup_html(doc):
	...
	return artech_engine.render_template(
...
def get_itemised_tax_breakup_data(doc):
	...
	for item_code, taxes in itemised_tax.items():
		...
		itemised_tax_data.append(artech_engine._dict({"item": item_code, "taxable_amount": taxable_amount, **taxes}))
...
def get_itemised_tax(doc, with_tax_account=False):
	...
	for row in doc.get("_item_wise_tax_details"):
		...
		tax_info = itemised_tax.setdefault(item_code, artech_engine._dict()).setdefault(
			tax.description,
			artech_engine._dict(
				{
					"tax_amount": 0.0,
...
def get_rounding_tax_settings():
	return artech_engine.get_single_value("Accounts Settings", "round_row_wise_tax")
	...
	def set_account_currency(self):
		...
		for d in self.doc.get(self.tax_field):
			if not d.account_currency:
				account_currency = artech_engine.get_cached_value("Account", d.expense_account, "account_currency")
