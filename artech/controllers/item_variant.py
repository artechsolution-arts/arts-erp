...
from artech.utilities.product import get_item_codes_by_attributes


class ItemVariantExistsError(artech_engine.ValidationError):
	pass


class InvalidItemAttributeValueError(artech_engine.ValidationError):
	pass


class ItemTemplateCannotHaveStock(artech_engine.ValidationError):
	pass
...
def get_variant(
	...
	"""
	item_template = artech_engine.get_doc("Item", template)
	...
	if not attribute_args:
		artech_engine.throw(_("Please specify at least one attribute in the Attributes table"))
...
def make_variant_based_on_manufacturer(template, manufacturer, manufacturer_part_no):
	...
	variant = artech_engine.new_doc("Item")
	...
	if not artech_engine.db.exists("Item Manufacturer", {"item_code": variant.name, "manufacturer": manufacturer}):
		manufacturer_doc = artech_engine.new_doc("Item Manufacturer")
...
def validate_item_variant_attributes(item, args=None):
	if isinstance(item, str):
		item = artech_engine.get_doc("Item", item)
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if increment == 0:
		...
		artech_engine.throw(_("Increment for Attribute {0} cannot be 0").format(attribute))
...
def validate_item_attribute_value(attributes_list, attribute, attribute_value, item, from_variant=True):
	allow_rename_attribute_value = artech_engine.db.get_single_value(
		...
		if from_variant:
			artech_engine.throw(
				_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
					artech_engine.bold(attribute_value), artech_engine.bold(attribute), artech_engine.bold(item)
				),
				InvalidItemAttributeValueError,
			...
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				artech_engine.bold(attribute_value), artech_engine.bold(item)
			)
			msg += "<br>" + _(
				"To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings."
			).format(artech_engine.bold(_("Allow Rename Attribute Value")))

			artech_engine.throw(msg, InvalidItemAttributeValueError, title=_("Edit Not Allowed"))


def get_attribute_values(item):
	if not artech_engine.flags.attribute_values:
		attribute_values = {}
		numeric_values = {}
		for t in artech_engine.get_all("Item Attribute Value", fields=["parent", "attribute_value"]):
			attribute_values.setdefault(t.parent.lower(), []).append(t.attribute_value)

		for t in artech_engine.get_all(
		...
		artech_engine.flags.attribute_values = attribute_values
		artech_engine.flags.numeric_values = numeric_values

	return artech_engine.flags.attribute_values, artech_engine.flags.numeric_values
...
def find_variant(template, args, variant_item_code=None):
	...
	for variant in possible_variants:
		variant = artech_engine.get_doc("Item", variant)
...
def create_variant(item: str, args: dict | str, use_template_image: bool = False):
	use_template_image = artech_engine.parse_json(use_template_image)
	if isinstance(args, str):
		args = json.loads(args)

	template = artech_engine.get_doc("Item", item)
	variant = artech_engine.new_doc("Item")
...
def enqueue_multiple_variant_creation(item: str, args: dict | str, use_template_image: bool = False):
	...
	if total_variants >= 600:
		artech_engine.throw(_("Please do not create more than 500 items at a time"))
		return
	if total_variants < 10:
		return create_multiple_variants(item, args, use_template_image)
	else:
		artech_engine.enqueue(
			"artech.controllers.item_variant.create_multiple_variants",
			item=item,
			args=args,
...
def create_multiple_variants(item, args, use_template_image=False):
	...
	template_item = artech_engine.get_doc("Item", item)
...
def copy_attributes_to_variant(item, variant):
	...
	allow_fields = [d.field_name for d in artech_engine.get_all("Variant Field", fields=["field_name"])]
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		item_attribute = artech_engine.db.sql(
		...
		if not item_attribute:
			continue
			# artech_engine.throw(_('Invalid attribute {0} {1}').format(artech_engine.bold(attr.attribute),
			# 	artech_engine.bold(attr.attribute_value)), title=_('Invalid Attribute'),
...
def create_variant_doc_for_quick_entry(template: str, args: dict | str):
	variant_based_on = artech_engine.db.get_value("Item", template, "variant_based_on")
	args = json.loads(args)
