...
from artech_engine.utils import DateTimeLikeObject, getdate, today
...
def validate_filters(filters):
	...
	if not filters.get("company"):
		filters["company"] = artech_engine.defaults.get_user_default("Company")
	for f in ["Fiscal Year", "Based On", "Period", "Company"]:
		if not filters.get(f.lower().replace(" ", "_")):
			artech_engine.throw(_("{0} is mandatory").format(_(f)))

	if not artech_engine.db.exists("Fiscal Year", filters.get("fiscal_year")):
		artech_engine.throw(_("Fiscal Year {0} Does Not Exist").format(filters.get("fiscal_year")))

	if filters.get("based_on") == filters.get("group_by"):
		artech_engine.throw(_("'Based On' and 'Group By' can not be same"))
	...
	if filters.get("period_based_on") and filters.period_based_on not in ["bill_date", "posting_date"]:
		artech_engine.throw(
			msg=_("{0} can be either {1} or {2}.").format(
				artech_engine.bold("Period based On"), artech_engine.bold("Posting Date"), artech_engine.bold("Billing Date")
			),
...
def get_data(filters, conditions):
	...
	year_start_date, year_end_date = artech_engine.get_cached_value(
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		...
		if trans in ["Sales Invoice", "Delivery Note", "Sales Order"]:
			...
			artech_engine.throw(_("Project-wise data is not available for Quotation"))
