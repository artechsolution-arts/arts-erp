...
from artech.accounts.party import render_address
from artech.controllers.accounts_controller import get_taxes_and_charges
from artech.controllers.sales_and_purchase_return import get_rate_for_return, is_batch_expired
from artech.controllers.stock_controller import StockController
from artech.stock.doctype.item.item import set_item_default
from artech.stock.get_item_details import get_bin_details, get_conversion_factor
from artech.stock.utils import get_combine_datetime, get_incoming_rate, get_valuation_method
...
class SellingController(StockController):
	...
	def onload(self):
		...
		if (
			self.get("company")
			and (
				default_selling_terms := artech_engine.get_value(
	...
	def validate_standalone_serial_nos_customer(self):
		...
		if self.doctype in ["Sales Invoice", "Delivery Note"]:
			...
			if serial_nos := artech_engine.get_all(
				"Serial No",
				...
				for sn in serial_nos:
					if sn.customer and sn.customer != self.customer:
						artech_engine.throw(
							...
							).format(artech_engine.bold(sn.name), artech_engine.bold(sn.customer)),
	...
	def set_missing_lead_customer_details(self, for_validate=False):
		...
		if customer:
			from artech.accounts.party import _get_party_details
			...
			from artech.crm.doctype.lead.lead import get_lead_details
	...
	def set_company_contact_person(self):
		...
		if self.company and self.meta.has_field("company_contact_person") and not self.company_contact_person:
			self.company_contact_person = artech_engine.get_cached_value(
				"Company", self.company, "default_sales_contact"
			)

	def remove_shipping_charge(self):
		if self.shipping_rule:
			shipping_rule = artech_engine.get_last_doc("Shipping Rule", self.shipping_rule)
	...
	def validate_sales_team(self, sales_team):
		...
		sales_person_status = artech_engine.db.get_all(
			"Sales Person", filters={"name": ["in", sales_persons]}, fields=["name", "enabled"]
		)

		for row in sales_person_status:
			if not row.enabled:
				artech_engine.throw(_("Sales Person <b>{0}</b> is disabled.").format(row.name))

	def validate_max_discount(self):
		for d in self.get("items"):
			if d.item_code:
				discount = flt(artech_engine.get_cached_value("Item", d.item_code, "max_discount"))

				if discount and flt(d.discount_percentage) > discount:
					artech_engine.throw(_("Maximum discount for Item {0} is {1}%").format(d.item_code, discount))
		...
		for d in self.get("items"):
			if d.meta.get_field("stock_qty"):
				if not d.conversion_factor:
					artech_engine.throw(_("Row {0}: Conversion Factor is mandatory").format(d.idx))
	...
	def validate_selling_price(self):
		def throw_message(idx, item_name, rate, ref_rate_field):
			throw(
				_(
					...
					bold("net rate"),
					bold(rate),
					bold(artech_engine.get_meta("Selling Settings").get_label("validate_selling_price")),
					get_link_to_form("Selling Settings"),
				),
				title=_("Invalid Selling Price"),
			)

		if self.get("is_return") or not artech_engine.get_single_value("Selling Settings", "validate_selling_price"):
			return
		...
		for item in self.items:
			...
			last_purchase_rate, is_stock_item = artech_engine.get_cached_value(
	...
	def get_already_delivered_qty(self, current_docname, so, so_detail):
		delivered_via_dn = artech_engine.db.sql(
		...
		delivered_via_si = artech_engine.db.sql(
	...
	def check_sales_order_on_hold_or_close(self, ref_fieldname):
		for d in self.get("items"):
			if d.get(ref_fieldname):
				status = artech_engine.db.get_value("Sales Order", d.get(ref_fieldname), "status")
				if status in ("Closed", "On Hold") and not self.is_return:
					artech_engine.throw(_("Sales Order {0} is {1}").format(d.get(ref_fieldname), status))
	...
	def update_reserved_qty(self):
		...
		for so, so_item_rows in so_map.items():
			if so and so_item_rows:
				sales_order = artech_engine.get_lazy_doc("Sales Order", so)
	...
	def set_incoming_rate(self):
		...
		from artech.stock.serial_batch_bundle import get_batch_nos, get_serial_nos
		...
		for d in items:
			if not artech_engine.get_cached_value("Item", d.item_code, "is_stock_item"):
				continue
	...
	def update_stock_ledger(self, allow_negative_stock=False):
		...
		for d in self.get_item_list():
			if artech_engine.get_cached_value("Item", d.item_code, "is_stock_item") == 1 and flt(d.qty):
	...
	def get_po_nos(self, ref_doctype, ref_fieldname, po_nos):
		...
		if doc_list:
			po_nos += [
				d.po_no
				for d in artech_engine.get_all(ref_doctype, "po_no", filters={"name": ("in", doc_list)})
				if d.get("po_no")
			]
	...
	def validate_for_duplicate_items(self):
		...
		if cint(artech_engine.get_single_value("Selling Settings", "allow_multiple_items")):
			return
		...
		item_stock_map = artech_engine._dict(
		...
		for d in self.get("items"):
			...
			duplicate_items_msg = _("Item {0} entered multiple times.").format(artech_engine.bold(d.item_code))
			duplicate_items_msg += "<br><br>"
			duplicate_items_msg += _("Please enable {} in {} to allow same item in multiple rows").format(
				artech_engine.bold(_("Allow Item to Be Added Multiple Times in a Transaction")),
				get_link_to_form("Selling Settings", "Selling Settings"),
			)
			if item_stock_map.get(d.item_code):
				if stock_items in check_list:
					artech_engine.throw(duplicate_items_msg)
				else:
	...
	def validate_target_warehouse(self):
		...
		for d in items:
			if d.get("target_warehouse") and d.get("warehouse") == d.get("target_warehouse"):
				warehouse = artech_engine.bold(d.get("target_warehouse"))
				artech_engine.throw(
					_(
		...
		if not self.get("is_internal_customer") and any(d.get("target_warehouse") for d in items):
			...
			artech_engine.msgprint(msg, title="Internal Transfer", alert=True)

	def validate_items(self):
		# validate items to see if they have is_sales_item enabled
		from artech.controllers.buying_controller import validate_item_type
	...
	def update_stock_reservation_entries(self) -> None:
		...
		if self._action == "cancel":
			for item in self.get("items"):
				...
				sre_list = artech_engine.db.get_all(
...
def set_default_income_account_for_item(obj):
	...
	"""
	company_default = artech_engine.get_cached_value("Company", obj.company, "default_income_account")
...
def get_serial_and_batch_bundle(child, parent, delivery_note_child=None):
	...
	if not artech_engine.get_single_value("Stock Settings", "auto_create_serial_and_batch_bundle_for_outward"):
		return

	item_details = artech_engine.db.get_value("Item", child.item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
