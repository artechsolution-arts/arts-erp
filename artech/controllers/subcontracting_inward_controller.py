...
from artech.stock.serial_batch_bundle import get_serial_batch_list_from_item
...
class SubcontractingInwardController:
	...
	def validate_material_receipt(self):
		...
		for item in self.items:
			if not artech_engine.get_cached_value("Item", item.item_code, "is_customer_provided_item"):
				artech_engine.throw(
	...
	def validate_returns(self):
		for item in self.items:
			if not item.scio_detail:
				...
				or artech_engine.get_cached_value("Subcontracting Inward Order Item", item.scio_detail, "item_code")
			):
				artech_engine.throw(
	...
	def validate_material_transfer(self):
		customer_warehouse = artech_engine.get_cached_value(
	...
	def validate_manufacture(self):
		if next(item for item in self.items if item.is_finished_item).t_warehouse != (
			fg_warehouse := artech_engine.get_cached_value("Work Order", self.work_order, "fg_warehouse")
		):
			artech_engine.throw(
				_(
		...
		items = [
			...
			and artech_engine.get_cached_value("Item", item.item_code, "is_customer_provided_item")
		]

		customer_warehouse = artech_engine.get_cached_value(
			"Subcontracting Inward Order", self.subcontracting_inward_order, "customer_warehouse"
		)
		if artech_engine.get_cached_value("Work Order", self.work_order, "skip_transfer"):
			...
			rm_item_dict = artech_engine._dict(
				{
					d.rm_item_code: artech_engine._dict(
	...
	def validate_warehouse_(self):
		if self.subcontracting_inward_order and self.purpose in [
			...
			for item in self.items:
				if self.purpose == "Material Transfer for Manufacture" and not artech_engine.get_cached_value(
	...
	def get_serial_nos_and_batches_from_sres(self, scio_detail, only_pending=True):
		serial_nos, batch_nos = [], artech_engine._dict()
			...
			if self._action in ["save", "submit"]:
				...
				for item in self.items:
					if not item.type and not item.is_legacy_scrap_item:
						delivered_qty, returned_qty = artech_engine.get_value(
							"Subcontracting Inward Order Item",
							item.scio_detail,
	...
	def validate_delivery_on_save(self):
		allow_delivery_of_overproduced_qty = artech_engine.get_single_value(
		...
		for item in self.items:
			...
			table = artech_engine.qb.DocType("Subcontracting Inward Order Item")
			query = (
				artech_engine.qb.from_(table)
				.select(
					(
			...
			if max_allowed_qty:
				...
				table = artech_engine.qb.DocType("Subcontracting Inward Order Secondary Item")
				query = (
					artech_engine.qb.from_(table)
	...
	def validate_receive_from_customer_cancel(self):
		if self.purpose == "Receive from Customer":
			for item in self.items:
				scio_rm_item = artech_engine.get_value(
					"Subcontracting Inward Order Received Item",
					item.scio_detail,
					["received_qty", "returned_qty", "work_order_qty"],
					as_dict=True,
	...
	def validate_manufacture_entry_cancel(self):
		if self.subcontracting_inward_order and self.purpose == "Manufacture":
			fg_item_name = artech_engine.get_cached_value(
				"Work Order", self.work_order, "subcontracting_inward_order_item"
			)
			produced_qty, delivered_qty = artech_engine.get_value(
			...
			for item in [item for item in self.items if not item.is_finished_item]:
				if item.type or item.is_legacy_scrap_item:
					scio_secondary_item = artech_engine.get_value(
	...
	def update_inward_order_item(self):
		if self.purpose == "Manufacture" and (
			scio_item_name := artech_engine.get_cached_value(
				"Work Order", self.work_order, "subcontracting_inward_order_item"
			)
		):
			if scio_item_name:
			...
			for item in self.items:
				...
				artech_engine.db.set_value(
					...
					item.scio_detail,
					fieldname,
					artech_engine.get_value(doctype, item.scio_detail, fieldname)
		...
		if self.subcontracting_inward_order:
			match self.purpose:
				...
				case "Return Raw Material to Customer":
					...
					artech_engine.qb.update(table).set(table.returned_qty, case_expr).where(
		...
		for item in self.items:
			if item.scio_detail:
				...
				scio_rm = artech_engine.new_doc(
					...
					main_item_code=artech_engine.get_cached_value(
						"Subcontracting Inward Order Item", item.against_fg, "item_code"
					),
				)
		...
		if data:
			...
			for d in result:
				...
				if not d.required_qty and not d.received_qty:
					...
					artech_engine.delete_doc("Subcontracting Inward Order Received Item", d.name)
				else:
			...
			if final_list := list(set(data.keys()) - set(deleted_docs)):
				artech_engine.qb.update(table).set(table.received_qty, case_expr_qty).set(
	...
	def update_inward_order_received_items_for_manufacture(self):
		...
		item_code_wh = artech_engine._dict(
			{
				(
					item.item_code,
					customer_warehouse
					if artech_engine.get_cached_value("Item", item.item_code, "is_customer_provided_item")
		...
		if data := data.run(as_dict=True):
			...
			if final_list := list(set([d.name for d in data]) - set(deleted_docs)):
				artech_engine.qb.update(table).set(table.consumed_qty, case_expr).where(
			...
			for extra_item in [
				...
				doc = artech_engine.new_doc(
					...
					reference_name=artech_engine.get_cached_value(
						"Work Order", self.work_order, "subcontracting_inward_order_item"
					),
					required_qty=0,
	...
	def update_inward_order_secondary_items(self):
		if (scio := self.subcontracting_inward_order) and self.purpose == "Manufacture":
			...
			secondary_items = artech_engine._dict(secondary_items)
			...
			if secondary_items:
				...
				if result:
					secondary_items_dict = artech_engine._dict(
						{
							(d.item_code, d.warehouse): artech_engine._dict(
					...
					for key, value in secondary_items_dict.items():
						if (
							...
							artech_engine.delete_doc("Subcontracting Inward Order Secondary Item", value.name)
						else:
					...
					if final_list := list(
						...
						artech_engine.qb.update(table).set(table.produced_qty, case_expr).where(
							(table.name.isin(final_list)) & (table.docstatus == 1)
						).run()
				...
				for secondary_item in [
					...
					doc = artech_engine.new_doc(
						...
						idx=artech_engine.db.count("Subcontracting Inward Order Secondary Item", {"parent": scio})
						+ 1,
						...
						delivered_qty=0,
						reference_name=artech_engine.get_value(
							"Work Order", self.work_order, "subcontracting_inward_order_item"
						),
					)
					doc.flags.skip_docstatus_validation = True
	...
	def remove_reference_for_additional_items(self):
		if self.subcontracting_inward_order:
			items = [
				...
				if item.scio_detail
				and (
					not artech_engine.db.exists("Subcontracting Inward Order Received Item", item.scio_detail)
					and not artech_engine.db.exists("Subcontracting Inward Order Item", item.scio_detail)
					and not artech_engine.db.exists("Subcontracting Inward Order Secondary Item", item.scio_detail)
				)
			]
			for item in items:
	...
	def create_stock_reservation_entries_for_inward(self):
		if self.purpose == "Receive from Customer":
			for item in self.items:
				...
				sre.has_serial_no = artech_engine.get_cached_value("Item", item.item_code, "has_serial_no")
				sre.has_batch_no = artech_engine.get_cached_value("Item", item.item_code, "has_batch_no")
				sre.reservation_based_on = "Qty" if not item.serial_and_batch_bundle else "Serial and Batch"
				if item.serial_and_batch_bundle:
					sabb = artech_engine.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
		...
		if self.purpose == "Return Raw Material to Customer":
			for item in self.items:
				...
				if serial_list or batch_list:
					...
					for row in result:
						if serial_list:
							artech_engine.get_doc("Serial and Batch Entry", row.sbe_name).db_set(
							...
							sbe_doc = artech_engine.get_doc("Serial and Batch Entry", row.sbe_name)
							sbe_doc.db_set(
								"delivered_qty",
					...
					for sre_name, qty in qty_to_deliver.items():
						sre_doc = artech_engine.get_doc("Stock Reservation Entry", sre_name)
						sre_doc.db_set(
							"delivered_qty",
					...
					for sre in sre_list:
						...
						sre_doc = artech_engine.get_doc("Stock Reservation Entry", sre.name)
						sre_doc.db_set(
							"delivered_qty",
							sre_doc.delivered_qty + (qty if self._action == "submit" else -qty),
						)
