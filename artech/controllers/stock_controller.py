...
from artech_engine.utils import cint, cstr, flt, get_link_to_form, getdate

import artech
from artech.accounts.general_ledger import (
	make_gl_entries,
	make_reverse_gl_entries,
	process_gl_map,
)
from artech.accounts.utils import cancel_exchange_gain_loss_journal, get_fiscal_year
from artech.controllers.accounts_controller import AccountsController
from artech.controllers.sales_and_purchase_return import (
...
from artech.stock.doctype.inventory_dimension.inventory_dimension import (
...
from artech.stock.stock_ledger import get_items_to_be_repost


class QualityInspectionRequiredError(artech_engine.ValidationError):
	pass


class QualityInspectionRejectedError(artech_engine.ValidationError):
	pass


class QualityInspectionNotSubmittedError(artech_engine.ValidationError):
	pass


class BatchExpiredError(artech_engine.ValidationError):
	pass
...
class StockController(AccountsController):
	...
	def validate_warehouse_of_sabb(self):
		...
		for row in self.items:
			...
			sabb_details = artech_engine.db.get_value(
			...
			if sabb_details.has_serial_no and doc_before_save and doc_before_save.get("items"):
				...
				if prev_row and prev_row[0].serial_and_batch_bundle != row.serial_and_batch_bundle:
					sabb_doc = artech_engine.get_doc("Serial and Batch Bundle", row.serial_and_batch_bundle)
		...
		if self.doctype in [
			...
			for item in self.get("items"):
				if (
					...
					and artech_engine.get_cached_value("Item", item.item_code, "is_stock_item")
				):
					artech_engine.toast(
	...
	def validate_items_exist(self):
		...
		exists_items = artech_engine.get_all("Item", filters={"name": ("in", items)}, pluck="name")
		non_exists_items = set(items) - set(exists_items)

		if non_exists_items:
			artech_engine.throw(_("Items {0} do not exist in the Item master.").format(", ".join(non_exists_items)))
			...
			if data:
				...
				artech_engine.throw(
					_("Serial and Batch Bundle {0} is already used in {1} {2}.").format(
						artech_engine.bold(data.serial_and_batch_bundle), data.voucher_type, data.voucher_no
					)
				)

	def get_item_wise_inventory_account_map(self):
		inventory_account_map = artech_engine._dict()
	...
	def use_item_inventory_account(self):
		return artech_engine.get_cached_value("Company", self.company, "enable_item_wise_inventory_account")

	def get_inventory_account_dict(self, row, inventory_account_map, warehouse_field=None):
		account_dict = artech_engine._dict()
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False, via_landed_cost_voucher=False):
		...
		if (
			cint(artech.is_perpetual_inventory_enabled(self.company))
	...
	def validate_serialized_batch(self):
		...
		for d in self.get("items"):
			...
			if flt(d.qty) > 0.0 and d.get("batch_no") and self.get("posting_date") and self.docstatus < 2:
				expiry_date = artech_engine.get_cached_value("Batch", d.get("batch_no"), "expiry_date")
	...
	def clean_serial_nos(self):
		from artech.stock.doctype.serial_no.serial_no import clean_serial_no_string
	...
	def make_bundle_using_old_serial_batch_fields(self, table_name=None, via_landed_cost_voucher=False):
		...
		if artech_engine.in_test and artech_engine.flags.use_serial_and_batch_fields:
			return
		...
		parent_details = artech_engine._dict()
				...
				if not data.get("qty"):
					artech_engine.throw(
						_("For the {0}, no stock is available for the return in the warehouse {1}.").format(
							artech_engine.bold(row.item_code), row.get(warehouse_field)
						)
					)
		...
		for row in self.get(table_name):
			...
			if data := available_dict.get(value):
				...
				if self.doctype in ["Sales Invoice", "Delivery Note"]:
					row.db_set(
						"incoming_rate", artech_engine.db.get_value("Serial and Batch Bundle", bundle, "avg_rate")
					)
	...
	def is_serial_batch_item(self, item_code) -> bool:
		if not artech_engine.db.exists("Item", item_code):
			artech_engine.throw(_("Item {0} does not exist.").format(bold(item_code)))

		item_details = artech_engine.db.get_value("Item", item_code, ["has_serial_no", "has_batch_no"], as_dict=1)
	...
	def update_bundle_details(self, bundle_details, table_name, row, is_rejected=False, parent_details=None):
		...
		if is_rejected:
			...
			qty = flt(
				...
				artech_engine.get_precision("Serial and Batch Entry", "qty"),
			)
		...
		bundle_details.update(
			{
				...
				"warehouse": warehouse,
				"batches": artech_engine._dict({row.batch_no: qty}) if row.batch_no else None,
	...
	def create_serial_batch_bundle(self, bundle_details, row):
		from artech.stock.serial_batch_bundle import SerialBatchCreation
	...
	def set_use_serial_batch_fields(self):
		if artech_engine.get_single_value("Stock Settings", "use_serial_batch_fields"):
			for row in self.items:
				row.use_serial_batch_fields = 1
	...
	def get_gl_entries(
		...
		for item_row in voucher_details:
			...
			if abs(sle_rounding_diff) > (1.0 / (10**precision)) and self.is_internal_transfer():
				...
				expense_account = artech_engine.get_cached_value("Company", self.company, "default_expense_account")
		...
		if warehouse_with_no_account:
			for wh in warehouse_with_no_account:
				if artech_engine.get_cached_value("Warehouse", wh, "company"):
					artech_engine.throw(
						_(
		...
		return process_gl_map(
			gl_list, precision=precision, from_repost=artech_engine.flags.through_repost_item_valuation
		)

	def get_debit_field_precision(self):
		if not artech_engine.flags.debit_field_precision:
			artech_engine.flags.debit_field_precision = artech_engine.get_precision("GL Entry", "debit_in_account_currency")

		return artech_engine.flags.debit_field_precision

	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		if self.doctype == "Stock Reconciliation":
			reconciliation_purpose = artech_engine.db.get_value(self.doctype, self.name, "purpose")
	...
	def get_stock_ledger_details(self):
		...
		table = artech_engine.qb.DocType("Stock Ledger Entry")
		...
		if not item.get("expense_account"):
			...
			artech_engine.throw(
				_("Row #{0}: Expense Account not set for the Item {1}. {2}").format(
					item.idx, artech_engine.bold(item.item_code), msg
				),
			...
			is_expense_account = (
				artech_engine.get_cached_value("Account", item.get("expense_account"), "report_type")
				== "Profit and Loss"
			)
			if (
				self.doctype
	...
	def set_serial_and_batch_bundle(self, table_name=None, ignore_validate=False):
		...
		for row in self.get(table_name):
			for field in QTY_FIELD.keys():
				if row.get(field):
					artech_engine.get_doc("Serial and Batch Bundle", row.get(field)).set_serial_and_batch_values(
	...
	def get_sl_entries(self, d, args):
		sl_dict = artech_engine._dict(
			{
				...
				"stock_uom": artech_engine.get_cached_value(
		...
		if self.docstatus == 2:
			from artech.deprecation_dumpster import deprecation_warning
	...
	def set_landed_cost_voucher_amount(self):
		for d in self.get("items"):
			lcv_item = artech_engine.qb.DocType("Landed Cost Item")
			query = (
				artech_engine.qb.from_(lcv_item)
	...
	def get_item_account_wise_lcv_entries(self):
		...
		landed_cost_vouchers = artech_engine.get_all(
		...
		for lcv in landed_cost_vouchers:
			landed_cost_voucher_doc = artech_engine.get_doc("Landed Cost Voucher", lcv.parent)

			based_on_field = "applicable_charges"
			# Use amount field for total item cost for manually cost distributed LCVs
			if landed_cost_voucher_doc.distribute_charges_based_on != "Distribute Manually":
				based_on_field = artech_engine.scrub(landed_cost_voucher_doc.distribute_charges_based_on)
			...
			if not sl_dict.get(dimension.target_fieldname) and dimension.fetch_from_parent:
				...
				if not sl_dict.get(dimension.target_fieldname):
					fieldname = next(
						(
							field.fieldname
							for field in artech_engine.get_meta(self.doctype).fields
	...
	def make_sl_entries(self, sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
		from artech.stock.serial_batch_bundle import update_batch_qty
		from artech.stock.stock_ledger import make_sl_entries
	...
	def validate_reserved_batches(self):
		if not artech_engine.db.get_single_value("Stock Settings", "enable_stock_reservation"):
			return
				...
				for row in reserved_batches_data:
					...
					artech_engine.throw(
						_(
							...
							artech_engine.bold(row.batch_no),
							artech_engine.bold(row.voucher_type),
							artech_engine.bold(row.voucher_no),
							artech_engine.bold(self.doctype),
							artech_engine.bold(self.name),
							artech_engine.bold(field[0]),
							artech_engine.bold(value),
						),
	...
	def get_reserved_batches(self, batches):
		...
		child_doc = artech_engine.qb.DocType("Serial and Batch Entry")
	...
	def make_gl_entries_on_cancel(self, from_repost=False):
		if not from_repost:
			cancel_exchange_gain_loss_journal(artech_engine._dict(doctype=self.doctype, name=self.name))
	...
	def get_serialized_items(self):
		...
		if item_codes:
			serialized_items = artech_engine.db.sql_list(
	...
	def validate_warehouse(self):
		from artech.stock.utils import validate_disabled_warehouse, validate_warehouse_company
	...
	def validate_inspection(self):
		...
		for row in self.get("items"):
			qi_required = False
			if inspection_required_fieldname and artech_engine.get_cached_value(
			...
			if qi_required:  # validate row only if inspection is required on item level
				if self.doctype in [
					...
					"Sales Invoice",
					"Delivery Note",
				] and artech_engine.get_single_value(
	...
	def validate_qi_presence(self, row):
		...
		if not row.quality_inspection:
			...
			if self.docstatus == 1:
				artech_engine.throw(msg, title=_("Inspection Required"), exc=QualityInspectionRequiredError)
			else:
				artech_engine.msgprint(msg, title=_("Inspection Required"), indicator="blue")

	def validate_qi_submission(self, row):
		"""Check if QI is submitted on row level, during submission"""
		action = artech_engine.get_single_value("Stock Settings", "action_if_quality_inspection_is_not_submitted")
		qa_docstatus = artech_engine.db.get_value("Quality Inspection", row.quality_inspection, "docstatus")

		if qa_docstatus != 1:
			link = artech_engine.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = _("Row #{0}: Quality Inspection {1} is not submitted for the item: {2}").format(
				row.idx, link, row.item_code
			)
			if action == "Stop":
				artech_engine.throw(msg, title=_("Inspection Submission"), exc=QualityInspectionNotSubmittedError)
			else:
				artech_engine.msgprint(msg, alert=True, indicator="orange")

	def validate_qi_rejection(self, row):
		"""Check if QI is rejected on row level, during submission"""
		action = artech_engine.get_single_value("Stock Settings", "action_if_quality_inspection_is_rejected")
		qa_status = artech_engine.db.get_value("Quality Inspection", row.quality_inspection, "status")
		...
		if qa_status == "Rejected":
			...
			if action == "Stop":
				artech_engine.throw(msg, title=_("Inspection Rejected"), exc=QualityInspectionRejectedError)
			else:
				artech_engine.msgprint(msg, alert=True, indicator="orange")
	...
	def update_blanket_order(self):
		...
		for blanket_order in blanket_orders:
			artech_engine.get_doc("Blanket Order", blanket_order).update_ordered_qty()

	def validate_customer_provided_item(self):
		for d in self.get("items"):
			# Customer Provided parts will have zero valuation rate
			if artech_engine.get_cached_value("Item", d.item_code, "is_customer_provided_item"):
	...
	def validate_multi_currency(self):
		if self.currency != self.company_currency:
			artech_engine.throw(_("Internal transfers can only be done in company's default currency"))

	def validate_packed_items(self):
		if self.doctype in ("Sales Invoice", "Delivery Note Item") and self.get("packed_items"):
			artech_engine.throw(_("Packed Items cannot be transferred internally"))
	...
	def validate_internal_transfer_qty(self):
		...
		precision = artech_engine.get_precision(self.doctype + " Item", "qty")

		over_receipt_allowance = artech_engine.get_single_value("Stock Settings", "over_delivery_receipt_allowance")
	...
	def get_item_wise_inter_transfer_qty(self):
		...
		parent_tab = artech_engine.qb.DocType(parent_doctype)
		child_tab = artech_engine.qb.DocType(child_doctype)

		query = (
			artech_engine.qb.from_(parent_doctype)
	...
	def get_item_wise_inter_received_qty(self):
		...
		parent_tab = artech_engine.qb.DocType(self.doctype)
		child_tab = artech_engine.qb.DocType(child_doctype)

		query = (
			artech_engine.qb.from_(self.doctype)
			.inner_join(child_tab)
			.on(child_tab.parent == parent_tab.name)
			.select(
				child_tab.item_code,
	...
	def validate_putaway_capacity(self):
		...
		from artech.stock.doctype.putaway_rule.putaway_rule import get_available_putaway_capacity
		...
		if not artech_engine.get_all("Putaway Rule", limit=1):
			return
		...
		if valid_doctype:
			...
			for item in self.get("items"):
				...
				rule = artech_engine.db.get_value(
					"Putaway Rule",
			...
			for rule, values in rule_map.items():
				if flt(values["qty_put"]) > flt(values["capacity"]):
					...
					artech_engine.throw(msg=message, title=_("Over Receipt"))

	def prepare_over_receipt_message(self, rule, values):
		message = _("{0} qty of Item {1} is being received into Warehouse {2} with capacity {3}.").format(
			artech_engine.bold(values["qty_put"]),
			artech_engine.bold(values["item"]),
			artech_engine.bold(values["warehouse"]),
			artech_engine.bold(values["capacity"]),
		)
		message += "<br><br>"
		rule_link = artech_engine.utils.get_link_to_form("Putaway Rule", rule)
	...
	def repost_future_sle_and_gle(self, force=False, via_landed_cost_voucher=False):
		...
		if force or future_sle_exists(args) or repost_required_for_queue(self):
			item_based_reposting = artech_engine.get_single_value("Stock Reposting Settings", "item_based_reposting")
	...
	def update_stock_reservation_entries(self):
		...
		if (
			...
			or (
				...
				and artech_engine.get_value("Subcontracting Order", self.subcontracting_order, "reserve_stock")
			)
			...
			for item in self.get(data_map[purpose or self.doctype]["table_name"]):
				...
				for sre in sre_list:
					...
					sre_doc = artech_engine.get_doc("Stock Reservation Entry", sre)

					working_qty = 0
					if sre_doc.reservation_based_on == "Serial and Batch":
						sbb = artech_engine.get_doc("Serial and Batch Bundle", item.serial_and_batch_bundle)
...
def show_accounting_ledger_preview(company: str, doctype: str, docname: str):
	filters = artech_engine._dict(company=company, include_dimensions=1)
	doc = artech_engine.get_lazy_doc(doctype, docname)
...
def show_stock_ledger_preview(company: str, doctype: str, docname: str):
	filters = artech_engine._dict(company=company)
	doc = artech_engine.get_lazy_doc(doctype, docname)
...
def get_accounting_ledger_preview(doc, filters):
	from artech.accounts.report.general_ledger.general_ledger import get_columns as get_gl_columns
...
def get_stock_ledger_preview(doc, filters):
	from artech.stock.report.stock_ledger.stock_ledger import get_columns as get_sl_columns
...
def get_sl_entries_for_preview(doctype, docname, fields):
	sl_entries = artech_engine.get_all(
...
def get_gl_entries_for_preview(doctype, docname, fields):
	return artech_engine.get_all("GL Entry", filters={"voucher_type": doctype, "voucher_no": docname}, fields=fields)
...
def repost_required_for_queue(doc: StockController) -> bool:
	...
	consuming_sles = artech_engine.db.get_all(
		"Stock Ledger Entry",
		filters={
...
def check_item_quality_inspection(doctype: str, items: str | list[dict]):
	...
	items_to_remove = []
	for item in items:
		if not artech_engine.db.get_value("Item", item.get("item_code"), inspection_fieldname_map.get(doctype)):
...
def make_quality_inspections(
	...
	for item in items:
		...
		quality_inspection = artech_engine.get_doc(
			{
				"company": company,
				"doctype": "Quality Inspection",
				"inspection_type": inspection_type,
				"inspected_by": artech_engine.session.user,
...
def is_reposting_pending():
	return artech_engine.db.exists(
...
def future_sle_exists(args, sl_entries=None):
	...
	if not hasattr(artech_engine.local, "future_sle"):
	...
	for d in data:
		artech_engine.local.future_sle[key][(d.item_code, d.warehouse)] = d.total_row
...
def validate_future_sle_not_exists(args, key, sl_entries=None):
	...
	if not sl_entries and hasattr(artech_engine.local, "future_sle"):
		if key not in artech_engine.local.future_sle:
			return False

		if not artech_engine.local.future_sle.get(key) or (
			item_key and item_key not in artech_engine.local.future_sle.get(key)
		):
			return True
...
def get_cached_data(args, key):
	if key not in artech_engine.local.future_sle:
		artech_engine.local.future_sle[key] = artech_engine._dict({})

	if args.get("item_code"):
		item_key = (args.get("item_code"), args.get("warehouse"))
		count = artech_engine.local.future_sle[key].get(item_key)

		return True if (count or count == 0) else False
	else:
		return artech_engine.local.future_sle[key]
...
def get_conditions_to_validate_future_sle(sl_entries):
	...
	for warehouse, items in warehouse_items_map.items():
		or_conditions.append(
			f"""warehouse = {artech_engine.db.escape(warehouse)}
				and item_code in ({", ".join(artech_engine.db.escape(item) for item in items)})"""
		)
...
def create_repost_item_valuation_entry(args):
	...
	repost_entry = artech_engine.new_doc("Repost Item Valuation")
...
def make_bundle_for_material_transfer(**kwargs):
	...
	bundle_doc = artech_engine.get_doc("Serial and Batch Bundle", kwargs.serial_and_batch_bundle)

	if not kwargs.type_of_transaction:
		kwargs.type_of_transaction = "Inward"

	bundle_doc = artech_engine.copy_doc(bundle_doc)
...
def get_item_wise_inventory_account_map(rows, company):
	...
	inventory_map = artech_engine._dict()
	...
	for row in rows:
		...
		if item_defaults.default_inventory_account:
			inventory_map[item_code] = artech_engine._dict(
				{
